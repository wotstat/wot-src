...
import { aw as a, a2 as n } from "./lib.js";
  ...
  r = (e, s) => {
    const a = (e, s) => {
      ...
      for (const i in n)
        s &&
          i in s &&
          ("object" == typeof n[i] ? (n[i] = a(n[i], s[i] ?? n[i])) : (n[i] = s[i] ?? n[i]));
      return n;
    };
    return a(e, s);
  };
function c(e, s) {
  const n = o[a(e)],
    i = n?.COMMON ? r(t.COMMON, n.COMMON) : t.COMMON;
  if (!s) return i;
  const c = n ? r(t[s], n[s]) : t[s],
    l = Object.keys(c).filter((e) => e in i);
  if (l.length > 0)
    throw new Error(
...
function h({ highlight: a, assets: i, counts: t, size: o = g.small, className: r = "" }) {
  const { icons: c, videos: h } = i,
    d = a ? "none" : c.default;
  return e.jsxs("div", {
    className: s(l.base, l[`base__size-${o}`], a && l.base__highlight, r),
    children: [
      a
        ? e.jsx("div", {
            className: l.media,
...
const d = "Icon_2beee90a",
  m = (e) => ("number" == typeof e ? `${e}rem` : e);
function u({ src: a, size: n, className: i }) {
  return e.jsx("div", {
    className: s(d, i),
    style: { backgroundImage: `url(${a})`, ...(n && { width: m(n.width), height: m(n.height) }) },
  });
}
const b = "Highlight_c2944c51",
  _ = "Highlight_base__highlight_8b97c5a1";
const N = "Title_1dcff219",
  f = "Title_wrapper_5727057f";
function I({ children: a, className: n, ...i }) {
  return e.jsx("div", {
    ...i,
    className: s(N, n),
    children: e.jsx("div", { className: f, children: a }),
  });
}
((I.Info = ({ size: s, ...a }) => e.jsx(u, { size: { width: 24, height: 24 }, ...a })),
  (I.Highlight = function ({ text: a, highlight: n, className: i }) {
    return e.jsx("div", { className: s(b, n && _, i), children: a });
  }));
const v = {
...
function O({ counts: s, size: a = g.small, eventName: n, className: t }) {
  const o = (function (e, s) {
      const { images: a, videos: n } = i(v, e);
      switch (s) {
        case g.small:
    ...
    { guaranteed: r } = c(n);
  return e.jsx(h, { className: t, highlight: s <= r.accent, counts: s, size: a, assets: o });
}
export { u as I, O as S, I as T, c as g, g as s };
