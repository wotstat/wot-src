import { j as s } from "../../../../chunks/vendor.js";
import { i as e, n as i, g as r, V as o, W as t, r as a } from "../../../../chunks/lib.js";
import { R as d } from "../../../../chunks/reward_wrapper.js";
import { D as n } from "../../../../chunks/divider.js";
import { E as l } from "../../../../chunks/extended_tooltip_decorator.js";
/* empty css                       */ const m = "daily",
  u = "premium_daily",
  p = "bonus",
  [_, c] = e()(
    ({ observableModel: s }) => ({
      ...s.primitives([
        "countdown",
        "missionType",
        "id",
        "icon",
        "description",
        "currentProgress",
        "totalProgress",
        "earned",
        "isCompleted",
        "animateCompletion",
      ]),
      bonuses: s.arrayClone("bonuses"),
    }),
    i,
  ),
  y = "DailyQuestTooltip_rewardsText_e3c9f036",
  b = "DailyQuestTooltip_divider_e2b4b1fd",
  j = "DailyQuestTooltip_rewardItem_9acd3b8a",
  g = "DailyQuestTooltip_rewards_bf27319f",
  h = r.resolve("strings"),
  x = r.resolve("aliases"),
  f = {
    [m]: h.readOrEmpty("user_missions.tooltip.daily_quests.daily_mission"),
    [u]: h.readOrEmpty("user_missions.tooltip.daily_quests.premium_daily_mission"),
    [p]: h.readOrEmpty("user_missions.tooltip.daily_quests.bonus_mission"),
  },
  v = () => {
    const { model: e } = c(),
      i = e.bonuses.get(),
      r = e.missionType.get(),
      a = e.countdown.get();
    return s.jsxs(l, {
      header: f[r],
      description: t(e.description.get()),
      invertedColors: !0,
      timerPath: "user_missions.tooltip.daily_quests.expires",
      timerTimeLeft: a,
      children: [
        s.jsx(n, { className: b }),
        s.jsx("div", {
          className: y,
          children: h.read("user_missions.tooltip.daily_quests.rewards"),
        }),
        s.jsx(d, {
          bonuses: i,
          questId: e.id.get(),
          resId: x.read((s) => s.user_missions.hangarWidget.Quests("resId")),
          size: o.Small,
          rewardItemClassMix: j,
          classMix: g,
        }),
      ],
    });
  };
a(s.jsx(_, { children: s.jsx(v, {}) }));
