...
def getAnnouncementType(hint, data):
    if hint.type == AnnouncementTypeEnum.RESPAWN:
        return _AnnouncementRespawn()
...
class AnnouncementGoal(object):
    ...
    def __init__(self, hint, data):
        self._hint = hint
        self._showTimer = data.get(b'param2') == b'True'
        self._duration = int(data.get(b'param1', b'0'))
        self._endTime = BigWorld.serverTime() + self.duration
        self._extraData = data.get(b'param3')
...
class _AnnouncementRespawn(AnnouncementGoal):
    ...
    def __init__(self):
        super(_AnnouncementRespawn, self).__init__(HINTS[AnnouncementTypeEnum.RESPAWN.value], {b'param1': b'0', b'param2': b'True'})
        return
    ...
    def getRemainingTime(self):
        ...
        if respInfo is not None:
            return int(math.ceil(respInfo.autoRespawnTime - BigWorld.serverTime()))
        else:
            return 0
