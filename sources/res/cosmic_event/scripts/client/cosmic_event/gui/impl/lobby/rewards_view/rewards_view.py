...
if typing.TYPE_CHECKING:
    ...
    SimpleBonusesType = TypeVar(b'SimpleBonusesType', bound=SimpleBonus)
_logger = logging.getLogger(__name__)

class BonusesView(ViewImpl):
    __slots__ = (b'_tooltips',)
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.gui_lootboxes.lobby.gui_lootboxes.tooltips.LootboxTooltip():
            ...
            if lootBoxIdStr:
                lootBox = self.__itemsCache.items.tokens.getLootBoxByID(lootBoxIdStr.get(b'lootBoxID'))
...
class RewardsWindow(LobbyNotificationWindow):
    ...
    def __init__(self, *args, **kwargs):
        super(RewardsWindow, self).__init__(wndFlags=(WindowFlags.WINDOW | WindowFlags.WINDOW_FULLSCREEN), content=RewardsView(kwargs.pop(b'quests')), *args, **kwargs)
        return
