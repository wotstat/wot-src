...
class CosmicProgressionTokenQuestsHandler(ServiceChannelHandler):
    ...
    def _showAward(self, ctx):
        _, message = ctx
        completedQuestsIDs = [questID for questID in message.data.get(b'completedQuestIDs', set()) if QUEST_STEP_REGEX.match(questID) or ACHIEVEMENT_STEP_REGEX.match(questID)]
...
class CosmicDailyQuestsHandler(ServiceChannelHandler):
    ...
    def _showAward(self, ctx):
        _, message = ctx
        for questID in message.data.get(b'detailedRewards', {}):
            if self._cosmicProgression.isCosmicProgressionQuest(questID) and not ACHIEVEMENT_STEP_REGEX.match(questID):
                self._systemMessages.proto.serviceChannel.pushClientMessage({b'rewards': (message.data[b'detailedRewards'][questID])}, self._CLIENT_MSG_TYPE)
