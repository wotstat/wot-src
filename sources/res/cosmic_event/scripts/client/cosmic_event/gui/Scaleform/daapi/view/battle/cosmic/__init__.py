...
def getViewSettings():
    ...
    return (
     ViewSettings(VIEW_ALIAS.COSMIC_BATTLE_PAGE, CosmicPage, b'cosmicBattlePage.swf', WindowLayer.VIEW, None, ScopeTemplates.DEFAULT_SCOPE),
     ComponentSettings(BATTLE_VIEW_ALIASES.BATTLE_LOADING, CosmicBattleLoading, ScopeTemplates.DEFAULT_SCOPE),
     ComponentSettings(BATTLE_VIEW_ALIASES.COSMIC_HUD, cosmic_hud.CosmicHud, ScopeTemplates.DEFAULT_SCOPE))


def getBusinessHandlers():
    return (
     _CosmicPackageBusinessHandler(),)
...
class _CosmicPackageBusinessHandler(PackageBusinessHandler):
    ...
    def __init__(self):
        _logger.debug(b'_CosmicPackageBusinessHandler.__init__')
        listeners = (
         (
          VIEW_ALIAS.COSMIC_BATTLE_PAGE, self.loadViewBySharedEvent),)
