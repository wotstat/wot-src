...
class _CosmicEventBattlesItem(_SelectorItem):
    ...
    __ACCEPTED_RESTRICTIONS = (
     PREBATTLE_RESTRICTION.VEHICLE_IN_BATTLE,)
    ...
    def hasDisabledFightButtonData(self, result):
        if not self.__cosmicEventBattleCtrl.isCosmicMode():
            return False
        if not result.isValid and result.restriction in self.__ACCEPTED_RESTRICTIONS:
            return True
        return False
    ...
    def getDisabledFightButtonLabel(self, result):
        ...
        if result.restriction == PREBATTLE_RESTRICTION.VEHICLE_IN_BATTLE:
            return backport.text(R.strings.cosmicEvent.fightButton.vehicleInBattle())
        return b''
