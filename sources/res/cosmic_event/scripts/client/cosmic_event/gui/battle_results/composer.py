...
BattleResult = namedtuple(b'BattleResult', (b'results', b'reusable'))
...
def showCosmicResultsView(arenaUniqueID, uiLoader=None):
    ...
    if currentView:
        currentView.destroy()
    g_eventBus.handleEvent(events.LoadGuiImplViewEvent(GuiImplViewLoadParams(layoutID=contentID, viewClass=CosmicPostBattleView, scope=ScopeTemplates.LOBBY_SUB_SCOPE), ctx={b'arenaUniqueID': arenaUniqueID}), scope=EVENT_BUS_SCOPE.LOBBY)
    return
