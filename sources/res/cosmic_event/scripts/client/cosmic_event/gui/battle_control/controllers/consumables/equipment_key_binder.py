...
class EquipmentKeyBinder(object):
    ...
    def __init__(self):
        ...
        _logger.debug(b'KeyBinder for cosmic was created')
        return
    ...
    def _activateEquipment(self, bwKey):
        ...
        if not equipmentCtrl:
            _logger.warning(b'KeyBinder: trying reach equipments controller that is not created yet')
            return
        else:
            intCD = self._keyBindings.get(bwKey, None)
            if intCD is None:
                _logger.info(b'KeyBinder: cannot find bwKey=%s in keyBinding=%s', str(self._keyBindings), bwKey)
                return
            _logger.debug(b'KeyBinder: activate ability with intCD %s', intCD)
            equipmentCtrl.changeSetting(intCD)
            return

    def _handleConsumableChoice(self, event):
        self._activateEquipment(event.ctx[b'key'])
        return

    def _getNewCommand(self):
        command = self._commandStart + self.keysOffset
        if command > CommandMapping.CMD_AMMO_CHOICE_0:
            _logger.warning(b'KeyBinder: have no free command slots. current keybinding = %s', self._keyBindings)
            return None
        else:
            return command
    ...
    def _onEquipmentRemoved(self, intCD, item):
        ...
        if bwKey:
            _logger.debug(b'KeyBinder: removing keybinding for the bwKey %s and %s', bwKey, intCD)
    ...
    def _rebindKeys(self):
        ...
        _logger.debug(b'KeyBinder: keys were rebinded %s', self._keyBindings)
        return
    ...
    def _createKeyBinding(self, intCD):
        ...
        if not keys:
            _logger.warning(b'KeyBinder: cant get a keybinding for this command')
            return
        bwKey, _ = keys
        self._keyBindings[bwKey] = intCD
        _logger.debug(b'KeyBinder: created keybinding=%s for equipment=%s, _keyBindings=%s', bwKey, intCD, self._keyBindings)
        return
