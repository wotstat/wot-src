...
class ClientCosmicEventBattleMode(cosmic_constants.CosmicEventBattleMode):
    ...
    def _client_gameControllers(self):
        ...
        return [
         (
          ICosmicEventProgressionController, CosmicProgressionController, False),
         (
          ICosmicEventBattleController, CosmicEventBattleController, False)]
    ...
    def _client_notificationActionHandlers(self):
        ...
        return (handlers.ProgressionDetailsActionHandler,
         handlers.CosmicEventOpenHandler)

    @property
    def _client_tokenQuestsSubFormatters(self):
        import cosmic_event.messenger.formatters.service_channel as formatters
        return (
         formatters.CosmicProgressionMessageFormatter(),
         formatters.CosmicVehicleRentMessageFormatter(),
         formatters.CosmicAchievementsMessageFormatter())
...
def preInit():
    ...
    OVERWRITE_CTRLS_DESC_MAP[cosmic_constants.ARENA_BONUS_TYPE.COSMIC_EVENT] = {(CTRL_MODE_NAME.ARCADE): (
                               CosmicControlMode, b'cosmicMode', 0)}
    return
...
def init():
    ...
    g_overrideScaleFormViewsConfig.initExtensionLobbyPackages(__name__, [
     b'cosmic_event.gui.Scaleform.daapi.view.lobby'])
    g_overrideScaleFormViewsConfig.initExtensionBattlePackages(__name__, [
     b'cosmic_event.gui.Scaleform.daapi.view.battle.cosmic',
     b'cosmic_event.gui.Scaleform.daapi.view.battle.shared'], cosmic_constants.ARENA_GUI_TYPE.COSMIC_EVENT)
