...
def getReserveIconPath(icon):
    return (b'img://gui/maps/icons/artefact/{}.png').format(icon)
...
class FLBattleTypeDescription(object):
    ...
    def getDescription(reservesModifier=None):
        return FLBattleTypeDescription.__getDescription(b'description', reservesModifier)

    @staticmethod
    def getShortDescription(reservesModifier=None):
        return FLBattleTypeDescription.__getDescription(b'shortDescription', reservesModifier)

    @staticmethod
    def getTitle(reservesModifier=None):
        return FLBattleTypeDescription.__getDescription(b'title', reservesModifier)

    @staticmethod
    def getBattleTypeIconPath(reservesModifier=None, sizeFolder=b'c_136x136'):
...
class AbilitiesTemplates(object):
    ...
    def skillParams(self):
        ...
        return {b'FixedTextParam': templateDefault, 
           b'DirectNumericTextParam': templateDefault, 
           b'DirectSecondsTextParam': templateSeconds, 
           b'DirectMetersTextParam': templateMeters, 
           b'MulDirectPercentageTextParam': templatePercents, 
           b'AddDirectPercentageTextParam': (self.percentsBySecond), 
           b'MulReciprocalPercentageTextParam': templatePercents, 
           b'AddReciprocalPercentageTextParam': templatePercents, 
           b'ShellStunSecondsTextParam': templateSeconds, 
           b'MultiMetersTextParam': templateMeters, 
           b'NestedMetersTextParam': templateMeters, 
           b'NestedSecondsTextParam': templateSeconds, 
           b'MulNestedPercentageTextParam': templatePercents, 
           b'AddNestedPercentageTextParam': templatePercents, 
           b'NestedShellStunSecondsTextParam': templateSeconds, 
           b'MulNestedPercentageTextTupleValueParam': templatePercents}
    ...
    for lvl, skillLevel in skillLevelData.levels.iteritems():
        ...
        for tooltipIdentifier in curLvlEq.tooltipIdentifiers:
            ...
            if param:
                ...
                tooltipParams.append({b'id': tooltipIdentifier, 
                   b'name': (i18n.makeString(tooltipName) if i18n.isValidKey(tooltipName) else b''), 
                   b'value': (str(param)), 
                   b'sign': (SKILL_PARAM_SIGN.get(tooltipIdentifier, b'')), 
                   b'isDynamic': False, 
                   b'valueTemplate': (str(backport.text(TEMPLATES.default if isinstance(param, str) else TEMPLATES.skillParams.get(tooltipRenderer, TEMPLATES.default))))})
...
def makeNewEpicBattleFinishResultLabel(finishReason, teamResult):
    ...
    reasonKey = (b'c_{}{}').format(resFinishReason, resTeamResult) if resFinishReason in frontlineBattleSpecificFinishReason else (b'c_{}').format(resFinishReason)
