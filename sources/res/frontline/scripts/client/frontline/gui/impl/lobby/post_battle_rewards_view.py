...
class _PostBattleRewardsCtx(object):
    ...
    __slots__ = (b'__ctx', b'__currLevel', b'__currFamePoints', b'__currProgress', b'__prevLevel', b'__prevFamePoints', b'__prevProgress')

    def __init__(self, ctx=None):
        self.__ctx = ctx or {}
        self.__currLevel, self.__currFamePoints = ctx.get(b'metaLevel', (1, 0))
        self.__prevLevel, self.__prevFamePoints = ctx.get(b'prevMetaLevel', (1, 0))
    ...
    @property
    def boosterXP(self):
        return self.__ctx.get(b'boosterFlXP', 0)

    @property
    def achievedXP(self):
        return self.__ctx.get(b'originalFlXP', 0)

    @property
    def arenaUniqueID(self):
        return self.__ctx.get(b'arenaUniqueID')

    @property
    def playerRank(self):
        return max(self.__ctx.get(b'playerRank', 0), 1)
