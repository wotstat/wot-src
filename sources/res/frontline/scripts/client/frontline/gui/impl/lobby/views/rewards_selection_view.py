...
def _isValidReward(level, tokenID):
    tokenLevel = tokenID.split(b':')[-1]
    return not level or int(tokenLevel) <= level


class RewardsSelectionView(SelectableRewardBase):
    __slots__ = (b'__onRewardsReceivedCallback', b'__onCloseCallback', b'__onLoadedCallback', b'__isViewLoaded', b'__isAutoDestroyWindowsOnReceivedRewards')
