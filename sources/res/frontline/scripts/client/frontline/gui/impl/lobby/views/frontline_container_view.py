...
TABS = [
 (
  ...
  (
   EpicBattleLogKeys.SUPPLY_OBJECTS_VIEW, EpicBattleLogTabs.SUPPLY_OBJECTS_TAB)),
 (
  TabType.PROGRESS,
...
class FrontlineContainerView(ViewImpl):
    ...
    __slots__ = (b'_activeTab', b'_tabTypeToID', b'_seasonId', b'__uiEpicBattleLogger')
    ...
    def createToolTipContent(self, event, contentId):
        if contentId == R.views.lobby.crew.tooltips.VehicleParamsTooltipView():
            paramName = event.getArgument(b'paramName', b'')
        ...
        if contentId == R.views.frontline.lobby.tooltips.UnlockConditionsTooltip():
            supplyName = event.getArgument(b'supplyObject', b'')
    ...
    def __onTabChange(self, args):
        self.__stopTabLogger()
        tabId = int(args.get(b'tabId', DEFAULT_TAB_ID))
...
def changeHeaderVisible(isVisible):
    g_eventBus.handleEvent(events.LobbyHeaderMenuEvent(events.LobbyHeaderMenuEvent.TOGGLE_VISIBILITY, ctx={b'state': (HeaderMenuVisibilityState.ALL if isVisible else HeaderMenuVisibilityState.ONLINE_COUNTER)}), EVENT_BUS_SCOPE.LOBBY)
    return
