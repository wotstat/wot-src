...
class FrontlinePlayerInfo(PlayerInfo):
    ...
    def packModel(cls, model, battleResults, vehicleSumInfo):
        ...
        if vehicleSumInfo.avatar is not None:
            model.setRank(vehicleSumInfo.avatar.extensionInfo.get(b'playerRank', 0))
...
class FrontlineTeamEfficiency(TeamStats):
    ...
    def packVehStatistics(cls, playerModel, summarizeInfo, battleResults):
        ...
        summVehStatsModel.setObjectivesReached(battleResults.results[b'common'][b'commonNumStarted'] > 0)
        summVehStatsModel.setObjectivesDestroyed(battleResults.results[b'common'][b'commonNumDestroyed'])
        summVehStatsModel.setZoneCaptured(battleResults.results[b'common'][b'commonNumCaptured'])
...
def getRankAchievementResultsModel(playerRank):
    achievementModel = AchievementModel()
    achievementModel.setName((b'rank{}').format(playerRank))
    achievementModel.setIconName((b'rank_{}').format(playerRank))
