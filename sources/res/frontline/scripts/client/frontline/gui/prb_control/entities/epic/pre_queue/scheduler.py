...
class EpicMetaScheduler(BaseScheduler):
    ...
    def __onEpicSettingsUpdated(self, diff):
        if b'epic_config' not in diff:
            return
        epicDiff = diff[b'epic_config']
        validVehicleLevels = epicDiff.get(b'validVehicleLevels', self.__validVehicleLevels)
        unlockableInBattleVehicleLevels = epicDiff.get(b'unlockableInBattleVehicleLevels', self.__unlockableInBattleVehicleLevels)
