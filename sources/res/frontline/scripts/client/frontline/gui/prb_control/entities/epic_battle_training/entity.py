...
class EpicBattleTrainingEntity(LegacyEntity):
    ...
    def __onSettingChanged(self, code, record=b'', callback=None):
        if code < 0:
            LOG_ERROR(b'Server return error for training change', code, record)
            if callback is not None:
                callback(False)
            return
    ...
    def __handleViewLoad(self, event):
        if isinstance(event, LoadGuiImplViewEvent) or event.alias in self.__loadEvents:
            self.setPlayerState(SetPlayerStateCtx(False, waitingID=b'prebattle/player_not_ready'))
        return
