...
class EpicEntity(PreQueueEntity):
    ...
    def _doQueue(self, ctx):
        ...
        LOG_DEBUG(b'Sends request on queuing to the epic battle', ctx)
        return

    def _doDequeue(self, ctx):
        account = BigWorld.player()
        account.FLAccountComponent.dequeueEpic()
        LOG_DEBUG(b'Sends request on dequeuing from the epic battle')
        return

    def _makeQueueCtxByAction(self, action=None):
        invID = g_currentVehicle.invID
        return EpicQueueCtx(invID, waitingID=b'prebattle/join')
