...
_EQUIPMENT_CONFIG_PATH = b'frontline/scripts/item_defs/vehicles/common/equipments/epic_equipments.xml'

def getEquipmentWorkingResult(item, result, error):
    if error is None and item.getStage() == EQUIPMENT_STAGES.DEPLOYING:
        return (False, _ActivationError(b'equipmentIsWorking', {b'name': (item.getDescriptor().userString)}))
    else:
        return (
         result, error)
...
def registerFLEquipmentsItems():
    ...
    equipmentList = {b'inspire': _FLInspireItem, 
       b'fl_regenerationKit': _FLRegenerationKit, b'stealth_radar': _FLStealthRadarItem}
