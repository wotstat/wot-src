...
_FRONTLINE_MUSIC_STATE_GROUP = b'STATE_epicnormandy_battle_music'
_SEVERITY_TO_MUSIC_ID = {(_Severity.NONE): b'STATE_epicnormandy_battle_silence', 
   (_Severity.LOW): b'STATE_epicnormandy_battle_music_01', 
   (_Severity.MEDIUM): b'STATE_epicnormandy_battle_music_02', 
   (_Severity.HIGH): b'STATE_epicnormandy_battle_music_03', 
   (_Severity.VERY_HIGH): b'STATE_epicnormandy_battle_music_00'}
...
class _FrontlineTimeRemainedCondition(_TimeRemainedCondition):
    ...
    def _isCriticalAchieved(self):
        ...
        if self._period == ARENA_PERIOD.BATTLE and self._totalTime is not None and self._totalTime <= criticalTimeValue:
            if self.__stopEpicTimeRemainedCooldown is not None:
                BigWorld.cancelCallback(self.__stopEpicTimeRemainedCooldown)
                LOG_DEBUG(b'[EPIC Drone] Time Remained. Cooldown canceled')
                self.__stopEpicTimeRemainedCooldown = None
            return self._updateValidValue(True)
        else:
            if self.isSatisfied():
                ...
                if self.__stopEpicTimeRemainedCooldown is None:
                    LOG_DEBUG(b'[EPIC Drone] Time Remained. Stop music EpicTimeRemainedCooldown has been started - musicStopPredelay: ', musicStopPredelay)
    ...
    def __onEpicTimeRemainedCooldownOver(self, startTime):
        LOG_DEBUG((b'[EPIC Drone] Time Remained. Cooldown ended. {} seconds passed. Music will be stopped.').format(time.time() - startTime))
        self.__stopEpicTimeRemainedCooldown = None
    ...
    def _validatePoints(self):
        ...
        for points in self._pointsToBase.itervalues():
            if self._stopCapturingCooldown is not None and points:
                if self._stopCapturingCooldown is not None:
                    ...
                    LOG_DEBUG(b'[EPIC Drone] Base Capturing. Cooldown stopped')
        ...
        if self.isSatisfied() and self._stopCapturingCooldown is None:
            if self.__baseCaptured is True:
                self.__baseCaptured = False
            LOG_DEBUG(b'[EPIC Drone] Base Capturing. Stop music cooldown has been started - musicStopPredelay: ', musicStopPredelay)
        ...
        if satisfied:
            for condition in satisfied:
                ...
                if not isPlaying:
                    ...
                    self._playMusic()
                LOG_DEBUG((b'[EPIC Drone] Satisfied conditions: {}').format(satisfied))
