...
class FrontlineBattleConsumablesPanel(FrontlineBattleConsumablesPanelMeta, ConsumablesPanel):
    ...
    def _onSlotBlocked(self, slotId):
        ...
        self.__addEquipmentSlotS(index, bwKey, self.__EMPTY_SCALE_FORM_KEY, 0, 0, 0, getReserveIconPath(b'empty_slot'), self._getToolTipEquipmentSlot(None, slotId), ANIMATION_TYPES.NONE, self.isRandomBattleReserves)
        return

    def _onSlotWaited(self, slotId, quantity):
        index = self._getSlotIndex(slotId)
        bwKey, sfKey = self._genKey(index)
        self.__addEquipmentSlotS(index, bwKey, sfKey, quantity + 1, 0, 0, getReserveIconPath(b'empty_slot'), b'', ANIMATION_TYPES.NONE, self.isRandomBattleReserves)
    ...
    def __addLockedInformationToEpicEquipment(self, idx):
        ...
        if not all((playerDataComp, vehicle, arena)):
            ...
            if self.isRandomBattleReserves:
                ...
                slotEventsConfig = arena.settings.get(b'epic_config', {}).get(b'epicMetaGame', {}).get(b'inBattleReservesByRank').get(b'slotActions', {}).get(vehClass, {})
            ...
            if rank is None or rank <= currentRank - 1:
                return
            rank += 1
            tooltipId = TOOLTIPS_CONSTANTS.EPIC_RANK_UNLOCK_INFO if rank > 1 or self.isRandomBattleReserves else b''
    ...
    def __addEquipmentLevelToSlot(self, idx, item):
        ...
        if b'level' not in itemName:
            return
        levelStr = itemName.partition(b'level')[-1]
        if not levelStr.isdigit():
            return
        self.as_updateLevelInformationS(idx, int(levelStr))
        return

    def __updateLockedInformation(self, idx, lockedID=None, tooltipStr=b'', isSlotEmpty=False):
        if lockedID is None:
