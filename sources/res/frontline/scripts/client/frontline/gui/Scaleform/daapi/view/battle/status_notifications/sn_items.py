...
class ResupplyTimerSN(sn_items.TimerSN):
    ...
    def __updateAdditionalState(self, state):
        if b'additionalState' in self._vo and self._vo[b'additionalState'] == state:
            return
        self._vo[b'additionalState'] = state
        if self._isVisible:
            self._sendUpdate()
        return
    ...
    def __onProgressUpdate(self, circleType, _, value):
        if circleType is not PROGRESS_CIRCLE_TYPE.RESUPPLY_CIRCLE:
            return
        valueStr = str(value)
        if b'additionalInfo' in self._vo and self._vo[b'additionalInfo'] == valueStr:
            return
        self._vo[b'additionalInfo'] = valueStr
        if self._isVisible:
            self._sendUpdate()
        return
...
class StealthInactiveSN(_StealthBaseSN):
    ...
    def _getTitle(self, value):
        equipment = getEquipmentById(value[b'equipmentID'])
