...
class FrontlineBattleUpgradePanel(FrontlineBattleUpgradePanelMeta, IArenaVehiclesController):
    __slots__ = (b'__attentionEffect', b'__offer', b'__cooldownID', b'__showCooldownID', b'__canSelect', b'__canShowNextTime', b'__isInGameMessage', b'__lastStatus')
    ...
    def __getReserveInfo(self, equipment, level, index):
        ...
        reserveInfo = {b'header': (i18n.makeString(equipment.userString)), 
           b'description': (self.__getShortDescription(equipment)), 
           b'module': {b'icon': (getReserveIconPath(equipment.iconName)), 
                       b'intCD': (equipment.compactDescr), 
                       b'level': level, 
                       b'available': True}, 
           b'category': category, 
           b'hotKeys': (getHotKeyInfoListByIndex(index))}
        return reserveInfo
    ...
    def __getNumberName(self, number):
        return backport.text(self.__getR().dyn((b'number_{}').format(number))())
    ...
    def __onShowReservePanel(self, offer, level, slotIdx):
        ...
        data = {b'firstItem': (self.__getReserveInfo(firstEquipment, firstOfferReserveLevel, self.__FIRST_OFFER_INDEX)), 
           b'secondItem': (self.__getReserveInfo(secondEquipment, secondOfferReserveLevel, self.__SECOND_OFFER_INDEX)), 
           b'title': (backport.text(self.__getR().title(), number_name=self.__getNumberName(slotIdx + 1))), 
