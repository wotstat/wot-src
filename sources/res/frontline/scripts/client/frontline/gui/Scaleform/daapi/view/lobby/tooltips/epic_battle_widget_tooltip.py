...
class EpicBattleWidgetTooltip(BlocksTooltipData):
    ...
    @staticmethod
    def __getPlayerProgressToLevelBlock(playerFamePts, famePtsToProgress, progressColor=b'orange'):
        res = formatters.packBlockDataItem(linkage=BLOCKS_TOOLTIP_TYPES.TOOLTIP_META_LEVEL_PROGRESS_BLOCK_LINKAGE, data={b'progressBarData': {b'value': playerFamePts, b'maxValue': famePtsToProgress, b'progressColor': progressColor}}, padding=formatters.packPadding(left=20), blockWidth=280)
        return res
    ...
    def __getCurrentMaxProgressBlock(playerFamePts, famePtsToProgress):
        ...
        currentPointMaxPoint = text_styles.concatStylesWithSpace(currentPoint, text_styles.main(b'/'), fameTo)
        marginTop = 0
