...
_EPIC_BASE = b'epic_base'
_ALLY_OWNER = b'ally'
_ENEMY_OWNER = b'enemy'
_logger = logging.getLogger(__name__)

class FrontlineMissionsPlugin(plugins.MarkerPlugin):
    __slots__ = (b'_isInFreeSpectatorMode',)
...
class SectorBasesPlugin(FrontlineMissionsPlugin, ChatCommunicationComponent):
    ...
    __slots__ = (b'_markers', b'__highlightedBaseID', b'__basesToBeActive', b'__capturedBases', b'__insideCircle', b'__clazz')
    ...
    def _onPlayerMissionUpdated(self, mission, _):
        ...
        if mission.isBaseMission():
            ...
            if markerID is not None and not self._isInFreeSpectatorMode:
                ...
                self._invokeMarker(markerID, b'setActive', True)
    ...
    def _updateHighlight(self):
        ...
        if markerID is not None:
            ...
            self._invokeMarker(markerID, b'setActive', not self._isInFreeSpectatorMode)
        return
    ...
    def __resetBaseHighlight(self):
        ...
        if marker is not None:
            ...
            self._invokeMarker(markerID, b'setActive', False)
            self.__highlightedBaseID = INVALID_TARGET_ID
        return
    ...
    def __onVehicleEntered(self, type_, idx, state):
        if type_ != PROGRESS_CIRCLE_TYPE.SECTOR_BASE_CIRCLE:
            ...
            if markerID is not None:
                self._invokeMarker(markerID, b'notifyVehicleInCircle', True)
            return
    ...
    def __onVehicleLeft(self, type_, idx):
        if type_ != PROGRESS_CIRCLE_TYPE.SECTOR_BASE_CIRCLE:
            ...
            if markerID is not None:
                self.__insideCircle = INVALID_TARGET_ID
                self._invokeMarker(markerID, b'notifyVehicleInCircle', False)
            return
    ...
    def __onSectorBaseAdded(self, sectorBase, isInBase=False):
        ...
        if markerID is None:
            ...
            if bool(sectorBase.isPlayerTeam()):
                ...
                owner = _ENEMY_OWNER
            self._invokeMarker(markerID, b'setOwningTeam', owner)
            self._invokeMarker(markerID, b'setIdentifier', sectorBase.baseID)
            self._invokeMarker(markerID, b'setIsEpicMarker', True)
            self._invokeMarker(markerID, b'setActive', isInBase)
    ...
    def __onSectorBaseCaptured(self, baseId, isPlayerTeam):
        ...
        if baseId in self._markers:
            ...
            markerID = marker.getMarkerID()
            marker.setOwningTeam(_ALLY_OWNER if isPlayerTeam else b'enemy')
        if markerID is not None:
    ...
    def __onSectorBasePointsUpdate(self, baseId, isPlayerTeam, points, capturingStopped, invadersCount, expectedCaptureTime):
        ...
        if markerID is not None:
            self._invokeMarker(markerID, b'setCapturePoints', points)
        return
    ...
    def __checkPlayerInsideCircle(self, baseId):
        markerID = self.__getMarkerIDForBaseID(baseId)
        if markerID is not None:
            self._invokeMarker(markerID, b'notifyVehicleInCircle', self.__insideCircle == baseId)
        return


class HeadquartersPlugin(FrontlineMissionsPlugin, ChatCommunicationComponent):
    __slots__ = (b'_markers', b'__isHQBattle', b'__visibleHQ', b'__hqMissionActive', b'__clazz', b'__entitiesDamageType')
    ...
    def resetHQHighlight(self):
        ...
        if marker is None:
            ...
            if not marker.getIsRepliedByPlayer():
                self._setMarkerSticky(markerID, False)
            self._invokeMarker(markerID, b'setHighlight', False)
            self.__visibleHQ = INVALID_TARGET_ID
            return
    ...
    def _onNearestObjectiveChanged(self, objID, _):
        if self.__hqMissionActive:
            ...
            if markerID and not self._isInFreeSpectatorMode:
                if not FrontlineMissionsPlugin._getAnyMarkerRepliedByPlayer(self._markers):
                    self._setMarkerSticky(markerID, True)
                self._invokeMarker(markerID, b'setHighlight', True)
        return
    ...
    def _updateHighlight(self):
        ...
        if marker is None:
            ...
            markerID = marker.getMarkerID()
            self._setMarkerSticky(markerID, not self._isInFreeSpectatorMode)
            self._invokeMarker(markerID, b'setHighlight', not self._isInFreeSpectatorMode)
            return
    ...
    def __onDestructibleEntityAdded(self, entity):
        ...
        if destructibleComponent is None:
            ...
            if entity is None:
                _logger.error(b'Expected DestructibleEntity not present!')
                return
            ...
            self._invokeMarker(handle, b'setOwningTeam', entity.isPlayerTeam)
            self._invokeMarker(handle, b'setDead', isDead)
            self._invokeMarker(handle, b'setIdentifier', entity.destructibleEntityID)
            self._invokeMarker(handle, b'setMaxHealth', entity.maxHealth)
            self._invokeMarker(handle, b'setHealth', int(entity.health))
            ...
            if entity.destructibleEntityID == self.__visibleHQ:
                ...
                self._invokeMarker(handle, b'setHighlight', False)
            marker.setState(ReplyStateForMarker.NO_ACTION)
            self._setActiveState(marker, marker.getState())
            if isDead:
    ...
    def __onDestructibleEntityStateChanged(self, entityId):
        ...
        if marker is not None:
            ...
            if not hq.isAlive():
                self._invokeMarker(markerID, b'setHealth', 0, self.__entitiesDamageType.get(entityId))
                self._invokeMarker(markerID, b'setDead', True)
                self._setMarkerSticky(markerID, False)
    ...
    def __onDestructibleEntityHealthChanged(self, entityId, newHealth, maxHealth, attackerID, attackReason, hitFlags):
        ...
        if marker is None:
            ...
            if hasImpactMask and battleSpamCtrl is not None and aInfo and aInfo.isAutoShootGunVehicle():
                hasImpactMask = battleSpamCtrl.filterMarkersHitState(entityId, (b'impact{}').format(attackerID))
            self.__entitiesDamageType[entityId] = self.__getVehicleDamageType(aInfo)
            self._invokeMarker(marker.getMarkerID(), b'setHealth', newHealth, self.__entitiesDamageType[entityId], hasImpactMask)
            return
...
class StepRepairPointPlugin(plugins.MarkerPlugin):
    ...
    def __onVehicleEntered(self, type_, idx, state):
        if type_ != PROGRESS_CIRCLE_TYPE.RESUPPLY_CIRCLE:
            ...
            if handle is not None:
                self._invokeMarker(handle, b'notifyVehicleInCircle', True)
                self._parentObj.invokeMarker(handle, b'setState', [state])
            return
    ...
    def __onVehicleLeft(self, type_, idx):
        if type_ != PROGRESS_CIRCLE_TYPE.RESUPPLY_CIRCLE:
            ...
            handle = self.__markers[idx]
            if handle is not None:
                self._invokeMarker(handle, b'notifyVehicleInCircle', False)
            return
    ...
    def __onTimerUpdated(self, type_, pointId, timeLeft):
        ...
        self._parentObj.invokeMarker(handle, b'setCooldown', [time_utils.getTimeLeftFormat(timeLeft)])
        return
...
class SectorWarningPlugin(plugins.MarkerPlugin):
    ...
    def init(self):
        ...
        if sectorWarningComponent is not None:
            ...
            _logger.error(b'Expected SectorWarningComponent not present!')
        return
    ...
    def __onTransitionTimerUpdated(self, sectorGroupID, seconds):
        sc = getattr(self.sessionProvider.arenaVisitor.getComponentSystem(), b'sectorComponent', None)
        if sc is None:
    ...
    def __updateTimer(self, marker, seconds):
        if seconds > 0:
            ...
            self._invokeMarker(marker.markerID, b'setCountdown', timeStr)
        else:
            self._invokeMarker(marker.markerID, b'clearCountdown')
        return
    ...
    def __onShowSectorWarning(self, edgeID, warningID, targetSectorGroupID):
        ...
        if sectorWarningComponent is None:
            ...
            if None in (edgeID, warningID, targetSectorGroupID):
                _logger.error(b'[SectorWarningPlugin] Wrong argument!')
                return
            ...
            self._invokeMarker(marker.markerID, b'showWarning', markerType)
            return
...
class SectorWaypointsPlugin(plugins.MarkerPlugin, IVehiclesAndPositionsController):
    __slots__ = (b'__markers', b'__currentWaypointSector', b'__markerActiveState', b'__suspendedForLane', b'__currentWaypointPositon', b'__marker', b'__timeCB', b'__currentEndTime', b'__suspendedForBase', b'__shownWaypoints')
    ...
    def __onGameNotificationRecieved(self, notificationType, data):
        if notificationType != EPIC_NOTIFICATION.ZONE_CAPTURED:
            ...
            if self.__suspendedForBase == data[b'id']:
                currentSuspensionLane = self.__suspendedForLane
                self.__suspendedForBase = None
                self.__suspendedForLane = None
                if self.__currentWaypointSector is not None:
                    playerData = getattr(self.sessionProvider.arenaVisitor.getComponentSystem(), b'playerDataComponent', None)
                    if playerData is None:
    ...
    def __onWaypointsForPlayerActivated(self, waypointSectorTimeTuple):
        componentSystem = self.sessionProvider.arenaVisitor.getComponentSystem()
        sectorComponent = getattr(componentSystem, b'sectorComponent', None)
        if sectorComponent is None:
            _logger.error(b'Expected SectorComponent not present!')
            return
        else:
            ...
            if not self.__marker:
                ...
                self._invokeMarker(self.__marker, b'isAttacker', False)
    ...
    def __onTransitionTimerUpdated(self, sectorGroupID, seconds):
        if self.__markerActiveState and self.__currentWaypointSector and self.__currentWaypointSector.groupID == sectorGroupID and self.__marker:
            if seconds >= 0:
                timeStr = time_utils.getTimeLeftFormat(seconds)
                self._invokeMarker(self.__marker, b'setCountdown', timeStr)
            else:
                self._invokeMarker(self.__marker, b'clearCountdown')
        return
...
class FrontlineMarkersManager(MarkersManager):
    ...
    def _setupPlugins(self, arenaVisitor):
        ...
        if arenaVisitor.hasSectors():
            setup[b'sector_bases'] = SectorBasesPlugin
            if avatar_getter.getPlayerTeam() == EPIC_BATTLE_TEAM_ID.TEAM_DEFENDER:
                setup[b'sector_waypoints'] = SectorWaypointsPlugin
            setup[b'sector_warnings'] = SectorWarningPlugin
        if arenaVisitor.hasDestructibleEntities():
            setup[b'hq'] = HeadquartersPlugin
        if arenaVisitor.hasStepRepairPoints():
            setup[b'step_repairs'] = StepRepairPointPlugin
        if arenaVisitor.hasRespawns():
            setup[b'vehicles'] = FrontlineRespawnableVehicleMarkerPlugin
        else:
            setup[b'vehicles'] = FrontlineVehicleMarkerPlugin
        return setup
