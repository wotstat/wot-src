...
HWEquipmentVariant = namedtuple(b'HWEquipmentVariant', (b'id', b'cooldownSeconds', b'durationSeconds', b'usageCost', b'equipmentItem', b'dynComponentsGroups'))
...
class HWEquipment(DynComponentsGroupEquipment):
    ...
    def _readConfig(self, xmlCtx, scriptSection):
        ...
        self._fallbackVariant = HWEquipmentVariant(id=b'', cooldownSeconds=self.cooldownSeconds, durationSeconds=self.durationSeconds, usageCost=self.usageCost, equipmentItem=self.equipmentItem, dynComponentsGroups=self.dynComponentsGroups)
        return
