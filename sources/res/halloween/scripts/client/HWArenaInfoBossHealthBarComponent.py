...
BossLive = namedtuple(b'Live', [b'maxHP', b'minHP'])
...
def getArenaInfoBossHealthBarComponent():
    ...
    if not (player and isPlayerAvatar() and player.arena and player.arena.arenaInfo):
        ...
        return getattr(arenaInfo, b'HWArenaInfoBossHealthBarComponent', None)
...
class HWArenaInfoBossHealthBarComponent(DynamicScriptComponent):
    ...
    def set_healthForLives(self, _):
        if self.__activate():
            ...
            _logger.error(b'Cannot activate the component with data: %s, %d and %s', self.bossVehicleIDs, self.lastHealth, self.healthForLives)
        return
