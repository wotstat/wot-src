...
class HWEnvironmentSwitcher(object):
    ...
    def __init__(self, spaceID):
        ...
        self._envSwitchDelay = section[b'envSwitchDelay'].asFloat
        return
...
class HWArenaPhasesComponent(DynamicScriptComponent):
    ...
    def set_activePhase(self, prev):
        _logger.info(b'HWArenaPhasesComponent set activePhase=%s', self.activePhase)
        self.onPhaseChanged(self)
        return

    def set_modifierName(self, prev):
        _logger.info(b'HWArenaPhasesComponent set modifierName=%s', self.modifierName)
        self.onPhaseModifierChanged(self)
        return

    def set_modifierColor(self, prev):
        _logger.info(b'HWArenaPhasesComponent set modifierColor=%s', self.modifierColor)
        self.onPhaseModifierChanged(self)
        return
    ...
    def getInstance():
        if not isPlayerAvatar():
            ...
            if not player.arena or not player.arena.arenaInfo:
                return
            return getattr(player.arena.arenaInfo, b'HWArenaPhasesComponent', None)
