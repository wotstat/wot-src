...
class HWVehicleSoundComponent(DynamicScriptComponent):
    ...
    def hwSoulsContainer(self):
        return self.entity.dynamicComponents.get(b'hwSoulsContainer')
    ...
    def onBossDamageReceived(self, attackerID, attackReasonID, damage):
        ...
        self._components.call(b'onBossDamageReceived', attackerID, attackReason, damage)
        return

    def _onVehicleKilled(self, victimID, *args):
        if self.entity.id == victimID:
            self.soundObject.stopAll()
        self._components.call(b'onVehicleKilled', victimID, *args)
        return

    def _onBossLivesChanged(self):
        self.soundObject.stopAll()
        self._components.call(b'onBossLivesChanged')
        return

    @property
    def _arenaSoundComponent(self):
        return BigWorld.player().arena.arenaInfo.dynamicComponents.get(b'HWArenaSoundComponent')
