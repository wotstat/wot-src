...
class HWEquipmentItem(DynComponentsGroupItem):
    ...
    def canActivate(self, entityName=None, avatar=None):
        ...
        if self.vehicle.hwSoulsContainer.souls < self._getUsageCost():
            return (False, _ActivationError(b'hwNotEnoughSouls', {b'name': (self._descriptor.userString)}))
        return (result, error)
    ...
    def _applyModifiers(self, key, value):
        vehicle = self.vehicle
        if vehicle and hasattr(vehicle, b'hwModifiers'):
...
class _HpRepairAndCrewHeal(HWEquipmentItem):
    ...
    def canActivate(self, entityName=None, avatar=None):
        ...
        if vehicle.health >= vehicle.maxHealth and not deviceStates and not avatar_getter.isVehicleStunned():
            return (False, _ActivationError(b'hpRepairAndCrewHeal', {b'name': (self._descriptor.userString)}))
...
class _AbilityWithDuration(HWEquipmentItem):
    ...
    def canActivate(self, entityName=None, avatar=None):
        ...
        if not vehicle:
            return (False, _ActivationError(b'equipmentAlreadyActivated', {b'name': (self._descriptor.userString)}))
...
class _NitroEquipmentItem(_AbilityWithDuration):
    ...
    def inCriticalCondition(self, avatar=None):
        ...
        if avatar.fireInVehicle:
            return (True, _ActivationError(b'hwVehicleIsOnFire', {b'name': (self._descriptor.userString)}))
        else:
            if avatar.isVehicleOverturned:
                return (True, _ActivationError(b'hwVehicleIsOverturned', {b'name': (self._descriptor.userString)}))
            if avatar.isVehicleDrowning:
                return (True, _ActivationError(b'hwVehicleIsDrowning', {b'name': (self._descriptor.userString)}))
            return (False, None)
...
class _FastReload(_AbilityWithDuration):
    INSTANT_RELOAD = b'instantReload'
    DEVICES = (b'gun',)
    ERROR_NAME = b'hw_ability_gun_destroyed'
    ...
    def _canActivateFast(self, entityName=None, avatar=None):
        ...
        if isGunDestroyed:
            return (False,
             _ActivationError(self.ERROR_NAME, {b'name': (self._descriptor.userString)}))
    ...
    def _canActivateInstant(self, entityName=None, avatar=None):
        ...
        if not result:
            ...
            return (
             False,
             _ActivationError(b'instantReload', {b'name': (self._descriptor.userString)}))
...
class _HWInvisibility(HWEquipmentItem):
    ...
    def canActivate(self, entityName=None, avatar=None):
        ...
        if hasActiveBuff:
            return (False, _ActivationError(b'hwInvisibility', {b'name': (self._descriptor.userString)}))
...
def registerHWEquipmentsItems():
    registerEquipmentItem(b'nitroRamDamage', _NitroEquipmentItem, _NitroEquipmentReplayItem)
    registerEquipmentItem(b'hpRepairAndCrewHeal', _HpRepairAndCrewHeal, HWEquipmentReplayItem)
    registerEquipmentItem(b'teamRepairKit', HWEquipmentItem, HWEquipmentReplayItem)
    registerEquipmentItem(b'fastReload', _FastReload, HWEquipmentReplayItem)
    registerEquipmentItem(b'damageShield', _AbilityWithDuration, HWEquipmentReplayItem)
    registerEquipmentItem(b'hwInvisibility', _HWInvisibility, HWEquipmentReplayItem)
    registerEquipmentItem(b'hwAoeDamageInstantShot', _HWInstantShot, HWEquipmentReplayItem)
    registerEquipmentItem(b'hwAoeStunInstantShot', _HWInstantShot, HWEquipmentReplayItem)
    registerEquipmentItem(b'hwAoeDrainEnemyHpInstantShot', _HWInstantShot, HWEquipmentReplayItem)
    return
