...
_NITRO_EQUIPMENT_NAME = b'nitroRamDamage'
_HW_EQUIPMENT = b'hwEquipment'
_HW_EMPTY_SLOT = b'hwEmptySlot'
...
class HWEquipmentPanelSounds(SoundComponentBase):
    ...
    def _getNitroEquipment(self):
        if self.vehicle is None:
            ...
            for intCD in self.vehicle.setups[b'eqsSetups'][0]:
    ...
    def _getPlayerVehicleSoulsContainer(self):
        ...
        if vehicle:
            return vehicle.dynamicComponents.get(b'hwSoulsContainer')
        else:
            return
...
class HWVehicleDetectorSounds(SoundComponentBase):
    ...
    def _onVehicleDetectorChangeDistance(self, ctx):
        distance = ctx.get(b'indicator', _VEH_DETECTOR_DISTANCE_FAR_AWAY)
...
class HWArenaLootSounds(SoundComponentBase):
    ...
    def _onLootCaptured(self, event):
        ctx = event.ctx
        loot, vehicleID, vehicleIDs = ctx[b'loot'], ctx[b'vehID'], ctx[b'vehIDs']
    ...
    def _onVehCapturingStart(self, event):
        ctx = event.ctx
        loot, vehicleID = ctx[b'loot'], ctx[b'vehID']
        player = BigWorld.player()
