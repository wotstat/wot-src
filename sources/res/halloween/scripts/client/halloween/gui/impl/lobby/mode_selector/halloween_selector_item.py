...
class HalloweenSelectorItem(ModeSelectorLegacyItem):
    ...
    def _onInitializing(self):
        ...
        for reward in self.__hwBattleCtrl.getModeSettings().modeSelectorShowRewards.get(b'rewards', []):
            ...
            if value == ModeSelectorRewardID.VEHICLE:
                ...
                if vehicle:
                    self._addReward(ModeSelectorRewardID.VEHICLE, locParams={b'name': (vehicle.userName)}, level=int2roman(vehicle.level), type=vehicle.type, isPremium=vehicle.isPremium)
            else:
    ...
    def __setBattlePassState(self, itemVM):
        ...
        isShown = self.__hwBattleCtrl.getModeSettings().modeSelectorShowRewards.get(b'battlePassPoints', False)
