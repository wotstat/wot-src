...
class CrewRewardNotification(NotificationBase):
    ...
    def __init__(self, resId, *args, **kwargs):
        ...
        self._filterState = FilterState(initialState={(FilterState.GROUPS.TANKMANKIND.value): (TankmanKind.RECRUIT.value)}, persistor=Persistor(storageKey=b'barracks', persistentGroups=[
    ...
    def _update(self):
        commanderID = self._getPayload().get(b'commanderID')
        if not commanderID:
            return
        else:
            ...
            name, surname = recruitInfo.getFullUserNameByNation(nationID=None).split(b' ', 1)
