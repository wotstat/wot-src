...
_IMAGE_FORMAT = b'.png'
_IMAGE_FORMAT_EXTEND = b'.extend.png'
C11BonusArgs = namedtuple(b'C11BonusArgs', [b'bonus', b'item', b'c11Item', b'data'])
...
def getImgName(path):
    if path is None:
        ...
        if path.endswith(_IMAGE_FORMAT_EXTEND):
            return path.split(b'/')[-1].replace(_IMAGE_FORMAT_EXTEND, b'').replace(b'-', b'_')
        return path.split(b'/')[-1].replace(_IMAGE_FORMAT, b'').replace(b'-', b'_')


def formatTokenBonusName(tokenID):
    return tokenID.replace(b':', b'_')


class HWItemsBonusFormatter(ItemsBonusFormatter):
    _OPTIONAL_DEVICE_TYPE = b'optionalDevice'
    _TROPHY_DEVICE_TYPE = b'trophyDevice'
    _MODERNIZED_DEVICE_TYPE = b'modernizedDevice'
    _CREW_BATTLE_BOOSTER_TYPE = b'crewBattleBooster'
    _DEVICE_BATTLE_BOOSTER_TYPE = b'deviceBattleBooster'
    _CONSUMABLE_TYPE = b'consumable'
    _SUBTYPE_ORDER = [
     _TROPHY_DEVICE_TYPE, _MODERNIZED_DEVICE_TYPE, _OPTIONAL_DEVICE_TYPE, 
     _CREW_BATTLE_BOOSTER_TYPE, 
     _DEVICE_BATTLE_BOOSTER_TYPE, _CONSUMABLE_TYPE]
...
class HWMetaPremiumDaysBonusFormatter(PremiumDaysBonusFormatter):
    ...
    def _format(self, bonus):
        return [
         PreformattedBonus(bonusName=bonus.getName(), userName=self._getUserName(bonus), images=self._getImages(bonus), tooltip=bonus.getTooltip(), isCompensation=self._isCompensation(bonus), label=b'')]
...
class HWFakeTokenMedalBonusFormatter(TokenBonusFormatter):
    ...
    def _getFormattedBonus(self, tokenID, token, bonus):
        ...
        if medalID:
            ...
            return PreformattedBonus(bonusName=b'dossier' + DOSSIER_ACHIEVEMENT_POSTFIX, images={(AWARDS_SIZES.SMALL): (backport.image(R.images.gui.maps.icons.achievement.dyn(medal)())), 
            ...
            return PreformattedBonus(label=b'', bonusName=HWStoryChoiceSettings.MEDAL_BONUS_NAME, images={(AWARDS_SIZES.SMALL): (self._FAKE_MEDAL_ICON), 
...
class HWTokenBonusFormatter(TokenBonusFormatter):
    ...
    _BATTLE_VEHICLE_BONUS_X5_TOKEN_PREFIX1 = b'xpx5_'
    _BATTLE_VEHICLE_BONUS_X5_TOKEN_PREFIX2 = b'expx5'
    ...
    def _getFormattedBonus(self, tokenID, token, bonus):
        ...
        if tokenID.startswith(self._BATTLE_VEHICLE_BONUS_X5_TOKEN_PREFIX1) or tokenID.lower().startswith(self._BATTLE_VEHICLE_BONUS_X5_TOKEN_PREFIX2):
            ...
            return PreformattedBonus(bonusName=formatTokenBonusName(tokenID), images={(AWARDS_SIZES.SMALL): b'', 
               (AWARDS_SIZES.BIG): (self._KEY_ICON)}, label=str(bonuseValue.get(tokenID, {}).get(b'count')), userName=backport.text(R.strings.halloween_lobby.keyCard.name()), tooltip=R.views.halloween.mono.lobby.tooltips.key_tooltip())
        return formatted
    ...
    def _formatComplexToken(self, complexToken, token, bonus):
        if complexToken.styleID == ArtefactsSettings.ARTEFACT:
            return PreformattedBonus(bonusName=complexToken.styleID, images={(AWARDS_SIZES.SMALL): b'', 
               (AWARDS_SIZES.BIG): b''}, label=b'')
        return super(HWTokenBonusFormatter, self)._formatComplexToken(complexToken, token, bonus)

    def _crewFormattedBonus(self, tokenID):
        return PreformattedBonus(bonusName=tokenID.replace(b':', b'_'), images={(AWARDS_SIZES.SMALL): b'', 
           (AWARDS_SIZES.BIG): b''}, label=b'')
...
class HWCerfTokenBonusFormatter(TokenBonusFormatter):
    ...
    def _getFormattedBonus(self, tokenID, token, bonus):
        ...
        imgName = b'tankman_twitch' if any(info and not info.isFemale() for info in recruitInfo) else b'tankwoman_twitch'
        return PreformattedBonus(bonusName=bonus.getName(), images={(AWARDS_SIZES.SMALL): (backport.image(bonusIcons.dyn(AWARDS_SIZES.SMALL).dyn(imgName)())), 
           (AWARDS_SIZES.BIG): (backport.image(bonusIcons.dyn(AWARDS_SIZES.BIG).dyn(imgName)()))}, label=b'', tooltip=makeTooltip(header=backport.text(R.strings.halloween_lobby.bundleView.tooltip.cerfToken.header()), body=backport.text(R.strings.halloween_lobby.bundleView.tooltip.cerfToken.body())))
...
class HWCustomizationsBonusFormatter(CustomizationsBonusFormatter):
    _ORDER = (b'style_3d', b'attachment', b'style', b'outfit', b'paint', b'camouflage', b'projectionDecal', b'decal', b'sequence', b'emblem', b'inscription', b'insignia', b'personalNumber', b'modification')

    def _getSubtypePriority(self, c11item):
        if self._getItemTypeName(c11item) == b'attachment':
...
class HWGoodiesBonusFormatter(GoodiesBonusFormatter):
    _BOOSTERS_ORDER = [
     b'booster_credits', b'booster_xp', b'booster_free_xp_and_crew_xp']
...
def getHWMetaFormattersMap():
    ...
    mapping.update({b'items': (HWItemsBonusFormatter()), 
       (Currency.CREDITS): (HWCreditsBonusFormatter()), 
       b'vehicles': (HWVehiclesBonusFormatter()), 
       b'battleToken': (HWTokenBonusFormatter()), 
       b'cerfToken': (HWCerfTokenBonusFormatter()), 
       ...
       b'dossier': (HWDossierBonusFormatter()), 
       b'goodies': (HWGoodiesBonusFormatter()), 
       b'battlePassPoints': (HWBattlePassBonusFormatter()), 
       b'crewBooks': (HWCrewBooksBonusFormatter()), 
       b'customizations': (HWCustomizationsBonusFormatter())})
    return mapping


def getAwardsHWMetaFormattersMap():
    mapping = getHWMetaFormattersMap()
    mapping.update({b'vehicles': (HWVehiclesAwardsBonusFormatter()), 
       b'customizations': (HWAwardsCustomizationsBonusFormatter())})
    return mapping


def getHWTwitchFormattersMap():
    mapping = getHWMetaFormattersMap()
    mapping.update({b'tmanToken': (HWTmanTemplateBonusFormatter())})
    return mapping


def getHWBattleResultFormattersMap():
    mapping = getHWMetaFormattersMap()
    mapping.update({b'battleToken': (HWBattleResultBonusFormatter())})
    return mapping
...
class HalloweenBonusesAwardsComposer(CurtailingAwardsComposer):
    ...
    def _packMergedBonuses(self, mergedBonuses, size=AWARDS_SIZES.SMALL):
        ...
        imgs = {(AWARDS_SIZES.SMALL): (RES_ICONS.getBonusIcon(AWARDS_SIZES.SMALL, b'default')), 
           (AWARDS_SIZES.BIG): (RES_ICONS.getBonusIcon(AWARDS_SIZES.BIG, b'default'))}
        return PreformattedBonus(bonusName=b'default', label=backport.text(R.strings.halloween_lobby.reward.rest(), count=mergedBonusCount), images=imgs, isSpecial=True, specialAlias=TOOLTIPS_CONSTANTS.ADDITIONAL_AWARDS, specialArgs=self._getShortBonusesData(mergedBonuses, size), userName=b'')
