...
class RewardPathView(ViewComponent[RewardPathViewModel]):
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.halloween.mono.lobby.tooltips.mission_tooltip():
            artefactID = event.getArgument(b'artefactID', b'')
    ...
    def __goToMission(self, args):
        artefactID = args.get(b'artefactID', None)
    ...
    def __fillArtefacts(self):
        ...
        with self.viewModel.transaction() as tx:
            ...
            for artefact in self._hwArtefactsCtrl.artefactsSorted():
                ...
                self.__bonusCache.update(fillRewards(artefact, artefactVM.getRewards(), self._MAX_BONUSES_IN_VIEW, self.__idGen, rewardsHighlight=[b'cerfToken'] if isRewardNeedHighlight else []))
