...
class HalloweenAreaMarkerEntriesPlugin(HalloweenMinimapScaleMixin, BaseAreaMarkerEntriesPlugin):
    ...
    def __onReplyFeedbackReceived(self, targetID, replierID, markerType, oldReplyCount, newReplyCount):
        ...
        if entry.hasAnimation() and oldReplyCount == 0 and newReplyCount == 0:
            self._parentObj.invoke(entry.getID(), b'triggerClickAnimation')
        isReply = newReplyCount > 0
        if entry.getIsReplied() != isReply:
            self._parentObj.invoke(entry.getID(), b'setMarkerReplied', isReply)
...
class HalloweenEquipmentsPlugin(HalloweenMinimapScaleMixin, EquipmentsPlugin):
    _SYMBOL_DEATH_ZONE = b'HWDeathZoneEntryUI'
    _SYMBOL_DEATH_ZONE_IGNITE = b'HWDeathZoneIgniteEntryUI'
    _SYMBOL_DEATH_ZONE_STUN = b'HWDeathZoneStunEntryUI'
    _SYMBOL_DEATH_ZONE_INTERVAL = b'HWDeathZoneDamageOvertimeEntryUI'
    HW_EQ_MARKER_TO_SYMBOL = {b'EventDeathZoneUI': _SYMBOL_DEATH_ZONE, 
       b'EventDeathZoneIgniteUI': _SYMBOL_DEATH_ZONE_IGNITE, 
       b'EventDeathZoneStunUI': _SYMBOL_DEATH_ZONE_STUN, 
       b'EventDeathZoneDamageOvertimeUI': _SYMBOL_DEATH_ZONE_INTERVAL}
...
class HalloweenMinimapComponent(HWMinimapMeta):
    ...
    def _setupPlugins(self, arenaVisitor):
        ...
        setup[b'area_markers'] = HalloweenAreaMarkerEntriesPlugin
        setup[b'personal'] = HWPersonalEntriesPlugin
        setup[b'vehicles'] = HalloweenMinimapVehiclesPlugin
        setup[b'area'] = HalloweenAreaStaticMarkerPlugin
        setup[b'equipments'] = HalloweenEquipmentsPlugin
        if IS_DEVELOPMENT:
            setup[b'teleport'] = HWTeleportPlugin
        if not BattleReplay.g_replayCtrl.isPlaying:
            setup[b'pinging'] = HalloweenMinimapPingPlugin
        return setup
