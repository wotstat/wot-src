...
class HWVehiclePreview(VehiclePreview, IGlobalListener):
    ...
    def setBottomPanel(self):
        if self.__ctx.get(b'isKingReward') or self.__ctx.get(b'isHeroTank'):
        ...
        if ctx[b'state'] == CameraMovementStates.MOVING_TO_OBJECT:
            if isinstance(entity, HeroTank):
                ...
                if descriptor:
                    ...
                    showHeroTankPreview(vehicleCD, previewAlias=HALLOWEEN_HANGAR_ALIASES.HALLOWEEN_VEHICLE_PREVIEW, backOutfit=self.__ctx.get(b'outfit'), backBtnLabel=backport.text(R.strings.menu.cst_item_ctx_menu.preview()), isHiddenMenu=True, isKingReward=self.__ctx.get(b'isKingReward', False))
