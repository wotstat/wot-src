...
BOSS_MARKER_ROLE = b'hwboss'
BOSS_ROLE = b'boss'
VEHICLE_MARKER = b'HWVehicleMarkerUI'

class HalloweenVehicleMarkerPlugin(RespawnableVehicleMarkerPlugin):
    ICON_HIT_BLOCKED = b'hit_blocked'
    ...
    def _restoreMarker(self, marker, vProxy, vInfo, guiProps):
        ...
        if marker.isAlive():
            ...
            self._updateMarkerState(marker.getMarkerID(), b'dead', True, b'')
    ...
    def __onVehicleBuffIconAdded(self, vehicleId, icon, priority):
        ...
        if marker is not None:
            self._invokeMarker(marker.getMarkerID(), b'showEnemyBuff', icon, priority)
        return

    def __onVehicleBuffIconRemoved(self, vehicleId, icon, priority):
        marker = self._markers.get(vehicleId)
        if marker is not None:
            self._invokeMarker(marker.getMarkerID(), b'removeEnemyBuff', icon, priority)
        return
        ...
        if not self.__isVehicleBoss(vehicleID):
            ...
            if self.__isBossVulnerable:
                ...
                if component is not None:
                    ...
                    self._invokeMarker(handle, b'setMaxHealth', maxHealth)
                    self._invokeMarker(handle, b'setHealth', currentHealth)
                    self._invokeMarker(handle, b'setBossMode', True)
            else:
                self._invokeMarker(handle, b'setBossMode', False)
            return
    ...
    def __onPhaseTimeChangedNotification(self, looterID, ownerID, amount):
        ...
        if marker is not None:
            ...
            self._invokeMarker(marker.getMarkerID(), b'addPhaseTime', text)
        return
