...
class HWConsumablesPanel(ConsumablesPanelMeta):
    ...
    HW_EQUIPMENTS_NOT_IN_PANEL = [b'nitroRamDamage']
    ...
    def _initIndexes(cls):
        cls._HW_EQUIPMENT_START_IDX, cls._HW_EQUIPMENT_END_IDX, cls._AMMO_START_IDX, cls._AMMO_END_IDX = _PRESETS_CUSTOM_INDEXES.get(getSettings(AccountSettingsKeys.AMMO_PANEL_PRESET), b'default')
        return
    ...
    def _onUpdateHWEquipment(self, onModifier=False):
        ctrl = self.sessionProvider.shared.equipments
        if ctrl is not None:
            for intCD, item in ctrl.iterEquipmentsByTag(b'hwEquipment'):
                if intCD not in self._cds:
                    continue
                idx = self._cds.index(intCD)
    ...
    def _isHalloweenEquipment(item):
        return b'hwEquipment' in item.getDescriptor().tags
    ...
    def _isHalloweenEmptySlot(cls, item):
        return cls._isHalloweenEquipment(item) and b'hwEmptySlot' in item.getDescriptor().tags
    ...
    def _isSuperShell(item):
        return b'hwSuperShell' in item.getDescriptor().tags
