...
class HWBuyAndInstallConsumablesProcessor(BuyAndInstallConsumablesProcessor):
    ...
    def _successHandler(self, code, ctx=None):
        additionalMessages = []
        if ctx:
            additionalMessages = [HWItemBuyProcessorMessage(self.itemsCache.items.getItemByCD(cd), count, Money.makeFromMoneyTuple(price)).makeSuccessMsg() for cd, price, count in ctx.get(b'eqs', [])]
...
class HWItemBuyProcessorMessage(ItemBuyProcessorMessage):
    ...
    def _getMsgCtx(self):
        ...
        res[b'kind'] = backport.text(R.strings.halloween_system_messages.hw_equipment.title())
        return res
