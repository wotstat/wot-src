...
class HalloweenEntryPoint(SquadEntryPoint):
    ...
    def makeDefCtx(self):
        return HalloweenSquadSettingsCtx(waitingID=b'prebattle/create', accountsToInvite=self._accountsToInvite, arenaUniqueID=self.arenaUniqueID)
    ...
    def _setDifficultyLevel(self, ctx, callback=None):
        ...
        if not pPermissions.canChangeRosters():
            ...
            if callback:
                callback(False)
            return
        self._requestsProcessor.doRequest(ctx, b'doUnitCmd', CLIENT_UNIT_CMD.SET_UNIT_DIFFICULTY_LEVEL, ctx.getQueueType(), 0, b'', callback=callback)
