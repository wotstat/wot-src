...
class SurveyChannelHandler(ServiceChannelHandler):
    ...
    def _showAward(self, ctx):
        _, message = ctx
        arenaUniqueID = message.data.get(b'arenaUniqueID', 0)
        timeout = message.data.get(b'timeout', 0)
        if arenaUniqueID == 0 or timeout == 0:
            _logger.warning(b'Some survey data is 0. timeout: %s ArenaID: %s', timeout, arenaUniqueID)
            return
