...
def _handlerOpenningFinished(eventData):
    ...
    if uniqueRewardsHandler:
        return StringEvent(LootBoxesStorageEventEnum.GOTO_UNIQUE_REWARDING, uniqueRewardsHandler=uniqueRewardsHandler, rewards=rewards, clientData=clientData)
...
def _handlerResponseReceived(result):
    if result and result.success:
        if result.auxData and result.auxData.get(b'clientData', {}).get(b'countOfOpened', 0) == 0:
...
@adisp_process(b'lootboxOpeninig')
...
def _handleRequestToOpen(context, event, eventLootBoxesCtrl=None):
    lootBox, count, keyID = event.getArgument(b'lootBox'), event.getArgument(b'count'), event.getArgument(b'keyID')
    result = None
    if eventLootBoxesCtrl.isEnabled() and lootBox is not None and lootBox.isEnabled:
        ...
        if result and result.success:
            ...
            auxData[b'clientData'] = {b'openWithKey': (True if keyID else False)}
            auxData[b'clientData'].update({b'usedKeys': {keyID: count}})
            auxData[b'clientData'][b'countOfOpened'] = auxData.get(b'extData', {}).get(b'openedLootBoxes', {}).get(lootBox.getID(), count)
            auxData[b'clientData'][b'giftsInfo'] = auxData.get(b'giftsInfo')
            auxData[b'clientData'][b'uniqueOpening'] = False
...
class LootBoxesContext(object):
    ...
    __slots__ = (b'__stateMachine', b'__stateMachineObserver', b'__currentState', b'onStateChanged', b'__asyncScope', b'__returnPlace')
    ...
    def init(self):
        Waiting.show(b'loadLootboxesStorage')
    ...
    def viewReady(self):
        ...
        Waiting.hide(b'loadLootboxesStorage')
        return
    ...
    def fini(self):
        ...
        if Waiting.isOpened(b'loadLootboxesStorage'):
            Waiting.hide(b'loadLootboxesStorage')
        return
