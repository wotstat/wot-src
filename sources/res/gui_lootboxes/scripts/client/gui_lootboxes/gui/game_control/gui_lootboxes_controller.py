...
class GuiLootBoxesController(IGuiLootBoxesController):
    __slots__ = (b'__em', b'__boxesCount', b'__isLootBoxesAvailable', b'__isActive', b'__bonusesConfig', b'__hangarOptimizer', b'__boxKeysCount')
    ...
    def onLobbyInited(self, event):
        ...
        g_clientUpdateManager.addCallbacks({b'lootBoxes': (self.__onBoxesUpdate)})
    ...
    def __onServerSettingsChange(self, settings):
        ...
        if GUI_LOOT_BOXES_CONFIG in settings:
            self.onStatusChange()
            self.__isActive = self.isEnabled()
        if b'lootBoxes_config' in settings:
    ...
    def __onBoxesUpdate(self, diff):
        if b'history' in diff:
            self.onBoxesHistoryUpdate(diff[b'history'])
        return
    ...
    def __updateBoxCountToGuaranteedBonus(self):
        opened = 0
        guaranteedBonusLimit = self.__boxInfo.get(EventLootBoxes.PREMIUM, {}).get(b'limit', 0)
        boxHistoryData = self.__boxInfo.get(EventLootBoxes.PREMIUM, {}).get(b'history')
        if boxHistoryData:
            _, bonusData, _ = boxHistoryData
            if bonusData is not None:
                _, opened, _ = bonusData.get(b'guaranteedBonusLimit', (0, 0, 0))
        ...
        if result and result.success and result.auxData:
            for lootBoxID, lootBoxData in result.auxData.iteritems():
                if lootBoxID in self.getBoxesIDs():
                    boxData = dict()
                    data = lootBoxData.get(b'bonus', {})
                    lbType = lootBoxData[b'type']
                    boxData[b'id'] = lootBoxID
                    boxData[b'limit'] = parseLimitBoxInfoSection(lootBoxData.get(b'config', {}))
                    boxData[b'slots'] = parseAllOfBonusInfoSection(data.get(b'allof', {}))
                    boxData[b'history'] = lootBoxData.get(b'history', (0, None, 0))
