...
def getLootBoxesBonusPackerMap():
    ...
    mapping.update({b'vehicles': (LootBoxVehiclesBonusUIPacker()), 
       b'crewBooks': (LootBoxCrewBookBonusUIPacker()), 
       b'tmanToken': (TmanTemplateBonusPacker()), 
       b'customizations': (LootBoxCustomizationBonusUIPacker()), 
       b'collectionItem': (LootBoxAnyCollectionItemBonusUIPacker()), 
...
def getLootboxesWithPossibleCompensationBonusPacker():
    mapping = getLootBoxesBonusPackerMap()
    mapping.update({b'vehicles': (LootBoxVehiclesWithPossibleCompensationBonusUIPacker())})
    return BonusUIPacker(mapping)


def getRewardsScreenDefaultBonusPackerMap():
    mapping = getLootBoxesBonusPackerMap()
    mapping.update({b'tankmen': (LootBoxTankmenBonusUIPacker()), 
       b'collectionItem': (LootBoxCollectionItemBonusUIPacker()), 
       b'battleToken': (LootBoxTokensBonusUIPacker()), 
...
def getAdditionalRewardsTooltipBonusPacker():
    ...
    mapping.update({b'customizations': (AdditionalRewardsCustomizationBonusUIPacker())})
    return BonusUIPacker(mapping)
...
def getMainRewardsBonusPacker():
    mapping = getRewardsScreenDefaultBonusPackerMap()
    mapping.update({b'tmanToken': (TmanTemplateRewardScreenBonusPacker()), 
       b'customizations': (LootBoxUniqueCustomizationBonusUIPacker()), 
       (LbC11nProgressTokenBonus.BONUS_NAME): (LootboxStyleProgressBonusUIPacker())})
...
class LootBoxCustomizationBonusUIPacker(CustomizationBonusUIPacker):
    ...
    def _packSingleBonus(cls, bonus, item, label):
        ...
        model.setItem(str(c11Item.itemTypeName) + b'_3d' if c11Item.itemTypeID == GUI_ITEM_TYPE.STYLE and c11Item.is3D else b'')
    ...
    def _getContentId(cls, bonus):
        ...
        for b in bonus.getCustomizations():
            if b is not None:
                compensation = b.get(b'customCompensation', None)
                if compensation:
                    compBonus = cls.__getCompBonus(compensation)
                    if compBonus:
...
class LootBoxCrewBookBonusUIPacker(CrewBookBonusUIPacker):
    ...
    def _getToolTip(cls, bonus):
        ...
        return [
         createTooltipData(tooltip=None, isSpecial=True, specialAlias=specialAlias, specialArgs=specialArgs)]
...
class LootBoxTokensBonusUIPacker(TokenBonusUIPacker):
    ...
    def _getToolTip(cls, bonus):
        ...
        for tokenID, record in bonus.getTokens().iteritems():
            if tokenID.startswith(TOKEN_COMPENSATION_PREFIX):
                ...
                bonus = first(getServiceBonuses(currency, value * record.count, isCompensation=True))
                if bonus:
                    result.append(createTooltipData(tooltip=None, isSpecial=True, specialAlias=None, specialArgs=[
                     cls.__getCompReasonBonus(item, itemID), bonus]))
    ...
    def __getCompReasonBonus(cls, item, itemID):
        if item == b'cllc':
            return AnyCollectionItemBonus(b'anyCollectionItem', itemID, False)
        else:
            return
...
class LootBoxCollectionItemBonusUIPacker(SimpleBonusUIPacker):
    ...
    @classmethod
    def _pack(cls, bonus):
        if cls._isValidBonus(bonus):
            return [cls._packSingleBonus(bonus, b'')]
        return []
    ...
    def _packSingleBonus(cls, bonus, label):
        ...
        cls._packCommon(bonus, model)
        item = bonus.getItem()
        model.setIcon((b'{}_{}_{}').format(item.type, bonus.getCollectionId(), bonus.getItemId()))
    ...
    def _getToolTip(cls, bonus):
        if cls._isValidBonus(bonus):
            return [createTooltipData(tooltip=None, isSpecial=True, specialAlias=None, specialArgs=[bonus.getItemId(), bonus.getCollectionId()])]
        else:
            return []
...
class LootBoxAnyCollectionItemBonusUIPacker(LootBoxCollectionItemBonusUIPacker):
    ...
    def _packSingleBonus(cls, bonus, label):
        ...
        model.setIcon((b'any_{}').format(bonus.getCollectionId()))
    ...
    def _getToolTip(cls, bonus):
        if cls._isValidBonus(bonus):
            return [createTooltipData(makeTooltip(header=backport.text(getCollectionRes(bonus.getCollectionId()).anyCollectionItem.tooltip.header()), body=backport.text(getCollectionRes(bonus.getCollectionId()).anyCollectionItem.tooltip.header())))]
        return []
...
class LootboxStyleProgressBonusUIPacker(StyleProgressBonusUIPacker):
    ...
    def _getIcon(cls, styleID, progressLevel):
        iconName = (b'style_progress_{styleID}_{progressLevel}').format(styleID=styleID, progressLevel=progressLevel)
        if R.images.gui.maps.icons.quests.bonuses.s600x450.dyn(iconName).exists():
            return iconName
