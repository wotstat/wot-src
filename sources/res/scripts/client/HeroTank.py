...
class HeroTank(ClientSelectableCameraVehicle):
    ...
    def __init__(self, name=b'HeroTank'):
    ...
    def _onVehicleLoaded(self):
        ...
        if self.enabled:
            g_eventBus.handleEvent(events.HangarVehicleEvent(events.HangarVehicleEvent.ON_HERO_TANK_LOADED, ctx={b'entity': self}), scope=EVENT_BUS_SCOPE.LOBBY)
        if self.__heroTankCD is None:
            self.removeModelFromScene()
        return

    def _onVehicleDestroy(self):
        g_eventBus.handleEvent(events.HangarVehicleEvent(events.HangarVehicleEvent.ON_HERO_TANK_DESTROY, ctx={b'entity': self}), scope=EVENT_BUS_SCOPE.LOBBY)
        return
...
def debugReloadHero(heroName, heroTankCtrl=None):
    if not IS_DEVELOPMENT:
        ...
        if heroTankCtrl is not None:
            heroCD = None if heroName == b'Reset' else vehicles.VehicleDescr(typeName=heroName).type.compactDescr
