...
_WOT_CLIENT_PARAM_NAME = b'wot_client_param'
_WOT_RESOURCE_CUSTOM_SCHEME = b'wotdata'
...
class EventListener(object):
    ...
    def onResourceLoadError(self, method, url, status, statusStr, error, requestHeaders, responseHeaders):
        _logger.warning(b'failed %s %s (status %d (%s)\terror %d)', method, url, status, statusStr, error)
        if _EXTENDED_LOGGING:
            from pprint import pformat
            _logger.warning(b'headers:\n\trequest:\n%s\n\tresponse:\n%s', pformat(requestHeaders), pformat(responseHeaders))
        return
