...
class StaticDeathZone(BigWorld.Entity):
    ...
    def _playDamageEffect(self, vehicleId, vfx):
        ...
        if effects:
            ...
            LOG_DEBUG_DEV(b'There are no effects ', vfx)
        return
    ...
    def _onAvatarReady(self):
        if self.isActive:
            ...
            if vehicleUnderFire is not None:
                timeToStrike = vehicleUnderFire[b'nextStrikeTime']
                if timeToStrike > 0:
                    deathzonesCtrl = self.sessionProvider.shared.deathzones
                    if deathzonesCtrl:
                        deathzonesCtrl.updateDeathZoneWarningNotification(self.zoneId, True, timeToStrike, vehicleUnderFire[b'waveDuration'])
    ...
    def _getVehicleUnderFire(self):
        ...
        for vehicleUnderFire in self.vehiclesUnderFire:
            if vehicleUnderFire[b'vehicleId'] == playerVehicleId:
