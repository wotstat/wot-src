...
class VehicleBase(object):
    ...
    def isWheeledTech(self):
        return b'wheeledVehicle' in self.typeDescriptor.type.tags
...
class SimulatedVehicle(BigWorld.Entity, VehicleBase):
    ...
    def gunOriginMatrix(self, gunInstallationIndex=DEFAULT_GUN_INSTALLATION_INDEX, gunIndex=0):
        ...
        if not gun.multiGun or isAutoShootGun(gun):
            gunOriginMatrix = self.appearance.compoundModel.node(b'HP_gunJoint')
        else:
    ...
    def gunFireMatrix(self, gunInstallationIndex=DEFAULT_GUN_INSTALLATION_INDEX, gunIndex=0):
        gun = self.typeDescriptor.gunInstallations[gunInstallationIndex].gun
        if not gun.multiGun or isAutoShootGun(gun):
            gunFireMatrix = self.appearance.compoundModel.node(b'HP_gunFire')
        else:
    ...
    def addModel(self, model):
        ...
        if not highlighter:
            _logger.error(b'Highlighter component is not ready/not created')
            return
        if highlighter.isOn:
    ...
    def updateBrokenTracks(self, trackStates):
        ...
        for index, trackState in enumerate(trackStates):
            if trackState[b'isBroken'] and not self.__brokenTrackVisible[index]:
                self.__brokenTrackVisible[index] = True
                self.appearance.addSimulatedCrashedTrack(index, self.simulationData_tracksInAir, trackState[b'hitPoint'])
