...
class BalanceEventHandler(C2WHandler):
    ...
    def init(self):
        ...
        g_clientUpdateManager.addCallbacks({(b'stats.{}').format(c): self.__onBalanceUpdate for c in Currency.ALL})
        g_clientUpdateManager.addCallback(b'stats.freeXP', self.__onBalanceUpdate)
        g_clientUpdateManager.addCallbacks({b'cache.dynamicCurrencies': (self.__onBalanceUpdate)})
        return
