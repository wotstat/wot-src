...
@w2capi(name=b'arenas', key=b'action')
class ArenasWebApi(object):

    @w2c(_ArenaInfoSchema, b'arena_info')
    def arenaInfo(self, cmd):
        arenaType = ArenaType.g_cache.get(cmd.arena_id)
        if arenaType is None:
            return {b'arena_id': (cmd.arena_id), 
               b'error': b'arena_id is invalid'}
        else:
            return self.__collectArenaInfo(arenaType)

    @w2c(_ArenasInfoSchema, b'arenas_info')
    ...
    def __collectArenaInfo(self, arenaType):
        arenaName = R.strings.arenas.num(arenaType.geometryName).dyn(b'name')()
        arenaDescription = R.strings.arenas.num(arenaType.geometryName).dyn(b'description')()
        gameplayName = R.strings.arenas.type.dyn(arenaType.gameplayName).dyn(b'name')()
        imageUrl = R.images.gui.maps.icons.map.num(arenaType.geometryName)()
        return {b'arena_name': (self.__getText(arenaName)), 
           b'arena_description': (self.__getText(arenaDescription)), 
           b'gameplay_name': (self.__getText(gameplayName)), 
           b'image_url': (self.__getImage(imageUrl))}
