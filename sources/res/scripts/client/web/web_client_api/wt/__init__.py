...
@w2capi(name=b'wt', key=b'action')
class WtWebApi(W2CSchema):

    @w2c(W2CSchema, name=b'show_event_storage_window')
    def showEventStorageWindow(self, _):
        ...
        def predicate(window):
            return window.content is not None and getattr(window.content, b'layoutID', None) == R.views.white_tiger.lobby.WtStorageView()
