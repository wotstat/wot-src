...
class AccessTokenWebApiMixin(object):
    ...
    @w2c(_RequestAccessTokenSchema, b'access_token')
    def accessToken(self, cmd):
        ...
        if accessTokenData is not None:
            yield {b'spa_id': (str(self._connectionMgr.databaseID)), b'access_token': (str(accessTokenData.accessToken)), 
               b'expires_in': (accessTokenData.expiresAt - time_utils.getCurrentTimestamp()), 
               b'periphery_id': (str(self._connectionMgr.peripheryID))}
        else:
            yield {b'error': b'Unable to obtain access token.'}
        return
