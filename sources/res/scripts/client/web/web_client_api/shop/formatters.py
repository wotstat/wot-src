...
def _formatVehicleRestore(item):
    if item.isRestorePossible():
        ...
        if item.hasLimitedRestore():
            ...
            restoreEndDate = time_utils.timestampToISO(restoreTimestamp) if restoreTimestamp < float(b'inf') else None
        else:
            restoreEndDate = None
    ...
    if item.isInInventory and item.activeInNationGroup:
        ...
        if not item.isRented:
            ...
            result[b'info'] = {b'event': ({b'type': (SHOP_RENT_SEASON_TYPE_MAP.get(event.seasonType, b'unknown')), b'id': (event.seasonID), b'duration': (_RENT_DURATION_MAP.get(event.duration, b'undefined')), b'expire': (event.expiryTime)} if event else None), 
               b'rentType': rentType, 
               b'time': (_formatTime(info.getTimeLeft())), 
               b'battles': (info.battlesLeft), 
               b'wins': (info.winsLeft)}
        return result
    return
...
def makeVehicleFormatter(includeInventoryFields=False):
    ...
    isOutfitLocked = Field(b'isOutfitLocked', (lambda i: i.isOutfitLocked))
    isRestoreWithStyle = Field(b'isRestoreWithStyle', (lambda i: i.isRestoreWithStyle))
    fields = [
     idField, 
     nameField, 
     shortName, 
...
def makeCustomizationFormatter():
    fields = [
     ...
     Field(b'inventoryCount', (lambda i: i.inventoryCount)),
     buyPriceField,
     sellPriceField]
    return Formatter(fields)
