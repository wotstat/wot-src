...
class TradeInShopVehicleStates(object):
    ...
    UNSUITABLE = b'UNSUITABLE'
    DAMAGED = b'DAMAGED'
    IN_TACTICAL_UNIT = b'IN_TACTICAL_UNIT'
...
class UnifiedTradeInWebApiMixin(object):
    ...
    @w2c(W2CSchema, b'get_unified_trade_in_enabled')
    def getUnifiedTradeInEnabled(self, cmd):
        enabled = bool(self._tradeIn.isEnabled() and self._tradeIn.hasAvailableOffer())
        return {b'isEnabled': enabled}

    @w2c(W2CSchema, b'get_unified_trade_in_info')
    def getUnifiedTradeInInfo(self, cmd):
        ...
        return {b'expirationDate': (self._tradeIn.getExpirationTime()), 
           ...
           b'vehiclesToSell': vehiclesToSell, 
           b'vehiclesToBuy': vehiclesToBuy}

    @w2c(_SelectedVehicleSchema, b'set_unified_trade_in_vehicle_to_sell')
    ...
    @w2c(_SelectedVehicleSchema, b'set_unified_trade_in_vehicle_to_buy')
    ...
    @w2c(W2CSchema, b'execute_unified_trade_in')
    ...
    def _getBuyVehicleFormatter(self):
        if self.buyVehicleFormatter is None:
            self.buyVehicleFormatter = TradeInFormatter(formatters.makeVehicleFormatter(True), [
             FormatterField(b'tradeOffPrice', (lambda i: i.tradeInBuyPrice.toDict() or None)),
             FormatterField(b'discountPercentage', (lambda i: i.discountPercentage))])
    ...
    def _getSellVehicleFormatter(self):
        ...
        if self.sellVehicleFormatter is None:
            self.sellVehicleFormatter = TradeInFormatter(formatters.makeVehicleFormatter(True), [
             FormatterField(b'isFreeExchange', (lambda i: i.isFreeExchange)),
             FormatterField(b'state', formatState)])
