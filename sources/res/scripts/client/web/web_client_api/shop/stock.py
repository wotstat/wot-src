...
class _PremiumPack(object):
    ...
    @property
    def userName(self):
        return backport.text(R.strings.menu.premium.packet.dyn((b'days{}').format(self.__duration))())
...
_InventoryEnhancements = namedtuple(b'_InventoryEnhancements', (b'id', b'count'))
_InstalledEnhancements = namedtuple(b'_InstalledEnhancements', (b'vehIntCD', b'enhancements'))
...
_ITEMS_CRITERIA_MAP = {(ShopItemType.VEHICLE): {b'inventory': (REQ_CRITERIA.INVENTORY | ~REQ_CRITERIA.VEHICLE.EVENT), 
                            b'premium': (REQ_CRITERIA.VEHICLE.PREMIUM), 
                            b'ready': (REQ_CRITERIA.VEHICLE.READY), 
                            b'sellable': (REQ_CRITERIA.VEHICLE.CAN_SELL), 
                            b'secret': (REQ_CRITERIA.SECRET), 
                            b'hidden': (REQ_CRITERIA.HIDDEN), 
                            b'is_premium_igr': (REQ_CRITERIA.VEHICLE.IS_PREMIUM_IGR), 
                            b'trade_in': (REQ_CRITERIA.VEHICLE.CAN_TRADE_IN), 
                            b'trade_off': (REQ_CRITERIA.VEHICLE.CAN_TRADE_OFF), 
                            b'skip_multinational_copies': (REQ_CRITERIA.VEHICLE.ACTIVE_IN_NATION_GROUP), 
                            b'collectible': (REQ_CRITERIA.COLLECTIBLE), 
                            b'earn_crystals': (REQ_CRITERIA.VEHICLE.EARN_CRYSTALS), 
                            b'event_battle': (REQ_CRITERIA.VEHICLE.EVENT_BATTLE)}, 
   (ShopItemType.CREW): {b'premium': (ShopCrewCriteria.PREMIUM)}, 
   (ShopItemType.EQUIPMENT): {b'inventory': (REQ_CRITERIA.INVENTORY), 
                              b'secret': (REQ_CRITERIA.SECRET), 
                              b'hidden': (REQ_CRITERIA.HIDDEN), 
                              b'builtin': (REQ_CRITERIA.EQUIPMENT.BUILTIN)}, 
   (ShopItemType.DEVICE): {b'inventory': (REQ_CRITERIA.INVENTORY), 
                           b'secret': (REQ_CRITERIA.SECRET), 
                           b'hidden': (REQ_CRITERIA.HIDDEN)}, 
   (ShopItemType.BATTLE_BOOSTER): {b'inventory': (REQ_CRITERIA.INVENTORY), 
                                   b'secret': (REQ_CRITERIA.SECRET), 
                                   b'hidden': (REQ_CRITERIA.HIDDEN)}, 
   (ShopItemType.MODULE): {b'inventory': (REQ_CRITERIA.INVENTORY), 
                           b'secret': (REQ_CRITERIA.SECRET), 
                           b'hidden': (REQ_CRITERIA.HIDDEN)}, 
   (ShopItemType.BOOSTER): {b'inventory': (REQ_CRITERIA.BOOSTER.IN_ACCOUNT), 
                            b'hidden': (REQ_CRITERIA.HIDDEN), 
                            b'enabled': (REQ_CRITERIA.BOOSTER.ENABLED)}, 
   (ShopItemType.SHELL): {b'inventory': (REQ_CRITERIA.INVENTORY), 
                          b'secret': (REQ_CRITERIA.SECRET), 
                          b'hidden': (REQ_CRITERIA.HIDDEN)}, 
   (ShopItemType.PREMIUM): {}, (ShopItemType.PAINT): {}, (ShopItemType.CAMOUFLAGE): {}, (ShopItemType.MODIFICATION): {}, (ShopItemType.STYLE): {b'on_account': (REQ_CRITERIA.CUSTOMIZATION.ON_ACCOUNT), 
                          b'inventory': (REQ_CRITERIA.CUSTOMIZATION.ON_ACCOUNT)}, 
   (ShopItemType.DECAL): {}, (ShopItemType.EMBLEM): {}, (ShopItemType.INSCRIPTION): {}, (ShopItemType.PROJECTION_DECAL): {}, (ShopItemType.CREW_BOOKS): {}, (ShopItemType.ATTACHMENT): {}, (ShopItemType.ENHANCEMENT): {b'inventory': (REQ_CRITERIA.INVENTORY)}}
...
def _parseCriteriaSpec(itemType, spec, idList=None):
    ...
    normalizedIds = [i.replace(b'!', b'') for i in ids]
    ...
    for critId, normalizedCritId in zip(ids, normalizedIds):
        try:
            ...
            if critId.startswith(b'!'):
                criteria = ~criteria
            compoundCriteria |= criteria
        except KeyError:
            raise SoftException((b'item type "{}" does not support criteria "{}"').format(itemType, critId))
...
class _GetItemsSchema(W2CSchema):
    ...
    criteria = Field(required=False, type=list, validator=(lambda value, data: _parseCriteriaSpec(data[b'type'], value)))
    fields = Field(required=False, type=list)
...
class ItemsWebApiMixin(object):
    ...
    @w2c(_GetItemsByCdSchema, b'get_items_by_cd')
    ...
    @w2c(_GetItemByCdSchema, b'get_item_by_cd')
    def getItemByCD(self, cmd):
        allowedFields = cmd.fields and set(cmd.fields)
        return self.__getItemByCD(cmd.cd, allowedFields)

    @w2c(_GetItemsSchema, b'get_items')
    ...
    def __makeFormatterKey(self, itemType, criteria):
        ...
        if itemType == ShopItemType.VEHICLE and criteria.lookInInventory():
            entries.append(b'inventory')
        elif itemType == ShopItemType.ENHANCEMENT:
            if criteria.lookInInventory():
                entries.append(b'inventory')
            else:
                entries.append(b'installed')
