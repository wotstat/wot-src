...
class SubCommand(object):
    __slots__ = (b'subSchema', b'handler')
...
class Field(object):
    __slots__ = (b'required', b'type', b'default', b'deprecated', b'validator', b'__weakref__')

    def __init__(self, required=False, type=None, default=None, deprecated=None, validator=None):
        if required and default is not None:
            _logger.warning(b'Do not use "required" and "default" at the same time!')
...
class W2CSchemaMeta(ISchemeMeta):
    ...
    def validate(cls, data):
        ...
        if cls.__unions__:
            ...
            if not found:
                raise WebCommandException(b'Command validation failed: Not found suitable parameters!')
        return
    ...
    def toObject(cls, data):
        ...
        data[b'custom_parameters'] = {}
        for name in customParameters:
            data[b'custom_parameters'][name] = data.pop(name)
    ...
    def __validateField(schema, key, data, required):
        if required and key not in data:
            return b"Command validation failed: '%s' parameter is missing!" % key
        else:
            if key in data:
                if schema.hasType() and not isinstance(data[key], schema.type):
                    return b"Command validation failed: Value '%s' for parameter '%s' is wrong!" % (data[key], key)
                if schema.hasValidator():
                    ...
                    if not isValid:
                        return (b'Invalid value for field {}: {}{}').format(key, data[key], errmsg)
                deprecatedMessage = schema.deprecated
                if deprecatedMessage is not None:
                    _logger.warning(b'"%s" parameter is deprecated (%s)!', key, deprecatedMessage)
            return
...
class Schema2Command(object):
    ...
    def __getattr__(self, name):
        ...
        raise AttributeError((b'Property is not found in {0}: {1}').format(self.__class__, name))
        return
...
def createSubCommandsHandler(commandName, commandSchema, keyField, subCommands, finiHandlerFunc=None):
    ...
    def handlerFunc(command, ctx):
        ...
        if key in subCommands and subCommands[key].hasHandler():
            ...
            raise WebCommandException(b"Unsupported value for '%s': '%s'" % (keyField, key))
        return
    ...
    def __call__(self, clazz):
        ...
        if self.__name and self.__key:
            ...
            def generate(key, scheme):
                ...
                def getter(api):
                    subcommands = {wrapper.w2c_name: SubCommand(wrapper.w2c_schema, partial(wrapper, api)) for _, wrapper in inspect.getmembers(clazz, inspect.ismethod) if getattr(wrapper, b'w2c_name', None)}
                    return [
            ...
            def getHandlers(api):
                return [createCommandHandler(commandName=wrapper.w2c_name, commandSchema=wrapper.w2c_schema, handlerFunc=partial(wrapper, api), finiHandlerFunc=partial(getattr(clazz, wrapper.w2c_fini), api) if wrapper.w2c_fini else None) for _, wrapper in inspect.getmembers(clazz, inspect.ismethod) if getattr(wrapper, b'w2c_name', None)]
...
def w2c(schema, name=b'', finiHandlerName=None):

    def decorator(fn):
        if inspect.isgeneratorfunction(fn):

            @uniprof.regionDecorator(label=(b'w2c {}').format(name), scope=b'wrap')
            def handler(self, cmd, ctx):
                _CallbackDispatcher(fn(self, cmd), ctx[b'callback'])
                return
            ...
            def handler(self, cmd, ctx):
                ...
                if b'ctx' in argspec.args and argspec.args.index(b'ctx') == 2:
                    return ctx[b'callback'](fn(self, cmd, ctx))
                return ctx[b'callback'](fn(self, cmd))
...
class WebCommandHandler(object):
    ...
    def handleCommand(self, data):
        _logger.debug(b'Web2Client handle: %s', data)
        try:
            parsed = json.loads(data, encoding=b'utf-8')
        except (TypeError, ValueError) as exception:
            raise WebCommandException(b'Command parse failed! Description: %s' % exception)
    ...
    def addHandler(self, handler):
        if handler.name not in self.__handlers:
            ...
            raise WebCommandException(b'Handler for command `%s` is already registered!' % handler.name)
        return
    ...
    def handleWebCommand(self, webCommand):
        ...
        if handler is None:
            raise WebCommandException(b"Command '%s' is unsupported!" % commandName)
    ...
    def __createCtx(self, commandName, webId):
        ...
        def callback(data):
            callbackData = {b'command': commandName, 
               b'data': data, 
               b'web_id': webId}
            _logger.debug(b'Web2Client callback: %s', callbackData)
            jsonMessage = json.dumps(callbackData)
            self.onCallback(jsonMessage)
            return

        return {b'browser_id': (self.__browserID), 
           b'browser_alias': (self.__alias), 
           b'browser_view': (self.__browserView), 
           b'callback': callback}
