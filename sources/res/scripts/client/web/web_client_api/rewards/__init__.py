...
def _achievementBlockValidator(block, _=None):
    if block not in ACCOUNT_DOSSIER_BLOCKS:
        raise WebCommandException((b'unsupported achievement block "{}"').format(block))
    return True
...
class _RequestAchievementsSchema(W2CSchema):
    medal_ids = Field(type=list, default=[])
    achievement_block = Field(type=basestring, validator=_achievementBlockValidator, default=b'singleAchievements')


@w2capi(name=b'rewards', key=b'action')
class RewardsWebApi(W2CSchema):
    itemsCache = dependency.descriptor(IItemsCache)
    eventsCache = dependency.descriptor(IEventsCache)

    @w2c(_RequestMedalsSchema, b'get_medals')
    def requestRewardInfo(self, cmd):
        responseData = []
        for recordId in cmd.medal_ids:
            record = DB_ID_TO_RECORD.get(int(recordId), (b'', b''))
            _, rewardId = record
            factory = getAchievementFactory(record).create()
            responseData.append({b'medal_id': recordId, 
               b'image': ((b'').join((b'gui/maps/icons/achievement/', rewardId, b'.png'))), 
               b'title': ((b'').join((b'#achievements:', rewardId))), 
               b'name': (factory.getUserName()), 
               b'description': (factory.getUserWebDescription()), 
               b'condition': (factory.getUserCondition())})

        return responseData

    @w2c(_RequestQuestBonusSchema, b'get_quest_bonuses')
    def requestQuestBonusesInfo(self, cmd):
        return self.__getBonusesInfoByQuestsBaseToken(questIdBase=cmd.quest_id_base)

    @w2c(_RequestAchievementsSchema, b'get_achievements_from_dossier')
