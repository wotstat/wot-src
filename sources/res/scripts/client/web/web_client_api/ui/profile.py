...
_PROFILE_TAB_ALIASES = {b'hof': (VIEW_ALIAS.PROFILE_HOF), 
   b'technique': (VIEW_ALIAS.PROFILE_TECHNIQUE_PAGE), 
   b'summary': (VIEW_ALIAS.PROFILE_SUMMARY_PAGE), 
   b'awards': (VIEW_ALIAS.PROFILE_AWARDS), 
   b'statistics': (VIEW_ALIAS.PROFILE_STATISTICS)}
...
class ProfileTabWebApiMixin(object):
    ...
    @w2c(_OpenProfileTabSchema, b'profile')
    def profile(self, cmd):
        g_eventBus.handleEvent(events.LoadViewEvent(SFViewLoadParams(VIEW_ALIAS.LOBBY_PROFILE), ctx={b'selectedAlias': (_PROFILE_TAB_ALIASES[cmd.selected_id])} if cmd.selected_id else None), scope=EVENT_BUS_SCOPE.LOBBY)
        return


class ProfileWindowWebApiMixin(object):

    @w2c(_OpenProfileWindowSchema, b'profile_window')
    def profile(self, cmd):
