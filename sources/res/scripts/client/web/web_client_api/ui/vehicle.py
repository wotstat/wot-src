...
def _isValidObtainingMethod(obtainingMethod, _):
    if ObtainingMethods.hasValue(obtainingMethod):
        return True
    raise SoftException((b'obtaining_method: "{}" is not supported').format(obtainingMethod))
    return
...
def _parseOffers(offers):
    return [_VehicleOfferEntry(id=_getOfferID(offer) or str(ndx), eventType=offer.get(b'event_type'), rent=offer.get(b'rent'), crew=_getOfferCrew(offer), name=_getOfferStr(offer, VEHICLE_PREVIEW.getOfferName), label=_getOfferStr(offer, VEHICLE_PREVIEW.getOfferLabel), left=_getRentLeft(offer), buyPrice=Money(**offer.get(b'buy_price', MONEY_UNDEFINED)), bestOffer=offer.get(b'best_offer'), buyParams=offer.get(b'buy_params'), preferred=bool(offer.get(b'preferred', False))) for ndx, offer in enumerate(offers)]
...
class VehiclePreviewWebApiMixin(object):
    ...
    def openVehicleOffersPreview(self, cmd):
        ...
        event_dispatcher.showVehiclePreview(vehTypeCompDescr=int(cmd.vehicle_id), offers=offers, title=getattr(cmd, b'title', b''), price=MONEY_UNDEFINED, oldPrice=MONEY_UNDEFINED, previewAlias=self._getVehiclePreviewReturnAlias(cmd))
        return
    ...
    @w2c(_BPBitStylePreviewSchema, b'bpbit_style_preview')
    ...
    @w2c(_VehicleStylePreviewWithTabsSchema, b'vehicle_style_preview_with_tabs')
