...
class _OpenRankedBattlesScheme(W2CSchema):
    ...
    pageParams = Field(required=False, type=basestring, default=b'')
...
class OpenRankedPagesMixin(object):
    ...
    @w2c(_OpenRankedBattlesScheme, name=b'open_ranked_page')
    def openRankedPage(self, cmd):
        self.__rankedController.showRankedBattlePage(ctx={b'selectedItemID': (cmd.pageId), b'showedFromWeb': True, b'webParams': (cmd.pageParams)})
        return

    @w2c(_OpenBrowserOverlaySchema, name=b'open_ranked_overlay')
