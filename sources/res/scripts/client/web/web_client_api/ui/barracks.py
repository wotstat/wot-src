...
def _isValidTankType(_, data):
    return _isValidParam(data, b'tank_type', _VEHICLE_CLASSES)


def _isValidTankmanRole(_, data):
    return _isValidParam(data, b'role', _TANKMAN_ROLES)
...
def _isOverParam(data, field):
    ...
    if loc == TManLocation.NEWBIES.value:
        _logger.warning(b'"%s" will be ignored, is over-param for location "%s",please, simplify your W2C', field, loc)
        return True
    return False
...
class BarracksWebApiMixin(object):
    ...
    @w2c(_BarracksTabSchema, b'barracks')
