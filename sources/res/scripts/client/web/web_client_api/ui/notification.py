...
class _NotificationSchema(W2CSchema):
    __unions__ = (b'message', b'i18n_key', b'key')
    ...
    key = Field(type=basestring, deprecated=b'prefer "i18n_key"')
...
class NotificationWebApiMixin(object):
    ...
    @w2c(_NotificationSchema, b'notification')
    def notification(self, cmd):
        smType = SM_TYPE.lookup(cmd.type)
        if smType is None:
            raise WebCommandException(b"Unknown notification's type: %s!" % cmd.type)
        if cmd.hasMessage():
