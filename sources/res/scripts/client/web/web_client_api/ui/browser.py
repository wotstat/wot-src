...
def _tryToCloseWulfView(view):
    ...
    if window is not None:
        ...
        raise WebCommandException((b'Parent window is None. View: {}').format(view))
    return


class OpenBrowserWindowWebApiMixin(object):

    @w2c(_OpenBrowserWindowSchema, b'browser')
...
class CloseBrowserWindowWebApiMixin(object):
    ...
    @w2c(W2CSchema, b'browser')
    def browser(self, cmd, ctx):
        browserAlias = ctx.get(b'browser_alias')
        browserId = ctx.get(b'browser_id')
        browserView = ctx.get(b'browser_view')
        if browserView and isinstance(browserView, wulf.View):
            ...
            if app is not None and app.containerManager is not None:
                ...
                if browserWindow is not None:
                    ...
                    raise WebCommandException((b'Browser window could not be found! May be alias "{}" is wrong or probably browser has unsupported layer.').format(windowAlias))
        else:
            raise WebCommandException((b'Unable to close Browser Window! view alias: {}, id: {}, instance: {}').format(browserAlias, browserId, browserView))
        ...
        if browserView and isinstance(browserView, wulf.View):
            ...
            if app is not None and app.containerManager is not None:
                ...
                if browserView is not None:
                    browserView.onCloseView()
                    return
        raise WebCommandException(b'Unable to find BrowserView!')
        return


class OpenExternalBrowserWebApiMixin(object):

    @w2c(_OpenExternalBrowserSchema, b'external_browser')
...
class OpenBrowserOverlayWebApiMixin(object):
    ...
    @w2c(_OpenBrowserOverlaySchema, b'browser_overlay')
    def openBrowserOverlay(self, cmd):
        showBrowserOverlayView(cmd.url, alias=VIEW_ALIAS.WEB_VIEW_TRANSPARENT if cmd.blur_bg else VIEW_ALIAS.BROWSER_LOBBY_TOP_SUB, browserParams={b'isCloseBtnVisible': (cmd.is_client_close_control)}, forcedSkipEscape=cmd.is_client_close_control)
        return


class OpenBuyGoldWebApiMixin(object):

    @w2c(_OpenBuyGoldOverlaySchema, b'buy_gold')
