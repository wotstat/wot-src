...
_BATTLE_PASS_CHAPTER_STATE_NAME = {(ChapterState.NOT_STARTED): b'not_started', (ChapterState.PAUSED): b'paused', 
   (ChapterState.ACTIVE): b'active', 
   (ChapterState.COMPLETED): b'completed', 
   (ChapterState.DISABLED): b'disabled'}
_BATTLEPASS_CHAPTER_TYPE_NAME = {(BattlePassChapterType.DEFAULT): b'default', 
   (BattlePassChapterType.MARATHON): b'marathon', 
   (BattlePassChapterType.RESOURCE): b'resource'}
...
def formatWalletCurrencyStatuses(stats):
    ...
    statuses[b'free_xp'] = WalletController.STATUS.getKeyByValue(stats.currencyStatuses[Currency.FREE_XP]).lower()
    return statuses
...
def formatBattlePassInfo(battlePass=None):
    return {b'isActive': (not battlePass.isPaused() and battlePass.isVisible()), 
       ...
       b'chapters': {chapterID: {b'isBought': (battlePass.isBought(chapterID=chapterID)), b'state': (_BATTLE_PASS_CHAPTER_STATE_NAME[battlePass.getChapterState(chapterID)]), b'chapterType': (_BATTLEPASS_CHAPTER_TYPE_NAME[BattlePassChapterType(battlePass.getChapterType(chapterID))])} for chapterID in battlePass.getChapterIDs() if not battlePass.isMarathonChapter(chapterID)}}
...
def formatReferralProgramInfo(lobbyContext=None, itemsCache=None):
    ...
    return {b'rp_pgb_points': rpPgbPoints, 
       b'rp_points': rpPoints, 
       b'rp_pgb_limit_points': pgbLimitPoints, 
       b'rp_time_left': timeLeft, 
       b'rp_config_limits': {b'pgbCapacity': (configLimits.pgbCapacity), 
                             b'pgbDayLimit': (configLimits.pgbDayLimit)}, 
       b'rp_passive_income': passiveIncome}
