...
def generateKey(url, appName=None):
    md = hashlib.md5()
    md.update(url)
    if appName == b'server_replays':
        return md.hexdigest() + b'.wotsrvreplay'
    return md.hexdigest()


def createManifestRecord(appName, host, files, code=b'OK', description=b'SUCCESS'):
    return {b'host': host, 
       b'status': {b'code': code, b'description': description}, b'files': (list(files)), 
       b'name': appName}
...
class WebExternalCache(IWebExternalCache):
    ...
    def __init__(self, cacheName, workersLimit=_WORKERS_LIMIT):
        ...
        _logger.info(b'WebDownloader created. Workers: %r', workersLimit)
        self._storage = ApplicationStorage(cacheName, workersLimit)
        _logger.info(b'WebStorage created. Name: %s Workers: %r', cacheName, workersLimit)
    ...
    def close(self):
        ...
        self.stopWorkers()
        self._storage = None
        _logger.info(b'WebStorage destroyed')
    ...
    def __closeDownloader(self):
        if self._downloader:
            ...
            _logger.info(b'WebDownloader destroyed')
        return
    ...
    def get(self, url, appName=None):
        ...
        if key in self._cache:
            ...
            if self._storage.isFileExist(res):
                _logger.debug(b'Resource: %s found in cache as: %s', url, res)
                return res
            self._cache.pop(key)
        _logger.debug(b'Resource %s not found in cache and will be loaded from Web.', url)
        return
    ...
    def getRelativeFromAbsolute(self, absolute):
        if absolute:
            try:
                ...
                _logger.exception(b'Error while getting relative path from: %s, root: %s', absolute, self.rootDirPath)
    ...
    def loadCustomUrls(self, urls, appName):
        ...
        for url in urls:
            ...
            if key not in self._cache or not self._storage.isAppFileExist(appName, key):
                _logger.debug(b'Resource not found in cache. Download from web: %s', url)
        ...
        if filesToDownload:
            ...
            _logger.info(b'There are %r new files to download', len(filesToDownload))
            _logger.info(b'Start downloading...')
            for url, name in viewitems(filesToDownload):
                self._downloader.download(url, partial(self._onResourceLoaded, name))

        else:
            _logger.info(b'There are no new files to download')
            self.prefetchEnd()
        return

    def prefetchStart(self, url):
        self._startTime = BigWorld.timeExact()
        _logger.info(b'Web prefetch started...')
        if url is not None:
            _logger.info(b'Download manifest file: %s', url)
            self._downloader.downloadLowPriority(url, self.__onManifestLoaded)
        else:
            _logger.error(b'Manifest url not found in gui_settings.xml')
            self.prefetchEnd()
        return

    def prefetchEnd(self):
        self.stopWorkers()
        _logger.info(b'Web prefetch finished')
        _logger.info(b'Summary: Downloaded: %r / %r. Stored: %r / %r. Size: %r', self._downloadedCnt, self._prefetchCnt, self._storedCnt, self._prefetchCnt, self._downloadedSize)
        delta = BigWorld.timeExact() - self._startTime
        _logger.info(b'Total prefetch time: %r', delta)
        self.onDownloadFinished()
        return

    def __onManifestLoaded(self, url, data):
        if data is not None:
            _logger.info(b'Manifest downloaded: %s', url)
            try:
            ...
            _logger.error(b'Manifest download error: %s', url)
            self.prefetchEnd()
        return
    ...
    def update(self, manifest):
        ...
        try:
            ...
            for data in manifest:
                appName = data[b'name']
                status = data[b'status']
                _logger.debug(b'Web status for: %s = %s : %s', appName, status[b'code'], status[b'description'])
                host = data[b'host']
        ...
        if self._prefetchCnt > 0:
            _logger.info(b'There are %r new files in manifest', self._prefetchCnt)
            _logger.info(b'Start downloading...')
            for url, name in viewitems(filesToDownload):
                self._downloader.downloadLowPriority(url, partial(self._onResourceLoaded, name))

        else:
            _logger.info(b'There are no new files in manifest')
            self.prefetchEnd()
        return
    ...
    def _onResourceLoaded(self, appName, url, data):
        if data is not None:
            key = generateKey(url, appName=appName)
            _logger.debug(b'Resource downloaded: %s size: %r', url, len(data))
            ...
            _logger.warning(b'Resource download error: %s', url)
    ...
    def _onResourceStored(self, url, key, filename, stored):
        if stored:
            ...
            _logger.debug(b'Resource: %s saved on disk as: %s', url, filename)
            self._storedCnt += 1
        else:
            _logger.error(b'Resource: %s writing error as: %s', url, filename)
...
class BaseExternalCache(WebExternalCache):
    ...
    def prefetchStart(self, callback=None, timeout=0.0):
        if self._state == CacheStates.INITIALIZED:
            ...
            _logger.debug(b'Cache already syncing')
            if callable(callback):
                callback(CachePrefetchResult.BUSY)
        else:
            _logger.error(b'Cache already closed or synced')
            if callable(callback):
    ...
    def decodeConfig(self, url, rawConfig):
        _logger.debug(b'Decoding config: %s.', url)
    ...
    def _onConfigSaved(self, url, key, config, filename, stored):
        if not self.syncing:
            _logger.debug(b'Saved config from: [%s], when stopped or destroyed (%s).', url, self._state)
            return
        if not stored:
            _logger.error(b'Config: %s writing error: %s.', url, filename)
            self._prefetchEnd(CachePrefetchResult.FAIL)
            return
        self._cache[key] = filename
        _logger.debug(b'Config: %s saved on disk as: %s.', url, filename)
        parsedUrl = parse.urlparse(url)
        host = parse.urlunsplit((parsedUrl.scheme, parsedUrl.netloc, b'', b'', b''))
        filePath = parsedUrl.path[1:] if parsedUrl.path.startswith(b'/') else parsedUrl.path
    ...
    def _onConfigLoaded(self, url, rawConfig):
        if not self.syncing:
            _logger.debug(b'Receive config from: [%s], when stopped or destroyed (%s).', url, self._state)
            return
        else:
            if not url or rawConfig is None:
                _logger.error(b'Config [%s] download error.', url)
                self._onManifestDownloadFailed()
                return
            try:
                config = self.decodeConfig(url, rawConfig)
            except Exception:
                _logger.exception(b'Config [%s] decoding error.', url)
                self._onManifestParsingFailed()
                return

            if self._CONFIGS_DIR_NAME:
                if not self._storage or self._storage.stopped:
                    _logger.error(b'Config [%s] save error. Storage not started.', url)
                    self._prefetchEnd(CachePrefetchResult.CLOSED)
                    return
                _logger.debug(b'Saving config [%s] to [%s].', url, self._CONFIGS_DIR_NAME)
                key = generateKey(url)
    ...
    def _prefetchEnd(self, result):
        _logger.debug(b'Stopping sync with result: %s', result)
    ...
    def _onResourceLoaded(self, appName, url, data):
        if not self.syncing:
            _logger.debug(b'Receive resource from: [%s], when stopped or destroyed (%s)', url, self._state)
            return None
        else:
    ...
    def _onResourceStored(self, url, key, filename, stored):
        if not self.syncing:
            _logger.debug(b'Receive save request for: [%s], when stopped or destroyed (%s)', url, self._state)
            return None
        else:
...
class BaseExternalCacheManager(object):
    ...
    def destroy(self):
        ...
        _logger.debug(b'External cache manager destroyed.')
        return

    def load(self):
        if self.isSyncing:
            _logger.debug(b'Can not load. External cache is syncing.')
            return None
        else:
            loaded = self._load()
            if self._cache:
    ...
    def restart(self):
        if self.isSyncing:
            _logger.debug(b'External cache manager restarting.')
    ...
    def get(self, url, relative=True):
        if self._cache is None:
            _logger.error(b'External cache not initialized.')
            return b''
        else:
            if relative:
    ...
    def _sync(self):
        if self.isSyncing:
            _logger.debug(b'External cache already syncing.')
            return
        else:
            self._destroyCache()
            ...
            if not self._cache:
                ...
                return
            _logger.debug(b'External cache initialized.')
            loaded = self._load()
            if loaded is None:
                self._destroyCache()
                self._onResponse(CachePrefetchResult.FAIL)
    ...
    def _destroyCache(self):
        if self._cache is not None:
            self._cache.close()
            _logger.debug(b'External cache instance destroyed.')
            self._cache = None
        return
    ...
    def _sendResult(self, callerID, result):
        ...
        if timerID is not None:
            ...
            _logger.error(b'External cache manager caller wrong timer id.')
        if callable(caller):
            caller(result)
        else:
            _logger.error(b'External cache manager caller %s not callable', caller)
        return
    ...
    def _onResponse(self, result):
        if result == CachePrefetchResult.RESTART:
            _logger.debug(b'External cache manager got restart response.')
            return
        self._sendResultToCallers(result)
        return

    def _onTimeout(self, callerID):
        _logger.debug(b'External cache manager caller (%s) request timeout.', callerID)
        self._sendResult(callerID, CachePrefetchResult.MGR_SYNC_TIMEOUT)
        return

    def _registerCaller(self, caller, timeout):
        if timeout <= 0:
            _logger.error(b'External cache manager wrong sync timeout: %s. Using default: %s.', timeout, self._DEFAULT_SYNC_TIMEOUT)
        ...
        if callable(caller):
            ...
            _logger.debug(b'External cache manager sync caller %s not callable.', caller)
        return
        ...
        if self._cache is not None:
            if not self._cache.syncing:
                try:
                    ...
                    _logger.exception(b'External cache can not load files.')

            else:
                _logger.warning(b'External cache can not load files while syncing.')
        else:
            _logger.warning(b'External cache not initialized.')
        return loaded
    ...
    def _createCacheInstance(self):
        ...
        try:
            ...
            _logger.exception(b'Can not create external cache instance.')
