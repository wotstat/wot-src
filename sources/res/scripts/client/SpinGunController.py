...
def getPlayerVehicleSpinGunController():
    ...
    if vehicle is not None and vehicle.isPlayerVehicle and vehicle.isAlive():
        return vehicle.dynamicComponents.get(b'spinGunController', None)
    else:
        return
...
class SpinGunSpinningAnimator(object):
    ...
    def __updateSpinAnimators(self, isActive, spinFactor, minSpinFactor):
        for animator in self.__spinAnimators:
            ...
            elif not animator.isControlledByClient():
...
class SpinGunController(BigWorld.DynamicScriptComponent):
    ...
    def getSpinningValue(self, stateStatus=None):
        ...
        if stateStatus.state == SpinGunState.SPIN_UP:
            return min(stateStatus.spinFactor + dt * self.__upSpeed, self.__maxValue)
        return max(stateStatus.spinFactor - dt * self.__downSpeed, self.__minValue)
