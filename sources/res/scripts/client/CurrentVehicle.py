...
_MODULES_NAMES = (
 b'turret', b'chassis', b'engine', b'gun', b'radio')
...
class _CachedVehicle(object):
    ...
    def _addListeners(self):
        g_clientUpdateManager.addCallbacks({b'inventory': (self._onInventoryUpdate)})
    ...
    def _changeDone(self):
        ...
        if isPlayerAccount():
            self.onChanged()
        Waiting.hide(b'updateCurrentVehicle')
        return
...
class _CurrentVehicle(_CachedVehicle):
    ...
    def onRotationUpdate(self, diff):
        ...
        if b'groupBattles' in diff:
            isVehicleChanged = self.item.rotationGroupIdx in diff[b'groupBattles']
        elif b'isGroupLocked' in diff:
            isVehicleChanged = self.item.rotationGroupIdx in diff[b'isGroupLocked']
        if isVehicleChanged:
            self.onChanged()
        return
    ...
    def getHangarMessage(self):
        ...
        if state == Vehicle.VEHICLE_STATE.IN_PREMIUM_IGR_ONLY:
            ...
            if self.item.isRented:
                rentLeftStr = getTimeLeftStr(b'#menu:vehicle/igrRentLeft/%s', self.item.rentInfo.getTimeLeft())
                message = i18n.makeString(b'#menu:currentVehicleStatus/' + state, icon=icon, time=rentLeftStr)
            else:
                message = i18n.makeString(b'#menu:tankCarousel/vehicleStates/inPremiumIgrOnly', icon=icon)
            return (
             state, message, stateLvl)
        message = b'#menu:currentVehicleStatus/' + state
    ...
    def _addListeners(self):
        ...
        g_clientUpdateManager.addCallbacks({b'cache.vehsLock': (self.onLocksUpdate), 
           b'groupLocks': (self.onRotationUpdate)})
    ...
    def _selectVehicle(self, vehInvID, callback=None, waitingOverlapsUI=False):
        if vehInvID == self.__vehInvID:
            return
        Waiting.show(b'updateCurrentVehicle', isSingle=True, overlapsUI=waitingOverlapsUI)
    ...
    def _onInventoryUpdate(self, invDiff):
        ...
        if b'compDescr' in vehsDiff and self.__vehInvID in vehsDiff[b'compDescr']:
            isVehicleSold = vehsDiff[b'compDescr'][self.__vehInvID] is None
            isVehicleDescrChanged = not isVehicleSold
        if isVehicleSold or self.__vehInvID == 0:
            self.selectVehicle()
        else:
            isRepaired = b'repair' in vehsDiff and self.__vehInvID in vehsDiff[b'repair']
            ...
            isVehicleChanged = any(self.__vehInvID in hive or (self.__vehInvID, b'_r') in hive for hive in vehsDiff.itervalues())
        ...
        if clientPrb is not None:
            ...
            for _, roster in rosters.iteritems():
                if BigWorld.player().id in roster:
                    vehCompDescr = roster[BigWorld.player().id].get(b'vehCompDescr', b'')
                    if vehCompDescr:
    ...
    def __repr__(self):
        return b'CurrentVehicle(%s)' % str(self.item)
...
class _CurrentPreviewVehicle(_CachedVehicle):
    ...
    def installComponent(self, newId):
        ...
        Waiting.show(b'applyModule', overlapsUI=False)
    ...
    def _selectVehicle(self, vehicleCD, vehicleStrCD=None, style=None, outfit=None):
        if self.isPresent() and self.item.intCD == vehicleCD:
            ...
            Waiting.show(b'updateCurrentVehicle', isSingle=True, overlapsUI=False)
            self.onChangeStarted()
