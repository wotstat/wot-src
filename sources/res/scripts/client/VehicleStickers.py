...
_TextureParams = namedtuple(b'TextureParams', (b'textureName', b'bumpTextureName', b'mirror'))
_CounterParams = namedtuple(b'CounterParams', (b'atlas', b'alphabet', b'mirror'))
_InsigniaParams = namedtuple(b'InsigniaParams', (b'textureName', b'emissionTextureName', b'emissionPatternName', b'emissionAnimationSpeed', b'emissionHDRBrightness', b'emissionLDRBrightness'))
_StickerSlotPair = namedtuple(b'_StickerSlotPair', (b'componentSlot', b'stickerParams'))
...
class ClanStickerPack(StickerPack):
    ...
    def bind(self, componentIdx, componentSlot):
        self._data[componentIdx].append(_StickerSlotPair(componentSlot, _TextureParams(b'', b'', False)))
        return
...
class InsigniaStickerPack(StickerPack):
    ...
    def bind(self, componentIdx, componentSlot):
        ...
        if componentIdx in Insignia.Indexes.ALL:
            if IS_EDITOR:
                defaultParams = _InsigniaParams(b'', b'', b'', 0, 0, 0)
    ...
    def _changeRankInTextureName(self, textureName):
        if textureName == b'':
            return b''
        constantPart, delimeterPart, changeablePart = textureName.rpartition(b'_')
        _, dotPart, extenstionPart = changeablePart.partition(b'.')
    ...
    def _convertToInsignia(self, item):
        ...
        emissionTextureName = self._changeRankInTextureName(item.emissionSettings[b'emissionMap'])
        emissionPatternName = self._changeRankInTextureName(item.emissionSettings[b'emissionPatternMap'])
        return _InsigniaParams(textureName, emissionTextureName, emissionPatternName, item.emissionSettings[b'emissionAnimationSpeed'], item.emissionSettings[b'deferredEmissionBrightness'], item.emissionSettings[b'forwardEmissionBrightness'])
...
class VehicleStickers(object):
    ...
    def _createComponentSlots(cls, vehicleDesc, showEmblemsOnGun, outfit):
        ...
        componentSlots = (
         (
          TankPartNames.HULL, vehicleDesc.hull.emblemSlots),
         (
          TankPartNames.GUN if showEmblemsOnGun else TankPartNames.TURRET, vehicleDesc.turret.emblemSlots),
         (
          TankPartNames.TURRET if showEmblemsOnGun else TankPartNames.GUN, [slot for slot in vehicleDesc.gun.emblemSlots if slot.type != b'insigniaOnGun']))
