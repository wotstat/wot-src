...
class TestMaterialParamManipulator(object):
    ...
    paramName = ComponentProperty(type=CGFMetaTypes.STRING, editorName=b'paramName', value=b'g_tintColor')
...
class HierarchyModifier(object):
    category = DEMO_CATEGORY
    domain = CGF.DomainOption.DomainClient
    gameObject1 = ComponentProperty(type=CGFMetaTypes.STRING, editorName=b'Game Object 1', value=b'GameObject1')
    gameObject2 = ComponentProperty(type=CGFMetaTypes.STRING, editorName=b'Game Object 2', value=b'GameObject2')
    gameObject3 = ComponentProperty(type=CGFMetaTypes.STRING, editorName=b'Game Object 3', value=b'GameObject3')
    areaTrigger1 = ComponentProperty(type=CGFMetaTypes.LINK, editorName=b'Area Trigger1', value=Triggers.AreaTriggerComponent)
    areaTrigger2 = ComponentProperty(type=CGFMetaTypes.LINK, editorName=b'Area Trigger2', value=Triggers.AreaTriggerComponent)
...
class HierarchyModifier2(object):
    category = DEMO_CATEGORY
    domain = CGF.DomainOption.DomainClient
    top = ComponentProperty(type=CGFMetaTypes.LINK, editorName=b'Top Object', value=CGF.GameObject)
    bottom = ComponentProperty(type=CGFMetaTypes.LINK, editorName=b'Bottom Object', value=CGF.GameObject)
    box = ComponentProperty(type=CGFMetaTypes.LINK, editorName=b'Box Object', value=CGF.GameObject)
    cameraTransform = ComponentProperty(type=CGFMetaTypes.LINK, editorName=b'Camera Transform', value=GenericComponents.TransformComponent)
    areaTrigger = ComponentProperty(type=CGFMetaTypes.LINK, editorName=b'Area Trigger', value=Triggers.AreaTriggerComponent)

    def unwrapFigure(self, where):
        _logger.debug(b'HierarchyModifier2. Trigger entered')
        CGF.changeHierarchy(where, b'Head', self.top.name)
        CGF.changeHierarchy(where, b'Back', self.bottom.name)
        CGF.changeHierarchy(where, b'Camera', self.top.name)
        CGF.changeHierarchy(where, b'LH', b'TL')
        CGF.changeHierarchy(where, b'RH', b'TR')
        CGF.changeHierarchy(where, b'LL', b'BL')
        CGF.changeHierarchy(where, b'RL', b'BR')
        return

    def wrapFigure(self, where):
        _logger.debug(b'HierarchyModifier2. Trigger exited')
        CGF.changeHierarchy(where, b'Head', self.box.name)
        CGF.changeHierarchy(where, b'Back', self.box.name)
        CGF.changeHierarchy(where, b'Camera', b'Bot')
        CGF.changeHierarchy(where, b'LH', self.box.name)
        CGF.changeHierarchy(where, b'RH', self.box.name)
        CGF.changeHierarchy(where, b'LL', self.box.name)
        CGF.changeHierarchy(where, b'RL', self.box.name)
        return
...
class TestModelSwapper(object):
    category = DEMO_CATEGORY
    domain = CGF.DomainOption.DomainClient
    model1 = ComponentProperty(type=CGFMetaTypes.STRING, value=b'', editorName=b'model1', annotations={b'path': b'*.model'})
    model2 = ComponentProperty(type=CGFMetaTypes.STRING, value=b'', editorName=b'model2', annotations={b'path': b'*.model'})
