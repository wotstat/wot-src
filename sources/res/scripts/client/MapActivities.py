...
class BaseMapActivity(object):
    ...
    def create(self, settings):
        ...
        self._arenaPeriod = settings.readInt(b'arenaPeriod', 0)
        return
    ...
    def _readInterval(self):
        self._interval = self._settings.readFloat(b'interval', 0.0)
        return

    def _readName(self):
        self._name = self._settings.readString(b'name', b'')
        return
...
class MapActivities(IMapActivities):
    ...
    def generateOfflineActivities(self, spacePath, usePossibility=True):
        xmlName = spacePath.split(b'/')[-1]
        settings = ResMgr.openSection(b'scripts/arena_defs/' + xmlName + b'.xml/mapActivities')
        ...
        for activityXML in settings.values():
            timeframe = activityXML.readVector2(b'startTime')
...
class WarplaneActivity(BaseMapActivity):
    ...
    def create(self, settings):
        ...
        self.__possibility = self._settings.readFloat(b'possibility', 1.0)
        self.clampStartTime()
        self.__firstLaunch = True
        self.__curve = BigWorld.WGActionCurve(self._settings)
        self.__modelName = self.__curve.getChannelProperty(0, b'modelName').asString
        ds = self.__curve.getChannelProperty(0, b'wwsoundName')
        self.__soundName = ds.asString if ds is not None else b''
    ...
    def setStartTime(self, startTime):
        if not self.__firstLaunch:
            self.pause()
        timeFrame = self._settings.readVector2(b'startTime')
    ...
    def __onModelLoaded(self, resourceRefs):
        if self.__modelName not in resourceRefs.failedIDs and not self.__isStopped:
            ...
            LOG_ERROR(b'Could not load model %s' % self.__modelName)
        return
    ...
    def __loadEffects(self):
        if self.__particle[0] is None and self.__particle[1] is None:
            if self.__curve is None:
                return
            propValue = self.__curve.getChannelProperty(0, b'effectHardpoint')
            hardPointName = propValue.asString if propValue is not None else b''
            if hardPointName == b'':
                return
            ds = self.__curve.getChannelProperty(0, b'effectName')
            effectName = ds.asString if ds is not None else b''
            if effectName != b'':
...
class ExplosionActivity(BaseMapActivity):
    ...
    def create(self, settings):
        ...
        self._readInterval()
        self.__possibility = self._settings.readFloat(b'possibility', 1.0)
        self.__position = self._settings.readVector3(b'position', (0.0, 0.0, 0.0))
        curveSettings = BigWorld.WGActionCurve(self._settings)
        self.__soundName = None
        self.__soundName = curveSettings.getChannelProperty(0, b'wwsoundName')
        if self.__soundName is not None:
            self.__soundName = self.__soundName.asString
        else:
            self.__soundName = b''
        waveImpulseDs = curveSettings.getChannelProperty(0, b'waveImpulse')
        if waveImpulseDs is not None:
            startImpulse = waveImpulseDs.readFloat(b'start', 0.0)
            endImpulse = waveImpulseDs.readFloat(b'end', 0.0)
            count = waveImpulseDs.readInt(b'count', 0) - 1
            time = waveImpulseDs.readFloat(b'time', 0.0)
        ...
        self.clampStartTime()
        self.__firstLaunch = True
        self.__modelName = curveSettings.getChannelProperty(0, b'modelName').asString
        BigWorld.loadResourceListBG((self.__modelName,), makeCallbackWeak(self.__onModelLoaded))
        self.__isOver = True
        return True
...
class ScenarioActivity(BaseMapActivity):
    ...
    def create(self, settings):
        BaseMapActivity.create(self, settings)
        trajectories = settings[b'trajectories']
...
def _createActivity(typeName):
    if typeName == b'warplane':
        return WarplaneActivity()
    else:
        if typeName == b'scenario':
            return ScenarioActivity()
        if typeName == b'explosion':
