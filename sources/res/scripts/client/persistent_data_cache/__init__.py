...
PDC_CREATE_ARG = b'pdcCreate'
PDC_OFF_ARG = b'pdcOff'
PDC_CACHE_FILE_PATH = b'./data.wgpdc'
...
def init(scriptsConfig, engineConfig, userPreferences, gameLoadingStep):
    ...
    if not clientVersion:
        _logger.error(b'Disabled. Broken client version.')
        if forceCreating:
    ...
    if not forceCreating:
        if not settings.enabled or constants.IS_DEVELOPMENT and not settings.devEnabled:
            _logger.info(b'Disabled in config.')
            return
        if hasPDCOffParam():
            _logger.info(b'Disabled by the script argument.')
            return
    elif hasPDCOffParam():
        _logger.error(b"Can't use 'pdcCreate' and 'pdcOff' arguments simultaneously!")
        BigWorld.quitWithExitCode(ExitCode.FAILED)
        return
    _logger.debug(b'Settings: %s', settings)
    ...
    if not forceCreating:
        ...
        if faultTolerance.isLimitsReached():
            _logger.error(b'Disabled. Fault tolerance limits reached.')
            return
