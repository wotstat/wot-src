...
HINT_SHOWN_STATUS = 1
_MAX_ACTIVE_HINTS = 1
_ONCE_ONLY_HINTS_PATH = (b'{0:>s}/once-only-hints.xml').format(settings.DOC_DIRECTORY)
_DYNAMIC_HINTS_PATH = (b'{0:>s}/dynamic-hints.xml').format(settings.DOC_DIRECTORY)

class HintsManager(object):
    __settingsCore = dependency.descriptor(ISettingsCore)
    __tutorialLoader = dependency.descriptor(ITutorialLoader)
    __slots__ = (b'_data', b'_gui', b'_dynamicHintsData', b'__activeHints', b'__postponedHints', b'__hintsWithClientTriggers')
    ...
    def __showHint(self, hint, silent=False):
        if len(self.__activeHints) >= _MAX_ACTIVE_HINTS:
            ...
            return
        text = hint[b'text']
        uniqueID = hintID = hint[b'hintID']
        props = HintProps(uniqueID, hintID, hint[b'itemID'], text, hasBox=hint[b'hasBox'], arrow=hint[b'arrow'], padding=None, updateRuntime=hint[b'updateRuntime'], hideImmediately=hint[b'hideImmediately'], checkViewArea=hint[b'checkViewArea'])
        actionType = hint.get(b'ignoreOutsideClick')
        isActive = self._gui.showHint(props, actionType, silent)
        if isActive:
            self.__activeHints[hint[b'itemID']] = hint
        return

    def __hideHint(self, itemID, hintID=b''):
    ...
    def __onItemLost(self, itemID):
        if itemID in self.__activeHints:
            ...
            if hint[b'equalActions']:
                self.__stopOnceOnlyHint(itemID, hint[b'hintID'])
            self.__hideHint(itemID)
        return
    ...
    def __getActiveHintIdByItemId(self, itemID):
        return self.__activeHints[itemID][b'hintID']
    ...
    def __stopOnceOnlyHint(self, itemID, hintID):
        if self._data is None:
            ...
            hints = self._data.hintsForItem(itemID)
            for hint in hints:
                if hint[b'hintID'] == hintID and hint[b'itemID'] not in self._dynamicHintsData.getHints():
                    self.__stopSettingsListening()
    ...
    def __onUpdateTutorialHints(self, targetID, state, arguments=b''):
        filteredHints = []
        for itemID, hints in self._data.getHints().iteritems():
            for hint in hints:
                if self.__filterHintByConditions(hint.get(b'conditions'), self.__limitedUIFilter, targetID, arguments):
    ...
    @staticmethod
    def __limitedUIFilter(condition, targetID, arguments=b''):
