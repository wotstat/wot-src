...
class HasTargetEffect(HasTargetID):
    ...
    def __repr__(self):
        return (b'HasTargetEffect(type = {0!r:s}, targetID = {1:>s})').format(EFFECT_TYPE_NAMES[self.getType()], self.getTargetID())
...
class SimpleEffect(HasTargetEffect):
    ...
    def __repr__(self):
        return (b'SimpleEffect(type = {0!r:s})').format(EFFECT_TYPE_NAMES[self.getType()])
