...
class _StateValue(object):
    ...
    def __repr__(self):
        return (b'_StateValue({})').format(self.value)
...
class CONDITION_STATE(object):
    ...
    RANGE = (
     ...
     LEVEL_RANGE, ALL_EQUIPMENT_INSTALLED, HAS_MULTIPLIED_XP)
    GAME_ITEM_SIMPLE_STATE = (
     SELECTED, PREMIUM, UNLOCKED, IN_INVENTORY, MONEY_ENOUGH, HAS_REGULAR_CONSUMABLES,
     ALL_EQUIPMENT_INSTALLED, HAS_OPTIONAL_DEVICES, HAS_MULTIPLIED_XP)
    GAME_ITEM_RELATE_STATE = (
     XP_ENOUGH, LEVEL, LEVEL_RANGE, MAY_INSTALL, INSTALLED, CREW_HAS_ANY_SKILL)
...
class ViewPresentCondition(ActiveCondition):
    ...
    def __init__(self, layer, viewAlias, state=CONDITION_STATE.ACTIVE):
        super(ViewPresentCondition, self).__init__((b'{}_{}').format(layer, viewAlias), CONDITION_TYPE.VIEW_PRESENT, state)
        self.__layer = layer
...
class Conditions(list):
    ...
    def __repr__(self):
        return (b'Conditions({0:s}): {1!r:s}, {2!r:s}').format(hex(id(self)), self[:], self._eitherBlocks)
