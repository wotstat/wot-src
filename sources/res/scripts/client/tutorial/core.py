...
class Tutorial(object):
    ...
    def run(self, dispatcher, ctx):
        ...
        if self._cache is None:
            ...
            if not self._stopped:
                LOG_ERROR(b'Tutorial is already running.')
                return False
            ...
            if not self._gui.init():
                ...
                LOG_ERROR(b'GUI can not init. Tutorial is stopping.')
                return False
            LOG_DEBUG(b'Start training', self.getID())
            self._stopped = False
    ...
    def stop(self, finished=False):
        if self._stopped:
            ...
            LOG_DEBUG(b'Tutorial is stopped', self.getID())
            self.onStopped()
            return
    ...
    def reload(self):
        if not self._stopped:
            ...
            LOG_ERROR(b'Tutorial is not running.')
        return

    def loadCurrentChapter(self, initial=False):
        LOG_DEBUG(b'Chapter is loading', self._currentChapter)
        self._gui.showWaiting(b'chapter-loading', isSingle=True)
        if self._data is not None:
            self._data.clear()
        ...
        if self._data is None:
            LOG_ERROR(b'Chapter documentation is not valid. Tutorial is stopping', self._currentChapter)
            self._gui.hideWaiting(b'chapter-loading')
            self.stop()
            return
        else:
            ...
            self._gui.lock()
            self._gui.hideWaiting(b'chapter-loading')
            LOG_DEBUG(b'Initializing tutorial chapter with state TutorialStateNextScene', self._nextScene.getID())
        ...
        if state is not None:
            if not isinstance(self._currentState, state):
                if self._currentState is not None:
                    self._currentState.destroy()
                    self._currentState = None
                LOG_DEBUG(b'Set new state', state.__name__)
            ...
            LOG_ERROR(b'Can not sets current state', stateID)
        return
    ...
    def storeEffectsInQueue(self, effects, benefit=False, isGlobal=False):
        if not effects:
            LOG_ERROR(b'Effect list is not defined')
        else:
        ...
        if ctx.initialChapter is not None:
            ...
            if fromCache is not None and self._settings.findChapterInCache:
                ...
                if self._descriptor.isChapterInitial(chapter_, completed):
                    ...
                    LOG_WARNING(b'Initial chapter not found in cache or bonuses is invalid', fromCache, chapter_.getBonusID() if chapter_ is not None else None, completed)
            if self._currentChapter is None:
                self._currentChapter = self._descriptor.getInitialChapterID(completed=completed)
            if self._currentChapter is None:
                LOG_ERROR(b'Initial chapter not found. Tutorial is stopping')
                self.stop()
                return False
            return True
    ...
    def __onPageChanging(self, sceneID):
        if self._data.isInScene(self._nextScene, sceneID):
            ...
            if scene is None:
                LOG_WARNING((b'Scene {0:>s} not found in chapter {1:>s}').format(sceneID, self._currentChapter))
                return
    ...
    def __onItemFound(self, itemID):
        ...
        LOG_DEBUG(b'invalidateFlags from __onItemFound', itemID)
    ...
    def __onItemLost(self, itemID):
        ...
        LOG_DEBUG(b'invalidateFlags from __onItemLost', itemID)
        self.invalidateFlags()
        return

    def __onViewLoaded(self, _):
        LOG_DEBUG(b'invalidateFlags from __onViewLoaded')
        self.invalidateFlags()
        return

    def __onViewDisposed(self, _):
        LOG_DEBUG(b'invalidateFlags from __onViewDisposed')
        self.invalidateFlags()
        return
