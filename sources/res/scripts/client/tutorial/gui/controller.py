...
_COMPONENT_PROPERTY_EFFECTS = {(_EFFECT_TYPES.DISPLAY): (b'visible',), 
   (_EFFECT_TYPES.ENABLED): (b'enabled',), 
   (_EFFECT_TYPES.LAYOUT): (b'layout',)}
_R_VIEWS_PREFIX = b'R.views.'
...
def _parseRView(view):
    ...
    return reduce(_reducer, view[len(_R_VIEWS_PREFIX):].split(b'.'), R.views)
...
def _getComponentDescr(componentID, cfgItem):
    if _isRView(cfgItem.view):
        ...
        if not viewId.isValid():
            _logger.error(b"Can't parse component's view. componentID: %s, viewId: %s", componentID, cfgItem.view)
        viewId = str(viewId())
    else:
...
class _ComponentViewBinding(object):
    __slots__ = (b'alias', b'desiredUniqueName', b'actualUniqueName')
...
class GuiController(IGuiController):
    __slots__ = (b'__guiImpls', b'_isEnabled', b'_componentViewBindings', b'_components', b'_componentProps', b'_pendingComponentAnimations', b'__hintsWithClientTriggers', b'_config', b'__hangarMenuButtonsOverride', b'__headerMenuButtonsOverride', b'__hangarHeaderEnabled', b'__battleSelectorHintOverride', b'__onComponentFoundHandlers', b'__descriptions')
    ...
    def setTriggers(self, componentID, triggers):
        ...
        if componentID not in self._components:
            _logger.error(b'setTriggers: component is not on scene!: %r', componentID)
            return
    ...
    def showInteractiveHint(self, componentID, content, triggers=None, silent=False):
        if not self._validate(componentID):
            ...
            if componentID not in self._components:
                if not silent:
                    _logger.error(b'showInteractiveHint - target component is not on scene!: %r', componentID)
                return False
            if b'padding' not in content:
                content[b'padding'] = self._config.getItem(componentID).padding
    ...
    def closeInteractiveHint(self, componentID):
        ...
        if componentID in self._components:
            ...
            _logger.debug(b"closeInteractiveHint: can't find component: %s", componentID)
        return
    ...
    def playComponentAnimation(self, componentID, animType):
        if not self._validate(componentID):
            ...
            if animParams is None:
                _logger.error(b'invalid animation type: %r', animType)
                return False
            if componentID in self._components:
                self.__doShowEffect(componentID, animType, animParams)
            elif componentID not in self._pendingComponentAnimations:
                _logger.debug(b'deferring playComponentAnimation (component not yet on scene): %r, %r, %r', componentID, animType, animParams)
                self._pendingComponentAnimations[componentID] = (animType, animParams)
            else:
                _logger.error(b'playComponentAnimation call already deferred - ignoring another one: %r, %r, %r', componentID, animType, animParams)
            return True
    ...
    def stopComponentAnimation(self, componentID, animType):
        if not self._validate(componentID):
            ...
            _logger.debug(b'stopComponentAnimation: %r, %r', componentID, animType)
            if componentID in self._components:
                ...
                if deferredAnim is not None:
                    ...
                    if deferredAnimType == animType:
                        _logger.debug(b'canceling deferred playComponentAnimation call: %r, %r', componentID, self._pendingComponentAnimations[componentID])
                        del self._pendingComponentAnimations[componentID]
                    else:
                        _logger.error(b'last deferred playComponentAnimation call had a different type: %r, %r, %r', componentID, animType, deferredAnimType)
            return
    ...
    def showBootcampHint(self, componentID):
        if not self._validate(componentID):
            return False
        if componentID not in self._components:
            _logger.error(b'showBootcampHint - target component is not on scene!: %r', componentID)
            return False
        params = self._config.getItem(componentID).bootcampHint
        _logger.debug(b'showBootcampHint: %r, %r', componentID, params)
    ...
    def hideBootcampHint(self, componentID):
        if not self._validate(componentID):
            return
        if componentID not in self._components:
            _logger.error(b"hideBootcampHint: can't find component %s", componentID)
            return
        _logger.debug(b'hideBootcampHint: %r', componentID)
    ...
    def setupViewContextHints(self, viewTutorialID, hintsData, hintsArgs=None):
        ...
        for hint in hintsDataCopy.get(b'hints', []):
            hintArgs = hintsArgs.get(hint.get(b'tooltipSpecial'))
            if hintArgs is not None:
                hint[b'args'] = hintArgs

        builder = hintsDataCopy.pop(b'builderLnk', b'')
        effectType = _EFFECT_TYPES.DEFAULT_OVERLAY
        for gui in self.__guiImpls:
            gui.showEffect(b'', viewTutorialID, effectType, hintsDataCopy, builder)
    ...
    def setup(self, isEnabled=False, path=b''):
        _logger.debug(b'setup: %r %r', isEnabled, path)
    ...
    def _validate(self, componentID):
        ...
        if component is None:
            _logger.error(b'Component is not found: %r', componentID)
            return False
        else:
            return True
    ...
    def _getGui(self, componentID):
        if componentID not in self._components:
            raise SoftException((b"Can't find component id: {}").format(componentID))
        return self._components[componentID]

    def __onComponentFound(self, gui, componentID, viewTutorialID):
        _logger.debug(b'onComponentFound: %r, %r', componentID, viewTutorialID)
        ...
        if bindings.desiredUniqueName is not None and bindings.desiredUniqueName != viewTutorialID:
            _logger.error(b'ignoring onComponentFound in unexpected view instance - expected: %r', bindings.desiredUniqueName)
            return
        else:
    ...
    def __onComponentDisposed(self, componentID):
        ...
        _logger.debug(b'onComponentDisposed: %r %s', componentID, b'' if found else b'(not found)')
    ...
    def __onEffectCompleted(self, componentID, effectType):
        _logger.debug(b'onEffectCompleted: %r, %r', componentID, effectType)
    ...
    def __setTriggers(self, componentID, triggers):
        ...
        if gui:
            ...
            _logger.info(b'clearTriggers: component is not on scene: %r', componentID)
            for gui in self.__guiImpls:
                gui.setTriggers(componentID, triggers)
    ...
    def __doShowEffect(self, componentID, effectType, effectData):
        ...
        effectBuilder = self._config.getItem(componentID).effectBuilders.get(effectType, b'')
        gui = self._getGui(componentID)
        _logger.debug(b'showEffect: %r %r %r %r %r', componentID, viewTutorialID, effectType, effectData, effectBuilder)
    ...
    def __doHideEffect(self, componentID, effectType):
        _logger.debug(b'hideEffect: %s %s', componentID, effectType)
        viewTutorialID = self.__getComponentViewID(componentID)
        effectBuilder = self._config.getItem(componentID).effectBuilders.get(effectType, b'')
        gui = self._getGui(componentID)
    ...
    def __doSetComponentProps(self, componentID, props):
        ...
        if props:
            _logger.error(b'__doSetComponentProps: unsupported properties: %r, %r', componentID, props)
        return
