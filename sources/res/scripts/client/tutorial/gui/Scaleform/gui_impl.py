...
class ScaleformGuiImpl(IGuiImpl):
    ...
    def showEffect(self, componentID, viewID, effectType, effectData, effectBuilder=b''):
        self.__proxy.as_showEffectS(viewID, componentID, effectType, {b'data': effectData, b'builder': effectBuilder})
        return
    ...
    def setDescriptions(self, descriptions):
        ...
        for descr in descriptions:
            descrs[descr.viewId].append({b'id': (descr.ID), 
               b'viewName': (descr.viewId), 
               b'path': (descr.path)})
