...
class SfLobbyDispatcher(GUIDispatcher):
    ...
    def __del__(self):
        LOG_MEMORY(b'SfLobbyDispatcher deleted')
        return
    ...
    def __handleStartTraining(self, event):
        if not event.settingsID:
            LOG_ERROR(b'Name of tutorial is not defined', event.settingsID)
            return
        Waiting.show(b'tutorial-chapter-loading', isSingle=True)
        self.startTraining(event.settingsID, event.getState())
        Waiting.hide(b'tutorial-chapter-loading')
        return
