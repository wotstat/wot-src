...
_EventClassByTriggerType = {(TUTORIAL_TRIGGER_TYPES.CLICK_TYPE): (
                                       ClickEvent, b'Player has clicked'), 
   (TUTORIAL_TRIGGER_TYPES.CLICK_OUTSIDE_TYPE): (
                                               ClickOutsideEvent, b'Player has clicked outside'), 
   (TUTORIAL_TRIGGER_TYPES.ESCAPE): (
                                   EscEvent, b'Player has pressed ESC'), 
   (TUTORIAL_TRIGGER_TYPES.ENABLED): (
                                    EnableEvent, b'Button has been enabled'), 
   (TUTORIAL_TRIGGER_TYPES.DISABLED): (
                                     DisableEvent, b'Button has been disabled'), 
   (TUTORIAL_TRIGGER_TYPES.VISIBLE_CHANGE): (
                                           VisibleChangeEvent, b'Component visibility has been changed'), 
   (TUTORIAL_TRIGGER_TYPES.ENABLED_CHANGE): (
                                           EnabledChangeEvent, b'Component enables has been changed')}
...
class SfLobbyProxy(GUIProxy):
    ...
    def getViewSettings(self):
        raise SoftException(b'Routine getViewSettings must be implemented')
        return

    def getViewsAliases(self):
        raise SoftException(b'Routine getViewsAliases must be implemented')
        return
    ...
    def getSceneID(self):
        ...
        LOG_DEBUG(b'GUI.getSceneID', sceneID)
        return sceneID
    ...
    def showWaiting(self, messageID, isSingle=False):
        Waiting.show((b'tutorial-{0:>s}').format(messageID), isSingle=isSingle)
        return

    def hideWaiting(self, messageID=None):
        if messageID is not None:
            Waiting.hide((b'tutorial-{0:>s}').format(messageID))
        else:
            Waiting.close()
        return
    ...
    def showI18nMessage(self, key, *args, **kwargs):
        guiType = None
        if b'msgType' in kwargs:
            guiType = SystemMessages.SM_TYPE.lookup(kwargs[b'msgType'])
        if guiType is None:
            guiType = SystemMessages.SM_TYPE.Information
        kwargs[b'type'] = guiType
    ...
    def showServiceMessage(self, data, msgTypeName):
        ...
        if msgType is None:
            LOG_ERROR(b'Message type not found', msgType)
        if self.proto:
    ...
    def __onViewLoadInit(self, pyEntity):
        if pyEntity.layer == WindowLayer.SUB_VIEW:
            ...
            LOG_DEBUG(b'GUI.onPageChanging', prevSceneID, b'->', sceneID)
            if prevSceneID is not None:
                self.effects.cancel(GUIEffectScope.SCENE, prevSceneID)
            if sceneID is None:
                self.clear()
                LOG_WARNING(b'Scene alias not found, page:', pageName)
            else:
    ...
    def __onViewLoaded(self, pyEntity, _):
        if pyEntity.layer == WindowLayer.SUB_VIEW:
            pageName = pyEntity.alias
            sceneID = self.config.getSceneID(pageName)
            LOG_DEBUG(b'GUI.onPageReady', sceneID)
    ...
    def __onComponentFound(self, event):
        if not event.targetID:
            LOG_ERROR(b'Key targetID is not defined in the event ON_COMPONENT_FOUND')
            return
    ...
    def __onComponentLost(self, event):
        if not event.targetID:
            LOG_ERROR(b'Key targetID is not defined in the event ON_COMPONENT_LOST')
            return
    ...
    def __onTriggerActivated(self, event):
        if not event.targetID:
            LOG_ERROR(b'Key targetID is not defined in the event ON_TRIGGER_ACTIVATED')
            return
        else:
            if not event.settingsID:
                LOG_ERROR(b'Key settingsID is not defined in the event ON_TRIGGER_ACTIVATED')
                return
            ...
            if eventClass is not None:
                ...
                LOG_ERROR(b'Type of event is not supported', triggerType)
            return

    def __onAnimationComplete(self, event):
        if not event.targetID:
            LOG_ERROR(b'Key targetID is not defined in event ON_ANIMATION_COMPLETE')
            return
        if not event.settingsID:
            LOG_ERROR(b'Key settingsID is not defined in event ON_ANIMATION_COMPLETE')
            return
