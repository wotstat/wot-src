...
class _Flag(object):
    ...
    def __repr__(self):
        return (b'{0:>s}: {1!r:s}').format(self.name, self.active)
...
class FlagSummary(object):
    ...
    def activateFlag(self, flagName):
        LOG_DEBUG(b'Activate flag: ', flagName)
        if flagName in self.__flags:
...
class VarSummary(object):
    ...
    def set(self, varID, value):
        if varID in self.__varSets:
            LOG_ERROR((b'Var {0:>s} in not mutable.').format(varID))
        else:
            LOG_DEBUG((b'Set var {0:>s}').format(varID), value)
