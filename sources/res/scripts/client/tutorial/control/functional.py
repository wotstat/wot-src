...
class FunctionalConnectedItemCondition(FunctionalCondition):
    ...
    def isConditionOk(self, condition):
        ...
        if isHintShown is None:
            LOG_DEBUG(b'invalid hintID in condition: ', condition.getID())
            return False
        else:
    ...
    def isConditionOk(self, condition):
        ...
        if getter:
            ...
            try:
                ...
                LOG_ERROR(b'Can not resolve condition', varID, e.message)
                return False

            if condition.isPositiveState():
                return result
            return not result
        LOG_ERROR(b'State of item can not be resolved', condition)
        return False
...
class FunctionalServiceCondition(FunctionalCondition):
    ...
    def isConditionOk(self, condition):
        ...
        if serviceClass is None:
            LOG_ERROR(b'Service cannot be loaded!', condition.getID(), condition.getPath())
            return False
        else:
            service = dependency.instance(serviceClass)
            if not hasattr(service, b'isEnabled'):
                LOG_ERROR(b'Service does not implement isEnabled method!', service)
                return False
...
class FunctionalClassCondition(FunctionalCondition):
    ...
    def isConditionOk(self, condition):
        ...
        if conditionClass is None:
            LOG_ERROR(b'Condition cannot be loaded!', condition.getID(), condition.getPath())
            return False
        else:
            conditionInstance = conditionClass()
            if not hasattr(conditionInstance, b'check'):
                LOG_ERROR(b'Condition does not implement check method!', conditionInstance)
                return False
    ...
    def isConditionActive(self, condition):
        ...
        if functional is None:
            ...
            if condType in _SUPPORTED_CONDITIONS:
                ...
                LOG_ERROR(b'Condition is not found', condType)
...
class FunctionalEffect(TutorialProxyHolder):
    ...
    def triggerEffect(self):
        raise NotImplementedError(b'method triggerEffect is not implemented')
        return
...
class FunctionalActivateEffect(FunctionalEffect):
    ...
    def triggerEffect(self):
        ...
        if not flags.isActiveFlag(targetID):
            ...
            LOG_DEBUG(b'invalidateFlags from FunctionalActivateEffect', targetID)
...
class FunctionalDeactivateEffect(FunctionalEffect):
    ...
    def triggerEffect(self):
        ...
        if flags.isActiveFlag(targetID):
            ...
            LOG_DEBUG(b'invalidateFlags from FunctionalDeactivateEffect', targetID)
            self._tutorial.invalidateFlags()
        return True
...
class FunctionalRunTriggerEffect(FunctionalEffect):
    ...
    def triggerEffect(self):
        ...
        if trigger is not None:
            ...
            LOG_ERROR(b'Trigger not found', self._effect.getTargetID())
            return False
        ...
        if command is not None:
            ...
            if argOverrides:
                if isinstance(command.args, dict):
                    ...
                    LOG_ERROR(b'cannot override GUI command args by name: arg list was not defined as dict', targetID)
            self._gui.invokeCommand(command)
            return True
        else:
            LOG_ERROR(b'Command not found', targetID)
            return False
    ...
    def triggerEffect(self):
        ...
        if dialog is not None:
            ...
            if not self._isRunning:
                LOG_ERROR(b'Can not play effect "ShowDialog"', dialog.getID(), dialog.getType())
        else:
            LOG_ERROR(b'Dialog not found', self._effect.getTargetID())
            self._isRunning = False
...
class FunctionalShowWindowEffect(FunctionalEffect):
    ...
    def triggerEffect(self):
        ...
        if window is not None:
            ...
            if not isRunning:
                LOG_ERROR(b'Can not play effect "ShowWindow"', window.getID(), window.getType())
        else:
            LOG_ERROR(b'PopUp not found', self._effect.getTargetID())
        return isRunning
...
class FunctionalSetGuiItemCriteria(FunctionalEffect):
    ...
    def triggerEffect(self):
        ...
        if criteria is None:
            LOG_ERROR(b'Criteria is not found', self._effect.getTargetID())
            return False
        else:
    ...
    def _playEffect(self, criteria, value):
        LOG_DEBUG(b'Set gui item criteria', criteria.getTargetID(), value)
...
class FunctionalSetGuiItemViewCriteria(FunctionalSetGuiItemCriteria):
    ...
    def _playEffect(self, criteria, value):
        LOG_DEBUG(b'Set gui item view criteria', criteria.getComponentIDs(), value)
...
class FunctionalSetAction(FunctionalEffect):
    ...
    def triggerEffect(self):
        ...
        if action is None:
            LOG_ERROR(b'Action is not found', self._effect.getTargetID())
            return False
        else:
            scope = self._funcChapterCtx if self.isGlobal() else self._funcScene
            if scope is None:
                LOG_ERROR(b'Scope (scene / chapter) is not available', self._effect.getTargetID())
                return False
...
class FunctionalSetVarAction(FunctionalEffect):
    ...
    def triggerEffect(self):
        ...
        if finder is None:
            LOG_ERROR(b'Var finder is not found', self._effect.getTargetID())
            return False
        else:
            ...
            if finderType == chapter.VAR_FINDER_TYPE.GAME_ATTRIBUTE:
                ...
                return True
            LOG_ERROR(b'Type of setter is not supported', finderType)
            return False
...
class FunctionalScene(TutorialProxyHolder):
    ...
    def __init__(self, scene):
        ...
        LOG_DEBUG(b'New functional scene', scene.getID())
        self._scene = scene
        self._actions = chapter.ActionsHolder()
    ...
    def enter(self):
        itemsOnScene = self._gui.getItemsOnScene()
        LOG_DEBUG(b'Enter to scene', self._scene.getID(), itemsOnScene)
    ...
    def leave(self):
        LOG_DEBUG(b'Leave scene', self._scene.getID())
...
class GoToSceneEffect(FunctionalEffect):
    ...
    def triggerEffect(self):
        ...
        if sceneID is None:
            LOG_ERROR(b'scene is not found', self._effect.getTargetID())
            return False
        else:
...
class FunctionalSelectVehicleByCDEffect(FunctionalEffect):
    ...
    def __doTrigger(self, *_):
        ...
        if vehicleCD is None:
            LOG_ERROR(b'invalid vehicle CD')
            return False
        else:
            vehicle = self.itemsCache.items.getItemByCD(vehicleCD)
            if vehicle is None:
                LOG_ERROR(b'vehicle not found in inventory')
                return False
