...
__all__ = (b'LinearCheckpointControllerTrigger',)
_EFFECT_TYPES_TO_SKIP_DURING_RESTORE = (
...
class LinearCheckpointControllerTrigger(TriggerWithSubscription):
    ...
    def toggle(self, isOn=True, **kwargs):
        ...
        if isRestoring:
            LOG_DEBUG(b'[BOOTCAMP] restoring checkpoint', nextCheckpoint.getID())
        else:
            LOG_DEBUG(b'[BOOTCAMP] reached checkpoint', nextCheckpoint.getID())
        ...
        if self.__restoredCheckpointIndex is None:
            ...
            if restoreCheckpointID is not None:
                if restoreCheckpointID in self.__checkpointsSequence:
                    ...
                    LOG_ERROR(b'unknown checkpoint:', restoreCheckpointID)
    ...
    def __getNextCheckpoint(self):
        if self.__nextCheckpointIndex < len(self.__checkpointsSequence):
            ...
            if checkpoint is None:
                LOG_ERROR(b'checkpoint not found:', nextCheckpointID)
            else:
                return checkpoint
        return
