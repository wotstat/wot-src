...
_STUN_AREA_DEFAULTS = {b'radius': 15.0, 
   b'areasNum': 3, 
   b'color': b'0xff000000', 
   b'visual': b'content/Interface/CheckPoint/CheckPoint_white.visual', 
   b'lineColor': b'0x0ff00000', 
   b'lineWidth': 3.0, 
   b'lineHeight': 3.0}
...
class LocationPointManager(CallbackDelayer):
    ...
    def addLocationPoint(self, position, targetID, creatorID, cmdID=None, activeTime=_DEFAULT_ACTIVE_COMMAND_TIME, markerText=b'', numberOfReplies=0, isTargetForPlayer=False):
        ctrl = self.sessionProvider.shared.feedback
        if not ctrl:
            _logger.warning(b'self.sessionProvider.shared.feedback is None')
            return
        else:
    ...
    def __onPrereqsLoaded(self, resourceRefs):
        for chatCmd, params in self.__visualisationData.iteritems():
            if params[b'visual'] not in resourceRefs.failedIDs:
                ...
                _logger.warning(b'Resource is not found: %s', self.__resources[b'visual'])
    ...
    def __removeMarkedArea(self, targetID):
        if targetID in self.__markedAreas:
            ...
            _logger.warning(b"Location area with ID %d doesn't exist!", targetID)
        return
    ...
    def __createArea(self, position, visualisationParams, areaIndex):
        markerRadius = visualisationParams[b'radius']
        offset = markerRadius / visualisationParams[b'areasNum']
        areaDiameter = 2.0 * (markerRadius - areaIndex * offset)
        adaptedAreaDesc = _EquipmentAdapter(areaDiameter, areaDiameter, visualisationParams[b'visual'], visualisationParams[b'color'], None)
    ...
    def __getAreaParamsConfig(section):
        ...
        if configSection and configSection.has_key(section):
            ...
            return {b'radius': (configSection.readFloat(b'radius', _STUN_AREA_DEFAULTS[b'radius'])), 
               b'areasNum': (configSection.readInt(b'areasNum', _STUN_AREA_DEFAULTS[b'areasNum'])), 
               b'color': (int(configSection.readString(b'color', _STUN_AREA_DEFAULTS[b'color']), 0)), 
               b'visual': (configSection.readString(b'visual', _STUN_AREA_DEFAULTS[b'visual'])), 
               b'lineColor': (int(configSection.readString(b'lineColor', _STUN_AREA_DEFAULTS[b'lineColor']), 0)), 
               b'lineWidth': (configSection.readFloat(b'lineWidth', _STUN_AREA_DEFAULTS[b'lineWidth'])), 
               b'lineHeight': (configSection.readFloat(b'lineHeight', _STUN_AREA_DEFAULTS[b'lineHeight']))}
