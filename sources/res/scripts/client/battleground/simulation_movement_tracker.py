...
class SimulationMovementTracker(CallbackDelayer):
    ...
    def getSnapshot(self, shotID, isKill):
        ...
        if not shotData:
            _logger.info(b'SimulationMovementTracker.getSnapshot: Cannot get movement data for shot "%s"', snapshotID)
            return ([], 0.0)
        else:
            ...
            if not data:
                _logger.info(b'SimulationMovementTracker.getSnapshot: Cannot find data snapshot for shot "%s"', snapshotID)
                return ([], 0.0)
            return (
             data[:], lastDuration)
    ...
    def saveSnapshot(self, shotID=0, isKill=False):
        ...
        snapshotID = self.__makeSnapshotID(shotID, isKill)
        if not snapshotID:
            _logger.error(b'saveSnapshot: Cannot make snapshotID for shotID: "%s" (isKill: %s)', shotID, isKill)
            return
    ...
    def setPendingShotID(self, shotID):
        if not self.__isTrackingActive:
            ...
            if not shotID:
                _logger.error(b'setPendingShotID: Should always have shotID at this point!')
            ...
            if not shotData:
                _logger.error(b'setPendingShotID: Cannot find shot data for "%s"', pendingID)
                return
    ...
    def __makeSnapshotID(shotID, isKill):
        ...
        if not shotID:
            ...
            return (b'{0}_{1}').format(shotID, b'kill' if isKill else b'shot')
