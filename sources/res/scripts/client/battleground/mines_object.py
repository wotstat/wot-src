...
def _getSequenceResourceMapping(path, spaceId):
    return {b'sequence': (Loader(AnimationSequence.Loader(path, spaceId)))}


def _getEffectResourceMapping(name):
    return {b'effectList': (Loader(_CONFIG_PATH, effectsListName=name))}
...
def loadMines(ownerVehicleID, callback, startEffectEnabled=True, dynamicObjectsCache=None, battleSession=None):
    ...
    hasActivationEffect = hasattr(effDescr, b'activationEffect') and effDescr.activationEffect
...
class MinesObject(TerrainAreaGameObject, CompositeLoaderMixin):
    ...
    def __init__(self, isAllyMine, hasActivationDelay, startEffectEnabled=True):
        ...
        self.__soundTimerEventStart = b'eb_ability_minefield_zone_timer_start'
        self.__soundTimerEventEnd = b'eb_ability_minefield_zone_timer_end'
    ...
    def __getSoundObjName(self, eventName):
        return (b'{}_{}').format(eventName, self.id)
