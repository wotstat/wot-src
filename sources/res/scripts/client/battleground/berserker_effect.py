...
class BerserkerEffectComponent(AvatarRelatedComponent):
    ...
    __APPEARANCE_COMPONENT_NAME = b'berserker'
    ...
    def __onEquipmentComponentUpdated(self, _, vehicleID, data):
        if data[b'duration'] > 0.0:
            self.__addEffect(vehicleID, data[b'duration'] + BigWorld.serverTime())
        elif math_utils.almostZero(data[b'duration']):
    ...
    def __getLoaders(self):
        ...
        loaders[b'turretEffectPlayer'] = Loader(AnimationSequence.Loader(effDescr.turretEffect.effectDescr.path, spaceId))
        loaders[b'hullEffectPlayer'] = Loader(AnimationSequence.Loader(effDescr.hullEffect.effectDescr.path, spaceId))
        return loaders
...
class _VehicleNodeEffect(SequenceComponent):
    ...
    def finished(self):
        if self.sequenceAnimator is not None:
            return self.sequenceAnimator.getCurrNodeName() == b'end' and not self.sequenceAnimator.isCurrNodePlaying()
        else:
            return False
    ...
    def __getTransform(self, vehicle):
        predefinedTransform = _TransformationParser.getTransform(vehicle.typeDescriptor.name.split(b':')[1], TankPartIndexes.getName(self._TANK_PART_INDEX), self._getModuleName(vehicle.typeDescriptor))
        translation = predefinedTransform.get(b'offset') if predefinedTransform is not None else None
        scale = predefinedTransform.get(b'scale') if predefinedTransform is not None else None
...
class _BerserkerEffectObject(ComponentController):
    ...
    def __finish(self):
        for component in self._components:
            component.setTrigger(b'stop')

        return


class _TransformationParser(object):
    __DEFAULT_SECTION = b'default'
