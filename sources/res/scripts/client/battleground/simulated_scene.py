...
class SimulatedScene(object):
    ...
    def destroy(self):
        ...
        if self.__deathSceneObject is None:
            _logger.error(b'SimulatedScene.destroy: No CGF scene object!')
            return
        else:
    ...
    def startAnimations(self, shotID):
        ...
        if self.__simulatedKillerID:
            killerPosition = Math.Vector3(self.__rawSimulationData.get(b'attacker').get(b'position'))
        else:
            killerPosition = None
    ...
    def updateVehicleEntities(self):
        for simVehID in self.__allSimulatedVehicles:
            ...
            if mechanicsInfo is not None:
                lowChargeShotVisualState = mechanicsInfo.get(b'lowChargeShotVisualState')
    ...
    def displayEffects(self, vehicleID):
        ...
        segments = self.__rawSimulationData[b'projectile'].get(b'segments', [])
        projectileData = self.__rawSimulationData[b'projectile']
        hasProjectilePierced = projectileData[b'hasProjectilePierced']
        hasNonPiercedDamage = projectileData[b'hasNonPiercedDamage']
        if segments:
            shellCompDecr = self.__rawSimulationData[b'projectile'].get(b'shellCompDescr', [])
        ...
        self.__effectsController.displayKillCamEffects(simulatedVehicle.appearance, simulatedVehicle.getMaxComponentIndex(), hasProjectilePierced, hasNonPiercedDamage, self.__isAttackerSPG(), self.__rawSimulationData[b'attacker'][b'spotted'], projectileData[b'shellKind'] == SHELL_TYPES.HIGH_EXPLOSIVE, projectileData[b'explosionRadius'], self.__trajectoryPoints, segments, projectileData[b'impactPoint'], projectileData[b'ricochetCount'] > 0)
        return

    def saveKillSnapshot(self):
        self.__movementTracker.saveSnapshot(isKill=True)
        if self.__deathSceneObject is None:
            _logger.error(b'SimulatedScene.saveKillSnapshot: No CGF scene object!')
            return
        else:
    ...
    def __createSimulatedVehicle(self, simulatedData, isPlayerVehicle=False):
        publicInfo = simulatedData.get(b'publicInfo', None)
        positionTuple = simulatedData.get(b'position', None)
        rotationTuple = simulatedData.get(b'rotation', None)
        if positionTuple is None or publicInfo is None or rotationTuple is None:
            return
        avatar = BigWorld.player()
        spaceID = avatar.spaceID
        vehID = BigWorld.createEntity(b'SimulatedVehicle', spaceID, 0, positionTuple, rotationTuple, {b'publicInfo': publicInfo, 
           b'isPlayerVehicle': isPlayerVehicle, 
           b'realVehicleID': (simulatedData.get(b'vehicleID', None)), 
           b'simulationData_position': (simulatedData.get(b'position', (0, 0, 0))), 
           b'simulationData_dynAttachmentsInfo': (simulatedData.get(b'dynAttachmentsInfo', None)), 
           b'simulationData_rotation': (simulatedData.get(b'rotation', (0, 0, 0))), 
           b'simulationData_velocity': (simulatedData.get(b'velocity', (0, 0, 0))), 
           b'simulationData_angVelocity': (simulatedData.get(b'angVelocity', (0, 0, 0))), 
           b'simulationData_simulationType': (simulatedData.get(b'simulationType', None)), 
           b'simulationData_health': (simulatedData.get(b'health', publicInfo[b'maxHealth'])), 
           b'simulationData_engineMode': (simulatedData.get(b'engineMode', (1, 0))), 
           b'simulationData_gunAngles': (simulatedData.get(b'gunAngles', (0, 0))), 
           b'simulationData_turretAndGunSpeed': (simulatedData.get(b'turretAndGunSpeed', (0, 0))), 
           b'simulationData_damageStickers': (simulatedData.get(b'damageStickers', [])), 
           b'simulationData_brokenTracks': (simulatedData.get(b'brokenTracks', [])), 
           b'simulationData_siegeState': (simulatedData.get(b'siegeState', False)), 
           b'simulationData_wheelsState': (simulatedData.get(b'wheelsState', 0)), 
           b'simulationData_wheelsSteering': (simulatedData.get(b'wheelsSteering', [])), 
           b'simulationData_tracksInAir': (simulatedData.get(b'trackInAir', (False, False)))})
    ...
    def __pauseSequences(self, isPaused):
        if self.__deathSceneObject is None:
            _logger.error(b'SimulatedScene.__pauseSequences: No CGF scene object! (isPaused: %s)', isPaused)
            return
        else:
            queue = CGF.CommandQueue(self.__deathSceneObject.spaceID)
            if isPaused:
    ...
    def __createSimulatedVehicles(self):
        self.__simulatedVictimID = self.__createSimulatedVehicle(self.__rawSimulationData.get(b'player', None), True)
        self.__allSimulatedVehicles.add(self.__simulatedVictimID)
        if self.__killerIsSpotted:
            self.__simulatedKillerID = self.__createSimulatedVehicle(self.__rawSimulationData.get(b'attacker', None))
            self.__allSimulatedVehicles.add(self.__simulatedKillerID)
        for otherVehicle in self.__rawSimulationData.get(b'others', None):
    ...
    def __isAttackerSPG(self):
        return b'vehicleType' in self.__rawSimulationData[b'attacker'] and self.__rawSimulationData[b'attacker'][b'vehicleType'] == VEHICLE_CLASS_NAME.SPG

    def __getTurretAndGunLimits(self, simVeh):
        if simVeh.typeDescriptor:
            turretYawLimits = simVeh.typeDescriptor.gun.turretYawLimits
            gunPitchLimits = simVeh.typeDescriptor.gun.pitchLimits[b'absolute']
        else:
    ...
    def __getMechanicsInfo(self, simVehID):
        if simVehID == self.__simulatedVictimID:
            return self.__rawSimulationData.get(b'player', {}).get(b'mechanicsInfo')
        else:
            if simVehID == self.__simulatedKillerID:
                return self.__rawSimulationData.get(b'attacker', {}).get(b'mechanicsInfo')
            return

    class _PostEffectsManager(object):

        def __init__(self, data):
            self.__killCamPostEffectSettings = PostEffectSettings(data.readFloat(b'contrast'), data.readFloat(b'saturation'), Math.Vector4(data.readVector4(b'vignette')))
...
class SimulationAnimator(CallbackPauseManager, TimeDeltaMeter):
    ...
    def stop(self):
        ...
        if self._simulatedVehicle is not None:
            if hasattr(self._simulatedVehicle, b'speedInfo'):
    ...
    def __stopTick(self):
        self.clearCallbacks()
        if self._simulatedVehicle is not None:
            if hasattr(self._simulatedVehicle, b'appearance'):
                self.__updateTrackAndWheelScroll(0.0, 0.0)
        return
    ...
    def __startSection(self, p0, p1, r0, r1, tY0, tY1, gP0, gP1, duration):
        ...
        if self.__sectionDuration < 0.001:
            _logger.warning(b'SimulationAnimator.__startSection: Too short animation, skip')
            self.__initSection(self.__section + 1)
            return
...
def _updateDynAttachments(simulatedVehicle, dynAttachmentInfo):
    ...
    for accessor in result:
        ...
        if not sequence:
            _logger.warning(b'Can not change position for a dynamic attachment')
            continue
