...
ModelSetting = namedtuple(b'ModelSetting', (b'scale', b'spacing', b'color'))
DashesParams = namedtuple(b'DashesParams', (b'size', b'color', b'shift', b'offset'))

def _readModelSetting(ctx, section):
    return ModelSetting(scale=_xml.readVector3(ctx, section, b'scale'), spacing=_xml.readFloat(ctx, section, b'spacing'), color=int(_xml.readString(ctx, section, b'color'), 0))


def _readVisualSettings():
    ctx = (
     None, CONFIG_FILE + b'/' + BORDER_VISUAL_TAG)
    settings = ResMgr.openSection(CONFIG_FILE)[BORDER_VISUAL_TAG]
    return VisualSetting(modelPath=_xml.readString(ctx, settings, b'modelPath'), overTerrainHeight=_xml.readFloat(ctx, settings, b'overTerrainHeight'), modelSettings={getattr(SECTOR_EDGE_STATE, item[1][1][b'edgeState'].asString.upper()): _readModelSetting(*item[1]) for item in _xml.getItemsWithContext(ctx, settings, b'modelSetting') if SECTOR_EDGE_STATE.hasKey(item[1][1][b'edgeState'].asString.upper())})
...
class BorderVisual(object):
    ...
    def create(self, fromPos, toPos, currentEdgeState=SECTOR_EDGE_STATE.NONE):
        if None in (fromPos, toPos):
            ...
            self.__rootModel = rootModel = BigWorld.Model(b'')
            ...
            for idx in range(0, maxNumModels):
                ...
                self.__attachNodes[idx] = attachNode = rootModel.node(b'', offset)
