...
import BigWorld, Math, AnimationSequence, CGF
...
def _getSequenceResourceMapping(desc, effectName, spaceID, includePaths=False):
    ...
    if effectDesc is not None:
        ...
        res = {b'sequence': (Loader(AnimationSequence.Loader(effectPath, spaceID)))}
        if includePaths:
            res[b'path'] = effectPath
        return res
    return {}
...
def _loadLootForeground(loot, spaceID, desc):
    loaders = {}
    if hasattr(desc, b'model'):
        modelName = desc.model.name
        offset = getattr(desc.model, b'overTerrainHeight', 0.0)
        if modelName is not None:
            modelAssembler = BigWorld.CompoundAssembler(b'model', spaceID)
            modelAssembler.addRootPart(modelName, b'root')
            borderName = getattr(desc.model, b'border', None)
            if borderName is not None:
                borderMatrix = Math.Matrix()
                modelAssembler.addPart(borderName, b'root', b'border', borderMatrix)
            loaders[b'model'] = Loader(modelAssembler, offset=offset)
    loadComponentSystem(loot.markingSmoke, loot.appendPiece, _getSequenceResourceMapping(desc, b'effect', spaceID), forceForegroundLoad=True)
    loadComponentSystem(loot.pickupEffect, loot.appendPiece, _getSequenceResourceMapping(desc, b'pickupEffect', spaceID), forceForegroundLoad=True)
    loadComponentSystem(loot, loot.appendPiece, loaders, forceForegroundLoad=True)
    _logger.info(b'[Loot] Loading of loot type %d done', loot.lootTypeID)
    return
...
class SteelHunterDynamicObjectsCachingManager(CGF.ComponentManager):
    ...
    def cacheForegroundLootLoading(self, loot):
        _logger.info(b'[Loot] Issuing caching of loot type %d creation', loot.lootTypeID)
    ...
    def __orderLootCache(self, lootType, lootDescr):
        if lootDescr is not None:
            ...
            if hasattr(lootDescr, b'model'):
                modelName = lootDescr.model.name
                offset = getattr(lootDescr.model, b'overTerrainHeight', 0.0)
                loaders = {}
                if modelName is not None:
                    modelAssembler = BigWorld.CompoundAssembler(b'model', spaceID)
                    modelAssembler.addRootPart(modelName, b'root')
                    borderName = getattr(lootDescr.model, b'border', None)
                    if borderName is not None:
                        borderMatrix = Math.Matrix()
                        modelAssembler.addPart(borderName, b'root', b'border', borderMatrix)
                    loaders = {b'model': (Loader(modelAssembler, offset=offset))}
                mapping = _getSequenceResourceMapping(lootDescr, b'effect', spaceID, True)
                effectsPaths = []
                if mapping:
                    loaders[b'effect'] = mapping[b'sequence']
                    effectsPaths.append(mapping[b'path'])
                mapping = _getSequenceResourceMapping(lootDescr, b'pickupEffect', spaceID, True)
                if mapping:
                    loaders[b'pickupEffect'] = mapping[b'sequence']
                    effectsPaths.append(mapping[b'path'])
    ...
    def __cacheResourceList(self, lootType, effectsPaths, resourceList):
        ...
        _logger.info(b'[Loot] Loot %d resources has been cached', lootType)
        toForegroundLoad = self.__cachingQueue[lootType]
        if not toForegroundLoad:
            return
        _logger.info(b'[Loot] Performing cached loot creation of type %d, amount %d', lootType, len(toForegroundLoad))
