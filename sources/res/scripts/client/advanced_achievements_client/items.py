...
class _Progress(object):
    ...
    def __repr__(self):
        return (b'{}: {} of {}').format(self.__class__.__name__, self.current, self.total)


class _Rewards(object):
    __slots__ = (b'__points', b'__bonuses')
...
class _BaseGuiAchievement(object):
    __slots__ = (b'_staticData', b'__dossierDescr')
    ...
    def getStringKey(self):
        return self._staticData.UI.get(self._FieldNames.KEY, b'')

    def getTheme(self):
        return self._staticData.UI.get(self._FieldNames.THEME, b'')

    def getBackground(self):
        return self._staticData.UI.get(self._FieldNames.BACKGROUND, b'')
    ...
    def getOwnScore(self):
        ...
        for stage in self._staticData.stages:
            if achievedValue >= stage.get(b'value'):
                currentValue += stage.get(b'points', 0)
            maxValue += stage.get(b'points', 0)
    ...
    def makeAchievement(achievementID, achievementCategory, dossierDescr=None):
        ...
        if staticData.conditions.get(b'requiredAchievementIDs'):
...
class RegularAchievement(_BaseGuiAchievement):
    ...
    def getProgress(self):
        achievedValue, achievedStage, _ = self._staticData.getCurrentDataFromDossier(self._getDossier())
        nextStage = self._staticData.getActiveStage(achievedValue, achievedStage)
        maxValue = nextStage[b'value'] if nextStage else achievedValue
        return _Progress(achievedValue, maxValue)

    def getConditionID(self):
        return self._staticData.conditions.get(b'vehicle')


class VirtualStepAchievement(RegularAchievement):
    __slots__ = (b'__realProgress', b'__stepData')
    ...
    def getProgress(self):
        currentValue = min(self.__realProgress.current, self.__stepData[b'value'])
        return _Progress(currentValue, self.__stepData[b'value'])

    def getOwnScore(self):
        achievedValue, _, _ = self._staticData.getCurrentDataFromDossier(self._getDossier())
        maxValue = 0
        currentValue = 0
        maxStageID = self.__stepData[b'id']
        for stage in self._staticData.stages:
            if stage[b'id'] > maxStageID:
                return _Progress(currentValue, maxValue)
            if achievedValue >= stage.get(b'value'):
                currentValue += stage.get(b'points', 0)
            maxValue += stage.get(b'points', 0)
    ...
    def getRewards(self):
        stageID = self.__stepData[b'id']
        return _Rewards(self._staticData.getStagePointsByValue(stageID), self._staticData.getStageBonusByValue(stageID))

    def getNextOrLastStageID(self):
        return self.__stepData[b'id']

    def getFakeAchievementForStage(self, _):
        raise SoftException((b'Cannot be called from {}').format(self.__class__.__name__))
        return
