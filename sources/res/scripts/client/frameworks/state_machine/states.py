...
class State(Node):
    __slots__ = (b'__stateID', b'__flags', b'__isEntered')
    ...
    def getInitial(self):
        ...
        if not children:
            ...
            if len(children) > 1:
                raise StateError((b'State {} should be have one initial state, found {}').format(self, children))
            return children[0]
    ...
    def addChild(self, child):
        raise StateError(b'Routine is not allowed in {}', self.__class__.__name__)
        return
...
class _StateTogglingSortKey(object):
    ...
    def _cmp(self, other):
        ...
        lca = visitor.getLCA([
         self.state, other.state], upper=self.state.getMachine())
