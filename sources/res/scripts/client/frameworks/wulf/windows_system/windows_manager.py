...
class WindowsManager(PyObjectEntity):
    __slots__ = (b'__eManager', b'onWindowStatusChanged', b'onWindowShowingStatusChanged', b'onViewStatusChanged')
    ...
    def getViewByLayoutID(self, layoutID):
        ...
        if len(views) > 1:
            _logger.warning(b'There are more than one view loaded with layoutID = %s', layoutID)
        if not views:
            return None
        else:
            return views[0]
    ...
    def loadView(self, layoutID, viewClass, *args, **kwargs):
        ...
        if pyView is None:
            ...
            _logger.warning(b'View is already loading/loaded: %r.', layoutID)
        return pyView
