from __future__ import absolute_import
import math
from future.utils import itervalues, viewvalues, viewitems
import BigWorld, Math, PlayerEvents
from constants import ARENA_PERIOD
from debug_utils import LOG_CURRENT_EXCEPTION
from helpers import isPlayerAvatar

class TRIGGER_TYPE(object):
    AIM = 0
    AREA = 1
    AIM_AT_VEHICLE = 3
    AUTO_AIM_AT_VEHICLE = 4
    VEHICLE_DESTROYED = 5
    PLAYER_VEHICLE_ON_SOFT_TERRAIN = 6
    PLAYER_VEHICLE_TRACKS_DAMAGED = 7
    PLAYER_SHOT_MISSED = 8
    SNIPER_MODE = 12
    PLAYER_TANKMAN_SHOOTED = 13
    PLAYER_VEHICLE_IN_FIRE = 14
    PLAYER_VEHICLE_OBSERVED = 15
    PLAYER_RECEIVE_DAMAGE = 16
    PLAYER_DISCRETE_SHOOT = 17
    PLAYER_DETECT_ENEMY = 18
    VEHICLE_VISUAL_VISIBILITY_CHANGED = 19
    PLAYER_DEVICE_CRITICAL = 20
    PLAYER_MOVE = 21
    SHOW_TRACER = 22
    PLAYER_SHOT_HIT = 23
    STUN = 24
    SIXTH_SENSE = 25
    PLAYER_ENTER_SPG_STRATEGIC_MODE = 26
    PLAYER_ENTER_SPG_SNIPER_MODE = 27
    PLAYER_LEAVE_SPG_MODE = 28
    CTRL_MODE_CHANGE = 29
    PLAYER_USED_AOE_EQUIPMENT = 30
    PLAYER_CONTINUOUS_BURST_START = 31
    PLAYER_CONTINUOUS_BURST_STOP = 32
    CURRENT_VEHICLE_AREA = 33


class ITriggerListener(object):

    def onTriggerActivated(self, args):
        return

    def onTriggerDeactivated(self, args):
        return


class TriggersManager(object):
    UPDATE_PERIOD = 0.05
    isActive = property((lambda self: self.__isEnabled and self.__isOnArena))

    def __init__(self):
        self.__autoTriggers = {}
        self.__activeAutoTriggers = {}
        self.__pendingManualTriggers = {}
        self.__activeManualTriggers = {}
        self.__listeners = set()
        self.__nextTriggerId = 1
        self.__cbID = None
        self.__isOnArena = False
        self.__isEnabled = False
        self.__shotPoints = []
        self.__explodePoints = []
        self.__previousCurrentVehicleID = None
        PlayerEvents.g_playerEvents.onArenaPeriodChange += self.__onArenaPeriodChange
        return

    def isEnabled(self):
        return self.__isEnabled

    def destroy(self):
        if self.__cbID is not None:
            BigWorld.cancelCallback(self.__cbID)
            self.__cbID = None
        self.clearTriggers(False)
        self.__listeners = None
        self.__isOnArena = False
        self.__isEnabled = False
        PlayerEvents.g_playerEvents.onArenaPeriodChange -= self.__onArenaPeriodChange
        return

    def enable(self, enable):
        self.__isEnabled = enable
        if not enable:
            self.clearTriggers(True)
            if self.__cbID is not None:
                BigWorld.cancelCallback(self.__cbID)
                self.__cbID = None
        else:
            self.__cbID = BigWorld.callback(self.UPDATE_PERIOD, self.update)
        return

    def clearTriggers(self, keepTriggersFromMap):
        self.__pendingManualTriggers = {}
        self.__activeManualTriggers = {}
        if not keepTriggersFromMap:
            self.__autoTriggers = {}
            self.__activeAutoTriggers = {}
        else:
            autoTriggers = self.__autoTriggers.copy()
            for k, v in viewitems(autoTriggers):
                tType = v[b'type']
                if tType not in (TRIGGER_TYPE.AIM, TRIGGER_TYPE.AREA, TRIGGER_TYPE.CURRENT_VEHICLE_AREA):
                    del self.__autoTriggers[k]
                    self.__activeAutoTriggers.get(tType, set()).discard(k)

        return

    def addListener(self, listener):
        if listener in self.__listeners:
            return
        self.__listeners.add(listener)
        if self.isActive:
            for tIDs in viewvalues(self.__activeAutoTriggers):
                for tID in tIDs:
                    listener.onTriggerActivated(self.__autoTriggers[tID])

            for trigger in viewvalues(self.__activeManualTriggers):
                listener.onTriggerActivated(trigger)

        return

    def delListener(self, listener):
        self.__listeners.discard(listener)
        return

    def addTrigger(self, tType, **params):
        params = dict(params) if params is not None else {}
        params[b'type'] = tType
        self.__autoTriggers[self.__nextTriggerId] = params
        self.__nextTriggerId = self.__nextTriggerId + 1
        return self.__nextTriggerId - 1

    def delTrigger(self, triggerID):
        if triggerID in self.__autoTriggers:
            del self.__autoTriggers[triggerID]
            for triggerIDS in itervalues(self.__activeAutoTriggers):
                if triggerID in triggerIDS:
                    triggerIDS.discard(triggerID)
                    break

        return

    def fireTrigger(self, tType, **kwargs):
        params = dict(kwargs) if kwargs is not None else {}
        params[b'type'] = tType
        if tType not in self.__pendingManualTriggers:
            self.__pendingManualTriggers[tType] = []
        self.__pendingManualTriggers[tType].append((False, params))
        return

    def fireTriggerInstantly(self, tType, **kwargs):
        kwargs[b'type'] = tType
        for listener in self.__listeners:
            listener.onTriggerActivated(kwargs)

        return

    def activateTrigger(self, triggerType, **kwargs):
        params = dict(kwargs) if kwargs is not None else {}
        params[b'type'] = triggerType
        if triggerType in self.__activeManualTriggers:
            self.deactivateTrigger(triggerType)
        if triggerType not in self.__pendingManualTriggers:
            self.__pendingManualTriggers[triggerType] = []
        self.__pendingManualTriggers[triggerType].append((True, params))
        return

    def deactivateTrigger(self, triggerType):
        if triggerType in self.__pendingManualTriggers:
            del self.__pendingManualTriggers[triggerType]
        if triggerType in self.__activeManualTriggers:
            if self.isActive:
                params = self.__activeManualTriggers[triggerType]
                for listener in self.__listeners:
                    listener.onTriggerDeactivated(params)

            del self.__activeManualTriggers[triggerType]
        return

    def update(self):
        self.__cbID = BigWorld.callback(self.UPDATE_PERIOD, self.update)
        if not self.isActive:
            return
        else:
            player = BigWorld.player()
            if not isPlayerAvatar():
                return
            camera = BigWorld.camera()
            playerVehicle = BigWorld.entities.get(player.playerVehicleID)
            currentVehicle = player.getVehicleAttached()
            try:
                for tType, data in viewitems(self.__pendingManualTriggers):
                    for isTwoState, args in data:
                        params = dict(args)
                        params[b'type'] = tType
                        for listener in self.__listeners:
                            listener.onTriggerActivated(params)

                        if isTwoState:
                            self.__activeManualTriggers[tType] = params

                self.__pendingManualTriggers = {}
                for tID, params in viewitems(self.__autoTriggers):
                    tType = params[b'type']
                    activeAutoTriggers = self.__activeAutoTriggers.setdefault(tType, set())
                    wasActive = tID in activeAutoTriggers
                    isActive = False
                    distance = -1.0
                    if tType == TRIGGER_TYPE.AREA:
                        isActive = self.__getAreaTriggerActivationStatus(playerVehicle, params, wasActive)
                    if tType == TRIGGER_TYPE.CURRENT_VEHICLE_AREA:
                        sameVeh = currentVehicle and currentVehicle.id == self.__previousCurrentVehicleID
                        isActive = sameVeh and self.__getAreaTriggerActivationStatus(currentVehicle, params, wasActive)
                    if tType == TRIGGER_TYPE.AIM:
                        isActive = self.__getAimTriggerActivationStatus(playerVehicle, camera, params)
                    params[b'distance'] = distance
                    if wasActive != isActive:
                        if isActive:
                            activeAutoTriggers.add(tID)
                            for listener in self.__listeners:
                                listener.onTriggerActivated(params)

                        else:
                            activeAutoTriggers.clear()
                            for listener in self.__listeners:
                                listener.onTriggerDeactivated(params)

                self.__explodePoints = []
                self.__shotPoints = []
                self.__previousCurrentVehicleID = currentVehicle.id if currentVehicle else None
            except Exception:
                LOG_CURRENT_EXCEPTION()

            return

    def getTriggerPosition(self, tType, name):
        for params in viewvalues(self.__autoTriggers):
            if params[b'type'] == tType and params[b'name'] == name:
                return params.get(b'position')

        return

    def getDistanceToTrigger(self, tType, name):
        for params in viewvalues(self.__autoTriggers):
            if params[b'type'] == tType and params[b'name'] == name:
                return params.get(b'distance')

        return

    def isAutoTriggerActive(self, tType, name):
        for tID, params in viewitems(self.__autoTriggers):
            if params[b'type'] == tType and params[b'name'] == name:
                return tID in self.__activeAutoTriggers.get(tType, set())

        return

    def __onArenaPeriodChange(self, period, periodEndTime, periodLength, periodAdditionalInfo):
        isOnArena = period == ARENA_PERIOD.BATTLE
        if not isOnArena and self.__isOnArena:
            self.clearTriggers(False)
        self.__isOnArena = isOnArena
        return

    def __getAreaTriggerActivationStatus(self, vehicle, params, wasActive):
        if vehicle is None:
            return False
        else:
            scale = Math.Vector3(params[b'scale'])
            if scale == Math.Vector3(1, 1, 1):
                triggerRadius = params[b'radius']
                if wasActive:
                    triggerRadius = triggerRadius + params[b'exitInterval']
                offset = vehicle.position - Math.Vector3(params[b'position'])
                offset.y = 0
                distance = offset.length
                return distance < triggerRadius
            targetMatrix = Math.Matrix()
            targetMatrix.setRotateY(-params[b'direction'].z)
            offset = vehicle.position - Math.Vector3(params[b'position'])
            offset.y = 0
            offset = targetMatrix.applyPoint(offset)
            originalSize = 5.0
            return -(-scale.x * originalSize < offset.x < scale.x * originalSize) and (scale.z) * originalSize < offset.z < scale.z * originalSize

    def __getAimTriggerActivationStatus(self, vehicle, camera, params):
        activationStatus = False
        camToTrigger = Math.Vector3(params[b'position']) - camera.position
        vehicleToTrigger = Math.Vector3(params[b'position']) - vehicle.position
        distance = camToTrigger.length
        if distance <= params[b'maxDistance'] and vehicleToTrigger.dot(camToTrigger) > 0.0:
            camToTrigger.normalise()
            dp = camToTrigger.dot(camera.direction)
            if dp > 0.0:
                sinAngle = math.sqrt(1.0 - dp * dp)
                activationStatus = sinAngle * distance < params[b'radius']
        return activationStatus


g_manager = None

def init():
    global g_manager
    g_manager = TriggersManager()
    return
