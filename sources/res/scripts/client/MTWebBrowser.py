...
_webAppLogger = logging.getLogger((b'{} (webapp)').format(__name__))
_BROWSER_KEY_LOGGING = False
_OLD_PREFIX = (b'').join([chr(c + 10) for c in [109, 101, 106]])
_MT_CLIENT_PARAM_NAME = b'mt_client_param'
_OLD_CLIENT_PARAM_NAME = _OLD_PREFIX + b'_client_param'
_MT_RESOURCE_CUSTOM_SCHEME = b'mtdata'
_OLD_RESOURCE_CUSTOM_SCHEME = _OLD_PREFIX + b'data'
_WEB_CACHE_FOLDER = b'web_cache'
_WEB_MANIFEST_KEY = b'webManifestURL'
_g_webCache = None

def initExternalCache():
    global _g_webCache
    if _g_webCache is None:
        _logger.info(b'WebExternalCache init')
...
def destroyExternalCache():
    ...
    if _g_webCache is not None:
        _g_webCache.close()
        _g_webCache = None
        _logger.info(b'WebExternalCache destroyed')
    return


def pauseExternalCache(value):
    if _g_webCache is not None:
        _logger.info(b'WebExternalCache setPause(%r)', value)
...
class MTWebBrowser(object):
    ...
    baseUrl = property((lambda self: b'' if self.__browser is None else self.__baseUrl))
    url = property((lambda self: b'' if self.__browser is None else self.__browser.url))
    ...
    def skipEscape(self, value):
        _logger.debug(b'skipEscape set %s (was: %s)', value, self.__skipEscape)
        self.__skipEscape = value
        return

    @ignoreKeyEvents.setter
    def ignoreKeyEvents(self, value):
        _logger.debug(b'ignoreKeyEvents set %s (was: %s)', value, self.__ignoreKeyEvents)
        self.__ignoreKeyEvents = value
        return

    @ignoreAltKey.setter
    def ignoreAltKey(self, value):
        _logger.debug(b'ignoreAltKey set %s (was: %s)', value, self.__ignoreAltKey)
        self.__ignoreAltKey = value
        return

    @ignoreCtrlClick.setter
    def ignoreCtrlClick(self, value):
        _logger.debug(b'ignoreCtrlClick set %s (was: %s)', value, self.__ignoreCtrlClick)
        self.__ignoreCtrlClick = value
        return

    @ignoreShiftClick.setter
    def ignoreShiftClick(self, value):
        _logger.debug(b'ignoreShiftClick set %s (was: %s)', value, self.__ignoreShiftClick)
        self.__ignoreShiftClick = value
        return

    @useSpecialKeys.setter
    def useSpecialKeys(self, value):
        _logger.debug(b'useSpecialKeys set %s (was: %s)', value, self.__useSpecialKeys)
        self.__useSpecialKeys = value
        return

    @allowMiddleClick.setter
    def allowMiddleClick(self, value):
        _logger.debug(b'allowMiddleClick set %s (was: %s)', value, self.__allowMiddleClick)
        self.__allowMiddleClick = value
        return

    @allowRightClick.setter
    def allowRightClick(self, value):
        _logger.debug(b'allowRightClick set %s (was: %s)', value, self.__allowRightClick)
        self.__allowRightClick = value
        return

    @allowMouseWheel.setter
    def allowMouseWheel(self, value):
        _logger.debug(b'allowMouseWheel set %s (was: %s)', value, self.__allowMouseWheel)
        self.__allowMouseWheel = value
        return

    def __init__(self, browserID, uiObj, url=b'about:blank', isFocused=False, handlers=None):
        ...
        _logger.info(b'INIT %s, id: %s', self.__baseUrl, self.__browserID)
        levelSetting = Settings.g_instance.engineConfig[b'webBrowser'][b'logVerbosity'].asString
        levelSettingEnum = LogSeverity[levelSetting]
        _webAppLogger.setLevel(_LOG_SEVERITY_TO_LOG_LEVEL_MAP[levelSettingEnum])
        return

    def create(self):
        _logger.info(b'CREATE %s id: %s', self.__baseUrl, self.__browserID)
        self.__browser = BigWorld.createWebView(self.__browserID)
        if self.__browser is None:
            _logger.error(b'create() NO BROWSER WAS CREATED: %s', self.__baseUrl)
            return False
        else:
            ...
            def injectBrowserKeyEvent(me, e):
                if _BROWSER_KEY_LOGGING:
                    _logger.debug(b'injectBrowserKeyEvent: %s', (
                     e.key,
                     e.isKeyDown(),
                     e.isAltDown(),
                     e.isShiftDown(),
    ...
    def ready(self, success):
        _logger.info(b'READY success: %r %s id: %s', success, self.__baseUrl, self.__browserID)
        ...
        if success:
            ...
            _logger.error(b' FAILED %s id: %s', self.__baseUrl, self.__browserID)
    ...
    def destroy(self):
        ...
        if self.__browser is not None:
            _logger.info(b'DESTROYED %s id: %s', self.__baseUrl, self.__browserID)
    ...
    def navigate(self, url):
        _logger.debug(b'navigate %s', url)
        self.__baseUrl = url
        if self.hasBrowser:
    ...
    def handleKeyEvent(self, event):
        ...
        if not self.isFocused:
            ...
            return False
        if _BROWSER_KEY_LOGGING:
            _logger.debug(b'handleKeyEvent %s', keyState)
    ...
    def browserAction(self, action):
        if self.hasBrowser and self.enableUpdate:
            if action == b'reload' and self.isNavigationComplete:
                self.refresh()
            elif action == b'loading' and not self.isNavigationComplete:
                self.navigateStop()
        return
    ...
    def addFilter(self, handler):
        if handler in self.__navigationFilters:
            _logger.error(b'Navigation filter is already added: %r', handler)
        else:
    ...
    def removeFilter(self, handler):
        if handler in self.__navigationFilters:
            ...
            _logger.error(b"Trying to delete navigation filter which doesn't exist: %r", handler)
        return
        ...
        for handler in self.__navigationFilters:
            try:
                ...
                if result.stopNavigation:
                    _logger.debug(b'Navigation filter triggered navigation stop: %s', handler)
                if result.closeBrowser:
                    _logger.debug(b'Navigation filter triggered browser close: %s', handler)
            except Exception:
    ...
    def setAllowAutoLoadingScreen(self, enabled):
        _logger.debug(b'setAllowAutoLoadingScreen %s', enabled)
    ...
    def __onLoadStart(self, url):
        if url == self.__browser.url:
            self.__isNavigationComplete = False
            _logger.debug(b'onLoadStart %s', self.__browser.url)
    ...
    def __onLoadEnd(self, url, isLoaded=True, httpStatusCode=None, errorDesc=None):
        ...
        if url == self.__browser.url or errorDesc:
            ...
            if not isLoaded or httpStatusCode and httpStatusCode >= 400 or errorDesc:
                _logger.error(b'FAILED Url: %s, Http code: %r, Browser error: %s', self.__browser.url, httpStatusCode, errorDesc)
            self.onLoadEnd(self.__browser.url, isLoaded, httpStatusCode)
        return

    def __onLoadingStateChange(self, isLoading):
        _logger.debug(b'onLoadingStateChange %r %r', isLoading, self.__allowAutoLoadingScreenChange)
    ...
    def __onDestroy(self):
        _logger.info(b'Destroy Web View %s - %r', self.__baseUrl, self.__browserID)
        self.destroy()
        return
    ...
    def __onConsoleMessage(self, level, message, lineNumber, source, viewId):
        ...
        if levelEnum != LogSeverity.disable:
            _webAppLogger.log(_LOG_SEVERITY_TO_LOG_LEVEL_MAP[levelEnum], b'%s, line %s, viewId %s: %s', source, lineNumber, viewId, message)
        return
    ...
    def __onTextureStateChanged(self, isOk):
        _logger.info(b'onTextureStateChanged isOk: %r', isOk)
...
class EventListener(object):
    ...
    def onChangeTitle(self, title):
        _logger.debug(b'onChangeTitle %s', title)
        self.onTitleChange(title)
        return
    ...
    def onBeginLoadingFrame(self, frameId, isMainFrame, url):
        if isMainFrame:
            _logger.debug(b'onBeginLoadingFrame(isMainFrame) %s', url)
    ...
    def onFailLoadingFrame(self, frameId, isMainFrame, url, errorCode, errorDesc):
        if isMainFrame:
            _logger.debug(b'onFailLoadingFrame(isMainFrame) %s, error: %r, text: %r', url, errorCode, errorDesc)
    ...
    def onFinishLoadingFrame(self, frameId, isMainFrame, url, httpStatusCode):
        if isMainFrame:
            _logger.debug(b'onFinishLoadingFrame(isMainFrame) %s, httpStatusCode: %r', url, httpStatusCode)
            self.onLoadEnd(url, not self.__urlFailed, httpStatusCode)
        return

    def onBrowserLoadingStateChange(self, isLoading):
        _logger.debug(b'onBrowserLoadingStateChange isLoading: %r', isLoading)
        self.onLoadingStateChange(isLoading)
        return

    def onDocumentReady(self, url):
        _logger.debug(b'onDocumentReady %s', url)
        self.onDOMReady(url)
        return
    ...
    def onShowCreatedWebView(self, url, isPopup):
        _logger.debug(b'onShowCreatedWebView %s isPopup: %r', url, isPopup)
        return
...
class WebBrowserManager(object):
    ...
    def __muteBackgroundAudio(self, isMute):
        if self.isBackgroundAudioMuted != isMute:
            SoundGroups.g_instance.playSound2D(b'ue_master_mute' if isMute else b'ue_master_unmute')
