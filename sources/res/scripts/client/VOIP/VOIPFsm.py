...
class VOIPFsm(object):
    ...
    def __setState(self, newState):
        if newState == self.__state:
            return
        _logger.debug(b'%s -> %s', _STATE_NAMES[self.__state], _STATE_NAMES[newState])
    ...
    def update(self, voip):
        if self.__state == _STATE.NONE:
        ...
        elif self.__state == _STATE.LOGGED_IN and voip.getUser() == b'':
            ...
            _logger.debug(b'goto LEAVING_CHANNEL state: %r %r %r', voip.isInDesiredChannel(), voip.isEnabled(), voip.getCurrentChannel())
            self.__setState(_STATE.LEAVING_CHANNEL)
        elif self.__state == _STATE.LEAVING_CHANNEL and not voip.getCurrentChannel():
            self.__setState(_STATE.LOGGED_IN)
        else:
            _logger.debug(b'%s not changed', _STATE_NAMES[self.__state])
        return
