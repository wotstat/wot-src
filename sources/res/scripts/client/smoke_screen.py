...
class _SmokeDebugVisualization(object):
    ...
    def __init__(self, position, equipmentID, endTime):
        ...
        self.__debugGizmo = DebugGizmo(b'objects/misc/bbox/cylinder1_proxy.model')
...
class SmokeScreen(object):
    ...
    def __loadSmokeScreen(self, equipmentID, position, team):
        ...
        if player.isSimulationSceneActive:
            ...
            settingsData = ResMgr.openSection(ENVIRONMENT_EFFECTS_CONFIG_FILE + b'/' + effectName)
    ...
    def enableSmokePostEffect(enabled, smokeInfos=None):
        if enabled:
            if smokeInfos:
                equipmentID = smokeInfos[b'equipmentID']
                ...
                _logger.warning(b'No smoke infos set for smoke posteffect, using default values.')
        ...
        elif not SmokeScreen.vignetteEnabled:
            return
