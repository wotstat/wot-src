...
class TargetMarkersComponent(DynamicScriptComponent, TriggersManager.ITriggerListener):
    ...
    def addMarker(self, markerSetting):
        targetId = markerSetting[b'targetId']
    ...
    def removeMarkers(self, markerSettings):
        for settings in markerSettings:
            self._deleteMarkerBySettingId(settings[b'targetId'], settings[b'settingId'])

        return

    def onTriggerActivated(self, args):
        triggerType = args.get(b'type')
            ...
            if triggerType == TriggersManager.TRIGGER_TYPE.VEHICLE_VISUAL_VISIBILITY_CHANGED:
                ...
                if vehicle is not None:
                    ...
                    for settings in self.markersSettings:
                        if vehicle.id == settings[b'targetId']:
                            if args[b'isVisible']:
    ...
    def _onAvatarReady(self):
        ...
        if self._isReconnect:
            for settings in self.markersSettings:
                targetId = settings[b'targetId']
                target = BigWorld.entities.get(settings[b'targetId'])
                lastVisiblePosition = settings[b'lastVisiblePosition']
    ...
    def _createVisibleMarker(self, vehicleId, settings, vehicle):
        if vehicle and not vehicle.isDestroyed:
            self._createMarker(vehicleId=vehicleId, settingId=settings[b'settingId'], markerType=settings[b'visibleStyle'], matrix=vehicle.matrix, vehicle=vehicle)
        return

    def _createInvisibleMarker(self, vehicleId, settings, position):
        matrix = Math.Matrix()
        matrix.setTranslate(position)
        self._createMarker(vehicleId=vehicleId, settingId=settings[b'settingId'], markerType=settings[b'invisibleStyle'], matrix=matrix)
        return
    ...
    def _createMarker(self, vehicleId, settingId, markerType, matrix, vehicle=None):
        ...
        if ctrl is None or not markerType:
            return
        params = {b'markerType': markerType, b'visible': True, 
           b'matrix': matrix, 
           b'targetID': vehicleId, 
           b'entity': vehicle}
