...
def _expectDir(path):
    try:
        if not os.path.exists(path):
            ...
            raise SoftException((b'path "{}" exists, but it is not a directory').format(path))
    ...
    def doWork(self, worker):
        ...
        try:
            try:
                ...
                _expectDir(dirPath)
                with open(self.__filename, b'wb') as f:
                    f.write(self.__data)
                    stored = True
...
class ApplicationStorage(object):
    ...
    def load(self):
        cache = {}
        cacheSize = 0
        _logger.info(b'WebCache start loading from disk...')
        ...
        if cacheSizeInMb > 0:
            ...
            _logger.info(b'WebCache is empty')
        _logger.info(b'WebCache loading finished')
        return cache
