    ...
    if Waiting.isVisible():
        ...
        if dialogsContainer is not None:
            ...
            if dialog is not None:
                dialog.onApply()
            dialog = dialogsContainer.getView(criteria={(POP_UP_CRITERIA.VIEW_ALIAS): b'tGreetingDialog'})
            if dialog is not None:
                dialog.cancel()
        ...
        if view and view.alias == VIEW_ALIAS.LOGIN and view.isCreated() and connectionMgr.isDisconnected() and not _isConnecting:
            ...
            return
        view = lobby.containerManager.getView(WindowLayer.VIEW)
        if view is not None and view.alias == b'lobby':
            if _justLogin:
                return
            subView = lobby.containerManager.getView(WindowLayer.SUB_VIEW)
            if subView.alias == b'hangar':
                _leaveDevRoom()
                BigWorld.callback(0.2, _detectCurrentScreen)
                return
            if subView.alias == b'trainingRoom':
                _enterBattle()
                return
        BigWorld.callback(0.2, _detectCurrentScreen)
        return
...
def _enterDevRoom():
    BigWorld.player().prb_createDev(_arenaId, _battleTime, b'')
    return
