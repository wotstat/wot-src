...
class _HttpResponse(object):
    ...
    def getLastModified(self):
        modified = self._getHeader(b'Last-Modified')
        if modified:
    ...
    def getExpires(self):
        expires = self._getHeader(b'Expires')
        if expires:
    ...
    def willClose(self):
        return hasattr(self._response, b'will_close') and self._response.will_close

    def _getHeader(self, header):
        return self._response and self._response.info().getheader(header)

    def _getCode(self):
        return getattr(self._response, b'code', None)
    ...
    def __repr__(self):
        return b'HttpResponse(code = %s, datalen = %s, modified = %s, expires = %s)' % (
...
def openUrl(url, timeout=_DEFAULT_TIMEOUT, modified=None, agent=b''):
    response = None
    if not agent:
    ...
    try:
        try:
            ...
            if modified:
                request.add_header(b'If-Modified-Since', formatdate(modified))
            ...
            LOG_WARNING(b'urllib2.HTTPError', e.code, url)
        except urllib2.URLError as e:
            LOG_WARNING(b'urllib2.URLError', e.reason, url)
...
def openPage(connection, page, modified=None, agent=b''):
    response = None
    if not agent:
        agent = _getClientVersion()
    try:
        ...
        if modified:
            headers[b'If-Modified-Since'] = formatdate(modified)
        connection.request(b'GET', page, headers=headers)
        response = connection.getresponse()
    except Exception as e:
        LOG_ERROR(b"Client couldn't download file", e, connection, page)
