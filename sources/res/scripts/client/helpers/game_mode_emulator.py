...
def gameModeVisibilityMask(spaceName):
    if b'gameMode' not in sys.argv:
        return DEFAULT_VISIBILITY_MASK
    gameModeArgIndex = sys.argv.index(b'gameMode') + 1
...
def createFakeAvatar():
    entityID = BigWorld.createEntity(b'OfflineEntity', BigWorld.camera().spaceID, 0, (0, 0, 0), (0, 0, 0), {})
