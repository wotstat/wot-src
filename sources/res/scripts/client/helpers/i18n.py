...
def makeString(key, *args, **kwargs):
    ...
    if args:
        try:
            ...
            _logger.warning(b"Arguments do not match string read by key '%r': %r", key, args)
            return key
        ...
        try:
            ...
            _logger.warning(b"Arguments do not match string read by key '%s': %s", key, kwargs)
            return key
...
def makeStringJSON(key, argsStr):
    try:
        ...
        for v in args:
            if isinstance(v, str):
                utf8args.append(v.encode(b'utf-8'))
            else:
                utf8args.append(v)
        ...
        _logger.exception(ex)
        _logger.warning(b'Failed to translate JSON-encoded string to dict or list: %r, %r', key, argsStr)
        return key
