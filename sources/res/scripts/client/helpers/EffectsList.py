...
class _EffectDesc(object):
    ...
    def __init__(self, dataSection):
        ...
        if nodeName:
            self._nodeName = [intern(name) for name in nodeName.split(b'/')]
        else:
            self._nodeName = []
        return
...
class _PixieEffectDesc(_EffectDesc):
    ...
    def reattach(self, elem, model):
        ...
        if newPos is not None:
            nodePos = newPos[0].split(b'/') if newPos[0] else []
...
class _ModelEffectDesc(_EffectDesc):
    ...
    def reattach(self, elem, model):
        ...
        if newPos is not None:
            nodeName = newPos[0].split(b'/') if newPos[0] else []
    ...
    def create(self, model, effects, args):
        ...
        if self._stopSyncVisual:
            ...
            if SoundGroups.DEBUG_TRACE_STACK is True:
                ...
                traceback.print_stack()
            if idd == 0:
                _logger.error(b'Failed to start sound effect, event %s', soundName)
        effects.append(elem)
        return
    ...
    def reattach(self, elem, model):
        if not _ALLOW_DYNAMIC_LIGHTS:
            ...
            if elem[b'newPos'] is not None:
                nodePos = elem[b'newPos'][0].split(b'/') if elem[b'newPos'][0] else []
...
def _getHitPoint(model, nodeName, args):
    ...
    if nodeDesc is not None:
        nodeName = nodeDesc[0].split(b'/') if nodeDesc[0] else []
