import logging, random, string, AnimationSequence, BigWorld, Math, WWISE, DecalMap, SoundGroups, helpers, material_kinds
...
class EffectsList(object):
    ...
    def detachAllFrom(self, data, keepPosteffects=False):
        ...
        if effects is None:
            ...
            if keepPosteffects:
                for elem in effects[:]:
                    if elem[b'typeDesc'].delete(elem, 2):
                        effects.remove(elem)

                return
            for elem in effects[:]:
                elem[b'typeDesc'].delete(elem, 0)
                effects.remove(elem)
...
class _ContinuousShotSoundEffectDesc(_ShotSoundEffectDesc):
    __slots__ = (b'_soundName', b'__pcSoundObjectIDs')
    TYPE = b'_ContinuousShotSoundEffectDesc'

    def __init__(self, dataSection):
        super(_ContinuousShotSoundEffectDesc, self).__init__(dataSection)
        self._soundName = (
         (
          dataSection.readString(b'soundStart/wwsoundPC', b''),
          dataSection.readString(b'soundStop/wwsoundPC', b'')),
         (
          dataSection.readString(b'soundStart/wwsoundNPC', b''),
          dataSection.readString(b'soundStop/wwsoundNPC', b'')))
...
class _NodeSoundEffectDesc(_BaseSoundEvent):
    ...
    effectUniqId = 0
    __slots__ = (b'_soundName', b'effectUniqId', b'_dopplerEffect')
...
class _TracerSoundEffectDesc(_NodeSoundEffectDesc):
    __slots__ = (b'_parameters', b'__stopEventSound', b'__tracerDelaySound')
    ...
    def _setStopEventsName(self, dataSection):
        stopSoundEventNames = dataSection[b'tracerStopSound']
    ...
    def create(self, model, effects, args):
        ...
        soundName, _ = self._getName(args)
        useStopEvent = bool(soundName and not soundName[0].startswith(b'flamer'))
        effectsCount = len(effects)
        soundObject = super(_TracerSoundEffectDesc, self).create(model, effects, args)
        if soundObject is None:
            return
        else:
            if len(effects) <= effectsCount:
                _logger.error(b'Tracer sound effect was created without registering effect elem')
                return soundObject
            elem = effects[-1]
            elem[b'tracerIsPlayer'] = isPlayer
            elem[b'useTracerStopEvent'] = useStopEvent
    ...
    def delete(self, elem, reason):
        ...
        if soundObject is not None:
            ...
            stopSound = None
            if elem.get(b'useTracerStopEvent', False):
                stopSound = self.__stopEventSound.getSound(elem.get(b'tracerIsPlayer', False), reason)
...
class _TracerStopEventSound(object):
    _TracerSounds = namedtuple(b'TracerSounds', b'stopPC stopNPC stopFlyOut')
    _EXPLOSION_REASON = 1
    __slots__ = (b'_soundsName',)

    def __init__(self, dataSection):
        defaultSounds = self._TracerSounds(stopPC=b'psb_pc_stop', stopNPC=b'psb_npc_stop', stopFlyOut=None)
        self._soundsName = defaultSounds
        if dataSection:
            self._soundsName = self._TracerSounds(stopPC=dataSection.readString(b'wwstopSoundPC', defaultSounds.stopPC), stopNPC=dataSection.readString(b'wwstopSoundNPC', defaultSounds.stopNPC), stopFlyOut=dataSection.readString(b'wwstopSoundFlyOut', defaultSounds.stopFlyOut))
        return
...
class _DecalEffectDesc(_EffectDesc):
    ...
    def create(self, model, effects, args):
        ...
        BigWorld.addDecal(self._groupName, center - extent, center + extent, size, (self._randomYaw or args)[b'yaw'] if 1 else random.uniform(0.0, 3.14), DecalMap.g_instance.getIndex(texName), DecalMap.g_instance.getIndex(bumpTexName), DecalMap.g_instance.getIndex(smTexName))
        return
...
class _LightEffectDesc(_EffectDesc):
    ...
    def reattach(self, elem, model):
        if not _ALLOW_DYNAMIC_LIGHTS:
            ...
            elem[b'model'] = model
            elem[b'node'] = _findTargetNode(model, nodePos)
            if elem[b'light'] is not None:
                elem[b'light'].source = elem[b'node'].actualNode
            return
    ...
    def _callbackCreate(self, elem):
        ...
        light.source = elem[b'node'].actualNode
...
class _TracerDelaySound(object):
    ...
    def create(self, elem, soundObject, args):
        ...
        if entityID > 0 and self.__needPlay(collisionTime, entityID):
            self.delete(elem)
            elem[b'tracerDelaySoundData'] = {b'soundObject': soundObject, 
               b'soundCallback': (BigWorld.callback(collisionTime - self.__soundDelayBeforeEnd, partial(self.__playSound, elem, self.__getSoundName(entityID))))}
            return True
        else:
            return False

    def delete(self, elem):
        data = elem.pop(b'tracerDelaySoundData', None)
        if data is not None:
            soundCallback = data.get(b'soundCallback', None)
    ...
    def __playSound(self, elem, soundName):
        ...
        data = elem.get(b'tracerDelaySoundData', None)
        if data is None:
            return
        else:
            soundObject = data.get(b'soundObject', None)
            if soundObject is not None:
                soundObject.play(soundName)
            data[b'soundCallback'] = None
            return
...
class _AutoShootEffectDesc(_EffectDesc):
    __slots__ = (b'effectsPrefab', b'activationSound', b'deactivationSound', b'shellEjectionSwitch')
    TYPE = b'_AutoShootEffectDesc'
    _SWITCH_NAME = b'SWITCH_ext_shell_ejection_autoshoot'

    def __init__(self, dataSection):
        super(_AutoShootEffectDesc, self).__init__(dataSection)
        self.effectsPrefab = dataSection.readString(b'effectsPrefab')
        self.activationSound = sound_readers.readWWTripleSoundConfig(dataSection[b'activationSound'])
        self.deactivationSound = sound_readers.readWWTripleSoundConfig(dataSection[b'deactivationSound'])
        self.shellEjectionSwitch = b''
...
class _FlamethrowerRibbonEffectDesc(_EffectDesc):
    __slots__ = (b'_id',)
    TYPE = b'_FlamethrowerRibbonEffectDesc'
...
_effectDescFactory = {b'pixie': _PixieEffectDesc, 
   ...
   b'continuousShotSound': _ContinuousShotSoundEffectDesc, 
   ...
   b'autoShoot': _AutoShootEffectDesc, 
   b'flamethrowerRibbon': _FlamethrowerRibbonEffectDesc}
