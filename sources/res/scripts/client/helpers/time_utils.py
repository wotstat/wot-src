...
class _TimeCorrector(object):
    ...
    def serverRegionalTime(self):
        regionalNewDayUTC = 0
        try:
            serverRegionalSettings = BigWorld.player().serverSettings[b'regional_settings']
            regionalNewDayUTC = serverRegionalSettings[b'starting_time_of_a_new_day']
        except Exception:
    ...
    def serverGameDayTime(self):
        ...
        try:
            serverRegionalSettings = BigWorld.player().serverSettings[b'regional_settings']
            regionalNewGameDayUTC = serverRegionalSettings[b'starting_time_of_a_new_game_day']
        except Exception:
            LOG_CURRENT_EXCEPTION()
...
def getServerRegionalDaysLeftInGameWeek():
    ...
    if b'starting_day_of_a_new_week' in regionalSettings:
        weekStartDayOffset = regionalSettings[b'starting_day_of_a_new_week']
...
def getServerGameDay():
    ...
    if b'starting_time_of_a_new_game_day' in regionalSettings:
        dayStartOffset = regionalSettings[b'starting_day_of_a_new_week']
    elif b'starting_time_of_a_new_day' in regionalSettings:
        dayStartOffset = regionalSettings[b'starting_time_of_a_new_day']
...
def getTillTimeString(timeValue, keyNamespace=b'', isRoundUp=False, sourceStrGenerator=None, removeLeadingZeros=False):
    ...
    fmtValues = {b'day': (str(datetime.timedelta(seconds=timeValue).days) if timeValue > ONE_YEAR else str(tm.tm_yday)), 
    ...
    return _ms((b'{}/{}').format(keyNamespace, fmtKey), **fmtValues)
...
_ISO8601_UTC_ONLY_PATTERN = re.compile(b'^(?P<timestamp>\\d{4}-\\d\\d-\\d\\dT\\d\\d:\\d\\d:\\d\\d)(?P<microseconds>.\\d+)?(?:z|\\+00:00)?$', re.IGNORECASE)

def getTimestampFromISO(timeISO):
    matchObject = _ISO8601_UTC_ONLY_PATTERN.match(timeISO)
    if matchObject is None:
        raise SoftException((b'Invalid timestamp "{}", use ISO 8601 format without time zone instead.').format(timeISO))
    groupDict = matchObject.groupdict()
    timestamp = groupDict[b'timestamp']
    microseconds = groupDict[b'microseconds']
    dt = datetime.datetime.strptime(timestamp, b'%Y-%m-%dT%H:%M:%S')
...
def getTimeLeftFormat(timeLeft, useMinutes=True, useHours=False):
    templateParts = [b'%S']
    if useMinutes:
        templateParts.insert(0, b'%M')
    if useHours:
        templateParts.insert(0, b'%H')
    template = (b':').join(templateParts)
