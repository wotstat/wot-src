...
SurfaceMaterial = namedtuple(b'SurfaceMaterial', (b'point', b'surfaceNormal', b'matKind', b'effectIdx'))

def calcSurfaceMaterialNearPoint(point, normal, spaceID, defaultEffectMaterial=b'ground'):
            ...
            if desc is not None:
                ...
                if descType == DESTR_TYPE_STRUCTURE:
                    ...
                    if moduleDesc is not None:
                        effectIdx = moduleDesc.get(b'effectMtrlIdx')
            else:
                matKind = 0
        else:
...
def isDestructibleBroken(chunkID, itemIndex, matKind, itemFilename):
    ...
    if desc is None:
        ...
        if ctrl is None:
            return False
        return ctrl.isDestructibleBroken(itemIndex, matKind, desc[b'type'])
