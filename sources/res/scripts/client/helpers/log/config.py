...
def setupFromXML(envKey=ENV_KEY, filename=XML_CFG_FILE, level=logging.INFO):
    ...
    if value:
        ...
        config = {}
    if config:
        if config.get(b'override_exception_hook', False):
...
def readXMLConfig(filename):
    ...
    if root is None:
        ...
        sectionCtx, section = resource_helper.getSubSection(ctx, root, b'common', safe=False)
        if section is not None:
            config.update(resource_helper.readDict(sectionCtx, section).value)
        for key in (b'filters', b'formatters', b'handlers', b'loggers'):
