import math, logging, sys, BigWorld, GUI, Keys, Math, ResMgr, game_mode_emulator
_logger = logging.getLogger(__name__)
g_offlineModeEnabled = b'offline' in sys.argv
...
CAMERAS_XML_NAME = ResMgr.appDirectory() + b'offline_mode_cameras.xml'
...
def _loadCameraTransforms():
    ...
    _logger.info(b'Loaded %s', CAMERAS_XML_NAME)
    return


def _saveCameraTransforms():
    rootDS = ResMgr.openSection(CAMERAS_XML_NAME, True)
    rootDS.deleteSection(b'cameras')
    camerasDS = rootDS.createSection(b'cameras')
    for c in g_cameraTransforms:
        ds = camerasDS.createSection(b'camera')
        ds.writeMatrix(b'transform', c.matrix)

    rootDS.save()
    rootDS = None
    _logger.info(b'Saved %s', CAMERAS_XML_NAME)
    return
...
def onStartup():
    try:
        idx = sys.argv.index(b'offline')
        offlineSpace = sys.argv[idx + 1]
        launch(offlineSpace)
        return True
    except (ValueError, IndexError) as e:
        BigWorld.criticalExit((b'Failed to launch offline mode: {}').format(e.message))
        return False
...
def _displayGUI(spaceName):
    global g_gui
    _clearGUI()
    g_gui = GUI.Text(b"Entering offline space '%s'...\n%s" % (spaceName, INSTRUCTIONS))
