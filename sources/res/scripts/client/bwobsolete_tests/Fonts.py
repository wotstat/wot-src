...
def _showFont(fontName, pos=(-1, -1, 0.5), size=(2, 2)):
    ...
    s.textureName = fontName.lower() + b'.dds'
    s.position = pos
    ...
    s.label.font = b'system_small.font'
...
def getAllFontNames():
    l = []
    all = ResMgr.openSection(b'system/fonts')
    for name, ds in all.items():
        if name.endswith(b'.font'):
            l.append(name)
...
def showFDFont():
    ...
    s.textureName = b'fantasydemofont_20_font_rt'
    s.size = (2, 2)
    return
...
def testResize1():
    t = GUI.Text(b'resize')
    t.font = b'resize.font'
    GUI.addRoot(t)
    fontTestGUIs.append(t)
    t.verticalAnchor = GUI.Simple.eVAnchor.BOTTOM
    _showFont(b'resize.font')
    return t


def testResize2():
    t2 = GUI.Text(b'resizebgman')
    t2.font = b'resize.font'
    GUI.addRoot(t2)
