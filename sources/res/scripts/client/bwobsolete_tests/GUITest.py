import math, BigWorld, GUI, Math, Keys, random
...
def mode_conv():
    ...
    def add_conv_test_child(p, hAnchor, vAnchor, pos):
        c = GUI.Simple(b'system/maps/col_white.bmp')
        ...
        p.addChild(c)
        _setAllModeCombinations(c, b'horizontalPositionMode')
        _setAllModeCombinations(c, b'verticalPositionMode')
        _setAllModeCombinations(c, b'widthMode')
        _setAllModeCombinations(c, b'heightMode')
        return

    clear()
    w = GUI.Window(b'system/maps/col_white.bmp')
    w.position = (0, 0.25, 1)
    _setAllModeCombinations(w, b'horizontalPositionMode')
    _setAllModeCombinations(w, b'verticalPositionMode')
    _setAllModeCombinations(w, b'widthMode')
    _setAllModeCombinations(w, b'heightMode')
...
class _Child(object):
    ...
    def handleMouseEnterEvent(self, comp):
        self.component.parent.hoverLabel.text = b'< !! >'
        return False
...
class _ParentAnchorWindow(object):
    ...
    def handleMouseEnterEvent(self, comp):
        self.component.hoverLabel.text = b'!!'
        return False
    ...
    def create(hAnchor, vAnchor, pos, colour):
        ...
        label = GUI.Text(b'%s, %s' % (hAnchor, vAnchor))
        label.font = b'default_smaller.font'
        ...
        hoverLabel = GUI.Text(b'')
        hoverLabel.colour = (255, 128, 64, 255)
        c.addChild(hoverLabel, b'hoverLabel')
...
class _FocusableComponent:
    ...
    def __init__(self):
        self.component = GUI.Simple(b'system/maps/col_white.bmp')
        self.component.script = self
    ...
    def handleKeyEvent(self, event):
        print b'handleKeyEvent', BigWorld.keyToString(event.key), event.isKeyDown()
        if event.key == Keys.KEY_SPACE:
            self.component.colour = (
             random.random() * 255, random.random() * 255, random.random() * 255, 255)
            return True
        return False

    def handleMouseButtonEvent(self, comp, event):
        print b'handleMouseButtonEvent', BigWorld.keyToString(event.key), event.isKeyDown(),
        if self.component.focus == False:
            print b'Setting focus to True!'
            self.component.focus = True
            self.component.colour = (128, 128, 12, 255)
        else:
            print b"We're already in focus."
        return True
...
class _GameSquare:
    ...
    def handleMouseEnterEvent(self, comp):
        if self.gameStarted:
            t = GUI.Text(b'YOU LOSE! Time: %.2f sec.' % (BigWorld.time() - self.gameStartTime))
    ...
    def create():
        ...
        c.colour = (
         ...
         255)
        c.position = (
         random.random() * 2 - 1, random.random() * 2 - 1, 1)
...
class _OrderStackingBlock:
    ...
    def create(position, colour):
        c = GUI.Simple(b'helpers/maps/col_%s.bmp' % colour)
        c.materialFX = GUI.Simple.eMaterialFX.SOLID
        c.size = (0.5, 0.5)
        c.position = position
        c.focus = True
...
class _OrderChangingButton:
    ...
    def create(backComponent, resortFn):
        c = GUI.Text(b'Click here to move black box forward')
...
def localReSort():
    clear()
    ourRoot = _OrderStackingBlock.create((0.0, 0.0, 0.0), b'white').component
    GUI.addRoot(ourRoot)
    red = _OrderStackingBlock.create((0.0, -0.2, 0.5), b'red').component
    ourRoot.addChild(red)
    yellow = _OrderStackingBlock.create((0.2, 0.0, 0.6), b'yellow').component
    ourRoot.addChild(yellow)
    black = _OrderStackingBlock.create((-0.2, 0.0, 0.7), b'black').component
...
class _MouseEventTester1:
    ...
    def handleMouseEnterEvent(self, component):
        self.label1.text = b'handleMouseEnter'
        return True

    def handleMouseLeaveEvent(self, component):
        self.label1.text = b'handleMouseLeave'
        return True

    def handleMouseClickEvent(self, component):
        self.label1.text = b'handleMouseClickEvent'
        return True

    def handleMouseButtonEvent(self, component, event):
        self.label1.text = b'handleMouseButtonEvent'
        return True

    def handleDragStartEvent(self, component):
        self.label3.text = b'handleDragStartEvent'
        return True

    def handleDragStopEvent(self, component):
        self.label3.text = b'handleDragStopEvent'
        return True

    def handleDragEnterEvent(self, component, dragged):
        self.label3.text = b'handleDragEnterEvent'
        return True

    def handleDragLeaveEvent(self, component, dragged):
        self.label3.text = b'handleDragLeaveEvent'
        return True

    def handleDropEvent(self, component, dragged):
        self.label3.text = b'handleDropEvent'
        return True

    def handleMouseEvent(self, comp, event):
        self.label2.visible = True
        self.label2.text = b'handleMouseEvent (%s %s %s)' % (event.dx, event.dy, event.dz)
    ...
    def create(parent, pos):
        s = GUI.Window(b'system/maps/col_white.bmp')
...
class _MouseEventTester2:
    ...
    def handleKeyEvent(self, event):
        self.label3.text = b'handleKeyEvent (%s, %s)' % (BigWorld.keyToString(event.key), event.isKeyDown())
        return False
