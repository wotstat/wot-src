import BigWorld, Math, GUI, math
from functools import partial
preload = BigWorld.Model(b'helpers/models/hemisphere.model')
preload2 = BigWorld.Model(b'helpers/models/entity_arrow.model')
preload3 = BigWorld.Model(b'helpers/models/directional_arrow.model')
...
class ActionDisplay:
    ...
    def __init__(self):
        ...
        self.actionText = GUI.Text(b'')
        self.guiAttachment.component = self.actionText
        return

    def angleGizmo(self, red=False):
        m = BigWorld.Model(b'helpers/models/hemisphere.model')
        m.gen_shadow = [b'green', b'red'][red]
        return m

    def angleGizmo2(self, red=False):
        m = BigWorld.Model(b'helpers/models/hemisphere.model')
        m.gen_shadow = [b'green_hollow', b'red_hollow'][red]
        return m

    def rulerGizmo(self, red=False):
        f = GUI.Frame2(b'helpers/maps/ruler.tga')
    ...
    def setAngles(self, g, min, max):
        ...
        g.gen_shadow.uTransform_diffuse = (
         m.get(0, 0), m.get(1, 0), 0, m.get(3, 0))
        g.gen_shadow.vTransform_diffuse = (m.get(0, 1), m.get(1, 1), 0, m.get(3, 1))
        g.gen_shadow.uTransform = (n.get(0, 0), n.get(1, 0), 0, n.get(3, 0))
        g.gen_shadow.vTransform = (n.get(0, 1), n.get(1, 1), 0, n.get(3, 1))
        return
    ...
    def showActionInfo(self, entity, actionName):
        ...
        try:
            ...
            self.actionText.text = b'Unmatched'
            return
    ...
    def addMatchedActionLabel(self):
        ...
        self.actionText.font = b'Heading.font'
...
def vizModel():
    m = BigWorld.Model(b'helpers/models/entity_arrow.model')
    return m


def transformVizModel():
    m = BigWorld.Model(b'helpers/models/basic_entity_arrow.model')
    return m


def velocityVizModel():
    m = BigWorld.Model(b'helpers/models/directional_arrow.model')
    return m
...
def dumpActionInfo(entity):
    ...
    for a in actions:
        ...
        print a
        print b'----------------------------------------------------'
        print b'trigger_minEntitySpeed', mi.trigger_minEntitySpeed
        print b'trigger_maxEntitySpeed', mi.trigger_maxEntitySpeed
        print b'trigger_minEntityAux1', mi.trigger_minEntityAux1
        print b'trigger_maxEntityAux1', mi.trigger_maxEntityAux1
        print b'trigger_minModelYaw', mi.trigger_minModelYaw
        print b'trigger_maxModelYaw', mi.trigger_maxModelYaw
        print b'cancel_minEntitySpeed', mi.cancel_minEntitySpeed
        print b'cancel_maxEntitySpeed', mi.cancel_maxEntitySpeed
        print b'cancel_minEntityAux1', mi.cancel_minEntityAux1
        print b'cancel_maxEntityAux1', mi.cancel_maxEntityAux1
        print b'cancel_minModelYaw', mi.cancel_minModelYaw
        print b'cancel_maxModelYaw', mi.cancel_maxModelYaw
...
def testSSO(entity=None):
    if entity == None:
        entity = BigWorld.player()
    m = BigWorld.Model(b'art/characters/foe/guard/guard.model')
    BigWorld.player().model = m
