    ...
    def validate(self, callback=None):
        ...
        for handler in handlers:
            try:
                ...
                _logger.error(b'There is exception while validating item %s', e.itemData)
                callback(e.code)
                return
...
class InventoryVehiclesValidator(AccountValidator):
    ...
    def __validateInventoryVehicles(self):
        ...
        yield wg_async.wg_await(wg_async.distributeLoopOverTicks(createVehicleDescrAsync(), minPerTick=10, maxPerTick=100, logID=b'createVehicleDescrAsync', tickLength=0.0))
        ...
        yield wg_async.wg_await(wg_async.distributeLoopOverTicks(validateTankmanAsync(), minPerTick=10, maxPerTick=100, logID=b'validateTankmanAsync', tickLength=0.0))
        return
...
class InventoryOutfitValidator(AccountValidator):
    ...
    def __validateInventoryOutfit(self):
        ...
        yield wg_async.wg_await(wg_async.distributeLoopOverTicks(validateOutfitsAsync(), minPerTick=10, maxPerTick=100, logID=b'validateOutfitsAsync', tickLength=0.0))
        return
...
class InventoryTankmenValidator(AccountValidator):
    ...
    def __validateInventoryTankmen(self):
        ...
        yield wg_async.wg_await(wg_async.distributeLoopOverTicks(validateInventoryTankmenAsync(), minPerTick=10, maxPerTick=100, logID=b'validateOutfitsAsync', tickLength=0.0))
        return
