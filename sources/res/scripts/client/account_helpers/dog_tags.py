...
class DogTags(object):
    ...
    def synchronize(self, isFullSync, diff):
        if isFullSync:
            self.__cache.clear()
        dataResetKey = (DT_PDATA_KEY, b'_r')
    ...
    def _makeDisplayableDT(clanProfile, playerDT):
        clanTag = clanProfile.getClanAbbrev() if clanProfile and clanProfile.isInClan() else b''
    ...
    def __handleSynchNotifications(self, dogTagsDiff):
        if PlayerDogTagStorage.key in dogTagsDiff:
            _logger.info(b'UPDATE Player DT: %r', self.__cache[PlayerDogTagStorage.key])
        if UnlockedComponentsStorage.key in dogTagsDiff:
            _logger.info(b'UPDATE unlocked components: %r', self.__cache[UnlockedComponentsStorage.key])
        return

    def __handleResetNotifications(self, resetData):
        _logger.info(b'RESET Player DT: %s', resetData)
        return
