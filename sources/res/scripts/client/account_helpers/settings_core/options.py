...
SettingsExtraData = namedtuple(b'SettingsExtraData', b'current options extraData')
...
class SettingAbstract(ISetting):
    PackStruct = namedtuple(b'PackStruct', b'current options')
...
class EnablingSettingMixin(object):
    ...
    def pack(self):
        return SettingsExtraData(self._get(), self._getOptions(), {b'enabled': (self._isEnabled())})._asdict()
...
class SettingsContainer(ISetting):
    ...
    def getSetting(self, name):
        ...
        LOG_WARNING(b"Failed to get a value of setting as it's not in indices: ", name)
...
class VOIPMasterSoundSetting(SoundSetting):
    ...
    def __init__(self, isPreview=False):
        super(VOIPMasterSoundSetting, self).__init__(b'masterVivox', isPreview)
        return
...
class VOIPMicSoundSetting(SoundSetting):
    ...
    def __init__(self, isPreview=False):
        super(VOIPMicSoundSetting, self).__init__(b'micVivox', isPreview)
        return
...
class StorageSetting(RegularSetting):
    ...
    def _set(self, value):
        ...
        setting = {b'option': (self.settingName), 
           b'value': value}
...
class UserPrefsBoolSetting(UserPrefsSetting):
    ...
    def _writeValue(self, section, value):
        if section is not None:
            ...
            _logger.warning(b'Section is not defined %s', section)
            return False
...
class PostMortemModeSetting(StorageDumpSetting):
    ...
    def _getOptions(self):
        settingsKey = b'#settings:game/%s/%s'
...
class VOIPSetting(AccountSetting):
    ...
    def __init__(self, isPreview=False):
        super(VOIPSetting, self).__init__(b'enableVoIP')
        self.isPreview = isPreview
        return
    ...
    def _set(self, isEnable):
        if isEnable is None:
            ...
            if prevVoIP != isEnable:
                ...
                LOG_NOTE(b'Change state of voip:', isEnable)
            return
...
class VOIPChannelSetting(UserPrefsInt64Setting):
    ...
    def _set(self, value):
        ...
        VOIP.getVOIPManager().applyChannelSetting(isEnabled, channelHash)
        LOG_NOTE(b'Change state of channel voip:', isEnabled, channelHash)
        return
...
class VOIPSupportSetting(ReadOnlySetting):
    ...
    def __isSupported(self):
        return VOIP.getVOIPManager().getVOIPDomain() != b'' and self.__isVoiceChatReady()
...
class MessengerDateTimeSetting(MessengerSetting):
    ...
    def __init__(self, bit, storage=None):
        super(MessengerDateTimeSetting, self).__init__(b'datetimeIdx', storage)
        self.bit = bit
        return
...
class GameplaySetting(StorageAccountSetting):
    ...
    def _set(self, value):
        if not self.__callable():
            LOG_WARNING(b'GameplaySetting is disabled', self.gameplayName)
            return
        settingValue = super(GameplaySetting, self)._get()
        settingValue ^= self.bit
        if value:
...
class GraphicSetting(SettingAbstract):
    ...
    def _getOptions(self):
        if self._currentValue is not None:
            ...
            for label, data, advanced, supported in self._currentValue.options:
                options.append({b'label': (b'#settings:graphicsSettingsOptions/%s' % str(label)), 
                   b'data': data, 
                   b'advanced': advanced, 
                   b'supported': supported})

            options = sorted(options, key=itemgetter(b'data'), reverse=True)
            return options
        else:
            return
    ...
    def _set(self, value):
        ...
        if originalValue != value:
            LOG_NOTE(b'Adjusted value has been set: `%s`' % value)
        return
    ...
    def _tryToSetValue(self, value):
        try:
            ...
            LOG_ERROR(b'Unable to set value `%s` for option `%s`' % (value, self.name))
            return False
...
class IncreaseEffectsContrastEnabledSetting(GraphicSetting):
    ...
    def pack(self):
        return {b'current': (self._get()), 
           b'options': (self._getOptions()), 
           b'extraData': {b'enabled': (self._switchEnabled())}}
...
class IGBHardwareAccelerationSetting(UserPrefsBoolSetting):
    ...
    def getDefaultValue(self):
        return Settings.g_instance.engineConfig[b'webBrowser'][b'hardwareAcceleration'].asBool


class SniperModeSwingingSetting(GraphicSetting):

    def __init__(self):
        super(SniperModeSwingingSetting, self).__init__(b'SNIPER_MODE_SWINGING_ENABLED')
        return

    def __recreateCamera(self):
        inputHandler = BigWorld.player().inputHandler
        if hasattr(inputHandler, b'ctrl'):
            if inputHandler.ctrl == inputHandler.ctrls[b'sniper']:
...
class MonitorSetting(SettingAbstract):
    ...
    def pack(self):
        ...
        result.update({b'real': (g_monitorSettings.activeMonitor)})
        return result
...
class WindowSizeSetting(PreferencesSetting):
    ...
    def _getOptions(self):
        ...
        for monitorModes in self.__getSuitableWindowSizes():
            modes = [b'%dx%d' % (width, height) for width, height in monitorModes]
            currentSizeWidth, currentSizeHeight = self._storage.windowSize
            current = b'%dx%d' % (currentSizeWidth, currentSizeHeight)
            if current not in modes:
                modes.append(b'%s*' % current)
    ...
    def _savePrefsCallback(self, prefsRoot):
        if self.__lastSelectedWindowSize is not None and g_monitorSettings.isMonitorChanged:
            devPref = prefsRoot[b'devicePreferences']
            devPref.writeInt(b'windowedWidth', self.__lastSelectedWindowSize[0])
            devPref.writeInt(b'windowedWidth', self.__lastSelectedWindowSize[1])
        return
...
class ResolutionSetting(PreferencesSetting):
    ...
    def _getOptions(self):
        ...
        for resolutions in self._getSuitableResolutions():
            ...
            for width, height in resolutions:
                ...
                formatedRes.append((b'{0}x{1} [{2}:{3}]').format(width, height, widthOpt, heightOpt))
    ...
    def _savePrefsCallback(self, prefsRoot):
        if self._lastSelectedVideoMode is not None and g_monitorSettings.isMonitorChanged:
            devPref = prefsRoot[b'devicePreferences']
            devPref.writeInt(b'fullscreenWidth', self._lastSelectedVideoMode[0])
            devPref.writeInt(b'fullscreenHeight', self._lastSelectedVideoMode[1])
        return
...
class BorderlessSizeSetting(ResolutionSetting):
    ...
    def _savePrefsCallback(self, prefsRoot):
        if self._lastSelectedVideoMode is not None and g_monitorSettings.isMonitorChanged:
            devPref = prefsRoot[b'devicePreferences']
            devPref.writeInt(b'borderlessWidth', self._lastSelectedVideoMode[0])
            devPref.writeInt(b'borderlessHeight', self._lastSelectedVideoMode[1])
        return
...
class VideoModeSettings(PreferencesSetting):
    ...
    FULLSCREEN = 1
    BORDERLESS = 2
    OPTIONS = {WINDOWED: b'windowed', 
       FULLSCREEN: b'fullscreen', 
       BORDERLESS: b'borderless'}
    ...
    def _getOptions(self):
        ...
        for data, label in allowScreenModes:
            result.append({b'data': data, 
               b'label': (b'#settings:screenMode/%s' % label)})
    ...
    def _savePrefsCallback(self, prefsRoot):
        prefsRoot[b'devicePreferences'].writeInt(b'windowMode', self.__videoMode)
        return
...
class VehicleMarkerSetting(StorageAccountSetting):
    ...
    class OPTIONS(CONST_CONTAINER):
        ...
        class PARAMS(CONST_CONTAINER):
            ICON = b'Icon'
            LEVEL = b'Level'
            HP_INDICATOR = b'HpIndicator'
            DAMAGE = b'Damage'
            HP = b'Hp'
            VEHICLE_NAME = b'VehicleName'
            PLAYER_NAME = b'PlayerName'
            AIM_MARKER_2D = b'AimMarker2D'

        @classmethod
        def getOptionName(cls, mType, mOption):
            return b'marker%s%s' % (mType, mOption)
        ...
        for mType in self.OPTIONS.TYPES.ALL():
            for param in self.OPTIONS.PARAMS.ALL():
                ...
                if param == self.OPTIONS.PARAMS.HP and forcePackingHP:
                    marker[on] = self.PackStruct(value, [b'#settings:marker/hp/type%d' % mid for mid in xrange(4)])._asdict()
                else:
                    marker[on] = value
...
class AimSetting(StorageAccountSetting):
    ...
    class OPTIONS(CONST_CONTAINER):
        NET = b'net'
        NET_TYPE = b'netType'
        CENTRAL_TAG = b'centralTag'
        CENTRAL_TAG_TYPE = b'centralTagType'
        RELOADER = b'reloader'
        RELOADER_TIMER = b'reloaderTimer'
        CONDITION = b'condition'
        MIXING = b'mixing'
        MIXING_TYPE = b'mixingType'
        CASSETTE = b'cassette'
        GUN_TAG = b'gunTag'
        GUN_TAG_TYPE = b'gunTagType'
        ZOOM_INDICATOR = b'zoomIndicator'
    ...
    def _get(self):
        ...
        for option in self.OPTIONS.ALL():
            if option in self.VIRTUAL_OPTIONS:
                default = self._default[self.VIRTUAL_OPTIONS[option][0]][b'type']
            else:
                default = self._default[option][b'alpha']
            if BattleReplay.isPlaying():
    ...
    def pack(self):
        ...
        for vname, (name, optsLen) in self.VIRTUAL_OPTIONS.iteritems():
            if vname in result:
                types = [{b'loc': (makeString(b'#settings:aim/%s/type%d' % (name, i))), b'label': (b'#settings:aim/%s/type%d' % (name, i)), b'index': i} for i in xrange(int(optsLen))]
                types = sorted(types, key=itemgetter(b'loc'))
                for item in types:
                    item.pop(b'loc', None)
    ...
    def fromAccountSettings(self, settings):
        ...
        for key, value in settings.items():
            if key not in self.VIRTUAL_OPTIONS:
                result[key] = value[b'alpha']
                for k, v in self.VIRTUAL_OPTIONS.items():
                    if v[0] == key:
                        result[k] = value[b'type']
...
class SPGAimSetting(StorageDumpSetting):
    ...
    def getExtraData(self):
        ...
        tooltipR = labelR.dyn(b'tooltip') if labelR else None
        data = {b'checkBoxLabel': (backport.text(labelR()) if labelR else b'')}
        if tooltipR:
            data[b'tooltip'] = makeTooltip(backport.text(tooltipR.header()), backport.text(tooltipR.body()))
        return data

    def pack(self):
        return {b'current': (self._get()), 
           b'options': (self._getOptions()), 
           b'extraData': (self.getExtraData())}

    def getDefaultValue(self):
        return AccountSettings.getSettingsDefault(b'spgAim').get(self.settingName, None)


class _BaseAimContourSetting(StorageDumpSetting):
    _RES_ROOT = None
    _OPTIONS_NUMBER = None
    _LOW_QUALITY_PRESETS = (b'LOW',)
    _DEFAULT_VALUE = None
    ...
    def getDefaultValue(self):
        if self._isHighQualityPreset():
            ...
            return AccountSettings.getSettingsDefault(b'contour').get(self.settingName, None)
    ...
    def _getOptions(self):
        return [{b'data': value, b'label': (backport.text(self._RES_ROOT.dyn((b'type{}').format(value))()))} for value in xrange(self._OPTIONS_NUMBER)]
...
class ContourSetting(_BaseAimContourSetting):
    ...
    def getExtraData(self):
        return [{b'tooltip': (makeTooltip(body=backport.text(self._RES_ROOT.dyn((b'type{}').format(value)).tooltip())))} for value in xrange(self._OPTIONS_NUMBER)]
...
class SPGStrategicCamMode(StorageDumpSetting):
    ...
    def getExtraData(self):
        ...
        data = {b'label': (backport.text(R.strings.settings.aim.spg.spgStrategicCamMode()))}
        if tooltipR:
            data[b'tooltip'] = makeTooltip(backport.text(tooltipR.header()), backport.text(tooltipR.body()))
        return data
    ...
    def _getOptions(self):
        settingsKey = b'#settings:aim/spg/{}/{}'
...
class SPGAimEntranceMode(StorageDumpSetting):
    ...
    def getDefaultValue(self):
        return AccountSettings.getSettingsDefault(b'spgAim').get(self.settingName, SPGAimEntranceModeOptions.SETTINGS_OPTIONS.index(SPGAimEntranceModeOptions.LAST))

    def getExtraData(self):
        return {b'label': (backport.text(R.strings.settings.aim.spg.aimEntranceMode()))}
...
class CarouselTypeSetting(StorageDumpSetting):
    ...
    def _getOptions(self):
        settingsKey = b'#settings:game/%s/%s'
        return [{b'label': (settingsKey % (self.settingName, cType))} for cType in self.CAROUSEL_TYPES]
...
class CustomizationDisplayTypeSetting(StorageDumpSetting):
    ...
    class OPTIONS(CONST_CONTAINER):
        ...
        NOT_HISTORICAL = b'notHistorical'
        ALL = b'all'
    ...
    def _getOptions(self):
        settingsKey = b'#settings:game/%s/%s'
        return [{b'label': (settingsKey % (self.settingName, cType))} for cType in self.CONTENT_TYPES]
...
class ControlSetting(SettingAbstract):
    ControlPackStruct = namedtuple(b'ControlPackStruct', b'current default')
...
class MouseSetting(ControlSetting):
    CAMERAS = {(CTRL_MODE_NAME.POSTMORTEM): (
                                   ArcadeCamera.getCameraAsSettingsHolder, b'postMortemMode/camera'), 
       (CTRL_MODE_NAME.ARCADE): (
                               ArcadeCamera.getCameraAsSettingsHolder, b'arcadeMode/camera'), 
       (CTRL_MODE_NAME.SNIPER): (
                               SniperCamera.getCameraAsSettingsHolder, b'sniperMode/camera'), 
       (CTRL_MODE_NAME.DUAL_GUN): (
                                 DualGunCamera.getCameraAsSettingsHolder, b'sniperMode/camera'), 
       (CTRL_MODE_NAME.TWIN_GUN): (
                                 twin_gun_camera.getCameraAsSettingsHolder, b'sniperMode/camera'), 
       (CTRL_MODE_NAME.STRATEGIC): (
                                  StrategicCamera.getCameraAsSettingsHolder, b'strategicMode/camera'), 
       (CTRL_MODE_NAME.ARTY): (
                             ArtyCamera.getCameraAsSettingsHolder, b'artyMode/camera'), 
       (CTRL_MODE_NAME.KILL_CAM): (
                                 kill_cam_camera.getCameraAsSettingsHolder, b'killCamMode/camera'), 
       (CTRL_MODE_NAME.DEATH_FREE_CAM): (
                                       free_camera.getCameraAsSettingsHolder, b'freeVideoMode/camera')}

    def __init__(self, mode, setting, default, isPreview=False, masterSwitch=b''):
    ...
    def _set(self, value):
        if self._isDisabledByMasterSwitch():
            LOG_WARNING(b'Mouse setting is disabled', self.mode, self.setting)
            return
        else:
            camera = self.getCamera()
            if camera is None:
                LOG_WARNING(b'Error while applying mouse settings: empty camera', self.mode, self.setting)
                return
    ...
    def _isControlModeAccessible(self):
        return BigWorld.player() is not None and hasattr(BigWorld.player(), b'inputHandler') and hasattr(BigWorld.player().inputHandler, b'ctrls')
...
class MouseSensitivitySetting(MouseSetting):
    ...
    def __init__(self, mode, masterSwitch=b''):
        super(MouseSensitivitySetting, self).__init__(mode, b'sensitivity', 1.0, masterSwitch=masterSwitch)
        return
...
class DynamicFOVMultiplierSetting(MouseSetting):
    ...
    def __init__(self, isPreview=False):
        super(DynamicFOVMultiplierSetting, self).__init__(CTRL_MODE_NAME.ARCADE, b'fovMultMinMaxDist', self.getDefaultValue(), isPreview)
        return
...
class DynamicFOVSetting(UserPrefsStringSetting):
    ...
    def _get(self):
        try:
            ...
            LOG_ERROR(b'Could not load dynamic fov setting')
            return self.DEFAULT
...
class BackDraftInversionSetting(StorageControlSetting, StorageAccountSetting):
    ...
    def __init__(self, storage, isPreview=False):
        super(BackDraftInversionSetting, self).__init__(b'backDraftInvert', storage, isPreview)
        return
...
class KeyboardSetting(ControlSetting):
    ...
    def setSystemValue(self, value):
        ...
        if self.cmd == b'CMD_VOICECHAT_MUTE' and self.app is not None and self.app.gameInputManager is not None:
...
class KeyboardSettings(SettingsContainer):
    KEYS_LAYOUT = (
     (
      b'movement',
      (
       (b'forward', b'CMD_MOVE_FORWARD'),
       (b'backward', b'CMD_MOVE_BACKWARD'),
       (b'left', b'CMD_ROTATE_LEFT'),
       (b'right', b'CMD_ROTATE_RIGHT'),
       (b'auto_rotation', b'CMD_CM_VEHICLE_SWITCH_AUTOROTATION'),
       (b'block_tracks', b'CMD_BLOCK_TRACKS'))),
     (
      b'cruis_control',
      (
       (b'forward_cruise', b'CMD_INCREMENT_CRUISE_MODE'),
       (b'backward_cruise', b'CMD_DECREMENT_CRUISE_MODE'),
       (b'stop_fire', b'CMD_STOP_UNTIL_FIRE'))),
     (
      b'firing',
      (
       (b'fire', b'CMD_CM_SHOOT'),
       (b'chargeFire', b'CMD_CM_CHARGE_SHOT'),
       (b'lock_target', b'CMD_CM_LOCK_TARGET'),
       (b'lock_target_off', b'CMD_CM_LOCK_TARGET_OFF'),
       (b'alternate_mode', b'CMD_CM_ALTERNATE_MODE'),
       (b'toggleFlashlight', b'CMD_TOGGLE_ARMOR_FLASHLIGHT'),
       (b'trajectory_view', b'CMD_CM_TRAJECTORY_VIEW'),
       (b'reloadPartialClip', b'CMD_RELOAD_PARTIAL_CLIP'),
       (b'specialAbility', b'CMD_CM_SPECIAL_ABILITY'))),
     (
      b'vehicle_other',
      (
       (b'showHUD', b'CMD_TOGGLE_GUI'),
       (b'showQuestProgress', b'CMD_QUEST_PROGRESS_SHOW'),
       (b'frontlineSelfDestruction', b'CMD_REQUEST_RECOVERY'),
       (b'showPersonalReserves', b'CMD_SHOW_PERSONAL_RESERVES'))),
     (
      b'equipment',
      (
       (b'item01', b'CMD_AMMO_CHOICE_1'),
       (b'item02', b'CMD_AMMO_CHOICE_2'),
       (b'item03', b'CMD_AMMO_CHOICE_3'),
       (b'item04', b'CMD_AMMO_CHOICE_4'),
       (b'item05', b'CMD_AMMO_CHOICE_5'),
       (b'item06', b'CMD_AMMO_CHOICE_6'),
       (b'item07', b'CMD_AMMO_CHOICE_7'),
       (b'item08', b'CMD_AMMO_CHOICE_8'),
       (b'item09', b'CMD_AMMO_CHOICE_9'),
       (b'item00', b'CMD_AMMO_CHOICE_0'))),
     (
      b'team_communication',
      (
       (b'highlightLocation', b'CMD_CHAT_SHORTCUT_CONTEXT_COMMAND'),
       (b'highlightTarget', b'CMD_CHAT_SHORTCUT_CONTEXT_COMMIT'),
       (b'showRadialMenu', b'CMD_RADIAL_MENU_SHOW'),
       (b'help', b'CMD_CHAT_SHORTCUT_HELPME'),
       (b'reloading', b'CMD_CHAT_SHORTCUT_RELOAD'),
       (b'fallBack', b'CMD_CHAT_SHORTCUT_BACKTOBASE'),
       (b'affirmative', b'CMD_CHAT_SHORTCUT_AFFIRMATIVE'),
       (b'negative', b'CMD_CHAT_SHORTCUT_NEGATIVE'),
       (b'thankYou', b'CMD_CHAT_SHORTCUT_THANKYOU'))),
     (
      b'camera',
      (
       (b'camera_up', b'CMD_CM_CAMERA_ROTATE_UP'),
       (b'camera_down', b'CMD_CM_CAMERA_ROTATE_DOWN'),
       (b'camera_left', b'CMD_CM_CAMERA_ROTATE_LEFT'),
       (b'camera_right', b'CMD_CM_CAMERA_ROTATE_RIGHT'))),
     (
      b'voicechat',
      (
       (b'pushToTalk', b'CMD_VOICECHAT_MUTE'),
       (b'voicechat_enable', b'CMD_VOICECHAT_ENABLE'))),
     (
      b'minimap',
      (
       (b'sizeUp', b'CMD_MINIMAP_SIZE_UP'),
       (b'sizeDown', b'CMD_MINIMAP_SIZE_DOWN'),
       (b'visible', b'CMD_MINIMAP_VISIBLE'))))
    IMPORTANT_BINDS = (b'forward', b'backward', b'left', b'right', b'fire', b'item01', b'item02', b'item03', b'item04', b'item05', b'item06', b'item07', b'item08', b'item09', b'item00')
    KEYS_TOOLTIPS = {b'auto_rotation': b'SettingKeySwitchMode', 
       b'chargeFire': b'SettingsKeyChargeFire', 
       b'highlightLocation': b'SettingsKeyHighlightLocation', 
       b'highlightTarget': b'SettingsKeyHighlightTarget', 
       b'showRadialMenu': b'SettingsKeyShowRadialMenu', 
       b'specialAbility': b'SettingKeySpecialAbility'}
    __hiddenGroups = set()

    def __init__(self):
        if not GUI_SETTINGS.minimapSize:
            self.hideGroup(b'minimap', hide=True)
        if not GUI_SETTINGS.voiceChat:
            self.hideGroup(b'voicechat', hide=True)
        settings = [
         (
          b'keysLayout', ReadOnlySetting(self._getLayout)),
         (
          b'keysTooltips', ReadOnlySetting((lambda : self.KEYS_TOOLTIPS)))]
        for group in self._getLayout(True):
            for setting in group[b'values']:
                settings.append((setting[b'key'], KeyboardSetting(setting[b'cmd'])))
        ...
        for groupName, groupValues in cls.KEYS_LAYOUT:
            if not isFull:
                if groupName in cls.__hiddenGroups:
                    continue
            if groupName == b'firing' and not GUI_SETTINGS.spgAlternativeAimingCameraEnabled:
                groupValues = list(groupValues)
                del groupValues[4]
            if groupName == b'vehicle_other':
                ...
                if not progressEnabled:
                    groupValues = list(groupValues)
                    del groupValues[2]
            layout.append({b'key': groupName, 
               b'values': [cls.__mapValues(*x) for x in groupValues]})
    ...
    def hideGroup(cls, group, hide):
        if hide:
            LOG_DEBUG(b'Hide settings group', group)
            cls.__hiddenGroups.add(group)
        else:
            LOG_DEBUG(b'Reveal settings group', group)
...
class _BaseSoundPresetSetting(AccountDumpSetting):
    ...
    def setSystemValue(self, value):
        try:
            ...
            LOG_ERROR(self, b'Unsupported value: %s' % value)
...
class ArmorFlashlightEnabledSettings(AccountDumpSetting):
    ...
    def pack(self):
        return {b'current': (self._get()), 
           b'extraData': (self._getExtraData())}

    @staticmethod
    def _getExtraData():
        localeFolder = R.strings.settings.aim.armorFlashlight.enabled
        data = {b'label': (backport.text(localeFolder.label())), 
           b'enabled': (isArmorFlashlightEnabled()), 
           b'tooltip': (makeTooltip(header=backport.text(localeFolder.tooltip.header()), body=backport.text(localeFolder.tooltip.body())))}
        return data
...
class ArmorFlashlightColorSchemasSettings(AccountDumpSetting):
    ...
    def pack(self):
        return {b'current': (self._get()), 
           b'options': (self._getOptions()), 
           b'extraData': {b'enabled': (isArmorFlashlightEnabled())}}
    ...
    def _getOptions(self):
        ...
        for schema in config.colorSchemas:
            ...
            options.append({b'label': (backport.text(optionR.label())), 
               b'tooltip': (makeTooltip(header=backport.text(optionR.tooltip.header()), body=backport.text(optionR.tooltip.body()))), 
               b'id': (schema.name)})
...
class ArmorFlashlightFillSettings(AccountDumpSetting):
    ...
    def _getOptions(self):
        ...
        return [{b'label': (backport.text(localeFolder.dyn(pattern.name)())), b'id': (pattern.name)} for pattern in getArmorFlashlightConfig().patterns]
...
class ArmorFlashlightResolutionSettings(AccountDumpSetting):
    ...
    def _getOptions(self):
        ...
        return [{b'label': (backport.text(localeFolder.dyn(resolution.name)())), b'name': (resolution.name), b'data': index} for index, resolution in enumerate(getArmorFlashlightConfig().resolutions)]
...
class SoundDevicePresetSetting(_BaseSoundPresetSetting):
    ...
    def _getOptions(self):
        ...
        if selectedID in mapping:
            ...
            LOG_ERROR(b'Selected preset is unresolved', selectedID)
        ...
        for baseID, selectedID, isValid in iterator():
            options.append({b'id': baseID, 
               b'label': (SETTINGS.sounds_sounddevice(selectedID)), 
               b'image': ((b'../maps/icons/settings/{}.png').format(baseID)), 
               b'tooltip': (SETTINGS.sounds_sounddevice(selectedID)), 
               b'speakerId': selectedID, 
               b'showDeviceAlert': (not isValid)})
...
class SoundSpeakersPresetSetting(SettingAbstract):
    ...
    def _get(self):
        ...
        if presetID in presetIDs:
            ...
            LOG_ERROR(b'Index of selected preset is not found', presetID)
            return
    ...
    def _set(self, value):
        ...
        if value < len(presetIDs):
            ...
            LOG_ERROR(b'Index of selected preset is invalid', value)
        return
    ...
    def _getOptions(self):
        ...
        for guiID in _makeSoundGuiIDsSeq():
            ...
            options.append({b'id': guiID, 
               b'label': label, 
               b'tooltip': device, 
               b'isAutodetect': (guiID == ACOUSTICS.TYPE_AUTO)})
...
class PhysicsQualitySoundSettings(UserPrefsStringSetting):
    ...
    def _getString(self):
        presetName = self._readValue(Settings.g_instance.userPrefs[b'soundPrefs'])
    ...
    def _save(self, presetID):
        return self._writeValue(Settings.g_instance.userPrefs[b'soundPrefs'], self._SETTINGS[presetID])
    ...
    def _getOptions(self):
        ...
        for guiID in self._SETTINGS:
            options.append({b'data': guiID, 
               b'label': (backport.text(locale.dyn(guiID)()))})
...
class PreviewSoundSetting(AccountSetting):
    ...
    def playPreviewSound(self, eventIdx):
        ...
        if self.__previewSound is not None:
            ...
            if self._WWISE_EVENTS.index(playingEvent) != eventIdx:
                ...
                self.__playSound()
            elif playingEvent != self._USER_SOUND:
                self.clearPreviewSound()
                self.__previewSound = SoundGroups.g_instance.getSound2D(eventToPlay)
            self.__playSound()
        else:
    ...
    def _getOptions(self):
        ...
        for sample in self._WWISE_EVENTS:
            option = {b'label': ((b'#settings:sound/{}/{}').format(self.key, sample))}
            if sample == self._USER_SOUND:
                option.update(tooltip=b'#settings:sounds/%s' % self._USER_SOUND)
    ...
    def _getExtraData(self):
        ...
        for sample in self._WWISE_EVENTS:
            if sample == self._USER_SOUND:
                canPreview = bool(ResMgr.isFile(b'audioww/%s.mp3' % self._USER_SOUND))
            else:
                canPreview = True
...
class DetectionAlertSound(PreviewSoundSetting):
    _USER_SOUND = b'sixthSense'
    _WWISE_EVENTS = (b'lightbulb', b'lightbulb_02', _USER_SOUND)


class ArtyShotAlertSound(PreviewSoundSetting):
    _USER_SOUND = b'soundExploring'
    _WWISE_EVENTS = (b'artillery_lightbulb', _USER_SOUND)


class AltVoicesSetting(StorageDumpSetting):
    ALT_VOICES_PREVIEW = itertools.cycle((b'wwsound_mode_preview01', b'wwsound_mode_preview02', b'wwsound_mode_preview03'))
    DEFAULT_IDX = 0
    ...
    def playPreviewSound(self, soundMgr=None):
        if self.isSoundModeValid():
            ...
            if sndMgr is None:
                LOG_ERROR(b'GUI sound manager is not found')
                return
    ...
    def _getOptions(self):
        ...
        for sm in self.__getSoundModesList():
            options.append({b'label': (sm.description), 
               b'tooltip': ((b'#settings:sounds/altVoice/{}').format(sm.name))})
    ...
    def __applyRegularMode(self, mode):
        ...
        if specialVoice is not None and not specialVoice.onlyInNational:
            _logger.debug(b'Use %s as special voice instead %s', specialVoice.languageMode, mode)
            return True
        else:
...
class SniperZoomSetting(StorageAccountSetting):
    SNIPER_TYPES = [
     b'remember', b'double', b'quadruple', b'octuple']
...
class GraphicsQualityNote(SettingAbstract):
    ...
    def _get(self):
        ...
        if contentTypeName is not None:
            return (b'{0}{1}  {2}{3}').format(b"<font face='$FieldFont' size='13' color='#595950'>", i18n.makeString(contentTypeName), icons.info(), b'</font>')
        else:
            return b''
...
class GraphicsHigtQualityNote(SettingAbstract):
    ...
    def _get(self):
        if ResMgr.activeContentType() in self._GRAPHICS_QUALITY_TYPES:
            return (b'{0}{1}  {2}{3}').format(b"<font face='$FieldFont' size='13' color='#595950'>", i18n.makeString(SETTINGS.GRAPHICSQUALITYHDSD_SD), icons.alert(), b'</font>')
        return b''
...
class AnonymizerSetting(AccountDumpSetting):
    ...
    def __init__(self, settingName):
        super(AnonymizerSetting, self).__init__(settingName, settingName, b'anonymized')
        return
    ...
    def getExtraData(self):
        ...
        body = (b'{body}{vspace}{footer}').format(body=backport.text((tooltip.body.clan if isInClan else tooltip.body.noClan)()), vspace=b'\n\n' if footer else b'', footer=text_styles.neutral(footer))
        header = backport.text(tooltip.header())
        return {b'checkBoxLabel': (backport.text(R.strings.settings.game.anonymizer())), 
           b'tooltip': (makeTooltip(header, body)), 
           b'visible': (self.__ctrl.isEnabled), 
           b'enabled': (not (self.__ctrl.isInBattle or self.__ctrl.isRestricted))}
...
class ShowDamageIconSetting(StorageAccountSetting):
    ...
    def getExtraData(self):
        templateName = b'html_templates:lobby/tooltips/settings_show_damage_icon'
        showDamageIconTooltipContent = (
         makeHtmlString(templateName, b'ricochet'),
         makeHtmlString(templateName, b'trackDamage'),
         makeHtmlString(templateName, b'criticalDamage'),
         makeHtmlString(templateName, b'blocked'),
         makeHtmlString(templateName, b'spacedArmorBlocked'),
         makeHtmlString(templateName, b'trackBlocked'),
         makeHtmlString(templateName, b'wheelBlocked'),
         makeHtmlString(templateName, b'missArmor'))
        return {b'checkBoxLabel': (backport.text(R.strings.settings.game.showDamageIcon())), 
           b'tooltip': (makeTooltip(backport.text(R.strings.tooltips.showDamageIcon.header()), (b'<br/>').join(showDamageIconTooltipContent)))}
...
class IncreasedZoomSetting(StorageAccountSetting, MouseAffectedSetting):
    ...
    def getExtraData(self):
        zooms = self._mouseSettings[0].getCamera().getConfigValue(b'zooms')[-2:]
        ...
        return {b'checkBoxLabel': (i18n.makeString(SETTINGS.GAME_INCREASEDZOOM_BASE, zooms=zoomStr)), 
           b'tooltip': (makeTooltip(TOOLTIPS.INCREASEDZOOM_HEADER, i18n.makeString(TOOLTIPS.INCREASEDZOOM_BODY, zooms=zoomStr)))}
...
class GroupSetting(StorageDumpSetting):
    ...
    def _getOptionData(self, key):
        return {b'label': (self._getOptionLabel(key)), 
           b'data': key}
...
class DamageIndicatorTypeSetting(GroupSetting):
    OPTIONS = {0: b'standard', 
       1: b'extended'}

    def __init__(self, settingName, storage, isPreview=False):
        super(DamageIndicatorTypeSetting, self).__init__(settingName, storage, options=self.OPTIONS, settingsKey=b'#settings:feedback/tab/damageIndicator/type/%s', isPreview=isPreview)
        return
...
class DamageLogDetailsSetting(GroupSetting):
    ...
    SHOW_BY_ALT_PRESS = 1
    HIDE = 2
    _OPTIONS = {SHOW_ALWAYS: b'always', 
       SHOW_BY_ALT_PRESS: b'byAlt', 
       HIDE: b'hide'}

    def __init__(self, settingName, storage, isPreview=False):
        super(DamageLogDetailsSetting, self).__init__(settingName, storage, options=self._OPTIONS, settingsKey=b'#settings:feedback/tab/damageLogPanel/details/%s', isPreview=isPreview)
        return
...
class DamageLogEventTypesSetting(GroupSetting):
    ...
    ONLY_NEGATIVE = 2
    _OPTIONS = {ALL: b'both', 
       ONLY_POSITIVE: b'positive', 
       ONLY_NEGATIVE: b'negative'}

    def __init__(self, settingName, storage, isPreview=False):
        super(DamageLogEventTypesSetting, self).__init__(settingName, storage, options=self._OPTIONS, settingsKey=b'#settings:feedback/tab/damageLogPanel/eventTypes/%s', isPreview=isPreview)
        return
...
class DamageLogEventPositionsSetting(GroupSetting):
    ALL_BOTTOM = 0
    NEGATIVE_AT_TOP = 1
    _OPTIONS = {ALL_BOTTOM: b'bottom', 
       NEGATIVE_AT_TOP: b'topBottom'}

    def __init__(self, settingName, storage, isPreview=False):
        super(DamageLogEventPositionsSetting, self).__init__(settingName, storage, options=self._OPTIONS, settingsKey=b'#settings:feedback/tab/damageLogPanel/eventPositions/%s', isPreview=isPreview)
        return
...
class BattleBorderMapModeShow(GroupSetting):
    ...
    HIDE = 2
    ALWAYS_HIDE = 3
    OPTIONS = {SHOW_BY_ALT_PRESS: b'alt', 
       SHOW_ALWAYS: b'always', 
       HIDE: b'hide', 
       ALWAYS_HIDE: b'alwaysHide'}

    def __init__(self, settingName, storage, isPreview=False):
        super(BattleBorderMapModeShow, self).__init__(settingName, storage, options=self.OPTIONS, settingsKey=b'#settings:feedback/tab/borderMap/showMode/%s', isPreview=isPreview)
        return
...
class BattleBorderMapType(GroupSetting):
    ...
    TYPE_DOTTED = 1
    TYPE_HIDE = 2
    OPTIONS = {TYPE_WALL: b'wall', 
       TYPE_DOTTED: b'dotted', 
       TYPE_HIDE: b'hide'}

    def __init__(self, settingName, storage, isPreview=False):
        super(BattleBorderMapType, self).__init__(settingName, storage, options=self.OPTIONS, settingsKey=b'#settings:feedback/tab/borderMap/typeBorder/%s', isPreview=isPreview)
        return
...
class QuestsProgressViewType(GroupSetting):
    TYPE_STANDARD = 0
    TYPE_HIDE = 1
    OPTIONS = {TYPE_STANDARD: b'standard', 
       TYPE_HIDE: b'hidden'}

    def __init__(self, settingName, storage, isPreview=False):
        super(QuestsProgressViewType, self).__init__(settingName, storage, options=self.OPTIONS, settingsKey=b'#settings:feedback/tab/questsProgress/type/%s', isPreview=isPreview)
        return
...
class QuestsProgressDisplayType(GroupSetting):
    SHOW_ALL = 0
    PROGRESS_ONLY = 1
    OPTIONS = {SHOW_ALL: b'showAll', 
       PROGRESS_ONLY: b'showProgress'}

    def __init__(self, settingName, storage, isPreview=False):
        super(QuestsProgressDisplayType, self).__init__(settingName, storage, options=self.OPTIONS, settingsKey=b'#settings:feedback/tab/questsProgress/standardConditions/%s', isPreview=isPreview)
        return
...
class SwitchSetupsInLoadingSetting(AccountSetting):
    _PackStructure = namedtuple(b'SwitchSetupsInLoadingSettingData', b'current options extraData')
    _ENABLED_BY_DEFAULT = (b'LOW',)
    ...
    def getExtraData(self):
        return {b'enabled': (self.__postProgressionCtrl.isSwitchSetupFeatureEnabled())}
