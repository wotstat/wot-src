...
class SettingAbstract(ISetting):
    ...
    def getType(self):
        _logger.warning(b'The method must be inherited for correct settings logging.')
        return
...
class VOIPCaptureDevicesSetting(UserPrefsStringSetting):
    ...
    def _getOptions(self):
        return [i18n.encodeUtf8(device.split(b'|', 1)[1].decode(sys.getfilesystemencoding())) for device in self._getRawOptions()]
    ...
    def _set(self, value):
        ...
        if vm.getCaptureDevices():
            ...
            LOG_DEBUG(b'Selecting new capture device', device)
...
class RandomOnly10ModeSetting(StorageAccountSetting):
    ...
    def getExtraData(self):
        return {b'enabled': (self.lobbyContext.getServerSettings().isOnly10ModeEnabled())}
...
class HangarCrewWidgetSetting(StorageDumpSetting):
    ...
    def _getOptions(self):
        settingsKey = b'#settings:game/%s/%s'
        return [{b'label': (settingsKey % (self.settingName, cType))} for cType in self.HANGER_CREW_WIDGET_TYPES]
...
class MouseSetting(ControlSetting):
    CAMERAS = {(CTRL_MODE_NAME.POSTMORTEM): (
       ...
       (CTRL_MODE_NAME.DUAL_GUN): (
                                 DualGunCamera.getCameraAsSettingsHolder, b'dualGunMode/camera'), 
       (CTRL_MODE_NAME.SPG_ONLY_ARTY_MODE): (
                                           OnlyArtyCamera.getCameraAsSettingsHolder, b'flamethrowerMode/camera'), 
       (CTRL_MODE_NAME.ASSAULT_SPG): (
                                    AssaultCamera.getCameraAsSettingsHolder, b'assaultMode/camera')}
...
class KeyboardSettings(SettingsContainer):
    ...
    KEYS_TOOLTIPS = {b'auto_rotation': b'SettingKeySwitchMode', 
       ...
       b'showQuestProgress': b'SettingsKeyShowQuestProgress'}
    __hiddenGroups = set()
...
class SniperModeStabilization(StorageDumpSetting):
    ...
    @staticmethod
    def getExtraData():
        return {b'checkBoxLabel': (i18n.makeString(SETTINGS.GAME_HORSTABILIZATIONSNP)), 
           b'tooltip': (makeTooltip(TOOLTIPS.HORSTABILIZATIONSNP_HEADER, i18n.makeString(TOOLTIPS.HORSTABILIZATIONSNP_BODY)))}
...
class InterfaceScaleSetting(UserPrefsFloatSetting):
    ...
    def __getScaleLabels(sizes):
        ...
        for scale in scales:
            scaleFormat = b'{:.0f}' if scale % 1 == 0 else b'{:.2f}'
...
class GraphicsQualityNote(SettingAbstract):
    ...
    def _get(self):
        if ResMgr.activeContentType() in self._GRAPHICS_QUALITY_TYPES:
            return (b'{0}{1}  {2}{3}').format(b"<font face='$FieldFont' size='13' color='#595950'>", i18n.makeString(SETTINGS.GRAPHICSQUALITYHDSD_SD), icons.info(), b'</font>')
        return b''
...
class SixthSenseIndicatorSize(GroupSetting):
    ...
    def __init__(self, settingName, storage):
        super(SixthSenseIndicatorSize, self).__init__(settingName, storage, options=self.OPTIONS, settingsKey=b'#settings:feedback/tab/sixthSense/size/%s')
        return
...
class SwitchSetupsInLoadingSetting(AccountSetting):
    ...
    _ENABLED_BY_DEFAULT = (b'LOW', b'MIN')
