...
def _initializeDefaultSettings(core, data, initialized):
    ...
    if igrCtrl.getRoomType() == constants.IGR_TYPE.NONE:
        ...
        try:
            value = section[b'replayPrefs'].readBool(b'enabled', None)
            if value:
                gameData[GAME.REPLAY_ENABLED] = 2
            elif value is not None:
                gameData[GAME.REPLAY_ENABLED] = 0
        except Exception:
            LOG_DEBUG(b'Replay preferences is not available.')
...
def _migrateTo5(core, data, initialized):
    data[b'gameData'][GAME.POSTMORTEM_MODE] = True
    return
...
def _migrateTo28(core, data, initialized):
    data[b'gameExtData'][GAME.CAROUSEL_TYPE] = 1
    return
...
def _migrateTo65(core, data, initialized):
    battlehudData = data[b'battleHud']
...
def _migrateTo103(core, data, initialized):
    ...
    settingsPrefix = b'uiSpamVisited_{}'
    bitMask = 0
...
def _migrateTo115(core, data, initialized):
    data[b'battlePassStorage'][BattlePassStorageKeys.FLAGS_VERSION_HOLIDAY] = 0
    return
...
def _migrateTo116(core, data, initialized):
    from account_helpers.settings_core.ServerSettingsManager import GAME_EXTENDED_2
    if initialized:
        newbiesConfigs = {b'gameData': {(GAME.REPLAY_ENABLED): 1, 
                         (GAME.SNIPER_MODE_STABILIZATION): True, 
                         (GAME.SHOW_VEH_MODELS_ON_MAP): 2}, 
           b'gameExtData': {(GAME.CAROUSEL_TYPE): 0, 
           ...
           b'dogTags': {(GAME.SHOW_VICTIMS_DOGTAG): False, 
                        (GAME.SHOW_DOGTAG_TO_KILLER): False, 
                        (GAME.SHOW_KILLERS_DOGTAG): False}, 
           b'graphicsData': {(GAME.LENS_EFFECT): False}, 
           b'battleHud': {(GAME.SHOW_VEHICLE_HP_IN_PLAYERS_PANEL): 0, 
                          (ScorePanelStorageKeys.SHOW_HP_VALUES): True}, 
           b'feedbackDamageIndicator': {(DAMAGE_INDICATOR.TYPE): 1, 
           ...
           b'feedbackBattleEvents': {(BATTLE_EVENTS.SHOW_IN_BATTLE): True, 
        ...
        newbiesAimData = {b'arcade': {b'netType': 0, 
                       b'centralTagType': 4}, 
           b'sniper': {b'net': 100, 
                       b'netType': 0, 
                       b'centralTag': 100, 
        ...
        if aimData:
            for aimDataType in newbiesAimData:
                if aimDataType in aimData:
                    data[b'aimData'][aimDataType].update(newbiesAimData[aimDataType])
                else:
                    data[b'aimData'][aimDataType] = newbiesAimData[aimDataType]

        else:
            data[b'aimData'] = newbiesAimData
        newbiesMarkersData = {b'enemy': {b'markerBaseHp': 1, 
           ...
           b'ally': {b'markerBaseHp': 1, 
                     ...
                     b'markerAltPlayerName': True, 
                     b'markerAltHp': 1}, 
           b'dead': {b'markerBaseHp': 3, 
                     b'markerBaseHpIndicator': False, 
                     b'markerBaseVehicleName': True, 
                     b'markerAltIcon': True, 
                     b'markerAltVehicleName': True, 
                     b'markerAltPlayerName': False, 
                     b'markerAltHp': 1}}
        markersData = data.get(b'markersData')
        if markersData:
            for markerType in newbiesMarkersData:
                if markerType in markersData:
                    data[b'markersData'][markerType].update(newbiesMarkersData[markerType])
                else:
                    data[b'markersData'][markerType] = newbiesMarkersData[markerType]

        else:
            data[b'markersData'] = newbiesMarkersData
    elif b'dogTags' in data:
        data[b'dogTags'][GAME.SHOW_KILLERS_DOGTAG] = True
    else:
        data[b'dogTags'] = {(GAME.SHOW_KILLERS_DOGTAG): True}
    return
...
def _migrateTo117(core, data, initialized):
    ...
    data[b'uiStorage'][UI_STORAGE_KEYS.LIMITED_UI_ALL_NOVICE_RULES_COMPLETED] = False
    if rulesCount == 0 or commonRules & allCommonRulesCompleted == allCommonRulesCompleted:
        data[b'uiStorage'][UI_STORAGE_KEYS.LIMITED_UI_ALL_NOVICE_RULES_COMPLETED] = True
    clear = data[b'clear']
    hintsToClear = {(b'onceOnlyHints', _getSettingsCache().getSectionSettings(SETTINGS_SECTIONS.ONCE_ONLY_HINTS, 0)): (1, 24, 25), 
       (b'onceOnlyHints2', _getSettingsCache().getSectionSettings(SETTINGS_SECTIONS.ONCE_ONLY_HINTS_2, 0)): (0,)}
...
def _migrateTo118(core, data, initialized):
    ...
    for bitPosition in (9, 18, 26):
        settingOffset = 1 << bitPosition
        if storedValue & settingOffset:
            clear[b'uiStorage'] = clear.get(b'uiStorage', 0) | settingOffset
...
def _migrateTo120(core, data, initialized):
    ...
    newbieGroup = itemsCache.items.stats.getABGroup(feature=b'newbieHints')
    abConfig = lobbyContext.getServerSettings().abFeatureTestConfig
    if not disabled and newbieGroup and hasattr(abConfig, b'newbieHints'):
        properties = abConfig.newbieHints.get(newbieGroup)[b'properties']
...
def _migrateTo125(core, data, initialized):
    ...
    for bitPosition in (27, 28):
        settingOffset = 1 << bitPosition
        if storedValue & settingOffset:
            clear[b'onceOnlyHints2'] = clear.get(b'onceOnlyHints2', 0) | settingOffset
...
def _migrateTo137(core, data, initialized):
    data[b'clear'][b'battlePassStorage'] = data[b'clear'].get(b'battlePassStorage', 0) | 134217728
    return
...
def _migrateTo139(core, data, initialized):
    from account_helpers.AccountSettings import AccountSettings
    newSettingsCounter = AccountSettings.getSettings(NEW_SETTINGS_COUNTER)
    newSettingsCounter[b'GameSettings'].update({(BattleCommStorageKeys.ENABLE_COMMENDATIONS_FEEDBACK): True})
    AccountSettings.setSettings(NEW_SETTINGS_COUNTER, newSettingsCounter)
    gameData = data[b'battleComm']
...
def _migrateTo142(core, data, initialized):
    from account_helpers.settings_core.ServerSettingsManager import SETTINGS_SECTIONS
    battlePassUpdateKey = b'battlePassStorage'
    ...
    if battlePassSettings & introOffset:
        ...
        data[b'clear'][battlePassUpdateKey] = data[b'clear'].get(battlePassUpdateKey, 0) | introOffset
    elif battlePassSettings & extraOffset:
        data[b'clear'][battlePassUpdateKey] = data[b'clear'].get(battlePassUpdateKey, 0) | extraOffset
    return
...
def _migrateTo144(core, data, initialized):
    from account_helpers.settings_core.ServerSettingsManager import SETTINGS_SECTIONS
    clear = data[b'clear']
    hintsToClear = {(b'onceOnlyHints2', _getSettingsCache().getSectionSettings(SETTINGS_SECTIONS.ONCE_ONLY_HINTS_2, 0)): (27, 28), 
       (b'onceOnlyHints3', _getSettingsCache().getSectionSettings(SETTINGS_SECTIONS.ONCE_ONLY_HINTS_3, 0)): (8, 9)}
    for sectionData, hintsPositions in hintsToClear.items():
        section, sectionValue = sectionData
        for bitPosition in hintsPositions:
            settingOffset = 1 << bitPosition
...
def _migrateTo148(core, data, initialized):
    ...
    hangarFiltersSections = (
     ...
     (
      SETTINGS_SECTIONS.RANKED_CAROUSEL_FILTER_1, b'rankedCarouselFilter1'),
     (
      SETTINGS_SECTIONS.EPICBATTLE_CAROUSEL_FILTER_1, SETTINGS_SECTIONS.EPICBATTLE_CAROUSEL_FILTER_1),
     (
      SETTINGS_SECTIONS.COMP7_CAROUSEL_FILTER_1, b'comp7CarouselFilter1'),
     (
     ...
     (
      SETTINGS_SECTIONS.COMP7_LIGHT_CAROUSEL_FILTER_1, b'comp7LightCarouselFilter1'))
    ...
    for sectionName, dataName in hangarFiltersSections:
        ...
        if storedValue & tier11Filter:
            data[b'clear'][dataName] = data[b'clear'].get(dataName, 0) | tier11Filter
...
def _migrateTo150(core, data, initialized):
    from account_helpers.settings_core.ServerSettingsManager import SETTINGS_SECTIONS
    clear = data[b'clear']
    hintsToClear = {(b'onceOnlyHints', _getSettingsCache().getSectionSettings(SETTINGS_SECTIONS.ONCE_ONLY_HINTS, 0)): (7,), 
       (b'onceOnlyHints2', _getSettingsCache().getSectionSettings(SETTINGS_SECTIONS.ONCE_ONLY_HINTS_2, 0)): (9, 24)}
    for sectionData, hintsPositions in hintsToClear.items():
        section, sectionValue = sectionData
        for bitPosition in hintsPositions:
            settingOffset = 1 << bitPosition
