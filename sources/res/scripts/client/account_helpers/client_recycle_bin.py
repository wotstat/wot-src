...
class ClientRecycleBin(object):
    ...
    def synchronize(self, isFullSync, diff):
        if isFullSync:
            self.__cache.clear()
        recycleBinFull = diff.get((b'recycleBin', b'_r'), {})
        if recycleBinFull:
            self.__cache = dict(recycleBinFull)
        for item in (b'recycleBin',):
    ...
    def restoreTankman(self, tmanInvID, useBerth, groupID, groupSize, callback):
        if self.__ignore:
            ...
            return
        if callback is not None:
            proxy = lambda requestID, resultID, errStr=b'', ext=None: callback(resultID, errStr, ext)
        else:
            proxy = None
    ...
    def restoreTankmans(self, tmanInvIDs, callback):
        if self.__ignore:
            ...
            return
        if callback is not None:
            proxy = lambda requestID, resultID, errStr=b'': callback(resultID, errStr)
        else:
            proxy = None
