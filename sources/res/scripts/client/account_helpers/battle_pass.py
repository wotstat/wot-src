...
def _handleNonPlayerIgnoreState(func):
    callbackNdx = inspect.getargspec(func).args.index(b'callback')

    @wraps(func)
    def wrapped(self, *args, **kwargs):
        if self._ignore:
            callback = kwargs.get(b'callback', args[callbackNdx] if callbackNdx < len(args) else None)
