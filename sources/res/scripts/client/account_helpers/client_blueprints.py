...
class ClientBlueprints(object):
    ...
    def synchronize(self, isFullSync, diff):
        _logger.debug(b'Synchronize information about account blueprints')
        if isFullSync:
            self.__cache.clear()
        if _BLUEPRINT_KEY in diff:
            synchronizeDicts(diff[_BLUEPRINT_KEY], self.__cache)
        _logger.debug(b'User blueprints: %s', self.__cache)
        return
    ...
    def convertBlueprintFragment(self, fragmentTypeCD, position, requestedCount, usedNationalFragments, callback):
        _logger.debug(b'Account.convertBlueprintFragment: fragmentTypeCD=%s', fragmentTypeCD)
        if callback is not None:
    ...
    def __onGetCacheResponse(self, callback, resultID):
        _logger.debug(b'Blueprint cache was got')
        if resultID < 0:
            if callback is not None:
                callback(resultID, None)
            return
        if callback is not None:
            callback(resultID, {b'cache': (self.__cache)})
        return
