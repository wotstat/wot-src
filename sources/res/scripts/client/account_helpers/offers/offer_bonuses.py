...
class OfferEconomyBonusMixin(OfferBonusMixin):
    ...
    def getOfferName(self):
        return backport.text(R.strings.quests.bonuses.dyn(self.getName()).description(), value=tagText(self.formatValue(), b'neutral'))

    def getOfferDescription(self):
        typeName = self.DESCRIPTION_NAME or b''
...
class CreditsOfferBonus(OfferEconomyBonusMixin, CreditsBonus):
    DESCRIPTION_NAME = b'economyCredits'


class GoldOfferBonus(OfferEconomyBonusMixin, GoldBonus):
    DESCRIPTION_NAME = b'economyGold'
...
class CrystalOfferBonus(OfferEconomyBonusMixin, CrystalBonus):
    DESCRIPTION_NAME = b'economyBonds'
    ICON_NAME = b'bon'


class FreeXpOfferBonus(OfferEconomyBonusMixin, FreeXpBonus):
    DESCRIPTION_NAME = b'economyConvertExp'


class PlusPremiumDaysOfferBonus(OfferEconomyBonusMixin, PlusPremiumDaysBonus):
    DESCRIPTION_NAME = b'economyPremium'

    def getOfferIcon(self):
        imgName = b'premium_icon_{}'
        resource = R.images.gui.maps.shop.premium.c_180x135.dyn(imgName.format(self.getValue()), R.images.gui.maps.shop.premium.c_180x135.dyn(imgName.format(b'wot')))()
...
class VehiclesOfferBonus(OfferBonusMixin, VehiclesBonus):
    ...
    def getOfferName(self):
        return (b' ').join([
         ...
         tagText(int2roman(self.displayedItem.level), b'neutral'),
    ...
    def isMaxCountExceeded(self):
        for vehicle, vehData in self.getVehicles():
            if vehicle.isInInventory and not vehicle.isRented:
                return True
            if b'customization' in vehData:
                styleId = vehData[b'customization'].get(b'styleId')
    ...
    @property
    def isWithCrew(self):
        return b'noCrew' not in self.displayedVehicleInfo
...
class OfferItemsBonusMixin(OfferBonusMixin):
    ...
    def displayedBonusData(self):
        ...
        for item, count in self._getItems():
            if item is self.displayedItem:
                bonusData = {b'items': {(self.displayedItem.intCD): count}}
                break
...
class CustomizationsOfferBonus(OfferBonusMixin, CustomizationsBonus):
    ...
    def getGiftCount(self):
        return self.displayedItem.get(b'value', 1)

    def isMaxCountExceeded(self):
        for itemData in self.getCustomizations():
            count = itemData.get(b'value', 0)
...
class NationalBlueprintOfferBonus(OfferBonusMixin, NationalBlueprintBonus):
    ...
    def getOfferIcon(self):
        return backport.image(self.getIconResource(b's180x135'))
...
class TokensOfferBonus(OfferBonusMixin, TokensBonus):
    ...
    def getOfferIcon(self):
        return self.getIconBySize(b's180x135')
...
OFFER_BONUSES = {(Currency.CREDITS): CreditsOfferBonus, 
   ...
   b'freeXP': FreeXpOfferBonus, 
   (PREMIUM_ENTITLEMENTS.PLUS): PlusPremiumDaysOfferBonus, 
   b'vehicles': VehiclesOfferBonus, 
   b'items': (ItemsOfferBonusFactory()), 
   b'customizations': CustomizationsOfferBonus, 
   b'crewSkins': (CrewSkinsOfferBonusFactory()), 
   b'blueprints': blueprintsOfferBonusFactory, 
   b'slots': CountableIntegralOfferBonus, 
   b'goodies': goodiesOfferBonusFactory, 
   b'tokens': tokensOfferFactory, 
   b'tankmen': {b'default': TankmenOfferBonus, 
                (_ET.PERSONAL_MISSION): TankwomanOfferBonus}, 
   b'berths': CountableIntegralOfferBonus}
