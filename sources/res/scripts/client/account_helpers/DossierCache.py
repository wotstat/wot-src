...
class DossierCache(object):
    ...
    def resynchronize(self, isFullResync=False):
        if isFullResync:
            ...
            self.__maxChangeTime = 0
        LOG_DEBUG(b'resynchronize', self.__maxChangeTime)
        if self.__ignore:
            return
        else:
            self.__isSynchronizing = True
    ...
    def get(self, dossierType, ownerID, callback=None):
        getter = lambda cache: cache.get((dossierType, ownerID), (0, b''))[1]
        self.__get(getter, b'', callback)
        return
    ...
    def __onSyncResponse(self, syncID, resultID, ext=None):
        ...
        if syncID != self.__syncID:
            return
        if resultID < 0:
            LOG_ERROR(b'Dossier synchronization failed. Repeating')
            self.resynchronize()
            return
        return
    ...
    def __onSyncComplete(self, syncID, data):
        if data is None:
            ...
            LOG_DEBUG(b'__onSyncComplete', actualCacheVersion, len(dossiersList))
