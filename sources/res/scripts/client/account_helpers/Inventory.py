...
class Inventory(object):
    ...
    def claimRewardForPostProgression(self, count, callback):
        ...
        proxy = None
        if callback is not None:
            proxy = lambda requestID, resultID, errorStr=b'', ext=None: callback(resultID, ext)
        self.__account._doCmdInt(AccountCommands.CLAIM_REWARD_FOR_POST_PROGRESSION, count, proxy)
        return

    def convertJunkTankmen(self, callback, errorStr=b'', ext=None):
        proxy = None
        if callback is not None:
    ...
    def addExpiringGoodie(self, goodieID, amount, expirePeriod, callback=None):
        if callback is None:
            callback = lambda reqID, resID, errStr: _logger.debug(b'addExpiringGoodie result: %s, %s, %s', reqID, resID, errStr)
