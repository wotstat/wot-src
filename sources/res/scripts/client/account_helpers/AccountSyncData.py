...
class AccountSyncData(object):
    ...
    def __init__(self):
        ...
        self.__persistentCache = SimpleCache(b'account_caches', b'data')
    ...
    def updatePersistentCache(self, ext, isFullSync):
        if ext.pop(b'__cache', None) is not None:
            if not self.__persistentCache.data:
                ...
                if accountDataPersistentHash(cacheData) != desc:
                    LOG_ERROR(b'Local account data cache is corrupted: resync')
                    self._resynchronize()
                    return False
            ...
            if cacheData is None:
                LOG_ERROR(b"Incorrect cache state while syncing data: server said to use cache but I don't have any")
                self._resynchronize()
                return False
    ...
    def __onSyncResponse(self, syncID, resultID, ext=None):
        ...
        if syncID != self.__syncID:
            return
        if resultID < 0:
            LOG_ERROR(b'Data synchronization failed.')
            self._resynchronize()
            return
        if self.revision != ext.get(b'prevRev', self.revision):
            LOG_ERROR(b'Incorrect diff received', self.revision, ext[b'prevRev'])
            self._resynchronize()
            return
        self.revision = ext.get(b'rev', self.revision)
    ...
    def __sendSyncRequest(self, rqID, proxy):
        ...
        gameParamsRevision = self.__account.serverSettings.get(b'rev', 0)
...
def isFullSyncDiff(diff):
    return diff.get(b'prevRev', None) is None
