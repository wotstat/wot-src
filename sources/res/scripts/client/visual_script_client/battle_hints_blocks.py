...
class CanShowHint(Block, HintsMeta):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._result = self._makeDataOutputSlot(b'result', SLOT_TYPE.BOOL, self._execute)
        return
    ...
    def _execute(self):
        ...
        if not model:
            self._result.setValue(False)
            errorVScript(self, (b'Hint [{}] does not exist.').format(hintId))
            return
...
class ReceiverMixin(object):
    ...
    def _getReceivers(self):
        ...
        if not controller:
            errorVScript(self, b'Can not access the BattleHintsController.')
            return []
        return [
         controller]
...
class ShowHint(ReceiverMixin, HintActionParamsMixin, BaseHintAction):
    RESERVED = HintActionParamsMixin.RESERVED + (b'immediately',)

    def __init__(self, *args, **kwargs):
        super(ShowHint, self).__init__(*args, **kwargs)
        self._immediately = self._makeDataInputSlot(b'immediately', SLOT_TYPE.BOOL)
...
class RemoveHint(ReceiverMixin, BaseHintAction):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._hide = self._makeDataInputSlot(b'hide', SLOT_TYPE.BOOL)
