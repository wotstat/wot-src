...
helpers, clientArena, dependency, battle_session = dependencyImporter(b'helpers', b'ClientArena', b'helpers.dependency', b'skeletons.gui.battle_session')

class ClientArena(Block, ArenaMeta):

    def __init__(self, *args, **kwargs):
        super(ClientArena, self).__init__(*args, **kwargs)
        self._arena = self._makeDataOutputSlot(b'arena', SLOT_TYPE.ARENA, self._execArena)
        return
...
class GetControlPoint(Block, ArenaMeta):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._team = self._makeDataInputSlot(b'teamId', SLOT_TYPE.INT)
        self._baseID = self._makeDataInputSlot(b'baseId', SLOT_TYPE.INT)
        self._value = self._makeDataOutputSlot(b'value', SLOT_TYPE.CONTROL_POINT, self._execValue)
        return
...
class OnCaptureControlPoint(TunableEventBlock, ArenaMeta):
    _EVENT_SLOT_NAMES = [
     b'onStarted', b'onStopped', b'onUpdated', b'onCompleted']

    def __init__(self, *args, **kwargs):
        super(OnCaptureControlPoint, self).__init__(*args, **kwargs)
        self._points = self._makeDataOutputSlot(b'points', SLOT_TYPE.INT, self._getPoints)
        self._invadersCount = self._makeDataOutputSlot(b'invadersCount', SLOT_TYPE.INT, self._getInvadersCount)
        self._controlPoint = self._makeDataInputSlot(b'controlPoint', SLOT_TYPE.CONTROL_POINT)
        return
    ...
    def arena(self):
        if not IS_VS_EDITOR:
            return getattr(BigWorld.player(), b'arena')
        else:
            return
    ...
    def validate(self):
        if not self._controlPoint.hasValue():
            return b'ControlPoint value is required'
...
class ArenaPeriod(Block, ArenaMeta):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._isPeriodActive = self._makeDataOutputSlot(b'isActive', SLOT_TYPE.BOOL, self._execIsStart)
        self._timeFromStartSlot = self._makeDataOutputSlot(b'timeFromStart', SLOT_TYPE.FLOAT, self._execTimeFromStart)
        self._timeToEndSlot = self._makeDataOutputSlot(b'timeToEnd', SLOT_TYPE.FLOAT, self._execTimeToEnd)
        self._periodType, = self._getInitParams()
        return

    @property
    def _arena(self):
        return getattr(BigWorld.player(), b'arena', ArenaPeriod.PlayerArenaDummy)
    ...
    @classmethod
    def blockIcon(cls):
        return b':vse/blocks/arena_time'

    def captionText(self):
        return b'Arena Period: ' + ARENA_PERIOD_NAMES.get(self._periodType, b'Unknown')

    @classmethod
    def initParams(cls):
        return [
         InitParam(b'Period', SLOT_TYPE.E_ARENA_PERIOD, ARENA_PERIOD.BATTLE)]
...
class ArenaPeriodStartEvent(Block, ArenaMeta):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._start = self._makeEventOutputSlot(b'start')
        self._update = self._makeEventOutputSlot(b'update')
        self._timeToEndSlot = self._makeDataOutputSlot(b'timeToEnd', SLOT_TYPE.FLOAT, self._execTimeToEnd)
        self._periodType, = self._getInitParams()
    ...
    def captionText(self):
        return b'Arena Period Start: ' + ARENA_PERIOD_NAMES.get(self._periodType, b'Unknown')
...
class OnBattleRoundFinished(Block, ArenaMeta):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._out = self._makeEventOutputSlot(b'onFinished')
        self._winner = self._makeDataOutputSlot(b'winnerTeam', SLOT_TYPE.INT, None)
        self._reason = self._makeDataOutputSlot(b'reason', SLOT_TYPE.E_FINISH_REASON, None)
        return
...
class GetArenaBorders(Block, ArenaMeta):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._min = self._makeDataOutputSlot(b'min', SLOT_TYPE.VECTOR2, None)
        self._max = self._makeDataOutputSlot(b'max', SLOT_TYPE.VECTOR2, None)
        if not IS_VS_EDITOR:
...
class GetVehicles(Block, ArenaMeta):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._team = self._makeDataInputSlot(b'team', SLOT_TYPE.INT)
        self._excludePlayer = self._makeDataInputSlot(b'excludePlayer', SLOT_TYPE.BOOL)
        self._excludeDestroyed = self._makeDataInputSlot(b'excludeDestroyed', SLOT_TYPE.BOOL)
        self._vehicles = self._makeDataOutputSlot(b'vehicles', arrayOf(SLOT_TYPE.VEHICLE), self._execute)
        return
...
class SetPrebattleCountdownTimerText(Block, ArenaMeta):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._header = self._makeDataInputSlot(b'header', SLOT_TYPE.STR)
        self._subheader = self._makeDataInputSlot(b'subheader', SLOT_TYPE.STR)
        self._battleStartMessage = self._makeDataInputSlot(b'battleStartMessage', SLOT_TYPE.STR)
        return
    ...
    def validate(self):
        if not self._header.hasValue():
            return b'header value is required.'
        if not self._battleStartMessage.hasValue():
            return b'battleStartMessage value is required.'
...
class CollideSegment(Block, ArenaMeta):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._in = self._makeEventInputSlot(b'in', self._collide)
        self._out = self._makeEventOutputSlot(b'out')
        self._spaceID = self._makeDataInputSlot(b'spaceId', SLOT_TYPE.INT)
        self._from = self._makeDataInputSlot(b'from', SLOT_TYPE.VECTOR3)
        self._to = self._makeDataInputSlot(b'to', SLOT_TYPE.VECTOR3)
        self._hitFlags = self._makeDataInputSlot(b'excludeHitFlags', SLOT_TYPE.INT)
        self._collision = self._makeDataOutputSlot(b'hasCollision', SLOT_TYPE.BOOL, None)
        self._collidePosition = self._makeDataOutputSlot(b'position', SLOT_TYPE.VECTOR3, None)
        return
