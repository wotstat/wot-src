...
class AbilityContextClient(VehicleContextClient):
    ...
    @vse_event_out(display_name=b'OnStartupCooldown', description=b'Calls when ability delayed before it can be activated', aspects=[
     ASPECT.CLIENT])
    def startupCooldown(self):
        return

    @vse_event_out(display_name=b'OnInterrupted', description=b'Calls when interrupted by player', aspects=[
     ASPECT.CLIENT])
