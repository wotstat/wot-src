...
cgf_helpers, mechanic_trackers = dependencyImporter(b'cgf_common.cgf_helpers', b'vehicles.mechanics.mechanic_trackers')
if not IS_VS_EDITOR:
...
class VehicleSelectableMechanicEventsBlock(VehicleMechanicEventsBlock):
    ...
    def initParams(cls):
        return [
         InitParam(b'Vehicle Mechanic', SLOT_TYPE.STR, buildStrKeysValue(*cls._getInitParamMechanics()), EDITOR_TYPE.STR_KEY_SELECTOR)]
...
class VehicleMechanicLifeCycleEventsBlock(VehicleMechanicEventsBlock, IComponentLifeCycleListenerLogic):
    _EVENTS_NAME = b'lifeCycle'

    def __init__(self, *args, **kwargs):
        super(VehicleMechanicLifeCycleEventsBlock, self).__init__(*args, **kwargs)
        self._onComponentAvatarReadySlot = self._makeEventOutputSlot(b'onComponentAvatarReady')
        self._onComponentParamsCollectedSlot = self._makeEventOutputSlot(b'onComponentParamsCollected')
        self._onComponentDestroyedSlot = self._makeEventOutputSlot(b'onComponentDestroyed')
        return
...
class VehicleMechanicStateEventsBlock(VehicleMechanicEventsBlock, IMechanicStatesListenerLogic):
    _EVENTS_NAME = b'states'

    def __init__(self, *args, **kwargs):
        super(VehicleMechanicStateEventsBlock, self).__init__(*args, **kwargs)
        self._onStatePreparedSlot = self._makeEventOutputSlot(b'onStatePrepared')
        self._onStateObservationSlot = self._makeEventOutputSlot(b'onStateObservation')
        self._onStateTransitionSlot = self._makeEventOutputSlot(b'onStateTransition')
        return
