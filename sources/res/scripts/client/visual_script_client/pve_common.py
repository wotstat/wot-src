...
class PropertySlotSpec(object):
    __slots__ = (b'slotName', b'slotType', b'defaultValue', b'editorData', b'required')
...
class ClientBattleHUDWidgetSettings(Block, PVEBattleHUDMeta):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self._SETTINGS_MODEL is None:
            errorVScript(self, (b'Missing widget settings model for "{}" block.').format(self.__name__))
        if self._WIDGET_TYPE is None:
            errorVScript(self, (b'Missing widget type for "{}" block.').format(self.__name__))
        if self._SETTING_TYPE is None:
            errorVScript(self, (b'Missing setting type for "{}" block.').format(self.__name__))
    ...
    def validate(self):
        for idx, slotSpec in enumerate(self._slotSpecs):
            ...
            if slotSpec.required and not slot.hasValue():
                return (b'{slotName} value is required').format(slotName=slotSpec.slotName)
