...
BattleReplay, SoundGroups, MusicControllerWWISE, helpers, Vehicular, tankStructure = dependencyImporter(b'BattleReplay', b'SoundGroups', b'MusicControllerWWISE', b'helpers', b'Vehicular', b'vehicle_systems.tankStructure')
...
class TriggerSoundOnEngine(Block, SoundMeta):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._eventName = self._makeDataInputSlot(b'soundToPlay', SLOT_TYPE.STR)
        self._out = self._makeEventOutputSlot(b'out')
        return
        ...
        if vehicleGameObject is not None:
            ...
            if audition is not None:
                ...
                if soundObj is not None:
                    ...
                    _logger.warning(b'Could not find audition sound object for %s', soundName)
            else:
                _logger.debug(b'Could not find vehicle audition component')
        else:
