    ...
    def logButtonClick(self, button):
        if button == DialogButtons.SUBMIT:
            ...
            if loggedItem is None:
                _logger.warning(b'[PR2UILOG] Cannot find dialog in SUBMIT_BUTTON_BY_DIALOG - dialog %s, keys %s', self._item, SUBMIT_BUTTON_BY_DIALOG.keys())
                return
        else:
            loggedItem = BUTTON_TO_ITEM_MAP.get(button)
            if loggedItem is None:
                _logger.warning(b'[PR2UILOG] Button could not be mapped to log item - button %s, keys %s', button, BUTTON_TO_ITEM_MAP.keys())
                return
...
class PersonalReservesFullStatsLogger(PersonalReservesMetricsLogger):
    ...
    def _getInfo(self):
        ...
        if arenaVisitor.getArenaPeriod() in (
         ARENA_PERIOD.PREBATTLE,
         ARENA_PERIOD.BATTLE,
