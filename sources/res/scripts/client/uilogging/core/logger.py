...
class UILoggingCore(IUILoggingCore):
    ...
    def _createLogRecord(self, feature, group, action, loglevel, **params):
        if not self._isEnabled:
            ...
            if log.broken:
                self._logger.warning(b'Broken %s.', log)
                return None
            return log
    ...
    def _isEnabled(self):
        if self._disabled or not self._handler:
            self._logger.debug(b'[disabled=%s|destroyed=%s] Disabled.', self._disabled, not self._handler)
            return False
        if BattleReplay.isPlaying():
            self._logger.debug(b'Watching replay. Disabled.')
            return False
        return True
    ...
    def _start(self, *args, **kwargs):
        if self._started:
            self._startSessionKeeper()
            self._logger.debug(b'Already started.')
            return
        playerID = getPlayerDatabaseID()
        self._logger.debug(b'Got player id: %s.', playerID)
        if not playerID:
            self._logger.warning(b'No player id, start failed.')
            return
        ...
        self._logger.debug(b'Started.')
        return

    def _stop(self, *args, **kwargs):
        self._shutdown()
        self._started = False
        self._logger.debug(b'Stopped.')
        return
    ...
    def _getSessionLifetime(self):
        ...
        self._logger.debug(b'Session keeper next tick = %s.', lifetime)
        if lifetime is None:
            self._stopSessionKeeper()
        return lifetime
    ...
    def _startSessionKeeper(self, *args, **kwargs):
        if self._sessionKeeper is None and self._ensureSession and self._isEnabled:
            ...
            self._logger.debug(b'Session keeper started.')
        return
    ...
    def _stopSessionKeeper(self):
        if self._sessionKeeper is not None:
            ...
            self._sessionKeeper = None
            self._logger.debug(b'Session keeper stopped.')
        return
