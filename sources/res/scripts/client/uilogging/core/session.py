...
class SessionData(object):
    ...
    def __repr__(self):
        return (b'<SessionData>(id={}, lifetime={})').format(self.id, self.lifetime)
...
class Session(object):
    ...
    def request(self):
        ...
        self._requesting = False
        self._logger.debug(b'Session data %s received and saved.', self._sessionData)
        raise AsyncReturn(self._sessionData)
        return
    ...
    def _getSessionData(self, callback):
        ...
        self._logger.debug(b'Session response: %s', response)
            ...
            if response.isSuccess():
                if isinstance(response.data, dict):
                    ...
                    if data.isValid:
                        ...
                        self._logger.warning(b'Invalid session data.')
                else:
                    self._logger.warning(b'Unsupported session response data type.')
            else:
                retry = True
        self._logger.debug(b'Session data: %s, retry: %s.', sessionData, retry)
