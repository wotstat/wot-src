...
def _ifDestroyed(result=None):
    ...
    def inner(function):
        ...
        def wrapper(self, *args, **kwargs):
            if self.destroyed:
                self._logger.debug(b'Already destroyed. Skipping function call with result: %s.', result)
                return result
...
class Delayer(object):
    __slots__ = (b'_callbackId', b'_destroyed')
...
class FeatureSettings(object):
    __slots__ = (b'feature', b'enabled', b'loglevel')

    def __init__(self, feature, enabled=True, params=None):
        self.feature = str(feature)
        self.enabled = enabled
        params = params or {}
        loglevel = params.get(b'loglevel')
...
class LogHandler(object):
    ...
    def add(self, log):
        if len(self._logs) + 1 > LOGS_MAX_QUEUE_SIZE:
            self._logger.warning(b'Max logs queue size reached. Skipping [%s] log.', log.feature)
            return
        if not self._features.verifyLog(log):
            self._logger.debug(b'[%s] log skipped by features settings.', log.feature)
            return
        self._logs.append(log)
        self._logger.debug(b'[%s] log added. Logs count: %s.', log.feature, len(self._logs))
        if self._isPlayer:
            self._getSession()
            self._startSender()
        return
    ...
    def logImmediately(self, log):
        ...
        if session is None:
            self._logger.warning(b'No session for immediate log.')
            return
        else:
    ...
    def getSessionLifetime(self):
        if not self._isPlayer:
            self._logger.debug(b'Getting session lifetime stopped. Player not account any more.')
            return None
        else:
            session = self._getSession()
            if session:
    ...
    def destroy(self, flush=False):
        self._logger.debug(b'Destroy in progress.')
        self._stopSender()
        self._logsBatchWaitingTime = 0
        self._destroyed = True
    ...
    def _send(self, session, logs, wait=True):
        self._logger.debug(b'Sending %s logs from player=%s.', len(logs), self._playerID)
        logs = [_log for _log in logs if session.verifyLog(_log) and self._features.verifyLog(_log)]
        self._logger.debug(b'Filtered logs count: %s.', len(logs))
        if not logs:
            return
        else:
            try:
            ...
            headers = {(HttpHeaders.CONTENT_TYPE.value): b'application/octet-stream', 
               (HttpHeaders.USER_ID.value): (str(self._playerID))}
            if session.token is not None:
                headers[HttpHeaders.AUTH_TOKEN.value] = str(session.token)
            BigWorld.fetchURL(url=session.url, callback=partial(self._receive, session.id, logs) if wait else (lambda x: x), headers=headers, timeout=HTTP_DEFAULT_TIMEOUT, method=b'POST', postData=postData)
            return

    @_ifDestroyed()
    def _receive(self, sessionID, logs, response):
        self._logger.debug(b'Received response with code %s.', response.responseCode)
        ...
        if response.responseCode != HTTP_OK_STATUS:
            ...
            rejected, accepted = body.get(b'rejected'), body.get(b'accepted')
            if rejected is None or accepted is None:
                self._logger.warning(b'Wrong response body structure.')
                self.destroy()
                return
            try:
                self._features.update(rejected, accepted)
            except Exception:
                self._logger.exception(b'Failed to update features.')
                self.destroy()
    ...
    def _startSender(self, *args, **kwargs):
        if self._sender is None and self._logs:
            ...
            self._logger.debug(b'Sender started.')
        return
    ...
    def _stopSender(self):
        if self._sender is not None:
            ...
            self._sender = None
            self._logger.debug(b'Sender stopped.')
        return
