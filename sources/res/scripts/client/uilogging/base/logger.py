...
def ifUILoggingEnabled(result=None):
    ...
    def inner(func):
        ...
        def wrapper(self, *args, **kwargs):
            if self.disabled:
                _logger.debug(b'UI logging disabled. %s log call skipped.', self)
                return result
...
class _BaseLogger(object):
    __slots__ = (b'_feature', b'_group', b'_logOnceSet', b'_timedActions', b'_suspendedActions')
    ...
    def __str__(self):
        return (b'<{}: {}, {}>').format(self.__class__.__name__, self._feature, self._group)
    ...
    def startAction(self, action):
        if action in self._timedActions:
            _logger.debug(b'%s action: %s already started. Setting new action.', self, action)
    ...
    def _logOnce(self, action, loglevel=LogLevels.INFO, **params):
        if action in self._logOnceSet:
            _logger.debug(b'%s log once action: %s already logged.', self, action)
            return
    ...
    def _stopAction(self, action, loglevel=LogLevels.INFO, timeLimit=0.0, **params):
        if action not in self._timedActions:
            _logger.debug(b'%s action: %s not started.', self, action)
            return
        else:
            if action in self._suspendedActions:
...
class MetricsLogger(_BaseLogger):
    ...
    def __init__(self, feature):
        super(MetricsLogger, self).__init__(feature, group=b'metrics')
        return
...
class FlowLogger(_BaseLogger):
    ...
    def __init__(self, feature):
        super(FlowLogger, self).__init__(feature, group=b'flow')
        return
