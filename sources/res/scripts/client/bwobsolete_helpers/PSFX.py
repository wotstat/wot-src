import BigWorld, Pixie, particles
from Keys import *
...
def attachBloodSpray(model, direction=0, numberOfSprays=1):
    ...
    if len(bloodSprayList) >= 1:
        ...
        src.setPositionSource([b'Point', (0.0, height, 0.0)])
        if direction == 0:
            src.setVelocitySource([b'Sphere', (0.0, 4.0, 0.0), 1.5, 0.5])
        elif direction == 1:
            src.setVelocitySource([b'Sphere', (-4.0, 0.0, 0.0), 1.5, 0.5])
        elif direction == 2:
            src.setVelocitySource([b'Sphere', (4.0, 0.0, 0.0), 1.5, 0.5])
...
def beginPlasmaWarp(target):
    ...
    if allFinished:
        warpEffects = []
    ps = Pixie.create(b'particles/plasma_suck.xml')
    try:
        target.node(b'biped Head').attach(ps)
    except:
...
def endPlasmaWarp(idx):
    ...
    try:
        target.node(b'biped Head').detach(ps)
    except:
