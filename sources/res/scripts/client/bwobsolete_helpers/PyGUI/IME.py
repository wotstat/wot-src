import BigWorld, GUI, Math, Utils
...
DEFAULT_FONT_NAME = b'default_small.font'
...
def logIMEEvent(event):
    print str(event) + b':'
    print b'   stateChanged=' + str(event.stateChanged) + b" (value='" + str(BigWorld.ime.state) + b"')"
    print b'   candidatesVisibilityChanged=' + str(event.candidatesVisibilityChanged) + b' (value=' + str(BigWorld.ime.candidatesVisible) + b')'
    print b'   candidatesChanged=' + str(event.candidatesChanged) + b" (value='" + str(BigWorld.ime.candidates) + b"', attrs=" + str(BigWorld.ime.compositionAttr) + b')'
    print b'   selectedCandidateChanged=' + str(event.selectedCandidateChanged) + b' (value=' + str(BigWorld.ime.selectedCandidate) + b')'
    print b'   compositionChanged=' + str(event.compositionChanged) + b" (value='" + str(BigWorld.ime.composition) + b"')"
    print b'   compositionCursorPositionChanged=' + str(event.compositionCursorPositionChanged) + b' (value=' + str(BigWorld.ime.compositionCursorPosition) + b')'
    print b'   readingVisibilityChanged=' + str(event.readingVisibilityChanged) + b' (value=' + str(BigWorld.ime.readingVisible) + b')'
    print b'   readingChanged=' + str(event.readingChanged) + b" (value='" + str(BigWorld.ime.reading) + b"')"
    return
...
class CompositionWindow(object):
    ...
    def __init__(self):
        self.comp = GUI.Window(b'system/maps/col_white.bmp')
        ...
        self.comp.addChild(GUI.Text(), b'text')
        ...
        self.comp.addChild(self.cursor.comp, b'cursor')
    ...
    def populate(self, fontName):
        ...
        for name, comp in self.comp.children:
            if name not in (b'cursor', b'text'):
                self.comp.delChild(comp)

        if BigWorld.ime.language == b'KOREAN':
    ...
    def _populateChineseJapanese(self, fontName):
        ...
        for attr, str in compBlocks:
            ...
            self.comp.addChild(w, b'compBlock%d' % idx)
            idx += 1
            fullWidth += w.width
    ...
    def candidateClipBounds(self):
        ...
        if BigWorld.ime.language == b'JAPANESE':
            ftcb = self._firstTargetConvertedBlock
            block = getattr(self.comp, b'compBlock%d' % ftcb, None)
...
class ReadingWindow(object):
    ...
    def __init__(self):
        ...
        self.comp.heightMode = GUI.Simple.eSizeMode.PIXEL
        self.comp.position.z = ALWAYS_ON_TOP_Z
        self.comp.addChild(GUI.Text(b''), b'text')
...
class CandidateWindow(object):
    ...
    def __init__(self):
        ...
        self.comp.widthMode = GUI.Simple.eSizeMode.PIXEL
        self.comp.heightMode = GUI.Simple.eSizeMode.PIXEL
        self.comp.addChild(GUI.Text(b''), b'candidateText')
    ...
    def populate(self, fontName):
        ...
        self._resize(sw, sh)
        if hasattr(self.comp, b'selectedCandidate'):
        ...
        if len(candidates) > 0:
            ...
            self.comp.addChild(selectedComp, b'selectedCandidate')
        self.comp.visible = True
        return True
...
def init():
    ...
    global _composition
    global _reading
    TRACE_MSG(b'Initialising IME components')
...
def refresh(positionClip, minClip, maxClip, fontName):
    if _composition is None or _candidates is None or _reading is None:
        ERROR_MSG(b'IME components have not been initialised.')
        return
    else:
        ...
        if gotReading:
            pos = (
             compPosMins[0] + 0.025, compPosMins[1] + (compPosMaxs[1] - compPosMins[1]) * 0.5)
