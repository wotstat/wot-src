import BigWorld, GUI, Math, Keys, Utils
...
class DraggableComponent:
    ...
    def onSave(self, dataSection):
        dataSection.writeBool(b'horzDrag', self.horzDrag)
        dataSection.writeBool(b'vertDrag', self.vertDrag)
        dataSection.writeBool(b'restrictToParent', self.restrictToParent)
        return

    def onLoad(self, dataSection):
        self.horzDrag = dataSection.readBool(b'horzDrag')
        self.vertDrag = dataSection.readBool(b'vertDrag')
        self.restrictToParent = dataSection.readBool(b'restrictToParent')
        return
...
def _horzAnchorOffset(component):
    ...
    if component.horizontalAnchor == GUI.Simple.eHAnchor.RIGHT:
...
def _vertAnchorOffset(component):
    ...
    if component.verticalAnchor == GUI.Simple.eVAnchor.BOTTOM:
...
class ComponentDragManager:
    ...
    def startDragging(self, component, horzDrag, vertDrag, restrictToParent, mpos):
        if self.component:
            print b'WARNING: startDragging called again without first calling stop!'
            self.stopDragging()
        self.horzDrag = horzDrag
        self.vertDrag = vertDrag
        self.restrictToParent = restrictToParent
        ...
        if hasattr(self.component.script, b'onBeginDrag'):
            self.component.script.onBeginDrag()
        return

    def stopDragging(self):
        if not self.component:
            print b'WARNING: stopDragging called without any component currently being dragged!'
        if self.component is not None:
            if hasattr(self.component.script, b'onEndDrag'):
    ...
    def handleMouseEvent(self, event):
        if self.component is None:
            ...
            if dragged and hasattr(self.component.script, b'onDragging'):
