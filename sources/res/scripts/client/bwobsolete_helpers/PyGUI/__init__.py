...
import EditUtils, Test, TextStyles, Utils, VisualStateComponent, IME
...
def handleIMEEvent(event):
    ...
    if focused is not None and hasattr(focused.script, b'handleIMEEvent'):
        handled = focused.script.handleIMEEvent(event)
    import BigWorld
    if BigWorld.ime.state == b'OFF':
        IME.hideAll()
    return handled
...
def PyGUIEvent(componentName, eventName, *args, **kargs):
    ...
    def addEvent(componentName, eventName, args, kargs, eventFunction):
        if not hasattr(eventFunction, b'_PyGUIEventHandler'):
