...
class TextField(PyGUIBase):
    factoryString = b'PyGUI.TextField'

    def __init__(self, component=None):
        PyGUIBase.__init__(self, component)
        self.lines = []
        self.lines.append(GUI.Text(b''))
    ...
    def onSave(self, section):
        ...
        section.writeWideString(b'transcript', self.transcript)
        section.writeVector4(b'colour', self.lines[0].colour)
        section.writeString(b'font', self.lines[0].font)
        return

    def onLoad(self, section):
        self.transcript = section.readWideString(b'transcript', u'')
        self.lines[0].colour = section.readVector4(b'colour')
        self.lines[0].font = section.readString(b'font', b'default_small.font')
        return
    ...
    def invalidate(self):
        ...
        textLine = b''
        firstWord = 0
        spaceWidth = self.lines[0].stringWidth(b' ')
        for i in xrange(0, len(words)):
            newWordLength = self.lines[0].stringWidth(words[i])
            w = w + newWordLength
            newLine = w > maxWidth or words[i] == b'[n]'
            if newLine:
