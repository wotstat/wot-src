...
class InternalBrowser(PyGUIBase):
    factoryString = b'PyGUI.InternalBrowser'
    ...
    def onLoad(self, dataSection):
        self.exactWidth = dataSection.readInt(b'exactWidth', 0)
        self.exactHeight = dataSection.readInt(b'exactHeight', 0)
        if self.exactHeight == 0 and self.exactWidth == 0:
            resQuality = b'EFFECT_SCROLLING'
        else:
            resQuality = b'EFFECT_QUALITY'
        componentWidth, componentHeight = Utils.pixelSize(self.component)
        self.webPage = BigWorld.WebPageProvider(componentWidth, componentHeight, True, False, b'LLMozlib', u'http://www.google.com', resQuality)
