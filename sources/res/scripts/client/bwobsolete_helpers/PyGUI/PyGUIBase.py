...
class PyGUIBase(object, Listenable):
    ...
    def getWindow(self):
        ...
        if isinstance(self, Window.Window):
            ...
            if self.component.parent and self.component.parent.script:
    ...
    def handleMouseEnterEvent(self, comp):
        if getattr(self, b'toolTipInfo', None):
            import ToolTip
    ...
    def onLoad(self, dataSection):
        if dataSection.has_key(b'toolTipInfo'):
            import ToolTip
    ...
    def onSave(self, dataSection):
        if hasattr(self, b'toolTipInfo') and self.toolTipInfo is not None:
            toolTipInfoSection = dataSection.createSection(b'toolTipInfo')
    ...
    def _bindEvents(self, cls):
        for name, function in cls.__dict__.iteritems():
            if hasattr(function, b'_PyGUIEventHandler'):
                for componentName, eventName, args, kargs in function._PyGUIEventHandler:
                    component = self.component
                    for name in componentName.split(b'.'):
                        component = getattr(component, name, None)
                        if component is None:
                            break

                    if component is None:
                        ERROR_MSG(b"PyGUIEvent: '%s' has no component named '%s'." % (str(self), componentName))
                        continue
