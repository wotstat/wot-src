import BigWorld, GUI, Keys, Utils
from PyGUIBase import PyGUIBase
ITEM_MARGIN = 0.05

class ScrollingList(PyGUIBase):
    factoryString = b'PyGUI.ScrollingList'
    ...
    def createItem(self):
        ...
        setattr(self.items, b'm%d' % (len(self.items.children),), g)
    ...
    def selectItem(self, idx=0, bringIntoView=True, animate=True, forceReselect=False):
        ...
        try:
            ...
            print b'ERROR: ScrollingList.selectItem callback', e

        return

    def executeSelected(self, playSound=True):
        BigWorld.playSound(b'ui/boop')
    ...
    def handleTraversalKeys(self, event):
        if not event.isKeyDown():
            ...
            if event.key in [Keys.KEY_JOYDDOWN, Keys.KEY_DOWNARROW, Keys.KEY_S]:
                ...
                BigWorld.playSound(b'ui/tick')
                return True
            if event.key in [Keys.KEY_JOYDUP, Keys.KEY_UPARROW, Keys.KEY_W]:
                if len(self.items.children) == 0:
                    return 1
                oidx = None
                idx = self.selection
    ...
    def handleKeyEvent(self, event):
        ...
        if event.isKeyDown():
            ...
            if key == [Keys.KEY_JOYA] or key == Keys.KEY_RETURN and not BigWorld.isKeyDown(Keys.KEY_LALT) and not BigWorld.isKeyDown(Keys.KEY_RALT):
                ...
                return 1
            if key in [Keys.KEY_JOYB, Keys.KEY_JOYBACK, Keys.KEY_ESCAPE,
             Keys.KEY_BACKSPACE]:
                if self.backFn != None:
                    self.active(0)
                    BigWorld.playSound(b'ui/boop')
    ...
    def onLoad(self, section):
        self.itemGuiName = section.readString(b'itemGui', b'')
        return
    ...
    def onBound(self):
        ...
        try:
            ...
            print b'the scrolling list should have a items area!!!'

        if hasattr(self.component, b'scrollUp'):
            self.scrollUp = self.component.scrollUp
        if hasattr(self.component, b'scrollDown'):
