...
class ToolTipInfo(object):
    ...
    def onLoad(self, dataSection):
        ...
        self.infoDictionary = {}
        if dataSection.has_key(b'items'):
            itemsDataSection = dataSection._items
            for key, value in itemsDataSection.items():
                self.infoDictionary[key] = value.asString

        self.infoArea = None
        if dataSection.has_key(b'infoArea'):
            infoArea = [
             0, 0, 0, 0]
            infoAreaDataSection = dataSection._infoArea
            infoArea[0] = infoAreaDataSection.readFloat(b'top')
            infoArea[1] = infoAreaDataSection.readFloat(b'left')
            infoArea[2] = infoAreaDataSection.readFloat(b'bottom')
            infoArea[3] = infoAreaDataSection.readFloat(b'right')
            self.infoArea = tuple(infoArea)
        self.delayType = None
        if dataSection.has_key(b'delayType'):
            if dataSection._delayType.asString == b'delay':
                self.delayType = ToolTip.DELAYED_TOOL_TIP
            else:
                self.delayType = ToolTip.IMMEDIATE_TOOL_TIP
        self.placement = None
        if dataSection.has_key(b'placement'):
            self.placement = dataSection._placement.asString
        return

    def onSave(self, dataSection):
        dataSection.writeString(b'templateName', self.templateName)
        if len(self.infoDictionary) > 0:
            itemsDataSection = dataSection.createSection(b'items')
            for key, value in self.infoDictionary.iteritems():
                itemsDataSection.writeString(key, value)

        if self.infoArea:
            infoAreaDataSection = dataSection.createSection(b'items')
            infoAreaDataSection.writeFloat(b'top', self.infoArea[0])
            infoAreaDataSection.writeFloat(b'left', self.infoArea[1])
            infoAreaDataSection.writeFloat(b'bottom', self.infoArea[2])
            infoAreaDataSection.writeFloat(b'right', self.infoArea[3])
        if self.delayType:
            delayString = b'delay' if self.delayType == ToolTip.DELAYED_TOOL_TIP else b'immediate'
            dataSection.writeString(b'delayType', delayString)
        if self.placement:
            dataSection.writeString(b'placement', self.placement)
        return
...
class ToolTip(Window):
    ...
    def onSave(self, dataSection):
        ...
        if len(self.items) > 0:
            itemSection = dataSection.createSection(b'items')
            for item in self.items:
...
class ToolTipManager(object):
    ...
    def addToolTipTemplate(self, templateName, guiFileName):
        if not self.toolTipGUIs.has_key(templateName):
            ...
            toolTipGUI.horizontalAnchor = b'CENTER'
            toolTipGUI.verticalAnchor = b'CENTER'
    ...
    def setupToolTip(self, component, toolTipInfo):
        if not self.toolTipGUIs.has_key(toolTipInfo.templateName):
            ERROR_MSG(b"Setting up tool tip with missing template '%s'" % (toolTipInfo.templateName,))
            return
        else:
            ...
            if toolTipInfo.infoArea:
                ...
                infoArea = (
                 0.0, 0.0, component.width, component.height)
