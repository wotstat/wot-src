import BigWorld, GUI, Math, string
WHITESPACE = string.whitespace.decode(b'latin_1')
...
def autoSelectFont(fonts, idealCharactersPerLine, regionWidth, textComponent):
    ...
    for fontName in fonts:
        ...
        fontWidth, fontHeight = textComponent.stringDimensions(b'@')
...
def applyMapping(component, mappingType, mapping):
    if mappingType == GUI.Simple.eSizeMode.PIXEL:
        ...
        if texture:
            ...
            clipMapping[0] = (
             float(mapping[0][0]) / width, float(mapping[0][1]) / height)
...
def moveBy(component, (x, y, z)):
    component.position = (
     component.position[0] + x, component.position[1] + y, component.position[2] + z)
...
def nearestRelativeParent(component, depth=0):
    if depth > 0 and hasattr(component, b'maxScroll'):
        return component
    else:
...
class BlinkingCursor(object):
    ...
    def __init__(self):
        self.comp = GUI.Simple(b'system/maps/col_white.bmp')
...
def fromUTF8Array(array):
    return (b'').join([b'%s' % chr(el) for el in array])
