...
def _niceFunctionString(f):
    if isinstance(f, partial):
        return b'partial %s %s %s:%d' % (f.func.func_name, f.args, f.func.func_code.co_filename, f.func.func_code.co_firstlineno)
    else:
        if isinstance(f, types.LambdaType):
            return b'lambda %s:%d' % (f.func_code.co_filename, f.func_code.co_firstlineno)
        if isinstance(f, types.FunctionType):
            return b'function %s %s:%d' % (f.func_name, f.func_code.co_filename, f.func_code.co_firstlineno)
        if isinstance(f, types.GeneratorType):
            return b'generator %s %s:%d' % (f.gi_frame.f_code.co_name, f.gi_frame.f_code.co_filename, f.gi_frame.f_lineno)
        if isinstance(f, _BWCoroutineFunction):
            return _niceFunctionString(f.coroutineFunction)
        if isinstance(f, _BWCoroutineMemberFunction):
            return _niceFunctionString(f.coroutineFunction)
        return str(f)
