import BigWorld, PSFX, particles, Pixie, math
...
def preload(list):
    list += [b'objects/models/fx/03_pchangs/shockwave.model']
    return
...
def fireballTripTime(source, target, srcoff=Vector3(0, 1.5, 0), dstoff=Vector3(0, 1.2, 0)):
    ...
    if tripTime == 0:
        ...
        tripTime = calculateTripTime(sourcePosition, targetPosition, speed)
    if tripTime == 0:
        print b'No speed solution for fireball'
        return 0
    return tripTime
...
def plasmaExplode(owner, targetModel, delTargetModel):
    m = BigWorld.Model(b'objects/models/fx/03_pchangs/shockwave.model')
    ...
    shake(targetModel)
    ps2 = Pixie.create(b'particles/plasma_blow.xml')
...
fxMap = {b'cannon': (
             smokeTrail, standardExplosion, 1, 1), 
   b'cannon_he': (
                smokeTrail, standardExplosion, 1, 1), 
   b'cannon_emp': (
                 smokeTrail, empExplosion, 1, 1), 
   b'cannon_plasma': (
                    standardTrail, plasmaExplosion, 0, 0), 
   b'throw': (
            standardTrail, None, 1, 1)}
