...
g_replayCtrl = None
REPLAY_FILE_EXTENSION = b'.wotreplay'
...
class CallbackDataNames(object):
    ...
    SHOW_AUTO_AIM_MARKER = b'showAutoAimMarker'
    HIDE_AUTO_AIM_MARKER = b'hideAutoAimMarker'
    ON_TARGET_VEHICLE_CHANGED = b'onTargetVehicleChanged'
    MT_CONFIG_CALLBACK = b'mapsTrainingConfigurationCallback'
    SHOW_BATTLE_HINT = b'show_battle_hint'
    HIDE_BATTLE_HINT = b'hide_battle_hint'
...
class BattleReplay(object):
    ...
    def play(self, fileName=None):
        ...
        if fileName is not None and fileName.rfind(b'.wotreplaylist') != -1:
            self.__playList = []
            self.__isPlayingPlayList = True
            try:
                with open(fileName, mode=b'r', encoding=b'utf-8') as f:
                    s = f.read()
    ...
    def onClientReady(self):
        ...
        if self.isRecording:
            ...
            self.__arenaStartTime = b'%02d.%02d.%04d %02d:%02d:%02d' % (
    ...
    def updateArenaInfo(self, vehicleName=None):
        ...
        arenaInfo = {b'dateTime': (self.__arenaStartTime), 
           ...
           b'vehicles': (self.__getArenaVehiclesInfo()), 
           b'battleType': (arena.bonusType), 
           b'clientVersionFromExe': (BigWorld.wg_getProductVersion()), 
           b'clientVersionFromXml': (getFullClientVersion()), 
    ...
    def onPlayerVehicleIDChanged(self):
        player = BigWorld.player()
        if self.isPlaying and hasattr(player, b'positionControl') and player.inputHandler.ctrl is not None:
    ...
    def __logSVNInfo(self):
        if self.isServerSideReplay:
            ...
            _logger.info(b'Current branch: %s', currentBranch)
            _logger.info(b'Current revision: %s', str(currentRevision))
            _logger.info(b'Replay branch: %s', replayBranch)
            _logger.info(b'Replay revision: %s', str(replayRevision))
            return
        ...
        if isPlayerVehicle:
            ...
            if personals is not None:
                for personal in viewvalues(personals):
                    for field in (b'damageEventList', b'xpReplay', b'creditsReplay', b'tmenXPReplay', b'flXPReplay', b'goldReplay', b'crystalReplay', b'eventCoinReplay', b'bpcoinReplay', b'freeXPReplay', b'avatarDamageEventList', b'equipCoinReplay', b'battlePassPointsReplay'):
                        personal[field] = None

                    for currency in viewvalues(personal.get(b'currencies', {})):
                        currency[b'replay'] = None

                    if personal.get(b'outfit', b''):
                        personal[b'outfit'] = None
