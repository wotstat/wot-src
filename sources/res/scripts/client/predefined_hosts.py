...
class _CSISRequestWorker(threading.Thread):
    ...
    def _makeUrl(self):
        url = self.__url
        if self.__params:
            data = urlencode([(b'periphery', param) for param in self.__params])
    ...
    def readScriptConfig(self, dataSection, userDataSection=None):
        ...
        for name, subSec in svrList:
            ...
            if host is not None:
                ...
                if not areaID:
                    areaID = None
                app = self._makeHostItem(name, shortName, host, urlToken=subSec.readString(b'url_token'), urlIterator=urls if len(urls) > 1 else None, keyPath=keyPath, areaID=areaID, peripheryID=subSec.readInt(b'periphery_id', 0))
                idx = len(self._hosts)
                url = app.url
