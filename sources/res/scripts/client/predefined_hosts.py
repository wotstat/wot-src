...
AUTO_LOGIN_QUERY_URL = b'auto.login.app:0000'
...
PingData = namedtuple(b'PingData', b'value, status')
...
def _isReplay(opType=b''):
    import BattleReplay
    if BattleReplay.isLoading() or BattleReplay.isPlaying():
        LOG_DEBUG((b'Replay is currently playing. {} requesting operations are forbidden').format(opType))
        return True
    return False
...
def _CSISResponseParser(out, requestedIDs):
    ...
    itemsSec = None
    if root is not None:
        itemsSec = root[b'items']
    if itemsSec is not None:
        for _, subSec in itemsSec.items():
            rType = subSec.readString(b'type')
            name = subSec.readInt(b'name')
            if rType == b'periphery' and name:
                result[name] = subSec.readInt(b'availability', HOST_AVAILABILITY.UNKNOWN)
...
class _CSISRequestWorker(threading.Thread):
    ...
    def _makeUrl(self):
        url = self.__url
        if self.__params:
            data = parse.urlencode([(b'periphery', param) for param in self.__params])
            url = (b'{0:>s}?{1:>s}').format(self.__url, data)
        return url
...
_HostItem = namedtuple(b'HostItem', (b' ').join([
 42, 
 43, 
 44, 
 45, 
...
def getHostURL(item, token2=None, useIterator=False):
    ...
    if token2 and item.urlToken:
        ...
        LOG_DEBUG(b'Gets alternative LoginApp url:', result)
    else:
        ...
        if useIterator and urls is not None:
            if urls.end():
                urls.cursor = 0
            url = urls.nextUrl()
            LOG_DEBUG(b'Gets next LoginApp url:', url)
    return result
...
class _PingRequester(object):
    ...
    def request(self, peripheries, forced=False):
        ...
        if not self.__isRequestPingInProgress and peripheries:
            ...
            if time.time() - self.__lastUpdateTime >= cooldownTime:
                ...
                try:
                    LOG_DEBUG(b'Ping starting', peripheries)
    ...
    def __pingCallback(self, result):
        self.__isRequestPingInProgress = False
        LOG_DEBUG((b'Ping performed {}').format(result))
...
class _PreDefinedHostList(object):
    ...
    def autoLoginQuery(self, callback):
        if callback is None:
            LOG_WARNING(b'Callback is not defined.')
            return
        else:
            if self.__autoLoginQueryState != AUTO_LOGIN_QUERY_STATE.DEFAULT:
                LOG_WARNING((b'Auto login query in process. Current state: {}').format(self.__autoLoginQueryState))
                return
            if len(self._hosts) < 2:
                callback(self.first())
                return
            if self.__recommended:
                LOG_DEBUG(b'Gets recommended from previous query', self.__recommended)
                host = self.__choiceFromRecommended()
                LOG_DEBUG(b'Recommended host', host)
                callback(host)
                return
    ...
    def readScriptConfig(self, dataSection, userDataSection=None):
        ...
        self.__csisUrl = dataSection.readString(b'csisUrl')
        self._hosts = []
        self._urlMap.clear()
        self._nameMap.clear()
        self._peripheryMap.clear()
        svrList = _readSvrList(dataSection, b'login') + _readSvrList(userDataSection, b'development/login')
        for name, subSec in svrList:
            name = subSec.readString(b'name')
            shortName = subSec.readString(b'short_name')
            urls = _LoginAppUrlIterator(subSec.readStrings(b'url'))
            host = urls.primary
            if host is not None:
                if not name:
                    name = host
                keyPath = subSec.readString(b'public_key_path')
                if not keyPath:
                    keyPath = None
                areaID = subSec.readString(b'game_area_id')
                if not areaID:
                    areaID = None
                app = self._makeHostItem(name, shortName, host, urlToken=subSec.readString(b'url_token'), urlIterator=urls if len(urls) > 1 else None, keyPath=keyPath, areaID=areaID, peripheryID=subSec.readInt(b'periphery_id', constants.STANDALONE_CLUSTER_ID))
                idx = len(self._hosts)
                url = app.url
                if url in self._urlMap:
                    LOG_WARNING(b'Host url is already added. This host is ignored', url)
                    continue
                self._urlMap[url] = idx
                urlToken = app.urlToken
                if urlToken:
                    if urlToken in self._urlMap:
                        LOG_WARNING(b'Alternative host url is already added. This url is ignored', app.url)
                    else:
    ...
    def first(self):
        ...
        return self._makeHostItem(b'', b'', b'')

    def byUrl(self, url):
        result = self._makeHostItem(b'', b'', url)
    ...
    def byName(self, name):
        result = self._makeHostItem(name, b'', b'')
    ...
    def shortList(self):
        ...
        if AUTO_LOGIN_QUERY_ENABLED and len(result) > 1 and len(self.peripheries()) > 1:
            result.insert(0, (AUTO_LOGIN_QUERY_URL, i18n.makeString(b'#menu:login/auto'), HOST_AVAILABILITY.IGNORED, None))
        return result
    ...
    def roamingHosts(self):
        p = BigWorld.player()
        result = []
        if hasattr(p, b'serverSettings') and b'roaming' in p.serverSettings:
            for peripheryID, name, shortName, host, keyPath in p.serverSettings[b'roaming'][3]:
    ...
    def _makeHostItem(self, name, shortName, url, urlToken=b'', urlIterator=None, keyPath=None, areaID=None, peripheryID=0):
        if not shortName:
            shortName = name
    ...
    def _determineRecommendHost(self):
        ...
        LOG_NOTE(b'Peripheries for autoconnect: ', peripheries)
        ...
        if not recommendLen:
            if queryResult:
                LOG_DEBUG(b'List of recommended is empty. Gets host by ping')
                self.__recommended = self.__filterRecommendedByPing(queryResult)
                LOG_DEBUG(b'Recommended by ping', self.__recommended)
                result = self.__choiceFromRecommended()
            else:
                LOG_DEBUG(b'Gets first as recommended')
                result = self.first()
        else:
            LOG_DEBUG(b'Recommended by CSIS', self.__recommended)
            if recommendLen > 1:
                self.__recommended = self.__filterRecommendedByPing(self.__recommended)
                LOG_DEBUG(b'Recommended by ping', self.__recommended)
            result = self.__choiceFromRecommended()
        LOG_NOTE(b'Chosen periphery for autoconnect: ', result)
        return result
    ...
    def __receiveCsisResponse(self, response):
        LOG_DEBUG(b'CSIS query received', response)
        self._isCSISQueryInProgress = False
