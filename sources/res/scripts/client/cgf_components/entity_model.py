...
class EntityPickerTarget(object):
    width = ComponentProperty(type=CGFMetaTypes.FLOAT, editorName=b'Width', value=1.0)
    height = ComponentProperty(type=CGFMetaTypes.FLOAT, editorName=b'Height', value=1.0)
    depth = ComponentProperty(type=CGFMetaTypes.FLOAT, editorName=b'Depth', value=1.0)
...
class EntityPickerTargetManager(CGF.ComponentManager):
    ...
    def onAdded(self, target, gameObject):
        ...
        if entity is not None:
            ...
            model.addMotor(BigWorld.Servo(createSRTMatrix((
             target.width, target.height, target.depth), (
             entityMatrix.yaw, entityMatrix.pitch, entityMatrix.roll), entityMatrix.translation)))
            entity.model = model
            entity.targetCaps = [
             1]
    ...
    def onRemoved(self, _, gameObject):
        entity = _getEntity(gameObject)
        if entity is not None and hasattr(entity, b'model'):
            entity.model = None
        return
