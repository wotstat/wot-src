...
class DynamicCollisionComponent(object):
    asset = ComponentProperty(type=CGFMetaTypes.STRING, editorName=b'Asset', value=b'', annotations={b'path': b'*.model'})
    ownerID = ComponentProperty(type=CGFMetaTypes.INT, editorName=b'OwnerID', value=0)
    ignore = ComponentProperty(type=CGFMetaTypes.BOOL, editorName=b'Ignored by Aim', value=False)
    matrix = Math.Matrix()
...
class CollisionComponentManager(CGF.ComponentManager):
    ...
    def __onResourcesLoaded(self, gameObject, vehicle, resourceRefs):
        ...
        collision = gameObject.createComponent(BigWorld.CollisionComponent, resourceRefs[b'dynamicCollision'])
        ...
        if vehicle and hasattr(vehicle, b'appearance'):
...
class ProjectileTargetRegistrar(CGF.ComponentManager):
    ...
    def onAdded(self, gameObject, target, collision):
        ...
        if vehicle is not None:
            ...
            target.resourceID = collision.asset.replace(b'.model', b'.havok')
        return


@registerRule
class DSCollisionRule(Rule):
    category = b'GameLogic'
