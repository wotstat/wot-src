...
class TooltipComponent(object):
    ...
    editorTitle = b'Tooltip'
    category = b'Common'
    tooltip = ComponentProperty(type=CGFMetaTypes.STRING, editorName=b'selectionId')
...
class TooltipManager(CGF.ComponentManager):
    ...
    def onTooltipAdded(self, _, tooltipComponent):
        g_eventBus.handleEvent(LobbySimpleEvent(LobbySimpleEvent.ENTITY_TOOLTIP_SHOW, ctx={b'selectionId': (tooltipComponent.tooltip)}))
        return
