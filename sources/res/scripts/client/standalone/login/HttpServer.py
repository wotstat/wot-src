...
class HttpServer(HTTPServer):
    ...
    def __init__(self, name, requestHandlerClass):
        ...
        for port_number in self.__PORTS_POOL:
            try:
                ...
                self._currentStatus = (b'Failed to bind socket to {0}:{1}').format(self.__BIND_ADDRESS, port_number)
                self._logStatus()
    ...
    def __setStatus(self, status):
        self._currentStatus = (b'{0} (http://localhost:{1}): {2}.').format(self.__name, self.server_port, status)
        return

    def _logStatus(self):
        sys.stdout.write(self._currentStatus + b'\n')
        return
