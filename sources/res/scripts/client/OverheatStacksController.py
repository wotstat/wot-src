...
@ReprInjector.simple(b'level', b'gainState', b'startTime', b'endTime', b'delayTimerElapsed', b'stackTimeElapsed')
class OverheatStacksState(typing.NamedTuple(b'OverheatStacksState', (
 (
  b'level', int), (b'gainState', STATE), (b'startTime', int), (b'endTime', int),
 (
  b'delayTimerElapsed', float), (b'stackTimeElapsed', float), (b'delayTimerDuration', float),
 (
  b'stackDuration', float), (b'dmgLevelBonus', float), (b'speedThreshold', float), (b'maxLevel', int),
 (
  b'heatingTime', float), (b'coolingTime', float))), IMechanicState):

    @classmethod
    def fromComponentStatus(cls, curLevel, gainState, timeElapsed, delayTimerElapsed, timeNextGain, params):
        return cls(curLevel, gainState, timeNextGain[b'startTime'] if timeNextGain is not None else 0, timeNextGain[b'endTime'] if timeNextGain is not None else 0, delayTimerElapsed if delayTimerElapsed is not None else 0.0, timeElapsed if timeElapsed is not None else 0.0, params.delayTimerDuration if params else 0.0, params.gainTime if params else 0.0, params.dmgLevelBonus if params else 0.0, params.gainMaxSpd if params else 0.0, params.levelMax if params else 0, params.heatingTime if params else 0.0, params.coolingTime if params else 0.0)
