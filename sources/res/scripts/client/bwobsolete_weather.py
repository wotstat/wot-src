import BigWorld, ResMgr, FX, random
from functools import partial
import traceback, Bloom
from Listener import Listenable
import Math
if BigWorld.component != b'editor':
    import game as Personality
else:
    import Personality
if BigWorld.component != b'editor':
    weatherXML = ResMgr.openSection(b'scripts/data/weather.xml')
else:
    weatherXML = ResMgr.openSection(b'scripts/data/weather.xml', True)

class WeatherSystem():

    def __init__(self, ds):
        self.name = ds.name
        self.skyBoxes = list(ds.readStrings(b'skyBox'))
        self.skyBoxModels = []
        self.rain = ds.readFloat(b'rain', 0.0)
        self.sun = ds.readVector4(b'sun', (1, 1, 1, 1))
        self.ambient = ds.readVector4(b'ambient', (1, 1, 1, 1))
        self.fog = ds.readVector4(b'fog', (1, 1, 1, 1))
        self.windSpeed = ds.readVector2(b'windSpeed', (0.0, 0.0))
        self.windGustiness = ds.readFloat(b'windGustiness', 0.0)
        self.skyBoxFogFactor = ds.readFloat(b'fogFactor', 0.15)
        self.bloom = ds[b'bloom']
        self.bloomPreset = ds.readString(b'bloom', b'Standard')
        self.fader = Math.Vector4Morph()
        self.fader.duration = 1.0
        f = self._fogAmount(self.fog[3])
        self.fader.target = (f, 0, 1, 0)
        self.fx = None
        self.fxName = ds.readString(b'sfx')
        self.loaded = False
        return
        ...
        if self.loaded:
            ...
            for i in self.skyBoxes:
                try:
                    ...
                    resourceRefs[i] = BigWorld.Model(b'')
    ...
    def _onLoadSkyBoxes(self, callback, resourceRef):
        ...
        for sb in self.skyBoxes:
            try:
                ...
                self.skyBoxModels.append(BigWorld.Model(b''))
    ...
    def fadeIn(self, fadingIn, fadeSpeed, immediate=False):
        ...
        if fadingIn:
            if not self.loaded:
                print b"calling fadeIn on a weather system that isn't loaded.  please call prepareResources() first", self.name
...
class Weather(Listenable):
    ...
    def toggleRandomWeather(self, force=None):
        ...
        if stateChanged:
            Personality.addChatMsg(-1, b'Random weather turned ' + (b'on', b'off')[turningOff])
        return
    ...
    def override(self, systemName, immediate=False):
        if systemName == None or systemName == b'':
            self.localOverride = None
