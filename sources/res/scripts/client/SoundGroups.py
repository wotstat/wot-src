...
CUSTOM_MP3_EVENTS = (b'sixthSense', b'soundExploring')
...
class SoundGroups(object):
    ...
    def __init__(self):
        ...
        self.__categories = {b'vehicles': (b'outside/vehicles', b'vehicles'), 
           ...
           b'micVivox': (), 
           b'masterFadeVivox': ()}
        defCategoryVolumes = {b'music': 0.5, b'masterVivox': 0.7, b'micVivox': 0.4}
    ...
    def setVolume(self, categoryName, volume, updatePrefs=True):
        ...
        if updatePrefs:
            self.__volumeByCategory[categoryName] = volume
            self.savePreferences()
        if categoryName in (b'music', b'ambient'):
    ...
    def savePreferences(self):
        ...
        for categoryName, volume in viewitems(self.__volumeByCategory):
            ds.writeFloat(b'volume_' + categoryName, volume)
    ...
    def prepareMP3(self, event):
        if event in CUSTOM_MP3_EVENTS:
            if not ResMgr.isFile(b'audioww/%s.mp3' % event):
                LOG_ERROR(b'SOUND: mp3 file is not exist', b'audioww/%s.mp3' % event)
            else:
    ...
    def playSound2D(self, event, checkSoundBankName=b''):
        if DEBUG_TRACE_SOUND is True:
            LOG_DEBUG(b'SOUND: playSound2D', event, checkSoundBankName)
        if DEBUG_TRACE_STACK is True:
            import traceback
            traceback.print_stack()
