import cPickle, AccountCommands, BattleReplay
...
class PlayerBootcampAccount(PlayerAccount):
    ...
    @staticmethod
    def finishBootcamp():
        LOG_DEBUG_DEV_BOOTCAMP(b'finishBootcamp called')
        g_bootcamp.finishBootcamp()
        return

    def showGUI(self, ctx):
        LOG_DEBUG_DEV_BOOTCAMP(b'showGUI called')
        g_bootcamp.hideActionWaitWindow()
        ctx = cPickle.loads(ctx)
        guiCtx = ctx[b'gui']
        self.lobbyContext.onAccountShowGUI(guiCtx)
        self._initTimeCorrection(guiCtx)
        bootcampCtx = ctx[b'bootcamp']
        self.databaseID = bootcampCtx[b'databaseID']
        currentLesson = bootcampCtx[b'lessonNum']
        isBattleLesson = bootcampCtx[b'isBattleLesson']
        g_bootcamp.setAccount(self)
        g_bootcamp.setContext(bootcampCtx)
        events.isPlayerEntityChanging = False
        g_bootcampEvents.onAccountShowGUI(ctx)
        if g_bootcamp.isRunning():
            g_bootcamp.onBattleLessonFinished(currentLesson, bootcampCtx[b'lessonResults'])
    ...
    def onBootcampEnqueued(self, number, queueLen, avgWaitingTime):
        LOG_DEBUG_DEV_BOOTCAMP(b'onBootcampEnqueued', number, queueLen, avgWaitingTime)
