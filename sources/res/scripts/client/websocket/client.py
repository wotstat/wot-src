...
try:
    ...
    _logger.error(b'CPython implementation of websocket is not found')
...
class Listener(object):
    __slots__ = (b'__em', b'onOpened', b'onFailed', b'onClosed', b'onMessage')
...
class _Reconnection(object):
    __slots__ = (b'__client', b'__callbackID', b'__expBackOff')
    ...
    def clear(self):
        if self.__callbackID is not None:
            _logger.debug(b'Reconnection. Re-connection to %s is canceled', self.__client.url)
    ...
    def __onOpened(self, server):
        _logger.debug(b'Reconnection. Server connection is opened: server = %s', server)
        self.__expBackOff.reset()
        return
...
class Client(object):
    __slots__ = (b'__weakref__', b'__impl', b'__reconnection', b'__isReadyToDestroy')
