...
ENVIRONMENT_EFFECTS_CONFIG_FILE = b'scripts/environment_effects.xml'

def _addModelToScene(model):
    player = BigWorld.player()
    if hasattr(player, b'isOnArena'):
...
class DebugGizmo(object):
    ...
    def __init__(self, modelName=b'helpers/models/position_gizmo.model'):
...
class FlockLike(object):
    ...
    def __init__(self):
        if FlockLike.__SoundNames is None:
            ...
            flockDataSect = ResMgr.openSection(ENVIRONMENT_EFFECTS_CONFIG_FILE + b'/birds')
            for value in flockDataSect.values():
                modelName = value.readString(b'modelName', b'')
                soundName = value.readString(b'wwsound', b'')
                if modelName != b'' and soundName != b'':
        ...
        try:
            for modelId in prereqs.keys():
                if modelId in prereqs.failedIDs:
                    LOG_ERROR(b'Failed to load flock model: %s' % modelId)
                    continue
                ...
                clipResource = model.deprecatedGetAnimationClipResource(b'FlockAnimAction')
    ...
    def _addSound(self, model, soundName=b''):
        if not model.sources:
            ...
            if soundName == b'':
                ...
                if soundName is None or soundName == b'':
                    return
            try:
