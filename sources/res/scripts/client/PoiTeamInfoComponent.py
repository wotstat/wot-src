...
class PoiTeamInfoComponent(ScriptComponent):
    ...
    def onEnterWorld(self, _):
        _logger.debug(b'PoiTeamInfoComponent.onEnterWorld. TeamID=%s', self.entity.teamID)
        return

    def onLeaveWorld(self):
        _logger.debug(b'PoiTeamInfoComponent.onLeaveWorld. TeamID=%s', self.entity.teamID)
        return

    def onPoiEquipmentUsed(self, vehicleID, equipmentID):
        equipment = vehicles.g_cache.equipments().get(equipmentID)
        if equipment is None:
            _logger.error(b'Equipment %s does not exist', equipmentID)
            return
        else:
