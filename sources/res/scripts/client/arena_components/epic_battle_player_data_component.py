...
class EpicBattlePlayerDataComponent(PlayerDataComponent):
    ...
    def activate(self):
        ...
        self.addSyncDataCallback(ARENA_SYNC_OBJECTS.SECTOR, b'playerSectorInfo', self.__vehiclePlayerSectorUpdated)
        self.addSyncDataCallback(ARENA_SYNC_OBJECTS.RESPAWN, b'nextLiveUpdateTimer', self.__onLiveUpdateTimerUpdated)
        self.addSyncDataCallback(ARENA_SYNC_OBJECTS.RESPAWN, b'livesByTeamGroup', self.__onLivesPerTeamGroupUpdated)
        self.addSyncDataCallback(ARENA_SYNC_OBJECTS.RESPAWN, b'respawnOffsets', self.__onRespawnOffsetsUpdated)
        self.addSyncDataCallback(ARENA_SYNC_OBJECTS.RESPAWN, b'outOfLives', self.__onPlayerOutOfLivesAdded)
        self.addSyncDataCallback(ARENA_SYNC_OBJECTS.RESPAWN, b'outOfLives_d', self.__onPlayerOutOfLivesDeleted)
        self.addSyncDataCallback(ARENA_SYNC_OBJECTS.FRONT_LINE, b'CoM', self.__onFrontlineCenterOfMassUpdated)
        g_playerEvents.onAvatarReady += self.setPlayerLaneByPlayerGroups
        return

    def deactivate(self):
        super(EpicBattlePlayerDataComponent, self).deactivate()
        self.removeSyncDataCallback(ARENA_SYNC_OBJECTS.SECTOR, b'playerSectorInfo', self.__vehiclePlayerSectorUpdated)
        self.removeSyncDataCallback(ARENA_SYNC_OBJECTS.RESPAWN, b'nextLiveUpdateTimer', self.__onLiveUpdateTimerUpdated)
        self.removeSyncDataCallback(ARENA_SYNC_OBJECTS.RESPAWN, b'livesByTeamGroup', self.__onLivesPerTeamGroupUpdated)
        self.removeSyncDataCallback(ARENA_SYNC_OBJECTS.RESPAWN, b'respawnOffsets', self.__onRespawnOffsetsUpdated)
        self.removeSyncDataCallback(ARENA_SYNC_OBJECTS.RESPAWN, b'outOfLives', self.__onPlayerOutOfLivesAdded)
        self.removeSyncDataCallback(ARENA_SYNC_OBJECTS.RESPAWN, b'outOfLives_d', self.__onPlayerOutOfLivesDeleted)
        self.removeSyncDataCallback(ARENA_SYNC_OBJECTS.FRONT_LINE, b'CoM', self.__onFrontlineCenterOfMassUpdated)
        g_playerEvents.onAvatarReady -= self.setPlayerLaneByPlayerGroups
        return
    ...
    def getFrontlineCenters(self):
        centerOfMass = self.getSyncDataObjectData(ARENA_SYNC_OBJECTS.FRONT_LINE, b'CoM')
        return centerOfMass

    def getRespawnOffsetsForTeam(self, team):
        respawnOffsets = self.getSyncDataObjectData(ARENA_SYNC_OBJECTS.RESPAWN, b'respawnOffsets')
    ...
    def getPhysicalSectorForAllVehicles(self):
        vehicleSectorIDs = self.getSyncDataObjectData(ARENA_SYNC_OBJECTS.SECTOR, b'playerSectorInfo')
    ...
    def getGameTimeToAddPerCapture(self, idInPlayerGroup):
        timesToAdd = self.getSyncDataObjectData(ARENA_SYNC_OBJECTS.SECTOR, b'gameTimeToAddPerCapture')
        if timesToAdd is not None:
            try:
                return timesToAdd[idInPlayerGroup - 1]
            except (TypeError, IndexError, KeyError):
                LOG_CURRENT_EXCEPTION(tags=(
                 (b"Failed to get 'gameTimeToAddPerCapture' from arena sector sync data for zone '{}'!").format(idInPlayerGroup), timesToAdd))

        return 0.0

    def getGameTimeToAddWhenAllCaptured(self):
        time = self.getSyncDataObjectData(ARENA_SYNC_OBJECTS.SECTOR, b'gameTimeToAddWhenAllCaptured')
        if time is not None:
            return time
        else:
            return 0.0
    ...
    def __getPlayerLives(self):
        if self.__playerLives == -1:
            livesPerPlayer = self.getSyncDataObjectData(ARENA_SYNC_OBJECTS.RESPAWN, b'vehLives')
    ...
    def __updatePlayerOutOfLives(self, playerList, hasRespawns):
        ...
        if not arena:
            return
        gameModeStats = dict((vehID, {b'hasRespawns': hasRespawns}) for vehID in playerList)
        arena.updateGameModeSpecificStats(False, gameModeStats)
        return
