...
class PlayerDataComponent(ClientArenaComponent):
    ...
    def __init__(self, componentSystem):
        ...
        LOG_DEBUG_DEV(b'PlayerDataComponent.__playerGroupsEnabled ', self.__playerGroupsEnabled, componentSystem.arenaType.numPlayerGroups > 0)
    ...
    def activate(self):
        ...
        if self.__playerGroupsEnabled:
            self.addSyncDataCallback(ARENA_SYNC_OBJECTS.PLAYER_GROUP, b'playerGroups', self._vehiclePlayerGroupsUpdated)
        if self.__playerIngameRanksEnabled:
            self.addSyncDataCallback(ARENA_SYNC_OBJECTS.PLAYER_RANK, b'ranksByTeam', self.__onTeamRanksUpdated)
        return

    def deactivate(self):
        super(PlayerDataComponent, self).deactivate()
        if self.__playerGroupsEnabled:
            self.removeSyncDataCallback(ARENA_SYNC_OBJECTS.PLAYER_GROUP, b'playerGroups', self._vehiclePlayerGroupsUpdated)
        if self.__playerIngameRanksEnabled:
            self.removeSyncDataCallback(ARENA_SYNC_OBJECTS.PLAYER_RANK, b'ranksByTeam', self.__onTeamRanksUpdated)
        return
    ...
    def __getPlayerGroups(self):
        if not self.__playerGroupsEnabled:
            ...
            playerGroups = self.getSyncDataObjectData(ARENA_SYNC_OBJECTS.PLAYER_GROUP, b'playerGroups')
            return playerGroups
    ...
    def __getPlayerRank(self):
        if self.__playerRank is not None:
            ...
            if self.__playerRank is None and playerVehicleID is not None:
                ranksPerTeam = self.getSyncDataObjectData(ARENA_SYNC_OBJECTS.PLAYER_RANK, b'ranksByTeam')
    ...
    def __onTeamRanksUpdated(self, args):
        arena = avatar_getter.getArena()
        playerVehicleId = avatar_getter.getPlayerVehicleID()
        LOG_DEBUG_DEV(b'__onTeamRanksUpdated', args)
        if arena is not None:
            key = b'playerRank'
