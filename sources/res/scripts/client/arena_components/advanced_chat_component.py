...
EMPTY_STATE = b''
MARKER_ACTION_POSITIVE = b'positive'
MARKER_ACTION_SUPPORTING_YOU = b'supportingYou'
...
AdvancedChatCommandData = namedtuple(b'AdvancedChatCommandData', (b'command', b'commandCreatorVehID', b'callbackID', b'owners'))

class MarkerInFocus(namedtuple(b'MarkerInFocus', (b'commandID', b'targetID', b'markerType'))):
...
class AdvancedChatComponent(ClientArenaComponent):
    ...
    def __onServerCommandReceived(self, cmd):
        ...
        if command is None:
            ...
            commandCreatorID = commandData[b'int64Arg1']
    ...
    def __handleServerLocationCommand(self, cmd):
        ...
        commandData = cmd.getCommandData()
        commandCreatorID = commandData[b'int64Arg1']
        commandGlobalName = commandData[b'strArg1']
            ...
            if playerVehID in commandData.owners or playerVehID == cmdTargetID or self.__isLastPrebattleMarkerOwner(cmdMarkerType, cmdID, commandData) or not self.__isAliveVehicle(senderVehID):
                ...
                if self.__isPrebattleWaypoint(cmdMarkerType, cmdID):
                    if senderVehID in commandData.owners:
                        ...
                        _logger.debug(b'Prebattle waypoint command data is incorrect')
                        return
    ...
    def __removeActiveCommands(self):
        ...
        if self.__markerInFocus:
            ...
            if action is None:
                _logger.debug(b'Action in removeActiveCommands was None, %s', self.__markerInFocus.commandID)
            else:
    ...
    def __onCommandReceived(self, cmd):
        if cmd.getCommandType() != MESSENGER_COMMAND_TYPE.BATTLE:
            ...
            if controller is None:
                _logger.error(b'Controller not found %s', cmd)
                return
        ...
        if arenaDP is None:
            ...
            if not isValidVehicle and commandName not in (
             ...
             BATTLE_CHAT_COMMAND_NAMES.DEFEND_BASE):
                _logger.warning(b'__getCommandCreatorVehID: Vehicle state is invalid - using senderID')
            if isValidVehicle:
                return vehicleID
            return 0
    ...
    def __addReplyToCommandList(self, replierVehicleID, targetID, repliedToCommandID):
        ...
        if markerType not in self._chatCommands or targetID not in self._chatCommands[markerType] or repliedToCommandID not in self._chatCommands[markerType][targetID]:
            _logger.error(b'Trying to add reply to non-existing element of markerType(%s), targetID(%d) and repliedToCommandID(%d)', markerType.name, targetID, repliedToCommandID)
            return
    ...
    def __handleReply(self, cmd):
        ...
        targetID = cmd.getFirstTargetID()
        replyToActionName = cmd.getCommandData()[b'strArg1']
        if replierVehicleID:
        ...
        if not isAlreadyAdded and wasMuted:
            if replyToActionName in _CHAT_CMD_CREATE_IF_NO_ORIGINAL_COMMAND_VEHICLES:
                protoData = {b'int32Arg1': targetID, b'int64Arg1': replierVehicleID, b'floatArg1': 0.0, b'strArg1': b'', b'strArg2': b''}
                ...
                protoData[b'int64Arg1'] = replierVehicleID
                ...
                _logger.debug(b'Reply to action name, no action needed (%s)', replyToActionName)
        else:
    ...
    def __onSettingsChanged(self, diff):
        ...
        if battleCommunicationEnabled is None:
            ...
            _logger.debug(b'IBC settings changed, adding/removing listeners.')
    ...
    def __removeCommandMarkerCB(self, removeCommandID, removeCommandTargetID):
        if _ACTIONS.battleChatCommandFromActionID(removeCommandID) is None:
            _logger.warning(b'Command with ID %s was not found.', removeCommandID)
            return
        else:
    ...
    def __onSettingsReady(self):
        _logger.debug(b'Settings are synced, checking the IBC.')
