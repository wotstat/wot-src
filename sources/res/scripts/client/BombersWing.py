...
CurveControlPoint = namedtuple(b'CurveControlPoint', [b'position', b'direction', b'time'])
BomberDesc = namedtuple(b'BomberDesc', [13, 14, 15, 16, 17])
...
class Bomber(object):
    ...
    def startAttack(self, isAttacking):
        if self.__sound is None:
            ...
            self.__sound.setRTPC(b'RTPC_ext_plane_bombing', 1 if isAttacking else 0)
            return
    ...
    def _onModelLoaded(self, resourceRefs):
        ...
        if self._desc.modelName not in resourceRefs.failedIDs:
            ...
            LOG_ERROR(b'Could not load model %s' % self._desc.modelName)
        return
...
class CompoundBomber(Bomber):
    ...
    def _onModelLoaded(self, resourceRefs):
        ...
        if self._desc.flyAnimation not in resourceRefs.failedIDs:
            ...
            LOG_ERROR(b'Could not load animation %s' % self._desc.flyAnimation)
        if self._desc.modelName not in resourceRefs.failedIDs:
            self._model = resourceRefs[self._desc.modelName]
            self.__onFlightStarted()
        else:
