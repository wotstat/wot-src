...
PlatoonTankInfo = namedtuple(b'PlatoonTankInfo', (b'canDisplayModel', b'vehCompDescr', b'vehOutfitCD', b'seasonType', b'marksOnGun', b'clanDBID', b'playerName', b'stFrags'))
...
class PlatoonTank(ClientSelectableCameraVehicle):
    ...
    if IS_DEVELOPMENT:
        ...
        def debugDisplayTankModel(self, tankTypeName):
            if tankTypeName:
                ...
                tankInfo = PlatoonTankInfo(True, vDescriptor.makeCompactDescr(), b'', SeasonType.SUMMER, 0, 0, vDescriptor.type.userString, 0)
            else:
                tankInfo = PlatoonTankInfo(True, b'', b'', SeasonType.SUMMER, 0, 0, b'', 0)
    ...
    def _updatePlatoonTank(self, updatedTankInfoDict):
        ...
        if self.__tankInfo == tankInfo:
            return
        _logger.debug(b'Updating platoon tank: slot: %s, tankInfo: %s', self.slotIndex, str(tankInfo))
        self.__tankInfo = tankInfo
    ...
    def _setVisible(self, visible):
        if visible and self.__tankInfo and self.__tankInfo.canDisplayModel and self.__tankInfo.vehCompDescr != b'':
