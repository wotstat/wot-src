...
class ExampleDestructible(BigWorld.Entity):
    ...
    __MODEL_NAME = b'content/MilitaryEnvironment/mleSU_83_02_OpelBlitz/normal/lod0/mleSU_83_02_OpelBlitz_01.model'
    ...
    def showDamage(self, componentID, isShotDamage):
        LOG_DEBUG(b'ExampleDestructible.showDamage %d %b', componentID, isShotDamage)
        return

    def set_health(self, prev):
        LOG_DEBUG(b'ExampleDestructible.set_health %d', self.health)
        self.__onHealthChanged()
        return

    def onEnterWorld(self, prereqs):
        if prereqs.failedIDs:
            LOG_ERROR(b'Failed to load model %s' % (prereqs.failedIDs,))
            return
