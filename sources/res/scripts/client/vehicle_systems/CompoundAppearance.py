...
class CompoundAppearance(CommonTankAppearance, CallbackDelayer):
        ...
        if vehicle is None:
            ...
            if not vehicle.isEnteringWorld and self.engineAudition:
                if wheelsIdx == -1:
                    ...
                    materialType = 0
                elif not self.wheelsAnimator.valid():
                    LOG_ERROR(b'Wheels animator not valid for vehicle. Vehicle id and CD: ', vehicle.id, vehicle.typeDescriptor.makeCompactDescr())
                    return
                ...
                if not destroy and vehicle.isPlayerVehicle and any(device.groupName == b'extraHealthReserve' for device in vehicle.getOptionalDevices() if device is not None):
    ...
    def __onMultiTrackChassisSound(self, vehicle, destroy, position):
        if destroy:
            SoundGroups.g_instance.playSoundPos(b'breakdown_treads_outer', position)
        elif vehicle.isPlayerVehicle:
            SoundGroups.g_instance.playSound2D(b'repair_treads_outer')
        return
    ...
    def __onModelsRefresh(self, modelState, resourceList):
        ...
        if self._vehicle is None:
            ...
            self.collisions = self.createComponent(BigWorld.CollisionComponent, resourceList[b'collisionAssembler'])
    ...
    def deviceStateChanged(self, deviceName, state):
        if not self.isUnderwater and self.detailedEngineState is not None and deviceName == b'engine':
