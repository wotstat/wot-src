...
def setupCollisions(vehicleDesc, collisions):
    ...
    for partName, hitTester in hitTestersByPart.iteritems():
        ...
        if not hitTester.bbox:
            _logger.error(b"Couldn't find bounding box for the part '%s' (collisions=%s)", partName, collisions)
...
def assembleVehicleTraces(appearance, f, lodStateLink=None):
    ...
    for _, tracesParams in tracesConfig.tracesParams.iteritems():
        if chassisConfig.generalWheelsAnimatorConfig is None:
            ...
            vehicleTraces.addTrackTrace(b'', wrOffset, tracesParams.centerOffsetFactor, tracesParams.size, length, tracesParams.bufferPrefs, False)
            vehicleTraces.addTrackTrace(b'', wlOffset, tracesParams.centerOffsetFactor, tracesParams.size, length, tracesParams.bufferPrefs, False)
        else:
            ...
            for trace in traceConfigs:
                ...
                vehicleTraces.addTrackTrace(b'', offset, tracesParams.centerOffsetFactor, tracesParams.size, trace[1], tracesParams.bufferPrefs, False)

    isLeftFlying = DataLinks.createBoolLink(appearance.flyingInfoProvider, b'isLeftSideFlying')
    isRightFlying = DataLinks.createBoolLink(appearance.flyingInfoProvider, b'isRightSideFlying')
...
def createMultiGunRecoils(appearance, lodLink, gunNodes):
    ...
    for gunInstance in gunNodes:
        ...
        if gunAnimatorNode is not None:
            gameObject = CGF.GameObject(appearance.spaceID, b'Multigun recoil ' + gunNodeName)
...
def assembleLinkedNodesAnimator(appearance):
    ...
    for part in (typeDescriptor.gun, typeDescriptor.turret, typeDescriptor.hull):
        ...
        partJoints = drivenJoints.get(skin, drivenJoints.get(b'default', None))
...
def assembleVehicleAudition(isPlayer, appearance):
    ...
    vehicleAudition.setTracksInfo((lambda : appearance.transmissionScroll), (lambda : appearance.transmissionSlip), (lambda : appearance.getWheelsSteeringMax()), DataLinks.createBoolLink(appearance.flyingInfoProvider, b'isFlying'))
...
def assembleDrivetrain(appearance, isPlayerVehicle):
    ...
    engineState.vehicleSpeedLink = DataLinks.createFloatLink(vehicleFilter, b'averageSpeed')
    engineState.rotationSpeedLink = DataLinks.createFloatLink(vehicleFilter, b'averageRotationSpeed')
    ...
    if wheeledVehicle and isPlayerVehicle:
        ...
        gearbox.engineModeLink = DataLinks.createIntLink(engineState, b'mode')
        gearbox.vehicleSpeedLink = DataLinks.createFloatLink(vehicleFilter, b'averageSpeed')
        gearbox.flyingLink = DataLinks.createBoolLink(appearance.flyingInfoProvider, b'isFlying')
    else:
        gearbox = None
    if isPlayerVehicle:
...
def assembleTracks(resourceRefs, vehicleDesc, appearance, splineTracksImpl, instantWarmup, lodLink=None):
    ...
    if vehicleFilter is not None:
        tracks.setTrackScrollLink(DataLinks.createFloatLink(vehicleFilter, b'leftTrackScroll'), DataLinks.createFloatLink(vehicleFilter, b'rightTrackScroll'))
    ...
    crashedTracksController = getattr(appearance, b'crashedTracksController', None)
...
def assembleCustomLogicComponents(appearance, typeDescriptor, attachments, modelAnimators):
    assemblers = [
     (
      b'flagAnimation', __assembleAnimationFlagComponent),
     (
      b'prefab', __assemblePrefabComponent)]
    if IS_EDITOR:
...
def updatePrefabAttachments(editorTank):
    ...
    if tankStyle is not None and isUndamaged:
        ...
        if modelsSet == b'' or modelsSet == typeDescriptor.type.edModelsSets.activeModelsSet:
...
def __assembleAnimationFlagComponent(appearance, attachment, attachments, modelAnimators):
    ...
    if mainAnimator is None:
        ...
        flagParts = tuple(a.partNodeAlias for a in attachments if a.attachmentLogic == b'flagPart')
        ...
        if appearance.filter is not None:
            appearance.flagComponent.vehicleSpeedLink = DataLinks.createFloatLink(appearance.filter, b'averageSpeed')
