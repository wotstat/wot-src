...
class _AlignmentCameraVisuals(object):
    ...
    def draw(self, aabbCenter, vehicleSize, frameCenter):
        ...
        DebugDrawer.DebugDrawer().line().zTest(False).colour(4294967040L).points([
...
class VehicleToCameraAlignmentComponent(object):
    category = b'Vehicle'
    editorTitle = b'Vehicle To Camera Alignment Component'
    serialName = b'BW::VehicleToCameraAlignmentComponent'
...
class VehicleToCameraAlignmentSystem(CGF.System):
    ...
    def update(self):
        for _ in self.reaction(self.Veh2CamAlignmentActivate):
            _logger.debug(b'VehicleToCameraAlignmentManager is activated')
    ...
    def __handleResize(self, event):
        ctx = event.ctx
        self.__screenTopLeftCorner.x = ctx[b'xmin']
        self.__screenTopLeftCorner.y = ctx[b'ymin']
        self.__screenBottomRightCorner.x = ctx[b'xmax']
        self.__screenBottomRightCorner.y = ctx[b'ymax']
        self.__screenWidth, self.__screenHeight = BigWorld.windowSize()
