...
class TrackCrashWithDebrisComponent(object):
    ...
    def __init__(self, isLeft, pairIndex, vehicleDescriptor, wheelsGameObject, boundEffects, vehicleFilter, isPlayerVehicle, shouldCreateDebris, hitPoint, modelsSet=b'default'):
    ...
    def createDebrisGameObject(self, spaceID):
        if self.__debrisGameObject is not None:
            _logger.error(b'Debris go already created, something went wrong')
...
if not IS_CGF_DUMP:
    ...
    def performNodeUnmap(self, nodeRemapper, customEffectManager):
        for fromNode, _ in nodeRemapper.nodes.iteritems():
            customEffectManager.remapNode(fromNode, b'')
