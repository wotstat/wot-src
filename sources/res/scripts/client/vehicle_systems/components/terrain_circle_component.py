...
class TerrainCircleComponent(CallbackDelayer):
    ...
    def configure(self, radius, terrainCircleSettings):
        ...
        if terrainColor == 0:
            g_logger.warning(b'The color of Terrain Circle is 0! Terrain circle will be invisible!')
    ...
    def destroy(self):
        if self.__vehicleID is not None:
            g_logger.error(b'destroy called when still attached.')
            self.detach()
        if self.__model is not None and self.__model.node(b'').inWorld:
    ...
    def setVisible(self, visible=True):
        if self.__vehicleID is None:
            ...
            g_logger.warning(b'setVisible: TerrainCircleComponent is not attached.')
            return
        else:
    ...
    def attach(self, vehicleID):
        if self.__areaVisual is None:
            g_logger.error(b'attach: TerrainCircleComponent is not yet created. Call configure first.')
            return
        else:
            if self.__vehicleID is not None:
                g_logger.error(b'attach: TerrainCircleComponent is already attached. Needs to be detached first.')
                return
    ...
    def detach(self):
        if self.__vehicleID is None:
            g_logger.error(b'detach: TerrainCircleComponent is not attached to anything.')
            return
        else:
            if self.__isVisible:
                self.__detachAreaVisualFromModel()
    ...
    def __attachAreaVisualToModel(self):
        ...
        node = self.__model.node(b'')
        if node is not None:
            node.attach(self.__areaVisual)
        else:
            g_logger.error(b'__attachAreaVisualToModel: Failed to attach: no area visual or attachment node not found.')
        return
    ...
    def __detachAreaVisualFromModel(self):
        ...
        if self.__areaVisual and node:
            ...
            g_logger.error(b'__detachAreaVisualFromModel: Failed to detach: no area visual or attachment node not found.')
        return
