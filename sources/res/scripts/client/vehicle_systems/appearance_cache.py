...
VehicleAppearanceCacheInfo = namedtuple(b'VehicleAppearanceCacheInfo', [8, 9, 10, 11, 12])
_AssemblerData = namedtuple(b'_AssemblerData', [b'appearance', b'typeDescr', b'prereqsNames'])

class _LoadInfo(object):
    __slots__ = (b'appearance', b'onConstructed', b'taskId', b'typeDescr')
...
class AppearanceCache(IAppearanceCache):
    __slots__ = (b'__arena', b'__appearanceCache', b'__assemblerCache', b'__loadingAssemblerQueue')
    ...
    def unloadResources(self, compactDescr):
        _logger.debug(b'unloadResources(%s)', repr(compactDescr))
        self.__validateResourceCache(compactDescr)
    ...
    def __validateAppearanceCache(self, key):
        ...
        if key in self.__appearanceCache:
            checker += 1
        if checker > 1:
            raise SoftException((b'Invalid appearance cache state for id = {}!').format(key))
        return
    ...
    def __validateResourceCache(self, compactDescr):
        ...
        if compactDescr in self.__resourceCache:
            checker += 1
        if checker > 1:
            raise SoftException((b'Invalid resource cache state for id = {}!').format(repr(compactDescr)))
        return

    def __construct(self, key, onFinishedCallback):
        _logger.debug(b'__construct(%d)', key[0])
    ...
    def __load(self, key, info, onLoadedCallback=None):
        ...
        if loadInfo is not None:
            ...
            style = appearance.outfit.style if hasattr(appearance, b'outfit') else None
            styleString = style.userString if style else b'None'
            _logger.info(b'AppearanceCache::__load: vehicleId(%d), vehicle(%s), style(%s)', key[0], info.typeDescr.name, styleString)
            taskId = BigWorld.loadResourceListBG(prereqs, functools.partial(self.__onAppearanceLoaded, key), loadingPriority(key[0]))
            _logger.debug(b'loadResourceListBG vehicle = (%d), task = (%d)', key[0], taskId)
    ...
    def __onAppearanceLoaded(self, key, resourceRefs):
        _logger.debug(b'__onAppearanceLoaded(%d)', key[0])
        info = self.__loadingAssemblerQueue.pop(key, None)
        if info is None:
            raise SoftException((b'appearance {} is loaded but is missing from loadingQueue! info = {}').format(key, info))
    ...
    def __onResourceLoaded(self, compactDescr, resourceRefs):
        _logger.debug(b'__onResourceLoaded(%s)', repr(compactDescr))
        if compactDescr in self.__resourceCache:
            raise SoftException((b'resource {} is already loaded!').format(repr(compactDescr)))
