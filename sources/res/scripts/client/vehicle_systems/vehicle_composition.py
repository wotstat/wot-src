...
def createVehicleComposition(gameObject, vehicleGameObject=CGF.GameObject.INVALID_GAME_OBJECT, prefabMap=None, followNodes=True, extraSlots=None, dynSlotNodes=None, extraSlotComponents=None, queue=None):
    ...
    if IS_UE_EDITOR:
        ...
        def predicate(_, nodeName):
            return nodeName.startswith(b'HP_') or nodeName.endswith(b'Collision') or nodeName == TankPartNames.GUN or nodeName in dynSlotNodes

    else:

        def predicate(_, nodeName):
            return nodeName.startswith(b'HP_') or nodeName == TankPartNames.GUN or nodeName in dynSlotNodes
