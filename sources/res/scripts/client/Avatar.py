...
class VehicleDeinitFailureException(SoftException):
    ...
    def __init__(self):
        super(VehicleDeinitFailureException, self).__init__(b'Exception during vehicle deinit has been detected, thus leading to unstable state of it. Please, check the first exception happened in this function call instead of analyzing c++ crash.')
        return
...
class PlayerAvatar(BigWorld.Entity, ClientChat, CombatEquipmentManager, AvatarObserver, TeamHealthbarMechanic, AvatarEpicData, AvatarRecoveryMechanic, VehiclesSpawnListStorage, VehicleRemovalController, VehicleHealthBroadcastListenerComponent, AvatarChatKeyHandling, TriggersController, VisualScriptController, AvatarPostmortemComponent):
    __onStreamCompletePredef = {STREAM_ID_AVATAR_BATTLE_RESULS: b'receiveBattleResults'}
    ...
    def __init__(self):
        _logger.info(b'client Avatar.init')
        ...
        self.__rangeStreamIDCallbacks.addRangeCallback((STREAM_ID_CHAT_MIN, STREAM_ID_CHAT_MAX), b'_ClientChat__receiveStreamedData')
        ...
        self.__magneticAutoAimTags = self.lobbyContext.getServerSettings().getMagneticAutoAimConfig().get(b'enableForTags', set())
        ...
        arenaExtraData = self.arenaExtraData if hasattr(self, b'arenaExtraData') else {}
        self.bonusCapsOverrides = arenaExtraData.get(b'bonusCapsOverrides')
        self.arena = None
        return
    ...
    def onBecomePlayer(self):
        uniprof.enterToRegion(b'avatar.entering')
        _logger.info(b'[INIT_STEPS] Avatar.onBecomePlayer')
        ...
        uniprof.enterToRegion(b'avatar.arena.loading')
        ...
        if self.arena.arenaType is None:
            ...
            self.__gunDamagedShootSound = SoundGroups.g_instance.getSound2D(b'gun_damaged')
            if not g_offlineMapCreator.Active():
                self.cell.switchObserverFPV(False)
            BigWorld.worldDrawEnabled(False)
            uniprof.exitFromRegion(b'avatar.entering')
    ...
    def onPrereqsLoaded(self, resNames, resourceRefs):
        ...
        for resName in resNames:
            if resName not in failedRefs:
                ...
                LOG_WARNING(b'Resource is not found', resName)

        return

    def onBecomeNonPlayer(self):
        uniprof.exitFromRegion(b'avatar.arena.battle')
        uniprof.enterToRegion(b'avatar.exiting')
        _logger.info(b'[INIT_STEPS] Avatar.onBecomeNonPlayer')
        try:
        ...
        uniprof.exitFromRegion(b'avatar.exiting')
        return

    def onEnterWorld(self, prereqs):
        _logger.info(b'[INIT_STEPS] Avatar.onEnterWorld')
    ...
    def onLeaveWorld(self):
        _logger.info(b'[INIT_STEPS] Avatar.onLeaveWorld')
        self.__consistentMatrices.notifyLeaveWorld(self)
        self.__cancelWaitingForCharge()
        return

    def onVehicleChanged(self):
        _logger.info(b'Avatar vehicle has changed to %s', self.vehicle)
    ...
    def onSpaceLoaded(self):
        _logger.info(b'[INIT_STEPS] Avatar.onSpaceLoaded')
    ...
    def onCmdResponse(self, requestID, resultID, errorStr):
        LOG_DEBUG(b'onCmdResponse requestID=%s, resultID=%s, errorStr=%s' % (requestID, resultID, errorStr))
    ...
    def onCmdResponseExt(self, requestID, resultID, errorStr, ext):
        ...
        if callback is not None:
            ...
            exceptionInCallback = ext.get(b'exception_message', None)
    ...
    def onIGRTypeChanged(self, data):
        try:
            ...
            g_playerEvents.onIGRTypeChanged(data.get(b'roomType'), data.get(b'igrXPFactor'))
        except Exception:
            LOG_ERROR(b'Error while unpickling igr data information', data)
        ...
        try:
            ...
            if BigWorld.isKeyDown(Keys.KEY_CAPSLOCK) and isDown and constants.HAS_DEV_RESOURCES:
                if key == Keys.KEY_ESCAPE:
                    ...
                    return True
                if key == Keys.KEY_1:
                    self.base.setDevelopmentFeature(0, b'heal', 0, b'')
                    return True
                if key == Keys.KEY_2:
                    self.base.setDevelopmentFeature(0, b'reload_gun', 0, b'')
                    return True
                if key == Keys.KEY_3:
                    self.base.setDevelopmentFeature(0, b'start_fire', 0, b'')
                    return True
                if key == Keys.KEY_4:
                    self.base.setDevelopmentFeature(0, b'explode', 0, b'')
                    return True
                if key == Keys.KEY_5:
                    self.base.setDevelopmentFeature(0, b'break_left_track', 0, b'')
                    return True
                if key == Keys.KEY_6:
                    self.base.setDevelopmentFeature(0, b'break_right_track', 0, b'')
                    return True
                if key == Keys.KEY_7:
                    self.base.setDevelopmentFeature(0, b'destroy_self', 0, b'')
                    return True
                if key == Keys.KEY_8:
                    self.base.setDevelopmentFeature(0, b'kill_engine', 0, b'')
                    return True
                if key == Keys.KEY_9:
                    self.base.setDevelopmentFeature(0, b'damage_device', 500, b'ammoBayHealth')
                    return True
                if key == Keys.KEY_0:
                    self.base.setDevelopmentFeature(0, b'damage_device', 500, b'fuelTankHealth')
                    return True
                if key == Keys.KEY_MINUS:
                    self.base.setDevelopmentFeature(0, b'damage_device', 500, b'engineHealth')
                    return True
                if key == Keys.KEY_EQUALS:
                    self.base.setDevelopmentFeature(0, b'damage_device', 500, b'gunHealth')
                    return True
                if key == Keys.KEY_F12:
                ...
                if key == Keys.KEY_R:
                    if mods == 0:
                        self.base.setDevelopmentFeature(0, b'pickup', 0, b'straight')
                    elif mods == 1:
                        if BigWorld.spaceReload(self.spaceID):
                            self.base.setDevelopmentFeature(0, b'hot_reload', 0, b'')
                    return True
                if key == Keys.KEY_T:
                    self.base.setDevelopmentFeature(0, b'log_tkill_ratings', 0, b'')
                    return True
                if key == Keys.KEY_N:
                    self.isTeleport = not self.isTeleport
                    return True
                if key == Keys.KEY_K:
                    self.base.setDevelopmentFeature(0, b'respawn_vehicle', 0, b'')
                    return True
                if key == Keys.KEY_O:
                    self.base.setDevelopmentFeature(0, b'pickup', 0, b'roll')
                    return True
                if key == Keys.KEY_P:
                    self.base.setDevelopmentFeature(0, b'captureClosestBase', 0, b'')
                    return True
                if key == Keys.KEY_Q:
                    self.base.setDevelopmentFeature(0, b'teleportToShotPoint', 0, b'')
                    return True
                if key == Keys.KEY_V:
                    self.base.setDevelopmentFeature(0, b'setSignal', 3, b'')
                    return True
                if key == Keys.KEY_C:
                    self.base.setDevelopmentFeature(0, b'navigateTo', 0, pickle.dumps((tuple(self.inputHandler.getDesiredShotPoint()), None, None), -1))
                    return True
                if key == Keys.KEY_PAUSE:
                    self.base.setDevelopmentFeature(0, b'togglePauseAI', 0, b'')
                    return True
                if key == Keys.KEY_Y:
                ...
                if key == Keys.KEY_BACKSLASH:
                    self.base.setDevelopmentFeature(0, b'killEnemyTeam', 0, b'')
                    return True
                if key == Keys.KEY_J:
                    self.base.setDevelopmentFeature(0, b'stun', 0, b'')
                    return True
                if key == Keys.KEY_I:
                    self.base.setDevelopmentFeature(0, b'kill_turret', 0, b'')
                    return True
                if key == Keys.KEY_SEMICOLON:
                    self.base.setDevelopmentFeature(0, b'kill_tankman', 0, b'loader')
                    return True
            ...
            if key == Keys.KEY_O and isDown and mods == 2 and constants.HAS_DEV_RESOURCES:
                occlussionWatcher = b'Occlusion Culling/Enabled'
                BigWorld.setWatcher(occlussionWatcher, BigWorld.getWatcher(occlussionWatcher) == b'false')
                return True
            ...
            for comp in self.dynamicComponents.values():
                if hasattr(comp, b'handleKey') and comp.handleKey(isDown, key, mods):
                    return True
    ...
    def set_playerVehicleID(self, *args):
        _logger.info(b'[INIT_STEPS] Avatar.set_playerVehicleID %d', self.playerVehicleID)
                ...
                if self.isGunLocked:
                    ...
                    if not isinstance(self.inputHandler.ctrl, (VideoCameraControlMode, ArcadeControlMode,
                     ...
                     PostMortemControlMode, DeathFreeCamMode, RespawnDeathMode)):
                        ...
                        self.inputHandler.onControlModeChanged(b'arcade', preferredPos=self.inputHandler.getDesiredShotPoint())
    ...
    def targetBlur(self, prevEntity):
        if not prevEntity:
            ...
            isVehicle = prevEntity.__class__.__name__ == b'Vehicle'
    ...
    def targetFocus(self, entity):
        ...
        isVehicle = entity.__class__.__name__ == b'Vehicle'
        if self.vehicle:
        ...
        if vehicle.id != self.playerVehicleID:
            ...
            if not self.__initProgress & _INIT_STEPS.VEHICLE_ENTERED:
                ...
                if vehicle.typeDescriptor.turret.ceilless is not None:
                    WWISE.WW_setRTCPGlobal(b'ceilless', 1 if vehicle.typeDescriptor.turret.ceilless else 0)
                else:
                    WWISE.WW_setRTCPGlobal(b'ceilless', 0)
    ...
    def __makeScreenShot(self, fileType=b'jpg', fileMask=b'./../screenshots/'):
    ...
    def __startVehicleVisual(self, vehicle, resetControllers=False):
        if self.__waitVehicleInfoForStartVisual(vehicle, resetControllers):
            ...
            if vehicle.isDestroyed or not hasattr(vehicle, b'isHidden'):
                return
            vehicle.startVisual()
            if vehicle.isHidden:
    ...
    def vehicle_onLeaveWorld(self, vehicle):
        ...
        if vehicle.id == self.playerVehicleID:
            _logger.debug(b'[INIT_STEPS] Avatar.vehicle_onLeaveWorld %d', vehicle.id)
            self.__initProgress &= ~_INIT_STEPS.VEHICLE_ENTERED
    ...
    def __onSetOwnVehicleAuxPhysicsData(self, prev):
        ...
        if vehicle and vehicle.id and vehicle.isStarted:
            ...
            syncStabilisedYPR = getattr(vehicle.filter, b'syncStabilisedYPR', None)
    ...
    def onKickedFromServer(self, reason, kickReasonType, expiryTime):
        LOG_DEBUG(b'onKickedFromServer', reason, kickReasonType, expiryTime)
    ...
    def onRandomEvent(self, eventName):
        _logger.info(b'onRandomEvent happened, event name: %s', eventName)
    ...
    def logBattleLoadingFinished(self):
        ...
        _logger.info(b'log battle loading finished, arena period: %s', self.__blArenaPeriod)
        return

    def onSwitchViewpoint(self, vehicleID, position):
        LOG_DEBUG(b'onSwitchViewpoint', vehicleID, position)
    ...
    def updateVehicleHealth(self, vehicleID, health, deathReasonID, isCrewActive, isRespawn):
        ...
        LOG_DEBUG_DEV(b'[RESPAWN] client.Avatar.updateVehicleHealth', vehicleID, health, deathReasonID, isCrewActive, isRespawn, wasAlive)
        ...
        if not isAlive and wasAlive:
            ...
            if health > 0 and not isCrewActive:
                self.soundNotifications.play(b'crew_deactivated')
                self.__deviceStates = {b'crew': b'destroyed'}
                ...
                self.soundNotifications.play(b'vehicle_destroyed')
                self.__deviceStates = {b'vehicle': b'destroyed'}
    ...
    def updateVehicleGunReloadTime(self, vehicleID, timeLeft, baseTime, clipTime=None):
        LOG_DEBUG_DEV((b'updateVehicleGunReloadTime vehicleID={}, timeLeft={} baseTime={}').format(vehicleID, timeLeft, baseTime))
    ...
    def updateDualGunState(self, vehicleID, activeGun, gunStates, cooldownTimes):
        ...
        LOG_DEBUG_DEV(b'updateDualGunState', vehicleID, activeGun, gunStates, cooldownTimes)
        return
    ...
    def updateVehicleAmmo(self, vehicleID, compactDescr, quantity, quantityInClipOrNextStage, previousStage, timeRemaining, totalTime, index=0):
        if not compactDescr:
            itemTypeIdx = ITEM_TYPE_INDICES[b'equipment']
        else:
            itemTypeIdx = getTypeOfCompactDescr(compactDescr)
        processor = self.__updateConsumablesProcessors.get(itemTypeIdx)
        if processor:
            if itemTypeIdx == ITEM_TYPE_INDICES[b'equipment'] and self.arena.period == ARENA_PERIOD.BATTLE:
            ...
            LOG_WARNING(b'Not supported item type index', itemTypeIdx)
        return

    __updateConsumablesProcessors = {(ITEM_TYPE_INDICES[b'shell']): b'_PlayerAvatar__processVehicleAmmo', 
       (ITEM_TYPE_INDICES[b'equipment']): b'_PlayerAvatar__processVehicleEquipments'}

    def resetVehicleAmmo(self, oldCompactDescr, newCompactDescr, quantity, stage, timeRemaining, totalTime, index):
        LOG_DEBUG_DEV((b'resetVehicleAmmo oldCompactDescr={}, newCompactDescr={}').format(oldCompactDescr, newCompactDescr))
    ...
    def __updateVehicleStatus(self, vehicleID):
        ...
        if self.isObserver() or observedVehID == vehicleID:
            ...
            if observedVehicleData:
                typeDescr = observedVehicleData[b'vehicleType']
            else:
                LOG_DEBUG_DEV((b'[updateVehicleStatus] Vehicle #{} is not in arena vehicles').format(observedVehID))
                return False
        return typeDescr
    ...
    def updateDestroyedDevicesIsRepairing(self, vehicleID, extraIndex, progress, timeLeft, repairMode):
        ...
        if not typeDescr:
            return
        LOG_DEBUG_DEV(b'DESTROYED_DEVICE_IS_REPAIRING (%s): %s%%, %s sec' % (
        ...
        self.guiSessionProvider.invalidateVehicleState(VEHICLE_VIEW_STATE.REPAIRING, (
         typeDescr.extras[extraIndex].name[:-len(b'Health')],
    ...
    def updateDualGunStatus(self, vehicleID, status, times):
        if not self.__updateVehicleStatus(vehicleID):
            ...
            LOG_DEBUG_DEV((b'Charger updated ( {}, {} )').format(status, times))
    ...
    def updateBurnoutWarning(self, vehicleID, status):
        if not self.__updateVehicleStatus(vehicleID):
            ...
            if status > 0:
                SoundGroups.g_instance.playSound2D(b'eng_damage_risk')
            vehicle = BigWorld.entity(self.playerVehicleID)
            if vehicle is not None:
    ...
    def showOwnVehicleHitDirection(self, hitDirYaw, attackerID, damage, crits, isBlocked, isShellHE, damagedID, attackReasonID):
        ...
        if self.isObserver() and BigWorld.player().getVehicleAttached().id == attackerID:
            return
        LOG_DEBUG_DEV((b'showOwnVehicleHitDirection: hitDirYaw={}, attackerID={}, damage={}, crits={}isBlocked={}, isHighExplosive={}, damagedID={}, attackReasonID={}').format(hitDirYaw, attackerID, damage, crits, isBlocked, isShellHE, damagedID, attackReasonID))
    ...
    def showVehicleDamageInfo(self, vehicleID, damageIndex, extraIndex, entityID, equipmentID, ignoreMessages=False):
        if not self.userSeesWorld():
            ...
            LOG_DEBUG_DEV(b'[showVehicleDamageInfo] Vehicle', vehicleID, damageCode, damageIndex, extraIndex, entityID, equipmentID)
            ...
            if self.isObserver() or observedVehID == vehicleID:
                ...
                if observedVehicleData:
                    ...
                    LOG_DEBUG_DEV((b'[showVehicleDamageInfo] Vehicle #{} is not in arena vehicles').format(observedVehID))
                    return
    ...
    def showShotResults(self, results):
        ...
        for r in results:
            ...
            if flags & VHF.VEHICLE_KILLED:
                hasKill = True
                continue
            if self.team == arenaVehicles[vehicleID][b'team'] and self.playerVehicleID != vehicleID:
        ...
        if hasKill:
            ...
            if not shotsResultSound:
                _logger.warning(b'IShotsResultSoundController is missing, add it to allow sounds on hit.')
                return
            ...
            if bestSound is not None:
                ...
                def checkFn(vehID):
                    if self.arena:
                        ...
                        return vehicleInfo and vehicleInfo[b'isAlive']
                    return False
                ...
                for burnEnemyVehID in burningEnemies:
                    self.soundNotifications.play(b'enemy_fire_started_by_player', checkFn=partial(checkFn, burnEnemyVehID), boundVehicleID=burnEnemyVehID)
        ...
        if code == 100:
            ...
            if constants.HAS_DEV_RESOURCES:
                ...
                if code == DEVELOPMENT_INFO.BONUSES:
                    ...
                    LOG_DEBUG(b'showDevelopmentInfo', code, params)
                if code == DEVELOPMENT_INFO.EXPLOSION_RAY:
                    start, direction, _, collDist = params
                    debugLines = getattr(self, b'_debugLines', None)
    ...
    def syncVehicleAttrs(self, vehicleID, attrs):
        _logger.debug(b'syncVehicleAttrs - initial %s', attrs)
        vehicle = BigWorld.entity(vehicleID)
        attrs = vehicle.events.collectVehicleAttrs(attrs) if vehicle is not None else attrs
        _logger.info(b'syncVehicleAttrs - collected %s', attrs)
    ...
    def showTracer(self, shooterID, shotID, isRicochet, effectsIndex, prefabEffIndex, shellTypeIdx, shellCaliber, refStartPoint, velocity, gravity, maxShotDist, gunIndex, gunInstallationIndex):
        ...
        if not isRicochet and shooter is not None and shooter.isStarted and effectsDescr.get(b'artilleryID') is None:
            shooterGun = None
            ...
            if shooterGun is None or not shooterGun.multiGun or isAutoShootGun(shooterGun):
                node = shooter.appearance.compoundModel.node(b'HP_gunFire')
            else:
    ...
    def explodeProjectile(self, shotID, effectsIndex, prefabEffIndex, effectMaterialIndex, shellTypeIdx, shellCaliber, endPoint, velocityDir, speed, damagedDestructibles):
        if self.userSeesWorld() and self.__projectileMover is not None:
            ...
            if damagedDestructibles:
                ...
                explInfo = (endPoint, velocityDir, physParams[b'shellVelocity'], physParams[b'shellMass'],
                 physParams[b'splashRadius'], physParams[b'splashStrength'])
                AreaDestructibles.g_destructiblesManager.onProjectileExploded(explInfo, damagedDestructibles)
            else:
                BigWorld.wg_havokExplosion(endPoint, physParams[b'splashStrength'], physParams[b'splashRadius'])
        return

    def onRoundFinished(self, winnerTeam, reason):
        LOG_DEBUG(b'onRoundFinished', winnerTeam, reason)
        g_playerEvents.onRoundFinished(winnerTeam, reason)
        return

    def onKickedFromArena(self, reasonCode):
        LOG_DEBUG(b'onKickedFromArena', reasonCode)
    ...
    def onBattleEvents(self, events):
        LOG_DEBUG(b'Battle events has been received: ', events)
        observedVehID = self.guiSessionProvider.shared.vehicleState.getControllingVehicleID()
    ...
    def battleEventsSummary(self, summary):
        LOG_DEBUG_DEV((b'Summary of battle events has been received:  data={}').format(summary))
        observedVehID = self.guiSessionProvider.shared.vehicleState.getControllingVehicleID()
        if self.isObserver() or observedVehID == self.playerVehicleID:
    ...
    def updateQuestProgress(self, questID, progressesInfo):
        LOG_DEBUG(b'[QUEST] Progress:', questID, progressesInfo)
    ...
    def updateVehicleQuickShellChanger(self, vehicleID, isActive):
        ...
        if vehicle and not almostZero(vehicle.quickShellChangerFactor - 1.0) and vehicle.isAlive():
            if isActive and not vehicle.quickShellChangerIsActive and self.soundNotifications:
                self.soundNotifications.play(b'gun_intuition')
        ...
        if not self.__isOnArena:
            ...
            if self.inputHandler.ctrl.isSelfVehicle():
                for deviceName, stateName in viewitems(self.__deviceStates):
                    msgName = self.__cantMoveCriticals.get(deviceName + b'_' + stateName)
    ...
    def autoAim(self, target=None, magnetic=False):
        if self.hasBonusCap(_CAPS.DISABLE_AUTO_AIM):
            ...
            if target is None:
                ...
                vehID = 0
            elif target.publicInfo[b'team'] == self.team:
                vehID = 0
    ...
    def __playOwnVehicleAutorotationSound(self, enabled):
        ...
        if typeDesc is None or not typeDesc.gun.turretYawLimits:
            return
        soundNotification = b'hulllock_on'
        if enabled:
            soundNotification = b'hulllock_off'
    ...
    def __getBattleEventsAdditionalData(self):
        ...
        return {b'role': (observedVeh.typeDescriptor.role)}
    ...
    def makeDefaultAvatarShotParams(self, sourceID=b'Avatar', gunIndexDelayed=0, shellKindIdx=0, predictShooting=True):
        vehicle = BigWorld.entity(self.playerVehicleID)
    ...
    def shootDualGun(self, chargeActionType, isPrepared=False, isRepeat=False):
        ...
        if self.isForcedGuiControlMode():
            ...
            if self.isGunLocked or self.__isOwnBarrelUnderWater():
                if not isRepeat:
                    self.showVehicleError(self.__cantShootCriticals[b'gun_locked'])
                return
            for deviceName, stateName in viewitems(self.__deviceStates):
                msgName = self.__cantShootCriticals.get(deviceName + b'_' + stateName)
                if msgName is not None:
                    if not isRepeat:
    ...
    def selectPlayer(self, vehicleID):
        if self.isForcedGuiControlMode() and not BattleReplay.isPlaying():
            ...
            if vehicleDesc[b'isAlive'] and (vehicleDesc[b'team'] == self.team or self.isObserver()):
                self.inputHandler.selectPlayer(vehicleID)
        return

    def leaveArena(self):
        _logger.info(b'Avatar.leaveArena')
    ...
    def __reportClientStats(self):
        ...
        if stats and self.lobbyContext.collectUiStats:
            report = {key: stats[key] for key in (b'ping_lt_50', b'ping_51_100', b'ping_101_150', b'ping_151_400', b'ping_gt_400', b'lag')}
    ...
    def getOwnVehicleMatrix(self, default=None):
        ...
        if observedMatrix is not None:
            ...
            if Math.Matrix(self.__ownVehicleMProv).translation == Math.Matrix().translation:
                LOG_WARNING(b'Invalid vehicle matrix')
    ...
    def getOwnVehicleSpeeds(self, getInstantaneous=False, isAttached=False):
        ...
        if self.__fwdSpeedometerLimit is None or self.__bckwdSpeedometerLimit is None:
            self.__fwdSpeedometerLimit, self.__bckwdSpeedometerLimit = physics[b'speedLimits']
        ...
        if speed > self.__fwdSpeedometerLimit:
            ...
            lim = _MAX_SPEED_MULTIPLIER * physics[b'speedLimits'][0]
        ...
        if speed < -self.__bckwdSpeedometerLimit:
            ...
            lim = _MAX_SPEED_MULTIPLIER * physics[b'speedLimits'][1]
            if self.__bckwdSpeedometerLimit > lim:
                self.__bckwdSpeedometerLimit -= _SPEEDOMETER_CORRECTION_DELTA
        rspeedLimit = physics[b'rotationSpeedLimit'] * _MAX_ROTATION_SPEED_MULTIPLIER
    ...
    def getOwnVehicleShotDispersionAngle(self, turretRotationSpeed, withShot=0):
        ...
        if autoShootGunCtrl is not None:
            ...
            shotFactor = descr.gun.shotDispersionFactors[b'afterShotInBurst']
    ...
    def tuneupVehiclePhysics(self, jsonStr):
        self.base.setDevelopmentFeature(0, b'tuneup_physics', 0, zlib.compress(jsonStr, 9))
        return

    def tuneupVehicle(self, jsonStr):
        self.base.setDevelopmentFeature(0, b'tuneup_vehicle', 0, zlib.compress(jsonStr, 9))
        return

    def receiveNotification(self, notification):
        LOG_DEBUG(b'receiveNotification', notification)
    ...
    def messenger_onActionByServer_chat2(self, actionID, reqID, args):
        ...
        LOG_DEBUG(b'messenger_onActionByServer', actions.getActionName(actionID), reqID, args)
    ...
    def onUnitError(self, requestID, curUnitMgrID, errorCode, errorString):
        LOG_DEBUG(b'PlayerAvatar.onUnitError: requestID=%s, unitMgrID=%s, errorCode=%s, errorString=%s' % (
         requestID, curUnitMgrID, errorCode, errorString))
        return

    def onUnitCallOk(self, requestID):
        LOG_DEBUG(b'PlayerAvatar.onUnitOk: requestID=%s' % requestID)
        return
    ...
    def activateGoodie(self, goodieID, callback=None):
        if callback is None:
            ...
            def __defaultLogger(resultID, errorCode):
                LOG_DEBUG(b'Action performed: "activateGoodie" resultID=%s, errorCode=%s' % (resultID, errorCode))
                return
    ...
    __cantShootCriticals = {b'gun_destroyed': b'cantShootGunDamaged', 
       b'vehicle_destroyed': b'cantShootVehicleDestroyed', 
       b'crew_destroyed': b'cantShootCrewInactive', 
       b'no_ammo': b'cantShootNoAmmo', 
       b'gun_reload': b'cantShootGunReloading', 
       b'gun_locked': b'cantShootGunLocked', 
       b'gun_overheated': b'cantShootGunOverheated', 
       b'low_charge_shot_blocking': b'lowChargeShotBlocking'}
    ...
    def setClientReady(self):
        LOG_DEBUG_DEV(b'Avatar.setClientReady')
    ...
    def waitForPlayerChoice(self):
        _logger.info(b'[INIT_STEPS] Avatar.waitForPlayerChoice')
    ...
    def setPlayerReadyToBattle(self):
        _logger.info(b'[INIT_STEPS] Avatar.setPlayerReadyToBattle')
    ...
    def __onInitStepCompleted(self):
        LOG_DEBUG(b'Avatar.__onInitStepCompleted()', self.__initProgress)
    ...
    __cantMoveCriticals = {b'engine_destroyed': b'cantMoveEngineDamaged', 
       b'leftTrack0_destroyed': b'cantMoveChassisDamaged', 
       b'rightTrack0_destroyed': b'cantMoveChassisDamaged', 
       b'vehicle_destroyed': b'cantMoveVehicleDestroyed', 
       b'crew_destroyed': b'cantMoveCrewInactive'}
    ...
    def __setIsOnArena(self, onArena):
        if self.__isOnArena == onArena:
            ...
            if not onArena:
                if self.gunRotator is not None:
                    self.gunRotator.stop()
            elif self.gunRotator is not None:
                self.gunRotator.start()
            self.moveVehicle(self.makeVehicleMovementCommandByKeys(), False)
            return
    ...
    def __showDamageIconAndPlaySound(self, damageCode, extra, vehicleID, ignoreMessages=False):
        ...
        if extra is not None:
            if damageCode in self.__damageInfoCriticals:
                deviceName = extra.name[:-len(b'Health')]
                if damageCode == b'DEVICE_REPAIRED_TO_CRITICAL':
                    deviceState = b'repaired'
                    ...
                    deviceState = b'critical'
                    soundType = b'critical'
                ...
                self.__deviceStates[deviceName] = b'critical'
            elif damageCode in self.__damageInfoDestructions:
                deviceName = extra.name[:-len(b'Health')]
                deviceState = b'destroyed'
                soundType = b'destroyed'
                self.__deviceStates[deviceName] = b'destroyed'
                if damageCode.find(b'TANKMAN_HIT') != -1 and not ignoreMessages:
                    self.playSoundIfNotMuted(b'crew_member_contusion')
                if damageCode.find(b'TANKMAN_HIT') != -1:
        ...
        if deviceState is not None:
            ...
            if deviceState == b'repaired':
                deviceState = b'critical'
            if damageCode.find(b'DEVICE_REPAIRED') != -1 and deviceName == b'gun':
                if self.__gunDamagedShootSound is not None and self.__gunDamagedShootSound.isPlaying:
                    self.__gunDamagedShootSound.stop()
            soundNotificationCheckFn = lambda : self.__deviceStates.get(deviceName, b'normal') == deviceState
    ...
    def __getSoundTypeForTracks(self, deviceName, extra):
        if b'functionalCanMove' in extra.sounds:
            tracksToCheck = [
             b'leftTrack0', b'rightTrack0']
            if deviceName in tracksToCheck:
                tracksToCheck.remove(deviceName)
            canMove = True
            for trackName in tracksToCheck:
                if trackName in self.__deviceStates and self.__deviceStates[trackName] == b'destroyed':
                    canMove = False
                    break

            if canMove and deviceName in self.__deviceStates and self.__deviceStates[deviceName] == b'destroyed':
                return b'functionalCanMove'
        return b'functional'

    __damageInfoCriticals = (
     b'DEVICE_CRITICAL', b'DEVICE_REPAIRED_TO_CRITICAL',
     b'DEVICE_CRITICAL_AT_SHOT', b'DEVICE_CRITICAL_AT_RAMMING', b'DEVICE_CRITICAL_AT_FIRE',
     b'DEVICE_CRITICAL_AT_WORLD_COLLISION', b'DEVICE_CRITICAL_AT_DROWNING',
     b'ENGINE_CRITICAL_AT_UNLIMITED_RPM', b'ENGINE_CRITICAL_AT_BURNOUT')
    __damageInfoDestructions = (
     b'DEVICE_DESTROYED', b'DEVICE_DESTROYED_AT_SHOT', b'DEVICE_DESTROYED_AT_RAMMING',
     b'DEVICE_DESTROYED_AT_FIRE', b'DEVICE_DESTROYED_AT_WORLD_COLLISION', b'DEVICE_DESTROYED_AT_DROWNING',
     b'TANKMAN_HIT', b'TANKMAN_HIT_AT_SHOT', b'TANKMAN_HIT_AT_WORLD_COLLISION', b'TANKMAN_HIT_AT_DROWNING',
     b'ENGINE_DESTROYED_AT_UNLIMITED_RPM', b'ENGINE_DESTROYED_AT_BURNOUT', b'DEVICE_DESTROYED_AT_MINEFIELD_ZONE')
    __damageInfoHealings = (
     b'DEVICE_REPAIRED', b'TANKMAN_RESTORED', b'FIRE_STOPPED')
    __damageInfoNoNotification = (
     b'DEVICE_CRITICAL', b'DEVICE_DESTROYED', b'TANKMAN_HIT', b'FIRE',
     b'DEVICE_CRITICAL_AT_DROWNING', b'DEVICE_DESTROYED_AT_DROWNING', b'TANKMAN_HIT_AT_DROWNING')
    ...
    def __onArenaVehicleKilled(self, targetID, attackerID, equipmentID, reason, numVehiclesAffected):
        ...
        isObservedVehicle = not self.__isVehicleAlive and targetID == getattr(self.inputHandler.ctrl, b'curVehicleID', None)
        if isMyVehicle or isObservedVehicle:
            ...
            deathInfo = {b'killerID': attackerID, b'reason': reason}
    ...
    def __onArenaPeriodChange(self, period, periodEndTime, periodLength, periodAdditionalInfo):
        ...
        if period == ARENA_PERIOD.BATTLE and period > self.__prevArenaPeriod:
            if self.arenaBonusType == constants.ARENA_BONUS_TYPE.EPIC_BATTLE:
                ...
                self.soundNotifications.play(b'start_battle')
    ...
    def __controlAnotherVehicleAfteraction(self, vehicleID, callback):
        ...
        if vehicle is None:
            ...
            self.base.setDevelopmentFeature(0, b'heal', 0, b'')
            self.base.setDevelopmentFeature(0, b'stop_bot', 0, b'')
            self.inputHandler.setKillerVehicleID(None)
            self.inputHandler.onControlModeChanged(b'arcade')
            if callback is not None:
                callback()
            return
    ...
    def __dumpVehicleState(self):
        ...
        LOG_NOTE(b'Arena type: ', self.arena.arenaType.geometryName)
        LOG_NOTE(b'Vehicle position: ', matrix.translation)
        LOG_NOTE(b'Vehicle direction (y, p, r): ', (matrix.yaw, matrix.pitch, matrix.roll))
        LOG_NOTE(b'Vehicle speeds: ', self.getOwnVehicleSpeeds())
        if self.vehicleTypeDescriptor is not None:
            LOG_NOTE(b'Vehicle type: ', self.vehicleTypeDescriptor.type.name)
            LOG_NOTE(b'Vehicle turret: ', self.vehicleTypeDescriptor.turret.name)
            LOG_NOTE(b'Vehicle gun: ', self.vehicleTypeDescriptor.gun.name)
        LOG_NOTE(b'Shot point: ', self.gunRotator._VehicleGunRotator__lastShotPoint)
        return
    ...
    def __reportLag(self):
        ...
        import time
        t = time.gmtime()
        msg += b'\ttime: %d/%0d/%0d %0d:%0d:%0d UTC\n' % t[:6]
        msg += b'\tAvatar.id: %d\n' % self.id
        ...
        msg += b'\tposition: ' + str(self.position)
        LOG_NOTE(msg)
        self.base.logLag()
        return
    ...
    def __updateVehicleSetting(self, vehicleID, code, value, fromServer=True):
        ...
        isOwnMultiGun = vehicle is not None and vehicle.isMainMultiGun and b'ownVehicle' in vehicle.dynamicComponents
        if code == VEHICLE_SETTING.CURRENT_SHELLS:
            ...
            if fromServer and isBeforeBattle and not isSelectionStarted and isOwnMultiGun:
                vehicle.dynamicComponents[b'ownVehicle'].resetGunReloadTime()
    ...
    def __applyTimeAndWeatherSettings(self, overridePresetID=None):
        ...
        if not presets or presets[0].get(b'name') is None:
            return
        try:
            presetID = overridePresetID if overridePresetID is not None else self.weatherPresetID
            preset = presets[presetID]
            system = weather.newSystemByName(preset[b'name'])
            windSpeed = preset.get(b'windSpeed')
            if windSpeed is not None:
                windSpeed = windSpeed.split()
                weather.windSpeed(windSpeed)
            elif system is not None:
                weather.windSpeed(system.windSpeed)
            windGustiness = preset.get(b'windGustiness')
            ...
            LOG_CURRENT_EXCEPTION()
            LOG_DEBUG(b"Weather system's ID was:", self.weatherPresetID)
    ...
    def __processVehicleEquipments(self, vehicleID, compactDescr, quantity, stage, timeRemaining, totalTime, index=None):
        if compactDescr:
            ...
            if descriptor.name == b'aimingStabilizerBattleBooster':
    ...
    def _doCmdNoArgs(self, cmd, callback):
        self.__doCmd(b'doCmdNoArgs', cmd, callback)
        return

    def _doCmdStr(self, cmd, s, callback):
        self.__doCmd(b'doCmdStr', cmd, callback, s)
        return

    def _doCmdInt(self, cmd, int1, callback):
        self.__doCmd(b'doCmdInt', cmd, callback, int1)
        return

    def _doCmdInt2(self, cmd, int1, int2, callback):
        self.__doCmd(b'doCmdInt2', cmd, callback, int1, int2)
        return

    def _doCmdInt3(self, cmd, int1, int2, int3, callback):
        self.__doCmd(b'doCmdInt3', cmd, callback, int1, int2, int3)
        return

    def _doCmdInt3Str(self, cmd, int1, int2, int3, s, callback):
        self.__doCmd(b'doCmdInt3Str', cmd, callback, int1, int2, int3, s)
        return

    def _doCmdInt4(self, cmd, int1, int2, int3, int4, callback):
        self.__doCmd(b'doCmdInt4', cmd, callback, int1, int2, int3, int4)
        return

    def _doCmdInt2Str(self, cmd, int1, int2, s, callback):
        self.__doCmd(b'doCmdInt2Str', cmd, callback, int1, int2, s)
        return

    def _doCmdIntArr(self, cmd, arr, callback):
        self.__doCmd(b'doCmdIntArr', cmd, callback, arr)
        return

    def _doCmdIntArrStrArr(self, cmd, intArr, strArr, callback):
        self.__doCmd(b'doCmdIntArrStrArr', cmd, callback, intArr, strArr)
        return

    def _doCmdStrArr(self, cmd, strArr, callback):
        self.__doCmd(b'doCmdStrArr', cmd, callback, strArr)
        return

    def _doCmdIntStr(self, cmd, int1, s, callback):
        self.__doCmd(b'doCmdIntStr', cmd, callback, int1, s)
        return
    ...
    def reloadPlans(self, *args):
        self.base.setDevelopmentFeature(0, b'reloadPlans', 0, zlib.compress(pickle.dumps(*args)))
        return
    ...
    def change_rank_xp(self, value):
        self.base.setDevelopmentFeature(0, b'change_rank_xp', value, b'')
        return

    def flGetAbilitiesProperties(self):
        self.base.setDevelopmentFeature(0, b'flGetAbilitiesProperties', 0, b'')
        return
    ...
    def __getAdditiveShotDispersionFactor(self, descriptor):
        ...
        return factors[b'additiveShotDispersionFactor']
...
def preload(alist):
    ds = ResMgr.openSection(b'precache.xml')
    if ds is not None:
...
def getVehicleShootingPositions(vehicle):
    ...
    turretYaw, gunPitch = decodeGunAngles(vehicle.gunAnglesPacked, vd.gun.pitchLimits[b'absolute'])
