...
class PlayerAvatar(BigWorld.Entity, ClientChat, CombatEquipmentManager, AvatarObserver, TeamHealthbarMechanic, AvatarEpicData, AvatarRecoveryMechanic, AvatarRespawnMechanic, VehiclesSpawnListStorage, VehicleRemovalController, VehicleHealthBroadcastListenerComponent, AvatarChatKeyHandling, TriggersController, VisualScriptController):
    __onStreamCompletePredef = {STREAM_ID_AVATAR_BATTLE_RESULTS: b'receiveBattleResults'}
    ...
    def onBecomePlayer(self):
        ...
        _logger.info(b'[INIT_STEPS] Avatar.onBecomePlayer: arenaId(%d)', self.arenaUniqueID)
        ...
        if self.arena.arenaType is None:
            ...
            if BattleReplay.isPlaying() and self.arenaGuiType == ARENA_GUI_TYPE.BOOTCAMP:
                ...
                lessonId = arenaInfo.get(b'lessonId', None)
                bootcampCtx = arenaInfo.get(b'bootcampCtx', None)
        ...
        if self.vehicle is not None:
            ...
            if self.vehicle.typeDescriptor.hasSiegeMode:
                ...
                if siegeState == VEHICLE_SIEGE_STATE.ENABLED:
                    switchingTime = siegeModeParams.get(b'switchOffTime', 0.0)
                elif siegeState == VEHICLE_SIEGE_STATE.DISABLED:
                    switchingTime = siegeModeParams.get(b'switchOnTime', 0.0)
                else:
    ...
    def set_playerVehicleID(self, *args):
        LOG_DEBUG(b'[INIT_STEPS] Avatar.set_playerVehicleID')
    ...
    def vehicle_onAppearanceReady(self, vehicle):
        LOG_DEBUG(b'Avatar.vehicle_onAppearanceReady')
        ...
        if vehicle.id != self.playerVehicleID:
            ...
            LOG_DEBUG(b'[INIT_STEPS] Avatar.vehicle_onAppearanceReady', vehicle.id)
    ...
    def vehicle_onLeaveWorld(self, vehicle):
        if vehicle.id == self.playerVehicleID:
            LOG_DEBUG(b'[INIT_STEPS] Avatar.vehicle_onLeaveWorld', vehicle.id)
            self.__initProgress &= ~_INIT_STEPS.VEHICLE_ENTERED
    ...
    def updateVehicleAmmo(self, vehicleID, compactDescr, quantity, quantityInClipOrNextStage, previousStage, timeRemaining, totalTime, index=None):
        LOG_DEBUG_DEV((b'updateVehicleAmmo vehicleID={}, compactDescr={} quantity={}, quantityInClip={}').format(vehicleID, compactDescr, quantity, quantityInClipOrNextStage))
        if not compactDescr:
    ...
    def updateSiegeStateStatus(self, vehicleID, status, timeLeft):
        ...
        if status in (constants.VEHICLE_SIEGE_STATE.SWITCHING_ON,
         constants.VEHICLE_SIEGE_STATE.SWITCHING_OFF):
            if b'autoSiege' not in typeDescr.type.tags:
                self.__cruiseControlMode = _CRUISE_CONTROL_MODE.NONE
            self.__updateCruiseControlPanel()
    ...
    def updateTargetingInfo(self, entityId, turretYaw, gunPitch, maxTurretRotationSpeed, maxGunRotationSpeed, shotDispMultiplierFactor, gunShotDispersionFactorsAfterShot, gunShotDispersionFactorsTurretRotation, chassisShotDispersionFactorsMovement, chassisShotDispersionFactorsRotation, aimingTime):
        LOG_DEBUG_DEV(b'updateTargetingInfo', entityId, turretYaw, gunPitch, maxTurretRotationSpeed, maxGunRotationSpeed, shotDispMultiplierFactor, gunShotDispersionFactorsAfterShot, gunShotDispersionFactorsTurretRotation, chassisShotDispersionFactorsMovement, chassisShotDispersionFactorsRotation, aimingTime)
    ...
    def showVehicleDamageInfo(self, vehicleID, damageIndex, extraIndex, entityID, equipmentID, ignoreMessages=False):
        if not self.userSeesWorld():
            ...
            damageContext = {b'damageCode': damageCode, 
               b'extra': extra}
            ...
            for enemyVehID, flags in enemies.iteritems():
                ...
                if flags & VHF.ATTACK_IS_EXTERNAL_EXPLOSION:
                    if flags & VHF.MATERIAL_WITH_POSITIVE_DF_PIERCED_BY_EXPLOSION:
                        sound = b'enemy_hp_damaged_by_near_explosion_by_player'
                    elif flags & VHF.IS_ANY_PIERCING_MASK:
                        sound = b'enemy_no_hp_damage_by_near_explosion_by_player'
                    ...
                    if flags & (VHF.GUN_DAMAGED_BY_PROJECTILE | VHF.GUN_DAMAGED_BY_EXPLOSION):
                        sound = b'enemy_hp_damaged_by_projectile_and_gun_damaged_by_player'
                    elif flags & (VHF.CHASSIS_DAMAGED_BY_PROJECTILE | VHF.CHASSIS_DAMAGED_BY_EXPLOSION):
                        sound = b'enemy_hp_damaged_by_projectile_and_chassis_damaged_by_player'
                    else:
                        sound = b'enemy_hp_damaged_by_projectile_by_player'
                elif flags & VHF.MATERIAL_WITH_POSITIVE_DF_PIERCED_BY_EXPLOSION:
                    sound = b'enemy_hp_damaged_by_explosion_at_direct_hit_by_player'
                elif flags & VHF.RICOCHET and not flags & VHF.DEVICE_PIERCED_BY_PROJECTILE:
                    sound = b'enemy_ricochet_by_player'
                    if len(enemies) == 1:
                    ...
                    if flags & (VHF.GUN_DAMAGED_BY_PROJECTILE | VHF.GUN_DAMAGED_BY_EXPLOSION):
                        sound = b'enemy_no_hp_damage_at_attempt_and_gun_damaged_by_player'
                    elif flags & (VHF.CHASSIS_DAMAGED_BY_PROJECTILE | VHF.CHASSIS_DAMAGED_BY_EXPLOSION):
                        sound = b'enemy_no_hp_damage_at_attempt_and_chassis_damaged_by_player'
                    else:
                        sound = b'enemy_no_hp_damage_at_attempt_by_player'
                        if len(enemies) == 1:
                    ...
                    sound = b'enemy_no_hp_damage_at_no_attempt_and_gun_damaged_by_player'
                elif flags & (VHF.CHASSIS_DAMAGED_BY_PROJECTILE | VHF.CHASSIS_DAMAGED_BY_EXPLOSION):
                    sound = b'enemy_no_hp_damage_at_no_attempt_and_chassis_damaged_by_player'
                else:
                    if flags & (VHF.ARMOR_WITH_ZERO_DF_NOT_PIERCED_BY_PROJECTILE | VHF.DEVICE_NOT_PIERCED_BY_PROJECTILE) or not flags & VHF.IS_ANY_PIERCING_MASK:
                        sound = b'enemy_no_piercing_by_player'
                    else:
                        sound = b'enemy_no_hp_damage_at_no_attempt_by_player'
                    if len(enemies) == 1:
                        TriggersManager.g_manager.fireTrigger(TRIGGER_TYPE.PLAYER_SHOT_NOT_PIERCED, targetId=enemyVehID)
    ...
    def syncVehicleAttrs(self, _, attrs):
        LOG_DEBUG(b'syncVehicleAttrs', attrs)
    ...
    def explodeProjectile(self, shotID, effectsIndex, effectMaterialIndex, endPoint, velocityDir, damagedDestructibles):
        if self.userSeesWorld() and self.__projectileMover is not None:
            ...
            explInfo = (
             endPoint, velocityDir, physParams[b'shellVelocity'], physParams[b'shellMass'],
            ...
            if damagedDestructibles:
                ...
                BigWorld.havokExplosion(endPoint, physParams[b'splashStrength'], physParams[b'splashRadius'])
        return

    def onRoundFinished(self, winnerTeam, reason, extraData):
        LOG_DEBUG(b'onRoundFinished', winnerTeam, reason, extraData)
        ...
        if self.isForcedGuiControlMode():
            ...
            if not canShoot:
                if error in self.__cantShootCriticals:
                    self.showVehicleError(self.__cantShootCriticals[error], args={b'typeDescriptor': (self.getVehicleDescriptor())})
                return
    ...
    def __verifyShooting(self, isRepeat=False):
        ...
        if not canShoot:
            ...
            if temperatureGunController is not None and temperatureGunController.isOverheated:
                if not isRepeat:
                    self.showVehicleError(self.__cantShootCriticals[b'gun_overheated'])
                return (False, error)
            return (True, error)
    ...
    def __reportClientStats(self):
        ...
        if sessionData and self.lobbyContext.collectUiStats:
            report = {key: sessionData[key] for key in sessionData if key in (b'ping_lt_50', b'ping_51_100', b'ping_101_150', b'ping_151_400', b'ping_gt_400', b'lag')}
    ...
    def getOwnVehicleShotDispersionAngle(self, turretRotationSpeed, withShot=0):
        ...
        if dualAccuracy is not None and hasattr(dualAccuracy, b'isShouldSkipDispersion'):
            shouldSkip = dualAccuracy.isShouldSkipDispersion()
        if withShot == 0 or shouldSkip:
            shotFactor = 0.0
        elif withShot == 1:
            shotFactor = descr.gun.shotDispersionFactors[b'afterShot'] * gunShotDispersionFactorsAfterShot
        else:
            shotFactor = descr.gun.shotDispersionFactors[b'afterShotInBurst'] * gunShotDispersionFactorsAfterShot
    ...
    def messenger_onActionByServer_chat2(self, actionID, reqID, args):
        ...
        if actionName in (b'command:DEFENDING_BASE', b'command:DEFEND_BASE', b'command:ATTACKING_BASE', b'command:ATTACK_BASE'):
            if args.has_key(b'strArg1'):
                args[b'strArg1'] = b''
        LOG_DEBUG(b'messenger_onActionByServer', actionName, reqID, args)
    ...
    def getCantMoveMessage(self, deviceName, deviceStates):
        if self.vehicleTypeDescriptor.isMultiTrack and _isDeviceTrack(deviceName):
            if self.__getVehicleMovementState() == VEHICLE_MOVEMENT_STATES.STOP:
                return b'cantMoveChassisDamaged'
            return None
        return self.__cantMoveCriticals.get((b'_').join((deviceName, deviceStates[deviceName])))

    def __getVehicleMovementState(self):
        isRightSideDestroyed = self.__isTrackSideDestroyed(b'rightTrack', self.__deviceStates)
        isLeftSideDestroyed = self.__isTrackSideDestroyed(b'leftTrack', self.__deviceStates)
    ...
    def __isTrackSideDestroyed(self, trackSide, damagedDevices):
        ...
        for device, state in list(damagedDevices.items()):
            if device.startswith(trackSide) and state == b'destroyed':
                brokenTracks += 1
    ...
    def __getSoundTypeForMultiTrackRepaired(self, prevState, extra):
        if b'functionalCanMove' in extra.sounds and prevState != self.__getVehicleMovementState():
            return b'functionalCanMove'
        else:
            return
    ...
    __damageInfoDestructions = (
     ...
     b'ENGINE_DESTROYED_AT_UNLIMITED_RPM', b'ENGINE_DESTROYED_AT_BURNOUT')
    ...
    def reloadPlans(self, *args):
        self.base.setDevelopmentFeature(0, b'reloadPlans', 0, zlib.compress(cPickle.dumps(*args)))
        return
...
def _isDeviceTrack(deviceName):
    return deviceName.startswith((b'leftTrack', b'rightTrack'))
