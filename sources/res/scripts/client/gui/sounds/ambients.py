...
def _getGFViewSoundEnv(viewImplAdaptor):
    ...
    if hasattr(view, b'getDynamicSoundEnv'):
        ...
        return getattr(view, b'__sound_env__', None)
...
class GuiAmbientsCtrl(object):
    ...
    def __registerGFSoundEnv(self, view):
        ...
        if soundEnvClass is not None:
            SOUND_DEBUG(b'Custom sound environ has been detected', type(view), soundEnvClass)
            self._gfCustomEnvs[view.uniqueID] = self._buildSoundEnv(soundEnvClass)
        else:
            SOUND_DEBUG(b'Custom sound environ has not been detected', type(view))
        return
    ...
    def __removeGFSoundEnv(self, view):
        ...
        SOUND_DEBUG(b'Custom sound environ has been stopped', type(view), env)
        return
