...
class _EnvironmentListNode(object):
    __slots__ = (b'__prev', b'__next', b'__environment', b'__enterState', b'__exitState', b'__envID', b'__cbkID')
    ...
    def __setEnterState(self):
        if self.__enterState:
            LOG_DEBUG((b'Set Enter UE sound state "{}" for previous finished environment: "{}"').format(self.__enterState, self.__environment))
    ...
    def __setExitState(self):
        if self.__cbkID is not None:
            ...
            if self.__exitState:
                LOG_DEBUG((b'Set Exit UE sound state "{}" for the stopped environment "{}"').format(self.__exitState, self.__environment))
                WWISE.WW_eventGlobalSync(self.__exitState)
            return


class _EnvironmentStatesSubSys(object):
    _envStateDefs = {b'login': (
                b'ue_01_loginscreen_enter', b'ue_01_loginscreen_exit', _LOGIN), 
       b'lobby': (
                b'ue_02_hangar_enter', b'ue_02_hangar_exit', _LOBBY), 
       b'queue': (
                b'ue_03_lobby_enter', b'ue_03_lobby_exit', _QUEUE), 
       b'battleLoading': (
                        b'ue_04_loadingscreen_enter', b'ue_04_loadingscreen_exit', _BATTLE_LOADING), 
       b'battle': (
                 b'ue_05_arena_enter', b'ue_05_arena_exit', _BATTLE), 
       b'battleResults': (
                        b'ue_06_result_enter', b'ue_06_result_exit', _BATTLE_RESULT)}
    ...
    def onEnvStop(self, newEnvironment):
        ...
        if state is not None:
            if self._head is not None:
                ...
                LOG_WARNING(b'Unexpected behaviour: list node could not be None in such situation...')
        return
...
class WWISESoundSystem(SoundSystemAbstract):
    ...
    def enableDynamicPreset(self):
        wwiseEvent = b'ue_set_preset_high_dynamic_range'
        self.sendGlobalEvent(wwiseEvent)
        LOG_DEBUG((b'WWISE: triggered {0}').format(wwiseEvent))
        return

    def disableDynamicPreset(self):
        wwiseEvent = b'ue_set_preset_low_dynamic_range'
        self.sendGlobalEvent(wwiseEvent)
        LOG_DEBUG((b'WWISE: triggered {0}').format(wwiseEvent))
        return
    ...
    def setBassBoost(self, isEnabled):
        if isEnabled:
            wwiseEvent = b'ue_set_preset_bassboost_on'
        else:
            wwiseEvent = b'ue_set_preset_bassboost_off'
    ...
    def setUserSpeakersPresetID(self, presetID):
        if presetID not in SPEAKERS_CONFIG.RANGE:
            LOG_ERROR((b'Invalid value of presetID {}').format(presetID))
            return
        if self.getUserSpeakersPresetID() == presetID:
            LOG_DEBUG(b'WWISE: Sounds preset is already set. Do nothing')
        else:
            WWISE.WW_setUserSpeakersConfig(presetID)
            LOG_DEBUG((b'WWISE: New sounds preset is set. New value is {}').format(presetID))
            Sound.reloadSoundEngine()
            LOG_DEBUG(b'WWISE: Sound system is reinitialized')
            BigWorld.reinitVideoSound()
        return

    def setSoundSystem(self, soundSystemID):
        if soundSystemID == _LAPTOP_SOUND_PRESET:
            wwiseEvent = b'ue_set_preset_acoustic_device_laptop'
            soundSystemID = 0
            WWISE.WW_setSoundSystem(soundSystemID)
            WWISE.WW_eventGlobalSync(wwiseEvent)
        else:
            wwiseEvent = b'ue_set_preset_acoustic_device_reset'
            WWISE.WW_eventGlobalSync(wwiseEvent)
            WWISE.WW_setSoundSystem(soundSystemID)
        LOG_DEBUG((b'WWISE: triggered {0}').format(wwiseEvent))
        LOG_DEBUG(b'WWISE: sound system has been applied: %d' % soundSystemID)
        return
