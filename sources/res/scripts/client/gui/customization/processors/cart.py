...
ProcessResult = namedtuple(b'ProcessResult', (b'items', b'descriptors', b'itemsType'))

class ProcessorSelector(object):
    __slots__ = (b'__processors',)
    ...
    def process(self, items):
        ...
        if itemsType not in self.__processors:
            _logger.error(b"Can't find processor by type %d", itemsType)
            return None
        else:
    ...
    def __getItemsType(self, items):
        if not items:
            _logger.error(b'Empty purchaseItems list')
            return
...
class BasePurchaseDescription(object):
    __slots__ = (b'intCD', b'identificator', b'selected', b'item', b'component', b'quantity', b'purchaseIndices', b'progressionLevel', b'_uiDataPacker', b'dependents', b'dependentOn', b'isFromInventory')
...
class StubItemPurchaseDescription(BasePurchaseDescription):
    __slots__ = (b'isFromInventory', b'isEdited')
    _StubItem = namedtuple(b'_StubItem', (b'intCD',))
...
class SeparateItemPurchaseDescription(BasePurchaseDescription):
    __slots__ = (b'intCD', b'identificator', b'selected', b'itemData', b'compoundPrice', b'quantity', b'isFromInventory', b'purchaseIndices', b'group', b'locked', b'isEdited')
...
class ItemsProcessor(object):
    __slots__ = (b'_stubItemDescriptionClass', b'_itemDescriptionClass', b'_itemUiDataPacker', b'_stubUiDataPacker')
...
class EditableStyleItemsProcessor(SeparateItemsProcessor):
    ...
    def _process(self, items):
        ...
        for idx, pItem in enumerate(items):
            if pItem.item.itemTypeID not in (GUI_ITEM_TYPE.STYLE, GUI_ITEM_TYPE.PROJECTION_DECAL,
...
class _SeasonPurchaseInfo(object):
    __slots__ = (b'__buckets', b'__keyFunc')
