...
class AverageProgressGetter(ValueProgressGetter):
    ...
    def getAverageGoal(cls, progress, counter):
        if counter is not None and counter.getGoal() != 0:
            ...
            _logger.error(b'Counter is not found or has wrong config for progress: %s!', progress.getProgressID())
            return 1.0
...
class ILabelGetter(object):
    ...
    def getBottomLabel(cls, progress):
        ...
        return i18n.makeString(PERSONAL_MISSIONS.CONDITIONS_CURRENTPROGRESS_BOTTOMLABEL, currentProgress=b'%s / %s' % (text_styles.stats(progress.getCurrent()), progress.getGoal()))

    @classmethod
    def _getCompleteLabel(cls, progress):
        currentProgress = b'%s / %s' % (
