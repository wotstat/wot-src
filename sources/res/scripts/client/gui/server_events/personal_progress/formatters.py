...
class ProgressesFormatter(object):
    ...
    def __addUniqueHeaderProgress(self, isCompleted, data):
        ...
        if data:
            ...
            headerString = i18n.makeString(key, count=b'%s' % maxProgress)
            ...
            return {b'progressType': (DISPLAY_TYPE.LIMITED), 
               b'orderType': orderType, 
               b'header': headerString, 
               b'valueTitle': (i18n.makeString(PERSONAL_MISSIONS.CONDITIONS_CURRENTPROGRESS_BOTTOMLABEL, currentProgress=b'%s / %s' % (repetitionProgress, maxProgress))), 
               b'value': currentProgress, 
               b'goal': maxProgress, 
               b'state': (firstCondition.getState())}
        return {b'progressType': (DISPLAY_TYPE.NONE), 
           b'orderType': orderType}
    ...
    def __addDummyHeaderProgress(self, isMain):
        ...
        return {b'progressType': (self._dummyHeaderType), b'orderType': orderType, 
           b'header': (i18n.makeString(key))}
...
class DetailedProgressFormatter(ProgressesFormatter):
    ...
    def _makeBodyProgressData(cls, progress, isLastProgress=False):
        ...
        if not isLastProgress:
            bodyProgressData[b'initData'][b'multiplier'] = b''
        return bodyProgressData


class PostBattleConditionsFormatter(object):
    __slots__ = (b'__event', b'__storage', b'__wasMultiplied', b'__wasFailed')
    ...
    def __init__(self, event, progressData):
        ...
        self.__wasFailed = data.get(b'wasFailed')
        self.__storage = PostBattleProgressStorage(event.getGeneralQuestID(), event.getConditionsConfig(), data.get(b'current'), event.isOneBattleQuest())
        return
    ...
    def getConditionsData(self, isMain):
        ...
        titleStatusValues = None if any(c[b'statusText'] for c in conditions) else self.__getTitleStatusValues(isMain)
    ...
    def __packBlock(self, text, statusValues=None):
        return {b'text': text, b'statusText': (self.__packStatusText(statusValues) if statusValues else b'')}
...
class PMTooltipConditionsFormatters(object):
    ...
    def format(self, event, isMain=None, isForNewTooltip=False):
        ...
        if isForNewTooltip:
            return [self._CONDITION(R.images.gui.maps.icons.personal_missions_30.missions_icons.c_90x90.dyn((b'icon_battle_condition_{}_90x90').format(c.getIconID()))(), c.getDescription(isForNewTooltip), c.isInOrGroup()) for c in storage.sortProgresses(storage.getBodyProgresses(isMain).itervalues())]
...
class PMAwardScreenConditionsFormatter(ProgressesFormatter):
    ...
    def _getIterateData(self, headerProgress, bodyProgresses):
        if headerProgress.getDisplayType() in (DISPLAY_TYPE.BIATHLON, DISPLAY_TYPE.SERIES, DISPLAY_TYPE.COUNTER):
            ...
            headerData[b'valueTitle'] = first(bodyProgresses).getTitle()
            headerData[b'conditions'] = [{b'icon': (self._getIcon(progress.getIconID())), b'tooltip': (makeTooltip(progress.getTitle(), progress.getDescription()))} for progress in bodyProgresses]
            return headerData
        return {}
