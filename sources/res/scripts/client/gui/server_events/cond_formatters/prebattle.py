...
class MissionsPreBattleConditionsFormatter(MissionsBattleConditionsFormatter):
    ...
    def __init__(self):
        super(MissionsPreBattleConditionsFormatter, self).__init__({b'bonusTypes': (_BattleBonusTypeFormatter()), 
           b'isSquad': (_BattleSquadFormatter()), 
           b'clanMembership': (_BattleClanMembershipFormatter()), 
           b'camouflageKind': (_BattleCamouflageFormatter()), 
           b'geometryNames': (_BattleMapFormatter())})
        return
...
class _BattleBonusTypeFormatter(ConditionFormatter):
    ...
    def _format(self, condition, event):
        ...
        if not event.isGuiDisabled():
            ...
            iconsList = (b'').join([iconData.icon for iconData in data])
            ...
            bTypes = (b', ').join([iconData.iconLabel for iconData in data])
    ...
    def _format(self, condition, event):
        ...
        if not event.isGuiDisabled():
            if condition.getValue():
                ...
                label = text_styles.main(labelKey)
                iconData = formatters.packMissionFormationElement(b'squad')
...
class _BattleClanMembershipFormatter(ConditionFormatter):
    ...
    def _format(self, condition, event):
        result = []
        if not event.isGuiDisabled():
            label = b'#quests:missionDetails/conditions/clanMembership/%s' % condition.getValue()
            ...
            tooltip = makeTooltip(label, b'#quests:details/conditions/clanMembership/%s/%s' % (
             condition.getValue(), condition.getArenaBonusType()))
...
class _BattleCamouflageFormatter(ConditionFormatter):
    ...
    def _format(self, condition, event):
        ...
        if event is None or not event.isGuiDisabled():
            ...
            if camos:
                mapsTypesStr = i18n.makeString(b'#quests:details/conditions/mapsType')
                mapsTypeLabels = [iconData.iconLabel for iconData in camos]
                maps = (b', ').join(mapsTypeLabels)
                ...
                if len(camos) > 1:
                    label = text_styles.main(b'#quests:missionDetails/conditions/mapsType')
                    iconsData = (b'').join([iconData.icon for iconData in camos])
                else:
    ...
    def _format(self, condition, event):
        ...
        if not event.isGuiDisabled():
            ...
            if event.hasBonusType(ARENA_BONUS_TYPE.MAPS_TRAINING):
                maps = [arenaTypeID for arenaTypeID in maps if arenaTypeID in ArenaType.g_cache and ArenaType.g_cache[arenaTypeID].gameplayName == b'maps_training']
            for atID in maps:
            ...
            if len(mapsLabels) > 1:
                labelKey = b'#quests:missionDetails/conditions/maps'
                tooltipHeaderKey = b'#quests:details/conditions/maps'
                if condition.isNegative():
                    tooltipHeaderKey = b'#quests:missionDetails/conditions/maps/not'
                    labelKey = b'#quests:details/conditions/maps/not'
            else:
                labelKey = findFirst(None, mapsLabels)
                tooltipHeaderKey = b'#quests:details/conditions/map'
                if condition.isNegative():
                    tooltipHeaderKey = b'#quests:details/conditions/maps/not'
                    labelKey = b'#quests:missionDetails/conditions/maps/not'
            tooltipHeader = i18n.makeString(tooltipHeaderKey)
            tooltipBody = (b', ').join(mapsLabels)
...
class _InstalledItemsConditionFormatter(SimpleMissionsFormatter):
    ...
    def _getDescription(self, condition):
        key = b'#quests:details/conditions/installedModule/%s' % condition.getItemType()
        moduleNames = (b', ').join([item.userName for item in condition.getItemsList()])
        if condition.isNegative():
            key = b'%s/not' % key
...
class _CorrespondedCamouflageFormatter(SimpleMissionsFormatter):
    ...
    def _getDescription(self, condition):
        key = b'installedCamouflage' if condition.getValue() else b'noInstalledCamouflage'
        return packDescriptionField(i18n.makeString(b'#quests:details/conditions/%s' % key))
