...
VEHICLE_CLASSES_SIMPLIFIED = {b'lightTank': (QUESTS.DETAILS_CONDITIONS_CLASSES_LIGHTTANK), 
   b'mediumTank': (QUESTS.DETAILS_CONDITIONS_CLASSES_MEDIUMTANK), 
   b'heavyTank': (QUESTS.DETAILS_CONDITIONS_CLASSES_HEAVYTANK), 
   b'SPG': (QUESTS.DETAILS_CONDITIONS_CLASSES_SPG), 
   b'AT-SPG': (QUESTS.DETAILS_CONDITIONS_CLASSES_AT_SPG)}
...
class MissionsVehicleListFormatter(MissionFormatter):
    ...
    def _getDescription(cls, condition):
        ...
        if condition.getDirectHitsReceived():
            ...
            if directHitsReceived and directHitsReceived < 0:
                labelKey = b'%s/negative' % labelKey
    ...
    def _formatClassesEnumeration(cls, condition):
        ...
        allowedClassesEnumeration = b' %s ' % i18n.makeString(QUESTS.DETAILS_CONDITIONS_ENUMERATE_OR)
