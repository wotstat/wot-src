...
class MissionsPostBattleConditionsFormatter(MissionsBattleConditionsFormatter):
    ...
    def __init__(self):
        super(MissionsPostBattleConditionsFormatter, self).__init__({b'vehicleKills': (VehiclesKillFormatter()), 
           ...
           b'results': (_BattleResultsFormatter()), 
           ...
           b'vehicleBlockedByArmor': (_VehicleBlockedByArmorFormatter())})
        return
...
class VehiclesDamageFormatter(MissionsVehicleListFormatter):
    ...
    def _getIconKey(cls, condition=None):
        ...
        if condition.getAttackReason() == ATTACK_REASON.RAM:
            return CONDITION_ICON.RAM
        if (b'classes' in condition.data or b'classesDiversity' in condition.data) and not cls.UTILITY_DAMAGE_TAGS.intersection(condition.data) or b'eventCount' in condition.data and cls.UTILITY_DAMAGE_TAGS.intersection(condition.data):
...
class _VehicleBlockedByArmorFormatter(MissionsVehicleListFormatter):
    ...
    def _getDescription(cls, condition=None):
        ...
        if condition.relationValue:
            damage = (b'{}').format(int(condition.relationValue))
