...
def packMissionVehicleProgress(groupByKey, current, total):
    ...
    return {b'progressLabel': (formatters.minimizedTitleCumulativeFormat(current, total)), 
       b'nationIcon': (b'../maps/icons/filters/nations/%s.png' % vehicle.nationName), 
       b'vehicleIcon': (vehicle.iconSmall), 
       b'vehicleName': (vehicle.shortUserName), 
       b'vTypeIcon': (b'../maps/icons/filters/tanks/%s.png' % vehicle.type), 
       b'level': (vehicle.level), 
       b'progress': {b'maxValue': total, 
                     b'value': current}}


def packMissionNationProgress(groupByKey, current, total):
    return {b'progressLabel': (formatters.minimizedTitleCumulativeFormat(current, total)), 
       b'nationIcon': (b'../maps/icons/filters/nations/%s.png' % groupByKey), 
       b'nationName': (i18n.makeString(b'#menu:nations/%s' % groupByKey)), 
       b'progress': {b'maxValue': total, 
                     b'value': current}}


def packMissionLevelProgress(groupByKey, current, total):
    levelValue = int(groupByKey.replace(b'level ', b''))
    return {b'progressLabel': (formatters.minimizedTitleCumulativeFormat(current, total)), 
       b'level': levelValue, 
       b'levelLabel': (i18n.makeString(QUESTS.MISSIONDETAILS_CONDITIONS_LEVEL)), 
       b'progress': {b'maxValue': total, 
                     b'value': current}}
...
def packMissionClassProgress(groupByKey, current, total):
    return {b'progressLabel': (formatters.minimizedTitleCumulativeFormat(current, total)), 
       b'vTypeIcon': (b'../maps/icons/filters/tanks/%s.png' % groupByKey), 
       b'vTypeLabel': (i18n.makeString(b'#quests:classes/small/%s' % groupByKey)), 
       b'progress': {b'maxValue': total, 
                     b'value': current}}


GROUP_BY_FORMATTERS_DATA = {b'vehicle': (
...
class MissionsBonusConditionsFormatter(MissionsBattleConditionsFormatter):
    ...
    def __init__(self):
        super(MissionsBonusConditionsFormatter, self).__init__({b'vehicleKillsCumulative': (_VehicleKillsCumulativeFormatter()), 
           b'vehicleDamageCumulative': (_VehicleDamageCumulativeFormatter()), 
           b'vehicleStunCumulative': (_VehicleStunCumulativeFormatter()), 
           b'cumulative': (_CumulativeResultFormatter()), 
           b'cumulativeExt': (_CumulativeResultFormatter()), 
           b'unit': (_CumulativeResultFormatter()), 
           b'cumulativeSum': (_CumulativeSumFormatter())})
        return
...
class _CumulativableFormatter(MissionFormatter, CumulativableFormatter):
    ...
    def _groupedByFormat(self, condition, event, progressData):
        ...
        if not event.isGuiDisabled():
            progressType = MISSIONS_ALIASES.CUMULATIVE
            if progressData:
                maxProgressItem = progressData.progressList[0][b'progress']
                current, total = maxProgressItem[b'value'], maxProgressItem[b'maxValue']
            else:
                current = ZERO_COUNT
                total = condition.getTotalValue()
            title = self._getTitle(current, total)
