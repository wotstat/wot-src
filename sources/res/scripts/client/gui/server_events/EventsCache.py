...
PM_TOKEN_PREFIXES = frozenset([
 ...
 b'free_award_list',
 b'regular_'])
_ProgressiveReward = namedtuple(b'_ProgressiveReward', (b'currentStep', b'probability', b'maxSteps'))

class _DailyQuestsData(object):
    __slots__ = (b'_lastReroll', b'_lastRerollPrem')
...
class DefaultQuestMaker(object):
    ...
    def __call__(self, qID, qData, progressRequester):
        ...
        return createQuest(self.__builders, qData.get(b'type', 0), qID, qData, progressRequester.getQuestProgress(qID), progressRequester.getTokenExpiryTime(qData.get(b'requiredToken')))
...
class EventsCache(IEventsCache):
    ...
    def update(self, diff=None, callback=None):
        ...
        if not self.getPersonalMissions().isQuestsProgressSynced():
            ...
            if diff is not None:
                isQPUpdated = b'quests' in diff or b'personalMissionQuests' in diff
                isQPUpdated |= b'pm_progress' in diff or b'pmQuestsStates' in diff.get(b'cache', {})
                if not isQPUpdated and b'tokens' in diff:
                    for tokenID in diff[b'tokens'].iterkeys():
                        if any(tokenID.startswith(t) for t in PM_TOKEN_PREFIXES):
                            isQPUpdated = True
                            break

                isEventsDataUpdated = (b'eventsData', b'_r') in diff or diff.get(b'eventsData', {})
                isNeedToInvalidate = isQPUpdated or isEventsDataUpdated
                hasVehicleUnlocks = False
                diffStats = diff.get(b'stats', {})
                for intCD in diffStats.get(b'unlocks', set()) | diffStats.get((b'unlocks', b'_r'), set()):
                    if getTypeOfCompactDescr(intCD) == GUI_ITEM_TYPE.VEHICLE:
                        hasVehicleUnlocks = True
                        break

                isNeedToClearItemsCaches = hasVehicleUnlocks or b'inventory' in diff and GUI_ITEM_TYPE.VEHICLE in diff[b'inventory']
            if isNeedToInvalidate:
                self.__invalidateData(_cbWrapper)
            elif isNeedToClearItemsCaches:
                self.__clearQuestsItemsCache()
            if isQPUpdated:
                _cbWrapper(True)
            else:
    ...
    def getActiveQuests(self, filterFunc=None):
        filterFunc = filterFunc or (lambda a: True)
        isPremiumQuestsEnable = self.lobbyContext.getServerSettings().getPremQuestsConfig().get(b'enabled', False)
    ...
    def getItemAction(self, item, isBuying=True, forCredits=False):
        ...
        for (_, value), actionID in values:
            if item.isPremium and value in (b'creditsPrice', b'creditsPriceMultiplier') and not forCredits:
                continue
    ...
    def getBoosterAction(self, booster, isBuying=True, forCredits=False):
        ...
        for (key, value), actionID in values:
            if forCredits and key == b'creditsPriceMultiplier':
                result.append((value, actionID))
            elif not forCredits and key == b'goldPriceMultiplier':
                result.append((value, actionID))
    ...
    def getEconomicsAction(self, name):
        ...
        resultMult = self.__actionsCache[ACTION_SECTION_TYPE.ECONOMICS][ACTION_MODIFIER_TYPE.DISCOUNT].get(b'%sMultiplier' % name, [])
    ...
    def getHeroTankAdventCalendarRedirectAction(self):
        ...
        def containsHeroToAdvent(a):
            return any(step.get(b'name') == b'HeroTankAdventCalendarRedirect' for step in a.getData().get(b'steps', []))
    ...
    def isBalancedSquadEnabled(self):
        return bool(self.__getUnitRestrictions().get(b'enabled', False))

    def getBalancedSquadBounds(self, tag=b'default'):
        restrictions = self.__getUnitRestrictions().get(b'restriction', [])
        restriction = findFirst((lambda r: tag in r[b'tags']), restrictions, {})
        return (restriction.get(b'lowerBound', 0), restriction.get(b'upperBound', 0))

    def isSquadXpFactorsEnabled(self):
        return bool(self.__getUnitXpFactors().get(b'enabled', False))

    def getSquadBonusLevelDistance(self):
        return set(self.__getUnitXpFactors().get(b'levelDistanceWithBonuses', ()))

    def getSquadPenaltyLevelDistance(self):
        return set(self.__getUnitXpFactors().get(b'levelDistanceWithPenalties', ()))

    def getSquadZeroBonuses(self):
        return set(self.__getUnitXpFactors().get(b'zeroBonusesFor', ()))

    def getSquadXPFactor(self):
        return self.__getUnitXpFactors().get(b'factor', 0)
    ...
    def getQuestsByTokenBonus(self, token):
        ...
        for q in self._getQuests(includePersonalMissions=True).itervalues():
            for t in q.getBonuses(b'tokens'):
    ...
    def getLobbyHeaderTabCounter(self):
        ...
        def containsLobbyHeaderTabCounter(a):
            return any(step.get(b'name') == b'LobbyHeaderTabCounterModification' for step in a.getData().get(b'steps', []))
    ...
    def _getActions(self, filterFunc=None):
        ...
        for aData in actions:
            if b'id' in aData:
                a = self._makeAction(aData[b'id'], aData)
                actionID = a.getID()
    ...
    def _makeAction(self, aID, aData):
        storage = self.__cache[b'actions']
        if aID in storage:
            return storage[aID]
        a = storage[aID] = createAction(aData.get(b'type', 0), aID, aData)
        return a
    ...
    def _makeQuestsRelations(cls, quests):
        ...
        for qID, q in quests.iteritems():
            if q.getType() not in (EVENT_TYPE.GROUP, EVENT_TYPE.PERSONAL_MISSION):
                for t in q.getRawBonuses().get(b'tokens', {}):
    ...
    def __invalidateData(self, callback=(lambda *args: None)):
        ...
        for q in self.getCurrentEvents().itervalues():
            dossierBonuses = q.getBonuses(b'dossier')
            if dossierBonuses:
    ...
    def __syncActionsWithQuests(self):
        self.__actions2quests.clear()
        self.__quests2actions.clear()
        quests = self.__cache[b'quests']
        actions = [item for item in self.__cache[b'actions']]
    ...
    def __getUnitRestrictions(self):
        return self.__getUnitData().get(b'restrictions', {})

    def __getUnitXpFactors(self):
        return self.__getUnitData().get(b'xpFactors', {})
    ...
    def __loadInvalidateCallback(self, duration):
        LOG_DEBUG(b'load quest window invalidation callback (secs)', duration)
        self.__clearInvalidateCallback()
    ...
    def __getPersonalMissionsHiddenQuests(self):
        if not self.__personalMissionsHidden:
            xmlPath = PERSONAL_MISSIONS_XML_PATH + b'/tiles.xml'
