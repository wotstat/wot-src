...
class DefaultQuestMaker(object):
    ...
    def __call__(self, qID, qData, progressRequester):
        ...
        return createQuest(self.__builders, qData.get(b'type', 0), qID, qData, progress=progressRequester.getQuestProgress(qID), expiryTime=progressRequester.getTokenExpiryTime(qData.get(b'requiredToken')))
...
class EventsCache(IEventsCache):
    ...
    def update(self, diff=None, callback=None):
        ...
        if diff is not None:
            ...
            isQuestDataUpdated = bool(diff.get(b'quests', {}))
    ...
    def _makeQuestsRelations(cls, quests):
        ...
        for qID, q in quests.iteritems():
            if q.getType() not in (EVENT_TYPE.GROUP, EVENT_TYPE.PERSONAL_MISSION):
                for tokenBonus in q.getBonuses(b'tokens'):
    ...
    def __onWeeklyQuestsInfoChange(self):
        self.__weeklyQuests = _WeeklyQuestsData(BigWorld.player().weeklyQuests.get(b'rerolls', {}))
        return
