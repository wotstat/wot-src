...
class CLASS_TYPE(CONST_CONTAINER):
    ...
    CONDITION_GROUP = b'ConditionsGroup'
...
class _Condition(_Typeable):
    ...
    def getCustomTitle(self):
        titleData = self._data.get(b'title')
        if titleData:
            if b'key' in titleData:
                return i18n.makeString(titleData[b'key'], value=_getCustomTitleValueFromConditionData(self._data))
            return getLocalizedData(self._data, b'title')
        else:
            return

    def getCustomDescription(self):
        descrData = self._data.get(b'description')
        if descrData:
            if b'key' in descrData:
                return i18n.makeString(descrData[b'key'], value=_getCustomDescriptionValueFromConditionData(self._data))
...
class _VehsListCondition(_Condition, _VehsListParser):
    ...
    def getDirectHitsReceived(self):
        return _getNodeValue(self._data, b'directHitsReceived', default=0)

    def getDamageDealt(self):
        return _getNodeValue(self._data, b'damageDealt', default=0)

    def getWhileMovingAtSpeed(self):
        return _getNodeValue(self._data, b'whileMovingAtSpeed', default=0)

    def getEnemyIsNotSpotted(self):
        return _getNodeValue(self._data, b'enemyIsNotSpotted', default=False)

    def getBeyondVisionRadius(self):
        return _getNodeValue(self._data, b'beyondVisionRadius', default=False)

    def getSpotEnemy(self):
        return _getNodeValue(self._data, b'spotEnemy', default=False)

    def getwhileEnemyInvisible(self):
        return _getNodeValue(self._data, b'whileEnemyInvisible', default=False)

    def getAttackReason(self):
        return ATTACK_REASONS[self.getAttackReasonIdx()]

    def getDistance(self):
        return _getNodeValue(self._data, b'distance', default=None)

    def getTimeLimit(self):
        return _getNodeValue(self._data, b'limittedTime', default=None)

    def getWhileInvisible(self):
        return _getNodeValue(self._data, b'whileInvisible', default=False)

    def getWhileFullHealth(self):
        return _getNodeValue(self._data, b'whileFullHealth', default=False)

    def getClassesDiversity(self):
        return _getNodeValue(self._data, b'classesDiversity', default=None)
...
class Token(_Requirement):
    ...
    def getConsumeCount(self):
        if self.isConsumable():
            consumeData, _ = self._data[b'consume']
            return dict(consumeData).get(b'value', 0)
        return 0
...
class BattleResults(_Condition, _Negatable, _Updatable):
    ...
    def __init__(self, path, data, localeKey=b'single'):
        ...
        self._compareWithMaxHealth = b'compareWithMaxHealth' in self._data
        self._relation = _findRelation(self._data.keys())
        self._relationValue = _getNodeValue(self._data, self._relation)
...
class VehicleKills(_VehsListCondition):
    SPECIAL_LABELS = {b'damageDealt': (QUESTS.DETAILS_CONDITIONS_DAMAGEDEALT), 
       b'spotEnemy': (QUESTS.DETAILS_CONDITIONS_SPOTENEMY), 
       b'whileMovingAtSpeed': (QUESTS.DETAILS_CONDITIONS_VEHICLEKILLS_WHILEMOVINGATSPEED), 
       b'enemyIsNotSpotted': (QUESTS.DETAILS_CONDITIONS_VEHICLEKILLS_ENEMYISNOTSPOTTED), 
       b'beyondVisionRadius': (QUESTS.DETAILS_CONDITIONS_VEHICLEKILLS_BEYONDVISIONRADIUS), 
       b'whileEnemyInvisible': (QUESTS.DETAILS_CONDITIONS_VEHICLEKILLS_WHILEENEMYINVISIBLE)}
...
class VehicleDamage(_CountOrTotalEventsCondition):
    SPECIAL_LABELS = {b'directHitsReceived': (QUESTS.DETAILS_CONDITIONS_DIRECTHITSRECEIVED), 
       b'whileMovingAtSpeed': (QUESTS.DETAILS_CONDITIONS_WHILEMOVINGATSPEED), 
       b'enemyIsNotSpotted': (QUESTS.DETAILS_CONDITIONS_ENEMYISNOTSPOTTED), 
       b'beyondVisionRadius': (QUESTS.DETAILS_CONDITIONS_BEYONDVISIONRADIUS), 
       b'whileEnemyInvisible': (QUESTS.DETAILS_CONDITIONS_WHILEENEMYINVISIBLE)}
...
class VehicleBlockedByArmor(_CountOrTotalEventsCondition):
    ...
    def __init__(self, path, data):
        super(VehicleBlockedByArmor, self).__init__(b'vehicleBlockedByArmor', dict(data), path)
        return

    def __repr__(self):
        return b'VehicleBlockedByArmor<%s=%d>' % (self._relation, self._relationValue)
