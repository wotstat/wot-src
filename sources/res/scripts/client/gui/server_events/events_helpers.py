...
def isDailySubsQuest(eventID):
    return (eventID and eventID.startswith(DAILY_QUEST_ID_PREFIX) and DAILY_SUBS_LEVEL_SUFFIX in eventID) or False


def isDailyPremiumQuest(eventID):
    return (eventID and eventID.startswith(DAILY_QUEST_ID_PREFIX) and DAILY_PREMIUM_LEVEL_SUFFIX in eventID) or False
...
def isPremiumQuestsEnable(lobbyContext=None, eventsCache=None):
    return lobbyContext.getServerSettings().getPremQuestsConfig().get(b'enabled', False) and len(eventsCache.getDailyPremiumQuests()) > 0 and lobbyContext.getServerSettings().getDailyQuestConfig().get(b'enabled', False)
...
def isDailyRegularQuestsEnabled(lobbyContext=None, eventsCache=None):
    return lobbyContext.getServerSettings().getDailyQuestConfig().get(b'regularQuestsEnabled', False) and lobbyContext.getServerSettings().getDailyQuestConfig().get(b'enabled', False)
...
def getRerollTimeoutPrem(lobbyContext=None):
    return lobbyContext.getServerSettings().getDailyQuestConfig().get(b'rerollTimeoutPrem', 0)


@dependency.replace_none_kwargs(lobbyContext=ILobbyContext)
def isRerollEnabled(lobbyContext=None):
    return lobbyContext.getServerSettings().getDailyQuestConfig().get(b'rerollEnabled', False) and lobbyContext.getServerSettings().getDailyQuestConfig().get(b'enabled', False)


@dependency.replace_none_kwargs(lobbyContext=ILobbyContext)
def isEpicQuestEnabled(lobbyContext=None):
    return lobbyContext.getServerSettings().getDailyQuestConfig().get(b'epicRewardEnabled', False) and lobbyContext.getServerSettings().getDailyQuestConfig().get(b'enabled', False)
...
def isIngameBrowserEventEnable(lobbyContext=None):
    return lobbyContext.getServerSettings().getIngameBrowserEventConfig().get(b'isEnabled', False)
