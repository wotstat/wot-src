...
class _BaseRecruitInfo(object):
    __slots__ = (b'_recruitID', b'_expiryTime', b'_nations', b'_freeSkills', b'_learntSkills', b'_freeXP', b'_roleLevel', b'_lastSkillLevel', b'_roles', b'_firstName', b'_lastName', b'_group', b'_icon', b'_sourceID', b'_isPremium', b'_isFemale', b'_hasNewSkill', b'_tankmanSkill')
    ...
    def getFullUserName(self):
        ...
        if not firstName and not lastName:
            return b''
        return (b'{} {}').format(firstName, lastName).strip()
...
class _TokenRecruitInfo(_BaseRecruitInfo):
    __slots__ = (b'__freeSkills', b'_isUnique')
        ...
        if roles:
            for role in roles:
                if skills_constants.SKILL_NAMES[role] not in allowedRoles:
                    logging.error(b'Requested role "%s" is not in the list of allowed roles "%s"', skills_constants.SKILL_NAMES[role], (b', ').join(map(str, allowedRoles)))
