...
def vehicleRequirementsCheck(quest, invVehicles, vehGetter):
    ...
    for vehCD in invVehicles:
        ...
        if minLevel <= vehType.level <= maxLevel and classifier.matchVehicle(vehType) and not checkForTags(vehType.tags, BATTLE_MODE_VEHICLE_TAGS):
...
class _PMBranch(object):
    __slots__ = (b'branch', b'questsProgress', b'vehRequirementsCache', b'hasQuestsForSelect', b'hasQuestsForReward', b'freeTokensCount', b'pawnedTokensCount', b'campaigns', b'operations', b'quests')
...
class PersonalMissionsCache(object):
    ...
    def __updateVehLevelRestrictions(self, quest):
        ...
        self.__vehLevelsRestrictions[operationID] = (
         min(currMin, quest.getVehMinLevel()), max(currMax, quest.getVehMaxLevel()))
        return
