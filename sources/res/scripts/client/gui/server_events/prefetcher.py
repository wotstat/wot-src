...
_DEFAULT_TOKENS_STYLES = [title.split(b'/')[-1] for title in QUESTS.TOKEN_DEFAULT_ENUM]
_DEFAULT_DECORATIONS = [title.split(b'_')[-1].replace(b'.png', b'') for title in RES_ICONS.MAPS_ICONS_MISSIONS_DECORATIONS_DECORATION_ENUM]
...
def _getTokensFromBonuses(quest):
    return (parseComplexToken(t) for t in itertools.chain.from_iterable(b.getValue().keys() for b in quest.getBonuses(b'tokens')))
    ...
    def _run(self, url, headers, ticket, filecache):
        ...
        try:
            try:
                ...
                LOG_WARNING(b'Error during processing', name)
...
_TokenInfoData = namedtuple(b'_TokenInfoData', [b'title', b'description'])
...
class TokenInfoSubRequester(SubRequester):
    ...
    def _handler(self, ticket, content):
        ...
        tokens = section[b'root/tokens']
        for item in tokens.values():
            tokenID = item[b'id'].asString
            string = item[b'title'].asString
            description = item.readString(b'description') if item.has_key(b'description') else None
            ticket = (
             tokenID,)
...
class TokenSaleSubRequester(SubRequester):
    ...
    def _handler(self, ticket, content):
        ...
        for item in data.get(b'data', []):
            attributes = item.get(b'attributes', {})
            properties = attributes.get(b'custom_properties', {})
    ...
    @staticmethod
    def _urlGetter(_):
        return (lambda *tokenWebIDs: GUI_SETTINGS.lookup(b'tokenShopAvailabilityURL') % (b',').join(tokenWebIDs))

    @staticmethod
    def _headers():
        apiKey = GUI_SETTINGS.lookup(b'tokenShopAPIKey')
        return {b'Authorization': ((b'Bearer {}').format(apiKey))}
...
class Prefetcher(object):
    ...
    def getTokenDetailedInfo(self, styleID):
        return self._requesters[b'tokenInfo'].pickup(styleID).description

    def isTokenOnSale(self, tokenWebID):
        return self._requesters[b'tokenSale'].pickup(tokenWebID)

    def init(self):
        self._asyncScope = AsyncScope()
        self._semaphore = AsyncSemaphore(0, self._asyncScope)
        self._requesters = {b'tokenImage': (TokenImagesSubRequester(self._eventsCache, self._semaphore)), 
           b'tokenInfo': (TokenInfoSubRequester(self._eventsCache, self._semaphore)), 
           b'tokenSale': (TokenSaleSubRequester(self._eventsCache, self._semaphore)), 
           b'decoration': (DecorationRequester(self._eventsCache, self._semaphore))}
        return
