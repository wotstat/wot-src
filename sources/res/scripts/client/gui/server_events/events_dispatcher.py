...
def showPersonalMission(missionID=None):
    ...
    if branchID == PM_BRANCH.PERSONAL_MISSION_3:
        ...
        g_eventBus.handleEvent(events.LoadViewEvent(SFViewLoadParams(PERSONAL_MISSIONS_ALIASES.PERSONAL_MISSIONS_PAGE_ALIAS), ctx={b'eventID': missionID}), EVENT_BUS_SCOPE.LOBBY)
    return
...
def showPersonalMissionsChain(operationID, chainID):
    ...
    if operationID in BRANCH_TO_OPERATION_IDS[PM_BRANCH.PERSONAL_MISSION_3]:
        ...
        g_eventBus.handleEvent(events.LoadViewEvent(SFViewLoadParams(PERSONAL_MISSIONS_ALIASES.PERSONAL_MISSIONS_PAGE_ALIAS), ctx={b'operationID': operationID, b'chainID': chainID}), EVENT_BUS_SCOPE.LOBBY)
    return
...
def showPersonalMissionOperationsPage(branchID, operationID):
    ...
    if branchID == PM_BRANCH.PERSONAL_MISSION_3:
        ...
        g_eventBus.handleEvent(events.LoadViewEvent(SFViewLoadParams(PERSONAL_MISSIONS_ALIASES.PERSONAL_MISSIONS_PAGE_ALIAS), ctx={b'branch': branchID, 
           b'operationID': operationID}), EVENT_BUS_SCOPE.LOBBY)
    return


def showPersonalMissionsOperationsMap(branchID=None):
    g_eventBus.handleEvent(events.LoadViewEvent(SFViewLoadParams(PERSONAL_MISSIONS_ALIASES.PERSONAL_MISSIONS_OPERATIONS), ctx={b'branch': branchID}), EVENT_BUS_SCOPE.LOBBY)
    return
...
def showMission(eventID, eventType=None):
    ...
    if eventType == constants.EVENT_TYPE.C11N_PROGRESSION:
        ...
        if eventID == BattlePassConsts.FAKE_QUEST_ID:
            ...
            return
        if eventID == PARAGONS_POST_BATTLE_FAKE_QUEST_ID:
        ...
        if quest is None or quest.isHidden():
            ...
            if events_helpers.isTankAcademyQuest(eventID):
                ...
                showTankAcademy()
            elif events_helpers.isArmoryYardQuest(eventID):
        ...
        if bonuses:
            ...
            if lootboxes:
                for lootboxId, lootboxInfo in lootboxes.iteritems():
                    showLootboxesAward(lootboxId=lootboxId, lootboxCount=lootboxInfo[b'count'], isFree=lootboxInfo[b'isFree'])
