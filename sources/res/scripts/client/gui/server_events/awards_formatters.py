...
AWARD_IMAGES = {(AWARDS_SIZES.SMALL): {(Currency.CREDITS): (RES_ICONS.MAPS_ICONS_QUESTS_BONUSES_SMALL_CREDITS), 
   ...
   (AWARDS_SIZES.S232X174): {(Currency.CREDITS): (RES_ICONS.getBonusIcon(AWARDS_SIZES.S232X174, Currency.CREDITS)), 
                             ...
                             b'creditsFactor': (RES_ICONS.getBonusIcon(AWARDS_SIZES.S232X174, Currency.CREDITS)), 
                             b'freeXP': (RES_ICONS.getBonusIcon(AWARDS_SIZES.S232X174, Currency.FREE_XP)), 
                             b'freeXPFactor': (RES_ICONS.getBonusIcon(AWARDS_SIZES.S232X174, Currency.FREE_XP)), 
                             b'tankmenXP': (RES_ICONS.getBonusIcon(AWARDS_SIZES.S232X174, b'tankmenXP')), 
                             b'tankmenXPFactor': (RES_ICONS.getBonusIcon(AWARDS_SIZES.S232X174, b'tankmenXP')), 
                             b'xp': (RES_ICONS.getBonusIcon(AWARDS_SIZES.S232X174, b'exp')), 
                             b'xpFactor': (RES_ICONS.getBonusIcon(AWARDS_SIZES.S232X174, b'exp')), 
                             b'dailyXPFactor': (RES_ICONS.getBonusIcon(AWARDS_SIZES.S232X174, Currency.FREE_XP))}}
...
def getRankedFormatterMap(context=None):
    ...
    mapping.update({b'tokens': tokenBonusFormatter, 
       b'selectableBonus': (RankedSelectableAwardFormatter(context.get(b'selectionsLeft') if context else None)), 
       b'battleToken': tokenBonusFormatter, 
       b'vehicles': (RankedVehiclesBonusFormatter()), 
...
def getPersonalMissionsOperationTooltipsPacker():
    ...
    mapping.update({b'vehicles': (PersonalMissionsVehiclesFormatter())})
    return AwardsPacker(mapping)
...
_PreformattedBonus = namedtuple(b'_PreformattedBonus', b'bonusName label userName images tooltip labelFormatter areTokensPawned specialArgs specialAlias isSpecial isCompensation align highlightType overlayType highlightIcon overlayIcon compensationReason postProcessTags isWulfTooltip itemTypeName')
        ...
        for b in bonuses:
            if b.isShowInGUI():
                ...
                if formatter:
                    ...
                    _logger.warn(b'No formatter found for %s', bonusName)
...
class TokenBonusFormatter(SimpleBonusFormatter):
    ...
    def _getFormattedBonus(self, tokenID, token, bonus):
        ...
        if complexToken.isDisplayable:
            ...
            formatted = self._formatPMToken(b'personal_missions_points', token, bonus)
    ...
    def _formatLootBoxToken(self, tokenID, token, bonus):
        ...
        tooltipBodyRes = getTextResource([b'boxTooltip', b'description', b'text'] + [lootBox.getCategory()], lootBox.getType())
        return PreformattedBonus(label=self._formatBonusLabel(token.count), userName=self._getLootboxUserName(lootBox), bonusName=bonus.TOKENS, labelFormatter=self._getLabelFormatter(bonus), images=images, tooltip=makeTooltip(header=self._getLootboxUserName(lootBox), body=backport.text(tooltipBodyRes()) if tooltipBodyRes.exists() else b''), align=self._getLabelAlign(bonus), isCompensation=self._isCompensation(bonus), itemTypeName=bonus.getName())
    ...
    def _formatPMToken(self, name, token, bonus):
        if token.count <= 0:
            ...
            def formatPMCountLabel(count, defaultStr=b''):
                if count > 1:
...
class RankedSelectableAwardFormatter(TokenBonusFormatter):
    ...
    def __getImages(self):
        ...
        return {size: backport.image(imagesRoot.dyn(size).dyn(b'deluxe_gift')()) for size in AWARDS_SIZES.ALL()}
...
class CustomizationsBonusFormatter(SimpleBonusFormatter):
    ...
    def _createCustomizationBonus(self, bonus, item, data):
        ...
        return PreformattedBonus(bonusName=bonus.getName(), images=self._getImages(c11nItem), userName=self._getUserName(c11nItem), label=self._formatBonusLabel(item.get(b'value')), labelFormatter=self._getLabelFormatter(bonus), isSpecial=True, specialAlias=TOOLTIPS_CONSTANTS.TECH_CUSTOMIZATION_ITEM_AWARD, specialArgs=CustomizationTooltipContext(itemCD=data.get(b'intCD')), isCompensation=self._isCompensation(bonus), align=LABEL_ALIGN.RIGHT, itemTypeName=self._getItemTypeName(c11nItem), postProcessTags=self._makePostprocessTags(c11nItem))
        ...
        for size in AWARDS_SIZES.ALL():
            if itemTypeName == SLOT_HIGHLIGHT_TYPES.BATTLE_BOOSTER_NAME:
                ...
                if name == SLOT_HIGHLIGHT_TYPES.EQUIPMENT_MODERNIZED:
                    name = (b'{}_{}').format(name, item.level)
