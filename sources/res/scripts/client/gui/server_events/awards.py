...
AwardsRibbonInfo = namedtuple(b'AwardsRibbonInfo', [
 23, 
 24, 
 25, 
 26, 
...
def packRibbonInfo(awards=None, awardForCompleteText=b'', awardReceivedText=b'', awardBonusStrText=b''):
...
class ExplosionBackAward(AwardAbstract):
    ...
    def getBackgroundAnimationData(self):
        if self.__useAnimation:
            return {b'image': (self.getAwardImage()), 
               b'animationPath': (AWARDWINDOW_CONSTANTS.EXPLOSION_BACK_ANIMATION_PATH), 
               b'animationLinkage': (AWARDWINDOW_CONSTANTS.EXPLOSION_BACK_ANIMATION_LINKAGE)}
        else:
            return
...
class AchievementsAward(AwardAbstract):
    ...
    def __init__(self, achieves):
        if not hasattr(achieves, b'__iter__'):
            raise UserWarning(b'Received argument is not iterable', achieves)
    ...
    def getDescription(self):
        ...
        return text_styles.main((b'\n\n').join(descr))

    def getExtraFields(self):
        result = []
        for a in self.__achieves:
            result.append({b'type': (a.getRecordName()[1]), 
               b'block': (a.getBlock()), 
               b'rank': (a.getValue()), 
               b'icon': {b'big': (a.getHugeIcon()), 
                         b'small': (a.getSmallIcon())}})

        return {b'achievements': result}


class FormattedAward(AwardAbstract):

    class _BonusFormatter(object):
        _BonusFmt = namedtuple(b'_BonusFmt', b'icon value tooltip bonusVO')
    ...
    class _SimpleFormatter(_BonusFormatter):
        ...
        def __init__(self, icon, tooltip=b''):
            self._icon = icon
    ...
    class _SimpleNoValueFormatter(_SimpleFormatter):
        ...
        def __call__(self, bonus):
            return [
             self._BonusFmt(self._icon, b'', self._tooltip, None)]
        ...
        def __call__(self, bonus):
            ...
            for booster, count in bonus.getBoosters().iteritems():
                if booster is not None and count:
                    ...
                    result.append(self._BonusFmt(b'', backport.getIntegralFormat(count), tooltip, self.__makeBoosterVO(booster)))
    ...
    def __init__(self):
        self._formatters = {(Currency.GOLD): (self._SimpleFormatter(RES_ICONS.MAPS_ICONS_QUESTS_BONUSES_SMALL_GOLD, tooltip=TOOLTIPS.AWARDITEM_GOLD)), 
           ...
           b'freeXP': (self._SimpleFormatter(RES_ICONS.MAPS_ICONS_QUESTS_BONUSES_SMALL_FREEEXP, tooltip=TOOLTIPS.AWARDITEM_FREEXP)), 
           b'premium': (self._SimpleNoValueFormatter(RES_ICONS.MAPS_ICONS_QUESTS_BONUSES_SMALL_PREMIUM_1, tooltip=TOOLTIPS.AWARDITEM_PREMIUM)), 
           b'items': (self._ItemsFormatter()), 
           b'goodies': (self._BoostersFormatter())}
        return
    ...
    def _getMainAwards(self, bonuses):
        ...
        for b in bonuses:
            ...
            if callable(formatter):
                for bonus in formatter(b):
                    awards.append({b'value': (bonus.value), 
                       b'itemSource': (bonus.icon), 
                       b'tooltip': (bonus.tooltip), 
                       b'boosterVO': (bonus.bonusVO)})
        ...
        if strAwardsList:
            strAwards = (b', ').join(strAwardsList)
        return (
         awards, strAwards)
...
class MotiveQuestAward(FormattedAward):
    ...
    def getWindowTitle(self):
        return _ms(b'#tutorial:tutorialQuest/awardWindow/title')

    def getBackgroundImage(self):
        return RES_ICONS.MAPS_ICONS_HANGARTUTORIAL_GOALSQUEST

    def getHeader(self):
        return _ms(b'#tutorial:tutorialQuest/awardWindow/header', qName=_ms(self.__quest.getUserName()))

    def getDescription(self):
        return self.__quest.getAwardMsg()

    def getBodyButtonText(self):
        return _ms(b'#tutorial:tutorialQuest/awardWindow/nextQuest')
...
class MissionAward(MissionAwardAbstract):
    ...
    def __init__(self, quest, ctx, proxyEvent):
        ...
        self._quest = quest
        self._eventsCache = ctx[b'eventsCache']
        self._proxyEvent = proxyEvent
        return

    def getWindowTitle(self):
        return _ms(b'#menu:awardWindow/title/taskComplete')

    def getBackgroundImage(self):
        return b'../maps/icons/quests/awards/mission_award_bg.png'

    def getRibbonImage(self):
        return b'../maps/icons/quests/awardRibbon.png'

    def getDescription(self):
        return text_styles.promoTitle(b'#menu:awardWindow/mission/complete')

    def getCurrentQuestHeader(self):
        return text_styles.highTitle(self._quest.getUserName())

    def getMainStatusText(self):
        return text_styles.success(b'#menu:awardWindow/mission/conditionComplete')
    ...
    def getAvailableText(self):
        ...
        return text_styles.standard(_ms(b'#menu:awardWindow/mission/available', count=count))

    def getNextButtonText(self):
        return _ms(b'#menu:awardWindow/mission/nextButton')

    def getNextButtonTooltip(self):
        return makeTooltip(b'#menu:awardWindow/mission/nextButton/tooltip/header', b'#menu:awardWindow/mission/nextButton/tooltip/body')
...
class OperationUnlockedAward(MissionAward):
    BODY_BUTTON_LINKAGE = b'LobbyMenuButton'

    def __init__(self, quest, ctx, proxyEvent):
        super(OperationUnlockedAward, self).__init__(quest, ctx, proxyEvent)
        _nextOperationID = ctx.get(b'nextOperationID', 1)
    ...
    def getBackgroundImage(self):
        return (b'../maps/icons/quests/awards/operation_{}_award_bg.png').format(self._nextOperation.getID())

    def getHeader(self):
        return text_styles.superPromoTitle(makeHtmlString(b'html_templates:lobby/textStyle', b'alignText', {b'align': b'center', 
           b'message': (_ms(PERSONAL_MISSIONS.OPERATIONUNLOCKEDWINDOW_HEADER, title=self._nextOperation.getShortUserName()))}))

    def getDescription(self):
        return b''

    def getAdditionalText(self):
        return text_styles.main(makeHtmlString(b'html_templates:lobby/textStyle', b'alignText', {b'align': b'center', 
           b'message': (_ms(PERSONAL_MISSIONS.OPERATIONUNLOCKEDWINDOW_DESCR, label=_ms(PERSONAL_MISSIONS.OPERATIONUNLOCKEDWINDOW_NEXTBUTTON)))}))
