...
class WQRerollCooldown(SyncValidator):
    ...
    def _validate(self):
        ...
        if nextRerollAvailableTimestamp > time_utils.getCurrentLocalServerTimestamp():
            return makeError(b'WQ_REROLL_TIMEOUT')
        return makeSuccess()
...
class WQNotCompletedValidator(SyncValidator):
    ...
    def _validate(self):
        if self.__quest.isCompleted():
            return makeError(b'WQ_QUEST_COMPLETED')
        return makeSuccess()
    ...
    def _confirm(self, callback):
        ...
        if numTanksInBattle > 0:
            ...
            builder.setMessageArgs(fmtArgs=[
             FmtArgs(warningString, b'warning', R.styles.NeutralTextBigStyle()),
             FmtArgs(timeLimitMsg, b'timeLimitMsg', R.styles.NeutralTextBigStyle())])
        else:
...
class WeeklyQuestReroll(Processor):
    ...
    def _errorHandler(self, code, errStr=b'', ctx=None):
        return makeI18nError(b'weekly_quests/reroll/%s' % errStr, defaultSysMsgKey=b'weekly_quests/reroll/unknown_error')

    def _successHandler(self, code, ctx=None):
        return makeI18nSuccess(b'weekly_quests/reroll/success')

    def _request(self, callback):
        _logger.debug(b'Make server request to reroll quest: %s', self._quest)
