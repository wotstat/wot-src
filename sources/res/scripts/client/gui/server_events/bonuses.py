...
class SimpleBonus(object):
    ...
    def isReceived(self):
        LOG_CODEPOINT_WARNING(b'SimpleBonus::isReceived')
        return False
...
class TicketTokensBonus(TokensBonus):
    ...
    def formatValue(self):
        ...
        amount = sum([data.get(b'count', 0) for tokenID, data in self._value.iteritems() if tokenID in ticketNameList])
        if bool(amount):
            return amount
        else:
            return
    ...
    def _format(self, styleSubset):
        tokenName = None
        if self._value.keys() and self._value.keys()[0].startswith(b'wtevent:'):
            tokenName = self._value.keys()[0].split(b':')[1]
        formattedValue = self.formatValue()
        if self._name is not None and formattedValue is not None:
            text = makeHtmlString((b'html_templates:lobby/quests/{}').format(styleSubset), tokenName if tokenName else self._name, {b'value': formattedValue})
            if text != self._name:
                return text
        return formattedValue
...
class StampTokensBonus(TokensBonus):
    ...
    def formatValue(self):
        ...
        amount = sum([data.get(b'count', 0) for tokenID, data in self._value.iteritems() if tokenID == stampName])
        if bool(amount):
            return amount
        else:
            return
...
class MainPrizeDiscountTokensBonus(TokensBonus):
    ...
    def __init__(self, name, value, isCompensation=False, ctx=None):
        ...
        self._name = b'mainPrizeDiscount'
        return
        ...
        for tokenID, tokenVal in self._value.iteritems():
            ...
            if lootBox is not None:
                ...
                if lootBoxType in wtLootboxesTypes:
                    templateStr = b'engineer' if lootBoxType == WhiteTigerLootBoxes.WT_BOSS else b'hunter'
                    result.append(makeHtmlString(b'html_templates:lobby/quests/bonuses/wtlootBox', templateStr, {b'lootBoxType': (lootBox.getType()), 
                       b'count': (tokenVal[b'count'])}))
                else:
                    result.append(makeHtmlString(b'html_templates:lobby/quests/bonuses', b'lootBox', {b'lootBoxType': (lootBox.getType()), 
                       b'name': (lootBox.getUserName()), b'count': (tokenVal[b'count'])}))
...
class TmanTemplateTokensBonus(TokensBonus):
    TANKMAN_RECEVIED_TOKEN_PREFIX = b'got_'
...
class WtTmanTemplateTokensBonus(TmanTemplateTokensBonus):
    WT_TANKMAN_TOKEN_PREFIX = b'tankmen_WT2025_'
    ...
    def formattedList(self):
        ...
        for tID, _ in self.getTokens().iteritems():
            recruitInfo = getRecruitInfo(tID)
            result.append(makeHtmlString(b'html_templates:lobby/quests/bonuses', b'tankman', {b'crewName': (recruitInfo.getFullUserNameByNation(0))}))
...
class WtCustomizationsBonus(CustomizationsBonus):
    ...
    def _format(self, styleSubset):
        ...
        if self._name is not None and item is not None:
            c11nItem = self.getC11nItem(item)
            itemType = item.get(b'custType') if item.get(b'custType') == c11nItem.itemTypeName else c11nItem.itemTypeName
            value = item.get(b'value')
            text = makeHtmlString((b'html_templates:lobby/quests/bonuses/{}/wtEvent').format(self._name), itemType, {b'name': (c11nItem.userName), b'value': value})
            if text != self._name:
                return text
        return item
