...
__all__ = (b'getServerEventsConfig', b'getBattleMattersController')

def getServerEventsConfig(manager):
    cache = _EventsCache()
    cache.init()
    manager.addInstance(IEventsCache, cache, finalizer=b'fini')
    return
...
def getBattleMattersController(manager):
    ...
    controller.init()
    manager.addInstance(IBattleMattersController, controller, finalizer=b'fini')
    return
