...
class Condition(object):
    __slots__ = (b'__requiredQuestionId', b'__requiredAnswers', b'__requiredOptionId', b'__quantifier', b'__isRequiredAnswer')
...
class Question(IQuestion):
    __slots__ = (b'__surveyGroup', b'__questionId', b'__isMultiple', b'__isRequired', b'__condition', b'__answers', b'__guiParameters', b'__linkedParameters')
    _mapboxCtrl = dependency.descriptor(IMapboxController)

    def __init__(self, *args, **kwargs):
        self.__surveyGroup = kwargs.get(b'surveyGroup')
        self.__questionId = kwargs.get(b'questionId')
        self.__isRequired = kwargs.get(b'isRequired', False)
        self.__isMultiple = kwargs.get(b'isMultiple', False)
        self.__condition = kwargs.get(b'condition')
        self.__answers = kwargs.get(b'answers')
        self.__guiParameters = kwargs.get(b'guiParameters')
        self.__linkedParameters = kwargs.get(b'linkedParameters')
        return
    ...
    def getImage(self):
        if self.__guiParameters.useMapId:
            return b'%s_%s' % (self.__guiParameters.image, self._mapboxCtrl.surveyManager.getMapId())
        else:
            if self.__guiParameters.useLinkedParams:
                ...
                if sourceQuestionId:
                    ...
                    return b'%s_%s' % (self.__guiParameters.image, replaceHyphenToUnderscore(choice))
            if self.__guiParameters is not None:
    ...
    def getTitleParameters(self):
        ...
        itemsResIds = [strPath.dyn((b'_').join((pathPrefix, replaceHyphenToUnderscore(source))))() for source in sources]
    ...
    def convertAnswers(self, answers, optionId):
        if len(answers) > 1:
            _logger.error(b'Incorrect answers for the question with questionId=%s', self.__questionId)
            return []
        if answers:
            return answers[0].get(b'choices', [])
        return []
    ...
    def validateAnswers(self, answers, oldAnswers):
        ...
        if groupsAnswers is None:
            ...
            newChoices = first(answers, {}).get(b'choices', [])
            if any([set(newChoices).issubset(group) for group in groupsAnswers]):
                return answers
            newChoice = set(newChoices) - set(first(oldAnswers, {}).get(b'choices', []))
            requiredGroup = findFirst((lambda group: newChoice.issubset(set(group))), groupsAnswers, [])
            answers[0][b'choices'] = set(requiredGroup).intersection(newChoices)
            return answers
...
class _TableQuestion(Question):
    __slots__ = (b'__options',)

    def __init__(self, *args, **kwargs):
        super(_TableQuestion, self).__init__(*args, **kwargs)
        self.__options = kwargs.get(b'options')
        return
    ...
    def convertAnswers(self, answers, optionId):
        answer = findFirst((lambda answer: answer.get(b'optionId') == optionId), answers)
        if answer is not None:
            return answer.get(b'choices', [])
        else:
            return []
    ...
    def validateAnswers(self, answers, oldAnswers):
        _logger.debug(b'Unsupported operation for a table question')
        return answers
...
class _InteractiveMapQuestion(Question):
    ...
    def convertAnswers(self, answers, optionId):
        if not answers:
            return []
        choices = answers[0].get(b'choices', [])
        return [json.dumps(item) for item in choices]

    def validateAnswers(self, answers, oldAnswers):
        _logger.debug(b'Unsupported operation for a question with the interactive map')
        return answers
...
class _VehicleQuestion(Question):
    ...
    def convertAnswers(self, answers, optionId):
        if answers:
            return first(answers, {}).get(b'choices', [])
        return []
...
class _TextQuestion(Question):
    ...
    def validateAnswers(self, answers, oldAnswers):
        _logger.debug(b'Unsupported operation for a text question')
        return answers


class AlternativeQuestion(IQuestion):
    __slots__ = (b'__questionId', b'_alternatives', b'__isSynchronizedAnswers')

    def __init__(self, *args, **kwargs):
        self.__questionId = kwargs.get(b'questionId', None)
        self._alternatives = kwargs.get(b'alternatives', [])
        self.__isSynchronizedAnswers = kwargs.get(b'isSynchronizedAnswers', False)
        return
...
class AlternativeOneManyQuestion(AlternativeQuestion):
    ...
    def synchronizeAnswers(self, surveyData, altQuestion, newAnswers):
        choices = set([choice for answer in newAnswers for choice in answer.get(b'choices', [])])
        ...
        if simpleQuestion is None or tableQuestion is None:
            _logger.error(b'Invalid alternatives for the question')
            return
        else:
    ...
    def __findSavedAnswers(qId, surveyData, choices):
        return [answer for answer in surveyData.get(qId, []) if answer.get(b'optionId') in choices]
