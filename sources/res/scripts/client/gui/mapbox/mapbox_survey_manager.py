...
class MapboxSurveyManager(object):
    __slots__ = (b'__questions', b'__currentQuestionIdx', b'__mapId', b'__surveyGroup', b'__surveyData')
    ...
    def startSurvey(self, mapId):
        ...
        if survey is None:
            raise SoftException(b'Invalid survey config for Mapbox mode')
    ...
    def resetSurvey(self, mapId):
        ...
        savedData = surveys.get(mapId, {}).get(b'data', {})
    ...
    def getSelectedAnswers(self, questionId, optionId=None):
        ...
        if question is None:
            _logger.error(b'Unable to get the saved answers for the question with id=%s', questionId)
            return []
        else:
            answers = self.__surveyData[b'data'].get(questionId, [])
            return question.convertAnswers(answers, optionId)

    def saveAnswers(self, questionId, answers):
        question = self.getQuestion(questionId)
        if question is None:
            _logger.error(b'There is not a question with id=%s', questionId)
            return
        else:
            surveyData = self.__surveyData[b'data']
    ...
    def getSurveyData(self):
        result = []
        for qId, answers in self.__surveyData[b'data'].iteritems():
            question = self.getQuestion(qId)
            for answer in answers:
                for choice in answer[b'choices']:
                    if question.getQuestionType() == QuestionType.INTERACTIVE_MAP:
                        choice = b'[%s, %s] %s' % (choice[b'x'], choice[b'y'], choice[b'comment'])
                    result.append({b'question_id': (qId.split(b'_')[0]), 
                       b'answer_choice': choice, 
                       b'answer_option': (answer[b'optionId'])})

        result.sort(key=operator.itemgetter(b'question_id', b'answer_choice', b'answer_option'))
        return {b'name': (self.__mapId), 
           b'answers': result}
