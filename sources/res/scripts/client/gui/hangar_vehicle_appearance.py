...
AnchorHelper = namedtuple(b'AnchorHelper', [54, 55, 56, 57, 58])
...
class HangarVehicleAppearance(ScriptGameObject):
    ...
    def __reload(self, vDesc, vState, outfit):
        _logger.info(b'HangarVehicleAppearance::__reload vehicle(%s), style(%s)', vDesc.name, outfit.style.userString if outfit.style else b'None')
        self.__clearModelAnimators()
        self.__loadState.unload()
        if self.fashion is not None:
            self.fashion.removePhysicalTracks()
    ...
    def __startBuild(self, vDesc, vState):
        ...
        for name, builders in physicalTracksBuilders.iteritems():
            for index, builder in enumerate(builders):
                resources.append(builder.createLoader(self.__spaceId, (b'{0}{1}PhysicalTrack').format(name, index), modelsSet))
    ...
    def __assembleModel(self):
        ...
        if not self.__isVehicleDestroyed:
            ...
            dirtEnabled = BigWorld.dirtEnabled() and b'HD' in self.__vDesc.type.tags
            if dirtEnabled:
    ...
    def __handleEntityUpdated(self, event):
        ...
        if ctx[b'entityId'] == self.__vEntity.id:
            self._reloadColliderType(ctx[b'state'])
