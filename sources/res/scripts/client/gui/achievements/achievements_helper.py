...
def convertDbIdsToAchievements(layout, dossier):
    ...
    for achievementID in layout:
        if achievementID > 0:
            ...
            retrievedAchievement = dossier.getTotalStats().getAchievement((b'rareAchievements', -achievementID))
        if retrievedAchievement is not None:
            achievements.append(retrievedAchievement)
        else:
            lostAchievements += 1
            LOG_ERROR(b'Achievement %d not found in dossier.' % achievementID)
