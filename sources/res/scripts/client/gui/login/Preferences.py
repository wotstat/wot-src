...
class Preferences(dict):
    ...
    def __readOldPreferencesFormat(self, loginInfo):
        self[b'login'] = BigWorld.wg_ucpdata(loginInfo.readString(b'login', b''))
        self[b'remember_user'] = loginInfo.readBool(b'rememberPwd', False)
        if self[b'remember_user']:
            pwdLengthToken2 = BigWorld.wg_ucpdata(loginInfo.readString(b'token2'))
            if pwdLengthToken2:
        ...
        self[b'session'] = BigWorld.wg_cpsalt(loginInfo.readString(b'salt', b''))
