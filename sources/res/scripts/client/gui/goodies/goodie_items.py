...
_BOOSTER_TYPE_NAMES = {(GOODIE_RESOURCE_TYPE.GOLD): b'booster_gold', 
   (GOODIE_RESOURCE_TYPE.CREDITS): b'booster_credits', 
   (GOODIE_RESOURCE_TYPE.XP): b'booster_xp', 
   (GOODIE_RESOURCE_TYPE.CREW_XP): b'booster_crew_xp', 
   (GOODIE_RESOURCE_TYPE.FREE_XP): b'booster_free_xp', 
   (GOODIE_RESOURCE_TYPE.FL_XP): b'booster_fl_xp', 
   (GOODIE_RESOURCE_TYPE.FREE_XP_CREW_XP): b'booster_free_xp_and_crew_xp', 
   (GOODIE_RESOURCE_TYPE.FREE_XP_MAIN_XP): b'booster_free_xp_and_crew_xp'}
...
DEMOUNT_KIT_NAMES = {(GOODIE_RESOURCE_TYPE.GOLD): b'common'}
...
_GUI_TYPE_TO_CLAN_RESERVE = {(GOODIE_RESOURCE_TYPE.CREDITS): b'COMBAT_PAYMENTS_2_0', 
   (GOODIE_RESOURCE_TYPE.XP): b'TACTICAL_TRAINING_2_0', 
   (GOODIE_RESOURCE_TYPE.FREE_XP): b'MILITARY_EXERCISES_2_0', 
   (GOODIE_RESOURCE_TYPE.CREW_XP): b'ADDITIONAL_BRIEFING_2_0'}
...
def getFullNameForBoosterIcon(boosterType, isPremium=False, isExpirable=False):
    return (b'{}{}{}').format(getBoosterGuiType(boosterType), b'_premium' if isPremium else b'', b'_expirable' if isExpirable else b'')
...
class _Goodie(object):
    ...
    @property
    def count(self):
        return self.__getStateAttribute(b'count', 0)

    @property
    def finishTime(self):
        return self.__getStateAttribute(b'finishTime', 0)

    @property
    def state(self):
        return self.__getStateAttribute(b'state', GOODIE_STATE.INACTIVE)

    @property
    def enabled(self):
        return self.__getDescrAttribute(b'enabled', False)

    @property
    def maxCount(self):
        return self.__getDescrAttribute(b'counter', 0)
    ...
    def isExpirable(self):
        return bool(self.__getDescrAttribute(b'expireAfter', None) or self.useby)

    @property
    def expireAfter(self):
        return self.__getDescrAttribute(b'expireAfter', None)

    @property
    def useby(self):
        return self.__getDescrAttribute(b'useby', 0)

    @property
    def expirations(self):
        useby = self.__getDescrAttribute(b'useby', 0)
        usebyFunc = partial(min, useby) if useby > 0 else (lambda value: value)
        expirations = {}
        expirableCount = 0
        for expirationDate, count in self.__getStateAttribute(b'expirations', {}).iteritems():
    ...
    @property
    def effectTime(self):
        return self.__getDescrAttribute(b'lifetime', 0)
...
class _PersonalDiscount(_Goodie):
    ...
    def getFormattedValue(self, formatter=None):
        value = (b'{}%').format(self.effectValue)
...
class Booster(BoosterUICommon):
    ...
    def __repr__(self):
        ...
        return (b'Booster(id={} ({}), state={}, count={}, expirations={})').format(self.boosterID, self.getTypeAsString(), stateStr, self.count, expirations)
...
class ClanReservePresenter(BoosterUICommon):
    ...
    def getFormattedValue(self, formatter=None):
        ...
        if len(valuesSet) == 1:
            strValue = (b'{}%').format(first(valuesSet))
        else:
            strValue = (b'{}%-{}%').format(min(valuesSet), max(valuesSet))
        if formatter is not None:
...
class DemountKit(_Goodie):
    ...
    def getIcon(self, size):
        return backport.image(R.images.gui.maps.icons.demountKit.dyn((b'{}_{}').format(self.demountKitGuiType, size))())
...
GoodieType = typing.TypeVar(b'GoodieType', bound=_Goodie)
BoostersType = typing.TypeVar(b'BoostersType', bound=BoosterUICommon)
