...
class BoosterStateProvider(IBoostersStateProvider):
    ...
    def fini(self):
        ...
        if hasattr(player, b'goodiesSnapshot'):
    ...
    def addBooster(self, bID, booster, stateInfo, resourceData):
        ...
        state = stateInfo[b'state']
        expirations, count = self._filterOutExpired(stateInfo)
        self.__personalGoodies[bID] = GoodieVariable(state, stateInfo[b'finishTime'], count, expirations)
    ...
    def buildCache(self):
        ...
        if goodiesSnapshot:
            for snapshot in goodiesSnapshot:
                bId = snapshot[b'goodieID']
                resource = snapshot[b'resource']
                descr = _NamedGoodieData(GOODIE_VARIETY.BOOSTER, (
                 GOODIE_TARGET_TYPE.ON_POST_BATTLE, None, 0), True, snapshot[b'lifetime'], snapshot[b'useby'], 0, False, None, (
                 resource[b'type'], resource[b'value'], resource[b'isPercentage']), bId in PR2BoosterIDs.ALL_EXPIRABLE_ITEMS, True)
                booster = Booster(bId, descr, self)
                self.addBooster(bId, booster, snapshot[b'stateInfo'], descr.resource)
                _logger.debug(b'[BoosterStateProvider] %d: %s', bId, booster)

            self.onStateUpdated()
        return

    def _filterOutExpired(self, stateInfo):
        if not stateInfo[b'expirations']:
            return (stateInfo[b'expirations'], stateInfo[b'count'])
        valid = {}
        expired = {}
        newCount = stateInfo[b'count']
        for timestamp, count in stateInfo[b'expirations'].iteritems():
        ...
        if stateInfo[b'state'] == GOODIE_STATE.ACTIVE and expired:
