...
def getDefaultBonusPackersMap():
    ...
    return {b'battlePassPoints': (BattlePassPointsBonusPacker()), 
       ...
       b'clans_customizations': (CustomizationBonusUIPacker()), 
       ...
       b'lootBoxToken': tokenBonusPacker, 
       b'entitlements': (EntitlementBonusUIPacker()), 
       b'preferredMapSlots': (PreferredMapSlotBonusUIPacker())}
...
class TokenBonusUIPacker(BaseBonusUIPacker):
    ...
    def _getContentId(cls, bonus):
        ...
        for token in bonusTokens:
            ...
            if name.endswith(GOLD_MISSION):
                ...
                lootboxKeyRes = R.views.dyn(b'gui_lootboxes').dyn(b'lobby').dyn(b'gui_lootboxes').dyn(b'tooltips').dyn(b'LootboxKeyTooltip')
    ...
    def __packLootboxToolTip(cls, complexToken, token):
        return {b'lootBoxID': (int(getLootBoxIDFromToken(token.id)))}

    @classmethod
    def __packLootboxKeyToolTip(cls, complexToken, token):
        return {b'lootBoxKeyID': (int(getLootBoxKeyIDFromToken(token.id)))}
...
class StyleProgressBonusUIPacker(BaseBonusUIPacker):
    ...
    def _packSingleBonus(cls, bonus):
        ...
        if camo is not None:
            ...
            _logger.error(b'Missing camouflage for StyleProgressBonus: styleID=%s; level=%s', styleID, progressLevel)
            icon = b''
            label = b''
        model.setIcon(icon)
        model.setLabel(label)
...
class TmanTemplateBonusPacker(BaseBonusUIPacker):
    ...
    def __packTmanTemplateToken(cls, tokenID, bonus):
        ...
        if recruitInfo is None:
            ...
            model = TokenBonusModel()
            cls._packCommon(bonus, model)
            model.setIcon(b'tankwoman' if recruitInfo.isFemale() else b'tankman')
...
class BonusUIPacker(object):
    ...
    def pack(self, bonus):
        ...
        _logger.error(b'Bonus packer for bonus type %s was not implemented yet.', bonus.getName())
        return []
