...
DOSSIER_BADGE_ICON_PREFIX = b'badge_'
DOSSIER_ACHIEVEMENT_POSTFIX = b'_achievement'
DOSSIER_BADGE_POSTFIX = b'_badge'
EXCHANGE_RATE_TOKEN = b'exchangeRateToken'
VEHICLE_RENT_ICON_POSTFIX = b'_rent'
...
WEEKLY_REWARDS_ORDER = (b'battlePassPoints', b'personal_missions_points')
...
def getDefaultBonusPackersMap():
    ...
    bonusPackers = {b'battlePassPoints': (BattlePassPointsBonusPacker()), 
       b'battleToken': tokenBonusPacker, 
       b'berths': simpleBonusPacker, 
       b'blueprints': blueprintBonusPacker, 
       b'blueprintsAny': blueprintBonusPacker, 
       b'creditsFactor': simpleBonusPacker, 
       b'crewBooks': (CrewBookBonusUIPacker()), 
       b'crewSkins': (CrewSkinBonusUIPacker()), 
       b'customizations': (CustomizationBonusUIPacker()), 
       b'dailyXPFactor': simpleBonusPacker, 
       b'dossier': (DossierBonusUIPacker()), 
       b'finalBlueprints': blueprintBonusPacker, 
       b'freeXP': simpleBonusPacker, 
       b'freeXPFactor': simpleBonusPacker, 
       b'goodies': (GoodiesBonusUIPacker()), 
       b'items': (ItemBonusUIPacker()), 
       b'lootBox': tokenBonusPacker, 
       b'meta': simpleBonusPacker, 
       b'slots': simpleBonusPacker, 
       b'strBonus': simpleBonusPacker, 
       b'tmanToken': (TmanTemplateBonusPacker()), 
       b'tankmen': (TankmenBonusUIPacker()), 
       b'tankmenXP': simpleBonusPacker, 
       b'tankmenXPFactor': simpleBonusPacker, 
       b'tokens': tokenBonusPacker, 
       b'vehicles': (VehiclesBonusUIPacker()), 
       b'xp': simpleBonusPacker, 
       b'xpFactor': simpleBonusPacker, 
       b'groups': (GroupsBonusUIPacker()), 
       b'progressionXPToken': tokenBonusPacker, 
       b'dogTagComponents': (DogTagComponentsUIPacker()), 
       ...
       b'currencies': CurrenciesBonusUIPacker, 
...
def getLocalizedBonusName(name):
    ...
    if localizedLabel.exists():
        ...
        _logger.warning(b"Localized text for the 'additional rewards tooltip' label for %s reward was not found.", name)
    return labelStr
...
class TokenBonusUIPacker(BaseBonusUIPacker):
    ...
    _RANKED_TOKEN_SOURCE = b'rankedPoint'
    _PM_POINTS_TOKEN_SOURCE = b'personal_missions_points'
    _BATTLE_BONUS_X5_TOKEN_SOURCE = b'bonus_battle_task'
    _CREW_BONUS_X3_TOKEN_SOURCE = b'crew_bonus_x3'
    _GOLD_MISSION_TOKEN_SOURCE = b'gold_mission'
    ...
    def _isTokenForTooltipValid(cls, tokenID):
        ...
        if tokenType == b'':
            ...
            if tooltipPacker is None:
                _logger.warning(b'There is not a tooltip creator for a token bonus %s', tokenType)
                return False
            return True
    ...
    def _getTokenBonusType(cls, tokenID, complexToken):
        ...
        if tokenID.split(b':')[0].endswith(GOLD_MISSION):
            return GOLD_MISSION
    ...
    def __packExchangeRateToken(cls, model, bonus, _, token):
        if token.count <= 0:
            ...
            model.setIconSmall(backport.image(R.images.gui.maps.icons.quests.bonuses.dyn(AWARDS_SIZES.SMALL).dyn(b'rate_' + bonus.resourceName)()))
            model.setIconBig(backport.image(R.images.gui.maps.icons.quests.bonuses.dyn(AWARDS_SIZES.BIG).dyn(b'rate_' + bonus.resourceName)()))
            model.setLabel(formatCountLabel(bonus.getCount()))
            return model
...
class Customization3Dand2DbonusUIPacker(CustomizationBonusUIPacker):
    _3D_STYLE_ICON_NAME = b'style_3d'
...
class WeeklyMissionPremiumDaysPacker(SimpleBonusUIPacker):
    ...
    def _packSingleBonus(cls, bonus, label):
        ...
        if days in cls._ICONS_AVAILABLE:
            ...
            model.setName(b'premium_universal')
        return model
...
class BonusUIPacker(object):
    ...
    def pack(self, bonus):
        ...
        _logger.error(b'BonusUIPacker.pack - Packer not implemented yet, name=%s', bonus.getName())
        return []
    ...
    def getToolTip(self, bonus):
        ...
        _logger.error(b'BonusUIPacker.getToolTip - Packer not implemented yet, name=%s', bonus.getName())
        return []
    ...
    def getContentId(self, bonus):
        ...
        _logger.error(b'BonusUIPacker.getContentId - Packer not implemented yet, name=%s', bonus.getName())
        return []
...
def getDailyMissionsMapping():
    ...
    mapping.update({b'vehicles': (DailyMissionsVehiclesBonusUIPacker())})
    return mapping
...
def getWeeklyMissionsMapping():
    mapping = getDefaultBonusPackersMap()
    mapping.update({b'premium_plus': (WeeklyMissionPremiumDaysPacker()), 
       b'vehicles': (DailyMissionsVehiclesBonusUIPacker())})
    return mapping
