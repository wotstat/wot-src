...
class EventBusListener(type):
    ...
    def __new__(mcs, name, bases, namespace):
        ...
        if not issubclass(cls, EventSystemEntity):
            raise SoftException(b'This meta should be applied only to EventSystemEntity classes')
        try:
        ...
        for attribute in namespace.values():
            if isinstance(attribute, FunctionType) and getattr(attribute, b'__event_bus_data__', None) is not None:
                event = attribute.__event_bus_data__
                if event not in handlers:
                    handlers[event] = attribute
                else:
                    raise SoftException(b'Event %s is handled multiple times in %s' % (event, cls))
