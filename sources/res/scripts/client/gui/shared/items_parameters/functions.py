    ...
    for _, tankman in vehicle.crew:
        ...
        for bonusSkills in itervalues(tankman.bonusSkills):
            for bonusSkill in bonusSkills:
                if bonusSkill and bonusSkill.name != b'any' and bonusSkill.level != tankmen.NO_SKILL and bonusSkill.isSkillActive:
        ...
        for _, tankman in vehicle.crew:
            ...
            for skill in tankman.skills:
                if skill.isSkillActive and skill.name in situationalBonuses:
                    skillLevelIncrease = getattr(tankmen.getSkillsConfig().getSkill(skill.name), b'crewLevelIncrease', 0.0)
            ...
            for bonusSkills in itervalues(tankman.bonusSkills):
                for bonusSkill in bonusSkills:
                    if bonusSkill and bonusSkill.isSkillActive:
                        skillLevelIncrease = getattr(tankmen.getSkillsConfig().getSkill(bonusSkill.name), b'crewLevelIncrease', 0.0)
...
def getClientShotDispersion(vehicleDescr, shotDispersionFactor):
    ...
    values = []
    multShotDispersionFactor = vehicleDescr.miscAttrs[b'multShotDispersionFactor'] * shotDispersionFactor
    ...
    if b'twinGun' in gun.tags:
        siegeVehDescr = vehicleDescr.siegeVehicleDescr
        multShotDispersionSiegeFactor = siegeVehDescr.miscAttrs[b'multShotDispersionFactor'] * shotDispersionFactor
...
def getLowChargeShotDispersion(vehDescr, originalShotDisp):
    ...
    modifier = extractModifier(modifiers, b'multShotDispersionFactor')
...
def _getLowChargeShotMiscAttr(miscAttrs, shotIdx, attribute):
    attr = (b'lowChargeShot/shot{}/{}').format(shotIdx, attribute)
