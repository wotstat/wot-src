...
NO_DATA = b'no data'
_DEFAULT_GUN_MECHANICS_FACTORS = {b'gun/extraReloadTime': 0.0}
...
def isAutoReloadGun(gun):
    if gun is not None:
        return b'autoreload' in gun.tags
    else:
        return False
...
def isAutoShootGun(gun):
    if gun is not None:
        return b'autoShoot' in gun.tags
    else:
        return False
...
def isUnlimitedClipGun(gun):
    if gun is not None:
        return b'unlimitedClip' in gun.tags
    else:
        return False
...
def isDualAccuracy(gun):
    if gun is not None:
        return b'dualAccuracy' in gun.tags
    else:
        return False
...
def calcGunParams(gunDescr, descriptors):
    ...
    for descr in descriptors:
        ...
        if currShellsCount > 1:
            ...
            _updateMinMaxValues(result, RELOAD_MAGAZINE_TIME_PROP_NAME, descr.reloadTime + factors[b'gun/extraReloadTime'] + mechanicsReloadDelay)
        ...
        if autoReload:
            ...
            reloadTime = descr.reloadTime + factors[b'gun/extraReloadTime'] + mechanicsReloadDelay
...
def getOptionalDeviceWeight(itemDescr, vehicleDescr):
    ...
    if vehicleDescr is not None:
        ...
        weight = ceil(vehicleDescr.physics[b'weight'] * mods[0] + mods[1])
