...
RELATIVE_POWER_PARAMS = (
 b'avgDamage', AVG_DAMAGE_PER_SECOND, b'avgPiercingPower', b'stunMinDuration', b'stunMaxDuration', b'reloadTime',
 TEMPERATURE_RELOAD_TIME, CONTINUOUS_SHOTS_PER_MINUTE, AUTO_RELOAD_PROP_NAME, b'reloadTimeSecs',
 TWIN_GUN_SWITCH_FIRE_MODE_TIME, b'clipFireRate', AUTO_SHOOT_CLIP_FIRE_RATE, SHELL_LOADING_TIME_PROP_NAME,
 BURST_FIRE_RATE, b'turboshaftBurstFireRate', DUAL_GUN_CHARGE_TIME, b'turretRotationSpeed', b'turretYawLimits',
 b'pitchLimits', b'gunYawLimits', b'aimingTime', SHOT_DISPERSION_ANGLE, DUAL_ACCURACY_COOLING_DELAY,
 b'avgDamagePerMinute', TEMPERATURE_AVG_DAMAGE_PER_MINUTE)
...
RELATIVE_MOBILITY_PARAMS = (
 ...
 b'speedLimits', TWIN_GUN_TOP_SPEED, WHEELED_SPEED_MODE_SPEED, TURBOSHAFT_SPEED_MODE_SPEED,
 ROCKET_ACCELERATION_SPEED_LIMITS, b'chassisRotationSpeed', ROCKET_ACCELERATION_REUSE_AND_DURATION,
 MAX_STEERING_LOCK_ANGLE, b'switchOnTime', b'switchOffTime', TURBOSHAFT_SWITCH_TIME)
...
RELATIVE_VISIBILITY_PARAMS = (b'circularVisionRadius', b'radioDistance')
...
def lackVehicleSteeringAngles(currentVehicle):
    ...
    chassis = vehicleDesc.type.xphysics[b'chassis'][vehicleDesc.chassis.name]
    axleSteeringAngles = chassis.get(b'axleSteeringAngles', None)
...
def getParameters(item, vehicleDescr=None):
    return get(item, vehicleDescr).get(b'parameters', {})
...
def getSituationalParams(skillNames):
    ...
    for skillName in skillNames:
        if skillName in CREW_MASTERY_BONUSES:
            situationalParams.append(b'situationalCrewLevelIncrease')
        ...
        if not GUI_SETTINGS.technicalInfo:
            ...
            for groupIdx, groupName in enumerate(RELATIVE_PARAMS):
                ...
                if isOpened:
                    ...
                    diffParams.update({b'vehIntCD': vehIntCD})
    ...
    def _getExtraParams(self, comparator, groupName, diffParams):
        ...
        if self._isExtraParamEnabled():
            ...
            if vehicle is None:
                return result
            installedBoosters = []
            if vehicle and vehicle.compactDescr == diffParams.get(b'vehIntCD'):
