...
_MODULE_TYPES_ORDER = (b'vehicleGun', b'vehicleTurret', b'vehicleEngine', b'vehicleChassis', b'vehicleRadio', b'vehicleFuelTank')
...
class VehicleModule(FittingItem):
    __slots__ = (b'_vehicleModuleDescriptor',)
...
class VehicleChassis(VehicleModule):
    ...
    def _getShortInfoKey(self):
        return (b'#menu:descriptions/{}').format(FITTING_TYPES.VEHICLE_WHEELED_CHASSIS if self.isWheeledChassis() else self.itemTypeName)
...
class VehicleTurret(VehicleModule):
    ...
    def mayInstall(self, vehicle, slotIdx=None, gunCD=0):
        if vehicle is None:
            ...
            if not installPossible and reason == b'not for this vehicle type':
                return (False, b'need gun')
            return (
    ...
    def getExtraIconInfo(self, vehDescr=None):
        if self.hasThermalVision():
            return backport.image(R.images.gui.maps.icons.modules.thermalVisionIcon())
        return
...
class VehicleGun(VehicleModule):
    __slots__ = (b'_defaultAmmo', b'_maxAmmo')
    ...
    def mayInstall(self, vehicle, slotIdx=None):
        ...
        if not installPossible and reason == b'not for current vehicle':
            return (False, b'need turret')
    ...
    def getExtraIconInfo(self, vehDescr=None):
        if self.isFlameGun():
            ...
            if self.isAutoReloadableDualGun(vehDescr):
                return backport.image(R.images.gui.maps.icons.modules.autoLoaderDualGunIcon())
            if self.hasDualGunDualAccuracy(vehDescr):
    ...
    def _getShortInfoKey(self, vehicleDescr=None):
        ...
        if self.isAutoReloadable(vehicleDescr):
            return (b'/').join((key, b'autoReload'))
        if self.isDualGun(vehicleDescr):
            return (b'/').join((key, b'dualGun'))
        if self.isAutoReloadableDualGun(vehicleDescr):
            return (b'/').join((key, b'autoReloadDualGun'))
        if self.isClipDualGun(vehicleDescr):
            return (b'/').join((key, b'clipDualGun'))
        return key
...
class Shell(FittingItem):
    ...
    @property
    def kind(self):
        if self.descriptor.distanceFactor is not None:
            return self.descriptor.kind + b'_DF'
        else:
            return self.descriptor.kind

    @property
    def longUserName(self):
        return self._getFormatLongUserName(b'kinds')

    @property
    def longUserNameAbbr(self):
        return self._getFormatLongUserName(b'kindsAbbreviation')

    @property
    def icon(self):
        return ICONS_MASK[:-4] % {b'type': (self.itemTypeName), 
           b'subtype': b'small/', 
           b'unicName': (self.descriptor.icon[0])}

    def getBonusIcon(self, size=b'small'):
        sizeFldr = R.images.gui.maps.icons.shell.dyn(size)
        if not sizeFldr:
            _logger.warn(b'Shell %s icon for size %s doesnt exists!', self.descriptor.iconName, size)
            return b''
