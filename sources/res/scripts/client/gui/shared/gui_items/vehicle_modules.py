...
MODULE_TYPES_ORDER = (b'vehicleGun', b'vehicleTurret', b'vehicleEngine', b'vehicleChassis', b'vehicleRadio', b'vehicleFuelTank')
...
class ModulesIconNames(CONST_CONTAINER):
    WHEELED_CHASSIS = b'wheeledChassis'
    CHASSIS = b'chassis'
    TURRET = b'tower'
    GUN = b'gun'
    ENGINE = b'engine'
...
class VehicleModule(FittingItem):
    ...
    def getBonusIcon(self, size=b'small'):
        if size == b'small':
            return self.icon
        bigIconName = self.iconName + b'Big'
...
class Shell(FittingItem):
    ...
    def getBonusIcon(self, size=b'small'):
        ...
        if not sizeFldr:
            _logger.warning(b'Shell %s icon for size %s doesnt exists!', self.descriptor.iconName, size)
            return b''
    ...
    def _getShortInfoKey(self):
        if self.isDamageMutable():
            return b'#menu:descriptions/mutableDamageShell'
