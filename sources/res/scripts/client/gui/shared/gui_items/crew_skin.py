...
def localizedFullName(crewSkin):
    if crewSkin.getFirstName():
        fullName = b'%s %s' % (i18n.makeString(crewSkin.getFirstName()), i18n.makeString(crewSkin.getLastName()))
    else:
...
class GenderRestrictionsLocales(object):
    KEYS = {(TANKMAN_SEX.MALE): b'man', 
       (TANKMAN_SEX.FEMALE): b'woman'}


class Rarity(object):
    STRINGS = {(CREW_SKIN_RARITY.COMMON): b'common', 
       (CREW_SKIN_RARITY.RARE): b'rare', 
       (CREW_SKIN_RARITY.EPIC): b'epic'}


class CrewSkin(FittingItem):
    __slots__ = (b'__id', b'__freeCount', b'__tankmenIDs')
    ...
    def getLocalizedFullName(self):
        if self.getFirstName():
            fullName = b'%s %s' % (i18n.makeString(self.getFirstName()), i18n.makeString(self.getLastName()))
        else:
    ...
    def getIconName(self):
        return self._skinData().iconID.split(b'.')[0]
