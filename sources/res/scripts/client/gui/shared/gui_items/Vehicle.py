...
EmblemSlotHelper = namedtuple(b'EmblemSlotHelper', [b'tankAreaSlot', b'tankAreaId'])
SlotHelper = namedtuple(b'SlotHelper', [b'tankAreaSlot', b'tankAreaId'])
...
class VEHICLE_TAGS(CONST_CONTAINER):
    PREMIUM = b'premium'
    PREMIUM_IGR = b'premiumIGR'
    CANNOT_BE_SOLD = b'cannot_be_sold'
    SECRET = b'secret'
    SECRET_EXTENDED = b'secretExtended'
    SPECIAL = b'special'
    OBSERVER = b'observer'
    DISABLED_IN_ROAMING = b'disabledInRoaming'
    EVENT = b'event_battles'
    EXCLUDED_FROM_SANDBOX = b'excluded_from_sandbox'
    TELECOM = b'telecom'
    UNRECOVERABLE = b'unrecoverable'
    CREW_LOCKED = b'lockCrew'
    CREW_HIDDEN = b'hideCrew'
    OUTFIT_LOCKED = b'lockOutfit'
    PROGRESSION_DECALS_ONLY = b'lockExceptProgression'
    OPTIONAL_DEVICES_LOCKED = b'lockOptionalDevices'
    EQUIPMENT_LOCKED = b'lockEquipment'
    STORAGE_HIDDEN = b'hideStorage'
    MODE_HIDDEN = b'mode_hidden'
    EPIC_BATTLES = b'epic_battles'
    BATTLE_ROYALE = b'battle_royale'
    RENT_PROMOTION = b'rent_promotion'
    EARN_CRYSTALS = b'earn_crystals'
    MAPS_TRAINING = b'maps_training'
    T34_DISCLAIMER = b't34_disclaimer'
    CLAN_WARS_BATTLES = b'clanWarsBattles'
    FUN_RANDOM = b'fun_random'
    COMP7_BATTLES = b'comp7'
    DEBUT_BOXES = b'debutBoxes'
...
RentPackagesInfo = namedtuple(b'RentPackagesInfo', (
 b'hasAvailableRentPackages',
 b'mainRentType',
 b'seasonType'))
CrystalsEarnedInfo = namedtuple(b'CrystalsEarnedInfo', (
 b'current',
 b'max'))
EliteStatusProgress = typing.NamedTuple(b'EliteStatusProgress', (
 (
  b'unlocked', typing.Set[int]),
 (
  b'toUnlock', typing.Set[int]),
 (
  b'total', typing.Set[int])))
NO_VEHICLE_ID = -1

class Vehicle(FittingItem):
    __slots__ = (b'__customState', b'_inventoryID', b'_xp', b'_dailyXPFactor', b'_isElite', b'_isFullyElite', b'_clanLock', b'_isUnique', b'_rentPackages', b'_rentPackagesInfo', b'_isDisabledForBuy', b'_isSelected', b'_restorePrice', b'_tradeInAvailable', b'_tradeOffAvailable', b'_tradeOffPriceFactor', b'_tradeOffPrice', b'_searchableUserName', b'_personalDiscountPrice', b'_rotationGroupNum', b'_rotationBattlesLeft', b'_isRotationGroupLocked', b'_isInfiniteRotationGroup', b'_settings', b'_lock', b'_repairCost', b'_health', b'_gun', b'_turret', b'_engine', b'_chassis', b'_radio', b'_fuelTank', b'_equipment', b'_bonuses', b'_crewIndices', b'_slotsIds', b'_crew', b'_lastCrew', b'_hasModulesToSelect', b'_outfitComponents', b'_isStyleInstalled', b'_slotsAnchors', b'_unlockedBy', b'_maxRentDuration', b'_minRentDuration', b'_slotsAnchorsById', b'_hasNationGroup', b'_extraSettings', b'_groupIDs', b'_postProgression', b'_invData', b'_proxy', b'_selectedComp7Skill')
    ...
    class VEHICLE_STATE(object):
        ...
        LOCKED = b'locked'
        CREW_NOT_FULL = b'crewNotFull'
        AMMO_NOT_FULL = b'ammoNotFull'
        AMMO_NOT_FULL_EVENTS = b'ammoNotFullEvents'
        SERVER_RESTRICTION = b'serverRestriction'
        RENTAL_IS_OVER = b'rentalIsOver'
        IGR_RENTAL_IS_OVER = b'igrRentalIsOver'
        IN_PREMIUM_IGR_ONLY = b'inPremiumIgrOnly'
        GROUP_IS_NOT_READY = b'group_is_not_ready'
        NOT_PRESENT = b'notpresent'
        UNAVAILABLE = b'unavailable'
        UNSUITABLE_TO_QUEUE = b'unsuitableToQueue'
        ...
        RENTABLE = b'rentable'
        RENTABLE_AGAIN = b'rentableAgain'
        DISABLED = b'disabled'
        SUBSCRIPTION_SUSPENDED = b'subscription_suspended'
        WOT_PLUS_EXCLUSIVE_VEHICLE_DISABLED = b'wot_plus_exclusive_vehicle_disabled'
    ...
    def __init__(self, strCompactDescr=None, inventoryID=-1, typeCompDescr=None, proxy=None, extData=None, invData=None):
        ...
        if self._proxy is not None and self._proxy.inventory.isSynced() and self._proxy.stats.isSynced() and self._proxy.shop.isSynced() and self._proxy.vehicleRotation.isSynced() and self._proxy.recycleBin.isSynced():
            ...
            invRentData = self._invData.get(b'rent')
        ...
        self._settings = self._invData.get(b'settings', 0)
        self._extraSettings = self._invData.get(b'extraSettings', 0)
        self._lock = self._invData.get(b'lock', (0, 0))
        self._repairCost, self._health = self._invData.get(b'repair', (0, 0))
        self._selectedComp7Skill = self._invData.get(b'selectedComp7Skill', 0)
        ...
        self.initCrew()
        self._lastCrew = self._invData.get(b'lastCrew')
        if self.canTradeIn:
    ...
    def calcCrewBonuses(self, crew, proxy, fromBattle):
        ...
        bonuses[b'equipment'] = 0.0
        for eq in self.consumables.installed.getItems():
            bonuses[b'equipment'] += eq.crewLevelIncrease

        for battleBooster in self.battleBoosters.installed.getItems():
            bonuses[b'equipment'] += battleBooster.getCrewBonus(self)

        bonuses[b'optDevices'] = self.descriptor.miscAttrs[b'crewLevelIncrease']
        bonuses[b'commander'] = 0
        commanderEffRoleLevel = 0
        brotherhoodSum = 0.0
        for tID in crew:
            if tID is None:
                continue
            tmanInvData = {b'compDescr': tID} if fromBattle else proxy.inventory.getItems(GUI_ITEM_TYPE.TANKMAN, tID)
            if not tmanInvData:
                continue
            tdescr = tankmen.TankmanDescr(compactDescr=tmanInvData[b'compDescr'])
            brotherhoodSum += tdescr.skillLevel(b'brotherhood') or 0
        ...
        bonuses[b'brotherhood'] = brotherhoodLevel * tankmen.getSkillsConfig().getSkill(b'brotherhood').crewLevelIncrease
        commanderEffRoleLevel += bonuses[b'brotherhood'] + bonuses[b'equipment'] + bonuses[b'optDevices']
        bonuses[b'commander'] += round(commanderEffRoleLevel / tankmen.COMMANDER_ADDITION_RATIO)
        return
    ...
    def _buildCrew(self, crew, proxy, fromBattle):
        ...
        for idx, tankmanID in enumerate(crew):
            ...
            if tankmanID is not None:
                if fromBattle:
                    tmanInvData = {b'compDescr': tankmanID, b'invID': (-1)}
                else:
                    tmanInvData = proxy.inventory.getItems(GUI_ITEM_TYPE.TANKMAN, tankmanID)
                tankman = self.itemsFactory.createTankman(strCompactDescr=tmanInvData[b'compDescr'], inventoryID=tmanInvData.get(b'invID', tankmanID), vehicle=self, proxy=proxy)
    ...
    def createAppliedOutfits(self, proxy):
        ...
        if styleOutfitData is not None:
            ...
            styleIntCD = vehicles.makeIntCompactDescrByID(b'customizationItem', CustomizationType.STYLE, styledOutfitComponent.styleId)
    ...
    def minRentPrice(self):
        ...
        if minRentPackage is not None:
            return minRentPackage.get(b'rentPrice', MONEY_UNDEFINED)
        else:
            return MONEY_UNDEFINED
    ...
    def hasBattleTurrets(self):
        vDescr = self.descriptor
        return len(vDescr.hull.fakeTurrets[b'battle']) != len(vDescr.turrets)
    ...
    @property
    def isScout(self):
        return checkForTags(self.tags, b'scout')
    ...
    def longUserName(self):
        typeInfo = getTypeInfoByName(b'vehicle')
        tagsDump = [typeInfo[b'tags'][tag][b'userString'] for tag in self.tags if typeInfo[b'tags'][tag][b'userString'] != b'']
        return b'%s %s' % ((b'').join(tagsDump), getUserName(self.descriptor.type))
    ...
    def fullDescription(self):
        ...
        if description.find(b'_descr') == -1:
            return description
        return b''
    ...
    def shortDescriptionSpecial(self):
        ...
        if description.find(b'_short_special') == -1:
            return description
        return b''
    ...
    def longDescriptionSpecial(self):
        ...
        if description.find(b'_long_special') == -1:
            return description
        return b''
    ...
    def mayPurchase(self, money):
        if self.isOnlyForEventBattles:
            return (False, b'isDisabledForBuy')
        if self.isDisabledForBuy:
            return (False, b'isDisabledForBuy')
        if self.isPremiumIGR:
            return (False, b'premiumIGR')
        return super(Vehicle, self).mayPurchase(money)

    def mayRent(self, money):
        if getattr(BigWorld.player(), b'isLongDisconnectedFromCenter', False):
    ...
    def getDisclaimerTag(self):
        ...
        if len(disclaimers) > 1:
            _logger.warning(b"Vehicle has several disclaimer tags. This design isn't provided and only one will be used.")
        if disclaimers:
    ...
    def getDisclaimerUrl(self):
        ...
        return vehicleDisclaimerURLs.get(self.getDisclaimerTag(), b'')

    def getRentPackage(self, rentID=None):
        if rentID is not None:
            for package in self.rentPackages:
                if package.get(b'rentID', None) == rentID:
                    return package

        elif self.rentPackages:
            return min(self.rentPackages, key=itemgetter(b'rentPrice'))
        return
    ...
    def getRentPackageActionPrc(self, rentID=None):
        ...
        if package:
            return getActionPrc(package[b'rentPrice'], package[b'defaultRentPrice'])
        return 0
    ...
    def __repr__(self):
        return b'Vehicle<id:%d, intCD:%d, nation:%d, lock:%s>' % (
    ...
    def _getShortInfo(self, vehicle=None, expanded=False):
        description = i18n.makeString(b'#menu:descriptions/' + self.itemTypeName)
        caliber = self.descriptor.gun.shots[0].shell.caliber
        armor = findVehicleArmorMinMax(self.descriptor)
        return description % {b'weight': (backport.getNiceNumberFormat(float(self.descriptor.physics[b'weight']) / 1000)), 
           b'hullArmor': (backport.getIntegralFormat(armor[1])), 
           b'caliber': (backport.getIntegralFormat(caliber))}
    ...
    def __getStyledOutfitComponent(self, proxy, style, styleProgressionLevel, styleSerialNumber, season):
        ...
        if diff is None:
            ...
            if component.styleId != diffComponent.styleId:
                _logger.error(b'Merging outfits of different styles is not allowed. ID1: %s ID2: %s', component.styleId, diffComponent.styleId)
                return component.copy()
...
def getTypeUserName(vehType, isElite):
    if isElite:
        return i18n.makeString(b'#menu:header/vehicleType/elite/%s' % vehType)
    return i18n.makeString(b'#menu:header/vehicleType/%s' % vehType)


def getWotPlusExclusiveVehicleTypeUserName(vehType):
    return i18n.makeString(b'#menu:header/wotPlusExclusiveVehicleType/%s' % vehType)


def getTypeShortUserName(vehType):
    return i18n.makeString(b'#menu:classes/short/%s' % vehType)


def _getLevelIconName(vehLevel, postfix=b''):
    return b'tank_level_%s%d.png' % (postfix, int(vehLevel))


def getLevelBigIconPath(vehLevel):
    return b'../maps/icons/levels/%s' % _getLevelIconName(vehLevel, b'big_')


def getLevelSmallIconPath(vehLevel):
    return b'../maps/icons/levels/%s' % _getLevelIconName(vehLevel, b'small_')


def getLevelIconPath(vehLevel):
    return b'../maps/icons/levels/%s' % _getLevelIconName(vehLevel)
...
def getIconResourceName(vehicleName):
    return vehicleName.replace(b':', b'_').replace(b'-', b'_')
...
def getUniqueIconPath(vehicleName, withLightning=False):
    if withLightning:
        return b'../maps/icons/vehicle/unique/%s' % getItemIconName(vehicleName)
    return b'../maps/icons/vehicle/unique/normal_%s' % getItemIconName(vehicleName)
...
def getTypeBigIconPath(vehicleType, isElite=False):
    return RES_ICONS.getVehicleTypeBigIcon(vehicleType, b'_elite' if isElite else b'')
...
def getTypeBigIconResource(vehicleType, isElite=False):
    return R.images.gui.maps.icons.vehicleTypes.big.dyn(replaceHyphenToUnderscore(vehicleType + b'_elite' if isElite else vehicleType))


def getType44x44IconResource(vehicleType, isElite=False):
    return R.images.gui.maps.icons.vehicleTypes.c_44x44.dyn(replaceHyphenToUnderscore(vehicleType + b'_elite' if isElite else vehicleType))
...
def _getActualName(name, tags, textPrefix=False):
    if checkForTags(tags, VEHICLE_TAGS.PREMIUM_IGR):
        ...
        return makeHtmlString(b'html_templates:igr/premium-vehicle', b'name', {b'vehicle': name})
    return name
...
def getLobbyDescription(vehicle):
    return text_styles.stats(i18n.makeString(b'#menu:header/level/%s' % vehicle.level)) + b' ' + text_styles.main(i18n.makeString(b'#menu:header/level', vTypeName=getTypeUserName(vehicle.type, vehicle.isElite)))
...
def getBattlesLeft(vehicle):
    if vehicle.isInfiniteRotationGroup:
        return i18n.makeString(b'#menu:infinitySymbol')
