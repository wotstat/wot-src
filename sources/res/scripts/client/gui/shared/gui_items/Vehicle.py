...
class Vehicle(FittingItem):
    ...
    def __init__(self, strCompactDescr=None, inventoryID=-1, typeCompDescr=None, proxy=None, extData=None, invData=None):
        ...
        if self._proxy is not None and self._proxy.inventory.isSynced() and self._proxy.stats.isSynced() and self._proxy.shop.isSynced() and self._proxy.vehicleRotation.isSynced() and self._proxy.recycleBin.isSynced():
            ...
            self._isElite = vehDescr.type.isEliteByDefault and vehDescr.type.compactDescr not in getDossiersCache()[b'vehiclesInTrees'] or self.intCD in self._proxy.stats.eliteVehicles
    ...
    def copyCrew(self, otherVehicle):
        if not isinstance(otherVehicle, Vehicle):
            _logger.error(b'Crew can only be copied from an instance of Vehicle')
            return
        else:
            defaultCrew = [
             None] * len(self._descriptor.type.crewRoles)
            self._invData[b'crew'] = otherVehicle._invData.get(b'crew', defaultCrew)
            self._invData[b'battleCrewCDs'] = otherVehicle._invData.get(b'battleCrewCDs')
            self.initCrew()
            return
    ...
    def calcCrewBonuses(self, crew, proxy, fromBattle):
        ...
        if commanderEffRoleLevel > 0:
            ...
            bonuses[b'commander'] += round_py2_style(old_div(commanderEffRoleLevel, tankmen.COMMANDER_ADDITION_RATIO))
        return
        ...
        if not skillNames:
            ...
            for slotIdx, tman in self.crew:
                ...
                for skillName in skillNames:
                    ...
                    elif skillName not in rolesBonusSkills[roleType]:
...
def getVehicleStateIcon(vState):
    icon = _VEHICLE_STATE_TO_ICON.get(vState, b'')
    return icon


def getVehicleStateAddIcon(vState):
    icon = _VEHICLE_STATE_TO_ADD_ICON.get(vState, b'')
    return icon
