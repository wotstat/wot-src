...
ItemSellSpec = namedtuple(b'ItemSellSpec', (b'typeIdx', b'intCD', b'count'))
ItemDeconstructSpec = namedtuple(b'ItemDeconstructSpec', (b'typeIdx', b'intCD', b'vehicleCD'))

def showMessage(scopeMsg, msg, item, msgType=SystemMessages.SM_TYPE.Error, **kwargs):
    kwargs[b'userString'] = item.userName
    kwargs[b'type'] = msgType
    if item.itemTypeID == GUI_ITEM_TYPE.VEHICLE:
        entity = b'vehicle'
    else:
        entity = b'item'
        kwargs[b'typeString'] = item.userType
...
def showInventoryMsg(msg, item, msgType=SystemMessages.SM_TYPE.Error, **kwargs):
    scopeMsg = b'#system_messages:inventory/{0:>s}/{1:>s}'
...
def showUnlockMsg(msg, item, msgType=SystemMessages.SM_TYPE.Error, **kwargs):
    scopeMsg = b'#system_messages:unlocks/{0:>s}/{1:>s}'
    showMessage(scopeMsg, msg, item, msgType=msgType, **kwargs)
    return


def showShopMsg(msg, item, msgType=SystemMessages.SM_TYPE.Error, **kwargs):
    scopeMsg = b'#system_messages:shop/{0:>s}/{1:>s}'
    showMessage(scopeMsg, msg, item, msgType=msgType, **kwargs)
    return
...
class GroupedItemAction(CachedItemAction):
    __slots__ = (b'_groupID', b'_groupSize')
    ...
    def _pushGroupedMessages(result):
        if result and result.auxData:
            if result.success:
                SystemMessages.pushMessage(result.userMsg + b' ' + (b', ').join(m.userMsg for m in result.auxData if m), type=result.sysMsgType, priority=result.msgPriority, messageData=result.msgData)
            else:
...
class SellItemAction(CachedItemAction):
    ...
    def doAction(self):
        ...
        if item.isInInventory:
            ...
            showInventoryMsg(b'not_found', item)
...
class SellMultipleItems(IGUIItemAction):
    ...
    @adisp_process
    def doAction(self):
        Waiting.show(b'storage/forSell')
        ...
        if result.userMsg:
            SystemMessages.pushI18nMessage(result.userMsg, type=result.sysMsgType)
        Waiting.hide(b'storage/forSell')
        return
    ...
    def doAction(self):
        ...
        if not self._mayObtainForMoney(item):
            if self._mayObtainWithMoneyExchange(item):
                ...
                showShopMsg(b'common_rent_or_buy_error', item)
...
class VehicleBuyAction(BuyAction):
    ...
    def doAction(self):
        ...
        if item.itemTypeID is not GUI_ITEM_TYPE.VEHICLE:
            LOG_ERROR(b'Value of int-type descriptor is not refer to vehicle', self.__vehCD)
            return
        else:
            if item.isInInventory and not item.isRented:
                showInventoryMsg(b'already_exists', item, msgType=SystemMessages.SM_TYPE.Warning)
            else:
                price = getGUIPrice(item, self._itemsCache.items.stats.money, self._itemsCache.items.shop.defaults.exchangeRate)
                if price is None:
                    showShopMsg(b'not_found', item)
                    return
...
class _UnlockItem(CachedItemAction):
    ...
    def __init__(self, itemCD, unlockProps):
        ...
        g_clientUpdateManager.addCallbacks({b'serverSettings.blueprints_config.isEnabled': (self.__onBlueprintsModeChanged), 
           b'serverSettings.blueprints_config.useBlueprintsForUnlock': (self.__onBlueprintsModeChanged)})
        return
    ...
    def _showResult(self, result):
        if result.success:
            self.__costCtx[b'vehXP'] = backport.getIntegralFormat(self.__costCtx[b'vehXP'])
            self.__costCtx[b'freeXP'] = backport.getIntegralFormat(self.__costCtx[b'freeXP'])
            showUnlockMsg(b'unlock_success', self._item, msgType=SystemMessages.SM_TYPE.PowerLevel, **self.__costCtx)
        elif result.userMsg:
...
class UnlockItemAction(_UnlockItem):
    ...
    @decorators.adisp_process(b'research')
...
class InstallItemAction(BuyAction):
    ...
    def doAction(self):
        if RequestState.inProcess(b'install'):
            SystemMessages.pushI18nMessage(b'#system_messages:inventory/item/equip_in_processing', type=SystemMessages.SM_TYPE.Warning)
        self.installItem(self._itemCD, self._rootCD, b'install')
        return
    ...
    def installItem(self, itemCD, rootCD, state):
        ...
        if itemTypeID not in GUI_ITEM_TYPE.VEHICLE_MODULES:
            raise SoftException((b'Specified type (itemTypeID={}) is not type of module').format(itemTypeID))
        vehicle = self._itemsCache.items.getItemByCD(rootCD)
        if not vehicle.isInInventory:
            raise SoftException((b'Vehicle (intCD={}) must be in inventory').format(rootCD))
...
class BuyAndInstallItemAction(InstallItemAction):
    ...
    @adisp_process
    def doAction(self):
        if RequestState.inProcess(b'buyAndInstall'):
            SystemMessages.pushI18nMessage(b'#system_messages:shop/item/buy_and_equip_in_processing', type=SystemMessages.SM_TYPE.Warning)
        yield self.buyAndInstallItem(self._itemCD, self._rootCD, b'buyAndInstall')
        return
...
class BuyAndInstallWithOptionalSellItemAction(BuyAndInstallItemAction):
    AUTO_SELL_KEY = b'sellPreviousModule'
    ...
    def doAction(self):
        ...
        isSuccess, data = yield self.buyAndInstallItem(self._itemCD, self._rootCD, b'buyAndInstall')
        _logger.debug(b'buyAndInstallAction was a success: %s data: %s', isSuccess, data)
    ...
    def sellModule(self, moduleToSellCD):
        ...
        _logger.debug(b'sellModule module %s isInInventory=%s itemsCache synced %s', moduleToSell.userName, moduleToSell.isInInventory, self._itemsCache.isSynced())
...
class AmmunitionBuyAndInstall(AsyncGUIItemAction):
    __slots__ = (b'_vehicle', b'_changedItems', b'__confirmOnlyExchange')
...
class BuyAndInstallShells(AsyncGUIItemAction):
    __slots__ = (b'__vehicle', b'__price', b'__confirmOnlyExchange')
    __itemsCache = dependency.descriptor(IItemsCache)
...
class BuyVehicleSlotAction(CachedItemAction):
    ...
    @decorators.adisp_process(b'buySlot')
    def doAction(self):
...
class BuyBerthsAction(CachedItemAction):
    ...
    @decorators.adisp_process(b'buyBerths')
    def doAction(self):
...
class BuyAndActivateBooster(ActivateBoosterAction):
    ...
    def doAction(self):
        ...
        if mustBuy:
            ...
            if not canPurchase:
                ...
                _logger.info(b'[BuyAndActivateBooster] booster can not be bought, reason %s', reason)
                return
        if self.isReplacement():
    ...
    def waitForClientUpdate(self, callback):
        _logger.debug(b'[BuyAndActivate] waiting for cache update')
        clientUpdated = True
        ...
        try:
            try:
                ...
                _logger.warning(b'TimeoutError: while waiting for client update after buying booster. Timeout period %d', BUY_AND_ACTIVATE_TIMEOUT)
                clientUpdated = False
...
class ConvertBlueprintFragmentAction(IGUIItemAction):
    ...
    @decorators.adisp_process(b'blueprints/convertFragments')
    def doAction(self):
...
class TmanAddSkillsAction(CachedItemAction):
    ...
    @decorators.adisp_process(b'studying')
    def doAction(self):
...
class FillAllTankmenSkillsAction(IGUIItemAction):
    ...
    @decorators.adisp_process(b'fillingAllTankmenSkills')
    def doAction(self):
...
class ResetAllTankmenSkillsAction(IGUIItemAction):
    ...
    @decorators.adisp_process(b'resettingAllTankmenSkills')
    def doAction(self):
...
class UseCrewBookAction(GroupedItemAction):
    ...
    @decorators.adisp_process(b'crewBooks/useCrewBook')
    def doAction(self):
...
class UseFreeXpToTankman(GroupedItemAction):
    ...
    @decorators.adisp_process(b'updatingSkillWindow')
    def doAction(self):
    ...
    def _pushGroupedMessages(result):
        if result:
            if result.success:
                msg = result.userMsg
                if result.auxData:
                    msg += b' ' + (b', ').join(m.userMsg for m in result.auxData if m)
...
class TankmanRetrainingAction(GroupedItemAction):
    ...
    @decorators.adisp_process(b'retraining')
    def doAction(self):
...
class TankmanChangeRoleAction(GroupedItemAction):
    ...
    @decorators.adisp_process(b'changingRole')
    def doAction(self):
...
class UpgradeOptDeviceAction(AsyncGUIItemAction):
    ...
    @adisp_async
    @decorators.adisp_process(b'moduleUpgrade')
    def _action(self, callback):
    ...
    def _confirm(self, callback):
        ...
        if result and data and data.get(b'needMoreCurrency', False):
...
class DeconstructOptDevice(AsyncGUIItemAction):
    ...
    @adisp_async
    @decorators.adisp_process(b'storage/forDeconstruct')
    def _action(self, callback):
...
class InstallBattleAbilities(AsyncGUIItemAction):
    __slots__ = (b'__vehicle', b'__classVehs', b'__setupItems')
...
class FrontlineInstallReserves(AsyncGUIItemAction):
    __slots__ = (b'__vehicle', b'__applyForAllOfType', b'__skillsInteractor')
    ...
    def _confirm(self, callback):
        if not self.__skillsInteractor:
            ...
            self.__applyForAllOfType = data.get(b'applyForAllOfType', False)
            callback(isOK)
        return


class BuyBattleAbilities(AsyncGUIItemAction):
    __slots__ = (b'__skillIDs',)
...
class RemoveOptionalDevice(AsyncGUIItemAction):
    __slots__ = (b'__vehicle', b'__device', b'__slotID', b'__destroy', b'__forFitting', b'__everywhere', b'__removeProcessor')
    ...
    def _confirm(self, callback):
        ...
        self.__removeProcessor.requestCtx[b'useDemountKit'] = data.get(b'useDemountKit', False)
        callback(isOk)
        return
...
class ChangeSetupEquipmentsIndex(AsyncGUIItemAction):
    __slots__ = (b'__vehicle', b'__groupID', b'__layoutIdx')
...
class SwitchPrebattleAmmoPanelAvailabilityAction(AsyncGUIItemAction):
    __slots__ = (b'__vehicle', b'__groupID', b'__enabled')
...
class DiscardPostProgressionPairs(AsyncGUIItemAction):
    __slots__ = (b'__vehicle', b'__stepIDs', b'__modIDs')
...
class SetEquipmentSlotType(AsyncGUIItemAction):
    ...
    @adisp_async
    @decorators.adisp_process(b'loadContent')
    def _action(self, callback):
    ...
    def _confirm(self, callback):
        ...
        if result.busy:
            ...
            _logger.debug(b'Select slot specialization dialog result: isOk: %r, idx: %r', isOk, idx)
            if isOk and idx > 0:
                self.__slotID = idx
                callback(True)
            else:
