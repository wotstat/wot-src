...
ICONS_MASK = b'../maps/icons/%(type)s/%(subtype)s%(unicName)s.png'
_RentalInfoProvider = namedtuple(b'RentalInfoProvider', (b'rentExpiryTime', b'compensations', b'battlesLeft', b'winsLeft', b'seasonRent', b'isRented', b'isTelecomRent', b'anyExpires', b'isExternalRent'))
SeasonRentInfo = namedtuple(b'SeasonRentInfo', (b'seasonType', b'seasonID', b'duration', b'expiryTime'))
...
class RentalInfoProvider(_RentalInfoProvider):
    ...
    def __new__(cls, additionalData=None, time=0, battles=0, wins=0, seasonRent=None, isRented=False, *args, **kwargs):
        ...
        if b'compensation' in additionalData:
            compensations = Money.makeFromMoneyTuple(additionalData[b'compensation'])
        else:
            compensations = MONEY_UNDEFINED
        isTelecomRent = TELECOM_RENTALS_RENT_KEY in additionalData
        useAnyExpiresRule = additionalData.get(b'anyExpires', False)
        isExternalRent = b'external_rent_entitlement' in additionalData
    ...
    def getTimeLeft(self):
        if self.rentExpiryTime != float(b'inf'):
    ...
    def getExpiryState(self):
        if self.anyExpires:
            return self.rentExpiryTime != float(b'inf') and self.getTimeLeft() <= 0 or (self.battlesLeft <= 0 or (self.winsLeft <= 0 or (not self.getActiveSeasonRent() if self.seasonRent != {} else False)))
        return self.rentExpiryTime != float(b'inf') and self.battlesLeft <= 0 and self.winsLeft <= 0 and self.getTimeLeft() <= 0 and not self.getActiveSeasonRent()
...
class FittingItem(GUIItem):
    __slots__ = (b'_buyPrices', b'_sellPrices', b'_mayConsumeWalletResources', b'_isHidden', b'_inventoryCount', b'_isUnlocked', b'_isBoughtForAltPrice', b'_rentInfo', b'_restoreInfo', b'_fullyConfigured', b'_isInitiallyUnlocked', b'_descriptor')
    ...
    def __repr__(self):
        return b'%s<intCD:%d, type:%s, nation:%d>' % (
    ...
    @property
    def userType(self):
        return getTypeInfoByName(self.itemTypeName)[b'userString']
    ...
    def longUserName(self):
        return self.userType + b' ' + self.userName
    ...
    @property
    def icon(self):
        return ICONS_MASK % {b'type': (self.itemTypeName), 
           b'subtype': b'', 
           b'unicName': (self.name.replace(b':', b'-'))}
    ...
    def _getShortInfo(self, vehicle=None, expanded=False):
        try:
            description = i18n.makeString(self._getShortInfoKey() + (b'Full' if expanded else b''))
    ...
    def _getShortInfoKey(self):
        return b'#menu:descriptions/' + self.itemTypeName
