...
class Target(Enum):
    DEFAULT = b'default'
    SKILLS_TRAINING = b'skillsTraining'
...
class Tankman(GUIItem):
    ...
    __slots__ = (b'__descriptor', b'_invID', b'_nationID', b'_itemTypeID', b'_itemTypeName', b'_combinedRoles', b'_dismissedAt', b'_isDismissed', b'_vehicleNativeDescr', b'_vehicleInvID', b'_vehicleDescr', b'_vehicleBonuses', b'_vehicleSlotIdx', b'_skills', b'_skillsMap', b'_bonusSkills', b'_skinID', b'_comparator', b'__brotherhoodMarkedAsActive')
    ...
    def __repr__(self):
        return b'Tankman<id:%d, nation:%d, vehicleID:%d>' % (self.invID, self.nationID, self.vehicleInvID)

    def _buildSkills(self, proxy):
        return [getTankmanSkill(skill, self.role, self, proxy) for skill in self.descriptor.skills if skill != b'any']
    ...
    def buildBonusSkills(self, proxy, bonusSkillsLevels=None, bonusRoles=None):
        ...
        if bonusRoles is None:
            ...
            for role, skillName in self.descriptor.getRoleBonusSkills(bonusRoles):
                ...
                skill = getTankmanSkill(skillName, role, self, proxy, level=level) if skillName != b'any' else None
    ...
    def getPossibleSkillsByRole(self, target=Target.DEFAULT):
        ...
        for skill in tankmen.COMMON_SKILLS_ORDERED:
            result.setdefault(b'common', []).append(getTankmanSkill(skill, self.role, tankman=self))
...
class BaseBookConvertingFormatter(object):
    ...
    def getTextMessage(self, header, qtyPrefix=b''):
        formatedDate = str(time.strftime(b'%d.%m.%Y %H:%M:%S', time.localtime(time.time())))
        message = backport.text(header, at=formatedDate)
        message += (b',\n').join((b'{} ({}{})').format(crewBook[b'name'], qtyPrefix, crewBook[b'count']) for crewBook in self.__crewBooks) + b'.'
        return message
...
def getBigIconPath(nationID, iconID):
    return __getIconPath(nationID, iconID, b'big')


def getSpecialIconPath(nationID, iconID):
    return __getIconPath(nationID, iconID, b'special')


def getBarracksIconPath(nationID, iconID):
    return __getIconPath(nationID, iconID, b'barracks')
