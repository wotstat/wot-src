...
SINGLE_SKILL_ON_VEHICLE = b'singleOnVehicle'
BROTHERHOOD_SKILL_NAME = b'brotherhood'
REPAIR_SKILL_NAME = b'repair'
NO_TANKMAN = -1
NO_SLOT = -1
MAX_ROLE_LEVEL = 100
MAX_SAME_SKILL = 2
RRLBonuses = namedtuple(b'RRLBonuses', b'commBonus, brothersBonus, eqsBonus, optDevsBonus, penalty')

class RealRoleLevel(namedtuple(b'RealRoleLevel', b'lvl_, bonuses_')):
    __slots__ = ()
...
class Tankman(GUIItem):
    ...
    __slots__ = (b'__descriptor', b'_invID', b'_nationID', b'_itemTypeID', b'_itemTypeName', b'_combinedRoles', b'_dismissedAt', b'_isDismissed', b'_vehicleNativeDescr', b'_vehicleInvID', b'_vehicleDescr', b'_vehicleBonuses', b'_vehicleSlotIdx', b'_skills', b'_skillsMap', b'_skinID', b'_comparator', b'__brotherhoodMarkedAsActive')
    ...
    def _buildSkills(self, proxy):
        return [getTankmanSkill(skill, self, proxy) for skill in self.descriptor.skills if skill != b'any']
    ...
    def chosenFreeSkillsCount(self):
        ...
        return tdescr.freeSkillsNumber - tdescr.freeSkills.count(b'any')
    ...
    def crewLevelIncrease(self):
        commBonus = self.vehicleBonuses.get(b'commander', 0)
        if self.descriptor.role == self.ROLES.COMMANDER:
            commBonus = 0
        brothersBonus = self.vehicleBonuses.get(b'brotherhood', 0)
        eqsBonus = self.vehicleBonuses.get(b'equipment', 0)
        optDevsBonus = self.vehicleBonuses.get(b'optDevices', 0)
    ...
    def extensionLessIconRank(self):
        return self.iconRank.split(b'.png')[0]
    ...
    def newFreeSkillsCount(self):
        return self.descriptor.freeSkills.count(b'any')
    ...
    def getSkillsToLearn(self):
        ...
        result.append({b'id': b'common', b'skills': commonSkills})
        for role in TANKMEN_ROLES_ORDER_DICT[b'plain']:
            ...
            result.append({b'id': role, b'skills': skills})
    ...
    def allSkillsLearned(self):
        allowedGroups = [
         b'common'] + list(self.combinedRoles)
    ...
    def getPossibleSkillsByRole(self):
        ...
        for skill in tankmen.COMMON_SKILLS_ORDERED:
            result.setdefault(b'common', []).append(getTankmanSkill(skill, tankman=self))
    ...
    def getFreeSkillsToLearn(self):
        ...
        result.append({b'id': (self.role), b'skills': skills})
        return result

    def hasSkillToLearn(self):
        for skillsData in self.getSkillsToLearn():
            if skillsData[b'skills']:
                return True
    ...
    def roles(self):
        if self.isInTank:
            return self.vehicleDescr.type.crewRoles[self.vehicleSlotIdx]
        return self.rolesInNativeVehicle()
    ...
    def __packSkill(self, skillItem):
        return {b'id': (skillItem.name), 
           b'iconName': (skillItem.extensionLessIconName), 
           b'name': (skillItem.userName), 
           b'desc': (skillItem.shortDescription), 
           b'enabled': True, 
           b'tankmanID': (self.invID), 
           b'isSituational': (skillItem.isSituational)}
    ...
    def __repr__(self):
        return b'Tankman<id:%d, nation:%d, vehicleID:%d>' % (
         self.invID, self.nationID, self.vehicleInvID)


class TankmanSkill(GUIItem):
    __slots__ = (b'_name', b'_level', b'_roleType', b'_isEnable', b'_isFemale', b'_isPermanent', b'_customName', b'_isAlreadyEarned', b'_packer', b'_typeName', b'_tags')
    _CUSTOM_NAME_EXT = b''
    ...
    def __init__(self, skillName, tankman=None, proxy=None, skillLevel=0):
        ...
        if self._CUSTOM_NAME_EXT:
            customName = (b'_').join((self._CUSTOM_NAME_EXT, BROTHERHOOD_SKILL_NAME))
    ...
    def __repr__(self):
        return b'TankmanSkill<name:%s, level:%d, isEnable:%s>' % (self.name, self.level, self.isEnable)
    ...
    def userName(self):
        if self._customName:
            resStr = (b'_').join((self._name, self._CUSTOM_NAME_EXT))
    ...
    def currentLvlDescription(self):
        if self.isPermanent:
            return getSkillCurrentLvlDescription(self.name) + backport.text(R.strings.item_types.tankman.skills.permanent_descr())
    ...
    def icon(self):
        if self._name == SKILLS_CONSTANTS.TYPE_NEW_SKILL:
            iconName = (b'{}.png').format(self.name)
        else:
            iconName = getSkillIconName(self.name)
        if self._customName:
            iconName = (b'{}_{}').format(self._CUSTOM_NAME_EXT, iconName)
        return iconName

    @property
    def extensionLessIconName(self):
        return self.icon[:-len(b'.png')]
    ...
    def bigIconPath(self):
        ...
        LOG_WARNING((b'no {} image in gui.maps.icons.tankmen.skills.big').format(self.icon))
        return EMPTY_STRING
    ...
    def smallIconPath(self):
        ...
        if root.isValid():
            return backport.image(root())
        LOG_WARNING((b'no {} image in gui.maps.icons.tankmen.skills.small').format(self.extensionLessIconName))
        return EMPTY_STRING
    ...
    def getMaxLvlDescription(self):
        ...
        keyArgs = skillParams.get(b'keyArgs', {})
...
def getFullUserName(nationID, firstNameID, lastNameID):
    ...
    return (firstUserName + b' ' + lastUserName).strip()
...
def getRoleBigIconPath(role):
    return b'../maps/icons/tankmen/roles/big/%s' % getRoleIconName(role)


def getRoleMediumIconPath(role):
    return b'../maps/icons/tankmen/roles/medium/%s' % getRoleIconName(role)


def getRoleSmallIconPath(role):
    return b'../maps/icons/tankmen/roles/small/%s' % getRoleIconName(role)


def getRoleWhiteIconPath(role):
    return (b'../maps/icons/tankmen/roles/white/{}').format(getRoleIconName(role))
...
def getDynIconName(iconName):
    return iconName.replace(b'-', b'_').rsplit(b'.', 1)[0]
...
def getBarracksIconPath(nationID, iconID):
    ...
    return backport.image(R.images.gui.maps.icons.tankmen.icons.barracks.tankman())
...
def getSmallIconPath(nationID, iconID):
    ...
    if dynAccessor.isValid():
        return backport.image(dynAccessor())
    return backport.image(R.images.gui.maps.icons.tankmen.icons.small.tankman())
...
def getRankBigIconPath(nationID, rankID):
    return b'../maps/icons/tankmen/ranks/big/%s' % getRankIconName(nationID, rankID)


def getRankSmallIconPath(nationID, rankID):
    return b'../maps/icons/tankmen/ranks/small/%s' % getRankIconName(nationID, rankID)
...
def getExtensionLessSkillIconName(skillName):
    return tankmen.getSkillsConfig().getSkill(skillName).icon[:-len(b'.png')]
...
def getSkillBigIconPath(skillName):
    ...
    if root.isValid():
        return backport.image(root())
    LOG_WARNING((b'no {} image in gui.maps.icons.tankmen.skills.big').format(getExtensionLessSkillIconName(skillName)))
    return EMPTY_STRING
...
def getSkillSmallIconPath(skillName):
    ...
    if root.isValid():
        return backport.image(root())
    LOG_WARNING((b'no {} image in gui.maps.icons.tankmen.skills.small').format(getExtensionLessSkillIconName(skillName)))
    return EMPTY_STRING


def getSkillIconPath(skillName, size=b'big'):
    sizeRoot = R.images.gui.maps.icons.tankmen.skills.dyn(size)
    if not sizeRoot.isValid():
        LOG_WARNING((b'no {} size in gui.maps.icons.tankmen.skills').format(size))
        return EMPTY_STRING
    icon = sizeRoot.dyn(getExtensionLessSkillIconName(skillName))
    if icon.isValid():
        return backport.image(icon())
    LOG_WARNING((b'no {} image in gui.maps.icons.tankmen.skills.{} ').format(getExtensionLessSkillIconName(skillName), size))
    return EMPTY_STRING
...
def getCrewSkinIconSmallWithoutPath(iconID):
    ...
    return (b'/').join(fullPath.rsplit(b'/')[-2:])
...
def _getSkillLevelWithIncrease(booster, skillProgress, tankman):
    ...
    if booster is None:
        ...
        if skillProgress == tankmen.NO_SKILL:
            return tankmen.MAX_SKILL_LEVEL
        if specialitySkillLevel < tankmen.MAX_SKILL_LEVEL:
            return tankmen.MAX_SKILL_LEVEL + tankman.crewLevelIncrease[0]
        if booster.perkLevelMultiplier is None:
            return specialitySkillLevel + tankman.crewLevelIncrease[0]
        return (specialitySkillLevel + tankman.crewLevelIncrease[0]) * booster.perkLevelMultiplier
...
SKILL_LEVEL_PROCESSORS = {b'repair': __averageAllLevelProcessor, 
   b'camouflage': __averageAllLevelProcessor, 
   b'brotherhood': __averageAllLevelProcessor, 
   b'fireFighting': __averageAllLevelProcessor, 
   b'loader_intuition': __averageLevelProcessor}
