...
_DisplayPrice = namedtuple(b'PriceTuple', (b'type', b'cost'))
...
def getMinRentItemPrice(item):
    ...
    if minRentPricePackage:
        minRentPriceValue = minRentPricePackage[b'rentPrice']
        minRentDefPriceValue = minRentPricePackage[b'defaultRentPrice']
...
class ItemPrice(object):
    __slots__ = (b'__price', b'__defPrice')
    ...
    def __repr__(self):
        return (b'ItemPrice(price: {}, defPrice: {})').format(self.price, self.defPrice)
...
class ItemPrices(object):
    __slots__ = (b'__itemPrice', b'__itemAltPrice')
