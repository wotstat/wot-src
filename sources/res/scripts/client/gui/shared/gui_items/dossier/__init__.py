...
class _Dossier(GUIItem):
    __slots__ = (b'_dossier', b'_dossierType', b'_playerDBID')
...
class VehicleDossier(_Dossier, stats.VehicleDossierStats):
    ...
    def __repr__(self):
        return b'VehicleDossier<vehTypeCD=%d; playerDBID=%r; roaming=%r>' % (
...
class AccountDossier(_Dossier, stats.AccountDossierStats):
    ...
    def __repr__(self):
        return b'AccountDossier<playerDBID=%r; roaming=%r>' % (
...
class TankmanDossier(_Dossier, stats.TankmanDossierStats):
    ...
    def getStats(self, tankman):
        result = [
         {b'label': b'common', 
            b'stats': (
                     self.__packStat(b'battlesCount', self.getBattlesCount()),
                     self.__packStat(b'avgExperience', self.getAvgXP()))}]
    ...
    def _getStudyingBlock(self, tankman):
        if not tankman.hasSkillToLearn() and tankman.descriptor.lastSkillLevel >= tankmen.MAX_SKILL_LEVEL:
            ...
            studyingBlock = {b'label': b'studying', 
               b'secondLabel': (i18n.makeString(MENU.CONTEXTMENU_PERSONALCASE_STATSBLOCKTITLE)), 
               b'isRowSeparator': True, 
               b'isPremium': (self.__currentVehicleIsPremium), 
               b'stats': [
                        self.__packStat(b'nextSkillXPLeft', tankman.getNextLevelXpCost(), imageType=imageType, image=image, isSecondActive=self.__currentVehicleIsPremium),
                        self.__packStat(b'nextSkillBattlesLeft', self.__getNextSkillBattlesLeft(tankman), usePremiumXpFactor=True, isSecondActive=self.__currentVehicleIsPremium)]}
            ...
            if showPassiveCrewXpInfo:
                statsBlock = studyingBlock.get(b'stats', [])
    ...
    def __getCurrentSkillIcon(self, tankman):
        if self.__isNewSkillReady(tankman):
            return (b'new_skill', b'new_skill.png')
        if self.tmanDescr.roleLevel != tankmen.MAX_SKILL_LEVEL or not self.tmanDescr.skills:
            return (b'role', b'%s.png' % self.tmanDescr.role)
        return (b'skill', tankman.skills[-1].icon)
    ...
    def __formatValueForUI(self, value):
        if value is None:
            return i18n.makeString(b'#menu:profile/stats/items/empty')
        else:
    ...
    def __packWotPlus(self, tankman):
        ...
        timeStr = time_utils.getTillTimeString(secUntilNextLevel, MENU.TIME_TIMEVALUE, removeLeadingZeros=True) if tankManIsEligible and secUntilNextLevel else b'--'
        return {b'name': b'timeUntilNextLevel', 
           b'value': timeStr, 
           b'secondValue': timeStr, 
           b'isLabelActive': (tankHasPassiveXp and tankManIsEligible), 
           b'isValueActive': (tankHasPassiveXp and tankManIsEligible and not self.__currentVehicleIsPremium), 
           b'isSecondValueActive': (tankHasPassiveXp and tankManIsEligible and self.__currentVehicleIsPremium)}
    ...
    def __packStat(self, name, value, imageType=None, image=None, usePremiumXpFactor=False, isSecondActive=False):
        ...
        return {b'name': name, 
           b'value': value, 
           b'secondValue': secondValue, 
           b'isValueActive': (not isSecondActive), 
           b'isSecondValueActive': isSecondActive, 
           b'imageType': imageType, 
           b'image': image}

    def __repr__(self):
        return b'TankmanDossier<stats.__totalStats=%r>' % self.__totalStats
...
class FortDossier(_Dossier, stats.FortDossierStats):
    ...
    def __repr__(self):
        return b'FortDossier<playerDBID=%r; roaming=%r>' % (
         self._playerDBID, self.isInRoaming())
...
class ClubMemberDossier(_Dossier, stats.ClubMemberDossierStats):
    ...
    def pack(self):
        return (ClubMemberDossier, self._dossier.makeCompactDescr(), self._clubDbID,
         self._playerDBID)
