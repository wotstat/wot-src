...
class LoyalServiceAchievement(DeprecatedAchievement):
    ...
    def __init__(self, name, block, dossier, value=None):
        ...
        if dossier is not None:
            self.__registrationDate = backport.getLongDateFormat(dossier.getDossierDescr()[b'total'][b'creationTime'])
        else:
    ...
    def getUserDescription(self):
        if self.__registrationDate:
            return i18n.makeString((b'#achievements:{}_descr').format(self._getActualName()), regDate=self.__registrationDate)
        return b''
