...
class MechEngineerAchievement(HasVehiclesList, NationSpecificAchievement):
    ...
    _MEDAL_NAME = b'mechanicEngineer'
    _LIST_NAME = b'vehiclesToResearch'
...
class VehicleCollectorAchievement(HasVehiclesList, NationSpecificAchievement):
    ...
    __slots__ = (b'__hasCollectibleVehicles',)
    _MEDAL_NAME = CollectorVehicleConsts.COLLECTOR_MEDAL_PREFIX
    _NATIONAL_VEHICLES = b'collectorVehiclesByNations'
    _LIST_NAME = b'vehiclesToBuy'
...
class TankExpertAchievement(HasVehiclesList, NationSpecificAchievement):
    ...
    def __init__(self, nationID, block, dossier, value=None):
        self.__vehTypeCompDescrs = self._parseVehiclesDescrsList(NationSpecificAchievement.makeFullName(b'tankExpert', nationID), dossier)
        NationSpecificAchievement.__init__(self, b'tankExpert', nationID, block, dossier, value)
        HasVehiclesList.__init__(self)
    ...
    def _parseVehiclesDescrsList(cls, name, dossier):
        if dossier is not None:
            return getTankExpertRequirements(dossier.getBlock(b'vehTypeFrags')).get(name, [])
        else:
            return []
