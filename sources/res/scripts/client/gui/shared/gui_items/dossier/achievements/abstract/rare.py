...
class RareAchievement(RegularAchievement):
    ...
    def requestImagePath(self):
        ...
        if imgPath is None:
            return b'R.images.gui.maps.icons.achievement.noImage'
        else:
            return imgPath
    ...
    def requestBigImagePath(self):
        ...
        if imgPath is None:
            return b'R.images.gui.maps.icons.achievement.big.noImage'
        else:
            return imgPath
    ...
    def getIcons(self):
        ...
        if memImgID is not None:
            icons[self.ICON_TYPE.IT_67X71] = b'img://%s' % str(memImgID)
        memBigImgID = self.requestBigImageID()
        if memBigImgID is not None:
            icons[self.ICON_TYPE.IT_180X180] = b'img://%s' % str(memBigImgID)
        return icons

    def _getIconName(self):
        return b'actionUnknown'

    def __repr__(self):
        return b'%s<rareID=%s; value=%s>' % (self.__class__.__name__,
