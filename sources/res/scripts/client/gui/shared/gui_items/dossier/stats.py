...
class _VehiclesStatsBlock(_StatsBlockAbstract):
    _VehiclesDossiersCut = namedtuple(b'VehiclesDossiersCut', (b',').join([
     b'battlesCount',
     b'wins',
     b'xp']))
...
class VersusAIStatsBlock(_BattleStatsBlock, _Battle2StatsBlock, _MaxStatsBlock):
    ...
    def _getStatsBlock(self, dossier):
        return dossier.getDossierDescr()[b'mt_versusAI']

    def _getStats2Block(self, dossier):
        return dossier.getDossierDescr()[b'mt_versusAI']

    def _getStatsMaxBlock(self, dossier):
        return dossier.getDossierDescr()[b'mt_maxVersusAI']
...
class AccountVersusAIStatsBlock(VersusAIStatsBlock, _VehiclesStatsBlock, _MaxVehicleStatsBlock):
    ...
    def _getVehDossiersCut(self, dossier):
        return dossier.getDossierDescr()[b'mt_versusAICut']
...
class AccountDossierStats(_DossierStats):
    ...
    def getComp7Stats(self, archive=None, season=None):
        if archive:
            return AccountComp7StatsBlock(self._getDossierItem(), (b'Archive{}').format(archive))
        if season:
            return AccountComp7StatsBlock(self._getDossierItem(), (b'Season{}').format(season))
...
class VehicleDossierStats(_DossierStats):
    ...
    def getTotalStats(self):
        return TotalStatsBlock(self._getDossierItem(), (self.getRandomStats(), self.getClanStats(), self.getCompanyStats(),
         ...
         self.getComp7Stats(season=4), self.getVersusAIStats()))
    ...
    def getComp7Stats(self, archive=None, season=None):
        if archive:
            return Comp7StatsBlock(self._getDossierItem(), (b'Archive{}').format(archive))
        if season:
            return Comp7StatsBlock(self._getDossierItem(), (b'Season{}').format(season))
        _logger.warning(b'comp7 season or archive number must be specified!')
        return
