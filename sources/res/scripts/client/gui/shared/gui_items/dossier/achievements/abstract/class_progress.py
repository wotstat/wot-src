...
class ClassProgressAchievement(SimpleProgressAchievement):
    __slots__ = (b'_currentProgressValue',)
    MIN_LVL = 4
    NO_LVL = 5
    ...
    def getNotificationInfo(self):
        notificationKey = b'#achievements:%s_notification%d' % (self._getActualName(), self._value)
        if i18n.doesTextExist(notificationKey):
            return i18n.makeString(notificationKey)
        return b''

    def _getUserNameCtx(self):
        return {b'rank': (i18n.makeString(b'#achievements:achievement/rank%d' % (self._value or self.MIN_LVL)))}
