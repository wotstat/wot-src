...
class VehicleRenter(VehicleReceiveProcessor):
    ...
    def _request(self, callback):
        ...
        BigWorld.player().shop.buyVehicle(self.item.nationID, self.item.innationID, self.buyShell, self._getApplyingConsumables(), self.buyCrew, self.crewType, self.rentPackageID, (lambda code: self._response(code, callback, ctx={b'rentID': (self.rentPackageID)})))
        return
...
class VehicleSeller(ItemProcessor):
    ...
    def _successHandler(self, code, ctx=None):
        ...
        if sysMsgR:
            if self.isDismantlingForMoney:
                ...
                sysMsgR = sysMsgR.success.dyn(b'default' if gainCurrencies else b'zero_cost', R.invalid)
            else:
    ...
    def __splitInventory(self):
        ...
        for vehicle in self.nationGroupVehs:
            ...
            vehInv = viewvalues(self.itemsCache.items.getItems(itemTypeID=itemTypeIds, criteria=REQ_CRITERIA.VEHICLE.SUITABLE([
             vehicle], itemTypeIds) | REQ_CRITERIA.INVENTORY, nationID=vehicle.nationID))
...
class VehicleTmenXPAccelerator(VehicleSettingsProcessor):
    ...
    def _errorHandler(self, code, errStr=b'', ctx=None):
        return makeI18nError(sysMsgKey=(b'vehicle_tmenxp_accelerator/{}').format(errStr), defaultSysMsgKey=b'vehicle_tmenxp_accelerator/server_error', vehName=self.item.userName)

    def _successHandler(self, code, ctx=None):
        return makeI18nSuccess(sysMsgKey=b'vehicle_tmenxp_accelerator/success' + str(self._value), vehName=self.item.userName, type=SM_TYPE.Information)
...
class VehicleAutoReturnProcessor(VehicleSettingsProcessor):
    ...
    def _errorHandler(self, code, errStr=b'', ctx=None):
        return makeI18nError(sysMsgKey=b'vehicle_auto_return/server_error')

    def _successHandler(self, code, ctx=None):
        return makeI18nSuccess(sysMsgKey=b'vehicle_auto_return/success' + str(self._value), type=SM_TYPE.Information)
...
class OptDevicesInstaller(Processor):
    ...
    def _errorHandler(self, code, errStr=b'', ctx=None):
        if ctx is not None and isinstance(ctx, dict) and ctx.get(b'exception_message'):
            errStr = ctx.get(b'exception_message')
...
class EasyTankEquipApplyProcessor(Processor):
    ...
    def __init__(self, currentVehicle, copyVehicle, optDevicesData=None, shellsData=None, eqsData=None, styleData=None):
        ...
        self.__optDevicesLayout = optDevicesData.get(b'optDevices', [])
        self.__demountByDemountKit = optDevicesData.get(b'demountByDemountKit', [])
        self.__demountForFree = optDevicesData.get(b'demountForFree', [])
        self.__shellsLayout = shellsData.get(b'shells', [])
        self.__eqsLayout = eqsData.get(b'eqs', [])
        self.__styleData = styleData.get(b'styleData', {})
        buyingPrice = ZERO_MONEY
        for data in [optDevicesData, shellsData, eqsData, styleData]:
            buyingPrice += data.get(b'price', ZERO_MONEY)

        addPlugins = [
         proc_plugs.MoneyValidator(buyingPrice, byCurrencyError=False),
         proc_plugs.VehicleValidator(self.__currentVehicle, False, prop={b'isBroken': True, b'isLocked': True}),
    ...
    def _successHandler(self, code, ctx=None):
        messages = []
        if ctx:
            messages.extend(self.__successHandlerForShells(ctx.get(b'shells')))
            messages.extend(self.__successHandlerForEqs(ctx.get(b'eqs')))
            messages.extend(self.__successHandlerForDemountedOptDevices(ctx.get(b'demountedOptDevices')))
            messages.extend(self.__successHandlerForOptDevices(ctx.get(b'optDevices')))
            messages.extend(self.__successHandlerForStyle(ctx.get(b'style')))
        ...
        if style:
            for styleCtx in style:
                for itemId, cost in styleCtx:
                    ...
                    msgCtx = {b'itemType': styleItemType, 
                       b'itemName': (item.userName), 
                       b'count': (backport.getIntegralFormat(1)), 
                       b'money': (formatPrice(price, useStyle=True))}
