...
class TankmanDismiss(ItemProcessor):
    ...
    def _request(self, callback):
        _logger.debug(b'Make server request to dismiss tankman: %s', self.item)
...
class TankmanTokenRecruit(Processor):
    ...
    def _successHandler(self, code, ctx=None):
        html = makeHtmlString(path=b'html_templates:lobby/processors/system_messages', key=b'recruit', ctx={b'fullName': (self.recruitInfo.getFullUserName()), 
           ...
           b'role': (getRoleUserName(self.role)), 
...
class TankmanEquip(GroupedRequestProcessor):
    ...
    def __init__(self, tankmanInvID, vehicleInvID, vehicleSlotIdx, groupID=0, groupSize=1):
        ...
        self._sysMsgPrefix = b'equip_tankman'
        ...
        if tankman is not None and anotherTankman is not None and anotherTankman.invID != tankman.invID:
            self._sysMsgPrefix = b'reequip_tankman'
    ...
    def _errorHandler(self, code, errStr=b'', ctx=None):
        return makeI18nError(sysMsgKey=(b'{}/{}').format(self._sysMsgPrefix, errStr), defaultSysMsgKey=(b'{}/server_error').format(self._sysMsgPrefix), auxData=self._makeErrorData(errStr), type=SM_TYPE.NotEnoughBerthError if errStr == b'not_enough_space' else SM_TYPE.Error)

    def _successHandler(self, code, ctx=None):
        return makeI18nSuccess(sysMsgKey=(b'{}/success').format(self._sysMsgPrefix), vehName=self.itemsCache.items.getVehicle(self._vehicleInvID).userName, auxData=self._makeSuccessData(ctx))
...
class QuickServiceTankmanEquip(TankmanEquip):
    ...
    def _successHandler(self, code, ctx=None):
        ...
        for act in ctx:
            if act.itemCount == 1:
                additionalMessages.append(makeI18nSuccess(sysMsgKey=(b'{}/success').format(self._sysMsgPrefix), vehName=self.itemsCache.items.getVehicle(self._vehicleInvID).userName, auxData=self._makeSuccessData(ctx)))

        return makeI18nSuccess(sysMsgKey=(b'{}/success').format(self._sysMsgPrefix), vehName=self.itemsCache.items.getVehicle(self._vehicleInvID).userName, auxData=additionalMessages[1:])
...
class CrewSkinUnequip(CrewSkinsProcessorBase):
    ...
    def _request(self, callback):
        _logger.debug(b'Make server request to equip crewSkin: %d', self._tmanInvID)
...
class CrewSkinEquip(CrewSkinsProcessorBase):
    ...
    def _request(self, callback):
        _logger.debug(b'Make server request to equip crewSkin : %d, %d', self._tmanInvID, self.__skinID)
...
class TankmanUnload(GroupedRequestProcessor):
    ...
    def _errorHandler(self, code, errStr=b'', ctx=None):
        return makeI18nError(sysMsgKey=(b'{}/{}').format(self.__sysMsgPrefix(ctx), errStr), defaultSysMsgKey=(b'{}/server_error').format(self.__sysMsgPrefix(ctx)), auxData=self._makeErrorData(errStr), type=SM_TYPE.NotEnoughBerthError if errStr == b'not_enough_space' else SM_TYPE.Error)

    def _successHandler(self, code, ctx=None):
        return makeI18nSuccess(sysMsgKey=(b'{}/success').format(self.__sysMsgPrefix(ctx)), auxData=self._makeSuccessData(ctx))

    @staticmethod
    def __sysMsgPrefix(ctx):
        if TankmanUnload.__tmanQuantity(ctx) > 1:
            return b'unload_crew'
        return b'unload_tankman'
...
class TankmanReturn(Processor):
    ...
    def _successHandler(self, code, ctx=None):
        return makeI18nSuccess(sysMsgKey=b'return_crew/success', type=SM_TYPE.Information)

    def _errorHandler(self, code, errStr=b'', ctx=None, **kwargs):
        if errStr == b'return_unavailable':
            kwargs.update({b'msgPriority': (NotificationPriorityLevel.MEDIUM), 
               b'type': (SM_TYPE.Information)})
        return makeI18nError(sysMsgKey=(b'return_crew/{}').format(errStr), defaultSysMsgKey=b'return_crew/server_error', **kwargs)
...
class ResetAllTankmenSkills(Processor):
    __PREFIX = b'reset_all_tankmen_skills'
    ...
    def _request(self, callback):
        _logger.debug(b'Make server request to reset all tankmen skills')
...
class TankmanRetraining(GroupedRequestProcessor):
    ...
    def _successHandler(self, code, ctx=None):
        ...
        changeRoleMsg = backport.text(R.strings.system_messages.retraining_change_tankman_role.success()) + b'\n' if self.__isRoleChange else b''
        if amount:
...
class TankmanAddSkills(ItemProcessor):
    ...
    def _errorHandler(self, code, errStr=b'', ctx=None):
        return makeI18nError(sysMsgKey=(b'{}/{}').format(self.__getSysMsgKey(), errStr), defaultSysMsgKey=(b'{}/server_error').format(self.__getSysMsgKey()))

    def _successHandler(self, code, ctx=None):
        return makeI18nSuccess(sysMsgKey=(b'{}/success').format(self.__getSysMsgKey()), type=SM_TYPE.Information, skill=(b', ').join(getTankmanSkill(skillName, self.skillsRole, self.item).userName for skillName in self.skillNames))
    ...
    def __getSysMsgKey(self):
        if len(self.skillNames) == 1:
            return b'add_tankman_skill'
        return b'add_tankman_skills'
...
class TankmanAddSkill(ItemProcessor):
    ...
    def _successHandler(self, code, ctx=None):
        return makeI18nSuccess(sysMsgKey=b'add_tankman_skill/success', skill=getTankmanSkill(self.skillName, self.item.role, self.item).userName, type=SM_TYPE.Information)
...
class TankmanChangePassport(ItemProcessor):
    ...
    def _request(self, callback):
        _logger.debug(b'Make server request to change tankman passport: %s, %s, %s, %s, %s, %s, %s', self.item.invID, self.isPremium, self.isFemale, self.firstNameGroup, self.firstNameID, self.lastNameGroup, self.lastNameID)
...
class TankmenJunkConverter(Processor):
    ...
    def _errorHandler(self, code, errStr=b'', ctx=None):
        return makeI18nError(sysMsgKey=b'conversion/error', defaultSysMsgKey=b'restore_tankman/server_error')

    def _successHandler(self, code, ctx=None):
        count = ctx.get(b'count', 0) if ctx is not None else 0
        return makeI18nSuccess(sysMsgKey=b'conversion/success', type=SM_TYPE.Information, number=count)

    def _request(self, callback):
        _logger.debug(b'Make server request to convert junk tankmen ')
...
class TransferTankmanXP(Processor, BaseBookConvertingFormatter):
    ...
    def _request(self, callback):
        _logger.debug(b'Make server request to transfer xp from %s to %s', self.sourceID, self.targetID)
        BigWorld.player().inventory.transferTankmenXp(self.sourceID, self.targetID, (lambda code: self._response(code, callback)))
        return

    def _errorHandler(self, code, errStr=b'', ctx=None):
        return makeI18nError(sysMsgKey=b'transferXP/server_error')
