...
def makeI18nError(sysMsgKey=b'', defaultSysMsgKey=b'', auxData=None, *args, **kwargs):
    ...
    return makeError(i18n.makeString(localKey, *args, **kwargs), kwargs.get(b'type', SM_TYPE.Error), auxData, msgPriority=kwargs.get(b'msgPriority'))
...
class Processor(object):
    ...
    def _lockedVehicleHandler(self):
        return makeI18nError(sysMsgKey=b'locked_vehicle_error')
...
class GroupedRequestProcessor(Processor):
    ...
    def __init__(self, request, *args, **kwargs):
        ...
        self.__params = args
        self.__groupID = kwargs.get(b'groupID') or int(BigWorld.serverTime())
        self.__groupSize = kwargs.get(b'groupSize', 1)
        return

    def _request(self, callback):
        _logger.debug(b'Make server request %s: %s', self.__request.__name__, (b', ').join(str(i) for i in self.__params))
    ...
    def _lockedVehicleHandler(self):
        return makeI18nError(sysMsgKey=b'locked_vehicle_error', auxData=self._makeErrorData())
