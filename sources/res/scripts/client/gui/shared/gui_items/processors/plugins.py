...
class BarracksSlotsValidator(SyncValidator):
    ...
    def _validate(self):
        if 0 < self.berthsNeeded > self.itemsCache.items.freeTankmenBerthsCount():
            return makeError(b'not_enough_space')
        return makeSuccess()
...
class BCBuyAndInstallConfirmator(BuyAndInstallConfirmator):
    ...
    def getPath(itemTypeName):
        ...
        if itemTypeName in BCBuyAndInstallConfirmator._VEHICLE_COMPONENTS_LABLES:
            dataStr = (b'/').join((BCBuyAndInstallConfirmator._BOOTCAM_LABELS_PATH,
             BCBuyAndInstallConfirmator._VEHICLE_COMPONENTS_LABLES[itemTypeName]))
        return dataStr

    def _makeMeta(self):
        dialogData = {b'label': (backport.text(R.strings.bootcamp.message.confirmBuyAndInstall.module.title()).format(self.item.longUserName)), 
           b'labelExecute': (backport.text(R.strings.bootcamp.message.confirmBuyAndInstall.module.buttonLabel())), 
           b'icon': (BCBuyAndInstallConfirmator.getPath(self.item.itemTypeName)), 
           b'costValue': (self.ctx[b'price']), 
           b'isBuy': True}
...
class PMSelectConfirmator(CheckBoxConfirmator):
    ...
    def _makeMeta(self):
        return CheckBoxDialogMeta(b'questsConfirmPm3Dialog' if self.settingFieldName == b'questsConfirmDialogShowPM3' else b'questsConfirmDialog', messageCtx={b'newQuest': (self.quest.getUserName()), 
...
class BattleBoosterConfirmator(I18nMessageAbstractConfirmator):
    ...
    def _makeMeta(self):
        ...
        ctx = {b'devices': ((b', ').join(set([device.userName for device in optDevicesList])))}
...
class StallStateValidator(SyncValidator):
    ...
    def _validate(self):
        if not self.__lobbyContext.getServerSettings().stallConfig.isEnabled:
            return makeError(b'stall_state_disabled')
        return makeSuccess()
...
class StallPurchaseParamsValidator(SyncValidator):
    ...
    def _validate(self):
        if self.__count < 1:
            return makeError(b'stall_purchase_count_less_than_one')
        products = self.__lobbyContext.getServerSettings().stallConfig.products
        if self.__productCode not in products.iterkeys():
            return makeError(b'stall_invalid_product_code')
        purchasedCount = self.__itemsCache.items.tokens.getTokenCount((b'{}_no_log').format(self.__productCode))
        if purchasedCount >= products[self.__productCode][b'dayLimit']:
            return makeError(b'stall_daily_purchase_limit_reached')
        return makeSuccess()
