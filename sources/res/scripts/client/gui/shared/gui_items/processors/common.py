...
class TankmanBerthsBuyer(Processor):
    ...
    def _errorHandler(self, code, errStr=b'', ctx=None):
        return makeI18nError(sysMsgKey=(b'buy_tankmen_berths/{}').format(errStr), defaultSysMsgKey=b'buy_tankmen_berths/server_error')

    def _successHandler(self, code, ctx=None):
        return makeI18nSuccess(sysMsgKey=b'buy_tankmen_berths/success', money=formatPrice(self.berthsPrice, useStyle=True, justValue=True), type=SM_TYPE.FinancialTransactionWithGold)
...
class GoldToCreditsExchanger(Processor):
    ...
    def _errorHandler(self, code, errStr=b'', ctx=None):
        return makeI18nError(sysMsgKey=(b'exchange/{}').format(errStr), defaultSysMsgKey=b'exchange/server_error', gold=int(self.gold))
...
class FreeXPExchanger(Processor):
    ...
    def __makeConfirmator(self):
        ...
        return plugins.HtmlMessageConfirmator(b'exchangeXPConfirmation', b'html_templates:lobby/dialogs', b'confirmExchangeXP', extra, sourceKey=b'XP_EXCHANGE_FOR_GOLD')
...
class CustomizationsBuyer(Processor):
    ...
    def _getMsgCtx(self):
        ...
        return {b'itemType': (styleItemType if self.item.itemTypeID == GUI_ITEM_TYPE.STYLE else self.item.userType), 
           ...
           b'money': (formatPrice(self._getTotalPrice(), useStyle=True))}
...
class _MapsBlackListSelector(Processor):
    ...
    def _successHandler(self, code, ctx=None):
        ...
        if not isPremiumActive and not isWotPlusActive:
            return makeI18nSuccess(sysMsgKey=(b'{}/success/noSubscriptions').format(self._getMessagePrefix()))
        if isPremiumActive and not isWotPlusActive:
            return makeI18nSuccess(sysMsgKey=(b'{}/success/premium').format(self._getMessagePrefix()))
        if not isPremiumActive and isWotPlusActive:
            return makeI18nSuccess(sysMsgKey=(b'{}/success/wotPlus').format(self._getMessagePrefix()))
    ...
    def _request(self, callback):
        if self.__toAddMapID <= self._SKIPPED_MAP_ID_LOWER_BOUND and self.__toRemoveMapId <= self._SKIPPED_MAP_ID_LOWER_BOUND:
            _logger.warning(b'Invalid blacklist maps. To addID=%d, toRemoveID=%d', self.__toAddMapID, self.__toRemoveMapId)
            return
        _logger.debug(b'Make server request to update blacklist maps. To addID=%d, toRemoveID=%d', self.__toAddMapID, self.__toRemoveMapId)
...
class UseCrewBookProcessor(GroupedRequestProcessor):
    ...
    def _makeSuccessData(self, *args, **kwargs):
        itemsCache = dependency.instance(IItemsCache)
        items = kwargs.get(b'ctx', [])
        auxData = []
        for idx, item in enumerate(items):
            name = itemsCache.items.getItemByCD(item.itemID).userName
            if item.itemCount == 1:
                sysMsgKey = b'crewBooksNotification/bookUsed'
                if idx != len(items) - 1:
                    sysMsgKey = b'crewBooksNotification/bookUsedOneOf'
                auxData.append(makeI18nSuccess(sysMsgKey=sysMsgKey, name=name))
            else:
                auxData.append(makeI18nSuccess(sysMsgKey=b'crewBooksNotification/booksUsed', name=name, count=item.itemCount))
...
class ClaimRewardForPostProgression(Processor):
    ...
    def _successHandler(self, code, ctx=None):
        ...
        text = backport.text(R.strings.system_messages.post_progression.success(), book_name=crewBook.getName().strip(), amount=str(int(ctx.get(b'count'))), at=formatedDate)
        return makeI18nSuccess(sysMsgKey=text, type=SM_TYPE.InformationHeader, priority=NotificationPriorityLevel.MEDIUM, auxData={b'header': (backport.text(R.strings.system_messages.post_progression.success.title()))})
...
class FillingAllUntrainedPerksProcessor(Processor):
    ...
    def _request(self, callback):
        _logger.debug(b'Make server request to fill untrained perks for all crew members.')
