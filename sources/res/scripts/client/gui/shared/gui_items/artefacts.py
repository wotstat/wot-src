...
TAG_NOT_FOR_SALE = b'notForSale'
TAG_TRIGGER = b'trigger'
TAG_BUILTIN_PERK_BOOSTER = b'builtinPerkBooster'
TAG_CREW_BATTLE_BOOSTER = b'crewSkillBattleBooster'
...
TAG_OPT_DEVICE_HIDE_IF_NOT_IN_SHOP = b'hideIfNotInShop'
TOKEN_OPT_DEVICE_SIMPLE = b'simple'
TOKEN_OPT_DEVICE_DELUXE = b'deluxe'
TOKEN_CREW_PERK_REPLACE = b'perk'
TOKEN_CREW_PERK_BOOST = b'boost'
...
class Equipment(VehicleArtefact):
    ...
    def getBonusIcon(self, size=b'small'):
        ...
        if result is None:
            result = RES_ICONS.getBonusIcon(size, self.name.split(b'_')[0])
        return result
        ...
        for idx, eq in enumerate(vehicle.consumables.installed):
            ...
            if eq.intCD != self.intCD:
                ...
                if not installPossible:
                    return (False, b'not with installed equipment')
...
class BattleBooster(Equipment):
    ...
    def getBonusIcon(self, size=b'small'):
        return RES_ICONS.getBonusIcon(size, self.name.split(b'_')[0])
    ...
    def getCrewBoosterDescription(self, isPerkReplace, formatter=None):
        if not self.isCrewBooster():
            raise SoftException(b'This description is only for Crew Booster!')
        ...
        if formatter is None:
            formatted = description.format(action=action, skillOrPerk=skillOrPerk, name=skillName, colorTagOpen=b'', colorTagClose=b'')
    ...
    def getCrewBoosterAction(self, isPerkReplace):
        if not self.isCrewBooster():
            raise SoftException(b'This action description is only for Crew Booster!')
    ...
    def getOptDeviceBoosterDescription(self, vehicle, valueFormatter=None):
        if not self.isEquipmentBooster():
            raise SoftException(b'This description is only for Battle Booster!')
    ...
    def getEconomicDirectivesDescription(self):
        if not self.isEconomicBooster():
            raise SoftException(b'This action description is only for Economic directives!')
...
class BattleAbility(Equipment):
    __slots__ = (b'_level', b'_unlocked', b'__weakref__')
    ...
    def mayInstall(self, vehicle, slotIdx=None):
        ...
        if not slotCheck:
            return (False, b'slot index exceeds limit of vehicle class')
        return self.descriptor.checkCompatibilityWithVehicle(vehicle.descriptor)
...
class OptionalDevice(RemovableDevice):
    ...
    def shortDescription(self):
        ...
        return description.format(colorTagOpen=b'', colorTagClose=b'')
    ...
    def getOverlayIconName(self):
        ...
        return (b'{}_overlay').format((b'{}_{}').format(type, self.level) if self.isModernized else type)

    def getOverlayType(self, vehicle=None):
        if self.isDeluxe:
            return SLOT_HIGHLIGHT_TYPES.EQUIPMENT_PLUS
        if self.isModernized:
            return (b'{}_{}').format(SLOT_HIGHLIGHT_TYPES.EQUIPMENT_MODERNIZED, self.level)
        if self.isUpgradable:
    ...
    def getBigOverlayType(self, vehicle=None):
        if self.isModernized:
            return (b'{}_{}').format(SLOT_HIGHLIGHT_TYPES.EQUIPMENT_MODERNIZED_BIG, self.level)
...
def getAbilityVO(vehicle):
    ...
    if abilityId:
        ...
        return {b'abilityName': (g_cache.getEquipmentByID(abilityId).name), 
           b'tooltip': (TOOLTIPS_CONSTANTS.ABILITY_LOBBY_TOOLTIP), 
           b'tooltipArgs': [
