...
class BattleBooster(Equipment):
    ...
    def getOverlayType(self, vehicle=None):
        ...
        if self.isCrewBooster() and not isLearnt:
            return b'battleBoosterReplace'
        return b'battleBooster'
    ...
    def getOptDeviceBoosterDescription(self, vehicle, valueFormatter=None):
        if self.isCrewBooster():
            raise SoftException(b'This description is only for Opt. Dev. Booster!')
...
class OptionalDevice(RemovableDevice):
    ...
    def getBonusOverlay(self, size=b'small'):
    ...
    def mayRemove(self, vehicle):
        try:
            ...
            return (False, b'not installed on vehicle')
    ...
    def getOverlayIconName(self):
        return (b'{}_overlay').format(self.getOverlayType())
