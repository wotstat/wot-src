...
_CAMO_ICON_TEMPLATE = b'img://camouflage,{width},{height},{options},"{texture}","{background}",{colors},{weights}'
_CAMO_ICON_URL = b'camo://{texture}?{params}'
_CAMO_SWATCH_WIDTH = 128
_CAMO_SWATCH_HEIGHT = 128
_CAMO_SWATCH_BACKGROUND = b'gui/maps/vehicles/camouflages/camo_back.dds'
_PERSONAL_NUM_ICON_TEMPLATE = b'img://personal_num,{width},{height},"{texture}","{fontPath}","{number}","{textureMask}","{background}"'
_PERSONAL_NUM_ICON_URL = b'pnum://{texture}?{params}'
_PN_SWATCH_WIDTH = 228
_PN_SWATCH_HEIGHT = 104
_TEXTURES_PATH = b'gui/maps/vehicles/'
_PREVIEW_ICON_TEMPLATE = b'img://preview_icon,{width},{height},{innerWidth},{innerHeight},"{texture}"'
_PREVIEW_ICON_URL = b'preview://{texture}?{params}'
_PREVIEW_ICON_SIZE = (
 226, 102)
...
def getTexturePath(itemTypeName, size, name, extension):
    return _TEXTURES_PATH + itemTypeName + b's/' + size + b'/' + name + b'.' + extension
...
class SpecialEvents(object):
    ...
    NY22 = b'NY2022_style'
    NY23 = b'NY2023_style'
    FOOTBALL18 = b'football2018'
    WINTER_HUNT = b'winter_hunt'
    KURSK_BATTLE = b'Kursk_battle'
...
def camoIconTemplate(texture, width, height, colors, background=_CAMO_SWATCH_BACKGROUND, options=ImageOptions.NONE):
    ...
    return _CAMO_ICON_TEMPLATE.format(width=width, height=height, options=options, texture=texture, background=background, colors=(b',').join(str(color) for color in colors), weights=(b',').join(str(weight) for weight in weights))


def camoIconUrl(texture, width, height, colors, background=_CAMO_SWATCH_BACKGROUND, options=ImageOptions.NONE):
    weights = Math.Vector4(*[(color >> 24) / 255.0 for color in colors])
    params = {b'back': background, 
       b'w': width, 
       b'h': height, 
       b'r': (colors[0]), 
       b'g': (colors[1]), 
...
def personalNumIconTemplate(number, width, height, texture, fontPath, textureMask=b'', background=b''):
...
def personalNumIconUrl(number, width, height, texture, fontPath, textureMask=b'', background=b''):
    params = {b'num': number, 
       b'w': width, 
       b'h': height, 
       b'font': fontPath, 
...
class ConcealmentBonus(object):
    __slots__ = (b'_camouflageId', b'_season')
    ...
    def getFormattedValue(self, vehicle):
        return (b'{:.0%}').format(self.getValue(vehicle))
...
class Customization(FittingItem):
    __slots__ = (b'_boundVehicles', b'_bonus', b'_installedVehicles', b'__noveltyData', b'__progressingData', b'__installedCount', b'__boundInventoryCount', b'__fullInventoryCount', b'__fullCount', b'__questProgressInfo')
    ...
    @property
    def icon(self):
        return self.texture.replace(b'gui/', b'../', 1)
    ...
    def getBonusIcon(self, size=b'small'):
    ...
    def specialEventIcon(self):
        imageKey = SpecialEvents.ICONS.get(self.specialEventTag, b'')
        if imageKey != b'':
    ...
    def specialEventName(self):
        nameKey = SpecialEvents.NAMES.get(self.specialEventTag, b'')
        if nameKey != b'':
    ...
    def getQuestsProgressionInfo(self):
        if not self.isQuestsProgression:
            ...
            if self.intCD in customizationCache.itemToQuestProgressionStyle:
                ...
                _logger.error(b'Wrong itemToQuestProgressionStyle info for compCD "%s" ', self.intCD)
            self.__questProgressInfo = (b'', -1)
            return (b'', -1)
    ...
    def isUnlockingExpired(self):
        ...
        for quest in quests:
            ...
            if not questAvailability.isValid and questAvailability.reason != b'requirements':
                return True
    ...
    def getTextureByProgressionLevel(baseTexture, progressionLevel):
        ...
        basePart, delimiter, _ = name.rpartition(b'_')
        if not basePart:
            _logger.error(b'Wrong name of texture for progression customization item (texture name is "%s" )', baseTexture)
            return b''
        name = basePart + delimiter + str(progressionLevel)
        texture = path + b'/' + name + ext
        if not ResMgr.isFile(texture):
            _logger.error(b'Failed to get texture by progression level. Base texture %s; level: %s', baseTexture, progressionLevel)
            return b''
        return texture
    ...
    def iconByProgressionLevel(self, progressionLevel):
        if self.isProgressive:
            return self.getTextureByProgressionLevel(self.texture, progressionLevel).replace(b'gui/', b'../', 1)
        else:
            return
...
class ProjectionDecal(Decal):
    ...
    def __getPreviewIcon(icon):
        ...
        iconPath = (b'{}_preview.png').format(f)
...
class PersonalNumber(Customization):
    ...
    def getIconApplied(self, component):
        return self.descriptor.previewTexture.replace(b'gui/', b'../', 1)

    def isWide(self):
        return True

    def numberIcon(self, number=b''):
        if not number:
    ...
    @property
    def itemFullTypeName(self):
        return self.itemTypeName + b'_' + str(self.digitsCount)

    def numberIconUrl(self, number=b''):
        if not number:
            return self.iconUrl
...
class Attachment(Customization):
    ...
    @property
    def texture(self):
        return getTexturePath(self.itemTypeName, b's104x104', self.descriptor.name, b'png')

    @property
    def previewTexture(self):
        return getTexturePath(self.itemTypeName, b's232x174', self.descriptor.name, b'png')

    def getIconApplied(self, component):
        return self.previewTexture.replace(b'gui/', b'../', 1)
...
class StatTracker(Attachment):
    ...
    def getStatValue(self, vehicle):
        ...
        if self.trackedStatistic == StatTrackerStatistic.KILLS:
            ...
            _logger.error(b'Unknown stat tracker statistics type: %s', self.trackedStatistic)
        return self.descriptor.adjustToAllowedNumber(value)


class Style(Customization):
    __slots__ = (b'_changableTypes', b'__outfits', b'__dependenciesByIntCD', b'__serialNumber')
    ...
    def getAlternateItem(self, itemType, itemID):
        ...
        if itemsOfType is not None and itemID in itemsOfType:
            return self._service.getItemByCD(makeIntCompactDescrByID(b'customizationItem', itemType, itemID))
        else:
            return
    ...
    def getUpgradePrice(self, currentLvl, targetLvl):
        ...
        def _getLevelPrice(level):
            ...
            if levelDescr:
                price = Money(**levelDescr[b'price'])
    ...
    def getOutfit(self, season, vehicleCD=b'', diff=None):
        if diff:
        ...
        if self.__dependenciesByIntCD is None:
            ...
            for ancestorID, dependentData in iteritems(self.descriptor.dependencies):
                ...
                for iType, iIds in iteritems(dependentData):
                    dependentIntCDs.extend([makeCD(b'customizationItem', iType, iId) for iId in iIds])

                dependentIntCDs = tuple(dependentIntCDs)
                self.__dependenciesByIntCD[makeCD(b'customizationItem', CustomizationType.CAMOUFLAGE, ancestorID)] = dependentIntCDs
    ...
    def getAdditionalOutfit(self, level, season, vehicleCD):
        additionalOutfit = self.descriptor.styleProgressions.get(level, {}).get(b'additionalOutfit', {})
    ...
    def __createOutfit(self, season, vehicleCD=b'', diff=None):
