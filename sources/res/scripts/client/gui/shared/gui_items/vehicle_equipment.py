...
LAYOUT_ITEM_SIZE = 2
EMPTY_ITEM = None
SUPPORT_EXT_DATA_FEATURES = b'vehiclePostProgressionFeatures'

class _Equipment(object):
    __slots__ = (b'_storage', b'__guiItemType', b'__capacity', b'_proxy')
    ...
    def __init__(self, guiItemType, capacity, proxy, fromRawData, *args):
        ...
        if args:
            if len(args) != self.getCapacity():
                _logger.warning(b'Length of arguments is not valid, args: %s for equipment: %s', args, self)
    ...
    def __getitem__(self, slotIdx):
        if slotIdx >= len(self._storage):
            _logger.error(b'Wrong slotIdx=[%d] for Equipment=[%s]', slotIdx, self)
            return EMPTY_ITEM
        else:
    ...
    def __repr__(self):
        return (b'{}, guiItemType: {}, capacity: {}').format(self.__class__.__name__, GUI_ITEM_TYPE_NAMES[self.__guiItemType], self.__capacity)
    ...
    def _validateIndex(self, idx):
        if idx >= self.getCapacity():
            raise SoftException((b'Index {} exceeds the layout size!').format(idx))
        return

    def _validateType(self, item):
        if item is not None and item.itemTypeID != self.__guiItemType:
            raise SoftException((b'The item {} is not suitable for {}!').format(item, self))
        return
...
class _EquipmentCollector(object):
    __slots__ = (b'_installed', b'_layout', b'__slots', b'__setupLayouts', b'_proxy', b'_vehDescr')
...
class _ShellsCollector(_EquipmentCollector):
    ...
    def _parse(self, vehDescr, capacity, invData):
        ...
        for layoutIdx in range(self.setupLayouts.capacity):
            ...
            installedItems = self.__parseInstalled(invData.get(b'shells', []), shellsLayout, vehDescr, capacity)
...
OptDeviceSlotData = typing.NamedTuple(b'OptDeviceSlotInfo', ((b'item', SupplySlot), (b'isDynamic', bool)))

class _OptDevicesCollector(_EquipmentCollector):
    __slots__ = (b'_dynSlotTypeIdx', b'_dynSlotType', b'_dynSlotTypeOptions')
    ...
    def getSlot(self, slotIdx):
        if slotIdx < 0 or slotIdx >= len(self.slots):
            raise SoftException(b'Wrong slotIdx=[%r]' % slotIdx)
...
class _EquipmentsSetupGroups(object):
    __slots__ = (b'_groups', b'__capacity')
    ...
    def _parse(self, invData, postProgressionFeatures, vehDescr):
        groups = invData.get(b'layoutIndexes', {}).copy()
        capacity = {}
...
class _EquipmentSetupLayout(object):
    __slots__ = (b'__groupID', b'__layoutIdx', b'__capacity', b'__setups', b'__slotsCapacity')
...
class VehicleEquipment(object):
    __slots__ = (b'__consumables', b'__battleBoosters', b'__battleAbilities', b'__shells', b'__optDevices', b'__setupLayouts')
    ...
    def __deepcopy__(self, memo=None):
        raise SoftException(b'Deep copy of VehicleEquipment is not supported')
        return
