...
def installModulesSet(vehicle, modules, notFitted):
    ...
    if modules:
        while modules:
            ...
            elif notFitReason == b'need turret':
...
def installEquipment(vehicle, intCD, slotIndex, itemsCache=None):
    ...
    if equipment:
        ...
        if not success:
            LOG_WARNING((b'Equipment could not been installed, reason: {}').format(reason))
            return
...
def installOptionalDevice(vehicle, newId, slotIndex, itemsCache=None):
    ...
    if mayInstall:
        ...
        LOG_WARNING((b'Component "{}" has not been installed. Reason: {}.').format(itemTypeID, reason))
...
def removeOptionalDevice(vehicle, slotIndex):
    ...
    if installedDevice is not None:
        ...
        LOG_WARNING((b"Couldn't remove optional device from slot {} because slot is already empty!").format(slotIndex))
        return (
         False, b'slot is empty')
...
def swapOptionalDevice(vehicle, leftID, rigthID):
    ...
    if maySwap:
        ...
        LOG_WARNING((b'Slots "{}" has not been swapped. Reason: {}.').format((leftID, rigthID), reason))
    return (
     maySwap, reason)
