...
class CachedBlur(object):
    __slots__ = (b'_blurId', b'_enabled', b'_fadeTime', b'_rectangles', b'_ownLayer', b'_blurAnimRepeatCount', b'__weakref__', b'_prevBlurRadius')
    ...
    def addRect(self, blurRect):
        ...
        if rectId not in self._rectangles:
            ...
            _logger.error(b'There is already added rectangular with the same rect id %d', rectId)
...
class _BlurManager(object):
    ...
    def _isBlurInCache(self, blur):
        if weakref.ref(blur) in self._cache:
            return True
        logging.error(b"Blur instance with id %s isn't found in cache (not registered or already deleted)", blur.blurId)
        return False
