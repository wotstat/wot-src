...
class BatchUsersInfoHelper(UsersInfoHelper):
    ...
    def getUserName(self, userID, scope=UserEntityScope.LOBBY, withEmptyName=False):
        user = self.getContact(userID, scope=scope)
        if not user.hasValidName():
            if userID > 0 and userID not in self._invalid[b'names']:
                self._invalid[b'names'].append(userID)
    ...
    def getUserRating(self, userDbID):
        user = self.getContact(userDbID)
        if not user.hasValidRating():
            if userDbID > 0 and userDbID not in self._invalid[b'ratings']:
                self._invalid[b'ratings'].append(userDbID)
        return user.getGlobalRating()

    def syncUsersInfo(self):
        if self._invalid[b'names']:
            self._rqCtrl.requestNicknames(self._invalid[b'names'][:self._MAX_CHUNK_SIZE], (lambda names, _: self.onUserNamesReceived(names)))
            del self._invalid[b'names'][:self._MAX_CHUNK_SIZE]
        if self._invalid[b'ratings']:
            self._rqCtrl.requestGlobalRatings(list(self._invalid[b'ratings']), self.onUserRatingsReceived)
            self._invalid[b'ratings'] = []
        return
    ...
    def hasInvalidName(self):
        return bool(self._invalid[b'names'])
