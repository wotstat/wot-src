...
def showFrontlineInfoWindow(autoscrollSection=b''):
...
def showConfigurableVehiclePreview(vehTypeCompDescr, previewAlias=VIEW_ALIAS.LOBBY_HANGAR, hiddenBlocks=None, itemPack=None, **kwargs):
    ...
    kwargs.update({b'itemCD': vehTypeCompDescr, 
       ...
       b'itemsPack': itemPack})
    ConfigurableVehiclePreviewState.goTo(**kwargs)
    return


def showVehiclePreview(vehTypeCompDescr, previewAlias=VIEW_ALIAS.LOBBY_HANGAR, vehStrCD=None, itemsPack=None, offers=None, price=MONEY_UNDEFINED, oldPrice=None, title=b'', description=None, endTime=None, buyParams=None, obtainingMethod=None, vehParams=None, **kwargs):
    ...
    if isHeroTank and allowToShowHeroTankOnScene and not (itemsPack or offers or vehParams):
        ...
        kwargs.update({b'itemCD': vehTypeCompDescr, 
           ...
           b'obtainingMethod': obtainingMethod, 
...
def showVehiclePreviewWithoutBottomPanel(vehCD, previewAlias=None, **kwargs):
    ...
    kwargs.update({b'itemCD': vehCD, 
       ...
       b'previewAlias': (previewAlias or VIEW_ALIAS.CONFIGURABLE_VEHICLE_PREVIEW)})
    ConfigurableVehiclePreviewState.goTo(**kwargs)
    return
...
def showDelayedReward():
    kwargs = {b'tab': (QUESTS_ALIASES.BATTLE_MATTERS_VIEW_PY_ALIAS), b'openVehicleSelection': True}
...
def goToHeroTankOnScene(vehTypeCompDescr, previewAlias=VIEW_ALIAS.LOBBY_HANGAR, previousBackAlias=None, hangarVehicleCD=None, instantly=False):
    ...
    for entity in BigWorld.entities.values():
        if entity and isinstance(entity, HeroTank):
            ...
            ClientSelectableCameraObject.switchCamera(entity, b'HeroTank', instantly)
            break
...
def showSettingsWindow(redefinedKeyMode=False, tabIndex=None, isBattleSettings=False):
    ...
    g_eventBus.handleEvent(events.LoadViewEvent(SFViewLoadParams(alias), ctx={b'redefinedKeyMode': redefinedKeyMode, 
       b'tabIndex': tabIndex, 
...
def showExchangeXPWindow(needXP=None):
    ctx = None if needXP is None else {b'currencyValue': needXP}
...
def showExchangeXPDialogWindow(needXP=None, parent=None):
    ...
    from gui.impl.dialogs import dialogs
    from gui.impl.pub.dialog_window import DialogButtons
    ctx = None if needXP is None else {b'currency': needXP}
...
def showLootBoxBuyWindow(eventName, lobbyCtx=None):
    if not lobbyCtx.getServerSettings().isLootBoxesEnabled():
        _logger.debug(b'LootBoxes are disabled on server. Nothing will be shown!')
        return
...
def showBrowserOverlayView(url, alias=VIEW_ALIAS.BROWSER_LOBBY_TOP_SUB, params=None, callbackOnLoad=None, webHandlers=None, forcedSkipEscape=False, browserParams=None, hiddenLayers=None, parent=None, callbackOnClose=None):
    if not url:
        ...
        ctx = {b'url': url, 
           ...
           b'hiddenLayers': (hiddenLayers or ()), 
           b'callbackOnClose': callbackOnClose}
...
def showSeniorityRewardVehiclesWindow(vehicles=None, fromEntryPoint=True, notificationMgr=None):
    ...
    if uiLoader.windowsManager.getViewByLayoutID(viewID) is None:
        ...
        _logger.error(b'SeniorityRewardVehiclesWindow already exists')
    return
...
def showSeniorityRewardAwardWindow(data, notificationMgr=None):
    ...
    if uiLoader.windowsManager.getViewByLayoutID(viewID) is None:
        ...
        _logger.error(b'SeniorityRewardAwardWindow already exists')
    return
...
def showBattlePassAwardsWindow(bonuses, data, useQueue=False, needNotifyClosing=True, starterPack=None, packageRewards=None, battlePass=None):
    ...
    if battlePass.isHoliday():
        data[b'chapter'] = battlePass.getHolidayChapterID()
...
def showStylePreview(vehCD, style, descr=b'', backCallback=None, *args, **kwargs):
    from gui.Scaleform.daapi.view.lobby.vehicle_preview.states import StylePreviewState
    StylePreviewState.goTo(itemCD=vehCD, style=style, styleDescr=descr, backCallback=backCallback, backPreviewAlias=kwargs.get(b'backPreviewAlias'), topPanelData=kwargs.get(b'topPanelData'), itemsPack=kwargs.get(b'itemsPack'), outfit=kwargs.get(b'outfit'))
    return
...
def showStyleProgressionPreview(vehCD, style, descr, *args, **kwargs):
    ...
    StyleProgressionPreviewState.goTo(itemCD=vehCD, style=style, styleDescr=descr, backPreviewAlias=kwargs.get(b'backPreviewAlias'), styleLevel=kwargs.get(b'styleLevel'))
    return


def showBattlePassStyleProgressionPreview(vehCD, style, descr, *args, **kwargs):
    from gui.Scaleform.daapi.view.lobby.vehicle_preview.states import StyleProgressionPreviewState
    StyleProgressionPreviewState.goTo(itemCD=vehCD, style=style, styleDescr=descr, styleLevel=kwargs.get(b'styleLevel'), chapterId=kwargs.get(b'chapterId'))
    return
...
def showStyleBuyingPreview(vehCD, style, descr, *args, **kwargs):
    ...
    StyleBuyingPreviewState.goTo(itemCD=vehCD, style=style, styleDescr=descr, backPreviewAlias=kwargs.get(b'backPreviewAlias'), styleLevel=kwargs.get(b'styleLevel'), price=kwargs.get(b'price'), buyParams=kwargs.get(b'buyParams'))
    return
...
def showShowcaseStyleBuyingPreview(vehCD, style, descr, *args, **kwargs):
    ...
    ShowcaseStyleBuyingPreviewState.goTo(itemCD=vehCD, style=style, styleDescr=descr, backPreviewAlias=kwargs.get(b'backPreviewAlias'), price=kwargs.get(b'price'), originalPrice=kwargs.get(b'originalPrice'), buyParams=kwargs.get(b'buyParams'), obtainingMethod=kwargs.get(b'obtainingMethod'), endTime=kwargs.get(b'endTime'), discountPercent=kwargs.get(b'discountPercent'))
    return
...
@wg_async
def showFrontlineConfirmDialog(skillsInteractor, vehicleType=b''):
...
def showOnboardingView(styleCD=None, isFirstRun=False, parent=None, guiLoader=None):
    ...
    if view is None:
        window = OnboardingWindow({b'styleCD': styleCD, b'isFirstRun': isFirstRun}, parent or getParentWindow())
        window.load()
    return
...
def showWotPlusInfoPage(source, useCustomSoundSpace=False, includeSubscriptionInfo=False):
    ...
    ctx = {b'url': url, 
       ...
       b'useCustomSoundSpace': useCustomSoundSpace}
...
def showBuyBattlePassOverlay(parent=None):
    ...
    if url:
        url = yield URLMacros().parse(url)
        g_eventBus.handleEvent(events.LoadViewEvent(SFViewLoadParams(VIEW_ALIAS.BROWSER_OVERLAY, parent=parent), ctx={b'url': url}), EVENT_BUS_SCOPE.LOBBY)
    return
...
def showPrebattleHintsWindow(hintModel, hintsViewClass=None):
    ...
    needToShow = getattr(hintsViewClass, b'needToShow')
...
def showTrophiesView(closeCallback, parentScreen, *args, **kwargs):
    ...
    if uiLoader.windowsManager.getViewByLayoutID(contentResId) is None and not fullScreenWindows:
        window = CatalogViewWindow(initAchievementIDs=[
         TROPHIES_ACHIEVEMENT_ID], achievementCategory=b'', closeCallback=closeCallback, uiParentScreen=parentScreen, *args, **kwargs)
        window.load()
    return


def showAdvancedAchievementsView(closeCallback=None):
    g_eventBus.handleEvent(events.LoadViewEvent(SFViewLoadParams(VIEW_ALIAS.LOBBY_PROFILE), ctx={b'selectedAlias': (VIEW_ALIAS.PROFILE_ACHIEVEMENTS_PAGE), b'closeCallback': closeCallback}), scope=EVENT_BUS_SCOPE.LOBBY)
    return
...
def showVehicleHubMechanicsVideo(mechanicsName):
    ...
    if urlDict is None:
        logging.warning(b'There is no mechanicsVideoUrls in gui_settings config.')
        return
    else:
...
def showRerollBoxDialog(eventName, price, currency):
    ...
    if guiLoader.windowsManager.getViewByLayoutID(layoutID) is None:
        ...
        isUserCancelAction = isinstance(data, dict) and data.get(b'isUserCancelAction')
