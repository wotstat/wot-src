...
def packBase(descrArgs, skillLevel, *args, **kwargs):
    keyArgs = {}
    kpiArgs = []
    lowEfficiency = kwargs.get(b'lowEfficiency', False)
    isTmanTrainedVeh = kwargs.get(b'isTmanTrainedVeh', False)
    customValues = kwargs.get(b'customValues')
    isSkillAlreadyEarned = kwargs.get(b'isSkillAlreadyEarned') or kwargs.get(b'isSkillAlreadyEarned')
    hasBooster = kwargs.get(b'hasBooster')
    for paramName, paramDescArgs in descrArgs:
        ...
        keyArgs[paramName] = {b'value': (formatters.getDescriptionValue(paramDescArgs, paramValue)), 
           b'color': color}
    ...
    return {b'keyArgs': keyArgs, b'kpiArgs': kpiArgs}
...
def _packGunnerRancorous(descrArgs, skillLevel, *args, **kwargs):
    ...
    kwargs.update({b'customValues': {b'damageMonitoringDelay': _customValue}})
...
def _packEnemyShotPredictor(descrArgs, skillLevel, *args, **kwargs):
    ...
    kwargs.update({b'customValues': {b'artNotificationDelayFactor': _customValue}})
    return packBase(descrArgs, skillLevel, *args, **kwargs)


def _parkSixthSense(descrArgs, skillLevel, *args, **kwargs):

    def _customValue(skillDescrArg, isKpi, customValues=kwargs.get(b'customValues', {})):
        if not isKpi:
    ...
    kwargs.update({b'customValues': {b'delay': _customValue}})
    return packBase(descrArgs, skillLevel, *args, **kwargs)
...
def _packBadRoadsKing(descrArgs, skillLevel, *args, **kwargs):
    ...
    kwargs.update({b'customValues': {b'softGroundFactor': _customValue}})
    return packBase(descrArgs, skillLevel, *args, **kwargs)
