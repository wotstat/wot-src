...
class _Bonuses(CONST_CONTAINER):
    BASIC_ROLE_BOOST_100 = b'basicRoleBoost_100'
    ANY_ADDITIONAL_SKILL = b'anyAdditionalSkill'
    SAME_BLOOD_ZERO_SKILL = b'sameBloodZeroSkill'


_BUNDLE_PRESETS = {b'offspring': (
                _Bonuses.BASIC_ROLE_BOOST_100,
                _Bonuses.ANY_ADDITIONAL_SKILL,
                _Bonuses.SAME_BLOOD_ZERO_SKILL)}

class _BonusPreset(namedtuple(b'BonusPreset', (b'imageRPath', b'textRPath', b'top', b'left', b'bottom', b'right'))):
...
class CrewBundleTooltipData(BlocksTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        if bundleName not in _BUNDLE_PRESETS:
            raise SoftException((b'Bundle "{}" is not supported.').format(bundleName))
        return [
