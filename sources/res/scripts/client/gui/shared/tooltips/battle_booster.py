...
class HeaderBlockConstructor(BattleBoosterTooltipBlockConstructor):
    ...
    def _getOverlay(self):
        ...
        overlayPath = R.images.gui.maps.shop.artefacts.c_180x135.dyn((b'{}_overlay').format(overlayType))()
        return overlayPath
    ...
    def _getIcon(self):
        ...
        if icon is None:
            _logger.warn(b'BattleBooster icon missed: R.images.gui.maps.shop.artefacts.c_180x135.%s', moduleName)
            return b''
        else:
            return icon()
...
class BattleBoosterInventoryBlockConstructor(InventoryBlockConstructor):
    ...
    def __init__(self, module, configuration, leftPadding, rightPadding):
        ...
        self._inInventoryBlockData[b'text'] = TOOLTIPS.BATTLEBOOSTER_INVENTORYCOUNT
...
class EffectsBlockConstructor(BattleBoosterTooltipBlockConstructor):
    ...
    def construct(self):
        ...
        if module.isBuiltinPerkBooster():
            ...
            block.append(formatters.packMultipleText(b'', formatters.getImage(backport.image(R.images.gui.maps.icons.personal_reserves.tooltips.lightening_icon()), width=10, height=16, vspace=-3), text_styles.yellowText(backport.text(R.strings.tooltips.boostersWindow.booster.activateInfo.bonus())), b' ', text_styles.main(self.module.descriptor.description)))
        else:
            kpi = first(module.getKpi(self.configuration.vehicle))
            if kpi:
                descriptor = text_styles.main(module.shortDescription.replace(b'%s ', b'', 1))
                ...
                _logger.warn(b'BattleBooster kpi missed %s', module.descriptor.iconName)
        return block
