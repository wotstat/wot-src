...
class MapTooltipData(ToolTipBaseData):
    ...
    def getDisplayableData(self, arenaID):
        ...
        return {b'mapName': (i18n.makeString(b'#arenas:%s/name' % arenaType.geometryName)), 
           b'gameplayName': (i18n.makeString(b'#arenas:type/%s/name' % arenaType.gameplayName)), 
           b'imageURL': (b'../maps/icons/map/%s.png' % arenaType.geometryName), 
           b'description': (i18n.makeString(b'#arenas:%s/description' % arenaType.geometryName))}
...
class ToolTipFortWrongTime(ToolTipBaseData):
    ...
    def getDisplayableData(self, wrongState, timePeriods):
        ...
        if wrongState == b'wrongTime':
            ...
            if (wrongState == b'lockTime' or wrongState == b'ownDefenceTime') and timePeriods is not None and len(timePeriods) >= 1:
...
class CURRENCY_SETTINGS(object):
    ...
    DEMOUNT_KIT_PRICE = b'demountKitPrice'
...
def makeRemovalPriceBlock(price, currencySetting, neededValue=None, oldPrice=None, percent=0, valueWidth=-1, leftPadding=61, forcedText=b'', isDeluxe=False, gap=15, canUseDemountKit=False, wotPlusStatus=False, isFreeToDemount=False, isFreeDeluxeEnabled=False, isFreeDemountEnabled=False):
    _int = backport.getIntegralFormat
    settings = _getCurrencySetting(currencySetting)
    if settings is None:
        return
...
class VehicleHistoricalReferenceTooltipData(BlocksTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        blocks = list()
        blocks.append(formatters.packImageBlockData(img=(b'../maps/icons/flags/160x100/{}.png').format(item.nationName), align=BLOCKS_TOOLTIP_TYPES.ALIGN_LEFT, padding={b'left': (-19)}))
        blocks.append(formatters.packTextBlockData(text_styles.highTitle(TOOLTIPS.VEHICLEPREVIEW_HISTORICALREFERENCE_TITLE), padding={b'top': (-72)}))
        blocks.append(formatters.packTextBlockData(text_styles.main(item.fullDescription), padding={b'top': 10}))
...
class TechTreeEventTooltipBase(BlocksTooltipData):
    ...
    def _packEarlyAccessBlock(self, linkage=BLOCKS_TOOLTIP_TYPES.TOOLTIP_BUILDUP_BLOCK_LINKAGE):
        ...
        blocks.append(formatters.packTextBlockData(text_styles.concatStylesToSingleLine(icon, b'   ', text_styles.main(description)), padding=formatters.packPadding(left=43)))
