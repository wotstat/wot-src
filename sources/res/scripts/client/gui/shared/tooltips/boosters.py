...
class BoosterTooltipData(BlocksTooltipData):
    ...
    def __getHeader(self, booster):
        ...
        descriptionText = R.strings.tooltips.boostersWindow.booster.activateInfo.dyn(b'improvedReserve' if booster.getIsPremium() else b'basicReserve')()
    ...
    def __getReceiveBlock(self, booster):
        if isinstance(booster, ClanReservePresenter):
            ...
            receiveText = R.strings.tooltips.boostersWindow.booster.activateInfo.dyn(b'paidReceiveInfo' if booster.boosterID in PREMIUM_BOOSTER_IDS else b'nonPaidReceiveInfo')()
        return formatters.packBuildUpBlockData([
    ...
    def __packGrayText(self, message):
        return formatters.packBuildUpBlockData([
         formatters.packAlignedTextBlockData(text=makeHtmlString(b'html_templates:lobby/textStyle', b'standardText', {b'message': message}), align=BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER, padding=formatters.packPadding(top=-8, bottom=-16))], linkage=BLOCKS_TOOLTIP_TYPES.TOOLTIP_BUILDUP_BLOCK_WHITE_BG_LINKAGE)

    def __getAccessCondition(self, questsResult):
        qNames = (b'"{}"').format((b', ').join(questsResult))
        return formatters.packBuildUpBlockData([
    ...
    def __getBoosterQuestNames(self, boosterID):
        ...
        for q in viewvalues(quests):
            bonuses = q.getBonuses(b'goodies')
            for b in bonuses:
