...
SPEC_PARAM_TEMPLATE = BULLET + b' ' + b'%s'
    ...
    if GUI_SETTINGS.technicalInfo:
        ...
        if hasVehicleDescrMechanic(vehicleDescriptor, VehicleMechanic.LOW_CHARGE_SHOT):
            ...
            if descriptor.isDamageMutable:
                ...
                damageValue = (b'{}-{}').format(backport.getNiceNumberFormat(shellParams.avgMutableDamage[0]), backport.getNiceNumberFormat(shellParams.avgMutableDamage[1]))
                note = ASTERISK
            ...
            if piercingPower != 0:
                ...
                if piercingPowerTable != NO_DATA and isDistanceDependent:
                    note = ASTERISK
                    value = (b'{}-{}').format(backport.getNiceNumberFormat(piercingPowerTable[0][1]), backport.getNiceNumberFormat(piercingPowerTable[-1][1]))
...
def getLowChargeShotParams(shellDescr, vDescr, vehAttrs=None):
    ...
    if vehAttrs is not None:
        factorName = (b'{}/gunShotsSpeed').format(VehicleMechanic.LOW_CHARGE_SHOT.value)
