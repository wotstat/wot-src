...
class VehicleFilterTooltip(BlocksTooltipData):
    ...
    def _packHiddenBlock(self):
        subBlocks = []
        if not self._rented:
            subBlocks.append(formatters.packTextParameterWithIconBlockData(name=text_styles.main(TANK_CAROUSEL_FILTER.INFOTIP_RENT), value=b'', icon=ICON_TEXT_FRAMES.RENTALS, padding=formatters.packPadding(left=-50, top=-3, bottom=-18), nameOffset=20))
        if not self._event:
            ...
            subBlocks.append(formatters.packTextBlockData(text=(b'{}      {}').format(icon, text), padding=formatters.packPadding(left=6, top=5, bottom=0)))
        return formatters.packBuildUpBlockData(subBlocks, linkage=BLOCKS_TOOLTIP_TYPES.TOOLTIP_BUILDUP_BLOCK_WHITE_BG_LINKAGE)
    ...
    def __packLevelBlock(self):
        string = (b', ').join(int2roman(level) for level in self._levels)
    ...
    def __packSpecialTypeBlock(self):
        string = b''
        addComma = lambda str: (b'{0}, ').format(str) if str else str
        if self._specials[b'premium']:
            string += _ms(TANK_CAROUSEL_FILTER.INFOTIP_ONLY_PREMIUM)
        if self._specials[b'elite']:
            string = addComma(string)
            string += _ms(TANK_CAROUSEL_FILTER.INFOTIP_ONLY_ELITE)
        if self._specials[b'favorite']:
            string = addComma(string)
            string += _ms(TANK_CAROUSEL_FILTER.INFOTIP_ONLY_FAVORITE)
        if self._specials[b'bonus']:
            icon = icons.makeImageTag(RES_ICONS.MAPS_ICONS_LIBRARY_MULTYXP)
            xpRate = text_styles.stats((b'x{0}').format(self.itemsCache.items.shop.dailyXPFactor))
            string = addComma(string)
            string += (b'{0}{1}').format(icon, xpRate)
        if self._specials[b'igr']:
            string = addComma(string)
    ...
    def __gatherData(self, tankCarousel):
        ...
        for level in VEHICLE_LEVELS:
            if filters[b'level_%d' % level]:
                self._levels.append(level)

        self._rented = filters[b'rented']
        self._event = filters[b'event']
        self._searchNameVehicle = filters[b'searchNameVehicle']
        self._specials = {b'premium': (filters[b'premium']), 
           b'elite': (filters[b'elite']), 
           b'igr': (filters[b'igr'] if constants.IS_KOREA else False), 
           b'bonus': (filters[b'bonus']), 
           b'favorite': (filters[b'favorite'])}
        return
