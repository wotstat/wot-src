...
class ShellBlockToolTipData(BlocksTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        showBasicData = self.__basicDataAllowed and params[b'isBasic']
...
class PriceBlockConstructor(ShellTooltipBlockConstructor):
    ...
    def construct(self):
        ...
        if buyPrice and sellPrice:
            LOG_ERROR(b'You are not allowed to use buyPrice and sellPrice at the same time')
            return
        else:
    ...
    def construct(self):
        ...
        if self.configuration.params:
            ...
            for parameter in params_formatters.getRelativeDiffParams(comparator):
                ...
                if delta > 0:
                    value -= delta
                block.append(formatters.packStatusDeltaBlockData(title=text_styles.middleTitle(MENU.tank_params(parameter.name)), valueStr=params_formatters.simplifiedDeltaParameter(parameter), statusBarData={b'value': value, 
                   b'delta': delta, 
                   b'minValue': 0, 
                   b'markerValue': (stockParams.get(parameter.name, value)), 
                   b'maxValue': (max(MAX_RELATIVE_VALUE, value + delta)), 
                   b'useAnim': False}, padding=formatters.packPadding(left=75)))
...
class _AdvancedCommonStatsBlockConstructior(_BaseCommonStatsBlockConstructor):
    ...
    def construct(self):
        ...
        if self.configuration.params:
            ...
            isDistanceDependent = self._params.pop(b'piercingPowerTable') is not None
            formattedParameters = params_formatters.getFormattedParamsList(shell.descriptor, self._params)
            ...
            for paramName, paramValue in formattedParameters:
                ...
                if paramValue is not None:
                    ...
                    paramUnits += _ASTERISK if isPiercingPower and not isDistanceDependent else b''
...
class CommonStatsBlockConstructor(_BaseCommonStatsBlockConstructor):
    ...
    def colorScheme(self):
        if self.configuration.colorless:
            return params_formatters.COLORLESS_SCHEME
        return params_formatters.BASE_SCHEME
    ...
    def __iterPiercingPowerTable(self, table, comparator):
        if comparator is not None:
            ...
            for (distance, value), (_, valueState) in izip(extendedTable.value, extendedTable.state):
                yield (
                 distance, params_formatters.formatParameter(b'piercingPower', value, valueState, self.colorScheme))

        else:
            for distance, value in table:
                yield (
                 distance, params_formatters.formatParameter(b'piercingPower', value))
    ...
    def _getAvgPiercingPowerPackInfo(self, pTable, tableData):
        ...
        if tableData and pTable is not None:
            value = b'%s-%s' % (tableData[0][0], tableData[-1][0])
    ...
    def _getAvgPiercingFootnote(self, pTable, tableData):
        if pTable is not None and tableData:
            ...
            if self.shell.type == SHELL_TYPES.FLAME:
                return backport.text(R.strings.menu.moduleInfo.params.noPiercingDistance.footnoteFlame())
            return backport.text(R.strings.menu.moduleInfo.params.noPiercingDistance.footnote())
    ...
    def construct(self):
        if not self.configuration.params:
            ...
            block = [
             formatters.packTitleDescBlock(title=text_styles.middleTitle(_ms(TOOLTIPS.TANKCARUSEL_MAINPROPERTY)), padding=formatters.packPadding(bottom=8))]
            asteriks = _ASTERISK
            asteriksTitles = []
            ...
            for paramName, paramValue in params_formatters.getFormattedParamsList(self.shell.descriptor, self._params):
                ...
                if paramName == b'damage':
                    asteriksTitle = self._getDamagePackInfo()
                if paramName == b'shotSpeed' and hasDistanceFactor:
                    continue
                if paramName == b'shotSpeedAccelerated' and not hasDistanceFactor:
                    continue
                if paramName == b'shotSpeedAccelerated' and hasDistanceFactor:
    ...
    def construct(self):
        ...
        if checkBuying:
            ...
            if not couldBeBought:
                status = b'#tooltips:shellFits/%s' % reason
        statusHeader, statusText = getComplexStatus(status)
        if statusHeader is not None or statusText is not None:
            block.append(formatters.packTitleDescBlock(title=text_styles.middleTitle(statusHeader if statusHeader is not None else b''), desc=text_styles.main(statusText if statusText is not None else b'')))
        return block
