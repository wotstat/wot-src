...
class CommonStatsBlockConstructor(ShellTooltipBlockConstructor):
        ...
        if not self.configuration.params:
            ...
            for paramName, paramValue in formattedParameters:
                if paramName == ModuleTooltipBlockConstructor.CALIBER:
                    continue
                if paramName == b'avgDamage' and shell.isDamageMutable():
                    continue
                if paramName == b'normalizationAngle' and not self.configuration.showNormalizationAngle:
                    continue
                if paramName == b'ricochetAngle' and not self.configuration.showReboundAngle:
                    continue
                if paramName == b'penetrationLoss' and not self.configuration.showPenetrationLoss:
                    continue
                if paramName == b'screensArmorMultiplier' and not self.configuration.showScreensArmorMultiplier:
                    continue
    ...
    def _applyParamMeta(self, paramName, tableData, footNotes):
        ...
        isPenetrationLoss = paramName == b'penetrationLoss'
    ...
    def _iteratePiercingPowerTable(table, comparator, colorScheme):
        if comparator is not None:
            ...
            for (_, value), (_, valueState) in zip(extendedTable.value, extendedTable.state):
                fmtValue = params_formatters.formatParameter(b'piercingPower', value, valueState, colorScheme)
                yield fmtValue

        else:
            for value in table:
                yield params_formatters.formatParameter(b'piercingPower', value)
