...
class VehicleCollectorTooltipData(BlocksTooltipData):
    ...
    def _packBlocks(self, nationName):
        ...
        if nationID is None:
            _logger.error(b'Incorrect nation %s', self.__nationName)
            return []
        else:
            items = [
    ...
    def __getDisabledStatusBlock(self):
        ...
        if helper.isAllCollectionPurchased(nationID):
            ...
            vehicleLevels = getCache()[b'collectorVehiclesLevelsByNations'].get(nationID, set())
            if not vehicleLevels:
                raise SoftException((b'There are not collectible vehicles in the nation tree of {}').format(self.__nationName))
...
class VehicleCollectorDisabledTooltipData(BlocksTooltipData):
    ...
    def getDisplayableData(self, *args, **kwargs):
        if self.__lobbyContext.getServerSettings().isCollectorVehicleEnabled():
            _logger.error(b'Vehicle collector is enabled')
            return {}
        return {b'header': (text_styles.critical(backport.text(R.strings.tooltips.collectibleVehicleTooltip.switchOff.header()))), 
           b'body': (text_styles.main(backport.text(R.strings.tooltips.collectibleVehicleTooltip.switchOff.text())))}
