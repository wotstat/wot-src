...
class ElenPreviewTooltipData(BlocksTooltipData, IGlobalListener):
    ...
    def _getHeader(self, currentEvent):
        ...
        if isRegistrationNeeded and not isRegistered:
            ...
            text = (b'{} {} {} {}').format(flagIcon, text_styles.tutorial(_ms(msgText)), text_styles.tutorial(time), text_styles.tutorial(timeName))
        else:
    ...
    def _getPrimeTimes(self, primeTimes):
        ...
        def getPrimeTimeBlock(pt):
            ...
            name += b':'
            return (b'{0} {1}').format(text_styles.main(name), text_styles.standard((b'{} - {}').format(pt.getStartLocalTime(), pt.getEndLocalTime())))
    ...
    def _getBottom(self, currentEvent):
        ...
        if noserver:
            return formatters.packTextBlockData(text=makeHtmlString(b'html_templates:lobby/textStyle', b'alignText', {b'align': b'center', 
               b'message': (text_styles.critical(_ms(TOOLTIPS.HANGAR_ELEN_BOTTOM_UNAVAILABLE)))}), padding=formatters.packPadding(top=3, bottom=16))
        if wrongBattleType:
            return formatters.packTextBlockData(text=makeHtmlString(b'html_templates:lobby/textStyle', b'alignText', {b'align': b'center', 
               b'message': (text_styles.critical(_ms(battleTypeText)))}), padding=formatters.packPadding(top=3, bottom=16))
        if wrongSquadState:
            return formatters.packTextBlockData(text=makeHtmlString(b'html_templates:lobby/textStyle', b'alignText', {b'align': b'center', 
               b'message': (text_styles.critical(_ms(TOOLTIPS.HANGAR_ELEN_BOTTOM_WRONGSQUADSTATE)))}), padding=formatters.packPadding(top=3, bottom=16))
        return formatters.packImageTextBlockData(title=b'', desc=text_styles.standard(_ms(battleTypeBodyText)), padding=formatters.packPadding(left=20, right=20, top=3, bottom=16))
...
class BabgesGroupTooltipData(BlocksTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        for bonus in args:
            blocks.append(formatters.packRendererTextBlockData(rendererType=b'AwardItemExUI', dataType=b'net.wg.gui.data.AwardItemVO', title=text_styles.middleTitle(bonus.name), rendererData={b'imgSource': (bonus.imgSource)}, desc=text_styles.main(bonus.desc), txtPadding=formatters.packPadding(left=8), padding=formatters.packPadding(top=-5, bottom=5)))
