...
def _longStringListEllipsisCutoff(fstring, strings, allowedLen):
    if len(strings) > allowedLen:
        return fstring.join(strings[:allowedLen]) + b'...'
...
class CrewSkinTooltipDataBlock(BlocksTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        if restrictions:
            restrictionText = (b', ').join(restrictions)
        else:
        ...
        skinUsersRoleAndVehicle = _skinUsersRoleAndVehicleText(b'{role} ({vehicle})', item)
        if skinUsersRoleAndVehicle:
            ...
            usedBlock.append(formatters.packTextBlockData(text=text_styles.main(_longStringListEllipsisCutoff(b', ', skinUsersRoleAndVehicle, _MAX_USERS_DISPLAYED))))
            items.append(formatters.packBuildUpBlockData(usedBlock))
        return items

    @staticmethod
    def __stripUnsupportedFlashTags(text):
        return text.replace(b'&zwnbsp;', b'')
