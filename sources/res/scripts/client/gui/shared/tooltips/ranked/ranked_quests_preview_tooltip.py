...
class RankedQuestsPreviewTooltip(BlocksTooltipData):
    ...
    def __packBottom(self, diff):
        ...
        if diff > 0:
            return [formatters.packTextBlockData(text_styles.alignText(text, b'center'), padding=formatters.packPadding(top=-7, bottom=12))]
        return []
    ...
    def __packQuestInfo(self, quest, isGlobalAvailable):
        ...
        if isCompleted:
            ...
            padding[b'left'] = 18
            descPadding[b'left'] = 4
            descPadding[b'top'] = -5
        elif not isAvailable and not isGlobalAvailable:
            titleIcon = icons.notAvailableRed() + b'  '
    ...
    def __packSingleReward(self, bonus):
        isMultiBonus = bonus.label.startswith(b'x')
