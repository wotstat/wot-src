...
class RankedCalendarStepsTooltip(BlocksTooltipData):
    ...
    def _packBlocks(self):
        ...
        for cycle in sorted(cycles.values()):
            ...
            item = formatter(backport.text(key, cycleNumber=seasonName, day0=(b'{:02d}').format(startDate.tm_mday), month0=(b'{:02d}').format(startDate.tm_mon), day1=(b'{:02d}').format(endDate.tm_mday), month1=(b'{:02d}').format(endDate.tm_mon)))
            if currentCycle:
