...
class RoleSkillBattleTooltipData(BlocksTooltipData):
    ...
    def _packBlocks(self, equipmentID):
        ...
        if equipment is None:
            _logger.error(b'Missing Role Skill for equipmentID = %d', equipmentID)
            return []
        else:
...
def getRoleSkillDescription(equipment):
    ...
    debuf = description.dyn(b'debuf')
    debuf = backport.text(debuf(), **params) if debuf.exists() and tooltipParams.has_key(b'debuf') else b''
