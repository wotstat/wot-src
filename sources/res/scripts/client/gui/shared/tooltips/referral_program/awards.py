...
class AwardsTooltipData(BlocksTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        if not data:
            return self.__packHeader(b'')
        return [self.__packHeader()] + [self.__packItemBlock(data.get(b'items', []))]
    ...
    def __packItemBlock(self, items):
        ...
        for itemData in items:
            count = itemData.get(b'count', 0)
            itemId = itemData.get(b'id', 0)
            itemType = itemData.get(b'type', b'')
            icon, title = self.__getItemData(itemType, itemId)
            if not icon:
                continue
            awardCount = backport.text(self.__resPath.awardCount(), count=count) if count > 1 else b' '
    ...
    def __getItemData(self, itemType, itemId):
        ...
        if itemType in ItemPackTypeGroup.CUSTOMIZATION:
            ...
            if item is None:
                _logger.warning(b"Award '%r' not supported", itemType)
                return (b'', b'')
