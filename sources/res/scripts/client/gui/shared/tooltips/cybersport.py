...
class CybersportSlotSelectedToolTipData(CybersportToolTipData):
    ...
    def getDisplayableData(self, index, unitMgrID=None):
        ...
        if dispatcher is not None:
            ...
            accountDBID = unit.getMembers()[index][b'accountDBID']
...
class CybersportUnitToolTipData(CybersportToolTipData):
    ...
    def getDisplayableData(self, data=None):
        if data is not None:
            return {b'unitComment': (data.description), 
               b'commanderName': (data.creatorName), 
               b'commanderRating': (data.rating), 
               b'commanderRatingDesc': (TOOLTIPS.CYBERSPORT_COMMANDER_STATS)}
        else:
        ...
        if dispatcher is not None:
            ...
            if entity:
                ...
                if restriction == UNIT_RESTRICTION.MIN_TOTAL_LEVEL:
                    ...
                    description = makeHtmlString(b'html_templates:lobby/cyberSport/unit', b'invalidLevelDescription', {b'description': description, b'reason': reason})
                elif canDoAction and not restriction:
                    statusLevel = b'info'
        result = {b'header': (ms(TOOLTIPS.CYBERSPORT_UNITLEVEL_TITLE)), 
           b'description': description, 
           b'level': (str(requiredLevel)), 
           b'icon': (RES_ICONS.MAPS_ICONS_LIBRARY_OKICON), 
           b'levelDescription': (ms(TOOLTIPS.CYBERSPORT_UNITLEVEL_BODY)), 
           b'statusMsg': statusMsg, 
           b'statusLevel': statusLevel}
        return result
