...
class UniqueCamouflageTooltip(BlocksTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        blocks.append(formatters.packTextBlockData(text_styles.main(b'UniqueCamouflageTooltip')))
        return blocks
...
class FreeSheetReturnTooltip(FreeSheetTooltip):
    ...
    def _getInfoBlock(self):
        return formatters.packBuildUpBlockData([
         formatters.packTextParameterWithIconBlockData(text_styles.neutral(TOOLTIPS.PERSONALMISSIONS_FREESHEET_INFO), b'', ICON_TEXT_FRAMES.ATTENTION, padding=formatters.packPadding(left=-60, bottom=-2))], linkage=BLOCKS_TOOLTIP_TYPES.TOOLTIP_BUILDUP_BLOCK_WHITE_BG_LINKAGE, padding=formatters.packPadding(top=-7, bottom=-3))
...
class FreeSheetNotEnoughTooltip(FreeSheetTooltip):
    ...
    def _getInfoBlock(self):
        return formatters.packBuildUpBlockData([
         formatters.packTextParameterWithIconBlockData(text_styles.alert(TOOLTIPS.PERSONALMISSIONS_FREESHEET_NOTENOUGH), b'', ICON_TEXT_FRAMES.ALERT, padding=formatters.packPadding(left=-60, bottom=-2))], linkage=BLOCKS_TOOLTIP_TYPES.TOOLTIP_BUILDUP_BLOCK_WHITE_BG_LINKAGE, padding=formatters.packPadding(top=-7, bottom=-3))
    ...
    def _getDescriptionBlock(self):
        return (
         formatters.packBuildUpBlockData([
          formatters.packTextParameterWithIconBlockData(text_styles.concatStylesToMultiLine(text_styles.middleTitle(TOOLTIPS.PERSONALMISSIONS_FREESHEET_USED_HEADER), text_styles.main(TOOLTIPS.PERSONALMISSIONS_FREESHEET_USED_TEXT)), b'', ICON_TEXT_FRAMES.LOCK, padding=formatters.packPadding(left=-60, bottom=-2))]),)
...
class OperationTooltipData(BlocksTooltipData):
    ...
    def _getMissionsBlock(cls, operation):
        ...
        items.append(formatters.packTextBlockData(text=text_styles.concatStylesWithSpace(text_styles.middleTitle(TOOLTIPS.PERSONALMISSIONS_OPERATION_MISSIONS_TITLE), _formatCompleteCount(len(completedQuests), totalCount), text_styles.standard(b'/ %s' % totalCount)), padding=formatters.packPadding(top=8, left=17)))
        for classifierAttr in operation.getIterationChain():
            ...
            items.append(formatters.packTitleDescParameterWithIconBlockData(title=text_styles.concatStylesWithSpace(_formatCompleteCount(completedCount, chainSize), text_styles.standard(b'/ %s' % chainSize)), value=text_styles.main(operation.getChainName(chainID)), icon=operation.getSmallChainIcon(chainID), iconPadding=formatters.packPadding(top=3, left=10), titlePadding=formatters.packPadding(left=10), padding=formatters.packPadding(left=156, bottom=-9)))
    ...
    def _getStatusBlock(self, operation):
        if operation.isDisabled():
            block = formatters.packBuildUpBlockData([
             formatters.packAlignedTextBlockData(text=text_styles.concatStylesWithSpace(icons.markerBlocked(-2), text_styles.error(TOOLTIPS.PERSONALMISSIONS_OPERATION_FOOTER_TITLE_NOTAVAILABLE)), align=BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER, padding=formatters.packPadding(top=-5)),
             formatters.packAlignedTextBlockData(text=text_styles.main(TOOLTIPS.PERSONALMISSIONS_OPERATION_FOOTER_DESCR_DISABLED), align=BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER, padding=formatters.packPadding(bottom=14))])
        elif not operation.isUnlocked():
            block = formatters.packBuildUpBlockData([
             formatters.packAlignedTextBlockData(text=text_styles.concatStylesWithSpace(icons.markerBlocked(-2), text_styles.error(TOOLTIPS.PERSONALMISSIONS_OPERATION_FOOTER_TITLE_NOTAVAILABLE)), align=BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER, padding=formatters.packPadding(top=-5)),
             formatters.packAlignedTextBlockData(text=text_styles.main(TOOLTIPS.PERSONALMISSIONS_OPERATION_FOOTER_DESCR_DOPREVOPERATION), align=BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER, padding=formatters.packPadding(bottom=14))])
            ...
            data = [
             formatters.packAlignedTextBlockData(text=text_styles.concatStylesWithSpace(icons.checkmark(-2), text_styles.bonusAppliedText(TOOLTIPS.PERSONALMISSIONS_OPERATION_FOOTER_TITLE_DONE)), align=BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER, padding=formatters.packPadding(top=-5))]
            currentCount = operation.getFreeTokensCount()
            totalCount = operation.getFreeTokensTotalCount()
            if currentCount < totalCount:
                data.append(formatters.packAlignedTextBlockData(text=text_styles.concatStylesToSingleLine(text_styles.main(TOOLTIPS.PERSONALMISSIONS_OPERATION_FOOTER_DESCR_FREESHEETS), missions_helper.getHtmlAwardSheetIcon(operation.getBranch()), text_styles.bonusAppliedText(currentCount), text_styles.main(b' / %s' % totalCount)), align=BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER, padding=formatters.packPadding(bottom=14)))
            else:
                currentCount = len(operation.getFullCompletedQuests())
                totalCount = operation.getQuestsCount()
                data.append(formatters.packAlignedTextBlockData(text=text_styles.concatStylesToSingleLine(text_styles.main(TOOLTIPS.PERSONALMISSIONS_OPERATION_FOOTER_DESCR_QUESTSFULLYDONE), text_styles.bonusAppliedText(currentCount), text_styles.main(b' / %s' % totalCount)), align=BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER, padding=formatters.packPadding(bottom=14)))
            ...
            block = formatters.packBuildUpBlockData([
             formatters.packAlignedTextBlockData(text=text_styles.concatStylesWithSpace(icons.inProgress(-1), text_styles.neutral(TOOLTIPS.PERSONALMISSIONS_OPERATION_FOOTER_TITLE_INPROGRESS)), align=BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER, padding=formatters.packPadding(top=-5)),
             formatters.packAlignedTextBlockData(text=text_styles.concatStylesToSingleLine(text_styles.main(footerDescrKey), text_styles.bonusAppliedText(currentCount), text_styles.main(b' / %s' % totalCount)), align=BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER, padding=formatters.packPadding(bottom=14))])
        elif not operation.hasRequiredVehicles():
            block = formatters.packBuildUpBlockData([
             formatters.packAlignedTextBlockData(text=text_styles.concatStylesWithSpace(icons.markerBlocked(-2), text_styles.error(TOOLTIPS.PERSONALMISSIONS_OPERATION_FOOTER_TITLE_NOTAVAILABLE)), align=BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER, padding=formatters.packPadding(top=-5)),
             formatters.packAlignedTextBlockData(text=text_styles.main(TOOLTIPS.PERSONALMISSIONS_OPERATION_FOOTER_DESCR_NOVEHICLE), align=BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER, padding=formatters.packPadding(bottom=14))])
        else:
            block = formatters.packBuildUpBlockData([
             formatters.packAlignedTextBlockData(text=text_styles.stats(TOOLTIPS.PERSONALMISSIONS_OPERATION_FOOTER_TITLE_AVAILABLE), align=BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER, padding=formatters.packPadding(top=-5)),
             formatters.packAlignedTextBlockData(text=text_styles.main(TOOLTIPS.PERSONALMISSIONS_OPERATION_FOOTER_DESCR_SELECTQUEST), align=BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER, padding=formatters.packPadding(bottom=14))])
        return block
...
class OperationPostponedTooltipData(BlocksTooltipData):
    ...
    def _getAwardsBlock(cls, bonuses, _):
        items = [
         formatters.packAwardsExBlockData(bonuses, columnWidth=128, rowHeight=128, horizontalGap=10, renderersAlign=formatters.RENDERERS_ALIGN_CENTER, padding=formatters.packPadding(top=18, bottom=5))]
...
class PersonalMissionInfoTooltipData(BlocksTooltipData):
    ...
    def _getTitleBlock(self, quest):
        ...
        if doesTextExist(description):
            ...
            info = text_styles.main((b'{}/description').format(description))
    ...
    def _getDescriptionBlock(self, localizationKey):
        blocks = []
        for i in range(1, 4):
            keyTitle = (b'{}/info/{}/title').format(localizationKey, i)
            keyDescr = (b'{}/info/{}/description').format(localizationKey, i)
    ...
    def _getLimiterBlock(self, localizationKey):
        blocks = []
        for i in range(1, 3):
            keyTitle = (b'{}/limiter/{}/title').format(localizationKey, i)
            keyDescr = (b'{}/limiter/{}/description').format(localizationKey, i)
            if doesTextExist(keyTitle) and doesTextExist(keyDescr):
    ...
    def _getWarningBlock(self, localizationKey):
        blocks = []
        keyWarning = (b'{}/warning').format(localizationKey)
        if doesTextExist(keyWarning):
            blocks.append(formatters.packImageTextBlockData(b'', text_styles.alert(keyWarning), RES_ICONS.MAPS_ICONS_LIBRARY_ALERTICON, imgPadding=formatters.packPadding(right=5, top=3), padding=formatters.packPadding(bottom=8)))
        if blocks:
            return formatters.packBuildUpBlockData(blocks, gap=-4, padding=formatters.packPadding(left=0, right=0, top=0, bottom=0), stretchBg=False)
        else:
            return
    ...
    def _getStatusBlock(cls, quest):
        ...
        if not isAvailable:
            ...
            if quest.areTokensPawned():
                text = text_styles.concatStylesWithSpace(icons.checkmark(-2), text_styles.bonusAppliedText(TOOLTIPS.PERSONALMISSIONS_MAPREGION_FOOTER_TITLE_DONEFREESHEET), missions_helper.getHtmlAwardSheetIcon(quest.getQuestBranch()), text_styles.stats(b'x%s' % quest.getPawnCost()))
            else:
    ...
    def _packBlocks(self, *args, **kwargs):
        blocks = [
         ...
         formatters.packBuildUpBlockData([
          ...
          self.__makeImageBlock(b'../maps/icons/tankmen/skills/big/new_skill.png', PERSONAL_MISSIONS.TANKWOMANTOOLTIPDATA_ADVANTAGES_NEWPERK),
          self.__makeImageBlock(b'../maps/icons/tankmen/skills/big/brotherhood.png', PERSONAL_MISSIONS.TANKWOMANTOOLTIPDATA_ADVANTAGES_BROTHERHOOD)])]
        return blocks

    def __makeImageBlock(self, icon, text, imgPaddingLeft=15, imgPaddingRight=30, imgPaddingTop=0):
        return formatters.packImageTextBlockData(title=text_styles.main(text), desc=b'', img=icon, imgPadding=formatters.packPadding(left=imgPaddingLeft, right=imgPaddingRight, top=imgPaddingTop), padding=formatters.packPadding(bottom=20))
...
class TankModuleTooltipData(BlocksTooltipData):
    ...
    def _packBlocks(self, operationID, classifier):
        ...
        bonus = findFirst((lambda q: q.getName() == b'completionTokens'), finalQuest.getBonuses(b'tokens'))
        formattedBonus = first(CompletionTokensBonusFormatter().format(bonus))
        operationTitle = str(operation.getVehicleBonus().userName).replace(b' ', b'&nbsp;')
...
class PM3GiftTokenTooltipData(BattlePassGiftTokenTooltipData):
    ...
    def _getBlocks(cls, shortName):
        ...
        blocks = [
         formatters.packTextBlockData(text=text_styles.highTitle(backport.text(rOffer.dyn(shortName)())))]
