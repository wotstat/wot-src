...
class VehCmpCustomizationTooltip(BlocksTooltipData):
    ...
    def __packBonusBlock(self):
        blocks = [
         formatters.packTextParameterBlockData(name=self.__camo.bonus.description, value=text_styles.bonusAppliedText((b'+{}').format(self.__camo.bonus.getFormattedValue(self.__vehicle))), valueWidth=53, gap=18, padding=formatters.packPadding(top=-5, bottom=-7))]
