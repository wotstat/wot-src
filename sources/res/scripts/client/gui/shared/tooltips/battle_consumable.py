...
class HeaderBlockConstructor(BattleConsumableTooltipBlockConstructor):
    ...
    def construct(self):
        tier = self.item.name.split(b'tier')
        if len(tier) > 1:
            _, level = tier
            desc = _ms(b'#fortifications:orderType/battleConsumable', level=int2roman(int(level)))
        else:
            desc = _ms(b'#fortifications:orderType/battleConsumable_no_level')
        block = []
