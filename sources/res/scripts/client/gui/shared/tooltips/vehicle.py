    ...
    def __createStatusBlock(self, vehicle, items, statsConfig, paramsConfig, valueWidth):
        ...
        if statsConfig.dailyXP and not vehicle.isWotPlus:
            ...
            if attrs & constants.ACCOUNT_ATTR.DAILY_MULTIPLIED_XP and vehicle.dailyXPFactor > 0:
                ...
                dailyXPText = text_styles.main(text_styles.expText((b'').join((b'x', xpFactor))))
...
class ExtendedVehicleInfoTooltipData(VehicleInfoTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        showDiscount = params.get(b'showDiscount', False)
        if showDiscount:
...
class BaseVehicleParametersTooltipData(ToolTipBaseData):
    ...
    def getDisplayableData(self, paramName, *args, **kwargs):
        ...
        return DecoratedTooltipWindow(tooltipView(paramName, self.context, self.readyForAdvanced(paramName)), useDecorator=False, parent=kwargs.get(b'parent'))
...
class DefaultCrewMemberTooltipData(BlocksTooltipData):
    ...
    def _packBlocks(self, tankmanID, slotIdx):
        ...
        mainRole = roles[0]
        bodyStr = (b'{}/{}').format(TOOLTIPS.VEHICLEPREVIEW_CREW, mainRole)
        ...
        if roles:
            rolesStr = (b', ').join([text_styles.stats(_ms(ITEM_TYPES.tankman_roles(r))) for r in roles])
...
class VehiclePreviewCrewMemberTooltipData(DefaultCrewMemberTooltipData):
    ...
    def _packBlocks(self, role, tankmanID, slotIdx, name, vehicleName, icon, description, skillsItems, *args, **kwargs):
        ...
        if skillsItems:
            ...
            if emptySkills:
                titleText = makeHtmlString(b'html_templates:lobby/textStyle', b'mainText', {b'message': (i18n.makeString(TOOLTIPS.VEHICLEPREVIEW_TANKMAN_NEWPERK_HEADER, quantity=emptySkills))})
...
class WotPlusBlockConstructor(VehicleTooltipBlockConstructor):
    ...
    def construct(self):
        ...
        if state is WotPlusState.CANCELLED:
            ...
            blocks.append(formatters.packTextParameterWithIconBlockData(name=text_styles.main(backport.text(R.strings.tooltips.vehicle.wotPlusRenting.remainingTime()) % {b'time': formattedTime}), value=b'', icon=ICON_TEXT_FRAMES.RENTALS, iconYOffset=2, gap=0, valueWidth=60, padding=formatters.packPadding(top=5, bottom=-15)))
        if state in [WotPlusState.CANCELLED, WotPlusState.ACTIVE]:
            ...
            if attrs & constants.ACCOUNT_ATTR.DAILY_MULTIPLIED_XP and self.vehicle.dailyXPFactor > 0:
                ...
                dailyXPText = text_styles.main(text_styles.expText((b'x{}').format(xpFactor)))
            ...
            blocks.append(formatters.packTextParameterWithIconBlockData(name=text_styles.main(TOOLTIPS.VEHICLE_WOTPLUSRENTING_INACTIVE), value=b'', icon=ICON_TEXT_FRAMES.RENTALS, iconYOffset=2, gap=0, valueWidth=60, padding=formatters.packPadding(left=0, top=0, bottom=-7)))
        blocks.append(formatters.packTextParameterBlockData(name=text_styles.stats(backport.text(R.strings.tooltips.vehicle.wotPlusRenting())), value=b'', valueWidth=0, padding=formatters.packPadding(left=0, top=0, bottom=10)))
        return (
         blocks, linkage)
...
class TelecomBlockConstructor(VehicleTooltipBlockConstructor):
    ...
    def construct(self):
        if self.vehicle.isTelecom:
            ...
            return [
             formatters.packTextBlockData(text=text_styles.main(backport.text(telecomTextRes(), tariff=backport.text(providerLocRes.tariff()) if providerLocRes else b'', provider=backport.text(providerLocRes.name()) if providerLocRes else b'')))]
        return []
...
class CommonStatsBlockConstructor(VehicleTooltipBlockConstructor):
    PARAMS = {(VEHICLE_CLASS_NAME.LIGHT_TANK): (
       ...
       (VEHICLE_CLASS_NAME.SPG): (b'avgDamage', b'stunMinDuration', b'stunMaxDuration', b'reloadTimeSecs', b'aimingTime', b'explosionRadius'), 
    ...
    __OVERRIDED_PARAM_NAMES = (
     (
      b'isTwinGunVehicle', {RELOAD_TIME_SECS_PROP_NAME: TWIN_GUN_RELOAD_TIME}),)
