...
class ReserveTooltipData(StrongholdTooltipData):
    BIG_ICON_SUFFIX = b'Big'
    BIG_GLOW_ICON_SUFFIX = b'BigGlow'
    ...
    def _packBlocks(self, *args):
        ...
        if reserve.intCD is None:
            _logger.error(b'%s intCD is None! Check wgsh version on staging.', reserveName)
    ...
    def __getHeaderBlock(self, item, name, level, count, isPlayerLegionary):
        ...
        countStr = (isPlayerLegionary or text_styles.main)(backport.text(R.strings.fortifications.reserves.tooltip.inStorage(), count=text_styles.expText(count))) if 1 else b''
        return formatters.packImageTextBlockData(title=text_styles.highTitle(item.shortUserName), desc=(b'\n').join([
         text_styles.neutral(backport.text(R.strings.fortifications.reserves.tooltip.level(), level=int2roman(level))),
         countStr]), img=backport.image(reserveIcon), imgPadding=formatters.packPadding(left=5, right=10), imgAtLeft=True, txtAlign=b'left', linkage=BLOCKS_TOOLTIP_TYPES.TOOLTIP_IMAGETEXT_BLOCK_LINKAGE, padding=formatters.packPadding(top=6), blockWidth=500)
    ...
    def __getAdditionalParamsBlock(self, reserve):
        block = [
         ...
         self.__packParameterBlock(reserve.getDescription(), b'+%s%%' % reserve.getBonusPercent(), b'')]
        return block
