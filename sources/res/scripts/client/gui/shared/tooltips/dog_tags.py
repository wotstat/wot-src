...
class DogTagInfoTooltip(BlocksTooltipData):
    ...
    def _packBlocks(self, compId, userID=None, withBackgroundId=None):
        ...
        if comp.viewType == ComponentViewType.ENGRAVING and withBackgroundId:
            typeLevelBlock = [
             text_styles.main(backport.text(R.strings.dogtags.customization.mergedTab()))]
        else:
            typeLevelBlock = [
        ...
        if comp.viewType == ComponentViewType.ENGRAVING and not comp.skipProgressInDescr:
            if comp.purpose != ComponentPurpose.STATIC:
                typeLevelBlock.append(text_styles.main(b', '))
                typeLevelBlock.append(text_styles.neutral((b'{} {}').format(backport.text(R.strings.dogtags.customization.tooltip.level()), componentProgress.grade + 1)))
            valueBlock.append(text_styles.main((b'{}: ').format(backport.text(R.strings.dogtags.customization.tooltip.value()))))
        ...
        if comp.viewType == ComponentViewType.BACKGROUND:
            ...
            images = [
             formatters.packImageBlockData(backgroundImg, padding=formatters.packPadding(top=self.__imagePaddingTop)),
