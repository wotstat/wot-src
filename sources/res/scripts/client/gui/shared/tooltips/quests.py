...
_RENT_TYPES = (b'rentDays', b'rentBattles', b'rentWins')
...
class QuestsPreviewTooltipData(BlocksTooltipData):
            ...
            if bonus.getName() == b'battleToken':
                ...
                if bonusFormat:
                    if isinstance(bonus, CustomizationsBonus):
                        ...
                        bonusNames.extend(bonusFormat.split(b', '))
    ...
    def _getBottom(self, value):
        ...
        return formatters.packTextBlockData(text=makeHtmlString(b'html_templates:lobby/textStyle', b'alignText', {b'align': b'center', 
           b'message': (formater((b'{0}{1}').format(icon, backport.text(tooltipText, count=value))))}), padding=formatters.packPadding(top=-10, bottom=10))

    def _packBattlePassRandomQuest(self, questName, isAvailable):
        blocks = []
        title = questName if isAvailable else (b'{} {}').format(icons.notAvailableRed(), questName)
        blocks.append(formatters.packImageTextBlockData(title=text_styles.middleTitle(title), desc=b'', imgPadding=formatters.packPadding(top=-13, left=-2), txtPadding=formatters.packPadding(top=-2), txtGap=6, padding=formatters.packPadding(bottom=15), txtOffset=20))
    ...
    def _packQuest(self, questName, bonuses, isAvailable):
        ...
        if bonusesLength > _MAX_BONUSES_PER_QUEST:
            ...
            formater = (b'{{}} {}').format(text_styles.stats(backport.text(R.strings.tooltips.hangar.header.quests.reward.rest(), count=bonusesLength - _MAX_BONUSES_PER_QUEST)))
        else:
            formater = b'{}'
        strBonus = (b', ').join(bonuses)
        title = questName if isAvailable else (b'{} {}').format(icons.notAvailableRed(), questName)
...
class ScheduleQuestTooltipData(BlocksTooltipData):
    ...
    def _getBody(self, eventID):
        ...
        if intervals:
            ...
            for low, high in intervals:
                times.append((b'{} - {}').format(backport.getShortTimeFormat(low), backport.getShortTimeFormat(high)))
    ...
    def _getSubBlock(self, header, items, padding=None):
        return formatters.packTitleDescBlock(title=text_styles.middleTitle(header), desc=text_styles.main((b', ').join(items)), padding=padding, descPadding=formatters.packPadding(left=20))
...
class UnavailableQuestTooltipData(BlocksTooltipData):
    ...
    def __getBootom(self, text):
        return formatters.packTextBlockData(text=makeHtmlString(b'html_templates:lobby/textStyle', b'alignText', {b'align': b'center', 
           b'message': (text_styles.error((b'{0} {1}').format(icons.notAvailable(), text)))}))

    def __getList(self, items):
        blocks = []
        for item in items:
            blocks.append(formatters.packTextParameterBlockData(name=text_styles.main(item.get(b'text')), value=text_styles.main(item.get(b'bullet', b'')), valueWidth=16))
    ...
    def __getNotVehicle(self):
        items = self.__getList([
         {b'text': (TOOLTIPS.QUESTS_UNAVAILABLE_VEHICLE_BODY), 
            b'bullet': (TOOLTIPS.QUESTS_UNAVAILABLE_BULLET)}])
    ...
    def __getRankedOverrides(self, quest):
        ...
        if body and header:
            ...
            return [
             formatters.packTextBlockData(text=text_styles.highTitle(header)),
             formatters.packBuildUpBlockData(self.__getList([{b'text': body, b'bullet': bullet}]), linkage=BLOCKS_TOOLTIP_TYPES.TOOLTIP_BUILDUP_BLOCK_WHITE_BG_LINKAGE),
...
class AdditionalAwardTooltipData(BlocksTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        for bonus in args:
            ...
            bonusName = bonusDict.get(b'name', b'')
            imgSource = bonusDict.get(b'imgSource', b'')
            label = bonusDict.get(b'label', b'')
            highlight = bonusDict.get(b'highlightIcon', b'')
            overlay = bonusDict.get(b'overlayIcon', b'')
            rendererData = {b'imgSource': imgSource, 
               b'label': label, 
               b'highlight': highlight, 
               b'overlay': overlay}
            if overlay:
                rendererType = b'EquipmentAwardItemExUI'
                padding = formatters.packPadding(top=-35, bottom=-30)
            else:
                rendererType = b'AwardItemExUI'
                padding = formatters.packPadding(top=-7, bottom=-12)
            items.append(formatters.packRendererTextBlockData(rendererType=rendererType, dataType=b'net.wg.gui.data.AwardItemVO', title=text_styles.main(bonusName), rendererData=rendererData, padding=padding, txtPadding=formatters.packPadding(top=15, left=15), titleAtMiddle=True))
...
class RentVehicleAwardTooltipData(BlocksTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        blocks = [
         formatters.packTextBlockData(text_styles.middleTitle(TOOLTIPS.QUESTS_AWARDS_VEHICLERENT_HEADER), padding=formatters.packPadding(top=8)),
         formatters.packTextParameterWithIconBlockData(name=text_styles.premiumVehicleName(TOOLTIPS.QUESTS_AWARDS_VEHICLERENT_EXPIRE), value=b'', icon=ICON_TEXT_FRAMES.RENTALS, padding=formatters.packPadding(left=-60), iconYOffset=3)]
        ...
        for rentVehicleData in args:
            ...
            vehicleName = rentVehicleData.get(b'vehicleName', b'')
            vehicleType = rentVehicleData.get(b'vehicleType', b'')
            isPremiumVehicle = rentVehicleData.get(b'isPremium', True)
            rentLeftCount = 0
            rentTypeName = None
