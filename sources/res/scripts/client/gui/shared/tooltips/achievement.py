...
class AchievementParamsField(ToolTipParameterField):
    ...
    def _getValue(self):
        ...
        if achievement is None:
            LOG_ERROR(b'There is error while building achievement tooltip', achievement)
            return result
        else:
...
class AchievementStatsField(ToolTipDataField):
    ...
    def _getValue(self):
        result = dict()
        achievement = self._tooltip.item
        if achievement and achievement.getType() == b'class':
            result[b'classParams'] = RECORD_CONFIGS.get(achievement.getName())
        return result
        ...
        if achievement.getRecordName() == MARK_OF_MASTERY_RECORD:
            if achievement.getCompDescr() is not None:
                if achievement.getPrevMarkOfMastery() < achievement.getMarkOfMastery():
                    records[b'current'] = makeString(b'#tooltips:achievement/newRecord')
                records[b'nearest'] = [
                 [
                  makeString(b'#tooltips:achievement/recordOnVehicle', vehicleName=self.itemsCache.items.getItemByCD(int(achievement.getCompDescr())).shortUserName),
            ...
            if achievement.getType() == b'series':
                vehicleRecords = set()
                    ...
                    if totalStats.isAchievementInLayout(achievement.getRecordName()):
                        ...
                        if vehAchieve and vehAchieve.getValue():
                            ...
                            if vehAchieve.getValue() == achievement.getValue():
                                records[b'current'] = vehicle.userName

                records[b'nearest'] = sorted(vehicleRecords, key=(lambda x: x[1]), reverse=True)[:3]
        return records
...
class AchievementTooltipData(ToolTipData):
    ...
    def __init__(self, context):
        ...
        self.fields = (
         ToolTipMethodField(self, b'name', b'getUserName'),
         ToolTipMethodField(self, b'icon', b'getHugeIcon'),
         ToolTipMethodField(self, b'type', b'getType'),
         ToolTipMethodField(self, b'section', b'getSection'),
         ToolTipMethodField(self, b'descr', b'getUserDescription'),
         ToolTipMethodField(self, b'showCondSeparator', b'getShowCondSeparator'),
         ToolTipMethodField(self, b'value', b'getValue'),
         ToolTipMethodField(self, b'localizedValue', b'getI18nValue'),
         ToolTipMethodField(self, b'historyDescr', b'getUserHeroInfo'),
         ToolTipMethodField(self, b'inactive', False, b'isActive'),
         ToolTipMethodField(self, b'condition', b'getUserCondition'),
         AchievementParamsField(self, b'params'),
         AchievementStatsField(self, b'stats'),
         AchievementIsInDossierField(self, b'isInDossier'),
         AchievementRecordsField(self, b'records'))
        return
...
class GlobalRatingTooltipData(ToolTipBaseData):
    ...
    def getDisplayableData(self, *args):
        return {b'name': (makeString(b'#achievements:globalRating')), 
           b'descr': (makeString(b'#achievements:globalRating_descr')), 
           b'isInDossier': True}
...
class BadgeTooltipData(BlocksTooltipData):
    ...
    def _packBlocks(self, badgeID):
        ...
        if badge is None:
            _logger.warning(b'Missing tooltip text for %r, please check badge.po', int(badgeID))
            return blocks
        else:
            tooltipData = [
