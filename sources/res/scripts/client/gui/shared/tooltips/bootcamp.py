...
class StatsTooltipData(BlocksTooltipData):
    ...
    def _packBlocks(self, label, description, icon):
        ...
        items.append(formatters.packBuildUpBlockData([
         ...
         formatters.packImageBlockData(img=icon, align=BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER),
...
class ProgressRewardTooltipData(BlocksTooltipData):
    ...
    def _packBlocks(self, label, description, icon, status=None, lesson=None, imgLeftPadding=50):
        ...
        blocks = [
         formatters.packTextBlockData(text_styles.highTitle(label)),
         formatters.packImageTextBlockData(img=icon, imgPadding=formatters.packPadding(left=imgLeftPadding)),
         formatters.packTextBlockData(text_styles.main(description))]
        if status:
