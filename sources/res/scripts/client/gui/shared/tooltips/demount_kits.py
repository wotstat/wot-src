    ...
    def _packBlocks(self, btnType=None, *args, **kwargs):
        ...
        if self._btnType is None:
            ...
            return formatters.packMoneyAndXpBlocks(tooltipBlocks, btnType=self._btnType, valueBlocks=[
             valueBlock], alternativeData={b'btnClickDesc': b'goldAlternative', b'btnClickDescAlign': (BLOCKS_TOOLTIP_TYPES.ALIGN_LEFT)})


class GoldStatsToolTipData(WGMCurrencyTooltip):

    def _getAlternativeData(self):
        return {b'btnClickDesc': b'goldAlternative', b'btnClickDescAlign': (BLOCKS_TOOLTIP_TYPES.ALIGN_LEFT)}
...
class NotEnoughMoneyTooltipData(ToolTipBaseData):
    ...
    def getDisplayableData(self, value, currencyType):
        header, body, icon = self.RESOURCES_BY_CURRENCY.get(currencyType, (b'', b'', b'', b''))
        formattedBody = makeHtmlString(b'html_templates:lobby/tooltips', b'not_enough_money_general', {b'description': (backport.text(body)), b'value': value, 
           b'icon': (backport.image(icon))})
        return {b'header': (backport.text(header)), 
           b'body': formattedBody}
