...
def packMapBoxBlockData(listIconSrc, columnWidth, rowHeight, linkage=BLOCKS_TOOLTIP_TYPES.TOOLTIP_TILE_LIST_BLOCK_LINKAGE, padding=None, verticalGap=0, horizontalGap=0, blockWidth=0):
    return packBlockDataItem(linkage, {b'dataType': b'net.wg.gui.lobby.hangar.mapBox.data.MapBoxItemVO', 
       ...
       b'rowHeight': rowHeight, 
       b'verticalGap': verticalGap, 
       b'horizontalGap': horizontalGap}, padding, blockWidth=blockWidth)
...
def packMoneyAndXpBlocks(tooltipBlocks, btnType, valueBlocks, alternativeData=None, hideActionBlock=False):
    ...
    titleKey = alternativeData.get(b'title', btnType)
    btnDescKey = alternativeData.get(b'btnDesc', btnType)
    btnClickDescKey = alternativeData.get(b'btnClickDesc', btnType)
    btnClickDescAlign = alternativeData.get(b'btnClickDescAlign', BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER)
    titleBlocks = [
     packTitleDescBlock(text_styles.highTitle(TOOLTIPS.getHeaderBtnTitle(titleKey)), None, padding=packPadding(bottom=15))]
    ...
    if not hideActionBlock and btnType != CURRENCIES_CONSTANTS.CRYSTAL:
        actionBlocks = [
         packAlignedTextBlockData(text=text_styles.standard(TOOLTIPS.getHeaderBtnClickDesc(btnClickDescKey)), align=btnClickDescAlign)]
