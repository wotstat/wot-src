...
class BlueprintTooltipData(BlocksTooltipData):
    ...
    def _getUnlockDiscountBlock(percentValue, xpValue, title, showPlus=False):
        ...
        percentStr = (b'').join((b'+' if showPlus else b'', str(percentValue), b'%'))
        discountValueStr = text_styles.concatStylesToSingleLine(text_styles.bonusLocalText(percentStr), text_styles.main(i18n.makeString(TOOLTIPS.VEHICLE_TEXTDELIMITER_OR).join((b'  ', b' '))), text_styles.expText(backport.getIntegralFormat(xpValue)), icons.xpCost())
...
class VehicleBlueprintTooltipData(BlueprintTooltipData, DynamicBlocksTooltipData):
    ...
    def _packBlocks(self, vehicleCD, isShortForm=False):
        ...
        if blueprintData is None:
            ...
            if self.__vehicle.isEarlyAccess:
                ...
                return [
                 formatters.packTitleDescBlock(title=title, desc=desc, gap=3, padding=formatters.packPadding(top=1, bottom=-10, left=-1, right=12))]
            self._setWidth(420)
    ...
    def __packGatherDescrBlock(self):
        ...
        alliance = self.__allianceName.replace(b'-', b'_')
    ...
    def __packStatusBlock(self):
        ...
        status = text_styles.alignText(status, b'center')
...
class BlueprintFragmentRandomTooltipData(BlueprintTooltipData):
    ...
    def __packRandomFragmentBlocks(self):
        self._items.append(formatters.packImageTextBlockData(title=text_styles.highTitle(self._getHeader()), img=RES_ICONS.getBlueprintFragment(b'small', b'random'), imgPadding=formatters.packPadding(top=3), txtPadding=formatters.packPadding(left=21)))
...
class BlueprintFragmentRandomNationalTooltipData(BlueprintTooltipData):
    ...
    def __packRandomFragmentBlocks(self):
        ...
        self._items.append(formatters.packTextBlockData(text=b'', padding=formatters.packPadding(bottom=-12)))
        return
...
class BlueprintFragmentTooltipData(BlueprintTooltipData):
    ...
    def __packIntelFragmentBlocks(self):
        self._items.append(formatters.packImageTextBlockData(title=text_styles.highTitle(TOOLTIPS.BLUEPRINT_BLUEPRINTFRAGMENTTOOLTIP_INTELFRAGMENT), img=RES_ICONS.getBlueprintFragment(b'small', b'intelligence'), imgPadding=formatters.packPadding(top=5), txtPadding=formatters.packPadding(top=5, left=19)))
    ...
    def __packVehicleFragmentBlocks(self):
        ...
        if blueprintData is None:
            ...
            gatheredInfoStr = text_styles.concatStylesWithSpace(text_styles.stats(int(blueprintData.filledCount)), text_styles.main((b' ').join((b'/', str(blueprintData.totalCount)))), text_styles.main(TOOLTIPS.BLUEPRINT_BLUEPRINTFRAGMENTTOOLTIP_FRAGMENTGATHERED))
...
class BlueprintEmptySlotTooltipData(BlueprintTooltipData):
    ...
    def _packBlocks(self, vehicleCD):
        ...
        statusStr = text_styles.alignText(statusStr, b'center')
    ...
    def __packDiscountBlock(self):
        fragmentBlock = formatters.packImageBlockData(img=RES_ICONS.getBlueprintFragment(b'small', b'vehicle'), padding=formatters.packPadding(top=4))
