...
class EpicSkillBaseTooltipData(BlocksTooltipData):
    ...
    def _constructHeader(self, skillLevel):
        block = [
         formatters.packImageTextBlockData(title=text_styles.highTitle(skillLevel.name), txtPadding=formatters.packPadding(left=19), img=backport.image(R.images.gui.maps.icons.epicBattles.skills.c_80x80.dyn(skillLevel.icon)()), titleAtMiddle=True)]
        return block
    ...
    def __constructStatsBlock(self, skillInfo):
        ...
        if staticBlock:
            staticBlocks.append(formatters.packTextBlockData(text=text_styles.middleTitle((b'{}{}').format(backport.text(R.strings.epic_battle.abilityInfo.properties.static()), backport.text(R.strings.common.common.colon()))), padding=formatters.packPadding(bottom=5)))
            staticBlocks.extend(staticBlock)
            blocks.append(staticBlocks)
        if dynamicBlock:
            dynamicBlocks.append(formatters.packTextBlockData(text=text_styles.middleTitle((b'{}{}').format(backport.text(R.strings.epic_battle.abilityInfo.properties.dynamic()), backport.text(R.strings.common.common.colon()))), padding=formatters.packPadding(bottom=10)))
