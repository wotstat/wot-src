     ...
     __processElen]:
        try:
            if BigWorld.player():
                ...
                te = time.time()
                rt = te - ts
                _logger.info(b'%s elapsed time: %s sec', func.__name__, rt)
                if not result:
                    break
                yield future_async.resignTickIfRequired(0.0)
            else:
                _logger.warn(b'onAccountShowGUI(): %s has been called for an already deleted PlayerAccount object.', func.__name__)
                break
        except future_async.BrokenPromiseError:
            _logger.debug(b'%s has been destroyed without user decision', func.__name__)
            break
...
def __notifyOnSyncComplete(ctx, callback=None):
    ...
    if not playerRef():
        _logger.warn(b'onSyncCompleted(): the item cache update callback has been called for an already deleted PlayerAccount object.')
        callback(False)
        return
