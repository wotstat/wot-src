...
__all__ = (b'LeavePrbModalEntity', b'DisconnectFromPeriphery', b'ConnectToPeriphery', b'PrbInvitesInit', b'ActionsChain')
...
class LeavePrbModalEntity(Action):
            ...
            if state.hasModalEntity:
                ...
                if factory:
                    ...
                    if ctx:
                        ...
                        LOG_ERROR(b'Leave modal entity. Can not create leave ctx', state)
                else:
                    LOG_ERROR(b'Leave modal entity. Factory is not found', state)
            else:
                LOG_DEBUG(b'Leave modal entity. Player has not prebattle')
    ...
    def __doLeave(self, dispatcher, ctx):
        ...
        if self._completed:
            LOG_DEBUG(b'Leave modal entity. Player left prebattle.')
        else:
            LOG_DEBUG(b'Leave modal entity. Action was failed.')
        self._running = False
        return
...
class LeavePrbEntity(Action):
        ...
        if dispatcher:
            ...
            if factory:
                ...
                if ctx:
                    ...
                    LOG_ERROR(b'Leave prb entity. Can not create leave ctx', state)
            else:
                LOG_ERROR(b'Leave prb entity. Factory is not found', state)
        return
    ...
    def __doLeave(self, dispatcher, ctx):
        self._completed = yield dispatcher.leave(ctx)
        if self._completed:
            LOG_DEBUG(b'Leave prb entity. Player left prebattle.')
        else:
            LOG_DEBUG(b'Leave prb entity. Action was failed.')
        self._running = False
        return
...
class SelectPrb(Action):
    ...
    def __doSelect(self, dispatcher):
        ...
        if self._completed:
            LOG_DEBUG(b'Select prebattle entity. Player has joined prebattle.')
        else:
            LOG_DEBUG(b'Select prebattle entity. Action was failed.')
        self._running = False
        return
    ...
    def isRunning(self):
        ...
        if app:
            ...
            if view and view.settings.alias == VIEW_ALIAS.LOGIN and view.isCreated() and self.connectionMgr.isDisconnected():
                LOG_DEBUG(b'Disconnect action. Player came to login')
                self._completed = True
                self._running = False
        return self._running
...
class ConnectToPeriphery(Action):
    ...
    def invoke(self):
        if self.__host and self.__credentials:
            if len(self.__credentials) < 2:
                ...
                LOG_ERROR(b'Connect action. Login info is invalid')
                return
    ...
    def __onLoginQueueClosed(self, _):
        ...
        self._completed = False
        self._running = False
        LOG_DEBUG(b'Connect action. Player exit from login queue')
        return
...
class PrbInvitesInit(Action):
    ...
    def invoke(self):
        ...
        if invitesManager:
            if invitesManager.isInited():
                LOG_DEBUG(b'Invites init action. Invites init action. List of invites is build')
                self._completed = True
            else:
                self._running = True
                invitesManager.onInvitesListInited += self.__onInvitesListInited
        else:
            LOG_ERROR(b'Invites init action. Invites manager not found')
            self._completed = False
        return
    ...
    def __onInvitesListInited(self):
        ...
        invitesManager = g_prbLoader.getInvitesManager()
        if invitesManager:
            LOG_DEBUG(b'Invites init action. List of invites is build')
            invitesManager.onInvitesListInited -= self.__onInvitesListInited
        else:
            LOG_ERROR(b'Invites manager not found')
        self._completed = True
        self._running = False
        return
...
class WaitFlagActivation(Action):
    ...
    def activate(self):
        LOG_DEBUG(b'Flag is activated')
        self._isActive = True
        return

    def inactivate(self):
        LOG_DEBUG(b'Flag is inactivated')
        self._isActive = False
        return
