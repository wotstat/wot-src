...
__all__ = (b'icons', b'text_styles', b'time_formatters')
...
def _getFormattedPrice(price, isBuying, checkGold):
    ...
    fmtCurrency = {currencyName: b'' for currencyName in Currency.ALL}
    ...
    for currencyName in fmtCurrency:
        ...
        if currencyValue is not None:
            fmtCurrency[currencyName] = makeHtmlString(template, currencyName + postfix, {b'value': (format_(currencyValue))})
...
def formatPrice(price, reverse=False, currency=Currency.CREDITS, useIcon=False, useStyle=False, ignoreZeros=False, justValue=False):
    ...
    for c in currencies:
        ...
        if useIcon:
            cIdentifier = makeHtmlString(b'html_templates:lobby/iconText', c)
            cSpace = b' ' if reverse else b''
        elif justValue:
            cIdentifier = b''
            cSpace = b''
        else:
            cIdentifier = makeString((b'#menu:price/{}').format(c))
            cSpace = b' ' if reverse else b': '
        outPrice.append((b'').join((cFormatted, cSpace, cIdentifier) if reverse else (
         cIdentifier, cSpace, cFormatted)))

    return (b', ').join(outPrice)
...
def moneyWithIcon(money, currType=None):
    ...
    if style is not None and icon is not None and value is not None:
        ...
        _logger.error(b'Unsupported currency for displaying with icon: %s', currType)
...
def getItemPricesVO(*itemPrices):
    ...
    for itemPrice in itemPrices:
        ...
        if action.isDefined():
            vo = {b'price': (getMoneyVO(itemPrice.price)), 
               b'defPrice': (getMoneyVO(itemPrice.defPrice)), 
               b'action': (getMoneyVO(action))}
            resultVO.append(vo)
        else:
            resultVO.append({b'price': (getMoneyVO(itemPrice.price))})
...
def updateActionInViewModel(currency, actionPriceModel, itemPrice):
    ...
    setNewMethodName = b'setNew' + currency.title()
    setOldMethodName = b'setOld' + currency.title()
    if hasattr(actionPriceModel, setNewMethodName):
        ...
        _logger.error(b"ActionPriceModel doesn't support method %s", setNewMethodName)
    if hasattr(actionPriceModel, setOldMethodName):
        ...
        _logger.error(b"ActionPriceModel doesn't support method %s", setOldMethodName)
    return
...
def getItemPricesVOWithReason(reason, *itemPrices):
    ...
    for itemPrice in itemPrices:
        action = itemPrice.getActionPrcAsMoney()
        if action.isDefined():
            vo = {b'price': (getMoneyVOWithReason(reason, itemPrice.price)), 
               b'defPrice': (getMoneyVO(itemPrice.defPrice)), 
               b'action': (getMoneyVO(action))}
            resultVO.append(vo)
        else:
            resultVO.append({b'price': (getMoneyVOWithReason(reason, itemPrice.price))})
...
def getUnlockDiscountXpVO(unlockProps, xpType):
    return {b'cost': (unlockProps.xpCost), 
       b'fullCost': (unlockProps.xpFullCost), 
       b'xpType': xpType}


def getItemSellPricesVO(sellCurrency, *sellPrices):
    return [{b'price': (getMoneyVO(sellPrice) or ((sellCurrency, 0),))} for sellPrice in sellPrices]


def getItemRestorePricesVO(*prices):
    return [{b'price': (getMoneyVO(price))} for price in prices]
...
def formatPurchaseItems(purchaseItems):
    ...
    for item in items:
        ...
        if item.itemTypeID == GUI_ITEM_TYPE.STYLE and item.isProgression:
            ...
            ctx = {b'styleName': (item.userName), 
               b'level': (int2roman(c11nService.getCurrentProgressionStyleLevel()))}
            resource = R.strings.messenger.serviceChannelMessages.sysMsg.customization.item.progressionStyle
        else:
            ctx = {b'itemType': (item.userType), 
...
def getRoleTextWithIcon(role, roleLabel):
    if role:
        return text_styles.concatStylesToSingleLine(getRoleIcon(roleLabel), makeHtmlString(b'html_templates:vehicleRoles', b'roleMain', {b'message': (getRoleText(roleLabel))}))
    return b''
...
def getRoleTextWithLabel(role, roleLabel):
    ...
    if role:
        ...
        roleStr = text_styles.main((b'').join([roleText, getRoleText(roleLabel)]))
    return roleStr
