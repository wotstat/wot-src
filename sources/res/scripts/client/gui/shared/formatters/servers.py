...
def formatPingStatus(csisStatus, isColorBlind, isSelected, pingStatus, pingValue, useBigSize=False):
    if pingStatus == PING_STATUSES.REQUESTED:
        ...
        if isColorBlind and pingStatus == PING_STATUSES.HIGH:
            colorBlindName = b'_color_blind'
        pingStatusIcon = makePingStatusIcon(pingStatus, colorBlindName)
        if useBigSize:
            return text_styles.concatStylesToSingleLine(text_styles.main(b' '), formattedPing, pingStatusIcon)
        return text_styles.concatStylesToSingleLine(formattedPing, b'', pingStatusIcon)


def makePingStatusIcon(pingStatus, colorBlindName=b''):
    icon = backport.image(R.images.gui.maps.icons.pingStatus.dyn((b'stairs_indicator_{}{}').format(pingStatus, colorBlindName), R.invalid)())
