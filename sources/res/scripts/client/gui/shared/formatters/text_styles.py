...
__all__ = (b'standard', b'main', b'mainBig', b'neutral', b'stats', b'statInfo', b'statusAttention', b'statusAlert', b'statusAttention', b'middleTitle', b'highTitle', b'highTitleAccented', b'highTitleDisabled', b'disabled', b'promoTitle', b'promoSubTitle', b'alert', b'alertBig', b'success', b'statsIncrease', b'error', b'warning', b'critical', b'expText', b'statsDecrease', b'expTextBig', b'gold', b'credits', b'crystal', b'defRes', b'counter', b'titleFont', b'tutorial', b'playerOnline', b'getRawStyles', b'getStyles', b'concatStylesToSingleLine', b'concatStylesToMultiLine', b'superPromoTitle', b'superPromoTitleEm', b'highlightText', b'unavailable', b'missionStatusAvailable', b'epicTitle', b'epicTitleYellow', b'heroTitle', b'heroTitleYellow', b'heroTitleTK', b'grandTitle', b'grandTitleYellow', b'grandTitleTK', b'grandTitleRed', b'textEpic', b'yellowText', b'greenText', b'poiCapturedBoldText', b'poiCapturedRegularText', b'prestige', b'statusSimple', b'statusBright', b'cream', b'creamBold', b'creamTitle', b'lightGray', b'lightBlue', b'bptaler')
...
def linkText(text):
    return _formatText(b'linkText', text)


def earningsInformation(text):
    return _formatText(b'earningsInformation', text)


def goldColor(text):
    return _formatText(b'goldColor', text)
...
def stPatrick(text):
    return _formatText(b'stPatrickText', text)
...
def discountSmallText(text):
    return _formatText(b'discountSmallText', text)
...
def prestige(text):
    return _formatText(b'prestigeText', text)


def statusSimple(text):
    return _formatText(b'statusSimpleText', text)


def statusBright(text):
    return _formatText(b'statusBrightText', text)


def cream(text):
    return _formatText(b'cream', text)


def creamBold(text):
    return _formatText(b'creamBold', text)


def creamTitle(text):
    return _formatText(b'creamTitle', text)


def lightGray(text):
    return _formatText(b'lightGray', text)


def lightBlue(text):
    return _formatText(b'lightBlue', text)


def bptaler(text):
    return _formatText(b'bptalerText', text)
...
def formatStyledText(text):
    styledTextPattern = b'{\\w+_(?:Open|Close)}'
    stylePattern = b'{(\\w+)_\\w+}'
