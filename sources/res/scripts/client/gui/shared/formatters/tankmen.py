...
def formatDeletedTankmanStr(tankman, itemsCache=None):
    ...
    return tankman.fullUserName + b' (%s, %s)' % (tankman.roleUserName, vehicle.userName)
        ...
        if itemPrice.isDefined():
            for currency in statsMoney.currencies:
                ...
                if currencyValue is not None:
                    ...
                    if not isEnough and b'exchangeRate' in kwargs and currency == Currency.CREDITS:
                        isEnough = canBuyWithGoldExchange(itemPrice.price, statsMoney, kwargs.get(b'exchangeRate'))
                    ...
                    if b'isBootcamp' in kwargs:
                        actionPriceModel.setIsBootcamp(kwargs.get(b'isBootcamp'))
