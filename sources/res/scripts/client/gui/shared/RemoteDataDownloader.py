...
class _HttpConnectionWorker(threads.Worker):
    ...
    def closeConnection(self):
        if self.__conn is not None:
            self.__conn.close()
            self.__conn = None
            LOG_DEBUG(b'Http connection has been closed', self.__conn)
        return
    ...
    def getConnection(self):
        ...
        if self.__conn is None:
            ...
            LOG_DEBUG(b'Http connection creating', self.__host, self.__conn)
        return self.__conn
...
class _LocalCache(local_cache.ShelfLocalCache):
    ...
    class _Record(namedtuple(b'_Record', [
     b'data', b'lastModified', b'expires', b'requestTime'])):

        def __repr__(self):
            return b'_Record(data=%s, lastModified=%s, expires=%s, requestTime=%s)' % (
             type(self.data), self.lastModified, self.expires, self.requestTime)

    def __init__(self):
        super(_LocalCache, self).__init__(b'external_cache', (b'data',), autoflush=60)
        return
...
class _RemoteDataDownloader(object):
    ...
    def __init__(self):
        ...
        self._pools = {b'clans': (_HttpHostDownloader(b'ce-ct.worldoftanks.net', 3)), 
           b'custom': (threads.ThreadPool(workersLimit=2))}
        return
    ...
    def getClanVehicleDecal(self, clanDbID, callback):
        return self.__getRemoteFileByConnection(b'clans', b'/dcont/clans/emblems/%d/emblem_64x64_tank.png' % clanDbID, callback)

    def getClanEmblem32x32(self, clanDbID, callback):
        return self.__getRemoteFileByConnection(b'clans', b'/dcont/clans/emblems/%d/emblem_32x32.png' % clanDbID, callback)

    def getClanEmblem64x64(self, clanDbID, callback):
        return self.__getRemoteFileByConnection(b'clans', b'/dcont/clans/emblems/%d/emblem_64x64.png' % clanDbID, callback)

    def getUrlData(self, url, callback):
        return self.__getRemoteFile(b'custom', url, callback, _HttpOpenUrlJob)
