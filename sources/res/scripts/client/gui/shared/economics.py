...
def getActionPrc(price, defaultPrice):
    ...
    def calculate(price, defaultPrice):
        ...
        if defaultPrice in (0, price):
            return 0
        return round_py2_style_int((1 - float(price) / defaultPrice) * 100)
    ...
    if proxy is not None and vehicle.isRentable:
        ...
        for rentType in sorted(filteredRentPackages):
            ...
            for packageID in sorted(costsOfCurrentType):
                ...
                result.append({b'rentID': (makeRentID(rentType, packageID)), 
                   b'rentPrice': rentPrice, 
                   b'defaultRentPrice': defaultRentPrice, 
                   b'seasonType': (currentPackage.get(b'seasonType', None))})
