...
class HangarSpace(IHangarSpace):
    ...
    def updateVehicle(self, vehicle, outfit=None):
        _logger.info(b'updateVehicle: vehicle(%s)', vehicle.name)
        if self.__inited:
            self.__isModelLoaded = False
    ...
    def __logStatistics(self):
        ...
        if player is not None:
            LOG_DEBUG_DEV(stats)
            if stats[b'system'] and hasattr(player, b'logClientSystem'):
                player.logClientSystem(stats[b'system'])
            if stats[b'session'] and hasattr(player, b'logClientSessionStats'):
                player.logClientSessionStats(stats[b'session'])
        return
