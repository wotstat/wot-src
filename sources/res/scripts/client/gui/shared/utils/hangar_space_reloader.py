...
class HangarSpaceReloader(IHangarSpaceReloader):
    ...
    def changeHangarSpace(self, spaceName, visibilityMask, waitingMessage=None, backgroundImage=None, actionChange=False, force=False):
        ...
        if not spaceName:
            _logger.error(b'Invalid space name: the name cannot be empty.')
            return (
             False, ErrorFlags.INVALID_NAME)
        else:
            reloadValid = True
            if force and self.hangarSpace.spaceLoading():
                _logger.info(b'Stop loading space "%s" to load another space "%s"', self.__loadingSpacePath, spaceName)
                self.hangarSpace.destroy()
            elif self.__loadingSpacePath is not None:
                _logger.error(b'Failed to load space "%s", because another space is loading: %s', spaceName, self.__loadingSpacePath)
                reloadValid = False
                errCode |= ErrorFlags.ALREADY_PROCESSING_RELOAD
            if not self.hangarSpace.spaceInited:
                _logger.error(b'Failed to load space "%s", because hangar is not inited.', spaceName)
                reloadValid = False
                errCode |= ErrorFlags.HANGAR_NOT_READY
            if self.hangarSpace.spaceLoading():
                _logger.error(b'Failed to load space "%s", because another space is loading.', spaceName)
                reloadValid = False
            ...
            if self.hangarSpace.spaceInited and hangarSpacePath is None:
                _logger.error(b'Abnormal behaviour: hangarSpace.spacePath is not initialized')
                reloadValid = False
                errCode |= ErrorFlags.SPACE_PATH_NOT_INITED
            spacePath = self.buildHangarSpacePath(spaceName)
            if spacePath == hangarSpacePath:
                _logger.warning(b'No need to load space "%s", because it is already loaded', spaceName)
                reloadValid = False
    ...
    def buildHangarSpacePath(spaceId):
        if spaceId.startswith(b'space'):
            return spaceId
        return (b'spaces/{}').format(spaceId)
