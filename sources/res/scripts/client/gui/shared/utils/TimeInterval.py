...
class TimeInterval(object):
    __slots__ = (b'__callbackID', b'__interval', b'__obj', b'__name')
    ...
    def start(self):
        if self.__callbackID is not None:
            _logger.error(b'To start a new time interval You should before stop already the running time interval.')
            return
        else:
...
class TimeIntervalEvent(SafeEvent):
    __slots__ = (b'_timeCallback', b'__timeInterval')
    ...
    def __init__(self, interval, timeCallback=None, manager=None):
        ...
        self.__timeInterval = TimeInterval(interval, self, b'_timeCallback')
        return
