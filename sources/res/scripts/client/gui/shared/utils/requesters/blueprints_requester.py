...
_BlueprintData = namedtuple(b'BlueprintData', (b'filledCount', b'totalCount', b'canConvert'))
...
class BlueprintsRequester(AbstractSyncDataRequester, IBlueprintsRequester):
    ...
    def getBlueprintCount(self, vehicleCD, vLevel):
        _logger.debug(b'Extract blueprint count for the vehicle=%s, level=%s ', vehicleCD, vLevel)
    ...
    def getBlueprintData(self, vehicleCD, vLevel):
        _logger.debug(b'Extract blueprint data for the vehicle=%s, level=%s ', vehicleCD, vLevel)
    ...
    def getBlueprintDiscount(self, vehicleCD, vLevel, potentialFilledCount=0):
        _logger.debug(b'getBlueprintDiscount: vehicle=%s, level=%s ', vehicleCD, vLevel)
    ...
    def getFragmentDiscountAndCost(self, vehicleCD, vLevel, xpFullCost):
        ...
        return (
         fragmentDiscount, round_py2_style_int(xpFullCost * fragmentDiscount * 0.01))
    ...
    def calculateCost(self, oldCost, discount):
        if not discount:
            return oldCost
        return round_py2_style_int(oldCost * (100 - discount) * 0.01)
    ...
    def _preprocessValidData(self, data):
        _logger.debug(b'Preprocess blueprint cache')
        if data is None or b'cache' not in data:
            return data
        self.clear()
        blueprintsDecodeData = self.__bpfDecoder(data[b'cache'])
