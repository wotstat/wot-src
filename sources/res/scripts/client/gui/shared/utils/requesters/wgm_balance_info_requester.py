...
_ACTION_PURCHASED = b'_purchased'
_ACTION_EARNED = b'_earned'
...
class WGMBalanceInfoRequester(object):
    ...
    def __cooldownCallback(self, callback):
        if isPlayerAccount():
            ...
            self.__request = None
            LOG_WARNING(b'The player is no longer an Account, the request to WGM is skipped')
        return
    ...
    def __processResponse(self, callback, resultID, errorStr, ext):
        ...
        if not AccountCommands.isCodeValid(resultID):
            LOG_WARNING((b'WGM Balance info is not received, resultID: {}, errorStr: {}').format(resultID, errorStr))
            ...
            for item in ext:
                if self.__checkFields(item):
                    ...
                    LOG_WARNING(b'Error parsing wgm response, row is: ', item)
