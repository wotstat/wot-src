...
class AbstractRequester(object):
    ...
    @wg_async
    def request(self):
        _logger.debug(b'Prepare requester %s', self.__class__.__name__)
        self.__synced = False
        if not isPlayerAccount():
            _logger.error(b'Player is not account, requester %s can not be invoked.', self.__class__.__name__)
        else:
            _logger.debug(b'Invoke requester %s', self.__class__.__name__)
    ...
    def _response(self, resID, value, callback=None):
        _logger.debug(b'Response %r is received for requester %s', resID, self.__class__.__name__)
        self.__synced = resID >= 0
        if resID < 0:
            _logger.error(b'There is error while getting data from cache: %r, %r, %r', self.__class__.__name__, code2str(resID), resID)
...
@ReprInjector.simple((b'getWaitingID', b'waitingID'), (b'getRequestType', b'requestType'))
class RequestCtx(object):
    __slots__ = (b'_waitingID', b'_callback')
    ...
    def setWaitingID(self, waitingID):
        if not self.isProcessing():
            ...
            _logger.error(b'In processing: %r', self)
        return
    ...
    def onResponseReceived(self, code):
        if code < 0:
            _logger.error(b'Server return error for request: %r, %r', code, self)
...
class RequestsByIDProcessor(object):
    ...
    def stopProcessing(self, resCode=RES_SUCCESS):
        while self._requests:
            ...
            data[0].stopProcessing(self._makeResponse(resCode, b'stop processing', ctx=ctx))
    ...
    def doRequestEx(self, ctx, callback, methodName, *args, **kwargs):
        ...
        if result:
            ...
            self._stopProcessing(ctx, b'request failure', callback)
        return result
    ...
    def doRawRequest(self, methodName, *args, **kwargs):
        ...
        if callable(method):
            ...
            _logger.error(b'Name of method is invalid: %r', methodName)
        return result
    ...
    def _sendRequest(self, ctx, methodName, chain, *args, **kwargs):
        ...
        if not requester:
            _logger.warning(b'There is not sender is present: %r', self)
            return (
             result, requestID)
        ...
        if callable(method):
            ...
            if requestID > 0:
                ...
                _logger.error(b'Request ID can not be null')
        else:
            _logger.error(b'Name of method is invalid: %r', methodName)
    ...
    def _makeResponse(self, code=0, txtMsg=b'', data=None, ctx=None):
...
_Response = namedtuple(b'_Response', [
 28, 29, 30, 31, 32])
_Response.__new__.__defaults__ = (0, b'', None, 0, None)
...
class ClientRequestsByIDProcessor(RequestsByIDProcessor):
    ...
    def _makeResponse(self, code=0, txtMsg=b'', data=None, ctx=None, extraCode=0, headers=None):
        response = self.__responseClass(code, txtMsg, data, extraCode, headers)
        if not response.isSuccess():
            _logger.warning(b'Client request error: %r, %r', ctx, response)
        return response
