...
class TokenRequester(object):
    ...
    def cancelRequest(self):
        ...
        if self.__callback is not None:
            _logger.info(b'Token request %s cancelled', requestId)
    ...
    def request(self, timeout=None, callback=None, allowDelay=False):
        ...
        if requester != self._getRequester():
            _logger.warning(b'Request cancelled because requester has been changed')
            if callback:
                callback(None)
            return
    ...
    def __onTimeout(self, requestID, tokenType):
        self.__clearTimeoutCb()
        self._onTokenReceived(requestID, tokenType, cPickle.dumps({b'error': b'TIMEOUT'}, -1))
        return
