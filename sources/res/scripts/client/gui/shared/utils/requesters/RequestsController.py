...
class RequestsController(object):
    ...
    def request(self, ctx, callback=(lambda *args: None), allowDelay=None):
        LOG_DEBUG(b'Send server request', self.__class__.__name__, ctx, callback, allowDelay)
        ...
        if handler:
            ...
            if not allowDelay:
                if self._cooldowns.validate(requestType, cooldown):
                    self.__doRequestError(ctx, b'cooldown', callback)
                else:
                    _doRequest()
            elif not self.__singulizer.register(ctx, callback):
                self.__rqQueue.append((requestType, ctx, _doRequest))
            self.__doNextRequest()
        else:
            self.__doRequestError(ctx, b'handler not found', callback)
        return
    ...
    def __onTimeout(self, cb, requestType, ctx):
        LOG_ERROR(b'Request timed out', self, requestType, ctx)
        self.__doRequestError(ctx, b'time out', cb)
        return
...
class RequestSingulizer(object):
    ...
    def register(self, ctx, callback):
        ...
        if key is not None:
            if key in self.__requestCallbacks:
                LOG_DEBUG(b'Adding consequent callback', key)
                self.__requestCallbacks[key].append(callback)
                return True
            LOG_DEBUG(b'Adding first callback', key)
    ...
    def processResult(self, ctx, args):
        key = ctx.getSingulizerKey()
        if key in self.__requestCallbacks:
            LOG_DEBUG(b'Retrieved results for singularized request', key, b', total saved calls:', len(self.__requestCallbacks[key]))
