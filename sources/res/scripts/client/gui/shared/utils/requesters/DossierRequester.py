...
class UserDossier(object):
    ...
    def __requestPlayerInfo(self, callback):
        ...
        def proxyCallback(value):
            if value is not None and len(value) > 1:
                ...
                self.__cache[b'serviceRecordCustomization'] = value[11]
    ...
    def getServiceRecordCustomization(self, callback):
        if not self.isValid:
            ...
            if self.__cache.get(b'serviceRecordCustomization') is None:
                self.__requestPlayerInfo(callback)
                return
            callback(self.__cache[b'serviceRecordCustomization'])
            return
