...
class VehicleRestoreInfo(_VehicleRestoreInfo):
    ...
    def isFinite(self):
        return (self.restoreType == RESTORE_VEHICLE_TYPE.PREMIUM) and 0 < self.getRestoreTimeLeft() < float(b'inf')
    ...
    def isUnlimited(self):
        return self.restoreType == RESTORE_VEHICLE_TYPE.ACTION and self.getRestoreCooldownTimeLeft() == 0 or self.restoreType == RESTORE_VEHICLE_TYPE.PREMIUM and self.getRestoreTimeLeft() == float(b'inf')
...
class RecycleBinRequester(AbstractSyncDataRequester, IRecycleBinRequester):
    ...
    def getOptDevices(self):
        return self.recycleBin.get(b'optional_devices', {})
