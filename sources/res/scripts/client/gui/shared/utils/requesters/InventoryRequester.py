...
class InventoryRequester(AbstractSyncDataRequester, IInventoryRequester):
    VEH_DATA = namedtuple(b'VEH_DATA', (b'compDescr', b'descriptor', b'invID', b'crew'))
    ITEM_DATA = namedtuple(b'ITEM_DATA', (b'compDescr', b'descriptor', b'count'))
    TMAN_DATA = namedtuple(b'TMAN_DATA', (b'compDescr', b'descriptor', b'vehicle', b'invID'))
    OUTFIT_DATA = namedtuple(b'OUTFIT_DATA', (b'compDescr', b'flags'))
    CUSTOMIZATION_PROGRESS_DATA = namedtuple(b'CUSTOMIZATION_PROGRESS_DATA', (b'currentLevel', b'currentProgressOnLevel', b'maxProgressOnLevel'))
        ...
        for cType, itemsData in newItemsInvData.iteritems():
            for itemId, itemData in itemsData.iteritems():
                if itemData is not None:
                    intCD = makeIntCompactDescrByID(b'customizationItem', cType, itemId)
    ...
    def updateC11nProgressionData(self):
        ...
        for cType, typeInvData in itemsInvData.iteritems():
            for idx, itemData in typeInvData.iteritems():
                itemIntCD = makeIntCompactDescrByID(b'customizationItem', cType, idx)
                self.__updateC11nProgressionDataForItem(itemIntCD, itemData)
    ...
    def getC11nOutfitsFromPool(self, vehicleIntCD):
        path = (
         GUI_ITEM_TYPE.CUSTOMIZATION, CustomizationInvData.OUTFITS_POOL, vehicleIntCD)
    ...
    def getItemsData(self, itemTypeID):
        ...
        for invID in invData.get(b'compDescr', {}).iterkeys():
    ...
    def getC11nSerialNumber(self, itemCD):
        _, itemType, itemID = parseIntCompactDescr(itemCD)
        path = (GUI_ITEM_TYPE.CUSTOMIZATION, CustomizationInvData.SERIAL_NUMBERS, itemType, itemID, b'serial_number')
        return self.getCacheValueByPath(path)

    def getFreeSlots(self, vehiclesSlots):

        def checker(vehData):
            flag = vehData.get(b'extraSettings', 0)
    ...
    def getInventoryEnhancements(self):
        return self.getCacheValue(b'enhancements', {})

    def getInstalledEnhancements(self):
        return self.getCacheValue(GUI_ITEM_TYPE.VEHICLE, {}).get(b'enhancements', {})
    ...
    def getDynSlotTypeID(self, vehIntCD):
        return self.getCacheValue(GUI_ITEM_TYPE.VEHICLE, {}).get(b'customRoleSlots', {}).get(vehIntCD, 0)
    ...
    def _response(self, resID, invData, callback=None):
        ...
        if invData is not None:
            for invID, vCompDescr in invData[GUI_ITEM_TYPE.VEHICLE][b'compDescr'].iteritems():
                self.__vehsCDsByID[invID] = vehicles.makeIntCompactDescrByID(b'vehicle', *vehicles.parseVehicleCompactDescr(vCompDescr))
    ...
    def __makeVehicle(self, vehInvID):
        if vehInvID not in self.__vehsCDsByID:
            ...
            compactDescr = value(b'compDescr')
            if compactDescr is None:
                return
            try:
                item = cache[typeCompDescr] = self.VEH_DATA(value(b'compDescr'), vehicles.VehicleDescr(compactDescr=compactDescr), vehInvID, value(b'crew', []))
            except Exception:
                LOG_DEBUG(b'Error while building vehicle from inventory', vehInvID, typeCompDescr)
                return
    ...
    def __makeTankman(self, tmanInvID):
        ...
        if tmanInvID in cache:
            ...
            if compactDescr is None:
                return
            item = cache[tmanInvID] = self.TMAN_DATA(compactDescr, tankmen.TankmanDescr(compactDescr), value(b'vehicle', VEHICLE_NO_INV_ID), tmanInvID)
            return item
    ...
    def __getTankmenData(self, inventoryID=None):
        ...
        if tankmanItemsData is None:
            ...
            result = dict()
            for invID in tankmanItemsData.get(b'compDescr', dict()).iterkeys():
                result[invID] = self.__getTankmanData(tankmanItemsData, invID)

            return result

    def __getTankmanData(self, tankmanItemsData, invID):
        if invID not in tankmanItemsData[b'compDescr'].keys():
            return
        else:
            result = dict()
    ...
    def __getVehiclesData(self, inventoryID=None):
        ...
        if vehItemsData is None:
            ...
            result = dict()
            for invID in vehItemsData.get(b'compDescr', dict()).iterkeys():
                result[invID] = self.__getVehicleData(vehItemsData, invID)

            return result

    def __getVehicleData(self, vehItemsData, invID):
        if invID not in vehItemsData[b'compDescr'].keys():
            return
        else:
            result = dict()
