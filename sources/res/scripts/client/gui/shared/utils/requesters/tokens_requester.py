...
class TokensRequester(AbstractSyncDataRequester, ITokensRequester):
    ...
    def getLootBoxesInfo(self):
        return self.getCacheValue(b'lootBoxes', {})
    ...
    def _requestCache(self, callback):
        ...
        if b'tokens' in result:
            ...
            self.__updateLootBoxes(result[b'tokens'], result.get(b'lootBoxes', {}))
        callback(result)
        return
    ...
    def __updateLootBoxes(self, tokensCache, lootboxes):
        for lootBoxTokenID, data in tokensCache.items():
            ...
            if lootBoxTokenID in self.__lootBoxCache:
                ...
                item.updateRotationStage(self.__getLootBoxRotationStage(lootboxes.get(b'history', {}), item))
...
class TokensProgressDelta(BaseDelta):
    ...
    def _getDataIterator(self, data):
        for tokenId, value in data.get(b'tokens', {}).iteritems():
            yield (
