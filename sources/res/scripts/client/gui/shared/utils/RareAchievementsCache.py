...
class RaresCache(IRaresCache):
    ...
    def getTitle(self, achieveID):
        return self.__cache.get(achieveID, {}).get(b'title') or i18n.makeString(b'#tooltips:achievement/action/unavailable/title')

    def getDescription(self, achieveID):
        return self.__cache.get(achieveID, {}).get(b'descr') or i18n.makeString(b'#tooltips:achievement/action/unavailable/descr')

    def getImageData(self, imgType, achieveID):
        return self.__cache.get(achieveID, {}).get(b'image', {}).get(imgType)

    def getHeroInfo(self, achieveID):
        return self.__cache.get(achieveID, {}).get(b'historyInfo')

    def getConditions(self, achieveID):
        return self.__cache.get(achieveID, {}).get(b'conditions')
