from gui.clientgw.base.handlers import RequestHandlers
from gui.clientgw.settings import WebRequestDataType

class RankRequestHandlers(RequestHandlers):

    def get(self):
        handlers = {(WebRequestDataType.RANKED_LEAGUE_POSITION): (self.__getRankedPosition), 
           (WebRequestDataType.RANKED_YEAR_POSITION): (self.__getRankedYearPosition)}
        return handlers

    def __getRankedPosition(self, ctx, callback):
        return self._requester.doRequestEx(ctx, callback, (b'rblb', b'user_ranked_position'))

    def __getRankedYearPosition(self, ctx, callback):
        return self._requester.doRequestEx(ctx, callback, (b'rblb', b'user_ranked_year_position'))
