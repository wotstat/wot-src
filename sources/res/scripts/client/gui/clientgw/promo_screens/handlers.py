...
class PromoScreensRequestHandlers(RequestHandlers):
    ...
    def __sendShownTeaser(self, ctx, callback):
        return self._requester.doRequestEx(ctx, callback, (b'promo_screens', b'send_teaser'), ctx.getPromoID())

    def __getUnreadCount(self, ctx, callback):
        return self._requester.doRequestEx(ctx, callback, (b'promo_screens', b'get_unread_count'))
