...
_CLAN_NOTIFY_CENTER_NOTIFICATION_TYPES = (
 NOTIFY_CENTER_DATA_PROXY_TYPE.CLAN_APP,
...
class _ClanDossier(object):
    ...
    def resyncClanInfo(self, force=False):
        if self.__waitForSync & SYNC_KEYS.CLAN_INFO:
            ...
            self.__changeWebInfo(SYNC_KEYS.CLAN_INFO, clanInfo, b'onClanInfoReceived')
            self.__waitForSync ^= SYNC_KEYS.CLAN_INFO
            return
    ...
    def processClanCache(self, cache):
        ...
        if apps:
            ...
            count += apps
            self.__changeWebInfo(SYNC_KEYS.APPS, count, b'onClanAppsCountReceived')
        invites = cache.get(ClanCache.KEYS.CLAN_INVITES) or 0
        if invites:
            count = self.__vitalInfo[SYNC_KEYS.INVITES] or 0
            count += invites
            self.__changeWebInfo(SYNC_KEYS.INVITES, count, b'onClanInvitesCountReceived')
        return
    ...
    def __changeWebInfo(self, fieldName, value, eventName):
        ...
        self._webCtrl.notify(b'onClanWebVitalInfoChanged', self.__clanDbID, fieldName, value)
        return
    ...
    def __applicationHandler(self, ctx, response):
        ...
        if count:
            self.__changeWebInfo(SYNC_KEYS.APPS, count - 1, b'onClanAppsCountReceived')
        ...
        self._webCtrl.notify(b'onClanAppStateChanged', ctx.getApplicationDbID(), state)
        return
    ...
    def __clanMembersNotificationHandler(self, notifID, item):
        ...
        if oldLength != newLength:
            self.__changeWebInfo(SYNC_KEYS.APPS, newLength, b'onClanAppsCountReceived')
        return
    ...
    def __clanInviteNotificationHandler(self, notifID, item):
        ...
        if count:
            self.__changeWebInfo(SYNC_KEYS.INVITES, count - 1, b'onClanInvitesCountReceived')
        return

    def __clanInviteCreatedNotificationHandler(self, notifID, item):
        currentInvitesCount = self.__vitalInfo[SYNC_KEYS.INVITES] or 0
        self.__changeWebInfo(SYNC_KEYS.INVITES, item.getNewInvitesCount() + currentInvitesCount, b'onClanInvitesCountReceived')
        return

    def __repr__(self):
        return b'ClanDossier(dbID = %d, my = %s, web = %s, cache = %s)' % (
         self.__clanDbID, self.__isMy, self.__vitalInfo, self.__webCache.keys())


@ReprInjector.simple((
 b'__state', b'state'), (
 b'__profile', b'profile'))
class WebController(WebListeners, IWebController):
    ...
    def simEnableClan(self, enable):
        ...
        clanSettings = {b'clientgwProfile': {b'isEnabled': enable, 
                                b'gateUrl': (settings.clientgw.getGateUrl())}}
        settings.update(clanSettings)
        g_clientUpdateManager.update({b'serverSettings': clanSettings})
        return
    ...
    def start(self, force=True):
        if self.__started and not force:
            _logger.debug(b'Clientgw already started.')
            return
        ...
        g_clientUpdateManager.addCallbacks({b'stats.clanInfo': (self.__onClanInfoChanged), 
           b'serverSettings.clientgw.isEnabled': (self.__onServerSettingChanged), 
           b'serverSettings.clanProfile.isEnabled': (self.__onClanEnableChanged)})
    ...
    def isEnabled(self):
        ...
        if settings is not None:
            return settings.clientgw.isEnabled()
        else:
            return True
    ...
    def getClanInfo(self):
        return {b'id': (self.__profile.getClanDbID()), 
           b'tag': (self.__profile.getClanAbbrev()), 
           b'joined_at': (self.__profile.getJoinedAt()), 
           b'name': (self.__profile.getClanName()), 
           b'full_name': (self.__profile.getClanFullName())}
    ...
    def _onProxyDataItemShowByDefault(self, notifID, item):
        if item.getType() in _CLAN_NOTIFY_CENTER_NOTIFICATION_TYPES:
            ...
            self.notify(b'onNotifyCenterNotificationReceived', notifID, item)
        return

    def _onClanMembersListChanged(self):
        memberIDs = getattr(BigWorld.player(), b'clanMembers', {}).keys()
        if self.__profile is not None:
            self.__profile.processClanMembersListChange(memberIDs)
        self.notify(b'onMembersListChanged', memberIDs)
        return
    ...
    def __onClanEnableChanged(self, *args):
        settings = self.lobbyContext.getServerSettings()
        self.notify(b'onClanEnableChanged', settings.clanProfile.isEnabled())
        return
