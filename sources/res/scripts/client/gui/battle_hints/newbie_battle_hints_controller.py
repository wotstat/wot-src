...
_PREFS_NAME = b'battleHintsDisplayHistory'
_MISSING = VariableState(b'missing')
_LOCKED = VariableState(b'locked')
_COMPLETED = None
NEWBIE_SETTINGS_MAX_BATTLES = 50
NEWBIE_HISTORY_MAX_BATTLES = 200
_ctrlLogger = getLogger(b'Ctrl')
_historyLogger = getLogger(b'History')

class NewbieBattleHintsHistory(object):
    __slots__ = (b'_history', b'_updated', b'_battlesCount', b'_enabled')
    ...
    def __init__(self):
        ...
        _historyLogger.debug(b'Initialized.')
        return
    ...
    def getDisplayCount(self, uniqueName):
        ...
        if self._history is _MISSING or self._history is _LOCKED or self._history is _COMPLETED:
            _historyLogger.debug(b'Can not get display count. Not loaded or completed.')
            return None
        else:
    ...
    def reset(self):
        ...
        self._history = _MISSING
        self._updated.clear()
        _historyLogger.debug(b'Reseted.')
        return
    ...
    def destroy(self):
        ...
        _historyLogger.debug(b'Destroyed.')
        return
    ...
    def _load(self):
        ...
        if loaded is _MISSING:
            ...
            _historyLogger.debug(b'Created: <%s>.', self._history)
        elif not isinstance(loaded, dict) and loaded is not _COMPLETED:
            self._history = {}
            _historyLogger.debug(b'Corrupted. Reset to empty state.')
        else:
            self._history = loaded
            _historyLogger.debug(b'Loaded.')
        return
    ...
    def _update(self, battleHint):
        ...
        if battleHint.uniqueName in self._updated:
            _historyLogger.debug(b'Can not update. <%s> already updated.', battleHint.uniqueName)
            return
        else:
            self._load()
            if self._history is _MISSING or self._history is _LOCKED or self._history is _COMPLETED:
                _historyLogger.debug(b'Can not update <%s>. Not loaded or completed.', battleHint.uniqueName)
                return
            self._updated.add(battleHint.uniqueName)
            self._history[battleHint.uniqueName] = self._history.get(battleHint.uniqueName, 0) + 1
            _historyLogger.debug(b'<%s> hint display count updated.', battleHint.uniqueName)
            return

    def _onItemsCacheUpdated(self, *args, **kwargs):
        if not self._itemsCache.items.isSynced():
            _historyLogger.debug(b'ItemCache items not synced.')
            return
    ...
    def _onDisconnected(self, *args, **kwargs):
        self._close()
        self._battlesCount = _MISSING
        _historyLogger.debug(b'On disconnected.')
        return
...
class NewbieBattleHintsController(INewbieBattleHintsController):
    ...
    def __init__(self):
        ...
        _ctrlLogger.debug(b'Initialized.')
        return

    def fini(self):
        self._uiLogger.finalize()
        self._history.destroy()
        _ctrlLogger.debug(b'Destroyed.')
        return
    ...
    def getDisplayCount(self, uniqueName):
        if not self.isEnabled() or not self.isUserSettingEnabled():
            _historyLogger.debug(b'Getting display count disabled by server or user.')
            return None
        else:
    ...
    def resetHistory(self):
        if not self.isEnabled():
            _ctrlLogger.debug(b'Reset history disabled by server.')
            return
        self._history.reset()
        return
