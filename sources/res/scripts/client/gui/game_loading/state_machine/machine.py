    ...
    def inner(function):
        ...
        def wrapper(self, *args, **kwargs):
            if not self.isRunning():
                _logger.warning(b'State machine is not running. Skipping %s call with result: %s.', function, result)
                return result
...
class GameLoadingStateMachine(StateMachine):
    __slots__ = (b'_cdnImages', b'_logos', b'_isGameLoadingComplete')
    ...
    def setGameLoadingComplete(self):
        _logger.debug(b'Game loading completed.')
    ...
    def wait(self, timeout=DEFAULT_WAITING_TIMEOUT):
        if not self.isRunning():
            _logger.error(b'Cannot wait. State machine is not running.')
        ...
        for state in self.getChildrenStates():
            if self.isStateEntered(state.getStateID()):
                try:
                    ...
                    _logger.warning(b'Waiting timeout <%s> reached.', timeout)
                except wg_async.BrokenPromiseError:
                    _logger.debug(b'State has been changed while waiting.')
