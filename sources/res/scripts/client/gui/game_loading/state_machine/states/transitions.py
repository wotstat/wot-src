...
class SourceToSingleTargetTransition(StringEventTransition):
    ...
    def execute(self, event):
        ...
        if result:
            ...
            if len(targetList) != 1:
                _logger.error(b'Single target from source transition assigned to multiple|none targets. %s', self)
                return False
            source = self.getSource()
            if not source:
                _logger.error(b'Single target from source transition missing source. %s', self)
                return False
