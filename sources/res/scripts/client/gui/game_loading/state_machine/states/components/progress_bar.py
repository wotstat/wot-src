...
class ProgressBarStateComponent(BaseTickingState):
    __slots__ = (b'_ticks', b'_progress', b'_progressLimit', b'_progressMax', b'_settings', b'_preferences')
    ...
    def _onExited(self):
        ...
        self._setInitialProgress()
        _logger.debug(b'[%s] max progress saved: %s.', self, self._progressMax)
        return
...
class MilestoneProgressBarStateComponent(ProgressBarStateComponent):
    __slots__ = (b'_milestonesHandler', b'_milestone', b'_milestoneLimit', b'_retainMilestones')
    ...
    def _onMilestoneReached(self, newMilestone):
        _logger.debug(b'[%s] Milestone reached: %s. New progress limit:  %s.', self, newMilestone.name, newMilestone.percent)
    ...
    def _onMilestoneTypeChanged(self, newMilestone):
        _logger.debug(b'[%s] Milestone type changed: %s. New progress limit:  %s.', self, newMilestone.name, newMilestone.percent)
        self._milestone = newMilestone
        return
