...
class LogosLoadingState(BaseViewResourcesTickingState):
    ...
    def _view(self, resource):
        if not game_loading_bindings.isViewOpened():
            _logger.debug(b'[%s] opening GF view.', self)
            game_loading_bindings.createLoadingView()
        data = {b'version': (getFullClientVersion() if resource.showVersion else b''), 
           b'copyright': (backport.text(R.strings.menu.copy()) if resource.showCopyright else b''), 
           b'logoType': (resource.type), 
           b'transitionTime': (resource.transition), 
           b'info': (resource.info), 
           b'infoStyle': (resource.infoStyle.value), 
           b'ageRatingPath': (self._ageRatingPath)}
        game_loading_bindings.setViewData(data)
        _logger.debug(b'[%s] image [%s] shown.', self, resource)
        return
