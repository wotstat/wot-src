...
class MilestoneStatusTextStateComponent(BaseTickingState):
    __slots__ = (b'_milestonesHandler', b'_milestone', b'_retainMilestones')
    ...
    def __init__(self, stateID, milestonesSettings, flags=StateFlags.UNDEFINED, tickingMode=TickingMode.MANUAL, onCompleteEvent=None):
        ...
        _logger.debug(b'[%s] Apply default milestone: %s. Status: %s.', self, self._milestone.name, self._milestone.status.text)
        return
    ...
    def _onMilestoneReached(self, newMilestone):
        _logger.debug(b'[%s] Milestone reached: %s. New text status:  %s.', self, newMilestone.name, newMilestone.status.text)
    ...
    def _onMilestoneTypeChanged(self, newMilestone):
        _logger.debug(b'[%s] Milestone type changed: %s. New text status:  %s.', self, newMilestone.name, newMilestone.status.text)
        self._milestone = newMilestone
        return
