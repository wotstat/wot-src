    ...
    def _getStateViewSettings(self, state):
        ...
        if statesSection is None:
            ...
            if stateSection is not None:
                showVfx = stateSection.readBool(b'showVfx', showVfx)
                showSmallLogo = stateSection.readBool(b'showSmallLogo', showSmallLogo)
                contentStateValue = stateSection.readInt(b'contentState', contentState.value)
                info = makeString(stateSection.readString(b'info', info))
                minimalDuration = stateSection.readFloat(b'minimalDuration', minimalDuration)
