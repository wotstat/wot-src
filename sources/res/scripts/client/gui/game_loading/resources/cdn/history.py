...
def getOrCreateViewHistory(dirPath):
    ...
    if _g_viewHistory is None:
        historyFilePath = os.path.normpath(os.path.join(dirPath, generateKey(b'history.json')))
        _g_viewHistory = ViewHistory(historyFilePath)
        _g_viewHistory.load()
        _logger.debug(b'View history created from: %s.', historyFilePath)
...
def saveViewHistory():
    if _g_viewHistory is not None:
        _g_viewHistory.save()
        _logger.debug(b'View history saved.')
    return


class ViewHistory(object):
    __slots__ = (b'_filePath', b'_history', b'_changed', b'_selectedSequence')
    ...
    def save(self):
        if self._changed:
            ...
            _logger.debug(b'History not changed. Saving skipped.')
        return
    ...
    def delete(self):
        if deleteFile(self._filePath):
            ...
            self._selectedSequence = None
            self._changed = False
            _logger.debug(b'History deleted.')
        return
    ...
    def selectSequence(self, sequence):
        ...
        if not rawSequence:
            _logger.warning(b'Can not select sequence: %s.', sequence.name)
            return
        if rawSequence == self._rawSelectedSequence:
            _logger.debug(b'Sequence: %s not changed.', sequence.name)
            return
        self._history[b'selected'] = rawSequence
        self._selectedSequence = sequence
        self._changed = True
        _logger.debug(b'Selected sequence: %s.', sequence.name)
        return

    def deleteSelectedSequence(self):
        if self._selectedSequence is not None or self._rawSelectedSequence is not None:
            self._history.pop(b'selected', None)
            self._selectedSequence = None
            self._changed = True
            _logger.debug(b'Selected sequence deleted.')
        return

    def getSequenceViewsCount(self, sequence):
        return self._history.get(b'watched', {}).get(sequence.name, {}).get(b'views', 0)

    def getSequenceSlideViewsCount(self, sequence, slide):
        return self._history.get(b'watched', {}).get(sequence.name, {}).get(b'slides', {}).get(slide.historyKey, 0)

    def addSequenceSlideViewsCount(self, sequence, slide, count=1):
        sequenceStorage = self._history.setdefault(b'watched', {}).setdefault(sequence.name, {})
        slidesStorage = sequenceStorage.setdefault(b'slides', {})
        ...
        sequenceStorage[b'views'] = slidesViewsCount[0] if slidesViewsCount else 0
        self._changed = True
        _logger.debug(b'Sequence %s slide %s view added.', sequence.name, slide)
        return
    ...
    def removeSequenceFromHistory(self, sequence):
        ...
        self._history.get(b'watched', {}).pop(sequence.name, None)
        self._changed = True
        return

    @property
    def _rawSelectedSequence(self):
        return self._history.get(b'selected')

    def __repr__(self):
        return (b'<SequencesViewHistory(path={}, sequence={}, changed={})>').format(self._filePath, self._selectedSequence, self._changed)
