...
def getBattlePassBonusPackersMap():
    ...
    mapping.update({b'berths': (BattlePassBerthsBonusPacker()), 
       ...
       b'token': (BattlePassTokenBonusPacker()), 
       b'vehicles': (BattlePassVehiclesBonusUIPacker()), 
       b'selectableBonus': selectBonusPacker, 
       ...
       b'dogTagComponents': (BattlePassDogTagComponentsUIPacker())})
    return mapping
...
class BattlePassCustomizationsBonusPacker(_BattlePassFinalBonusPacker):
    ...
    def _packSingleBonus(cls, bonus, item, data):
        ...
        if iconName == b'style' and customizationItem.modelsSet:
            iconName = b'style_3d'
...
class SelectBonusPacker(BaseBonusUIPacker):
    ...
    def _getToolTip(cls, bonus):
        tooltipData = []
        for tokenID in bonus.getTokens().iterkeys():
            tooltipData.append(createTooltipData(tooltip=None, isSpecial=True, specialAlias=TOOLTIPS_CONSTANTS.BATTLE_PASS_GIFT_TOKEN, specialArgs=[
...
class ExtendedItemBonusUIPacker(ItemBonusUIPacker):
    ...
    def _packSingleBonus(cls, bonus, item, count):
        ...
        model.setBigIcon(item.name if item.itemTypeID in (GUI_ITEM_TYPE.BATTLE_BOOSTER,
         GUI_ITEM_TYPE.OPTIONALDEVICE) else item.getGUIEmblemID())
        ...
        if item.itemTypeID == GUI_ITEM_TYPE.BATTLE_BOOSTER:
            if item.isEconomicBooster():
                model.setBigIcon(item.name + b'BattleBooster')
        return model
...
class BattlePassDogTagComponentsUIPacker(DogTagComponentsUIPacker):
    ...
    @classmethod
    def _getToolTip(cls, bonus):
        return [cls._getDogTagTooltip(dogTagRecord, bonus.getContext().get(b'withBackground', None)) for dogTagRecord in bonus.getUnlockedComponents()]
    ...
    def _getDogTagTooltip(cls, dogTagRecord, withBackground=None):
        return createTooltipData(tooltip=None, isSpecial=True, specialAlias=TOOLTIPS_CONSTANTS.DOG_TAGS_INFO, specialArgs=[
         dogTagRecord.componentId, None, withBackground])
