...
CUSTOMIZATION_BONUS_NAME = b'customizations'
TANKMAN_BONUS_NAME = b'tmanToken'
VEHICLE_BONUS_NAME = b'vehicles'
...
def getBattlePassUrl(urlPathName):
    return (b'').join((
     GUI_SETTINGS.battlePassBaseUrls[b'webBridgeRootURL'],
...
def getIsBpPointsShopEntryPointActive():
    return getBattlePassShopEntryPointsConfig()[b'isBpPointsShopEntryPointActive']


def getIsBpCoinShopEntryPointActive():
    return getBattlePassShopEntryPointsConfig()[b'isBpCoinShopEntryPointActive']
...
def isIntroVideoExist():
    return bool(GUI_SETTINGS.battlePassUrls.get(b'introVideo'))


def isExtraIntroVideoExist():
    return bool(GUI_SETTINGS.battlePassUrls.get(b'extraIntroVideo'))
...
def showVideo(videoSource, onVideoClosed=None, isAutoClose=False):
    if not videoSource:
        _logger.error(b'videoSource is not specified!')
        if callable(onVideoClosed):
            onVideoClosed()
        return
    if not videoSource.exists():
        _logger.error(b'videoSource is invalid!')
        if callable(onVideoClosed):
            onVideoClosed()
        return
...
def showBPGamefaceVideo(chapter, level, battlePass=None, onVideoClosed=None):
    ...
    if chapter not in chapterIDs or level is None:
        _logger.error(b'Both chapter and level must be specified and correct!')
    else:
        Waiting.show(b'loadBattlePassAwardVideo')
        ...
        window.load()
        Waiting.hide(b'loadBattlePassAwardVideo')
    return
