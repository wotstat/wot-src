...
DEFAULT_SUB_SCHEME = b'default'
COLOR_BLIND_SUB_SCHEME = b'color_blind'

class _GuiColorsLoader(object):
    XML_PATH = b'gui/gui_colors.xml'
    ...
    COLOR_BLIND_TAG = b'color_blind'
    DEFAULT_SCHEME = {b'alias_color': DEFAULT_ALIAS_COLOR, 
       b'rgba': DEFAULT_RGBA_COLOR, 
       b'transform': {b'mult': DEFAULT_TRANSFORM_COLOR_MULT, 
                      b'offset': DEFAULT_TRANSFORM_COLOR_OFFSET}, 
       b'adjust': {b'offset': DEFAULT_ADJUST_OFFSET}}
    ...
    def __overrideTags(self, rootSection, baseHash):
        ...
        for key in keys:
            if key == self.GROUP:
                groupNames = baseHash[key].split(b' ')
                del baseHash[key]
                for groupName in groupNames:
                    ...
                    if defaultExists != overrideDefaultExists and len(keys) > 1:
                        if defaultExists:
                            LOG_ERROR(b'schemeGroup tag requires "default" tag. Failed Tag:\n' + str(baseHash))
                        else:
                            LOG_ERROR(b'schemeGroup tag requires to delete a "default" tag. Failed Tag:\n' + str(baseHash))
                        return baseHash
    ...
    def __readTransformSection(self, section):
        ...
        processed = section[b'transform']
        if processed is not None:
            mult = self.__readVector4(processed, b'mult')
            offset = self.__readVector4(processed, b'offset')
        return {b'mult': mult, b'offset': offset}
    ...
    def getSubScheme(self, schemeName, group):
        ...
        if scheme is None:
            LOG_WARNING(b'Color scheme not found', schemeName, group)
            return self.DEFAULT_SCHEME
        else:
            if group in scheme:
                return scheme[group]
            return scheme[b'default']

    def getSubSchemeToFlash(self, schemeName, group):
        result = self.getSubScheme(schemeName, group)
        transform = result[b'transform']
        return {b'adjust': {b'offset': (result[b'adjust'][b'offset'].tuple())}, 
           b'transform': {b'mult': (transform[b'mult'].tuple()), 
                          b'offset': (transform[b'offset'].tuple())}, 
           b'rgba': (result[b'rgba'].tuple()), 
           b'alias_color': (result[b'alias_color'])}
...
def load():
    ...
    if _g_instance is None:
        ...
        try:
            ...
            LOG_ERROR(b'There is error while loading colors xml data')
