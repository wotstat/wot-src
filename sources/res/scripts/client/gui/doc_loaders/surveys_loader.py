...
def _readCondition(section, isRequired):
    if not section.has_key(b'condition'):
        return None
    else:
        subSection = section[b'condition']
        requiredQuestionId = subSection[b'requiredQuestionId'].asString
        requiredOptionId = subSection.readString(b'requiredOptionId')
        requiredAnswers = [answerId for answerId in subSection[b'requiredAnswers'].asString.split(b' ')]
        if not requiredAnswers:
            raise SoftException(b'Unfilled required answers for the condition')
        innerSubsection = subSection[b'requiredAnswers']
