...
def _getButtonsVO(vehicle):
    ...
    baseVO.update({b'compareBtnVisible': False, b'goToVehicleViewBtnVisible': False})
    return baseVO


def getDataVO(vehicle):
    baseVO = getBaseDataVO(vehicle)
    baseVO.update({b'vehicleButton': (_getButtonsVO(vehicle))})
    return baseVO
...
def getTitleVO(vehicle):
    ...
    baseVO.update({b'tankNameStr': (text_styles.grandTitle(backport.text(titleRes, title=tankUserName))), 
       b'tankNameStrSmall': (text_styles.promoTitle(backport.text(titleRes, title=tankUserName)))})
    return baseVO
