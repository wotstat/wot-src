...
def _getButtonsVO(vehicle, goToVehicleAllowed, cmpBasket=None):
    ...
    baseVO.update({b'compareBtnEnabled': comparisonState, 
       ...
       b'previewBtnEnabled': (vehicle.isPreviewAllowed() and goToVehicleAllowed), 
...
def getDataVO(vehicle, freeExp, goToVehicleAllowed):
    ...
    baseVO.update({b'showDemountAllPairsBtn': (progressionAvailability.result and showDemountAllPairs), 
       ...
       b'vehicleButton': (_getButtonsVO(vehicle, goToVehicleAllowed)), 
       b'vehicleInfo': {b'isElite': (vehicle.isElite), b'freeExp': freeExp, b'earnedXP': (vehicle.xp)}, b'demountAllButtonLabel': (text_styles.concatStylesToSingleLine(icons.makeImageTag(demountIcon, width=24, height=24, vSpace=-6), backport.text(R.strings.veh_post_progression.vehPostProgressionView.button.demountAllPairs())))})
    return baseVO
