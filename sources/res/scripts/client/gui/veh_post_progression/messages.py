...
def makeUnlockFeatureMessage(featureName, vehicleUserName, priority=NotificationPriorityLevel.HIGH):
    ...
    if msgKey:
        return makeSuccess(backport.text(msgKey.body(), vehicle=vehicleUserName), msgType=SM_TYPE.InformationHeader, msgData={b'header': (backport.text(msgKey.title()))}, msgPriority=priority)
    else:
        return
...
def makePurchasePerksMsg(vehicle, stepIDs, price):
    ...
    if len(names) > 1:
        ...
        locName = str(names[0]) if names else b''
        perkName = R.strings.veh_skill_tree.tooltips.title.dyn(locName)()
        perkName = perkName or locName
        ctx[b'perk'] = backport.text(perkName)
    userMsg = backport.text(msgKey(), **ctx)
    spentString = makeSpentString(price, ignoreCurrencies=(ExtendedCurrency.XP,))
    return makeSuccess((b' ').join((userMsg, spentString)), SM_TYPE.VehicleSkillTreePerksResearched, perksUnlockMsgs, msgData={b'title': (backport.text(msgTitle()))})
