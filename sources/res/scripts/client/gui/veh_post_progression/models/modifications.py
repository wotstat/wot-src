...
class PostProgressionActionTooltip(Enum):
    SIMPLEMOD = b'simplemod'
    MULTIMOD = b'multimod'
    FEATURE = b'feature'
    ROLESLOT = b'roleSlotFeature'
...
class PostProgressionActionItem(PurchaseProvider):
    __slots__ = (b'_descriptor', b'_parentStepID', b'_price', b'_state', b'_tooltip')
    ...
    def __repr__(self):
        return (b'{} <id: {}, name: {}>').format(self.__class__.__name__, self.actionID, self.getTechName())
    ...
    def getLocSplitNameRes(self):
        splitName = R.strings.artefacts.dyn(self.getLocName()).dyn(b'splitName')
        if splitName.exists():
            return splitName
...
class MultiModsItem(PostProgressionActionItem):
    __slots__ = (b'__modifications', b'__purchasedIdx', b'__idToIdx')
    ...
    def __repr__(self):
        innerRepr = (b' with mods: {}, purchasedIdx: {}').format(self.modifications, self.getPurchasedIdx())
...
class RoleSlotModItem(FeatureModItem):
    ...
    def __repr__(self):
        innerRepr = (b' with state: {}, category: {}').format(self.getState(), self.getSlotCategory())
