...
BonusResource = namedtuple(b'Bonus', (b'resourceType', b'resourceValue', b'isPercent'))
BonusNameToPromoStr = {b'credits': (PromoBonus.CREDITS.value)}
...
class BonusItem(object):
    ...
    def packBonusModelData(cls, bonusIDs, bonusModelsArray):
        for bonusID in bonusIDs:
            model = PetBonusModel()
            model.setId(bonusID)
            model.setName(cls.getBonusName(bonusID) or b'')
