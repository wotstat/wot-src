...
class CollectionsCdnCache(BaseExternalCache):
    ...
    __CDN_CACHE_DIR = b'collections_cache'
    __CDN_WORKERS = 16
    ...
    def getImages(self, imagesIDs):
        config = _getConfig()
        if config is None:
            _logger.warning(b'Config does not exist')
            return []
        else:
