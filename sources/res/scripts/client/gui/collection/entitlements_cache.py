...
COLLECTIONS_TAG = b'collections'
COLLECTIONS_TAG_PREFIX = b'collections_{}'
_MAX_RETRY_ATTEMPTS = 5
_INITIAL_ATTEMPTS_COUNT = 0
_RETRY_DELAY = 1

class EntitlementsCache(object):
    __slots__ = (b'__balanceCache', b'__isSyncing', b'__pendingEntitlementAttempts', b'__isInited', b'__retryCallback')
        ...
        if self.__isSyncing:
            ...
            if self.__web.isAvailable():
                ...
                if response.isSuccess():
                    ...
                    self.__balanceCache.update({entitlement[b'code']: self.__createEntitlementFromResponse(entitlement) for entitlement in result if entitlement.get(b'amount', 0) > 0})
    ...
    def __getPendingEntitlementTags(self):
        ...
        for code in self.__pendingEntitlementAttempts.iterkeys():
            _, _, collectionId, _ = code.split(b'_')
