...
def showCollectionStylePreview(styleCD, backCallback=None, backBtnDescrLabel=b'', itemsCache=None):
...
def getItemInfo(itemId, collectionId, collections=None):
    ...
    largeImage = backport.image(largeImageRes()) if largeImageRes.exists() else b''
    mediumImageRes = R.images.gui.maps.icons.collectionItems.c_600x450.dyn(itemResKey)
    mediumImage = backport.image(mediumImageRes()) if mediumImageRes.exists() else b''
    smallImageRes = R.images.gui.maps.icons.collectionItems.c_400x300.dyn(itemResKey)
    smallImage = backport.image(smallImageRes()) if smallImageRes.exists() else b''
...
def getCollectionFullFeatureName(collection):
    ...
    if season.isValid():
        return backport.text(notifications.templates.featureSeason(), feature=backport.text(feature()), season=backport.text(season()))
    return backport.text(feature())


def getUsableItemName(item):
    if item.type == b'customizationItem':
        return getCustomizationName(item.relatedId)
    if item.type == b'dossier':
        return getDossierName(item.relatedId)
    if item.type == b'tankman':
...
def getUnusableItemName(collectionId, item, collectionsSystem=None):
    ...
    itemTextRes = collRes.dyn(b'item') if collRes else None
    if itemTextRes:
...
def getItemPreviewType(item):
    ...
    if itemType in USABLE_COLLECTION_ENTITIES:
        if itemType == b'customizationItem':
            return getCustomizationPreviewType(item.relatedId)
        if itemType == b'dossier':
