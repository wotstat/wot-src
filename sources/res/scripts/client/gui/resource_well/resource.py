...
import account_helpers, logging
if typing.TYPE_CHECKING:
    from typing import Dict, List, Tuple
    from gui.impl.backport import TooltipData
_logger = logging.getLogger(__name__)
_INTELLIGENCE_BLUEPRINT = b'intelligence'
...
class TankPremiumDayResource(Resource):
    ...
    def inventoryCount(self):
        ...
        expiryUTCTime = premiumInfo.get(PREMIUM_TYPE.PLUS, {}).get(b'expiryTime', 0)
...
def _entitlementResourceFactory(resources):
    ...
    for resource in resources:
        if resource.name == PREMIUM_ENTITLEMENTS.PLUS:
            ...
            _logger.error(b"ResourceWell: resource type '%s' is not supported", resource.name)
        ...
        for name, count in resource.iteritems():
            ...
            if resourceConfig is not None:
                ...
                if callable(factory):
                    ...
                    _logger.error(b"ResourceWell: resource type '%s' is not supported", resType)
