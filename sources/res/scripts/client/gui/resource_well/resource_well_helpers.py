...
def getProgressionState(resourceWell=None):
    ...
    if resourceWell.getCurrentPoints():
        return ProgressionState.ACTIVE
    return ProgressionState.NO_PROGRESS
...
def getSerialNumber(resourceWell=None, itemsCache=IItemsCache):
    ...
    return itemsCache.items.inventory.getC11nSerialNumber(itemCD=style.intCD) or b''
...
def getAvailableRewardData(resourceWell=None):
    ...
    if resourceWell.getRewardLeftCount(True):
        ...
        if resourceWell.getRewardLeftCount(False):
            return (findFirst((lambda (rewardID, reward): not reward.isSerial), rewards.iteritems())[0], False)
        _logger.warning(b'No available rewards for resource well!')
        return (None, False)
