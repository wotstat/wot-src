...
def showPaymentMethodLinkNotification(method, imageUrl):
    ...
    SystemMessages.pushMessage(text=backport.text(R.strings.messenger.serviceChannelMessages.sysMsg.titles.paymentMethodLink(), method=method), type=SystemMessages.SM_TYPE.PaymentMethodLinkWgnc, messageData={b'imageBlock': (_packImageBlock(localPath))})
    return
...
def showPaymentMethodUnlinkNotification(method, imageUrl):
    helper = WebImageHelper()
    localPath = yield helper.getLocalPath(imageUrl)
    SystemMessages.pushMessage(text=backport.text(R.strings.messenger.serviceChannelMessages.sysMsg.titles.paymentMethodUnlink(), method=method), type=SystemMessages.SM_TYPE.PaymentMethodUnlinkWgnc, messageData={b'imageBlock': (_packImageBlock(localPath))})
    return
