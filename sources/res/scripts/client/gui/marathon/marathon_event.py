...
class MarathonEvent(object):
    ...
    def getQuestsData(self, prefix=None, postfix=None):
        return self.__getProgress(b'quests', prefix, postfix)

    def getTokensData(self, prefix=None, postfix=None):
        return self.__getProgress(b'tokens', prefix, postfix)
    ...
    def getMarathonFlagState(self, vehicle):
        return {b'flagHeaderIcon': (self.__getHangarFlagHeaderIcon()), 
           b'flagStateIcon': (self.__getHangarFlagStateIcon(vehicle)), 
           b'flagMain': (self.getHangarFlag()), 
           b'tooltip': (self.__getTooltip()), 
           b'enable': (self.isAvailable()), 
           b'visible': (self.isEnabled())}
    ...
    def _getUrl(self, urlType=MarathonConfig.URL):
        ...
        if not baseUrl:
            _logger.warning(b'Marathon url from marathon_config.xml is absent or invalid: %s', baseUrl)
        return baseUrl

    def __getRewardShownMarkKey(self, key):
        return (b'_').join([key, self._data.tokenPrefix])
