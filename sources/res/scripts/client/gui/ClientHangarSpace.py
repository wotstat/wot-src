...
class ClientHangarSpace(object):
    ...
    def create(self, isPremium, onSpaceLoadedCallback=None):
        ...
        self.__vEntityId = BigWorld.createEntity(b'HangarVehicle', self.__spaceId, 0, _CFG[b'v_start_pos'], (
         _CFG[b'v_start_angles'][2], _CFG[b'v_start_angles'][1], _CFG[b'v_start_angles'][0]), dict())
...
class _ClientHangarSpacePathOverride(object):
    ...
    def setPath(self, path, visibilityMask=None, environment=b'', isPremium=None, isReload=True, event=None):
        if path is not None and not path.startswith(b'spaces/'):
            path = b'spaces/' + path
