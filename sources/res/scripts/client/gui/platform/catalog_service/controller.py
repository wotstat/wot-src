...
class _PurchaseDescriptor(object):
    __slots__ = (b'__entitlements', b'__metadataWot', b'__currencies', b'__isEntitlementsInvalid', b'__tokens', b'__titleID', b'__productExtraData', b'__iconID', b'__productName', b'__mainAmount', b'__displayWays')
                ...
                for entitlement in self.__entitlements:
                    ...
                    if entCode.startswith(TOKEN_ENTITLEMENT_PREFIX):
                        if entCode[len(TOKEN_ENTITLEMENT_PREFIX):] == tID:
                            ...
                            imgBig = self.__extractValue(self.__metadataWot.get((b'{}_image_large').format(metadataPrefix), {}).get(b'data', {}).get(b'url', {}))
                            imgSmall = self.__extractValue(self.__metadataWot.get((b'{}_icon_url_big').format(metadataPrefix), {}).get(b'data', {}).get(b'url', {}))
    ...
    def __getMetadataDisplayWays(self):
        params = self.__metadataWot.get(b'params', {})
...
class _PurchasePackage(object):
    ...
    def _initDescriptor(self, dataDict):
        metadataWot = dataDict.get(b'metadata', {}).get(b'wot')
        if not metadataWot:
            _logger.error(b'Could not find "metadata/wot" section in the obtained product descriptor!')
        ...
        return _PurchaseDescriptor(entitlements, dataDict.get(b'currencies'), metadataWot)
