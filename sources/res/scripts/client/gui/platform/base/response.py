...
class PlatformResponse(Response):
    ...
    @classmethod
    def createUnexpectedError(cls, txtStr=b''):
    ...
    @classmethod
    def createHttpError(cls, extraCode, txtStr=b'', data=None, headers=None):
        return cls.createError(Codes.HTTP_REQUEST_ERROR, extraCode, txtStr, data, headers)

    @classmethod
    def createSuccess(cls, extraCode=httplib.OK, txtStr=b'', data=None, headers=None):
        return cls._create(Codes.SUCCESS, extraCode, txtStr, data, headers)

    @classmethod
    def createError(cls, code, extraCode=httplib.SERVICE_UNAVAILABLE, txtStr=b'', data=None, headers=None):
    ...
    @classmethod
    def _create(cls, code, extraCode, txtStr=b'', data=None, headers=None):
        return cls(code.value, txtStr, data or {}, extraCode, headers or {})

    def _isError(self, errorType, *errorNames):
        return bool(set(errorNames) & set(self.getData().get(b'errors', {}).get(errorType, [])))
