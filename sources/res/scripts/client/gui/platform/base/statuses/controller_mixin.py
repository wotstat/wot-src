...
class StatusesMixin(object):
    ...
    def _updateStatus(self, status, context=DEFAULT_CONTEXT):
        ...
        if current == status:
            self._logger.debug(b'Status: (%s|%s) not changed.', context, current)
            return
        self._logger.debug(b'Status for context %s changed from %s to %s.', context, current, status)
    ...
    def _processStatusTokens(self, validator):
        for token, context, status in self._statusTokens:
            if validator(token):
                ...
                self._logger.debug(b'Token (%s|%s) received.', context, token)
