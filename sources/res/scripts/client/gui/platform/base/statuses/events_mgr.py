...
class StatusEventsManager(object):
    ...
    def subscribe(self, statusType, handler, context=DEFAULT_CONTEXT):
        ...
        self._logger.debug(b'Subscribed to (%s|%s|%s).', context, statusType, handler)
        return
    ...
    def unsubscribe(self, statusType, handler, context=DEFAULT_CONTEXT):
        ...
        if event:
            event -= handler
            self._logger.debug(b'Unsubscribed from (%s|%s|%s).', context, statusType, handler)
        return

    def send(self, status, context=DEFAULT_CONTEXT):
        event = self._get(context, status.type)
        if event:
            self._logger.debug(b'Sending %s for (%s|%s).', status, context, event)
            event(status)
        return
    ...
    def clear(self):
        self._em.clear()
        self._events.clear()
        self._logger.debug(b'All events cleared.')
        return
