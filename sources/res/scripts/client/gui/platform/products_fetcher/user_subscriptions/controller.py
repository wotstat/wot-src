...
class PlatformGetUserSubscriptionsParams(object):
    ...
    def __str__(self):
        return (b'status: {status}').format(status=self.status)
...
class UserSubscriptionsFetchController(IUserSubscriptionsFetchController):
    ...
    def getSubscriptions(self, clearCache=False):
        _logger.debug(b'Trying to fetch subscriptions')
        if self._fetchResult.isProcessed and not clearCache:
            _logger.debug(b'Return subscriptions from cache')
        ...
        if requestSuccess and subscriptionsData:
            _logger.debug(b'Subscriptions request from %s has been successfully processed.', str(subscriptionParams))
            subsLookup = {}
            for subscriptionData in subscriptionsData:
                productCode = subscriptionData.get(b'product_code')
    ...
    def _requestSubscriptions(self, params, callback):
        ...
        _logger.debug(b'Request subscriptions for params %s', params)
