...
class WGNPSteamAccRequestController(StatusesMixin, WGNPRequestController, IWGNPSteamAccRequestController):
    ...
    def getEmailStatus(self, waitingID=None):
        ...
        self._logger.debug(b'Getting email status from cache=%s, waitingID=%s.', status, waitingID)
    ...
    def addEmail(self, email, waitingID=None):
        self._logger.debug(b'Adding waitingID=%s.', waitingID)
        ...
        if response.isSuccess():
            ...
            self._updateStatus(SteamAccEmailStatus(StatusTypes.ADDED, data={b'email': email}))
    ...
    def confirmEmail(self, code, waitingID=None):
        self._logger.debug(b'Confirm email with waitingID=%s.', waitingID)
