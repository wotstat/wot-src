...
class EmptyAppFactory(AlwaysValidObject, IAppFactory):
    ...
    def createLobby(self):
        _logger.debug(b'EmptyAppFactory.createLobby')
        return

    def destroyLobby(self):
        _logger.debug(b'EmptyAppFactory.destroyLobby')
        return

    def reloadLobbyPackages(self):
        _logger.debug(b'EmptyAppFactory.reloadLobbyPackages')
        return

    def createBattle(self, _):
        _logger.debug(b'EmptyAppFactory.createBattle')
        return

    def destroyBattle(self):
        _logger.debug(b'EmptyAppFactory.destroyBattle')
        return

    def getWaitingWorker(self):
        _logger.debug(b'EmptyAppFactory.getWaitingWorker')
        return self.__waiting


class AS3_AppFactory(IAppFactory):
    __slots__ = (b'__apps', b'__packages', b'__importer', b'__waiting', b'__ctrlModeFlags', b'__weakref__', b'__gui')
    ...
    def createLobby(self):
        _logger.info(b'Creating app: %s', _SPACE.SF_LOBBY)
    ...
    def reloadLobbyPackages(self):
        _logger.info(b'Reload app: %s', _SPACE.SF_LOBBY)
        lobby = self.__apps[_SPACE.SF_LOBBY]
    ...
    def destroyLobby(self):
        if _SPACE.SF_LOBBY in self.__apps:
            lobby = self.__apps[_SPACE.SF_LOBBY]
            if lobby:
                _logger.info(b'Destroying app: %s', _SPACE.SF_LOBBY)
                lobby.close()
    ...
    def destroyBattle(self):
        _logger.info(b'Destroying app: %s', _SPACE.SF_BATTLE)
    ...
    def attachCursor(self, appNS, flags=_CTRL_FLAG.GUI_ENABLED):
        if appNS not in self.__apps:
            ...
            _logger.debug(b'Attach cursor: %s', appNS)
            self.__ctrlModeFlags[appNS] = flags
            app = self.__apps[appNS]
            if app is not None:
                app.attachCursor(flags=flags)
            else:
                _logger.debug(b'Can not attach cursor because of app is not found: %s', appNS)
            return
    ...
    def detachCursor(self, appNS):
        if appNS not in self.__apps:
            ...
            _logger.debug(b'Detach cursor: %s', appNS)
            self.__ctrlModeFlags[appNS] = _CTRL_FLAG.CURSOR_DETACHED
            app = self.__apps[appNS]
            if app is not None:
                app.detachCursor()
            else:
                _logger.debug(b'Can not detach cursor because of app is not found: %s', appNS)
            return
    ...
    def syncCursor(self, appNS, flags=_CTRL_FLAG.GUI_ENABLED):
        if appNS not in self.__apps:
            ...
            _logger.debug(b'Sync cursor: %s', appNS)
            self.__ctrlModeFlags[appNS] = flags
            app = self.__apps[appNS]
            if app is not None:
    ...
    def destroy(self):
        for appNS in self.__apps.iterkeys():
            _logger.info(b'Destroying app: %s', appNS)
    ...
    def goToLobby(self, appNS):
        ...
        if self.bootcampCtrl.isInBootcamp():
            libs.extend([b'BCGuiControlsLobbyBattle.swf', b'BCGuiControlsLobby.swf'])
    ...
    def getMainWindow(cls):
        ...
        if not windows:
            _logger.error(b'The main window does not exist')
            return None
        else:
            return windows[0]
    ...
    def _toggleBattleLoading(toggle):
        event = events.GameEvent(events.GameEvent.BATTLE_LOADING, ctx={b'isShown': toggle})
        g_eventBus.handleEvent(event, EVENT_BUS_SCOPE.BATTLE)
        return
