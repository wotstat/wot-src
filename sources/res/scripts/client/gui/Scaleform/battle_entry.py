...
BATTLE_OPTIMIZATION_CONFIG = {(BATTLE_VIEW_ALIASES.MINIMAP): (OptimizationSetting(b'minimapAlphaEnabled', True)), 
...
class BattleEntry(AppEntry):
    ...
    def __init__(self, appNS, ctrlModeFlags, arenaGuiType):
        super(BattleEntry, self).__init__(R.entries.battle(), appNS, ctrlModeFlags, daapiBridge=DAAPIRootBridge(initCallback=b'registerBattleTest'))
        self._arenaGuiType = arenaGuiType
        self.__input = None
        return

    @uniprof.regionDecorator(label=b'gui.battle', scope=b'enter')
    ...
    @uniprof.regionDecorator(label=b'gui.battle', scope=b'exit')
