...
def convertToResource(message):
    ...
    if splitted:
        ...
        for item in splitted:
            resourceID = resourceID.dyn(item.replace(b'-', b'_'))
...
class Waiting(object):
    ...
    def show(cls, message, isSingle=False, interruptCallback=None, overlapsUI=True, isAlwaysOnTop=False, backgroundImage=None, softStart=False, showBg=True):
        ...
        if not resourceID:
            _logger.error(b'Waiting can not be shown. Resource is not found: %s', message)
            return
    ...
    def hide(cls, message):
        resourceID = convertToResource(message)
        if not resourceID:
            _logger.error(b'Waiting can not be hidden. Resource is not found: %s', message)
            return
