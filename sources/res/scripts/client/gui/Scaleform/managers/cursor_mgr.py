...
class CursorManager(CursorManagerMeta):
    ...
    RESIZE = b'resize'
    MOVE = b'move'
    DRAG_OPEN = b'dragopen'
    DRAG_CLOSE = b'dragclose'
    __DAAPI_ERROR = b'flashObject is Python Cursor class can`t be None!'
    ...
    def attachCursor(self, flags=_CTRL_FLAG.GUI_ENABLED):
        ...
        if not mcursor.active:
            ...
            _logger.debug(b'Cursor is attached')
    ...
    def detachCursor(self):
        mcursor = GUI.mcursor()
        if mcursor.active:
            _logger.debug(b'Cursor is detached')
