...
TWEEN_CONSTRAINTS_FILE_PATH = b'gui/tween_constraints.xml'
ERROR_NOT_SUCH_FILE = b'Not such file '
REQUIRED_PARAMETERS = [
 11, 
 12, 
 13, 
    ...
    def __loadTweenConstraintsXML(self):
        if ResMgr.isFile(TWEEN_CONSTRAINTS_FILE_PATH):
            ...
            for ctx_, subSection in resource_helper.getIterator(ctx, section):
                item = resource_helper.readItem(ctx_, subSection, name=b'setting')
    ...
    def __checkRequiredValues(self, settings):
        for key in REQUIRED_PARAMETERS:
            if key not in settings:
                LOG_ERROR(b'In the ' + TWEEN_CONSTRAINTS_FILE_PATH + b' there is no required parameter ' + key)
...
class _AbstractTween(AbstractTweenMeta):
    ...
    def __applyQuickSet(self, quickSet):
        ...
        getPaused = getattr(quickSet, b'getPaused', None)
        getLoop = getattr(quickSet, b'getLoop', None)
        getDelay = getattr(quickSet, b'getGlobalDelay', None)
    ...
    def setDuration(self, duration):
        if duration == 0:
            LOG_WARNING(b'Duration will be more than 0')
            return
        if self.isComplete:
