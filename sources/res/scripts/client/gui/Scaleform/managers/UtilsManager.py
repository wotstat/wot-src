...
class UtilsManager(UtilsManagerMeta):
    ...
    def changeStringCasing(self, string, isUpper, _):
        return utils.changeStringCasing(str(string).decode(b'utf-8'), isUpper)
    ...
    def getHtmlIconText(cls, properties):
        template = b"<img src='{0}' width='{1}' height='{2}' vspace='{3}' hspace='{4}'/>"
    ...
    def intToStringWithPrefixPatern(self, value, count, fill):
        return (b'{0:' + str(fill) + b'>' + str(count) + b'}').format(value)

    def isTwelveHoursFormat(self):
        return getClientLanguage() == b'en'
