...
WindowGeometry = namedtuple(b'WindowGeometry', (b'x', b'y', b'width', b'height'))
...
class stored_window(object):
    ...
    def __init__(self, dataType, targetID, sideEffect=None):
        ...
        if sideEffect is not None and not callable(sideEffect):
            raise UserWarning(b'Value of sideEffect is not callable')
        self.__sideEffect = sideEffect
        return

    def __call__(self, clazz):
        if not hasattr(clazz, b'__mro__'):
            raise SoftException(b'First argument is not class')
        if WindowViewMeta not in clazz.__mro__:
            raise SoftException(b'Class must be extends WindowViewMeta')
        ...
        if not getattr(clazz, b'__stored_window__', False):
...
class WindowStoredData(object):
    __slots__ = (b'_trusted', b'_geometry')
...
class UniqueWindowStoredData(WindowStoredData):
    __slots__ = (b'_uniqueName',)

    def __init__(self, name, *args):
        super(UniqueWindowStoredData, self).__init__(*args)
        if not isinstance(name, basestring):
            _logger.warning(b'Unique name must be string. It is ignored: %r', name)
            name = b''
            self._trusted = False
    ...
    def __repr__(self):
        return (b'UniqueWindowStoredData(uniqueName = {0:>s}, geometry = {1!r:s}, trusted = {2!r:s})').format(self._uniqueName, self._geometry, self._trusted)
...
class CarouselWindowStoredData(WindowStoredData):
    ...
    def __repr__(self):
        return (b'CarouselWindowStoredData(clientID = {0:n}, geometry = {1!r:s}, trusted = {2!r:s})').format(self._clientID, self._geometry, self._trusted)

    @classmethod
    def make(cls, window):
        getter = getattr(window, b'getClientID', None)
        if getter and callable(getter):
            clientID = getter()
        else:
            _logger.error(b'Can not get clientID: %r', window)
            return
...
class ChannelWindowStoredData(WindowStoredData):
    __slots__ = (b'_protoType', b'_channelID')
    ...
    def __repr__(self):
        return (b'ChannelWindowStoredData(protoType = {0:n}, channelID = {1!r:s}, geometry = {2!r:s}, trusted = {3!r:s})').format(self._protoType, self._channelID, self._geometry, self._trusted)

    @classmethod
    def make(cls, window):
        getter = getattr(window, b'getProtoType', None)
        if getter and callable(getter):
            protoType = getter()
        else:
            _logger.error(b'Can not get protoType: %r', window)
            return
        getter = getattr(window, b'getChannelID', None)
        if getter and callable(getter):
            channelID = getter()
        else:
            _logger.error(b'Can not get channelID: %r', window)
            return
...
class _WindowsStoredDataManager(object):
    ...
    def start(self):
        ...
        for record in records:
            if len(record) < 2:
                _logger.error(b'Invalid record: %r', record)
                continue
            targetID, dataType = record[:2]
            if not targetID & TARGET_ID.ALL:
                _logger.error(b'Invalid target ID: %r', record)
                continue
            if not self.isTargetEnabled(targetID):
                _logger.warning(b'Target is not enabled. Records are ignored to load: %r, %r', self.__targetMask, record)
                continue
            if dataType not in DATA_TYPE.RANGE:
                _logger.error(b'Invalid data type: %r', record)
                continue
            if dataType not in self.__supported:
                _logger.error(b'Data type is not supported: %r', dataType)
                continue
            try:
        ...
        if not self.__isStarted:
            ...
            for targetID, windowsData in viewitems(self.__storedData):
                if not self.__targetMask & targetID and windowsData:
                    _logger.warning(b'Target is not enabled. Records are ignored to flush: %r, %r', targetID, self.__targetMask)
                    continue
                for data in windowsData:
                    if not data.isTrusted():
                        _logger.debug(b'Data is not trusted. It do not write to file: %r', data)
                        continue
                    record = data.pack()
                    if record is None:
                        continue
