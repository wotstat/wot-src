...
class _WaitingTask(object):
    __slots__ = (b'__messageID', b'__isBlocking', b'__interruptCallbacks', b'__isAlwaysOnTop', b'__backgroundImage', b'__softStart', b'__showBg')
    ...
    def __repr__(self):
        return (b'_WaitingTask({}): message={}, interruptCallbacks={}, isBlocking={}, isAlwaysOnTop={}, bkImg={}, softStart={}, showBg={}').format(id(self), self.__messageID, self.__interruptCallbacks, self.__isBlocking, self.__isAlwaysOnTop, self.__backgroundImage, self.__softStart, self.__showBg)
...
class WaitingWorker(IWaitingWorker):
    __slots__ = (b'__appFactory', b'__blocking', b'__nonBlocking', b'__waitingStack', b'__suspendStack', b'__isShown', b'__transition', b'__transitionMessageID', b'__resumeLockers')
    ...
    def show(self, messageID, isSingle=False, interruptCallback=None, isBlocking=True, isAlwaysOnTop=False, backgroundImage=None, softStart=False, showBg=True):
        ...
        if hasAlwaysOnTopWaiting and isAlwaysOnTop:
            _logger.error(b'Failed to show waiting with messageID="%r", because second "always-on-top" waiting exists', messageID)
            return
        else:
    ...
    def resume(self, lockerID=None, hard=False):
        ...
        if not self.isSuspended():
            ...
            if self.isResumeLocked() and not hard:
                _logger.debug(b'waiting resume locked')
                return
            if hard:
                self.__resumeLockers.clear()
    ...
    def snapshort(self):
        return {b'waiting': (self.__waitingStack[:]), b'suspend': (self.__suspendStack[:])}
    ...
    def __onViewLoaded(self, pyView, _):
        ...
        if alias == VIEW_ALIAS.LOBBY:
            if not isinstance(pyView, IWaitingWidget):
                _logger.error(b'Non blocking waiting is not available. View does not inherit IWaitingComponent: %r', pyView)
                return
            ...
            if not isinstance(pyView, IWaitingWidget):
                _logger.error(b'Blocking waiting is not available. View does not inherit IWaitingComponent: %r', pyView)
                return
