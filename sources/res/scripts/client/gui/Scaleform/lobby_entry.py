...
def getLobbyStateMachine():
    ...
    if app:
        return getattr(app, b'stateMachine', None)
    else:
        return


_UntrackedStateForwardedParams = namedtuple(b'_UntrackedStateForwardedParams', [b'loadParams', b'args', b'kwargs'])
...
class LobbyEntry(AppEntry):
    ...
    @uniprof.regionDecorator(label=b'gui.lobby', scope=b'enter')
    ...
    @uniprof.regionDecorator(label=b'gui.lobby', scope=b'exit')
    ...
    def loadView(self, loadParams, *args, **kwargs):
        ...
        def getViewScopeAndLayer(loadParams, *args, **kwargs):
            ...
            if settings:
                ...
                _logger.info(b'Fake creating view of class %s for LSM to get the layer', loadParams.viewClass.__name__)
        ...
        if matchingState:
            if not self.__stateMachine.isStateEntered(matchingState.getStateID()):
                _logger.warning(b'View "%s" has a matching state %s, navigate to it instead of direct load. Attempting to extract params and navigate to the matching state.', loadParams.viewKey, matchingState)
                params = dict(kwargs)
                for arg in args:
            ...
            if untrackedState:
                _logger.info(b'No matching state for view "%s", navigating to untracked state', loadParams.viewKey)
