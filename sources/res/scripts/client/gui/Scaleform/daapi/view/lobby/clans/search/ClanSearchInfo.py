...
class ClanSearchInfo(ClanSearchInfoMeta, ClanListener, ClanEmblemsHelper):
    ...
    def _updateDetailedInfo(self):
        ...
        wins = formatField(getter=self.__selectedClan.getBattleXpAvg, formatter=(lambda value: backport.getNiceNumberFormat(value) + b'%'))
        avgExp = formatField(getter=self.__selectedClan.getBattlesPerformanceAvg, formatter=backport.getIntegralFormat)
        stats = [
         _packItemData(battlesCount, CLANS.SEARCH_INFO_STATS_BATTLES, CLANS.SEARCH_INFO_STATS_BATTLES_TOOLTIP, b'avgBattlesCount40x32.png'),
         _packItemData(wins, CLANS.SEARCH_INFO_STATS_WINS, CLANS.SEARCH_INFO_STATS_WINS_TOOLTIP, b'avgWins40x32.png'),
         _packItemData(avgExp, CLANS.SEARCH_INFO_STATS_AVGEXP, CLANS.SEARCH_INFO_STATS_AVGEXP_TOOLTIP, b'avgExp40x32.png')]
        self.as_setDataS({b'clanId': clanID, 
           b'clanName': clanName, 
           b'ratingTitle': (text_styles.main(CLANS.SEARCH_INFO_RATINGTITLE)), 
           b'rating': (text_styles.promoTitle(rating)), 
           b'stats': stats})
        return
    ...
    def _updateSetaledState(self):
        ...
        if reason == CLIENT_CLAN_RESTRICTIONS.NO_RESTRICTIONS:
            ...
            sendRequestBtnVisible = False
        self.as_setStateDataS({b'requestSentVisible': requestSentVisible, 
           b'sendRequestBtnVisible': sendRequestBtnVisible, 
           b'sendRequestBtnEnabled': sendRequestBtnEnabled, 
           b'sendRequestTooltip': sendRequestTooltip, 
           b'alertIconVisible': (sendRequestBtnVisible and not sendRequestBtnEnabled)})
        return

    def __initControls(self):
        self.as_setInitDataS({b'ratingDescription': (text_styles.stats(_ms(CLANS.SEARCH_INFO_RATINGDESCRIPTION))), 
           b'ratingTooltip': (CLANS.SEARCH_INFO_RATINGDESCRIPTION_TOOLTIP), 
           b'requestSent': (text_styles.success(_ms(CLANS.SEARCH_INFO_REQUESTSENT))), 
           b'clanProfileBtnLabel': (_ms(CLANS.SEARCH_INFO_CLANPROFILEBTN)), 
           b'sendRequestBtnLabel': (_ms(CLANS.SEARCH_INFO_SENDREQUESTBTN))})
        self.as_setWaitingVisibleS(False)
        return
