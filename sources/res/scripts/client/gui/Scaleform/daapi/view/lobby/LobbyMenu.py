...
def _getVersionMessage():
    return (
     (b'{0} {1}').format(text_styles.main(i18n.makeString(MENU.PROMO_PATCH_MESSAGE)), text_styles.stats(getShortClientVersion())),)
...
class LobbyMenu(LobbyMenuMeta):
    ...
    def refuseTraining(self):
        isOk = yield DialogsInterface.showI18nConfirmDialog(b'refuseTraining')
        if isOk:
    ...
    def logoffClick(self):
        isOk = yield DialogsInterface.showI18nConfirmDialog(b'disconnect', focusedID=DIALOG_BUTTON_ID.CLOSE)
        if isOk:
    ...
    def bootcampClick(self):
        if not self.bootcamp.canRun() and not self.bootcamp.isInBootcamp():
            SystemMessages.pushI18nMessage(b'#system_messages:prebattle/bootcamp/inOtherQueue', type=SystemMessages.SM_TYPE.Warning)
            self.destroy()
        else:
    ...
    def __updateNewSettingsCount(self):
        userLogin = getattr(BigWorld.player(), b'name', b'')
        if userLogin == b'':
            return
        toShow, toHide = [], []
        counts = {b'settingsBtn': (getCountNewSettings()), 
           b'postBtn': (self.promo.getPromoCount())}
        if not self.bootcamp.isInBootcamp():
            counts[b'manualBtn'] = self.manualController.getNewContentCount()
        for componentID, count in counts.iteritems():
            if count > 0:
                toShow.append({b'componentId': componentID, b'count': (str(count))})
            else:
    ...
    def __onServerSettingChanged(self, diff):
        if b'isManualEnabled' in diff:
            self.__updateManualBtn()
        if b'isFieldPostEnabled' in diff:
            self.__updatePostButton()
        return
