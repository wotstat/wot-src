...
def getSmokeDataByPredicate(smokeInfo, predicate):
    if smokeInfo is None or not predicate:
        return (None, None)
    if predicate(smokeInfo[b'equipmentID']):
...
def parseSmokeData(smokesInfo):
    if smokesInfo:
        maxEndTime, eqId = max((smokeInfo[b'endTime'], smokeInfo[b'equipmentID']) for smokeInfo in smokesInfo)
        return (
...
def getCircularVisionAngle(vehicle=None):
    if vehicle is None:
        ...
        if hasattr(player, b'getVehicleAttached'):
            ...
            if vehicle is None:
                _logger.warning(b'Vehicle has not been created yet!')
                return
        else:
            _logger.warning(b'Player is not Avatar!')
            return
    if not hasattr(vehicle, b'coneVisibility'):
        _logger.info(b'Vehicle attribute "coneVisibility" is not found!')
        return
    else:
...
def canVehicleSpawnBot(vehicleName, brCtrl=None):
    ...
    return any(eqID for eqID in vehicleEquipmentIDs if hasattr(equipments[eqID], b'botType'))
...
def getAvailableNationsNames(lobbyContext=None):
    ...
    names = [values.split(b':')[0] for values in keys]
    return names


def getAvailableVehicleTypes():
    return frozenset((b'lightTank', b'mediumTank', b'heavyTank'))
