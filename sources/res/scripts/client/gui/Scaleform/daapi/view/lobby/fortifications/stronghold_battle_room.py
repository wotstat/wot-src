...
class StrongholdBattleRoom(FortClanBattleRoomMeta, IUnitListener, IStrongholdListener, UsersInfoHelper):
    ...
    class TIMER_GLOW_COLORS(CONST_CONTAINER):
        NORMAL = int(b'BB6200', 16)
        ALERT = int(b'BB2B00', 16)
    ...
    def inviteFriendRequest(self):
        self.fireEvent(events.LoadViewEvent(SFViewLoadParams(FORTIFICATION_ALIASES.STRONGHOLD_SEND_INVITES_WINDOW_PY), ctx={b'prbName': (PREBATTLE_TYPE_NAMES[PREBATTLE_TYPE.STRONGHOLD]), 
           b'ctrlType': (CTRL_ENTITY_TYPE.UNIT), 
           b'showClanOnly': False}), scope=EVENT_BUS_SCOPE.LOBBY)
        return
    ...
    def openConfigureWindow(self):
        ...
        windowSize = GUI_SETTINGS.lookup(b'StrongholdsPopupWebWindowSize')
        browserSize = (windowSize.get(b'width', 800), windowSize.get(b'height', 600))
    ...
    def onUpdateHeader(self, header, isFirstBattle, isUnitFreezed):
        ...
        if not self.__firstShowMainForm:
            ...
            self.as_setBattleRoomDataS(self._makeFortClanBattleRoomVO(0, b'', b'', b'', b'', False, False, header.isSortie()))
    ...
    def _makeFortClanBattleRoomVO(self, mapId, headerDescr, mineClanName, enemyClanName, waitForBattleDescr, isMapEnabled, isBattleTimerVisible, isSortie):
        ...
        vo[b'levelsRange'] = self.prbEntity.getRosterSettings().getLevelsRange()
        return vo
    ...
    def _selectReserve(self, ctx):
        reserveID = ctx.get(b'newId', None)
        isRemove = ctx.get(b'isRemove', None)
        if reserveID is not None:
            if not isRemove:
                self.sendRequest(SetReserveUnitCtx(reserveID, waitingID=b'prebattle/change_settings', isRemove=isRemove))
            else:
                self.sendRequest(UnsetReserveUnitCtx(reserveID, waitingID=b'prebattle/change_settings', isRemove=isRemove))
        return
    ...
    def _updateTitle(self, isSortie, direction, resourceMultiplier):
        if isSortie:
            ...
            title = i18n.makeString(FORTIFICATIONS.STRONGHOLDTITLE_STRONGHOLD) % {b'direction': direction}
            if resourceMultiplier > 1:
                title += b' (x%s)' % resourceMultiplier
        self.fireEvent(events.RenameWindowEvent(events.RenameWindowEvent.RENAME_WINDOW, ctx={b'data': title}), scope=EVENT_BUS_SCOPE.LOBBY)
        return
    ...
    def _onMatchmakingTimerChanged(self, event):
        ...
        playerClanName = None
        enemyClanName = None
        currentBattle = data[b'currentBattle']
        mapId = currentBattle.getMapId() if currentBattle else 0
        enemyclanData = data[b'enemyClan']
        enemyVisible = enemyclanData is not None
        isInBattle = self.prbEntity.getFlags().isInArena()
        textid = data[b'textid']
        maxLvl = data[b'maxLevel']
        level = int2roman(maxLvl)
        if data[b'isSortie']:
            headerDescr = i18n.makeString(FORTIFICATIONS.STRONGHOLDINFO_SORTIE) % {b'level': level}
            timetext = None
            if textid in (
             FORTIFICATIONS.SORTIE_INTROVIEW_FORTBATTLES_ENDOFBATTLESOON,
             FORTIFICATIONS.SORTIE_INTROVIEW_FORTBATTLES_NEXTTIMEOFBATTLESOON):
                timetext = makeHtmlString(b'html_templates:lobby/fortifications/introView', b'fortBattles', {b'text': (time_utils.getTimeLeftFormat(data[b'dtime']))})
            elif textid in (
             FORTIFICATIONS.SORTIE_INTROVIEW_FORTBATTLES_NEXTTIMEOFBATTLETOMORROW,
             FORTIFICATIONS.SORTIE_INTROVIEW_FORTBATTLES_NEXTTIMEOFBATTLETODAY):
                timetext = makeHtmlString(b'html_templates:lobby/fortifications/introView', b'fortBattles', {b'text': (backport.getShortTimeFormat(data[b'peripheryStartTimestamp']))})
            wfbDescr = i18n.makeString(textid, nextDate=timetext)
        else:
            direction = vo_converters.getDirection(data[b'direction'])
            headerDescr = i18n.makeString(FORTIFICATIONS.STRONGHOLDINFO_STRONGHOLD) % {b'direction': direction}
            if textid != FORTIFICATIONS.ROSTERINTROWINDOW_INTROVIEW_FORTBATTLES_NEXTTIMEOFBATTLESOON:
                timetext = None
                if textid in (
                 FORTIFICATIONS.SORTIE_INTROVIEW_FORTBATTLES_NEXTTIMEOFBATTLETOMORROW,
                 FORTIFICATIONS.SORTIE_INTROVIEW_FORTBATTLES_NEXTTIMEOFBATTLETODAY):
                    timetext = makeHtmlString(b'html_templates:lobby/fortifications/introView', b'fortBattles', {b'text': (backport.getShortTimeFormat(data[b'matchmakerNextTick']))})
                wfbDescr = i18n.makeString(textid, nextDate=timetext)
            else:
                isBattleTimerVisible = True
                fontColors = GUI_SETTINGS.lookup(b'StrongholdsTimerColors')
                colorRegular = fontColors.get(b'regular', b'#FFDD99')
                colorAlarm = fontColors.get(b'alarm', b'#ff7f00')
                enemyClanName = b'?'
                if enemyVisible:
                    clColor = enemyclanData.getColor()
                    clColor = b'#%06x' % clColor if clColor else b'#ffffff'
                    enemyClanName = (b"<b><font face='$FieldFont' color='{0}'>[{1}]</font></b>").format(clColor, enemyclanData.getTag())
                clan = data[b'clan']
                if clan:
                    clColor = clan.getColor()
                    clColor = b'#%06x' % clColor if clColor else b'#ffffff'
                    playerClanName = (b"<b><font face='$FieldFont' color='{0}'>[{1}]</font></b>").format(clColor, clan.getTag())
                self.as_setTimerDeltaS(vo_converters.makeClanBattleTimerVO((isInBattle or data)[b'dtime'] if 1 else 0, (b"<font face='$FieldFont' size='18' color='{0}'>###</font>").format(colorRegular), (b"<font face='$FieldFont' size='18' color='{0}'>###</font>").format(colorAlarm), self.TIMER_GLOW_COLORS.NORMAL, self.TIMER_GLOW_COLORS.ALERT, b'00', 0 if data[b'isFirstBattle'] else 1))
                self.as_updateReadyStatusS(self.prbEntity.getFlags().isInQueue(), self.__enemyReadyIndicator)
        self.as_setBattleRoomDataS(self._makeFortClanBattleRoomVO(mapId, headerDescr, playerClanName, enemyClanName, wfbDescr, enemyVisible, isBattleTimerVisible, data[b'isSortie']))
        if data[b'forceUpdateBuildings']:
            self.__forceUpdateBuildings()
        return
        ...
        if GUI_ITEM_TYPE.VEHICLE in diff:
            ...
            if mnVehs:
                for slotIndex, vehCDs in viewitems(self.__vehiclesInSlotFilters):
                    ...
                    self.sendRequest(ChangeVehiclesInSlotFilterCtx(slotIndex, vehCDs or [], waitingID=b'prebattle/change_settings'))
    ...
    def __onUpdateFilterVehicles(self, event):
        slotIndex = event.ctx.get(b'slotIndex', -1)
        if slotIndex:
            self.__setVehicles(slotIndex, event.ctx.get(b'items', None))
            self._updateMembersData()
        return
    ...
    def __setVehicles(self, slotIndex, vehicles=None):
        ...
        self.sendRequest(ChangeVehiclesInSlotFilterCtx(slotIndex, vehicles or [], waitingID=b'prebattle/change_settings'))
        return
    ...
    def __packFilterItem(vType, value):
        return {b'icon': ((b'').join((b'../maps/icons/filters/tanks/', vType, b'.png'))), 
           b'filterValue': value, 
           b'selected': False, 
           b'tooltip': (makeTooltip((b'#menu:carousel_tank_filter/{}').format(vType), b'#fortifications:tooltip/vehicleTypes/body'))}
    ...
    def __setFilters(self, slotIndex, filterState):
        ...
        self.sendRequest(ChangeVehTypesInSlotFilterCtx(slotIndex, vehTypesInFilter, waitingID=b'prebattle/change_settings'))
        return
