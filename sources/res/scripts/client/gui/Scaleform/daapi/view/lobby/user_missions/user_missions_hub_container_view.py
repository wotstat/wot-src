...
class UserMissionsHubContainerView(UserMissionsHubContainerViewMeta):
    ...
    def __init__(self, ctx):
        self.__tabID = ctx.get(b'tab')
        self.__questId = ctx.get(b'questId')
        self.__commonTabView = None
        self.__eventID = ctx.get(b'eventID')
        self.__groupID = ctx.get(b'groupID')
        self.__showMissionDetails = ctx.get(b'showMissionDetails')
        self.__challengeID = ctx.get(b'challengeID')
    ...
    def __onShowTab(self, resetFilters=False):
        ...
        self.as_setBackgroundS(backport.image(background()) if background.exists() else b'')
        return

    def __showMission(self):
        if self.__tabID == TabId.COMMON and self.__groupID is not None:
            self.__commonTabView.as_scrollToItemS(b'blockId', self.__groupID)
