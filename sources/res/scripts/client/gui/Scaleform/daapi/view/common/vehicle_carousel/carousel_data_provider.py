...
def getStatusStrings(vState, vStateLvl=Vehicle.VEHICLE_STATE_LEVEL.INFO, substitute=b'', style=None, ctx=None):
    ctx = ctx or {}
...
def _getVehicleDataVO(vehicle, bootcampCtrl, debutBoxCtrl, earlyAccessCtrl, paragonsCtrl, battleModifiersController=None):
    ...
    if vState in (Vehicle.VEHICLE_STATE.RENTABLE, Vehicle.VEHICLE_STATE.RENTABLE_AGAIN):
        ...
        if rentPackagesInfo.seasonType:
            customStateExt = b'/' + SEASON_NAME_BY_TYPE.get(rentPackagesInfo.seasonType)
    smallStatus, largeStatus = getStatusStrings(vState + customStateExt, vStateLvl, substitute=rentInfoText, ctx={b'icon': (icons.premiumIgrSmall()), b'battlesLeft': (getBattlesLeft(vehicle))})
    ...
    if debutBoxCtrl.isEnabled() and Vehicle.VEHICLE_STATE.UNSUITABLE_TO_QUEUE not in vState and debutBoxCtrl.isQuestsAvailableOnVehicle(vehicle) and debutBoxCtrl.isQuestsAvailableOnBattle():
        debutBoxesImage = getButtonsAssetPath(b'debut_boxes')
    else:
        debutBoxesImage = b''
    ...
    tankType = (b'{}_elite').format(vehicle.type) if vehicle.isElite else vehicle.type
    ...
    extraImage = RES_ICONS.MAPS_ICONS_LIBRARY_RENT_ICO_BIG if showIcon else b''
    data = {b'id': (vehicle.invID), 
       b'intCD': (vehicle.intCD), 
       b'infoText': largeStatus, 
       b'infoHoverText': largeHoverStatus, 
       b'smallInfoText': smallStatus, 
       b'smallInfoHoverText': smallHoverStatus, 
       b'clanLock': (vehicle.clanLock), 
       b'lockBackground': (_isLockedBackground(vState, vStateLvl)), 
       b'unlockedInBattle': False, 
       b'icon': (vehicle.icon), 
       b'iconAlt': (getIconPath(b'noImage')), 
       b'iconSmall': (vehicle.iconSmall), 
       b'iconSmallAlt': (getSmallIconPath(b'noImage')), 
       b'label': (labelStyle(label)), 
       b'level': (vehicle.level), 
       b'premium': (vehicle.isPremium), 
       b'favorite': (vehicle.isFavorite), 
       b'nation': (vehicle.nationID), 
       b'xpImgSource': bonusImage, 
       b'debutBoxesImgSource': debutBoxesImage, 
       b'tankType': tankType, 
       b'rentLeft': rentInfoText, 
       b'clickEnabled': (vehicle.isInInventory and vehicle.activeInNationGroup or vehicle.isRentPromotion), 
       b'alpha': 1, 
       b'infoImgSrc': (getVehicleStateIcon(vState)), 
       b'additionalImgSrc': (getVehicleStateAddIcon(vState)), 
       b'isCritInfo': (vStateLvl == Vehicle.VEHICLE_STATE_LEVEL.CRITICAL), 
       b'isRentPromotion': (vehicle.isRentPromotion and not vehicle.isRented), 
       b'isNationChangeAvailable': (vehicle.hasNationGroup), 
       b'isEarnCrystals': (vehicle.isEarnCrystals), 
       b'isCrystalsLimitReached': isCrystalsLimitReached, 
       b'isUseRightBtn': True, 
       b'tooltip': (battleModifiersController.tooltipConstant), 
       b'isWotPlusSlot': (vehicle.isWotPlus), 
       b'extraImage': extraImage}
    if earlyAccessCtrl.isEnabled():
        data.update({b'isEarlyAccess': (vehicle.intCD in earlyAccessCtrl.getPostProgressionVehicles() and earlyAccessCtrl.isPostProgressionQueueSelected())})
...
class CarouselDataProvider(SortableDAAPIDataProvider):
    ...
    def _getVehicleStats(self, vehicle):
        if vehicle.isOnlyForBattleRoyaleBattles:
            return {b'statsText': b'', 
               b'visibleStats': False}
        else:
            intCD = vehicle.intCD
            ...
            if intCD in vehicleRandomStats:
                ...
                if isMarkOfMasteryAchieved(markOfMastery):
                    markOfMasteryText = makeHtmlString(b'html_templates:lobby/tank_carousel/statistic', b'markOfMastery', ctx={b'markOfMastery': markOfMastery})
                else:
                    markOfMasteryText = b''
                ...
                winsText = makeHtmlString(b'html_templates:lobby/tank_carousel/statistic', b'wins', ctx={b'wins': winsEfficiencyStr})
                ...
                if marksOnGun.getValue() > 0:
                    marksOnGunText = makeHtmlString(b'html_templates:lobby/tank_carousel/statistic', b'marksOnGun', ctx={b'count': (marksOnGun.getValue())})
                template = b'{}    {}  {}' if vehicle.isEarnCrystals else b'{}   {}     {}'
                statsText = template.format(markOfMasteryText, winsText, marksOnGunText)
            else:
                statsText = b'#menu:tankCarousel/statsStatus/unavailable'
            return {b'statsText': (text_styles.stats(statsText)), 
               b'visibleStats': (self._showVehicleStats)}
