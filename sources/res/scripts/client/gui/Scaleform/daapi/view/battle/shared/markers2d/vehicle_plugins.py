...
class VehicleMarkerPlugin(MarkerPlugin, ChatCommunicationComponent, IArenaVehiclesController):
    __slots__ = (b'_markers', b'_markersStates', b'_clazz', b'_markerTimers', b'_isSquadIndicatorEnabled', b'_playerVehicleID', b'__showDamageIcon', b'__markerHitStates', b'__callbackIDs', b'__targetedTankMarkerID', b'__targetedMarkerFromCppID', b'__followingIgnoredTank')
    ...
    def _updateHealthMarker(self, vehicleID, handle, newHealth, *args):
        self._invokeMarker(handle, b'updateHealth', newHealth, *args)
        return

    def _setVehicleInfoMarker(self, vehicleID, handle, *args):
        self._invokeMarker(handle, b'setVehicleInfo', *args)
        return
    ...
    def _onVehicleFeedbackReceived(self, eventID, vehicleID, value):
        ...
        if eventID in MARKER_HIT_EVENTS and self.__showDamageIcon:
            ...
            self._updateMarkerState(handle, b'dead', value)
    ...
    def _getHitStateVO(self, eventID, vehicleID):
        ...
        return (newState if allowedState else b'', backport.text(rText) if rText > 0 else b'', icon, isFrequent)
    ...
    def __onCameraChanged(self, mode, vehicleID=0):
        ...
        if self.__followingIgnoredTankId > 0:
            ...
            if oldMarker and oldVehicleData[b'isAlive']:
        ...
        if not self._needsMarker(vInfo):
            ...
            if marker.isActive():
                if not marker.isAlive():
                    self._updateMarkerState(markerID, b'dead', True, b'')
    ...
    def _updateMarkerState(self, handle, newState, immediate, text=b'', iconAnimation=b'', isFrequent=False):
        if newState or text or iconAnimation:
            self._invokeMarker(handle, b'updateState', newState, immediate, text, iconAnimation, isFrequent)
        return
...
class RespawnableVehicleMarkerPlugin(VehicleMarkerPlugin):
    ...
    def _restoreMarker(self, marker, vProxy, vInfo, guiProps):
        self._invokeMarker(marker.getMarkerID(), b'restore')
