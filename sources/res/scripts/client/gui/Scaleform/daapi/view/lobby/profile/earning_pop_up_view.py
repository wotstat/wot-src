...
class _EarningPopUpView(ViewImpl):
    ...
    __slots__ = (b'__achievementQueue', b'__uiLogging', b'__viewLifecycleWatcher')
    ...
    def __onGoToAchievement(self, args):
        isTrophy = args.get(b'isTrophy')
        isMultiple = args.get(b'isMultiple')
        achievementId = int(args.get(b'achievementId'))
        category = args.get(b'category')
