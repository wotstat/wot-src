...
def getBuyProductMethod(money):
    ...
    if not currency:
        raise SoftException(b'Dyn money is empty')
...
def mayObtainForMoney(money):
    ...
    raise SoftException((b'Unsupported currency: {}').format(money.getCurrency()))
    return
...
def getUserMoney(currency, itemsCache=None):
    ...
    raise SoftException((b'Unsupported currency: {}').format(currency))
    return
