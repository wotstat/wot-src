...
class PveScaleCenteredPersonalEntriesPlugin(PersonalEntriesPlugin):
    ...
    def _onMinimapBordersUpdated(self, event):
        borders = event.ctx.get(b'minimapBorders', PVE_MINIMAP_DEFAULT_BORDERS)
    ...
    def _onZoomLevelUpdated(self, event):
        zoomLevel = event.ctx.get(b'zoomLevel', 0)
        if zoomLevel < PVE_MINIMAP_DEFAULT_ZOOM:
            _logger.warning(b'zoomLevel is out of scope: %f', zoomLevel)
        self._centerMapBasedOnMode()
...
class PveMinimapComponent(PveMinimapMeta):
    ...
    def _setupPlugins(self, arenaVisitor):
        ...
        setup[b'personal'] = PveScaleCenteredPersonalEntriesPlugin
        return setup
    ...
    def _setMinimapShortcutLabel(self):
        self.as_setShorcutLabelS(b'' if not self._canToggleFullMap else getReadableKey(CommandMapping.CMD_MINIMAP_VISIBLE))
        return
