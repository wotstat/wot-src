...
class BattleSessionList(PrebattlesListWindow, BattleSessionListMeta):
    ...
    def requestToJoinTeam(self, prbID, prbType):
        ...
        if self.lobbyContext.isAnotherPeriphery(item.peripheryID):
            self.fireEvent(events.LoadViewEvent(SFViewLoadParams(PREBATTLE_ALIASES.AUTO_INVITE_WINDOW_PY), ctx={b'prbID': prbID}), scope=EVENT_BUS_SCOPE.LOBBY)
        else:
    ...
    def onFocusIn(self, alias):
        self.fireEvent(FocusEvent(FocusEvent.COMPONENT_FOCUSED, {b'clientID': (self.getClientID())}))
        return
    ...
    def __requestToJoin(self, prbID, prbType, prbClusterID):
        yield self.prbDispatcher.join(JoinBattleSessionCtx(prbID, prbType, prbClusterID, b'prebattle/join'))
        return
    ...
    def __onBSListReceived(self, sessions):
        ...
        for bs in sessions:
            ...
            result.append({b'prbID': (bs.prbID), 
               b'prbType': (bs.prbType), 
               b'descr': (formatters.getPrebattleFullDescription(bs.description)), 
               b'opponents': (formatters.getPrebattleOpponentsString(bs.description)), 
               b'startTime': startTime, 
               b'unitName': detachment, 
               b'vehicleLevel': vehicleLvl})

        self.as_refreshListS(result)
        return

    def __getClanBattleStartTime(self, battleSession):
        arenaName = getArenaShortName(battleSession.arenaTypeID) or b''
