...
class PostProgressionContextMenuHandler(SimpleVehicleCMHandler):
    ...
    def __init__(self, cmProxy, ctx=None):
        super(PostProgressionContextMenuHandler, self).__init__(cmProxy, ctx, {(VEHICLE.INFO): b'showVehicleInfo', 
    ...
    def __addCompareOption(self, options, vehicle):
        if self.__cmpBasket.isEnabled():
            options.append(self._makeItem(VEHICLE.COMPARE, MENU.contextmenu(VEHICLE.COMPARE), {b'enabled': (self.__cmpBasket.isReadyToAdd(vehicle))}))
        return
    ...
    def __addSelectOption(self, options, vehicle):
        options.append(self._makeItem(VEHICLE.SELECT, MENU.CONTEXTMENU_SELECTVEHICLEINHANGAR, {b'enabled': (vehicle.isInInventory or vehicle.rentalIsOver)}))
        return True
