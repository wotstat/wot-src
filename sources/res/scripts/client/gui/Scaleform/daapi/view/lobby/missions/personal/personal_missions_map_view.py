...
class PersonalMissionsMapView(PersonalMissionsMapViewMeta, PersonalMissionsNavigation):
    ...
    def refresh(self):
        ...
        quests = sorted(self.getChain().itervalues(), key=operator.methodcaller(b'getID'))
        questClassifier = b''
        enabled = False
        ...
        self.as_setPlanDataS({b'regions': questsData, 
           b'enabled': enabled, 
           b'chainID': chainID, 
           b'operationID': (operation.getID()), 
           b'awardsBlockVO': awards})
        return
    ...
    def __getQuestStatusData(self, chainID, quest):
        ...
        vo = {b'id': questId, 
           b'state': state, 
           b'isLocked': (isFinal and not isUnlocked and not areTokensPawned or isDisabled), 
           b'isOnPause': isOnPause, 
           b'tooltipData': (getMapRegionTooltipData(state, quest)), 
           b'isTokenPawned': areTokensPawned, 
           b'isFinal': isFinal, 
           b'vehType': chainID, 
           b'sheet': (AwardSheetPresenter.getIcon(AwardSheetPresenter.Size.TINY))}
        return vo

    def __getFinalQuestAwardsVO(self, quest):
        tokenAward = findFirst((lambda q: q.getName() == b'completionTokens'), quest.getBonuses(isMain=True))
        ...
        elif chainID == ALLIANCE_CHAIN_ID and (operationID == OPERATION_ID_CHIMERA or operationID == OPERATION_ID_OBJECT_279):
            mainAwardTextID = MAIN_AWARD_TEXT_PM2_ID_HULL
        mainAwardText = _ms(_MAIN_AWARD_TEXT_PM2[mainAwardTextID])
        return {b'mainAwardText': mainAwardText, 
           b'mainAward': (first(mainAwards))}
