...
class PostbattleRankedAwardsView(RankedBattlesAwardsViewMeta):
    ...
    __slots__ = (b'__awardsSequence', b'__rankedInfo')

    def __init__(self, ctx=None):
        super(PostbattleRankedAwardsView, self).__init__()
        self.__awardsSequence = ctx[b'awardsSequence']
        self.__rankedInfo = ctx[b'rankedInfo']
        return
    ...
    def _populate(self):
        ...
        self.as_setDataS({b'vosSequence': vosSequence, 
           b'title': (backport.text(R.strings.ranked_battles.awards.congratulation())), 
           b'nextButtonLabel': (backport.text(R.strings.ranked_battles.awards.yes()))})
        return
