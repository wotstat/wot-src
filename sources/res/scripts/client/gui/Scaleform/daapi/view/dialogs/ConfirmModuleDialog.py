...
class ConfirmModuleDialog(ConfirmItemWindowMeta):
    ...
    def _prepareAndSendData(self):
        ...
        if item is not None:
            ...
            resultData = {b'id': (self.meta.getTypeCompDescr()), 
               b'price': (getMoneyVO(actualPrices)), 
               b'actionPriceData': action, 
               b'icon': icon, 
               b'name': (item.userName), 
               b'description': (item.getShortInfo()), 
               b'currency': currency, 
               b'defaultValue': (self.meta.getDefaultValue(item)), 
               b'maxAvailableCount': (self.meta.getMaxAvailableItemsCount(item)), 
               b'hasSeveralPrices': hasSeveralPrices, 
               b'moduleLabel': (item.getGUIEmblemID()), 
               b'level': (item.level), 
               b'linkage': (CONFIRM_DIALOG_ALIASES.MODULE_ICON), 
               EXTRA_MODULE_INFO: extraData, 
               FIELD_HIGHLIGHT_TYPE: (item.getHighlightType()), 
               b'overlayType': (item.getOverlayType())}
            self.as_setDataS(resultData)
        else:
            LOG_ERROR(b"Couldn't find module with given compact:", self.meta.getTypeCompDescr())
            self.onWindowClose()
        return
