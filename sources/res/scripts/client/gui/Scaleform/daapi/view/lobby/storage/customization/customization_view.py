...
class StorageCategoryCustomizationView(StorageCategoryCustomizationViewMeta):
    ...
    def _getVO(self, item, vehicleCD=None):
        ...
        vo = createStorageDefVO(itemID=item.intCD, title=title, description=customizationSuitableText, count=count, price=priceVO if isAvailableForSell else None, image=icon, imageAlt=b'altimage', contextMenuId=CONTEXT_MENU_HANDLER_TYPE.STORAGE_CUSTOMZIZATION_ITEM, enabled=isAvailableForSell)
