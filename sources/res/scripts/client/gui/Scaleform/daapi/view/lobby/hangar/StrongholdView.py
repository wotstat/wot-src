...
class StrongholdView(LobbySubView, StrongholdViewMeta):
    ...
    def __init__(self, ctx=None):
        ...
        self.__browserId = 0
        self.__url = ctx.get(b'url', getStrongholdUrl()) if ctx is not None else getStrongholdUrl()
        return
    ...
    def __loadBrowser(self, width, height):
        if self.__url:
            ...
            LOG_ERROR(b'Setting "StrongholdsTabUrl" missing!')
        return
    ...
    def __showBrowser(self):
        ...
        self.fireEvent(events.StrongholdEvent(events.StrongholdEvent.STRONGHOLD_LOADED, {b'browserID': (self.__browserId)}), scope=EVENT_BUS_SCOPE.STRONGHOLD)
        return
