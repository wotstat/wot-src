...
class MissionsTabBarView(ViewImpl):
    __slots__ = (b'__tabsData', b'__onTabSelectionChangedCallback')
    ...
    def setTabs(self, tabs):
        ...
        with self.viewModel.transaction() as model:
            ...
            for tabData in tabs:
                ...
                viewModel.setAlias(tabData[b'alias'])
                viewModel.setIcon(tabData[b'icon'])
                viewModel.setLinkage(tabData[b'linkage'])
                viewModel.setTooltipHeader(_ms(tabData[b'tooltip'] + b'/header'))
                viewModel.setTooltipBody(_ms(tabData[b'tooltip'] + b'/body'))
                viewModel.setUnseenCount(tabData[b'unseenCount'])
                viewModel.setEnabled(tabData[b'enabled'])
                if tabData[b'selected']:
    ...
    def __onTabSelectionChanged(self, args=None):
        index = int(args[b'index'])
        if index <= len(self.__tabsData):
            tabData = self.__tabsData[index]
            if tabData is not None:
                if self.__onTabSelectionChangedCallback:
                    self.__onTabSelectionChangedCallback(tabData[b'alias'], tabData[b'linkage'], tabData[b'prefix'])
        else:
            _logger.error(b'Invalid tab index selected %d', index)
        return
