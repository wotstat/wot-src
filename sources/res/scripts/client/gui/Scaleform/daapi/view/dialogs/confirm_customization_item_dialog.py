...
class ConfirmCustomizationItemDialog(ConfirmItemWindowMeta):
    ...
    def _populate(self):
        ...
        g_clientUpdateManager.addCallbacks({b'stats': (self.__onUpdate)})
    ...
    def _prepareAndSendData(self):
        ...
        if item is not None:
            ...
            if item.isProgressive and self.meta.vehicle is not None:
                ...
                icon = item.icon
            smallSlotVO = {b'itemIcon': icon, 
               b'isBgVisible': False, 
               b'isFrameVisible': True, 
               b'iconWidth': iconWidth, 
               b'iconHeight': iconHeight}
            stepFactor = self.meta.getStepFactor(item)
            resultData = {b'id': (item.intCD), 
               b'price': (getMoneyVO(actualPrices)), 
               b'actionPriceData': action, 
               b'name': (item.userName), 
               b'description': (item.userType), 
               b'currency': currency, 
               b'defaultValue': (self.meta.getDefaultValue(item) * stepFactor), 
               b'maxAvailableCount': (self.meta.getMaxAvailableItemsCount(item)), 
               b'hasSeveralPrices': hasAlternativePrice, 
               b'smallSlotVO': smallSlotVO, 
               b'countLabel': countLabel, 
               b'stepSize': stepFactor}
            self.as_setDataS(resultData)
        else:
            LOG_ERROR(b"Couldn't find given customization item: ", self.meta.getItem())
            self.onWindowClose()
        return
