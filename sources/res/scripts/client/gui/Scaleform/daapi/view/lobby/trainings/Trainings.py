...
class Trainings(TrainingsListBase):
    ...
    def _getViewData(self):
        return {b'title': (backport.text(R.strings.menu.training.title())), 
           b'descr': (backport.text(R.strings.menu.training.description())), 
           b'battleTypeID': (BATTLE_TYPES.TRAINING)}

    @adisp_process
    def joinTrainingRequest(self, prbID, prbClusterID):
        yield self.prbDispatcher.join(JoinTrainingCtx(prbID, prbClusterID, waitingID=b'prebattle/join'))
        return

    def createTrainingRequest(self):
        settings = TrainingSettingsCtx()
        self.fireEvent(events.LoadViewEvent(SFViewLoadParams(PREBATTLE_ALIASES.TRAINING_SETTINGS_WINDOW_PY), ctx={b'isCreateRequest': True, b'settings': settings}), scope=EVENT_BUS_SCOPE.LOBBY)
        return
