...
_ViewSettings = namedtuple(b'_ViewSettings', (b'viewID', b'linkage', b'name', b'hasView'))
...
class GunMarkerComponent(IGunMarkerComponent):
    ...
    def addView(self, container):
        if self._view is not None:
            _logger.error(b'GunMarkerComponent "%s" is already created.', self._name)
            return
        else:
            self._view = self._createView(container)
            _logger.debug(b'GunMarkerComponent "%s" is created', self._name)
            if self._isActive:
    ...
    def removeView(self, container):
        if self._view is None:
            _logger.error(b'GunMarkerComponent "%s" is already removed.', self._name)
            return
        else:
    ...
    def __init__(self, components):
        ...
        for component in components:
            ...
            if name in self.__components:
                raise SoftException((b'Name of component must be unique. {} is already existed').format(name))
