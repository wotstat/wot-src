...
def getRankedFightBtnTooltipData(result):
    ...
    if state == PRE_QUEUE_RESTRICTION.MODE_NOT_SET:
        ...
        levelStr = toRomanRangeString(result.ctx[b'levels'])
        ...
        typeSubStr = text_styles.neutral(result.ctx[b'forbiddenType'])
        ...
        classSubStr = text_styles.neutral(getTypeUserName(result.ctx[b'forbiddenClass'], False))
...
def getEpicFightBtnTooltipData(result):
    ...
    if state == PRE_QUEUE_RESTRICTION.MODE_NOT_AVAILABLE:
        ...
        body = text_styles.main(backport.text(R.strings.menu.headerButtons.fightBtn.tooltip.epicBattleVehLevel.body(), requirements=text_styles.neutral(backport.text(R.strings.menu.headerButtons.fightBtn.tooltip.epicBattleVehLevel.requirements(), level=toRomanRangeString(result.ctx[b'levels'])))))
...
def getEventTooltipData():
    ...
    body = i18n.makeString(TOOLTIPS.EVENT_SQUAD_DISABLE_BODY, tankName=b'')
    return makeTooltip(header, body)
...
def getMapboxFightBtnTooltipData(result):
    ...
    if restriction in (PRE_QUEUE_RESTRICTION.MODE_NOT_AVAILABLE, UNIT_RESTRICTION.CURFEW):
        ...
        levels = backport.text(strPath.mapboxVehLevel.levelSubStr(), levels=toRomanRangeString(result.ctx[b'levels']))
...
def getVersusAIFightBtnTooltipData(result):
    ...
    if restriction == PRE_QUEUE_RESTRICTION.LIMIT_LEVEL:
        levels = backport.text(resShortCut.versusAIVehLevel.levelSubStr(), levels=toRomanRangeString(result.ctx[b'levels']))
