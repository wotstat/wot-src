...
class SelectableCrewbookTooltipData(BlocksTooltipData):
    ...
    def _packBlocks(self, item, **kwargs):
        return [
         self.__packImageBlock(item.name),
         self.__packMainBlock(item),
         self.__packFooterBlock(item.name)]

    @staticmethod
    def __packFooterBlock(itemName):
        return formatters.packBuildUpBlockData([
         formatters.packTextBlockData(text=text_styles.standard(backport.text(R.strings.tooltips.selectableCrewbook.dyn(itemName).footer())), padding=formatters.packPadding(top=8))], padding=formatters.packPadding(left=-1))
    ...
    def __packMainBlock(cls, item):
        ...
        blocks = [
         formatters.packTextBlockData(text=text_styles.highTitle(backport.text(strPath.dyn(item.name).title())))]
