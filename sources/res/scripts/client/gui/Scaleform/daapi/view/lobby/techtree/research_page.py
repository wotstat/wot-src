...
def _formatBenefits(benefitData):
    ...
    for _, (icon, title, body) in enumerate(benefitData, 1):
        formattedBenefits.append({b'iconSrc': icon, 
           b'labelStr': (text_styles.concatStylesToMultiLine(text_styles.highTitle(title), text_styles.main(body)))})

    return formattedBenefits


def _getRestoreBannerStr(param):
    if param != float(b'inf'):
...
def _getUnlockedBannerStr(param):
    return text_styles.concatStylesToMultiLine(text_styles.mainBig(backport.text(R.strings.blueprints.blueprintScreen.researchSale.label())), text_styles.grandTitle((b'').join((b'-', str(param), b'%'))))


def _getRentBannerStr(param):
    return text_styles.concatStylesToMultiLine(text_styles.mainBig(backport.text(R.strings.menu.research.premium.discount())), text_styles.grandTitle((b'').join((b'-', str(param), b'%'))))


def _getActionBannerStr(paramDiscount, paramDate=b''):
    discountStr = text_styles.grandTitle((b'').join((b'-', str(paramDiscount), b'%')))
    if not paramDate:
        return discountStr
...
class Research(ResearchMeta):
    ...
    def __del__(self):
        _logger.debug(b'ResearchPage deleted')
        return
    ...
    def _getRootData(self):
        ...
        comparisonState, comparisonTooltip = resolveStateTooltip(self._cmpBasket, root, enabledTooltip=b'', fullTooltip=TOOLTIPS.RESEARCHPAGE_VEHICLE_BUTTON_COMPARE_DISABLED)
        ...
        result = {b'vehicleTitle': {b'intCD': (self._data.getRootCD()), 
                             b'tankTierStr': (text_styles.grandTitle(tankTier)), 
                             b'tankNameStr': (text_styles.grandTitle(tankName)), 
                             b'tankTierStrSmall': (text_styles.promoTitle(tankTier)), 
                             b'tankNameStrSmall': (text_styles.promoTitle(tankName)), 
                             b'typeIconPath': (getTypeBigIconPath(root.type, root.isElite)), 
                             b'isElite': (root.isElite), 
                             b'statusStr': (self.__getRootStatusStr(root)), 
                             b'roleText': (getRoleTextWithIcon(root.role, root.roleLabel)), 
                             b'ability': (getAbilityVO(self.vehicle))}, 
           b'vehicleButton': {b'shopIconPath': (getShopVehicleIconPath(STORE_CONSTANTS.ICON_SIZE_MEDIUM, root.name.split(b':')[1])), 
                              b'compareBtnVisible': (not self.__bootcamp.isInBootcamp()), 
                              ...
                              b'isEarlyAccess': isEarlyAccess, 
                              b'vehicleId': (self._data.getRootCD()), 
                              b'vehicleState': nodeState, 
                              b'isInInventory': (NODE_STATE.inInventory(nodeState)), 
                              b'previewAlias': (VIEW_ALIAS.LOBBY_RESEARCH), 
                              b'cmHandlerType': (CONTEXT_MENU_HANDLER_TYPE.RESEARCH_VEHICLE)}, 
           b'isInteractive': (self.__getIsInteractive(root, rootNode)), 
           b'buttonLabel': (self.__getMainButtonLabel(root, rootNode)), 
           b'blueprintLabel': (self.__getResearchPageBlueprintLabel(rootNode)), 
           b'blueprintProgress': (rootNode.getBlueprintProgress()), 
           b'blueprintCanConvert': (bpfProps.canConvert if bpfProps is not None else False), 
           b'bpbGlowEnabled': isNext2Unlock, 
           b'itemPrices': (rootNode.getItemPrices()), 
           b'discountStr': (self.__getDiscountBannerStr(root, rootNode)), 
           b'rentBtnLabel': (self.__getRentButtonLabel(rootNode)), 
           b'changeNationBtnVisibility': tankHasNationGroup, 
           b'isTankNationChangeAvailable': isNationChangeAvailable, 
           b'nationChangeIsNew': (not AccountSettings.getSettings(NATION_CHANGE_VIEWED)), 
           b'nationChangeTooltip': (self.__getNationChangeTooltip(root) if isShownNationChangeTooltip else b'')}
        return result
    ...
    def __getDiscountBannerStr(self, root, rootNode):
        ...
        if NODE_STATE.isRestoreAvailable(nodeState):
            ...
            restoreDueDate = getDueDateOrTimeStr(restoreFinishTime) if restoreFinishTime < float(b'inf') else float(b'inf')
            if restoreDueDate:
        ...
        if not NODE_STATE.inInventory(nodeState) and NODE_STATE.isActionVehicle(nodeState) or NODE_STATE.isCollectibleActionVehicle(nodeState):
            ...
            if discount != 0:
                actionDueDate = b'' if isPersonalDiscount else getDueDateOrTimeStr(rootNode.getActionFinishTime(), isShortDateFormat=True)
    ...
    def __getViewLayoutData(self):
        ...
        result[b'isPremiumLayout'] = root.isPremium
        if root.isPremium:
            benefitData = []
            for benefitGetter in _BENEFIT_GETTERS:
                benefitGetter(benefitData, root)

            result[b'benefitsData'] = _formatBenefits(benefitData)
        return result

    def __getBackBtnData(self):
        result = {b'backBtnLabel': (backport.text(R.strings.menu.viewHeader.backBtn.label())), 
           b'backBtnDescrLabel': (getBackBtnDescription(self._exitEvent, self._previewAlias))}
        return result
