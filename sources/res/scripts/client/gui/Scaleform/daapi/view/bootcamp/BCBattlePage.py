...
_BOOTCAMP_MINIMAP_SIZE_SETTINGS_KEY = b'bootcampMinimapSize'

class _BCComponentsConfig(ComponentsConfig):
    BC_FINISH_SOUND_PLAYER = b'bcFinishSoundPlayer'

    def __init__(self):
        super(_BCComponentsConfig, self).__init__((
         (
...
class BootcampMinimapComponent(ClassicMinimapComponent):
    ...
    def _setupPlugins(self, arenaVisitor):
        ...
        setup[b'bootcamp'] = BootcampTargetPlugin
        setup[b'settings'] = BootcampMinimapSettingsPlugin
        try:
            lessonId = arenaVisitor.getArenaExtraData()[b'lessonId']
            if BATTLE_VIEW_ALIASES.MINIMAP in getBattleSettings(lessonId).hiddenPanels:
                setup[b'settings'] = BootcampMinimapDisablePlugin
        except KeyError:
            LOG_ERROR_BOOTCAMP(b"Extra data doesn't contain lessonId")
...
class BCBattlePage(BCBattlePageMeta):
    ...
    def onAnimationsComplete(self):
        LOG_DEBUG_DEV_BOOTCAMP(b'onAnimationsComplete')
    ...
    def __hideGUIForWinMessage(self):
        ...
        self.fireEvent(events.GameEvent(events.GameEvent.GUI_VISIBILITY, {b'visible': False}), scope=EVENT_BUS_SCOPE.BATTLE)
