...
_ERROR_ICON_DESC = {b'image': (RES_ICONS.MAPS_ICONS_CUSTOMIZATION_INSCRIPTION_CONTROLLER_ICON_ERROR)}
_ENTER_ICON_DESC = {b'image': (RES_ICONS.MAPS_ICONS_CUSTOMIZATION_INSCRIPTION_CONTROLLER_ENTER_BTN)}
_BACKSPACE_ICON_DESC = {b'image': (RES_ICONS.MAPS_ICONS_CUSTOMIZATION_INSCRIPTION_CONTROLLER_BSPACE_BTN)}
_DEL_ICON_DESC = {b'text': (READABLE_KEY_NAMES.KEY_DELETE)}
_PRESS_ENTER_HINT_SHOWN_FIELD = b'isPressEnterHintShown'
_ENTER_NUMBER_HINT_SHOWN_FIELD = b'isEnterNumberHintShown'
...
class CustomizationInscriptionController(CustomizationInscriptionControllerMeta, CallbackDelayer):
    ...
    def __showProhibitedHint(self, number):
        message = makeHtmlString(b'html_templates:lobby/customization', b'inscription_hint', {b'value': number})
    ...
    def __showPromptHint(self, showImmediately=False):
        message = _ms(VEHICLE_CUSTOMIZATION.PROPERTYSHEET_INSCRIPTIONCONTROLLER_PROMPT, start=formatPersonalNumber(b'1', self._digitsCount), end=b'9' * self._digitsCount)
        if showImmediately:
    ...
    def __getHintVO(self, hintMessage, icons=None, duration=0, delay=0):
        return {b'hintMessage': hintMessage, 
           b'icons': icons, 
           b'duration': duration, 
           b'delay': delay}
