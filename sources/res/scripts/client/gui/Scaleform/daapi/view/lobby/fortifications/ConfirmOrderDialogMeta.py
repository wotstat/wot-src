...
class UnfreezeVehicleDialogMeta(I18nConfirmDialogMeta):
    ...
    def __init__(self, playerName, vehicleName, price, balance):
        ...
        super(UnfreezeVehicleDialogMeta, self).__init__(b'wgshEventUnfreezeVehicle', focusedID=DIALOG_BUTTON_ID.SUBMIT)
        return

    def getMessage(self):
        priceStyle = text_styles.discountSmallText if self.__price <= self.__balance else text_styles.failedStatusText
        return makeHtmlString(b'html_templates:lobby/dialogs', b'wgshEventUnreezeVehicle', {b'first_part': (text_styles.leadingText(text_styles.alert(backport.text(R.strings.dialogs.wgshEventUnfreezeVehicle.messagePart.first())), 8)), 
           b'second_part': (backport.text(R.strings.dialogs.wgshEventUnfreezeVehicle.messagePart.second(), player_name=text_styles.discountSmallText(self.__playerName))), 
           b'third_part': (text_styles.leadingText(backport.text(R.strings.dialogs.wgshEventUnfreezeVehicle.messagePart.third(), vehicle_name=text_styles.discountSmallText(self.__vehicleName)), 8)), 
           b'fourth_part': (backport.text(R.strings.dialogs.wgshEventUnfreezeVehicle.messagePart.fourth(), price=priceStyle(self.__price), balance=text_styles.discountSmallText(self.__balance), icon=icons.makeImageTag(backport.image(R.images.gui.maps.icons.library.wgshSpareParts()))))})
