...
class PointsOfInterestPanel(PointsOfInterestNotificationPanelMeta, PointsOfInterestListener):
    ...
    def onPoiAdded(self, poiState):
        poiVO = {b'id': (poiState.id), 
           b'type': (POI_TYPE_UI_MAPPING[poiState.type]), 
           b'status': (POI_STATUS_UI_MAPPING[poiState.status.statusID]), 
           b'isAlly': (self.__isAlly(poiState)), 
           b'progress': (poiState.progress)}
