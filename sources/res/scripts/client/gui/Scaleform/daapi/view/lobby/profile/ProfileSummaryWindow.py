...
class ProfileSummaryWindow(ProfileSummaryWindowMeta, ClanEmblemsHelper, ClanListener):
    ...
    def _requestClanInfo(self):
        ...
        if clanInfo is not None:
            clanInfo = ClanInfo(*clanInfo)
            clanData = {b'id': clanDBID, 
               b'header': (clanInfo.getClanName()), 
               b'topLabel': (_ms(PROFILE.PROFILE_SUMMARY_CLAN_POST)), 
               b'topValue': (text_styles.main(getClanRoleString(clanInfo.getMembersFlags()))), 
               b'bottomLabel': (_ms(PROFILE.PROFILE_SUMMARY_CLAN_JOINDATE)), 
               b'bottomValue': (text_styles.main(backport.getLongDateFormat(clanInfo.getJoiningTime())))}
    ...
    def _getClanBtnParams(self, isVisible):
        ...
        return {b'btnLabel': (_ms(PROFILE.PROFILE_SUMMARY_CLAN_BTNLABEL)), 
