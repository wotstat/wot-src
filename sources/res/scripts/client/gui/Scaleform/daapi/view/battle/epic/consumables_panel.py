...
class EpicBattleConsumablesPanel(ConsumablesPanel):
    ...
    def _getEquipmentIcon(self, idx, item, icon):
        ...
        return super(EpicBattleConsumablesPanel, self)._getEquipmentIcon(idx, item, icon)
    ...
    def _updateEquipmentSlot(self, idx, item):
        ...
        if prevQuantity < quantity:
            self.as_updateLockedInformationS(idx, self._EMPTY_LOCKED_SLOT, b'')
    ...
    def __addLockedInformationToEpicEquipment(self, idx):
        ...
        if playerDataComp is None:
            ...
            if rank <= currentRank - 1:
                return
            rank += 1
            tooltipId = TOOLTIPS_CONSTANTS.EPIC_RANK_UNLOCK_INFO if rank > 1 else b''
