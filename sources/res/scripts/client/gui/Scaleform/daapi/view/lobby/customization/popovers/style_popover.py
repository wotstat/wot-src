...
class StylePopoverDataProvider(SortableDAAPIDataProvider):
    ...
    def buildList(self):
        ...
        for season in POPOVER_SEASONS_ORDER:
            ...
            seasonTitle = makeHtmlString(b'html_templates:lobby/customization/StylePopoverSeasonName', seasonName, ctx={b'align': b'LEFT'})
            itemsData = self.__getSeasonItemsData(style, season, vehicleDescriptor)
            if itemsData:
                seasonGroupVO = {b'name': seasonTitle, b'itemIcons': itemsData}
    ...
    def __getModifiedOutfit(self, season, style, vehicleCD=b''):
