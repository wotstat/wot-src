...
class BCResearchItemsData(ResearchItemsData):
    ...
    def __init__(self, dumper):
        ...
        self.__overrideResearch = lessonNum < g_bootcamp.getContextIntParameter(b'researchFreeLesson')
        self.__secondVehicleResearch = lessonNum < g_bootcamp.getContextIntParameter(b'researchSecondVehicleLesson')
        nationData = g_bootcamp.getNationData()
        self.__firstVehicleNode = nationData[b'vehicle_first']
        self.__secondVehicleNode = nationData[b'vehicle_second']
        self.__moduleNodeCD = nationData[b'module']
        return
...
class BCResearch(Research):
    ...
    BC_MESSAGE_WINDOW_OPEN_SOUND_ID = b'bc_info_line_woosh'
    SECOND_VEHICLE_FLAG = b'SecondVehicleIsOnResearch'
    ...
    @simpleLog(action=BC_LOG_ACTIONS.UNLOCK_ITEM, restrictions={b'lesson_id': (lambda x: x <= LIMITS.RESEARCH_MAX_LESSON)}, logOnce=True)
    ...
    @simpleLog(action=BC_LOG_ACTIONS.BUY_ITEM, restrictions={b'lesson_id': (lambda x: x <= LIMITS.RESEARCH_MAX_LESSON)}, logOnce=True)
    ...
    def redraw(self):
        ...
        nationData = g_bootcamp.getNationData()
        isSecondVehicle = self._data.getRootCD() == nationData[b'vehicle_second']
    ...
    def _getRootData(self):
        ...
        result[b'vehicleButton'][b'compareBtnEnabled'] = False
        result[b'vehicleButton'][b'goToVehicleViewBtnVisible'] = False
        return result
