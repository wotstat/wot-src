...
class ConfirmExchangeDialog(ConfirmExchangeDialogMeta):
    ...
    def onSelectedAmountChanged(self, goldValue, needItemsValue):
        if goldValue != self.__selectedGold:
            ...
            goldValue = 0
        self.__selectedGold, self.__selectedResource = handleAndRoundStepperInput({b'gold': goldValue, b'currency': needItemsValue}, exchangeRate=self.meta.getExchangeProvider(), validateGold=True)
        ...
        g_eventBus.handleEvent(events.ExchangeRatesDiscountsEvent(events.ExchangeRatesDiscountsEvent.ON_SELECTED_AMOUNT_CHANGED, {b'amount': (self.__selectedGold)}), scope=EVENT_BUS_SCOPE.LOBBY)
        return

    def openDiscountInfoPage(self):
        showAllPersonalDiscountsWindow(exchangeRateType=self.meta.getExchangeType(), selectedValue={b'currency': (self.__selectedResource)})
        return
