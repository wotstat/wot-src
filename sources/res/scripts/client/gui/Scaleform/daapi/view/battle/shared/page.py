...
class SharedPage(BattlePageMeta):
    ...
    def __del__(self):
        _logger.debug(b'SharedPage is deleted')
        return
    ...
    @uniprof.regionDecorator(label=b'avatar.show_gui', scope=b'enter')
    def _populate(self):
        self._startBattleSession()
    ...
    @uniprof.regionDecorator(label=b'avatar.show_gui', scope=b'exit')
    def _dispose(self):
    ...
    def _toggleGuiVisible(self):
        ...
        self.fireEvent(events.GameEvent(events.GameEvent.GUI_VISIBILITY, {b'visible': (self._isVisible)}), scope=EVENT_BUS_SCOPE.BATTLE)
    ...
    def _setComponentsVisibility(self, visible=None, hidden=None):
        ...
        if visible or hidden:
            _logger.debug(b'Sets components visibility: visible = %r, hidden = %r', visible, hidden)
    ...
    def _setComponentsVisibilityWithFade(self, visible=None, hidden=None):
        ...
        if hidden is None:
            hidden = set()
        if visible or hidden:
            _logger.debug(b'Sets components visibility with fade: visible = %r, hidden = %r', visible, hidden)
    ...
    def _startBattleSession(self):
        if self.sessionProvider.registerViewComponents(*self.__componentsConfig.getConfig()):
            ...
            _logger.warning(b'View components can not be added into battle session provider')
    ...
    def __handleCalloutDisplayEvent(self, event):
        self._processCallout(needShow=event.ctx[b'isDown'])
        return
