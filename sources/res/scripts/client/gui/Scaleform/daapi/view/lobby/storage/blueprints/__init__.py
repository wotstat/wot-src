...
def blueprintExitEvent():
    return events.LoadViewEvent(SFViewLoadParams(VIEW_ALIAS.LOBBY_STORAGE), ctx={b'defaultSection': (STORAGE_CONSTANTS.BLUEPRINTS)})
...
class BlueprintsStorageCarouselDataProvider(StorageCarouselDataProvider):
    ...
    def _buildVehicle(self, item):
        ...
        intelligenceIcon = RES_ICONS.getBlueprintFragment(b'special', b'intelligence')
        ...
        return {b'id': (item.intCD), 
           b'title': name, 
           b'description': description, 
           b'image': image, 
           b'imageAlt': (getShopVehicleIconPath(STORE_CONSTANTS.ICON_SIZE_SMALL, b'empty_tank')), 
           b'fragmentsCost': fragmentsCost, 
           b'intelligenceCostText': intelligenceCostText, 
           b'fragmentsProgress': fragmentsProgress, 
           b'hasDiscount': (discount > 0), 
           b'availableToUnlock': availableToUnlock, 
           b'convertAvailable': (availableCount > 0), 
           b'contextMenuId': (CONTEXT_MENU_HANDLER_TYPE.STORAGE_BLUEPRINTS_ITEM)}
    ...
    def __formatFragmentProgress(self, current, total, discount):
        return text_styles.alignText((b'').join((
         text_styles.credits(backport.getIntegralFormat(current)),
         text_styles.main((b'').join((b' / ', backport.getIntegralFormat(total)))),
         text_styles.credits((b'').join((b'   ', backport.getIntegralFormat(discount), b'%'))) if discount > 0 else b'')), b'right')
    ...
    def __formatFragmentsCost(self, intelligenceCost, intelligenceIcon, nationalsCost):
        ...
        for index, (nId, cost) in enumerate(nationalsCost.iteritems()):
            ...
            nationalCostTexts.append({b'costStr': (self.__formatFragment(nationalsCost, nationalIcon)), 
               b'delimeterOffset': (-5), 
               b'hasDelimeter': (index < lastPriceIdx)})
