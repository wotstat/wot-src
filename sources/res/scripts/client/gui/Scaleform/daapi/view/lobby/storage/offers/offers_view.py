...
class StorageCategoryOffersView(StorageCategoryOffersViewMeta):
    ...
    def _syncOffers(self, callback=None):
        ...
        currentIDs = {offerVO[b'id'] for offerVO in self._dataProvider.collection}
    ...
    def _getVO(self, offer):
        ...
        vo = createStorageDefVO(offer.id, title, description, 0, None, self._offersProvider.getCdnResourcePath(offer.cdnLogoPath, relative=False), b'altimage', contextMenuId=None, sellBtnLabel=backport.text(R.strings.storage.buttonLabel.select()))
        return vo
