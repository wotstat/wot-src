...
class CustomizationItemCMHandler(AbstractContextMenuHandler):
    ...
    def __getSellBtn(self, item):
        ...
        if showAlert:
            ...
            price = sellPriceVO[0][b'price']
            sellPriceVO[0] = {b'price': price}
        else:
            tooltipVO = None
        ...
        btn = self._makeItem(optId=CustomizationOptions.SELL, optLabel=MENU.cst_item_ctx_menu(CustomizationOptions.SELL), optInitData={b'data': ({b'price': (first(sellPriceVO))} if availableForSale else None), 
