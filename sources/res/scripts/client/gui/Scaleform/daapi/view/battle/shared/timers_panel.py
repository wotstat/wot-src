...
class _BaseTimersCollection(object):
    __slots__ = (b'_timers', b'_panel', b'_clazz')
...
class _TimersCollection(_BaseTimersCollection):
    ...
    def addTimer(self, typeID, viewID, totalTime, finishTime, startTimer=None):
        ...
        _logger.debug(b'Adds destroy timer %s', timer)
    ...
    def removeTimer(self, typeID):
        if typeID in self._timers:
            _logger.debug(b'Removes timer %s', typeID)
            timer = self._timers.pop(typeID)
            timer.hide()
        return
...
class _StackTimersCollection(_BaseTimersCollection):
    __slots__ = (b'_currentTimer', b'_currentSecondaryTimers', b'_priorityMap', b'_prioritySecondaryMap', b'_maxDisplayedSecondaryTimers')
    ...
    def addSecondaryTimer(self, typeID, viewID, totalTime, finishTime, startTime=None):
        ...
        _logger.debug(b'Adds secondary timer %s', timer)
        self._timers[typeID] = timer
        timerPriority = _TIMERS_PRIORITY[timer.typeID, timer.viewID]
        ...
        if not self._currentSecondaryTimers:
            ...
            timer.show()
        elif timer not in self._currentSecondaryTimers:
            self._currentSecondaryTimers.append(timer)
...
class TimersPanel(TimersPanelMeta, MethodsRules):
    ...
    def _getNotificationTimerData(self, typeId, iconName, linkage, color=BATTLE_NOTIFICATIONS_TIMER_COLORS.ORANGE, noiseVisible=False, pulseVisible=False, text=b'', countdownVisible=True, isCanBeMainType=False, priority=10000, iconOffsetY=0, description=b''):
        return {b'typeId': typeId, 
           b'iconName': iconName, 
           b'linkage': linkage, 
           b'color': color, 
           b'noiseVisible': noiseVisible, 
           b'pulseVisible': pulseVisible, 
           b'text': text, 
           b'countdownVisible': countdownVisible, 
           b'isCanBeMainType': isCanBeMainType, 
           b'priority': priority, 
    ...
    def __hideAll(self):
        ...
        if self.__stunSoundPlaying:
            stunStopSoundName = b'flamer_stun_effect_end' if self.__stunType == StunTypes.FLAME.value else b'artillery_stun_effect_end'
    ...
    def _updateInspireTimer(self, isSourceVehicle, isInactivation, endTime, duration, primary=True, equipmentID=None):
        _logger.debug(b'[INSPIRE] %s _updateInspireTimer: isSourceVehicle: %s, isInactivation: %s, endTime: %s, duration: %s, primary: %s', BigWorld.player().id, isSourceVehicle, isInactivation, endTime, duration, primary)
    ...
    def __playStunSoundIfNeed(self, isVisible, timerType, stunType):
        ...
        stunStartSoundName, stunStopSoundName = (b'flamer_stun_effect_start', b'flamer_stun_effect_end') if stunType == StunTypes.FLAME.value else (b'artillery_stun_effect_start', b'artillery_stun_effect_end')
        if isVisible:
    ...
    def __updateSmokeTimer(self, smokeInfo):
        if smokeInfo is not None:
            equipment = vehicles.g_cache.equipments()[smokeInfo[b'equipmentID']]
            duration = equipment.expireDelay if smokeInfo[b'expiring'] else equipment.totalDuration
            self._showTimer(_TIMER_STATES.SMOKE, duration, _TIMER_STATES.WARNING_VIEW, smokeInfo[b'endTime'])
        else:
    ...
    def __updateHealingTimer(self, isSourceVehicle, isInactivation, endTime, duration, senderKey):
        _logger.debug(b'[HEAL_POINT] %s __updateHealingTimer: isSourceVehicle: %s, isInactivation: %s, endTime: %s, duration: %s, senderKey: %s', BigWorld.player().id, isSourceVehicle, isInactivation, endTime, duration, senderKey)
    ...
    def __updateRepairingTimer(self, isSourceVehicle, isInactivation, endTime, duration, senderKey):
        _logger.debug(b'[REPAIR_POINT] %s __updateRepairingTimer: isSourceVehicle: %s, isInactivation: %s, endTime: %s, duration: %s, senderKey: %s', BigWorld.player().id, isSourceVehicle, isInactivation, endTime, duration, senderKey)
    ...
    def __initData(self):
        self.as_setInitDataS({b'mainTimers': (self._generateMainTimersData()), 
           b'secondaryTimers': (self._generateSecondaryTimersData())})
        return

    @MethodsRules.delayable(b'__initData')
    @MethodsRules.delayable()
    ...
    @MethodsRules.delayable(b'__onVehicleControlling')
    ...
    def __showAbilityTimer(self, value):
        isFinished = value.get(b'finishing', False)
        if isFinished:
            self._hideTimer(_TIMER_STATES.ABILITY)
        else:
            self._showTimer(_TIMER_STATES.ABILITY, value.get(b'duration'), _TIMER_STATES.WARNING_VIEW, value.get(b'endTime'))
        return
