...
class FiltrableInventoryCategoryByVehicleTabView(ItemsWithVehicleFilterTabViewMeta):
    ...
    def _parseLoadedFilters(self, filterDict):
        vehicleCD = filterDict[b'vehicleCD']
    ...
    def _prepareDataForFilterSaving(self):
        ...
        data[b'vehicleCD'] = self._selectedVehicle.intCD if self._selectedVehicle else None
        return data
    ...
    def __onVehicleSelected(self, event):
        if event.ctx and event.ctx.get(b'vehicleId') and self.getActive():
            self._selectedVehicle = vehicle = self._itemsCache.items.getItemByCD(event.ctx[b'vehicleId'])
    ...
    def __makeVehicleVO(vehicle):
        if vehicle is None:
            ...
            vo.update({b'type': ((b'{}_elite').format(vehicle.type) if vehicle.isPremium else vehicle.type)})
            return vo
