...
_TAB_CUSTOM_NAME_GETTER = {(TabID.STYLE): (lambda style: style.userName if style is not None else b'')}
...
class VehiclePreviewTopPanelTabs(VehiclePreviewTopPanelTabsMeta):
    ...
    def setData(self, **kwargs):
        self.__view.setData(kwargs.get(b'tabIDs'), kwargs.get(b'currentTabID'), kwargs.get(b'style'))
        return
...
class VehiclePreviewTopPanelTabsView(ViewImpl):
    ...
    def __updateVMData(self, *_):
        with self.viewModel.transaction() as tx:
            ...
            for tabID in self.__tabIDs:
                ...
                tabNames.addString(getTabName(self.__style) if callable(getTabName) else b'')
    ...
    def __onTabChanged(self, *args, **kwargs):
        self.__currentTabID = TabID(first(args).get(b'selectedTab'))
        self.viewModel.setCurrentTabID(self.__currentTabID)
        command = _TAB_COMMAND.get(self.__currentTabID)
        if callable(command):
            backCallback = self.__parentCtx.get(b'backCallback') or self.__parentCtx.get(b'previewBackCb')
            command(self.__parentCtx.get(b'itemCD'), style=self.__style if self.__currentTabID == TabID.STYLE else None, topPanelData=self.__makeTopPanelData(), itemsPack=self.__parentCtx.get(b'itemsPack'), backCallback=backCallback)
        elif self.__currentTabID in PERSONAL_NUMBER_STYLE_TABS:
            self.__processPersonalNumberStyleTab()
        return

    def __makeTopPanelData(self):
        return {b'linkage': (VEHPREVIEW_CONSTANTS.TOP_PANEL_TABS_LINKAGE), b'tabIDs': (self.__tabIDs), 
           b'currentTabID': (self.__currentTabID), 
           b'style': (self.__style)}

    def __processPersonalNumberStyleTab(self):
        style = self.__parentCtx.get(b'numberStyle') if self.__currentTabID == TabID.PERSONAL_NUMBER_VEHICLE else None
        g_currentPreviewVehicle.selectNoVehicle()
        g_currentPreviewVehicle.selectVehicle(self.__parentCtx.get(b'itemCD'), style=style)
        return
