...
class AmmunitionPanel(AmmunitionPanelMeta, IGlobalListener):
    __slots__ = (b'__hangarMessage', b'__uiLogger')
    ...
    def _populate(self):
        ...
        g_clientUpdateManager.addCallbacks({b'inventory': (self.__inventoryUpdateCallBack)})
    ...
    def _update(self, onlyMoneyUpdate=False):
        if g_currentVehicle.isPresent():
            ...
            if statusId != Vehicle.VEHICLE_STATE.UNDAMAGED:
                msgString = makeHtmlString(b'html_templates:vehicleStatus', msgLvl, {b'message': msg})
            ...
            self.as_updateVehicleStatusS({b'message': msgString, 
               b'rentAvailable': rentAvailable, 
               b'isElite': isElite, 
               b'tankType': ((b'{}_elite').format(vehicle.type) if isElite else vehicle.type), 
               b'vehicleLevel': ((b'{}').format(int2roman(vehicle.level)) if viewState.isLevelShown() else b''), 
               b'vehicleName': ((b'{}').format(vehicle.shortUserName)), 
               b'roleId': (vehicle.role if viewState.isRoleShown() else ROLE_TYPE.NOT_DEFINED), 
               b'roleMessage': (getRoleMessage(g_currentVehicle.item.role) if viewState.isRoleShown() else b''), 
