...
class CrewPerkTooltipData(ToolTipBaseData):
    ...
    def getDisplayableData(self, skillName, skillRole, tankmanId, skillLevel=None, showAdditionalInfo=True, crewCustomName=b'', isBonus=None, skillIdx=-1, guiTankman=None, vehicle=None, *args, **kwargs):
        parent = kwargs.pop(b'parent', None)
...
class TankmanTooltipData(ToolTipBaseData):
    ...
    def getDisplayableData(self, *args, **kwargs):
        parent = kwargs.get(b'parent')
        kwargs[b'layoutID'] = parent.content.layoutID if parent else None
...
class SkillsEfficiencyTooltipAdditional(ToolTipBaseData):
    ...
    def getDisplayableData(self, *args, **kwargs):
        parent = kwargs.pop(b'parent', None)
        return DecoratedTooltipWindow(AdvancedTooltipView(b'skillEfficiency', backport.text(R.strings.tooltips.skillsEfficiency.header()), backport.text(R.strings.tooltips.skillsEfficiency.altDescription())), parent, useDecorator=False)
...
class MentorAssignmentTooltipAdditional(ToolTipBaseData):
    ...
    def getDisplayableData(self, tankmanID=None, *args, **kwargs):
        return DecoratedTooltipWindow(AdvancedTooltipView(movie=b'mentoringLicense', header=backport.text(R.strings.tooltips.mentorAssignment.advanced.header()), description=backport.text(R.strings.tooltips.mentorAssignment.advanced.description())), parent=kwargs.pop(b'parent', None), useDecorator=False)
