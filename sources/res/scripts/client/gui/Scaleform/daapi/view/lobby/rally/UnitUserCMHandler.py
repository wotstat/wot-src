...
KICK_FROM_UNIT = b'kickPlayerFromUnit'
GIVE_LEADERSHIP = b'giveLeadership'
TAKE_LEADERSHIP = b'takeLeadership'
TAKE_ARTILLERY_EQUIPMENT_COMMANDER = b'takeArtilleryEquipmentCommander'
GIVE_ARTILLERY_EQUIPMENT_COMMANDER = b'giveArtilleryEquipmentCommander'
TAKE_INSPIRE_EQUIPMENT_COMMANDER = b'takeInspireEquipmentCommander'
GIVE_INSPIRE_EQUIPMENT_COMMANDER = b'giveInspireEquipmentCommander'
...
class UnitUserCMHandler(BaseUserCMHandler, IGlobalListener):
    ...
    def _getHandlers(self):
        ...
        handlers.update({KICK_FROM_UNIT: b'kickPlayerFromUnit', 
           GIVE_LEADERSHIP: b'giveLeadership', 
           TAKE_LEADERSHIP: b'takeLeadership', 
           TAKE_ARTILLERY_EQUIPMENT_COMMANDER: b'takeArtilleryEquipmentCommander', 
           GIVE_ARTILLERY_EQUIPMENT_COMMANDER: b'giveArtilleryEquipmentCommander', 
    ...
    def _kickPlayerFromUnit(self, databaseID):
        yield self.prbDispatcher.sendPrbRequest(KickPlayerUnitCtx(databaseID, b'prebattle/kick'))
        return

    @adisp_process
    def _giveLeadership(self, databaseID):
        yield self.prbDispatcher.sendPrbRequest(GiveLeadershipUnitCtx(databaseID, b'prebattle/giveLeadership'))
        return

    @adisp_process
    def _takeLeadership(self):
        yield self.prbDispatcher.sendPrbRequest(GiveLeadershipUnitCtx(getAccountDatabaseID(), b'prebattle/takeLeadership'))
        return

    @adisp_process
    def _giveEquipmentCommander(self, databaseID, role):
        yield self.prbDispatcher.sendPrbRequest(GiveEquipmentCommanderCtx(databaseID, role, b'prebattle/giveEquipmentCommander'))
        return
