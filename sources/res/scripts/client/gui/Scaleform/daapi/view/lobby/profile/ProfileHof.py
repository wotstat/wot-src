...
class ProfileHof(ProfileHofMeta):
    ...
    _errorsStatusMap = {b'1004': (PROFILE_CONSTANTS.HOF_SPECIAL_CASES), 
       b'1005': (PROFILE_CONSTANTS.HOF_SPECIAL_CASES), 
       b'1015': (PROFILE_CONSTANTS.HOF_SPECIAL_CASES), 
       b'1016': (PROFILE_CONSTANTS.HOF_SPECIAL_CASES), 
       b'1003': (PROFILE_CONSTANTS.HOF_RESULTS_HIDE), 
       b'1006': (PROFILE_CONSTANTS.HOF_RESULTS_EXCLUSION), 
       b'1007': (PROFILE_CONSTANTS.HOF_RESULTS_INCLUSION)}
    _requestRetriesCount = 3
    _retryDelay = 0.5
    _bgPath = b'../maps/icons/hof/hof_back_landing.png'
    ...
    def changeStatus(self):
        if self.__status == PROFILE_CONSTANTS.HOF_RESULTS_SHOW:
            success = yield DialogsInterface.showI18nConfirmDialog(b'hof/excludeRating')
            if success:
            ...
            LOG_WARNING(b'Something went wrong! Getting actual status.')
    ...
    def onSectionActivated(self):
        if self.lobbyContext.getServerSettings().bwHallOfFame.isStatusEnabled:
            if self.__requestProcessing:
                LOG_WARNING(b'ProfileHof request canceled: another request is processing')
            else:
                self.__getRatingStatus()
        else:
    ...
    def _dispose(self):
        if self.__retryCallback:
            LOG_WARNING(b'ProfileHof request canceled: ProfileHof view was disposed')
    ...
    def __getCountersList(self):
        ...
        for buttonName in self._buttonsWithCounter:
            if isHofButtonNew(buttonName):
                counters.append({b'componentId': buttonName, b'count': b'1'})
    ...
    def __getRatingStatus(self):
        ...
        def handleError(errorCode):
            ...
            if status:
                ...
                LOG_ERROR(b'Unknown error code: ' + str(errorCode))
            return
