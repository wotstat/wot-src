...
class BCMessageWindow(BCMessageWindowMeta):
    ...
    BC_NEW_MODULE_UNLOCK_SOUND_ID = b'bc_new_module_unlock'
    BC_RESOURCE_FLY_SOUND_ID = b'bc_resources_fly'
    BC_RESOURCE_GLOW_SOUND_ID = b'bc_resources_glow'
    BC_INFO_LINE_DISAPPEAR_SOUND_ID = b'bc_info_line_disappear'
    FLY_ANIMATIONS = [
     b'gold', b'prem', b'credits', b'experience']
    RTPC_EXT_BC_ELEMENT_POSITION = b'RTPC_ext_bc_element_position'
    ...
    def onMessageRemoved(self):
        callback = self._content.get(b'callback')
    ...
    def _populate(self):
        ...
        self.as_setMessageDataS(self._content[b'messages'])
    ...
    def __setCameraDisabled(self, disabled):
        self.fireEvent(CameraRelatedEvents(CameraRelatedEvents.FORCE_DISABLE_IDLE_PARALAX_MOVEMENT, ctx={b'isDisable': disabled, b'setIdle': True, b'setParallax': True}), EVENT_BUS_SCOPE.LOBBY)
        self.fireEvent(CameraRelatedEvents(CameraRelatedEvents.FORCE_DISABLE_CAMERA_MOVEMENT, ctx={b'disable': disabled}), EVENT_BUS_SCOPE.LOBBY)
        return
