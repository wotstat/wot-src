...
class EpicBattleTimer(EpicBattleTimerMeta):
    ...
    def _sendTime(self, minutes, seconds):
        self.as_setTotalTimeWithSecondsS((b'{:02d}').format(minutes), (b'{:02d}').format(seconds), (minutes * 60 + seconds) * self.__overTimeMaxTimeFac)
        return
    ...
    def _populate(self):
        ...
        overTimeComp = getattr(self.sessionProvider.arenaVisitor.getComponentSystem(), b'overtimeComponent', None)
