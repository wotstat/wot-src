...
class RankedCarouselDataProvider(BattlePassCarouselDataProvider):
    ...
    def _buildVehicle(self, vehicle):
        ...
        result[b'hasRankedBonus'] = self.__rankedController.hasVehicleRankedBonus(vehicle.intCD)
        ...
        if suitResult is not None:
            if state == Vehicle.VEHICLE_STATE.UNSUITABLE_TO_QUEUE:
                result[b'lockedTooltip'] = getUnsuitable2queueTooltip(suitResult, resShortCut)
            result[b'clickEnabled'] = True
            result[b'hasRankedBonus'] = False
        return result
