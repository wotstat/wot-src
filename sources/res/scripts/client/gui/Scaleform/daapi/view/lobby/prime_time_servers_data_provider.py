...
class PrimeTimesServersDataProvider(ServersDataProvider):
    ...
    def getDefaultSelectedServer(self, serversList):
        ...
        for server in serversList:
            serverPeriods = self.primeTimes[server[b'shortname']]
            for periodStart, _ in serverPeriods:
                if periodStartMin is None:
                    chosenServer = server[b'id']
    ...
    def _makeVO(self, index, item):
        ...
        vo[b'shortname'] = item[b'shortname']
        vo[b'schedules'] = (b'\n').join(periodsStr)
        vo[b'selected'] = False
        vo[b'maxPrimeTimes'] = self.__maxPeriodLen
        return vo
