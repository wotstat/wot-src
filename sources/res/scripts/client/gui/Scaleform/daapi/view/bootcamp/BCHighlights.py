...
class BCHighlights(BCHighlightsMeta):
    BUTTON_SOUNDS = (
     b'InBattleExtinguisher', b'InBattleHealKit', b'InBattleRepairKit', BOOTCAMP_UI_COMPONENTS.START_BATTLE_BUTTON)
    HIGHLIGHT_NO_SOUNDS = (
     b'LoadingRightButton', b'LoadingLeftButton', BOOTCAMP_UI_COMPONENTS.WELCOME_START_BATTLE_BUTTON)
    ...
    def __init__(self, settings):
        ...
        self.__descriptors = settings[b'descriptors']
    ...
    def onHighlightAnimationComplete(self, componentID):
        LOG_DEBUG_DEV_BOOTCAMP(b'BCHighlights_onHintAnimationComplete', componentID)
        self.hideHint(componentID, shouldStop=False)
        return

    def onComponentTriggered(self, componentID):
        LOG_DEBUG_DEV_BOOTCAMP(b'BCHighlights_onComponentTriggered', componentID)
        return

    def showHint(self, componentID):
        LOG_DEBUG_DEV_BOOTCAMP(b'BCHighlights_showHint', componentID)
        self.__activeHints.add(componentID)
        self.as_addHighlightS(componentID)
        if componentID not in self.HIGHLIGHT_NO_SOUNDS:
            soundID = b'bc_new_ui_element_button' if componentID in self.BUTTON_SOUNDS else b'bc_new_ui_element'
            ...
            if activeSound is not None and activeSound.isPlaying:
                LOG_DEBUG_DEV_BOOTCAMP((b'BCHighlights_showHint - skipping {0} (already playing from component {1})').format(soundID, activeSoundComponentID))
                return
            ...
            self.__soundsBySoundID[soundID] = (
             componentID, snd)
            LOG_DEBUG_DEV_BOOTCAMP(b'BCHighlights_showHint - playing', soundID)
            snd.play()
        return
    ...
    def hideHint(self, componentID, shouldStop=True):
        ...
        if snd is not None:
            LOG_DEBUG_DEV_BOOTCAMP(b'BCHighlights_hideHint', componentID, shouldStop)
            activeSoundComponentID, activeSound = self.__soundsBySoundID.get(soundID, (None, None))
            if activeSound is snd:
                if shouldStop:
                    LOG_DEBUG_DEV_BOOTCAMP(b'BCHighlights_hideHint - stopping', soundID)
                    snd.stop()
                elif snd.isPlaying:
                    self.__orphanedSounds.append(activeSound)
                LOG_DEBUG_DEV_BOOTCAMP(b'BCHighlights_hideHint - removing from active sounds', soundID)
