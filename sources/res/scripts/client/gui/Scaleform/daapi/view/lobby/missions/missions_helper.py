...
class _RankedDetailedMissionInfo(_DetailedMissionInfo):
    ...
    def _getInfo(self, statusData, isAvailable, errorMsg, mainQuest=None):
        ...
        if isDaily and isCompleted and not isAnyPrimeLeftNextDay:
            ...
            updateDict[b'statusLabel'] = text_styles.concatStylesWithSpace(_getNotAvailableIconTag(), statusLabel)
            updateDict[b'resetDateLabel'] = backport.text(R.strings.ranked_battles.quests.detailed.notInLeagues())
...
class _DetailedPersonalMissionInfo(_MissionInfo):
    ...
    def _getStatusFields(self, isAvailable, errorMsg):
        if not isAvailable:
            ...
            if statusData:
                return statusData
            return {b'showIcon': False, 
               b'addBottomStatusText': (self.__getAddBottomInfo()), 
               b'statusLabel': None, 
               b'status': (MISSIONS_STATES.NONE), 
               b'statusTooltipData': None}

    def _getUnavailableStatusFields(self, errorMsg):
        if errorMsg == b'branchNotStarted':
            return self._getBranchNotStartedStatusFields()
        if errorMsg == b'branchInactive':
    ...
    def _getBranchIsInactiveStatusFields(self):
        ...
        bottomStatusTooltipData = {b'tooltip': (makeTooltip(header=TOOLTIPS.PERSONALMISSIONS_OPERATION_FOOTER_ACTIVATECAMPAIGN_HEADER, body=TOOLTIPS.PERSONALMISSIONS_OPERATION_FOOTER_ACTIVATECAMPAIGN_BODY))}
        return {b'showIcon': False, 
           b'addBottomStatusText': statusText, 
           b'status': (MISSIONS_STATES.IS_ON_PAUSE), 
    ...
    def _getBranchNotStartedStatusFields(self):
        ...
        return {b'showIcon': True, 
           b'addBottomStatusText': statusText, 
           b'status': (MISSIONS_STATES.NOT_AVAILABLE), 
           b'bottomStatusTooltipData': None}
    ...
    def _getNoVehicleStatusFields(self):
        ...
        bottomStatusTooltipData = {b'tooltip': (makeTooltip(header=TOOLTIPS.PERSONALMISSIONS_STATUS_LOCKEDBYVEHICLE_HEADER, body=_ms(bodyTooltip, vehType=_ms(MENU.classesShort(self.event.getQuestClassifier().classificationAttr)), minLevel=minLevel, maxLevel=maxLevel)))}
        statusData = self._getQuestCompletionStatusFields()
        if statusData:
    ...
    def _getInfo(self, statusData, isAvailable, errorMsg, mainQuest=None):
        ...
        data.update({b'buttonState': (self.__getBtnStates(isAvailable, errorMsg))})
    ...
    def __getBtnStates(self, isAvailable, errorMsg):
        ...
        if quest.canBePawned():
            states |= PERSONAL_MISSIONS_BUTTONS.HOLD_AWARD_SHEET_BTN_VISIBLE
            isSuitableBranch = errorMsg not in (b'branchNotStarted', b'branchInactive') and not isPM3Active
