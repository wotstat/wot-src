...
def createStorageDefVO(itemID, title, description, count, price, image, imageAlt, itemType=b'', nationFlagIcon=b'', enabled=True, available=True, contextMenuId=b'', additionalInfo=b'', actionButtonLabel=None, actionButtonIcon=None, actionButtonTooltip=None, active=GOODIE_STATE.INACTIVE, upgradable=False, upgradeButtonIcon=None, upgradeButtonTooltip=b'', extraParams=(), specializations=()):
    return {b'id': itemID, 
       b'title': title, 
...
def dummyFormatter(label, btnLabel=b'', btnTooltip=b''):
...
def getDeletedOptDevicesVo(item):
    return createStorageDefVO(itemID=item.intCD, title=getStorageItemName(item), description=getStorageItemDescr(item), count=0, price=b'', image=getStorageItemIcon(item, STORE_CONSTANTS.ICON_SIZE_SMALL), imageAlt=ALT_IMAGE, itemType=item.getOverlayType(), extraParams=getItemExtraParams(item))
