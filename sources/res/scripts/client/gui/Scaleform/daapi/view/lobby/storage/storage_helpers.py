...
def getTopVehicleByNation(nationName):
    ...
    return topVehicles.get(nationName, b'')
...
def getStorageItemDescr(item):
    ...
    if itemType in GUI_ITEM_TYPE.VEHICLE_MODULES:
        return _generateDescr(item, i18n.makeString(STORAGE.MODULECOMPATIBLE_LABEL), STORAGE.MODULECOMPATIBLE_MOREVEHICLES, (b'vehicles', b'clipVehicles', b'uniChargedVehicles'), _MAX_COMPATIBLE_VEHS_COUNT)
    else:
        if itemType == GUI_ITEM_TYPE.SHELL:
            return _generateDescr(item, i18n.makeString(STORAGE.SHELLCOMPATIBLE_LABEL), STORAGE.SHELLCOMPATIBLE_MOREGUNS, (b'shellGuns',), _MAX_COMPATIBLE_GUNS_COUNT)
        ...
        template = g_htmlTemplates[b'html_templates:lobby/popovers'][b'equipment']
        ...
        if itemType == GUI_ITEM_TYPE.EQUIPMENT:
            ...
            if cooldownSeconds > 0:
                tpl = g_htmlTemplates[b'html_templates:lobby/popovers/equipment']
                cooldown = tpl.format(b'whiteSpanish', ctx={b'message': cooldownSeconds})
...
def createStorageDefVO(itemID, title, description, count, price, image, imageAlt, itemType=b'', nationFlagIcon=b'', enabled=True, available=True, contextMenuId=b'', additionalInfo=b'', actionButtonIcon=None, actionButtonTooltip=None, upgradable=False, upgradeButtonIcon=None, upgradeButtonTooltip=b'', extraParams=(), specializations=(), sellBtnLabel=None):
    return {b'id': itemID, 
       b'title': title, 
       ...
       b'imageAlt': imageAlt, 
       b'type': itemType, 
       b'nationFlagIcon': nationFlagIcon, 
       b'enabled': enabled, 
       b'available': available, 
       b'additionalInfo': additionalInfo, 
       b'actionButtonIcon': actionButtonIcon, 
       b'actionButtonTooltip': actionButtonTooltip, 
       b'upgradable': upgradable, 
       b'upgradeButtonIcon': upgradeButtonIcon, 
       b'upgradeButtonTooltip': upgradeButtonTooltip, 
       b'extraParams': extraParams, 
       b'specializations': specializations, 
       b'sellButtonLabel': (sellBtnLabel if sellBtnLabel is not None else backport.text(R.strings.storage.buttonLabel.sell())), 
       b'contextMenuId': contextMenuId}
...
def getStorageVehicleVo(vehicle):
    ...
    vo = createStorageDefVO(vehicle.intCD, name, description, vehicle.inventoryCount, getItemPricesVO(vehicle.getSellPrice())[0], imageSmall, getShopVehicleIconPath(STORE_CONSTANTS.ICON_SIZE_SMALL, b'empty_tank'), itemType=vehicle.getHighlightType(), nationFlagIcon=RES_SHOP.getNationFlagIcon(nations.NAMES[vehicle.nationID]), contextMenuId=CONTEXT_MENU_HANDLER_TYPE.STORAGE_VEHICLES_REGULAR_ITEM)
    vo.update({b'infoImgSrc': stateIcon, 
       b'infoText': stateText})
    if vehicle.canTradeOff:
        vo.update({b'tradeOffPrice': {b'price': (getMoneyVO(vehicle.tradeOffPrice))}})
    return vo
...
def _calculateVehicleMaxAdditionalPrice(vehicle):
    ...
    return sum(item.getSellPrice().price.get(Currency.CREDITS, 0) * getattr(item, b'count', 1) for item in items)


def getStorageItemName(item):
    if item.itemTypeID in GUI_ITEM_TYPE.VEHICLE_MODULES:
        return (b'{} {}').format(text_styles.neutral(int2roman(item.level)), item.longUserName)
...
def _generateDescr(item, label, moreLabel, paramsNames, maxItemsCount):
    ...
    for paramType, compatible in moduleCompatibles:
        if paramType in paramsNames:
            compatiblesSet |= set(compatible.split(b', '))

    compatiblesSet = list(compatiblesSet)
    compatiblesCount = len(compatiblesSet)
    descrItems = [
     label,
     (b', ').join(compatiblesSet[:maxItemsCount])]
...
def dummyFormatter(label, iconSource=RES_ICONS.MAPS_ICONS_LIBRARY_ALERTBIGICON, btnLabel=b'', btnTooltip=b'', smallIcon=False):
    return {b'iconSource': iconSource, 
       b'htmlText': (text_styles.main(_ms(label))), 
       b'alignCenter': True, 
       b'inlineIcon': True, 
       b'smallIcon': smallIcon, 
       b'btnVisible': (bool(btnLabel)), 
       b'btnLabel': btnLabel, 
       b'btnTooltip': btnTooltip, 
...
def getItemVo(item):
    ...
    nationFlagIcon = RES_SHOP.getNationFlagIcon(nations.NAMES[itemNationID]) if itemNationID != nations.NONE_INDEX else b''
...
class OptDeviceBonusesDescriptionBuilder(object):
    KPI_VALUE_TEMPLATE = b'{{colorTagOpen}}{}{{colorTagClose}}'
    KPI_HTML_TEMPLATE = g_htmlTemplates[b'html_templates:lobby/popovers'][b'equipment']
    ...
    def getDescriptionByKpiStrings(self, item):
        if any(kpi.type == KPI.Type.AGGREGATE_MUL for kpi in item.getKpi()):
            return [
             item.shortDescriptionSpecial.format(colorTagOpen=self.KPI_HTML_TEMPLATE.source[b'colorTagOpen'], colorTagClose=self.KPI_HTML_TEMPLATE.source[b'colorTagClose'])]
    ...
    def __createKpiString(self, kpi):
        ...
        description = (b' ').join((generalValue, backport.text(bonus) if bonus else b''))
...
def getCategoriesIcons(device):
    return [backport.image(R.images.gui.maps.icons.specialization.dyn((b'medium_{}_off').format(category))()) for category in SlotCategories.ORDER if category in device.descriptor.categories]
