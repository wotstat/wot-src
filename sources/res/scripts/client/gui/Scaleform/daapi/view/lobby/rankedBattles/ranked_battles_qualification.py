...
class RankedBattlesQualification(RankedBattlesDivisionQualificationMeta):
    ...
    def __setQualificationProgress(self, rank):
        ...
        current = stats.divisionsStats.get(divisionID, {}).get(b'battles', 0)
        ...
        if numQualificationQuestBattles:
            ...
            if isCompleted:
                ...
                qualificationDescr = text_styles.main(backport.text(R.strings.ranked_battles.division.status.qualificationDescr.progress(), battlesTotal=total, battlesList=(b', ').join([str(x) for x in numQualificationQuestBattles])))
    ...
    def __setQualificationSteps(self, divisionID):
        steps = self.__rankedController.getStatsComposer().divisionsStats.get(divisionID, {}).get(b'stepsCount', 0)
        self.as_setQualificationStepsDataS(shared_vos.getStatVO(ranked_formatters.getIntegerStrStat(float(steps)), b'qualificationSteps', b'steps', b'qualificationSteps'))
        return
    ...
    def __setQualificationEfficiency(self, divisionsID):
        ...
        self.as_setQualificationEfficiencyDataS(shared_vos.getStatVO(ranked_formatters.getFloatPercentStrStat(divisionEfficiency), b'qualificationEfficiency', b'efficiency', b'qualificationEfficiency'))
        return
