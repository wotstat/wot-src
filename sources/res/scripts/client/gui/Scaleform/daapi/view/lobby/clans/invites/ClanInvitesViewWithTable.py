...
class ClanInvitesViewWithTable(ClanInvitesViewWithTableMeta):
    ...
    def _makeData(self):
        return {b'tableHeaders': (self._makeHeaders())}
    ...
    @staticmethod
    def _packHeaderColumnData(headerId, label, buttonWidth, tooltip, icon=b'', sortOrder=-1, showSeparator=True, textAlign=b'center', enabled=True, defaultSortDirection=b'descending'):
        return {b'id': headerId, 
           b'label': (_ms(label)), 
           b'iconSource': icon, 
    ...
    def _showDummy(self, header, body=b'', icon=None, btnVisible=False, btnLabel=b'', btnTooltip=b'', alignCenter=True):
        self.as_showDummyS({b'iconSource': icon, 
           b'htmlText': (str().join((
                       text_styles.middleTitle(header),
                       clans_fmts.getHtmlLineDivider(3),
                       text_styles.main(body)))), 
           b'alignCenter': alignCenter, 
...
class ClanInvitesAbstractDataProvider(SortableDAAPIDataProvider):
    ...
    def buildList(self, cache, showMoreButton=False):
        ...
        if showMoreButton:
            self.__list.append({b'hasShowMoreButton': True, 
               b'showMoreButtonEnabled': True})
        return
