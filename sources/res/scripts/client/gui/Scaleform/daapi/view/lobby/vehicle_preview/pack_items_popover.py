...
class PackItemsPopover(PackItemsPopoverMeta):
    ...
    def __init__(self, ctx=None):
        ...
        for item in self.__data.get(b'items'):
            ...
            self.__rawItems.append(ItemPackEntry(type=item.get(b'type'), id=int(item.get(b'id')), count=item.get(b'rawCount') or 0, groupID=item.get(b'groupID'), title=item.get(b'title'), description=item.get(b'description')))
