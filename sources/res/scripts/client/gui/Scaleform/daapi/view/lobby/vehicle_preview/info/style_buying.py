...
class StyleBuyingProcessor(object):
    ...
    def __init__(self, confirmationKey=b'buyConfirmation'):
    ...
    def buy(self, price):
        if self.__buyParams is not None and self.__style:
            ...
            raise SoftException(b'buyParams and style attributes are expected to be not None')
        return
    ...
    def __obtainByDynamicCurrency(self, money):
        ...
        if not method:
            raise SoftException((b"Can't find buy method for dyn currency: {}").format(money.getCurrency()))
        if dyn_utils.mayObtainForMoney(money):
...
def _buyRequestConfirmation(productName, priceStr, key):
    return DialogsInterface.showDialog(meta=I18nConfirmDialogMeta(key=key, messageCtx={b'product': productName, 
       b'price': priceStr}, focusedID=DIALOG_BUTTON_ID.SUBMIT))
