...
class CommonContextMenuHandler(SimpleVehicleCMHandler):
    ...
    def __init__(self, cmProxy, ctx=None, handlers=None):
        ...
        if handlers is not None:
            handlers.update({(VEHICLE.PREVIEW): b'showVehiclePreview', 
               (VEHICLE.SELL): b'sellVehicle', 
               (VEHICLE.RESEARCH): b'researchVehicle', 
               (VEHICLE.BUY): b'buyVehicle', 
               (VEHICLE.SELECT): b'selectVehicleInHangar', 
    ...
    def _generateOptions(self, ctx=None):
        ...
        if vehicle.isPurchased:
            options.append(self._makeItem(VEHICLE.SELL, MENU.contextmenu(VEHICLE.SELL), {b'enabled': (vehicle.canSell)}))
            ...
            options.append(self._makeItem(VEHICLE.BUY, label, {b'enabled': btnEnabled}))
            ...
            options.append(self._makeItem(VEHICLE.RESEARCH, MENU.contextmenu(VEHICLE.RESEARCH), {b'enabled': isAvailableToUnlock}))
...
class VehicleCompareContextMenuHandler(CommonContextMenuHandler):
    ...
    def __init__(self, cmProxy, ctx=None):
        handlers = {(VEHICLE.STATS): b'showVehicleStats', 
           (_OptIds.COPY): b'copyComparedVehicle'}
    ...
    def _manageStartOptions(self, options, vehicle):
        options.append(self._makeItem(_OptIds.COPY, VEH_COMPARE.MENU_CLONEFORCOMPARE, {b'enabled': (not self.comparisonBasket.isFull())}))
        if vehicle.isInInventory:
