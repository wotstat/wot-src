...
def makeBrowserParams(waitingMessage=R.invalid(), isModal=False, isHidden=False, bgAlpha=1.0, isCloseBtnVisible=False):
    ...
    return {b'waitingMessage': (backport.msgid(waitingMessage)), 
       b'isModal': isModal, 
       b'isHidden': isHidden, 
       b'bgAlpha': bgAlpha, 
       b'isCloseBtnVisible': isCloseBtnVisible}
...
class BrowserView(LobbySubView, BrowserScreenMeta):
    ...
    def __init__(self, ctx=None):
        self.__ctx = ctx
        self.__initSoundSpace(self.__getFromCtx(b'soundSpaceID'))
        super(BrowserView, self).__init__(ctx)
        self.__browserId = None
        self.__hasFocus = False
        self.__browser = None
        self.__browserComponent = None
        self.__browserParams = (ctx or {}).get(b'browserParams', makeBrowserParams())
    ...
    def onCloseView(self):
        returnAlias = self.__getFromCtx(b'returnAlias', VIEW_ALIAS.LOBBY_HANGAR)
    ...
    def _onRegisterFlashComponent(self, viewPy, alias):
        ...
        if alias == VIEW_ALIAS.BROWSER:
            viewPy.init(self.__browserId, self.__getFromCtx(b'webHandlers'), alias=self.alias)
    ...
    def _invalidate(self, *args, **kwargs):
        ...
        if self.__isBrowserLoading:
            self.__pendingRequest = self.__getFromCtx(b'url')
        elif self.__browser:
            self.__configureBrowser()
            self.__navigateTo(self.__getFromCtx(b'url'))
        else:
            self.__loadBrowser()
        if self.__browserComponent:
            self.__browserComponent.init(self.__browserId, self.__getFromCtx(b'webHandlers'), alias=self.alias)
        return

    def _dispose(self):
        if self.__browserComponent:
            self.__browserComponent.onError -= self.__onError
        returnCallback = self.__getFromCtx(b'returnClb')
        if returnCallback is not None:
            callbackArgs = {b'byUser': (self.__closedByUser)}
            if self.__browser:
                callbackArgs[b'url'] = self.__browser.url
    ...
    def __onServerSettingChanged(self, diff):
        handler = self.__getFromCtx(b'onServerSettingsChange')
        if callable(handler):
            handler(self, diff)
        return
    ...
    def __configureBrowser(self):
        ...
        if browser:
            browser.useSpecialKeys = self.__getFromCtx(b'useSpecialKeys', False)
            browser.allowRightClick = self.__getFromCtx(b'allowRightClick', False)
            browser.setDisabledKeys(self.__getFromCtx(b'disabledKeys', tuple()))
            self.__browser = browser
            self.__updateSkipEscape()
        else:
            LOG_ERROR(b'Failed to create browser!')
        return
