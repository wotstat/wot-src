...
class TradeOffWidget(TradeOffWidgetMeta, EventSystemEntity):
    ...
    def __createVO(self, vehicle=None):
        ...
        vo = {b'showTradeOff': (vehicle is None), b'available': vehiclesAvailable}
        if vehicle is not None:
            vo.update({b'vehicleNationFlag': (RES_ICONS.getTradeInNationFlag(vehicle.nationName)), 
               b'vehicleType': (Vehicle.getTypeSmallIconPath(vehicle.type, vehicle.isElite)), 
               b'vehicleLevel': (RES_ICONS.getLevelIcon(vehicle.level)), 
               b'vehicleIcon': (getSmallIconPath(vehicle.name)), 
               b'vehicleTitle': (vehicle.shortUserName)})
        return vo
