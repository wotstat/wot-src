...
def getViewSettings():
    ...
    return (
     ConditionalViewSettings(VIEW_ALIAS.LOBBY_RESEARCH, BootcampComponentOverride(Research, BCResearch), b'research.swf', WindowLayer.SUB_VIEW, None, VIEW_ALIAS.LOBBY_RESEARCH, ScopeTemplates.LOBBY_SUB_SCOPE, True),)
...
class _TechTreePackageBusinessHandler(PackageBusinessHandler):
    ...
    def __loadResearch(self, event):
        ctx = event.ctx
        self.__exitEvent = ctx.get(b'exit')
