...
class EpicBattleRespawn(EpicRespawnViewMeta, IEpicRespawnView):
    ...
    def _populate(self):
        ...
        if ctrl is not None:
            ...
            LOG_ERROR(b'Respawn Controller not available!')
    ...
    def onRespawnBtnClick(self):
        LOG_DEBUG(b'py_onRespawnBtnClick')
        ctrl = self.sessionProvider.dynamic.respawn
        if ctrl is not None:
            ctrl.respawnPlayer()
        else:
    ...
    def onRespawnInfoUpdated(self, respawnInfo):
        ...
        if not vehicle or vehicle.intCD != respawnInfo.vehicleID:
            ...
            vehicle = self.__carousel.getSelectedVehicle()
        if not vehicle:
            LOG_ERROR(b'RESPAWN: THIS SHOULD NEVER HAPPEN! Selected Veh ID: ', respawnInfo.vehicleID, b' available Vehicles: ', self.__carousel.getTotalVehiclesCount())
        return
    ...
    def setSelectedPoint(self, pointId):
        ...
        if ctrl is not None and 0 <= pointId < len(self.__lastRespawnPositions):
            ctrl.requestPointForRespawn(self.__lastRespawnPositions[pointId][b'position'])
    ...
    def setRespawnInfo(self, respawnInfo):
        ...
        self.as_setRespawnLocationsS([self.__makePointVO(getX(point[b'position']), getY(point[b'position']), bool(point[b'isEnemyNear'])) for point in self.__lastRespawnPositions])
        positions = [point[b'position'] for point in self.__lastRespawnPositions]
