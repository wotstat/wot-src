...
def _resourceParam(value):
    return (b'').join((text_styles.defRes(value), icons.nutStat()))
...
def _getDataObject(key, i18nFunc, value, ratingOutdated=False, tooltipData=None):
    tooltip = _getTooltip(i18nFunc(key + b'/tooltip/header'), i18nFunc(key + b'/tooltip/body'), ratingOutdated)
    if ratingOutdated:
...
def _getDataObjectTruncatedValue(key, i18nFunc, value):
    tooltip = _getTooltip(_ms(i18nFunc(key + b'/tooltip/header'), value=value), _ms(i18nFunc(key + b'/tooltip/body'), value=value))
    data = SUtils.getDetailedDataObject(i18nFunc(key), value, tooltip)
    data.update({b'isUseTextStyle': True, 
       b'truncateVo': {b'isUseTruncate': True, 
                       b'textStyle': b'statsText', 
                       b'maxWidthTF': 100}})
    return data
...
def _getLevelParams(i18nFunc, battlesCountGetter, winsEfficiencyGetter, eloRatingGetter, placeGetter, favArenaNameGetter, ratingOutdated):
    ...
    return [
     _getDataObject(b'battles', i18nFunc, battlesCount),
     _getDataObject(b'wins', i18nFunc, winsEfficiency),
     _getDataObject(b'eloRating', i18nFunc, eloRating),
     _getDataObject(b'place', i18nFunc, place, ratingOutdated),
     _getDataObjectTruncatedValue(b'favoriteMap', i18nFunc, favArenaName)]
...
class FortGlobalMapStatistics(BaseDictStatisticsVO):
    ...
    def _getHeaderData(self, data):
        stats = data[b'stats']
        return (
         PUtils.packLditItemData(formatField(getter=stats.getBattlesCount, formatter=backport.getIntegralFormat), CL.GLOBALMAPVIEW_STATS_BATTLES, CL.GLOBALMAPVIEW_STATS_BATTLES_TOOLTIP, b'battles40x32.png'),
         PUtils.packLditItemData(formatField(getter=stats.getWinsEfficiency, formatter=PUtils.formatFloatPercent), CL.GLOBALMAPVIEW_STATS_WINS, CL.GLOBALMAPVIEW_STATS_WINS_TOOLTIP, b'wins40x32.png'),
         PUtils.packLditItemData(formatField(getter=stats.getCapturedProvincesCount, formatter=backport.getIntegralFormat), CL.GLOBALMAPVIEW_STATS_SEIZED, CL.GLOBALMAPVIEW_STATS_SEIZED_TOOLTIP, b'seizedProvinces40x32.png'))

    def _getDetailedData(self, data):
        stats = data[b'stats']
        ratings = data[b'ratings']
        favouriteAttrs = data[b'favouriteAttrs']
