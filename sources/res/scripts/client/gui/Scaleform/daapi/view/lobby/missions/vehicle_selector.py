    ...
    def __updateSelectedVehicle(self):
        ...
        if self.__isQuestForBattleRoyale:
            ...
            if suitableVehicles and vehicle and vehicle.intCD in suitableVehicles:
                ...
                selectedVeh = {b'buyTank': True, 
                   b'iconSmall': (RES_ICONS.MAPS_ICONS_LIBRARY_EMPTY_SELECTION), 
                   b'smallInfoText': (style(label)), 
                   b'tooltip': (TOOLTIPS.MISSIONS_VEHICLE_SELECTOR_SELECT)}
                status = b''
            else:
...
class _MissionsCarouselDataProvider(CarouselDataProvider):
    ...
    def setCriteria(self, criteria, extraConditions):
        ...
        self._filter.reset(exceptions=(b'inventory',))
        return
    ...
    def _buildVehicle(self, vehicle):
        ...
        if vehicle.isInInventory and vehicle.activeInNationGroup:
            for condition in self.__extraConditions:
                ...
                if not isOk:
                    smallStatus, largeStatus = getStatusStrings(reason, ctx={b'factor': xpFactor})
                    vehicleVO.update(smallInfoText=smallStatus, infoText=largeStatus, lockBackground=True, clickEnabled=False, infoImgSrc=b'', isCritInfo=False)
                    break
            else:
