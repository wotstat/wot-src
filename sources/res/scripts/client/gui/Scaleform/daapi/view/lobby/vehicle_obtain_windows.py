...
VehicleBuyWindowState = namedtuple(b'VehicleBuyWindowState', (b'buyAmmo', b'buySlot', b'crewType', b'rentID'))
...
class VehicleBuyWindow(VehicleBuyWindowMeta):
    ...
    def __init__(self, ctx=None):
        ...
        self.nationID = ctx.get(b'nationID')
        self.inNationID = ctx.get(b'itemID')
        self.vehicle = None
        self.tradeInVehicleToSell = None
        self.__state = VehicleBuyWindowState(False, False, -1, -1)
        if ctx.get(b'isTradeIn', False):
    ...
    def onTradeInClearVehicle(self):
        self.tradeInVehicleToSell = None
        self.as_setTradeInWarningMessagegeS(b'')
        return
    ...
    def _getGuiFields(self, vehicle):
        return {b'title': (i18n.makeString(DIALOGS.BUYVEHICLEWINDOW_TITLE, vehiclename=vehicle.userName)), 
           b'submitBtnLabel': (i18n.makeString(DIALOGS.BUYVEHICLEWINDOW_SUBMITBTN)), 
           b'cancelBtnLabel': (i18n.makeString(DIALOGS.BUYVEHICLEWINDOW_CANCELBTN))}

    def _isTradeIn(self):
        return self.tradeIn.isEnabled() and self.vehicle.canTradeIn

    def __getTradeInGuiFields(self, vehicle):
        return {b'tradeInTitle': (i18n.makeString(DIALOGS.BUYVEHICLEWINDOW_TRADEIN_TITLE, vehiclename=vehicle.userName)), 
           b'tradeInSubmitBtnLabel': (i18n.makeString(DIALOGS.BUYVEHICLEWINDOW_TRADEIN_SUBMITBTN)), 
           b'tradeInCancelBtnLabel': (i18n.makeString(DIALOGS.BUYVEHICLEWINDOW_TRADEIN_CANCELBTN))}
    ...
    def _getContentFields(self, vehicle):
        ...
        return {b'slotCheckboxTooltip': slotCheckboxTooltip, 
           b'priceLabel': (i18n.makeString(DIALOGS.BUYVEHICLEWINDOW_PRICELABEL, vehiclename=vehicle.shortUserName)), 
           b'crewCheckbox': (i18n.makeString(DIALOGS.BUYVEHICLEWINDOW_TANKMENCHECKBOX)), 
           b'warningMsg': (i18n.makeString(DIALOGS.BUYVEHICLEWINDOW_WARNING) if constants.IS_KOREA else None)}
    ...
    def __getTradeInContentFields(self, vehicle):
        if self.selectedTab == _TABS.UNDEFINED:
            if vehicle.mayPurchase(self.itemsCache.items.stats.money)[0]:
                ...
                self.selectedTab = _TABS.TRADE
        return {b'tradeInPriceLabel': (i18n.makeString(DIALOGS.BUYVEHICLEWINDOW_TRADEIN_PRICELABEL, vehiclename=vehicle.shortUserName)), 
           b'tradeInCrewCheckbox': (i18n.makeString(DIALOGS.BUYVEHICLEWINDOW_TRADEIN_TANKMENCHECKBOX)), 
           b'tradeInVehiclePrices': (self._getVehiclePrice(vehicle).price.toMoneyTuple()), 
           b'tradeInVehiclePricesActionData': (self._getItemPriceActionData(vehicle)), 
           b'tradeInStudyLabel': (i18n.makeString(DIALOGS.BUYVEHICLEWINDOW_TRADEIN_STUDYLABEL, count=text_styles.stats(str(len(vehicle.crew))))), 
           b'hasTradeOffVehicles': (self.tradeIn.getTradeInDiscounts(vehicle) is not None), 
           b'selectedTab': (self.selectedTab)}
    ...
    def _initData(self, *args):
        ...
        if self.vehicle is None:
            LOG_ERROR(b"Vehicle Item mustn't be None!", b'NationID:', self.nationID, b'InNationID:', self.inNationID)
        elif self.vehicle.isInInventory and not self.vehicle.isRented:
            self.onWindowClose()
        else:
            isTradeIn = self._isTradeIn()
            initData = {b'headerData': (self._getOptainHeaderData(self.vehicle)), 
               b'isTradeIn': isTradeIn, 
               b'contentData': (self._getBuyContentData(self.vehicle, stats, isTradeIn))}
    ...
    def _getContentLinkageFields(self):
        ...
        return {b'contentLinkage': (tradeInUI if self._isTradeIn() else buyUI), 
           b'isContentDAAPI': False}
    ...
    def _getOptainHeaderData(self, vehicle):
        ...
        return {b'userName': (vehicle.userName), 
           b'levelStr': levelStr, 
    ...
    def _getBuyContentData(self, vehicle, stats, isTradeIn):
        ...
        if slotPrice != slotDefaultPrice:
            slotActionPriceData = packActionTooltipData(ACTION_TOOLTIPS_TYPE.ECONOMICS, b'slotsPrices', True, slotPrice, slotDefaultPrice)
        tankmenTotalLabel = i18n.makeString(DIALOGS.BUYVEHICLEWINDOW_TANKMENTOTALLABEL, count=str(tankMenCount))
        studyData = []
        for index, (tCost, defTCost, typeID) in enumerate(zip(shop.getTankmanCostWithGoodyDiscount(vehicle.level), shopDefaults.tankmanCost, (b'free', b'school', b'academy'))):
            if tCost[b'isPremium']:
            ...
            if price != defPrice:
                studyPriceActionData = packActionTooltipData(ACTION_TOOLTIPS_TYPE.ECONOMICS, b'%sTankmanCost' % currency, True, totalPrice, totalDefPrice)
            studyData.insert(0, {b'id': typeID, 
               b'price': price, 
               b'crewType': index, 
               b'actionPrice': studyPriceActionData, 
               b'label': (b'%d%% - %s' % (tCost[b'roleLevel'], formatedPrice))})

        initData = {b'tankmenTotalLabel': tankmenTotalLabel, 
           b'vehiclePrices': (self._getVehiclePrice(vehicle).price.toMoneyTuple()), 
           b'vehiclePricesActionData': vehiclePricesActionData, 
           b'isRentable': (vehicle.isRentable), 
           b'rentDataDD': (self._getRentData(vehicle, vehiclePricesActionData)), 
           b'ammoPrice': (ammoPrice.price.getSignValue(Currency.CREDITS)), 
           b'ammoActionPriceData': ammoActionPriceData, 
           b'slotPrice': slotPrice, 
           b'slotActionPriceData': slotActionPriceData, 
           b'isStudyDisabled': (vehicle.hasCrew), 
           b'isNoAmmo': (not vehicle.hasShells), 
           b'studyData': studyData, 
           b'nation': (self.nationID)}
    ...
    def _getVehiclePrice(self, vehicle):
        ...
        price, defPrice = rentPackage[b'rentPrice'], rentPackage[b'defaultRentPrice']
        return ItemPrice(price, defPrice)
    ...
    def _getRentData(self, vehicle, vehiclePricesActionData):
        ...
        for rentPackage in rentPackages:
            ...
            if rentType == constants.RentType.TIME_RENT:
                ...
                raise SoftException((b'Unknown rentType=[{}] in rentPackages ID [{}]!').format(rentType, rentID))
            actionRentPrice = None
            if rentPackage[b'rentPrice'] != rentPackage[b'defaultRentPrice']:
                actionRentPrice = packItemRentActionTooltipData(vehicle, rentPackage)
            result.append({b'itemId': rentID, 
               b'label': label, 
               b'price': (rentPackage[b'rentPrice'].toMoneyTuple()), 
               b'enabled': enabled, 
               b'actionPrice': actionRentPrice})

        result = self._addPriceBlock(result, vehicle, vehiclePricesActionData)
        selectedId = -1
        for ddItem in result:
            if ddItem[b'enabled']:
                selectedId = ddItem[b'itemId']
                break

        return {b'data': result, b'selectedId': selectedId}

    def _addPriceBlock(self, result, vehicle, vehiclePricesActionData):
        result.append({b'itemId': (-1), 
           b'label': (i18n.makeString(MENU.SHOP_MENU_VEHICLE_RENT_FOREVER)), 
           b'price': (vehicle.buyPrices.itemPrice.price.toMoneyTuple()), 
           b'enabled': (not vehicle.isDisabledForBuy and not vehicle.isHidden), 
           b'actionPrice': vehiclePricesActionData})
        return result
    ...
    def __requestForMoneyObtain(self, data):
        ...
        if isTradeIn:
            ...
            confirmationType = b'tradeInConfirmation'
            addition = b''
            operations = []
            if tradeOffVehicle.hasCrew:
                operations.append(b'crew')
            if tradeOffVehicle.hasShells:
                operations.append(b'shells')
            if tradeOffVehicle.hasConsumables:
                operations.append(b'equipments')
            if tradeOffVehicle.hasOptionalDevices:
                operations.append(b'optionalDevices')
            if tradeOffVehicle.postProgressionAvailability(unlockOnly=True).result:
                operations.append(b'pairModifications')
            if operations:
                operationsStr = [i18n.makeString(b'#dialogs:%s/message/%s' % (confirmationType, o)) for o in operations]
                addition = i18n.makeString(b'#dialogs:%s/message/addition' % confirmationType, operations=(b', ').join(operationsStr))
            ctx = {b'vehName': (neutral(tradeOffVehicle.userName)), b'addition': addition}
    ...
    def __setTradeOffVehicle(self, _=None):
        ...
        tradeOffVehicleVo[b'actionPrice'] = self._getItemPriceActionData(self.tradeInVehicleToSell)
        tradeOffData = {b'vehicleVo': tradeOffVehicleVo, 
           b'price': (self.tradeInVehicleToSell.tradeOffPrice.getSignValue(Currency.GOLD)), 
           b'status': tradeOffVehicleStatus}
        self.as_updateTradeOffVehicleS(tradeOffData)
        self.as_setTradeInWarningMessagegeS(i18n.makeString(DIALOGS.TRADEINCONFIRMATION_MESSAGE, vehName=self.tradeInVehicleToSell.userName, addition=b''))
        return
...
class VehicleRestoreWindow(VehicleBuyWindow):
    ...
    def _addPriceBlock(self, result, vehicle, vehiclePricesActionData):
        ...
        result.insert(0, {b'itemId': (-1), 
           b'label': (i18n.makeString(MENU.SHOP_MENU_VEHICLE_RESTORE)), 
           b'price': (vehicle.restorePrice.toMoneyTuple()), 
    ...
    def _getGuiFields(self, vehicle):
        return {b'title': (i18n.makeString(DIALOGS.RESTOREVEHICLEDIALOG_TITLE, vehiclename=vehicle.userName)), 
           b'cancelBtnLabel': (i18n.makeString(DIALOGS.RESTOREVEHICLEDIALOG_CANCELBTN)), 
           b'submitBtnLabel': (i18n.makeString(DIALOGS.RESTOREVEHICLEDIALOG_SUBMITBTN))}

    def _getContentFields(self, vehicle):
        vehicleName = vehicle.shortUserName if vehicle.isRentable else vehicle.userName
        return {b'priceLabel': (i18n.makeString(DIALOGS.RESTOREVEHICLEDIALOG_PRICELABEL, vehiclename=vehicleName)), 
           b'crewCheckbox': (i18n.makeString(DIALOGS.RESTOREVEHICLEDIALOG_TANKMENCHECKBOX)), 
           b'warningMsg': (i18n.makeString(DIALOGS.RESTOREVEHICLEDIALOG_WARNING) if constants.IS_KOREA else None)}
