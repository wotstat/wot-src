...
class Hangar(LobbySelectableView, HangarMeta, IGlobalListener):
    _SOUND_STATE_PLACE = b'STATE_hangar_place'
    _SOUND_STATE_PLACE_GARAGE = b'STATE_hangar_place_garage'
    __background_alpha__ = 0.0
    __SOUND_SETTINGS = CommonSoundSpaceSettings(name=b'hangar', entranceStates={_SOUND_STATE_PLACE: _SOUND_STATE_PLACE_GARAGE, (StatesGroup.HANGAR_FILTERED): (States.HANGAR_FILTERED_OFF)}, exitStates={}, persistentSounds=(), stoppableSounds=(), priorities=(), autoStart=True, enterEvent=b'', exitEvent=b'')
    ...
    def _populate(self):
        ...
        g_clientUpdateManager.addCallbacks({b'inventory': (self.__updateAlertMessage)})
    ...
    def __switchParams(self, force=False):
        ...
        linkage = b'VehicleParametersUI' if linkage is None else linkage
    ...
    def __showArmoryYardVehiclePreview(self, vehicleCD):
        if self.__armoryYardCtrl.isActive() and self.__armoryYardCtrl.getFinalRewardVehicle().intCD == vehicleCD:
            shared_events.showHeroTankPreview(vehicleCD, bottomPanelTextData={b'uniqueVehicleTitle': (text_styles.tutorial(backport.text(R.strings.vehicle_preview.buyingPanel.uniqueVehicleLabel())))})
            return True
        return False
    ...
    def __handleSelectedEntityUpdated(self, event):
        ctx = event.ctx
        if ctx[b'state'] != CameraMovementStates.FROM_OBJECT:
    ...
    def __updateState(self, force=False):
        ...
        self.as_setupAmmunitionPanelS({b'maintenanceEnabled': isMaintenanceEnabled, 
           b'maintenanceTooltip': maintenanceTooltip, 
           b'maintenanceVisible': (state.isMaintenanceVisible()), 
           b'customizationEnabled': isC11nEnabled, 
           b'customizationTooltip': customizationTooltip, 
           b'customizationVisible': (state.isCustomizationVisible()), 
           b'changeNationVisible': changeNationVisibility, 
           b'changeNationEnable': isNationChangeAvailable, 
           b'changeNationTooltip': changeNationTooltip, 
           b'changeNationIsNew': changeNationIsNew})
    ...
    def __onServerSettingChanged(self, diff):
        if b'isRegularQuestEnabled' in diff:
            self.__updateHeaderComponent()
            self.__updateHeaderEpicWidget()
    ...
    def __onOptDeviceClick(self, event):
        ...
        if self.hangarSpace.spaceLoading():
            _logger.warning(b'Optional Device click was not handled (ctx=%s). HangarSpace is currently loading.', ctx)
