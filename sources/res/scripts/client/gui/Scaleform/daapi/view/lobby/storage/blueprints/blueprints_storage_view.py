...
class StorageCategoryBlueprintsView(StorageCategoryBlueprintsViewMeta, StorageCarouselEnvironment):
    ...
    def _populate(self):
        ...
        g_clientUpdateManager.addCallbacks({b'blueprints': (self.__onUpdateBlueprints), 
           b'serverSettings.blueprints_config.levels': (self.__onUpdateBlueprints)})
        ...
        self.updateSearchInput(self.filter.get(b'searchNameVehicle'))
    ...
    def _dispose(self):
        ...
        if self.__needToResetScrollTo:
            self.filter.update({b'can_convert': False})
    ...
    def __makeFragmentVO(count, iconName, tooltipData=None):
        ...
        return {b'hasFragments': (count > 0), 
           b'label': label, 
           b'iconSmall': (RES_ICONS.getBlueprintFragment(b'small', iconName)), 
           b'iconBig': (RES_ICONS.getBlueprintFragment(b'big', iconName)), 
    ...
    def __updateIntelligence(self):
        self.as_updateIntelligenceDataS(self.__makeFragmentVO(self._itemsCache.items.blueprints.getIntelligenceCount(), b'intelligence', BlueprintTypes.INTELLIGENCE_DATA))
        return
    ...
    def __updateFilterWarning(self):
        ...
        if hasNoFilterResults and not hasNoVehicles:
            if self.__areCertainLevelsChosenInFilter((b'level_1', b'level_11')):
    ...
    def __restoreCarouselState(self):
        self.as_updateCanConvertS(self.filter.get(b'can_convert'))
        self.updateSearchInput(self.filter.get(b'searchNameVehicle'))
        self.applyFilter()
        scrollTo = self.filter.get(b'scroll_to')
