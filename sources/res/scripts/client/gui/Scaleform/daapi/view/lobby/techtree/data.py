...
class _ItemsData(object):
    ...
    def __init__(self, dumper, itemsCache=None):
        ...
        if dumper is not None and isinstance(dumper, _BaseDumper):
            ...
            raise SoftException(b'Dumper is invalid.')
        self._nodes = []
        self._nodesIdx = {}
    ...
    def __del__(self):
        _logger.debug(b'Data deleted: %s', self)
        return
    ...
    def getInventoryVehicles(self):
        ...
        _logger.debug(b'getInventoryVehicles: %r', nodeCDs)
...
class ResearchItemsData(_ItemsData):
    ...
    def invalidateItem(self, itemCD):
        if self._topLevelCDs is not None and self._topLevel is not None:
            ...
            if idx is not None and 0 <= idx < len(self._topLevel):
                return self._dumper.invalidateCachedItemData(b'top', idx, self._topLevel[idx], self.getRootItem())
        if self._nodesIdx is not None and self._nodes is not None:
            ...
            if idx is not None and 0 <= idx < len(self._nodes):
                return self._dumper.invalidateCachedItemData(b'nodes', idx, self._nodes[idx], self.getRootItem())
        return
    ...
    def getUrgentIds(self, nodeCD):
        ...
        _logger.debug(b'[ModuleDependencies] nodeCD = %s, module dependencies %s', nodeCD, moduleDependencies)
        for node in self._nodes:
            nodeCD = node.getNodeCD()
    ...
    def invalidateHovered(self, nodeCD):
        ...
        if nodeCD == -1:
            ...
            _logger.debug(b'[ModuleDependencies] nodes with "dashed" state cleared: %s', result)
        else:
            conflicted = self._moduleInstaller.updateConflicted(nodeCD, self.getRootItem())
            moduleDependencies = [moduleCD for moduleTypes in conflicted for moduleCD in moduleTypes]
            _logger.debug(b'[ModuleDependencies] nodeCD = %s, module dependencies %s', nodeCD, moduleDependencies)
            ...
            _logger.debug(b'[ModuleDependencies] nodes with "dashed" state set: %s', result)
        return result
    ...
    def _findSelectedNode(self, nationID):
        ...
        if nodeCD in self._nodesIdx.keys():
            ...
            if vehicle.isInInventory:
                ...
                _logger.error(b'Current vehicle not found in inventory: %d', nodeCD)
        elif vehicle.isCollectible:
            _logger.info(b'Current vehicle with id=%d is collectible. It is not in nation tree', nodeCD)
        elif vehicle.isHidden:
            _logger.debug(b'Current vehicle is hidden. Is it define in nation tree: %d', nodeCD)
        else:
            _logger.error(b'Current vehicle not found in nation tree: %d', nodeCD)
        return
