...
CustomizationAnchorInitVO = namedtuple(b'CustomizationAnchorInitVO', (b'anchorUpdateVOs', b'typeRegions'))
CustomizationAnchorsStateVO = namedtuple(b'CustomizationAnchorsStateVO', (b'uid', b'value'))
CustomizationAnchorsSetVO = namedtuple(b'CustomizationAnchorsSetVO', (b'rendererList',))
CustomizationAnchorPositionVO = namedtuple(b'CustomizationAnchorPositionVO', (b'zIndex', b'slotId'))
AnchorPositionData = namedtuple(b'AnchorPositionData', (b'angleToCamera', b'clipSpacePos', b'slotId'))
_WAITING_MESSAGE = b'loadHangarSpace'
...
class MainView(LobbySubView, CustomizationMainViewMeta):
    ...
    def showBuyWindow(self, ctx=None):
        ...
        elif isGamefaceBuyViewOpened:
            _logger.debug(b'Gameface customization cart is already opened, ignore event')
            return
        _logger.info(b'Gameface customization cart is opened')
        ctx = ctx or {}
    ...
    def onPressEscBtn(self):
        if self.__propertiesSheet.handleEscBtn():
            ...
            if self.__ctx.mode.selectedItem is not None:
                ...
                self.__ctx.mode.unselectSlot()
            elif self.__ctx.mode.returnToPreviousTab():
                return
    ...
    def changeSeason(self, seasonIdx, needToKeepSelect):
        ...
        if seasonIdx in SEASON_IDX_TO_TYPE:
            ...
            _logger.error(b'Wrong season index %(seasonIdx)d', {b'seasonIdx': seasonIdx})
    ...
    def __updateInnerEntryPoints(self):
        ...
        statsTrackerEntry = {b'itemId': (CUSTOMIZATION_CONSTS.INNER_ENTRY_POINT_STATS_TRACKER), 
           b'label': (text_styles.middleTitle(backport.text(R.strings.vehicle_customization.statsTracker.entryPoint()))), 
           b'isVisible': (isStatTrackerTabEnabled()), 
           b'isSelected': (self.__ctx.mode.tabId == CustomizationTabs.STAT_TRACKERS), 
           b'hasNovelty': (bool(statTrackerNovelty))}
        progressiveEntry = {b'itemId': (CUSTOMIZATION_CONSTS.INNER_ENTRY_POINT_PROGRESSIVE), 
           b'label': (text_styles.middleTitle(backport.text(R.strings.vehicle_customization.progression.entryPoint()))), 
           b'isVisible': (any(g_currentVehicle.item.getAnchors(GUI_ITEM_TYPE.PROJECTION_DECAL, areaId) for areaId in Area.ALL)), 
           b'isSelected': False, 
           b'hasNovelty': False}
    ...
    def _invalidate(self, *args, **kwargs):
        ...
        callback = (args[0] or {}).get(b'callback')
    ...
    def _getUpdatedAnchorsData(self):
        ...
        for zIdx, anchorVO in enumerate(anchorVOs):
            anchorPositionVOs.append(CustomizationAnchorPositionVO(zIdx, anchorVO[b'slotId'])._asdict())
    ...
    def __onServerSettingChanged(self, diff):
        if b'isCustomizationEnabled' in diff and not diff.get(b'isCustomizationEnabled', True):
    ...
    def __setSeasonData(self):
        ...
        for season in SEASONS_ORDER:
            ...
            seasonRenderersList.append({b'nameText': (VEHICLE_CUSTOMIZATION.getSeasonName(seasonName)), 
               b'nameSelectedText': (VEHICLE_CUSTOMIZATION.getSeasonSelectedName(seasonName)), 
               b'seasonImageSrc': (RES_ICONS.getSeasonImage(seasonName)), 
               b'seasonBGImageSrc': (RES_ICONS.getSeasonBGImage(seasonName)), 
               b'seasonShineImageSrc': (RES_ICONS.getSeasonShineImage(seasonName)), 
               b'isFilled': isFilled, 
               b'forceAnim': False, 
               b'tooltip': (makeTooltip(body=VEHICLE_CUSTOMIZATION.getSheetSeasonName(seasonName)))})
    ...
    def __setHeaderInitData(self):
        ...
        if tabId == CustomizationTabs.STYLES_3D:
            ...
            template = b'#vehicle_customization:customization/header/counter/' + typeName
            itemsCounter = textStyle(_ms(template, filled=filledSlotsCount, available=slotsCount))
        else:
            itemsCounter = b''
        self.as_setHeaderDataS({b'tankTier': (str(int2roman(vehicle.level))), 
           b'tankName': (vehicle.shortUserName), 
           b'tankInfo': itemsCounter, 
           b'tankType': ((b'{}_elite').format(vehicle.type) if vehicle.isElite else vehicle.type), 
           b'isElite': (vehicle.isElite), 
           b'closeBtnTooltip': (VEHICLE_CUSTOMIZATION.CUSTOMIZATION_HEADERCLOSEBTN), 
           b'isQuestProgressionInfoBtnVisible': isQuestProgressionInfoBtnVisible})
        return

    def __onProlongStyleRent(self):
        ctx = {b'prolongStyleRent': True}
    ...
    def __onAnchorsStateChanged(self, changedStates):
        ...
        self.as_setAnchorsStateS({b'anchorsData': anchorStateVOs})
        return
