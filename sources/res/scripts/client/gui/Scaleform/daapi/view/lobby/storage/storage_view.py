...
class StorageView(LobbySubView, StorageViewMeta):
    ...
    def __init__(self, ctx=None):
        ...
        context = ctx or {}
        self.__switchSection(sectionName=context.get(b'defaultSection', STORAGE_CONSTANTS.FOR_SELL), sectionTab=context.get(b'defaultTab'), skipRefresh=True)
        return
    ...
    def findActiveSectionAndTabId(self):
        ...
        for section in self.components.values():
            if not section or not section.getActive():
                continue
            sectionId = first(s[b'id'] for s in sectionsList if s[b'linkage'] == section.getAlias())
            tabs = section.getTabsData() if hasattr(section, b'getTabsData') else []
            activeTabLinkage = first(tabLinkage for tabLinkage, tabComponent in section.components.items() if tabComponent.getActive())
            for tab in tabs:
                if tab[b'linkage'] == activeTabLinkage:
                    return (sectionId, tab[b'id'])
    ...
    def _invalidate(self, *args, **kwargs):
        ...
        self.__switchSection(sectionName=(args[0] or {}).get(b'defaultSection', STORAGE_CONSTANTS.FOR_SELL))
        return
    ...
    def __initialize(self):
        self.as_setDataS({b'bgSource': (backport.image(R.images.gui.maps.icons.storage.background())), 
           b'sections': (self.__sections), 
           b'showDummyScreen': (self.__showDummyScreen)})
    ...
    def __createSections(self):
        ...
        return [section for section in getSectionsList() if section[b'id'] not in notActiveSection]

    def __setActiveSectionIdx(self, sectionAlias):
        for idx, section in enumerate(self.__sections):
            if section[b'linkage'] == sectionAlias:
    ...
    def __isSectionExist(self, tabId):
        return tabId in [s[b'id'] for s in self.__sections]
