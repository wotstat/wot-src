...
_SHOW_RIBBON_SOUND_NAME = b'show_ribbon'
_SHOW_RIBBON_EXP_SOUND_NAME = b'show_ribbon_role_exp'
_HIDE_RIBBON_SOUND_NAME = b'hide_ribbon'
_CHANGE_RIBBON_SOUND_NAME = b'increment_ribbon_counter'
...
def _getVehicleData(arenaDP, vehArenaID):
    ...
    if isBattleRoyale(vehicleType.tags) and isSpawnedBot(vehicleType.tags):
        return (vehicleName, b'')
    return (vehicleName, vehicleType.classTag or b'')
...
def _enemyDetectionRibbonFormatter(ribbon, arenaDP, updater):
    ...
    if count > 1:
        ...
        if vIDs:
            ...
            _logger.error(b'Enemy detection ribbon has no vehicle ID! %s', ribbon.getID())
            vehicleName = b''
            vehicleClassTag = b''
...
def _singleVehRibbonFormatter(ribbon, arenaDP, updater):
    if ribbon.getDamageSource() == DAMAGE_SOURCE.PLAYER:
        ...
        vehicleName, vehicleClassTag = b'', ribbon.getDamageSource()
    bonusRibbonLabelID = _BRL.BASE_BONUS_LABEL if ribbon.isRoleBonus() else _BRL.NO_BONUS
...
class BattleRibbonsPanel(RibbonsPanelMeta, IArenaVehiclesController):
    ...
    def onHide(self, ribbonID):
        ribbon = self.__ribbonsAggregator.getRibbon(ribbonID)
        _logger.debug(b'RIBBON PANEL: onHide: ribbonID=%s, ribbon="%s"', ribbonID, ribbon)
    ...
    def __playSound(self, eventName):
        ...
        if soundNotifications and hasattr(soundNotifications, b'play') and hasattr(soundNotifications, b'isPlaying'):
    ...
    def __invalidateRibbon(self, ribbon, method):
        if not self.__canBeShown(ribbon):
            _logger.debug(b'Delaying ribbon processing %s', ribbon)
        ...
        if self._shouldShowRibbon(ribbon):
            ...
            if formatter:
                ...
                _logger.error(b'Could not find formatter for ribbon type %s', ribbonType)
        else:
            self.__ribbonsAggregator.resetRibbonData(ribbon.getID())
        return

    def __addRibbon(self, ribbonID, ribbonType=b'', leftFieldStr=b'', vehName=b'', vehType=b'', rightFieldStr=b'', bonusRibbonLabelID=_BRL.NO_BONUS, role=b''):
        _logger.debug(b'RIBBON PANEL: as_addBattleEfficiencyEventS: ribbonID=%s, ribbonType="%s", ", leftFieldStr="%s, vehName="%s", vehType="%s", rightFieldStr="%s", bonusRibbonLabelID=%s, role=%s.', ribbonID, ribbonType, leftFieldStr, vehName, vehType, rightFieldStr, bonusRibbonLabelID, role)
    ...
    def __updateRibbon(self, ribbonID, ribbonType=b'', leftFieldStr=b'', vehName=b'', vehType=b'', rightFieldStr=b'', bonusRibbonLabelID=_BRL.NO_BONUS, role=b''):
        _logger.debug(b'RIBBON PANEL: as_updateBattleEfficiencyEventS: ribbonID=%s, ribbonType="%s", ", leftFieldStr="%s, vehName="%s", vehType="%s", rightFieldStr="%s", bonusRibbonLabelID=%s, role=%s.', ribbonID, ribbonType, leftFieldStr, vehName, vehType, rightFieldStr, bonusRibbonLabelID, role)
    ...
    def __getActivatedAbilityModifier(self):
        ...
        return _ABILITY_MODIFIERS_BY_CDS.get(validItnCD, b'')
    ...
    def __onGUIVisibilityChanged(self, event):
        self.__isVisible = event.ctx[b'visible']
        return
