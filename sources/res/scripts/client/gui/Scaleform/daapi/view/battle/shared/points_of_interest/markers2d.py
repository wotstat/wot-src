...
_POI_MARKER_SYMBOL = b'PointOfInterestMarkerUI'
...
class PointsOfInterestPlugin(plugins.MarkerPlugin, PointsOfInterestListener):
    ...
    def onPoiAdded(self, poiState):
        ...
        if poi is None:
            _logger.error(b'Missing PointOfInterest id=%s', poiID)
            return
        else:
    ...
    def __updateType(self, markerID, state):
        ...
        self._invokeMarker(markerID, b'setType', poiType)
        return
    ...
    def __updateStatus(self, markerID, poiState):
        ...
        self._invokeMarker(markerID, b'setStatus', statusID, duration)
        return

    def __updateProgress(self, markerID, poiState):
        progress = poiState.progress
        self._invokeMarker(markerID, b'setProgress', progress)
        return

    def __updateTeam(self, markerID, poiState):
        isAlly = bool(poiState.invader)
        self._invokeMarker(markerID, b'setIsAlly', isAlly)
        return
