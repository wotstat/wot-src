...
class VehiclePreviewBottomPanelTradeIn(VehiclePreviewBottomPanelTradeInMeta):
    ...
    def __getVO(self):
        ...
        return {b'currentPrice': (getItemPricesVO(self.__tradeIn.getTradeInPrice(self.__tradeInVehicleToBuy))), 
           b'selectedPrice': (None if isFreeExchange else self.__getSelectedPrice()), 
           b'statusText': (text_styles.greenText(statusText) if isFreeExchange else statusText), 
           b'statusOk': statusOk, 
           b'tradeOffAvailable': tradeOffAvailable}
