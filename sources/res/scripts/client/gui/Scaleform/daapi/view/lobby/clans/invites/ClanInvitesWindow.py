...
class ClanInvitesWindow(ClanInvitesWindowMeta, ClanListener, ClanEmblemsHelper):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.__actualRequestsCount = b'0'
        self.__processedInvitesCount = b'0'
    ...
    def _onPaginatorListChanged(self, alias, f, selectedID, isFullUpdate, isReqInCoolDown, result):
        ...
        if alias == CLANS_ALIASES.CLAN_PROFILE_REQUESTS_VIEW_ALIAS:
            if f == CLANS_ALIASES.INVITE_WINDOW_FILTER_ACTUAL:
                ...
                LOG_DEBUG((b'Unexpected behaviour: unknown filter {} for alias {}').format(f, alias))
            ...
            LOG_DEBUG(b'Unexpected behaviour: unknown view alias ', alias)
    ...
    def _updateClanInfo(self):
        self.as_setClanInfoS({b'name': (self.clanProfile.getClanFullName()), 
           b'bgIcon': (RES_ICONS.MAPS_ICONS_CLANS_INVITESWINDOW_CC_HEADER_BACK), 
           b'creationDate': (i18n.makeString(CLANS.CLAN_HEADER_CREATIONDATE, creationDate=backport.getShortDateFormat(self.clanInfo.getCreatedAt())))})
        return
    ...
    def _updateHeaderState(self):
        ...
        if freePlaces == 0:
            ...
            freePlacesInClanText = gui.makeHtmlString(b'html_templates:lobby/research', b'warningMessage', {b'text': freePlacesInClanText})
        else:
            inviteButtonEnabled = True
            inviteButtonTooltip = _ms(CLANS.CLANINVITESWINDOW_TOOLTIPS_HEADER_INVITEBUTTON)
        self.as_setHeaderStateS({b'inviteButtonEnabled': inviteButtonEnabled, 
           b'inviteButtonText': (CLANS.CLANINVITESWINDOW_HEADER_INVITEINCLAN), 
           b'inviteButtonTooltip': (makeTooltip(body=inviteButtonTooltip)), 
           b'freePlacesInClanText': freePlacesInClanText})
        return

    def __updateTabsState(self):
        self.as_setDataS({b'tabDataProvider': [
                              {b'label': (i18n.makeString(CLANS.CLANINVITESWINDOW_TABREQUESTS, value=self.__actualRequestsCount)), 
                                 b'linkage': (CLANS_ALIASES.CLAN_PROFILE_REQUESTS_VIEW_LINKAGE)},
                              {b'label': (i18n.makeString(CLANS.CLANINVITESWINDOW_TABINVITES, value=self.__processedInvitesCount)), 
                                 b'linkage': (CLANS_ALIASES.CLAN_PROFILE_INVITES_VIEW_LINKAGE)}]})
        return
