...
def _validateVehicle(vehicle):
    if not isinstance(vehicle, Vehicle):
        raise SoftException((b'vehicle has to be valid gui_items.Vehicle, got: {}').format(vehicle))
    return


class VehPostProgressionEntryPoint(EventSystemEntity):
    __slots__ = (b'_eliteWatchedList', b'_watchedListDirty', b'_parentObj', b'_vehicle', b'_modulesToUnlock')
    ...
    def __getPostProgressionData(self):
        ...
        if not vehicle or not vehicle.isPostProgressionExists or g_bootcamp.isRunning():
            return {b'state': (POSTPROGRESSION_CONSTS.RESEARCH_STATE_HIDDEN)}
        eliteProgress = vehicle.getEliteStatusProgress()
        self._modulesToUnlock = eliteProgress.toUnlock
        return {b'state': (self.__getUnlockState()), 
           b'vehicleId': (vehicle.intCD), 
           b'moduleIds': ((self._vehicle.isElite or eliteProgress).toUnlock if 1 else []), 
           b'label': (self.__getLabel(eliteProgress)), 
           b'showCounter': (needToShowCounter(vehicle))}
