from gui.Scaleform.daapi.view.lobby.storage.inventory.filters.filter_by_vehicle import FiltrableInventoryCategoryByVehicleTabView
from gui.Scaleform.locale.RES_ICONS import RES_ICONS
from gui.Scaleform.locale.TOOLTIPS import TOOLTIPS
from gui.shared.gui_items import GUI_ITEM_TYPE
from gui.shared.utils.functions import makeTooltip
from gui.shared.utils.requesters.ItemsRequester import REQ_CRITERIA
from shared_utils import CONST_CONTAINER

class _ModuleFilterBit(CONST_CONTAINER):
    GUN = 1
    TURRET = 2
    ENGINE = 4
    CHASSIS = 8
    RADIO = 16


_TYPE_FILTER_ITEMS = [
 {b'filterValue': (_ModuleFilterBit.GUN), 
    b'selected': False, 
    b'tooltip': (makeTooltip(body=TOOLTIPS.STORAGE_FILTER_MODULES_BTNS_TYPE_GUNS)), 
    b'icon': (RES_ICONS.MAPS_ICONS_STORAGE_FILTERS_ICON_BUTTON_GUN)},
 {b'filterValue': (_ModuleFilterBit.TURRET), 
    b'selected': False, 
    b'tooltip': (makeTooltip(body=TOOLTIPS.STORAGE_FILTER_MODULES_BTNS_TYPE_TOWERS)), 
    b'icon': (RES_ICONS.MAPS_ICONS_STORAGE_FILTERS_ICON_BUTTON_TURRET)},
 {b'filterValue': (_ModuleFilterBit.ENGINE), 
    b'selected': False, 
    b'tooltip': (makeTooltip(body=TOOLTIPS.STORAGE_FILTER_MODULES_BTNS_TYPE_ENGINES)), 
    b'icon': (RES_ICONS.MAPS_ICONS_STORAGE_FILTERS_ICON_BUTTON_ENGINE)},
 {b'filterValue': (_ModuleFilterBit.CHASSIS), 
    b'selected': False, 
    b'tooltip': (makeTooltip(body=TOOLTIPS.STORAGE_FILTER_MODULES_BTNS_TYPE_CHASSIS)), 
    b'icon': (RES_ICONS.MAPS_ICONS_STORAGE_FILTERS_ICON_BUTTON_TRUCK)},
 {b'filterValue': (_ModuleFilterBit.RADIO), 
    b'selected': False, 
    b'tooltip': (makeTooltip(body=TOOLTIPS.STORAGE_FILTER_MODULES_BTNS_TYPE_RADIOS)), 
    b'icon': (RES_ICONS.MAPS_ICONS_STORAGE_FILTERS_ICON_BUTTON_RADIO)}]
_TYPE_ID_BIT_TO_TYPE_ID_MAP = {(_ModuleFilterBit.GUN): (GUI_ITEM_TYPE.GUN), 
   (_ModuleFilterBit.TURRET): (GUI_ITEM_TYPE.TURRET), 
   (_ModuleFilterBit.ENGINE): (GUI_ITEM_TYPE.ENGINE), 
   (_ModuleFilterBit.CHASSIS): (GUI_ITEM_TYPE.CHASSIS), 
   (_ModuleFilterBit.RADIO): (GUI_ITEM_TYPE.RADIO)}

class ModulesTabView(FiltrableInventoryCategoryByVehicleTabView):
    filterItems = _TYPE_FILTER_ITEMS

    def _getClientSectionKey(self):
        return b'storage_modules'

    def _getItemTypeID(self):
        return GUI_ITEM_TYPE.VEHICLE_MODULES

    def _getFilteredCriteria(self):
        criteria = super(ModulesTabView, self)._getFilteredCriteria()
        typeIds = [_TYPE_ID_BIT_TO_TYPE_ID_MAP[bit] for bit in _TYPE_ID_BIT_TO_TYPE_ID_MAP.iterkeys() if self._filterMask & bit]
        if typeIds:
            criteria |= REQ_CRITERIA.ITEM_TYPES(*typeIds)
        if self._selectedVehicle:
            criteria |= REQ_CRITERIA.VEHICLE.SUITABLE([self._selectedVehicle])
        return criteria

    def _getRequestCriteria(self, invVehicles):
        criteria = REQ_CRITERIA.INVENTORY
        criteria |= REQ_CRITERIA.TYPE_CRITERIA(GUI_ITEM_TYPE.VEHICLE_MODULES, REQ_CRITERIA.VEHICLE.SUITABLE(invVehicles))
        return criteria
