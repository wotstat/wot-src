...
class ProfileAwards(ProfileAwardsMeta):
    ...
    def _sendAccountData(self, targetData, accountDossier):
        ...
        self.as_responseDossierS(self._battlesType, {b'achievementsList': packedList, 
           b'totalItemsList': totalItemsList, 
           b'battlesCount': (targetData.getBattlesCount())}, b'', b'')
        return

    def _populate(self):
        super(ProfileAwards, self)._populate()
        initData = {b'achievementFilter': {b'dataProvider': [
                                  ...
                                  b'selectedItem': (self.__achievementsFilter)}}
    ...
    def _onRareImageReceived(self, imgType, rareID, imageData):
        if imgType == IMAGE_TYPE.IT_67X71:
            ...
            achievement = stats.getAchievement((b'rareAchievements', rareID))
    ...
    def __packProviderItem(key):
        return {b'label': (i18n.makeString(key)), 
           b'key': key}
