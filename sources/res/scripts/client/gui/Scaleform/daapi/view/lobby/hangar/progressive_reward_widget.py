...
class ProgressiveRewardWidget(ProgressiveRewardWidgetMeta):
    ...
    def __onServerSettingsChange(self, diff):
        configs = {b'progressive_reward_config', IS_LOOT_BOXES_ENABLED}
    ...
    def __update(self):
        ...
        if pr is None:
            _logger.warning(b'There is not info about progressive reward')
            self.as_setDataS({b'isEnabled': False})
            return
        else:
