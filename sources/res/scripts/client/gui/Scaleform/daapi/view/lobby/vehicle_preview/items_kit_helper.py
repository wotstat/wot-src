...
OFFER_CHANGED_EVENT = b'offerChanged'
...
def __getPremiumPlusIcon(days):
    r = R.images.gui.maps.icons.quests.bonuses.small.dyn((b'premium_plus_{}').format(days))
...
StyleInstallInfo = namedtuple(b'StyleInstallInfo', (b'canInstall', b'style', b'vehicle'))
...
def getCDFromId(itemType, itemId):
    if itemType in _CUSTOMIZATION_ITEM_TYPE:
        return makeCD(b'customizationItem', _CUSTOMIZATION_TYPES_MAP[itemType], itemId)
    return itemId
...
def getItemDescription(rawItem, item):
    description = b''
    if item is not None:
        if hasattr(item, b'fullDescription'):
    ...
    if not description:
        description = rawItem.description or rawItem.title or b''
    return description


def getTooltipText(item):
    text = b''
    if item and item.itemTypeName != b'':
...
def showItemTooltip(toolTipMgr, rawItem, item):
    ...
    if tooltipType is not None:
        ...
        toolTipMgr.onCreateTypedTooltip(tooltipType, [rawItem.id] + buildArgs[1:], b'INFO')
    else:
        header = getItemTitle(rawItem, item)
        body = getItemDescription(rawItem, item)
        tooltip = makeTooltip(header, body)
        toolTipMgr.onCreateComplexTooltip(tooltip, b'INFO')
    return
...
def showAwardsTooltip(toolTipMgr, itemType, data):
    ...
    if tooltipType is not None:
        toolTipMgr.onCreateTypedTooltip(tooltipType, [data], b'INFO')
    return
...
def _createItemVO(rawItem, itemsCache, goodiesCache, slotIndex, rawTooltipData=None):
    ...
    if rawItem == _BOX_ITEM:
        ...
        if rawItem.type in ItemPackTypeGroup.CREW:
            ...
            if rawItem.type == ItemPackType.CUSTOM_PREMIUM_PLUS and not __getPremiumPlusIconExists(count):
                countFormat = (b'x{}').format(count) if count > 1 else b''
            else:
                countFormat = b''
        else:
            countFormat = (b'x{}').format(count) if count > 1 else b''
    return {b'id': (str(cd)), 
       b'icon': icon, 
       b'overlayType': overlay, 
       b'type': (rawItem.type if rawItem is not None else BOX_TYPE), 
       b'iconAlt': (RES_ICONS.MAPS_ICONS_ARTEFACT_NOTFOUND), 
       b'slotIndex': slotIndex, 
       b'count': countFormat, 
       b'rawData': rawTooltipData}
...
def _getBoxTooltipVO(rawItems, itemsCache, goodiesCache):
    ...
    for rawItem in rawItems:
        ...
        if fittingItem is not None and fittingItem.itemTypeID in GUI_ITEM_TYPE.CUSTOMIZATIONS:
            icon = RES_ICONS.getBonusIcon(b'small', fittingItem.itemTypeName)
        else:
            icon = getItemIcon(rawItem, fittingItem)
        if fittingItem is not None:
            ...
            overlay = SLOT_HIGHLIGHT_TYPES.NO_HIGHLIGHT
        items.append({b'id': (str(rawItem.id)), 
           b'icon': icon, 
           b'overlayType': overlay, 
           b'type': (rawItem.type), 
           b'count': (str(rawItem.count) if rawItem.type not in _UNCOUNTABLE_ITEM_TYPE and rawItem.count > 1 else b''), 
           b'description': (getItemDescription(rawItem, fittingItem)), 
           b'title': (getItemTitle(rawItem, fittingItem, forBox=True)), 
           b'groupID': (rawItem.groupID), 
           b'rawCount': (rawItem.count)})

    vo = {b'icon': (backport.image(R.images.gui.maps.icons.rankedBattles.boxes.c_48x48.metal_1())), 
       b'count': (len(rawItems)), 
       b'items': items}
    return vo
...
class _NodeContainer(object):
            ...
            if slotIdx < self._maxChildCount:
                ...
                if mayInstall:
                    ...
                    if counterLimit == _UNLIMITED_ITEMS_COUNT or itemsCount <= counterLimit:
                        ...
                        return
                    self._children.append(item.replace({b'count': counterLimit}))
                    itemsLeft = itemsCount - counterLimit
                    item = item.replace({b'count': itemsLeft})
    ...
    def wrapVO(items):
        result = {}
        if items:
            result = {b'items': items, b'isEnabled': True, 
               b'topTitle': b'', 
               b'topTitleSmall': b''}
        return result
...
class _GiftNodeContainer(_NodeContainer):
    ...
    def wrapVO(items):
        ...
        if items:
            result = {b'items': items, b'isEnabled': True, 
               b'topTitle': (text_styles.concatStylesToSingleLine(icons.makeImageTag(source=backport.image(R.images.gui.maps.icons.library.icon_gift()), width=17, height=15, vSpace=0), text_styles.vehicleStatusCriticalTextSmall(VEHICLE_PREVIEW.BUYINGPANEL_PRESENT))), 
               b'topTitleSmall': (icons.makeImageTag(source=backport.image(R.images.gui.maps.icons.library.icon_gift()), width=17, height=15, vSpace=0))}
        return result
...
class _CustomCrewSkillsNodeContainer(_NodeContainer):
    ...
    def parseCrewSkills(self, item):
        return {idx: tankmanItem.get(b'skills', []) + tankmanItem.get(b'freeSkills', []) for idx, tankmanItem in enumerate(item.extra[b'tankmen'])}
    ...
    if itemsPack:
        ...
        for item in itemsPack:
            if item.type not in ItemPackTypeGroup.DISCOUNT + ItemPackTypeGroup.VEHICLE:
                result.append({b'img': (getItemIcon(item, None)), b'text': (getItemTitle(item, None, additionalInfo=True))})
...
def addCompensationInfo(itemsVOs, itemsPack, itemsCache=None):
    compensationInfo = [{b'id': (str(item.id)), b'type': (item.type), b'hasCompensation': (getCompensateItemsCount(item, itemsCache) > 0)} for item in collapseItemsPack(itemsPack)]

    def hasCompensation(itemVO):
        for ci in compensationInfo:
            if ci[b'id'] == str(itemVO[b'id']) and ci[b'type'] == itemVO[b'type']:
                return ci[b'hasCompensation']

        return False

    for itemsVO in itemsVOs:
        for vo in itemsVO[b'items']:
            vo[b'hasCompensation'] = hasCompensation(vo)
            for insideBoxVO in vo[b'rawData'][b'items'] if vo[b'rawData'] else []:
                insideBoxVO[b'hasCompensation'] = hasCompensation(insideBoxVO)
