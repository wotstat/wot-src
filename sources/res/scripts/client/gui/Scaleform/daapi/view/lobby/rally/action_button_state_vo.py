...
class ActionButtonStateVO(dict):
    ...
    def __init__(self, unitEntity):
        ...
        self.__INVALID_UNIT_MESSAGES = {(UNIT_RESTRICTION.UNDEFINED): (
           ...
           (UNIT_RESTRICTION.RANK_RESTRICTION): (
                                               backport.text(R.strings.ranked_battles.unit.message.rankRestriction(), rank=self.__rankedCtrl.getRankSquadRestriction()), {}), 
           (UNIT_RESTRICTION.DIVISION_RESTRICTION): (
                                                   backport.text(R.strings.ranked_battles.unit.message.divisionRestriction()), {})}
        ...
        self[b'stateString'] = self.__stateTextStyleFormatter(i18n.makeString(stateKey, **stateCtx))
        self[b'label'] = self._getLabel()
        self[b'isEnabled'] = self.__isEnabled
        self[b'isReady'] = self._playerInfo.isReady
        self[b'toolTipData'] = self.__toolTipData
        return

    def getSimpleState(self):
        stateKey, stateCtx = self.__getState()
        return re.sub(b'<.*/> ', b'', i18n.makeString(stateKey, **stateCtx))
    ...
    def __getNotAvailableIcon(self):
        return icons.makeImageTag(RES_ICONS.MAPS_ICONS_LIBRARY_REDNOTAVAILABLE, 12, 12, 0, 0) + b' '
    ...
    def _rotationGroupBlockMessage(self):
        ...
        return (
         CYBERSPORT.WINDOW_UNIT_MESSAGE_VEHICLEINNOTREADY_ROTATIONGROUPLOCKED, {b'groupNum': (g_currentVehicle.item.rotationGroupNum)})
    ...
    def _clanMembersNotEnoughMessage(self):
        ...
        return (FORTIFICATIONS.ROSTER_CLANMEMBERSNOTENOUGH,
         {b'minClanMembers': minClanMembers, 
            b'actualClanMembers': actualClanMembers})
    ...
    def getInvalidVehicleLevelsMessage(ctx):
        if ctx is None:
            ...
            vehLevels = ctx.get(b'vehLevels', ())
            stateString = CYBERSPORT.WINDOW_UNIT_MESSAGE_INVALIDLEVELERROR_UNRESOLVED
            if vehLevels:
                stateStringCandidate = CYBERSPORT.window_unit_message_invalidlevelerror((b'_').join(str(l) for l in vehLevels))
