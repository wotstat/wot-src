...
class SimpleCustomizationTooltip(BlocksTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        blockData = formatters.packBuildUpBlockData(blocks, gap=-6, padding={b'bottom': (-5)})
        return [
         blockData]
    ...
    def _packNonHistoricBlock(topPadding):
        ...
        blocks = [
         formatters.packTextBlockData(text=text_styles.middleTitle(backport.text(title())), padding={b'top': topPadding, b'bottom': 10}),
...
class NonHistoricTooltip(SimpleCustomizationTooltip):
    ...
    def _packItemBlocks(self):
        ...
        blocks = [
         formatters.packImageTextBlockData(title=text_styles.middleTitle(backport.text(nonHistoricTitle())), img=backport.image(img()), imgPadding={b'left': (-3), b'top': (-4)}),
...
class FantasticalTooltip(SimpleCustomizationTooltip):
    ...
    def _packItemBlocks(self):
        ...
        blocks = [
         formatters.packImageTextBlockData(title=text_styles.middleTitle(backport.text(fantasticalTitle())), img=backport.image(img()), imgPadding={b'left': (-3), b'top': (-4)}),
...
class ChainedTooltip(SimpleCustomizationTooltip):
    ...
    def _packChainedBlock():
        ...
        blocks = [
         formatters.packImageTextBlockData(title=text_styles.middleTitle(backport.text(title())), img=backport.image(img()), imgPadding={b'left': 4, b'right': 6, b'top': 4}, padding={b'bottom': 10}),
         formatters.packTextBlockData(text=text_styles.main(backport.text(desc())))]
        return blocks
...
class PopoverTooltip(SimpleCustomizationTooltip):
    ...
    def __packPopoverBlock(cls):
        ...
        blocks = [
         ...
         formatters.packTextBlockData(text=text_styles.main(backport.text(desc(), mapName=backport.text(mapName()))), padding={b'top': 10})]
        return blocks
...
class ElementTooltip(BlocksTooltipData):
    ...
    ALL_SEASON_MAP_ICON = b'all_season'
    HISTORICAL_ICON = b'historical'
    NON_HISTORICAL_ICON = b'non_historical'
    FANTASTICAL_ICON = b'fantastical'
    RENTABLE_ICON = b'rentable'
    EDITABLE_DISABLE_ICON = b'editable_disable'
    EDITED_ICON = b'edited'
    EDITABLE_ICON = b'editable'
    NON_EDITABLE_ICON = b'non_editable'
    PROGRESSION_REWIND_ICON = b'progression_rewind'
    SERIAL_NUMBER_ICON = b'serial_number'
    QUEST_PROGRESSION_ICON = b'questProgression'
    OPENED_ICON = b'opened'
    SPECIAL_EFFECT_ICON = b'special_effect'
    ...
    def _packItemBlocks(self, statsConfig):
        ...
        if statsConfig.buyPrice or statsConfig.sellPrice or statsConfig.inventoryCount:
            ...
            if inventoryBlocks[b'data'][b'blocksData']:
        ...
        if not self._item.requiredToken:
            ...
            if isCompleted or tokenCount >= self._item.descriptor.requiredTokenCount:
                ...
                if not isValid or not isAvailable:
                    ...
                    timeLeft = time_utils.getServerTimeDiffInLocal(time_utils.makeLocalServerTime(questsConfig[self._item.descriptor.requiredTokenCount][b'finishTime']))
    ...
    def _packQuestsBlock(self):
        ...
        def formatCondition(quest, isCompleted, isAvailable, current, total, description):
            if current == total or quest.isCompleted():
                ...
                if isAvailable:
                    value = (b'{} / {}').format(text_styles.neutral(current), total)
                else:
                    value = text_styles.standard((b'{} / {}').format(current, total))
        ...
        if quests:
            ...
            for quest in quests:
                ...
                battleCountCondition = quest.bonusCond.getConditions().find(b'battles')
                battlesCount = None
                ...
                for orItem in postBattleCond:
                    ...
                    for andItem in andItems:
                        ...
                        if andItem.conditionData:
                            description = andItem.conditionData.get(b'data', {}).get(b'description')
                        elif descrData and first(descrData.args):
                            description = CARD_FIELDS_FORMATTERS[descrData.formatterID](*descrData.args)
                        else:
                            description = b''
                            _logger.error(b'Description is not provided for questID: %d', 0)
    ...
    def _packSpecialBlock(self):
        ...
        if not specials:
            ...
            if len(specials) > 1:
                specials = [(b'<li>{}</li>').format(s) for s in specials]
    ...
    def _packRarityHeaderBlock(self):
        ...
        if rarity in Rarity.UI_EFFECT:
            videoSource = (b'videos/rarity/{0}.usm').format(rarity)
        imageOffset = 100
    ...
    def _packInventoryBlock(self, showBuyPrice, showSellPrice, showInventoryCount):
        ...
        if showSellPrice and not (self._item.isHidden or self._item.isRentable):
            for itemPrice in self._item.sellPrices:
                ...
                actionPercent = itemPrice.getActionPrc()
                if actionPercent > 0:
                    subBlocks.append(formatters.packTextParameterWithIconBlockData(name=text_styles.main(TOOLTIPS.ACTIONPRICE_SELL_BODY_SIMPLE), value=text_styles.concatStylesToSingleLine(text_styles.credits(backport.getIntegralFormat(value)), b'    ', icons.credits()), icon=b'alertMedium', valueWidth=88, padding=formatters.packPadding(left=-5)))
                else:
        ...
        if self._item.isQuestsProgression and self._item.itemTypeID == GUI_ITEM_TYPE.STYLE and self._item.descriptor.questsProgression:
            ...
            countText = (b'{} / {}').format(text_styles.stats(itemsOpened), text_styles.main(totalItems))
    ...
    def _packBonusBlock(self, bonus, camo, isApplied):
        ...
        block = formatters.packCustomizationCharacteristicBlockData(text=text_styles.main(text_styles.main(self.bonusDescription)), icon=b'+' + bonusPercent, isTextIcon=True)
        blocks.append(block)
    ...
    def _packProgressBlock(self):
        ...
        if level > self._item.getMaxProgressionLevel():
            ...
            conditions = self._item.progressionConditions.get(level, []).get(b'conditions')
            if not conditions:
                return None
            if unlockedLevel > 0:
            ...
            desc = text_styles.concatStylesToMultiLine(b'', self.__packProgressDescriptionText(conditions[0], showCurrentProgress=showCurrentProgress))
    ...
    def __packInheritorBlock(self, ancestors):
        ...
        blocks = [
         ...
         self.__packDependentContentFormat(text=(b', ').join([backport.text(quotedText(), getItemByCD(intCD).userName) for intCD in ancestors]), icon=backport.image(R.images.gui.maps.icons.customization.customization_icon.c_16x16.camouflage()))]
    ...
    def __packAncestorBlock(self, dependentItems):
        ...
        for key, value in viewitems(itemsBySlot):
            blocks.append(self.__packDependentContentFormat(text=(b', ').join([backport.text(quotedText(), slotTypeName) for slotTypeName in value]), icon=backport.image(image.dyn(key)())))
...
class ElementAwardTooltip(ElementTooltip):
    ...
    def _packBonusBlock(self, bonus, camo, isApplied):
        ...
        bonusPercent = (b'{min:.0f}-{max:.0f}%').format(min=minBonus * 100, max=maxBonus * 100)
