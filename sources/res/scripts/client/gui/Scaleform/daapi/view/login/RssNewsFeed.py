...
class RssNewsFeed(RssNewsFeedMeta):
        ...
        if linkToOpen:
            ...
            if GUI_SETTINGS.loginRssFeed.internalBrowser:
                ...
                if browser is not None:
                    ...
                    _logger.error(b'Attempting to open internal browser, but browseris not exist. External browser will be opened: %r', linkToOpen)
            _logger.debug(b'Open browser: %r', linkToOpen)
    ...
    def __requestFeed(self):
        ...
        if GUI_SETTINGS.loginRssFeed.show:
            ...
            _logger.debug(b'Requesting login RSS news: %s', requestUrl)
        return
    ...
    def __onRssFeedReceived(self, data):
        if self.isDisposed():
            ...
            self.__feed = []
            for entry in reversed(data.get(b'entries', [])):
                data = self.__prepareData(entry)
                if data is not None:
                    self.__feed.append(data)

            _logger.debug(b'RSS feed received, entries count %d', len(self.__feed))
    ...
    def __prepareData(self, entryData):
        description = entryData.get(b'description')
        if description is not None:
            try:
                section = ResMgr.DataSection()
                section.createSectionFromString(description)
                _, section = findFirst((lambda (name, _): name == b'div'), section.items())
                description = section.asWideString
                if len(description) > self.DESCRIPTION_MAX_LENGTH:
                    description = description[:self.DESCRIPTION_CUT_LENGTH] + self.DESCRIPTION_TAIL
            except Exception:
                _logger.exception(b'Invalid RSS entry description: %r, %r', entryData, description)
                return

        return {b'id': (entryData.get(b'id', str(uuid.uuid4()))), b'link': (entryData.get(b'link')), 
