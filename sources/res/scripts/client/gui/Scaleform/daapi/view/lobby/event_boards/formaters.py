...
def formatVehicleNameWithTypeIcon(vehicle, path):
    ...
    key = b'vehicle_prem' if vehicle.isPremium else b'vehicle'
    return makeHtmlString(path, key, {b'msg': ((b'{} {}{}').format(level, icon, vehicle.userName))})
...
def formatVehicleNationAndTypeIcon(vehicle, path):
    ...
    level = int2roman(vehicle.level)
    return makeHtmlString(path, b'vehicle', {b'msg': ((b'{}{}{}').format(iconNation, iconType, level))})
...
def vehicleTypeText(vType):
    return _ms((b'#quests:classes/{}').format(vType))
...
def getClanTag(abbrev, color):
    if isinstance(color, int):
        color = (b'#{}').format(hex(color)[2:].zfill(6))
    if abbrev:
        return (b'<font color="{color}">[{abbrev}]</font>').format(abbrev=abbrev, color=color)
    return b''


def getFullName(name, clanAbbrev, clanColor):
    if clanAbbrev:
        return (b'{name} {clanTag}').format(name=name, clanTag=getClanTag(clanAbbrev, clanColor))
    return name


def getString(value, default=b'--'):
...
def getStatusTitleStyle(text):
    return makeHtmlString(b'html_templates:lobby/elen/status', b'title', {b'msg': text})


def getStatusCountStyle(text):
    return makeHtmlString(b'html_templates:lobby/elen/status', b'count', {b'msg': text})


def timeEndStyle(text):
    return makeHtmlString(b'html_templates:lobby/elen/status', b'time', {b'msg': text})


def formatDate(ts, default=b'--'):
    if ts is not None:
...
def formatTimeAndDate(ts):
    return (b'{0:>s} {1:>s}').format(formatDate(ts), formatTime(ts))
...
_additionalFormatters = {(CALCULATION_METHODS.MAX): {1: {b'valueTime': formatTime}}}
