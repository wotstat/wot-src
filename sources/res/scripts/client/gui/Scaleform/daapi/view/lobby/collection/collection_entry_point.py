...
def isCollectionEntryPointAvailable(data, collectionsController=None):
    return collectionsController.getCollection(data.get(b'collectionId', None)) is not None
...
class CollectionEntryPoint(CollectionEntryPointMeta):
    ...
    def setData(self, data):
        self.__collectionData = {b'collectionId': (data.get(b'collectionId', None)), 
           b'eventEndDate': (getTimestampByStrDate(data[b'collectionEventEndDate']) if b'collectionEventEndDate' in data else 0)}
...
class CollectionEntryPointView(ViewImpl):
    __slots__ = (b'__collectionId', b'__eventEndDate')
    ...
    def setCollectionData(self, collectionData):
        self.__collectionId = collectionData[b'collectionId']
        self.__eventEndDate = collectionData[b'eventEndDate']
    ...
    def _getListeners(self):
        return (
         (
          events.CollectionsEvent.NEW_ITEM_SHOWN, self.__update, EVENT_BUS_SCOPE.LOBBY),)
