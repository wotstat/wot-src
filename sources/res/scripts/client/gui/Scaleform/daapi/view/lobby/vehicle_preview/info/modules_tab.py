    ...
    def _update(self):
        if g_currentPreviewVehicle.isPresent():
            ...
            for device in devices:
                device[b'isDisabledBgVisible'] = modulesEnabled

            self.as_setDataS({b'devices': devices})
        return
...
def _makeStatusText(textRes, iconRes, style):
    return style((b'').join((backport.text(textRes()), _makeStatusIcon(iconRes))))
