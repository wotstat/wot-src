...
class PersonalMissionDetailsContainerView(LobbySubView, PersonalMissionDetailsContainerViewMeta):
    ...
    def __setData(self):
        self.__datailedList = []
        pages = []
        for idx, q in enumerate(sorted(self.__quests.itervalues(), key=methodcaller(b'getID'))):
    ...
    def _onSettingsChanged(self, diff):
        ...
        if self.__branch == PM_BRANCH.REGULAR and b'isRegularQuestEnabled' in diff:
            disabledBranch = not diff[b'isRegularQuestEnabled']
        if self.__branch == PM_BRANCH.PERSONAL_MISSION_2 and b'isPM2QuestEnabled' in diff:
            disabledBranch = not diff[b'isPM2QuestEnabled']
        disabledOp = False
        if b'disabledPMOperations' in diff and diff[b'disabledPMOperations']:
            disabledOp = self.__operationID in diff[b'disabledPMOperations'].keys()
        disabledQuest = False
        if b'disabledPersonalMissions' in diff and diff[b'disabledPersonalMissions']:
            disabledQuest = self.__selectedQuestID in diff[b'disabledPersonalMissions']
