...
class PersonalEntriesPlugin(common.SimplePlugin, IArenaVehiclesController):
    __slots__ = (b'__isAlive', b'__isObserver', b'__playerVehicleID', b'__viewPointID', b'__animationID', b'__deadPointID', b'__cameraID', b'__cameraIDs', b'__yawLimits', b'__circlesID', b'__circlesVisibilityState', b'__killerVehicleID', b'__defaultViewRangeCircleSize', b'__isAwaitingRespawn')
    ...
    def _calcCircularVisionRadius(self):
        ...
        return min(vehAttrs.get(b'circularVisionRadius', visibilityMinRadius), self._arenaVisitor.getVisibilityMaxRadius())
...
class ArenaVehiclesPlugin(common.EntriesPlugin, IVehiclesAndPositionsController):
    ...
    def _setVehicleInfo(self, vehicleID, entry, vInfo, guiProps, isSpotted=False):
        ...
        if classTag is not None:
            ...
            self._invoke(entry.getID(), b'setVehicleInfo', vehicleID, self._getDisplayedVehicleClassTag(vInfo), self._getDisplayedName(vInfo), guiPropsName, animation)
        return
    ...
    def __setDestroyed(self, vehicleID, entry):
        ...
        if self.__showDestroyEntries and entry.setAlive(False) and not entry.wasSpotted():
            ...
            self._invoke(entry.getID(), b'setDead', isPermanent)
        else:
    ...
    def _setInAoI(self, entry, isInAoI):
        if entry.setInAoI(isInAoI):
            self._invoke(entry.getID(), b'setInAoI', isInAoI)
        return
    ...
    def __playSpottedSound(entry):
        ...
        if soundNotifications is not None:
            soundNotifications.play(b'enemy_sighted_for_team', position=Math.Matrix(entry.getMatrix()).translation)
        return
...
class MinimapUpdatersPlugin(common.SimplePlugin):
    __slots__ = (b'__updatersCollection',)
    ...
    def onStateObservation(self, state):
        if state.state != SIGHT_POINTER_STATE.ACTIVE:
            ...
            if self.__entryId is None:
                _logger.error(b'no SightPointer cone entry present while in active mode')
                return
            if angleChanged:
