...
class VehicleAnchorsUpdater(object):
    ...
    def setAnchors(self, displayObjects):
        if self.__vehicleCustomizationAnchors is None:
            _logger.error(b'Missing WGVehicleCustomizationAnchors.')
            return
        else:
            self.__delAllAnchors()
            ...
            for displayObject in displayObjects:
                if not hasattr(displayObject, b'slotData') or displayObject.slotData is None:
                    _logger.error(b'Incorrect anchor displayObject. Missing slotData section. %s', displayObject)
                    continue
