...
class ProfileTabNavigator(ProfileTabNavigatorMeta):
    ...
    def onTabChange(self, tabId):
        self.__safeCall(self.components.get(self.tabId), b'onSectionDeactivated')
        self.tabId = tabId
        self.__safeCall(self.components.get(self.tabId), b'onSectionActivated')
        return

    def onViewReady(self, alias):
        if alias in self.gf_views_map:
            closeCallback = self.__selectedData.get(b'closeCallback') if self.__selectedData else None
