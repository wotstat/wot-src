...
class PointsOfInterestStatsController(PointsOfInterestListener):
    ...
    def __addCapturedPoint(self, poiID, vehicleID, poiState):
        ...
        if item is None:
            if poiState is None:
                _logger.warning(b'PoiStateComponent not found when POI captured')
                return
...
class PoiStatusItem(object):
    ...
    def getVO(self):
        return {b'vehicleID': (self.invader), 
           b'type': (POI_TYPE_UI_MAPPING[self.type]), 
           b'progress': (1 if self.isCaptured else self.progress / 100)}
