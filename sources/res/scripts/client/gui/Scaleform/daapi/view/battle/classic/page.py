...
class DynamicAliases(CONST_CONTAINER):
    ...
    FINISH_SOUND_PLAYER = b'finishSoundPlayer'
    DRONE_MUSIC_PLAYER = b'droneMusicPlayer'
...
class ClassicPage(SharedPage):
    ...
    def __del__(self):
        LOG_DEBUG(b'ClassicPage is deleted')
        return
    ...
    def _handleRadialMenuCmd(self, event):
        self._toggleRadialMenu(isShown=event.ctx[b'isDown'])
        return
    ...
    def _handleToggleFullStats(self, event):
        self._toggleFullStats(event.ctx[b'isDown'], tabAlias=TabsAliases.STATS)
        return

    def _handleToggleFullStatsQuestProgress(self, event):
        self._toggleFullStats(event.ctx[b'isDown'], tabAlias=TabsAliases.QUESTS_PROGRESS)
        return

    def _handleToggleFullStatsPersonalReserves(self, event):
        self._toggleFullStats(event.ctx[b'isDown'], tabAlias=TabsAliases.BOOSTERS)
        return
    ...
    def _getComponentsVideoModeSwitching(self, ctrlMode):
        ...
        if ctrlMode not in CTRL_MODE_NAME.POSTMORTEM_CONTROL_MODES:
            ...
            if vehicle is not None:
                ...
                LOG_ERROR(b'classic/page.py _changeCtrlMode vehicle is None in replay!')
        battleCtx = self.sessionProvider.getCtx()
        if battleCtx.isPlayerObserver():
