...
class ProgressiveStylePopover(CustomizationProgressiveKitPopoverMeta):
    ...
    def __update(self, *_):
        ...
        if self.__style is not None and (not self.__style.isEditable or not self.__style.isQuestsProgression):
            ...
            self.as_setItemsS({b'items': (self.__itemsList)})
            self.__setHeader()
    ...
    def __makeItemDataVO(self, itemData):
        ...
        if itemData.isRemoved or not itemData.isRemovable:
            ...
            countLabel = text_styles.main((b'{} ').format(len(itemData.slotsIds)))
            price = None
        else:
            countLabel = text_styles.main((b'{} x ').format(len(itemData.slotsIds)))
        ...
        itemDataVO = {b'id': (item.intCD), 
           b'icon': icon, 
           b'userName': name, 
           b'numItems': countLabel, 
           b'customizationDisplayType': (item.customizationDisplayType()), 
           b'price': price, 
           b'isApplied': isApplied, 
           b'isWide': (item.isWide()), 
           b'itemsList': (itemData.slotsIds), 
           b'isDim': (item.isDim()), 
           b'isDisabled': (itemData.isRemoved), 
           b'disabledLabel': disabledLabel, 
           b'isRemovable': (itemData.isRemovable), 
           b'seasonType': (itemData.season), 
           b'progressionLevel': progressionLevel, 
           b'isProgressiveLocked': isLocked, 
           b'rarityIcon': rarityIconSource, 
           b'rarityBackground': rarityBackgroundIconSource}
        return itemDataVO

    @staticmethod
    def __getSeasonGroupVO(season):
        seasonName = SEASON_TYPE_TO_NAME[season]
        seasonTitle = makeHtmlString(b'html_templates:lobby/customization/StylePopoverSeasonName', seasonName, ctx={b'align': b'CENTER'})
        seasonGroupVO = {b'isTitle': True, 
           b'titleLabel': seasonTitle}
    ...
    def __hasIsRemovable(self):
        for item in self.__itemsList:
            if item.get(b'isRemovable'):
                return True

        return False

    def __getBaseOutfit(self, season, vehicleCD=b''):
        if season == SeasonType.ALL:
