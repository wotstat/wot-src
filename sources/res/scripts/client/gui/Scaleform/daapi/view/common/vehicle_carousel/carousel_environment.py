...
def formatCountString(currentVehiclesCount, totalVehiclesCount):
    ...
    return (b'{} / {}').format(style(currentVehiclesCount), text_styles.main(totalVehiclesCount))
...
class CarouselEnvironment(CarouselEnvironmentMeta, IGlobalListener, ICarouselEnvironment):
    ...
    _DISABLED_FILTERS = []
    _CAROUSEL_FILTERS = (b'bonus', b'favorite', b'elite', b'premium') + ((b'igr',) if IS_KOREA else ())

    def __init__(self):
        super(CarouselEnvironment, self).__init__()
        self._usedFilters = ()
        self._carouselDPConfig = {b'carouselFilter': None, 
           b'itemsCache': None}
    ...
    def updateHotFilters(self):
        ...
        self.as_setCarouselFilterS({b'hotFilters': hotFilters})
        return
    ...
    def _populate(self):
        ...
        self.applyFilter()
        self.updateAvailability()
        self.as_setInitDataS({b'counterCloseTooltip': (makeTooltip(b'#tooltips:tanksFilter/counter/close/header', b'#tooltips:tanksFilter/counter/close/body'))})
        return
    ...
    def _makeFilterVO(cls, filterID, contexts, filters):
        ...
        return {b'id': filterID, 
           b'value': (getButtonsAssetPath(filterCtx.asset or filterID)), 
           b'selected': (filters[filterID]), 
           b'enabled': True, 
           b'tooltip': (makeTooltip((b'#tank_carousel_filter:tooltip/{}/header').format(filterID), i18n.makeString((b'#tank_carousel_filter:tooltip/{}/body').format(filterID), **filterCtx.ctx)))}
