...
_TabData = namedtuple(b'_TabData', (b'alias', b'linkage', b'tooltip', b'label'))
_TABS_DATA_ORDERED = [
...
class SessionStatsOverview(SessionStatsOverviewMeta):
    ...
    def _showHof(self):
        ...
        g_eventBus.handleEvent(events.LoadViewEvent(SFViewLoadParams(VIEW_ALIAS.LOBBY_PROFILE), ctx={b'hofPageUrl': url}), scope=EVENT_BUS_SCOPE.LOBBY)
        return
    ...
    def __makeInitVO(self):
        ...
        for tabData in _TABS_DATA_ORDERED:
            tabsData.append({b'alias': (tabData.alias), 
               b'linkage': (tabData.linkage), 
               b'selected': (tabData.alias == self._currentTabAlias), 
               b'tooltip': (tabData.tooltip), 
               b'label': (tabData.label)})
        ...
        returnVal = {b'battleCount': battleCnt, 
           b'lastBattleCount': prevBattleCnt, 
           b'title': (self.__getTitle(battleCnt)), 
           b'headerImg': (self.__getHeaderImg()), 
           b'tabs': tabsData, 
           b'isExpanded': isExpanded}
    ...
    def __onServerSettingChanged(self, diff):
        ...
        if b'sessionStats' in diff or (b'sessionStats', b'_r') in diff:
            isSessionStatsEnabled = diff[b'sessionStats'].get(b'isSessionStatsEnabled', False)
            if not isSessionStatsEnabled:
                self.fireEvent(events.HidePopoverEvent(events.HidePopoverEvent.HIDE_POPOVER))
                return
            isLinkWithHoFEnabled = diff[b'sessionStats'].get(b'isLinkWithHoFEnabled', False)
    ...
    def __getButtonStates(self):
        ...
        return [
         {b'btnLabel': label, 
            b'btnTooltip': moreBtnTooltip, 
            b'btnEnabled': isHofBtnUnlocked, 
            b'btnVisible': isHofBtnVisible},
         {b'btnLabel': (backport.text(R.strings.session_stats.resetBtn.label())), 
            b'btnTooltip': clearBtnTooltip, 
            b'btnEnabled': clearBtnEnabled, 
            b'btnVisible': True},
         {b'btnIcon': (RES_ICONS.MAPS_ICONS_MESSENGER_ICONSETTINGS), 
            b'btnTooltip': (makeTooltip(header=backport.text(R.strings.session_stats.tooltip.settingsBtn.header()), body=backport.text(R.strings.session_stats.tooltip.settingsBtn.body()))), 
            b'btnEnabled': True, 
            b'btnVisible': True}]

    def __timeToClearText(self, timeToClear):
        if timeToClear.tm_hour == 0 and timeToClear.tm_min == 0:
            result = b' ' + backport.text(R.strings.tooltips.template.time.lessThenMinute())
        else:
