...
class MapboxProgressionTooltip(BlocksTooltipData):
    ...
    def __packMapsProgressionBlock(self, progressionData):
        ...
        for mapName, mapData in sortedProgressionData:
            ...
            if iconPath == -1:
                _logger.error(b'Invalid icon for map with name %s', mapName)
                icon = b''
            else:
                icon = backport.image(iconPath)
            mapVOs.append({b'icon': icon, 
               b'count': counter, 
    ...
    def __packTotalProgressionBlock(progressionData):
        ...
        items.append(formatters.packBlockDataItem(linkage=BLOCKS_TOOLTIP_TYPES.TOOLTIP_EPIC_PROGRESS_BLOCK_LINKAGE, data={b'value': (min(progress, total)), b'maxValue': total, b'progressColor': progressColor}, blockWidth=330, padding=formatters.packPadding(top=-11, bottom=8, left=20)))
        if progress < total:
