...
I18N_PRICE_KEY = b'{0:>s}/messagePrice'
I18N_TITLE_KEY = b'{0:>s}/title'
I18N_MESSAGE_KEY = b'{0:>s}/message'
I18N_CANCEL_KEY = b'{0:>s}/cancel'
I18N_SUBMIT_KEY = b'{0:>s}/submit'

def _getDialogStr(i18nKey):
    return b'#dialogs:%s' % i18nKey
...
class IDialogMeta(object):
    ...
    def getEventType(self):
        raise NotImplementedError(b'Dialog event type must be specified')
        return

    def getViewScopeType(self):
        raise NotImplementedError(b'Dialog scope must be specified')
        return
...
class ConfirmDialogButtons(InfoDialogButtons):
    ...
    def getLabels(self):
        return [
         {b'id': (DIALOG_BUTTON_ID.SUBMIT), 
            b'label': (self._submit), 
            b'focused': True},
         {b'id': (DIALOG_BUTTON_ID.SUBMIT), 
            b'label': (self._close), 
            b'focused': False}]
...
class I18nInfoDialogButtons(ISimpleDialogButtonsMeta):
    ...
    def __init__(self, i18nKey=b'common', buttonID=DIALOG_BUTTON_ID.CLOSE):
    ...
    def getLabels(self):
        return [
         {b'id': (self._buttonID), 
            b'label': (_getDialogStr(I18N_CANCEL_KEY.format(self._i18nKey))), 
            b'focused': True}]


class I18nConfirmDialogButtons(I18nInfoDialogButtons):

    def __init__(self, i18nKey=b'common', focusedIndex=None):
        super(I18nInfoDialogButtons, self).__init__()
        self._i18nKey = i18nKey
...
class BCConfirmDialogMeta(IDialogMeta):
    ...
    def getLabelExecute(self):
        return self.__meta.get(b'labelExecute', b'')

    def getIcon(self):
        return self.__meta.get(b'icon', b'')

    def getCostValue(self):
        return self.__meta.get(b'costValue', 0)

    def getLabel(self):
        return self.__meta.get(b'label', b'')

    def getIsBuy(self):
        return self.__meta.get(b'isBuy', False)

    def getIsTraining(self):
        return self.__meta.get(b'isTraining', False)

    def getMessage(self):
        return self.__meta.get(b'message', b'')
...
class IconDialogMeta(I18nConfirmDialogMeta):
    ...
    def getIcon(self):
        ...
        if not result:
            result = self._messageCtx.get(b'icon')
        return result
...
class IconPriceDialogMeta(IconDialogMeta):
    ...
    def __init__(self, key, titleCtx=None, messageCtx=None, meta=None, focusedID=None):
        ...
        self._item = messageCtx.get(b'item')
        return
    ...
    def __calcMessagePrice(self):
        ...
        if not result:
            result = self._messageCtx.get(b'price')
        return result
    ...
    def __calcAction(self):
        ...
        if not result:
            result = self._messageCtx.get(b'action')
        return result
...
class HtmlMessageDialogMeta(SimpleDialogMeta):
    ...
    def __init__(self, path, key, ctx=None, scope=ScopeTemplates.VIEW_SCOPE, sourceKey=b'text'):
...
class DisconnectMeta(I18nInfoDialogMeta):
    ...
    def __init__(self, reason=None, kickReasonType=ACCOUNT_KICK_REASONS.UNKNOWN, expiryTime=None):
        super(DisconnectMeta, self).__init__(b'disconnected', scope=ScopeTemplates.GLOBAL_SCOPE)
        self.reason = reason
        self.kickReasonType = kickReasonType
        self.expiryTime = expiryTime
        if hasattr(BigWorld.player(), b'setForcedGuiControlMode'):
    ...
    def getMessage(self):
        formatArgs = {b'reason': b'', b'expiryTime': b''}
        if self.reason:
            formatArgs[b'reason'] = i18n.makeString(DIALOGS.DISCONNECTED_REASON, i18n.makeString(self.reason))
        if self.expiryTime:
            expiryTime = time_utils.makeLocalServerTime(int(self.expiryTime))
            formatArgs[b'expiryTime'] = b'%s %s' % (backport.getLongDateFormat(expiryTime),
...
class I18PunishmentDialogMeta(I18nInfoDialogMeta):
    ...
    def __init__(self, key, titleCtx=None, messageCtx=None, meta=None, scope=ScopeTemplates.VIEW_SCOPE):
        ...
        self.__penaltyType = messageCtx.get(b'penaltyType')
        return

    def getMessage(self):
        msg = self._makeString(b'%s/message/%s' % (self._key, self.__penaltyType), self._messageCtx)
        if self.__penaltyType == b'penalty':
            msg = msg + makeHtmlString(b'html_templates:lobby/battle_results', b'penalty_extra_msg')
        return msg

    def getMsgTitle(self):
        return self._makeString(b'%s/msgTitle/%s' % (self._key, self.__penaltyType), {})
...
class CheckBoxDialogMeta(I18nConfirmDialogMeta):
    ...
    def getButtonsSubmitCancel(self):
        ...
        return {b'submit': (i18n.makeString(submit[b'label'])), b'cancel': (i18n.makeString(cancel[b'label']))}

    def getCheckBoxButtonLabel(self):
        return self._makeString(b'%s/checkBox' % self._key, {})
