...
class EpicBattlesList(TrainingsListBase):
    ...
    def _getViewData(self):
        return {b'title': (backport.text(R.strings.menu.epicTraining.title())), 
    ...
    def joinTrainingRequest(self, prbID):
        yield self.prbDispatcher.join(JoinEpicBattleTrainingCtx(prbID, waitingID=b'prebattle/join'))
        return

    def createTrainingRequest(self):
        settings = EpicTrainingSettingsCtx()
        self.fireEvent(events.LoadViewEvent(SFViewLoadParams(PREBATTLE_ALIASES.EPIC_TRAINING_SETTINGS_WINDOW_PY), ctx={b'isCreateRequest': True, b'settings': settings}), scope=EVENT_BUS_SCOPE.LOBBY)
        return
