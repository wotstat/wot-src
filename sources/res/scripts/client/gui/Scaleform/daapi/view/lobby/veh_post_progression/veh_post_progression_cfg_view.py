...
_HERO_PREVIEW_ALIASES = (
 VIEW_ALIAS.HERO_VEHICLE_PREVIEW, VIEW_ALIAS.RESOURCE_WELL_HERO_VEHICLE_PREVIEW)
...
class VehiclePostProgressionCfgView(VehiclePostProgressionViewMeta):
    ...
    def _onExit(self):
        if (self._exitEvent.alias in _HERO_PREVIEW_ALIASES or self._exitEvent.ctx.get(b'backToHeroTank')) and self._exitEvent.ctx.get(b'itemCD') == self.__heroTanks.getCurrentTankCD():
            self.__goToHeroTank()
        else:
    ...
    def __onResearchAction(self):
        ...
        exitToResearchPage = events.LoadViewEvent(SFViewLoadParams(VIEW_ALIAS.LOBBY_RESEARCH), ctx={(BackButtonContextKeys.ROOT_CD): (self._vehicle.intCD), (BackButtonContextKeys.EXIT): exitToTechTree})
        g_eventBus.handleEvent(exitToResearchPage, scope=EVENT_BUS_SCOPE.LOBBY)
        return

    def __goToHeroTank(self):
        ctx = self._exitEvent.ctx
        shared_events.goToHeroTankOnScene(vehTypeCompDescr=ctx.get(b'itemCD'), previewAlias=ctx.get(b'previewAlias'), previewBackCb=ctx.get(b'previewBackCb'), previousBackAlias=ctx.get(b'previousBackAlias'), hangarVehicleCD=ctx.get(b'hangarVehicleCD'))
        return
