...
def _packColumn(columndID, label, buttonWidth, tooltip, enabled, icon=b'', sortOrder=-1, showSeparator=True, textAlign=b'left'):
    return {b'id': columndID, 
       b'label': (_ms(label)), 
       b'iconSource': icon, 
...
class ClanProfileTableStatisticsView(ClanProfileTableStatisticsViewMeta):
    ...
    def setProxy(self, proxy, clanDossier):
        ...
        data = {b'rendererLinkage': listItemRendererLinkage, 
           b'headers': headers, 
           b'isListVisible': hasProvinces, 
           b'noDataText': (text_styles.highTitle(_ms(CLANS.GLOBALMAPVIEW_NOPROVINCE))), 
           b'isNoDataTextVisible': (not hasProvinces)}
        if hasProvinces:
            data[b'defaultSortField'] = _SORT_IDS.PROVINCE
            data[b'defaultSortDirection'] = b'ascending'
    ...
    def _prepareHeaders(self, showTreasury, enabled):
        ...
        if showTreasury:
            headers.extend([
             _packColumn(_SORT_IDS.PRIMETIME, CLANS.GLOBALMAPVIEW_TABLE_PRIMETIME, 130, CLANS.GLOBALMAPVIEW_TABLE_PRIMETIME_TOOLTIP, enabled, textAlign=b'right'),
             _packColumn(_SORT_IDS.DAYS, CLANS.GLOBALMAPVIEW_TABLE_DAYS, 130, CLANS.GLOBALMAPVIEW_TABLE_DAYS_TOOLTIP, enabled, textAlign=b'right'),
             _packColumn(_SORT_IDS.INCOME, CLANS.GLOBALMAPVIEW_TABLE_INCOME, 118, CLANS.GLOBALMAPVIEW_TABLE_INCOME_TOOLTIP, enabled, textAlign=b'right')])
        else:
            headers.extend([
             _packColumn(_SORT_IDS.PRIMETIME, CLANS.GLOBALMAPVIEW_TABLE_PRIMETIME, 200, CLANS.GLOBALMAPVIEW_TABLE_PRIMETIME_TOOLTIP, enabled, textAlign=b'right'),
             _packColumn(_SORT_IDS.DAYS, CLANS.GLOBALMAPVIEW_TABLE_DAYS, 178, CLANS.GLOBALMAPVIEW_TABLE_DAYS_TOOLTIP, enabled, textAlign=b'right')])
        return headers
...
class _ClanProfileProvinceDataProvider(SortableDAAPIDataProvider):
    ...
    def _makeVO(self, province):
        ...
        result = {b'front': (b'%s %s' % (self.__getFront(province),
                    text_styles.standard(formatField(province.getFrontLevel, formatter=int2roman)))), 
           b'province': (self.__getProvinceName(province)), 
           b'map': (self.__getMap(province)), 
           b'primeTime': (text_styles.main(province.getUserPrimeTime())), 
           b'days': (text_styles.main(backport.getIntegralFormat(self.__getDays(province)))), 
           b'isRobbed': isRobbed}
        if isRobbed:
            restoreTime = province.getPillageEndDatetime()
            result.update({b'robbedTooltip': (makeTooltip(None, text_styles.concatStylesToMultiLine(text_styles.main(_ms(CLANS.GLOBALMAPVIEW_TABLE_PROVINCEROBBED_TOOLTIP_NOINCOME)), text_styles.neutral(_ms(CLANS.GLOBALMAPVIEW_TABLE_PROVINCEROBBED_TOOLTIP_RESTORETIME, date=text_styles.main(formatters.formatShortDateShortTimeString(restoreTime)))))))})
        if self.__showTreasuryData:
            result.update({b'income': (text_styles.gold(backport.getIntegralFormat(self.__getIncome(province)))), 
               b'noIncomeIconVisible': (not province.isHqConnected() or isRobbed), 
               b'noIncomeTooltip': (CLANS.GLOBALMAPVIEW_NOINCOME_TOOLTIP)})
        return result
