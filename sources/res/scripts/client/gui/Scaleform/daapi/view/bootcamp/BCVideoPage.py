import functools, BigWorld, Windowing, SoundGroups, ScaleformFileLoader
...
class BCVideoPage(BCOutroVideoPageMeta):
    ...
    def _populate(self):
        ...
        movies = [(b'/').join((SCALEFORM_SWF_PATH_V3, m[b'video-path'])) for m in self.content[b'messages']]
    ...
    def __showVideo(self):
        if self.content[b'messages']:
            self._message = self.content[b'messages'].pop(0)
            movie = self._message[b'video-path']
            self.soundManager.playInstantSound(self._message[b'event-start'])
            self.as_playVideoS({b'source': movie, 
               b'fitToScreen': (self._message[b'video-fit-to-screen'] == b'true'), 
               b'volume': (self._DEFAULT_MASTER_VOLUME * SoundGroups.g_instance.getMasterVolume())})
            if self.content[b'voiceovers']:
                self._keypoints = [float(point[b'keypoint']) if b'keypoint' in point and point[b'keypoint'] else 0.0 for point in self.content[b'voiceovers'] if point[b'voiceover']]
    ...
    def __onPause(self):
        self.soundManager.playInstantSound(self._message[b'event-pause'])
    ...
    def __onResume(self):
        self.soundManager.playInstantSound(self._message[b'event-resume'])
    ...
    def _onFinish(self):
        self.soundManager.playInstantSound(self._message[b'event-stop'])
        if self.__pauseDelayerID is not None:
            BigWorld.cancelCallback(self.__pauseDelayerID)
            self.__pauseDelayerID = None
        if self.__soundDelayer is not None:
