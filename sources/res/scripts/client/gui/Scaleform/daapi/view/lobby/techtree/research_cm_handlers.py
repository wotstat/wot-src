...
class ResearchVehicleContextMenuHandler(SimpleVehicleCMHandler):
    ...
    def __init__(self, cmProxy, ctx=None):
        super(ResearchVehicleContextMenuHandler, self).__init__(cmProxy, ctx, {(VEHICLE.INFO): b'showVehicleInfo', 
           (VEHICLE.PREVIEW): b'showVehiclePreview', 
    ...
    def _initFlashValues(self, ctx):
        ...
        self._nodeState = int(ctx.nodeState)
        vehicle = self.itemsCache.items.getItemByCD(self._nodeCD)
        self._previewAlias = getattr(ctx, b'previewAlias', WulfPreviewAlias.WULF_TECHTREE)
    ...
    def _generateOptions(self, ctx=None):
        ...
        if vehicle.isPreviewAllowed():
            options.append(self._makeItem(VEHICLE.PREVIEW, MENU.CONTEXTMENU_SHOWVEHICLEPREVIEW, {b'enabled': (self._hangarSpace.spaceInited)}))
...
class BlueprintVehicleContextMenuHandler(SimpleVehicleCMHandler):
    ...
    def _generateOptions(self, ctx=None):
        ...
        if not vehicle.isEarlyAccess:
            options = [self._makeItem(VEHICLE.UNLOCK, MENU.CONTEXTMENU_UNLOCK, {b'enabled': (NODE_STATE.isAvailable2Unlock(self._nodeState))}),
