...
_ButtonState = namedtuple(b'_ButtonState', (b'enabled', b'itemPrice', b'label', b'icon', b'iconAlign', b'isAction', b'actionTooltip', b'tooltip', b'title', b'isMoneyEnough', b'isUnlock', b'isPrevItemsUnlock', b'customOffer', b'isShowSpecial', b'isHeroTankFromBoxes'))
...
class _CouponData(object):
    __slots__ = (b'__item', b'__selected', b'__discount')
...
class VehiclePreviewBottomPanel(VehiclePreviewBottomPanelMeta):
    ...
    def setBundlePreviewMetricsLogger(self, bundlePreviewMetricsLogger):
        if isinstance(bundlePreviewMetricsLogger, ShopBundleVehiclePreviewMetricsLogger):
            ...
            _logger.warning(b'[SHOPUILOG] expected instance of class ShopBundleVehiclePreviewMetricsLogger.')
        return
    ...
    def setPackItems(self, packItems, price, oldPrice, title):
        self.__title = title if title is not None else b''
        self.__price = price
    ...
    def onOfferSelected(self, offerID):
        ...
        if self.__currentOffer:
            ...
            g_eventBus.handleEvent(HasCtxEvent(ctx={b'vehicleItems': [
                               ItemPackEntry(id=vehicle.intCD, groupID=crew.groupID)], 
               b'crewItems': [
                            crew], 
               b'offer': (self.__currentOffer)}, eventType=OFFER_CHANGED_EVENT))
    ...
    def showTooltip(self, intCD, itemType):
        ...
        if itemType == BOX_TYPE:
            toolTipMgr.onCreateComplexTooltip(makeTooltip(TOOLTIPS.VEHICLEPREVIEW_BOXTOOLTIP_HEADER, TOOLTIPS.VEHICLEPREVIEW_BOXTOOLTIP_BODY), b'INFO')
            return
        try:
            try:
                itemId = int(intCD)
    ...
    def _populate(self):
        ...
        g_clientUpdateManager.addCallbacks({b'stats.freeXP': (self.__updateBtnState), 
           b'inventory': (self.__updateBtnState), 
           b'serverSettings.blueprints_config': (self.__onBlueprintsModeChanged)})
    ...
    def __update(self, collapseItems=False):
        if self.__cachedVehiclesVOs:
            g_currentPreviewVehicle.selectVehicle(self.__cachedVehiclesVOs[0][b'intCD'])
            self.as_setSetVehiclesDataS({b'vehicles': (self.__cachedVehiclesVOs)})
        if self.__couponInfo:
        ...
        if collapseItems and self.__cachedCollapsedItemsVOs:
            self.as_setSetItemsDataS({b'items': (self.__cachedCollapsedItemsVOs)})
        elif self.__cachedItemsVOs:
            self.as_setSetItemsDataS({b'items': (self.__cachedItemsVOs)})
        self.__updateBtnState()
        return

    def __getOfferByID(self, offerID):
        return findFirst((lambda o: o.buy_params[b'transactionID'] == offerID), self.__offers)
    ...
    def __getConfirmationDialogKey(self):
        key = b'buyConfirmation'
        if self.__isReferralWindow():
            key = b'referralReward'
        if self.__items:
            key = self.__obtainingInfo.confirmation_key
        return key

    def __buyRequestConfirmation(self, key=b'buyConfirmation'):
        product = self.__title if self.__couponInfo is None else g_currentPreviewVehicle.item.shortUserName
        return DialogsInterface.showDialog(meta=I18nConfirmDialogMeta(key=key, messageCtx={b'product': product, 
           b'price': (formatPrice(self.__getPackPrice(), reverse=True, useIcon=True))}, focusedID=DIALOG_BUTTON_ID.SUBMIT))
    ...
    def __onVehicleLoading(self, ctxEvent):
        ...
        if vehicle is None:
            ...
            if not ctxEvent.ctx.get(b'started') and groupID in self.__styleByGroup:
    ...
    def __updateBtnState(self, *_):
        ...
        if item is None:
            ...
            buyingPanelData.update({b'isReferralEnabled': (self.__isReferralWindow())})
            if not self.__isHeroTankFromBoxes():
                hasExternalLink = (yield self.__hasExternalLink()) if 1 else self._lootBoxesCtrl.getBoxesCount(self._lootBoxesCtrl.mainEntryPoint) == 0
                if hasExternalLink:
                    btnIcon = backport.image(R.images.gui.maps.icons.library.buyInWeb())
                    buyingPanelData.update({b'buyButtonIcon': btnIcon, 
                       b'buyButtonIconAlign': b'right'})
                heroTankUrl = self.__isHeroTank and (self._heroTanks.getCurrentShopUrl()) or (self._heroTanks.getCurrentRelatedURL)()
                if heroTankUrl:
                    buyingPanelData.update({b'isBuyingAvailable': True, 
                       b'itemPrice': None})
    ...
    def __onServerSettingsChanged(self, diff):
        if self._lobbyContext.getServerSettings().isShopDataChangedInDiff(diff, b'isEnabled') or CollectorVehicleConsts.CONFIG_NAME in diff:
            self.__updateBtnState()
        return
    ...
    def __getBtnDataPack(self):
        ...
        if not walletAvailable:
            ...
            buyButtonTooltip = _buildBuyButtonTooltip(b'endTime')
        ...
        if specialData is not None and specialData.buyButtonLabel:
            ...
            buttonLabel = buttonData[b'label']
            enabled = buttonData[b'enabled']
            buttonIcon = buttonData[b'btnIcon']
            buttonIconAlign = buttonData[b'btnIconAlign']
            buyButtonTooltip = buttonData[b'btnTooltip']
            customOffer = buttonData[b'customOffer']
            ...
            price = currentOffer.buyParams[b'priceAmount']
            priceCode = currentOffer.buyParams[b'priceCode']
    ...
    def __getBtnDataUnlockedVehicle(self, vehicle):
        ...
        if minRentPricePackage:
            isAction = minRentPricePackage[b'rentPrice'] != minRentPricePackage[b'defaultRentPrice']
        ...
        if not walletAvailable:
            ...
            if currency == Currency.GOLD:
                ...
                buyButtonTooltip = _buildBuyButtonTooltip(b'notEnoughCredits')
                isMoneyEnough = False
    ...
    def __getBtnDataCollectibleVehicle(self, vehicle):
        ...
        if not isVehicleCollectorEnabled:
            ...
            key = b'notUnlockedNation'
            tooltip = makeTooltip(header=TOOLTIPS.vehiclepreview_buybutton_all(key, b'header'), body=_getCollectibleWarningStr(TOOLTIPS.vehiclepreview_buybutton_all(key, b'body'), vehicle)) if resultVO.isMoneyEnough else resultVO.tooltip
    ...
    def __getBtnDataLockedVehicle(self, vehicle):
        ...
        if not isAvailableToUnlock:
            if not isXpEnough:
                tooltip = _buildBuyButtonTooltip(b'notEnoughXp')
            elif not walletAvailable:
                tooltip = _buildBuyButtonTooltip(b'walletUnavailable')
            elif any(bool(cd in unlocks) for cd in g_techTreeDP.getTopLevel(nodeCD)):
                tooltip = _buildBuyButtonTooltip(b'parentModuleIsLocked')
            else:
                tooltip = _buildBuyButtonTooltip(b'parentVehicleIsLocked')
        specialData = getHeroTankPreviewParams() if self.__isHeroTank else None
        if specialData is not None and specialData.buyButtonLabel:
    ...
    def __getCurrentOfferTitle(self):
        if self.__offers and self.__currentOffer:
            if self.__currentOffer.eventType == b'frontline':
    ...
    def __getCurrentOfferDescription(self):
        if self.__currentOffer and self.__currentOffer.eventType == b'frontline':
            return {b'header': (backport.text(R.strings.vehicle_preview.buyingPanel.offer.rent.frontline.description.header())), 
               b'body': (backport.text(R.strings.vehicle_preview.buyingPanel.offer.rent.frontline.description.body.credits()))}
        else:
            return
    ...
    def __setUsageLeftTime(self, leftTime):
        self.as_updateLeftTimeS(formattedTime=(b'{} {}').format(self.__timeLeftIcon, text_styles.tutorial(time_utils.getTillTimeString(leftTime, MENU.VEHICLEPREVIEW_TIMELEFT))), hasHoursAndMinutes=True)
        return

    def __setShortLeftTime(self, leftTime):
        self.as_updateLeftTimeS(formattedTime=(b'{} {}').format(self.__timeLeftIcon, text_styles.tutorial(time_utils.getTillTimeString(leftTime, MENU.VEHICLEPREVIEW_TIMELEFTSHORT))), hasHoursAndMinutes=True)
        return
    ...
    def __setDateLeftTime(self):
        ...
        self.as_setSetTitleTooltipS(tooltip)
        self.as_updateLeftTimeS(formattedTime=b'')
        return

    def __timeOver(self):
        self.__endTime = None
        self._disableBuyButton = True
        formattedTime = (b'{} {}').format(icons.makeImageTag(RES_ICONS.MAPS_ICONS_LIBRARY_ALERTICON2, vSpace=-2), text_styles.alert(MENU.VEHICLEPREVIEW_ENDTIME))
    ...
    def __purchaseOffer(self):
        ...
        cycles = [r[b'cycle'] for r in rent if r.get(b'cycle')]
        seasons = [r[b'season'] for r in rent if r.get(b'season')]
        showVehicleRentDialog(g_currentPreviewVehicle.item.intCD, RentType.SEASON_CYCLE_RENT if cycles else RentType.SEASON_RENT, cycles if cycles else seasons, GameSeasonType.EPIC if self.__currentOffer.eventType == b'frontline' else None, self.__currentOffer.buyPrice, self.__currentOffer.buyParams)
        return
    ...
    def __updateEnabledState(collection, enabled):
        if collection is None:
            ...
            for item in collection:
                if b'isEnabled' in item:
                    item[b'isEnabled'] = enabled
