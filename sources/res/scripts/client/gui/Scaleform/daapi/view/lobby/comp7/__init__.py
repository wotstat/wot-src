...
def getViewSettings():
    ...
    return (
     ViewSettings(HANGAR_ALIASES.COMP7_PRIME_TIME_ALIAS, Comp7PrimeTimeView, HANGAR_ALIASES.COMP7_PRIME_TIME, WindowLayer.SUB_VIEW, HANGAR_ALIASES.COMP7_PRIME_TIME_ALIAS, ScopeTemplates.LOBBY_SUB_SCOPE, True),
     GroupedViewSettings(HANGAR_ALIASES.COMP7_PUNISHMENT_DIALOG, PunishmentDialog, b'comp7PunishmentDialog.swf', WindowLayer.TOP_WINDOW, b'', None, ScopeTemplates.DYNAMIC_SCOPE, isModal=True, canDrag=False),
     ComponentSettings(HANGAR_ALIASES.COMP7_ENTRY_POINT, Comp7EntryPoint, ScopeTemplates.DEFAULT_SCOPE))


def getBusinessHandlers():
    return (
     Comp7PackageBusinessHandler(), Comp7LobbyDialogsHandler())
...
class Comp7PackageBusinessHandler(PackageBusinessHandler):
    ...
    def __init__(self):
        listeners = (
         (
          HANGAR_ALIASES.COMP7_PRIME_TIME_ALIAS, self.loadViewByCtxEvent),)
...
class Comp7LobbyDialogsHandler(PackageBusinessHandler):
    ...
    def __init__(self):
        listeners = (
         (
          ShowDialogEvent.SHOW_COMP7_PUNISHMENT_DIALOG, self.__punishmentWindowHandler),)
