...
_MODENAME_TO_PO_FILE = {(SELECTOR_BATTLE_TYPES.RANKED): b'ranked_battles', 
   (SELECTOR_BATTLE_TYPES.MAPBOX): b'mapbox', 
   (SELECTOR_BATTLE_TYPES.COMP7): b'comp7'}
...
def packCalendarBlock(modeCtrl, selectedTime, modeName):
    ...
    for serverName in natsorted(serversPeriodsMapping.keys()):
        ...
        blocks.append(__packServerTimeBlock(serverStr=text_styles.main(backport.text(R.strings.dyn(modeName).calendarDay.serverName(), server=serverName)), timeStr=text_styles.stats((b'\n').join(periodsStr))))
...
def _getTimeBlock(isSeasonStarted, tillEnd, timeStamp, modeName, timeStringGetter, leftPadding=0):
    ...
    return formatters.packTextBlockData((b'{} {}').format(text_styles.main(backport.text(tillEndStr() if tillEnd else tillStartStr())), text_styles.stats(timeStringGetter(timeStamp))), padding=formatters.packPadding(left=leftPadding))
