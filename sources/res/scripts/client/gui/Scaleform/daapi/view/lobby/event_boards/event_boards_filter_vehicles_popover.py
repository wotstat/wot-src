...
class EventBoardsFilterVehiclesPopover(EventBoardsResultFilterVehiclesPopoverViewMeta, VehicleSelectorBase):
    ...
    def updateData(self):
        ...
        vehicles = self._updateData(allVehicles, compatiblePredicate=(lambda vo: vo[b'inHangar']))
    ...
    def __initControls(self):
        ...
        self.as_setInitDataS({b'tableHeaders': headers, 
           ...
           b'header': (text_styles.highTitle(_ms(EVENT_BOARDS.POPOVER_TITLE_VEHICLE))), 
           b'okButtonLabel': (_ms(okButtonLabel))})
        return
...
class VehiclesDataProvider(SortableDAAPIDataProvider):
    ...
    def __init__(self):
        ...
        self.__list = []
        self._sort = ((b'level', False), (b'nations', True), (b'type', True))
        return
    ...
    def __selectVehicle(self, vehCD):
        ids = [vo[b'dbID'] for vo in self.__list]
        if ids and vehCD not in ids:
            vehCD = ids[0]
        for vo in self.__list:
            vo[b'selected'] = vo[b'dbID'] == vehCD
