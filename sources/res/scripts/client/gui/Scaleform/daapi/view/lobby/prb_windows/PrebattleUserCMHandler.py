...
KICK_FROM_PREBATTLE = b'kickPlayerFromPrebattle'
...
class PrebattleUserCMHandler(AppealCMHandler, ILegacyListener):
    ...
    def _addPrebattleInfo(self, options, userCMInfo):
        ...
        if self._canKickPlayer():
            options.append(self._makeItem(KICK_FROM_PREBATTLE, MENU.contextmenu(KICK_FROM_PREBATTLE), {b'enabled': (not disabled)}))
        return options

    def _getHandlers(self):
        handlers = super(PrebattleUserCMHandler, self)._getHandlers()
        handlers.update({KICK_FROM_PREBATTLE: b'kickPlayerFromPrebattle'})
        return handlers
    ...
    def _kickPlayerFromPrebattle(self, databaseID):
        ...
        yield self.prbDispatcher.sendPrbRequest(KickPlayerCtx(playerInfo.accID, b'prebattle/kick'))
        return
