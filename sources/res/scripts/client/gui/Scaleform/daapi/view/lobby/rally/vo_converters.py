...
def makeSlotLabel(unitFlags, slotState, isCreator=False, vehCount=0, checkForVehicles=True, isRequired=False, ignoreIfEmpty=False):
    ...
    if slotState.isFree:
        ...
        slotLabel = makeHtmlString(b'html_templates:lobby/cyberSport/unit', template)
    elif slotState.isClosed:
        slotLabel = makeHtmlString(b'html_templates:lobby/cyberSport/unit', SLOT_LABEL.CLOSED)
    return slotLabel
...
def makeBattleRoyaleSlotLabel(slotState):
    slotLabel = SLOT_LABEL.DEFAULT
    if slotState.isFree:
        slotLabel = makeHtmlString(b'html_templates:lobby/cyberSport/unit', SLOT_LABEL.BATTLE_ROYALE)
    return slotLabel
...
def makeVehicleBasicVO(vehicle, levelsRange=None, vehicleTypes=None):
    if vehicle is None:
        ...
        return {b'intCD': (vehicle.intCD), 
           ...
           b'name': (vehicle.name), 
           b'userName': (vehicle.userName), 
           b'shortUserName': (vehicle.shortUserName), 
           b'level': ((vehicle.isOnlyForBattleRoyaleBattles or vehicle).level if 1 else 0), 
           b'type': ((vehicle.isOnlyForBattleRoyaleBattles or vehicle).type if 1 else b''), 
           b'typeIndex': (VEHICLE_TABLE_TYPES_ORDER_INDICES_REVERSED[vehicle.type]), 
           b'smallIconPath': iconPath, 
           b'isReadyToFight': True, 
           b'enabled': enabled, 
           b'tooltip': tooltip, 
           b'state': b'', 
           b'isEventVehicle': False}
...
def makeVehicleVO(vehicle, levelsRange=None, vehicleTypes=None, isCurrentPlayer=True):
    ...
    if vehicleVO is None:
        ...
        if not (vState == Vehicle.VEHICLE_STATE.UNDAMAGED or vState == Vehicle.VEHICLE_STATE.IN_PREBATTLE or vState in Vehicle.GROUP_STATES):
            vehicleVO[b'isReadyToFight'] = vehicle.isReadyToFight
            if vState == Vehicle.VEHICLE_STATE.IN_PREMIUM_IGR_ONLY:
                vehicleVO[b'state'] = makeHtmlString(b'html_templates:lobby', b'inPremiumIgrOnly')
            else:
                vehicleVO[b'state'] = i18n.makeString(b'#menu:tankCarousel/vehicleStates/%s' % vState)
        if not vehicleVO[b'isReadyToFight']:
            vehicleVO[b'enabled'], vehicleVO[b'tooltip'] = False, makeTooltip(b'#tooltips:vehicleStatus/%s/header' % vState, b'#tooltips:vehicleStatus/body')
        return vehicleVO
...
def makeIntroVehicleVO(vehicle, isReadyVehicle, warnTooltip, levelsRange=None, vehicleTypes=None, isCurrentPlayer=True):
    ...
    if introVehicleVO is None:
        ...
        introVehicleVO[b'isReadyVehicle'] = isReadyVehicle
        introVehicleVO[b'warnTooltip'] = warnTooltip
        return introVehicleVO


def makeFiltersVO(nationIDRange, vTypeRange, vLevelRange):
    return {b'nationIDRange': nationIDRange, 
       b'vTypeRange': vTypeRange, 
       b'vLevelRange': vLevelRange}
...
def makeUserVO(user, colorGetter, isPlayerSpeaking=False, lobbyContext=None):
    ...
    if lobbyContext is not None:
        ...
        regionCode = None
    return {b'isInvite': False, 
       b'dbID': (user.getID()), 
       b'accID': (-1), 
       b'isCommander': True, 
       b'userName': (user.getName()), 
       b'fullName': (user.getFullName()), 
       b'clanAbbrev': (user.getClanAbbrev()), 
       b'region': regionCode, 
       b'colors': colors, 
       b'rating': None, 
       b'readyState': False, 
       b'tags': tags, 
       b'isPlayerSpeaking': isPlayerSpeaking, 
       b'isOffline': (not user.isOnline()), 
       b'igrType': 0, 
       b'isRatingAvailable': True}
...
def makePlayerVO(pInfo, user, colorGetter, isPlayerSpeaking=False, isIncludeAccountWTR=False):
    ...
    playerVO = {b'isInvite': (pInfo.isInvite()), 
       b'dbID': (pInfo.dbID), 
       b'accID': (pInfo.accID), 
       b'isCommander': (pInfo.isCommander()), 
       b'userName': (pInfo.name), 
       b'fullName': (pInfo.getFullName()), 
       b'clanAbbrev': (pInfo.clanAbbrev), 
       b'region': (pInfo.getRegion()), 
       b'colors': colors, 
       b'rating': rating, 
       b'readyState': (pInfo.isReady), 
       b'tags': tags, 
       b'isPlayerSpeaking': isPlayerSpeaking, 
       b'isOffline': (pInfo.isOffline()), 
       b'igrType': (pInfo.igrType), 
       b'isRatingAvailable': True, 
       b'badgeVisualVO': badgeVO, 
       b'timeJoin': (pInfo.getTimeJoin()), 
       b'extraData': (pInfo.getExtraData())}
    if isIncludeAccountWTR:
        playerVO[b'accountWTR'] = backport.getIntegralFormat(pInfo.accountWTR) if pInfo.accountWTR else b'-'
    return playerVO
...
def makeSortiePlayerVO(pInfo, user, colorGetter, isPlayerSpeaking=False):
    ...
    sortiePlayerVO[b'isLegionaries'] = True
    sortiePlayerVO[b'clanAbbrev'] = pInfo.clanAbbrev
    sortiePlayerVO[b'isRatingAvailable'] = True
    sortiePlayerVO[b'statusIcon'] = makeCandidateIconPath(pInfo, user)
...
def makeStaticFormationPlayerVO(pInfo, user, colorGetter, isPlayerSpeaking=False):
    ...
    staticFormationPlayerVO[b'isLegionaries'] = pInfo.isLegionary()
    staticFormationPlayerVO[b'isRatingAvailable'] = pInfo.isLegionary() and not pInfo.isInSlot
...
def makeClanBattlePlayerVO(pInfo, user, colorGetter, isPlayerSpeaking=False):
    ...
    clanBattleVO[b'clanAbbrev'] = None
    clanBattleVO[b'isRatingAvailable'] = False
    return clanBattleVO
...
_UNIT_RESTRICTION_TO_LABEL = {(UNIT_RESTRICTION.MAX_TOTAL_LEVEL): b'levelError', 
   (UNIT_RESTRICTION.MIN_TOTAL_LEVEL): b'levelError', 
   (UNIT_RESTRICTION.INVALID_TOTAL_LEVEL): b'levelWarning'}
_ROLE_ICONS = {(UNIT_ROLE.CAN_USE_EXTRA_EQUIPMENTS): b'gunner', 
   (UNIT_ROLE.CAN_USE_BOOST_EQUIPMENTS): b'inspire', 
   (UNIT_ROLE.CAN_USE_EXTRA_EQUIPMENTS | UNIT_ROLE.CAN_USE_BOOST_EQUIPMENTS): b'gunnerAndInspire'}

def makeTotalLevelLabel(unitStats, restriction=b''):
    templateKey = b'sumLevelLabel'
    if restriction:
        templateKey = _UNIT_RESTRICTION_TO_LABEL.get(restriction, templateKey)
    else:
        templateKey = b'levelOk'
    label = makeHtmlString(b'html_templates:lobby/cyberSport/unit', templateKey, {b'sumLevels': (unitStats.curTotalLevel)})
    return label


def makeUnitStateLabel(unitState):
    return makeHtmlString(b'html_templates:lobby/cyberSport', b'teamUnlocked' if unitState.isOpened() else b'teamLocked', {})
...
def _getSlotsData(unitMgrID, fullData, levelsRange=None, checkForVehicles=True, maxPlayerCount=MAX_PLAYER_COUNT_ALL, withPrem=False):
    ...
    for slotInfo in fullData.slotsIterator:
        ...
        slot = {b'rallyIdx': unitMgrID, 
           b'isCommanderState': isPlayerCreator, 
           b'isCurrentUserInSlot': isPlayerInSlot, 
           b'playerStatus': playerStatus, 
           b'isClosed': (slotState.isClosed), 
           b'isFreezed': (unitState.isFreezed()), 
           b'slotLabel': slotLabel, 
           b'player': slotPlayerUI, 
           b'canBeTaken': (canAssign and canTakeSlot), 
           b'compatibleVehiclesCount': vehCount, 
           b'selectedVehicle': vehicleVO, 
           b'selectedVehicleLevel': (1 if slotState.isClosed else slotLevel), 
           b'restrictions': restrictions, 
           b'isEvent': False, 
           b'rating': rating, 
           b'isLegionaries': isLegionaries, 
           b'isLocked': isLocked, 
           b'role': role, 
           b'roleIcon': (_ROLE_ICONS.get(role & equipmentCommanderRoles, b''))}
        if slotInfo.profileVehicle is not None and slotInfo.profileVehicle.prestigeLevel > 0:
            slot[b'prestigeLevel'] = slotInfo.profileVehicle.prestigeLevel
        if withPrem:
            slot[b'hasPremiumAccount'] = player and player.hasPremium
        ...
        if unit.isEvent():
            ...
            additionMsg = b''
            if isVisibleAdtMsg:
                additionMsg = i18n.makeString(MESSENGER.DIALOGS_EVENTSQUAD_VEHICLE, vehName=b'')
            slot.update({b'isVisibleAdtMsg': isVisibleAdtMsg, 
               b'additionalMsg': additionMsg})
...
def _getBalancedSquadInfo(isPlayerCreator, levelsRange, player, unit, vehicle):
    ...
    if isVisibleAdtMsg:
        ...
        additionMsg = b''
    return {b'isVisibleAdtMsg': isVisibleAdtMsg, 
       b'additionalMsg': additionMsg}
...
def _updateSpecialBattleSlotInfo(player, vehicle, message):
    ...
    if vehicle is None:
        ...
        result = {b'isVisibleAdtMsg': isVisibleAdtMsg, 
           b'additionalMsg': additionalMsg}
    return result
...
def _getXPFactorSlotInfo(unit, eventsCache, slotInfo):
    ...
    return {b'isVisibleAdtMsg': isVisibleAdtMsg, b'additionalMsg': additionalMsg, 
       b'slotNotificationIcon': slotNotificationIcon}
...
def makeUnitShortVO(unitEntity, unitMgrID=None, maxPlayerCount=MAX_PLAYER_COUNT_ALL):
    fullData = unitEntity.getUnitFullData(unitMgrID=unitMgrID)
    return {b'isFreezed': (fullData.flags.isLocked()), 
       b'hasRestrictions': (fullData.unit.isRosterSet(ignored=settings.CREATOR_ROSTER_SLOT_INDEXES)), 
       b'slots': (_getSlotsData(unitMgrID, fullData, unitEntity.getRosterSettings().getLevelsRange(), maxPlayerCount=maxPlayerCount)), 
...
def makeSimpleClanListRenderVO(member, intTotalMining, intWeekMining, role, roleID):
    ...
    return {b'dbID': databaseID, 
       b'uid': databaseID, 
       b'himself': (bool(BigWorld.player().databaseID == databaseID)), 
       b'userName': (member.getName()), 
       b'playerRole': role, 
       b'playerRoleID': roleID, 
       b'thisWeek': week, 
       b'allTime': allTime, 
       b'intWeekMining': intWeekMining, 
       b'intTotalMining': intTotalMining, 
       b'fullName': (member.getFullName())}
...
def makeUnitVO(unitEntity, unitMgrID=None, maxPlayerCount=MAX_PLAYER_COUNT_ALL, withPrem=False):
    ...
    return {b'isCommander': isPlayerCreator, 
       ...
       b'slots': (_getSlotsData(unitMgrID, fullData, unitEntity.getRosterSettings().getLevelsRange(), maxPlayerCount=maxPlayerCount, withPrem=withPrem)), 
       b'description': (unitEntity.getCensoredComment(unitMgrID=unitMgrID))}
    ...
    if index is None:
        ...
        for rosterSlot in rosterSlotConditions:
            ...
            if vehTypeCD is not None:
                conditions.append({b'vehicle': (makeVehicleVO(vehicleGetter(vehTypeCD), levelsRange))})
            else:
                params = {b'nationIDRange': (tuple()), 
                   b'vTypeRange': (tuple()), 
                   b'vLevelRange': (tuple())}
                ...
                if not isDefault:
                    if not rosterSlot.isNationMaskFull():
                        ...
                        params[b'nationIDRange'] = [name for name in NATIONS_NAMES if 1 << NATIONS_INDICES[name] & nationMask]
                    if not rosterSlot.isVehClassMaskFull():
                        vehClassMask = rosterSlot.vehClassMask
                        params[b'vTypeRange'] = [vClass for vClass in VEHICLE_CLASSES if 1 << VEHICLE_CLASS_INDICES[vClass] & vehClassMask]
                    levels = rosterSlot.levels
                    if levels != rosterSlot.DEFAULT_LEVELS:
                        params[b'vLevelRange'] = levels
...
def getUnitRosterModel(vehiclesData, conditions, isCreator):
    return {b'vehicles': vehiclesData, 
       b'conditions': conditions, 
       b'isCreator': isCreator}
...
def makeBuildingIndicatorsVO(buildingLevel, progress, hpVal, hpTotalVal, defResVal, maxDefResVal):
    ...
    hpTotalFormatted = str(backport.getIntegralFormat(hpTotalVal)) + b' '
    formattedHpTotal = (b'').join((
    ...
    maxDefDerFormatted = str(backport.getIntegralFormat(maxDefResVal)) + b' '
    formattedDefResTotal = (b'').join((
     text_styles.standard(maxDefDerFormatted), icons.nut()))
    hpProgressLabels = {b'currentValue': (str(backport.getIntegralFormat(hpVal))), b'currentValueFormatter': hpValueFormatter, 
       b'totalValue': formattedHpTotal, 
       b'separator': b'/'}
    storeProgressLabels = {b'currentValue': (str(backport.getIntegralFormat(defResVal))), b'currentValueFormatter': defResValueFormatter, 
       b'totalValue': formattedDefResTotal, 
       b'separator': b'/'}
    result = {b'hpLabel': (i18n.makeString(FORTIFICATIONS.BUILDINGPOPOVER_INDICATORS_HPLBL)), b'defResLabel': (i18n.makeString(FORTIFICATIONS.BUILDINGPOPOVER_INDICATORS_DEFRESLBL)), 
       b'hpCurrentValue': hpVal, 
       b'hpTotalValue': hpTotalVal, 
       b'defResCurrentValue': defResVal, 
       b'defResCompensationValue': (max(0, defResVal - maxDefResVal)), 
       b'defResTotalValue': maxDefResVal, 
       b'hpProgressLabels': hpProgressLabels, 
       b'defResProgressLabels': storeProgressLabels}
    return result


__emptyTypeMap = {SUPPORT_TYPE: b'fireSupportPlus', 
   BOOST_TYPE: b'inspirePlus', 
   REQUISITION_TYPE: b'requisitionPlus', 
   HEAVYTRUCKS_TYPE: b'heavyTrucksPlus'}
...
def makeReserveSlotVO(reserveType, groupType, reserveId, level, slotIndex, tooltip, tooltipType):
    ...
    return {b'slotType': slotType, 
       b'level': level, 
       b'slotIndex': slotIndex, 
       b'id': reserveId, 
       b'tooltipType': tooltipType, 
       b'tooltip': tooltip}
...
__typeMapVO = {ARTILLERY_STRIKE: b'artillerySupport', 
   REQUISITION: b'requisition', 
   HIGH_CAPACITY_TRANSPORT: b'heavyTrucks', 
   INSPIRATION: b'inspire'}
...
def makeReserveModuleData(mID, moduleType, level, count, isSelected, showExtendedParams, paramsItems):
    return {b'id': mID, 
       b'moduleLabel': (getReserveNameVO(moduleType)), 
       b'level': level, 
       b'count': count, 
       b'isSelected': isSelected, 
       b'showExtendedParams': showExtendedParams, 
       b'paramsItems': paramsItems, 
       b'tooltipType': (geReserveTooltipType(moduleType))}
...
def makeFortClanBattleRoomVO(mapId, headerDescr, mineClanName, enemyClanName, waitForBattleDescr, isMapEnabled, isBattleTimerVisible, isSortie):
    ...
    if mapId == 0:
        isMapEnabled = False
    return {b'mapID': mapId, 
       b'mapName': mapName, 
       b'headerDescr': headerDescr, 
       b'mineClanName': mineClanName, 
       b'enemyClanName': enemyClanName, 
       b'waitForBattleDescr': waitForBattleDescr, 
       b'isMapEnabled': isMapEnabled, 
       b'isBattleTimerVisible': timerVisible, 
       b'isBattleType': (not isSortie), 
       b'teamHeader': (i18n.makeString(FORTIFICATIONS.STRONGHOLD_TEAMHEADER))}
...
def makeClanBattleTimerVO(deltaTime, htmlFormatter, alertHtmlFormatter, glowColor, alertGlowColor, timerDefaultValue, hintState):
    return {b'deltaTime': deltaTime, 
       b'htmlFormatter': htmlFormatter, 
       b'alertHtmlFormatter': alertHtmlFormatter, 
       b'glowColor': glowColor, 
       b'alertGlowColor': alertGlowColor, 
       b'timerDefaultValue': timerDefaultValue, 
       b'state': hintState}


__attack = (b'attack1', b'attack2', b'attack3', b'attack4', b'attack5')
__defense = (b'defense1', b'defense2', b'defense3', b'defense4', b'defense5')
...
def makeConfigureButtonVO(isEnabled, isRoomStatusEnabled):
    return {b'isEnabled': isEnabled, 
       b'isRoomStatusEnabled': isRoomStatusEnabled}
...
def makeTableHeaderVO(currentPlayerCount, maxPlayerCount, currentLegCount, maxLegCount):
    ...
    label = total + b' (' + legionariesIcon + b' ' + str(currentLegCount) + b' / ' + str(maxLegCount) + b')'
    return [
     {b'buttonWidth': 212, 
        b'buttonHeight': 40, 
        b'sortOrder': 0, 
        b'toolTip': b'-', 
        b'defaultSortDirection': b'ascending', 
        b'label': label, 
        b'showSeparator': True, 
        b'textAlign': b'left'},
     {b'buttonWidth': 67, 
        b'buttonHeight': 40, 
        b'sortOrder': 0, 
        b'toolTip': b'-', 
        b'defaultSortDirection': b'ascending', 
        b'label': (i18n.makeString(FORTIFICATIONS.STRONGHOLDTABLE_RATING)), 
        b'showSeparator': True},
     {b'buttonWidth': 216, 
        b'buttonHeight': 40, 
        b'sortOrder': 0, 
        b'toolTip': b'-', 
        b'defaultSortDirection': b'ascending', 
        b'label': (i18n.makeString(FORTIFICATIONS.STRONGHOLDTABLE_TECH)), 
        b'showSeparator': False}]
