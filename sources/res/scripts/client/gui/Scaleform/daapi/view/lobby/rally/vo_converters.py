...
def makeVehicleBasicVO(vehicle, levelsRange=None, vehicleTypes=None):
    if vehicle is None:
        ...
        return {b'intCD': (vehicle.intCD), 
           ...
           b'tooltip': tooltip, 
           b'state': b'', 
           b'isEventVehicle': isBattleRoyaleVehicle}
            ...
            if vehTypeCD is not None:
                ...
                if not isDefault:
                    if not rosterSlot.isNationMaskFull():
                        ...
                        params[b'nationIDRange'] = filter((lambda k, mask=nationMask: 1 << NATIONS_INDICES[k] & mask), NATIONS_NAMES)
                    if not rosterSlot.isVehClassMaskFull():
                        vehClassMask = rosterSlot.vehClassMask
                        params[b'vTypeRange'] = filter((lambda k, mask=vehClassMask: 1 << VEHICLE_CLASS_INDICES[k] & mask), VEHICLE_CLASSES)
