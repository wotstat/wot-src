...
class StrongholdConfirmDialogMeta(I18nDialogMeta):
    ...
    def __init__(self, key, subKey=b'', titleCtx=None, messageCtx=None, focusedID=None):
        _key = b'stronghold/{0}{1}'
        buttons = I18nConfirmDialogButtons(_key.format(key, b''), focusedID)
...
class _RallyScopeDialogMeta(I18nDialogMeta):
    ...
    def __init__(self, ctrlType, entityType, meta):
        super(_RallyScopeDialogMeta, self).__init__(b'', None, meta=meta)
    ...
    def getViewScopeType(self):
        if self.__key in _VIEW_SCOPES:
            ...
            LOG_DEBUG(b'View scope is not defined', self.__key)
...
class RallyLeaveDisabledDialogMeta(RallyScopeInfoDialogMeta):
    ...
    def __init__(self, ctrlType, entityType):
        super(RallyLeaveDisabledDialogMeta, self).__init__(ctrlType, entityType, b'leaveDisabled')
        return
...
_ENTITY_TO_ANOTHER_PREFIX = {(_C_TYPE.PREQUEUE, _Q_TYPE.RANDOMS): (b'', b'goToAnother'), 
   (_C_TYPE.PREQUEUE, _Q_TYPE.UNKNOWN): (b'', b'goToAnother'), 
   (_C_TYPE.LEGACY, _P_TYPE.TRAINING): (b'goToIntro', b'goToAnother'), 
   (_C_TYPE.LEGACY, _P_TYPE.CLAN): (b'', b'goToAnother'), 
   (_C_TYPE.LEGACY, _P_TYPE.TOURNAMENT): (b'', b'goToAnother'), 
   (_C_TYPE.UNIT, _P_TYPE.UNIT): (b'goToIntro', b'goToAnother'), 
   (_C_TYPE.UNIT, _P_TYPE.SQUAD): (b'goToSquad', b'goToAnother'), 
   (_C_TYPE.UNIT, _P_TYPE.EVENT): (b'goToSquad', b'goToAnother'), 
   (_C_TYPE.UNIT, _P_TYPE.MAPBOX): (b'goToSquad', b'goToAnother'), 
   (_C_TYPE.UNIT, _P_TYPE.BATTLE_ROYALE): (b'goToSquad', b'goToAnotherSubMode'), 
   (_C_TYPE.PREQUEUE, _Q_TYPE.EPIC): (b'', b'goToAnother')}
_DEFAULT_CONFIRM = b'leave'
...
_INTRO_TO_ANOTHER_PREFIX = {(_C_TYPE.UNIT, _P_TYPE.SQUAD): b'goToSquad', 
   (_C_TYPE.UNIT, _P_TYPE.EVENT): b'goToSquad'}
