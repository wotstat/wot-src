...
_WP_CURRENCY = b'wotPlusVSD'
_SETTINGS_KEY = b'vehicleSellDialog'
_SETTINGS_OPEN_ENTRY = b'isOpened'
_INVENTORY_SHELLS = b'inventoryShells'
_BARRACKS_DROP_DOWN_DATA_PROVIDER = [{b'label': (R.strings.menu.barracks.btnUnload())}, {b'label': (R.strings.menu.barracks.btnDissmiss())}]
...
class VehicleSellDialog(VehicleSellDialogMeta):
    ...
    __slots__ = (b'__vehInvID', b'__vehicle', b'__nationGroupVehicles', b'__controlNumber', b'__enteredControlNumber', b'__income', b'__accountMoney', b'__isCrewDismissal', b'__vehicleSellPrice', b'__items', b'__otherVehicleShells', b'__isDemountKitEnabled')

    def __init__(self, ctx=None):
        super(VehicleSellDialog, self).__init__()
        self.__vehInvID = ctx.get(b'vehInvID', 0)
        self.__vehicle = None
    ...
    def _populate(self):
        ...
        data = {b'accountMoney': (self.__accountMoney.toDict()), 
           b'sellVehicleVO': (self.__getSellVehicleData()), 
           b'modulesInInventory': (self.__prepareInventoryModules()), 
           b'shellsInInventory': (self.__prepareInventoryShells()), 
           b'isSlidingComponentOpened': (_getSlidingComponentOpened()), 
           b'optionalDevicesOnVehicle': optionalDevicesOnVehicle, 
           b'shellsOnVehicle': shellsOnVehicle, 
           b'equipmentsOnVehicle': equipmentsOnVehicle, 
           b'battleBoostersOnVehicle': battleBoostersOnVehicle, 
           b'customizationOnVehicle': customizationOnVehicle}
    ...
    def __getSellVehicleData(self):
        ...
        if self.__vehicle.isElite:
            description = backport.text(R.strings.tooltips.tankCaruselTooltip.vehicleType.elite.dyn(vehType.replace(b'-', b'_'))())
        else:
            description = backport.text(R.strings.dialogs.vehicleSellDialog.vehicleType.dyn(vehType.replace(b'-', b'_'))())
        ...
        return {b'intCD': (self.__vehicle.intCD), 
           b'userName': (self.__vehicle.userName), 
           b'icon': (self.__vehicle.icon), 
           b'level': (self.__vehicle.level), 
           b'isElite': (self.__vehicle.isElite), 
           b'isWotPlus': (self.__vehicle.isWotPlus), 
           b'isPremium': (self.__vehicle.isPremium), 
           b'hasNationGroup': (self.__vehicle.hasNationGroup), 
           b'type': vehType, 
           b'nationID': (self.__vehicle.nationID), 
           b'sellPrice': (self.__vehicleSellPrice.toMoneyTuple()), 
           b'priceTextValue': priceTextValue, 
           b'priceTextColor': priceTextColor, 
           b'currencyIcon': currencyIcon, 
           b'action': vehicleAction, 
           b'hasCrew': hasCrew, 
           b'isRented': (self.__vehicle.isRented), 
           b'description': description, 
           b'roleStr': roleStr, 
           b'priceLabel': (backport.text(R.strings.dialogs.vehicleSellDialog.vehicle.emptySellPrice())), 
           b'crewLabel': (backport.text(R.strings.dialogs.vehicleSellDialog.crew.label())), 
           b'inNationGroupDescription': (backport.text(R.strings.dialogs.vehicleSellDialog.message.multinational())), 
           b'crewTooltip': crewTooltip, 
           b'barracksDropDownData': barracksDropDownData, 
           b'isPostProgressionUnlocked': (self.__vehicle.postProgressionAvailability(unlockOnly=True).result)}
    ...
    def __subscribe(self):
        g_clientUpdateManager.addCallbacks({b'stats.gold': (self.__onSetGoldHandler), 
           b'stats.crystal': (self.__onSetCrystalHandler), 
           b'stats.equipCoin': (self.__onSetEquipCoinHandler), 
           b'stats.vehicleSellsLeft': (self.__onSellLimitUpdate), 
           b'stats.tankmenBerthsCount': (self.__onBerthsCountUpdate), 
           b'goodies': (self.__onSetGoodiesHandler)})
    ...
    def __getControlQuestion(self, usingGold=False):
        ...
        question = makeHtmlString(b'html_templates:lobby/dialogs', b'vehicleSellQuestion', {b'controlNumber': currencyFormatter})
        return question
    ...
    @decorators.adisp_process(b'sellVehicle')
...
class _VSDItemData(object):
    __slots__ = (b'_itemSellPrice', b'_itemRemovalPrice', b'_flashData', b'__fittingItem', b'__itemType')
    ...
    def __init__(self, item, itemType=None):
        ...
        self._flashData = {b'count': 1, 
           b'toInventory': True, 
           b'userName': (item.userName), 
           b'sellPrice': (itemPrice.price.toMoneyTuple())}
        if itemPrice.isActionPrice():
            self._flashData[b'action'] = packItemActionTooltipData(item, False)
        return
    ...
    def setItemID(self, value):
        self._flashData[b'itemID'] = value
        return

    @property
    def itemSellPrice(self):
        return self._itemSellPrice * self._flashData[b'count']
    ...
    def toInventory(self):
        return self._flashData[b'toInventory']

    @toInventory.setter
    def toInventory(self, value):
        self._flashData[b'toInventory'] = value
        return

    @property
    def removeCurrency(self):
        return self._flashData.get(b'removeCurrency', None)

    @removeCurrency.setter
    def removeCurrency(self, value):
        self._flashData[b'removeCurrency'] = value
        return

    @property
    def isRemovableForMoney(self):
        return not self._flashData.get(b'isRemovable', True)
...
class _ShellData(_VSDItemData):
    ...
    def __init__(self, shell, inInventory, onOtherVehicle):
        ...
        self._flashData[b'kind'] = shell.kind
        self._flashData[b'count'] = shell.inventoryCount if inInventory else shell.count
...
class _BoosterData(_VSDItemData):
    ...
    def __init__(self, booster):
        ...
        if not booster.isForSale:
            self._flashData[b'sellPrice'] = MONEY_UNDEFINED.toMoneyTuple()
            self._flashData[b'onlyToInventory'] = True
        return
...
class _CustomizationData(_VSDItemData):
    ...
    def __init__(self, customization, count):
        ...
        self._flashData[b'onlyToInventory'] = customization.isRentable or customization.isHidden or customization.isForbiddenToSell
        self._flashData[b'count'] = count
        return
...
class _ModulesData(_VSDItemData):
    ...
    def __init__(self, module):
        ...
        self._flashData[b'count'] = module.inventoryCount
        return
...
class _OptionalDeviceData(_VSDItemData):
    ...
    def __init__(self, optDevice):
        ...
        self._flashData[b'isRemovable'] = optDevice.isRemovable
        self._flashData[b'isModernized'] = optDevice.isModernized
        self._flashData[b'isWotPlusEnabled'] = isWotPlusEnabled
        self._flashData[b'alertIconDataID'] = None
        ...
        if removalPrice.isActionPrice():
            self._flashData[b'removeActionPrice'] = packActionTooltipData(ACTION_TOOLTIPS_TYPE.ECONOMICS, b'paidRemovalCost', True, removalPrice.price, removalPrice.defPrice)
        ...
        self._flashData[b'removePrice'] = self._itemRemovalPrice.toDict()
    ...
    def setAlertIconTooltip(self, toInventory=True):
        ...
        if not toInventory:
            self._flashData[b'isAlertVisible'] = True
            if self._flashData[b'isModernized']:
                key = b'alertIconDeconstructEnabled' if isOptDeviceRestoreEnabled else b'alertIconDeconstructDisabled'
            else:
                key = b'alertIconSellEnabled' if isOptDeviceRestoreEnabled and not self.isRegular else b'alertIconSellDisabled'
            self._flashData[b'alertIconDataID'] = (b'#tooltips:vehicleSellDialog/renderer/{}').format(key)
        elif self.isRegular and not isSubscriptionEnabled and isFreeEquipmentDemountingEnabled:
            self._flashData[b'isAlertVisible'] = True
            self._flashData[b'alertIconDataID'] = b'#tooltips:vehicleSellDialog/renderer/alertIconRemove'
        else:
            self._flashData[b'isAlertVisible'] = False
        return
