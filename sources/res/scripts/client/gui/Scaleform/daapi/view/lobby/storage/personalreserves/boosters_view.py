...
class StorageCategoryPersonalReservesView(StorageCategoryPersonalReservesViewMeta):
    ...
    def _populate(self):
        ...
        g_clientUpdateManager.addCallbacks({b'goodies': (self.__onUpdateBoosters), 
        ...
        if boosters:
            for booster in sorted(boosters, cmp=self.__sort):
                ...
                if limitResource:
                    ...
                    additionalInfo = b''
                vo = createStorageDefVO(booster.boosterID, text_styles.hightlight(_ms(MENU.BOOSTER_DESCRIPTION_EFFECTVALUETIME, effectValue=booster.getFormattedValue(), effectTime=booster.getEffectTimeStr(hoursOnly=True))), text_styles.main(influence), booster.count, getItemPricesVO(booster.getSellPrice())[0], booster.getShopIcon(STORE_CONSTANTS.ICON_SIZE_SMALL), booster.getShopIcon(), b'altimage', enabled=booster.isReadyToActivate, available=booster.isAvailable, active=booster.state, contextMenuId=None, additionalInfo=additionalInfo)
                if booster.expiryTime:
                    ...
                    expTimeStr = makeHtmlString(b'html_templates:lobby/textStyle/', b'clockWithText', {b'text': (backport.text(R.strings.personal_reserves.booster.useby.timeLeft.dyn(timeKey)(), d=timeLeft))})
