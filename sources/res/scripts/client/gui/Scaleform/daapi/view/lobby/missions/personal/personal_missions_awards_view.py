...
_FRAME_NAME = (b'stugiv', b't28concept', b't55', b'obj260', b'excalibur', b'chimera', b'obj729')
...
class PersonalMissionsAwardsView(LobbySubView, PersonalMissionsAwardsViewMeta, PersonalMissionsNavigation):
    ...
    def __getAwardsVO(self):
        ...
        vo = {b'vehicleAward': (self.__getVehicleAwardVO()), 
           b'additionalAwards': (self.__getAdditionalAwards()), 
           b'mainAwards': (self.__getMainAwards()), 
           b'backBtnLabel': (PERSONAL_MISSIONS.HEADER_BACKBTN_LABEL), 
           b'backBtnDescrLabel': (PERSONAL_MISSIONS.HEADER_BACKBTN_DESCRLABEL_OPERATION), 
           b'bgIconSource': bgIconSource}
        return vo
    ...
    def __getVehicleAwardVO(self):
        ...
        return {b'vehicleSlotData': {b'vehicleId': vehicleId, 
                                b'vehicleImg': (RES_ICONS.getPersonalMissionVehicleImg(vehicleId)), 
                                b'vehicleSmallImg': (RES_ICONS.getPersonalMissionVehicleSmallImg(vehicleId)), 
                                b'pathImg': (RES_ICONS.getPersonalMissionVehiclePathsImg(vehicleId)), 
                                b'pathSmallImg': (RES_ICONS.getPersonalMissionVehiclePathsSmallImg(vehicleId))}, 
           b'slots': slots}
    ...
    def __getVehiclePartSlot(self, tokenType, iconSource, isCollected, canUnlock, unlockCost):
        ...
        slotData = {b'tokenType': tokenType, 
           b'iconSource': iconSource, 
           b'isCollected': isCollected, 
           b'canUnlock': canUnlock, 
           b'allianceIcon': allianceIcon, 
           b'unlockBtnLabel': (makeHtmlString(b'html_templates:lobby/quests/personalMission', b'unlockBtnLabel', {b'branch': (self.getBranch()), b'value': unlockCost})), 
           b'tooltipData': {b'isSpecial': True, 
                            b'specialAlias': (TOOLTIPS_CONSTANTS.PERSONAL_MISSIONS_TANKMODULE), 
                            b'specialArgs': [
                                           self.getOperationID(), tokenType]}}
        return slotData

    def __getAdditionalAwards(self):
        return {b'title': (text_styles.highlightText(PERSONAL_MISSIONS.PERSONALMISSIONAWARDVIEW_BOTTOMTABS_AWARDS_ADDITIONALAWARDS)), 
           b'awards': (self.__addAwardsFormatter.getFormattedBonuses(self.getOperation(), AWARDS_SIZES.BIG))}

    def __getMainAwards(self):
        return {b'title': (text_styles.highlightText(PERSONAL_MISSIONS.AWARDSVIEW_MAINAWARDS_TITLE)), 
           b'awards': (self.__mainAwardsFormatter.getFormattedBonuses(self.getOperation(), AWARDS_SIZES.BIG))}

    def __updateHeader(self):
        vehicle = self.__getVehicleAward()
        operationVO = {b'level': (MENU.levels_roman(vehicle.level)), 
           b'title': (self.getOperation().getShortUserName()), 
           b'description': (PERSONAL_MISSIONS.AWARDSVIEW_DESCRIPTION), 
           b'vehIcon': (getTypeBigIconPath(vehicle.type, vehicle.isElite))}
