import CommandMapping, BigWorld
...
class DynamicAliases(CONST_CONTAINER):
    EPIC_FINISH_SOUND_PLAYER = b'epicFinishSoundPlayer'
    EPIC_DRONE_MUSIC_PLAYER = b'epicDroneMusicPlayer'
...
_EPIC_BATTLE_EXTENDED_COMPONENTS = EPIC_BATTLE_CLASSIC_CONFIG + ComponentsConfig(config=(
 (
 ...
 (
  DynamicAliases.EPIC_FINISH_SOUND_PLAYER, finish_sound_player.EpicFinishSoundPlayer),))
...
_PAGE_STATE_TO_CONTROL_PARAMS = {(PageStates.TABSCREEN, False): (
   ...
   (PageStates.RESPAWN, False): (
                               BATTLE_VIEW_ALIASES.EPIC_RESPAWN_VIEW, True, True), 
...
_STATE_TO_UI = {(PageStates.GAME): _GAME_UI, 
   ...
   (PageStates.COUNTDOWN): (_GAME_UI.difference({
                          ...
                          BATTLE_VIEW_ALIASES.EPIC_MISSIONS_PANEL,
                          BATTLE_VIEW_ALIASES.GAME_MESSAGES_PANEL,
                          BATTLE_VIEW_ALIASES.EPIC_PROGRESSION_CMP}).union({
                          BATTLE_VIEW_ALIASES.PREBATTLE_TIMER,
                          BATTLE_VIEW_ALIASES.PREBATTLE_AMMUNITION_PANEL})), 
   (PageStates.SPECTATOR_DEATHCAM): (_SPECTATOR_UI.union({
                                   BATTLE_VIEW_ALIASES.DAMAGE_PANEL,
                                   BATTLE_VIEW_ALIASES.EPIC_REINFORCEMENT_PANEL,
                                   BATTLE_VIEW_ALIASES.CONSUMABLES_PANEL,
                                   BATTLE_VIEW_ALIASES.SUPER_PLATOON_PANEL})), 
   (PageStates.SPECTATOR_FREE): (_SPECTATOR_UI.union({BATTLE_VIEW_ALIASES.SUPER_PLATOON_PANEL})), 
   (PageStates.SPECTATOR_FOLLOW): (_SPECTATOR_UI.union({
                                 BATTLE_VIEW_ALIASES.DAMAGE_PANEL,
                                 BATTLE_VIEW_ALIASES.RECOVERY_PANEL,
                                 BATTLE_VIEW_ALIASES.STATUS_NOTIFICATIONS_PANEL,
                                 BATTLE_VIEW_ALIASES.GAME_MESSAGES_PANEL})), 
   (PageStates.GAME_OVER): (_GAME_UI.difference({
                          ...
                          BATTLE_VIEW_ALIASES.THERMAL_VISION_INDICATOR,
                          BATTLE_VIEW_ALIASES.DUAL_GUN_PANEL}))}
_EPIC_EXTERNAL_COMPONENTS = (
 crosshair.CrosshairPanelContainer, markers2d.EpicMarkersManager)
...
class EpicBattlePage(EpicBattlePageMeta, BattleGUIKeyHandler):
    ...
    def _handleToggleFullStatsQuestProgress(self, event):
        self._toggleFullStats(event.ctx[b'isDown'])
        return
    ...
    def handleEscKey(self, isDown):
        ...
        if isMapVisible:
            self._setComponentsVisibility(hidden=[BATTLE_VIEW_ALIASES.EPIC_OVERVIEW_MAP_SCREEN,
             BATTLE_VIEW_ALIASES.EPIC_DEPLOYMENT_MAP])
    ...
    def _handleToggleOverviewMap(self, event):
        ...
        if cmdMap.isFired(CommandMapping.CMD_MINIMAP_VISIBLE, event.ctx[b'key']):
            self._toggleOverviewMap()
        return
    ...
    def __onRespawnVisibility(self, isVisible, fromTab=False):
        ...
        elif not isVisible and self.__pageState in (
