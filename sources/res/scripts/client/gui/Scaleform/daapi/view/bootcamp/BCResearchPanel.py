...
class BCResearchPanel(ResearchPanel):
    ...
    @simpleLog(action=BC_LOG_ACTIONS.RESEARCH_BUTTON_PRESSED, logOnce=True, restrictions={b'lesson_id': 2})
    def goToResearch(self):
        if g_currentVehicle.isPresent():
            shared_events.showResearchView(g_currentVehicle.item.intCD)
        else:
            LOG_ERROR(b'Current vehicle is not preset')
        return

    def as_updateCurrentVehicleS(self, data):
        if b'isElite' in data:
            data[b'isElite'] = False
