...
class MissionsTokenPopover(MissionsTokenPopoverMeta):
    ...
    def onBuyBtnClick(self):
        self.fireEvent(events.OpenLinkEvent(events.OpenLinkEvent.TOKEN_SHOP, params={b'name': (self._token.getWebID())}))
        self.destroy()
        return
    ...
    def _populate(self):
        ...
        for quest in sorted(quests.values(), key=missionsSortFunc, reverse=True):
            ...
            result.append({b'headerText': header, 
               b'descrText': descr, 
               b'imgSrc': icon, 
        ...
        if not GUI_SETTINGS.tokenShopURL:
            buyBtnVisible = False
        self.as_setStaticDataS({b'headerText': (text_styles.highTitle(ms(self._token.getUserName()))), 
           b'descrText': (text_styles.main(descrText)), 
           b'imageSrc': (self._token.getImage(TOKEN_SIZES.MEDIUM)), 
           b'buyBtnLabel': (QUESTS.MISSIONS_TOKENPOPOVER_BUYBTN_LABEL), 
           b'buyBtnVisible': buyBtnVisible})
        return
