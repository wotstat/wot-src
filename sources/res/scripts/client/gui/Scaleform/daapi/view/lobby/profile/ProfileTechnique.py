...
class ProfileTechnique(ProfileTechniqueMeta):
    ...
    def __init__(self, *args):
        ...
        self._selectedVehicleIntCD = selectedData.get(b'itemCD') if selectedData else None
    ...
    def showVehiclesRating(self):
        ...
        g_eventBus.handleEvent(events.LoadViewEvent(SFViewLoadParams(VIEW_ALIAS.BROWSER_VIEW), ctx={b'url': (getHofRatingUrlForVehicle(self._selectedVehicleIntCD)), 
           b'returnAlias': (VIEW_ALIAS.LOBBY_PROFILE), 
           b'allowRightClick': True, 
           b'webHandlers': (createHofWebHandlers()), 
           b'itemCD': (self._selectedVehicleIntCD), 
    ...
    def _getInitData(self, accountDossier=None, isFallout=False):
        ...
        initVehicleSorting = self._selectedData.get(b'initVehicleSorting') if self._selectedData else None
        if initVehicleSorting is not None:
            tabData = findFirst((lambda d: d.get(b'id') == initVehicleSorting.get(b'selectedColumnStr')), tabHeaderData)
            if tabData is not None:
                storedData[b'selectedColumn'] = tabHeaderData.index(tabData)
                storedData[b'selectedColumnSorting'] = initVehicleSorting.get(b'selectedColumnSorting', b'descending')
        initData = {b'dropDownProvider': (self._makeBattleTypesDropDown(accountDossier, forVehiclesPage=True)), b'tableHeader': tabHeaderData, 
           b'selectedColumn': (storedData[b'selectedColumn']), 
           b'selectedColumnSorting': (storedData[b'selectedColumnSorting'])}
    ...
    def _setRatingButton(self):
        if self._battlesType == PROFILE_DROPDOWN_KEYS.ALL and self.lobbyContext.getServerSettings().isHofEnabled():
            self.as_setRatingButtonS({b'enabled': True, b'visible': True})
            if isHofButtonNew(PROFILE_CONSTANTS.HOF_VIEW_RATING_BUTTON):
                self.as_setBtnCountersS([{b'componentId': (PROFILE_CONSTANTS.HOF_VIEW_RATING_BUTTON), b'count': b'1'}])
            else:
                self.as_setBtnCountersS([])
        else:
            self.as_setRatingButtonS({b'enabled': False, b'visible': False})
        return
    ...
    def setSelectedTableColumn(self, index, sortDirection):
        ...
        storedData[b'selectedColumn'] = index
        storedData[b'selectedColumnSorting'] = sortDirection
    ...
    def _getDefaultTableHeader(self, isFallout=False, showMarkOfMastery=True):
        ...
        result = [
         self._createTableBtnInfo(b'nationIndex', 36, 0, PROFILE.SECTION_TECHNIQUE_SORT_TOOLTIP_NATION, b'ascending', iconSource=RES_ICONS.MAPS_ICONS_FILTERS_NATIONS_ALL, inverted=True),
         self._createTableBtnInfo(b'typeIndex', 34, 1, PROFILE.SECTION_TECHNIQUE_SORT_TOOLTIP_TECHNIQUE, b'descending', iconSource=RES_ICONS.MAPS_ICONS_FILTERS_TANKS_ALL),
         self._createTableBtnInfo(b'level', 32, 2, PROFILE.SECTION_TECHNIQUE_SORT_TOOLTIP_LVL, b'descending', iconSource=RES_ICONS.MAPS_ICONS_BUTTONS_TAB_SORT_BUTTON_LEVEL),
         self._createTableBtnInfo(b'shortUserName', 154, 7, PROFILE.SECTION_TECHNIQUE_SORT_TOOLTIP_NAME, b'ascending', label=PROFILE.SECTION_TECHNIQUE_BUTTONBAR_VEHICLENAME, inverted=True, sortType=b'string'),
         self._createTableBtnInfo(b'battlesCount', 74, 3, PROFILE.SECTION_TECHNIQUE_SORT_TOOLTIP_BATTLESCOUNT, b'descending', iconSource=RES_ICONS.MAPS_ICONS_FILTERS_BATTLES),
         self._createTableBtnInfo(b'winsEfficiency', 44 if isPrestigeVisible else 74, 4, PROFILE.SECTION_TECHNIQUE_SORT_TOOLTIP_WINS if isFallout else PROFILE.SECTION_TECHNIQUE_SORT_TOOLTIP_WINRATE, b'descending', iconSource=RES_ICONS.MAPS_ICONS_FILTERS_WINS),
         self._createTableBtnInfo(b'avgExperience', 74 if isPrestigeVisible else 90, 5, PROFILE.SECTION_TECHNIQUE_SORT_TOOLTIP_AVGEXP, b'descending', iconSource=RES_ICONS.MAPS_ICONS_FILTERS_AVGEXP)]
        if isPrestigeVisible:
            result.append(self._createTableBtnInfo(b'prestigeLevel', 54, 8, PROFILE.SECTION_TECHNIQUE_SORT_TOOLTIP_PRESTIGELEVEL, b'descending', iconSource=RES_ICONS.MAPS_ICONS_FILTERS_PRESTIGELEVEL))
        if showMarkOfMastery:
            ...
            result.append(self._createTableBtnInfo(b'markOfMastery', 62 if isPrestigeVisible else 70, 6, PROFILE.SECTION_TECHNIQUE_SORT_TOOLTIP_MARKSOFMASTERY, b'descending', iconSource=RES_ICONS.MAPS_ICONS_FILTERS_MARKOFMASTERY, enabled=markOfMasteryEnabled))
        return result

    def _createTableBtnInfo(self, iconId, buttonWidth, sortOrder, toolTip, defaultSortDirection, label=b'', iconSource=b'', inverted=False, sortType=b'numeric', showSeparator=True, enabled=True):
        return {b'id': iconId, 
           ...
           b'ascendingIconSource': (RES_ICONS.MAPS_ICONS_BUTTONS_TAB_SORT_BUTTON_ASCPROFILESORTARROW), 
           b'descendingIconSource': (RES_ICONS.MAPS_ICONS_BUTTONS_TAB_SORT_BUTTON_DESCPROFILESORTARROW), 
           b'buttonHeight': 40, 
           b'enabled': enabled}
    ...
    def _sendAccountData(self, targetData, accountDossier):
        ...
        self.as_responseDossierS(self._battlesType, self._getTechniqueListVehicles(targetData), b'', self._getEmptyScreenLabel())
        return
    ...
    def __getBattleRoyaleTechniqueListVehicles(self, targetData):
        ...
        for vehicleCD, data in vehicles.items():
            ...
            if vehicle is not None:
                result.append({b'id': vehicleCD, 
                   b'inventoryID': (vehicle.invID), 
                   b'shortUserName': (vehicle.shortUserName), 
                   b'battlesCount': battlesCount, 
                   b'avgExperience': avgXP, 
                   b'userName': (vehicle.userName), 
                   b'typeIndex': (VEHICLE_TABLE_TYPES_ORDER_INDICES_REVERSED[vehicle.type]), 
                   b'nationIndex': (GUI_NATIONS_ORDER_INDEX[NAMES[vehicle.nationID]]), 
                   b'nationID': (vehicle.nationID), 
                   b'level': (vehicle.level), 
                   b'tankIconPath': (backport.image(iconResId)), 
                   b'typeIconPath': (b'../maps/icons/filters/tanks/%s.png' % vehicle.type), 
                   b'winsCount': winsCount, 
                   b'avgDamage': avgDamage, 
                   b'avgFrags': avgFrags})
    ...
    def __getBattleRoyaleTableHeader(self):
        return (
         self._createTableBtnInfo(b'nationIndex', 36, 0, PROFILE.SECTION_TECHNIQUE_SORT_TOOLTIP_NATION, b'ascending', iconSource=backport.image(R.images.gui.maps.icons.filters.nations.all()), inverted=True),
         self._createTableBtnInfo(b'typeIndex', 34, 1, PROFILE.SECTION_TECHNIQUE_SORT_TOOLTIP_CLASS, b'descending', iconSource=backport.image(R.images.gui.maps.icons.filters.tanks.all())),
         self._createTableBtnInfo(b'shortUserName', 171, 7, PROFILE.SECTION_TECHNIQUE_SORT_TOOLTIP_NAME, b'ascending', label=backport.text(R.strings.profile.section.technique.buttonBar.vehicleName()), inverted=True, sortType=b'string'),
         self._createTableBtnInfo(b'battlesCount', 64, 2, PROFILE.SECTION_TECHNIQUE_SORT_TOOLTIP_BATTLESCOUNT, b'descending', iconSource=backport.image(R.images.gui.maps.icons.battleRoyale.achievements.battles())),
         self._createTableBtnInfo(b'winsCount', 64, 3, PROFILE.SECTION_TECHNIQUE_SORT_TOOLTIP_WINS, b'descending', iconSource=backport.image(R.images.gui.maps.icons.battleRoyale.achievements.wins())),
         self._createTableBtnInfo(b'avgExperience', 64, 4, PROFILE.SECTION_TECHNIQUE_SORT_TOOLTIP_AVGEXP, b'descending', iconSource=backport.image(R.images.gui.maps.icons.battleRoyale.achievements.avgExp())),
         self._createTableBtnInfo(b'avgDamage', 64, 5, PROFILE.SECTION_TECHNIQUE_SORT_TOOLTIP_AVGDAMAGE, b'descending', iconSource=backport.image(R.images.gui.maps.icons.battleRoyale.achievements.avgDamage())),
         self._createTableBtnInfo(b'avgFrags', 67, 6, PROFILE.SECTION_TECHNIQUE_SORT_TOOLTIP_AVGFRAGS, b'descending', iconSource=backport.image(R.images.gui.maps.icons.battleRoyale.achievements.avgFrags())))
        ...
        for intCD, vehParams in viewitems(targetData.getVehicles()):
            ...
            if vehicle is not None:
                ...
                if self._battlesType == PROFILE_DROPDOWN_KEYS.FALLOUT:
                    ...
                    winsEfficiencyStr = backport.getIntegralFormat(round_py2_style_int(winsEfficiency)) + b'%'
                if showMarkOfMastery:
                ...
                if isPrestigeVisible:
                    ...
                    prestigeLevel = ProfileUtils.UNAVAILABLE_VALUE
                result.append({b'id': intCD, 
                   b'inventoryID': (vehicle.invID), 
                   b'isOtherPlayer': (self._userID is not None), 
                   b'shortUserName': (vehicle.shortUserName), 
                   b'battlesCount': battlesCount, 
                   b'winsEfficiency': winsEfficiency, 
                   b'winsEfficiencyStr': winsEfficiencyStr, 
                   b'avgExperience': avgXP, 
                   b'userName': (vehicle.userName), 
                   b'typeIndex': (VEHICLE_TABLE_TYPES_ORDER_INDICES_REVERSED[vehicle.type]), 
                   b'nationIndex': (GUI_NATIONS_ORDER_INDEX[NAMES[vehicle.nationID]]), 
                   b'nationID': (vehicle.nationID), 
                   b'level': (vehicle.level), 
                   b'markOfMastery': markOfMastery, 
                   b'markOfMasteryBlock': (ACHIEVEMENT_BLOCK.TOTAL), 
                   b'tankIconPath': (vehicle.iconSmall), 
                   b'typeIconPath': (b'../maps/icons/filters/tanks/%s.png' % vehicle.type), 
                   b'isInHangar': isInHangar, 
                   b'compareModeAvailable': (self.comparisonBasket.isEnabled()), 
                   b'prestigePoints': avgPrestigePoints, 
                   b'prestigeLevel': prestigeLevel})
    ...
    def _receiveVehicleDossier(self, vehicleIntCD, databaseId):
        ...
        if not vehDossier:
            ...
            if statsFromSeason:
                ...
                raise SoftException(b'Profile Technique: Unknown battle type: ' + self._battlesType)
            ...
            self.as_responseVehicleDossierS({b'detailedData': preparedStatistics, 
               b'achievements': achievementsList})
            return
