...
class ProfileTechnique(ProfileTechniqueMeta):
    ...
    def _getInitData(self, accountDossier=None, isFallout=False):
        ...
        return {b'dropDownProvider': dropDownProvider, 
           b'tableHeader': (self.__getTableHeader(isFallout)), 
    ...
    def _getDefaultTableHeader(self, isFallout=False):
        result = [
         ...
         self._createTableBtnInfo(b'battlesCount', 74, 3, PROFILE.SECTION_TECHNIQUE_SORT_TOOLTIP_BATTLESCOUNT, b'descending', label=PROFILE.SECTION_SUMMARY_SCORES_TOTALBATTLES),
         self._createTableBtnInfo(b'winsEfficiency', 74, 4, PROFILE.SECTION_TECHNIQUE_SORT_TOOLTIP_WINS if isFallout else PROFILE.SECTION_TECHNIQUE_SORT_TOOLTIP_WINRATE, b'descending', label=PROFILE.SECTION_TECHNIQUE_BUTTONBAR_TOTALWINS),
         self._createTableBtnInfo(b'avgExperience', 90, 5, PROFILE.SECTION_TECHNIQUE_SORT_TOOLTIP_AVGEXP, b'descending', label=PROFILE.SECTION_TECHNIQUE_BUTTONBAR_AVGEXPERIENCE)]
        if isComp7Archive(self._battlesType) or isComp7Season(self._battlesType):
            result.append(self._createTableBtnInfo(b'prestigePoints', 83, 6, PROFILE.SECTION_TECHNIQUE_SORT_TOOLTIP_PRESTIGEPOINTS, b'descending', label=PROFILE.SECTION_TECHNIQUE_BUTTONBAR_PRESTIGEPOINTS))
        else:
            markOfMasteryEnabled = self._battlesType in (PROFILE_DROPDOWN_KEYS.ALL, PROFILE_DROPDOWN_KEYS.EPIC_RANDOM)
            result.append(self._createTableBtnInfo(b'markOfMastery', 83, 6, PROFILE.SECTION_TECHNIQUE_SORT_TOOLTIP_MARKSOFMASTERY, b'descending', label=PROFILE.SECTION_TECHNIQUE_BUTTONBAR_CLASSINESS, enabled=markOfMasteryEnabled))
        return result
    ...
    def _sendAccountData(self, targetData, accountDossier):
        ...
        self.as_responseDossierS(self._battlesType, self._getTechniqueListVehicles(targetData), b'', self.getEmptyScreenLabel())
        return
    ...
    def __getBattleRoyaleTableHeader(self):
        return (
         ...
         self._createTableBtnInfo(b'avgFrags', 80, 6, PROFILE.SECTION_TECHNIQUE_SORT_TOOLTIP_AVGFRAGS, b'descending', iconSource=backport.image(R.images.gui.maps.icons.battleRoyale.achievements.avgFrags())))
        ...
        for intCD, vehParams in targetData.getVehicles().iteritems():
            ...
            if vehicle is not None:
                ...
                if self._battlesType == PROFILE_DROPDOWN_KEYS.FALLOUT:
                    ...
                    winsEfficiencyStr = backport.getIntegralFormat(round(winsEfficiency)) + b'%'
                ...
                result.append({b'id': intCD, 
                   b'inventoryID': (vehicle.invID), 
                   b'isOtherPlayer': (self._userID is None), 
                   b'shortUserName': (vehicle.shortUserName), 
                   b'battlesCount': battlesCount, 
