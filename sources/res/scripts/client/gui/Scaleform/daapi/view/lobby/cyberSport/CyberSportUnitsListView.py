...
class CyberSportUnitsListView(CyberSportUnitsListMeta):
    ...
    def _populate(self):
        ...
        self.as_setHeaderS({b'title': (text_styles.promoTitle(CYBERSPORT.WINDOW_UNITLISTVIEW_TITLE)), 
           b'createBtnLabel': (CYBERSPORT.WINDOW_UNITLISTVIEW_CREATE_BTN), 
           b'createBtnTooltip': None, 
           b'createBtnEnabled': True, 
           b'columnHeaders': (self.__getColumnHeaders())})
    ...
    def _doEnableNavButtons(self, isEnabled):
        self.as_updateNavigationBlockS({b'previousVisible': True, 
           b'previousEnabled': isEnabled, 
           b'nextVisible': True, 
           b'nextEnabled': isEnabled})
        return
    ...
    def __getColumnHeaders(self):
        return [
         self.__createHedader(b'', 82, b'center', RES_ICONS.MAPS_ICONS_STATISTIC_RATING24),
    ...
    def __createHedader(self, label, buttonWidth, position=b'left', iconSource=None):
        return {b'label': label, 
           b'buttonWidth': buttonWidth, 
           b'iconSource': iconSource, 
           b'enabled': False, 
           b'textAlign': position}
    ...
    def __onUnitsListUpdated(self, selectedID, isFullUpdate, isReqInCoolDown, units):
        ...
        if self._searchDP.collection:
            ...
            self.as_setDummyS({b'htmlText': (text_styles.main(CYBERSPORT.WINDOW_UNITLISTVIEW_NOITEMS)), 
               b'alignCenter': True})
        return

    def __setDetails(self, vo):
        linkage = CYBER_SPORT_ALIASES.COMMNAD_DETAILS_LINKAGE_JOIN_TO_NONSTATIC
        self.as_setDetailsS({b'viewLinkage': linkage, 
           b'data': vo})
