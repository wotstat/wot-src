...
class ClanProfileMainWindow(ClanProfileMainWindowMeta, ClanListener):
    ...
    def __init__(self, ctx):
        ...
        self.__clanDBID = ctx[b'clanDbID']
        self.__clanDossier = None
        self.__clanAbbrev = ctx[b'clanAbbrev']
        return
    ...
    def _populate(self):
        ...
        self._updateData()
        self.as_setWindowTitleS((b'').join((i18n.makeString(CLANS.CLANPROFILE_MAINWINDOW_TITLE),
         b' [', self.__clanAbbrev, b']')))
    ...
    def _updateData(self):
        ...
        tabsData = [
         {b'label': (CLANS.CLANPROFILE_MAINWINDOWTAB_SUMMARY), 
            b'linkage': (CLANS_ALIASES.CLAN_PROFILE_SUMMARY_VIEW_LINKAGE)},
         {b'label': (CLANS.CLANPROFILE_MAINWINDOWTAB_PERSONNEL), 
            b'linkage': (CLANS_ALIASES.CLAN_PROFILE_PERSONNEL_VIEW_LINKAGE)},
         {b'label': (CLANS.CLANPROFILE_MAINWINDOWTAB_STRONGHOLDS), 
            b'linkage': (CLANS_ALIASES.CLAN_PROFILE_STRONGHOLDS_VIEW_LINKAGE)}]
        if isGlobalMapEnabled:
            tabsData.append({b'label': (CLANS.CLANPROFILE_MAINWINDOWTAB_GLOBALMAP), 
               b'linkage': (CLANS_ALIASES.CLAN_PROFILE_GLOBALMAP_VIEW_LINKAGE)})
        self.as_setDataS({b'waitingMsg': (backport.msgid(R.strings.waiting.loadingData())), 
           b'tabDataProvider': tabsData})
        return
