...
class TankSetupCMLabel(object):
    DEMOUNT = b'demount'
    DEMOUNT_FROM_SETUP = b'demountFromSetup'
    DEMOUNT_FROM_SETUPS = b'demountFromSetups'
    DESTROY = b'destroy'
    SELECT = b'select'
    PUT_ON_FIRST = b'putOnFirst'
    PUT_ON_SECOND = b'putOnSecond'
    PUT_ON_THIRD = b'putOnThird'
    TAKE_OFF = b'takeOff'
    UNLOAD = b'unload'
    PUT_ON_LIST = (
...
class BaseTankSetupContextMenu(ContextMenu):
    ...
    def _sendSlotAction(self, actionType, **kwargs):
        ...
        if view is None or self._slotType != view.getSelectedSetup():
            return
        actionArgs = {b'actionType': actionType, b'intCD': (kwargs.get(b'intCD', self._intCD)), b'installedSlotId': (kwargs.get(b'installedSlotId', self._installedSlotId))}
        if b'currentSlotId' in kwargs:
            actionArgs[b'currentSlotId'] = kwargs[b'currentSlotId']
