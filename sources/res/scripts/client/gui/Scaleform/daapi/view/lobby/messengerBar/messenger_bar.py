...
def _formatIcon(iconName, width=32, height=32, path=b'html_templates:lobby/messengerBar'):
    return makeHtmlString(path, b'iconTemplate', {b'iconName': iconName, b'width': width, b'height': height})
            ...
            if not cMgr.isViewAvailable(WindowLayer.SUB_VIEW, {(POP_UP_CRITERIA.VIEW_ALIAS): (VIEW_ALIAS.VEHICLE_COMPARE)}):
                ...
                if not vehCmpData.isFromCache():
                    if self.comparisonBasket.getVehiclesCount() == 1:
                        ...
                        vehName = (b'  ').join([int2roman(vehicle.level), vehicle.shortUserName])
                        vehTypeIcon = RES_ICONS.maps_icons_vehicletypes_gold(vehicle.type + b'.png')
                        self.__view.as_showAddVehicleCompareAnimS({b'vehName': vehName, 
                           b'vehType': vehTypeIcon})
    ...
    def __onViewAddedToContainer(self, _, pyEntity):
        if pyEntity.layer == WindowLayer.WINDOW and pyEntity.alias == VEHICLE_COMPARE_CONSTANTS.VEHICLE_COMPARE_CART_POPOVER:
            if self.__currentCartPopover is not None:
                raise SoftException(b'Attempt to initialize object 2nd time!')
...
class MessengerBar(MessengerBarMeta, IGlobalListener):
    ...
    def __updateSessionStatsHint(self, visible):
        self.as_setSessionStatsButtonSettingsUpdateS(visible, b'!')
        return
    ...
    def __onReferralButtonUpdated(self, event):
        isEnabled = event.ctx.get(b'isEnabled', False)
    ...
    def __updateInitData(self):
        ...
        self.as_setInitDataS({b'channelsLocked': isChannelsLocked, 
           b'channelsHtmlIcon': (backport.image(R.images.gui.maps.icons.messenger.iconChannels())), 
           b'isReferralEnabled': (self.__isReferralProgramGUIEnabled()), 
           b'referralCounter': (self._referralCtrl.getBubbleCount()), 
           b'isReferralScoresLimitIndication': (self._referralCtrl.isShouldIndicate()), 
           b'referralHtmlIcon': (backport.image(R.images.gui.maps.icons.messenger.iconReferral())), 
           b'referralTooltip': (TOOLTIPS.LOBY_MESSENGER_REFERRAL_BUTTON), 
           b'contactsHtmlIcon': (backport.image(R.images.gui.maps.icons.messenger.iconContacts())), 
           b'vehicleCompareHtmlIcon': (backport.image(R.images.gui.maps.icons.messenger.iconComparison())), 
           b'contactsTooltip': (TOOLTIPS.LOBY_MESSENGER_CONTACTS_BUTTON), 
           b'channelsTooltipData': channelsTooltipData, 
           b'vehicleCompareTooltip': (TOOLTIPS.LOBY_MESSENGER_VEHICLE_COMPARE_BUTTON), 
           b'sessionStatsHtmlIcon': (backport.image(R.images.gui.maps.icons.messenger.iconSessionStats()))})
        return
    ...
    def __getChannelsLockedTooltipData():
        return {b'tooltipId': (TOOLTIPS_CONSTANTS.NEWBIE_RESTRICTIONS_TOOLTIP), 
           b'isWulfTooltip': True, 
           b'tooltipArgs': None, 
           b'label': None}
