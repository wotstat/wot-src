...
class DemonstratorWindow(DemonstratorWindowMeta, IGlobalListener):
    ...
    __slots__ = (b'__mapListDP', b'__isDemonstratorExpert', b'__gameplaySelected', b'__mapSelected', b'__spawnSelected', b'__levelSelected', b'__vehicleSelected', b'__availableBattleTypes')
    ...
    def onMapSelected(self, index):
        self.__mapSelected = self.__mapListDP.collection[index][b'id']
    ...
    def onBattleStart(self):
        ...
        if dispatcher is not None:
            dispatcher.doAction(PrbAction(b'', mmData=int(mmData)))
            self.onWindowClose()
        return
    ...
    def __onServerSettingChanged(self, diff):
        if b'randomMapsForDemonstrator' in diff or (b'randomMapsForDemonstrator', b'_r') in diff:
            self.__setLevelsList()
            self.__setMapsList()
            self.__disableBattleButton()
        return
    ...
    def __setGameplayTabs(self):
        self.__availableGameplayTypes = [(GAMEPLAY.CTF, self.__settingsCore.getSetting(GAME.GAMEPLAY_CTF)),
         (
          GAMEPLAY.ASSAULT, self.__settingsCore.getSetting(GAME.GAMEPLAY_ASSAULT)),
         (
          GAMEPLAY.DOMINATION, self.__settingsCore.getSetting(GAME.GAMEPLAY_DOMINATION))]
        gameplayTabs = list({b'label': (backport.text(R.strings.arenas.type.dyn(battleType).name())), b'enabled': isEnabled} for battleType, isEnabled in self.__availableGameplayTypes)
    ...
    def __setSpawnsList(self):
        ...
        spawnsList = [{b'label': (backport.text(R.strings.menu.demonstrator.window.any())), b'enabled': isEnabled}, {b'label': (backport.text(R.strings.menu.demonstrator.window.spawn.green())), b'enabled': isEnabled}, {b'label': (backport.text(R.strings.menu.demonstrator.window.spawn.red())), b'enabled': isEnabled}]
    ...
    def __setMapsList(self):
        ...
        self.__availableMapsLength = len(list(ifilter(itemgetter(b'enabled'), self.__mapListDP.collection)))
        self.__setSpawnsList()
        self.__setLevelsList()
        return
    ...
    def __setLevelsList(self):
        ...
        battleLevelsSelector = [
         {b'label': (backport.text(R.strings.menu.demonstrator.window.any())), 
            b'enabled': (self.__availableMapsLength != 0)}]
        for availableLevel, isEnabled in availableLevels:
            levelToRoman = [int2roman(level) for level in availableLevel]
            label = (b', ').join(levelToRoman)
            battleLevelsSelector.append({b'label': label, b'enabled': isEnabled})
    ...
    def __init__(self):
        ...
        for arenaTypeID, arenaType in ArenaType.g_cache.iteritems():
            ...
            if iconRes.isValid():
                ...
                _logger.warning(b'Resource id is not found for area geometry %s', arenaType.geometryName)
                icon = b''
            self.__cache.append({b'id': arenaTypeID, 
               b'name': (arenaType.name), 
               b'gameplayName': (arenaType.gameplayName), 
               b'icon': icon, 
               b'points': (self.__getPointsList(arenaType)), 
               b'enabled': False})

        self.__cache = sorted(self.__cache, key=(lambda x: (x[b'gameplayName'].lower(), x[b'name'].lower())))
        return
    ...
    def __getPointsList(self, arenaType):
        ...
        for team, teamSpawnPoints in enumerate(arenaType.teamSpawnPoints, 1):
            for spawn, spawnPoint in enumerate(teamSpawnPoints, 1):
                ...
                result.append({b'posX': posX, 
                   b'posY': posY, 
                   b'pointType': b'spawn', 
                   b'color': (SPAWNS.BLUE if team == self.__playerTeam else SPAWNS.RED), 
                   b'id': (spawn + 1 if len(teamSpawnPoints) > 1 else 1)})
        ...
        for team, teamBasePoints in enumerate(arenaType.teamBasePositions, 1):
            for baseNumber, basePoint in enumerate(teamBasePoints.values(), 2):
                ...
                result.append({b'posX': posX, 
                   ...
                   b'pointType': b'base', 
                   b'color': (SPAWNS.BLUE if team == self.__playerTeam else SPAWNS.RED), 
                   b'id': (baseNumber + 1 if len(teamBasePoints) > 1 else 1)})
        ...
        if arenaType.controlPoints:
            for index, controlPoint in enumerate(arenaType.controlPoints, 2):
                ...
                result.append({b'posX': posX, 
                   b'posY': posY, 
                   b'pointType': b'control', 
                   b'color': b'empty', 
                   b'id': (index if len(arenaType.controlPoints) > 1 else 1)})
...
class MapListDataProvider(ListDAAPIDataProvider):
    ...
    def buildList(self, currentVehicleLevel=0, gameplayName=None):
        self.clear()
        filteredList = [mapData for mapData in self._cache.cache if mapData[b'gameplayName'] == gameplayName]
        serverSettings = self.__lobbyContext.getServerSettings()
        randomMaps = serverSettings.getRandomMapsForDemonstrator()
        for item in filteredList:
            arenaTypeID = item[b'id']
            geometryID = arenaTypeID & 65535
            gameplayID = arenaTypeID >> 16
            item[b'enabled'] = (geometryID, gameplayID) in randomMaps[currentVehicleLevel]
