...
class BadgesPage(BadgesPageMeta):
    ...
    def __init__(self, ctx=None, *args, **kwargs):
        ...
        self.__backViewName = ctx.get(b'backViewName', b'') if ctx is not None else b''
    ...
    def _populate(self):
        ...
        self.as_setStaticDataS({b'header': {b'backBtnLabel': (backport.text(R.strings.badge.badgesPage.header.backBtn.label())), 
                       b'backBtnDescrLabel': (self.__backViewName), 
                       b'descrTf': (text_styles.main(BADGE.BADGESPAGE_HEADER_DESCR)), 
                       b'playerText': (self.lobbyContext.getPlayerFullName(userName))}})
        self.__updateBadges()
    ...
    def __sentSuffixBadgesVO(self):
        ...
        suffixesVO = {b'checkboxLabel': (backport.text(R.strings.badge.badgesPage.header.suffixSetting.label())), 
           b'checkboxTooltip': (makeTooltip(TOOLTIPS.BADGEINFO_TITLE, TOOLTIPS.BADGEINFO_TEXT)), 
           b'checkboxSelected': (self.badgesController.getSuffix() is not None), 
           b'selectedItemIdx': 0, 
           b'items': []}
        if self.__badgesCollector.getSuffixAchievedBadges():
            ...
            suffixesVO[b'selectedItemIdx'] = self.__selectedItemIdx
            suffixesVO[b'items'] = suffixBadgesVO
    ...
    def __updateBadges(self):
        ...
        if self.badgesController.getPrefix() is not None:
            ...
            self.as_setSelectedBadgeS({b'icon': (backport.image(R.images.gui.maps.icons.library.badges.c_48x48.badge_empty())), 
               b'isDynamic': False}, selected=False)
        ...
        self.as_setReceivedBadgesS({b'badgesData': receivedBadgesVO})
        self.as_setNotReceivedBadgesS({b'title': (text_styles.highTitle(BADGE.BADGESPAGE_BODY_UNCOLLECTED_TITLE)), 
           b'badgesData': notReceivedBadgesVO})
        return
