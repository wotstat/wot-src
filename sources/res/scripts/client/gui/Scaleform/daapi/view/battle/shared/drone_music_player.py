...
class _MusicID(CONST_CONTAINER):
    INTENSIVE = b'wwmusicIntensive'
    RELAXED = b'wwmusicRelaxed'
    STOP = b'wwmusicStop'
...
class _RtpcEvents(CONST_CONTAINER):
    ALLIES_INVADERS_COUNT = b'RTPC_ext_base_capturing_invaders_ally'
    ENEMIES_INVADERS_COUNT = b'RTPC_ext_base_capturing_invaders_enemy'
    ALLIES_BASE_POINTS_CAPTURING = b'RTPC_ext_base_capturing_score_ally'
    ENEMIES_BASE_POINTS_CAPTURING = b'RTPC_ext_base_capturing_score_enemy'
...
class _BaseCaptureCondition(_Condition):
    ...
    def _validatePoints(self):
        ...
        for points in viewvalues(self.__pointsToBase):
            if self._stopCapturingCooldown is not None and points:
                BigWorld.cancelCallback(self._stopCapturingCooldown)
                LOG_DEBUG(b'[Drone] Base Capturing. Cooldown stopped')
                self._stopCapturingCooldown = None
                return False
        ...
        if self.isSatisfied() and self._stopCapturingCooldown is None:
            LOG_DEBUG(b'[Drone] Base Capturing. Stop music cooldown has been started')
    ...
    def _onCooldownOver(self, startTime):
        LOG_DEBUG((b'[Drone] Cooldown ended. {} seconds passed. Music will be stopped.').format(time.time() - startTime))
        self._stopCapturingCooldown = None
...
class DroneMusicPlayer(IBattleFieldListener, IAbstractPeriodView, ITeamBasesListener, IViewComponentsCtrlListener, IArenaLoadController):
    ...
    _SETTING_TO_CONDITION_MAPPING = {b'vehiclesRemained': (
       ...
       b'capturedPoints': (
                         ...
                         (lambda name, key, data: (
                          data[name].getValue(key), data[b'musicStopPredelay'].getValue(key)))), 
       b'hpPercentRemained': (
                            (lambda player: not player.sessionProvider.arenaVisitor.isArenaFogOfWarEnabled()),
                            (
    ...
    def _initializeMusicData(self):
        ...
        if not wwSetup:
            ...
            for tagName in _MusicID.ALL():
                ...
                if musicID is None:
                    LOG_ERROR((b"Music ID for '{}' tag is not defined! Music drone can't be initialized properly!").format(tagName))
    ...
    def _delegateToConditions(self, event, *args, **kwargs):
        for condition in self._conditions:
            ...
            if callable(notifier):
                ...
                LOG_ERROR(b'Listener method not found', condition, event)

        return

    def _launchEvent(self, soundType):
        soundID = self._musicSetup[soundType]
        LOG_DEBUG((b'[Drone] Attempt to launch Drone event "{}"').format(soundID))
    ...
    def _validateConditions(self):
        ...
        if satisfied:
            if not self.__isMusicCurrentlyPlaying():
                ...
                LOG_DEBUG((b'[Drone] Satisfied conditions: {}').format(satisfied))
                self.__playMusic()
    ...
    def __stopMusic(self):
        LOG_DEBUG((b'[Drone] Playing music "{}" has been stopped').format(self.__playingMusicID))
        self._launchEvent(_MusicID.STOP)
        self.__playingMusicID = None
        return

    def __playMusic(self):
        LOG_DEBUG((b'[Drone] Music "{}" has been launched').format(self.__playingMusicID))
