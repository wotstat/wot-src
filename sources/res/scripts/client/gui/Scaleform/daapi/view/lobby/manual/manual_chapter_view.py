...
class ManualChapterView(ManualViewBase, ManualChapterViewMeta):
    BC_HANGAR_GUIDE_CARD_SWIPE_SOUND_ID = b'bc_hangar_guide_card_swipe'
    BC_GUIDE_ELEMENT_BUTTON_SOUND_ID = b'bc_guide_element_button'
    mapsTrainingController = dependency.descriptor(IMapsTrainingController)

    def __init__(self, ctx=None):
        super(ManualChapterView, self).__init__(ctx)
        self.__chapterIndex = self._ctx[b'chapterIndex']
        self._pageIndex = self._ctx.get(b'pageIndex', None)
    ...
    def setData(self, chapterIndex, pageIndex):
        ...
        self.markPageAsRead(int(data[b'details'][pageIndex][b'id']))
        return
    ...
    def openVideo(self, url):
        ...
        ctx = {b'url': url, 
           ...
           b'returnAlias': (VIEW_ALIAS.MANUAL_CHAPTER_VIEW)}
    ...
    def markChapterPagesAsRead(self, chapterUIData):
        ...
        for index, page in enumerate(chapterUIData[b'details']):
            if chapterUIData[b'pages'][index][b'hasNewContent'] and int(page[b'id']) not in allUnReadPages:
                chapterUIData[b'pages'][index][b'hasNewContent'] = False

        return chapterUIData

    def updatePaginator(self):
        data = self.markChapterPagesAsRead(self.chapterData)
        self.as_setPagesS(data[b'pages'])
        return
