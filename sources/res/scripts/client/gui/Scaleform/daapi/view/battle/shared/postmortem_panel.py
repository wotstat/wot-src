...
_POSTMORTEM_PANEL_SETTINGS_PATH = b'gui/postmortem_panel.xml'
_VEHICLE_SMALL_ICON_RES_PATH = b'../maps/icons/vehicle/small/{0}.png'
_BR_VEHICLE_SMALL_ICON_RES_PATH = b'../maps/icons/battleRoyale/vehicles/{0}.png'
_ATTACK_REASON_CODE_TO_MSG = {(ATTACK_REASON_INDICES[b'shot']): b'DEATH_FROM_SHOT', 
   (ATTACK_REASON_INDICES[b'fire']): b'DEATH_FROM_FIRE', 
   (ATTACK_REASON_INDICES[b'ramming']): b'DEATH_FROM_RAMMING', 
   (ATTACK_REASON_INDICES[b'world_collision']): b'DEATH_FROM_WORLD_COLLISION', 
   (ATTACK_REASON_INDICES[b'death_zone']): b'DEATH_FROM_DEATH_ZONE', 
   (ATTACK_REASON_INDICES[b'drowning']): b'DEATH_FROM_DROWNING', 
   (ATTACK_REASON_INDICES[b'overturn']): b'DEATH_FROM_WORLD_COLLISION', 
   (ATTACK_REASON_INDICES[b'artillery_protection']): b'DEATH_FROM_ARTILLERY_PROTECTION', 
   (ATTACK_REASON_INDICES[b'artillery_sector']): b'DEATH_FROM_SECTOR_PROTECTION', 
   (ATTACK_REASON_INDICES[b'bombers']): b'DEATH_FROM_SECTOR_BOMBERS', 
   (ATTACK_REASON_INDICES[b'recovery']): b'DEATH_FROM_RECOVERY', 
   (ATTACK_REASON_INDICES[b'artillery_eq']): b'DEATH_FROM_SHOT', 
   (ATTACK_REASON_INDICES[b'bomber_eq']): b'DEATH_FROM_SHOT', 
   (ATTACK_REASON_INDICES[ATTACK_REASON.MINEFIELD_EQ]): b'DEATH_FROM_MINE_EXPLOSION', 
   (ATTACK_REASON_INDICES[ATTACK_REASON.FIRE_CIRCLE]): b'DEATH_FROM_FIRE_CIRCLE', 
   (ATTACK_REASON_INDICES[ATTACK_REASON.THUNDER_STRIKE]): b'DEATH_FROM_THUNDER_STRIKE', 
...
class _ENTITIES_POSTFIX(object):
    UNKNOWN = b'_UNKNOWN'
    SELF_SUICIDE = b'_SELF_SUICIDE'
    ALLY_SELF = b'_ALLY_SELF'
    ENEMY_SELF = b'_ENEMY_SELF'


class _BasePostmortemPanel(PostmortemPanelMeta):
    __slots__ = (b'_messages', b'_deathInfo')
    ...
    def _prepareMessage(self, code, killerVehID, device=None):
        ...
        if context.isTeamKiller(killerVehID):
            _, colors = self._messages[b'DEATH_FROM_TEAM_KILLER']
    ...
    def _onShowVehicleMessageByCode(self, code, postfix, entityID, extra, equipmentID, ignoreMessages):
        ...
        if equipmentID:
            ...
            extCode = (b'{0}_{1}').format(code, postfix)
...
class _SummaryPostmortemPanel(_BasePostmortemPanel):
    ...
    def _addGameListeners(self):
        ...
        if ctrl is not None:
            ...
            if deathInfo:
                self.__prepareMessageFromSummary(deathInfo[b'killerID'], deathInfo[b'reason'])
        return
    ...
    def __prepareMessageFromSummary(self, vehID, deathReasonCode):
        ...
        if not isInPostmortem:
            ...
            if deathReasonCode not in _ATTACK_REASON_CODE_TO_MSG:
                return
            if deathReasonCode == ATTACK_REASON_INDICES[b'shot']:
                if vehID != 0:
                    self._prepareMessage(_ATTACK_REASON_CODE_TO_MSG[deathReasonCode], killerVehID=vehID)
            elif deathReasonCode == ATTACK_REASON_INDICES[b'fire']:
...
class PostmortemPanel(_SummaryPostmortemPanel):
    __slots__ = (b'__playerInfo', b'_isPlayerVehicle', b'_maxHealth', b'__healthPercent', b'_isInPostmortem', b'_deathAlreadySet', b'__isColorBlind', b'__wasPostmortemInfoHidden')
    ...
    def __init__(self):
        ...
        self.__isColorBlind = self.settingsCore.getSetting(b'isColorBlind')
    ...
    def _addGameListeners(self):
        ...
        if self.__arenaInfo and hasattr(self.__arenaInfo, b'dogTagsInfo') and self._hasBonusCap(ARENA_BONUS_TYPE_CAPS.DOG_TAG):
    ...
    def _preloadDTImages(self, usedDogTagsComponents, skipSameTeam=True):
        ...
        if componentImages:
            _logger.debug(b'PostmortemPanel preloading %s', str(componentImages))
    ...
    def _showOwnDeathInfo(self):
        if self._deathAlreadySet:
            ...
            if deathInfo:
                killerVehID = deathInfo[b'killerVehicle']
                battleCtx = self.sessionProvider.getCtx()
                ...
                self.as_setDeadReasonInfoS(b'', False, None, None, None, None, None)
        return
    ...
    def _makeKillerVO(self, vInfoVO):
        ...
        userVO = {b'userName': (fullName.playerName), 
           b'fakeName': (fullName.playerFakeName), 
           b'clanAbbrev': (playerVO.clanAbbrev), 
           b'region': (fullName.regionCode), 
           b'igrType': (playerVO.igrType), 
           b'tags': (self.__userInfoHelper.getUserTags(playerVO.avatarSessionID, playerVO.igrType))}
        badgeID = vInfoVO.selectedBadge
        badge = buildBadge(badgeID, vInfoVO.getBadgeExtraInfo())
        if badge is not None:
            userVO[b'badgeVisualVO'] = badge.getBadgeVO(ICONS_SIZES.X24, {b'isAtlasSource': True}, shortIconName=True)
        return userVO

    def _makeReasonInfo(self, deathInfo):
        colors = deathInfo[b'colors']
        if self.__isColorBlind:
            color = colors[1]
        else:
            color = colors[0]
        names = {b'device': b'', b'entity': b'', b'killer': b'', b'color': color}
        device = deathInfo[b'device']
        if device:
            names[b'device'] = device
        reason = b''
        try:
            reason = deathInfo[b'text'] % names
        except TypeError:
    ...
    def _showPlayerInfo(self):
        ctx = {b'name': (self.__playerInfo.playerFullName), 
           b'health': (self.__healthPercent)}
        template = b'other'
        msg = makeHtmlString(b'html_templates:battle/postmortemMessages', template, ctx=ctx)
    ...
    def _buildDogTag(dogTag):
        return DisplayableDogTag(PlayerDogTag.fromDict(dogTag), dogTag[b'playerName'], dogTag[b'clanTag'])

    def __onKillerDogTagSet(self, dogTagInfo):
        dogTagModel = layoutComposer.getModel(self._buildDogTag(dogTagInfo[b'dogTag']))
        _logger.info(b'PostmortemPanel.__onKillerDogTagSet: dogTagInfo %s, dogTagModel %s', str(dogTagInfo), str(dogTagModel))
        killCamCtrl = self.sessionProvider.shared.killCamCtrl
    ...
    def __onVictimDogTagSet(self, dogTagInfo):
        dogTagModel = layoutComposer.getModel(self._buildDogTag(dogTagInfo[b'dogTag']))
        _logger.info(b'PostmortemPanel.__onVictimDogTagSet: dogTagInfo %s, dogTagModel %s', str(dogTagInfo), str(dogTagModel))
