...
class ClanPersonalInvitesView(ClanPersonalInvitesViewMeta, ClanListener):
    ...
    def _makeHeaders(self):
        return [
         ...
         self._packHeaderColumnData(b'avgXP', b'', 98, CLANS.CLANPERSONALINVITESWINDOW_TOOLTIPS_TABLE_INVITES_AVGXP, RES_ICONS.MAPS_ICONS_STATISTIC_AVGEXP24, enabled=True),
...
class PersonalInvitesDataProvider(ClanInvitesAbstractDataProvider):
    ...
    def _makeVO(self, item, extraData):
        ...
        outcome = {b'dbID': (item.getDbID()), 
           ...
           b'avgXP': (formatField(getter=item.getBattlesPerformanceAvg, formatter=backport.getIntegralFormat)), 
           ...
           b'enabled': (status == CLAN_INVITE_STATES.ACTIVE or status == CLAN_INVITE_STATES.ERROR), 
