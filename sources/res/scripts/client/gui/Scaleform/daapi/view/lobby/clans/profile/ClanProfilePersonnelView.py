...
class _ClanMembersDataProvider(SortableDAAPIDataProvider, UsersInfoHelper):
    ...
    def _makeVO(self, memberData):
        ...
        return {b'dbID': memberDBID, 
           ...
           b'post': (formatField(getter=memberData.getRoleString)), 
           b'postIcon': (memberData.getRoleIcon()), 
           b'personalRating': (formatField(getter=memberData.getGlobalRating, formatter=backport.getIntegralFormat)), 
           b'battlesCount': (formatField(getter=memberData.getBattlesCount, formatter=backport.getIntegralFormat)), 
           b'wins': (formatField(getter=memberData.getBattlesPerformanceAvg, formatter=(lambda x: backport.getNiceNumberFormat(x) + b'%'))), 
           b'awgExp': (formatField(getter=memberData.getBattleXpAvg, formatter=backport.getIntegralFormat)), 
           b'daysInClan': (formatField(getter=memberData.getDaysInClan, formatter=backport.getIntegralFormat)), 
