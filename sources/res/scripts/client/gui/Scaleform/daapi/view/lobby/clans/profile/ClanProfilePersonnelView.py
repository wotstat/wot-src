...
class _SORT_IDS(object):
    ...
    BATTLES_PERFORMANCE = b'battlesPerformance'
    AWG_XP = b'awgXP'
    DAYS_IN_CLAN = b'daysInClan'


def _packStat(description, tooltip, icon, isEnabled, text):
    return {b'type': (HeaderItemsTypes.COMMON), 
       b'text': text, 
       b'description': (_ms(description)), 
       b'iconPath': (ProfileUtils.getIconPath(icon)), 
       b'tooltip': tooltip, 
       b'enabled': isEnabled}


def _packColumn(columdID, label, buttonWidth, tooltip, icon=b'', sortOrder=-1, showSeparator=True, defaultSortDirection=b'descending'):
    return {b'id': columdID, 
       b'label': (_ms(label)), 
       b'iconSource': icon, 
...
class ClanProfilePersonnelView(ClanProfilePersonnelViewMeta):
    ...
    def setClanDossier(self, clanDossier):
        ...
        statistics = [
         _packStat(CLANS.PERSONNELVIEW_CLANSTATS_AVGPERSONALRATING, CLANS.PERSONNELVIEW_CLANSTATS_AVGPERSONALRATING_TOOLTIP, b'avgPersonalRating40x32.png', *self.__membersDP.getAvgGlobalRating()),
         _packStat(CLANS.PERSONNELVIEW_CLANSTATS_AVGBATTLESCOUNT, CLANS.PERSONNELVIEW_CLANSTATS_AVGBATTLESCOUNT_TOOLTIP, b'avgBattlesCount40x32.png', *self.__membersDP.getAvgBattlesCount()),
         _packStat(CLANS.PERSONNELVIEW_CLANSTATS_AVGWINS, CLANS.PERSONNELVIEW_CLANSTATS_AVGWINS_TOOLTIP, b'avgWins40x32.png', *self.__membersDP.getAvgPerformanceBattles()),
         _packStat(CLANS.PERSONNELVIEW_CLANSTATS_AVGEXP, CLANS.PERSONNELVIEW_CLANSTATS_AVGEXP_TOOLTIP, b'avgExp40x32.png', *self.__membersDP.getAvgXp())]
        headers = [
         _packColumn(_SORT_IDS.USER_NAME, _ms(CLANS.PERSONNELVIEW_TABLE_PLAYER, count=text_styles.stats(str(membersCount)), max=str(MAX_MEMBERS_IN_CLAN)), 223, CLANS.PERSONNELVIEW_TABLE_PLAYER_TOOLTIP, defaultSortDirection=b'ascending'),
         _packColumn(_SORT_IDS.POST, CLANS.PERSONNELVIEW_TABLE_POST, 275, CLANS.PERSONNELVIEW_TABLE_POST_TOOLTIP),
         _packColumn(_SORT_IDS.RATING, b'', 100, CLANS.PERSONNELVIEW_TABLE_PERSONALRATING_TOOLTIP, RES_ICONS.MAPS_ICONS_STATISTIC_RATING24),
         _packColumn(_SORT_IDS.BATTLES_COUNT, b'', 100, CLANS.PERSONNELVIEW_TABLE_BATTLESCOUNT_TOOLTIP, RES_ICONS.MAPS_ICONS_STATISTIC_BATTLES24),
         _packColumn(_SORT_IDS.BATTLES_PERFORMANCE, b'', 100, CLANS.PERSONNELVIEW_TABLE_WINS_TOOLTIP, RES_ICONS.MAPS_ICONS_STATISTIC_WINS24),
         _packColumn(_SORT_IDS.AWG_XP, b'', 100, CLANS.PERSONNELVIEW_TABLE_AVGEXP_TOOLTIP, RES_ICONS.MAPS_ICONS_STATISTIC_AVGEXP24),
         _packColumn(_SORT_IDS.DAYS_IN_CLAN, b'', 100, CLANS.PERSONNELVIEW_TABLE_DAYSINCLAN_TOOLTIP, RES_ICONS.MAPS_ICONS_STATISTIC_DAYSINCLAN24, showSeparator=False)]
        self.as_setDataS({b'title': (text_styles.standard(CLANS.PERSONNELVIEW_TITLE)), 
           b'tableHeaders': headers, 
           b'statistics': statistics, 
           b'defaultSortField': (_SORT_IDS.POST), 
    ...
    def onHeaderButtonClick(self, actionID):
        if actionID == OPEN_CLAN_CHANNEL_ACTION_ID:
            ...
            if channel is not None:
                ...
                LOG_WARNING(b"Clan channel couldn't find!")
...
class _ClanMembersDataProvider(SortableDAAPIDataProvider, UsersInfoHelper):
    ...
    def getAvgGlobalRating(self):
        return _getAvgStringValue(self.__accountsList, b'getGlobalRating', formatter=backport.getIntegralFormat)

    def getAvgBattlesCount(self):
        return _getAvgStringValue(self.__accountsList, b'getBattlesCount', formatter=backport.getIntegralFormat)

    def getAvgPerformanceBattles(self):
        return _getWeighedAvgStringValue(self.__accountsList, b'getBattlesPerformanceAvg', b'getBattlesCount', formatter=(lambda v: backport.getNiceNumberFormat(v) + b'%'))

    def getAvgXp(self):
        return _getWeighedAvgStringValue(self.__accountsList, b'getBattleXpAvg', b'getBattlesCount', formatter=backport.getIntegralFormat)
    ...
    def _makeVO(self, memberData):
        ...
        if contactEntity:
            ...
            userVO = {b'userProps': {b'userName': (self.__getMemberName(memberData))}}
        return {b'dbID': memberDBID, 
           b'userName': (self.__getMemberName(memberData)), 
           b'post': (items.formatField(getter=memberData.getRoleString)), 
           b'postIcon': (memberData.getRoleIcon()), 
           b'personalRating': (items.formatField(getter=memberData.getGlobalRating, formatter=backport.getIntegralFormat)), 
           b'battlesCount': (items.formatField(getter=memberData.getBattlesCount, formatter=backport.getIntegralFormat)), 
           b'wins': (items.formatField(getter=memberData.getBattlesPerformanceAvg, formatter=(lambda x: backport.getNiceNumberFormat(x) + b'%'))), 
           b'avgXP': (items.formatField(getter=memberData.getBattleXpAvg, formatter=backport.getIntegralFormat)), 
           b'daysInClan': (items.formatField(getter=memberData.getDaysInClan, formatter=backport.getIntegralFormat)), 
           b'canShowContextMenu': (memberDBID != getAccountDatabaseID()), 
           b'contactItem': userVO}
