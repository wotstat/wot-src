...
class RegularMissionVehicleSelector(MissionVehicleSelector):
    ...
    def _getTitle(self):
        ...
        count = (b'{} / {}').format(suitableStyle(suitableVehiclesCount), text_styles.main(totalVehiclesCount))
...
class RegularVehicleSelectorCarousel(MissionVehicleSelectorCarousel):
    ...
    def _setCarouselFilters(self):
        ...
        self.as_initCarouselFilterS({b'isVisible': True, 
           b'mainBtn': {b'value': (getButtonsAssetPath(b'params')), 
                        b'tooltip': (TANK_CAROUSEL_FILTER.TOOLTIP_PARAMS)}, 
           b'hotFilters': [
                         {b'value': (QUESTS.QUESTS_TABLE_INHANGAR), 
                            b'tooltip': (TANK_CAROUSEL_FILTER.TOOLTIP_INVENTORY), 
                            b'selected': (filters[b'inventory'])}]})
        return
