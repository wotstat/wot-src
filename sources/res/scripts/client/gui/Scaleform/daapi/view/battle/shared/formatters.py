...
def getHealthPercent(health, maxHealth):
    if not (maxHealth > 0 and maxHealth >= health):
        LOG_WARNING((b'Maximum health is not valid! health={}, maxHealth={}').format(health, maxHealth))
        return 0.0
...
def formatHealthProgress(health, maxHealth):
    if not (maxHealth > 0 and maxHealth >= health):
        LOG_ERROR((b'Maximum health is not valid! health={}, maxHealth={}').format(health, maxHealth))
    return b'%d/%d' % (normalizeHealth(health), maxHealth)
