        ...
        if g_currentPreviewVehicle.isPresent():
            ...
            if item.buyPrices.itemPrice.defPrice.get(Currency.GOLD):
                ...
                if item.isPreferential:
                    ...
                    bonuses.append({b'iconSrc': (backport.image(R.images.gui.maps.shop.kpi.preferential())), 
                       b'labelStr': (text_styles.concatStylesToMultiLine(text_styles.highTitle(backport.text(R.strings.vehicle_preview.infoPanel.preferentialTitle())), text_styles.main(backport.text(text))))})
