...
class _RestoreVehiclesDataProvider(StorageCarouselDataProvider):
    ...
    def _buildVehicle(self, vehicle):
        ...
        vo.update({b'price': (getItemPricesVO(restorePrice)[0]), 
           b'isMoneyEnough': enoughCredits, 
           b'enabled': (enoughCredits and restoreAvailable), 
           b'description': description, 
           b'timerText': timerText, 
           b'timerIcon': timerIcon, 
           b'contextMenuId': (CONTEXT_MENU_HANDLER_TYPE.STORAGE_VEHICLES_RESTORE_ITEM), 
           b'sellButtonLabel': (backport.text(R.strings.storage.buttonLabel.restore()))})
        return vo
