...
TankStatsHeaderVO = namedtuple(b'TankStatsHeaderVO', (b'tankName', b'flagImage', b'tankImage', b'isFavorite', b'favoriteText', b'tankParams', b'smallSize', b'tankType', b'tankTier', b'tankNameSmall', b'isElite'))
TankBattleResultsStatsVO = namedtuple(b'TankBattleResultsStatsVO', (b'lastBattle',))
TankBattleEfficiencyStatsVO = namedtuple(b'TankBattleEfficiencyStatsVO', (b'battleEfficiency', b'collapseLabel'))
TankStatusVO = namedtuple(b'TankStatusVO', b'status')
...
class SessionStatsTankInfo(BlocksTooltipData):
    ...
    def _getBattleEfficiencyData(self):
        ...
        return TankBattleEfficiencyStatsVO(battleEfficiency=packBattleEfficiencyData(self.vehicleStats, parameters), collapseLabel=b'')
    ...
    def _getHeaderBlockData(self):
        vehInfo = self.vehicle.name.split(b':')
        tankImage = R.images.gui.maps.icons.vehicle.c_420x307.dyn(self._tankNameToRClassName(vehInfo[1]))()
        if tankImage == -1:
            _logger.error(b'no background image for tooltip for tank %s', str(self._tankNameToRClassName(vehInfo[1])))
            tankImage = R.images.gui.maps.icons.vehicle.c_420x307.dyn(b'tank_empty')()
        return TankStatsHeaderVO(tankName=text_styles.grandTitle(self.vehicle.userName), isFavorite=self.vehicle.isFavorite, flagImage=backport.image(R.images.gui.maps.icons.flags.c_362x362.dyn(vehInfo[0])()), tankImage=backport.image(tankImage) if tankImage != -1 else b'', tankParams=self._getTankParamsVO(True), favoriteText=text_styles.bonusPreviewText(backport.text(R.strings.session_stats.tankInfo.main())), smallSize=True, tankType=(b'{}_elite').format(self.vehicle.type) if self.vehicle.isElite else self.vehicle.type, tankTier=text_styles.grandTitle(str(int2roman(self.vehicle.level))), tankNameSmall=text_styles.grandTitle(self.vehicle.shortUserName), isElite=self.vehicle.isElite)
    ...
    def _getTankParamsVO(self, smallLayout=False):
        ...
        if not smallLayout:
            ...
            tankItems.append(self._getTankParamVO(text_styles.main(backport.text(R.strings.session_stats.tankInfo.level())), backport.image(R.images.gui.maps.icons.levels.dyn((b'tank_level_{}').format(self.vehicle.level))())))
        markOfMastery = self.randomStats.getMarkOfMasteryForVehicle(self.vehicle.intCD)
        if isMarkOfMasteryAchieved(markOfMastery):
            tankItems.append(self._getTankParamVO(text_styles.main(backport.text(R.strings.achievements.markOfMastery(), name=text_styles.stats(_TANK_MASTER_TYPES_TEXT[markOfMastery - 1]))), backport.image(R.images.gui.maps.icons.library.proficiency.dyn((b'class_icons_{}_small').format(markOfMastery))())))
        ...
        if marksOnGun.getValue() > 0:
            tankItems.append(self._getTankParamVO(text_styles.main(backport.text(R.strings.session_stats.tankInfo.markOnGun(), progress=text_styles.stats((b'{}{}').format(marksOnGun.getDamageRating(), b'%')))), _MARK_ON_GUN_ICONS[marksOnGun.getValue() - 1]))
        else:
            tankItems.append(self._getTankParamVO(text_styles.main(backport.text(R.strings.session_stats.tankInfo.markOnGun.warning(), progress=text_styles.stats((b'{}{}').format(marksOnGun.getDamageRating(), b'%')))), backport.image(R.images.gui.maps.icons.library.mark_on_gun_icon())))
