...
ProgressionLevelIconVO = namedtuple(b'MetaLevelVO', (b'seasonLevel', b'playerLevel', b'bgImageId'))
...
def getFinishBadgeBonuses(questsProgressData, finishQuestID, eventsCache=None, itemsCache=None):
    ...
    if finishQuest is None:
        ...
        if all(b.isAchieved for b in chain.from_iterable(d.getBadges() for d in finishQuest.getBonuses(b'dossier'))):
            return finishQuest.getBonuses()
...
def getProgressionIconVO(cycleNumber, playerLevel, epicController=None):
    ...
    return ProgressionLevelIconVO(b'' if season and season.isSingleCycleSeason() else int2roman(cycleNumber), playerLevelStr, _getProgressionIconBackgroundId(playerLevel))
