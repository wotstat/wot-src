...
class ClassicMinimapComponent(component.MinimapComponent):
    ...
    def _setupPlugins(self, arenaVisitor):
        ...
        setup[b'points'] = TeamsOrControlsPointsPlugin
        if not BattleReplay.g_replayCtrl.isPlaying:
            setup[b'pinging'] = ClassicMinimapPingPlugin
        if IS_DEVELOPMENT:
...
class GlobalSettingsPlugin(common.SimplePlugin):
    __slots__ = (b'_currentSizeSettings', b'__isVisible', b'_sizeIndex', b'__canChangeAlpha')
    _AccountSettingsClass = AccountSettings

    def __init__(self, parentObj):
        super(GlobalSettingsPlugin, self).__init__(parentObj)
        self._currentSizeSettings = b'minimapSize'
    ...
    def applyNewSize(self, sizeIndex):
        LOG_DEBUG(b'Size index of minimap is changed', sizeIndex)
    ...
    def __setSizeByStep(self, step):
        ...
        if self._sizeIndex != newIndex:
            LOG_DEBUG(b'Try to change size index of minimap by step', newIndex)
    ...
    def __handleMinimapCmd(self, event):
        self.__handleKey(event.ctx[b'key'])
        return
...
class TeamsOrControlsPointsPlugin(common.EntriesPlugin):
    __slots__ = (b'_personalTeam', b'_entriesStorage', b'_markerIDs', b'_hasActiveCommit')
    ...
    def _onRemoveCommandReceived(self, removeID, markerType):
        ...
        if removeID in self._markerIDs:
            ...
            return
        _logger.error(str(removeID) + b' not found in markerIDs')
        return
