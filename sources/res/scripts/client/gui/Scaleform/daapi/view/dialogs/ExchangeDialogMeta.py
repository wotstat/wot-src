...
class _ExchangeDialogMeta(I18nConfirmDialogMeta):
    ...
    def makeVO(self):
        ...
        return {b'title': (self.getTitle()), 
           ...
           b'needItemsType': (self._needItemsType()), 
    ...
    def _getExchangeBlockData(self, resToExchange):
        ...
        return {b'goldStepperTitle': goldStepperTitleFmt, 
           ...
           b'defaultExchangeRate': (submitter.baseExchangeRate), 
           b'exchangeRate': (submitter.exchangeRate), 
           b'discountLimit': (submitter.exchangeRateDiscountLimit), 
           b'isDiscountAvailable': (submitter.exchangeRate != submitter.baseExchangeRate), 
           b'maxDiscountsAppliedForMoreInfo': MAX_SHOW_DISCOUNTS_INDEX, 
           b'isDiscountLimited': (submitter.isExchangeRateDiscountLimited), 
           b'limitRestrictionsBtnText': (backport.text(R.strings.personal_exchange_rates.common.limitRestrictions())), 
           b'discountLimitText': (backport.text(R.strings.personal_exchange_rates.common.limitExceeded())), 
           b'discountLimitOverExceededText': (backport.text(R.strings.personal_exchange_rates.common.limitOverExceeded())), 
           b'defaultGoldValue': goldForExchange, 
           b'goldStepSize': STEP_SIZE, 
           b'maxGoldValue': maxGold, 
           b'maxNeedItemsValue': maxNeedItems, 
...
class ExchangeCreditsWebProductMeta(_ExchangeDialogMeta, _ExchangeCreditsSubscriber):
    ...
    def __init__(self, name, count, price, key=b'confirmExchangeDialog/exchangeCredits', isModal=False):
...
class _ExchangeXpSubmitter(_ExchangeSubmitterBase, _XpTranslationExchangeRate):
    ...
    def _getMaxExchangeValue(self):
        ...
        maxAvailableXpForGold = calculateMaxPossibleFreeXp(sum(map(operator.attrgetter(b'xp'), eliteVehicles)))
