...
class AnimatedBattleHint(AnimatedBattleHintMeta):
    ...
    def __onEvent(self, event):
        ...
        if handler:
            ...
            _logger.error(b'Unexpected event action received: action = %s, ctx = %s', event.action, event.ctx)
        return

    def __setPenetration(self, ctx):
        self.as_setPenetrationS(ctx[b'penetrationType'], ctx[b'isColorBlind'])
        return

    def __showHint(self, ctx):
        self.__hideCallback = ctx[b'hideCallback']
        self.__hintType = ctx[b'hintType']
        self.__frameId = HINT_TYPE_TO_FRAME.get(self.__hintType, BATTLE_TOP_HINT_CONSTS.FRAME_EMPTY)
        self.__message = ctx[b'message']
        self.__completed = ctx[b'completed']
