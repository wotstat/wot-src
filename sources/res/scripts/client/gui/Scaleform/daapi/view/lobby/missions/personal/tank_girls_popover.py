...
class TankgirlsPopover(TankgirlsPopoverMeta):
    ...
    def __update(self):
        ...
        for quest, opName in events_helpers.getTankmanRewardQuests():
            tankwomenQuests.append({b'id': (str(quest.getID())), 
               b'operationLabel': (text_styles.standard(opName)), 
               b'missionLabel': (text_styles.main(quest.getShortUserName())), 
               b'recruitBtnLabel': (PERSONAL_MISSIONS.PERSONALMISSIONS_TANKGIRLSPOPOVER_RECRUITBTN)})
