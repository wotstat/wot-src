...
class BattleLoading(BaseBattleLoadingMeta, IArenaVehiclesController):
    ...
    def _setTipsInfo(self):
        ...
        if not _isBattleLoadingShowed():
            ...
            criteria.setContext(ctx={b'battlesCount': battlesCount, 
               b'vehicleType': (arenaDP.getVehicleInfo().vehicleType)})
    ...
    def _addArenaTypeData(self):
        self.as_setMapIconS(self._arenaVisitor.getArenaIcon(b'battleLoading'))
        return
    ...
    def _makeVisualTipVO(self, arenaDP, tip=None):
        ...
        vo = {b'settingID': settingID, 
           b'tipIcon': (tipIconPath if settingID == BattleLoadingTipSetting.OPTIONS.VISUAL else None), 
           b'arenaTypeID': (self._arenaVisitor.type.getID()), 
           b'minimapTeam': (arenaDP.getNumberOfTeam()), 
           b'showMinimap': (settingID == BattleLoadingTipSetting.OPTIONS.MINIMAP), 
           b'showTipsBackground': (settingID == BattleLoadingTipSetting.OPTIONS.MINIMAP)}
    ...
    def _getViewSettingByID(self, settingID):
        ...
        if settingID == BattleLoadingTipSetting.OPTIONS.TEXT:
            result.update({b'leftTeamTitleLeft': (-410), 
               b'rightTeamTitleLeft': 204, 
               b'tipTitleTop': 536, 
               b'tipBodyTop': 562, 
