...
def getViewSettings():
    ...
    return (
     ViewSettings(VIEW_ALIAS.LOBBY_PROFILE, ProfilePage, b'profile.swf', WindowLayer.SUB_VIEW, VIEW_ALIAS.LOBBY_PROFILE, ScopeTemplates.LOBBY_SUB_SCOPE),
     GroupedViewSettings(VIEW_ALIAS.PROFILE_WINDOW, ProfileWindow, b'profileWindow.swf', WindowLayer.WINDOW, b'profileWindow', None, ScopeTemplates.DEFAULT_SCOPE),
...
class ProfilePackageBusinessHandler(PackageBusinessHandler):
    ...
    def __loadOrUpdateView(self, event):
        ...
        if view and event.ctx.get(b'selectedAlias'):
