...
class EventBoardsAwardsOverlay(EventBoardsAwardsOverlayMeta):
    ...
    def setOpener(self, view):
        ...
        if eventData.getType() == EVENT_TYPE.VEHICLE:
            ...
            if _leaderboardID is not None:
                ...
                fields = ((b'level', False), (b'nations', True), (b'type', True))
            ...
            header = {b'filters': (makeFiltersVO(eventData.getType(), leaderboards, leaderboardID, category=b'awards'))}
    ...
    def _setData(self, lid):
        ...
        if eventType == EVENT_TYPE.VEHICLE:
            ...
            title = _ms(b'#event_boards:awards/vehicle')
            bgPath = None
        elif eventType == EVENT_TYPE.NATION:
            title = _ms((b'#event_boards:awards/nation/{}').format(leaderboard))
            bgPath = (b'../maps/icons/eventBoards/flagsOverlay/{}.png').format(leaderboard)
        elif eventType == EVENT_TYPE.LEVEL:
            title = _ms(b'#event_boards:awards/level', level=int2roman(leaderboard))
            bgPath = None
        elif eventType == EVENT_TYPE.CLASS:
            title = _ms((b'#event_boards:awards/class/{}').format(leaderboard))
            bgPath = None
        else:
            title = None
            bgPath = None
        ...
        for group in awardsGroups:
            ...
            awards.append({b'icon': icon, 
               b'positionDescr': (str(_min) if _min == _max else (b'{} - {}').format(_min, _max)), 
               b'awards': (awardsFormatter.getFormattedBonuses(group.getRewards())), 
               b'tooltip': (_ms(TOOLTIPS.ELEN_AWARDSOVERLAY_GROUP_HEADER, number=int2roman(categoryNumber)))})

        data = {b'eventID': (eventData.getEventID()), 
           b'title': title, 
           b'bgPath': bgPath, 
           b'awardsData': {b'tableData': awards}}
