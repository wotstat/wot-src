...
def createPlugins():
    resultPlugins = {b'core': CorePlugin, 
       ...
       b'aimDistance': AimDistancePlugin, 
       b'gunMarkersInvalidate': GunMarkersInvalidatePlugin, 
       b'shotResultIndicator': (DevShotResultIndicatorPlugin if IS_DEVELOPMENT else ShotResultIndicatorPlugin), 
       b'shotDone': ShotDonePlugin, 
       b'speedometerWheeledTech': SpeedometerWheeledTech, 
       b'siegePlugin': SiegeModePlugin, 
       b'dualgun': DualGunPlugin, 
       b'artyCamDist': ArtyCameraDistancePlugin, 
       b'spgShotResultIndicator': SPGShotResultIndicatorPlugin, 
       b'mutableDamage': MutableDamagePlugin, 
       b'aimDamage': AimDamagePlugin, 
       b'cameraTransition': CameraTransitionPlugin, 
       b'vehicleMechanics': VehicleMechanicsPlugin}
...
def _makeSettingsVO(settingGetter):
    ...
    for view in _SETTINGS_VIEWS:
        ...
        commonSettings.update({b'spgScaleWidgetEnabled': (settingGetter(SPGAim.SPG_SCALE_WIDGET)), 
           b'isColorBlind': (settingGetter(GRAPHICS.COLOR_BLIND))})
...
class AmmoPlugin(CrosshairPlugin):
    __slots__ = (b'__guiSettings', b'__burstSize', b'__shellsInClip', b'__autoReloadCallbackID', b'__autoReloadSnapshot', b'__scaledInterval', b'__reloadAnimator', b'__isShowingAutoloadingBoost')
    ...
    def __isGunAutoReloadTimerCritical(self):
        ...
        for device, status, _ in getStateValue(VEHICLE_VIEW_STATE.DEVICES) or []:
            if device in {b'ammoBay', b'gunner1', b'gunner2'} and status in {b'destroyed', b'critical'}:
                return True
...
class SiegeModePlugin(CrosshairPlugin):
    ...
    def __getNetSeparatorOverride(self, vehicleDescr):
        ...
        if b'autoreload' in gun.tags:
            return CROSSHAIR_CONSTANTS.NET_SEPARATOR_TYPE_EMPTY
        if b'controllableReload' in gun.tags:
            return CROSSHAIR_CONSTANTS.NET_SEPARATOR_TYPE_SHORT
...
class SpeedometerWheeledTech(CrosshairPlugin):
    ...
    def __getMaxSpeeds(self, vehicle):
        ...
        if siegeVehicleDescr is not None:
            ...
            siegeMaxSpd = round_py2_style_int(siegeEngineCfg[b'smplFwMaxSpeed'])
...
class DualGunPlugin(CrosshairPlugin):
    __slots__ = (b'__isDualGunVehicle',)
    ...
    def __onSPGShotsIndicatorStateChanged(self, shotStates):
        ...
        if self.__isEnabled and vehicle is not None:
            ...
            if needIndicatorsUpdate:
                self.__updateUIIndicators()
            self._parentObj.as_setShotFlyTimesS([{b'shellIntCD': k, b'flyTime': (v[0] if v[1] else -1.0)} for k, v in viewitems(self.__currentFlyTimes)])
        return
...
class MutableDamagePlugin(_DistancePlugin):
    __slots__ = (b'__targetID', b'__shellDescr')
...
class AimDamagePlugin(CrosshairPlugin):
    __slots__ = (b'__shellDamage', b'__vehicleDamageRate', b'__highDamageReference', b'__mediumDamageReference', b'__hasDamageReferences', b'__damageEvents', b'__gunInstallations')
...
class CameraTransitionPlugin(CrosshairPlugin):
    ...
    def __onSniperCameraTransition(self, event):
        transitionSide, transitionTime = event.ctx.get(b'transitionSide'), event.ctx.get(b'transitionTime')
        self.parentObj.as_runCameraTransitionFxS(transitionSide, transitionTime * MS_IN_SECOND)
        self._playSound2D(self._TRANSITION_SOUNDS.get(event.ctx.get(b'ctrlModeName')))
        return
