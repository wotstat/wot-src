...
_BACK_BTN_LABELS = {(VIEW_ALIAS.LOBBY_HANGAR): b'hangar', 
   (VIEW_ALIAS.LEGACY_LOBBY_HANGAR): b'hangar', 
   (VIEW_ALIAS.LOBBY_STORE): b'shop', 
   (VIEW_ALIAS.LOBBY_STORAGE): b'storage', 
   (VIEW_ALIAS.LOBBY_TECHTREE): b'researchTree', 
...
_TABS_DATA = (
 {b'id': (VEHPREVIEW_CONSTANTS.BROWSE_LINKAGE), 
    ...
    b'viewId': (VEHPREVIEW_CONSTANTS.BROWSE_LINKAGE)},
 {b'id': (VEHPREVIEW_CONSTANTS.MODULES_LINKAGE), 
    ...
    b'viewId': (VEHPREVIEW_CONSTANTS.MODULES_LINKAGE)},
 {b'id': (VEHPREVIEW_CONSTANTS.CREW_TAB_INJECT), 
    b'label': (VEHICLE_PREVIEW.INFOPANEL_TAB_CREWINFO_NAME), 
    b'linkage': (VEHPREVIEW_CONSTANTS.CREW_TAB_INJECT), 
    b'viewId': (VEHPREVIEW_CONSTANTS.CREW_TAB_INJECT)})
...
class VehiclePreview(LobbySelectableView, VehiclePreviewMeta):
    ...
    def __init__(self, ctx=None):
        ...
        self.__description = ctx.get(b'description')
        ...
        self._heroInteractive = not (self._itemsPack or self.__offers or ctx.get(b'offerID', 0) or self.__topPanelData or self._backAlias in notInteractive) and ctx.get(b'heroInteractive', True)
    ...
    def handleSelectedEntityUpdated(self, event):
        ...
        if ctx[b'state'] == CameraMovementStates.MOVING_TO_OBJECT and not isinstance(entity, HeroTank):
    ...
    def _onRegisterFlashComponent(self, viewPy, alias):
        if alias == VEHPREVIEW_CONSTANTS.TOP_PANEL_TABS_PY_ALIAS:
            ...
            crewText = self.__ctx.get(b'crewText')
    ...
    def _getData(self):
        ...
        result = {b'showPostProgressionBtn': showPostProgressionBtn, 
    ...
    def __updateCrewTabLabel(self):
        for tab in _TABS_DATA:
            if tab[b'linkage'] == VEHPREVIEW_CONSTANTS.CREW_TAB_INJECT:
                vehicle = g_currentPreviewVehicle.item
    ...
    def __getInfoPanelListDescription(vehicle):
        ...
        descriptions = (b'crew', b'crewSkills', b'crewEquips', b'crewSkillsEquips')
            ...
            if self._previewBackCb and not entity:
                ...
                if entity:
                    ...
                    if self._itemsCache.items.inventory.getItemData(compactDescr) is not None:
                        ...
                        event_dispatcher.showVehiclePreview(compactDescr, previewAlias=self._previousBackAlias, outfit=self.__ctx.get(b'backOutfit'))
                else:
                    event_dispatcher.showHangar()
            else:
                self.destroy()
    ...
    def _onInventoryChanged(self, *_):
        if not BuyVehicleWindow.getInstances() and not self.__ctx.get(b'skipInventoryUpdate', False):
            g_currentPreviewVehicle.selectNoVehicle()
        return
