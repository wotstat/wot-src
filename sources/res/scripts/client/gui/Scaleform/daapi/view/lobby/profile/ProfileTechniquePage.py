...
class ProfileTechniquePage(ProfileTechniquePageMeta):
    ...
    def setIsInHangarSelected(self, value):
        ...
        if self._data is not None:
            self.as_responseDossierS(self._battlesType, self._getTechniqueListVehicles(self._data), b'', self._getEmptyScreenLabel())
        return
    ...
    def updateView(self, ctx):
        isPrestigeSorting = ctx.get(b'initVehicleSorting', {}).get(b'selectedColumnStr') == b'prestigeLevel'
        if isPrestigeSorting:
            self._selectedData = ctx
            self._selectedVehicleIntCD = self._selectedData.get(b'itemCD', -1)
