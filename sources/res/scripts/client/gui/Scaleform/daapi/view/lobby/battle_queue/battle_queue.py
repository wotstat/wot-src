...
class _WinbackQueueProvider(RandomQueueProvider):
    ...
    def getAdditionalParams(self):
        ...
        return {b'subTitle': ((b' ').join((subTitleMainText, text_styles.stats(subTitleBattlesLeft)))), 
           b'footerText': (backport.text(R.strings.winback.winbackBattleQueue.footer.text()))}
...
class BattleQueue(BattleQueueMeta, LobbySubView):
    ...
    def __updateTimer(self):
        ...
        timeLabel = b'%d:%02d' % divmod(elapsedTime, 60)
        if self.__provider is not None and self.__provider.needAdditionalInfo():
            timeLabel = text_styles.concatStylesToSingleLine(timeLabel, b'*')
