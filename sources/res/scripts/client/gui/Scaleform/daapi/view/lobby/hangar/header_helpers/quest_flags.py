...
class BattleQuestsFlag(BaseQuestFlag):
    ...
    def formatQuests(cls, vehicle, params):
        ...
        if totalCount > 0:
            ...
            label, questType = b'', cls._QUEST_TYPE
        quests = [
        ...
        return wrapQuestGroup(HANGAR_HEADER_QUESTS.QUEST_GROUP_COMMON, b'', quests)
...
class MapboxQuestsFlag(BaseQuestFlag):
    ...
    def formatQuests(cls, vehicle, params):
        if vehicle is None:
            ...
            return wrapQuestGroup(HANGAR_HEADER_QUESTS.QUEST_GROUP_PERSONAL, b'', quests)
...
class RankedQuestsFlag(BaseQuestFlag):
    ...
    def formatQuests(cls, vehicle, params):
        ...
        return wrapQuestGroup(cls._QUEST_TYPE, b'', questsVo)
...
class ElenQuestsFlag(BaseQuestFlag):
    ...
    def formatQuests(cls, vehicle, params):
        if vehicle is None:
            ...
            return wrapQuestGroup(HANGAR_HEADER_QUESTS.QUEST_GROUP_EVENTS, b'', quests)
    ...
    def formatQuests(cls, vehicle, params):
        if vehicle is None:
            ...
            if not marathons:
                return
            result = []
            isGroupped = params.get(b'isGrouped', True)
            for index, marathonEvent in enumerate(marathons):
                flagVO = marathonEvent.getMarathonFlagState(vehicle)
                if flagVO[b'visible']:
                    quest = headerQuestFormatterVo(flagVO[b'enable'], flagVO[b'flagHeaderIcon'], b'', (b'').join((cls._QUEST_TYPE, str(index))), flag=flagVO[b'flagMain'], stateIcon=flagVO[b'flagStateIcon'], questID=marathonEvent.prefix, tooltip=flagVO[b'tooltip'], isTooltipSpecial=flagVO[b'enable'])
                    if not isGroupped:
                        wrappedGroup = wrapQuestGroup((b'').join((HANGAR_HEADER_QUESTS.QUEST_GROUP_MARATHON, str(index))), b'', [quest])
