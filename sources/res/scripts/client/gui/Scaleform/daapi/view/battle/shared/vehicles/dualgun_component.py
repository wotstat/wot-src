...
class ReloadFactorsState(object):
    __slots__ = (b'__flags', b'__weakref__')
...
class ReloadingAffectPolicy(object):
    __slots__ = (b'__stateID', b'__reloadingFactors')
...
class DeviceAffectPolicy(ReloadingAffectPolicy):
    __AFFECT_DEVICES = (b'ammoBay', b'loader')
    __AFFECT_STATES = (b'critical', b'destroyed')
...
class DualGunComponent(DualGunPanelMeta, IPrebattleSetupsListener):
    ...
    def __onDualGunStateUpdated(self, value):
        ...
        if len(states) < 2:
            LOG_WARNING(b'Got incorrect dualgun states length, aborting')
            return
    ...
    def __updateDualGunState(self, states, cooldownTimes):
        ...
        if debuff.leftTime > 0:
            ...
            if self.__clipType in (CROSSHAIR_CASSETTE_TYPES.MULTIPLE_BARREL_AUTOLOADER,
             CROSSHAIR_CASSETTE_TYPES.MULTIPLE_BARREL_CASSETTE):
    ...
    def __chargeReleased(self, event):
        ...
        keyDown = event.ctx.get(b'keyDown')
    ...
    def __hasNegativeEffect(self):
        if self.__clipType in (CROSSHAIR_CASSETTE_TYPES.MULTIPLE_BARREL_AUTOLOADER,
         CROSSHAIR_CASSETTE_TYPES.MULTIPLE_BARREL_CASSETTE):
            return False
        return self.__reloadingState.hasNegativeEffect()
