...
def _makeServerString(serverInfo, isServerNameShort=False):
    server = text_styles.neutral(text_styles.concatStylesToSingleLine(serverInfo.getShortName() if isServerNameShort else serverInfo.getName(), b' (', text_styles.neutral(serverInfo.getPingValue()), makePingStatusIcon(serverInfo.getPingStatus()), b')'))
...
class ServerListItemPresenter(object):
    ...
    def asDict(self):
        return {b'label': (self.__name), 
           b'id': (self.__peripheryID), 
           b'csisStatus': (self.__csisStatus), 
           b'data': (self.__hostName), 
           b'enabled': (self.isEnabled()), 
           b'timeLeft': (self.getTimeLeft()), 
           b'shortname': (self.__shortName), 
           b'pingValue': (self.__pingValue), 
           b'pingStatus': (self.__pingStatus), 
           b'tooltip': (self._buildTooltip(self.__peripheryID))}
...
class PrimeTimeViewBase(LobbySubView, PrimeTimeMeta, Notifiable, IPreQueueListener):
    ...
    def selectServer(self, idx):
        ...
        self.__serversDP.setSelectedID(vo[b'id'])
    ...
    def _getStatusText(self):
        ...
        params[b'serverName'] = self._connectionMgr.serverUserNameShort
    ...
    def _getTimeText(self, serverInfo):
        if serverInfo is None:
            ...
            params[b'serverName'] = serverInfo.getName()
            return backport.text(resSection.dyn(periodInfo.periodType.value, resSection.undefined)(), **params)
    ...
    def _prepareData(self, serverList, serverInfo):
        ...
        return {b'warningIconSrc': (self._getWarningIcon()), 
           b'status': (text_styles.grandTitle(self._getStatusText())), 
           b'serversText': (text_styles.expText(self._getServerText(serverList, serverInfo))), 
           b'timeText': (text_styles.expText(self._getTimeText(serverInfo))), 
           b'showAlertBG': (self._isAlertBGVisible()), 
    ...
    def __updateServersDP(self):
        ...
        currentSet = set(s.get(b'id') for s in self.__serversDP.collection)
