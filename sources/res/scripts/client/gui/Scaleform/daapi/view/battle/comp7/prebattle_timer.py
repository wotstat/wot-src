...
class Comp7PrebattleTimer(Comp7PrebattleTimerMeta):
    ...
    def _getMessage(self):
        ...
        if self._state == COUNTDOWN_STATE.START and not self.__isSelectionConfirmed:
    ...
    def __getInfoVO(self):
        return {b'readyBtnLabel': (backport.text(self.__RES_ROOT.info.readyBtn.label())), b'readyBtnTooltip': (makeTooltip(body=backport.text(self.__RES_ROOT.info.readyBtn.tooltip.body())))}
    ...
    def __getWinString(battleCtx):
        ...
        if hasControlPoint:
            return backport.text(R.strings.arenas.type.comp7.description())
        return backport.text(R.strings.arenas.type.dyn((b'comp7_{}').format(1 if hasBase else 2)).dyn(b'timer_message')())
