...
class _SelectorItem(object):
    __slots__ = (b'_label', b'_data', b'_order', b'_selectorType', b'_isVisible', b'_isExtra', b'_isSelected', b'_isNew', b'_isDisabled', b'_isLocked')
    ...
    def getVO(self):
        return {b'label': (self.getFormattedLabel()), 
           b'data': (self._data), 
           b'disabled': (self._isDisabled), 
           b'icon': (self.getLargerIcon()), 
           b'active': (self._isSelected), 
           b'isNew': (self.isShowNewIndicator()), 
           b'specialBgIcon': (self.getSpecialBGIcon())}
...
class _DisabledSelectorItem(_SelectorItem):
    ...
    def select(self):
        _logger.warning(b'That routine can not be invoked')
        return
...
class _BattleSelectorItems(object):
    ...
    def select(self, action, onlyActive=False):
        if action in self.__items:
            ...
            _logger.error(b'Can not invoke action: %s', action)
        return
    ...
    def isSelected(self, action):
        ...
        _logger.error(b'Action not found: %s', action)
        return False
...
class _SquadItem(SelectorItem):
    ...
    def getFormattedLabel(self):
        ...
        if self._isDescription:
            ...
            return (b'').join((title, b'\n', description))
        return title

    def getVO(self):
        vo = super(_SquadItem, self).getVO()
        vo[b'tooltip'] = self._createTooltip()
        return vo
...
class SpecialSquadItem(_SquadItem):
    ...
    def getVO(self):
        ...
        if self._isSpecialBgIcon:
            vo[b'specialBgIcon'] = backport.image(R.images.gui.maps.icons.lobby.eventPopoverBtnBG())
        return vo
...
class _BattleRoyaleItem(SelectorItem):
    ...
    def getVO(self):
        ...
        if isEnabled and isActive and self.__battleRoyaleController.getCurrentSeason() is not None:
            vo[b'specialBgIcon'] = backport.image(R.images.gui.maps.icons.buttons.selectorRendererBGEvent())
        return vo
    ...
    def __getPerformanceAlarmStr(self):
        ...
        if attentionText and iconPath:
            return icons.makeImageTag(iconPath, vSpace=-3) + b' ' + attentionText
        else:
            return
...
def _addEventBattlesType(items):
    items.append(_EventBattlesItem(b'Event Battle', PREBATTLE_ACTION_NAME.EVENT_BATTLE, 2, SELECTOR_BATTLE_TYPES.EVENT))
    return
...
def create():
    ...
    if _g_items is None:
        ...
        _logger.warning(b'Item already is created')
