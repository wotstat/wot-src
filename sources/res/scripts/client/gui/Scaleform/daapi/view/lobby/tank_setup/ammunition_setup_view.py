...
class AmmunitionSetupView(AmmunitionSetupViewMeta):
    ...
    def __onAmmunitionSetupViewResized(self, event):
        self.as_gfSizeUpdatedS(event.ctx.get(b'offsetX'), event.ctx.get(b'width'), event.ctx.get(b'bottomMargin'))
        return

    def __onUpdateTTC(self, event):
        vehicleItem = event.ctx.get(b'vehicleItem')
        sectionName = event.ctx.get(b'sectionName')
