...
class MapsTrainingBattleGoals(BattleHintComponent, MapsTrainingGoalsMeta, IArenaVehiclesController):
    ...
    def _showHint(self, hintData):
        hintType = hintData[b'hintType']
        descr = hintData.get(b'description1')
        if descr is None:
            return
        else:
            if hintType is HintType.GOAL:
                descr = descr.format(count=hintData[b'targetsCount'], total=hintData[b'targetsTotal'])
            if hintType is HintType.TIMER_GREEN:
                self.as_showHintS(hintType.value, hintData.get(b'description2'), descr)
            else:
