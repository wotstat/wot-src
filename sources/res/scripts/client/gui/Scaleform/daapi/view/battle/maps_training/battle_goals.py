...
class MapsTrainingBattleGoals(BattleHintComponent, MapsTrainingGoalsMeta, IArenaVehiclesController):
    ...
    def __init__(self):
        super(MapsTrainingBattleGoals, self).__init__(battleHintsQueueParams=BattleHintQueueParams(name=b'maps_training', withFadeOut=False))
        self.goalsByType = {vehClass: {b'vehClass': vehClass, b'total': 0} for vehClass in VEHICLE_CLASS_TAGS}
        self._loaded = False
        return
    ...
    def _setGoals(self):
        ...
        playerClass = vehicles.getVehicleClassFromVehicleType(playerVehicle[b'vehicleType'].type)
        playerTeam = playerVehicle[b'team']
        goals = mapScenarios[playerTeam][playerClass][b'goals']
        sortedData = []
        for vehCls in VEHICLE_CLASSES_ORDER:
            goal = self.goalsByType[vehCls]
            goal[b'total'] = goals[vehCls]
    ...
    def _showHint(self, model, params):
        ...
        message = vo.get(b'message')
        if message is None:
            return
        else:
            if model.props.hintType is HintType.TIMER_GREEN:
                self.as_showHintS(model.props.hintType.value, vo.get(b'message2'), message)
            else:
