...
class BattleStatisticsDataController(BattleStatisticDataControllerMeta, IVehiclesAndPersonalInvitationsController):
    ...
    def updateTriggeredChatCommands(self, chatCommands, arenaDP):
        ...
        for vehicleID, state in viewitems(chatCommands):
            ...
            entry = {b'chatCommandName': (str(chatCommandName)), 
               b'vehicleID': (int(vehicleID))}
            data.append(entry)

        updateList = {b'chatCommands': data}
        self.as_updateTriggeredChatCommandsS(updateList)
        return

    def updatePriorityChatCommand(self, vehicleID, chatCommand, durationMS):
        data = {b'vehicleID': vehicleID, 
           b'chatCommand': chatCommand, 
           b'chatCommandDurationMS': durationMS}
    ...
    def __setArenaDescription(self):
        ...
        arenaInfoData = {b'mapName': (battleCtx.getArenaTypeName()), 
           b'winText': (battleCtx.getArenaWinString()), 
           b'winTextShort': (self._getArenaWinTextShort()), 
           b'battleTypeLocaleStr': (battleCtx.getArenaDescriptionString(isInBattle=False)), 
           b'battleTypeIconPathBig': (battleCtx.getBattleTypeIconPathBig()), 
           b'battleTypeIconPathSmall': (battleCtx.getBattleTypeIconPathSmall()), 
           b'allyTeamName': (battleCtx.getTeamName(enemy=False)), 
           b'enemyTeamName': (battleCtx.getTeamName(enemy=True))}
        ...
        if not self.__canDisplayPMMission(self.sessionProvider.getArenaDP()):
            ...
            if selectedQuest:
                if progressID and conditionVO:
                    selectedQuest.setMissionConditionState(vehCmpDescr, progressID, conditionVO.get(b'state', 0))
                self.as_setQuestStatusS(self.__getStatusData(selectedQuest))
