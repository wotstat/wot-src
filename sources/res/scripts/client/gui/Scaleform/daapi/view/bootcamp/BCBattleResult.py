...
_SNDID_ACHIEVEMENT = b'result_screen_achievements'
_SNDID_BONUS = b'result_screen_bonus'
_AMBIENT_SOUND = b'bc_result_screen_ambient'
...
class BCBattleResult(BCBattleResultMeta):
    ...
    def __init__(self, ctx=None):
        ...
        if b'arenaUniqueID' not in ctx:
            raise UserWarning(b'Key "arenaUniqueID" is not found in context', ctx)
        if not ctx[b'arenaUniqueID']:
            raise UserWarning(b'Value of "arenaUniqueID" must be greater than 0')
    ...
    @simpleLog(argsIndex=0, resetTime=False, logOnce=True, argMap=BC_AWARDS_MAP, argMapSection=(lambda : g_bootcamp.getContext()[b'lessonNum']))
    ...
    def _populate(self):
        ...
        self.app.as_loadLibrariesS([b'guiControlsLobbyBattleDynamic.swf',
         b'guiControlsLobbyDynamic.swf'])
    ...
    def __setBattleResults(self):
        ...
        self.as_setDataS(vo)
        self.uiBootcampLogger.log(action=DEPRECATED_BC_LOG_ACTIONS.SHOW, finishReason=vo.get(b'finishReason', None))
        self.__hasShowRewards = vo[b'showRewards']
        self.__hasBonusInMedals = vo[b'hasUnlocks']
        self.__hasBonusInStats = vo[b'xp'][b'value'] > 0 or vo[b'credits'][b'value'] > 0
        return
