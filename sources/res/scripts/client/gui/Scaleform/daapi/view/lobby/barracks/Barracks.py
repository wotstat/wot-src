...
_COUNTERS_MAP = {RECRUIT_NOTIFICATIONS: (b'locationButtonBar', 5)}

def _packCounterVO(componentId, count, selectedIdx=0):
    return {b'componentId': componentId, 
       b'count': count, 
...
class Barracks(BarracksMeta, LobbySubView, IGlobalListener):
    ...
    def openPersonalCase(self, tankmanInvID, tabID):
        if self.filter[b'location'] == BARRACKS_CONSTANTS.LOCATION_FILTER_NOT_RECRUITED:
            return
    ...
    def setTankmenFilter(self):
        self.as_setTankmenFilterS(self.filter[b'nation'], self.filter[b'role'], self.filter[b'tankType'], self.filter[b'location'], self.filter[b'nationID'])
        return

    def setFilter(self, nation, role, tankType, location, nationID):
        self.filter[b'nation'] = nation
        self.filter[b'role'] = role
        self.filter[b'tankType'] = tankType
        self.filter[b'location'] = location
        self.filter[b'nationID'] = nationID
    ...
    def actTankman(self, invID):
        if self.filter[b'location'] != BARRACKS_CONSTANTS.LOCATION_FILTER_NOT_RECRUITED:
            ...
            if tankman is None:
                _logger.error(b'Attempt to dismiss tankman by invalid invID: %r', invID)
                return
            if tankman.isDismissed:
                ...
                if tmanVehile is None:
                    _logger.error(b"Target tankman's vehicle is not found in inventory %r %r", tankman, tankman.vehicleInvID)
                    return
    ...
    def onCountersVisited(self, counters):
        for counterGfxData in counters:
            ...
            valToSearch = (counterData[b'componentId'], counterData[b'selectedIdx'])
    ...
    def __updateLocationFilter(self):
        ...
        if nationID is not None:
            self.filter[b'nationID'] = nationID
        tankType = data.get(b'tankType')
        if tankType is not None:
            self.filter[b'tankType'] = tankType
        role = data.get(b'role')
        if role is not None:
            self.filter[b'role'] = role
        return
    ...
    def __updateTanksList(self):
        ...
        for module in modulesAll:
            if self.filter[b'nation'] != -1 and self.filter[b'nation'] != module.descriptor.type.id[0] or self.filter[b'tankType'] != b'None' and self.filter[b'tankType'] != -1 and self.filter[b'tankType'] != module.type:
                continue
            data.append({b'data': {b'type': (module.type), 
                         b'nationID': (module.descriptor.type.id[0]), 
                         b'typeID': (module.descriptor.type.id[1])}, 
               b'label': (module.descriptor.type.shortUserString)})

        self.as_updateTanksListS(data)
        return

    def __updateTankmen(self, *args):
        isNotRecruited = self.filter[b'location'] == BARRACKS_CONSTANTS.LOCATION_FILTER_NOT_RECRUITED
        self.__switchTankmanFiltersEnable(not isNotRecruited)
        if self.filter[b'location'] == BARRACKS_CONSTANTS.LOCATION_FILTER_DISMISSED:
    ...
    def __showActiveTankmen(self, criteria):
        ...
        self.as_setTankmenS({b'tankmenCount': (self.__getTankmenCountStr(self.__dataProvider.filteredCount, totalCount=self.__dataProvider.totalCount)), 
           b'placesCount': (self.__getPlaceCountStr(free=self.__dataProvider.placeCount, totalCount=self.itemsCache.items.stats.tankmenBerthsCount)), 
           b'placesCountTooltip': None, 
           b'hasNoInfoData': False})
        return
    ...
    def __showDismissedTankmen(self, criteria):
        ...
        self.as_setTankmenS({b'tankmenCount': (self.__getTankmenCountStr(tankmenInSlots=filteredCount, totalCount=totalCount)), 
           b'placesCount': (self.__getPlaceCountStr(free=icons.info(), totalCount=placeCount)), 
           b'placesCountTooltip': (self.__getPlacesCountTooltip(placeCount=placeCount)), 
           b'hasNoInfoData': hasNoInfoData, 
           b'noInfoData': noInfoData})
        return
    ...
    def __showNotRecruitedTankmen(self):
        ...
        self.as_setTankmenS({b'tankmenCount': (self.__getTankmenCountStr(totalCount=count)), 
           b'placesCount': b'', 
           b'placesCountTooltip': None, 
           b'hasNoInfoData': hasNoInfoData, 
           b'noInfoData': noInfoData})
        ...
        if hasNoInfoData:
            if self.filter[b'location'] == BARRACKS_CONSTANTS.LOCATION_FILTER_DISMISSED:
                if totalCount < 1:
                    ...
                    noInfoData = {b'title': (text_styles.highTitle(MENU.BARRACKS_NORECOVERYTANKMEN_TITLE)), 
                       b'message': (text_styles.main(_ms(MENU.BARRACKS_NORECOVERYTANKMEN_MESSAGE, price=moneyWithIcon(tankmenRestoreConfig.cost), totalDays=freeDays + billableDays, freeDays=freeDays, paidDays=billableDays)))}
                else:
                    noInfoData = {b'message': (text_styles.main(MENU.BARRACKS_NOFILTEREDRECOVERYTANKMEN_MESSAGE))}
            elif self.filter[b'location'] == BARRACKS_CONSTANTS.LOCATION_FILTER_NOT_RECRUITED:
                noInfoData = {b'title': (text_styles.highTitle(MENU.BARRACKS_NONOTRECRUITEDTANKMEN_TITLE)), b'message': (text_styles.main(MENU.BARRACKS_NONOTRECRUITEDTANKMEN_MESSAGE))}
    ...
    def __buildCriteria(self):
        ...
        if self.filter[b'nation'] != -1:
            criteria |= REQ_CRITERIA.NATIONS([self.filter[b'nation']])
        if self.filter[b'role'] != b'None':
            criteria |= REQ_CRITERIA.TANKMAN.ROLES(self.filter[b'role'])
        if self.filter[b'tankType'] != b'None':
            criteria |= REQ_CRITERIA.CUSTOM((lambda tankman: tankman.vehicleNativeType == self.filter[b'tankType']))
        if self.filter[b'location'] == BARRACKS_CONSTANTS.LOCATION_FILTER_TANKS or self.filter[b'location'] == b'':
            criteria |= REQ_CRITERIA.TANKMAN.IN_TANK
        elif self.filter[b'location'] == BARRACKS_CONSTANTS.LOCATION_FILTER_BARRACKS:
            criteria |= ~REQ_CRITERIA.TANKMAN.IN_TANK
        if self.filter[b'nationID'] is not None:
            vehicle = self.itemsCache.items.getItem(GUI_ITEM_TYPE.VEHICLE, int(self.filter[b'nationID']), int(self.filter[b'location']))
    ...
    def __setCountersData(self, preferenceName, counter):
        counters = [
         _packCounterVO(componentId=_COUNTERS_MAP[preferenceName][0], count=str(counter) if counter else b'', selectedIdx=_COUNTERS_MAP[preferenceName][1])]
