...
import ArenaType
_VOICEOVERS = (b'vo_mt_loading_first_battle', b'vo_mt_loading_next_battle')
...
class MapsTrainingPreBattleTimer(MapsTrainingPrebattleTimerMeta):
    ...
    def _populate(self):
        ...
        self.__playerClass = vehicles.getVehicleClassFromVehicleType(playerVehicle[b'vehicleType'].type)
        self.__playerTeam = playerVehicle[b'team']
        ...
        self.as_setSideS(backport.text(strings.prebattle.scenario(), map=backport.text(R.strings.arenas.dyn((b'c_{}').format(mapName)).name()), scenario=backport.text(strings.scenarioNum()).format(scenario=scenarioIndex), vehicleType=backport.text(strings.vehicleType.dyn(self.__playerClass)())))
    ...
    def __setGoalsData(self):
        ...
        mapScenarios = self.mapsTrainingController.getConfig()[b'scenarios'][geometryID]
        goals = mapScenarios[self.__playerTeam][self.__playerClass][b'goals']
        text = backport.text(R.strings.maps_training.prebattle.targets(), count=sum(goals.values()))
        self.as_updateS([{b'vehClass': vehCls, b'total': (goals[vehCls])} for vehCls in VEHICLE_CLASSES_ORDER], text)
    ...
    def __playSoundNotification(self):
        ...
        if hasattr(player, b'soundNotifications'):
