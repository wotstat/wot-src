...
class CustomLoginStatuses(CONST_CONTAINER):
    ANOTHER_PERIPHERY = b'another_periphery'
    CHECKOUT_ERROR = b'checkout_error'
    CENTER_RESTART = b'centerRestart'
    VERSION_MISMATCH = b'versionMismatch'
    ACCESS_FORBIDDEN_TO_PERIPHERY = b'accessForbiddenToPeriphery'
...
class LoginView(LoginPageMeta):
    ...
    def onLogin(self, userName, password, serverName, isSocialToken2Login):
        if self._loginMode.showRememberServerWarning:
            ...
            builder.setMessagesAndButtons(R.strings.dialogs.dyn(b'loginToPeripheryAndRemember'))
    ...
    def _onLoggedOn(self, *args):
        ...
        self._autoSearchVisited = selectedServer[b'data'] == AUTO_LOGIN_QUERY_URL if selectedServer is not None else False
        self.__customLoginStatus = None
        return
    ...
    @uniprof.regionDecorator(label=b'offline.login', scope=b'enter')
    def _populate(self):
        View._populate(self)
    ...
    @uniprof.regionDecorator(label=b'offline.login', scope=b'exit')
    def _dispose(self):
    ...
    def update(self):
        self.as_setDefaultValuesS({b'loginName': (self._loginMode.login), b'pwd': (self._loginMode.password), 
           b'memberMe': (self._loginMode.rememberUser), 
           b'memberMeVisible': (self._loginMode.rememberPassVisible), 
           b'isIgrCredentialsReset': (GUI_SETTINGS.igrCredentialsReset), 
           b'showRecoveryLink': (not GUI_SETTINGS.isEmpty(b'recoveryPswdURL')), 
           b'keyboardLang': (self.__lang), 
           b'capsLockState': (self.__capsLockState)})
    ...
    def _onKickedWhileLogin(self, peripheryID):
        ...
        if not self.__loginRetryDialogShown and peripheryID >= 0:
            self.__showLoginRetryDialog({b'waitingOpen': (backport.text(R.strings.waiting.titles.dyn(self.__customLoginStatus)())), 
               b'waitingClose': (backport.msgid(R.strings.waiting.buttons.cease())), 
               b'message': (backport.text(R.strings.waiting.message.dyn(self.__customLoginStatus)(), self.__getServerText(self.__customLoginStatus, self.connectionMgr.serverUserName)))})
        return
    ...
    def _onHandleQueue(self, queueNumber):
        ...
        message = backport.text(R.strings.waiting.message.queue(), self.__getServerText(b'overload', serverName), queueNumber)
        if showAutoSearchBtn:
            message = backport.text(R.strings.waiting.message.useAutoSearch(), self.__getServerText(b'overload', serverName), queueNumber, serverName)
        if not self.__loginQueueDialogShown:
            ...
            ctx = {b'title': (backport.msgid(R.strings.waiting.titles.queue())), 
               b'message': message, 
               b'cancelLabel': cancelBtnLbl, 
               b'showAutoLoginBtn': showAutoSearchBtn}
    ...
    def __loginRejectedUpdateNeeded(self):
        success = yield DialogsInterface.showI18nConfirmDialog(b'updateNeeded')
    ...
    def __loginRejectedBan(self, responseData):
        bansJSON = responseData.get(b'bans')
        bans = json.loads(bansJSON)
        expiryTime = int(bans.get(b'expiryTime', b'0'))
        reason = bans.get(b'reason', b'')
        ...
        if reason.startswith(b'#'):
            reason = _ms(reason)
        if expiryTime > 0:
            if bans.get(b'curfew') is not None and bans.get(b'reason', b'') != b'#ban_reason:curfew_ban':
                self.as_setErrorMessageS(reason, INVALID_FIELDS.ALL_VALID)
            else:
                expiryTime = makeLocalServerTime(expiryTime)
                expiryTime = backport.getLongDateFormat(expiryTime) + b' ' + backport.getLongTimeFormat(expiryTime)
    ...
    def __loginRejectedRateLimited(self):
        ...
        if not self.__loginRetryDialogShown:
            self.__showLoginRetryDialog({b'waitingOpen': (backport.msgid(R.strings.waiting.titles.queue())), 
               b'waitingClose': (backport.msgid(R.strings.waiting.buttons.exitQueue())), 
               b'message': (backport.text(R.strings.waiting.message.autoLogin(), self.__getServerText(b'overload', self.connectionMgr.serverUserName)))})
        return
    ...
    def __loginRejectedWithCustomState(self):
        if self.__customLoginStatus == CustomLoginStatuses.ACCESS_FORBIDDEN_TO_PERIPHERY:
            if self.connectionMgr.peripheryRoutingGroup is not None:
                peripheriesStr = (b', ').join(p.shortName for p in self.connectionMgr.availableHosts)
            else:
                peripheriesStr = b''
    ...
    def __showLoginRetryDialog(self, data):
        self._clearLoginView()
        self.fireEvent(LoadViewEvent(SFViewLoadParams(VIEW_ALIAS.LOGIN_QUEUE), title=data[b'waitingOpen'], message=data[b'message'], cancelLabel=data[b'waitingClose'], showAutoLoginBtn=False), EVENT_BUS_SCOPE.LOBBY)
    ...
    def __closeLoginRetryDialog(self, event=None):
        ...
        self.fireEvent(LoginEvent(LoginEventEx.CANCEL_LGN_QUEUE, b''))
    ...
    def __closeLoginQueueDialog(self):
        self.fireEvent(LoginEvent(LoginEvent.CANCEL_LGN_QUEUE, b''))
    ...
    def __getServerText(self, key, serverName):
        return makeHtmlString(b'html_templates:login/server-state', key, {b'message': (backport.text(R.strings.waiting.message.server.dyn(key)(), server=serverName))})

    def __onBootcampStartChoice(self, _):
        WWISE.WW_eventGlobal(b'loginscreen_mute')
        return
