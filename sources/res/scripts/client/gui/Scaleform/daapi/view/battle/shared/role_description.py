...
_ROLE_HTML_TEMPLATE = b'html_templates:vehicleRoles/roleDescription'
...
class RoleDescription(RoleDescriptionMeta):
    ...
    def __setData(self, typeDescriptor):
        ...
        roleStr = backport.text(roleExp.roleGroupName.dyn(roleName, b'')())
        classWithRole = backport.text(roleExp.roleName.dyn(roleName, b'')(), groupName=makeHtmlString(_ROLE_HTML_TEMPLATE, b'role', {b'role': roleStr}))
        title = makeHtmlString(_ROLE_HTML_TEMPLATE, b'className', {b'className': classWithRole})
        actions = getActionsByRole(roleId)
        self.as_setDataS({b'roleIcon': (backport.image(R.images.gui.maps.icons.roleExp.roles.c_100x100.dyn(roleName, b'')())), 
           b'roleDescription': title, 
           b'roleActions': [self.__getRoleActionVO(action) for action in actions]})
        return

    @staticmethod
    def __getRoleActionVO(action):
        actionLbl = ACTION_TYPE_TO_LABEL[action]
        return {b'image': (backport.image(R.images.gui.maps.icons.roleExp.actions.c_128x128.dyn(actionLbl, b'')())), 
           b'description': (backport.text(R.strings.menu.roleExp.action.dyn(actionLbl, b'')()))}
