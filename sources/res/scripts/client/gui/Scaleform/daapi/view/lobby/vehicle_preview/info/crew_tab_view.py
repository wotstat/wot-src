...
class CrewTabView(ViewImpl):
    __slots__ = (b'_toolTipMgr', b'_title', b'_crewItemPack', b'_isCustomCrew', b'_vehicle', b'_crew')
    ...
    def createToolTip(self, event):
        if event.contentID == R.views.common.tooltip_window.backport_tooltip_content.BackportTooltipContent():
            ...
            if tooltipId == TooltipConstants.SKILL:
                skillName = str(event.getArgument(b'skillName'))
                roleName = str(event.getArgument(b'roleName'))
                skillLevel = crewMemberRealSkillLevel(self._vehicle, skillName)
                args = [skillName, roleName, None, skillLevel, None, event.getArgument(b'customName')]
            ...
            if tooltipId == TooltipConstants.VEHICLE_PREVIEW_CREW_MEMBER:
                tankmanIdx = int(event.getArgument(b'index'))
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.lobby.crew.tooltips.EmptySkillTooltip():
            LOG_DEBUG_DEV(b'createToolTipContent EmptySkillTooltip')
            tankmanIndex = int(event.getArgument(b'tankmanID'))
            _, tankman = self._crew[tankmanIndex]
            skillIndex = int(event.getArgument(b'skillIndex'))
    ...
    def _fillExtraInfo(self, vm):
        ...
        if self._isCustomCrew:
            ...
            isLockedCrew = self._crewItemPack.extra.get(b'isLockedCrew', False)
        else:
    ...
    def _getVehicle(self):
        ...
        tankmenItems = self._crewItemPack.extra.get(b'tankmen', [])
    ...
    def _onOfferChanged(self, event):
        ctx = event.ctx
        self.updateData(self._title, ctx.get(b'vehicleItems'), ctx.get(b'crewItems'))
        return
