...
class BattleSessionWindow(BattleSessionWindowMeta):
    ...
    def requestToKickPlayer(self, pID):
        yield self.prbDispatcher.sendPrbRequest(KickPlayerCtx(pID, b'prebattle/kick'))
        return
    ...
    def __setSorting(self):
        data = [{b'name': (backport.text(key)), b'id': (str(index))} for index, (key, _, _) in enumerate(self.__SORTINGS_AND_COMPARATORS)]
