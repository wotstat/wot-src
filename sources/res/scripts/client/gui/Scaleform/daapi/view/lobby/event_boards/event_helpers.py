...
class _SelectionTask(_Task):
    ...
    def getInfo(self):
        ...
        if method in self.selectionWithCardinality:
            ...
            result = result % {b'cardinality': (str(cardinality)), b'distance': (str(distance))}
...
class _EventTypeTask(_Task):
    ...
    def getTooltip(self):
        ...
        if eventType == _et.NATION:
            ...
            levels = (b', ').join([int2roman(value) for value in levelList])
...
class _VehiclesCondition(_Condition):
    ...
    def getInfo(self):
        ...
        if singleVehicle:
            ...
            vehicleName = formatVehicleNameWithTypeIcon(vehicle, b'html_templates:lobby/elen/objective')
...
class _TopLeaderboard(object):
    ...
    def getInfo(self):
        ...
        return {b'eventID': (top.getEventID()), 
           b'title': (text_styles.highlightText(self.__getTitle())), 
           b'isAvailable': True, 
           b'ribbonTooltip': (self.__getRibbonTooltip()), 
           b'descriptionTooltip': (self.__getDescriptionTooltip()), 
           b'amountTooltip': (self.__getAmountTooltip()), 
           b'awards': None, 
           b'ribbon': (self.__getRibbon()), 
           b'status': None, 
           b'background': (b'marathon' if category else b'default'), 
           b'amount': (str(top.getMyValue())), 
           b'type': (self.__getRaitingType()), 
           b'description1': (self.__getMyPosititon()), 
           b'description2': (self.__getStatusValue()), 
           b'uiDecoration': (self.__getDecoration()), 
           b'uiPicture': (self.__getPicture()), 
           b'cardID': (str(top.getLeaderboardID()))}
    ...
    def __getTitle(self):
        ...
        if self.__eventType == _et.VEHICLE:
            ...
            return formatVehicleNameWithTypeIcon(vehicle, b'html_templates:lobby/elen/top')
    ...
    def __getPicture(self):
        if self.__eventType == _et.NATION:
            ...
            if self.__eventType == _et.VEHICLE:
                ...
                name = vehicle.name.split(b':', 1)[-1].lower()
    ...
    def __getMyPosititon(self):
        ...
        return text_styles.main((u'{} {}').format(_ms(EVENT_BOARDS.TOP_POSITION), self._top.getMyPosition()))
...
class EventInfo(object):
    ...
    def getTaskInfo(self):
        return {b'title': (text_styles.highTitle(_ms(EVENT_BOARDS.BLOCK_TASK))), 
           b'uiIcon1': (self.__getTaskIcon(1)), 
           b'uiIcon2': (self.__getTaskIcon(2)), 
           b'uiIcon3': (self.__getTaskIcon(3)), 
           b'description1': (self._objectiveTask.getInfo()), 
           b'description2': (self._selectionTask.getInfo()), 
           b'description3': (self._eventTypeTask.getInfo()), 
           b'label': (_ms(EVENT_BOARDS.BUTTON_REGULATIONS)), 
           b'buttonTooltip': (makeTooltip(EVENT_BOARDS.BUTTON_REGULATIONS, EVENT_BOARDS.BUTTON_REGULATIONS_TOOLTIP)), 
           b'description1Tooltip': (self._objectiveTask.getTooltip()), 
           b'description2Tooltip': (self._selectionTask.getTooltip()), 
           b'description3Tooltip': (self._eventTypeTask.getTooltip()), 
           b'buttonVisible': (bool(self._event.getManual()))}

    def getConditionInfo(self):
        return {b'title': (text_styles.highTitle(_ms(EVENT_BOARDS.BLOCK_OBJECTIVE))), 
           b'uiIcon1': (self.__getConditionIcon(1)), 
           b'uiIcon2': (self.__getConditionIcon(2)), 
           b'uiIcon3': (self.__getConditionIcon(3)), 
           b'description1': (self._battleTypeCondition.getInfo()), 
           b'description2': (self._primeTimeCondition.getInfo()), 
           b'description3': (self._vehiclesCondition.getInfo()), 
           b'label': (_ms(EVENT_BOARDS.VEHICLES_VEHICLE)), 
           b'buttonTooltip': (makeTooltip(EVENT_BOARDS.VEHICLES_VEHICLE, EVENT_BOARDS.VEHICLES_VEHICLE_TOOLTIP)), 
           b'description1Tooltip': (self._battleTypeCondition.getTooltip()), 
           b'description2Tooltip': (self._primeTimeCondition.getTooltip()), 
           b'description3Tooltip': (self._vehiclesCondition.getTooltip()), 
           b'buttonVisible': (bool(len(self._event.getLimits().getVehiclesWhiteList()) > 1))}

    def getAwardInfo(self):
        return {b'title': (text_styles.highTitle(_ms(EVENT_BOARDS.BLOCK_AWARD))), 
           b'label': (_ms(EVENT_BOARDS.BUTTON_AWARDS)), 
           b'buttonTooltip': (makeTooltip(EVENT_BOARDS.BUTTON_AWARDS, EVENT_BOARDS.BUTTON_AWARDS_TOOLTIP)), 
           b'iconTooltip': (makeTooltip(body=EVENT_BOARDS.TOOLTIP_AWARDICON)), 
           b'uiIcon': (self._event.getPromoBonuses())}
    ...
    def getServerData(self):
        ...
        if not finished and not self._canJoin and _psr.SPECIALACCOUNT in self._stateReasons:
            result = {b'reloginBlock': {b'title': (formatAllertTextWithIcon(text_styles.alert(EVENT_BOARDS.STATUS_CANTJOIN_REASON_SPECIAL))), 
                                 b'description': b''}, 
               b'isRelogin': True}
            return result
        ...
        if not isAvailableServer and isAvailableAnyServer and self._playerState.getPlayerState() == _es.JOINED:
            buttons = [{b'label': (str(self._lobbyContext.getPeripheryName(int(pt.getServer()), False))), b'server': (pt.getServer()), b'tooltip': (makeTooltip(self._lobbyContext.getPeripheryName(int(pt.getServer()), False), TOOLTIPS.ELEN_BUTTON_SERVER_BODY))} for pt in availableServers]
            result = {b'serverBlock': {b'title': title, b'buttons': buttons}, b'isUnsuitableServer': True}
            ...
            result = {b'reloginBlock': {b'title': (EVENT_BOARDS.SERVER_NOSUITABLE_HEADER), b'description': description, 
                                 b'descriptionTooltip': (self.__getPrimeTimesTooltip())}, 
               b'isRelogin': True}
        return result
    ...
    def getStatusData(self):
        ...
        titleTooltip = None
        buttonRegistrationLabel = b''
        description1Tooltip = b''
        description2Tooltip = b''
        ...
        return {b'title': title, b'description1': description1, 
           b'description2': description2, 
           b'isRegistrationTop': isRegistrationTop, 
           b'isRegistration': isRegistration, 
           b'isRegistrationEnabled': isRegistrationButtonEnabled, 
           b'registrationTooltip': registrationTooltip, 
           b'titleTooltip': titleTooltip, 
           b'ratingTooltip': (makeTooltip(TOOLTIPS.ELEN_BUTTON_RAITING_HEADER, TOOLTIPS.ELEN_BUTTON_RAITING_BODY)), 
           b'isRating': started, 
           b'buttonRegistrationLabel': buttonRegistrationLabel, 
           b'description1Tooltip': description1Tooltip, 
           b'description2Tooltip': description2Tooltip}
    ...
    def __getTaskIcon(self, iconNum):
        ...
        if iconNum == 1:
            ...
            parameter = b'xp' if parameter == _op.ORIGINALXP else parameter
            maxOrSum = b'max' if method == _cm.MAX else b'sum'
...
class EventHeader(object):
    ...
    def getParticipateInfo(self):
        ...
        if event.isFinished():
            ...
            result = {b'participate': participate, b'participateTooltip': participateTooltip, 
               b'buttonLabel': buttonLabel, 
               b'buttonTooltip': buttonTooltip, 
               b'isButton': isButton, 
               b'isButtonEnabled': isButtonEnabled, 
               b'eventID': (event.getEventID()), 
               b'title': title, 
               b'isButtonRegistration': isButtonRegistration, 
               b'isButtonRegistrationEnabled': isButtonRegistrationEnabled, 
               b'titleTooltip': titleTooltip, 
               b'buttonRegistrationTooltip': buttonRegistrationTooltip, 
               b'buttonRegistrationLabel': buttonRegistrationLabel}
            return result


BONUS_PRIORITY = (b'gold', b'crystal', b'tokens', b'battleToken', b'vehicles', b'badgesGroup', b'dossier', b'lootbox', b'credits', b'items', b'goodies', b'customizations', b'tankmen', b'premium', b'slots', b'berths')

def _readEventBoardsRewards(rewards):
    badgesGroupData = rewards.pop(b'badgesGroup', None)
    if badgesGroupData is not None:
        return {b'badgesGroup': (int(badgesGroupData[b'value']))}
    else:
        return {}
...
class CustomXMLGenerator(xmltodict.XMLGenerator):
    ...
    def __init__(self, out=None, encoding=b'iso-8859-1', short_empty_elements=False):
        xmltodict.XMLGenerator.__bases__.__init__(self)
        buf = BufferedIOBase()
        buf.writable = lambda : True
        buf.write = out.write
        ioWrapper = TextIOWrapper(buf, encoding=encoding, errors=b'xmlcharrefreplace', newline=b'\n')
...
def convertRewardsDictToBonusObjects(dictData, key=b'rewards'):
    try:
        bonuses = _readEventBoardsRewards(dictData[key])
        xmlData = xmltodict.unparse({key: (dictData[key])}, xml_generator_cls=CustomXMLGenerator).encode(b'utf-8')
        ...
        LOG_ERROR(b'WGELEN: Failed to parse rewards data!')
        return []
...
def checkEventExist(method):
    ...
    def methodWrapper(self, eventID, *args):
        if self.eventsController.getEventsSettingsData().getEvent(eventID) is not None:
            ...
            LOG_DEBUG(b'WGELEN: Event with id "%s" does not exist.' % eventID)
