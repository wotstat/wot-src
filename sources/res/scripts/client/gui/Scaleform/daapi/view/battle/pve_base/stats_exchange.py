...
class PveTeamsSortedIDsComposer(BiSortedIDsComposer):
    ...
    def __init__(self):
        super(PveTeamsSortedIDsComposer, self).__init__(left=AllySortedIDsComposer(voField=b'leftItemsIDs', sortKey=PveVehicleInfoSortKey), right=EnemySortedIDsComposer(voField=b'rightItemsIDs', sortKey=PveVehicleInfoSortKeyEnemyList))
        return
...
class PveVehicleInfoComponent(VehicleInfoComponent):
    ...
    def addVehicleInfo(self, vInfoVO, overrides):
        ...
        self._data.update({b'prestigeMarkId': 0, 
           b'prestigeLevel': 0, 
           b'teamPanelMode': (vInfoVO.teamPanelMode), 
           b'highlight': False, 
           b'showFrags': True, 
           b'showVehicleTypeIcon': False, 
           b'countLives': livesCnt})
        ...
        if settingsCtrl is not None:
            ...
            if settings is not None and vInfoVO.isEnemy() and canHighlight:
                self._data[b'highlight'] = settings.highlightElite
            settings = settingsCtrl.getSettings(WidgetType.ALLY_LIST)
            if settings is not None:
                self._data[b'showFrags'] = settings.showFrags
                self._data[b'showVehicleTypeIcon'] = settings.showVehicleTypeIcon
        ...
        if data:
            ...
            if settingsCtrl is not None:
                ...
                if settings is not None and not settings.showSpottedIcon:
                    data[b'spottedStatus'] = VehicleSpottedStatus.DEFAULT
        return data
