...
class EarlyAccessVehiclePreview(VehiclePreview):
    ...
    __BOTTOM_PANEL_FLASH_PATH = b'net.wg.gui.lobby.vehiclePreview.bottomPanel.VPBottomPanelEarlyAccess'
    ...
    def __init__(self, ctx=None):
        ...
        self.__isFromVehicleView = ctx.get(b'isFromVehicleView', False)
        self.__vehicleCD = ctx.get(b'itemCD', None)
    ...
    def _getData(self):
        ...
        result[b'showCloseBtn'] = not self.__isFromVehicleView
        return result
    ...
    def _getBackBtnLabel(self):
        if (self.__isFromVehicleView or self._backAlias == VIEW_ALIAS.LOBBY_HANGAR) and not self.__earlyAccessController.isPaused():
            return backport.text(R.strings.early_access.vehiclePreview.backToVehicleScreen())
        return super(EarlyAccessVehiclePreview, self)._getBackBtnLabel()

    def _getExitEvent(self):
        exitEvent = super(EarlyAccessVehiclePreview, self)._getExitEvent()
        exitEvent.ctx.update({b'isFromVehicleView': (self.__isFromVehicleView)})
        return exitEvent
