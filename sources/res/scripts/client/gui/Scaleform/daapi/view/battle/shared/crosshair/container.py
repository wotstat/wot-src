...
class AutoloaderBoostSoundEvents(object):
    __slots__ = ()
    __EVENTS = {(AUTOLOADERBOOSTVIEWSOUNDS.START): b'gun_rld_automat_reloading_boost_start', 
       (AUTOLOADERBOOSTVIEWSOUNDS.PROGRESS): b'gun_rld_automat_reloading_boost_progress', 
       (AUTOLOADERBOOSTVIEWSOUNDS.MAX): b'gun_rld_automat_reloading_boost_max'}
    ...
    def play(state):
        ...
        if eventName:
            ...
            _logger.error(b"Autoloader boost events map do not have state '%r'", state)
        return
...
class CrosshairPanelContainer(ExternalFlashComponent, CrosshairPanelContainerMeta):
    ...
    def createGunMarkers(self, markersInfo, vehicleInfo):
        if self.__gunMarkers is not None:
            LOG_WARNING(b'Set of gun markers is already created.')
            return
        else:
    ...
    def invalidateGunMarkers(self, markersInfo, vehicleInfo):
        if self.__gunMarkers is None:
            LOG_WARNING(b'Set of gun markers is not created')
            return
        else:
    ...
    def __handleRoleHintToggled(self, event):
        self.__toggleFade(event.ctx.get(b'isShown', False))
        return
    ...
    def __exceptionalFadeIn(self):
        ...
        _logger.error(b'as_isFadedS must be called by GameEvent.ROLE_HINT_TOGGLE in __handleRoleHintToggled')
        return
    ...
    def __setGunMarkers(self, gunMarkers):
        ...
        LOG_DEBUG(b'Present view settings of gun markers', viewSettings)
        for item in viewSettings:
            ...
            if self.as_createGunMarkerS(item.viewID, item.linkage, item.name):
                ...
                LOG_DEBUG(b'Gun marker has been created', item)
            else:
                LOG_ERROR(b'Gun marker can not be created', item)
    ...
    def __clearGunMarkers(self):
        if self.__gunMarkers is None:
            ...
            viewSettings = self.__gunMarkers.getViewSettings()
            LOG_DEBUG(b'Previous view settings of gun markers', viewSettings)
            for item in viewSettings:
                if not item.hasView:
                    continue
                if self.as_destroyGunMarkerS(item.name):
                    self.__gunMarkers.removeView(self.component, item.name)
                    LOG_DEBUG(b'Gun marker has been destroyed', item)
