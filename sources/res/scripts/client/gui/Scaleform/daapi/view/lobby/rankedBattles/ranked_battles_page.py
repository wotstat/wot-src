...
class RankedMainPage(LobbySubView, RankedBattlesPageMeta):
    ...
    def _getSelectedIdx(self, menuItems):
        for idx, item in enumerate(menuItems):
            if item[b'id'] == self._selectedItemID and item.get(b'enabled', True):
                return idx

        return 0

    def _processContext(self, ctx):
        self._selectedItemID = ctx.get(b'selectedItemID', self._selectedItemID)
        if self._selectedItemID == RANKEDBATTLES_CONSTS.RANKED_BATTLES_INFO_ID and ctx.get(b'showedFromWeb', False):
            stateFlags = self.__getShowStateFlags()
            stateFlags[b'isRankedWelcomeViewShowed'] = True
...
class RankedMainSeasonOffPage(RankedMainPage):
    ...
    def _processContext(self, ctx):
        ...
        self.__prevSeason = ctx[b'prevSeason']
    ...
    def _updateMenuItems(self, isRankedShopEnabled, yearLBState, isYearRewardEnabled, yearLBSize):
        ...
        self.as_setDataS({b'menuItems': menuItems, 
           b'selectedIndex': (self._getSelectedIdx(menuItems))})
        return
...
class RankedMainSeasonOnPage(RankedMainPage):
    ...
    def _processContext(self, ctx):
        ...
        self.__selectedRewardsItemID = ctx.get(b'rewardsSelectedTab', None)
        return
