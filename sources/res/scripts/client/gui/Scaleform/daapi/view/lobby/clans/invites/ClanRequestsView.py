...
class ClanRequestsView(ClanRequestsViewMeta):
    ...
    def _makeHeaders(self):
        return [
         ...
         self._packHeaderColumnData(b'avgXP', b'', 98, CLANS.CLANINVITESWINDOW_TOOLTIPS_TABLE_REQUESTS_AVGXP, RES_ICONS.MAPS_ICONS_STATISTIC_AVGEXP24),
...
class RequestDataProvider(ClanInvitesAbstractDataProvider):
    ...
    def _makeVO(self, item, extraData):
        ...
        return {b'dbID': (item.getDbID()), 
           ...
           b'avgXP': (formatField(getter=item.getBattleXpAvg, formatter=backport.getIntegralFormat)), 
