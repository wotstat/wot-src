...
class TwitchAwardsComposer(CurtailingAwardsComposer):
    ...
    def _packMergedBonuses(self, mergedBonuses, size=AWARDS_SIZES.SMALL):
        ...
        imgSource = imgPath.rsplit(b'/', maxsplit=1)[-1]
    ...
    def _getPreformattedTankwomanBonus(self, operation):
        if not operation.isFullCompleted():
            ...
            if total:
                ...
                return PreformattedBonus(bonusName=self._TANKWOMAN_BONUS, label=b'%s / %s' % (formatCompleteCount(current, total), str(total)), images=images, labelFormatter=text_styles.main)
        return
