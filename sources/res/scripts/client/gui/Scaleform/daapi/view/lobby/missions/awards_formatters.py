...
def formatShortData(bonus, size):
    return {b'name': (bonus.userName), 
       b'label': (bonus.getFormattedLabel()), 
       b'imgSource': (bonus.getImage(b'tooltip') or bonus.getImage(size))}
...
class CurtailingAwardsComposer(QuestsBonusComposer):
    ...
    def _getDisplayAndMergedBonuses(self, preformattedBonuses):
        if self._isMergeAllow() and len(preformattedBonuses) > self._displayedRewardsCount:
            ...
            return (
             preformattedBonuses[:sliceIdx], preformattedBonuses[sliceIdx:])
        return (
         preformattedBonuses, [])
    ...
    def _packBonus(self, bonus, size=AWARDS_SIZES.SMALL):
        ...
        return {b'label': (bonus.getFormattedLabel()), 
           ...
           b'isWulfTooltip': (bonus.isWulfTooltip), 
           b'specialAlias': (bonus.specialAlias), 
           b'specialArgs': (bonus.specialArgs), 
           b'hasCompensation': (bonus.isCompensation), 
           b'hasAnimation': False, 
           b'compensationReason': compensationReason, 
           b'align': (bonus.align), 
           b'highlightType': (bonus.getHighlightType(size)), 
           b'overlayType': (bonus.getOverlayType(size)), 
           b'highlightIcon': (bonus.getHighlightIcon(size)), 
           b'overlayIcon': (bonus.getOverlayIcon(size)), 
           b'gap': (bonus.getGap(size)), 
           b'padding': (bonus.getPadding(size))}

    def _packMergedBonuses(self, mergedBonuses, size=AWARDS_SIZES.SMALL):
        mergedBonusCount = len(mergedBonuses)
        return {b'label': (text_styles.stats(i18n.makeString(QUESTS.MISSIONS_AWARDS_MERGED, count=mergedBonusCount))), 
           b'imgSource': (RES_ICONS.getBonusIcon(size, b'default')), 
           b'isSpecial': True, 
           b'specialAlias': (TOOLTIPS_CONSTANTS.ADDITIONAL_AWARDS), 
           b'specialArgs': (self._getShortBonusesData(mergedBonuses, size))}

    @classmethod
    def _getShortBonusesData(cls, preformattedBonuses, size=AWARDS_SIZES.SMALL):
        bonuses = []
        for bonus in preformattedBonuses:
            shortData = {b'name': (bonus.userName), 
               b'label': (bonus.getFormattedLabel()), 
               b'imgSource': (bonus.getImage(size)), 
               b'highlightIcon': (bonus.getHighlightIcon(size)), 
               b'overlayIcon': (bonus.getOverlayIcon(size))}
...
class TwitchAwardsComposer(CurtailingAwardsComposer):
    ...
    def _packBonus(self, bonus, size=AWARDS_SIZES.SMALL):
        ...
        image = imagePath.split(b'/')[-1] if imagePath else b''
        packed[b'imgSource'] = image
        name = bonus.bonusName
        if name == b'dossier':
            name = b'dossier_achievement'
        packed[b'name'] = name
        return packed

    def _packMergedBonuses(self, mergedBonuses, size=AWARDS_SIZES.SMALL):
        mergedBonusCount = len(mergedBonuses)
        imgPath = RES_ICONS.getBonusIcon(size, b'default')
        imgSource = imgPath.split(b'/')[-1]
        return {b'name': b'groups', 
           b'imgSource': imgSource, 
           b'label': (i18n.makeString(QUESTS.MISSIONS_AWARDS_MERGED, count=mergedBonusCount)), 
           b'isSpecial': True, 
           b'specialAlias': (TOOLTIPS_CONSTANTS.ADDITIONAL_AWARDS), 
           b'specialArgs': (self._getShortBonusesData(mergedBonuses, size))}
...
class BattlePassAwardsComposer(CurtailingAwardsComposer):
    ...
    def _packMergedBonuses(self, mergedBonuses, size=AWARDS_SIZES.SMALL):
        ...
        for bonus in mergedBonuses:
            if bonus.postProcessTags != b'blueprints':
                blueprints = False

        if blueprints:
            return {b'label': b'', 
               b'imgSource': (RES_ICONS.getBonusIcon(size, b'blueprint_tube')), 
               b'isSpecial': True, 
               b'specialAlias': (TOOLTIPS_CONSTANTS.ADDITIONAL_AWARDS), 
               b'specialArgs': (self._getShortBonusesData(mergedBonuses, size))}
        label = b''
...
class RawLabelBonusComposer(QuestsBonusComposer):
    ...
    def _packBonus(self, bonus, size=AWARDS_SIZES.SMALL):
        ...
        res.update({b'label': (bonus.label)})
        return res
...
class AwardsWindowComposer(CurtailingAwardsComposer):
    ...
    def _getShortBonusesData(cls, preformattedBonuses, size=AWARDS_SIZES.SMALL):
        ...
        for bonus in preformattedBonuses:
            shortData = {b'name': (bonus.userName), 
               b'label': (bonus.getFormattedLabel()), 
               b'imgSource': (bonus.getImage(AWARDS_SIZES.SMALL))}
            bonuses.append(shortData)
...
class BonusNameQuestsBonusComposer(PackRentVehiclesAwardComposer):
    ...
    def _packBonus(self, bonus, size=AWARDS_SIZES.SMALL):
        ...
        packBonus[b'bonusName'] = bonus.bonusName
        return packBonus
...
def _getBonusesWithModifyTokens(bonuses, freeTokenName, addTokensCount, hasPawned):
    if addTokensCount > 0:
        ...
        if not hasFreeTokens:
            value = {freeTokenName: {b'count': addTokensCount}}
            newBonuses.append(FreeTokensBonus(value, ctx=ctx, hasPawned=hasPawned))
        bonuses = newBonuses
    return bonuses
...
class PersonalMissionsAwardComposer(CurtailingAwardsComposer):
    ...
    def getFormattedBonuses(self, bonuses, size=AWARDS_SIZES.SMALL, gap=0, isObtained=False, obtainedImage=b'', obtainedImageOffset=0):
        preformattedBonuses = self.getPreformattedBonuses(bonuses)
    ...
    def _packBonuses(self, preformattedBonuses, size, gap=0, isObtained=False, obtainedImage=b'', obtainedImageOffset=0):
        result = []
    ...
    def _packBonus(self, bonus, size=AWARDS_SIZES.SMALL, gap=0, isObtained=False, obtainedImage=b'', obtainedImageOffset=0):
        ...
        data.update(tokensPawnedImage=RES_ICONS.MAPS_ICONS_LIBRARY_LOCKINDICATOR if bonus.areTokensPawned else b'')
        return data
...
def _getTankwomansCountInOperation(operation):
    ...
    for quest in operation.getFinalQuests().itervalues():
        tankmen = quest.getBonuses(b'tankmen')
        if tankmen:
            total += 1
...
class MainOperationAwardComposer(PersonalMissionsAwardComposer):
    ...
    def _getBonuses(self, operation):
        ...
        bonuses = baseQuest.getBonuses(b'dossier', ctx=ctx)
...
class TooltipOperationAwardComposer(MainOperationAwardComposer):
    _BONUSES_ORDER = (b'vehicles', b'tankwoman', b'customizations', b'dossier')
    ...
    def _getPreformattedTankwomanBonus(self, operation):
        if not operation.isFullCompleted():
            ...
            images = dict((size, RES_ICONS.getBonusIcon(size, b'tankwoman')) for size in AWARDS_SIZES.ALL())
            return PreformattedBonus(bonusName=self._TANKWOMAN_BONUS, label=b'%s / %s' % (currentStr, str(total)), images=images, labelFormatter=text_styles.main)
        else:
            return


class TooltipPostponedOperationAwardComposer(TooltipOperationAwardComposer):
    POSTPONE_PERSONAL_MISSION_TOKEN = b'pm2_t%s_early_access'
