...
class EpicBattleCarouselDataProvider(HangarCarouselDataProvider):
    ...
    def __buildSeparatorItems(self):
        ...
        for level in range(MIN_VEHICLE_LEVEL, MAX_VEHICLE_LEVEL + 1):
            self.__separatorItems.append({b'levelInfo': (self.__getLevelInfo(level)), 
               b'clickEnabled': (isVehLevelUnlockableInBattle(level))})
    ...
    def _buildVehicle(self, vehicle):
        ...
        if state == Vehicle.VEHICLE_STATE.UNSUITABLE_TO_QUEUE:
            result[b'lockedTooltip'] = makeTooltip(backport.text(resShortCut.header()), backport.text(resShortCut.body()))
        if state == Vehicle.VEHICLE_STATE.WILL_BE_UNLOCKED_IN_BATTLE:
            result[b'unlockedInBattle'] = True
    ...
    def __getLevelInfo(self, level):
        if isVehLevelUnlockableInBattle(level):
            ...
            return {b'level': level, 
               b'isCollapsed': (self.__isHintsShown()), 
               b'isCollapsible': True, 
               b'infoText': levelInfoText}
        return {b'level': level, 
           b'isCollapsed': True, 
