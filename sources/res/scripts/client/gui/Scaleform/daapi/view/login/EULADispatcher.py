...
EULA_TEMPLATES_FILE_PATH = b'gui/EULA_templates.xml'
EULA_FILE_PATH = b'text/EULA.xml'
...
class EULADispatcher(EventSystemEntity):
    ...
    def _populate(self):
        ...
        if self.__eulaText:
            ...
            self.fireEvent(LoadViewEvent(SFViewLoadParams(VIEW_ALIAS.EULA_FULL if isShowFullEULA else VIEW_ALIAS.EULA), ctx={b'text': (self.__eulaText)}), EVENT_BUS_SCOPE.LOBBY)
        return
    ...
    def processLicense(self, callback=None):
        ...
        self.serverVersion = int(filters[b'version'])
        self.isShow = False
    ...
    def __saveVersionFile(self):
        ...
        version = {b'version': (self.EULAVersion.xmlVersion)}
    ...
    def __readEULAShort(self):
        return makeHtmlString(b'html_templates:lobby/dialogs', b'eula', {b'eulaURL': (GUI_SETTINGS.eula.url.format(getClientLanguage()))})
    ...
    def __readEULAFull(self):
        if not GUI_SETTINGS.eula.full:
            ...
            text = []
            if dSection is None:
                LOG_WARNING(b'Can not open file:', EULA_FILE_PATH)
                self.__showLicense = False
                return b''
            try:
...
class _LinkTemplate(_TagTemplate):
    ...
    def execute(self, section, processor, result):
        name = section[b'name'].asWideString if section.has_key(b'name') else section.asWideString
        if section.has_key(b'url'):
            if section[b'url'].asWideString == b'#eulaUrl':
                lng = getClientLanguage()
                url = GUI_SETTINGS.eula.url.format(lng)
            else:
                url = section[b'url'].asWideString
        else:
...
class _LicenseXMLProcessor(object):
    ...
    def __loadTemplates(self):
        ...
        if dSection is None:
            LOG_ERROR(b'Can not open file:', EULA_TEMPLATES_FILE_PATH)
        for tagName, child in dSection.items():
            className = child.readString(b'class')
            if className is None:
                continue
            clazz = globals().get(className)
            if clazz is None:
                LOG_ERROR(b'Not found class:', clazz)
                continue
            args = []
            argsSection = child[b'args'] if child.has_key(b'args') else []
