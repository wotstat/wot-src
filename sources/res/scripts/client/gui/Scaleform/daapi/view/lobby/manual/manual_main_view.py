...
class ManualMainView(ManualViewBase, ManualMainViewMeta):
    ...
    def __init__(self, ctx=None):
        ...
        self.__backCallback = self._ctx.get(b'backCallback')
        return
    ...
    def addCounter(param):
        ...
        for chapter in chapters:
            length = len(chapter)
            param[index][b'counter'] = str(length) if length > 0 else b''
            index += 1
    ...
    def onChapterOpenedS(self, chapterIndex):
        _logger.debug(b'ManualMainView. Chapter selected: %s', chapterIndex)
        shared_events.openManualPage(chapterIndex)
        return

    def onBackButton(self):
        if self.__backCallback is None:
            _logger.warning(b'ManualMainView. Trying to go back with None backCallback')
            return
        else:
            self.__backCallback()
            return
    ...
    def _populate(self):
        ...
        ctx = self._ctx
        if ctx and b'chapterIndex' in ctx:
            self.manualController.showChapterView(ctx[b'chapterIndex'], ctx[b'pageIndex'])
        return
