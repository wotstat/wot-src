...
class RankedBattlesRewardsView(RankedBattlesRewardsMeta, IResetablePage):
    ...
    def _populate(self):
        ...
        self.as_setCountersS([
         {b'componentId': (_RBC.RANKED_BATTLES_REWARDS_YEAR_ID), 
            b'count': (main_page_vos.getBubbleLabel(AccountSettings.getCounters(RANKED_AWARDS_COUNTER)))}])
        return
...
_DEFAULT_STYLED_VEHICLES = (b'uk:GB91_Super_Conqueror', b'china:Ch41_WZ_111_5A', b'ussr:R148_Object_430_U', b'japan:J20_Type_2605')
...
class RankedBattlesRewardsLeaguesView(RankedBattlesRewardsLeaguesMeta, IResetablePage):
    ...
    def _backToLeaguesCallback(cls):
        cls.__rankedController.showRankedBattlePage(ctx={b'selectedItemID': (_RBC.RANKED_BATTLES_REWARDS_ID), 
           b'rewardsSelectedTab': (RANKEDBATTLES_ALIASES.RANKED_BATTLES_REWARDS_LEAGUES_UI)})
        return
    ...
    def _populate(self):
        ...
        self.as_setRewardsS({b'leagues': data, 
           b'description': descr})
        return
    ...
    def __getLeaguesData(self):
        ...
        for data in leaguesRewardsData:
            leagueID = data[b'league']
            awards = data[b'awards']
            badgeBonus = findAndPreformatBonus(awards, b'dossier')
            styleBonus = findAndPreformatBonus(awards, b'customizations')
            if leagueID and styleBonus and badgeBonus:
                ...
                self.__styleDescriptions[styleID] = backport.text(R.strings.ranked_battles.rewardsView.tabs.leagues.description.dyn(b'league%s' % leagueID)())
                ...
                _logger.error(b'Invalid awards or leagueID in quests')
    ...
    def __showStylePreview(self, styleID):
        ...
        styleDescr = self.__styleDescriptions.get(styleID, b'')
...
class RankedBattlesRewardsYearView(RankedBattlesRewardsYearMeta, IResetablePage):
    ...
    def __getAwardsData(self, points, rewardingComplete, isAwarded):
        ...
        for awardName in YEAR_AWARDS_ORDER:
            ...
            pointsVO = {}
            if not isAwarded:
                pointsVO = {b'text': (text_styles.promoTitle(minPoints)), b'image': (backport.image(R.images.gui.maps.icons.rankedBattles.ranked_point_28x28()))}
            data.append({b'id': awardName, 
               b'status': status, 
