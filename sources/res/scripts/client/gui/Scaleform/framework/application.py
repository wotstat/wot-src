...
_WULF_COMPONENT_CLASS = b'WulfFlashComponent'

class DAAPIRootBridge(object):
    __slots__ = (b'__pyScript', b'__rootPath', b'__initCallback', b'__isInited')

    def __init__(self, rootPath=b'root', initCallback=b'registerApplication'):
    ...
    def _onSWFInited(self):
        ...
        if root:
            ...
            _logger.error(b'Flash root is not found: %s', self.__rootPath)
        return
...
class AppEntry(FlashComponentWrapper, ApplicationMeta):
    ...
    def afterCreate(self):
        ...
        _logger.debug(b'AppEntry.afterCreate: %s', self.__ns)
    ...
    def beforeDelete(self):
        _logger.debug(b'AppEntry.beforeDelete: %s', self.__ns)
    ...
    def updateScale(self):
        ...
        self.settingsCore.options.getSetting(b'interfaceScale').setSystemValue(index)
        return
    ...
    def fireEventAfterInitialization(self, event, scope=EVENT_BUS_SCOPE.DEFAULT):
        if self.__initialized:
            ...
            self.__firingsAfterInit[event.eventType] = {b'event': event, b'scope': scope}
        return

    def setBackgroundAlpha(self, value, notSilentChange=True):
        self.movie.backgroundAlpha = value
        if notSilentChange:
            self.fireEvent(GameEvent(GameEvent.ON_BACKGROUND_ALPHA_CHANGE, {b'alpha': value}), EVENT_BUS_SCOPE.GLOBAL)
        return

    def getBackgroundAlpha(self):
        return self.movie.backgroundAlpha

    def printObjectsReport(self, gc=True, flags=16655, movieName=b''):
    ...
    def _setup(self):
        raise NotImplementedError(b'App._setup must be overridden')
        return

    def _loadWaiting(self):
        raise NotImplementedError(b'App._loadWaiting must be overridden')
        return
    ...
    def __onAppResolutionChanged(self, event):
        ...
        if b'scale' not in ctx:
            ...
            return
        self.updateStage(ctx[b'width'], ctx[b'height'], ctx[b'scale'])
        return
