...
class ToolTip(ToolTipMgrMeta):
    ...
    def getTypedTooltipDefaultBuildArgs(self, tooltipType):
        ...
        return tuple(reversed([(argName, defaultValue) for argName, defaultValue in itertools.izip_longest(reversed(spec.args), reversed(spec.defaults or [])) if argName != b'self']))
    ...
    def onCreateComplexTooltip(self, tooltipID, stateType):
        if self._areTooltipsDisabled:
            ...
            id = _id_generator.next()
            region = (b'Complex tooltip {} {}').format(tooltipID, id)
