...
class SCOPE_TYPE(object):
    ...
    WINDOW_VIEWED_MULTISCOPE = b'WindowViewed'


class SimpleScope(object):
    __slots__ = (b'__scopeType', b'__parentScope', b'__dependencedScopes')

    def __init__(self, scopeType, parentScope):
        super(SimpleScope, self).__init__()
        if scopeType is None:
            raise SoftException(b'ScopeType cannot be None!')
    ...
    def __repr__(self):
        return (b'{}[{}]=[scopeType={}, parentScope={}]').format(self.__class__.__name__, hex(id(self)), self.__scopeType, self.__parentScope)

    def _setParentScope(self, parentScope):
        if self.__parentScope is not None:
            raise SoftException(b'parent scope can be set at one time only!')
...
class MultipleScope(SimpleScope):
    ...
    def __init__(self, scopeType, parentScopes):
        if not parentScopes:
            raise SoftException(b'parentScopes list can not be empty')
        if len(parentScopes) == 1:
            raise SoftException(b'MultipleScope can not have an only one parent')
        ...
        while i < len(parentScopes):
            if parentScopes[i] is not None:
                ...
                raise SoftException(b'One of parents is None!')
            i += 1
