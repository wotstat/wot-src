...
class PackageBusinessHandler(object):
    ...
    __slots__ = (b'_listeners', b'_scope', b'_app', b'_appNS')
    ...
    def loadViewWithGenName(self, alias, parent=None, *args, **kwargs):
        self._app.loadView(SFViewLoadParams(alias, name=(b'rw{0}').format(self.__counter.next()), parent=parent), *args, **kwargs)
        return
...
class PackageImporter(object):
    __slots__ = (b'_aliases', b'_handlers', b'_contextMenuTypes')
    ...
    def unload(self, seq=None):
        ...
        for path in seq:
            _logger.debug(b'Tries to unload GUI package "%s"', path)
    ...
    def _loadPackage(self, path, arenaGuiType, isExtention):
        if self.isPackageLoaded(path):
            return
        _logger.debug(b'Tries to load GUI package "%s"', path)
        ...
        try:
            ...
            _logger.exception(b'Package "%s" can not be loaded', path)
            raise SoftException((b'Package {0} does not have method getContextMenuHandlers').format(path))
        ...
        try:
            ...
            _logger.exception(b'Package "%s" can not be loaded', path)
            raise SoftException((b'Package {0} does not have method getBusinessHandlers').format(path))
        ...
        for handler in handlers:
            if not isinstance(handler, PackageBusinessHandler):
                ...
                raise SoftException((b'Package {0} has invalid business handler {1}').format(path, handler.__class__.__name__))
            handler.init()
