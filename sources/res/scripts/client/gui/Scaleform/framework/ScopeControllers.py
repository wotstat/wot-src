...
class ScopeController(DisposableEntity):
    ...
    def isViewLoading(self, pyView=None, key=None):
        ...
        if key is not None:
            ...
            raise SoftException(b'pyView or pyView alias can not be None!')
            return
    ...
    def addLoadingView(self, pyView, addAsGlobal):
        if pyView is None:
            raise SoftException(b'pyView can not be None!')
        if addAsGlobal:
            pyView.onDispose += self._handleViewDispose
    ...
    def _getScopeControllerForScope(self, scope):
        ...
        if existingScopeController is None:
            ...
            if outcome is None:
                raise ScopeControllerError((b'Could not to construct scopeController for {}').format(scope))
            return outcome
    ...
    def __searchOwnerAndCreateControllerChain(self, scope):
        ...
        if not ownScopes:
            raise ScopeControllerError((b'Could not to construct scopeController for {} - own scopes can not be found').format(scope))
        newController = None
        for ownScope in ownScopes:
            ...
            if ownController is not None:
                ...
                raise ScopeControllerError(b'Could not create a controllers chain!')
    ...
    def __repr__(self):
        return (b'{}[{}]=[type=[{}], mainView=[{}], views=[{}], loadingViews=[{}], child=[{}]]').format(self.__class__.__name__, hex(id(self)), self.__currentType, self.__mainView, self.__views, self.__loadingViews, self.__subControllers)
