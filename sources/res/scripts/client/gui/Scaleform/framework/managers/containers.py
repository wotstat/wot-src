...
class ExternalCriteria(object):
    ...
    def find(self, name, obj):
        raise NotImplementedError(b'ExternalCriteria.find must be implemented')
        return
...
class ViewContainer(object):
    __slots__ = (b'__layer', b'__child', b'_views', b'_loadingViews', b'__containerManager', b'__parentContainer')
    ...
    def __repr__(self):
        return (b'{}[{}]=[layer=[{}], views=[{}], loadingViews=[{}], child=[{}]]').format(self.__class__.__name__, hex(id(self)), self.__layer, self._views, self._loadingViews, self.__child)
    ...
    def addView(self, pyView):
        if not self.isViewCompatible(pyView):
            _logger.error(b'Cannot add view %r to container %r. Incompatible view type.', pyView, self)
            return False
    ...
    def removeView(self, pyView):
        ...
        if container is None:
            _logger.warning(b'There is no container "%d" to remove view "%r"!', layer, pyView)
            return False
        else:
    ...
    def addLoadingView(self, pyView):
        if not self.isViewCompatible(pyView):
            _logger.error(b'Cannot add loading view %r to container %r. Incompatible view type.', pyView, self)
            return False
    ...
    def removeLoadingView(self, pyView):
        ...
        container = self.findContainer(layer)
        if container is None:
            _logger.warning(b'There is no container %d to remove loading view %r!', layer, pyView)
            return False
        else:
    ...
    def getView(self, criteria):
        if isinstance(criteria, dict):
            ...
            _logger.error(b'Criteria is invalid: %r', criteria)
            view = None
        return view
    ...
    def _addView(self, pyView):
        ...
        if pyView.isCreated():
            self.__addViewSubContainers(pyView)
        _logger.debug(b'View with key %s has been added to container %r', viewKey, self)
        return True
    ...
    def __addViewSubContainers(self, pyView):
        for subContainerSettings in pyView.getSubContainersSettings():
            ...
            _logger.debug(b'Sub container %r is created for view %r.', subContainerSettings, str(pyView))
    ...
    def __findByExCriteria(self, criteria):
        ...
        return findFirst(find, iteritems(self._views), (b'', None))[1]
...
class ChainItem(object):
    __slots__ = (b'loadParams', b'args', b'kwargs')
    ...
    def __repr__(self):
        return (b'{}[{}]=[loadParams={}, args={}, kwargs={}').format(self.__class__.__name__, hex(id(self)), self.loadParams, self.args, self.kwargs)
...
class _LoadingChain(object):
    ...
    def __repr__(self):
        return (b'{}[{}]=[currentView={}, queue={}, ').format(self.__class__.__name__, hex(id(self)), self.__currentView, self.__queue)
    ...
    def run(self):
        if self.__currentItem is not None:
            _logger.warning(b'Chain of views loading is already running: %r.', self)
            return
        else:
            self.__loadNextView()
            return
        ...
        while not stop:
            if not self.__queue:
                ...
                if self.__chainManager is not None:
                    ...
                    _logger.warning(b'Could not load view with key %s. Chain manager is not specified.', viewKey)
    ...
    def __onViewDisposed(self, pyView):
        if pyView == self.__currentView:
            ...
            _logger.warning(b'Got onDispose event for view %r. Unexpected case.', pyView)
        return
    ...
    def __onViewCreated(self, pyView):
        if pyView == self.__currentView:
            ...
            _logger.warning(b'Got onCreated event for view %r. Unexpected case.', pyView)
        return
...
class ContainerManager(ContainerManagerMeta, IContainerManager):
    ...
    def _dispose(self):
        ...
        for layer, layerName in enumerate(LAYER_NAMES.LAYER_ORDER):
            ...
            if container is not None:
                _logger.debug(b'Destroy container %s (%r)', layerName, container)
                container.destroy()
    ...
    def destroyViews(self, alias, name=None):
        ...
        for view in views:
            if not view.isDisposed():
                _logger.debug(b'The view %r will be destroyed...', view)
                view.destroy()
    ...
    def loadChain(self, chainItems):
        ...
        if self.__chainMng.addChain(chain):
            ...
            _logger.warning(b'Could not add a new chain. Loading of chain [%r] is canceled.', chainItems)
            chain.destroy()
        return
    ...
    def load(self, loadParams, *args, **kwargs):
        ...
        if viewLoadingItem is not None:
            _logger.debug(b'View with key %s is already loading. item=[%r]', viewKey, viewLoadingItem)
            ...
            if loadParams.loadMode == ViewLoadMode.DEFAULT:
                ...
                _logger.warning(b'Unsupported load mode %r. View loading will be skipped.', loadParams)
                view = None
        else:
            ...
            if view is None:
                ...
                if view is None:
                    chain = self.__chainMng.getChainByViewKey(viewKey)
                    if chain is not None:
                        _logger.warning(b'View with loadParams=%r is in the loading chain %r. The request will be skipped.', loadParams, chain)
                    else:
                        _logger.debug(b'Load view with loadParams=%r. Loader=[%r]', loadParams, self.__loader)
                    if loadParams.loadMode == ViewLoadMode.DEFAULT:
                    ...
                    _logger.debug(b'View with key %s (%r) is already pre-loaded.', viewKey, view)
                elif loadParams.loadMode == ViewLoadMode.DEFAULT:
                    _logger.debug(b'Load view with loadParams=%r from the cache. Cache=[%r]', loadParams, self.__viewCache)
                ...
                _logger.debug(b'View with key %s (%r) is already loaded.', viewKey, view)
                layer = view.layer
    ...
    def getView(self, layer, criteria=None):
        ...
        if container is not None:
            ...
            _logger.warning(b'Could not found container %s.', layer)
            return
    ...
    def registerViewContainer(self, layer, uniqueName):
        ...
        _logger.debug(b'A new container [layer=%d, name=%s] has been registered.', layer, uniqueName)
        return

    def unregisterViewContainer(self, layer):
        self.as_unregisterContainerS(layer)
        _logger.debug(b'The container [layer=%d] has been unregistered.', layer)
        return
    ...
    def __addLoadingView(self, pyView):
        ...
        if viewContainer is not None:
            ...
            _logger.warning(b'Loading of view %r is requested but the container %d is still not exist!', pyView, layer)
    ...
    def __showAndInitializeView(self, pyView):
        layer = pyView.layer
        if layer is None:
            _logger.error(b'Type of view is not defined. View %r will be destroyed.', pyView)
            pyView.destroy()
            return False
        else:
            status = False
            ...
            if container is not None:
                ...
                if container.addView(pyView):
                    ...
                    _logger.error(b'%r view cannot be added to container %r and will be destroyed.', pyView, container)
                    pyView.destroy()
            else:
                _logger.error(b'Layer %s of view %r is not supported or container has not been properly created', layer, pyView)
                pyView.destroy()
            return status
    ...
    def __addViewToCache(self, pyView):
        ...
        if view is not None:
            _logger.warning(b'The view with key %r is already in the cache.', pyView.key)
            if view != pyView:
                view.destroy()
                if self.__viewCache.addView(pyView):
                    _logger.debug(b'View %r has been added to the cache %r', pyView, self.__viewCache)
                else:
                    _logger.debug(b'Cannot add view %r in the cache %r', pyView, self.__viewCache)
    ...
    def __onViewLoaded(self, pyView, loadParams):
        ...
        if loadMode == ViewLoadMode.DEFAULT:
            if self.__scopeController.isViewLoading(pyView=pyView):
                ...
                _logger.debug(b'%r view loading is cancelled because its scope has been destroyed.', pyView)
                pyView.destroy()
        elif loadMode == ViewLoadMode.PRELOAD:
            self.__addViewToCache(pyView)
        else:
            _logger.warning(b'Unsupported load mode %s. View %r will be destroyed.', loadMode, pyView)
            pyView.destroy()
        return
