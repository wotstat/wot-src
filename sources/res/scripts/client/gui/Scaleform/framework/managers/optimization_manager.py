...
_PERMANENT_SETTING_ID = b''
_logger = logging.getLogger(__name__)

class OptimizationSetting(object):
    __slots__ = (b'__id', b'__isInvert')
...
class GraphicsOptimizationManager(GraphicsOptimizationManagerMeta):
    ...
    def registerOptimizationArea(self, x, y, width, height):
        if width < 0 or height < 0:
            _logger.warning(b'Optimization rect width and height can not be less than 0')
    ...
    def unregisterOptimizationArea(self, optimizationID):
        ...
        if optimizationID in self.__cache:
            ...
            LOG_ERROR(b'Graphics optimization ID is not found', optimizationID)
        return
    ...
    def __handleGuiVisibility(self, event):
        self.__isUiVisible = event.ctx[b'visible']
...
class ExternalFullscreenGraphicsOptimizationComponent(object):
    ...
    def __onAppResolutionChanged(self, event):
        app = self.__appLoader.getApp()
        ctx = event.ctx
        if app and self.__optimizationID is not None and b'width' in ctx and b'height' in ctx:
            app.graphicsOptimizationManager.updateOptimizationArea(self.__optimizationID, 0, 0, ctx[b'width'], ctx[b'height'])
        return
