...
class DAAPIModuleFactory(EntityFactory):
    ...
    def validate(self, settings):
        ...
        if BaseDAAPIModule not in getattr(settings.clazz, b'__mro__', tuple()):
...
class ViewFactory(DAAPIModuleFactory):
    ...
    def validate(self, settings):
        ...
        if View not in getattr(settings.clazz, b'__mro__', tuple()):
...
class EntitiesFactories(object):
    __slots__ = (b'__settings', b'__factories', b'__eventToAlias', b'__aliasToEvent', b'__layers')
