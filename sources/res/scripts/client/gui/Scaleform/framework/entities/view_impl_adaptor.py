...
class ViewImplAdaptor(DisposableEntity, ViewInterface):
    __slots__ = (b'__window', b'__loadID', b'__scope', b'__key', b'__layer')
    ...
    def __repr__(self):
        return (b'{}(key={})').format(self.__class__.__name__, self.__key)
    ...
    def setAlias(self, alias):
        raise SoftException(b'This method is not implemented')
        return
    ...
    def setView(self, view, parent=None):
        if self.__window is not None:
            _logger.exception(b'View has been already set! %r new value: %r', self, view)
    ...
    def __onStatusChanged(self, state):
        if state == WindowStatus.LOADED:
            if self.getState() == EntityState.CREATED:
                _logger.exception(b'View already created: %r', self)
            self.create()
    ...
    def __startToLoad(self):
        self.__loadID = None
        if self.__window is None:
            raise SoftException(b'Window is not defined.')
        self.__window.load()
        return
