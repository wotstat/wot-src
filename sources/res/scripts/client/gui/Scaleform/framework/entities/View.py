...
_ViewKey = namedtuple(b'_ViewKey', [b'alias', b'name'])
...
class ViewKey(_ViewKey):
    ...
    def __repr__(self):
        return (b'{}[alias={}, name={}]').format(self.__class__.__name__, self.alias, self.name)
...
class View(AbstractViewMeta, ViewInterface):
    ...
    def __repr__(self):
        return (b'{}[{}]=[key={}, scope={}, state={}]').format(self.__class__.__name__, hex(id(self)), self.key, self.__scope, self.getState())

    def __del__(self):
        _logger.debug(b'View deleted: %r', self)
        return
        ...
        if self.__settings is not None:
            if self.__settings.scope == ScopeTemplates.DYNAMIC_SCOPE:
                if scope != ScopeTemplates.DYNAMIC_SCOPE:
                    ...
                    raise SoftException((b'View.__scope cannot be a ScopeTemplates.DYNAMIC value. This value might have only settings.scope for {} view.').format(self.alias))
            else:
                raise SoftException((b'You can not change a non-dynamic scope. Declare ScopeTemplates.DYNAMIC in settings for {} view').format(self.alias))
        else:
            _logger.error(b'Can not change a current scope, until unimplemented __settings ')
        return
    ...
    def setSettings(self, settings):
        ...
        if settings is not None:
            ...
            _logger.error(b'settings can`t be None!')
        return
    ...
    def setUniqueName(self, name):
        if name is not None:
            ...
            _logger.debug(b'Unique name cannot be set to None: %r', self)
        return
    ...
    def setupContextHints(self, hintID, hintsArgs=None):
        if hintID is not None:
            ...
            if hintsData is not None:
                ...
                _logger.error(b'Hint layout is not defined %r', hintID)
        return

    def onFocusIn(self, alias):
        self.fireEvent(FocusEvent(FocusEvent.COMPONENT_FOCUSED, ctx={b'alias': alias}))
        return
    ...
    def _destroy(self):
        ...
        if self.__key.name and self.__key.alias:
            uniprof.exitFromRegion((b'Scaleform {} {}').format(self.__key.name, self.__uid))
