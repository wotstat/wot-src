...
class BaseDAAPIComponent(BaseDAAPIComponentMeta):
    ...
    def componentsSnapshot(self):
        return [(viewPy.flashObject, viewAlias, getattr(viewPy, b'componentsSnapshot', [])) for viewAlias, viewPy in viewitems(self.__components)]
    ...
    def registerFlashComponent(self, component, alias, *args):
        ...
        if not isinstance(componentPy, BaseDAAPIModule):
            _logger.error(b'Registered component %r should extend a BaseDAAPIModule class!', componentPy)
            return
        else:
            if alias in self.__components.keys():
                _logger.warning(b'Class with alias `%s` already registered in object %r. It will be rewritten.', alias, self)
    ...
    def unregisterFlashComponent(self, alias):
        ...
        try:
            ...
            _logger.critical(b'Could not unregister component because there is no registered component with such alias: %s', alias)
    ...
    def __unregisterPythonComponent(self, alias, viewPy):
        if viewPy not in self.__components.values():
            _logger.warning(b'There is no flash object registered in %r: %r', self, viewPy)
            return
        try:
