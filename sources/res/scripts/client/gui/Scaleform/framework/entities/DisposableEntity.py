...
class DisposableEntity(object):
    ...
    def create(self):
        if self.__lcState in (EntityState.UNDEFINED, EntityState.DISPOSED):
            ...
            _logger.debug(b'Create call is postponed for %r object. Disposing is in progress.', self)
            self.__postponedState = EntityState.CREATING
        else:
            _logger.debug(b'Entity %r is already created! Current state %r.', self, self.__lcState)
        return
    ...
    def validate(self, *args, **kwargs):
        if self.__lcState == EntityState.CREATED:
            ...
            _logger.debug(b'Invalidate call is skipped because object %r is destroyed or has not been created yet. Current state %r.', self, self.__lcState)
        else:
            _logger.debug(b'Invalidate call is skipped because initialization of object %r is in progress.', self)
        return
    ...
    def destroy(self):
        if self.__lcState in (EntityState.UNDEFINED, EntityState.CREATED):
            ...
            _logger.debug(b'Destroy call is postponed for %r object. Initialization is in progress', self)
            self.__postponedState = EntityState.DISPOSING
        else:
            _logger.debug(b'Entity %r is already destroyed! Current state %r.', self, self.__lcState)
        return
    ...
    def __invalidatePostponedState(self):
        if self.__postponedState in (EntityState.DISPOSING, EntityState.DISPOSED):
            _logger.debug(b'Call postponed destroy call for %r', self)
            self.destroy()
        elif self.__postponedState in (EntityState.CREATING, EntityState.CREATED):
            _logger.debug(b'Call postponed create call for %r', self)
            self.create()
        self.__postponedState = EntityState.UNDEFINED
        return
