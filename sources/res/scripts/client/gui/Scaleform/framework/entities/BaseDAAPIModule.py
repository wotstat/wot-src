...
class BaseDAAPIModule(BaseDAAPIModuleMeta):
    ...
    def setFlashObject(self, movieClip, autoPopulate=True, setScript=True):
        if movieClip is not None:
            ...
            try:
                ...
                raise SoftException(b'Can not initialize daapi in ' + str(self))

            if autoPopulate:
                if self.isCreated():
                    _logger.error(b'Object is already created! Please, set flag autoPopulate=False: %r', self)
                else:
                    self.create()
        else:
            _logger.error(b'flashObject reference can`t be None!')
        return
    ...
    def _destroy(self):
        ...
        if self.flashObject is not None:
            try:
                ...
                _logger.exception(b'Error during flash disposing: %r', self)
    ...
    def _printOverrideError(self, methodName):
        _logger.error(b'Method must be override!: %s.%s', self.__class__.__name__, methodName)
        return
