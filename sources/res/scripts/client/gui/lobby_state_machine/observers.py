...
class _StateClosingObserver(BaseStateObserver):
    ...
    def onStateChanged(self, state, stateEntered, event=None):
        lsm = self.__lsmRef()
        if lsm is None:
            _logger.warning(b'LobbyStateMachine is None! State changes should not have happened. State: %r, stateEntered: %r, event: %r', state, stateEntered, event)
            return
        else:
...
class _ViewKillingObserver(BaseStateObserver):
    ...
    def destroyViewsOfExitedStates(self):
        lsm = self.__lsmRef()
        if lsm is None:
            _logger.warning(b'LobbyStateMachine is None! Cannot destroy views of exited states.')
            return
        else:
            ...
            for window in self.__windowsWithViewKeys(viewKeys):
                if window.windowStatus not in (WindowStatus.DESTROYING, WindowStatus.DESTROYED):
                    _logger.debug(b'Killing window: %s', window.content)
                    window.destroy()
