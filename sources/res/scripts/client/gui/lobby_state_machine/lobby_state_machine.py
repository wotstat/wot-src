...
class LobbyStateMachine(StateMachine, EventsHandler):
    ...
    def getStateByCls(self, cls):
        if not issubclass(cls, LobbyState):
            raise StateMachineError((b'LobbyStateMachine cannot contain states of type {}.').format(cls))
    ...
    def start(self, doValidate=True):
        ...
        if doValidate:
            for state in self.visitInOrder((lambda node: isinstance(node, LobbyState))):
                parentClsRef = getattr(type(state), b'_PARENT_CLS', None)
                if parentClsRef is not None and not isinstance(state.getParent(), parentClsRef()):
                    raise NodeError(b'State %s declared as child of %s, but its parent is %s' % (
                ...
                for transition in state.getChildren((lambda node: isinstance(node, NavigationTransition))):
                    ...
                    if not sameSubtree and transition.record:
                        raise TransitionError(b'%s from %s to %s is a cross subtree transition with record flag set!Record flags on cross subtree transitions are not allowed.' % (
    ...
    def post(self, event):
        if not isinstance(event, (NavigationEvent, BackNavigationEvent, _BackNavigationEvent)):
            raise StateMachineError((b'LobbyStateMachine only accepts events of NavigationEvent type. Invalid event: {}').format(event))
        if not self.isRunning():
            _logger.info(b'State machine is not running. Ignoring navigation to %r', event)
            return
        else:
            if isinstance(event, (BackNavigationEvent, _BackNavigationEvent)):
                if event.requestingState is None:
                    ...
                    _logger.info(b'Navigating back requested by None, resolved to (%s)', event.requestingState.getStateID())
                else:
                    _logger.info(b'Navigating back requested by (%s)', event.requestingState.getStateID())
            ...
            if self.__currentlyProcessedEvent:
                _logger.info(b'Queued navigation to %s', event.targetStateID)
                self.__navigationQueue.append(event)
                return
            _logger.info(b'Navigating to %s', event.targetStateID)
            self.__currentlyProcessedEvent = event
            try:
                super(LobbyStateMachine, self).post(event)
            except Exception as e:
                _logger.error(b'An exception was caught while processing event: %r.', event)
                _logger.exception(e)

            self.__currentlyProcessedEvent = None
            if self.__navigationQueue:
                queuedEvent = self.__navigationQueue.pop(0)
                _logger.info(b'Popping %s from navigation queue', queuedEvent)
    ...
    def __updateVisibleRoute(self):
        ...
        if visibleStateWithDescription:
            ...
            if not isinstance(currentNavigationDescription, LobbyStateDescription):
                _logger.error(b'%r implements an old version of getNavigationDescription. Please update it to return LobbyStateDescription!', visibleStateWithDescription)
        ...
        _logger.debug(b'Visible route: %s', visibleState.getStateID() if visibleState else b'NO STATE VISIBLE')
        _logger.debug(b'Routes: [%s]', (b', ').join(s.getStateID() for s in enteredLeafStates))
        _logger.debug(b'Current state description: "%s" (state: %s)', currentNavigationDescription, visibleStateWithDescription.getStateID() if visibleStateWithDescription else None)
        if visibleStateWithDescription is not visibleState:
            _logger.debug(b'\tDescription derived from (%s) instead of visible state (%s) because it has no description', visibleStateWithDescription.getStateID() if visibleStateWithDescription else None, visibleState.getStateID() if visibleState else None)
        _logger.debug(b'Back navigation description: "%s" (state: %s)', self.__backNavigationDescription, visualBackNavigationTarget.getStateID())
        backNavigationTarget, _ = self.__getBackNavigationTargetAndParams(visibleState)
        _logger.debug(b'Back navigation would return to: %s', backNavigationTarget.getStateID())
        _logger.debug(b'Params of entered state: %s', self.__visibleRouteInfo.params)
        _logger.debug(b'Recorded states: %s', self.__recordedStates)
        _logger.info(b'Visible route changed to: %r', visibleState)
    ...
    def __findRelevantTransition(self, transitions):
        ...
        if len(recordTransitions) > 1:
            raise TransitionError(b'Multiple record transitions are not allowed.')
