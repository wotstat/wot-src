...
_NAVIGATION_ROUTE_KEY = b'route'
_NAVIGATION_PARAMS_KEY = b'params'
...
class Router(EventsHandler):
    ...
    def _encodeRoute(self, stateId):
        ...
        if stateId.startswith(self._routePrefix):
            ...
            _logger.warning(b'Entered state (%r) does not contain specified route prefix (%s)', stateId, self._routePrefix)
        return route

    def _encodeParams(self, params):
        encodedParams = b''
        if UntrackedState.LOAD_PARAMS_KEY in params:
            _logger.info(b'Marshalling _UntrackedStateForwardedParams into JSON is unsupported. Removing "%s" from params. Params pre-removal: %r', UntrackedState.LOAD_PARAMS_KEY, params)
            params = dict(params)
            del params[UntrackedState.LOAD_PARAMS_KEY]
        try:
            encodedParams = json.dumps(params)
        except (TypeError, UnicodeDecodeError) as e:
            _logger.info(b'Unable to marshall params into JSON: %r. Exception: %r', params, e)
    ...
    def _decodeJsonParams(self, jsonParams):
        ...
        try:
            ...
            _logger.warning(b'Failed unmarshalling malformed json: %s. Exception: %r', jsonParams, e)

        return params

    def __updateModel(self):
        _logger.debug(b'Encoded route: %r', self._route)
        _logger.debug(b'Encoded params: %r', self._params)
    ...
    def __navigateTo(self, navigationInfo):
        stateId = self._decodeStateId(navigationInfo.get(_NAVIGATION_ROUTE_KEY, b''))
        params = self._decodeJsonParams(navigationInfo.get(_NAVIGATION_PARAMS_KEY, b'{}') or b'{}')
        _logger.debug(b'Decoded stateId: %r', stateId)
        _logger.debug(b'Decoded params: %r', params)
...
class SubstateRouter(BaseStateObserver):
    ...
    def __init__(self, lsm, routableView, rootState):
        ...
        if not self.__rootState.getStateID().startswith(self.__router.routePrefix):
            _logger.warning(b'Router prefix (%s) is not part of the state (%r) owning the router.', self.__router.routePrefix, self.__rootState)
        return
    ...
    def onEnterState(self, state, event):
        ...
        if self.__router.routePrefix not in stateId:
            _logger.warning(b'Entered state (%s) which is not a child of state (%r) where the router is rooted.', state, self.__rootState)
