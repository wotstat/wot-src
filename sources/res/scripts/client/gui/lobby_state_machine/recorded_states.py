...
class _RecordedStates(EventsHandler):
    ...
    def __repr__(self):
        return (b'{}').format([(s.getStateID(), p) for s, p in self.__stateWithParamStack])
    ...
    def __onWindowStatusChanged(self, uniqueWindowId, status):
        ...
        if entriesToRemove:
            _logger.debug(b'Clearing entries in LSM recorded states because a %r was closed', view)
            _logger.debug(b'Recorded states before cleaning: %s', self)
            for entry in entriesToRemove:
                self.__stateWithParamStack.remove(entry)

            _logger.debug(b'Recorded states after cleaning: %s', self)
