...
class FieldsReplacer(object):
    ...
    def __init__(self, result, isPersonal):
        ...
        if isWithFlamethrower or result.vehicle is not None and vehicles.isFlamethrower(result.vehicle.intCD):
            self.replacerFields[b'explosionHits'] = b'flameExplosionHits'
        return
...
class RankedBattlesVehicleStatsBlock(RegularVehicleStatsBlock):
    ...
    def __makeRankIcon(self, rankID):
        ...
        if displayInfo.isGroup:
            return backport.image(R.images.gui.maps.icons.rankedBattles.ranks.c_24x24.dyn((b'ranks_group{}_{}').format(displayInfo.division, displayInfo.level))())
        return backport.image(R.images.gui.maps.icons.rankedBattles.ranks.c_24x24.dyn((b'rank{}_{}').format(displayInfo.division, displayInfo.level))())


class EpicVehicleStatsBlock(RegularVehicleStatsBlock):
    __slots__ = (b'playerRank', b'respawns', b'__allAdded')
...
class EpicVehicleStatValuesBlock(base.StatsBlock):
    __slots__ = (b'_team', b'_isPersonal', b'_filters', b'shots', b'hits', b'explosionHits', b'damageDealt', b'sniperDamageDealt', b'destructiblesDamageDealt', b'equipmentDamageDealt', b'directHitsReceived', b'piercingsReceived', b'noDamageDirectHitsReceived', b'explosionHitsReceived', b'damageBlockedByArmor', b'teamHitsDamage', b'spotted', b'damagedKilled', b'damageAssisted', b'equipmentDamageAssisted', b'damageAssistedStun', b'stunNum', b'capturePoints', b'timesDestroyed', b'damageToSupplies', b'damageFromSupplies', b'suppliesDestroyed', b'distributedSupplyDamage', b'distributedSupplyBasePoints', b'teamSpecificStat', b'__rawDamageAssistedStun', b'__rawStunNum', b'__fieldsReplacer')
    ...
    def setRecord(self, result, reusable):
        ...
        if isinstance(result, VehicleDetailedInfo):
            self.addFilters((b'distributedSupplyDamage', b'distributedSupplyBasePoints'))
        self.shots = style.getIntegralFormatIfNoEmpty(result.shots)
        self.hits = (result.directEnemyHits, result.piercingEnemyHits)
        self.explosionHits = style.getIntegralFormatIfNoEmpty(result.explosionHits)
        self.damageDealt = style.getIntegralFormatIfNoEmpty(result.damageDealt)
...
class AllEpicVehicleStatValuesBlock(base.StatsBlock):
    ...
    def setRecord(self, result, reusable):
        ...
        add = self.addNextComponent
        stunFilter = _getStunFilter()
        supplyDistributedFilter = (b'distributedSupplyDamage', b'distributedSupplyBasePoints')
        for vehicle in iterator:
...
class RankedResultsTeamPartDataStatsBlock(base.StatsBlock):
    __slots__ = (b'listData', b'backgroundType', b'backgroundBlink', b'icon', b'capacity', b'isColorBlind', b'iconType', b'iconMethod')
