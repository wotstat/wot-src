...
_ARENA_TYPE_FORMAT = b'#arenas:type/{0}/name'
_ARENA_TYPE_EXT_FORMAT = b'#menu:loading/battleTypes/{0}'
_ARENA_FULL_NAME_FORMAT = b'#battle_results:common/arena/fullName'
_ARENA_ICON_PATH = b'../maps/icons/map/stats/%s.png'
...
def getRegularFinishResultResource(finishReason, teamResult):
    ...
    reasonKey = (b'c_{}{}').format(finishReason, teamResult) if isExtermination else (b'c_{}').format(finishReason)
...
def makeEpicBattleFinishResultLabel(finishReason, teamResult):
    ...
    if finishReason in {FINISH_REASON.EXTERMINATION, FINISH_REASON.TIMEOUT, FINISH_REASON.DESTROYED_OBJECTS}:
        return backport.text(R.strings.battle_results.finish.reason.dyn((b'c_{}{}').format(finishReason, teamResult))())
    return backport.text(R.strings.battle_results.finish.reason.dyn((b'c_{}').format(finishReason))())
...
class ArenaNameItem(base.StatsItem):
    ...
    def _convert(self, record, reusable):
        return backport.text(R.strings.arenas.dyn((b'c_{}').format(reusable.common.arenaType.getGeometryName())).name())
...
class ObjectivesReachedVO(base.StatsItem):
    ...
    def _convert(self, record, reusable):
        if record:
            ...
            reached = yes if record[b'commonNumStarted'] > 0 else no
        else:
            reached = b''
...
class ObjectivesDestroyedVO(base.StatsItem):
    ...
    def _convert(self, record, reusable):
        if record:
            destroyed = str(record[b'commonNumDestroyed'])
        else:
            destroyed = b'0'
...
class BasesCapturedVO(base.StatsItem):
    ...
    def _convert(self, record, reusable):
        if record:
            captured = str(record[b'commonNumCaptured'])
        else:
            captured = b'0'
...
class FinishResultMeta(base.DictMeta):
    ...
    def __init__(self, finishReasonLabel, finishReasonClarificationLabel, shortResultLabel, fullResultLabel):
        meta = {finishReasonLabel: b'', 
           finishReasonClarificationLabel: b'', shortResultLabel: b'', fullResultLabel: b''}
        auto = (
         (
          0, base.StatsItem(finishReasonLabel, b'finishReasonLabel')),
         (
          1, base.StatsItem(finishReasonClarificationLabel, b'finishReasonClarificationLabel')),
         (
          2, base.StatsItem(shortResultLabel, b'shortResultLabel')),
         (
          3, base.StatsItem(fullResultLabel, b'fullResultLabel')))
        super(FinishResultMeta, self).__init__(meta, auto)
        return


class RegularFinishResultBlock(base.StatsBlock):
    __slots__ = (b'finishReasonLabel', b'finishReasonClarificationLabel', b'shortResultLabel', b'fullResultLabel')
    ...
    def setRecord(self, result, reusable):
        ...
        self.finishReasonClarificationLabel = backport.text(R.strings.battle_results.finish.clarification.finishAllPlayersLeft()) if reusable.common.finishAllPlayersLeft else b''
        return
...
class AllyTeamClanTitle(base.StatsItem):
    ...
    def _convert(self, value, reusable):
        return (b'{} [{}]').format(backport.text(R.strings.battle_results.team.stats.ownTeam()), reusable.players.getFirstAllyClan(reusable.getPersonalTeam()).clanAbbrev)
...
class EnemyTeamClanTitle(base.StatsItem):
    ...
    def _convert(self, value, reusable):
        return (b'{} [{}]').format(backport.text(R.strings.battle_results.team.stats.enemyTeam()), reusable.players.getFirstEnemyClan(reusable.getPersonalTeam()).clanAbbrev)


class ClanInfoBlock(base.StatsBlock):
    __slots__ = (b'clanDBID', b'clanAbbrev')
    ...
    def setRecord(self, record, reusable):
        if record is not None:
            ...
            self.clanAbbrev = (b'[{}]').format(record.clanAbbrev)
        return


class ClansInfoBlock(base.StatsBlock):
    __slots__ = (b'allies', b'enemies')
