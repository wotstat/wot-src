...
TitleAndDescription = namedtuple(b'TitleAndDescription', b'title, description, descriptionIcon')
Shield = namedtuple(b'Shield', b'shieldCount, shieldIcon, plateIcon')
IconsAndShield = namedtuple(b'IconsAndShield', b'icon, shield')

def _getTopsLoseIcon(topNumber):
    return backport.image(R.images.gui.maps.icons.rankedBattles.tops.lose.c_106x98.dyn((b'top{}').format(topNumber))())


def _getTopsTopIcon(topNumber):
    return backport.image(R.images.gui.maps.icons.rankedBattles.tops.top.c_106x98.dyn((b'top{}').format(topNumber))())


def _getTopsNotEffectiveIcon(topNumber):
    return backport.image(R.images.gui.maps.icons.rankedBattles.tops.notEffective.c_106x98.dyn((b'top{}').format(topNumber))())
...
class RankedInfoHelper(object):
    ...
    __slots__ = (b'_reusable',)
    _RESOURCE_MAP = {(MiniSteps.STEPS_EARNED): (
                                RANKEDBATTLES_ALIASES.BACKGROUND_STATE_TOP2,
                                RANKEDBATTLES_ALIASES.BACKGROUND_STATE_TOP,
                                _getTopsTopIcon), 
       (MiniSteps.STEP_EARNED): (
                               RANKEDBATTLES_ALIASES.BACKGROUND_STATE_TOP,
                               RANKEDBATTLES_ALIASES.BACKGROUND_STATE_TOP,
                               _getTopsTopIcon), 
       (MiniSteps.STEP_NOT_CHANGED): (
                                    RANKEDBATTLES_ALIASES.BACKGROUND_STATE_NOTEFFECTIVE,
                                    RANKEDBATTLES_ALIASES.BACKGROUND_STATE_NOTEFFECTIVE,
                                    _getTopsNotEffectiveIcon), 
       (MiniSteps.STEP_LOST): (
                             RANKEDBATTLES_ALIASES.BACKGROUND_STATE_LOSE,
                             RANKEDBATTLES_ALIASES.BACKGROUND_STATE_LOSE,
                             _getTopsLoseIcon)}
...
class RankedResultsInfoHelper(RankedInfoHelper):
    ...
    def makeStatusLabel(self):
        ...
        resultSubLabel = b''
        if rankState in (_RCS.LEAGUE_EARNED, _RCS.DIVISION_EARNED, _RCS.QUAL_EARNED, _RCS.QUAL_UNBURN_EARNED):
            return b''
    ...
    def getPlayerStandoff(self, team, position, stepChanges, updatedStepChanges):
        ...
        return (
         RANKEDBATTLES_ALIASES.STANDOFF_MINUS, configStepChanges)
    ...
    def getStandoff(self, xp, xpToCompare, position, isLoser, isTop, lastStandoffInfo=None):
        ...
        if isTop:
            ...
            return (
             RANKEDBATTLES_ALIASES.STANDOFF_INVISIBLE, stepsDiff)
...
class RankedChangesInfoHelper(RankedInfoHelper):
    ...
    def makeTitleAndDescription(self, allyVehicles):
        ...
        winKey = b'win' if isWin else b'lose'
        topKey = b'inTop' if topNumber >= position else b'notInTop'
        ...
        if rankState in (_RCS.DIVISION_EARNED, _RCS.LEAGUE_EARNED, _RCS.QUAL_EARNED, _RCS.QUAL_UNBURN_EARNED):
    ...
    def makeIcons(self):
        ...
        if state == RANKEDBATTLES_ALIASES.SUBTASK_STATE_LEAGUE:
            ...
            icon = backport.image(R.images.gui.maps.icons.rankedBattles.divisions.c_58x80.dyn((b'c_{}').format(self.rankedController.getDivision(rankInfo.accRank + 1).getID()))())
    ...
    def _getDescriptionIcon(self, state, stepChanges, updatedStepChanges):
        if stepChanges is None:
            ...
            if subTaskState not in (RANKEDBATTLES_ALIASES.SUBTASK_STATE_STAGE, RANKEDBATTLES_ALIASES.SUBTASK_STATE_LEAGUE):
                if stepChanges == MiniSteps.STEP_LOST and self.settingsCore.getSetting(b'isColorBlind'):
    ...
    def _getLoserBounds(self, position):
        ...
        if loserRankChanges[position] >= 0:
    ...
    def _getWinnerBounds(self, position):
        ...
        if winnerRankChanges[position] > 0:
    ...
    def __getRankIcon(self, rankID):
        ...
        return backport.image(R.images.gui.maps.icons.rankedBattles.ranks.c_58x80.dyn(b'rank%s_%s' % (division.getID(), division.getRankUserName(rankID)))())
...
class RankChangesBlock(base.StatsBlock):
    __slots__ = (b'state', b'linkage', b'title', b'description', b'descriptionIcon', b'icon', b'plateIcon', b'shieldIcon', b'shieldCount')
