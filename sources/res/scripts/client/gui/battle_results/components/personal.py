...
class DynamicPremiumState(base.StatsItem):
    __slots__ = (b'__arenaBonusType', b'__arenaUniqueID', b'__postBattlePremium', b'__postBattlePremiumPlus', b'__xpDiff', b'__creditsDiff')
    __itemsCache = dependency.descriptor(IItemsCache)
    __battleResults = dependency.descriptor(IBattleResultsService)
    __lobbyContext = dependency.descriptor(ILobbyContext)
    __UNPROFITABLE_ARENA_TYPES = (ARENA_BONUS_TYPE.TRAINING, ARENA_BONUS_TYPE.EPIC_BATTLE_TRAINING,
    ...
    def __isDailyBonusVisible(self):
        if self.__getIsApplied():
            return True
        isBonusEnabled = self.__lobbyContext.getServerSettings().getAdditionalBonusConfig().get(b'enabled', False)
...
class PremiumInfoBlock(base.StatsBlock):
    __slots__ = (b'creditsPremiumBonusStr', b'xpPremiumBonusStr', b'premiumBonusStr', b'isGetPremium', b'isUpgradeToPremiumPlus', b'backgroundIcon', b'inBattleQueue', b'__xpDiff', b'__creditsDiff', b'__canUpgradeToBasic', b'__canUpgradeToPlus', b'visibleDetailsBtn', b'__adsCase')
    __lobbyContext = dependency.descriptor(ILobbyContext)
    __itemsCache = dependency.descriptor(IItemsCache)
    __allPremiumPlusCases = (b'credits', b'premium', b'squad', b'bonus', b'quests')
    __epicPremiumPlusCases = (b'premium', b'squad', b'credits')
    ...
    def __init__(self, meta=None, field=b'', *path):
        ...
        self.creditsPremiumBonusStr = b''
        self.xpPremiumBonusStr = b''
        self.premiumBonusStr = b''
        self.backgroundIcon = b''
    ...
    def __setPremiumBonusData(self):
        ...
        piggyBankMaxAmount = piggyBankConfig.get(b'creditsThreshold', 0)
        period = piggyBankConfig.get(b'cycleLength', time_utils.ONE_DAY)
        periodInDays = ceil(period / time_utils.ONE_DAY)
        bonusConfig = self.__lobbyContext.getServerSettings().getAdditionalBonusConfig()
        multiplier = premacc_helpers.validateAdditionalBonusMultiplier(bonusConfig.get(b'bonusFactor', 1))
        if not self.__itemsCache.items.stats.isActivePremium(PREMIUM_TYPE.PLUS):
            if self.__itemsCache.items.stats.isActivePremium(PREMIUM_TYPE.BASIC):
                ...
                iconName = (b'bonus_x{}').format(multiplier) if self.__adsCase == b'bonus' else self.__adsCase
...
class PlayerRank(base.StatsItem):
    ...
    def _convert(self, value, reusable):
        ...
        if b'playerRank' in avatarInfo.extensionInfo:
            return avatarInfo.extensionInfo[b'playerRank']
        return 0
...
class KillerPlayerNameBlock(shared.PlayerNameBlock):
    __slots__ = (b'killerID',)

    def __init__(self, killerID, meta=None, field=b'', *path):
    ...
    def setRecord(self, result, reusable):
        if reusable is not None:
            ...
            LOG_ERROR(b'Killer is not found. ID %s' % self.killerID)
        return
...
class PersonalVehicleNamesBlock(base.StatsBlock):
    ...
    def setRecord(self, result, reusable):
        for _, item in reusable.personal.getVehicleItemsIterator():
            self.addNextComponent(base.DirectStatsItem(b'', item.shortUserName))
...
class PersonalVehicleTypeIconsBlock(base.StatsBlock):
    ...
    def setRecord(self, result, reusable):
        for _, item in reusable.personal.getVehicleItemsIterator():
            self.addNextComponent(base.DirectStatsItem(b'', getTypeBigIconPath(item.type)))
...
class EpicVehicleNamesBlock(PersonalVehicleNamesBlock):
    ...
    def setRecord(self, result, reusable):
        self.addNextComponent(base.DirectStatsItem(b'', i18n.makeString(BATTLE_RESULTS.ALLVEHICLES)))
        super(EpicVehicleNamesBlock, self).setRecord(result, reusable)
        return


class PersonalVehicleBlock(base.StatsBlock):
    __slots__ = (b'isVehicleStatusDefined', b'vehicleIcon', b'vehicleLevel', b'nationName', b'killerID', b'vehicleState', b'vehicleStatePrefix', b'vehicleStateSuffix', b'isPrematureLeave', b'isKilledByTeamKiller', b'deathReason')
    ...
    def setRecord(self, result, reusable):
        killerID = result.get(b'killerID', 0)
        deathReason = result.get(b'deathReason', DEATH_REASON_ALIVE)
        self.killerID = killerID
...
class DamageDetailsBlock(_DetailsBlock):
    __slots__ = (b'piercings', b'damageDealtValues', b'damageDealtNames', b'_isEmpty')
...
class ArmorUsingDetailsBlock(_DetailsBlock):
    __slots__ = (b'usedArmorCount', b'armorValues', b'armorNames', b'_isEmpty')
...
class AssistDetailsBlock(_DetailsBlock):
    __slots__ = (b'damageAssisted', b'damageAssistedValues', b'damageAssistedNames', b'_isEmpty')
...
class StunDetailsBlock(_DetailsBlock):
    __slots__ = (b'stunNum', b'stunValues', b'stunNames', b'stunDuration', b'_isEmpty')
...
class CritsDetailsBlock(_DetailsBlock):
    __slots__ = (b'critsCount', b'criticalDevices', b'destroyedDevices', b'destroyedTankmen', b'_isEmpty')
    ...
    def setRecord(self, result, reusable):
        ...
        self.critsCount = backport.getIntegralFormat(crits[b'critsCount'])
...
class TeamBaseDetailsBlock(base.StatsBlock):
    __slots__ = (b'_showCapturePoints', b'_showDefencePoints', b'captureTotalItems', b'defenceTotalItems', b'captureValues', b'captureNames', b'defenceValues', b'defenceNames', b'label', b'isEnemyBase')
...
class EnemyTeamBaseDetailBlock(TeamBaseDetailsBlock):
    __slots__ = (b'_showCapturePoints', b'label', b'isEnemyBase')
...
class AllyTeamBaseDetailBlock(TeamBaseDetailsBlock):
    __slots__ = (b'_showDefencePoints', b'label')
...
class EnemyDetailsBlock(_DetailsBlock):
    __slots__ = (b'vehicleIcon', b'vehicleName', b'vehicleIntCD', b'vehicleID', b'deathReason', b'spotted', b'piercings', b'damageDealt', b'killCount', b'_isEmpty')
...
class TotalEfficiencyDetailsHeader(base.StatsBlock):
    __slots__ = (b'kills', b'damageDealt', b'criticalDamages', b'damageBlockedByArmor', b'damageAssisted', b'damageAssistedStun', b'spotted', b'killsTooltip', b'damageDealtTooltip', b'criticalDamagesTooltip', b'damageBlockedTooltip', b'damageAssistedTooltip', b'spottedTooltip', b'damageAssistedStunTooltip', b'hasEfficencyStats')
    ...
    def setRecord(self, result, reusable):
        ...
        if info is None:
            LOG_ERROR(b'ERROR: TotalEfficiencyDetailsHeader:setRecord: getPersonalVehiclesInfo returned NONE!')
            return
        else:
            value = info.kills
            self.kills = numbers.formatInt(value, _UNDEFINED_EFFICIENCY_VALUE)
            self.killsTooltip = self.__makeEfficiencyHeaderTooltip(b'summKill', value)
            value = info.damageDealt
            self.damageDealt = numbers.makeStringWithThousandSymbol(value, formatter=self.__formatter)
            self.damageDealtTooltip = self.__makeEfficiencyHeaderTooltip(b'summDamage', value)
            value = info.critsCount
            self.criticalDamages = numbers.formatInt(value, _UNDEFINED_EFFICIENCY_VALUE)
            self.criticalDamagesTooltip = self.__makeEfficiencyHeaderTooltip(b'summCrits', value)
            ...
            self.damageBlockedTooltip = self.__makeEfficiencyHeaderTooltip(b'summArmor', value)
            value = info.damageAssisted
            self.damageAssisted = numbers.makeStringWithThousandSymbol(value, formatter=self.__formatter)
            self.damageAssistedTooltip = self.__makeEfficiencyHeaderTooltip(b'summAssist', value)
            ...
            self.damageAssistedStunTooltip = self.__makeEfficiencyHeaderTooltip(b'summStun', value)
            value = info.spotted
            self.spotted = numbers.formatInt(value, _UNDEFINED_EFFICIENCY_VALUE)
            self.spottedTooltip = self.__makeEfficiencyHeaderTooltip(b'summSpotted', value)
    ...
    def __formatter(cls, value):
        return (b'{:,}').format(value).replace(b',', b' ')
