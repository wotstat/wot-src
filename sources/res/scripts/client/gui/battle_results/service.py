...
class BattleResultsService(IBattleResultsService):
    ...
    __slots__ = (b'__battleResults', b'__statsCtrls', b'__buy', b'__eventsManager', b'onResultPosted', b'__appliedAddXPBonus', b'__playerSatisfactionRatings')
    ...
    def requestEmblem(self, ctx, callback=None):
        ...
        if fetcher is not None:
            ...
            _logger.warning(b'Icon fetcher is not found %r', ctx)
            if callback is not None:
                callback(None)
        return
    ...
    def getStatsCtrl(self, arenaUniqueID):
        if arenaUniqueID not in self.__statsCtrls:
            _logger.error(b'Missing suitable battle results presenter.')
            return None
        else:
    ...
    def __onAddXPBonusChanged(self, isBonusApplied=True):
        g_eventBus.handleEvent(events.LobbySimpleEvent(events.LobbySimpleEvent.PREMIUM_XP_BONUS_CHANGED, ctx={b'isBonusApplied': isBonusApplied}))
        return
...
class _PostBattleStatesObserver(BaseStateObserver):
    ...
    def onEnterState(self, state, event):
        g_eventBus.handleEvent(events.LobbySimpleEvent(events.LobbySimpleEvent.BATTLE_RESULTS_PROCESSED), scope=EVENT_BUS_SCOPE.LOBBY)
        _logger.info(b'Battle results processed in state %s', state)
        return
