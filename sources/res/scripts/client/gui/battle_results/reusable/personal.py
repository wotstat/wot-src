...
_LifeTimeInfo = namedtuple(b'_LifeTimeInfo', (b'isKilled', b'lifeTime'))
...
class PersonalAvatarInfo(object):
    __slots__ = (b'__accountDBID', b'__clanDBID', b'__team', b'__isPrematureLeave', b'__fairplayViolations', b'__squadBonusInfo', b'__winnerIfDraw', b'__eligibleForCrystalRewards', b'__extInfo', b'__playerSatisfactionRating', b'__commendationsReceived')
...
class PersonalInfo(shared.UnpackedInfo):
    __slots__ = (b'__avatar', b'__vehicles', b'__lifeTimeInfo', b'__isObserver', b'_economicsRecords', b'__questsProgress', b'__PM2Progress', b'__rankInfo', b'__isTeamKiller', b'__progressiveReward', b'__premiumMask', b'__isWotPlus', b'__wotPlusTier', b'__isAddXPBonusApplied', b'__c11nProgress', b'__dogTags', b'__goldBankGain', b'__xpProgress', b'__prestigeResults', b'__questTokensCount', b'__questTokensConvertion')
    itemsCache = dependency.descriptor(IItemsCache)
    ...
    def __collectRequiredData(self, info):
        ...
        if infoAvatar:
            ...
            self.__PM2Progress.update(infoAvatar.get(b'PM2Progress', {}))
            self.__questTokensCount.update(infoAvatar.get(b'questTokensCount', {}))
            self.__questTokensConvertion.update(infoAvatar.get(b'questTokensConvertion', {}))
        ...
        for item in items:
            ...
            killerID = data.get(b'killerID', 0)
            lifeTime = data.get(b'lifeTime', 0)
            if killerID and lifeTime:
                lifeTimes.append(lifeTime)
            self.__isTeamKiller = data.get(b'isTeamKiller', False)
            self.__premiumMask = data.get(b'premMask', PREMIUM_TYPE.NONE)
            self.__isWotPlus = data.get(b'isWoTPlus', False)
            self.__wotPlusTier = data.get(b'wotPlusTier', 0)
            self.__questsProgress.update(data.get(b'questsProgress', {}))
            self.__PM2Progress.update(data.get(b'PM2Progress', {}))
            ...
            self.__prestigeResults[intCD] = data.get(b'prestigeResults', {})
