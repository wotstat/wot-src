...
_VehicleShortInfo = namedtuple(b'_ShortVehicleInfo', (b'intCD', b'team', b'accountDBID', b'deathReason', b'isTeamKiller'))
...
def _getVehiclesGenerator(vehicles):
    for vehicleID, data in vehicles.iteritems():
        ...
        for item in data:
            if item is None:
                continue
            if b'typeCompDescr' not in item:
                continue
            accountDBID = item.get(b'accountDBID', 0L)
            intCD = item[b'typeCompDescr']
            info = _VehicleShortInfo(intCD, item.get(b'team', 0), accountDBID, item.get(b'deathReason', DEATH_REASON_ALIVE), item.get(b'isTeamKiller', False))
            items.append(info)
...
class VehiclesInfo(shared.UnpackedInfo):
    __slots__ = (b'__vehicles', b'__vehicleToAccountID', b'__accountToVehicleID', b'__details', b'__bonusType')
