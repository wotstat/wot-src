...
def _fetchRecord(results, recordName):
    if recordName in results:
        ...
        if record is None:
            LOG_WARNING(b'Record is not valid in the results. Perhaps, client and server versions of battle_results.g_config are different.', recordName)
        return record
    LOG_WARNING(b'Record is not found in the results', recordName, results.keys())
    return
    return
...
def createReusableInfo(results):
    if _RECORD.ARENA_UNIQUE_ID in results:
        ...
        LOG_WARNING((b'Battle results must be contain key {}').format(_RECORD.ARENA_UNIQUE_ID), results.keys())
        return
    unpackedRecords = []
    ...
    if record is None:
        ...
        LOG_WARNING(b'Records are not valid in the results. Perhaps, client and server versions of battle_results.g_config are different.', *[(record, results[record]) for record in unpackedRecords])
        return


class _ReusableInfo(object):
    __slots__ = (b'__arenaUniqueID', b'__clientIndex', b'__premiumState', b'__common', b'__personal', b'__players', b'__vehicles', b'__avatars', b'__squadFinder', b'__premiumPlusState', b'__isAddXPBonusApplied', b'__battlePassProgress')
    ...
    def canUpgradeToPremium(self):
        return self.__premiumState & PREMIUM_STATE.BUY_ENABLED > 0 and self.__premiumState & PREMIUM_STATE.HAS_ALREADY == 0 and not self.isPostBattlePremium and self.__common.arenaBonusType in (ARENA_BONUS_TYPE.REGULAR, ARENA_BONUS_TYPE.EPIC_RANDOM,
         ARENA_BONUS_TYPE.EPIC_BATTLE)

    @property
    def canUpgradeToPremiumPlus(self):
        return self.__premiumPlusState & PREMIUM_STATE.BUY_ENABLED > 0 and self.__premiumPlusState & PREMIUM_STATE.HAS_ALREADY == 0 and not self.isPostBattlePremiumPlus and self.__common.arenaBonusType in (ARENA_BONUS_TYPE.REGULAR, ARENA_BONUS_TYPE.EPIC_RANDOM,
         ARENA_BONUS_TYPE.EPIC_BATTLE)
    ...
    def getPlayerInfoByVehicleID(self, vehicleID):
        ...
        if accountDBID:
            ...
            botName = botInfo.realName if botInfo else b''
    ...
    def getPersonalDetailsIterator(self, result):
        ...
        for _, vData in self.__personal.getVehicleCDsIterator(result):
            ...
            bases = teamBasesInfoCls(vData.get(b'capturePoints', 0), vData.get(b'droppedCapturePoints', 0))
    ...
    def getPersonalVehiclesInfo(self, result):
        ...
        for intCD, records in self.__personal.getVehicleCDsIterator(result):
            critsRecords = []
            if b'details' in records:
                playerTeam = self.__personal.avatar.team
                playerDBID = self.__personal.avatar.accountDBID
                getVehicleInfo = self.__vehicles.getVehicleInfo
                details = records[b'details']
                for (vehicleID, _), data in details.iteritems():
                    vehicleInfo = getVehicleInfo(vehicleID)
                    if vehicleInfo.accountDBID != playerDBID and vehicleInfo.team != playerTeam:
                        critsRecords.append(data[b'crits'])
