...
def makeMarkOfMasteryFromPersonal(results):
    ...
    if not markOfMastery:
        ...
        factory = getAchievementFactory((b'achievements', MARK_OF_MASTERY))
        if factory is not None:
...
class _VehicleInfo(object):
    __slots__ = (b'_vehicleID', b'_player', b'_deathReason')
...
class VehicleDetailedInfo(_VehicleInfo):
    ...
    def makeForVehicle(cls, vehicleID, vehicle, player, vehicleRecords, critsRecords=None):
        ...
        if vehicleRecords.get(b'details', 0):
            for data in vehicleRecords[b'details'].values():
                info._rickochetsReceived += data[b'rickochetsReceived']
