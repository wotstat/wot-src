...
PrestigeProgress = namedtuple(b'PrestigeProgress', (b'vehCD', b'oldGradeType', b'currentGradeType', b'oldGrade', b'currentGrade', b'oldLvl', b'newLvl', b'currentXP', b'currentNextLvlXP', b'oldXP', b'oldNextLvlXP', b'gainedXP'))
DogTagProgress = namedtuple(b'DogTagProgress', [
 b'componentId', b'compGrade', b'dogTagType', b'unlockType'])
...
def getPrestigeProgress(reusable):
    ...
    if not data:
        ...
        if not hasVehiclePrestige(vehCD, checkElite=True):
            return None
        if not prestigeData or prestigeData[b'oldLevel'] <= 0 or prestigeData[b'newLevel'] <= 0:
            return None
        return createPrestigeInfo(vehCD, prestigeData)


def createPrestigeInfo(vehCD, prestigeData):
    oldLvl = prestigeData[b'oldLevel']
    gainedPoints = prestigeData[b'gainedPoints']
    if getCurrentGrade(oldLvl, vehCD) == MAX_GRADE_ID or gainedPoints == 0:
        return None
    newLvl = prestigeData[b'newLevel']
    newLvlPoints = prestigeData[b'newPoints']
    ...
    oldPoints = prestigeData[b'oldPoints']
...
def getDogTagsProgress(reusable, lobbyContext=None):
    if not lobbyContext.getServerSettings().isDogTagInPostBattleEnabled():
        ...
        unlockedDogTags = [createDogTagInfo(compId, b'unlock') for compId in dogTags.get(b'unlockedComps', [])]
        upgradedDogTags = [createDogTagInfo(compId, b'upgrade') for compId in dogTags.get(b'upgradedComps', [])]
        return (
