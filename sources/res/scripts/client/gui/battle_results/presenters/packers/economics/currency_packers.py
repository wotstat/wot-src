...
CurrencyRecord = namedtuple(b'CurrencyRecord', (b'recordNames', b'subtractRecords', b'baseAccountValueExtractor', b'premiumAccountValueExtractor', b'detailsValuesExtractors', b'capsToBeChecked', b'paramName', b'label', b'modifiers', b'showZeroValue', b'currencyType'))
CurrencyGroup = namedtuple(b'CurrencyGroup', (b'label', b'records'))
...
class CurrencyPacker(ICurrencyPacker):
    ...
    def packModel(cls, model, currencyType, battleResults):
        ...
        if recordsExtractors is None or recordsConfigsExtractor is None:
            _logger.error(b'Invalid currency data')
            return
        else:
    ...
    def _packGroup(cls, model, records, battleResults, recordsConfigsExtractor, sourceConfig):
        if sourceConfig is None:
            ...
            if not recordsConfigs:
                _logger.error(b'Invalid config data for currency tooltip')
                return
...
class DetailedCurrencyPacker(ICurrencyPacker):
    ...
    def _packGroup(cls, model, records, battleResults, recordsConfigsExtractor, sourceConfig):
        if sourceConfig is None:
            ...
            if not recordsConfigs:
                raise SoftException(b'Invalid config data for currency')
