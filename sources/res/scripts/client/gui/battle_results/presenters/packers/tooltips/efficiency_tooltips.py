...
def getPlayerResults(battleResults, userName):
    ...
    if userName is not None:
        ...
        playerResults = reusable.getPersonalVehiclesInfo(results[b'personal'])
    ...
    def getStatuses(cls, reusable, results, userName, isZeroValuesVisible=False):
        ...
        if not reasons:
            ...
            for deathReason in reasons:
                killStatus = cls._STATUS.dyn((b'kill{}').format(deathReason))
...
class EfficiencyTooltipsPacker(ITooltipPacker):
    ...
    def packTooltip(cls, model, battleResults, ctx=None):
        if not ctx[b'paramType']:
            raise SoftException(b'Missing parameter type for the EfficiencyTooltip')
        paramType = ctx[b'paramType']
        isZeroValuesVisible = ctx.get(b'isZeroValuesVisible', False)
        isAdditionalValuesVisible = ctx.get(b'isAdditionalValuesVisible', False)
        userName = ctx.get(b'userName')
...
class CriticalDamageTooltipPacker(ITooltipPacker):
    ...
    def packTooltip(cls, model, battleResults, ctx=None):
        if not ctx[b'paramType']:
            raise SoftException(b'Missing parameter type for the CriticalDamageTooltip')
