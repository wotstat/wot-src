...
class HangarConfig(object):
    ...
    def loadConfig(self, xml, defaultCfg=None):
        ...
        self.cameraName = self.loadConfigValue(b'camera_name', xml, xml.readString, defaultCfg)
        self.translation = self.loadConfigValue(b'translation', xml, xml.readVector3, defaultCfg)
        return
    ...
    def loadCustomizationConfig(self, xml):
        ...
        self.shadowForwardYOffset = self.loadConfigValue(b'shadow_forward_y_offset', xml, xml.readFloat, defaultFakeShadowOffsetsCfg)
        self.shadowDeferredYOffset = self.loadConfigValue(b'shadow_deferred_y_offset', xml, xml.readFloat, defaultFakeShadowOffsetsCfg)
        self.cameraName = self.loadConfigValue(b'camera_name', xml, xml.readString, self)
        self.translation = self.loadConfigValue(b'translation', xml, xml.readVector3, self)
        pivotCfg = xml[b'pivotTransform']
        if pivotCfg:
            self.pivotTransform[b'yaw'] = self.loadConfigValue(b'yaw', pivotCfg, pivotCfg.readFloat, self)
            self.pivotTransform[b'pitch'] = self.loadConfigValue(b'pitch', pivotCfg, pivotCfg.readFloat, self)
            self.pivotTransform[b'shift'] = self.loadConfigValue(b'shift', pivotCfg, pivotCfg.readFloat, self)
            self.pivotTransform[b'pivotDir'] = self.loadConfigValue(b'pivotDir', pivotCfg, pivotCfg.readVector3, self)
            self.pivotTransform[b'maxVehicleWidth'] = self.loadConfigValue(b'maxVehicleWidth', pivotCfg, pivotCfg.readFloat, self)
            self.pivotTransform[b'maxVehicleHeight'] = self.loadConfigValue(b'maxVehicleHeight', pivotCfg, pivotCfg.readFloat, self)
            self.pivotTransform[b'offsetDistance'] = self.loadConfigValue(b'offsetDistance', pivotCfg, pivotCfg.readFloat, self)
        return

    def loadSecondaryConfig(self, xml):
        defaultShadowOffsetsCfg = {b'shadow_forward_y_offset': 0.0, b'shadow_deferred_y_offset': 0.0}
        self.vStartPos = self.loadConfigValue(b'v_start_pos', xml, xml.readVector3, self)
        self.vStartAngles = self.loadConfigValue(b'v_start_angles', xml, xml.readVector3, self)
        for i in range(0, 3):
            self.vStartAngles[i] = self[b'v_start_angles'][i] = math.radians(self[b'v_start_angles'][i])
