...
class WinbackEntity(PreQueueEntity):
    ...
    def _doDequeue(self, ctx):
        ...
        _logger.debug(b'Sends request on dequeuing from the winback battle')
        return
    ...
    def forceRandomLeave(self):
        try:
            ...
            _logger.debug(b'Sends force request on dequeuing from the winback battle')
    ...
    def onEnqueued(self, queueType, *args):
        if queueType != self._queueType and queueType == QUEUE_TYPE.RANDOMS:
            ...
            _logger.warning(b'An error occurred when entering the random mode queue, the queue was canceled')
            return
