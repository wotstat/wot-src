...
class EventBattleEntity(PreQueueEntity):
    ...
    def _doQueue(self, ctx):
        ...
        LOG_DEBUG(b'Sends request on queuing to the event battles', ctx)
        return

    def _doDequeue(self, ctx):
        BigWorld.player().dequeueEventBattles()
        LOG_DEBUG(b'Sends request on dequeuing from the event battles')
        return
    ...
    def _makeQueueCtxByAction(self, action=None):
        return EventBattleQueueCtx(g_currentVehicle.item.invID, waitingID=b'prebattle/join')
