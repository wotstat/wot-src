...
class EventSquadSettingsCtx(SquadSettingsCtx):
    __slots__ = (b'_keepCurrentView',)

    def __init__(self, waitingID=b'', accountsToInvite=None, keepCurrentView=False):
...
class EventBattleSquadEntryPoint(SquadEntryPoint):
    ...
    def makeDefCtx(self):
        return EventSquadSettingsCtx(waitingID=b'prebattle/create', accountsToInvite=self._accountsToInvite, keepCurrentView=self._keepCurrentView)
...
class EventBattleSquadEntity(SquadEntity):
        ...
        if self.isCommander(dbID=selfDBID):
            ...
            for slot in self.getSlotsIterator(unitId, unit):
                ...
                if pInfo.isReady and self.isCommander(dbID=dbID):
                    return pInfo.extraData.get(b'eventEnqueueData', {})
    ...
    def __showDialog(self, ctx):
        DialogsInterface.showDialog(rally_dialog_meta.createLeaveInfoMeta(ctx, b'eventDisabled'), (lambda _: None))
        return
