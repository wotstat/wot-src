...
class BaseVehiclesWatcher(object):
    ...
    def start(self):
        ...
        g_clientUpdateManager.addCallbacks({b'inventory': (self._update), 
           b'eventsData': (self._update)})
        self._isWatching = True
        _logger.info(b"BaseVehiclesWatcher:start() self = '%r'", self)
        return
    ...
    def stop(self):
        ...
        self._isWatching = False
        _logger.info(b"BaseVehiclesWatcher:stop() self = '%r'", self)
        return
