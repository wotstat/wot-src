...
class AbstractActionsHandler(object):
    ...
    def _sendBattleQueueRequest(self, vInvID=0, action=1):
        ctx = BattleQueueUnitCtx(b'prebattle/battle_queue', action=action)
        ...
        if pInfo.isCommander():
            ...
            if self._canDoAutoSearch(unit, stats):
                ...
                if self._entity.getFlags().isDevMode():
                    DialogsInterface.showDialog(rally_dialog_meta.UnitConfirmDialogMeta(PREBATTLE_TYPE.UNIT, b'startBattle'), (lambda result: self._sendBattleQueueRequest() if result else None))
                else:
