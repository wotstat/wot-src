...
class LegacyIntroEntryPoint(BasePrbEntryPoint):
    ...
    def create(self, ctx, callback=None):
        raise SoftException(b'PrbIntro is not create entity')
        return
...
class LegacyEntryPoint(BasePrbEntryPoint):
    ...
    def create(self, ctx, callback=None):
        LOG_ERROR(b'Routine "create" must be implemented in subclass')
        return
...
class _LegacyEntity(BaseLegacyEntity, ListenersCollection):
    ...
    def request(self, ctx, callback=None):
        ...
        if handler:
            LOG_DEBUG(b'Prebattle request', ctx)
...
class LegacyIntroEntity(_LegacyEntity):
    ...
    def _onListReceived(self, prebattles):
        self._invokeListeners(b'onLegacyListReceived', prebattles)
        return
...
class LegacyEntity(_LegacyEntity):
    ...
    def init(self, clientPrb=None, ctx=None):
        ...
        if clientPrb is not None:
            ...
            LOG_ERROR(b'ClientPrebattle is not defined')
    ...
    def getEntityType(self):
        if self._settings:
            return self._settings[b'type']
        return 0

    def getBonusType(self):
        if self._settings:
            return self._settings[b'bonusType']
        return 0
    ...
    def assign(self, ctx, callback=None):
        ...
        if prevTeam is nextTeam:
            if not pPermissions.canAssignToTeam(team=nextTeam, isSelfAssignment=pID == account_helpers.getPlayerID()):
                LOG_ERROR(b'Player can not change roster', nextTeam, assigned)
                if callback:
                    callback(False)
                return
        elif not pPermissions.canChangePlayerTeam():
            LOG_ERROR(b'Player can not change team', prevTeam, nextTeam)
            if callback:
                callback(False)
            return
        result = self.getLimits().isMaxCountValid(nextTeam, assigned)
        if result is not None and not result.isValid:
            LOG_ERROR(b'Max count limit', nextTeam, assigned)
    ...
    def setTeamState(self, ctx, callback=None):
        ...
        if not self.getPermissions().canSetTeamState(team=team):
            LOG_ERROR(b'Player can not change state of team', team)
            if callback:
                callback(False)
            return
    ...
    def setPlayerState(self, ctx, callback=None):
        ...
        if playerInfo is not None:
            ...
            LOG_ERROR(b'Account info not found in prebattle.rosters', ctx)
            if callback:
                callback(False)
        return
    ...
    def kickPlayer(self, ctx, callback=None):
        ...
        if not pPermissions.canKick(team=team):
            LOG_ERROR(b'Player can not kick from team', team, pPermissions)
            if callback:
                callback(False)
            return
        if assigned and self.getPlayerInfo(pID=pID, rosterKey=rosterKey).isReady():
            if self.getTeamState(team=team).isInQueue():
                LOG_ERROR(b'Player is ready, assigned and team is ready or locked', ctx)
                if callback:
                    callback(False)
                return
        ctx.startProcessing(callback)
    ...
    def swapTeams(self, ctx, callback=None):
        ...
        if pPermissions.canChangePlayerTeam():
            ...
            LOG_ERROR(b'Player can not swap teams', pPermissions)
            if callback:
                callback(False)
        return
    ...
    def sendInvites(self, ctx, callback=None):
        ...
        if pPermissions.canSendInvite():
            ...
            LOG_ERROR(b'Player can not send invite', pPermissions)
            if callback:
                callback(False)
        return
    ...
    def prb_onSettingUpdated(self, settingName):
        ...
        LOG_DEBUG(b'prb_onSettingUpdated', settingName, settingValue)
        self._invokeListeners(b'onSettingUpdated', self, settingName, settingValue)
        return

    def prb_onPropertyUpdated(self, propertyName):
        propertyValue = prb_getters.getClientPrebattle().properties[propertyName]
        LOG_DEBUG(b'prb_onPropertyUpdated', propertyName, propertyValue)
        self._invokeListeners(b'onPropertyUpdated', self, propertyName, propertyValue)
        return

    def prb_onTeamStatesReceived(self):
        team1State = self.getTeamState(team=1)
        team2State = self.getTeamState(team=2)
        LOG_DEBUG(b'prb_onTeamStatesReceived', team1State, team2State)
        if self._deferredReset and not self.getTeamState().isInQueue():
            self._deferredReset = False
            self.resetPlayerState()
        self._invokeListeners(b'onTeamStatesReceived', self, team1State, team2State)
        return

    def prb_onPlayerStateChanged(self, pID, roster):
        accountInfo = self.getPlayerInfo(pID=pID)
        LOG_DEBUG(b'prb_onPlayerStateChanged', accountInfo)
        self._invokeListeners(b'onPlayerStateChanged', self, roster, accountInfo)
        return

    def prb_onRosterReceived(self):
        LOG_DEBUG(b'prb_onRosterReceived')
        rosters = self.getRosters()
        self._invokeListeners(b'onRostersChanged', self, rosters, True)
        team = self.getPlayerTeam()
        self._invokeListeners(b'onPlayerTeamNumberChanged', self, team)
        return

    def prb_onPlayerRosterChanged(self, pID, prevRoster, roster, actorID):
        LOG_DEBUG(b'prb_onPlayerRosterChanged', pID, prevRoster, roster, actorID)
        ...
        if pID == account_helpers.getPlayerID():
            ...
            if currentTeam is not prevTeam:
                self._invokeListeners(b'onPlayerTeamNumberChanged', self, currentTeam)
        return

    def prb_onPlayerAdded(self, pID, roster):
        LOG_DEBUG(b'prb_onPlayerAdded', pID, roster)
    ...
    def prb_onPlayerRemoved(self, pID, roster, name):
        LOG_DEBUG(b'prb_onPlayerRemoved', pID, roster, name)
        rosters = self.getRosters(keys=[roster])
    ...
    def prb_onKickedFromQueue(self):
        LOG_DEBUG(b'prb_onKickedFromQueue')
    ...
    def _setTeamReady(self, ctx, callback=None):
        ...
        if result is None or result.isValid:
            ...
            LOG_ERROR(b'Team is invalid', notValidReason)
            if callback:
                callback(False)
    ...
    def _setPlayerNotReady(self, ctx, callback=None):
        ...
        if assigned and self.getTeamState(team=team).isInQueue():
            LOG_ERROR(b'Account assigned and team is ready or locked')
            if callback:
                callback(False)
            return
        ctx.startProcessing(callback)
    ...
    def _getPlayersStateStats(self, rosterKey):
        ...
        if clientPrb:
            ...
            limitMaxCount = teamLimits[b'maxCount'][not assigned]
            for _, accInfo in players.iteritems():
                state = accInfo.get(b'state', PREBATTLE_ACCOUNT_STATE.UNKNOWN)
