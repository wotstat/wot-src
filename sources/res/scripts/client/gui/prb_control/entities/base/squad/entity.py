...
class SquadEntryPoint(UnitEntryPoint):
    ...
    def makeDefCtx(self):
        return SquadSettingsCtx(waitingID=b'prebattle/create', accountsToInvite=self._accountsToInvite)

    def join(self, ctx, callback=None):
        super(SquadEntryPoint, self).join(ctx, callback)
        LOG_ERROR(b'Player can join to squad by invite only')
        if callback:
            callback(False)
        return
...
class SquadEntity(UnitEntity):
    ...
    def __resourceSplitter(self, resourceStr):
        resourceList = resourceStr.split(b'/')
        if not resourceList:
            return None
        else:
    ...
    def __showDefaultDialog(self, meta, callback, parent=None):
        ...
        if res:
            ...
            result = yield wg_await(showDynamicButtonInfoDialogBuilder(res, None, b'', parent))
            callback(result)
        return
