...
class UnitLevelsValidator(ExceptDevModeValidator):
    ...
    def _validate(self):
        ...
        if self._areVehiclesSelected(stats) and stats.curTotalLevel < roster.getMinTotalLevel():
            return ValidationResult(False, UNIT_RESTRICTION.MIN_TOTAL_LEVEL, {b'level': (roster.getMinTotalLevel())})
        if stats.curTotalLevel > roster.getMaxTotalLevel():
            return ValidationResult(False, UNIT_RESTRICTION.MAX_TOTAL_LEVEL, {b'level': (roster.getMaxTotalLevel())})
