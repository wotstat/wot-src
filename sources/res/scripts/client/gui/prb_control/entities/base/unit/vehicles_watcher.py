...
class UnitVehiclesWatcher(object):
    ...
    def init(self):
        g_clientUpdateManager.addCallbacks({b'inventory.1': (self.__onVehiclesUpdated)})
    ...
    def validate(self, update=False):
        ...
        if pInfo.isInSlot:
            ...
            if not roster.checkVehicleList(vehCDs, pInfo.slotIdx) and not pInfo.isCommander():
                self.__entity.request(AssignUnitCtx(pInfo.dbID, UNIT_SLOT.REMOVE, b'prebattle/assign'))
        elif update:
