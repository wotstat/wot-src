...
class TeamNoPlayersInBattle(ITeamLimit):
    ...
    def __isPlayerInBattle(self, player):
        return player[b'state'] & PREBATTLE_ACCOUNT_STATE.IN_BATTLE != 0
...
class TeamAllPlayersReady(AbstractTeamIsValid):
    ...
    def __isPlayerNotReady(self, player):
        return player[b'state'] & PREBATTLE_ACCOUNT_STATE.NOT_READY != 0
...
class VehiclesLevelLimit(ITeamLimit):
    ...
    def __calculate(self, rosters):
        ...
        for roster in rosters.itervalues():
            if not roster[b'state'] & PREBATTLE_ACCOUNT_STATE.READY:
                continue
            vehCompDescr = roster.get(b'vehCompDescr', b'')
            if vehCompDescr:
