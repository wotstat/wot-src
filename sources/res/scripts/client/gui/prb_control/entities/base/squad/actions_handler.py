...
class SquadActionsHandler(AbstractActionsHandler):
    ...
    def executeInit(self, ctx):
        ...
        if ctx is not None:
            ...
            if initCtx is not None and initCtx.getRequestType() is REQUEST_TYPE.CREATE:
                ...
                squadCtx = {b'showInvitesWindow': showInvitesWindow}
    ...
    def processInvites(self, accountsToInvite):
        if accountsToInvite:
            self._entity.request(SendInvitesCtx(accountsToInvite, b''))
    ...
    def _showInviteSentMessage(self, accountsToInvite):
        ...
        for dbID in accountsToInvite:
            user = getUser(dbID)
            if user is not None:
                SystemMessages.pushI18nMessage(b'#system_messages:prebattle/invites/sendInvite/name', type=SystemMessages.SM_TYPE.Information, name=user.getFullName())
            else:
