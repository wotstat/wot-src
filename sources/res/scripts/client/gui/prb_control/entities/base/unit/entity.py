...
class _UnitIntroEntryPoint(BasePrbEntryPoint):
    ...
    def create(self, ctx, callback=None):
        raise SoftException(b'UnitIntro is not create entity')
        return
    ...
    def join(self, ctx, callback=None):
        if not prb_getters.hasModalEntity() or ctx.isForced():
            ...
            LOG_ERROR(b'First, player has to confirm exit from the current prebattle/unit')
            if callback is not None:
                callback(False)
        return
    ...
    def create(self, ctx, callback=None):
        if not prb_getters.hasModalEntity() or ctx.isForced():
            ...
            if unitMgr:
                ...
                LOG_ERROR(b'Unit manager is not defined')
        else:
            LOG_ERROR(b'First, player has to confirm exit from the current prebattle/unit')
            if callback is not None:
                callback(False)
...
class _UnitEntity(BaseUnitEntity, ListenersCollection):
    ...
    def invalidateSelectedVehicles(self, vehInvCDs):
        ...
        if hasInvalidation:
            return SetVehicleUnitCtx(waitingID=b'prebattle/change_settings')
        else:
            return
    ...
    def request(self, ctx, callback=None):
        ...
        if requestType in self._requestHandlers:
            LOG_DEBUG(b'Unit request', ctx)
            self._requestHandlers[requestType](ctx, callback=callback)
        else:
            LOG_ERROR(b'Handler not found', ctx)
            if callback:
                callback(False)
        return
    ...
    def getSlotsIterator(self, unitMgrID, unit):
        ...
        for slotIdx in self._rosterSettings.getAllSlotsRange():
            ...
            if not state.isFree and slotIdx in members:
                dbID = members[slotIdx].get(b'accountDBID', -1)
                if dbID in players:
    ...
    def isCommander(self, dbID=None, unitMgrID=None):
        ...
        if unit is not None:
            return UnitPermissions.isCommander(unit.getPlayers().get(dbID, {}).get(b'role', 0))
        else:
...
class _UnitIntroEntity(_UnitEntity):
    ...
    def getUnit(self, unitMgrID=None, safe=True):
        if unitMgrID is None and not safe:
            LOG_ERROR(b'Unit index is not defined')
            return (None, None)
        else:
...
class UnitEntity(_UnitEntity):
    ...
    def rejoin(self):
        self._addClientUnitListeners()
        LOG_DEBUG(b'onUnitRejoin')
        self._invokeListeners(b'onUnitRejoin')
        return
    ...
    def initEvents(self, listener):
        ...
        if listener in self.getListenersIterator():
            ...
            LOG_ERROR(b'Listener not found', listener)
        return
    ...
    def getPermissions(self, dbID=None, unitMgrID=None):
        ...
        if unit is None:
            ...
            players = unit.getPlayers()
            if dbID in players:
                roles |= players[dbID].get(b'role', roles)
    ...
    def resetPlayerState(self):
        ...
        if pInfo.isCommander() and flags.isInSearch():
            self._requestsProcessor.doRawRequest(b'stopAutoSearch')
        elif pInfo.isReady:
    ...
    def resetPlayerReadiness(self):
        ctx = SetReadyUnitCtx(False, b'prebattle/player_not_ready')
    ...
    def assign(self, ctx, callback=None):
        ...
        if pInfo.dbID == -1:
            LOG_ERROR(b'Player is not in unit', ctx)
            if callback:
                callback(False)
            return
    ...
    def invite(self, ctx, callback=None):
        ...
        if not pPermissions.canSendInvite():
            LOG_ERROR(b'Player can not send invites', pPermissions)
            if callback:
                callback(False)
            return
        self._requestsProcessor.doRequest(ctx, b'invite', ctx.getDatabaseIDs(), ctx.getComment(), callback=callback)
    ...
    def kick(self, ctx, callback=None):
        ...
        if not pPermissions.canKick():
            LOG_ERROR(b'Player can not can another players', pPermissions)
            if callback:
                callback(False)
            return
        self._requestsProcessor.doRequest(ctx, b'kick', ctx.getPlayerID(), callback=callback)
        return

    def setVehicle(self, ctx, callback=None):
        pPermissions = self.getPermissions()
        if not pPermissions.canSetVehicle():
            LOG_ERROR(b'Player can not set vehicle', pPermissions)
            if callback:
                callback(False)
            return
            ...
            if vehsList:
                for vehInvID in vehsList:
                    if vehInvID != INV_ID_CLEAR_VEHICLE and self.itemsCache.items.getVehicle(vehInvID) is None:
                        LOG_ERROR(b'Vehicle is not in inventory', ctx)
                        if callback:
                            callback(False)
                        return

            self._requestsProcessor.doRequest(ctx, b'setVehicleList', vehicleList=vehsList, callback=callback)
    ...
    def setPlayerReady(self, ctx, callback=None):
        ...
        if not pInfo.isInSlot:
            LOG_ERROR(b'Player is not in slot', ctx)
            if callback:
                callback(False)
            return
        if pInfo.isReady is isReady:
            LOG_DEBUG(b'Player already ready', ctx)
            if callback:
                callback(True)
            return
        if not self.isVehiclesReadyToBattle():
            LOG_DEBUG(b'Vehicles are not ready to battle', ctx)
            if callback:
                callback(False)
            return
        if isReady:
    ...
    def closeSlot(self, ctx, callback=None):
        ...
        if not 1 << slotIdx & settings.UNIT_CLOSED_SLOTS_MASK:
            LOG_ERROR(b'Player can not close given slot', ctx)
            if callback:
                callback(False)
            return
        ...
        if isClosed:
            if slotState.isClosed:
                LOG_DEBUG(b'Slot already is closed', ctx)
                if callback:
                    callback(True)
                return
            if not slotState.isFree:
                LOG_ERROR(b'Player can close a free slot', ctx)
                if callback:
                    callback(False)
                return
        elif not slotState.isClosed:
            LOG_DEBUG(b'Slot already is opened', ctx)
            if callback:
                callback(True)
            return
        pPermissions = self.getPermissions()
        if not pPermissions.canChangeClosedSlots():
            LOG_ERROR(b'Player can not change closed slots', pPermissions)
            if callback:
                callback(False)
            return
        self._requestsProcessor.doRequest(ctx, b'closeSlot', slotIdx, isClosed, callback=callback)
    ...
    def changeOpened(self, ctx, callback=None):
        ...
        if isOpened is ctx.isOpened():
            LOG_DEBUG(b'Unit already is opened/closed', ctx)
            if callback:
                callback(True)
            return
        ...
        if not pPermissions.canChangeUnitState():
            LOG_ERROR(b'Player can not change unit state', pPermissions)
            if callback:
                callback(False)
            return
    ...
    def changeComment(self, ctx, callback=None):
        ...
        if not pPermissions.canChangeComment():
            LOG_ERROR(b'Player can not change comment', pPermissions)
            if callback:
                callback(False)
            return
        self._requestsProcessor.doRequest(ctx, b'setComment', cgi.escape(ctx.getComment()), callback=callback)
        self._cooldown.process(settings.REQUEST_TYPE.CHANGE_UNIT_STATE, coolDown=ctx.getCooldown())
        return
    ...
    def lock(self, ctx, callback=None):
        ...
        if isLocked is ctx.isLocked():
            LOG_DEBUG(b'Unit already is locked/unlocked', ctx)
            if callback:
                callback(True)
            return
        if self._isInCoolDown(settings.REQUEST_TYPE.CHANGE_UNIT_STATE, coolDown=ctx.getCooldown()):
        ...
        if not pPermissions.canChangeUnitState():
            ...
            if callback:
                callback(False)
            return
        self._requestsProcessor.doRequest(ctx, b'lockUnit', isLocked=ctx.isLocked(), callback=callback)
        self._cooldown.process(settings.REQUEST_TYPE.CHANGE_UNIT_STATE, coolDown=ctx.getCooldown())
        return
    ...
    def setRostersSlots(self, ctx, callback=None):
        ...
        if not pPermissions.canChangeRosters():
            LOG_ERROR(b'Player can not change rosters', pPermissions)
            if callback:
                callback(False)
            return
        self._requestsProcessor.doRequest(ctx, b'setAllRosterSlots', ctx.getRosterSlots(), callback=callback)
        return

    def doAutoSearch(self, ctx, callback=None, userFilterFlags=0):
        pPermissions = self.getPermissions()
        if not pPermissions.canStartAutoSearch():
            LOG_ERROR(b'Player can not start/stop auto search', pPermissions)
            if callback:
                callback(False)
            return
        flags = self.getFlags()
        if ctx.isRequestToStart():
            ...
            if self.isParentControlActivated(callback=callback):
                return
            if flags.isInSearch():
                LOG_DEBUG(b'Unit already started auto search')
                if callback:
                    callback(False)
                return
            ...
            if stats.curTotalLevel > roster.getMaxTotalLevel():
                LOG_ERROR(b'Total level is invalid', stats)
                if callback:
                    callback(False)
                return
            self._requestsProcessor.doRequest(ctx, b'startAutoSearch', userFilterFlags, callback=callback)
        elif not flags.isInSearch():
            LOG_DEBUG(b'Unit did not start auto search')
            if callback:
                callback(False)
        else:
            self._requestsProcessor.doRequest(ctx, b'stopAutoSearch', callback=callback)
    ...
    def doBattleQueue(self, ctx, callback=None):
        ...
        if ctx.isRequestToStart():
            ...
            if not pPermissions.canStartBattleQueue():
                LOG_ERROR(b'Player can not start battle queue', pPermissions)
                if callback:
                    callback(False)
                return
            if flags.isInQueue():
                LOG_DEBUG(b'Unit already started battle queue')
                if callback:
                    callback(True)
                return
            ...
            if not stateAllowStartBattle.isValid:
                ...
                return
            if not self.isVehiclesReadyToBattle():
                LOG_ERROR(b'Vehicles are not ready to battle', ctx)
                if callback:
                    callback(False)
                return
            vehInfos = self.getVehiclesInfo()
            ...
            LOG_ERROR(b'Player can not stop battle queue', pPermissions)
            if callback:
                callback(False)
            return
        if not flags.isInQueue():
            LOG_DEBUG(b'Unit did not start battle queue')
            if callback:
                callback(True)
        else:
    ...
    def giveLeadership(self, ctx, callback=None):
        ...
        if not pPermissions.canChangeLeadership():
            LOG_ERROR(b'Player can not give leadership to player players', pPermissions)
            if callback:
                callback(False)
            return
        otherPPermissions = self.getPermissions(ctx.getPlayerID())
        if not otherPPermissions.canLead():
            LOG_ERROR(b'Player can not take leadership', pPermissions)
            if callback:
                callback(False)
            return
        self._requestsProcessor.doRequest(ctx, b'giveLeadership', ctx.getPlayerID(), callback=callback)
        return
    ...
    def changeDivision(self, ctx, callback=None):
        ...
        pPermissions = self.getPermissions()
        if not pPermissions.canChangeRosters():
            LOG_ERROR(b'Player can not change division', pPermissions)
            if callback:
                callback(False)
            return
        self._requestsProcessor.doRequest(ctx, b'changeSortieDivision', division=ctx.getDivisionID(), callback=callback)
    ...
    def togglePlayerReadyAction(self, launchChain=False):
        ...
        if launchChain:
            if notReady:
                selVehCtx = SetVehicleUnitCtx(vTypeCD=g_currentVehicle.item.intCD, waitingID=b'prebattle/change_settings')
    ...
    def unit_onUnitFlagsChanged(self, prevFlags, nextFlags):
        ...
        self._invokeListeners(b'onUnitFlagsChanged', flags, timeLeftInIdle)
        ...
        for slotIdx in self._rosterSettings.getAllSlotsRange():
            if prevFlags is not nextFlags:
                if slotIdx not in members:
                    continue
                dbID = members[slotIdx][b'accountDBID']
    ...
    def unit_onUnitVehicleChanged(self, dbID, vehInvID, vehTypeCD):
        ...
        self._invokeListeners(b'onUnitVehiclesChanged', dbID, (vInfo,))
        g_eventDispatcher.updateUI()
        return
    ...
    def unit_onUnitVehiclesChanged(self, dbID, vehicles):
        ...
        self._invokeListeners(b'onUnitVehiclesChanged', dbID, vInfos)
        g_eventDispatcher.updateUI()
        return
    ...
    def unit_onUnitPlayerAdded(self, playerID, playerData):
        ...
        self._invokeListeners(b'onUnitPlayerAdded', pInfo)
        return

    def unit_onUnitPlayerInfoChanged(self, playerID, playerData):
        unitMgrID, unit = self.getUnit(safe=False)
        pInfo = self._buildPlayerInfo(unitMgrID, unit, playerID, unit.getPlayerSlotIdx(playerID), playerData)
        self._invokeListeners(b'onUnitPlayerInfoChanged', pInfo)
        self._actionsHandler.setPlayerInfoChanged()
        return

    def unit_onUnitPlayerProfileVehicleChanged(self, accountDBID):
        self._invokeListeners(b'onUnitPlayerProfileVehicleChanged', accountDBID)
        g_eventDispatcher.updateUI()
        return

    def unit_onUnitSearchFlagsChanged(self, flags):
        self._invokeListeners(b'onUnitSearchFlagsChanged', flags)
        return
    ...
    def unit_onUnitPlayerRemoved(self, playerID, playerData):
        ...
        self._invokeListeners(b'onUnitPlayerRemoved', pInfo)
        return

    def unit_onUnitSettingChanged(self, opCode, value):
        self._invokeListeners(b'onUnitSettingChanged', opCode, value)
        return
    ...
    def unit_onUnitRosterChanged(self):
        ...
        if rosterSettings != self._rosterSettings:
            ...
            self._vehiclesWatcher.validate()
        self._invokeListeners(b'onUnitRosterChanged')
        return

    def unit_onUnitMembersListChanged(self):
        self._invokeListeners(b'onUnitMembersListChanged')
        g_eventDispatcher.updateUI()
        return

    def unit_onUnitPlayersListChanged(self):
        self._actionsHandler.setPlayersChanged()
        self._invokeListeners(b'onUnitPlayersListChanged')
        return
    ...
    def unit_onUnitPlayerVehDictChanged(self, playerID):
        ...
        self._invokeListeners(b'onUnitPlayerVehDictChanged', pInfo)
        return

    def unit_onUnitExtraChanged(self, extras):
        self._invokeListeners(b'onUnitExtraChanged', extras)
        return

    def unit_onUnitSizeChanged(self):
        self._invokeListeners(b'onUnitSizeChanged')
        return
    ...
    def _unassign(self, ctx, callback=None):
        ...
        if not pPermissions.canAssignToSlot(dbID):
            LOG_ERROR(b'Player can not unassign from slot', pPermissions)
            if callback:
                callback(False)
            return
        if slotIdx == settings.CREATOR_SLOT_INDEX:
            LOG_ERROR(b'In first slot can be creator only')
            if callback:
                callback(False)
            return
        self._requestsProcessor.doRequest(ctx, b'unassign', dbID, callback=callback)
        return
    ...
    def _assign(self, ctx, callback=None):
        ...
        pPermissions = self.getPermissions()
        if not pPermissions.canAssignToSlot(dbID):
            LOG_ERROR(b'Player can not assign to slot', pPermissions)
            if callback:
                callback(False)
            return
        self._requestsProcessor.doRequest(ctx, b'assign', dbID, slotIdx, callback=callback)
        return
    ...
    def _reassign(self, ctx, callback=None):
        ...
        if pPermissions.canReassignToSlot():
            self._requestsProcessor.doRequest(ctx, b'reassign', dbID, slotIdx, callback=callback)
        elif pPermissions.canAssignToSlot(dbID):
            self._requestsProcessor.doRequest(ctx, b'assign', dbID, slotIdx, callback=callback)
        else:
            LOG_ERROR(b'Player can not (re)assign to slot', pPermissions)
            if callback:
                callback(False)
        return
    ...
    def _setVehicle(self, ctx, callback=None):
        ...
        if vehInvID:
            ...
            LOG_ERROR(b'Context is invalid', ctx)
            if callback:
                callback(False)
            return
        vInfos = self.getVehiclesInfo()
        if findFirst((lambda vInfo: vInfo.vehInvID == vehInvID), vInfos) is not None:
            LOG_DEBUG(b'Player already selected vehicle', ctx)
            if callback:
                callback(True)
            return
        setReadyAfterVehicleSelect = ctx.setReady
        self._requestsProcessor.doRequest(ctx, b'setVehicle', vehInvID=vehInvID, setReady=setReadyAfterVehicleSelect, callback=callback)
    ...
    def _clearVehicle(self, ctx, callback=None):
        ...
        if findFirst((lambda vInfo: vInfo.vehInvID), vInfos) is None:
            LOG_DEBUG(b'There is not vehicle in slot', ctx)
            if callback:
                callback(True)
            return
        self._requestsProcessor.doRequest(ctx, b'setVehicle', INV_ID_CLEAR_VEHICLE, callback=callback)
        return
    ...
    def _resetPlayerReadiness(self, ctx, callback=None):
        pInfo = self.getPlayerInfo()
        if not pInfo.isInSlot:
            LOG_DEBUG(b'Player is not in slot', ctx)
            if callback:
                callback(False)
            return
        if self._isInCoolDown(settings.REQUEST_TYPE.SET_PLAYER_STATE, coolDown=ctx.getCooldown()):
    ...
    def _setReady(self, ctx, callback=None):
        ...
        if not pPermissions.canSetReady():
            LOG_ERROR(b'Player can not set ready state', pPermissions)
            if callback:
                callback(False)
            return
        self._requestsProcessor.doRequest(ctx, b'setReady', ctx.isReady(), ctx.resetVehicle, callback=callback)
        self._cooldown.process(settings.REQUEST_TYPE.SET_PLAYER_STATE, coolDown=ctx.getCooldown())
        return
    ...
    def _doStopBattleRequest(self, ctx, callback):
        self._requestsProcessor.doRequest(ctx, b'stopBattle', callback=callback)
        return
