...
class TournamentDynamicRosterSettings(DynamicRosterSettings):
    ...
    def _extractSettings(self, unit, tournamentData):
        if not tournamentData.isValid():
            _logger.error(b'Unit roster is not definded')
...
class TournamentEntryPoint(UnitEntryPoint):
    ...
    def join(self, ctx, callback=None):
        ...
        processor.doRequest(TournamentJoinBattleCtx(ctx.getID()), b'join', callback=onResponse)
        return
...
class TournamentBrowserEntity(UnitBrowserEntity):
    ...
    def leave(self, ctx, callback=None):
        processor = TournamentUnitRequestProcessor()
        processor.doRequest(TournamentLeaveModeCtx(ctx.getID()), b'leave_mode')
...
class TournamentEntity(UnitEntity):
    ...
    def rejoin(self):
        ...
        if self.isTournamentUnitWaitingForData():
            _logger.debug(b'force wgsh request on end of battle')
            self.requestUpdateTournament()
        return
    ...
    def unit_onUnitFlagsChanged(self, prevFlags, nextFlags):
        ...
        if regularBattleEnd or wgshBattleEnd:
            _logger.debug(b'force wgsh request on end of battle (r,x): %s, %s', str(regularBattleEnd), str(wgshBattleEnd))
            self.requestUpdateTournament()
            self.requestSlotVehicleFilters()
    ...
    def setReserve(self, ctx, callback=None):
        ...
        if not pPermissions.canChangeConsumables():
            _logger.error(b'Player can not change consumables %s', str(pPermissions))
            if callback:
                callback(False)
            return
    ...
    def requestMaintenanceUpdate(self):
        self._invokeListeners(b'onTournamentMaintenance', False)
        self.requestUpdateTournament()
        return
    ...
    def requestUpdateTournament(self):
        if self._requestsProcessor:
            ...
            rev = self._getUnitRevision()
            ctx = TournamentUpdateCtx(unitMgrId=unitMgrId, rev=rev, waitingID=b'')
            self._requestsProcessor.doRequest(ctx, b'updateTournament', callback=self.__onTournamentUpdate)
        return

    def tournamentDataChanged(self):
        self._updateMatchmakingTimer()
        self._invokeListeners(b'onTournamentDataChanged')
        return
    ...
    def _updateMatchmakingTimer(self):
        ...
        if tempInactiveMatchingButton != self.__isInactiveMatchingButton:
            self._invokeListeners(b'onTournamentOnReadyStateChanged')
        ...
        if isinstance(response, Response):
            ...
            if isinstance(data, dict):
                ...
                if webReqID is not None:
                    _logger.debug(b'Web response requestID = {%s}', str(webReqID))
    ...
    def __onUpdateHeader(self):
        ...
        self._invokeListeners(b'onUpdateHeader', header, isFirstBattle, self.isTournamentUnitFreezed())
        return

    def __onUpdateTimer(self):
        self._invokeListeners(b'onUpdateTimer', self.__tournamentSettings.getTimer())
        return
    ...
    def __onUpdateReserve(self):
        self._invokeListeners(b'onUpdateReserve', self.__tournamentSettings.getReserve(), self.__tournamentSettings.getReserveOrder())
        return
