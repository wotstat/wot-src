...
class StrongholdDynamicRosterSettings(DynamicRosterSettings):
    ...
    def _extractSettings(self, unit, strongholdData):
        if not strongholdData.isValid():
            LOG_ERROR(b'Unit roster is not definded')
...
class StrongholdEntity(UnitEntity):
    ...
    def onUnfreezeVehicleInSlot(self, slotIdx, parent=None):
        if not self.__isStrongholdEventEnabled():
            ...
            self.showDialog(UnfreezeVehicleDialogMeta(slotInfo.player.name, vehicle.shortUserName if vehicle is not None else b'', price, self.__eventFrozenVehiclesRequester.getSparePartsBalance()), (lambda result: self.__onUnfreezeVehicleDialogCallback(result, slotInfo.player.dbID, vehicleCD, price)), parent=parent)
            return
    ...
    def __onUnfreezeVehicleDialogCallback(self, result, playerSpaID, vehicleCD, price):
        ...
        self._requestsProcessor.doRequest(ctx, b'unfreezeVehicle', callback=partial(self.__unfreezeVehicleRequestCallback, playerSpaID=playerSpaID))
        return

    def __unfreezeVehicleRequestCallback(self, response, playerSpaID):
        rawData = response.getData()
        if response.isSuccess():
            vehicleCD = rawData.get(b'vehicle_cd')
            vehicle = self.itemsCache.items.getItemByCD(vehicleCD)
            SystemMessages.pushMessage(backport.text(R.strings.system_messages.unit.warnings.wgshEvent.vehicleUnfrozen(), vehicleName=vehicle.shortUserName if vehicle is not None else b''), type=SM_TYPE.Warning, priority=NotificationPriorityLevel.MEDIUM)
            ...
            if isinstance(rawData, dict):
                resMsg = R.strings.system_messages.unit.error.wgshEvent.dyn(rawData.get(b'title', b''))
            else:
