...
class MapsTrainingEntity(PreQueueEntity):
    _QUEUE_TIMEOUT_MSG_KEY = b'#maps_training:arena_start_errors/prb/kick/timeout'
    ...
    def _doQueue(self, ctx):
        ...
        LOG_DEBUG(b'Sends request on queuing to the maps training', ctx)
        return

    def _doDequeue(self, ctx):
        BigWorld.player().dequeueMapsTraininig()
        LOG_DEBUG(b'Sends request on dequeuing from the maps training')
        return
    ...
    def _makeQueueCtxByAction(self, action=None):
        ...
        return MapsTrainingQueueCtx(mapGeometryID, vehCompDescr, team, waitingID=b'prebattle/join')
