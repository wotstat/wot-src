...
class TrainingSettingsCtx(TeamSettingsCtx):
    __slots__ = (b'__arenaTypeID', b'__roundLen', b'__isDevBattle')

    def __init__(self, waitingID=b'', isOpened=True, comment=b'', isRequestToCreate=True, arenaTypeID=0, roundLen=900, flags=_FUNCTIONAL_FLAG.UNDEFINED, isDevBattle=False):
    ...
    def fetch(cls, settings):
        return TrainingSettingsCtx(isOpened=settings[b'isOpened'], comment=settings[b'comment'], isRequestToCreate=False, arenaTypeID=settings[b'arenaTypeID'], roundLen=settings[b'roundLength'], isDevBattle=settings[b'extraData'].get(b'isDevBattle', False))
