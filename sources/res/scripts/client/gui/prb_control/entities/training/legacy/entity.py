...
class TrainingEntryPoint(LegacyEntryPoint):
    ...
    def create(self, ctx, callback=None):
        if not isinstance(ctx, TrainingSettingsCtx):
            LOG_ERROR(b'Invalid context to create training', ctx)
            ...
            LOG_ERROR(b'First, player has to confirm exit from the current prebattle', prb_getters.getPrebattleType())
            if callback is not None:
                callback(False)
        return
...
class TrainingEntity(LegacyEntity):
    ...
    def init(self, clientPrb=None, ctx=None):
        ...
        if self.storage.arenaGuiType is None:
            self.storage.arenaGuiType = self.getSettings()[b'arenaGuiType']
    ...
    def prb_onPropertyUpdated(self, propertyName):
        checker = getTrainingRoomHandler(self.getSettings()[b'arenaGuiType']).getPrebattlePropertyChecker()
    ...
    def changeSettings(self, ctx, callback=None):
        if ctx.getRequestType() != REQUEST_TYPE.CHANGE_SETTINGS:
            LOG_ERROR(b'Invalid context for request changeSettings', ctx)
            if callback is not None:
                callback(False)
            return
        ...
        if isOpenedChanged:
            if pPermissions.canMakeOpenedClosed():
                self.__settingRecords.append(b'isOpened')
            else:
                LOG_ERROR(b'Player can not make training opened/closed', pPermissions)
                rejected = True
        if isCommentChanged:
            if pPermissions.canChangeComment():
                self.__settingRecords.append(b'comment')
            else:
                LOG_ERROR(b'Player can not change comment', pPermissions)
                rejected = True
        if isArenaTypeIDChanged:
            if pPermissions.canChangeArena():
                self.__settingRecords.append(b'arenaTypeID')
            else:
                LOG_ERROR(b'Player can not change comment', pPermissions)
                rejected = True
        if isRoundLenChanged:
            if pPermissions.canChangeArena():
                self.__settingRecords.append(b'roundLength')
            else:
                LOG_ERROR(b'Player can not change comment', pPermissions)
                rejected = True
        if rejected:
        ...
        ctx.startProcessing(callback=callback)
        if isOpenedChanged:
            player.prb_changeOpenStatus(ctx.isOpened(), partial(self.__onSettingChanged, record=b'isOpened', callback=ctx.stopProcessing))
        if isCommentChanged:
            player.prb_changeComment(ctx.getComment(), partial(self.__onSettingChanged, record=b'comment', callback=ctx.stopProcessing))
        if isArenaTypeIDChanged:
            player.prb_changeArena(ctx.getArenaTypeID(), partial(self.__onSettingChanged, record=b'arenaTypeID', callback=ctx.stopProcessing))
        if isRoundLenChanged:
            player.prb_changeRoundLength(ctx.getRoundLen(), partial(self.__onSettingChanged, record=b'roundLength', callback=ctx.stopProcessing))
        if not self.__settingRecords:
            if callback is not None:
                callback(False)
        else:
    ...
    def changeUserObserverStatus(self, ctx, callback=None):
        if ctx.isObserver():
            ...
            self._setPlayerReady(SetPlayerStateCtx(ctx.isReadyState(), ctx.isInitial(), waitingID=b'prebattle/player_ready'), self.__onPlayerReady)
    ...
    def changeArenaVoip(self, ctx, callback=None):
        ...
        if pPermissions.canChangeArenaVOIP():
            ...
            LOG_ERROR(b'Player can not change arena VOIP', pPermissions)
            if callback is not None:
                callback(False)
        return
    ...
    def _validateStatusIfModeChanged(self):
        ...
        if self.storage.isObserver:
            resetCtx = SetPlayerObserverStateCtx(False, False, waitingID=b'prebattle/change_user_status')
        else:
            resetCtx = SetPlayerStateCtx(False, waitingID=b'prebattle/player_not_ready')
    ...
    def __enterTrainingRoom(self, isInitial=False):
        self.__updateVehiclesWatcher()
        if self.storage.arenaGuiType is not None and self.storage.arenaGuiType != self.getSettings()[b'arenaGuiType']:
            self._validateStatusIfModeChanged()
            return
        else:
            if self.storage.isObserver:
                self.changeUserObserverStatus(SetPlayerObserverStateCtx(True, True, isInitial=isInitial, waitingID=b'prebattle/change_user_status'), self.__onPlayerReady)
            else:
                self.setPlayerState(SetPlayerStateCtx(True, isInitial=isInitial, waitingID=b'prebattle/player_ready'), self.__onPlayerReady)
            return
    ...
    def __onSettingChanged(self, code, record=b'', errorCode=None, callback=None):
        if code < 0:
            LOG_ERROR(b'Server return error for training change', code, record, errorCode)
            if callback is not None:
    ...
    def __validateStatus(self, resetResult):
        ...
        self._setPlayerReady(SetPlayerStateCtx(True, waitingID=b'prebattle/player_ready'), self.__onPlayerReady)
        return

    def __updateVehiclesWatcher(self):
        watcherType = getTrainingRoomHandler(self.getSettings()[b'arenaGuiType']).getVehicleWatcherType()
    ...
    def __updateTrainingLimits(self):
        limits = getTrainingRoomHandler(self.getSettings()[b'arenaGuiType']).getPrebattleLimits()
        if limits is None:
