...
class RankedEntity(PreQueueEntity):
    ...
    def _doQueue(self, ctx):
        ...
        _logger.debug(b'Sends request on queuing to the ranked battle %s', ctx)
        return

    def _doDequeue(self, ctx):
        BigWorld.player().dequeueRanked()
        _logger.debug(b'Sends request on dequeuing from the ranked battle')
        return
    ...
    def _makeQueueCtxByAction(self, action=None):
        ...
        return RankedQueueCtx(invID, waitingID=b'prebattle/join')
    ...
    def __processWelcome(self):
        ...
        if rankedWelcomeCallback is None:
            if not filters[b'isRankedWelcomeViewShowed']:
