...
class ESportActionsHandler(UnitActionsHandler):
        ...
        if pInfo.isCommander():
            ...
            if self._canDoAutoSearch(unit, stats):
                ...
                if self._entity.getFlags().isDevMode():
                    DialogsInterface.showDialog(rally_dialog_meta.UnitConfirmDialogMeta(PREBATTLE_TYPE.UNIT, b'startBattle'), (lambda result: self._entity.doBattleQueue(BattleQueueUnitCtx(b'prebattle/battle_queue')) if result else None))
                else:
    ...
    def __onKickedFromQueue(self):
        SystemMessages.pushI18nMessage(b'#system_messages:arena_start_errors/prb/kick/timeout', type=SystemMessages.SM_TYPE.Warning)
        return
