...
class _Comp7PlayerValidator(UnitPlayerValidator):
    ...
    def _validate(self):
        if self.__comp7Ctrl.isBanned:
            ...
            if ratings and max(ratings) - min(ratings) > self.__comp7Ctrl.getPlatoonRatingRestriction():
                return ValidationResult(False, UNIT_RESTRICTION.RATING_RESTRICTION, None)
    ...
    def __getPlayersRatings(self):
        ...
        for slotData in self.__platoonCtrl.getPlatoonSlotsData():
            ...
            if playerData is None:
                continue
            comp7EnqueueData = playerData.get(b'extraData', {}).get(b'comp7EnqueueData', {})
            playersRating.append(comp7EnqueueData.get(b'rating', 0))
...
class _Comp7ModeStatusValidator(BaseActionsValidator):
    ...
    def __isModeOfflineForPlayer(pInfo):
        comp7EnqueueData = pInfo.extraData.get(b'comp7EnqueueData', {})
        isOnline = bool(comp7EnqueueData.get(b'isOnline', 0))
        return not isOnline
