        ...
        if not self.__mapboxController.isEnabled():
            ...
            if isPrimeTime != self.__isPrimeTime:
                ...
                if not isPrimeTime and self.__mapboxController.getCurrentCycleID() is not None:
                    SystemMessages.pushMessage(backport.text(R.strings.system_messages.mapbox.notification.primeTime()), type=SystemMessages.SM_TYPE.PrimeTime, messageData={b'title': (backport.text(R.strings.system_messages.royale.notification.primeTime.title()))})
