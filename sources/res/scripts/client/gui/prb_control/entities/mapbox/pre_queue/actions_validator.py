    ...
    def validateForMapbox(vehicle):
        if vehicle is None:
            ...
            if vehicle.level not in config.levels:
                return ValidationResult(False, PRE_QUEUE_RESTRICTION.LIMIT_LEVEL, {b'levels': (config.levels)})
            if vehicle.intCD in config.forbiddenVehTypes:
                return ValidationResult(False, PRE_QUEUE_RESTRICTION.LIMIT_VEHICLE_TYPE, {b'forbiddenType': (vehicle.shortUserName)})
            if vehicle.type in config.forbiddenClassTags:
                return ValidationResult(False, PRE_QUEUE_RESTRICTION.LIMIT_VEHICLE_CLASS, {b'forbiddenClass': (vehicle.type)})
            return
