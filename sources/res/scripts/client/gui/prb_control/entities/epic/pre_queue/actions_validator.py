...
class EpicVehicleValidator(BaseActionsValidator):
    ...
    def _validate(self):
        ...
        if vehicle.level not in config.validVehicleLevels:
            return ValidationResult(False, PRE_QUEUE_RESTRICTION.LIMIT_LEVEL, {b'levels': vehicleLevelsForStartBattle})
        forbiddenVehicles = config.forbiddenVehTypes
        if vehicle.intCD in forbiddenVehicles:
            return ValidationResult(False, PREBATTLE_RESTRICTION.VEHICLE_NOT_SUPPORTED, {b'vehicles': forbiddenVehicles})
        if isVehLevelUnlockableInBattle(vehicle.level):
            return ValidationResult(False, PRE_QUEUE_RESTRICTION.VEHICLE_WILL_BE_UNLOCKED, {b'unlockableInBattleVehLevels': (config.unlockableInBattleVehLevels), 
               b'vehicleLevelsForStartBattle': vehicleLevelsForStartBattle})
