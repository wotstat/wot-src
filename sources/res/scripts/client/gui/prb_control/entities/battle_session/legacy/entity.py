...
class BattleSessionListEntryPoint(LegacyEntryPoint):
    ...
    def create(self, ctx, callback=None):
        raise SoftException(b'BattleSession can be created through the web only')
        return
...
class BattleSessionEntity(LegacyEntity):
    ...
    def getRoles(self, pDatabaseID=None, clanDBID=None, team=None):
        ...
        if self._settings is None or self._settings[b'type'] != PREBATTLE_TYPE.CLAN:
            return result
        if not result:
    ...
    def doAction(self, action=None):
        if self.getPlayerInfo().isReady():
            ...
            self.setPlayerState(SetPlayerStateCtx(True, waitingID=b'prebattle/player_ready'))
        return True
