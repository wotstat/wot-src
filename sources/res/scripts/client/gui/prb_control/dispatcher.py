...
class _PreBattleDispatcher(ListenersCollection):
    ...
    def __del__(self):
        LOG_DEBUG(b'_PreBattleDispatcher deleted')
        return
    ...
    def create(self, ctx, callback=None):
        if ctx.getRequestType() != _RQ_TYPE.CREATE:
            LOG_ERROR(b'Invalid context to create prebattle/unit', ctx)
        ...
        if entry is None:
            LOG_ERROR(b'Entry not found', ctx)
            if callback is not None:
                callback(False)
            return
        ...
        if not result:
            ...
            return
        ctx.setForced(True)
        LOG_DEBUG(b'Request to create', ctx)
    ...
    def join(self, ctx, callback=None):
        if ctx.getRequestType() != _RQ_TYPE.JOIN:
            LOG_ERROR(b'Invalid context to join prebattle/unit', ctx)
            if callback is not None:
                callback(False)
            return
        entry = self.__factories.createEntry(ctx)
        ...
        if not result:
            ...
            return
        ctx.setForced(True)
        LOG_DEBUG(b'Request to join', ctx)
        self.__requestCtx = ctx
    ...
    def leave(self, ctx, callback=None, ignoreConfirmation=False, parent=None):
        if ctx.getRequestType() != _RQ_TYPE.LEAVE:
            LOG_ERROR(b'Invalid context to leave prebattle/unit', ctx)
            if callback is not None:
                callback(False)
            return
        if self.__requestCtx.isProcessing():
            LOG_ERROR(b'Request is processing', self.__requestCtx)
            if callback is not None:
                callback(False)
            return
    ...
    def __leaveLogic(self, ctx, callback):
        ...
        if entity.hasLockedState():
            ...
            if callback is not None:
                callback(False)
            return
        LOG_DEBUG(b'Request to leave formation', ctx)
        self.__requestCtx = ctx
    ...
    def unlock(self, unlockCtx, callback=None):
        ...
        if factory is None:
            LOG_ERROR(b'Factory is not found', state)
            if callback is not None:
                callback(True)
            return
    ...
    def select(self, entry, callback=None):
        ...
        if not result:
            ...
            return
        ctx.setForced(True)
        LOG_DEBUG(b'Request to select', ctx)
        self.__requestCtx = ctx
    ...
    def doAction(self, action=None):
        if self.__entity.needsCheckVehicleForBattle:
            if not (g_currentVehicle.isPresent() or g_currentPreviewVehicle.isPresent()):
                ...
                return False
        LOG_DEBUG(b'Do GUI action', action)
    ...
    def doLeaveAction(self, action, callback=None):
        factory = self.__factories.get(self.__entity.getCtrlType())
        if factory is None:
            LOG_ERROR(b'Factory is not found', self.__entity)
            if callback is not None:
                callback(True)
    ...
    def gs_onTillBanNotification(self, isPlayTimeBan, timeTillBlock):
        if prb_getters.isParentControlActivated():
            ...
            if isPlayTimeBan:
                SystemMessages.pushI18nMessage(key.format(b'playTimeNotification'), timeTillBlock, type=SystemMessages.SM_TYPE.Warning)
            else:
                _, blockTime = self.gameSession.getCurfewBlockTime()
                formatter = lambda t: time.strftime(b'%H:%M', time.localtime(t))
                SystemMessages.pushI18nMessage(key.format(b'midnightNotification'), type=SystemMessages.SM_TYPE.Warning, blockTime=formatter(blockTime))
        return
    ...
    def notifyPrbEntitySwitched(self):
        if self.__isStarted:
            self._invokeListeners(b'onPrbEntitySwitched')
        return
    ...
    def __validateJoinOp(self, ctx):
        ...
        if self.__entity.isPlayerJoined(ctx):
            LOG_DEBUG(b'Player already joined', ctx)
            self.__entity.showGUI(ctx)
            return False
        if self.__entity.hasLockedState():
            SystemMessages.pushI18nMessage(b'#system_messages:prebattle/hasLockedState', type=SystemMessages.SM_TYPE.Warning)
            return False
        return True

    def __unsetEntity(self, ctx=None):
        if not isinstance(self.__entity, NotSupportedEntity):
            self._invokeListeners(b'onPrbEntitySwitching')
    ...
    def __setEntity(self, ctx):
        ...
        if created is not None:
            ...
            if self.__entity is not None and not isinstance(self.__entity, NotSupportedEntity):
                factory = self.__factories.get(self.__entity.getCtrlType())
                if factory is not None:
                    _logger.info(b"__setEntity() new entity '%r' was created, previous entity '%r' was stopped", created, self.__entity)
        ...
        LOG_DEBUG(b'Entity have been updated', ctx.getFlagsToStrings())
        ctx.clear()
...
class _PrbPeripheriesHandler(object):
    ...
    def __del__(self):
        LOG_DEBUG(b'_PrbPeripheriesHandler deleted')
        return
    ...
    def join(self, peripheryID, ctx, postActions=None, finishActions=None):
        if not self.lobbyContext.isAnotherPeriphery(peripheryID):
            LOG_ERROR(b'Player is in given periphery', peripheryID)
            return
        else:
            if postActions:
                if isinstance(postActions, types.ListType):
                    ...
                    LOG_ERROR(b'Argument "postActions" is invalid', postActions)
                    return
            else:
                actionsList = []
            actionsList.extend([
            ...
            if finishActions:
                if isinstance(finishActions, types.ListType):
                    ...
                    LOG_ERROR(b'Argument "finishActions" is invalid', finishActions)
                    return
            self.__joinCtx = ctx
            if self.__joinChain is not None:
                self.__joinChain.onStopped -= self.__onJoinChainStopped
...
class _PrbControlLoader(IPrbControlLoader):
    __slots__ = (b'__prbDispatcher', b'__invitesManager', b'__autoNotifier', b'__peripheriesHandler', b'__storage', b'__isEnabled')
