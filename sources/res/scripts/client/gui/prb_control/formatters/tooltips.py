...
def getAbsenceCrewList():
    ...
    for slotIdx, tman in vehicle.crew:
        if tman is None:
            ...
            crewList += (b', ' if crewList else b'') + userString
...
def getCrewNotFullTooltip():
    ...
    return makeTooltip(b'#tooltips:redButton/disabled/crew/notFull/header', i18n.makeString(b'#tooltips:redButton/disabled/crew/notFull/body') % crewList)


def getVehicleStateInvalidTooltip(restriction):
    return (b'#tooltips:redButton/disabled/{0:>s}').format(restriction)
...
def getVehicleClassInvalidTooltip(teamsLimit, restriction):
    ...
    return makeTooltip(i18n.makeString((b'#tooltips:redButton/disabled/{0:>s}/header').format(restriction)), i18n.makeString((b'#tooltips:redButton/disabled/{0:>s}/body').format(restriction), minLevel, maxLevel))
    ...
    if not restriction:
        ...
        if restriction == PREBATTLE_RESTRICTION.CREW_NOT_FULL:
            ...
            if PREBATTLE_RESTRICTION.inVehClassLimit(restriction):
                ...
                LOG_ERROR(b'Formatter not found', restriction)
        return tooltip
