...
def getJoinFailureMessage(errorCode):
    if errorCode in JOIN_FAILURE_NAMES:
        ...
        error = errorCode
    return i18n.makeString(b'#system_messages:arena_start_errors/join/%s' % error)
...
def getKickReasonMessage(reasonCode):
    if reasonCode in KICK_REASON_NAMES:
        ...
        reason = reasonCode
        LOG_ERROR(b'Invalid sub key %s for localization #system_messages:arena_start_errors/kick/' % reason)
    return i18n.makeString(b'#system_messages:arena_start_errors/kick/%s' % reason)
...
def getPrbKickedFromQueueMessage(prbTypeName):
    ...
    if guiName != b'squad':
        guiName = b'default'
    return i18n.makeString((b'#system_messages:prebattle_start_failed/kickedFromQueue/{0:>s}').format(guiName))


def getVehicleNotPresentMessage():
    return i18n.makeString(b'#system_messages:prebattle/vehicleInvalid/no_selectedVehicle')


def getVehicleNotReadyMessage():
    return i18n.makeString(b'#system_messages:prebattle/vehicleInvalid/no_readyVehicle')


def getVehicleNotSupportedMessage():
    return i18n.makeString(b'#system_messages:prebattle/vehicleInvalid/vehicleNotSupported')


def getVehicleEpicOnlyMessage():
    return i18n.makeString(b'#system_messages:prebattle/vehicleInvalid/epicOnly')


def getVehicleWillBeUnlockedInBattleMessage():
    return i18n.makeString(b'#system_messages:prebattle/vehicleInvalid/willBeUnlockedInBattle')


def getClassLimitMessage4Vehicle(teamLimits):
    classesList = [i18n.makeString(b'#menu:classes/%s' % clazz) for clazz in teamLimits[b'classes']]
    return i18n.makeString(b'#system_messages:prebattle/vehicleInvalid/limits/classes') % (b', ').join(classesList)


def getNationLimitMessage4Vehicle(teamLimits):
    nationsList = [i18n.makeString(b'#menu:nations/%s' % nation) for nation in teamLimits[b'nations']]
    return i18n.makeString(b'#system_messages:prebattle/vehicleInvalid/limits/nations') % (b', ').join(nationsList)


def getLevelLimitMessage4Vehicle(teamLimits):
    minLevel, maxLevel = prb_getters.getLevelLimits(teamLimits)
    return i18n.makeString(b'#system_messages:prebattle/vehicleInvalid/limits/level', minLevel, maxLevel)
...
def getMinCountLimitMessage4Team(teamLimits):
    return i18n.makeString(b'#system_messages:prebattle/teamInvalid/limit/minCount', minCount=teamLimits[b'minCount'])


def getTotalLevelLimitMessage4Team(teamLimits):
    minTotalLevel, maxTotalLevel = prb_getters.getTotalLevelLimits(teamLimits)
    return i18n.makeString(b'#system_messages:prebattle/teamInvalid/limit/totalLevel', minTotalLevel=minTotalLevel, maxTotalLevel=maxTotalLevel)


def getLevelLimitMessage4Team(teamLimits):
    minLevel, maxLevel = prb_getters.getLevelLimits(teamLimits)
    return i18n.makeString(b'#system_messages:prebattle/teamInvalid/limits/level', minLevel=minLevel, maxLevel=maxLevel)


def getRotationVehicleIsLockedMessage():
    return i18n.makeString(b'#system_messages:rotation/vehicleIsLocked')
    ...
    if reason in PREBATTLE_RESTRICTION.SERVER_LIMITS:
        if reason in _INVALID_TEAM:
            if entity:
                ...
                LOG_ERROR(b'Entity is not defined')
                teamLimits = LIMIT_DEFAULTS
            message = _INVALID_TEAM[reason](teamLimits)
        else:
            message = i18n.makeString((b'#system_messages:prebattle/teamInvalid/{0:>s}').format(reason))
    else:
        LOG_ERROR(b'Reason can not be converted', reason)
        message = reason
    return message


def getInvalidTeamServerMessage(errStr, entity=None):
    if errStr in (b'INVALID_EVENT_TEAM', b'EVENT_DISABLED'):
...
def getInvalidVehicleMessage(reason, entity=None):
    if reason in _INVALID_VEHICLE_STATE:
        ...
        if reason in _INVALID_VEHICLE_IN_TEAM:
            ...
            message = i18n.makeString(b'#system_messages:prebattle/vehicleInvalid/%s' % reason)
    else:
        LOG_ERROR(b'Reason can not be converted', reason)
        message = reason
    return message
...
def getPlayerStateChangedMessage(prbName, playerInfo):
    if playerInfo.isOffline():
        key = (b'#system_messages:{0:>s}/memberOffline').format(prbName)
    elif playerInfo.isReady():
        key = (b'#system_messages:{0:>s}/memberReady').format(prbName)
    else:
        key = (b'#system_messages:{0:>s}/memberNotReady').format(prbName)
    return i18n.makeString(key, playerInfo.getFullName())


def getPlayerAddedMessage(prbName, playerInfo):
    return i18n.makeString((b'#system_messages:{0:>s}/memberJoined').format(prbName), playerInfo.getFullName())


def getPlayerRemovedMessage(prbName, playerInfo):
    return i18n.makeString((b'#system_messages:{0:>s}/memberLeave').format(prbName), playerInfo.getFullName())


def getPlayerAssignFlagChanged(actorInfo, playerInfo):
    _, assigned = decodeRoster(playerInfo.roster)
    if assigned:
        key = b'#system_messages:memberRosterChangedMain'
    else:
        key = b'#system_messages:memberRosterChangedSecond'
...
def makeEntityI18nKey(ctrlType, entityType, prefix):
    if ctrlType in (CTRL_ENTITY_TYPE.LEGACY, CTRL_ENTITY_TYPE.UNIT):
        ...
        name = b'rally'
    return (b'{0}/{1}').format(name, prefix)


def getLeaveDisabledMessage(ctrlType, entityType):
    return (b'#system_messages:{0}').format(makeEntityI18nKey(ctrlType, entityType, b'leaveDisabled'))
