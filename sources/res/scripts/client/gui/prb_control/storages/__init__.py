...
__all__ = (b'RECENT_PRB_STORAGE', b'storage_getter', b'legacy_storage_getter', b'prequeue_storage_getter', b'PrbStorageDecorator', b'makeQueueName')

def _makeUniqueName(ctrlName, entityName):
    return (b'{}_{}').format(ctrlName, entityName)


def _makeLegacyName(legacyType):
    if legacyType not in _P_TYPE.LEGACY_PREBATTLES:
        raise SoftException((b'Legacy type is invalid {}').format(legacyType))
...
def makeQueueName(queueType):
    if queueType not in _Q_NAMES:
        raise SoftException((b'Queue type is invalid {}').format(queueType))
    return _makeUniqueName(_C_NAMES[_C_TYPE.PREQUEUE], _Q_NAMES[queueType])


RECENT_PRB_STORAGE = b'recentPrbStorage'
