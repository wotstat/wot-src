...
class UnitFactory(ControlFactory):
    ...
    def createLeaveCtx(self, flags=FUNCTIONAL_FLAG.UNDEFINED, entityType=0):
        return LeaveUnitCtx(waitingID=b'prebattle/leave', flags=flags, entityType=entityType)
    ...
    def __createByAccountState(self, ctx):
        ...
        if unitMrg is None:
            ...
            if unitMrg.id:
                ...
                if entity is None:
                    LOG_ERROR(b'Unit is not found in unit manager', unitMrg.id, unitMrg.unit)
                    unitMrg.leave()
                    return
