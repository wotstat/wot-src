...
class PreQueueFactory(ControlFactory):
    ...
    def createEntry(self, ctx):
        LOG_ERROR(b'preQueue functional has not any entries')
        return
    ...
    def createLeaveCtx(self, flags=_FLAG.UNDEFINED, entityType=0):
        return LeavePreQueueCtx(waitingID=b'prebattle/leave', flags=flags, entityType=entityType)
    ...
    def __createDefaultEntity(self):
        if prb_getters.isInBootcampAccount():
            ...
            if self.whiteTigerStorage is not None and self.whiteTigerStorage.isModeSelected():
