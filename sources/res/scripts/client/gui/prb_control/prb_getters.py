...
if TYPE_CHECKING:
    ...
    PrbEntityType = TypeVar(b'PrbEntityType', bound=BasePrbEntity)

def getQueueType():
    return getattr(BigWorld.player(), b'battleQueueType', QUEUE_TYPE.UNKNOWN)


def getClientPrebattle():
    return getattr(BigWorld.player(), b'prebattle', None)
...
def getPrebattleAutoInvites():
    ...
    if hasattr(player, b'prebattleAutoInvites'):
...
def getPrebattleLocalizedData(extraData=None):
    ...
    if extraData is None:
        extraData = getPrebattleSettings()[b'extraData']
    if extraData:
        from helpers import getClientLanguage
        lng = getClientLanguage()
        ld = extraData.get(b'localized_data', {})
        if ld:
            if lng in ld:
                led = ld[lng]
...
def getCreatorFullName(lobbyContext=None):
    ...
    creatorName = settings[b'creator']
    clanAbbrev = settings[b'creatorClanAbbrev']
    if lobbyContext is not None:
        creatorRegion = lobbyContext.getRegionCode(settings[b'creatorDBID'])
    else:
        creatorRegion = None
    if clanAbbrev:
        fullName = (b'{0:>s}[{1:>s}]').format(creatorName, clanAbbrev)
    else:
        fullName = creatorName
    if creatorRegion:
        fullName = (b'{0:>s} {1:>s}').format(fullName, creatorRegion)
    return fullName


def areSpecBattlesHidden():
    return not getattr(BigWorld.player(), b'prebattleAutoInvites', None)
...
def hasOpenTeamRoles(settings):
    teamRoles = settings[b'teamRoles']
...
def getClientUnitMgr():
    return getattr(BigWorld.player(), b'unitMgr', None)


def getUnitMgrID():
    return getattr(getClientUnitMgr(), b'id', None)


def getBattleID():
    return getattr(getClientUnitMgr(), b'battleID', 0)


def getClientUnitBrowser():
    return getattr(BigWorld.player(), b'unitBrowser', None)
...
def getUnit(safe=False):
    ...
    if not unitMgr:
        if not safe:
            raise SoftException(b'Unit manager not found')
        return
    unit = None
    try:
        unit = unitMgr.unit
        ...
        if not safe:
            raise SoftException(b'Unit not found')
