...
def AutoInvitesIterator():
    ...
    def comparator(obj, other):
        return cmp(obj[1].get(b'startTime', time.time()), other[1].get(b'startTime', time.time()))
...
class PrbListItem(object):
    __slots__ = (b'prbID', b'time', b'arenaTypeID', b'creator', b'clanAbbrev', b'playersCount', b'isOpened', b'comment', b'creatorIgrType', b'creatorDbId', b'badges', b'prbClusterID')
    ...
    def __repr__(self):
        return (b'PrbListItem(prbID = {0:n}, arenaTypeID = {1:n}, creator = {2:>s}, playersCount = {3:n}, isOpened = {4!r:s}, time = {5:n}, creatorIgrType = {6:n}, creatorDbId = {7:n})').format(self.prbID, self.arenaTypeID, self.getCreatorFullName(), self.playersCount, self.isOpened, self.time, self.creatorIgrType, self.creatorDbId)

    def getCreatorFullName(self):
        if self.clanAbbrev:
            fullName = (b'{0:>s}[{1:>s}]').format(self.creator, self.clanAbbrev)
        else:
...
class AutoInviteItem(object):
    __slots__ = (b'prbID', b'peripheryID', b'description', b'startTime', b'isValid', b'prbType', b'addInfo', b'arenaTypeID')
    ...
    def __repr__(self):
        return (b'AutoInviteItem(prbID = {0:n}, peripheryID = {1:n}, type = {2:n} description = {3!r:s}, startTime = {4:n}, isValid = {5!r:s})').format(self.prbID, self.prbType, self.peripheryID, self.description, self.startTime, self.isValid)
