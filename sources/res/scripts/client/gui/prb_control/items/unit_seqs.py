...
def UnitsListIterator(requester, data):
    units = sorted(data.iteritems(), key=(lambda item: item[1][b'cmdrRating']), reverse=True)
...
class UnitsListItem(object):
    __slots__ = (b'cfdUnitID', b'unitMgrID', b'creator', b'rating', b'playersCount', b'commandSize', b'vehicles', b'flags', b'isRosterSet', b'peripheryID', b'description', b'extra')
    ...
    def __repr__(self):
        return (b'UnitsListItem(cfdUnitID={0:n}, unitMgrID = {1:n}, creator = {2!r:s}, rating = {3:n}, peripheryID = {4:n}, size = {5:n}/{6:n}, flags = {7!r:s}), description = {8:s}').format(self.cfdUnitID, self.unitMgrID, self.creator, self.rating, self.peripheryID, self.playersCount, self.commandSize, self.flags, self.description)
