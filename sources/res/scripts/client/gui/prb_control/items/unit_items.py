...
class PlayerUnitInfo(object):
    __slots__ = (b'dbID', b'unitMgrID', b'unit', b'name', b'rating', b'accountWTR', b'role', b'accID', b'vehDict', b'isReady', b'isInSlot', b'slotIdx', b'regionCode', b'clanDBID', b'clanAbbrev', b'timeJoin', b'igrType', b'badges', b'hasPremium', b'extraData')
    itemsCache = dependency.descriptor(IItemsCache)
    lobbyContext = dependency.descriptor(ILobbyContext)

    def __init__(self, dbID, unitMgrID, unit, nickName=b'', rating=0, accountWTR=0, role=0, accountID=0, vehDict=None, isReady=False, isInSlot=False, slotIdx=-1, clanAbbrev=None, timeJoin=0, igrType=0, clanDBID=None, badges=None, **kwargs):
        self.dbID = dbID
...
class VehicleInfo(object):
    __slots__ = (b'vehInvID', b'vehTypeCD', b'vehLevel', b'vehClassIdx')
    itemsCache = dependency.descriptor(IItemsCache)
    ...
    def __repr__(self):
        return (b'VehicleInfo(vehInvID = {0:n}, vehTypeCD = {1:n}, vehLevel = {2:n}, vehClassIdx = {3})').format(self.vehInvID, self.vehTypeCD, self.vehLevel, self.vehClassIdx)
...
class SlotState(object):
    ...
    def __repr__(self):
        return (b'SlotState(isClosed = {0!r:s}, isFree = {1!r:s})').format(self.isClosed, self.isFree)


class SlotInfo(object):
    __slots__ = (b'index', b'state', b'player', b'vehicle', b'profileVehicle')
    ...
    def __repr__(self):
        return (b'SlotInfo(index = {0:n}, state = {1!r:s}, player = {2!r:s}, vehicle = {3!r:s}, profileVehicle = {4!r:s})').format(self.index, self.state, self.player, self.vehicle, self.profileVehicle)


class UnitFlags(object):
    __slots__ = (b'__flags', b'__flagsDiff', b'__isReady')
    ...
    def __repr__(self):
        return (b'UnitFlags(bitmask = {0!r:s}, isReady = {1!r:s}, diff = {2!r:s})').format(self.__flags, self.__isReady, self.__flagsDiff)
...
UnitStats = namedtuple(b'UnitStats', (b'readyCount', b'occupiedSlotsCount', b'openedSlotsCount', b'freeSlotsCount', b'curTotalLevel', b'levelsSeq'))
UnitStats.__new__.__defaults__ = (
 0, 0, 0, 0, 0, ())
UnitFullData = namedtuple(b'UnitStats', (b'unit', b'flags', b'stats', b'playerInfo', b'slotsIterator'))
UnitFullData.__new__.__defaults__ = (
 None, UnitFlags(0), UnitStats(), PlayerUnitInfo(-1L, 0, None),
 SlotInfo(-1, SlotState()))

@ReprInjector.simple((b'_minLevel', b'minLevel'), (b'_maxLevel', b'maxLevel'), (b'_maxSlots', b'maxSlots'), (b'_maxClosedSlots', b'maxClosedSlots'), (b'_maxEmptySlots', b'maxEmptySlots'), (b'_minTotalLevel', b'minTotalLevel'), (b'_maxTotalLevel', b'maxTotalLevel'), (b'_maxLegionariesCount', b'maxLegionariesCount'))
class UnitRosterSettings(object):
    TOTAL_SLOTS = 15
    _lobbyContext = dependency.descriptor(ILobbyContext)
    __slots__ = (b'_minLevel', b'_maxLevel', b'_maxSlots', b'_maxClosedSlots', b'_maxEmptySlots', b'_minTotalLevel', b'_maxTotalLevel', b'_maxLegionariesCount', b'__weakref__')
    ...
    def getLevelsRange(self, minLevel=-1, maxLevel=-1):
        ...
        if minLevel == -1 and maxLevel == -1:
            return list(set(possibleSquadsVehicleLevels) & set(range(self._minLevel, self._maxLevel + 1)))
    ...
    def __eq__(self, other):
        for fn in self.__slots__:
            if not fn.startswith(b'__') and getattr(self, fn) != getattr(other, fn):
                return False
...
class DynamicRosterSettings(UnitRosterSettings):
    ...
    def _extractSettings(self, unit):
        ...
        if roster is not None:
            kwargs[b'minLevel'], kwargs[b'maxLevel'] = roster.SLOT_TYPE.DEFAULT_LEVELS
            kwargs[b'maxSlots'] = roster.MAX_SLOTS
            kwargs[b'maxClosedSlots'] = roster.MAX_CLOSED_SLOTS
            kwargs[b'maxEmptySlots'] = roster.MAX_EMPTY_SLOTS
            kwargs[b'minTotalLevel'] = roster.MIN_UNIT_POINTS_SUM
            kwargs[b'maxTotalLevel'] = roster.MAX_UNIT_POINTS_SUM
            kwargs[b'maxLegionariesCount'] = unit.getLegionaryMaxCount()
        else:
            LOG_ERROR(b'Unit roster is not defined')
        return kwargs
...
class SupportedRosterSettings(object):
    ...
    def last(cls, prbType):
        ...
        raise SoftException((b'Unit type is not supported {0}').format(prbType))
        return
