...
_PrbInviteData = namedtuple(b'_PrbInviteData', (
 b'clientID',
 b'createTime',
 b'type',
 b'comment',
...
class PrbInviteWrapper(_PrbInviteData):
    ...
    def senderFullName(self):
        ...
        if fullName != b'':
            fullName = (b'{}{}').format(self.getCreatorBadgeImgStr(), fullName)
        return fullName
    ...
    def accept(self, callback=None):
        if not self.anotherPeriphery:
            ...
            _logger.error(b'Invalid periphery. %s', (
             (
    ...
    def revoke(self, callback=None):
        _logger.warning(b'Old-style invite can not be revoked')
        return
    ...
    def _merge(self, other):
        data = {}
        if other.creator:
            data[b'creator'] = other.creator
        if other.creatorID is not None:
            data[b'creatorID'] = other.creatorID
        if other.creatorClanAbbrev:
            data[b'creatorClanAbbrev'] = other.creatorClanAbbrev
        if other.createTime is not None:
            data[b'createTime'] = other.createTime
        if other.expiryTime:
            data[b'expiryTime'] = other.expiryTime
        if other.ownerID:
            data[b'ownerID'] = other.ownerID
        if other.id:
            data[b'id'] = other.id
        if other.extraData:
            data[b'extraData'] = other.extraData
        data[b'state'] = other.state
        if other.count:
            data[b'count'] = other.count
        if other.comment or other.isActive():
            data[b'comment'] = other.comment
...
class PrbInvitationWrapper(PrbInviteWrapper):
    ...
    def __new__(cls, clientID=-1, id=-1, type=0, status=None, sentAt=None, expiresAt=None, ownerID=None, creatorID=None, senderVehID=None, receiverID=None, receiverSID=None, info=None, sender=str(), senderBadges=None, senderClanAbbrev=None, receiver=str(), receiverClanAbbrev=None, **kwargs):
        ...
        result = PrbInviteWrapper.__new__(cls, clientID, sentAt, type, info.get(b'comment', b''), sender, creatorID, senderVehID, senderClanAbbrev, senderBadges, receiver, receiverID, receiverClanAbbrev, status, 1, peripheryID, prbID, info, False, ownerID, expiresAt, id, **kwargs)
        return result

    @classmethod
    def getPrbInfo(cls, inviteInfo):
        return (inviteInfo.get(b'peripheryID', 0), inviteInfo.get(b'unitMgrID', 0))
    ...
    def isFromHangar(self):
        return self.extraData.get(b'arenaUniqueID') is None

    def isSameBattle(self, arenaUniqueID):
        invArenaUniqueID = self.extraData.get(b'arenaUniqueID')
    ...
    def revoke(self, callback=None):
        _logger.warning(b'Need to call valid Account method')
        return
...
def _getOldInvites():
    return getattr(BigWorld.player(), b'prebattleInvites', {})


def _getNewInvites():
    if hasattr(BigWorld.player(), b'prebattleInvitations'):
...
class InvitesManager(UsersInfoHelper):
    ...
    def __del__(self):
        _logger.debug(b'InvitesManager deleted')
    ...
    def acceptInvite(self, inviteID, postActions=None):
        try:
            ...
            _logger.error(b'Invite ID is invalid. %s', (inviteID, self._IDMap))
            return
        ...
        if invite.alwaysAvailable is True:
            ...
            if another:
                if g_preDefinedHosts.periphery(invite.peripheryID) is None:
                    _logger.error(b'Periphery not found')
                    result = False
                elif self.lobbyContext.getCredentials() is None:
                    _logger.error(b'Login info not found')
                    result = False
                    ...
                    _logger.error(b'Roaming is not supported')
                    result = False
                else:
    ...
    def _rebuildInvitesLists(self):
        ...
        self._buildReceivedInvitesList([
         ...
         (
          sorted(_getNewInvites().values(), key=operator.itemgetter(b'sentAt'), reverse=False),
    ...
    def _getNewInviteMaker(self, rosterGetter):
        ...
        def _inviteMaker(item):
            peripheryID, prebattleID = PrbInvitationWrapper.getPrbInfo(item.get(b'info', {}))
            creatorIDScope = UserEntityScope.LOBBY
            receiverIDScope = UserEntityScope.LOBBY
            creatorID = item.get(b'senderDBID', 0)
            receiverID = item.get(b'receiverDBID', 0)
            if creatorID <= 0:
                if self.appLoader.getSpaceID() == GuiGlobalSpaceID.BATTLE:
                    ctx = self.sessionProvider.getCtx()
                    if b'senderVehID' in item:
                        creatorID = ctx.getSessionIDByVehID(item[b'senderVehID'])
                        creatorIDScope = UserEntityScope.BATTLE
            else:
                item[b'senderVehID'] = 0
            if receiverID <= 0:
                if self.appLoader.getSpaceID() == GuiGlobalSpaceID.BATTLE:
                    ctx = self.sessionProvider.getCtx()
                    if b'receiverVehID' in item:
                        receiverID = ctx.getSessionIDByVehID(item[b'receiverVehID'])
                        receiverIDScope = UserEntityScope.BATTLE
            else:
                item[b'receiverVehID'] = 0
            inviteID = self._makeInviteID(prebattleID, peripheryID, creatorID, receiverID)
    ...
    def __onPrebattleInvitesChanged(self, diff):
        step = PRB_INVITES_INIT_STEP.CONTACTS_RECEIVED
        if self.__inited & step != step:
            _logger.debug(b'Received invites are ignored. Manager waits for client will receive contacts')
            return
        if (b'prebattleInvites', b'_r') in diff:
            self._rebuildInvitesLists()
        if b'prebattleInvites' in diff:
    ...
    def __onSettingsChanged(self, diff):
        if b'invitesFromFriendsOnly' in diff:
            invitations = []
...
class AutoInvitesNotifier(object):
    ...
    def __del__(self):
        _logger.debug(b'AutoInvitesNotifier deleted')
        return
