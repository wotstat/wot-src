...
class GiftSystemMessagesProxy(ServiceChannelFormatter):
    ...
    def format(self, message, *args):
        ...
        if not messageData or not isinstance(messageData, dict):
            _logger.error(b'GiftSystemMessagesProxy received unsupported message data=%s', messageData)
            return []
        else:
            eventID = messageData.get(b'eventID', GiftEventID.UNKNOWN)
            ...
            if eventHub is not None:
                try:
                    ...
                    _logger.exception(b'GiftSystemMessagesProxy received message with incomplete data for IncomeSysMessage')

            else:
                _logger.warning(b'GiftSystemMessagesProxy received message for non-existent eventID=%s', eventID)
            return []
