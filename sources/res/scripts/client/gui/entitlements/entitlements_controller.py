...
class EntitlementsController(IEntitlementsController):
    __slots__ = (b'__cache', b'__cacheWaitSync', b'__lastFailedEntitlements')
            ...
            if self.__webController.isAvailable():
                ...
                if response.isSuccess():
                    ...
                    if self.__cache is not None:
                        self.__cache.update(codes, result.get(b'data', {}))
                        self.__lastFailedEntitlements.difference_update(codes)
                    else:
                        self.__cache = AgateEntitlementsModel(result.get(b'data', {}))
                ...
                _logger.warning(b"Failed to get entitlements data. Web controller isn't available")
                callback(False)
            return
