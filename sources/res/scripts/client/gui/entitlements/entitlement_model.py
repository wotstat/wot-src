...
class AgateEntitlement(object):
    __slots__ = (b'__name', b'__amount', b'__expiresAt', b'__isOnHold')
    ...
    def createFromResponseData(cls, data, onHold=False):
        return AgateEntitlement(data.get(b'code', b''), data.get(b'amount', 0), expiresAt=data.get(b'expires_at', None), onHold=onHold)

    def __repr__(self):
        return (b'<AgateEntitlement(name={}, amount={}, expiresAt={}, onHold={})>').format(self.__name, self.__amount, self.__expiresAt, self.__isOnHold)


class AgateEntitlementsModel(object):
    __slots__ = (b'__balance', b'__onHoldConsumed', b'__onHoldGranted')
    ...
    def __parseResponseData(self, responseData):
        balance = self.__getDictFromResponseList(responseData.get(b'balance', []))
        onHoldConsumed = self.__getDictFromResponseList(responseData.get(b'on_hold', {}).get(b'consumed', []), True)
        onHoldGranted = self.__getDictFromResponseList(responseData.get(b'on_hold', {}).get(b'granted', []), True)
    ...
    def __getDictFromResponseList(self, responseList, onHold=False):
        return {balanceEntitlement.get(b'code', b''): AgateEntitlement.createFromResponseData(balanceEntitlement, onHold) for balanceEntitlement in responseList}
