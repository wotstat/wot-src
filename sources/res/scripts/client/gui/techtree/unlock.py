from collections import namedtuple
import BigWorld
from debug_utils import LOG_ERROR, LOG_DEBUG
from gui.ClientUpdateManager import g_clientUpdateManager
from gui.Scaleform.daapi.view import dialogs
from gui.techtree.settings import RequestState, UnlockStats
from gui.techtree.techtree_dp import g_techTreeDP
from gui.impl import backport
from gui.shared import event_dispatcher
from gui.shared.formatters import text_styles
from gui.shared.gui_items import GUI_ITEM_TYPE
from gui.shared.gui_items.processors import Processor, plugins as proc_plugs
UnlockItemCtx = namedtuple(b'UnlockItemCtx', (b'itemCD', b'itemTypeID', b'parentCD', b'unlockIdx'))

def makeCostCtx(vehXP, xpCost, xpDiscount):
    freeXP = xpCost - vehXP
    if freeXP < 0:
        freeXP = 0
    if xpCost < vehXP:
        vehXP = xpCost
    return {b'xpCost': xpCost, b'vehXP': vehXP, b'freeXP': freeXP, b'xpDiscount': xpDiscount}


class UnlockItemConfirmator(proc_plugs.DialogAbstractConfirmator):

    def __init__(self, unlockCtx, costCtx, activeHandler=None, isEnabled=True):
        super(UnlockItemConfirmator, self).__init__(activeHandler, isEnabled)
        self._unlockCtx = unlockCtx
        self._costCtx = costCtx
        self.__itemType = None
        g_clientUpdateManager.addCallbacks({b'serverSettings.blueprints_config.isEnabled': (self.__onBlueprintsModeChanged), 
           b'serverSettings.blueprints_config.useBlueprintsForUnlock': (self.__onBlueprintsModeChanged)})
        return

    def __del__(self):
        self.__destroy()
        super(UnlockItemConfirmator, self).__del__()
        return

    def _makeMeta(self):
        item = self.itemsCache.items.getItemByCD(self._unlockCtx.itemCD)
        xpCost = backport.getIntegralFormat(self._costCtx[b'xpCost'])
        freeXp = backport.getIntegralFormat(self._costCtx[b'freeXP'])
        ctx = {b'xpCost': (text_styles.expText(xpCost)), 
           b'freeXP': (text_styles.expText(freeXp)), 
           b'typeString': (item.userType), 
           b'userString': (item.userName)}
        self.__itemType = item.itemTypeID
        if self.__itemType == GUI_ITEM_TYPE.VEHICLE:
            key = b'confirmUnlockVehicle'
        else:
            key = b'confirmUnlockItem'
        return dialogs.I18nConfirmDialogMeta(b'confirmUnlock', meta=dialogs.HtmlMessageLocalDialogMeta(b'html_templates:lobby/dialogs', key, ctx=ctx))

    def __destroy(self):
        self._unlockCtx = None
        self._costCtx = None
        self.__itemType = None
        g_clientUpdateManager.removeObjectCallbacks(self)
        return

    def __onBlueprintsModeChanged(self, _):
        if self.__itemType == GUI_ITEM_TYPE.VEHICLE:
            self.__destroy()
            event_dispatcher.showHangar()
        return


class UnlockItemValidator(proc_plugs.SyncValidator):

    def __init__(self, unlockCtx, costCtx, isEnabled=True):
        super(UnlockItemValidator, self).__init__(isEnabled)
        self.__unlockCtx = unlockCtx
        self.__costCtx = costCtx
        return

    def __del__(self):
        self.__unlockCtx = None
        self.__costCtx = None
        return

    def _validate(self):
        itemCD, itemTypeID, parentCD, unlockIdx = self.__unlockCtx[:]
        itemGetter = self.itemsCache.items.getItemByCD
        xpCost = self.__costCtx[b'xpCost']
        vehicle = itemGetter(parentCD)
        item = itemGetter(itemCD)
        if vehicle is None:
            LOG_ERROR(b'Vehicle is invalid', self.__unlockCtx)
            return proc_plugs.makeError(b'vehicle_invalid')
        else:
            if item is None:
                LOG_ERROR(b'Item is invalid', self.__unlockCtx)
                return proc_plugs.makeError(b'item_invalid')
            if vehicle.itemTypeID != GUI_ITEM_TYPE.VEHICLE:
                LOG_ERROR(b'Int compact descriptor is not for vehicle', parentCD)
                return proc_plugs.makeError(b'vehicle_invalid')
            if not vehicle.isUnlocked:
                LOG_ERROR(b'Vehicle is not unlocked', itemCD, parentCD)
                return proc_plugs.makeError(b'vehicle_locked')
            if item.isUnlocked:
                return proc_plugs.makeError(b'already_unlocked')
            stats = self.itemsCache.items.stats
            unlockStats = UnlockStats(stats.unlocks, stats.vehiclesXPs, stats.freeXP)
            if itemTypeID == GUI_ITEM_TYPE.VEHICLE:
                result, _ = g_techTreeDP.isNext2Unlock(itemCD, **unlockStats._asdict())
                if not result:
                    LOG_ERROR(b'Required items are not unlocked', self.__unlockCtx)
                    return proc_plugs.makeError(b'required_locked')
            else:
                _xpCost, _itemCD, required = vehicle.getUnlocksDescr(unlockIdx)
                if _itemCD != itemCD:
                    LOG_ERROR(b'Item is invalid', self.__unlockCtx)
                    return proc_plugs.makeError(b'item_invalid')
                if _xpCost != xpCost:
                    LOG_ERROR(b'XP cost is invalid', self.__unlockCtx)
                    return proc_plugs.makeError(b'xp_cost_invalid')
                if not unlockStats.isSeqUnlocked(required):
                    LOG_ERROR(b'Required items are not unlocked', self.__unlockCtx)
                    return proc_plugs.makeError(b'required_locked')
            if unlockStats.getVehTotalXP(parentCD) < xpCost:
                LOG_ERROR(b'XP not enough for unlock', self.__unlockCtx)
                return proc_plugs.makeError()
            if RequestState.inProcess(b'unlock'):
                return proc_plugs.makeError(b'in_processing')
            return proc_plugs.makeSuccess()


class UnlockItemProcessor(Processor):

    def __init__(self, vehTypeCD, unlockIdx, plugins=None):
        if plugins is None:
            plugins = []
        super(UnlockItemProcessor, self).__init__(plugins)
        self.vehTypeCD = vehTypeCD
        self.unlockIdx = unlockIdx
        return

    def _request(self, callback):
        RequestState.sent(b'unlock')
        BigWorld.player().stats.unlock(self.vehTypeCD, self.unlockIdx, (lambda code: self._response(code, callback)))
        return

    def _response(self, code, callback, errStr=b'', ctx=None):
        LOG_DEBUG(b'Server response', code, errStr, ctx)
        RequestState.received(b'unlock')
        if code < 0:
            return callback(self._errorHandler(code, errStr=b'server_error', ctx=ctx))
        return callback(self._successHandler(code, ctx=ctx))
