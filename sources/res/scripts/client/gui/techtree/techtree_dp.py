...
class DisplaySettingsModel(models.Model):
    __slots__ = (b'rowsNumber', b'columnsNumber', b'premiumRowsNumber')
...
displaySettingsSchema = schemas.Schema(fields={b'rowsNumber': (fields.Integer(required=True)), 
   b'columnsNumber': (fields.Integer(required=True)), 
   b'premiumRowsNumber': (fields.Integer(required=True))}, modelClass=DisplaySettingsModel, checkUnknown=True)
SUPPORTED_LINES = {
 b'horizontal', b'vertical', b'H_V'}
...
class TechTreeDataProvider(object):
    __slots__ = (b'__loaded', b'__availableNations', b'__displayInfo', b'__topLevels', b'__topItems', b'__nextLevels', b'__settings', b'__unlockPrices', b'__announcements', b'__announcementCDToName', b'__nextAnnouncements', b'__nodes', b'__nextTypeIDs', b'__source')
    ...
    def load(self, isReload=False):
        ...
        try:
            try:
                ...
                self.__settings = shared[b'settings']
    ...
    def getAvailableNations(self):
        if self.__availableNations is None:
            ...
            if section is None:
                _xml.raiseWrongXml(None, self.__source.shared, b'can not open or read')
            xmlCtx = (
             None, self.__source.shared)
    ...
    def __readShared(self, clearCache=False):
        ...
        shared = {b'settings': {}, b'default': {}}
        section = ResMgr.openSection(self.__source.shared)
        if section is None:
            _xml.raiseWrongXml(None, self.__source.shared, b'can not open or read')
        xmlCtx = (
         None, self.__source.shared)
        shared[b'settings'] = self.__readSettings(xmlCtx, section)
        if self.__availableNations is None:
            self.__availableNations = self.__readAvailableNations((
             None, self.__source.shared), section)
    ...
    def __readDefaultLine(self, shared, xmlCtx, section):
        ...
        xPath = (b'{0:>s}/default-line').format(self.__source.shared)
        xmlCtx = (None, xPath)
        name = _xml.readString(xmlCtx, defSec, b'line')
        if name not in SUPPORTED_LINES:
            raise _ConfigError(xmlCtx, (b'line is not supported {}').format(name))
    ...
    def __readNodeLines(self, parentCD, nation, xmlCtx, section, shared):
        ...
        for name, sub in linesSec.items():
            ...
            lineName = sub.readString(b'line') if b'line' in sub.keys() else shared[b'default'][b'line']
            if lineName not in SUPPORTED_LINES:
                raise _ConfigError(xmlCtx, (b'line is not supported {}').format(lineName))
            result.append({b'lineName': lineName, 
               b'childID': (node.nodeCD)})
    ...
    def __readSettings(self, _, root):
        rawData = section2dict.parse(root[b'settings'])
