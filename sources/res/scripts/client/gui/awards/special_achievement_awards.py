...
class VehicleCollectorAward(ExplosionBackAward):
    ...
    def getAwardImage(self):
        medalName = (b'').join((CollectorVehicleConsts.COLLECTOR_MEDAL_PREFIX, str(self.__nationID)))
...
class BoosterAward(ExplosionBackAward):
    ...
    def getDescription(self):
        localKey = b'#menu:awardWindow/boosterAward/description/timeValue/%s'
        if self._booster.expiryTime:
            timeValue = text_styles.stats(i18n.makeString(localKey % b'defined', effectTime=self._booster.getEffectTimeStr(), tillTime=self._booster.getExpiryDate()))
        else:
            timeValue = text_styles.stats(i18n.makeString(localKey % b'undefined', effectTime=self._booster.getEffectTimeStr()))
...
class TelecomAward(AwardAbstract):
    ...
    def __getVehicleDetails(self, vehicleCD):
        ...
        details[b'type'] = item.typeUserName
        details[b'nation'] = backport.text(R.strings.menu.nations.dyn(item.nationName)())
        details[b'level'] = int2roman(item.level)
        details[b'vehicle'] = item.userName
        return details
    ...
    def getDescription(self):
        ...
        description = backport.text(self.__addProviderToRes(descriptionRes)(), tariff=backport.text(providerLocRes.tariff()) if providerLocRes else b'', vehicles=vehicles, prem=premText)
