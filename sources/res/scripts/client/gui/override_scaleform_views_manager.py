...
class _OverrideScaleFormViewsManager(object):
    __slots__ = (b'_activeViewAliases', b'_activeContextMenuAliases', b'_battlePackages', b'_lobbyPackages')
        ...
        for path in paths:
            ...
            for setting in settings:
                if setting.alias in self._activeViewAliases[guiType]:
                    raise SoftException((b'Active extensions:{0}, {1} in wot_ext contains duplicate gui views alias={2}').format(self._activeViewAliases[guiType][setting.alias], extensionName, setting.alias))
            ...
            for handler in handlers:
                ...
                if alias in self._activeContextMenuAliases[guiType]:
                    raise SoftException((b'Active extensions:{0}, {1} in wot_ext contains duplicate context menu alias={2}').format(self._activeContextMenuAliases[guiType][alias], extensionName, alias))
