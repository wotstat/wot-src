...
class _LimitedUIRule(namedtuple(b'_LimitedUIRule', (b'idx', b'expression', b'tokens', b'message', b'ruleType'))):
    __slots__ = ()
...
class _LimitedUIRules(object):
    ...
    def completeRule(self, ruleID):
        rule = self.getRule(ruleID)
        if rule is None:
            _logger.warning(b"Couldn't complete rule, ruleID was not found: %s", ruleID.name())
            return
        else:
            ...
            for ruleType, ruleIDs in self.__postponedCompletedRules.items():
                if ruleType == LuiRuleTypes.VERSIONED:
                    ...
                    for ruleID in ruleIDs:
                        rule = self.getRule(ruleID)
                        if rule is None:
                            _logger.warning(b"Couldn't complete postponed rule, ruleID was not found: %s", ruleID.name())
                            continue
    ...
    def __normalizeRuleItem(cls, data, rulesIDs, item):
        ...
        if ruleDependencies:
            while ruleDependencies:
                ...
                expressionStr = expressionStr.replace(ruleDependency, (b'({})').format(dependencyExpression))
