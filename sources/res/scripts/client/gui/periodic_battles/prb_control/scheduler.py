...
class PeriodicScheduler(BaseScheduler):
    ...
    def _doLeave(self):
        if self._entity is None:
            ...
            self._entity.leave(LeavePreQueueCtx(waitingID=b'prebattle/leave'))
            return
    ...
    def __show(self, isInit=False):
        ...
        if not self.__isConfigured and self._hasConfiguredNotification():
            SystemMessages.pushMessage(backport.text(self._getResRoot().notification.notSet(), **self._getMessageParams()), messageData={b'title': (backport.text(self._getResRoot().notification.notSet.title()))}, type=SystemMessages.SM_TYPE.PeriodicBattlesNotSet)
        elif not self.__isPrimeTime:
            SystemMessages.pushMessage(backport.text(self._getResRoot().notification.primeTime(), **self._getMessageParams()), messageData={b'title': (backport.text(self._getResRoot().notification.primeTime.title()))}, type=SystemMessages.SM_TYPE.PrimeTime)
        elif not isInit:
            SystemMessages.pushMessage(backport.text(self._getResRoot().notification.available(), **self._getMessageParams()), messageData={b'title': (backport.text(self._getResRoot().notification.available.title()))}, type=SystemMessages.SM_TYPE.PeriodicBattlesAvailable)
        return
