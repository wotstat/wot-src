...
def showMain(eventName, subViewID=None, category=b'', count=0, bonuses=None, *args, **kwargs):
    from gui.impl.lobby.lootbox_system.states import LootBoxMainState
    LootBoxMainState.goTo({b'subViewID': subViewID, 
...
def showInfo(eventName, category=b''):
    from gui.impl.lobby.lootbox_system.states import LootBoxInfoState
    LootBoxInfoState.goTo({b'category': category, b'eventName': eventName})
    return


def showAutoOpen(eventName, rewards, boxes):
    from gui.impl.lobby.lootbox_system.states import LootBoxAutoOpenState
    LootBoxAutoOpenState.goTo({b'eventName': eventName, b'rewards': rewards, b'boxes': boxes})
    return
...
def showItemPreview(itemType, itemID, styleID, customization=None, itemsCache=None):
    if itemType == b'vehicles':
    ...
    elif itemType == b'customizations':
        ...
        _logger.error(b'Type "%s" is not supported for preview', itemType)
    return
...
def showVehicleStylePreview(style):
    if style.isProgression:
        raise SoftException(b'Progression styles is not supported')
