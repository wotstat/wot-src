...
RewardsGroup = namedtuple(b'RewardsGroup', (b'name', b'bonusTypes', b'bonuses', b'filterFuncs'))
REWARDS_GROUP_NAME_RES = R.strings.lootbox_system.autoOpenView.rewardGroups

def isOptionalDevice(itemTypeID, _):
    return itemTypeID == ITEM_TYPE_INDICES[b'optionalDevice']


def isBattleBooster(itemTypeID, itemCD):
    if itemTypeID != ITEM_TYPE_INDICES[b'equipment']:
        return False
...
def noCompensation(info, _):
    return not (info.get(b'compensatedNumber', 0) > 0 or info.get(b'customCompensation', (0, 0)) != (0, 0))
...
def _mergeCompensations(bonusesGroup):
    ...
    if bonusesGroup.name == REWARDS_GROUP_NAME_RES.other():
        vehicles = bonusesGroup.bonuses.get(b'vehicles', [])
    ...
    if compBonuses:
        del bonusesGroup.bonuses[b'vehicles']
