...
class DebugMonitorView(DebugView):
    ...
    def __init__(self, textureName=b'', parentGUI=None):
    ...
    def __setItem(self, nameStr, valueStr, dividerStr, item=None):
        if item is None:
            item = DebugViewItem()
            item.setFont((b'system_small.font', b'system_small.font'))
            item.setColour(((255, 255, 255, 255), (255, 255, 255, 255)))
            newKeyname = b'key_' + str(self.getItemsCount())
    ...
    def __buildContentItemsBy(self, content, baseIndent=0, baseKeyname=b''):
        result = []
        try:
            if isinstance(content, dict):
                for itemName, itemValue in sorted(content.items()):
                    ...
                    if isinstance(itemValue, dict):
                        result += [(b' ' * baseIndent + itemName + b':', b'', b'')]
                    ...
                    if isinstance(itemValue, (int, float, bool, str)) or itemValue is None:
                        ...
                        valueStr1 = b'%0.3f' % itemValue if isinstance(itemValue, float) else str(itemValue)
                        valueStr2 = b'%0.3f' % deltaValue if isinstance(deltaValue, float) else str(deltaValue)
                        itemValueStr = valueStr1 + b' (' + valueStr2 + b')'
                        result += [(b' ' * baseIndent + itemName, itemValueStr, b' = ')]

        except Exception:
            LOG_ERROR(b"<DebugMonitor>: can't build content items. Look exception:")
