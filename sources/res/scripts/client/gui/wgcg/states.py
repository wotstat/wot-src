...
class _State(object):
    ...
    def _makeErrorResponse(self):
        return WgcgRequestResponse(ResponseCodes.UNKNOWN_ERROR, b'Request cannot be sent from the current state', None)
...
class UnavailableState(_WebState):
    ...
    def _sendRequest(self, ctx, callback, allowDelay=True):
        if ctx.getRequestType() == WebRequestDataType.PING:
            ...
            result = WgcgRequestResponse(ResponseCodes.WGCG_ERROR, b'WGCG is not available.', None)
...
class AvailableState(_WebState):
    ...
    def _sendRequest(self, ctx, callback, allowDelay=True):
        if ctx.isAuthorizationRequired() and not self.isLoggedOn():
            self.__waitingRequests.append((
             ctx,
             callback,
             WgcgRequestResponse(ResponseCodes.AUTHENTIFICATION_ERROR, b'The user is not authorized.', None),
             allowDelay))
            self.login()
        else:
            self.clanSync(ctx)
            if self._webCtrl and self._webCtrl.simWGCGEnabled():
                ...
                result = WgcgRequestResponse(ResponseCodes.WGCG_ERROR, b'Simulated WGCG error!', None)
            resultCode = result.code
            if resultCode == ResponseCodes.WGCG_ERROR:
                LOG_DEBUG(b'WGCG error has occurred! The state will be changed to NA.')
                ...
                LOG_WARNING(b'Failed to do WGCG request. Need to relogin.', ctx)
                self.__waitingRequests.append((ctx, callback, result, allowDelay))
                self.__loginState = LOGIN_STATE.LOGGED_OFF
                self.login()
            else:
    ...
    def __doLogin(self, callback):
        if not LOGIN_STATE.canDoLogin(self.__loginState):
            ...
            return
        LOG_DEBUG(b'Wgcg gate login processing...')
        ...
        if response and response.isValid():
            ...
            if response.getDatabaseID() == pDbID:
                LOG_DEBUG(b'Trying to login to the wgcg lib...')
                ...
                if result.isSuccess():
                    ...
                    if data:
                        ...
                        LOG_DEBUG(b"Response of login to the wgcg doesn't contain data")
                else:
                    LOG_WARNING(b'There is error while logging in to wgcg gate', result)
            else:
                LOG_DEBUG(b'Response spa token for wgcg gate not for current player', response.getDatabaseID(), pDbID)
        else:
            LOG_WARNING(b'There is error while getting spa token for wgcg gate', response)
    ...
    def __doLogOut(self, callback):
        LOG_DEBUG(b'Wgcg gate logout processing...')
