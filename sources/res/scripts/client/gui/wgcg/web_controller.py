...
class WebController(WebListeners, IWebController):
    ...
    def simEnableClan(self, enable):
        ...
        clanSettings = {b'wgcgProfile': {b'isEnabled': enable, 
                            b'gateUrl': (settings.wgcg.getGateUrl())}}
    ...
    def start(self, force=True):
        if self.__started and not force:
            _logger.debug(b'Wgcg already started.')
            return
        ...
        g_clientUpdateManager.addCallbacks({b'stats.clanInfo': (self.__onClanInfoChanged), 
           b'serverSettings.wgcg.isEnabled': (self.__onServerSettingChanged), 
    ...
    def _onProxyDataItemShowByDefault(self, notifID, item):
        if item.getType() in _CLAN_WGNC_NOTIFICATION_TYPES:
            ...
            self.notify(b'onWgncNotificationReceived', notifID, item)
        return
