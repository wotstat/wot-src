...
class BaseExternalBattleUnitRequestHandlers(RequestHandlers):
    ...
    def __updateExternalBattleBase(self, api, ctx, callback, *args, **kwargs):
        ...
        def nextRequestCallback(firstResponse):
            ...
            self._requester.doRequestEx(ctx, partial(mergeResponsesCallback, firstResponse), (api, b'get_wgsh_account_unit_info'), self.__getPeripheryIDStr(), ctx.getUnitMgrID(), ctx.getRev())
            return

        self._requester.doRequestEx(ctx, nextRequestCallback, (api, b'get_wgsh_common_unit_info'), self.__getPeripheryIDStr(), ctx.getUnitMgrID(), ctx.getRev())
        return
    ...
    def __unfreezeVehicle(self, api, ctx, callback, *args, **kwargs):
        self._requester.doRequestEx(ctx, callback, (api, b'event_unfreeze_vehicle'), *ctx.getRequestArgs())
        return
