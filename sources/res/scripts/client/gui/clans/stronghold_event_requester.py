...
class FrozenVehiclesRequester(object):
    ...
    def setInitialDataAndStart(self, data):
        for playerData in data:
            ...
            if spaID is None:
                continue
            allVehicles = playerData.get(b'all_vehicles')
            if allVehicles is not None and allVehicles:
                self.__cache[spaID] = FrozenVehiclesConstants.ALL_VEHICLES_FROZEN
                continue
            self.__cache[spaID] = set([v.get(b'vehicle_cd', -1) for v in playerData.get(b'vehicles', [])])
    ...
    def _onMessage(self, message):
        ...
        for record in message:
            ...
            if spaID is None:
                continue
            vehicleCds = set(record.get(b'vehicle_cds', []))
