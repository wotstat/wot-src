...
class ClientMissionsProvider(IBaseProvider, IGlobalListener):
    ...
    def __onServerSettingChanged(self, diff):
        if self.__formActive:
            if b'elenSettings' in diff and b'isElenEnabled' in diff[b'elenSettings']:
                enabled = diff[b'elenSettings'][b'isElenEnabled']
                if not enabled:
    ...
    def __showElenPopupDlg(self):
        yield DialogsInterface.showI18nInfoDialog(b'elenDisabled')
