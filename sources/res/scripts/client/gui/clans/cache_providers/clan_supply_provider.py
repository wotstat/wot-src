...
class ClanSupplyProvider(BaseProvider):
    ...
    def _getSettings(self):
        return {(DataNames.PROGRESSION_SETTINGS): (RequestSettings(context=contexts.ProgressionSettingsCtx(), isCached=True, updatePeriodType=UpdatePeriodType.BY_TIME, updateKwargs={b'updateTime': (time_utils.HALF_HOUR)})), 
    ...
    def __questInfoUpdater(cachedData, item):
        ...
        if item.getStatus() == QuestStatus.REWARD_PENDING:
            for idx, cachedQuest in enumerate(cachedData.quests):
                ...
                questAsDict.update({b'current_progress': (cachedQuest.required_progress), 
                   b'status': (QuestStatus.REWARD_PENDING)})
            ...
            questAsDict.update({b'current_progress': (item.getProgress()), 
               b'status': (item.getStatus())})
