...
_ClanInfo = namedtuple(b'_ClanInfo', [
 23, 24, 25, 26, 
 27])
...
class ClanAccountProfile(object):
    ...
    def getClanName(self):
        return passCensor(self._getClanInfoValue(0, b''))

    def getClanFullName(self):
        return (b'{} {}').format(clans_fmts.getClanAbbrevString(self.getClanAbbrev()), self.getClanName())

    def getClanAbbrev(self):
        return passCensor(self._getClanInfoValue(1, b''))
    ...
    def resync(self, force=False):
        LOG_DEBUG(b'Full account clan profile resync initiated')
        ...
        if self.isInClan():
            ...
            if response.isSuccess():
                ...
                if self._vitalWebInfo[SYNC_KEYS.CLAN_INFO] != info:
                    self._syncState |= SYNC_KEYS.CLAN_INFO
                    self.__changeWebInfo(SYNC_KEYS.CLAN_INFO, info, b'onAccountClanInfoReceived')
            else:
                self._vitalWebInfo[SYNC_KEYS.CLAN_INFO] = ctx.getDefDataObj()
        self._waitForSync ^= SYNC_KEYS.CLAN_INFO
        return
        ...
        if response.isSuccess():
            if requestType == WebRequestDataType.CREATE_APPLICATIONS:
                ...
                if apps:
                    ...
                    self.__changeWebInfo(SYNC_KEYS.APPS, appsCount, b'onAccountAppsReceived')
                ...
                if len(statuses) == 1 and statuses[0] == CLAN_INVITE_STATES.ACTIVE:
                    ...
                    if count is not None and count != self._vitalWebInfo[SYNC_KEYS.INVITES]:
                        self.__changeWebInfo(SYNC_KEYS.INVITES, count, b'onAccountInvitesReceived')
                ...
                if len(statuses) == 1 and statuses[0] == CLAN_INVITE_STATES.ACTIVE:
                    ...
                    if count is not None and count != self._vitalWebInfo[SYNC_KEYS.APPS]:
                        self.__changeWebInfo(SYNC_KEYS.APPS, count, b'onAccountAppsReceived')
    ...
    def processWgncNotification(self, notifID, item):
        ...
        def __updateAfterAction():
            ...
            if count:
                self.__changeWebInfo(SYNC_KEYS.APPS, count - 1, b'onAccountAppsReceived')
            return
    ...
    def _resyncBwInfo(self, clanDbID=0, clanBwInfo=None):
        ...
        if needToRaiseEvent:
            self._clansCtrl.notify(b'onAccountClanProfileChanged', self)
        if hasEnteredClan:
            showClanJoinAward(self.getClanAbbrev(), self.getClanName(), self.getClanDbID())
        return
    ...
    def __changeWebInfo(self, fieldName, value, eventName):
        ...
        self._clansCtrl.notify(b'onAccountWebVitalInfoChanged', fieldName, value)
        return

    def __changeInvitesState(self, invites, state):
        self._clansCtrl.notify(b'onClanInvitesStateChanged', [invite.getDbID() for invite in invites], state)
    ...
    def __repr__(self):
        ...
        if self.isInClan():
            args.extend([
             b'dbID = %s' % self._clanDbID,
             b'abbrev = %s' % self.getClanAbbrev()])
        else:
            args.append(b'no clan')
        if self._vitalWebInfo[SYNC_KEYS.CLAN_INFO]:
            args.append(b'cooldown = %s' % self._vitalWebInfo[SYNC_KEYS.CLAN_INFO].getClanCooldownTill())
        if self._vitalWebInfo[SYNC_KEYS.INVITES]:
            args.append(b'invites = %d' % len(self._vitalWebInfo[SYNC_KEYS.INVITES]))
        if self._vitalWebInfo[SYNC_KEYS.APPS]:
            args.append(b'apps = %d' % len(self._vitalWebInfo[SYNC_KEYS.APPS]))
        return b'%s(%s)' % (self.__class__.__name__, (b', ').join(args))
