...
class ClanNotificationController(GameWindowController, IClanNotificationController, MethodsRules):
    ...
    CLAN_NEWS_ALIAS = b'clanNews'
    DEPRECATED_ALIASES = (b'wgcq_hangar_bubble',)
    ...
    @MethodsRules.delayable(b'onLobbyInited')
    def setCounters(self, alias, count, isIncrement=False):
        if not self.__enabled:
            return
        if alias in self.DEPRECATED_ALIASES:
            _logger.info(b'Received a deprecated alias = %s for the clan counter.', alias)
            return
    ...
    def _openWindow(self, url, _=None):
        ...
        if browserView:
            return
        ctx = {b'url': url, 
           b'webHandlers': (createNotificationWebHandlers()), 
           b'browser_alias': (VIEW_ALIAS.CLAN_NOTIFICATION_WINDOW), 
           b'showCloseBtn': True, 
