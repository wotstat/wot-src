...
class DebutBoxesController(IDebutBoxesController):
    ...
    def getQuestForVehicle(self, vehicle):
        if not vehicle.isRented and not checkForTags(vehicle.tags, UNSUPPORTED_TAGS) or VEHICLE_TAGS.DEBUT_BOXES in vehicle.tags:
            return findFirst((lambda quest: quest.vehicleReqs.isAvailable(vehicle)), self.__quests.values(), None)
        else:
            return
    ...
    def __getTimer(self):
        ...
        if serverTime < config.endDate:
            return config.endDate - serverTime
        return 0
    ...
    def __tryNotifyStateChanged(self):
        ...
        if config.isEnabled:
            ...
            messageData = {b'header': (backport.text(R.strings.system_messages.debutBoxes.enabled.header()))}
        else:
            ...
            messageData = {b'header': (backport.text(R.strings.system_messages.debutBoxes.disabled.header()))}
    ...
    def __onServerSettingsChanged(self, diff):
        ...
        if {
         b'isEnabled', b'startDate', b'endDate'}.intersection(debutBoxesDiff):
            self.__notifier.startNotification()
            self.__tryNotifyStateChanged()
            if not self.isEnabled():
                self.__clearCarouselFilters()
