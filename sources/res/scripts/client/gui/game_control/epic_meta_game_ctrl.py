...
PERFORMANCE_GROUP_LIMITS = {(EPIC_PERF_GROUP.HIGH_RISK): [{(EPIC_META_GAME_LIMIT_TYPE.SYSTEM_DATA): {b'osBit': 1, b'graphicsEngine': 0}}, {(EPIC_META_GAME_LIMIT_TYPE.HARDWARE_PARAMS): {(HARDWARE_SCORE_PARAMS.PARAM_GPU_MEMORY): 490}}, {(EPIC_META_GAME_LIMIT_TYPE.SYSTEM_DATA): {b'graphicsEngine': 0}, (EPIC_META_GAME_LIMIT_TYPE.HARDWARE_PARAMS): {(HARDWARE_SCORE_PARAMS.PARAM_RAM): 2900}}], (EPIC_PERF_GROUP.MEDIUM_RISK): [{(EPIC_META_GAME_LIMIT_TYPE.HARDWARE_PARAMS): {(HARDWARE_SCORE_PARAMS.PARAM_GPU_SCORE): 150}}, {(EPIC_META_GAME_LIMIT_TYPE.HARDWARE_PARAMS): {(HARDWARE_SCORE_PARAMS.PARAM_CPU_SCORE): 50000}}]}
...
class _FrontLineSounds(object):
    __SELECT_EVENT = b'gui_eb_mode_enter'
    __DESELECT_EVENT = b'gui_eb_mode_exit'
    __STATE_GROUP = b'STATE_gamemode'
    __STATE_SELECTED = b'STATE_gamemode_frontline'
    __STATE_DESELECTED = b'STATE_gamemode_default'
...
class EpicBattleMetaGameController(Notifiable, SeasonProvider, IEpicBattleMetaGameController, IGlobalListener):
    ...
    def showProgressionDuringSomeStates(self, showDefaultTab=False):
        ...
        if not isHangarAvailable():
            showFrontlineContainerWindow((showDefaultTab or getProperTabWhileHangarUnavailable)() if 1 else None)
        return
    ...
    def __setData(self):
        self.__skillData = {}
        skills = self.__metaSettings.rewards.get(b'combatReserves', {})
