...
class GameSessionController(IGameSessionController, IGlobalListener, Notifiable):
    ...
    def init(self):
        ...
        LOG_DEBUG(b'GameSessionController::init')
        return

    def fini(self):
        LOG_DEBUG(b'GameSessionController::fini')
        self._stop()
    ...
    def onLobbyStarted(self, ctx):
        self.__sessionStartedAt = ctx.get(b'aogasStartedAt', -1)
        LOG_DEBUG(b'GameSessionController::start', self.__sessionStartedAt)
    ...
    def onLobbyInited(self, event):
        ...
        g_clientUpdateManager.addCallbacks({b'premium': (self.__onAccountChanged), 
           b'stats.restrictions': (self.__onRestrictionsChanged), 
           b'stats.playLimits': (self.__onPlayLimitsChanged), 
           b'cache.gameRestrictions.session': (self.__onParentControlChanged), 
           b'cache.gameRestrictions.session_r': (self.__onParentControlChanged)})
    ...
    def isPrivateMessagesForbidden(self):
        return self._gameRestrictions.privateChat.get(b'bantype') in (
    ...
    def isNonFriendPrivateMessagesForbidden(self):
        return self._gameRestrictions.privateChat.get(b'bantype') == constants.BAN_TYPE.NONFRIEND_PRIVATE_COMMUNICATION_DENIED

    @property
    def privateMessagesRestrictionReason(self):
        return self._gameRestrictions.privateChat.get(b'reason', None)
    ...
    def getParentControlNotificationMeta(self):
        ...
        if self.isPlayTimeBlock:
            return I18nInfoDialogMeta(b'koreaPlayTimeNotification')
        else:
            if self.__curfewBlockTime is not None:
                notifyStartTime, blockTime = self.getCurfewBlockTime()

                def formatter(t):
                    return time.strftime(b'%H:%M', time.localtime(t))

                return I18nInfoDialogMeta(b'koreaParentNotification', messageCtx={b'preBlockTime': (formatter(notifyStartTime)), 
                   b'blockTime': (formatter(blockTime))})
            return
    ...
    def _stop(self, doNotifyInStart=False):
        LOG_DEBUG(b'GameSessionController::stop')
    ...
    def __loadBanCallback(self, banTimeLeft=0):
        ...
        LOG_DEBUG(b'Game session block callback', banTimeLeft)
    ...
    def __onBanNotifyHandler(self):
        LOG_DEBUG(b'GameSessionController:__onBanNotifyHandler')
        playTimeLeft = min([self.getDailyPlayTimeLeft(), self.getWeeklyPlayTimeLeft()])
        playTimeLeft = max(playTimeLeft, 0)
        banTime = time.strftime(b'%H:%M', time.localtime(time_utils.getCurrentTimestamp() + playTimeLeft))
        self.__lastBanMsg = (
    ...
    def __onParentControlChanged(self, diff):
        if diff and b'kick_at' not in diff:
            return
        if not diff:
            timeLimit = 0
        else:
    ...
    def __getCurfewBlockTime(cls, restrictions):
        if _BAN_RESTR in restrictions and restrictions[_BAN_RESTR]:
            ...
            if ban is not None and ban.get(b'reason') is not None:
                return (
                 ban[b'curfew'].get(b'from', time_utils.ONE_DAY) + cls.TIME_RESERVE,
                 ban[b'curfew'].get(b'to', time_utils.ONE_DAY))
        return (None, None)
    ...
    def __getNearestCurfew(cls, restrictions):
        ...
        for _, restr in restrictions.items():
            if b'curfew' not in restr:
                continue
            if restr is None:
                continue
            curfew = restr[b'curfew']
            curfewFrom = curfew[b'from']
            if curfewNearest is None or curfewStarts(curfewFrom, elapsedSecondsOfCurDay) < curfewStarts(curfewNearest[b'curfew'][b'from'], elapsedSecondsOfCurDay):
                curfewNearest = restr
