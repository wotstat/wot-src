...
class _CloseWindowWebApi(CloseWindowWebApi):
    ...
    @w2c(W2CSchema, b'advent_calendar')
...
_HeroAdventActionInfo = namedtuple(b'HeroAdventActionInfo', (b'isEnabled', b'vehicleCD', b'styleId', b'offerType', b'endTimestamp'))
...
class _HeroAdventActionHelper(object):
    ...
    def __fetchInfo(self):
        if self.__isByServerSwitchEnabled():
            ...
            if response.isSuccess():
                data = (response.getData() or {}).get(b'data', {})
                offerType = data.get(b'offer_type', None)
                if offerType is None:
                    _logger.warning(b'Unsupported calendar offer type %s.', offerType)
                self.__updateInfo(data.get(b'vehicle_cd', 0), data.get(b'style_id', 0), offerType, data.get(b'finish_date', 0))
                self.__postponeRecall()
            else:
                _logger.warning(b'Unable to fetch Hero-Tank info. Code: %s.', response.getCode())
        else:
            _logger.debug(b'Unable to fetch Hero-Tank info. Reason: Disabled by Server.')
        return
    ...
    def __isByServerSwitchEnabled(self):
        ...
        return (action[b'isEnabled']) and action[b'start'] <= time_utils.getServerUTCTime() < action[b'finish']
...
class CalendarController(GameWindowController, ICalendarController):
    ...
    def _openWindow(self, url, invokedFrom=None):
        if self.appLoader.getSpaceID() != GuiGlobalSpaceID.LOBBY:
            ...
            try:
                ...
                _logger.error(b'Could not allocate a browser ID for calendar')
                return

            self.__openBrowser(self.__browserID, url, _BROWSER_SIZE, invokedFrom)
            self.__setShowTimestamp(time_utils.getServerUTCTime())
            _logger.debug(b'Calendar opened in web browser (browserID=%d)', self.__browserID)
            return
    ...
    def __onBrowserDeleted(self, browserID):
        if browserID == self.__browserID:
            _logger.debug(b'Calendar web browser destroyed (browserID=%d)', browserID)
            self.__browserID = None
        return
    ...
    def __getShowTimestamp(self):
        ...
        if tstampStr:
            try:
                ...
                _logger.warning(b'Invalid calendar show timestamp')
    ...
    def __openBrowser(self, browserID, url, browserSize, invokedFrom):
        ...
        def showBrowserWindow():
            ctx = {b'size': browserSize, 
               b'title': (i18n.makeString(MENU.ADVENTCALENDAR_WINDOW_TITLE)), 
               b'handlers': browserHandlers, 
               b'browserID': browserID, 
               b'alias': (VIEW_ALIAS.ADVENT_CALENDAR), 
               b'showCloseBtn': False, 
               b'showWaiting': True, 
