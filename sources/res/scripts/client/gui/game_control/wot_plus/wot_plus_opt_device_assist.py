    ...
    def _searchVehicleLoadoutsInCache(self, vehicle):
        ...
        if vehIntCD in self._vehicleToVehicleMap:
            ...
            if optDevicesLoadouts:
                ...
                _logger.error(b'Attempt to retrieve copied vehicle %s as %s: No vehicle in cache. Invalid cache!', vehIntCD, proxyVehicle)
            ...
            if optDevicesLoadouts:
                ...
                _logger.error(b'Attempt to retrieve vehicle similar to %s as %s: No vehicle in cache. Invalid cache!', vehIntCD, proxyVehicle)
        ...
        return (optDevicesSource, linkedVehicle, optDevicesLoadouts)
    ...
    def _syncServerSettings(self, serverDiff):
        ...
        for compDescrTo, compDescrFrom in serverDiff.get(OptionalDevicesUsageConst.COPY, {}).items():
            if self._getOptionalDevicesFromCache(compDescrFrom):
                ...
                _logger.error(b'Attempt to copy %s vehicle: vehicle not present in cache', compDescrFrom)
...
class _RemoteDataProvider(_IOptDeviceAssistDataProvider):
    ...
    def fillPresets(self, loadouts):
        if not loadouts:
            _logger.warning(b'Trying to fill empty presets')
            return
    ...
    def _getRawPreset(self, loadoutType, vehicleCD):
        rawPreset = {b'vehicleId': vehicleCD}
...
class OptionalDevicesAssistantCtrl(object):
    OPT_DEVICE_ASSIST_DATA_CHANGED = b'optDeviceAssistDataChanged'
    ...
    def addSupportedBattleType(cls, battleType, loadoutType):
        if battleType not in PREBATTLE_TYPE.RANGE:
            _logger.warning(b'Trying to add prebattle type %s which is not in PREBATTLE_TYPE', battleType)
            return
    ...
    def fillRemotePresets(self, presets):
        _logger.debug(b'Filling presets for remote data provider')
    ...
    def getPopularOptDevicesPresets(self, vehicle):
        ...
        if self._remoteDataProvider.initiated:
            ...
            _logger.debug(b'Returning data from remote data provider')
