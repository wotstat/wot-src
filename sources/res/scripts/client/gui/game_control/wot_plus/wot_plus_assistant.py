...
class WotPlusAssistant(object):
    ...
    def addLoadoutType(cls, loadoutType):
        if loadoutType not in SupportedWotldaLoadoutType:
            _logger.warning(b'Trying to add loadout type %s which is not supported', loadoutType)
            return
        if loadoutType in cls._LOADOUT_TYPES_TO_REQUEST:
            _logger.warning(b'Trying to add loadout type %s which is already in request', loadoutType.value)
            return
    ...
    def subscriptionValidated(self):
        if self._waitingForResponse:
            _logger.debug(b'WotPlusAssistant in waiting for response and somebody tries to start another one, skipping')
    ...
    def _fetchLoadouts(self, loadoutTypes):
        ...
        if response.isNotModified():
            _logger.debug(b'Loadouts data for subscription is not modified, skipping update')
