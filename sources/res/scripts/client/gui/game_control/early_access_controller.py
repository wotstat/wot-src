...
class EarlyAccessController(IEarlyAccessController, SeasonProvider, IGlobalListener):
    ...
    _POST_PROGRESSION_QUEUES = (
     QUEUE_TYPE.RANDOMS, QUEUE_TYPE.COMP7)
    ...
    def isProgressionQuest(questID):
        return questID.startswith(getGroupName(b''))
    ...
    def getTokensForQuest(self, questID):
        ...
        if not quest:
            return 0
        tokens = quest.getBonuses(b'tokens')
    ...
    def getEAToken(self):
        ...
        if seasonID == 0:
            return b''
        return makeEarlyAccessToken(seasonID)

    def iterProgressionQuests(self):
        currentSeason = self.getCurrentSeason()
        cycles = sorted(currentSeason.getAllCycles().values(), key=attrgetter(b'ID'))
    ...
    def iterAllCycles(self, now=None):
        ...
        cycles = sorted(currentSeason.getAllCycles().values(), key=attrgetter(b'ID')) if currentSeason else []
        for cycle in cycles[:-1]:
            yield (
             str(cycle.ID), cycle)
    ...
    def getRequiredVehicleTypeAndLevelsForQuest(self, questID):
        ...
        if questID is None:
            ...
            vehicleType = self.getVehicleTypeAndLevelsByVehicleCD(lastPostrpVehicle)[0] if lastPostrpVehicle else b''
        else:
            quest = self.__eventsCache.getQuestByID(questID)
            if quest is not None:
                ...
                if conditions:
                    ...
                    vehicleType = vehicleClasses[0] if vehicleClasses else b''
    ...
    def isPostProgressionQueueSelected(self):
        if self.prbDispatcher is not None:
            return any(self.prbDispatcher.getFunctionalState().isQueueSelected(queueType) for queueType in self._POST_PROGRESSION_QUEUES)
        else:
            return False

    def isFilterDisabledInQueue(self):
        if self.prbDispatcher is not None:
            return any(self.prbDispatcher.getFunctionalState().isQueueSelected(queueType) for queueType in self._EXCLUDE_FILTER_QUEUES)
        else:
            return False

    def getVehicleTypeAndLevelsByVehicleCD(self, vehCD):
        vehItem = self.__itemsCache.items.getItemByCD(vehCD)
        if vehItem is not None:
            return (vehItem.type, vehItem.level)
        else:
            return (
             b'', MIN_VEHICLE_LEVEL)
    ...
    def __findVehicleWithMinLevel(self):
        ...
        if affectedVehicles:
            return min(affectedVehicles.keys(), key=(lambda cd: self.__itemsCache.items.getItemByCD(cd).level))
        else:
            return
    ...
    def __findPostrpVehicleWithMaxLevel(self):
        ...
        if postprVehicles:
            return max(postprVehicles, key=(lambda cd: self.__itemsCache.items.getItemByCD(cd).level))
        else:
            return
...
class _EarlyAccessSystemMessagesController(object):
    ...
    def __getStartSeasonTimer(self):
        ...
        for _, season in settings.seasons.iteritems():
            startSeason = season[b'startSeason'] if season else 0
