...
class ResourceWellController(IResourceWellController, EventsHandler):
    ...
    def getRewardVehicle(self):
        return first(first(self.__getConfig().rewards.itervalues()).bonus.get(b'vehicles', {}).keys())

    def getRewardStyleID(self):
        topReward = findFirst((lambda reward: reward.isSerial), self.__getConfig().rewards.itervalues())
        return first(topReward.bonus[b'vehicles'].values())[b'customization'].get(b'styleId', 0)
    ...
    def __getSerialRewardLeftCount(self):
        if not self.__serialNumberRequester.isDataAvailable():
            ...
            if remainingValuesCount > rewardLimit:
                _logger.error(b'Remaining values count cannot exceed reward limit!')
                return 0
            ...
            if givenValuesCount > rewardLimit:
                _logger.error(b'Given values count cannot exceed reward limit!')
                return 0
    ...
    def __onServerSettingsChanged(self, diff):
        ...
        if b'finishTime' in resourceWellDiff or b'startTime' in resourceWellDiff:
            self.__onEventUpdated()
        if b'isEnabled' in resourceWellDiff:
            self.__onEventUpdated()
    ...
    def __onClientUpdated(self, diff, _):
        if RESOURCE_WELL_PDATA_KEY in diff and b'initialAmounts' in diff[RESOURCE_WELL_PDATA_KEY]:
            self.__setNumberInitialValues()
        return
