...
class RentalsController(IRentalsController):
    ...
    def getRentPackagesInfo(self, rentPrices, currentRentInfo):
        ...
        mainRentType = None
        mainRentTypeWeight = float(b'-inf')
        seasonType = None
        if self.isEnabled():
                ...
                if packages:
                    ...
                    if hasAvailableRentPackages:
                        ...
                        if rentTypeWeight > mainRentTypeWeight:
                            ...
                            seasonType = next(iter(viewvalues(packages))).get(b'seasonType', None)

        return (
         hasAvailableRentPackages, mainRentType, seasonType)

    def getRentPriceOfPackage(self, vehicle, rentType, packageID, package):
        rentPrice = package.get(b'cost', (0, 0))
        if rentType == RentType.SEASON_RENT:
            seasonType = package.get(b'seasonType', None)
            cycleRentPrice = package.get(b'defaultCycleCost', (0, 0))
    ...
    def __seasonFilter(self, seasonID, seasonType, package):
        ...
        if not currentSeason.getCycleInfo():
            ...
            if currentCycleNumber >= numCycles:
                return False
            cyclesWithinSeasonRentIsActive = package.get(b'cycles', [])
