...
_XML_PATH = ITEM_DEFS_PATH + b'special_voices.xml'
_FULL_CREW_CONDITION = b'isFullCrew'
_VoiceoverParams = namedtuple(b'_VoiceoverParams', [b'languageMode', b'genderSwitch', b'onlyInNational'])
_genderStrToSwitch = {b'male': (CREW_GENDER_SWITCHES.MALE), 
   b'female': (CREW_GENDER_SWITCHES.FEMALE)}
...
class SpecialSoundCtrl(ISpecialSoundCtrl):
    ...
    def __setVoiceoverByVehicleOrTankman(self, vehiclePublicInfo, isPlayerVehicle):
        ...
        if self.__setSpecialVoiceByVehicle(vehicleType.name, isPlayerVehicle):
            ...
            if crewGroups:
                ...
                _logger.error(b'There is not information about vehicle commander to extract correct sound mode')
    ...
    def __setArenaMusicByStyle(self, vehiclePublicInfo, isPlayerVehicle):
        ...
        if arena is None:
            ...
            if arenaVisitor.bonus.hasRespawns():
                _logger.debug(b'Skip special arena sound according to game mode')
                return
    ...
    def __readSpecialVoices(self):
        ...
        if rootSection is None:
            _logger.error(b'Could not open special voices xml: %s', _XML_PATH)
            return
        else:
            voiceoverSection = rootSection[b'voiceover']
            if voiceoverSection is not None:
                for source, paramSection in voiceoverSection.items():
                    tag = paramSection.readString(b'tag')
                    onlyInNational = paramSection.readBool(b'onlyInNational')
                    genderStr = paramSection.readString(b'gender')
                    gender = _genderStrToSwitch.get(genderStr, CREW_GENDER_SWITCHES.DEFAULT)
                    languageMode = paramSection[b'languageMode']
            ...
            arenaMusicSection = rootSection[b'arenaMusic']
            if arenaMusicSection is not None:
                for source, paramSection in arenaMusicSection.items():
                    ...
                    tag = paramSection.readString(b'tag')
                    arenaMusic = {}
                    musicSetupSection = paramSection[b'wwmusicSetup']
    ...
    def __setSpecialVoice(self, params):
        ...
        if params.onlyInNational and setting.getSystemModeType() == AltVoicesSetting.SOUND_MODE_TYPE.REGULAR:
            _logger.debug(b'%s can be used only in national sound mode', params.languageMode)
            return
        if not soundGroupInst.soundModes.setMode(params.languageMode):
            _logger.warning(b'Could not set special voice: %s', params.languageMode)
            return
