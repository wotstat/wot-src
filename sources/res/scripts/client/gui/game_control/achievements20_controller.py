...
class Achievements20Controller(IAchievements20Controller):
    ...
    def onLobbyInited(self, event):
        ...
        g_clientUpdateManager.addCallbacks({b'stats.credits': (self.__onUpdate), 
           b'stats.gold': (self.__onUpdate), 
           b'goodies': (self.__onUpdate)})
        return
    ...
    def showNewSummaryEnabled(self):
        data = {}
        if isWTREnabled():
            data[b'type'] = Achievements20SystemMessages.FIRST_ENTRY
        else:
            data[b'type'] = Achievements20SystemMessages.FIRST_ENTRY_WITHOUT_WTR
    ...
    def showRatingUpgrade(self):
        ...
        self.__pushAchievements20SystemMessage({b'type': (Achievements20SystemMessages.RATING_UPGRADE), b'rank': rank, b'subRank': subRank})
        return

    def showRatingChanged(self):
        self.__pushAchievements20SystemMessage({b'type': (Achievements20SystemMessages.RATING_DOWNGRADE)})
        return
    ...
    def showRankedComplete(self):
        ...
        rating = getNormalizedValue(self.__itemsCache.items.sessionStats.getAccountWtr())
        rank, subRank, _ = checker.getStage(rating)
        self.__pushAchievements20SystemMessage({b'type': (Achievements20SystemMessages.RATING_COMPLETE), b'rank': rank, b'subRank': subRank})
        return

    def showEditAvailable(self):
        self.__pushAchievements20SystemMessage({b'type': (Achievements20SystemMessages.EDITING_AVAILABLE)})
        return
