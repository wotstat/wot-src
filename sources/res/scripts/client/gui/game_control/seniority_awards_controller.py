...
REG_EXP_QUEST_YEAR_TIER = b':([Y, y]\\d+):'
REG_EXP_QUEST_TEST_GROUP = b':([A,a,B,b][T,t])'
WDR_CURRENCY = b'wdrcoin'
...
class SeniorityAwardsController(ISeniorityAwardsController):
    ...
    def getVehiclesForSelectionCount(self):
        ...
        for quest in self.__eventsCache.getHiddenQuests(__vehicleSelectionFilterFunc).values():
            vehicleBonuses = quest.getBonuses(b'vehicles')
            if vehicleBonuses:
    ...
    def vehicleSelectionQuestPrefix(self):
        return self.vehicleSelectionQuestPattern.format(id=b'')
    ...
    def maxCategory(self):
        return (b'ctg{}').format(len(self.config.categories))
    ...
    def getVehicleSelectionRewards(self):
        ...
        bonusVehicles = {}
        for quest in self.__eventsCache.getHiddenQuests(__vehicleSelectionFilterFunc).values():
            rewardId = quest.getID().split(b':')[-1]
            for vehBonus in quest.getBonuses(b'vehicles'):
        ...
        if regexp:
            for questID in completedQuests:
                ...
                if seniorityLvlSearch is not None:
                    seniorityLvl = seniorityLvl or seniorityLvlSearch.groups(default=b'')[0]
    ...
    def __onClaimRewardFailed(self):
        self.__cancelClaimTimeout()
        self.__hideWaiting()
        SystemMessages.pushI18nMessage(b'#system_messages:seniority_awards/claim_reward_failed', type=SystemMessages.SM_TYPE.Error, priority=NotificationPriorityLevel.HIGH)
        return

    @staticmethod
    def __showWaiting():
        Waiting.show(b'claimSeniorityAwards')
        return

    @staticmethod
    def __hideWaiting():
        Waiting.hide(b'claimSeniorityAwards')
        return
    ...
    def __hasClientTokens(self):
        pattern = self.claimVehicleRewardTokenPattern.format(id=b'')
