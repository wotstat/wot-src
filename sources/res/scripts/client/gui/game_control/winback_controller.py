...
class WinbackController(IWinbackController):
    ...
    def parseOfferToken(self, token):
        ...
        offerType = token.split(b':')[2]
        offerLevel = getLevelFromSelectableToken(token)
        return {b'name': (TOKEN_TO_REWARD_MAPPING[offerType]), b'level': offerLevel, b'token': token}
    ...
    def hasWinbackOfferToken(self):
        ...
        for token in tokens:
            if self.isWinbackOfferToken(token) and token.endswith(b'_gift'):
                return True
    ...
    def getPossibleQuestsNames(self):
        ...
        winbackQuestTemplate = b'{}{}{}_'
        possibleQuestsNames = {}
        possibleQuestsNames[b'dNormalQuestsBody'] = winbackQuestTemplate.format(winbackQuestPrefix, DEFAULT_CHAIN_VERSION, WinbackQuestTypes.NORMAL.value)
        possibleQuestsNames[b'dCompensationQuestsBody'] = winbackQuestTemplate.format(winbackQuestPrefix, DEFAULT_CHAIN_VERSION, WinbackQuestTypes.COMPENSATION.value)
        possibleQuestsNames[b'cNormalQuestsBody'] = winbackQuestTemplate.format(winbackQuestPrefix, self.chainVersion, WinbackQuestTypes.NORMAL.value)
        possibleQuestsNames[b'cCompensationQuestsBody'] = winbackQuestTemplate.format(winbackQuestPrefix, self.chainVersion, WinbackQuestTypes.COMPENSATION.value)
    ...
    def __initQuestsChain(self):
        if self.winbackConfig.isEnabled and self.winbackConfig.isProgressionEnabled and self.__state == _WinbackState.IN_PROGRESS:
            ...
            actualQuestsByType = {chekpoint: {(WinbackQuestTypes.NORMAL): (allWinbackQuests.get(questNames[b'cNormalQuestsBody'] + chekpoint, allWinbackQuests.get(questNames[b'dNormalQuestsBody'] + chekpoint))), (WinbackQuestTypes.COMPENSATION): (allWinbackQuests.get(questNames[b'cCompensationQuestsBody'] + chekpoint, allWinbackQuests.get(questNames[b'dCompensationQuestsBody'] + chekpoint)))} for chekpoint in questChainChekpoints}
            self.__questsChain = {quest.getID(): quest for quest in viewvalues(actualQuestsByType) if quest}
        else:
            self.__questsChain = {}
        return
