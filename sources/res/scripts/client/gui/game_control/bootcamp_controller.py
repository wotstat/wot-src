...
BootcampDialogConstants = namedtuple(b'BootcampDialogConstants', b'dialogType dialogKey focusedID needAwarding premiumType')
_GREEN = b'green'
_YELLOW = b'yellow'
_GRAY = b'gray'
_REWARD = b'\n{}'
...
class BootcampController(IBootcampController):
    ...
    def startBootcamp(self, inBattle):
        ...
        if self.__inBootcamp:
            LOG_ERROR(b'Trying to startBootcamp - is already started')
            return
        if not inBattle:
    ...
    def stopBootcamp(self, inBattle):
        ...
        if not self.__inBootcamp:
            LOG_ERROR(b'Trying to stop bootcamp - is already stopped')
            return
        if inBattle:
    ...
    def hasFinishedBootcampBefore(self):
        return g_bootcamp.getParameters().get(b'completed', 0) > 0

    def runCount(self):
        return g_bootcamp.getParameters().get(b'runCount', 0)

    def needAwarding(self):
        return g_bootcamp.getParameters().get(b'needAwarding', False)
    ...
    def getAwardVehicles(self):
        if self.nationData:
            return [self.nationData[b'vehicle_first'], self.nationData[b'vehicle_second']]
        return []
    ...
    def runBootcamp(self):
        if self.isInBootcamp():
            ...
            DialogsInterface.showI18nInfoDialog(b'bootcampCenterUnavailable', (lambda result: None))
        else:
    ...
    def __format(text, style, **kwargs):
        return makeHtmlString(b'html_templates:bootcamp/message', style, ctx={b'text': (backport.text(text, **kwargs))})
