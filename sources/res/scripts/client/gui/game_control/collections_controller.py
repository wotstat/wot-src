...
class CollectionsSystemController(ICollectionsSystemController, EventsHandler):
    ...
    def getCollection(self, collectionId):
        ...
        if collection is None and collectionId != 0:
            _logger.error(b'Collection with id <%s> does not exist!', collectionId)
        return collection
    ...
    def getCollectionByName(self, collectionName):
        ...
        if collection is None:
            _logger.error(b'Collection with name <%s> does not exist!', collectionName)
        return collection
    ...
    def _getCallbacks(self):
        return (
         (
          b'cache.isEmergencyModeEnabled', self.__onEmergencyModeChanged),)
    ...
    def __onChatMessageReceived(self, *ctx):
        ...
        if self.isEnabled() and message.type == SYS_MESSAGE_TYPE.collectionEntitlementReceived.index():
            entitlementCode = message.data.get(b'entitlementName', b'')
