...
_BATTLE_MATTERS_COMPLETE = b'battle_matters_complete'
...
_AB_TEST_TOKEN_PREFIX = b'bm_test'
_AB_TEST_ALTERNATIVE_CONF = b'ALT'
_QuestIDDescr = namedtuple(b'_QuestIDDescr', b'type, number, abTestCohort')
_QuestIDRePattern = re.compile(b'^(battle_matters|intermediate_battle_matters|compensation_battle_matters)(?:_(.+))?_(\\d+)$')
...
class BattleMattersController(IBattleMattersController):
    ...
    __slots__ = (b'_em', b'onStateChanged', b'onFinish', b'_isEnabled', b'_prevFinishStateFlag', b'_isPaused', b'_isAvailable', b'__delayedRewardOfferCurrencyTokenPattern', b'__delayedRewardOfferVisibilityTokenPattern', b'__isWaitingToken', b'__savedRewards', b'__hasDelayedRewards', b'__finishState', b'__hintHelper', b'__abTestCohort', b'__isAbTestDefaultConf', b'__progressWatcher')
    ...
    def __init__(self):
        ...
        self.__delayedRewardOfferCurrencyTokenPattern = b''
        self.__delayedRewardOfferVisibilityTokenPattern = b''
        self.__savedRewards = {}
    ...
    def isBattleMattersQuestIDForThisPlayer(self, questID):
        if not self.isBattleMattersQuestID(questID):
            ...
            if questIDDescr is None:
                return False
            return self.__isAbTestDefaultConf and not questIDDescr.abTestCohort or questIDDescr.abTestCohort == self.__abTestCohort
    ...
    def getQuestsWithDelayedReward(self):
        ...
        def filterFunc(quest):
            return any(self.__isAnyTokensMatchPattern(bonus.getValue(), self.__delayedRewardOfferVisibilityTokenPattern) for bonus in quest.getBonuses(b'tokens'))
    ...
    def hasDelayedRewardToken(self, delayedRewardToken):
        if not self.isDelayedRewardToken(delayedRewardToken):
            ...
            return self.__itemsCache.items.tokens.getToken(delayedRewardToken) is not None

    def isDelayedRewardObtained(self, delayedRewardToken):
        if not self.isDelayedRewardToken(delayedRewardToken):
            return False
        return self.hasDelayedRewardToken(delayedRewardToken) and not self.__battleMattersSelectableRewardMgr.isAvailableBonus(delayedRewardToken)
    ...
    def getDelayedRewardVehiclesLevel(self, delayedRewardToken):
        ...
        return properties.get(b'giftVehiclesLevel')
    ...
    def getABTestConfiguration(self):
        if self.__isAbTestDefaultConf:
            return b'default'
        return self.__abTestCohort
    ...
    def __saveRewards(self, questsData, clientCtx=None):
        ...
        if questsData is not None:
            questIDs = [qID for qID in questsData.get(b'completedQuestIDs', set()) if self.isBattleMattersQuestID(qID)]
    ...
    def __addRewards(self, quest, questsData, isInPair, sequenceNumber=None):
        ...
        self.__savedRewards.setdefault(order, {}).setdefault(b'quests', {}).update({(quest.getID()): (questsData.get(b'detailedRewards', {}).get(quest.getID(), {}))})
    ...
    def __readABTestConf(self):
        ...
        return (
         isAbTestDefaultConf, abTestCohort)
