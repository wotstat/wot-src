...
class PlatoonController(IPlatoonController, IGlobalListener, CallbackDelayer):
    ...
    def onPrbEntitySwitched(self):
        ...
        if not self.__executeQueue:
            return
        self.__executeQueue = False
        self.prbDispatcher.doAction(PrbAction(b''))
        ...
        if queueType in QUEUE_TYPE_TO_PREBATTLE_ACTION_NAME:
            ...
            if navigationPossible:
                if self.prbDispatcher:
                    ...
                    _logger.error(b'Prebattle dispatcher is not defined or navigation not possible')
        else:
            _logger.error(b'QueueType not found %d', queueType)
        return

    def startSearch(self):
        if isinstance(self.prbEntity, UnitEntity):
            if not self.isInSearch():
                ctx = AutoSearchUnitCtx(b'prebattle/auto_search')
                _logger.debug(b'Unit request: %s', str(ctx))
    ...
    def cancelSearch(self):
        if isinstance(self.prbEntity, UnitEntity) and self.isInSearch():
            ctx = AutoSearchUnitCtx(b'prebattle/auto_search', action=0)
        ...
        if self.isAnyPlatoonUIShown() and toggleUI:
            ...
            if self.hasSearchSupport() and self.__isActiveSearchView:
                if not (self.isInSearch() or self.__startAutoSearchOnUnitJoin):
                    _logger.error(b'Invalid Platoon UI state!')
    ...
    def onUnitFlagsChanged(self, flags, timeLeft):
        if self.getPrbEntityType() not in PREBATTLE_TYPE.SQUAD_PREBATTLES:
            return
        _logger.debug(b'PlatoonController: onUnitFlagsChanged')
    ...
    def onUnitVehiclesChanged(self, dbID, vInfos):
        if self.getPrbEntityType() not in PREBATTLE_TYPE.SQUAD_PREBATTLES:
            ...
            _logger.debug(b'PlatoonController: onUnitVehiclesChanged')
            entity = self.prbEntity
    ...
    def onUnitMembersListChanged(self):
        if self.getPrbEntityType() not in PREBATTLE_TYPE.SQUAD_PREBATTLES:
            return
        _logger.debug(b'PlatoonController: onUnitMembersListChanged')
        self.__updatePlatoonTankInfo()
        return
    ...
    def onUnitPlayerAdded(self, pInfo):
        if self.getPrbEntityType() not in PREBATTLE_TYPE.SQUAD_PREBATTLES:
            return
        _logger.debug(b'PlatoonController: onUnitPlayerAdded')
    ...
    def onUnitPlayerRemoved(self, pInfo):
        if self.getPrbEntityType() not in PREBATTLE_TYPE.SQUAD_PREBATTLES:
            return
        _logger.debug(b'PlatoonController: onUnitPlayerRemoved')
    ...
    def onUnitPlayerStateChanged(self, pInfo):
        if self.getPrbEntityType() not in PREBATTLE_TYPE.SQUAD_PREBATTLES:
            return
        _logger.debug(b'PlatoonController: onUnitPlayerStateChanged')
        self.onMembersUpdate()
        if self.__isActiveSearchView:
            if not self.isInSearch():
                self.__isActiveSearchView = False
    ...
    def orderSlotsBasedOnDisplaySlotsIndices(self, slots):
        ...
        emptySlot = findFirst((lambda slot: slot[b'player'] is None), slots)
        orderedSlots = [emptySlot] * len(slots)
        for slot in slots:
            playerAccId = slot[b'player'][b'accID'] if slot[b'player'] else -1
            for playerId, displayIndex in self.__tankDisplayPosition.items():
                if playerAccId == playerId:
                    if displayIndex >= len(orderedSlots):
                        _logger.warning(b'The number %s of display slots in the platoon is invalid', displayIndex)
                        return slots
    ...
    def getSquadManStates(self, player, role):
        if player is not None:
            ...
            if role is not None and role & UNIT_ROLE.IN_ARENA:
                return b'inBattle'
            if player[b'readyState']:
                return b'ready'
            if accID != player[b'accID']:
                return b'notReady'
            return b'notReadyPlayer'
        if self.isInSearch():
            return b'searching'
        else:
            return b'empty'
    ...
    def __showWindow(self, ePlatoonLayout, dropdownOffset=None):
        ...
        if self.prbEntity.getEntityType() == PREBATTLE_TYPE.STRONGHOLD:
            ...
            if layout is None:
                _logger.error(b'Layout %s is missing.', ePlatoonLayout)
                return
            position = self.__calculateDropdownMove(dropdownOffset)
            window = layout.windowClass(position)
            if window is None:
                _logger.error(b'Window creation of type %s is failing', ePlatoonLayout)
                return
            window.load()
            if ePlatoonLayout == EPlatoonLayout.MEMBER:
                window.center()
    ...
    def __startListening(self):
        _logger.debug(b'PlatoonController: start listening')
        unitMgr = prb_getters.getClientUnitMgr()
        if unitMgr:
            _logger.debug(b'PLATOON: UnitMgr!')
    ...
    def __onChannelControllerInited(self, event):
        ctx = event.ctx
        prbType = ctx.get(b'prbType', 0)
        if not prbType:
            _logger.debug(b'Prebattle type %d is not defined', prbType)
            return
        else:
            ...
            if controller is None:
                _logger.error(b'Channel controller is not defined %s', str(ctx))
                return
            ctx.clear()
    ...
    def __unitMgrOnUnitJoined(self, unitMgrID, prbType):
        _logger.debug(b'PlatoonController: __unitMgrOnUnitJoined')
        self.__tankDisplayPosition.clear()
    ...
    def __unitMgrOnUnitLeft(self, unitMgrID, isFinishedAssembling):
        _logger.debug(b'PlatoonController: __unitMgrOnUnitLeft')
    ...
    def __platoonTankDestroyed(self, event):
        self.__currentlyDisplayedTanks.discard(event.ctx.get(b'entity'))
    ...
    def __clearPlatoonTankInfo(self):
        entity = self.prbEntity
        if entity is not None and hasattr(entity, b'getRosterSettings'):
            unitSlotCount = entity.getRosterSettings().getMaxSlots()
    ...
    def __updateDisplaySlotsIndices(self):
        ...
        if len(playerIds) > maxSlotCount:
            _logger.warning(b'The number of players in slot (%s) is higher then max slots to display (%s). This state should not happen for this type of unit.', len(playerIds), maxSlotCount)
        if maxSlotCount >= 3:
