...
class WotldaRequester(object):
    ...
    def getSubscriptionLoadouts(self, clientCacheUpdatedAt, loadoutTypes):
        ...
        _logger.debug(b'Requesting equipment for %s ', context.getLoadoutTypesForRequest())
        response = yield wg_await(self._getLoadouts(context))
        if response.isSuccess():
            _logger.debug(b'Loadouts obtained, result = %s', response.getData())
        raise AsyncReturn(response)
        return
    ...
    def _getLoadouts(self, context):
        ...
        try:
            ...
            _logger.debug(b'Request session timout reached.')
            response.setException(e)
        except BrokenPromiseError as e:
            _logger.debug(b'Broken promise error.')
            response.setException(e)
        except Exception as e:
            _logger.debug(b'Failed to get loadouts data.')
            response.setException(e)
