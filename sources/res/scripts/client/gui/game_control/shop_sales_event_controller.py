...
_SHOP_SALES_ENTRY_POINT = b'ShopSalesEntryPoint'
...
class ShopSalesEventController(IShopSalesEventController, _Notifiable, EventsHandler):
    ...
    def getEventPhase(self, timestamp):
        if timestamp < self.activePhaseStartTime:
            ...
            if timestamp < self.activePhaseFinishTime:
                return EventPhase.IN_PROGRESS
            if timestamp < self.eventFinishTime:
    ...
    def getEventPhaseTimeRange(self, phase):
        ...
        if phase == EventPhase.FINISHED:
            return (self.activePhaseFinishTime, self.eventFinishTime)
        return (
         self.eventFinishTime, sys.maxint)
    ...
    def openMainView(self, url=None, origin=None):
        ...
        _logger.info(b'I will open shop sales with url %s', url)
        ...
        for item in removed:
            if item.eventType == _HANGAR_ENTRY_POINTS:
                ...
                for entryData in notificationEntries:
                    if str(entryData.get(b'id')) == _SHOP_SALES_ENTRY_POINT:
                        self.__reset()
                        self.__updatePhase()
    ...
    def __parseNotificationData(self, item):
        if item is None:
            ...
            for entryData in notificationEntries:
                if str(entryData.get(b'id')) == _SHOP_SALES_ENTRY_POINT:
                    self.__enabled = True
                    self.__url = entryData.get(b'shopSalesUrl', b'')
                    try:
                        self.__eventStartTime = getTimestampByStrDate(entryData.get(b'startDate', b''))
                        self.__eventFinishTime = getTimestampByStrDate(entryData.get(b'endDate', b''))
                        self.__activePhaseStartTime = getTimestampByStrDate(entryData.get(b'activePhaseStartTime', b''))
                        self.__activePhaseFinishTime = getTimestampByStrDate(entryData.get(b'activePhaseFinishTime', b''))
                    except ValueError as exc:
                        _logger.error(b'Invalid event config: %s', exc)
                        self.__reset()
    ...
    def __getToNextNotifyDelta(self):
        ...
        if endTimestamp == sys.maxint:
            return 0
        return endTimestamp - getServerUTCTime() + 1
...
def getShopSalesEntryPointIsActive(shopSalesEventController=None):
    _logger.debug(b'isShopSalesEntryPointAvailable: %s', shopSalesEventController.isShopSalesEntryPointAvailable())
