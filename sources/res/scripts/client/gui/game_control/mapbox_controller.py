...
ProgressionData = namedtuple(b'ProgressionData', (b'surveys', b'rewards', b'minRank', b'totalBattles', b'nextSubstage'))
MapData = namedtuple(b'MapData', (b'progress', b'total', b'passed', b'available', b'url'))
RewardData = namedtuple(b'RewardData', (b'bonusList', b'status'))
_LAST_PERIOD = 0
...
class MapboxController(Notifiable, SeasonProvider, IMapboxController, IGlobalListener):
    ...
    def handleSurveyCompleted(self, surveyData):
        ...
        if self.__webCtrl.isAvailable():
            ...
            if result.isSuccess():
                mapId = surveyData[b'name']
                self.onMapboxSurveyCompleted(mapId)
                self.surveyManager.resetSurvey(mapId)
            yield proxy()
        else:
            _logger.error(b'Survey completed request not sent due to CLIENTGW unavailability')
        return
    ...
    def getUnseenItemsCount(self):
        ...
        if not self.isEnabled() or self.getCurrentCycleID() is None or progressionData is None:
            return 0
        return len([mapName for mapName, mapData in progressionData.surveys.iteritems() if not self.__settingsManager.isMapVisited(mapName) and mapData.available and mapData.progress >= mapData.total]) + len([rewardItem for battles, reward in progressionData.rewards.iteritems() if battles <= progressionData.totalBattles for rewardItem in reward.bonusList if rewardItem.getName() == b'selectableCrewbook'])
...
class MapboxProgressionDataProvider(Notifiable):
    ...
    __slots__ = (b'onProgressionDataUpdated', b'__progressionData', b'__isSyncing', b'__isShuttingDown', b'__isStarted', b'__restartNotifier', b'__randomRestartProgressionDelay')
    ...
    def getProgressionData(self):
        if not self.__progressionData:
            ...
            return ProgressionData({key: MapData(value[b'progress'], value[b'total'], value[b'passed'], value[b'available'], value[b'url']) for key, value in self.__progressionData.get(b'surveys', {}).iteritems()}, {value[b'battles']: RewardData(formatMapboxBonuses(value[b'reward']), value[b'status']) for value in self.__progressionData.get(b'rewards', [])}, self.__progressionData.get(b'min_rank'), self.__progressionData.get(b'total_battles_amount'), self.__getProgressionRestartTimeWithRandomDelay())
    ...
    def forceUpdateProgressData(self):
        try:
            ...
            _logger.debug(b'%s has been destroyed before got a response to remote mapbox progression request', self)
            result = False
    ...
    @server_settings.serverSettingsChangeListener(Configs.MAPBOX_CONFIG.value)
    def __onServerSettingsChanged(self, diff):
        if b'progressionUpdateInterval' in diff[Configs.MAPBOX_CONFIG.value]:
            self.startNotification()
        return
    ...
    def __getProgressionRestartTimeWithRandomDelay(self):
        endTime = self.__progressionData.get(b'next_substage_at')
        if endTime:
...
class MapboxSettingsManager(object):
    ...
    def clearSettings(self):
        self.__settings[b'visited_maps'] *= 0
        self.__settings[b'stored_rewards'].clear()
        self.__settings[b'previous_battles_played'] = 0
        return

    def isMapVisited(self, mapName):
        return mapName in self.__settings[b'visited_maps']

    def addVisitedMap(self, mapName):
        if mapName not in self.__settings[b'visited_maps']:
            self.__settings[b'visited_maps'].append(mapName)
        return

    def storeReward(self, numBattles, rewardIdx, rewardIconName):
        self.__settings[b'stored_rewards'].setdefault(numBattles, {})
        self.__settings[b'stored_rewards'][numBattles].update({rewardIdx: rewardIconName})
        return

    def getStoredReward(self, numBattles, rewardIdx):
        return self.__settings[b'stored_rewards'].get(numBattles, {}).pop(rewardIdx, b'')

    def setPrevBattlesPlayed(self, numBattles):
        self.__settings[b'previous_battles_played'] = numBattles
        return

    def getPrevBattlesPlayed(self):
        return self.__settings.get(b'previous_battles_played', 0)

    def storeCycle(self, isActive, cycleId):
        if not isActive or cycleId is None:
            self.__settings[b'lastCycleId'] = None
        elif self.__settings[b'lastCycleId'] != cycleId:
            self.__settings[b'lastCycleId'] = cycleId
