...
MapData = namedtuple(b'MapData', (b'progress', b'total', b'passed'))
...
class MapboxController(Notifiable, SeasonProvider, IMapboxController, IGlobalListener):
    ...
    def handleSurveyCompleted(self, surveyData):
        ...
        if self.__webCtrl.isAvailable():
            ...
            _logger.error(b'Survey completed request not sent due to WGCG unavailability')
        return
...
class MapboxProgressionDataProvider(Notifiable):
    ...
    def getProgressionData(self):
        if not self.__progressionData:
            ...
            return ProgressionData({key: MapData(value[b'progress'], value[b'total'], value[b'passed']) for key, value in iteritems(self.__progressionData.get(b'surveys', {}))}, {value[b'battles']: RewardData(formatMapboxBonuses(value[b'reward']), value[b'status']) for value in self.__progressionData.get(b'rewards', [])}, self.__progressionData.get(b'min_rank'), self.__progressionData.get(b'total_battles_amount'), self.__getProgressionRestartTimeWithRandomDelay())
