...
import BigWorld, Event, season_common
...
class RankedBattlesController(IRankedBattlesController, Notifiable, SeasonProvider, IGlobalListener):
    ...
    def onLobbyInited(self, event):
        ...
        g_clientUpdateManager.addCallbacks({b'ranked': (self.__onUpdateRanked), 
           b'ranked.accRank': (self.__onUpdateLeagueSync), 
    ...
    def isSeasonRewarding(self):
        ...
        notInLeaguesQuestID = NOT_IN_LEAGUES_QUEST.format(prevSeason.getSeasonID() if prevSeason is not None else b'')
    ...
    def isSuitableVehicle(self, vehicle):
        ...
        if vehicle.level < config.minLevel or config.maxLevel < vehicle.level:
            ...
            ctx = {b'levels': (lrange(config.minLevel, config.maxLevel + 1))}
    ...
    def getCurrentDivision(self):
        return findFirst(operator.methodcaller(b'isCurrent'), self.getDivisions())
        ...
        if isCurrent:
            ...
            for qID in sorted(seasonQuests, reverse=True):
                ...
                if season == seasonID and not isSprinter:
                    ...
                    result.append({b'league': leagueID, b'awards': (quest.getBonuses(bonusName=bonusName))})
    ...
    def getRankDisplayInfoForBattle(self, rankID):
        ...
        if division is None:
            _logger.warning(b'Wrong ranked_config.xml, cannot be found division for rank %s.', rankID)
            ...
            _logger.error(b'Wrong ranked_config.xml, wrong rankGroups section!!!')
            return BattleRankInfo(-1, -1, False)
    ...
    def runQuests(self, quests):
        ...
        for quest in quests:
            if not quest.isCompleted():
                ...
                _logger.warning(b'Quest is already completed: %s', quest.getID())
    ...
    def showRankedBattlePage(self, ctx):
        if not self.__canShowRankedPage():
            _logger.info(b"Can't open ranked battles page. Forwarding ctx to web fallback.")
            self.__openWebPageByCtx(ctx)
            return
        else:
            if not self.__canShowSelectedPageID(ctx):
                _logger.info(b"Can't open ranked battles page on selected tab. Call is skipped.")
                return
            if self.getCurrentSeason() is not None:
                ...
                prevSeason = self.getPreviousSeason()
                if prevSeason:
                    ctx.update({b'prevSeason': prevSeason})
                    self.__saveWebOpenPageCtx(ctx)
                    _showRankedBattlePageSeasonOff(ctx)
                else:
                    _logger.warning(b'There is no current or previous season to show RankedMainSeasonOn(Off)Page')
            return
    ...
    def __onShowBattleResults(self, reusableInfo, composer, resultsWindow):
        ...
        if arenaBonusType == ARENA_BONUS_TYPE.RANKED and arenaUniqueID not in self.__arenaBattleResultsWasShown:
            ...
            _logger.debug(b'Ranked Overlay windows will not be shown, received arenaBonusType: %s', arenaBonusType)
        return
    ...
    def __canShowSelectedPageID(self, ctx):
        selectedItemID = ctx.get(b'selectedItemID', b'')
        if selectedItemID == RANKEDBATTLES_CONSTS.RANKED_BATTLES_YEAR_RATING_ID and not self.isYearLBEnabled():
            _logger.info(b"Can't open ranked battles page on year leaderboard tab.")
            return False
        else:
            if selectedItemID == RANKEDBATTLES_CONSTS.RANKED_BATTLES_SHOP_ID and not self.isRankedShopEnabled():
                _logger.info(b"Can't open ranked battles page on ranked shop tab.")
                return False
            if selectedItemID == RANKEDBATTLES_CONSTS.RANKED_BATTLES_REWARDS_ID and not self.isYearRewardEnabled():
                rewardsSelectedTab = ctx.get(b'rewardsSelectedTab', b'')
                if rewardsSelectedTab == RANKEDBATTLES_ALIASES.RANKED_BATTLES_REWARDS_YEAR_UI:
                    _logger.info(b"Can't open ranked battles page on year reward tab without year rewards.")
                    return False
                if self.getCurrentSeason() is None:
                    _logger.info(b"Can't open ranked battles page on rewards tab in season gap without year rewards.")
                    return False
            return True
        ...
        if shieldConfig is None:
            ...
            if currentShield is not None:
                ...
                if accPrevShieldHP is None:
                    _logger.warning(b'Info about rank shield not available for %s in clientShields', rankID)
                    accPrevShieldHP = hp
    ...
    def __getRankedDossier(self):
        return self.__itemsCache.items.getAccountDossier().getDossierDescr()[b'rankedSeasons']
    ...
    def __getRankedSettings(self):
        ...
        for season in generalSettings.seasons.values():
            if cycleID in season.get(b'cycles', {}):
                return generalSettings.replace(season).replace(season[b'cycles'][cycleID])
    ...
    def __buildDivisions(self):
        ...
        startRanks = sorted([data[b'startRank'] for data in itervalues(self.__rankedSettings.divisions) if not data[b'isLeague']])
    ...
    def __openWebPageByCtx(self, ctx):
        url = alias = None
        selectedItemID = ctx.get(b'selectedItemID', b'')
        clientParams = ctx.get(b'clientParams', {})
        clientParams[b'isLobbySub'] = True
    ...
    def __saveWebOpenPageCtx(self, ctx):
        if self.__rankedWebOpenPageCtx is not None:
            logging.error(b"Previous open ranked page web ctx wasn't processed. Too frequent requests from web.")
        self.__rankedWebOpenPageCtx = ctx if ctx.get(b'webParams', b'') or ctx.get(b'clientParams', {}) else None
        return
    ...
    def __showBetweenSeason(self):
        ctx = {b'selectedItemID': (RANKEDBATTLES_CONSTS.RANKED_BATTLES_RANKS_ID)}
    ...
    def __showYearlyLeaders(self):
        ctx = {b'selectedItemID': (RANKEDBATTLES_CONSTS.RANKED_BATTLES_YEAR_RATING_ID)}
        self.showRankedBattlePage(ctx)
        return
