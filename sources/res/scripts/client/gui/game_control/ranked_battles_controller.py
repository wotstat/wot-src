...
class RankedBattlesController(IRankedBattlesController, Notifiable, SeasonProvider, IGlobalListener):
    ...
    def isSuitableVehicle(self, vehicle):
        ...
        if vehicle.level < config.minLevel or config.maxLevel < vehicle.level:
            ...
            ctx = {b'levels': (range(config.minLevel, config.maxLevel + 1))}
    ...
    def awardWindowShouldBeShown(self, rankChangeInfo):
        if rankChangeInfo.prevAccRank == self.getMaxPossibleRank():
            return False
        if rankChangeInfo.stepChanges <= 0 and rankChangeInfo.prevAccRank == ZERO_RANK_ID:
            return False
        return True
    ...
    def showRankedAwardWindow(self, rankedInfo, questsProgress):
        ...
        if season is None:
            ...
            if not self.awardWindowShouldBeShown(rankedInfo):
                return
            awardsSequence = []
    ...
    def getRankSquadRestriction(self):
        return self.__rankedSettings.squadRankRestrictions.get(b'rankDiff')

    def getDivisionSquadRestriction(self):
        if self.__rankedSettings.squadRankRestrictions.get(b'sameDivisionOnly'):
            return True
        else:
            return
    ...
    def __buildDivisions(self):
        ...
        startRanks = sorted([data[b'startRank'] for data in rankedDivisions() if not data[b'isLeague'] and data[b'startRank'] > 0])
