...
_PromoData = namedtuple(b'_PromoData', [b'url', b'closeCallback', b'source'])
...
class PromoController(IPromoController):
    ...
    def setNewTeaserData(self, teaserData):
        timestamp = teaserData[b'timestamp']
    ...
    def showFieldPost(self):
        ...
        self.__showBrowserView(url, loadingCallback, soundSpaceID=b'field_post')
        return

    @adisp_process
    def showLastTeaserPromo(self):
        rowUrl = self.__promoData.get(b'url', b'')
    ...
    def __onPromoClosed(self, **kwargs):
        ...
        actionType = PromoLogActions.CLOSED_BY_USER if kwargs.get(b'byUser') else PromoLogActions.KILLED_BY_SYSTEM
        self.__logger.logAction(action=actionType, type=PromoLogSubjectType.PROMO_SCREEN_OR_INDEX, url=kwargs.get(b'url'))
        return

    @adisp_process
    def __requestPromoCount(self):
        if not self.isActive():
            _logger.warning(b'Trying to request unread promos count when promo functionality is disabled')
            return
    ...
    def __updateTeaserData(self, teaserData):
        self.__lastUpdateTimeMark = teaserData[b'timestamp']
        self.__promoData = teaserData[b'lastPromo']
        self.__updatePromoCount(teaserData[b'count'])
        if self.__promoData.get(b'url'):
            self.__tryToShowTeaser()
        return
    ...
    def __showTeaser(self):
        if self.isActive():
            if len(self.onNewTeaserReceived) > 1:
                _logger.info(b'PromoController has multiple onNewTeaserReceived listeners!')
            self.onNewTeaserReceived(self.__promoData, self.__onTeaserShown, self.__onTeaserClosed)
        else:
            _logger.warning(b'Impossible to show teaser, functionality is disabled')
        return
    ...
    def __processPromo(self, promos):
        if not self.__isPromoOpen and not self.__waitingForWebBridgeData:
            logData = {b'action': (PromoLogActions.OPEN_IN_OLD), b'type': (PromoLogSubjectType.PROMO_SCREEN)}
            if self.__pendingPromo is not None:
                promoData = self.__pendingPromo
                logData.update({b'source': (promoData.source), b'url': (promoData.url)})
            ...
            if promo:
                logData.update({b'source': (PromoLogSourceType.SSE), b'url': (promo.data)})
    ...
    def __addAuthParams(self, url, callback):
        ...
        if not accessTokenData:
            callback(url)
            return
        params = {b'access_token': (str(accessTokenData.accessToken)), b'spa_id': (BigWorld.player().databaseID)}
    ...
    def __addSteamParams(self, url):
        if not url:
            return url
        params = {b'is_steam': (int(self.__loginManager.isWgcSteam))}
...
def _showBrowserView(url, returnClb, soundSpaceID=None, guiLoader=None):
    ...
    if guiLoader.windowsManager.findViews(_predicate):
        _logger.debug(b'BrowserView with url %s is already opened', url)
        return
    ...
    def _returnCallback(*args, **kwargs):
        if kwargs.pop(b'forceClosed', False):
            showHangar()
