...
AOGAS_NOTIFY_MSG = Enumeration(b'Notification message for Anti-online game addiction system', [
 (
  b'AOND_1', {b'timeout': _DEFAULT_AOGAS_NOTIFY_TIMEOUT}),
 (
  b'AOND_2', {b'timeout': _DEFAULT_AOGAS_NOTIFY_TIMEOUT}),
 (
  b'AOND_3', {b'timeout': _DEFAULT_AOGAS_NOTIFY_TIMEOUT}),
 (
  b'AOND_MORE_3', {b'timeout': _DEFAULT_AOGAS_NOTIFY_TIMEOUT}),
 (
  b'AOND_MORE_5', {b'timeout': _DEFAULT_AOGAS_NOTIFY_TIMEOUT})], instance=AttributeEnumItem)
...
class AOGASController(IAOGASController):
    ...
    def __init__(self):
        ...
        self.__notificator = _AOGASNotificator(self, b'_AOGASController__notifyAccount')
        return
    ...
    def onLobbyInited(self, event):
        LOG_DEBUG(b'enableNotifyAccount ', self.__lastNotifyMessages)
    ...
    def onAvatarBecomePlayer(self):
        LOG_DEBUG(b'disableNotifyAccount')
        self.__isNotifyAccount = False
        return
    ...
    def __requestRequiredInfo(self):
        BigWorld.player().stats.get(b'attrs', self.__receiveAccountAttrs)
        return

    def __receiveAccountAttrs(self, resultID, attrs):
        if resultID < 0:
            LOG_ERROR(b'Server return error: ', resultID, attrs)
            return
        if self.__isAogasEnabled and ACCOUNT_ATTR.AOGAS & attrs != 0 or AOGAS_FORCE_START_NOTIFY:
            BigWorld.player().stats.get(b'accOnline', self.__receiveAccOnline)
    ...
    def __receiveAccOnline(self, resultID, accOnline):
        if resultID < 0:
            LOG_ERROR(b'Server return error: ', resultID, accOnline)
            return
        delta = round(time.time() - self.__aogasStartedAt)
        AOND = delta + accOnline
        LOG_DEBUG(b'Calculate AOND (seconds,seconds,seconds) : ', AOND, delta, accOnline)
...
class _AOGASNotificator(object):
    ...
    def start(self, AOND):
        ...
        if notificated:
            ...
            LOG_DEBUG(b'AOGAS started (seconds,seconds)', self.__AOND, notifyPeriod)
            ...
            LOG_DEBUG(b'AOGAS started (seconds,seconds,seconds)', self.__AOND, notifyTime, nextNotifyDelay)
    ...
    def __doNotify(self, messages):
        ...
        if notifyHandler is not None and callable(notifyHandler):
            ...
            for message in messages:
                ...
                LOG_DEBUG(b'notify (seconds, message)', self.__AOND, message)

        else:
            LOG_ERROR(b'Not found notify handler ', self.__scope(), self.__function)
        return
