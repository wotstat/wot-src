...
class MapsTrainingController(IMapsTrainingController, IGlobalListener):
    ...
    ifEnabled = condition(b'isMapsTrainingEnabled')
    ifMapsTrainingPrbActive = condition(b'isMapsTrainingPrbActive')
    _UNDEFINED_VALUE = -1
    ...
    def setSelectedVehicle(self, vehicleName):
        if vehicleName:
            ...
            self.__vehCompDescr = vehicles.makeIntCompactDescrByID(b'vehicle', nationID, vehID)
        else:
            self.__vehCompDescr = self._UNDEFINED_VALUE
    ...
    def getPageCtx(self):
        return {b'map': (ArenaType.g_geometryCache[self.__mapGeometryID].geometryName if self.__mapGeometryID != self._UNDEFINED_VALUE else b''), 
           b'vehicleType': (getVehicleClass(self.__vehCompDescr) if self.__vehCompDescr != self._UNDEFINED_VALUE else b''), 
           b'side': (self.__team)}
    ...
    def __parseConfiguration(self, configuration):
        ...
        self.__config.update({b'maps': configMaps, 
           b'vehicles': configVehicles, 
           b'scenarios': confScenarios, 
           b'rewards': configRewards})
        return
    ...
    def __onServerSettingChanged(self, diff):
        ...
        if isPlayerAccount():
            ...
            if not self.__lastRequestTime or cooldownDelta > REQUEST_COOLDOWN.MAPS_TRAINING_INITIAL_CONFIGURATION:
                ...
                self.__configIsOld = False
        if not diff.get(b'isMapsTrainingEnabled', True):
