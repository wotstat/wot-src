...
class _QuestCache(object):
    __slots__ = (b'__invVehicles', b'__cache', b'__eventsCache')
    ...
    def __filterFunc(cls, event):
        ...
        if isPremium(event.getGroupID()) and not cls.lobbyContext.getServerSettings().getPremQuestsConfig().get(b'enabled', False):
            return False
...
class QuestsController(IQuestsController):
    __slots__ = (b'__quests', b'eventsCache')
    ...
    def onLobbyStarted(self, ctx):
        ...
        g_clientUpdateManager.addCallbacks({b'inventory.1': (self.__invalidateEventsData), 
           b'stats.unlocks': (self.__onStatsUnlocked)})
