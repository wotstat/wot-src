...
if TYPE_CHECKING:
    ...
    LegacyEntityType = TypeVar(b'LegacyEntityType', bound=LegacyEntity)
...
class BoostersController(IBoostersController, IGlobalListener):
    ...
    def onLobbyInited(self, event):
        ...
        g_clientUpdateManager.addCallbacks({b'cache.activeOrders': (self._update), 
           b'goodies': (self._update)})
    ...
    def __processFirstExpirationsNotification(self, accountSettings):
        ...
        if totalBoostersCount > 0:
            self.systemMessages.proto.serviceChannel.pushClientMessage({b'values': (str(totalBoostersCount))}, SCH_CLIENT_MSG_TYPE.PERSONAL_RESERVES_FIRST_LOGIN)
    ...
    def __processSoonExpirationsNotification(self):
        ...
        if totalBoostersCount > 0:
            self.systemMessages.proto.serviceChannel.pushClientMessage({b'values': (str(totalBoostersCount))}, SCH_CLIENT_MSG_TYPE.PERSONAL_RESERVES_SOON_EXPIRATION)
