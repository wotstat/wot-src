...
class NotificationTypeTemplate(Enum):
    PASSIVE_XP = (b'PassiveXpEnabledMessage', b'PassiveXpDisabledMessage')
    GOLD_RESERVES = (b'GoldReserveEnabledMessage', b'GoldReserveDisabledMessage')
    DAILY_ATTENDACES = (b'DailyAttendancesEnabledMessage', b'DailyAttendancesDisabledMessage')
    FREE_DEMOUNT = (b'WotPlusFreeDemountUnlockMessage', b'WotPlusFreeDemountExpireMessage')
    EXCLUDED_MAPS = (b'BonusExcludedMapAvailable', b'BonusExcludedMapUnavailable')
    BATTLE_BONUSES = (b'BattleBonusesEnabledMessage', b'BattleBonusesDisabledMessage')
    BADGE = (b'BadgeEnabledMessage', b'BadgeDisabledMessage')
    ADDITIONAL_XP = (b'AdditionalXpEnabledMessage', b'AdditionalXpDisabledMessage')
    OPTIONAL_DEVICES_ASSISTANT = (b'OptionalDevicesAssistantEnabledMessage', b'OptionalDevicesAssistantDisabledMessage')
    CREW_ASSISTANT = (b'CrewAssistantEnabledMessage', b'CrewAssistantDisabledMessage')
    SERVICE_RECORD = (b'ServiceRecordCustomizationEnabledMessage', b'ServiceRecordCustomizationDisabledMessage')
    PRO_BOOST = (b'WotPlusProBoostEnabledMessage', b'WotPlusProBoostDisabledMessage')
    BATTLE_PASS = (b'WotPlusBattlePassEnabledMessage', b'WotPlusBattlePassDisabledMessage')
...
class _ProBoostMixin(object):
    ifAccount = condition(b'_account')
...
class WotPlusController(IWotPlusController, _ProBoostMixin, CallbackDelayer):
    ...
    def selectIdleCrewXPVehicle(self, vehicleInvID, successCallback=None, errorCallback=None):
        ...
        def _onIdleCrewXPVehicleSelected(_, requestID, errorStr, errorMsg=None):
            if AccountCommands.isCodeValid(requestID):
                _logger.debug(b'[WotPlusController] _onIdleCrewXPVehicleSelected SUCCESS')
                if successCallback:
                    successCallback()
                return
    ...
    def activateProBoostOnVehicle(self, vehicleInvID=-1, successCallback=None, errorCallback=None):
        ...
        def _onProBoostVehicleSelected(_, requestID, errorStr, errorMsg=None):
            if AccountCommands.isCodeValid(requestID):
                _logger.debug(b'[WotPlusController] _onProBoostVehicleSelected SUCCESS')
                if successCallback:
                    successCallback()
                return
            _logger.warning((errorStr, errorMsg))
    ...
    def getGoldReserve(self):
        return self._cache.get(b'piggyBank', {}).get(b'gold')
    ...
    def toggleWotPlusDev(self, tier=WotPlusTier.CORE):
        ...
        if tier not in availableTiers and not self.hasSubscription():
            LOG_ERROR_DEV(b'The selected tier is not supported. The supported tiers are ', availableTiers)
            return
    ...
    def _resolveSubscriptionAndSteamState(self, clearCache=False):
        if self._invalidationInProgress:
            _logger.debug(b'Wot plus is waiting for another subscription invalidation, skipping')
            return
        else:
    ...
    def _onCmdResponseReceived(self, resultID, requestID, errorStr, errorMsg=None):
        if not AccountCommands.isCodeValid(requestID):
            _logger.error(b'Received invalid response: resultId: %s, requestId: %s, error: %s, message: %s', resultID, requestID, errorStr, errorMsg)
        return
    ...
    def _onRenewableSubscriptionStatusChanged(self):
        ...
        _logger.debug(b'Renewable subscription state updated, cache is synchronized = %s', self._cache)
        currentState = self.hasSubscription()
        stateChanged = previousState != currentState
        _logger.debug(b'Renewable subscription state is changed, prev = %s, current = %s', previousState, currentState)
        self._invalidateSubscriptionState(stateChanged)
        return

    def _scheduleInvalidation(self):
        _logger.debug(b'Scheduling subscription invalidation for %s seconds', self._SUBSCRIPTION_INVALIDATE_TIMEOUT)
        self.delayCallback(self._SUBSCRIPTION_INVALIDATE_TIMEOUT, self._invalidateCallback)
        return

    def _cancelScheduledInvalidation(self):
        _logger.debug(b'Canceling scheduled subscription invalidation')
        self.stopCallback(self._invalidateCallback)
        return

    @wg_async
    def _invalidateSubscriptionState(self, stateChanged=False):
        _logger.debug(b'Invalidating subscription')
