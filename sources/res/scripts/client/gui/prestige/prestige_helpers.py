...
PrestigeVehiclesDossiersCut = namedtuple(b'PrestigeVehiclesDossiersCut', [
 b'currentLevel',
 b'remainingPoints'])
...
def getCurrentProgress(vehCD, currentLevel, remainingPts, lobbyContext=None):
    ...
    if currentLevel >= pointsLength:
        return (1, 1)
    if currentLevel < 1:
        _logger.error(b'Points for vehicle (intCD = %s) not have this level = %s. Points = %s', vehCD, currentLevel, points)
        return (-1, -1)
...
def prestigePointsToXP(points, lobbyContext=None):
    ...
    if prestigeCoefficient == 0:
        _logger.warning(b'Prestige coefficient is 0. Check configs.')
        return points
...
def showPrestigeVehicleStats(vehIntCD, lobbyContext=None):
    ...
    showVehicleStats(vehIntCD, selectedAlias=VIEW_ALIAS.PROFILE_TECHNIQUE_PAGE, initVehicleSorting={b'selectedColumnSorting': b'descending', 
       b'selectedColumnStr': b'prestigeLevel'})
    return
