...
class TeaserViewer(object):
    ...
    def __onImageLoaded(self, requestedTeaserID, image):
        ...
        self.__stopTimer()
        isShopPromo = self.__teaserData.get(b'promoType') == self._SALE_PROMO_TYPE
        self.__viewProxy.as_showTeaserS({b'postTitle': (i18n.makeString(MENU.PROMO_TEASER_TITLE)), 
           b'postCounter': (self.__promoCount), 
           b'descr': (self.__teaserData[b'description']), 
           b'title': (self.__teaserData.get(b'version', b'')), 
           b'isVideo': (bool(self.__teaserData.get(b'video'))), 
           b'isShopPromo': isShopPromo, 
           b'image': (self.__imagePath)})
    ...
    def __timerTick(self):
        timeLeft = int(self.__teaserData.get(b'finishTime') - time_utils.getServerUTCTime())
        if timeLeft > 0:
