...
class ContactsUploader(object):
    __slots__ = (b'__arenaUniqueID', b'__callbackID', b'__idToActions', b'__idToNames')
    ...
    def start(self, arenaUniqueID, contactsBlob):
        _logger.info(b'contacts uploader starts upload arenaID %s', arenaUniqueID)
    ...
    def stop(self):
        _logger.info(b'Contacts Uploader: finish uploading previous arena. Stopped.')
    ...
    def _onShadowActionFailed(self, dbID, actionID, error):
        _logger.debug(b'Contacts Uploader: on shadow action failed dbID:%s, action:%s, error:%s', dbID, actionID, error)
        self._onShadowActionDone(dbID, actionID)
        return

    def _onShadowActionDone(self, dbID, actionID):
        _logger.debug(b'Contacts Uploader: on shadow action done dbID:%s, action:%s', dbID, actionID)
        if dbID in self.__idToActions:
            ...
            if actionsList[0] == actionID:
                ...
                return
        _logger.warning(b'Contacts Uploader: got feedback on action which is not setted up by me')
        return

    def _flushArenaRelations(self):
        self.__callbackID = None
        _logger.debug(b'Contacts Uploader: try to flush arena relations for arenaID:%s', self.__arenaUniqueID)
    ...
    def __onFlushResponse(self, _, resultID, __):
        if resultID != AccountCommands.RES_SUCCESS:
            _logger.debug(b'Contacts Uploader: server error, delaying flush for arenaID:%s', self.__arenaUniqueID)
    ...
    def __tryUploadContact(self, dbID=None, leftActions=None):
        if not self.isProcessing:
            _logger.warning(b'Contacts Uploader: abort uploading because of stop processing')
            return
        else:
            ...
            _logger.debug(b'Contacts Uploader: starting uploading action dbID:%s, name:%s, action:%s', dbID, name, action)
