...
class PureDialogBuilder(object):
    ...
    __slots__ = (b'_windowClass', b'__title', b'__formattedTitle', b'__titleArgs', b'__icon', b'__backImg', b'__preset', b'__flags', b'__buttons', b'__showBalance', b'__checkboxLabel', b'__checkboxCheckedByDefault')

    def __init__(self):
        super(PureDialogBuilder, self).__init__()
        self._windowClass = PureDialogWindow
        self.__title = R.invalid()
        self.__formattedTitle = b''
    ...
    def build(self, parent=None):
        if not self.__buttons:
            _logger.error(b"Dialog buttons can't be empty")
            return
        else:
    ...
    def addButton(self, name, label, isFocused, soundDown=None, rawLabel=b''):
...
class SimpleDialogBuilder(PureDialogBuilder):
    __slots__ = (b'_windowClass', b'__message', b'__formattedMessage', b'__messageArgs')

    def __init__(self):
        super(SimpleDialogBuilder, self).__init__()
        self._windowClass = SimpleDialogWindow
        self.__message = R.invalid()
        self.__formattedMessage = b''
        self.__messageArgs = _makeMessageArgs()
        return

    def build(self, parent=None):
        if self.__message == R.invalid() and self.__formattedMessage == b'':
            _logger.error(b"Dialog message can't be empty")
            return
        else:
...
class ResSimpleDialogBuilder(SimpleDialogBuilder):
    ...
    def setMessagesAndButtons(self, message, buttons=R.strings.dialogs.common, focused=DialogButtons.SUBMIT, btnDownSounds=None):
        self.setMessage(message.dyn(b'message')())
        self.setTitle(message.dyn(b'title')())
        self.setCheckboxLabel(message.dyn(b'checkboxLabel'))
