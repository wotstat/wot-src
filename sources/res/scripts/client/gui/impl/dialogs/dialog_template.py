...
class DialogTemplateView(FullScreenDialogBaseView):
    __slots__ = (b'_subViews', b'__buttonPresenters', b'__uniqueID', b'__focusingSystem')
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.dialogs.common.DialogTemplateGenericTooltip():
            buttonID = event.getArgument(b'buttonID')
    ...
    def _buttonClickHandler(self, args):
        self._setResult(args.get(b'buttonID'))
        return

    def addButton(self, buttonPresenter):
        buttonID = buttonPresenter.buttonID
        msg = b'there is already registered button with the given buttonID: ' + buttonID
    ...
    def setSubView(self, placeHolderName, view):
        msg = b'You must specify all sub-views before template has been loaded'
