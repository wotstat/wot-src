...
def vehicleToIconSetDataFormatter(vehicle, backgrounds, overlays):
    artefacts = R.images.gui.maps.shop.vehicles.num(b'180x135')
    return IconSetData(artefacts.dyn(getNationLessName(replaceHyphenToUnderscore(vehicle.name)))(), (b'{}  %(typeIcon)  {}').format(int2roman(vehicle.level), vehicle.shortUserName), [
     ImageSubstitution(R.images.gui.maps.icons.vehicleTypes.dyn(getIconResourceName(vehicle.type))(), b'typeIcon', 3, -5, -5, -5)])


def optionalDeviceToIconSetDataFormatter(optDevice, backgrounds, overlays):
    artefacts = R.images.gui.maps.shop.artefacts.num(b'180x135')
    backgrounds.append(R.images.gui.maps.icons.demountKit.dyn(optDevice.getHighlightType() + b'_highlight')())
...
class ItemIcons(MultipleIconsSet):
    ...
    def __init__(self, items, isItemExchange=False):
        ...
        for itemCD in items:
            ...
            if not item:
                _logger.warning(b'wrong itemCD %s', itemCD)
                continue
