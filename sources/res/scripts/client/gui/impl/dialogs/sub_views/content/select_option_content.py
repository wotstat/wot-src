...
class SelectOptionBasePresenter(object):
    __slots__ = (b'__viewModel', b'__isDisabled', b'tooltipFactory')
...
class SelectOptionContent(ViewImpl):
    __slots__ = (b'__presenterList', b'onSelectionChanged', b'__selectedIndex')
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.dialogs.common.DialogTemplateGenericTooltip():
            index = int(event.getArgument(b'index', -1))
...
class MoneyOption(SelectOptionBasePresenter):
    ...
    def _updateIcon(self):
        ...
        if currency == Currency.GOLD:
            ...
            raise SoftException(b'Given currency (%s) is not supported. Please provide an icon' % currency)
        return
...
class DemountKitOption(SelectOptionBasePresenter):
    ...
    def initialize(self):
        ...
        g_clientUpdateManager.addCallbacks({b'goodies': (self._goodiesChangeHandler)})
...
class MapOption(SelectOptionBasePresenter):
    ...
    def _updateIcon(self, mapId):
        ...
        if iconRes.isValid():
            ...
            raise SoftException((b'Icon is not found for map {}').format(mapId))
        return
    ...
    def _updateText(self, mapId):
        ...
        if nameRes.isValid():
            ...
            raise SoftException((b'Name is not found for map {}').format(mapId))
        return
