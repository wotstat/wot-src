...
class ViewImpl(View, EventsHandler, patched_typing.Generic[TViewModel]):
    ...
    def createPopOver(self, event):
        ...
        if content is not None:
            if not isinstance(content, PopOverViewImpl):
                raise SoftException(b'PopOver content should be derived from PopOverViewImpl.')
        ...
        if event.contentID == R.aliases.common.contextMenu.Backport():
            ...
            if content is not None:
                if not isinstance(content, ContextMenuContent):
                    raise SoftException(b'Context menu content should be derived from ContextMenuContent.')
    ...
    def __createSpecialTooltipContentWindow(self, event, parentWindow):
        ...
        tooltipArgs = event.getArgument(b'tooltipArgs', None)
        if not tooltipId:
            _logger.error(b'SpecialTooltipContent. tooltipId is not specified')
            return
        else:
            ...
            if content is not None:
                ...
                return window
            raise SoftException(b'Preparing of BackportTooltip with json args failed: backportTooltipContent is None or invalid.')
            return

    def __createAdvancedTooltipContentWindow(self, event, parentWindow):
        normalContent = int(event.getArgument(b'normalContent'))
        advancedContent = int(event.getArgument(b'advancedContent'))
    ...
    def __createBackportPopOverContent(self, event):
        ...
        popoverId = event.getArgument(b'popoverId')
        popoverArgs = event.getArgument(b'popoverArgs', None)
        if not popoverId:
            _logger.error(b'PopOver. popoverId is not specified')
            return
        else:
            if popoverArgs:
