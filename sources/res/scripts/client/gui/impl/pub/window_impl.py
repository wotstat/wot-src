...
class WindowImpl(Window):
    ...
    def __init__(self, wndFlags, *args, **kwargs):
        ...
        settings.layer = kwargs.pop(b'layer', WindowLayer.UNDEFINED)
        settings.areaID = kwargs.pop(b'areaID', R.areas.default())
        settings.decorator = kwargs.pop(b'decorator', None)
        settings.content = kwargs.pop(b'content', None)
        settings.parent = kwargs.pop(b'parent', None)
        settings.name = kwargs.pop(b'name', b'')
        settings.ownerViewID = kwargs.pop(b'ownerViewID', None)
    ...
    def move(self, x, y, xAnchor=PositionAnchor.LEFT, yAnchor=PositionAnchor.TOP):
        ...
        if area is not None:
            ...
            _logger.error(b'Window %r can not moved due to it is not added to area.', self)
            return False
    ...
    def center(self):
        ...
        if area is not None:
            ...
            _logger.error(b'Window %r can not be centered due to it is not added to area.', self)
            return False
    ...
    def cascade(self):
        ...
        if area is not None:
            ...
            _logger.error(b'Window %r can not be cascaded due to it is not added to area.', self)
            return False
