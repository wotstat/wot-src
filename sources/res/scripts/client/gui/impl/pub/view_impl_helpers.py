...
def createBackportContextMenuWindow(event, parentWindow):
    menuId = event.getArgument(b'menuId')
    menuArgs = event.getArgument(b'menuArgs', None)
    if not menuId:
        _logger.error(b'Context menu - menuId is not specified')
        return
    else:
        ...
        if contextMenuData is not None:
            ...
            window.load()
            return window
        raise SoftException(b'Preparing of BackportContextMenu with json args failed: contextMenuData is None or invalid.')
        return
...
def createWulfTooltipWindow(event, parentWindow):
    ...
    if not tooltipId:
        _logger.error(b'WulfTooltipContent. TooltipId is not specified.')
        return
    else:
...
def createParamTooltipWindow(event, parentWindow):
    ...
    tooltipType = event.getArgument(b'type')
    params = event.getArgument(b'params', None)
    resId = event.getArgument(b'resId', None)
    if not tooltipType:
        _logger.error(b'Can not create ParamTooltipWindow: tooltip type is not specified')
        return
    else:
        if not resId:
            _logger.error(b'Can not create ParamTooltipWindow: resId is not specified')
            return
