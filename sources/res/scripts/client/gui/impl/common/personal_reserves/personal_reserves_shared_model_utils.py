...
class BoosterModelData(namedtuple(b'BoosterModelArgs', [16, 17, 18, 19, 
 20])):

    def __new__(cls, resourceType, category, booster=None, depotCount=0, forcePremium=False):
        return cls._make((resourceType, category, booster, depotCount, forcePremium))
...
def getPersonalBoosterModelDataByResourceType(cache):
    ...
    sortedBoostersList = sorted(enabledBoosters.values(), key=(lambda booster_: (
     ...
     booster_.expiryTime if booster_.expiryTime else NO_FUTURE_GIVEN)))
    boostersByResource = {}
