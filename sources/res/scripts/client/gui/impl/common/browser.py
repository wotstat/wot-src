...
TViewModel = typing.TypeVar(b'TViewModel', bound=BrowserModel)

class Browser(ViewImpl[TViewModel]):
    __slots__ = (b'__url', b'__browserId', b'__browser', b'__webCommandHandler', b'__webHandlersMap', b'onBrowserObtained', b'__eventManager', b'__backUrl')
    __browserCtrl = dependency.descriptor(IBrowserController)

    def __init__(self, url=b'', settings=None, webHandlersMap=None, preload=False, *args, **kwargs):
    ...
    def __onCreateVebView(self):
        if self.__browserId:
            _logger.error(b'Browser creation was already requested. id: %i', self.__browserId)
            return
        self.__loadBrowser()
        return
    ...
    def __onFocus(self):
        if not self.__browser:
            _logger.error(b'Browser not created')
            return
