...
class BootcampNationView(TutorialWulfWindowView):
    __slots__ = (b'__nationsOrder', b'__promoNationsOrder', b'__isPromote', b'__vehicleSecondName', b'__loadingTanksNum')
    ...
    SECOND_SLOT = 1
    THIRD_SLOT = 2
    BUTTON_HINT_EFFECT = b'BootcampNationSelectButton'
    NATIONS_HINT_EFFECT = b'BootcampNationMainNations'
    ...
    def _onLoading(self, *args, **kwargs):
        ...
        self.__nationsOrder, self.__promoNationsOrder = g_bootcamp.getNationsOrder(b'unlocked', b'locked')
    ...
    def _fillNationsList(self, nationsListModel, nationsOrder, isPromote):
        for nationId in nationsOrder:
            ...
            nationModel.setIcon(R.images.gui.maps.icons.bootcamp.rewards.dyn((b'nationsSelect_{}').format(nationId))())
            if isPromote:
                ...
                vehicle = self.itemsCache.items.getVehicles()[nationData[b'vehicle_first']]
    ...
    def __onNationShow(self, args):
        nationId = args.get(b'id')
        isPromote = args.get(b'isPromote')
        with self.viewModel.transaction() as model:
    ...
    def __updateSelectedNation(self, model, nationId, isPromote):
        ...
        if self.__isPromote:
            ...
            platoonTanks = dict()
            for index, vehicleId in enumerate(nationData[b'preview_vehicles']):
                sectionName = (b'preview_vehicle_{}').format(index + 1)
                ...
                if index == self.SECOND_SLOT:
                    ...
                    platoonTanks[index] = PlatoonTankInfo(True, vehicleDescr.makeCompactDescr(), b'', None, 0, 0, vehicleDescr.type.userString, STAT_TRACK_FRAGS_NOT_SUPPORTED)
            ...
            vehicleDescr = vehicles.VehicleDescr(typeName=nationData[b'vehicle_first_descr'])
    ...
    def __updateSecondVehicleMarker(self, isVisible):
        ...
        if entity is None or entity.model is None:
            return
        g_eventBus.handleEvent(events.HangarVehicleEvent(events.HangarVehicleEvent.BOOTCAMP_SECOND_TANK_MARKER, ctx={b'entity': entity, 
           b'playerName': (self.__vehicleSecondName), 
           b'isVisible': isVisible}), scope=EVENT_BUS_SCOPE.LOBBY)
        return
