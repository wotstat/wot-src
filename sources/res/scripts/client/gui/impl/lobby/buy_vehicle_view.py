...
_TANKMAN_KEYS = (b'', b'creditsTankman', b'goldTankman')
_ACADEMY_SLOT = 2
...
class BuyVehicleView(ViewImpl, EventSystemEntity, IPrbListener):
    ...
    def __init__(self, **kwargs):
        ...
        ctx = kwargs.get(b'ctx')
        if ctx is not None:
            self.__nationID = ctx.get(b'nationID')
            self.__inNationID = ctx.get(b'itemID')
            self.__previousAlias = ctx.get(b'previousAlias')
            self.__actionType = ctx.get(b'actionType')
            self.__showOnlyCongrats = ctx.get(b'showOnlyCongrats')
            self.__congratsViewSettings = ctx.get(b'congratulationsViewSettings')
    ...
    def _initialize(self, *args, **kwargs):
        ...
        vehType = self.__vehicle.type.replace(b'-', b'_')
        ...
        with self.viewModel.transaction() as vm:
            ...
            vm.setTankType((b'{}_elite').format(vehType) if isElite else vehType)
    ...
    def __onCheckboxWithoutCrewChanged(self, args):
        self.__isWithoutCommander = args[b'selected']
        with self.viewModel.transaction() as vm:
    ...
    def __onCommanderLvlChange(self, args):
        ...
        self.__selectedCardIdx = int(args[b'value'])
    ...
    def __playSlotAnimation(self):
        if self.viewStatus != ViewStatus.LOADED:
            _logger.warning(b'Can not show slot animation! The view is not loaded anymore.')
            return
    ...
    def __showCongratulationsView(self):
        if self.viewStatus != ViewStatus.LOADED:
            _logger.warning(b'Can not show congratulations! The view is not loaded anymore.')
            return
        else:
            self.viewModel.setIsContentHidden(True)
            with self.viewModel.congratulationAnim.transaction() as vm:
                vehicleType = (b'{}_elite').format(self.__vehicle.type) if self.__vehicle.isElite else self.__vehicle.type
                image = self.__vehicle.getShopIcon(size=STORE_CONSTANTS.ICON_SIZE_LARGE)
                defaultImage = getShopVehicleIconPath(STORE_CONSTANTS.ICON_SIZE_LARGE, b'empty_tank')
                settings = self.__congratsViewSettings
                if settings and b'bgSource' in settings:
                    self.viewModel.setBgSource(settings[b'bgSource'])
                if settings and b'backBtnEnabled' in settings:
                    vm.setNeedBackBtn(settings[b'backBtnEnabled'])
                if settings and b'backBtnLabel' in settings:
                    vm.setBackBtnLbl(settings[b'backBtnLabel'])
    ...
    def __updateSlotPrice(self):
        ...
        with self.viewModel.equipmentBlock.slot.transaction() as slotVm:
            ...
            if isInit:
                self.__addVMsInActionPriceList(listArray, slotItemPrice, not self.__isGoldAutoPurchaseEnabled, tooltipData=_TooltipExtraData(b'slotsPrices', ACTION_TOOLTIPS_TYPE.ECONOMICS))
            else:
        ...
        with self.viewModel.equipmentBlock.transaction() as equipmentBlockVm:
            ...
            if self.__selectedRentID >= 0:
                ...
                if rentType == constants.RentType.TIME_RENT:
                    ...
                    selectedRentSeason = rentPackage[b'seasonType']
            else:
    ...
    def __getBackportTooltipData(self, event):
        ...
        if not tooltipId:
            ...
            if tooltipId == TOOLTIPS_CONSTANTS.TRADE_IN_STATE_NOT_AVAILABLE:
                ...
                args = (
                 event.getArgument(b'tooltipType'),
                 event.getArgument(b'key'),
                 (
                  event.getArgument(b'newCredits'), event.getArgument(b'newGold'), event.getArgument(b'newCrystal')),
                 (
                  event.getArgument(b'oldCredits'), event.getArgument(b'oldGold'), event.getArgument(b'oldCrystal')),
                 event.getArgument(b'isBuying'))
            else:
                args = None
