...
class BaseRewardWindowContent(ViewImpl):
    __slots__ = (b'__items', b'_eventName')
    _BONUSES_ORDER = (
     ...
     b'customizations', b'tokens', b'goodies', Currency.EVENT_COIN, Currency.BPCOIN)
    ...
    def _initTooltip(self, bonus, index):
        self.__items[index] = createTooltipData(tooltip=bonus.get(b'tooltip', None), isSpecial=bonus.get(b'isSpecial', False), specialAlias=bonus.get(b'specialAlias', b''), specialArgs=bonus.get(b'specialArgs', None))
        return
    ...
    def _setBonusModel(self, rewardTx, bonus, index):
        rewardTx.setIcon(bonus.get(b'imgSource', b''))
        rewardTx.setLabelStr(bonus.get(b'label', b'') or b'')
        rewardTx.setTooltipId(index)
        rewardTx.setHighlightType(bonus.get(b'highlightIcon', b'') or b'')
        rewardTx.setOverlayType(bonus.get(b'overlayIcon', b'') or b'')
        rewardTx.setHasCompensation(bonus.get(b'hasCompensation', False) or False)
        rewardTx.setLabelAlign(bonus.get(b'align', b'center') or b'center')
        return


class PiggyBankRewardWindowContent(BaseRewardWindowContent):
    __slots__ = (b'_credits', b'_isPremActive')

    def __init__(self, settings, ctx=None):
        super(PiggyBankRewardWindowContent, self).__init__(settings, ctx)
        if ctx is not None:
            self._isPremActive = ctx.get(b'isPremActive', False)
            self._credits = ctx.get(b'credits', 0)
        else:
    ...
    def _getBonuses(self):
        return [CreditsBonus(b'credits', self._credits)]
...
class QuestRewardWindowContent(BaseRewardWindowContent):
    __slots__ = (b'_quest', b'_vehicles')
...
class DynamicRewardWindowContent(BaseRewardWindowContent):
    __slots__ = (b'__bonuses', b'_eventName')
    _BONUSES_ORDER = (
     Currency.GOLD, b'vehicles', b'premium_plus', b'dossier', b'customizations', b'slots', b'goodies', b'blueprints',
     b'blueprintsAny', b'items', Currency.CRYSTAL, Currency.CREDITS, b'freeXP', b'tokens')

    def __init__(self, settings, ctx=None):
        super(DynamicRewardWindowContent, self).__init__(settings, ctx)
        self.__bonuses = None
        if ctx is not None:
            self.__bonuses = ctx.get(b'bonuses', None)
        return
...
class DynamicRewardWindow(RewardWindowBase):
    __slots__ = (b'_eventName',)

    def __init__(self, ctx=None, parent=None):
        self._eventName = ctx[b'eventName']
...
class GiveAwayRewardWindowContent(QuestRewardWindowContent):
    __slots__ = (b'__items', b'_eventName', b'_quest', b'_vehicles')
    _BONUSES_ORDER = (
     Currency.CRYSTAL, b'badgesGroup', b'dossier', b'vehicles', Currency.CREDITS, b'customizations', b'items')
