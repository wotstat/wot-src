...
class SelectableRewardBase(ViewImpl):
    __slots__ = (b'__selectedTab', b'__tabs', b'__selectableRewards', b'__cart', b'_packer')
    ...
    def _sortContent(self):
        ...
        for tabName in self.__tabs:
            self.__tabs[tabName][b'rewards'] = OrderedDict(sorted(self.__tabs[tabName][b'rewards'].iteritems(), cmp=self._getItemsComparator(tabName)))
    ...
    def _prepareRewardsData(self, tabName):
        for rewardName, reward in self.__tabs[tabName][b'rewards'].iteritems():
            ...
            elif count >= reward[b'limit'] > 0 or count >= self.__tabs[tabName][b'limit'] and count >= self.__tabs[tabName][b'limitSelectCount'] or self.__getTotalTabCount(tabName) >= self.__tabs[tabName][b'limit']:
    ...
    def __processTabs(self):
        for reward in self.__selectableRewards:
            ...
            tabContent.setdefault(b'limitSelectCount', 0)
            tabContent.setdefault(b'rewards', OrderedDict())
            tabContent[b'limit'] += self._helper.getRemainedChoices(reward)
            tabContent[b'limitSelectCount'] += self._helper.getGiftTokenCount(reward)
    ...
    def __processRewards(self):
        for selectableReward in self.__selectableRewards:
            ...
            for giftID, gift in offer.iteritems():
                ...
                rewardOp = self._createReward if currentTab[b'rewards'].get(rewardName) is None else self.__updateReward
