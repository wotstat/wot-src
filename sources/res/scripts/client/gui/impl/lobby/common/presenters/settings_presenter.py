...
if typing.TYPE_CHECKING:
    ...
    Accessor = TypedDict(b'Accessor', {b'value': Any, b'getter': Callable, b'setter': Callable})
...
class SettingsPresenter(ViewComponent[SettingsModel]):
    ...
    def __init__(self, modelCls, accountSettingsKey, readOnly=False):
        ...
        for section, settings in AccountSettings.getSettings(accountSettingsKey).items():
            ...
            if section not in submodels:
                _logger.error(b'Settings section "%s" is not defined in %s!', section, self.viewModel)
                validated = False
                break
            settingsMap[section] = {}
            for key, value in settings.items():
                submodel = submodels[section]
                capitalizedName = b'%c%s' % (key[0].upper(), key[1:])
                getter = getattr(submodel, b'get%s' % capitalizedName, None)
                setter = getattr(submodel, b'set%s' % capitalizedName, None)
                if getter is None or setter is None:
                    _logger.error(b'Setting property "%s" is not defined in %s', key, submodel)
                    validated = False
                    break
                valueT = type(value)
                prop = getter()
                ...
                if valueT is not propT:
                    _logger.error(b'Setting property "%s" type mismatch %s != %s', key, valueT, propT)
                    validated = False
                    break
                settingsMap[section][key] = {b'value': value, b'getter': getter, b'setter': setter}

        if not validated:
            _logger.error(b'Settings validation failed, check %s entry in AccountSettings and the %s model definition', self.__settingsKey, self.viewModel.__class__.__name__)
            return
        else:
    ...
    def __fillModel(self):
        for _, settings in self.__settingsMap.items():
            for _, accessor in settings.items():
                accessor[b'setter'](accessor[b'value'])
    ...
    def __onUpdateSetting(self, section, key, value):
        _logger.debug(b'Updating setting %s/%s: %s', section, key, value)
        if section not in self.__settingsMap or key not in self.__settingsMap[section]:
            _logger.error(b'Attempting to update unknown setting %s/%s', section, key)
            return
        if self.__readOnly:
            _logger.error(b'Attempting to update a read-only setting %s/%s', section, key)
            return
        self.__settingsMap[section][key][b'value'] = value
        AccountSettings.setSettings(self.__settingsKey, {section: {key: accessor[b'value'] for key, accessor in settings.items()} for section, settings in self.__settingsMap.items()})
        return
