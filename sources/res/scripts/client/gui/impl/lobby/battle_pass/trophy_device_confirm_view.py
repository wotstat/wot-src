...
class TrophyDeviceUpgradeConfirmView(DialogWindow):
    __slots__ = (b'__trophyBasicModule', b'__upgradePrice', b'__goldOperationsEnabled')
    ...
    def _getResultData(self):
        return {b'needMoreCurrency': (not self.__trophyBasicModule.mayPurchaseUpgrade(self.__itemsCache.items) and self.__trophyBasicModule.mayPurchaseUpgradeWithExchange(self.__itemsCache.items))}
...
class TrophyDeviceUpgradeConfirmDialogContent(DialogContent):
    __slots__ = (b'__trophyBasicModule', b'__trophyUpgadedModule', b'__strValueKPI')
    __itemsCache = dependency.descriptor(IItemsCache)
        ...
        with self.getViewModel().transaction() as model:
            ...
            for baseKpi, upgradedKpi in zip(self.__trophyBasicModule.getKpi(), self.__trophyUpgadedModule.getKpi()):
                if baseKpi.name != upgradedKpi.name:
                    _logger.error(b'KPI in basic and upgraded module doesnt has same order')
                    continue
