...
class BattlePassEntryPointComponent(BattlePassEntryPointMeta):
    __slots__ = (b'__view', b'__isSmall')
...
class BaseBattlePassEntryPointView(IGlobalListener, EventsHandler):
    ...
    def isBought(self):
        ...
        if all(self.__battlePass.isBought(chapterID=chapter) for chapter in chapterIDs):
            return True
        return self.__battlePass.isResourceChaptersBought() and self.__battlePass.isCompleted() and not self.isCompleted
    ...
    def __onClientUpdated(self, diff, _):
        if self.isCompleted and diff.get(b'cache', {}).get(b'dynamicCurrencies', {}).get(CurrencyBP.BIT.value, b''):
            self._updateData()
        return
...
class BattlePassEntryPointView(ViewImpl, BaseBattlePassEntryPointView):
    __battlePass = dependency.descriptor(IBattlePassController)
    __settingsCore = dependency.descriptor(ISettingsCore)
    __slots__ = (b'__isSmall', b'__notifications', b'__isAttentionTimerStarted')
