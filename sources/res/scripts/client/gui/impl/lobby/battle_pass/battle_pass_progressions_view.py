...
class ProgressionPresenter(ViewComponent[BattlePassProgressionsViewModel]):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.__chapterID = (self.__battlePass.isHoliday() or kwargs)[b'chapterID'] if 1 else self.__battlePass.getHolidayChapterID()
        return
    ...
    def updateInitialData(self, **kwargs):
        chapterID = kwargs.get(b'chapterID')
        origin = kwargs.get(b'originStateID')
    ...
    def __setFinalRewardsWidget(self, model):
        if not self.__battlePass.getRewardTypes(self.__chapterID):
            _logger.error(b'Cannot find rewards for widget in progression for chapter=%s', self.__chapterID)
            return
        else:
    ...
    def __onBPTalerUpdated(self, *data):
        if data[0].get(b'cache', {}).get(b'dynamicCurrencies', {}).get(CurrencyBP.TALER.value, b''):
