...
class BattlePassProgressionsView(ViewImpl):
    __slots__ = (b'__tooltipItems', b'__viewActive', b'__notifier', b'__chapterID', b'__showReplaceRewardAnimations', b'__specialTooltipItems', b'__voiceoverParams', b'__sound')
    ...
    ANIMATION_PURCHASE_LEVELS = b'animPurchaseLevels'
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.__chapterID = kwargs.get(b'chapterID') or self.__getDefaultChapterID()
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.lobby.battle_pass.tooltips.BuyStagesFooterTooltipView():
            return BuyStagesFooterTooltipView(event.getArgument(b'isActive'))
    ...
    def __updateQuest(self, model=None):
        ...
        if quest is None:
            ...
            model.widget3dStyle.setMarathonRewardId(first(tokens) or b'')
            return
    ...
    def __setCharacterWidget(self, model, awardType=BattlePassConsts.REWARD_FREE):
        ...
        if characterBonus is None:
            ...
            model.chapterCharacter.setVoiceTag(voiceTag or b'')
    ...
    def __clearChapterCharacter(model):
        model.chapterCharacter.setIcon(b'')
        model.chapterCharacter.setTankman(b'')
        skillsArray = model.chapterCharacter.getSkills()
        skillsArray.clear()
        skillsArray.invalidate()
        return
    ...
    def __onMissionsTabChanged(self, event):
        viewActive = event.ctx.get(b'alias') == QUESTS_ALIASES.BATTLE_PASS_MISSIONS_VIEW_PY_ALIAS
    ...
    def __onGoToChapter(self, args):
        chapter = args.get(b'chapterNumber')
        if chapter is None:
            return
        else:
    ...
    def __makeSeasonTimeText(self):
        ...
        return backport.text(endStringRes(), seasonNum=int2roman(self.__battlePass.getSeasonNum()), endDay=endTime.tm_mday, endMonth=backport.text(R.strings.menu.dateTime.months.num(endTime.tm_mon)()), endTime=formatDate(b'%H:%M', endTimestamp))

    def __showBuyWindow(self, backCallback=None):
        showBattlePassBuyWindow({b'backCallback': backCallback, 
           b'destroyAfterCallback': True})
        self.destroy()
        return

    def __showBuyLevelsWindow(self, backCallback=None):
        showBattlePassBuyLevelWindow({b'backCallback': backCallback, 
           b'chapterID': (self.__chapterID)})
        return
