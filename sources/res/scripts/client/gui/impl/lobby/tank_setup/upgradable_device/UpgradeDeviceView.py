...
class UpgradableDeviceUpgradeConfirmView(DialogTemplateView):
    __slots__ = (b'__currentModule', b'__upgradePrice', b'__vehicle', b'__onDeconstructed')
    ...
    def __getSubmitTooltipFactory(self):
        ...
        if self.__isSubmitDisabled():
            return SimpleTooltipContent(body=body)
        else:
            return
        ...
        for i, _ in enumerate(modules):
            ...
            for j, _ in enumerate(moduleKpis):
                ...
                if any(module.getKpi()[j].name != kpiName for module in modules):
                    _logger.error(b'KPI in basic and upgraded module dont have same order')
                    continue
