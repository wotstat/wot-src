...
_PanelSlotTooltip = namedtuple(b'_PanelSlotTooltip', b'tooltip, emptyTooltip')
...
def getSlotTooltipData(event, vehicle, currentSlotID, currentSection=None, tooltipsMap=None):
    ...
    if tooltipBuilder is None:
        ...
        if event.getArgument(b'fieldType') == TankSetupFields.TANK_SETUP_CARD:
...
def getSlotSpecTooltipData(event, tooltipId):
    isClickable = event.getArgument(b'isClickable')
    isDyn = event.getArgument(b'isDyn')
    spec = event.getArgument(b'spec')
...
def getShellsPriceDiscountTooltipData(event, tooltipId):
    return createTooltipData(isSpecial=True, specialAlias=tooltipId, specialArgs=(
     event.getArgument(b'price'),
     event.getArgument(b'defPrice'),
     event.getArgument(b'currencyType')))
