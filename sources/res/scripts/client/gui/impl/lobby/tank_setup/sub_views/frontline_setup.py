...
SKILL_PARAM_TEMPLATES = {b'FixedTextParam': TEMPLATE_DEFAULT, 
   b'DirectNumericTextParam': TEMPLATE_DEFAULT, 
   b'DirectSecondsTextParam': TEMPLATE_SECONDS, 
   b'DirectMetersTextParam': TEMPLATE_METERS, 
   b'MulDirectPercentageTextParam': TEMPLATE_PERCENTS, 
   b'AddDirectPercentageTextParam': TEMPLATE_PERCENTS_BY_SECOND, 
   b'MulReciprocalPercentageTextParam': TEMPLATE_PERCENTS, 
   b'AddReciprocalPercentageTextParam': TEMPLATE_PERCENTS, 
   b'ShellStunSecondsTextParam': TEMPLATE_SECONDS, 
   b'MultiMetersTextParam': TEMPLATE_METERS, 
   b'NestedMetersTextParam': TEMPLATE_METERS, 
   b'NestedSecondsTextParam': TEMPLATE_SECONDS, 
   b'MulNestedPercentageTextParam': TEMPLATE_PERCENTS, 
   b'AddNestedPercentageTextParam': TEMPLATE_PERCENTS, 
   b'NestedShellStunSecondsTextParam': TEMPLATE_SECONDS, 
   b'MulNestedPercentageTextTupleValueParam': TEMPLATE_PERCENTS}
HIDDEN_PARAMS = [
 b'inactivationDelay',
...
class EpicBattleSetupSubView(BaseEquipmentSetupSubView):
    ...
    __slots__ = (b'__tooltipMgr', b'__uiEpicBattleLogger', b'__pendingPurchaseSkillIds', b'__pendingPurchaseItemIntCDs', b'__totalPurchasePrice', b'__selectedSlotId', b'__itemDetailsLevelMap', b'__isCurrentlyActiveSubView')
    ...
    def __onEpicUpdated(self, diff):
        if not self.__epicController.isEnabled():
            ...
            return
        if b'abilityPts' in diff:
            pointsAmount = diff[b'abilityPts']
        ...
        for _, skillLevelData in skillData.levels.iteritems():
            ...
            for tooltipIdentifier in curLvlEq.tooltipIdentifiers:
                ...
                if param:
                    ...
                    skillParam.setName(i18n.makeString(tooltipName) if i18n.isValidKey(tooltipName) else b'')
                    skillParam.setValue(str(param))
                    skillParam.setSign(SKILL_PARAM_SIGN.get(tooltipIdentifier, b''))
