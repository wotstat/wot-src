...
class ArrayOptDeviceProvider(BaseArrayProvider):
    ...
    def _fillEffects(model, item):
        ...
        effectR = first(itemR.dyn(b'effect').values(), R.invalid)
    ...
    def _fillVehicle(self, model, vehCD):
        ...
        if not vehicle:
            _logger.warning(b'There is invalid vehicle compact descriptor %s', vehCD)
            return
...
class BaseOptDeviceProvider(VehicleBaseArrayProvider):
    ...
    def _fillWotPlusStatus(self, model, item):
        ...
        if item.isModernized:
            model.setDestroyTooltipBodyPath(b'destroy_modernized')
            return
        if item.isDeluxe and not self._wotPlusController.getSettingsStorage().isFreeDeluxeEquipmentDemountingEnabled():
            model.setDestroyTooltipBodyPath(b'destroy_without_wotplus_demount')
            return
        model.setDestroyTooltipBodyPath(b'destroy')
        return
