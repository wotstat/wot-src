...
class MainTankSetupView(BaseSubModelView):
    __slots__ = (b'_subViews', b'_builder', b'__isLocked')
    ...
    def onLoading(self, selectedSection=b'', selectedSlot=None):
        ...
        if selectedSection:
            if selectedSection in self._subViews:
                ...
                _logger.error(b'There is not selectedSection=[%s] in subViews=[%s]', selectedSection, self._subViews)
        return
    ...
    def switch(self, setupName, slotID):
        if setupName not in self._subViews:
            _logger.error(b'MainTankSetupView doesnt have sub view by name: %s', setupName)
