...
class BootcampConsumableSetupSubView(ConsumableSetupSubView):
    ...
    def onLoading(self, currentSlotID, *args, **kwargs):
        ...
        if any(self._interactor.getCurrentLayout()):
            return
        itemCD = g_bootcamp.getNationData()[b'consumable']
        item = self._itemsCache.items.getItemByCD(itemCD)
        if item.isInInventory:
            self._onSelectItem({b'intCD': itemCD, b'isAutoSelect': True})
        else:
            items = self._provider.getItemsList()
            for item in items:
                if item.isInInventory:
                    self._onSelectItem({b'intCD': (item.intCD), b'isAutoSelect': True})
                    break
    ...
    def _onSelectItem(self, args):
        self.uiBootcampLogger.log(BC_LOG_ACTIONS.SELECT, item_id=args.get(b'intCD'))
