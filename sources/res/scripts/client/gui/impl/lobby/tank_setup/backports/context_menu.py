...
def getContextMenuData(event, uniqueID, selectedSetup):
    ...
    if cmType == TankSetupFields.TANK_SETUP_CARD:
        contextMenuArgs = {paramName: event.getArgument(paramName) for paramName in (b'intCD', b'slotType', b'isMounted', b'isMountedMoreThanOne', b'installedSlotId', b'itemInstalledSetupIdx', b'itemInstalledSetupSlotIdx', b'isDisabled', b'shellsCounts')}
...
def getHangarContextMenuData(event, uniqueID):
    ...
    if cmType == TankSetupFields.AMMO_PANEL_SLOT:
        contextMenuArgs = {paramName: event.getArgument(paramName) for paramName in (b'intCD', b'slotType', b'isMounted', b'isMountedMoreThanOne', b'installedSlotId', b'itemInstalledSetupIdx', b'itemInstalledSetupSlotIdx', b'shellsCounts')}
        contextMenuArgs[b'emitterUID'] = uniqueID
        if contextMenuArgs[b'intCD'] > 0:
