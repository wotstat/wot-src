...
class BaseHangarAmmunitionSetupView(BaseAmmunitionSetupView):
    ...
    __slots__ = (b'__blur', b'_isClosed', b'__closeConfirmatorHelper', b'onClose', b'onAnimationEnd', b'__moneyCache', b'_previousSectionName')
    ...
    def __init__(self, layoutID=R.views.lobby.tanksetup.HangarAmmunitionSetup(), **kwargs):
        ...
        self._previousSectionName = kwargs.get(b'selectedSection')
        self.onClose = Event()
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.dialogs.common.DialogTemplateGenericTooltip():
            ...
            if contentID == R.views.lobby.tanksetup.tooltips.SetupTabTooltipView():
                name = event.getArgument(b'name', b'')
                return SetupTabTooltipView(name)
            if contentID == R.views.lobby.tanksetup.tooltips.PopularLoadoutsTooltip():
                return PopularLoadoutsTooltip(vehCompDescr=int(event.getArgument(b'sourceVehicleCompDescr', 0)), optionalDevicesResultType=int(event.getArgument(b'optionalDevicesResultType', 0)))
            ...
            if contentID == R.views.lobby.tanksetup.tooltips.WarningTooltipView():
                reason = WarningDescription(event.getArgument(b'reason'))
                isCritical = event.getArgument(b'isCritical')
    ...
    def _getEquipmentsPairs(self, groupID):
        ...
        if groupID == TankSetupGroupsId.EQUIPMENT_AND_SHELLS:
            ...
            raise SoftException((b'Vehicle setup group id must match for any type of equipments. groupID {}').format(groupID))
        return eqPairs
    ...
    def _initialize(self, *args, **kwargs):
        ...
        self.__updateTTC(kwargs.get(b'selectedSection'))
        return
    ...
    def _finalize(self):
        ...
        if self.__blur is not None:
            self.__blur.fini()
        g_eventBus.handleEvent(CameraRelatedEvents(CameraRelatedEvents.FORCE_DISABLE_IDLE_PARALAX_MOVEMENT, ctx={b'isDisable': False, b'setIdle': True, b'setParallax': True}), EVENT_BUS_SCOPE.LOBBY)
        return
    ...
    def __updateTTC(self, sectionName=None):
        ...
        if currentSubView is not None:
            ...
            g_eventBus.handleEvent(AmmunitionSetupViewEvent(AmmunitionSetupViewEvent.UPDATE_TTC, {b'vehicleItem': vehicleAfterInstall, 
               b'sectionName': sectionName}), EVENT_BUS_SCOPE.LOBBY)
    ...
    def __onAnimationEnd(self):
        if self.__blur is not None:
            self.__blur.enable()
        if not self.viewModel.getIsReady():
            g_eventBus.handleEvent(CameraRelatedEvents(CameraRelatedEvents.FORCE_DISABLE_IDLE_PARALAX_MOVEMENT, ctx={b'isDisable': True, b'setIdle': True, b'setParallax': True}), EVENT_BUS_SCOPE.LOBBY)
    ...
    def __onDragDropSwap(self, args):
        actionArgs = {b'actionType': (BaseSetupModel.DRAG_AND_DROP_SLOT_ACTION)}
        dragId = args[b'dragId']
        dropId = args[b'dropId']
        if self._tankSetup.getSelectedSetup() == TankSetupConstants.SHELLS:
            actionArgs[b'leftID'], actionArgs[b'rightID'] = sorted((dragId, dropId))
        else:
            actionArgs[b'installedSlotId'] = dragId
            actionArgs[b'currentSlotId'] = dropId
