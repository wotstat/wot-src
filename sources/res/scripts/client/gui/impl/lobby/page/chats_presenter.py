...
def _convertDPItemToModel(item):
    ...
    message.setId(item[b'clientID'])
    message.setName(i18n.makeString(item[b'label']))
    message.setSystem(not item[b'canClose'])
    message.setSelected(item[b'isWindowOpened'])
    message.setViewed(not item[b'isNotified'])
    message.setTooltipId(item[b'tooltipData'][b'tooltipId'])
    return message
...
class ChatsPresenter(ViewComponent[ChatModel]):
    ...
    def __onWindowPopulate(self, event):
        self.__requestChange(event.ctx[b'clientID'], b'isWindowOpened', True)
        return

    def __onWindowMinimize(self, event):
        self.__requestChange(event.ctx[b'clientID'], b'isWindowOpened', False)
        return

    def __requestChange(self, clientID, key, value):
        g_eventBus.handleEvent(ChannelManagementEvent(clientID, events.ChannelManagementEvent.REQUEST_TO_CHANGE, {b'key': key, 
           b'value': value}), scope=EVENT_BUS_SCOPE.LOBBY)
        return
    ...
    def __updateModel(self, *_):
        ...
        for idx, channel in enumerate(channels):
            if channel[b'label'] == LAZY_CHANNEL.COMMON and not self.__lobbyContext.getServerSettings().isHangarGeneralChatEnabled():
                continue
            message = _convertDPItemToModel(channel)
            message.setOrder(idx)
            messages[str(channel[b'clientID'])] = message

        for idx, prbChannel in enumerate(preBattleChannels):
            message = _convertDPItemToModel(prbChannel)
            message.setOrder(idx)
            message.setPrebattle(True)
            messages[str(prbChannel[b'clientID'])] = message

        return

    def __getChannelsList(self):
        return (
         self.__carouselHandler.channelsDP.collection, self.__carouselHandler.preBattleChannelsDP.collection)
