...
class LoadoutPresenterBase(ViewComponent[TViewModel], ITankSetupCMHandler):
    ...
    def setLastSlotAction(self, *args, **kwargs):
        ...
        if actionType == BaseSetupModel.SWAP_SLOTS_ACTION:
            leftID = kwargs[b'leftID']
            rightID = kwargs[b'rightID']
    ...
    def _getCallbacks(self):
        return (
         (
          (b'stats.{}').format(Currency.CRYSTAL), self._onCurrencyUpdate),
         (
          (b'stats.{}').format(Currency.GOLD), self._onCurrencyUpdate),
         (
          (b'stats.{}').format(Currency.CREDITS), self._onCurrencyUpdate))

    def _onLoading(self, *args, **kwargs):
        self._slotActions = self.createSlotActions()
        for value in self._slotActions.values():
            if not callable(value):
                _logger.error(b'Slot action call function  %s is not callable', value)
    ...
    def _onSlotAction(self, args):
        actionType = args.pop(b'actionType')
        actionMethod = self._slotActions.get(actionType)
        if actionMethod is not None:
            actionMethod(args)
        else:
            _logger.error(b'__slotActions doesnt exist action type : %s(viewName %s)', actionType, self.__class__.__name__)
        return
    ...
    def _onSwapSlots(self, actionType, args):
        currentSlotId = int(args.get(b'currentSlotId'))
        installedSlotId = int(args.get(b'installedSlotId', -1))
    ...
    def _onDragAndDropSlots(self, actionType, args):
        leftID = int(args.get(b'leftID'))
        rightID = int(args.get(b'rightID'))
        self._swapSlots(leftID, rightID, actionType)
        self._updateModel()
        return
    ...
    def _onRevertItem(self, args):
        slotID = int(args.get(b'currentSlotId', args.get(b'installedSlotId', self._currentSlotIndex)))
    ...
    def __onSelectItem(self, args):
        itemCD = args.get(b'intCD')
        currentSlotId = int(args.get(b'currentSlotId', -1))
        if currentSlotId == -1:
            currentSlotId = self._currentSlotIndex
        isAutoSelect = bool(args.get(b'isAutoSelect', False))
