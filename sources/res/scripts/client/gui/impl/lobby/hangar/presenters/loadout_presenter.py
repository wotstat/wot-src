...
class _LoadoutStatesObserver(BaseStateObserver):
    ...
    def onStateChanged(self, state, stateEntered, event=None):
        if state.getStateID() == self._stateID:
            if not stateEntered:
                self.onPanelSlotSelect(None, b'', AmmunitionPanelModel.NO_SLOT_SELECTED)
...
class LoadoutPresenter(ViewComponent[AmmunitionPanelModel]):
    ...
    def _getBackportTooltipData(self, event):
        if event.getArgument(b'superTooltip', False):
            return None
        else:
    ...
    def __onChangeSetupIndex(self, args):
        jsonArgs = json.loads(args.get(b'args'))
        hudGroupID = jsonArgs[b'groupId']
        currentIndex = jsonArgs[b'currentIndex']
