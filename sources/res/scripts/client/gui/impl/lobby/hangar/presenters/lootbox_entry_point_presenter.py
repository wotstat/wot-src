...
class LootboxEntryPointPresenter(ViewComponent[EntryPointViewModel]):
    ...
    def __onServerSettingsChanged(self, diff):
        ...
        if isEnabled != wasEnabled or any(name in diff for name in (
         LOOTBOX_SYSTEM_CONFIG, b'lootBoxes_config', b'lootboxes_tooltip_config')):
    ...
    def __getEnabled(self):
        ...
        if lootboxesActive and not self.__lootBoxes.getActiveBoxes(self.__lootBoxes.mainEntryPoint):
            _logger.error(b'There is no boxes with %s type, check LootBox config.', self.__lootBoxes.mainEntryPoint)
            return False
