...
class VehicleMenuPresenter(ViewComponent[VehicleMenuModel], IPrbListener):
    ...
    def __onNavigate(self, args):
        entry = args.get(b'entry')
        _logger.debug(b'Navigate to %s', entry)
        if self.__isVehicleChanging:
            _logger.debug(b'Vehicle is changing, canceling the navigation')
            return
        if self.cameraController.isEnabled:
            _logger.debug(b'Navigate to %s disabled by free camera', entry)
            return
