...
GUINode = namedtuple(b'GUINode', (b'id', b'state', b'unlockProps'))
MenuItemEntry = namedtuple(b'MenuItemEntry', [b'handler', b'showCondition', b'enabledCondition'])
...
RANDOM_MENU_ITEMS = OrderedDict(list(SHARED_MENU_ITEMS.items()) + [
 ...
 TOURNAMENT_MENU_ITEM, 
 CLANS_MENU_ITEM])
registerMenuItems(b'hangar', RANDOM_MENU_ITEMS)
...
def take3DStyles(customizationSerivce):
    ...
    for itemID in customizationCache[type]:
        intCD = vehicles.makeIntCompactDescrByID(b'customizationItem', type, itemID)
