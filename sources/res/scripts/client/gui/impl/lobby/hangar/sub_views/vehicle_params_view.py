...
class _VehicleParamsPresenterBase(ViewComponent[VehicleParamsViewModel]):
    ...
    def _getLocalizedName(self, param, applyFormatting=True):
        ...
        if KPI.Name.hasValue(param.name):
            key = b'positive' if param.value >= 0 else b'negative'
            name = VEHICLE_ATTR_TO_KPI_NAME_MAP.get(param.name, param.name)
            return json.dumps({b'key': key, b'name': name})
        return
    ...
    def _createGroupViewModel(self, groupName, comparator):
        ...
        return {b'Id': groupName, 
           ...
           b'IsSituational': (hasSituationalEffect(param.name, self.comparator)), 
           b'TooltipID': (self._getTooltipID()), 
           b'BuffIconType': (getGroupIcon(param, self.comparator)), 
           b'IsOpen': (self._getIsOpened(groupName=groupName)), 
    ...
    def _createIndicator(self, param):
        ...
        return {b'Value': (param.value), 
           ...
           b'ModifiedPercent': (int(param.value * 100 / (maxValue - self._DEFAULT_MIN_VALUE))), 
           b'CurrentPercent': (int(self._stockParams[param.name] * 100 / (maxValue - self._DEFAULT_MIN_VALUE)))}
    ...
    def _createParam(self, param, groupName, highlight=HighlightType.NONE):
        if param.value is None:
            ...
            return {b'Id': (param.name), 
               ...
               b'Value': (formatParameterValue(param.name, param.value, self._applyFormatting, param.state, allowSmartRound=False, nDigits=self._getRoundNDigits())), 
               b'TooltipID': (self._getAdvancedParamTooltip()), 
               b'Name': (self._getLocalizedName(param, self._applyFormatting))}
