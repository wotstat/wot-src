...
class WotPlusEntrySubPresenter(BaseMenuEntrySubPresenter, IGlobalListener):
    ...
    def _getState(self):
        ...
        if not ProBoostUtils.isGameModeCompatibleForProBoost():
            params = {b'tooltipKey': (VehicleMenuModel.PRO_BOOST_TOOLTIP_INCOMPATIBLE_MODE)}
        ...
        if not self.__wotPlusCtrl.canBeProBoosted(g_currentVehicle.intCD):
            params = {b'tooltipKey': (VehicleMenuModel.PRO_BOOST_TOOLTIP_INCOMPATIBLE_VEHICLE)}
        ...
        if isActiveState and isLockedState:
            params = {b'tooltipKey': (VehicleMenuModel.PRO_BOOST_TOOLTIP_LOCKED_ACTIVE), b'isActive': True, 
               b'expirationTimestamp': cooldownTimestamp}
        ...
        if isActiveState:
            params = {b'tooltipKey': (VehicleMenuModel.PRO_BOOST_TOOLTIP_ACTIVE), b'isActive': True}
        ...
        if isLockedState and boostedVehicle:
            ...
            params = {b'tooltipKey': (VehicleMenuModel.PRO_BOOST_TOOLTIP_LOCKED), 
               b'expirationTimestamp': cooldownTimestamp, 
               b'vehicle': (getItemByCompactDescr(boostedVehicleCD).userString)}
    ...
    def __getType48x48IconResource(self, vehicleType, isElite=False):
        return R.images.gui.maps.icons.vehicleTypes.c_48x48.dyn(replaceHyphenToUnderscore(vehicleType + b'_elite' if isElite else vehicleType))
