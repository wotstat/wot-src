...
class TooltipBuilder(object):
    ...
    def _formNoteText(self, startDate):
        startDateString = backport.text(self._RES_ROOT.questsView.chapter.disabled.doFuture.noteDate(), color_open=b'%(brown_open)s', startDate=backport.getDateTimeFormat(startDate), color_close=b'%(brown_close)s')
    ...
    def _formNoteRangeDate(self, startDate, endDate, endPostprogressionDate):
        mainProgressString = backport.text(self._RES_ROOT.tankView.questsWidget.mainProgress.noteDate(), color_open=b'%(brown_open)s', startDate=backport.getShortDateFormat(startDate), endDate=backport.getShortDateFormat(endDate), color_close=b'%(brown_close)s')
        postProgressString = backport.text(self._RES_ROOT.tankView.questsWidget.postprogression.noteDate(), color_open=b'%(brown_open)s', endDate=backport.getShortDateFormat(endPostprogressionDate), color_close=b'%(brown_close)s')
...
class QuestsTooltipBuilder(TooltipBuilder):
    ...
    def getHeader(self):
        ...
        if self._state == QuestsViewTooltipStates.QUEST.value:
            return backport.text(self._RES_ROOT.questsView.quest.disabled.doFuture.header())
        return b''
    ...
    def __formRequiredVehiclesText(self):
        ...
        return backport.text(self._RES_ROOT.questsView.chapter.postprogression.disabled.doFuture.body(), text=backport.text(self._RES_ROOT.vehicleInfo.postprogression.vehicleType.dyn(vehType.replace(b'-', b'_'))(), vehicleInfo=vehicleInfoText))
    ...
    def __getCycleState(self):
        ...
        if eaState == State.POSTPROGRESSION or eaState == State.BUY:
            ...
            return (
             self.CycleState.DOINFUTURE, None)
...
class VehicleTooltipBuilder(TooltipBuilder):
    ...
    def getHeader(self):
        if self._state == VehicleViewTooltipStates.QUESTSWIDGET.value:
            return backport.text(self._RES_ROOT.tankView.questsWidget.header())
        return b''

    def getBody(self):
        if self._state == VehicleViewTooltipStates.QUESTSWIDGET.value:
            return backport.text(self._RES_ROOT.tankView.questsWidget.body())
        return b''
...
class TooltipBuilderProvider(object):
    ...
    def getBuilder(cls, tooltipState, id):
        if tooltipState in [state.value for state in QuestsViewTooltipStates]:
            ...
            if tooltipState in [state.value for state in VehicleViewTooltipStates]:
                return VehicleTooltipBuilder(tooltipState, id)
            return


class EarlyAccessStateTooltipView(ViewImpl):
    __slots__ = (b'__state', b'__id')

    def __init__(self, state, id=b''):
