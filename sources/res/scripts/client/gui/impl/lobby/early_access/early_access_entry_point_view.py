...
class EarlyAccessEntryPointView(ViewImpl):
    ...
    def createToolTipContent(self, event, contentID):
        ...
        if contentID == R.views.lobby.early_access.tooltips.EarlyAccessCommonDescriptionTooltip():
            return ViewImpl(ViewSettings(contentID, model=ViewModel()))
        return super(EarlyAccessEntryPointView, self).createToolTipContent(event=event, contentID=contentID)
    ...
    def _getEvents(self):
        return (
         (
          self.__earlyAccessCtrl.onQuestsUpdated, self.__updateModel),
         (
          self.__earlyAccessCtrl.onUpdated, self.__updateModel),
         (
          self.viewModel.onAction, self.__goToNextScreen))
