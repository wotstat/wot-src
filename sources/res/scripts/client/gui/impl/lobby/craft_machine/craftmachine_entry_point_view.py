...
class CraftmachineEntryPointView(ViewImpl):
    ...
    def __init__(self, flags=ViewFlags.VIEW):
        ...
        self.__additionalUrl = b''
        self.__endDateUI = b''
        self.__startDateUI = b''
    ...
    def __parseAdditionalData(self, item):
        if item is None:
            ...
            for entryData in notificationEntries:
                if str(entryData.get(b'id')) == HANGAR_ALIASES.CRAFT_MACHINE_ENTRY_POINT:
                    additionalUrl = entryData.get(b'craftMachineAdditionalUrl')
                    if additionalUrl is not None:
                        self.__additionalUrl = str(additionalUrl)
                    self.__startDateUI = entryData.get(b'startDateUI', b'')
                    self.__endDateUI = entryData.get(b'endDateUI', b'')
    ...
    def __openCraftMachine(self):
        ...
        if url is None:
            _logger.error(b'Could not find craft machine URL, check clan_config')
            return
        else:
