...
def nationTechTreeTooltipDecorator(method):
    ...
    def wrapper(self, event, *args, **kwargs):
        if event.contentID == R.views.common.tooltip_window.backport_tooltip_content.BackportTooltipContent() and event.getArgument(b'tooltipId') == VehicleTechTreeModel.TECHTREE_NATION_TOOLTIP:
            nation = event.getArgument(b'nation')
            if isEventNation(nations.INDICES[nation]):
                tooltipData = createTooltipData(isSpecial=True, specialArgs=(
                 nation,), specialAlias=TOOLTIPS_CONSTANTS.TECHTREE_NATION_EVENT)
