...
class VehicleTechTree(ViewImpl):
    __slots__ = (b'__treeData', b'__listener', b'__invalidator', b'__customHints', b'__ctx', b'__graphicOptimization', b'__paragonsEntryPoint')
    ...
    def createToolTipContent(self, event, contentID):
        ...
        if contentID == R.views.lobby.techtree.tooltips.ParagonsLockedTooltip():
            return ParagonsLockedTooltip(vehicleCD=event.getArgument(b'vehicleCD'))
        if contentID == R.views.lobby.paragons.tooltips.ResetButtonTooltip():
            return ResetButtonTooltip(branchID=event.getArgument(b'branchID'), layoutID=R.views.lobby.paragons.tooltips.ResetButtonTooltip())
        return super(VehicleTechTree, self).createToolTipContent(event, contentID)
    ...
    def getTooltipData(self, event):
        ...
        if tooltipId == VehicleTechTreeModel.TECHTREE_VEHICLE_TOOLTIP:
            nodeId = int(event.getArgument(b'nodeId'))
            node = self.__treeData.getNodeByItemCD(nodeId)
            return createTooltipData(isSpecial=True, specialArgs=(
             node, nodeId), specialAlias=TOOLTIPS_CONSTANTS.TECHTREE_VEHICLE)
        else:
            if tooltipId == BlueprintScreenTooltips.TOOLTIP_BLUEPRINT:
                return createTooltipData(isSpecial=True, specialAlias=TOOLTIPS_CONSTANTS.BLUEPRINT_INFO, specialArgs=(
                 int(event.getArgument(b'nodeId')),))
            if tooltipId == BlueprintScreenTooltips.TOOLTIP_BLUEPRINT_CONVERT_COUNT:
                return createTooltipData(isSpecial=True, specialAlias=TOOLTIPS_CONSTANTS.BLUEPRINT_CONVERT_INFO, specialArgs=(
                 int(event.getArgument(b'nodeId')),))
            if tooltipId == VehicleTechTreeModel.VEHICLE_COLLECTOR_TOOLTIP:
                return createTooltipData(isSpecial=True, specialArgs=(
                 event.getArgument(b'nation'),), specialAlias=TOOLTIPS_CONSTANTS.VEHICLE_COLLECTOR_INFO)
            if tooltipId == VehicleTechTreeModel.BLUEPRINT_FRAGMENT_INFO:
                specialArgs = BlueprintTypes.INTELLIGENCE_DATA if event.getArgument(b'isUniversal') else getNationalFragmentCD(SelectedNation.getIndex())
                return createTooltipData(isSpecial=True, specialArgs=(
                 specialArgs,), specialAlias=VehicleTechTreeModel.BLUEPRINT_FRAGMENT_INFO)
            return
    ...
    def updateEarlyAccessState(self):
        with self.viewModel.transaction() as ts:
            ...
            earlyAccessNation = nations.NAMES[self.__earlyAccessController.getNationID()] if self.__earlyAccessController.isQuestActive() else b''
    ...
    def _getListeners(self):
        return (
         (
          MESSENGER_VIEW_ALIAS.CHANNEL_MANAGEMENT_WINDOW, self.__closePremiumPanel, EVENT_BUS_SCOPE.LOBBY),
         (
    ...
    def __goToPremiumShop(self, nationName, level):
        self.__stopTopOfTheTreeSounds()
        params = {b'nation': nationName, b'level': level, b'vehicleFilterByUrl': _VEHICLE_URL_FILTER_PARAM}
...
class VehicleTechTreeInvalidator(IPage):
    __slots__ = (b'__viewRef', b'__dataProvider')
    __cmpBasket = dependency.descriptor(IVehicleComparisonBasket)
    __itemsCache = dependency.descriptor(IItemsCache)
    __techTreeEventsListener = dependency.descriptor(ITechTreeEventsListener)
    __lobbyContext = dependency.descriptor(ILobbyContext)
