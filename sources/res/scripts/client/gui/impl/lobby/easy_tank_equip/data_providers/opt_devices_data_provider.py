...
class OptDevicesDataProvider(BaseDataProvider):
    ...
    def _getPresetDataForApplying(self):
        ...
        data.update({b'optDevices': (self.__optDevicesPresets.values()[self.currentPresetIndex]), 
           b'demountByDemountKit': (demountInfo.byDemountKit), 
           b'demountForFree': (demountInfo.forFree)})
        return data
    ...
    def __setOptDevicesPresets(self):
        ...
        if not loadout:
            _logger.warning(b'No easy tank loadouts were found for vehicle %d.', self.vehicle.intCD)
            return
    ...
    def __getAdvancedOptDevices(self, optDevicesTags, standardOptDevices):
        ...
        for deviceIndex, deviceTag in enumerate(optDevicesTags):
            ...
            if not highestPriorityDevice:
                _logger.warning(b'Device with tag %s is not found for advanced loadout. Vehicle=%d, device position=%d', deviceTag, self.vehicle.intCD, deviceIndex + 1)
