...
CardInfo = NamedTuple(b'CardInfo', [
 (
  b'card', Type[BaseCard]),
 (
  b'model', ProposalModel)])
...
class EasyTankEquipPresenter(ViewComponent[EasyTankEquipViewModel]):
    ...
    def createToolTip(self, event):
        if event.contentID == R.views.common.tooltip_window.backport_tooltip_content.BackportTooltipContent():
            ...
            targetId = event.getArgument(b'targetId')
            if tooltipId == TooltipConstants.PRICE_DISCOUNT:
                specialArgs = (event.getArgument(b'price'), event.getArgument(b'defPrice'), event.getArgument(b'currencyType'))
                if all(specialArgs):
    ...
    def _getCallbacks(self):
        ...
        moneyCallbacks = tuple(((b'stats.{}').format(c), self.__onMoneyUpdated) for c in currencies)
    ...
    def __initializeCards(self):
        cardsLoadingMap = {(ProposalType.CREW): (CardInfo(CrewCard, self.viewModel.crewProposal)), (ProposalType.SHELLS): (CardInfo(ShellsCard, self.viewModel.shellsProposal)), 
    ...
    def __onSelectProposal(self, proposalType):
        ...
        if not provider:
            _logger.warning(b'The provider for the card was not found when selecting card. ProposalType: %s.', proposalType)
            return
        if provider.isProposalDisabled() or provider.isCurrentPresetDisabled():
            _logger.warning(b'The disabled card can not be selected. ProposalType: %s.', proposalType)
            return
    ...
    def __onSwitchPreset(self, proposalType, presetIndex):
        provider = self.__providers.get(proposalType)
        if not provider:
            _logger.warning(b'The provider for the card was not found when switching preset. ProposalType: %s.', proposalType)
            return
    ...
    def __onInventoryUpdate(self, diff):
        ...
        for guiItemType, proposalType in GUI_ITEM_TYPE_TO_PROPOSAL_MAP.items():
            if guiItemType in diff:
                provider = self.__providers.get(proposalType)
                if not provider:
                    _logger.warning(b'The provider for the card was not found when updating inventory. ProposalType: %s.', proposalType)
                    continue
    ...
    @decorators.adisp_process(b'easyTankEquipApply')
    ...
    def __getPresetsDataForApplying(self):
        applyingParamsMap = {(ProposalType.SHELLS): b'shellsData', 
           (ProposalType.CONSUMABLES): b'eqsData', 
           (ProposalType.OPT_DEVICES): b'optDevicesData', 
           (ProposalType.STYLES): b'styleData'}
        applyingData = {}
        for proposalType, provider in self.__providers.items():
    ...
    def __getCrewPresetDataForApplying(self):
        ...
        if not provider:
            _logger.warning(b'The crew provider for the card was not found when applying preset.')
            return
