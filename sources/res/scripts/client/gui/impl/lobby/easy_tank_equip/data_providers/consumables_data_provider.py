...
ConsumableTagsInfo = NamedTuple(b'ConsumableTagsInfo', [
 (
...
class ConsumablesDataProvider(BaseDataProvider):
    DEFAULT_CONFIG = {(ConsumablesPresetType.STANDARD): [
                                        ConsumableTagsInfo(includedTags=[b'medkit'], excludedTags=[b'premium_equipment']),
                                        ConsumableTagsInfo(includedTags=[b'repairkit'], excludedTags=[b'premium_equipment']),
                                        ConsumableTagsInfo(includedTags=[b'extinguisher'], excludedTags=[b'premium_equipment'])], 
       (ConsumablesPresetType.ADVANCED): [
                                        ConsumableTagsInfo(includedTags=[b'medkit', b'premium_equipment'], excludedTags=[]),
                                        ConsumableTagsInfo(includedTags=[b'repairkit', b'premium_equipment'], excludedTags=[]),
                                        ConsumableTagsInfo(includedTags=[b'extinguisher', b'premium_equipment'], excludedTags=[])], 
       (ConsumablesPresetType.IMPROVED): [
                                        ConsumableTagsInfo(includedTags=[b'medkit', b'premium_equipment'], excludedTags=[]),
                                        ConsumableTagsInfo(includedTags=[b'repairkit', b'premium_equipment'], excludedTags=[]),
                                        ConsumableTagsInfo(includedTags=[b'stimulator', b'premium_equipment'], excludedTags=[])]}
    ...
    def _getPresetDataForApplying(self):
        ...
        data.update({b'eqs': (self.__consumablesPresets.values()[self.currentPresetIndex])})
        return data
    ...
    def __addBuiltInConsumables(self):
        ...
        for builtinIntCD in builtInConsumablesIDs:
            ...
            if not builtin:
                _logger.warning(b'Failed to get builtin gui item. IntCD: %s.', builtinIntCD)
                continue
        ...
        for builtin in builtinConsumables:
            for items in self.__consumablesPresets.values():
                ...
                if indexToChange is None:
                    _logger.warning(b'Failed to add builtin gui item to preset. IntCD: %s.', builtin.intCD)
        ...
        for items in self.__consumablesPresets.values():
            ...
            for __ in range(excessItemsCount):
                ...
                for index, item in enumerate(reversed(items)):
                    ...
                    _logger.warning(b'Builtin consumable was removed from preset. Index: %s.', indexToRemove)
    ...
    def __replaceConsumables(self):
        ...
        for replacedItemTags, newItemTags in self.REPLACE_CONSUMABLES_MAP:
            ...
            if not replacedItem:
                _logger.warning(b'Failed to get replaced gui item. IncludedTags: %s, excludedTags: %s.', replacedItemTags.includedTags, replacedItemTags.excludedTags)
                continue
            newItem = self.__getConsumable(newItemTags)
            if not newItem:
                _logger.warning(b'Failed to get new gui item for replace. IncludedTags: %s, excludedTags: %s.', newItemTags.includedTags, newItemTags.excludedTags)
                continue
