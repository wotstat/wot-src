...
class MapBoxAwardsView(ViewImpl):
    __slots__ = (b'__numBattles', b'__tooltips', b'__reward')
    ...
    def __onPick(self, args):
        rewardModel = self.viewModel.rewards.getItem(int(args[b'index']))
        selectableCrewbook = first([item for item in self.__reward if item.getName() == b'selectableCrewbook'])
