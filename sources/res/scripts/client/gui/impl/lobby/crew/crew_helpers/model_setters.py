...
def setTmanSkillsModel(sm, tman, useOnlyFull=False, possibleSkillsLevels=None, fillBonusSkills=True, isRecruit=False, compVeh=None, customCrewName=b''):
    if tman is None:
...
def setTmanMajorSkillsModel(listVM, tman, useOnlyFull=False, possibleSkillsLevels=None, customCrewName=b'', isRecruit=False):
    listVM.clear()
...
def setTmanBonusSkillsModel(listVM, tman, bonusSlotsLevels=None, selectedRole=None, selectedSkills=None):
    ...
    for role, skills in tman.bonusSkills.iteritems():
        for skill, lvl in zip(skills, bonusSlotsLevels):
            ...
            key = b'full' if lvl == MAX_SKILL_LEVEL else b'notfull'
            if skillName:
    ...
    listVM.clear()
    skillModels = list(chain(selected[b'full'], empty[b'full'], selected[b'notfull'], empty[b'notfull']))
