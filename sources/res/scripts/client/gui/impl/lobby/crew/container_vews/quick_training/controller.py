...
_ACTION_KEY = b'action'
_ACTION_TYPE_KEY = b'actionType'
_ADD_KEY = b'add'
_PERCENT_KEY = b'percent'
CREW_BOOKS_PURCHASE_ENABLED = b'isCrewBooksPurchaseEnabled'
IS_MENTORING_LICENSE_ENABLED = b'isMentoringLicenseEnabled'
FREE_XP_STEPPER_ACTION = {b'perk': {_ADD_KEY: (lambda stepper: stepper.getSkillUpXpCost()), 
             b'subtract': (lambda stepper: stepper.getSkillDownXpCost())}, 
   _PERCENT_KEY: {_ADD_KEY: (lambda stepper: stepper.getLevelUpXpCost()), 
                  b'subtract': (lambda stepper: stepper.getLevelDownXpCost())}}
...
class QuickTrainingInteractionController(InteractionController):
    ...
    def onInventoryUpdate(self, invDiff):
        ...
        if GUI_ITEM_TYPE.VEHICLE in invDiff:
            updatedCrew = invDiff[GUI_ITEM_TYPE.VEHICLE].get(b'crew', {})
