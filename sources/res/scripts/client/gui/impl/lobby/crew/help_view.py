    ...
    def _fillModel(self):
        ...
        with self.viewModel.transaction() as vm:
            ...
            for index, slide in enumerate(slidesConfig):
                ...
                slideModel.setTitle(slide[b'title']())
                if self.__navigateFrom and self.__navigateFrom == slide[b'navigateFrom']:
                    vm.setSelectedSlideIndex(index)
                sections = slideModel.getSections()
                sections.clear()
                for section in slide[b'sections']:
                    sectionModel = HelpSlideSectionViewModel()
                    sectionModel.setSize(SLIDE_SIZES[section[b'size']])
                    sectionModel.setImage(section[b'image']())
                    sectionModel.setDescription(section[b'description']())
