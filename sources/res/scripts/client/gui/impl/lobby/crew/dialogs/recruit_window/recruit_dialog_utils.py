...
def getIconName(iconID):
    return iconID.split(b'.')[0]


def getIcon(iconName=b'', isFemale=False):
    specialID = R.images.gui.maps.icons.tankmen.icons.special.dyn(attr=(b'{}').format(replaceHyphenToUnderscore(iconName)))
    if specialID is not None and specialID.exists():
        return (specialID(), True)
    else:
        bigID = R.images.gui.maps.icons.tankmen.icons.big.dyn(attr=(b'{}').format(replaceHyphenToUnderscore(iconName)))
...
def getIconBackground(resurceID=None, dynIconName=None, dynPath=RECRUIT_BG_DYN):
    ...
    if resurceID and resurceID[:2] == b'ny':
