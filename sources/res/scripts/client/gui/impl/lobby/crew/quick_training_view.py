...
class QuickTrainingView(BaseCrewView):
    __slots__ = (b'__selectedTankmanID', b'__selectedVehicleID', b'__doNotOpenTips', b'__closedTips', b'_stepper', b'_uiData')
    ...
    def __init__(self, layoutID, **kwargs):
        ...
        self.__selectedTankmanID = kwargs.get(b'tankmanInvID', NO_TANKMAN)
        self.__selectedVehicleID = kwargs.get(b'vehicleInvID', NO_VEHICLE_ID)
    ...
    def _getCallbacks(self):
        return (
         ...
         (
          b'stats.freeXP', self.__onFreeXpChanged))
    ...
    def __onServerSettingsChange(self, diff):
        if b'isCrewBooksPurchaseEnabled' in diff:
    ...
    def __onInventoryUpdate(self, invDiff):
        if GUI_ITEM_TYPE.VEHICLE in invDiff:
            vehsCompDescr = invDiff[GUI_ITEM_TYPE.VEHICLE].get(b'compDescr', {})
        ...
        if GUI_ITEM_TYPE.TANKMAN in invDiff:
            compDescr = invDiff.get(GUI_ITEM_TYPE.TANKMAN, {}).get(b'compDescr', {})
    ...
    def __onFreeXpChanged(self, *_):
        with self.viewModel.transaction() as vm:
            self.__setFreeXPModelData(vm)
        self._updateTmanSkillsLevels(b'update')
        return
    ...
    def __clearSelected(self, isLearningInProgress=False):
        ...
        if not isLearningInProgress:
            self._updateTmanSkillsLevels(b'reset')
        return

    @jsonArgsConverter((b'intCD',))
    def __onBookMouseEnter(self, intCD):
        ...
        if self.IS_MUTUAL_CALCULATIONS_SUPPORTED:
            self._updateTmanSkillsLevels(b'reset', int(bool(self._uiData.freeXp)))
        else:
            self._updateTmanSkillsLevels(b'update')
        return

    @jsonArgsConverter((b'intCD', b'count'))
    ...
    def __onFreeXpStepperAction(self, data):
        actionType = data.get(b'actionType', b'update')
        action = int(data.get(b'action', 0))
    ...
    @jsonArgsConverter((b'isSelected',))
    ...
    def __onSelectFreeXp(self, isSelected, isUpdate=True):
        if isSelected:
            if self.tankman is not None:
                self.__updateSlotIndex()
                self._updateTmanSkillsLevels(b'perk', int(isUpdate))
            else:
                self._updateFullTankmenData(isForXp=False)
                self._updateTmanSkillsLevels(b'reset')
        else:
    ...
    def _onWidgetSlotClick(self, tankmanInvID, slotIdx):
        ...
        if self.__selectedTankmanID != NO_TANKMAN:
            if self.__isSelectionRequired():
                self._crewWidget.updateSlotIdx(slotIdx)
            self._updateTmanSkillsLevels(b'reset', isForXp and self.tankman is not None)
        self._updateTmanSkillsLevels(b'reset', isForXp and self.tankman is not None)
        return
    ...
    def _updateTmanSkillsLevels(self, actionType=b'reset', action=0):
