...
DocumentRecord = namedtuple(b'DocumentRecord', [b'id', b'group', b'value'])

class TRAINING_TIPS(CONST_CONTAINER):
    NOT_TRAINED_THIS_VEHICLE = b'notTrainedThisVehicle'
    NOT_FULL_CREW = b'notFullCrew'
    NOT_FULL_AND_NOT_TRAINED_CREW = b'notFullAndNotTrainedCrew'
    LOW_PE_CREW = b'LowPECrew'
    LOW_PE_NOT_FULL_CREW = b'LowPENotFullCrew'
    LOW_PE_NOT_TRAINED_CREW = b'LowPENotTrainedCrew'
    LOW_PE_NOT_TRAINED_NOT_FULL_CREW = b'LowPENotTrainedNotFullCrew'
    LOW_PE_TIPS_PERSONAL = b'LowPEtipsPersonal'
    WILL_FULL_TRAINED_PERSONAL = b'WillFullTrainedPersonal'
    WILL_FULL_TRAINED_FEW_MEMBERS = b'WillFullTrainedFewMembers'
    WILL_FULL_TRAINED_CREW = b'WillFullTrainedCrew'
    FULL_TRAINED_PERSONAL = b'FullTrainedPersonal'
    FULL_TRAINED_FEW_MEMBERS = b'FullTrainedFewMembers'
    ALL_FULL_TRAINED = b'AllFullTrained'
...
def saveDoNotOpenTip(doNotOpenTip):
    ...
    if userPrefs is None:
        ...
        if not userPrefs.has_key(Settings.QUICK_TRANING_TIPS):
            userPrefs.write(Settings.QUICK_TRANING_TIPS, b'')
        ds = userPrefs[Settings.QUICK_TRANING_TIPS]
...
ALT_VOICES_PREVIEW = itertools.cycle((b'vo_enemy_hp_damaged_by_projectile_by_player', b'vo_enemy_fire_started_by_player', b'vo_enemy_killed_by_player'))
...
def packCompensationData(books, rewardsArray, tooltipData, itemsCache=None):
    ...
    booksDataByType = defaultdict((lambda : {b'amount': 0, b'books': []}))
    for key, value in books.iteritems():
        book = itemsCache.items.getItemByCD(key)
        if book is None:
            continue
        typeData = booksDataByType[book.getBookType()]
        typeData[b'amount'] += value
        typeData[b'books'].append((book, value))
    ...
    for key in bookTypeOrder:
        ...
        tooltipData[key] = sorted(value[b'books'], cmp=(lambda item, other: cmp(GUI_NATIONS_ORDER_INDEX.get(item[0].getNation()), GUI_NATIONS_ORDER_INDEX.get(other[0].getNation()))))
        reward = RewardItemModel()
        reward.setIcon(key + b'_pack')
...
BOOSTER_ICON_MAPPING = {b'enemyShotPredictorBattleBooster': b'commander_enemyShotPredictor', 
   b'practicalityBattleBooster': b'commander_practical', 
   b'lastEffortBattleBooster': b'radioman_lastEffort', 
   b'sixthSenseBattleBooster': b'commander_sixthSense'}

@dependency.replace_none_kwargs(itemsCache=IItemsCache)
def packBoostersCompensationData(boosters, rewardsArray, tooltipData, itemsCache=None):
    for boosterReplacement in boosters:
        newEqCD = boosterReplacement[b'newEqCD']
        count = boosterReplacement[b'count']
        tooltipId = str(newEqCD)
        tooltipData[tooltipId] = {b'oldDirectiveID': (boosterReplacement[b'oldEqCD']), 
           b'newDirectiveID': newEqCD, 
           b'amount': count}
        ...
        reward.setName(b'items')
        reward.setType(b'items')
...
def getMetoringLicensesAmount(goodiesCache=None):
    mentoringLicense = goodiesCache.getMentoringLicense(currency=b'gold')
    if mentoringLicense:
