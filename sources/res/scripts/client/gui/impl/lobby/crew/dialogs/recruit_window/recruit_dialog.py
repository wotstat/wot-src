...
class BaseRecruitDialog(BaseCrewDialogTemplateView):
    __slots__ = (b'_selectedNation', b'_selectedVehType', b'_selectedVehicle', b'_selectedSpecialization', b'_recruitContent', b'_isSlotChanged')
...
class TokenRecruitDialog(BaseRecruitDialog):
    ...
    __SOUND_SETTINGS = CommonSoundSpaceSettings(name=b'hangar', entranceStates={(SOUNDS.STATE_PLACE): (CREW_SOUNDS.STATE_PLACE_BARRAKS), (StatesGroup.HANGAR_FILTERED): (States.HANGAR_FILTERED_OFF)}, exitStates={}, persistentSounds=(), stoppableSounds=(), priorities=(), autoStart=True, enterEvent=SOUNDS.WOMAN_AWARD_WINDOW, exitEvent=b'')
    ...
    def _setResult(self, result):
        if result == DialogButtons.SUBMIT:
            ...
            if vehicle and self._hasIrrelevantSkils(vehicle):
                confirmResult = yield wg_await(showRecruitConfirmIrrelevantConversionDialog({b'tokenData': (self.__tokenData), 
                   b'selectedRole': (self._selectedSpecialization), 
                   b'selectedVehicle': vehicle}))
...
class QuestRecruitDialog(BaseRecruitDialog):
    __slots__ = (b'__mission', b'__isFemale', b'__vehicleSlotToUnpack', b'__vehicle')
    _itemsCache = dependency.descriptor(IItemsCache)
    ...
    def _unpackQuestRecruit(self, callback=None):
        ...
        if res.userMsg:
            SystemMessages.pushMessage(res.userMsg, type=res.sysMsgType)
        newTankman = self._itemsCache.items.getTankman(res.auxData.get(b'tmanInvID', NO_TANKMAN))
