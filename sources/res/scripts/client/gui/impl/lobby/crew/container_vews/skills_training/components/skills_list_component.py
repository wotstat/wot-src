...
class SkillsListComponent(ComponentBase):
    ...
    def createToolTip(self, event):
        if event.contentID == R.views.common.tooltip_window.backport_tooltip_content.BackportTooltipContent():
            ...
            if tooltipId == TooltipConstants.SKILL_ALT:
                positionX = event.getArgument(b'positionX', event.mouse.positionX)
                args = [str(event.getArgument(b'skillName')), self.context.role, self.context.tankman.invID, None,
    ...
    def __fillSkillsList(self, skillsListVM, skills, checkIrrelevant=True, popularityData=None, sortFunction=None):
        ...
        for skill in skills:
            ...
            skillVM.setDescription(formatDescription(skill.maxLvlDescription, skillParams.get(b'keyArgs', {})))
    ...
    def _onSkillHover(self, kwargs):
        self.events.onSkillHover(kwargs.get(b'id'))
        return

    def _onSkillOut(self, kwargs):
        self.events.onSkillOut(kwargs.get(b'id'))
        return

    def _onSkillClick(self, kwargs):
        self.events.onSkillClick(kwargs.get(b'id'))
        return
            ...
            if orderSetDataIndex != -1:
                ...
                def __getSortOrder(orderSet, dataIndex, tankmanSkill):
                    ...
                    if skillOrder is not None:
                        ...
                        _logger.warning(b"Couldn't find %s order for tankman skill=%s", b'common' if dataIndex == 0 else b'legendary', tankmanSkill.name)
                        return 0
