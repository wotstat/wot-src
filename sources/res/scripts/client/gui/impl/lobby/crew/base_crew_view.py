...
class BaseCrewView(BaseCrewSoundView, IGlobalListener):
    __slots__ = (b'_isHangar', b'_crewWidget', b'_currentViewID', b'_currentViewKey', b'_uiLogger')

    def __init__(self, settings, parentViewKey=None):
        self._isHangar = bool(self.gui.windowsManager.findWindows((lambda window: getattr(window.content, b'alias', None) == VIEW_ALIAS.LOBBY_HANGAR)))
        self._crewWidget = None
        self._currentViewID = settings.kwargs.get(b'currentViewID', settings.layoutID)
        self._currentViewKey = None
        self._uiLogger = CrewBringToFrontViewLogger(self, settings.layoutID, parentViewKey if parentViewKey else LAYOUT_ID_TO_ITEM.get(settings.kwargs.get(b'previousViewID')))
    ...
    def toggleMenu(isVisible=False):
        g_eventBus.handleEvent(events.LobbyHeaderMenuEvent(events.LobbyHeaderMenuEvent.TOGGLE_VISIBILITY, ctx={b'state': (HeaderMenuVisibilityState.ALL if isVisible else HeaderMenuVisibilityState.NOTHING)}), scope=EVENT_BUS_SCOPE.LOBBY)
        return
    ...
    def _setCrewWidget(self, **kwargs):
        ...
        slotIdx = kwargs.get(b'slotIdx', NO_SLOT)
        previousViewID = kwargs.get(b'previousViewID', None)
