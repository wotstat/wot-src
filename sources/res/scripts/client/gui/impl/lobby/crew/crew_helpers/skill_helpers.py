...
def getVehicleWithSkilledTman(skilledTman, tankman, tankmanVehicle, skillName=b''):
...
def formatDescription(description, params):
    for props in params:
        color = params[props].get(b'color', b'')
        value = params[props].get(b'value', b'')
        description = description.replace(b'%(' + props + b')s', b'{' + color + b'_Start}' + value + b'{' + color + b'_End}')
