...
class MentorAssignmentDialog(BaseCrewDialogTemplateView):
    __slots__ = (b'__sourceTankman', b'__targetTankman', b'__targetVehicle', b'__totalXp', b'__loseXp', b'_toolTipMgr')
    ...
    def createToolTip(self, event):
        if event.contentID == R.views.common.tooltip_window.backport_tooltip_content.BackportTooltipContent():
            ...
            if tooltipId == TooltipConstants.SKILL:
                isDonor = event.getArgument(b'isDonor')
                ...
                args = [skillName, roleName, 0, 0, True, 5, 0, 6, tankman, vehicle]
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.lobby.crew.tooltips.EmptySkillTooltip():
            isDonor = event.getArgument(b'isDonor')
            tankman = self.__sourceTankman if isDonor else self.__targetTankman
            return EmptySkillTooltip(tankman, int(event.getArgument(b'skillIndex')))
    ...
    def __onInputChange(self, event):
        isDisabled = str(event.get(b'input')) != str(self.__totalXp)
    ...
    def __getSkillLevel(self, isDonor, skillName, event):
        if isDonor:
            ...
            return event.getArgument(b'level')
