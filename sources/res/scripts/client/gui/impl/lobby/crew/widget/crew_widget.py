...
BuildedMessage = NamedTuple(b'BuildedMessage', [
 (
  b'text', str),
 (
...
class CrewWidget(ViewImpl, IGlobalListener):
    __slots__ = (b'__toolTipMgr', b'__currentViewID', b'__previousViewID', b'__currentTankman', b'__currentVehicle', b'onSlotClick', b'__currentSlotIdx', b'onChangeCrewClick', b'onSlotTrySelect', b'__isButtonBarVisible')
    ...
    def createToolTipContent(self, event, contentID):
        ...
        if contentID == R.views.lobby.crew.CrewHeaderTooltipView():
            ...
            if contentID == R.views.lobby.crew.tooltips.EmptySkillTooltip():
                tman = self.itemsCache.items.getTankman(int(event.getArgument(b'tankmanID')))
    ...
    def createContextMenu(self, event):
        if event.contentID != R.views.common.BackportContextMenu():
            ...
            tankmanID = event.getArgument(b'tankmanID')
            if tankmanID == NO_TANKMAN:
                return None
            contextMenuArgs = {b'tankmanID': tankmanID, b'slotIdx': (event.getArgument(b'slotIdx')), 
               b'previousViewID': (self.__currentViewID)}
    ...
    def createPopOverContent(self, event):
        args = {b'crewIds': (getLowEfficiencyCrew(self.__currentVehicle)), b'vehicleCD': (self.__currentVehicle.intCD)}
    ...
    def _getCallbacks(self):
        return (
         (
          b'inventory.13.2', self.__updateTankmen),
         (
          b'inventory.8.compDescr', self.__updateTankmen),
         (
          b'idleCrewXP', self.__idleCrewXPUpdated),
         (
          b'inventory.1', self.__onVehicleDossierUpdate))
    ...
    def __onVehicleDossierUpdate(self, diff=None):
        crews = diff.get(b'crew', {})
        if self.__currentVehicle:
            settings = diff.get(b'settings', {})
            eqs = diff.get(b'eqs', {})
            compDescr = diff.get(b'compDescr', {})
            boosters = diff.get(b'boosters', {})
    ...
    def __onChangeCrewClick(self, args):
        slotIdx = int(args.get(b'slotIdx'))
    ...
    def _onSlotClick(self, args):
        tankmanID = int(args.get(b'tankmanID'))
        slotIdx = int(args.get(b'slotIdx'))
        self.updateSlotIdx(slotIdx)
    ...
    def __buildConfirmationMessage(self):
        ...
        if previousVehicle:
            ...
            vehicleToName = (b'{} %(typeIconTo) {}').format(int2roman(self.__currentVehicle.level), self.__currentVehicle.userName)
            ...
            message = BuildedMessage(text=finalString, iconFrom=R.images.gui.maps.icons.vehicleTypes.num(b'24x24').dyn(getIconResourceName(previousVehicle.type)), iconTo=R.images.gui.maps.icons.vehicleTypes.num(b'24x24').dyn(getIconResourceName(self.__currentVehicle.type)), vehFromCD=previousVehicle.intCD, vehToCD=self.__currentVehicle.intCD)
        return message
...
class QuickTrainingCrewWidget(CrewWidget):
    __slots__ = (b'_cachePossibleSkillsLevels', b'_cachePossibleSkillsEfficiency')
