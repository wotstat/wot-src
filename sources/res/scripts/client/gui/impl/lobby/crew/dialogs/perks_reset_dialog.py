...
class PerksResetDialog(BaseCrewDialogTemplateView):
    __slots__ = (b'_tankman', b'_priceListContent', b'_isFreePerkReset', b'_lastSoundEvent', b'_isFreeByGold', b'_moneyBalance')
    ...
    def _onPriceChange(self, index=None):
        ...
        if submitBtn is not None:
            ...
            if not isDisabled:
                priceNamed = namedtuple(b'priceNamed', [b'xpReuseFraction', b'xpAmountLoss', b'skillsCountLoss'])
    ...
    @decorators.adisp_process(b'deleting')
        ...
        if result.userMsg:
            if not useRecertificationForm or freeDrop:
                ...
                action = {b'sentTime': (time.time()), 
                   b'data': {b'type': (SYS_MESSAGE_TYPE.recertificationResetUsed.index()), 
                             b'data': {b'currencyType': b'blanks', b'count': 1}}}
