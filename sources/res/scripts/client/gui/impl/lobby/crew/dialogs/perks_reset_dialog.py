...
class PerksResetDialog(BaseCrewDialogTemplateView):
    __slots__ = (b'_tankman', b'_priceListContent', b'_uiEpicBattleLogger', b'_isFreePerkReset')
    ...
    def __processReset(self, tankman, dropSkillKey, price, freeDrop):
        ...
        if result.userMsg:
            if not useRecertificationForm:
                ...
                data = {b'currencyType': b'', b'count': 0}
                if not freeDrop:
                    data[b'currencyType'] = b'blanks'
                    data[b'count'] = 1
                    messageType = SYS_MESSAGE_TYPE.recertificationResetUsed
                    action = {b'sentTime': (time.time()), 
                       b'data': {b'type': (messageType.index()), 
                                 b'data': data}}
