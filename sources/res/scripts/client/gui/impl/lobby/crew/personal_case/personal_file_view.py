...
class PersonalFileView(IPersonalTab, BasePersonalCaseView):
    __slots__ = (b'tankmanID', b'tankman', b'canLearn', b'__uiTooltipLogger', b'__toolTipMgr')
    ...
    def __init__(self, layoutID=R.views.lobby.crew.personal_case.PersonalFileView(), **kwargs):
        self.tankmanID = kwargs.get(b'tankmanID')
        self.canLearn = False
    ...
    def createToolTip(self, event):
        if event.contentID == R.views.common.tooltip_window.backport_tooltip_content.BackportTooltipContent():
            ...
            if tooltipId == TooltipConstants.SKILL:
                args = [
                 str(event.getArgument(b'skillName')), self.tankmanID, None, False, True, self.getParentWindow()]
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.lobby.crew.tooltips.PerkAvailableTooltip():
            return PerkAvailableTooltip(self.tankmanID)
        return super(PersonalFileView, self).createToolTipContent(event, contentID)
    ...
    def _getEvents(self):
        ...
        return eventsTuple + (
         (
          self.viewModel.onIncrease, self.__onIncrease),
         (
