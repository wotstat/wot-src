...
class TankmanTooltip(ViewImpl):
    __slots__ = (b'tankmanID', b'isCalledFromCrewWidget')
    ...
    def _setBonuses(self, vm, tankman, vehicle):
        ...
        brotherhoodBonus = tankman.vehicleBonuses.get(b'brotherhood', 0)
        ...
        if tankman.role != Tankman.ROLES.COMMANDER and isCommanderInVehicle:
            commanderBonus = tankman.vehicleBonuses.get(b'commander', 0)
            ...
            for optDevice in optDevices:
                if optDevice.descriptor.factorsContainCrewLevelIncrease():
                    ...
                    if optDevice.isInstalled(vehicle):
                        artefactsBonus = optDevice.descriptor.getFactorValue(vehicle.descriptor, b'miscAttrs/crewLevelIncrease')
                        finalEffVal += artefactsBonus
                        bnsId = optDevice.name.split(b'_tier')[0]
                        self.__addModifier(vm.getConsumables(), artefactsBonus, R.images.gui.maps.icons.vehParams.tooltips.bonuses.dyn(bnsId.split(b'_class')[0], R.invalid)(), backport.text(R.strings.artefacts.dyn(bnsId).name()))
    ...
    @staticmethod
    def __addModifier(modifierViewList, value, icon, title, description=b''):
