...
class PersonalDataView(IPersonalTab, ViewImpl):
    __slots__ = (b'tankmanID', b'filterPanelWidget', b'__filterState', b'__tankman')
    ...
    def __fillDocuments(self, cards):
        for icon, firstname, lastname in self.__dataProviders[b'documents'].items():
    ...
    def __fillDocumentCard(self, card, cardId, icon, firstname, lastname):
        card.setId(cardId)
        card.setName(b'%s %s' % (firstname, lastname))
    ...
    def __fillCrewSkins(self, cards):
        for item in self.__dataProviders[b'skins'].items():
