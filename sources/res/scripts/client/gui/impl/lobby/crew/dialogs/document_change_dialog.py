...
class DocumentChangeDialog(BaseCrewDialogTemplateView):
    __slots__ = (b'__tankmanInvID', b'__initialData', b'__firstNamesList', b'__lastNamesList', b'__firstNameIdx', b'__lastNameIdx')
    ...
    def _updateViewModel(self, tankman):
        ...
        self.__firstNamesList = getDocGroupValues(tankman, config, operator.attrgetter(b'firstNamesList'), config.getFirstName)
        self.__lastNamesList = getDocGroupValues(tankman, config, operator.attrgetter(b'lastNamesList'), config.getLastName)
