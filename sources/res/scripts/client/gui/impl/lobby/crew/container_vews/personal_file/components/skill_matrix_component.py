...
class SkillMatrixComponent(ComponentBase):
    ...
    def createToolTip(self, event):
        if event.contentID == R.views.common.tooltip_window.backport_tooltip_content.BackportTooltipContent():
            ...
            if tooltipId == TooltipConstants.SKILL:
                skillName = event.getArgument(b'skillName')
                roleName = event.getArgument(b'roleName')
                args = [skillName, roleName, self.context.tankmanID, None, True, None,
                 event.getArgument(b'isBonus'), int(event.getArgument(b'skillIndex'))]
            ...
            if tooltipId == TooltipConstants.DIRECTIVE:
                return createAndLoadBackportTooltipWindow(self.parent.getParentWindow(), isSpecial=True, tooltipId=TOOLTIPS_CONSTANTS.BATTLE_BOOSTER_BLOCK, specialArgs=[
                 int(event.getArgument(b'intCD')), 0, -1,
                 self.context.tankman.getVehicle()])
        return

    def createToolTipContent(self, event, contentID):
        if contentID == R.views.lobby.crew.tooltips.QualificationTooltip():
            return QualificationTooltip(event.getArgument(b'index'), event.getArgument(b'role'), event.getArgument(b'isBonusQualification'), self.context.tankman.isFemale)
        ...
        if contentID == R.views.lobby.crew.tooltips.EmptySkillTooltip():
            return EmptySkillTooltip(self.context.tankman, int(event.getArgument(b'skillIndex')))
        return
    ...
    def _onSkillClick(self, kwargs):
        data = {b'tankmanInvID': (int(self.context.tankman.invID)), 
           b'role': (kwargs.get(b'role'))}
        self.events.onSkillClick(**data)
        return

    def _onSetAnimationInProgress(self, kwargs):
        data = {b'isEnabled': (kwargs.get(b'isEnabled'))}
