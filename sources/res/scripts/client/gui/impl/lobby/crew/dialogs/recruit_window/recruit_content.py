...
class RecruitContent(object):
    __slots__ = (b'__viewModel', b'__selectedNationID', b'__selectedNation', b'__selectedVehType', b'__selectedVehicle', b'__selectedSpecialization', b'onRecruitContentChanged', b'__isFemale', b'__predefinedNations', b'__predefinedRoles', b'__predefinedVehicle', b'__predefinedVehicleType')
    ...
    def __setPredefinedTankman(self, predefinedData):
        self.__isFemale = predefinedData.get(b'isFemale', False)
        slotToUnpack = predefinedData.get(b'slotToUnpack')
        if slotToUnpack != -1:
            vehicle = predefinedData.get(b'predefinedVehicle')
            ...
            predefinedNations = predefinedData.get(b'predefinedNations')
            if predefinedNations:
                self.__predefinedNations = predefinedNations
                self.__fillNations()
            predefinedRoles = predefinedData.get(b'predefinedRoles')
            if predefinedRoles:
    ...
    def __fillNations(self):
        with self.viewModel.transaction() as tx:
            ...
            if len(nationsList) == 1:
                self.__onNationChange({b'id': (nationsList[FIRST_ELEMENT])})
        return
    ...
    def __fillVehTypes(self):
        with self.viewModel.transaction() as tx:
            ...
            if len(vehTypeList) == 1:
                self.__onVehTypeChange({b'id': (vehTypeList[FIRST_ELEMENT])})
        return
    ...
    def __getVehTypeList(self):
        ...
        if self.__predefinedRoles:
            ...
            if not filteredVehTypes and self.__selectedNation != NO_DATA_VALUE:
                _logger.warning(b"Couldn't get vehTypes intersections with predefinedRoles: %s Check Token settings", self.__predefinedRoles)
    ...
    def __fillVehicles(self):
        with self.viewModel.transaction() as tx:
            ...
            if len(nationVehicleList) == 1:
                self.__onVehicleChange({b'id': (nationVehicleList[FIRST_ELEMENT][FIRST_ELEMENT])})
        return
    ...
    def __getVehicleList(self):
        ...
        if self.__predefinedRoles:
            ...
            if not filteredVehicles and self.__selectedVehType != NO_DATA_VALUE:
                _logger.warning(b"Couldn't get vehicles intersections with predefinedRoles: %s Check Token settings", self.__predefinedRoles)
    ...
    def __fillRoles(self):
        with self.viewModel.transaction() as tx:
            ...
            if len(rolesList) == 1:
                self.__onSpecializationChange({b'id': (rolesList[FIRST_ELEMENT])})
        return
