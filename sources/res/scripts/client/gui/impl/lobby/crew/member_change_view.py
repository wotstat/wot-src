...
class MemberChangeView(BaseCrewView, BaseTankmanListView):
    ...
    def __init__(self, layoutID, **kwargs):
        ...
        self.__filterState = FilterState(initialState=self._getDefaultFilters(), isWotPlus=self.__currentVehicle.isWotPlus, persistor=Persistor(storageKey=b'memberChange', persistentGroups=[
    ...
    def _getCallbacks(self):
        return (
         ...
         (
          b'inventory.8.compDescr', self._onCrewChanged),
         (
          b'tokens', self._onCrewChanged),
         (
          b'potapovQuests', self._onCrewChanged))
