...
DEFAULT_OPERATION_DATA = {b'isPriceApplicable': None, 
   b'isCardDisabled': None, 
   b'skillEfficiencies': None, 
   b'cost': None}

class RetrainMassivePriceList(BasePriceList):
    __slots__ = (b'_tankmen', b'_vehicle')
    ...
    def _deselectCurrentCard(self, vm):
        if self._selectedCardIndex is None:
            ...
            state = CardState.DISABLED if all(self.selectedOperationData[b'isCardDisabled']) else CardState.DEFAULT
    ...
    def _getOperationData(self, cost):
        ...
        return {b'isPriceApplicable': isApplicableForTankmen, 
           b'isCardDisabled': isCardDisabledForAllTankmen, 
           b'skillEfficiencies': ser, 
           b'cost': cost}
