...
class EditableStyleMode(CustomMode):
    ...
    def _onStart(self):
        if self._ctx.modeId not in CustomizationModes.STYLED:
            _logger.error(b'Failed to start Style Edit Mode. Style Edit Mode could be started only from Styled mode.')
            self._ctx.changeMode(self._ctx.prevMode)
            return
        outfit = self._ctx.mode.currentOutfit
        if not outfit.id:
            _logger.error(b'Failed to start Style Edit Mode. No applied style.')
            self._ctx.changeMode(self._ctx.prevMode)
            return
        style = self._service.getItemByID(GUI_ITEM_TYPE.STYLE, outfit.id)
        if not style.isEditable:
            _logger.error(b'Failed to start Style Edit Mode. Applied style is not editable.')
            self._ctx.changeMode(self._ctx.prevMode)
            return
    ...
    def _getAnchorsData(self):
        if self.slotType in EDITABLE_STYLE_APPLY_TO_ALL_AREAS_TYPES:
            return [EDITABLE_STYLE_APPLY_TO_ALL_AREAS_TYPES[self.slotType]]
        return super(EditableStyleMode, self)._getAnchorsData()
    ...
    def __installItemToAllAreas(self, item, season=None):
        ...
        if item.isStyleOnly or isBaseItem or self.isPossibleToInstallToAllTankAreas(item.intCD, item.itemTypeID):
            ...
            _logger.warning(b'Failed to apply item: %s to all tank areas', item)
            return False
        ...
        if selItemTypeID == GUI_ITEM_TYPE.CAMOUFLAGE:
            ...
            if camoDependencies:
                ...
                if not currentOutfit:
                    _logger.error(b'Could not find outfit for required season "%d"', season)
                    return
                camoDependencies = styleDependencies.get(selItemID, {})
    ...
    def __getSuitableIntCD(self, unsuitableItem, dependentByType):
        ...
        if posUnsuitType == CustomizationType.DECAL:
            ...
            for suitableID in dependentByType:
                posiblySuitableIntCD = makeIntCompactDescrByID(b'customizationItem', posUnsuitType, suitableID)
            ...
            suitableItemIntCD = makeIntCompactDescrByID(b'customizationItem', posUnsuitType, dependentByType[0])
