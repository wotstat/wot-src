...
class CustomizationOptions(object):
    ...
    BUY_MORE = b'buyMore'
    SELL = b'sell'
    REMOVE_FROM_TANK = b'removeFromTank'
    PROLONGATION_ON = b'autoprolongationOn'
    PROLONGATION_OFF = b'autoprolongationOff'
    STYLE_INFO = b'styleInfo'
    EDIT_STYLE = b'editStyle'
    BASE_STYLE = b'baseStyle'
    MARK_FAVORITES = b'markFavorites'
...
class CustomizationItemCMHandler(AbstractContextMenuHandler):
    ...
    def __init__(self, cmProxy, ctx=None):
        self._intCD = 0
        super(CustomizationItemCMHandler, self).__init__(cmProxy, ctx, {(CustomizationOptions.BUY): b'buyItem', 
           (CustomizationOptions.SELL): b'sellItem', 
           (CustomizationOptions.REMOVE_FROM_TANK): b'removeItemFromTank', 
           (CustomizationOptions.PROLONGATION_ON): b'changeAutoRent', 
           (CustomizationOptions.PROLONGATION_OFF): b'changeAutoRent', 
           (CustomizationOptions.STYLE_INFO): b'showStyleInfo', 
           (CustomizationOptions.EDIT_STYLE): b'enterEditMode', 
           (CustomizationOptions.BASE_STYLE): b'clearStyle', 
           (CustomizationOptions.MARK_FAVORITES): b'markFavorites'})
    ...
    def clearStyle(self):
        if self.__ctx.modeId in CustomizationModes.ALL_STYLES:
            ...
            _logger.error(b'Failed to install EditableStyle base outfit. Style/EditableStyle mode must be selected.')
        return
    ...
    def __getStyleInfoBtn(self, item):
        ...
        btn = self._makeItem(optId=CustomizationOptions.STYLE_INFO, optLabel=MENU.cst_item_ctx_menu(CustomizationOptions.STYLE_INFO), optInitData={b'enabled': enabled})
        return btn
    ...
    def __getEditStyleBtn(self, item):
        ...
        btn = self._makeItem(optId=CustomizationOptions.EDIT_STYLE, optLabel=MENU.cst_item_ctx_menu(CustomizationOptions.EDIT_STYLE), optInitData={b'enabled': (bool(editingReason) and not isUsedUp)})
        return btn
    ...
    def __getClearStyleBtn(self, item):
        ...
        if isStyleInstalled and style.isEditable:
            ...
            enabled = False
        btn = self._makeItem(optId=CustomizationOptions.BASE_STYLE, optLabel=MENU.cst_item_ctx_menu(CustomizationOptions.BASE_STYLE), optInitData={b'enabled': enabled})
        return btn
    ...
    def __getBuyBtn(self, item):
        ...
        btn = self._makeItem(optId=CustomizationOptions.BUY, optLabel=MENU.cst_item_ctx_menu(textKey), optInitData={b'data': ({b'price': (first(buyPriceVO))} if availableForPurchase else None), 
           b'enabled': availableForPurchase}, optSubMenu=None, linkage=b'CurrencyContextMenuItem')
        return btn
    ...
    def __getRentBtn(self, item):
        ...
        btn = self._makeItem(optId=optId, optLabel=MENU.cst_item_ctx_menu(optId), optInitData={b'enabled': enabled, 
           b'iconType': optId})
        return btn
    ...
    def __getSellBtn(self, item):
        ...
        if showAlert:
            ...
            price = sellPriceData[0][b'price']
            sellPriceData[0] = {b'price': price}
        else:
            tooltipVO = None
        ...
        btn = self._makeItem(optId=CustomizationOptions.SELL, optLabel=MENU.cst_item_ctx_menu(CustomizationOptions.SELL), optInitData={b'data': ({b'price': (first(sellPriceData))} if availableForSale else None), 
           b'enabled': availableForSale, 
           b'showAlert': showAlert, 
           b'tooltipVO': tooltipVO}, optSubMenu=None, linkage=b'CurrencyContextMenuItem')
        return btn
    ...
    def __getRemoveBtn(self, item):
        ...
        if itemType == GUI_ITEM_TYPE.STYLE:
            ...
            textKey = (b'/').join((CustomizationOptions.REMOVE_FROM_TANK, seasonName))
        if self.__ctx.modeId == CustomizationModes.EDITABLE_STYLE and itemType in EDITABLE_STYLE_IRREMOVABLE_TYPES:
            ...
            enabled = isInstalled
        btn = self._makeItem(optId=CustomizationOptions.REMOVE_FROM_TANK, optLabel=MENU.cst_item_ctx_menu(textKey), optInitData={b'enabled': enabled})
        return btn

    def __getFavoritesBtn(self, item):
        textKey = b'unmarkAsFavorite' if item.markedAsFavorite else b'markAsFavorite'
        btn = self._makeItem(optId=CustomizationOptions.MARK_FAVORITES, optLabel=MENU.cst_item_ctx_menu(textKey), optInitData={b'enabled': True})
        return btn
