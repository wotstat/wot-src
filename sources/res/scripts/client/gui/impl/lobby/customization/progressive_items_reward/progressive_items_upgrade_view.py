...
class ProgressiveItemsUpgradeView(ViewImpl):
    __slots__ = (b'__item', b'__vehicle', b'__level', b'__itemsInNeedToUpgrade')
    ...
    def _onLoading(self, itemCD, vehicleCD, progressionLevel, showSecondButton):
        ...
        if self.__item is None:
            raise SoftException(b'invalid item: &s', itemCD)
        if self.__vehicle is None:
            raise SoftException(b'invalid vehicle: &s', vehicleCD)
    ...
    def __addListeners(self):
        ...
        g_clientUpdateManager.addCallbacks({b'cache.vehsLock': (self._updateButtons)})
        return
