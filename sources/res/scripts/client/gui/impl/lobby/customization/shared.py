...
STYLE_2D_PREFIX = b'2d'
STYLE_3D_PREFIX = b'3d'
...
class CustomizationTabs(object):
    ...
    STYLED_3D = 8
    ALL = (
     PAINTS, CAMOUFLAGES, PROJECTION_DECALS, EMBLEMS, INSCRIPTIONS, MODIFICATIONS, STYLED_2D, STYLED_3D)
    CUSTOM_ALL = (PAINTS, CAMOUFLAGES, PROJECTION_DECALS, EMBLEMS, INSCRIPTIONS, MODIFICATIONS)
    STYLES_ALL = (STYLED_3D, STYLED_2D)
    STYLES_2D = (STYLED_2D,)
    STYLES_3D = (STYLED_3D,)
    REGIONS = (
     PAINTS, CAMOUFLAGES, MODIFICATIONS, STYLED_2D, STYLED_3D)
    GROUPS = (
     (
      STYLED_3D,), (STYLED_2D,), (CAMOUFLAGES, PAINTS, PROJECTION_DECALS, EMBLEMS, INSCRIPTIONS, MODIFICATIONS))
...
_CUSTOM_ITEM_TYPE_TO_TAB = {value: tab for value in CustomizationTabs.CUSTOM_ALL}
...
SCALE_SIZE = (
 backport.text(R.strings.vehicle_customization.customization.Popover.scale.small()),
 backport.text(R.strings.vehicle_customization.customization.Popover.scale.normal()),
 backport.text(R.strings.vehicle_customization.customization.Popover.scale.large()))
TYPES_ORDER = (
 GUI_ITEM_TYPE.PAINT,
 GUI_ITEM_TYPE.CAMOUFLAGE,
 GUI_ITEM_TYPE.PROJECTION_DECAL,
...
def checkSlotsFilling(outfit, slotType):
    ...
    if slotType == GUI_ITEM_TYPE.STYLE:
        return (
         1, 0 if outfit.style is None else 1)
    for areaId in Area.ALL:
        regionsIndexes = getAvailableRegions(areaId, slotType)
...
def __resetC11nItemsNoveltyParamsMerger(merged, callParams):
    items = callParams.kwargs.get(b'items', [])
    items.extend(merged.kwargs.get(b'items', []))
    return CallParams(kwargs={b'items': items})
