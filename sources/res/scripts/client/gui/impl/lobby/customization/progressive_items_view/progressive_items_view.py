import weakref, logging, BigWorld
...
class ProgressiveItemsView(ViewImpl):
    __slots__ = (b'__customizationView', b'_itemsProgressData', b'_possibleItems', b'_vehicle', b'__blur', b'__layoutID', b'__urlMacros', b'__guiSettings')
    ...
    def __init__(self, layoutID, customizationView, *args, **kwargs):
        ...
        self.__guiSettings = GUI_SETTINGS.progressiveItems.get(b'tutorialVideo', {})
        return

    def createToolTip(self, event):
        if event.contentID == R.views.common.tooltip_window.backport_tooltip_content.BackportTooltipContent():
            intCD = int(event.getArgument(b'id'))
            level = int(event.getArgument(b'level'))
            window = BackportTooltipWindow(self.__getTooltipData(intCD, event.getArgument(b'tooltip'), level), self.getParentWindow())
            window.load()
            return window
    ...
    def _onLoading(self, *args, **kwargs):
        ...
        itemIntCD = kwargs.get(b'itemIntCD')
    ...
    def _onSelectItem(self, args=None):
        if args is not None:
            intCD = int(args[b'intCD'])
            level = int(args[b'level'])
    ...
    def _playSounds(self, args=None):
        if args is not None:
            isTutorialPageOpened = bool(args[b'isTutorialPageOpened'])
    ...
    def __showVideo(self):
        url = yield self.__urlMacros.parse(self.__guiSettings.get(b'url'))
    ...
    def __setEachLevelInfo(self, model, item):
        for level in xrange(1, model.getMaxLevel() + 1):
            ...
            levelConditions = item.progressionConditions.get(level, {}).get(b'conditions')
            if level == model.getCurrentLevel() and levelConditions:
                levelInfo.setInProgress(True)
                levelInfo.progressBlock.setUnlockCondition(_ms(levelConditions[0].get(b'description', b'')))
                currProgress = int(item.getCurrentProgressOnCurrentLevel(self._vehicle))
                currProgress = currProgress if currProgress > 0 else 0
                maxProgress = int(levelConditions[0].get(b'value', b'1'))
                if maxProgress > 1:
