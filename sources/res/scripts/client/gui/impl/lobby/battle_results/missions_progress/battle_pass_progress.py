...
class BattlePassProgressPresenter(ViewComponent[BattlePassProgressModel], IProgressionCategoryPresenter):
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.lobby.tooltips.AdditionalBattlePassRewardsTooltip():
            showFromIndex = int(event.getArgument(b'showFromIndex'))
            rewardType = event.getArgument(b'rewardType')
            level = int(event.getArgument(b'level'))
            chapter = int(event.getArgument(b'chapter'))
    ...
    def __packExternalPoints(self, model):
        ...
        if isWotPlusBattlePassAvailableForAnyTier():
            ...
            for extSource, extData in externalPoints.iteritems():
                isAcquired = extData.get(b'acquired', False)
                pointModel = model.getBpTopExternalPointsType()()
                pointModel.setPoints(extData.get(b'points', 0))
