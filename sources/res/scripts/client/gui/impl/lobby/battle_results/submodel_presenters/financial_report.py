...
class ManageableBonusSubPresenter(BattleResultsSubPresenter):
    ...
    def packBattleResults(self, battleResults):
        ...
        if isPersonalTeamWin:
            ...
            bonusFactor = self.__lobbyContext.getServerSettings().getAdditionalBonusConfig().get(b'bonusFactor', 1)
    ...
    def _getCallbacks(self):
        return (
         (
          b'stats.applyAdditionalXPCount', self.__onXpBonusStatusChanged),
         (
    ...
    def __onLocalStorageUpdated(self, event):
        ctx = event.get(b'localStorage', b'')
