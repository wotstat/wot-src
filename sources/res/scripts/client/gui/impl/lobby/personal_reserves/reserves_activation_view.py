    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.common.personal_reserves.ReservesDisabledTooltip():
            ...
            if contentID == R.views.lobby.personal_reserves.BoosterTooltip():
                boosterId = int(event.getArgument(b'boosterId', 0))
                specialAlias = event.getArgument(b'specialAlias')
                if not specialAlias:
                    _logger.warning(b'Requested backport tooltip but specialAlias is missing!')
                    return None
                if not boosterId:
                    _logger.warning(b'Cannot open tooltip. Missing boosterId in tooltip params!')
                    return None
                if specialAlias == TOOLTIPS_CONSTANTS.CLAN_RESERVE_INFO:
                    ...
                    if not hasClanReserve:
                        _logger.warning(b'Cannot open clan tooltip. Such clan booster does not exists!')
                        return None
