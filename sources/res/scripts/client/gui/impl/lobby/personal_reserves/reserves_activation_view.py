...
class ReservesActivationView(ReservesViewMonitor):
    ...
    def _createBackportTooltip(self, event):
        if not event.hasArgument(b'specialAlias'):
            _logger.warning(b'Requested backport tooltip but specialAlias is missing!')
            return
        else:
            if event.getArgument(b'boosterId', 0) <= 0:
                _logger.warning(b'Missing booster ID!')
                return
            specialAlias = event.getArgument(b'specialAlias')
            boosterId = int(event.getArgument(b'boosterId'))
            ...
            _logger.warning(b'Requested unsupported backport tooltip alias: %s', specialAlias)
            return
