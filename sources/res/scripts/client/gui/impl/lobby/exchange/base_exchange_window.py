...
class BaseExchangeWindow(ComponentsPresenterView, FullScreenDialogBaseView):
    ...
    def __init__(self, settings, exchangeRateType, ctx=None):
        ...
        self.__backgroundImage = ctx.pop(b'backgroundImage', 0)
        self.__exchangeRateType = exchangeRateType
        self.__blur = ctx.pop(b'blur', None)
    ...
    def _onLoading(self, *args, **kwargs):
        ...
        if not isDiscountViewed(self.__exchangeRateType):
            ...
            g_eventBus.handleEvent(events.ExchangeRatesDiscountsEvent(events.ExchangeRatesDiscountsEvent.ON_PERSONAL_DISCOUNT_VIEWED, {b'type': (self.__exchangeRateType)}), scope=EVENT_BUS_SCOPE.LOBBY)
        return
    ...
    def _getCallbacks(self):
        return (
         (
          b'stats.gold', self.__updateData),)
    ...
    def _onSelectedValueChanged(self, params=None, model=None):
        if not params:
            ...
            params = (currentValue or self)._initValues if 1 else {b'currency': currentValue}
    ...
    def _onOpenAllDiscountsWindow(self):
        showAllPersonalDiscountsWindow(exchangeRateType=self.__exchangeRateType, selectedValue={b'currency': (self.viewModel.getResourceAmountForExchange())})
        return
    ...
    def __onSelectedAmountChanged(self, params):
        ...
        g_eventBus.handleEvent(events.ExchangeRatesDiscountsEvent(events.ExchangeRatesDiscountsEvent.ON_SELECTED_AMOUNT_CHANGED, {b'amount': (self.viewModel.getGoldAmountForExchange())}), scope=EVENT_BUS_SCOPE.LOBBY)
        return
