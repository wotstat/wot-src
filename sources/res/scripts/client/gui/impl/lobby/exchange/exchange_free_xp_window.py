...
class ExchangeFreeXPView(BaseExchangeWindow):
    ...
    def _getCallbacks(self):
        ...
        return callbacks + (
         (
          b'stats.vehTypeXP', self.__updateData),
         (
          b'stats.freeXP', self.__updateData))
    ...
    def _onExchange(self, data):
        goldPrice = int(data.get(b'gold', 0))
        if goldPrice < 1:
            _logger.debug(b"Parameter 'gold' parameter is incorrect %d", goldPrice)
            return
        selectedVehicles = data.get(b'selectedVehicles', b'')
        if selectedVehicles:
            selectedVehicles = [int(num) for num in selectedVehicles.strip(b'[]').split(b',')]
        if not selectedVehicles:
            _logger.debug(b"Parameter 'selectedVehicles' is missed for xp conversion")
            return
    ...
    def __onVehiclesSelectedChanged(self, event=None, vehiclesChanged=True, model=None):
        if event:
            self.__selectedXpFromVehicles = int(event.get(b'currency', 0))
...
class ExchangeXPWindowDialog(ExchangeFreeXPView):
    __slots__ = (b'_result', b'_exchangeAmount')
