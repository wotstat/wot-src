...
class AllPersonalDiscountsView(ViewImpl):
    ...
    def _updateModel(self, model=None):
        if self.exchangeRate is None or len(self.exchangeRate.allPersonalLimitedDiscounts) <= MAX_SHOW_DISCOUNTS_INDEX:
            _logger.debug(b'There are no exchangeRate or amount %s of discounts is incorrect', len(self.exchangeRate.allPersonalLimitedDiscounts))
    ...
    def __updateAllModel(self):
        if len(self.exchangeRate.allPersonalLimitedDiscounts) < 1:
            _logger.debug(b'There are not enough discounts for this window')
    ...
    def __updateDiscounts(self, model):
        ...
        if not discounts:
            _logger.debug(b'There are no limited discounts in the exchange')
    ...
    def __getSelectedGold(self):
        selectedGold = self.__selectedValue.get(b'gold', 0)
        selectedCurrency = self.__selectedValue.get(b'currency', 0)
