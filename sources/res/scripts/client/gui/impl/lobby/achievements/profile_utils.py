...
def getRating(itemsCache=None, userId=None):
    if isWTREnabled():
        ...
        if userId is not None:
            ...
            return result.get(b'globalRating', 0)
...
def _receiveRating(itemsCache, userId, result):
    ...
    result[b'globalRating'] = yield req.getGlobalRating()
    return
...
def getProfileCommonInfo(dossier):
    ...
    return {b'registrationDate': (b'%s' % getRegionalDateTime(dossier[b'total'][b'creationTime'], DateTimeFormatsEnum.FULLDATE)), 
       b'lastBattleDate': lastBattleDate, 
       b'lastBattleTime': lastBattleTime}
...
def formatPercent(value):
    value = text_styles.concatStylesWithSpace(backport.getNiceNumberFormat(value), b'%')
    return value
...
class AdvancedAchievementsDogTagsBonusPacker(DogTagComponentsUIPacker):
    ...
    def _getBonusIterator(cls, bonus):
        ...
        for dogTagRecord in unlockedBonuses:
            ...
            if component.purpose == ComponentPurpose.COUPLED:
                if component.viewType == ComponentViewType.ENGRAVING and any(value is not None and component.coupledComponentId == value[b'id'] for value in bonus.getValue()):
                    yield (
    ...
    def _getCoupledDogTagTooltip(cls, component):
        return TooltipData(tooltip=None, isSpecial=True, specialAlias=None, specialArgs=[{b'backgroundId': (component.coupledComponentId), b'engravingId': (component.componentId)}])
...
def getAdvancedAchievementsBonusPackersMap():
    ...
    mapping.update({b'customizations': (AdvancedAchievementsCustomizationsBonusPacker()), 
       b'dogTagComponents': (AdvancedAchievementsDogTagsBonusPacker())})
    return mapping
...
def fillDetailsModel(achievement, tooltipData, detailsModel=None, itemsCache=None):
    ...
    with detailsModel.transaction() as model:
        ...
        receivedDate = getRegionalDateTime(timestamp, DateTimeFormatsEnum.FULLDATE) if isCompleted else b''
        receivedShortDate = getRegionalDateTime(timestamp, DateTimeFormatsEnum.SHORTDATE) if isCompleted else b''
...
def getTrophiesData():
    return {b'key': b'trophies', 
       b'type': (AdvancedAchievementType.CATEGORY), 
       b'background': b'trophies', 
       b'iconPosition': (AdvancedAchievementIconPosition.CENTER), 
       b'isTrophy': True}


def createAdvancedAchievementsCatalogInitAchievementIDs(achievementID, achievementCategory):
    if achievementCategory == b'vehicleAchievements':
        categoryDependencies = VEHICLE_ACHIEVEMENTS_DEPENDENCIES
    elif achievementCategory == b'customizationAchievements':
        categoryDependencies = CUSTOMIZATION_ACHIEVEMENTS_DEPENDENCIES
    else:
        raise SoftException((b'Unknown advanced achievement category: {}').format(achievementCategory))
...
def getRewardViewBonusPacker():
    ...
    mapping.update({b'dogTagComponents': (RewardViewDogTagsBonusPacker())})
