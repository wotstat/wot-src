...
_SubModelInfo = typing.NamedTuple(b'_SubModelInfo', [
 (
  b'ID', AchievementsViews),
 (
  b'presenter', SubModelPresenter),
 (
  b'canBeLoaded', typing.Optional[typing.Callable[[], bool]])])
AchievementsViewCtx = namedtuple(b'AchievementsViewCtx', (b'menuName', b'userID', b'closeCallback'))
...
class AchievementMainView(BaseAchievementView):
    __slots__ = (b'__contentPresentersMap', b'__regionName')
    ...
    def _onClose(self):
        if self._ctx.userID is None:
            ...
            g_eventBus.handleEvent(events.Achievements20Event(events.Achievements20Event.CLOSE_SUMMARY_VIEW, {b'databaseID': (self._ctx.userID)}), scope=EVENT_BUS_SCOPE.LOBBY)
        return
