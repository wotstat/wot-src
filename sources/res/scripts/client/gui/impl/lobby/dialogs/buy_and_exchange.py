...
TViewModel = typing.TypeVar(b'TViewModel', bound=DialogWithExchange)
_logger = logging.getLogger(__name__)

class BuyAndExchange(FullScreenDialogView, patched_typing.Generic[TViewModel]):
    __slots__ = (b'_exchangeContent', b'__stateMachine', b'__price')
    ...
    def _onAcceptClicked(self):
        if self.__stateMachine.isCurrentState(BuyAndExchangeStateEnum.BUY_CONTENT):
            ...
            _logger.warning(b'Accept button should be disabled')
        else:
            self.__stateMachine.transit(BuyAndExchangeEventEnum.ACCEPT_CLICKED)
        return

    def _stateToContent(self):
        return {(BuyAndExchangeStateEnum.BUY_CONTENT): (BuyAndExchangeBottomContentType.DEAL_PANEL), (BuyAndExchangeStateEnum.NEED_EXCHANGE): (BuyAndExchangeBottomContentType.DEAL_PANEL), 
