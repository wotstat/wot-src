...
class ServiceRecordCustomizationConfirmDialog(BaseDialog):
    ...
    def _buildResourcesParams(self):
        return {b'titleString': (backport.text(R.strings.dialogs.wotPlusServiceRecordCustomizationDialog.title()))}

    def _setButtons(self):
        with self.viewModel.transaction() as vm:
            buttonsArray = vm.getButtons()
            buttonsArray.clear()
            confirmButton = MonoButtonTemplate(MonoDialogTemplateViewModel.ACTION_CONFIRM, R.strings.dialogs.wotPlusServiceRecordCustomizationDialog.save(), b'Button', ButtonType.PRIMARY, False)
            cancelButton = MonoButtonTemplate(MonoDialogTemplateViewModel.ACTION_CANCEL, R.strings.dialogs.common.cancel(), b'Button', ButtonType.SECONDARY, False)
            self._addButton(confirmButton)
            self._addButton(cancelButton)
        return

    def _onAction(self, event):
        act = event.get(b'action')
