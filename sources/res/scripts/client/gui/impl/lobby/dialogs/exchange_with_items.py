...
class ExchangeWithItems(BuyAndExchange):
    __slots__ = (b'__items', b'__totalPrice', b'_mainContent')
    ...
    def _stateToContent(self):
        return {(BuyAndExchangeStateEnum.EXCHANGE_CONTENT): (BuyAndExchangeBottomContentType.EXCHANGE_PANEL), (BuyAndExchangeStateEnum.EXCHANGE_IN_PROCESS): (BuyAndExchangeBottomContentType.EXCHANGE_PANEL), 
...
class ExchangeToUpgradeDevice(ExchangeWithItems):
    ...
    def __init__(self, device):
        if not device.isUpgradable:
            _logger.warning(b"Device doesn't upgradable!")
