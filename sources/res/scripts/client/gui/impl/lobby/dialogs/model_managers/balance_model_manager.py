...
class BalanceModelManager(object):
    ...
    __slots__ = (b'__stats', b'__currencyIndexes', b'__isGoldAutoPurchaseEnabled', b'__viewModel')
    __CURRENCY_FORMATTER = {(Currency.CREDITS): (NumberFormatType.INTEGRAL), 
       ...
       b'freeXP': (NumberFormatType.INTEGRAL)}
    ...
    def start(self, currencies):
        ...
        g_clientUpdateManager.addCallback(b'stats.freeXP', self.__onFreeXpUpdated)
    ...
    def __addCurrency(self, currency, value):
        ...
        if constants.IS_SINGAPORE and currency in self.__SPECIAL_TOOLTIPS:
            ...
            currencyModel.setSpecialTooltip(b'')
    ...
    def __onFreeXpUpdated(self, value):
        self.__onCurrencyUpdated(b'freeXP', value)
        return
    ...
    def __onCurrencyUpdated(self, currency, value):
        if currency in self.__currencyIndexes:
            ...
            self.viewModel.currency.getItem(index).setValue(self.__getCurrencyFormat(currency, value) if value is not None and self.__isGoldAutoPurchaseEnabled else b'')
        return
    ...
    def __onWalletChanged(self, status):
        ...
        self.__onCurrencyUpdated(b'freeXP', self.__stats.actualFreeXP if status[b'freeXP'] == CurrencyStatus.AVAILABLE else None)
        return
