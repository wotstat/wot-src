...
class ExchangeContent(ExchangeDiscountView):
    ...
    def _updateDiscountInfo(self, model=None):
        ...
        self.__onSelectedAmountChanged({b'gold': (self._viewModel.fromItem.getValue())})
        super(ExchangeContent, self)._updateDiscountInfo(model=model.exchangeRate.discount)
        return

    def __onOpenAllDiscountsWindow(self):
        showAllPersonalDiscountsWindow(exchangeRateType=self._exchangeRate.getExchangeRateName, selectedValue={b'currency': (self._viewModel.toItem.getValue())})
        return
    ...
    def __updateTooltipInformation(self):
        g_eventBus.handleEvent(events.ExchangeRatesDiscountsEvent(events.ExchangeRatesDiscountsEvent.ON_SELECTED_AMOUNT_CHANGED, {b'amount': (self._viewModel.fromItem.getValue())}), scope=EVENT_BUS_SCOPE.LOBBY)
        return
...
class ExchangeMoneyInfo(object):
    ...
    def __init__(self, currencyType=b''):
        if currencyType not in Currency.ALL:
            _logger.error(b'%s is not Currency!', currencyType)
