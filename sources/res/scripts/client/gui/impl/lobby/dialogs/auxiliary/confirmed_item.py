...
CTX_VEHICLE_INV_ID = b'vehicleInvID'

class ConfirmedItem(object):
    __slots__ = (b'_item', b'_ctx', b'_warnings')
...
class ConfirmedOptDevice(ConfirmedArtefact):
    ...
    def getOptItemDescKey(self):
        ...
        if isDeluxe:
            if isDeluxeEnabled:
                if isActive:
                    return b'itemWotPlus'
                return b'itemDeluxeWotPlus'
            return b'itemDeluxe'
        ...
        if isRegular:
            if isActive:
                return b'itemWotPlus'
            return b'itemWithDemountKitWotPlus'
        return b'itemWithDemountKit'
