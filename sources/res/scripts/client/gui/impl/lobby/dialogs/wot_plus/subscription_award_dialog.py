...
class SubscriptionAwardDialog(BaseDialog):
    ...
    def __buildWotPlusCoreContent(self):
        content = {b'descriptionStringParams': (json.dumps({b'date': (getRegionalDateTime(self._expirationTime or 0, DateTimeFormatsEnum.SHORTDATE))}))}
        coreBenefits = getAvailableCoreBonuses(self._storage)
        benefitsData = self.__buildBenefits(coreBenefits)
        content.update({b'benefits': (json.dumps(benefitsData))})
        return content

    def __buildWotPlusProContent(self):
        content = {b'descriptionStringParams': (json.dumps({b'date': (getRegionalDateTime(self._expirationTime or 0, DateTimeFormatsEnum.SHORTDATE)), 
                                        b'planInterval': (backport.text(PERIODICITY_TO_RESOURCE[self._periodicity or WotPlusPeriodicityEnum.P6MONTHS]))}))}
        ...
        content.update({b'benefits': (json.dumps(coreBenefitsData)), b'emphasizedBenefits': (json.dumps(proBenefitsData))})
        return content
    ...
    def __buildWotPlusCoreResources(self):
        ...
        return {b'titleString': titleString, 
           b'iconImage': (backport.image(R.images.gui.maps.icons.subscription.activation_dialog.core())), 
           b'iconGlowImage': (backport.image(R.images.gui.maps.icons.subscription.activation_dialog.icon_glow_core())), 
           b'descriptionString': (backport.text(R.strings.dialogs.wotPlusActivationDialog.core.description()))}

    def __buildWotPlusProResources(self):
        return {b'titleString': (backport.text(R.strings.dialogs.wotPlusActivationDialog.pro.heading())), 
           b'iconImage': (backport.image(R.images.gui.maps.icons.subscription.activation_dialog.pro())), 
           b'iconGlowImage': (backport.image(R.images.gui.maps.icons.subscription.activation_dialog.icon_glow_pro())), 
           b'descriptionString': (backport.text(R.strings.dialogs.wotPlusActivationDialog.pro.description()))}
    ...
    def __buildBenefits(self, benefitsList):
        ...
        for benefit in benefitsList:
            ...
            benefitsData.append({b'type': (packedBonus.getName()), b'label': (packedBonus.getLabel())})
    ...
    def _sendNotificationsOnClose(self):
        ...
        elif self._messageType == SYS_MESSAGE_TYPE.wotPlusUpgrade.index():
            return
        ...
        self._systemMessages.proto.serviceChannel.pushClientMessage({b'expiryTime': (self._expirationTime), 
           b'periodicity': (self._periodicity)}, sysMessageType)
