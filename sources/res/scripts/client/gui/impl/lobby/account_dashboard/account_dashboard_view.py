...
class Feature(CONST_CONTAINER):
    ...
    PREMIUM_ACCOUNT = b'premiumAccount'
    BONUS_XP = b'bonusXp'
    RESERVER_STOCK = b'reserveStock'
    PREMIUM_QUESTS = b'premiumQuests'
    EXCLUDED_MAPS = b'excludedMaps'
    DOG_TAGS = b'dogTags'
    PARENTAL_CONTROL = b'parentalControl'


class SOUNDS(CONST_CONTAINER):
    STATE_PLACE = b'STATE_hangar_place'
    STATE_PLACE_AD = b'STATE_hangar_place_acc_dashboard'
    EVENT_ENTER = b'ev_acc_dashboard_enter'
    EVENT_EXIT = b'ev_acc_dashboard_exit'


class AccountDashboardView(ViewImpl):
    __slots__ = (b'_features', b'modelDataControllers', b'_wotPlusUILogger')
    ...
    def createToolTipContent(self, event, contentID):
        ...
        _logger.error(b'Crew header view tried creating invalid tooltip with contentID %d', contentID)
        return
