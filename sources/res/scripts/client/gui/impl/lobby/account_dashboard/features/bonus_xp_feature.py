...
class BonusXPFeature(FeatureItem):
    ...
    def __startListening(self):
        ...
        g_clientUpdateManager.addCallbacks({b'applyAdditionalXPCount': (self.__onUpdate)})
    ...
    def __updateModel(self, model=None):
        ...
        submodel.setIsEnabled(config.get(b'enabled', False))
        submodel.setMultiplier(int(config.get(b'bonusFactor')))
        submodel.setUsesLeft(self.__itemsCache.items.stats.applyAdditionalXPCount)
        submodel.setTotalUses(config.get(b'applyCount') if self.__itemsCache.items.stats.isActivePremium(PREMIUM_TYPE.PLUS) else 0)
        return
