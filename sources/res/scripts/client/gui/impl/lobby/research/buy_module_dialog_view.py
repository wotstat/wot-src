...
class BuyModuleDialogView(FullScreenDialogView):
    __slots__ = (b'_module', b'_previousModule', b'_returnedData', b'_currency', b'_isAutoSellEnabled', b'_mountDisabledReason')
    ...
    def createToolTipContent(self, event, contentID):
        _logger.debug(b'BuyModule::createToolTipContent')
        if contentID == R.views.lobby.research.InsufficientCreditsTooltip():
            missingAmount = event.getArgument(b'missingAmount')
        ...
        if contentID == R.views.lobby.research.SoldModuleInfoTooltip():
            ...
            _logger.debug(b'[BuyModuleView] Compatible vehicles: %s', compatibleVehicles)
    ...
    def _setTitleBody(self, model):
        label = b'confirmBuyAndInstall' if self._mayInstall() else b'confirmBuy'
        model.setTitleBody(R.strings.dialogs.dyn(label).title())
        return

    def _setTitle(self, model):
        self._setTitleBody(model)
        self._setTitleArgs(model.getTitleArgs(), ((b'moduleName', self._module.userName),))
        return

    def _setMountDisabledReason(self, model):
        reason = b'none' if self._mayInstall() else self._mountDisabledReason
        reasonEnum = MountDisabledReason(b'invalid')
        try:
            reasonEnum = MountDisabledReason(reason)
        except ValueError as e:
            _logger.error(b'[BuyModuleView] Unexpected string received for "reason" why module cannot be installed. ViewModel property will be set to MountDisabledReason.Invalid: %s', e)
    ...
    def _onAcceptClicked(self, sellPreviousModule):
        _logger.debug(b'[BuyModuleView] in _onAcceptClicked - sellPreviousModule %s', sellPreviousModule)
