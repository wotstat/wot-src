...
class YearlyRewardsPage(PageSubModelPresenter):
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.lobby.comp7.tooltips.SeasonPointTooltip():
            params = {b'state': (SeasonPointState(event.getArgument(b'state')))}
    ...
    def __setCards(self, model):
        ...
        for rewardsData in sorted(self.__comp7Controller.getYearlyRewards().main, key=(lambda data: data[b'cost'])):
            currentPointsStates = pointStatesGenerator.getNext(rewardsData[b'cost'] - prevRewardsCost)
            ...
            self.__setRewards(cardModel, rewardsData[b'bonus'])
    ...
    def __getRewardStateForCard(self, cardPointsState):
        ...
        if not any(pointState == SeasonPointState.NOTACHIEVED for pointState in cardPointsState):
