...
_TYPE_TO_GF_ENUM = {b'page': (Types.PAGE), 
   b'maps': (Types.MAPS), 
   b'seasonVehicles': (Types.SEASONVEHICLES)}

class WhatsNewView(ViewImpl, IGlobalListener):
    __slots__ = (b'__isIntro',)
    ...
    def __setComp7IntroShown(self):
        if not self.__settingsCore.isReady:
            _logger.error(b'Can not save Comp7IntroShown settings: settings are not ready')
            return
        defaults = AccountSettings.getFilterDefault(GUI_START_BEHAVIOR)
        stateFlags = self.__settingsCore.serverSettings.getSection(GUI_START_BEHAVIOR, defaults)
