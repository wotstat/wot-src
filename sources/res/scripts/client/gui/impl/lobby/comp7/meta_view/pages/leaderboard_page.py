...
def _getBackportContextMenuData(event):
    contextMenuArgs = {b'dbID': (event.getArgument(b'spaID')), 
...
class LeaderboardPage(PageSubModelPresenter):
    __slots__ = (b'__elitePosition', b'__loadingStateManager', b'__requestedPageData', b'__lastUpdateTime')
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.lobby.comp7.tooltips.DivisionTooltip():
            params = {b'rank': (Rank(event.getArgument(b'rank'))), b'division': (Division(event.getArgument(b'division'))), 
               ...
               b'type': (event.getArgument(b'type')), 
               b'elitePercent': (event.getArgument(b'elitePercent'))}
            return DivisionTooltip(params=params)
        if contentID == R.views.lobby.comp7.tooltips.LeaderboardRewardTooltip():
            place = event.getArgument(b'place', 0)
    ...
    def __getRecordModel(self, record):
        ...
        model.setClanTag(record.getClanTag() or b'')
        model.setClanTagColor(record.getClanColor() or b'')
...
class _LoadingStateManager(object):
    __REQUESTS_COUNT_KEY = b'requestsCount'
    __ERRORS_KEY = b'errors'
    __generation = 0
    ...
    def __init__(self, viewModel):
        ...
        _logger.debug(b'Created new loading state manager, generation %d', self.__generation)
        return
    ...
    def finishRequest(self, chainMark, isSuccessfull):
        if chainMark not in self.__registeredChains:
            _logger.error(b'Finished not registered request')
            return
