...
class _SelectionProvider(object):
    __slots__ = (b'__mainStepsBorder', b'__mainStepsSelection', b'__multiStepsSelection', b'__postProgression')
...
class PostProgressionBaseComponentView(ViewImpl):
    __slots__ = (b'onCustomProgressionState', b'onViewRendered', b'_selectionProvider', b'_vehicle', b'_eventManager')
    ...
    def _updateViewModel(self, viewModel, availabilityReason, completion):
        viewModel.setVehicleRole(self._vehicle.roleLabel if self._vehicle.role else b'')
        return
    ...
    def _parseTooltipEvent(self, event):
        stepId = event.getArgument(b'stepId')
        modId = event.getArgument(b'modificationId')
    ...
    def __onMainStepSelectClick(self, args):
        self._selectionProvider.selectMainStep(int(args[b'stepID']))
