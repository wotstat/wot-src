...
class PostProgressionCfgComponentView(PostProgressionBaseComponentView):
    ...
    __slots__ = (b'onGoBackAction', b'onResearchAction', b'__intCD', b'__balance', b'__creditsRate', b'__togglingSteps', b'__lockTogglingCallbackID')
    ...
    def _addListeners(self):
        ...
        g_clientUpdateManager.addCallbacks({(b'stats.{}').format(c): self.__updateMoney for c in ExtendedCurrency.ALL})
        return
    ...
    def _onMainStepActionClick(self, args):
        ...
        action = vehicle.postProgression.getStep(int(args[b'stepID'])).action.getServerAction(factory, vehicle)
        yield factory.asyncDoAction(action)
        return

    @adisp_process
    def _onPrebattleSwitchToggleClick(self, args):
        stepID = int(args[b'stepID'])
        if stepID in self.__togglingSteps:
            ...
            action = factory.getAction(factory.SWITCH_PREBATTLE_AMMO_PANEL_AVAILABILITY, self._vehicle, groupID, args[b'active'])
            yield factory.asyncDoAction(action)
    ...
    def _onMultiStepActionClick(self, args):
        vehicle = self._vehicle
        stepID, modificationID = int(args[b'stepID']), int(args[b'modificationID'])
