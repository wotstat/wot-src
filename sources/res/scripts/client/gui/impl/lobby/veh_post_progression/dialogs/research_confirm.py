...
class PostProgressionResearchConfirm(FullScreenDialogView[ResearchStepsDialog]):
    __slots__ = (b'_buyContent', b'_mainContent', b'__stepIDs', b'__vehicle', b'__price', b'__steps', b'__xpBalance', b'__postProgression')
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.__stepIDs = kwargs.get(b'stepIDs', ())
        ...
        if not self.__vehicle.postProgressionAvailability():
            ...
            for stepID in self.__stepIDs:
                ...
                if step.isRestricted() or step.isReceived():
                    _logger.error(b'restricted or already unlocked step in chain %s', stepID)
                    return False
...
class PostProgressionResearchMainContent(BaseSubModelView):
    ...
    def __fillModificationModel(self, step):
        ...
        shortModModel.setModificationType(str(_ACTION_TYPE_TO_MODIFICATION_TYPE.get(step.action.actionType, b'')))
