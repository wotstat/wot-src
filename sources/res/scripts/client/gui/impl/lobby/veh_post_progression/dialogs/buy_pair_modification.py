...
class BuyPairModificationDialog(BuyAndExchange[AmmunitionBuyModel]):
    __slots__ = (b'_buyContent', b'_mainContent', b'__vehicle', b'__toStepID', b'__price', b'__modID', b'__item')
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.__vehicle = kwargs.get(b'vehicle', None)
        self.__toStepID = kwargs.get(b'stepID', 0)
        self.__price = EXT_MONEY_ZERO
        self.__modID = kwargs.get(b'modID', 0)
        self.__item = None
        self.__updateData()
        super(BuyPairModificationDialog, self).__init__(settings, self.__price, kwargs.get(b'startState', None))
        return
    ...
    def _onLoading(self, *args, **kwargs):
        ...
        self._mainContent = BuyPairModificationMainContent(viewModel=self.viewModel.mainContent, items=[
         self.__item.getModificationByID(self.__modID)], vehicleInvID=self.__vehicle.invID, itemsType=ACTION_TYPE_TO_FITTING_TYPE.get(self.__item.actionType, b''))
    ...
    def __updateData(self):
        ...
        if not self.__vehicle.postProgressionAvailability():
            ...
            if not step.isReceived() or step.action.getPurchasedID() == self.__modID:
                _logger.error(b'not received or already purchased pair %s', self.__toStepID)
                return False
