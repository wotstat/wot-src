...
class EntryPointTooltip(ViewImpl):
    ...
    def _getEvents(self):
        return (
         (
          self.__resourceWell.onNumberRequesterUpdated, self.__fillRewardInfo),
         (
          self.__resourceWell.onEventUpdated, self.__fillEventTime))
    ...
    def __getRewardCountState(self):
        ...
        if regularRewardCount:
            return (EventState.REGULARREWARDAVAILABLE, regularRewardCount)
        return (EventState.NOREWARDS, 0)
