...
class NationChangeScreen(ViewImpl):
    ...
    _DOG_SOUND = b'rudy'
    _HANGAR_SOUND_FILTERED_STATE_NAME = b'STATE_hangar_filtered'
    _HANGAR_SOUND_FILTERED_STATE_ON = b'STATE_hangar_filtered_on'
    _HANGAR_SOUND_FILTERED_STATE_OFF = b'STATE_hangar_filtered_off'
    __LAYOUTS_IN_SETUP = 4
    __slots__ = (b'__currentVehicle', b'__targetVehicle', b'__icons')
    ...
    def __init__(self, viewKey, ctx=None):
        ...
        vehicle = self.__itemsCache.items.getItemByCD(ctx.get(b'vehicleCD', None))
    ...
    def createToolTip(self, event):
        if event.contentID == R.views.common.tooltip_window.backport_tooltip_content.BackportTooltipContent():
            ...
            if tooltipId == TOOLTIPS_CONSTANTS.TANKMAN:
                ...
                args = (self.getParentWindow(), int(event.getArgument(b'intCD')))
    ...
    def __getBackportTooltipData(self, event):
        tooltipId = event.getArgument(b'tooltipId')
        vehicleIntCD = int(event.getArgument(b'vehicleIntCD'))
        if tooltipId == TOOLTIPS_CONSTANTS.CAROUSEL_VEHICLE:
            args = [
             vehicleIntCD]
        elif tooltipId == TOOLTIPS_CONSTANTS.NATION_CHANGE_BATTLE_BOOSTER:
            args = [
             vehicleIntCD, int(event.getArgument(b'intCD')), int(event.getArgument(b'layoutIDx'))]
        else:
            args = [
             vehicleIntCD, int(event.getArgument(b'intCD'))]
    ...
    def __setCrewViewModelData(self, tankSlotVM, guiVh):
        ...
        isDogInCrew = b'dog' in guiVh.tags
    ...
    def __addListeners(self):
        ...
        g_clientUpdateManager.addCallbacks({b'inventory.1.compDescr': (self.__onVehiclesInInventoryUpdate)})
    ...
    def __removeListeners(self):
        ...
        self.viewModel.onHangarBtnClick -= self.__onHangarBtnClick
        self.viewModel.onDogClick -= self.__onDogClick
        g_clientUpdateManager.removeCallback(b'inventory.1.compDescr', self.__onVehiclesInInventoryUpdate)
        self.__lobbyContext.getServerSettings().onServerSettingsChange -= self.__onServerSettingsChange
        return

    @serverSettingsChangeListener(b'isNationChangeEnabled', SERVER_SETTINGS_KEY)
