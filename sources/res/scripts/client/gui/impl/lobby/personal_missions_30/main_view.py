...
class MainView(ViewImpl):
    ...
    def __onPmEventsSync(self, diff):
        ...
        pm3Quests = {}
        if diff:
            pm3Quests = diff.get(b'potapovQuests', {}).get(b'pm3', {}).get((b'selected', b'_r'), set())
            if diff.get(b'pm3_progress', {}):
                for questName in diff.get(b'pm3_progress', {}):
    ...
    def __claimReward(self, detailName):
        ...
        if res and res.success:
            ...
            showPM30RewardsWindow(ctx={b'questID': (quest.getID()), 
