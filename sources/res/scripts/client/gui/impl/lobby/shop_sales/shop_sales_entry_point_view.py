...
class ShopSalesEntryPointView(ViewImpl):
    ...
    def _getEvents(self):
        return (
         (
          self.__shopSales.onSettingsChanged, self.__updateViewModel),
         (
          self.__shopSales.onPhaseChanged, self.__updateViewModel),
         (
          self.viewModel.onActionClick, self.__openShopSales))
