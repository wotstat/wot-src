...
class CarouselVehicleTooltipView(ViewComponent[CarouselVehicleTooltipModel]):
    ...
    def _onLoading(self, *args, **kwargs):
        ...
        if vehicle is None:
            _logger.error(b'No vehicle for with inventoryId %s for displaying a tooltip', self._inventoryId)
            return
        else:
...
def getUnsuitable2queueTooltip(parent, event, validationResult, resPath, **kwargs):
    ...
    if validationResult.restriction == PRE_QUEUE_RESTRICTION.LIMIT_LEVEL:
        levelStr = toRomanRangeString(validationResult.ctx[b'levels'])
        ...
        typeSubStr = validationResult.ctx[b'forbiddenType']
        ...
        classSubStr = getTypeUserName(validationResult.ctx[b'forbiddenClass'], False)
