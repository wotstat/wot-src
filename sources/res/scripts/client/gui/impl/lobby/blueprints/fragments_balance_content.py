...
class FragmentsBalanceContent(ViewImpl):
    ...
    def createToolTip(self, event):
        if event.contentID == R.views.common.tooltip_window.backport_tooltip_content.BackportTooltipContent():
            ...
            fragmentCD = int(event.getArgument(b'fragmentCD'))
    ...
    def _initialize(self, *args, **kwargs):
        ...
        if not nationName:
            logger.warning(b'Can not get a vehicle object and nation name')
        ...
        with self.viewModel.transaction() as model:
            ...
            model.setAllianceName(nations.ALLIANCES_TAGS_ORDER[allianceId].replace(b'-', b'_'))
    ...
    def __getFragmentCount(self):
        ...
        if self.__vehicle is None:
            logger.warning(b'Can not get a vehicle compact descriptor')
        else:
