...
class BlueprintsConversionView(FullScreenDialogView):
    ...
    def createToolTip(self, event):
        if event.contentID == R.views.common.tooltip_window.backport_tooltip_content.BackportTooltipContent():
            ...
            itemCD = event.getArgument(b'itemCD')
    ...
    def _initialize(self):
        ...
        g_clientUpdateManager.addCallbacks({b'serverSettings.blueprints_config.levels': (self.__onBlueprintsSettingsChanged), 
           b'serverSettings.blueprints_config.isEnabled': (self.__onBlueprintsModeChanged), 
           b'serverSettings.blueprints_config.useBlueprintsForUnlock': (self.__onBlueprintsModeChanged), 
           b'blueprints': (self.__onBlueprintsBalanceChanged)})
        return
    ...
    def _onLoading(self, *args, **kwargs):
        with self.viewModel.transaction() as model:
            ...
            self._setTitleArgs(model.getTitleArgs(), (
             (
              b'vehName', self.__vehicle.shortUserName.replace(b' ', b'&nbsp;')),))
            ...
            model.setAllianceName(allianceName.replace(b'-', b'_'))
    ...
    def __onSelectItem(self, args=None):
        selectedItemIdx = int(args.get(b'selectedItem'))
        with self.viewModel.transaction() as model:
    ...
    def __onSliderShift(self, args=None):
        index = int(args.get(b'index'))
        newCount = int(args.get(b'newCount'))
        with self.viewModel.transaction() as model:
    ...
    def __setBalanceBlock(self, fragmentsBalance):
        nationName = self.__vehicle.nationName if self.__vehicle is not None else b''
