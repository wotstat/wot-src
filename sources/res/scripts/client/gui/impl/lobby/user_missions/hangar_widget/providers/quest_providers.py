    ...
    def getQuests(self):
        if not self._checkEnabled():
            ...
            for raw in rawQuests:
                levelKey = (b'{}_{}').format(self._KEY, raw.getLevel())
                weightConfig = self.config.getWeightByName(levelKey)
                if weightConfig is None:
                    _logger.error(b'Invalid weightConfig %s', self._KEY)
                    return []
                ...
                if raw.isBonus():
                    dailyQuest.setItemType(b'bonus')
...
class PremiumQuestProvider(QuestProviderBase):
    ...
    def getQuests(self):
        if not self._checkEnabled():
            ...
            if weightConfig is None:
                _logger.error(b'Invalid weightConfig %s', ConfigKeys.PREMIUM_DAILY)
                return []
