...
class DailyMissionsBlockPresenter(BaseMissionsBlockPresenter[DailyMissionsBlockModel]):
    ...
    def _onServerSettingsChanged(self, diff=None):
        ...
        if DAILY_QUESTS_CONFIG in diff:
            ...
            rerollStateChanged = b'rerollEnabled' in dqDiff and dqDiff[b'rerollEnabled'] is not self._isRerollEnabled
            stateChanged = b'enabled' in dqDiff and dqDiff[b'enabled'] is not self._isBlockEnabled
            rerollTimeoutChanged = b'rerollTimeout' in dqDiff and dqDiff[b'rerollTimeout'] != self._rerollTimeout
            with self.viewModel.transaction() as tx:
        ...
        if PremiumConfigs.PREM_QUESTS in diff:
            ...
            stateChanged = b'enabled' in premDiff and premDiff[b'enabled'] is not self._hasPremiumMissions
            if stateChanged:
                with self.viewModel.transaction() as tx:
    ...
    @decorators.adisp_process(b'dailyQuests/waitReroll')
    @args2params(str)
