...
DEFAULT_EVENT_NAME = b'default_stronghold_event_name'
...
def getSettings(settingName, defaultValue=None):
    ...
    if eventName is None:
        _logger.info(b'Can not to get setting by name %s because event name is empty. Check settings of the event', settingName)
        return
    else:
...
def setSettings(settingName, value):
    ...
    eventName = getStrongholdEventName(useDefault=False)
    if eventName is None:
        _logger.info(b'Can not to set setting by name %s because event name is empty. Check settings of the event', settingName)
        return False
    else:
        section = settings.get(eventName, None)
        if section is None:
