...
class ProgressiveRewardAwardView(ViewImpl):
    ...
    __slots__ = (b'__items', b'__bonuses', b'__specialRewardType', b'__currentStep')
    ...
    def _initialize(self, bonuses, specialRewardType, currentStep):
        ...
        g_clientUpdateManager.addCallbacks({b'blueprints': (self.__update), 
           b'serverSettings.blueprints_config': (self.__update), 
           b'serverSettings.progressive_reward_config': (self.__update)})
    ...
    def __setSteps(self, currentStep):
        ...
        if progressive is None:
            _logger.warning(b'Progressive config is missing on server!')
            return
        else:
    ...
    def __setBonuses(self, bonuses):
        with self.getViewModel().transaction() as tx:
            ...
            for index, reward in enumerate(bonuses):
                ...
                self.__items[index] = createTooltipData(tooltip=ttTarget.get(b'tooltip', None), isSpecial=ttTarget.get(b'isSpecial', False), specialAlias=ttTarget.get(b'specialAlias', b''), specialArgs=ttTarget.get(b'specialArgs', None))
    ...
    def __onSpecialActionButtonClick(self, responseDict):
        congratsType = responseDict.get(b'congratsType')
        if congratsType in BLUEPRINTS_CONGRAT_TYPES:
            vehicleCD = _getVehicleCD(responseDict.get(b'congratsSourceId'))
    ...
    def __getBackportTooltipData(self, event):
        ...
        if tooltipId is None:
            ...
            if tooltipId == BlueprintScreenTooltips.TOOLTIP_BLUEPRINT:
                vehicleCD = _getVehicleCD(event.getArgument(b'vehicleCD'))
                if vehicleCD is not None:
