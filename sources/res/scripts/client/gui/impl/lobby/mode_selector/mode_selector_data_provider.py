...
class ModeSelectorDataProvider(IGlobalListener):
    __slots__ = (b'onListChanged', b'_items')
        ...
        if selected is None:
            ...
            for idx, item in enumerate(self.itemList):
                ...
                if item.isSelectable:
                    isSelected = item.modeName == selected.getData() or getattr(item, b'isSelected', (lambda : False))()
    ...
    def _updateItemsPosition(self):
        ...
        for item in self.itemList:
            ...
            while counts[columnId] >= self.MAX_COLUMN_SIZE and columnId != ModeSelectorColumns.COLUMN_0:
                _logger.warning(b"Incorrect settings. Can't add item %s to the column %s. Max column size has been reached. Move it to the previous column", item.modeName, columnId)
                columnId = columnId - 1
        ...
        for key, value in counts.iteritems():
            if key == ModeSelectorColumns.COLUMN_0 and value > 1:
                _logger.warning(b'There must be a sole item in the first column')
            if key != ModeSelectorColumns.COLUMN_0 and value > self.MAX_COLUMN_SIZE:
                _logger.warning(b'Max column size  %s has been overflown', self.MAX_COLUMN_SIZE)
