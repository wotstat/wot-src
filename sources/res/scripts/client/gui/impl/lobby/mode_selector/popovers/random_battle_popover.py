...
_GAMEPLAY_STANDARD = b'gameplay_standard'
_subLocaleBySettingID = OrderedDict()
_subLocaleBySettingID[_GAMEPLAY_STANDARD] = b'default'
_subLocaleBySettingID[GAME.GAMEPLAY_DOMINATION] = b'domination'
_subLocaleBySettingID[GAME.GAMEPLAY_ASSAULT] = b'assault'
_subLocaleBySettingID[GAME.GAMEPLAY_EPIC_STANDARD] = b'epicStandard'
_subLocaleBySettingID[GAME.GAMEPLAY_DEV_MAPS] = b'devMaps'

class RandomBattlePopover(PopOverViewImpl):
    __slots__ = (b'__initialItems', b'__currentItems')
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.lobby.mode_selector.tooltips.AlertTooltip():
            return AlertTooltip(event.getArgument(b'header', b''), event.getArgument(b'body', b''), event.getArgument(b'alert', b''))
        else:
            return
    ...
    def _createItem(self, setting, title, tooltip, enabled):
        alert = backport.text(tooltip.attention()) if tooltip.dyn(b'attention') else b''
