...
class MarathonRewardWindowView(ViewImpl):
    ...
    def __init__(self, ctx, *args, **kwargs):
        ...
        self.__marathon = self.marathonController.getMarathon(ctx.get(b'marathonPrefix'))
    ...
    def __formatterRewards(self, rewards, addTooltip=False):
        ...
        for reward in rewards:
            ...
            model.setIcon(getRewardImage(reward.images[b'big']))
    ...
    def __convertRewards(self, ctx):
        self.__rewards = ctx.get(b'rewards', [])
        self.__rewards = splitBonuses(self.__rewards)
        crewRewards = ctx.get(b'crewRewards', None)
        if crewRewards:
