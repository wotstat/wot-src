...
_MARATHON_PREFIX = b'winter_marathon'
...
class MarathonEntryPoint(ViewImpl):
    ...
    def __marathonFilterFunc(self, q, postfix=b''):
        marathonEvent = self._marathonsCtrl.getMarathon(_MARATHON_PREFIX)
        currentPhase, _ = marathonEvent.getMarathonProgress()
        return q.getID().startswith((b'{0}s{1}{2}').format(marathonEvent.tokenPrefix, currentPhase + 1, postfix))

    def __marathonPostFilterFunc(self, q, postfix=b''):
        marathonEvent = self._marathonsCtrl.getMarathon(_MARATHON_PREFIX)
        return q.getID().startswith((b'{0}{1}s').format(marathonEvent.tokenPrefix, postfix)) and q.isAvailable()[0]
