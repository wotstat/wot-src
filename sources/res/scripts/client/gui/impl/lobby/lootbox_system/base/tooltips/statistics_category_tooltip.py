...
class StatisticsCategoryTooltipView(ViewImpl):
    __slots__ = (b'__bonusesCategory', b'__eventName')
...
def _mergeCustomizations(customizations):
    ...
    for customization in customizations:
        unicItem = (
         customization[b'id'], customization[b'custType'])
        if customization.get(b'compensatedNumber', 0):
            result.append(customization)
        elif unicItem not in checking:
            checking[unicItem] = customization
            result.append(customization)
        else:
            checking[unicItem][b'value'] += customization.get(b'value', 0)
...
def _packCrewMembers(rewards, model):
    for recruit, count in _iterRecruits(rewards):
        bonusModel = StatisticsCategoryTooltipBonusModel()
        bonusModel.setName(b'tmanToken')
        bonusModel.setValue(recruit.getGroupName())
        bonusModel.setIcon((b'tank{}man').format(b'wo' if recruit.isFemale() else b''))
        bonusModel.setCount(count)
...
def _packBlueprints(rewards, model):
    ...
    for blueprint in _mergeNationalBlueprints(blueprints):
        ...
        bonusModel.setIcon((isinstance(blueprint, LootBoxRandomNationalBonus) or blueprint.getImageCategory)() if 1 else b'randomNational')
        label = b''
...
def _iterVehicles(rewards, itemsCache=None):
    return ((itemsCache.items.getItemByCD(vehicleCD), compensationData.get(b'compensatedNumber', 0)) for vehiclesData in rewards.get(Type.VEHICLES.value, {}) for vehicleCD, compensationData in vehiclesData.iteritems())


@dependency.replace_none_kwargs(customization=ICustomizationService)
def _iter3DStyles(rewards, customization=None):
    return ((s, data.get(b'compensatedNumber', 0)) for s, data in _iterStyles(rewards) if s.itemTypeName == Type.STYLE.value and s.is3D and not s.isLockedOnVehicle)


@dependency.replace_none_kwargs(customization=ICustomizationService)
def _iter2DStyles(rewards, customization=None):
    return ((s, data.get(b'compensatedNumber', 0), data.get(b'value', 0)) for s, data in _iterStyles(rewards) if s.itemTypeName == Type.STYLE.value and not s.is3D and not s.isLockedOnVehicle)


def _iterRecruits(rewards):
    for tokenID, tokenData in rewards.get(b'tokens').iteritems():
        recruit = getRecruitInfo(tokenID)
        if recruit is not None:
            yield (
             recruit, tokenData[b'count'])
...
def _iterStyles(rewards, customization=None):
    return ((customization.getItemByID(GUI_ITEM_TYPE.STYLE, cData[b'id']), cData) for cData in (c for c in rewards.get(Type.CUSTOMIZATIONS.value)) if cData[b'custType'] == GUI_ITEM_TYPE_NAMES[GUI_ITEM_TYPE.STYLE])
...
def _iterCustomizations(rewards, customization=None):
    ...
    return ((customization.getItemByID(getItemTypeID(cData[b'custType']), cData[b'id']), cData[b'value']) for cData in (c for c in rewards.get(Type.CUSTOMIZATIONS.value, {})) if cData[b'custType'] not in excludedTypes)


@dependency.replace_none_kwargs(customization=ICustomizationService)
def _iterAttachments(rewards, customization=None):
    return ((customization.getItemByID(getItemTypeID(cData[b'custType']), cData[b'id']), cData[b'value']) for cData in (c for c in rewards.get(Type.CUSTOMIZATIONS.value, {})) if cData[b'custType'] == GUI_ITEM_TYPE_NAMES[GUI_ITEM_TYPE.ATTACHMENT])


@dependency.replace_none_kwargs(itemsCache=IItemsCache)
def _iterItems(rewards, itemsCache=None):
    return ((itemsCache.items.getItemByCD(itemCD), count) for itemCD, count in rewards.get(b'items', {}).iteritems())


@dependency.replace_none_kwargs(goodiesCache=IGoodiesCache)
def _iterGoodies(rewards, goodiesCache=None):
    return ((goodiesCache.getGoodie(goodieID), data[b'count']) for goodieID, data in rewards.get(b'goodies', {}).iteritems())
...
def _isCrewBook(item):
    return item.itemTypeName == b'crewBook'
