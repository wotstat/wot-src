...
class CompareAmmunitionPanelView(ViewImpl):
    __slots__ = (b'__ammunitionPanel', b'__vehItem')
    ...
    def __onPanelSlotClear(self, args):
        slotID = int(args.get(b'slotId'))
        sectionName = args.get(b'sectionType')
    ...
    def __onPanelSectionSelected(args):
        selectedSection = args.get(b'selectedSection')
        if selectedSection == TankSetupConstants.TOGGLE_SHELLS:
            selectedSlot = int(args[b'selectedSlot'])
    ...
    def __onDragDropSwap(self, args):
        sectionName = args[b'sectionType']
        leftID, rightID = sorted((int(args[b'dragId']), int(args[b'dropId'])))
        if sectionName == TankSetupConstants.OPT_DEVICES:
