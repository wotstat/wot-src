...
class SkillSelectView(ViewImpl):
    __slots__ = (b'__paramsView', b'__toolTipMgr', b'__cmpConf', b'__skillsManager', b'__selectedSkills', b'__vehicle')
    ...
    def _onClick(self, event):
        self.__updateSkills(int(event.get(b'rowIndex')), int(event.get(b'skillIndex')), event.get(b'skillType'))
        return
    ...
    def __updateActionButtons(self, vm):
        ...
        for idx, (_, skills) in defaultSkills.items():
            _, selectedSkills = self.__selectedSkills.get(idx, (b'', []))
