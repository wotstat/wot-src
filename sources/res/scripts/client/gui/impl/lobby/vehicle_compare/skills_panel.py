...
class CompareSkillsPanelView(ViewImpl):
    __slots__ = (b'__toolTipMgr', b'__cmpConf')
    ...
    def createToolTip(self, event):
        if event.contentID == R.views.common.tooltip_window.backport_tooltip_content.BackportTooltipContent():
            ...
            if tooltipId == TooltipConstants.SKILL:
                args = [
                 str(event.getArgument(b'skillName')), event.getArgument(b'roleName'), None,
                 event.getArgument(b'level')]
    ...
    def _fillViewModel(self):
        ...
        skillsToShow = {skill for skill in selectedSkills.values()}
