...
class VehicleForChooseTooltipContent(ViewImpl):
    ...
    def _initialize(self, *args, **kwargs):
        ...
        with self.viewModel.transaction() as tx:
            vList = Array()
            for vehData in kwargs.get(b'vehicles', []):
                vR = VehicleInfoModel()
                vR.setIsElite(vehData.get(b'isElite', False))
                vR.setVehicleName(vehData.get(b'vehicleName', b''))
                vR.setVehicleType(vehData.get(b'vehicleType', b''))
                vR.setVehicleLvl(vehData.get(b'vehicleLvlNum', 1))
