    ...
    def asyncPack(self, bonus, callback=None):
        ...
        for tID in bonusTokens.iterkeys():
            ...
            if not iconSmallPath:
                _logger.warning(b"Couldn't obtain big image for %s!", tID)
            elif not iconBigPath:
                _logger.warning(b"Couldn't obtain small image for %s!", tID)
            else:
        ...
        for tID in bonusTokens.iterkeys():
            if tID.startswith(OFFER_TOKEN_PREFIX):
                ...
                if hasRent:
                    ...
                    result.append(createTooltipData(isSpecial=True, specialAlias=VEH_FOR_CHOOSE_ID, specialArgs={b'vehicles': uiVehs}))
                    continue
...
class _TmanTemplateProductBonusPacker(BaseBonusUIPacker):
    ...
    def _packTmanTemplateToken(self, tokenID, bonus, callback=None):
        ...
        if not iconSmallPath or not iconBigPath:
            ...
            if not iconSmallPath:
                _logger.warning(b"Couldn't obtain big image for %s!", tokenID)
                iconSmallPath = iconPath
            elif not iconBigPath:
                _logger.warning(b"Couldn't obtain small image for %s!", tokenID)
    ...
    def _getProductToken(cls, tokenName):
        ...
        return (b'{}_{}').format(tankmen.RECRUIT_TMAN_TOKEN_PREFIX, tokenData[b'sourceID'])
...
def getMultipleProductAwardsBonusPacker(productCode):
    ...
    mapping.update({b'vehicles': (MultiAwardVehiclesBonusUIPacker()), 
       b'tmanToken': (_TmanTemplateProductBonusPacker(productCode)), 
