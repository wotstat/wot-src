...
class TokenDataPrefetcher(_IPrefetcher):
    ...
    def prefetch(self, bonus, callback):
        for tID in bonus.getTokens():
            ...
            if not iconBigPath:
                _logger.warning(b"Couldn't add token award %s because big image has not been obtained!", tID)
                callback(False)
                return
            if not iconSmallPath:
                _logger.warning(b"Couldn't add token award %s because small image has not been obtained!", tID)
                callback(False)
                return
