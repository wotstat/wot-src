...
_COLLECTION_NAME_TO_SOUNDSPACE = {b'mt_birthday2023': COLLECTIONS_MT_BIRTHDAY23_SOUND_SPACE}

class CollectionView(ViewImpl):
    __slots__ = (b'__backCallback', b'__backBtnText', b'__collection', b'__page', b'__pagesCount', b'__rewardTooltips', b'__content', b'__bonusData', b'_COMMON_SOUND_SPACE')
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.lobby.battle_pass.tooltips.BattlePassCoinTooltipView():
            ...
            if contentID == R.views.lobby.collection.tooltips.CollectionItemTooltipView():
                if event.hasArgument(b'itemId'):
                    return CollectionItemTooltipView(itemId=event.getArgument(b'itemId'), collectionId=self.__collection.collectionId)
                tooltipData = self.getTooltipData(event)
                if tooltipData is not None:
                    return CollectionItemTooltipView(*tooltipData.specialArgs)
            if contentID == R.views.lobby.tooltips.AdditionalRewardsTooltip():
                tooltipIds = map(int, event.getArgument(b'hiddenRewards').split(_SEPARATOR))
    ...
    def __onViewLoaded(self, args):
        self.__pagesCount = int(args.get(b'pagesCount'))
    ...
    def __fillPagesBackgrounds(self, model=None):
        generalBg = self.__getContent(Group.BG, self.__collection.name, b'bgMain')
        if generalBg:
        ...
        for pbg in range(1, self.__pagesCount + 1):
            ...
            pageBg = self.__getContent(Group.BG, self.__collection.name, (b'bgPage{}').format(pbg), optionalContent=True)
            if pageBg:
    ...
    def __generateContentData(self):
        contentIDs = [makeImageID(Group.BG, self.__collection.name, b'bgMain')]
        for pbgID in range(1, self.__pagesCount + 1):
            contentIDs.append(makeImageID(Group.BG, self.__collection.name, (b'bgPage{}').format(pbgID)))
    ...
    def __onSetRewardReceived(self, args):
        requiredCount = int(args.get(b'requiredItemsCount'))
    ...
    def __onOpenItemPreview(self, args):
        itemId = int(args.get(b'itemId'))
        page = int(args.get(b'currentPage'))
        collectionId = self.__collection.collectionId
    ...
    def __onTabSelected(self, args):
        collectionName = args.get(b'collectionName')
    ...
    def __getContent(self, group, sub, name, optionalContent=False):
        path = self.__content.get(group, {}).get(sub, {}).get(name, b'')
        if not path and not optionalContent:
            _logger.warning(b'Resource: %s not found', (b'/').join((group, sub, name)))
        return path

    def __onHeaderMenuClick(self, event):
        if event.ctx.get(b'alias') == VIEW_ALIAS.LOBBY_HANGAR:
            setHangarState()
        self.destroyWindow()
        return
