...
class CollectionView(ViewComponent):
        ...
        if contentID == R.views.mono.battle_pass.tooltips.bpcoin():
            ...
            if contentID == R.views.lobby.collection.tooltips.CollectionItemTooltipView():
                if event.hasArgument(b'itemId'):
                    return CollectionItemTooltipView(itemId=event.getArgument(b'itemId'), collectionId=self.__collection.collectionId, isDetailed=True)
    ...
    def getContext(self):
        return {b'page': (self.__page), b'collectionId': (self.__collectionId)}
    ...
    def __fillPagesBackgrounds(self, model=None):
        ...
        for pbg in range(1, self.__pagesCount + 1):
            ...
            pageBg = self.__getContent(Group.BG, self.__collection.name, (b'bgPage{}').format(pbg))
            if pageBg:
