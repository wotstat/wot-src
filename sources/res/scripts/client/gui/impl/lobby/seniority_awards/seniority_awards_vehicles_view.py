...
_SENIORITY_VEHICLES_ORDER = (
 b'uk:GB156_Black_Prince_II',
 b'uk:GB146_Gabler_s_Destroyer',
 b'germany:G170_PzKpfwIV_Ausf_F2',
 b'ussr:R197_KV_1S_MZ',
 b'germany:G158_VK2801_105_SPXXI',
 b'usa:A134_M24E2_SuperChaffee',
 b'usa:A130_Super_Hellcat',
 b'ussr:R160_T_50_2')
...
class SeniorityRewardVehiclesView(ViewImpl):
        ...
        if self.__seniorityAwardsCtrl.isVehicleSelectionAvailable:
            ...
            try:
                try:
                    ...
                    _logger.debug(b'%s has been destroyed before %s completed', self, self.__seniorityAwardsCtrl.vehicleSelectionQuestPattern.format(id=self.__vehicleSelectionQuestId))
                    result = VehicleSelectionState.SELECTION_FAILED
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.lobby.ranked.tooltips.RankedBattlesRolesTooltipView():
            vehicleCD = getVehicleCD(event.getArgument(b'vehicleCD'))
            if vehicleCD:
                return VehicleRolesTooltipView(vehicleCD)
            _logger.warning(b'Parameter vehicleCD is missing')
    ...
    def _onLoading(self, vehicles, fromEntryPoint, *args, **kwargs):
        ...
        if not self.__seniorityAwardsCtrl.isVehicleSelectionAvailable and not vehicles:
            _logger.error(b'Wrong data to show view')
            return
    ...
    def __handleOnMoreRewards(self):
        if self.__state in (ViewState.VIEW_REWARD, ViewState.VIEW_REWARD_AFTER_SELECTION):
            ...
            _logger.error(b'MoreRewardsBtnClick was called with invalid state - %s', self.__state)
        return
    ...
    def __handleShowHangar(self):
        if self.__state == ViewState.VIEW_REWARD_AFTER_SELECTION:
            if self.__vehicles:
                ...
                _logger.error(b"Can't find vehicle to show")
            self.destroyWindow()
        else:
            _logger.error(b'ShowHangar was called with invalid state - %s', self.__state)
        return

    @wg_async
    def __handleOnSelectVehicleReward(self, event):
        if self.__state == ViewState.SELECTION:
            self.__selectedVehicleId = event.get(b'selectedId', b'')
            if not self.__selectedVehicleId:
                _logger.error(b"Invalid selected vehicleId: '%s'", self.__selectedVehicleId)
                self.destroyWindow()
                return
            self.__showWaiting()
            ...
            if result == VehicleSelectionState.RECIEVED:
                ...
                if vehicle:
                    ...
                    self.destroyWindow()
                    _logger.error(b'Cannot find recieved reward vehicle')
            ...
            _logger.error(b"SelectVehicleRewardBtnClick was called with invalid state: '%s'", self.__state)
        return
    ...
    @staticmethod
    def __showWaiting():
        Waiting.show(b'selectSeniorityAwards')
        return

    @staticmethod
    def __hideWaiting():
        Waiting.hide(b'selectSeniorityAwards')
        return
    ...
    def __onVehicleSelectionChanged(self, state):
        ...
        if state == VehiclesForSelectionState.VEHICLES_CHANGED:
            ...
            return
        _logger.error(b'Vehicle selection state is undefined %s', state)
        return
