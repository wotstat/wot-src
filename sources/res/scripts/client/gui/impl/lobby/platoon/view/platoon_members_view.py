...
def getMemberContextMenuData(event, platoonSlotsData):
    ...
    if userName:
        for it in platoonSlotsData:
            playerData = it[b'player']
            if playerData is not None and playerData[b'userName'] == userName:
                return createContextMenuData(CONTEXT_MENU_HANDLER_TYPE.UNIT_USER, playerData)

    _logger.warning(b'Username %s not found in platoon slots data', userName)
    return
...
class _LayoutStyle(Enum):
    ...
    HORIZONTAL_SHORT = b'horizontal_short'
    VERTICAL = b'vertical'
...
class SquadMembersView(ViewImpl, CallbackDelayer):
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.lobby.platoon.AlertTooltip():
            ...
            if contentID == R.views.lobby.platoon.WTRTooltip():
                slotId = event.getArgument(b'slotId')
    ...
    def _convertSlotLabelData(slotLabelArray, slotLabelHtml):
        ...
        for element in slotLabelElements:
            ...
            model.setType(element.get(b'type', Types.TEXT))
            model.setContent(element.get(b'text', b''))
            model.setStyleJson(element.get(b'style', b''))
        ...
        with self.viewModel.transaction() as model:
            ...
            for slotData in slots:
                ...
                if playerData:
                    ...
                    slotLabelHtml = slotData.get(b'slotLabel', b'')
                    if slotLabelHtml:
    ...
    def _setPlayerData(self, accID, isWTREnabled, slotData, playerData, slotModel):
        slotModel.player.commonData.setName(playerData.get(b'userName', b''))
        slotModel.player.commonData.setColor(b'#DE1E7E')
        slotModel.player.setIsReady(playerData.get(b'readyState', False))
        slotModel.player.setAccID(str(playerData.get(b'dbID', None)))
        slotModel.player.setIsCommander(playerData.get(b'isCommander', False))
        slotModel.player.setIsCurrentUser(accID == playerData.get(b'accID', None))
        slotModel.player.commonData.setClanTag(playerData.get(b'clanAbbrev', b''))
        slotModel.player.setIsPrem(slotData.get(b'hasPremiumAccount', False))
        slotModel.player.commonData.setRating(playerData.get(b'accountWTR', b'') if isWTREnabled else b'')
        badgeIconPath = playerData.get(b'badgeVisualVO', {}).get(b'icon', b'')
        ...
        isAdditionalMsgVisible = slotData.get(b'isVisibleAdtMsg', False)
        if isAdditionalMsgVisible:
            additionalMsg = slotData.get(b'additionalMsg', b'')
        ...
        colors = playerData.get(b'colors', [])
        if len(colors) > 1:
            slotModel.player.commonData.setColor(b'#' + (b'{0:06X}').format(colors[1 if isOffline else 0]))
        tags = playerData.get(b'tags', [])
    ...
    def _setVehicleData(self, slotData, slotModel):
        ...
        if vehicle:
            vehicleItem = self._itemsCache.items.getItemByCD(vehicle.get(b'intCD', 0))
    ...
    def _initWindowData(self):
        with self.viewModel.transaction() as model:
            ...
            fileName = (b'{battleType}_{layout}_list').format(battleType=self._prebattleType.value, layout=layoutStyle.value)
    ...
    def _setHeaderBg(self, fileName, model):
        ...
        if fileNameRes.exists():
            ...
            _logger.warning(b'R.images.gui.maps.icons.platoon.members_window.backgrounds %s not found', fileName)
        return
    ...
    def _onInviteFriends(self):
        g_eventBus.handleEvent(events.LoadViewEvent(SFViewLoadParams(PREBATTLE_ALIASES.SEND_INVITES_WINDOW_PY), ctx={b'prbName': b'unit', 
    ...
    def _updateReadyButton(self, *args):
        ...
        with self.viewModel.transaction() as model:
            if not self._platoonCtrl.isInCoolDown(REQUEST_TYPE.SET_PLAYER_STATE):
                model.btnSwitchReady.setIsEnabled(actionButtonStateVO[b'isEnabled'] and not isInQueue)
            ...
            model.btnSwitchReady.setDescription(i18n.makeString(actionButtonStateVO[b'toolTipData'] + b'/body'))
            model.setFooterMessage(simpleState)
            model.setIsFooterMessageGrey(actionButtonStateVO[b'isEnabled'] or onlyReadinessText or isInQueue)
        return
    ...
    def __onFocusChange(self, *args, **kwargs):
        if args:
            self.__setPreBattleCarouselFocus(args[0][b'isFocused'])
        return
...
class BattleRoyalMembersView(SquadMembersView):
    ...
    def __sortCommanderUser(slot):
        player = slot.get(b'player', {})
        if player is not None:
            return player.get(b'isCommander')
        else:
            return False
...
class Comp7MembersView(SquadMembersView):
    ...
    def __sortCurrentUser(slot):
        ...
        return accID != player.get(b'accID')
    ...
    def _setModeSlotSpecificData(self, slotData, slotModel):
        playerData = slotData.get(b'player', {})
        queueInfo = playerData.get(b'extraData', {}).get(b'comp7EnqueueData', {})
        rank = queueInfo.get(b'rank', 0)
        rating = queueInfo.get(b'rating', 0)
        isOnline = bool(queueInfo.get(b'isOnline', 0))
        division = self.__getDivision(rank, rating)
        if division is None:
            _logger.error(b"Failed to get player's division. dbID: %d; rank: %d; rating: %d", playerData.get(b'dbID'), rank, rating)
            return
        else:
...
class RankedMembersView(SquadMembersView):
    ...
    def _setModeSlotSpecificData(self, slotData, slotModel):
        playerData = slotData.get(b'player', {})
        queueInfo = playerData.get(b'extraData', {}).get(b'rankedEnqueueData', {})
        rankID = queueInfo.get(b'rank', 0)
    ...
    def _getWindowInfoTooltipHeaderAndBody(self):
        ...
        keys = [b'restriction']
        keys.extend([key for key, restriction in ((b'rank', rankRestriction), (b'division', divisionRestriction)) if restriction is not None])
        tooltipHeader = backport.text(R.strings.platoon.members.header.tooltip.ranked.header())
        tooltipBody = backport.text(R.strings.platoon.members.header.tooltip.ranked.dyn((b'_').join(keys))(), rank=rankRestriction)
        return (
         tooltipHeader, tooltipBody)
...
class MembersWindow(PreloadableWindow):
    ...
    def __init__(self, initialPosition=None):
        ...
        if viewClass:
            ...
            content = None
            _logger.debug(b'PrbType is unknown %d', prbType)
