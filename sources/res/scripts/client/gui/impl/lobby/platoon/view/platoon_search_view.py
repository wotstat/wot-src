...
class SearchView(ViewImpl, CallbackDelayer):
    ...
    def __onQueueInfoReceived(self, queueInfo):
        ...
        numInQueue = actualQueueInfo[b'numInQueue']
        with self.viewModel.transaction() as model:
...
class SearchWindow(PreloadableWindow):
    ...
    def __init__(self):
        ...
        if popoverParams is not None:
            ...
            _logger.warning(b'Initializing window with empty popover parameters')
        return
    ...
    def _onContentReady(self):
        if not self._isPreloading():
            g_eventBus.handleEvent(PlatoonDropdownEvent(PlatoonDropdownEvent.NAME, ctx={b'showing': True}))
