...
def getQueueInfoByQueueType(queueInfo, queueType):
    defaultQueueInfo = {b'numInQueue': 0}
    if queueInfo:
...
def formatSearchEstimatedTime(seconds):
    if seconds > 60 or seconds < 1:
        return i18n.makeString(PLATOON.SEARCHING_ESTIMATED_MORETHAN) % {b'seconds': 60}
    if seconds <= 5:
        seconds = 5
    elif seconds <= 10:
        seconds = 10
        ...
        seconds = 60
    return i18n.makeString(PLATOON.SEARCHING_ESTIMATED_LESSTHAN) % {b'seconds': seconds}
