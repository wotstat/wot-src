...
def addPlatoonSearchViewByPrbType(prbType, searchView, personality):
    if prbType in PRB_TYPE_TO_SEARCH_VIEW_CONTENT_FACTORY:
        raise SoftException((b'PRB_TYPE_TO_SEARCH_VIEW_CONTENT_FACTORY already has pre battle type:{prb}. Personality: {p}').format(prb=prbType, p=personality))
    PRB_TYPE_TO_SEARCH_VIEW_CONTENT_FACTORY.update({prbType: searchView})
    msg = (b'prbType:{prb} was added to PRB_TYPE_TO_SEARCH_VIEW_CONTENT_FACTORY. Personality: {p}').format(prb=prbType, p=personality)
    logging.debug(msg)
    return
