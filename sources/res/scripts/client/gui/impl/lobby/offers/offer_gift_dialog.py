...
class OffersSimpleDialogWindow(OffersDialogWindowMixin, SimpleDialogWindow):
    ...
    def __init__(self, offerID, price, *args, **kwargs):
        ...
        if offer.showPrice:
            kwargs[b'bottomContent'] = DialogPricesContent()
...
class _OffersSimpleDialogBuilder(SimpleDialogBuilder):
    ...
    def description(self):
        ...
        if self._gift.rentType != RentType.NO_RENT:
            ...
            strArgs[b'rentValue'] = rentValue % dict(value=self._gift.rentValue)
...
class _OffersPureDialogBuilder(PureDialogBuilder):
    ...
    def __init__(self, offerID, giftID, cdnTitle=b''):
    ...
    def title(self):
        ...
        giftName = giftName.format(neutralOpen=b'', neutralClose=b'', expTagOpen=b'', expTagClose=b'')
        return backport.text(R.strings.offers.giftDialog.title.common(), name=giftName)


def makeOfferGiftDialog(offerID, giftID, cdnTitle=b''):
    offersProvider = dependency.instance(IOffersDataProvider)
    offer = offersProvider.getOffer(offerID)
    if offer is None:
        _logger.error(b'Wrong offerID=%s', offerID)
    gift = offer.getGift(giftID)
    if gift is None:
        _logger.error(b'Wrong giftID=%s for offerID=%s', offerID, giftID)
    if gift.isVehicle:
