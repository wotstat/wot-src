...
class OfferRewardWindow(ViewImpl):
    ...
    _COMMON_SOUND_SPACE = CommonSoundSpaceSettings(name=b'offer_reward', entranceStates={}, exitStates={}, persistentSounds=(), stoppableSounds=(), priorities=(), autoStart=True, enterEvent=b'gui_hangar_award_screen', exitEvent=b'gui_hangar_award_screen_stop')

    def __init__(self, layoutID, offerID, giftID, cdnTitle=b'', cdnDescription=b'', cdnIcon=b''):
        ...
        with self._viewModel.transaction() as model:
            if self._gift.fromCdn:
                ...
                if formattedBonus is not None:
                    icon = getGfImagePath(formattedBonus.get(b'imgSource')) or b''
                    self._tooltipData = TooltipData(tooltip=formattedBonus.get(b'tooltip', None), isSpecial=formattedBonus.get(b'isSpecial', False), specialAlias=formattedBonus.get(b'specialAlias', b''), specialArgs=formattedBonus.get(b'specialArgs', None), isWulfTooltip=formattedBonus.get(b'isWulfTooltip', False))
