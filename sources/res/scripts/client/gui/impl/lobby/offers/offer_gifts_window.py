...
BONUSES_WITHOUT_COUNTER = {
 Currency.CREDITS, Currency.GOLD, Currency.CRYSTAL, b'freeXP',
 PREMIUM_ENTITLEMENTS.PLUS, b'vehicles'}
...
class OfferGiftsView(ViewImpl):
    ...
    def _onLoading(self, *args, **kwargs):
        ...
        if offerItem is not None:
            ...
            with self._viewModel.transaction() as model:
                ...
                description = localization.readString(b'description') if localization else b''
                linkText = localization.readString(b'linkText') if localization else b''
                ...
                title = localization.readString(b'name') if localization else b''
                model.setName(title)
    ...
    def _getGiftResources(self, gift):
        if gift.fromCdn:
            ...
            title = locFile.readString(b'name') if locFile else b''
            description = locFile.readString(b'description') if locFile else b''
        ...
        price = gift.price
        imgPath = getGfImagePath(icon) or b''
        return (title, description, imgPath, count, price)

    def _onGiftClicked(self, args):
        giftID = args.get(b'index')
        gift = self._offerItem.getGift(giftID)
        if gift is None:
            _logger.error(b'Unknown gift id=%s for offer id=%s', giftID, self._offerID)
            return
        else:
            localization = ResMgr.openSection(self._offersProvider.getCdnResourcePath(gift.cdnLocFilePath, relative=False))
            onGiftConfirm = partial(self._onGiftConfirm, self._offerID, giftID, cdnTitle=localization.readString(b'name') if localization else b'', cdnDescription=localization.readString(b'description') if localization else b'', cdnIcon=getGfImagePath(self._offersProvider.getCdnResourcePath(gift.cdnIconPath)))
            if gift.isVehicle:
    ...
    @adisp_process
    def _onGiftConfirm(self, offerID, giftID, cdnTitle=b'', cdnDescription=b'', cdnIcon=b''):
    ...
    def _onLearnMore(self):
        ...
        if localization:
            url = localization.readString(b'linkUrl', b'')
            if url:
