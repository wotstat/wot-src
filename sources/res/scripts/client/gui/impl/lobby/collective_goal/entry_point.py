...
class EntryPoint(ViewImpl):
    ...
    def _getEvents(self):
        return (
         (
          self.viewModel.showProgression, self.__showEventsTab),
         (
          self.__collectiveGoalEntryPointController.onDataUpdated, self.__onDataUpdated),
         (
          self.__collectiveGoalEntryPointController.onEventUpdated, self.__onEventUpdated))

    def createToolTipContent(self, event, contentID):
        if contentID == R.views.lobby.collective_goal.tooltips.EntryPointTooltip():
    ...
    def __getEventState(self):
        ...
        if self.__collectiveGoalEntryPointController.isCompleted() or self.__collectiveGoalEntryPointController.isFinished():
