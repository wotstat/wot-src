...
class PrestigePresenter(SubModelPresenter, IPresenterLocationController):
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.mono.vehicle_hub.tooltips.prestige_reward_tooltip():
            level = event.getArgument(b'level')
            if level is not None:
                ...
                if milestones and state:
                    ...
                    _logger.error(b'Bonus cannot be parsed')
                else:
                    _logger.error(b'Bonus with passed level is not found: %d', level)
            else:
                _logger.error(b'Missing level to show tooltip')
        return super(PrestigePresenter, self).createToolTipContent(event, contentID)

    def _getCallbacks(self):
        return (
         (
          b'stats.dossier', self.__dossierUpdate),
         (
          b'stats.prestigeMilestonesAchieved', self.__prestigeMilestonesAchievedUpdate))
    ...
    def __onRewardPreview(self, level):
        ...
        if not bonus:
            ...
            if not customizations:
                return
            itemTypeID = getItemTypeID(customizations[0].get(b'custType'))
