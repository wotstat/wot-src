...
class _ModeBase(object):
    __slots__ = (b'isEntered', b'_p', b'needsFadeDelay')
...
class _NominalMode(_ArmorMode):
    ...
    def enter(self):
        ...
        _logger.debug(b'Armor mode switched to nominal armor')
        return
...
class _PenetrationMode(_ArmorMode):
    ...
    def enter(self):
        ...
        _logger.debug(b'Armor mode switched to penetration chance')
        return
...
class _NoArmorMode(_ModeBase):
    ...
    def enter(self):
        ...
        _logger.debug(b'Armor mode switched to no_armor')
        return
...
class ArmorSubPresenter(SubPresenterBase):
    __slots__ = (b'_isMouseOver', b'_tooltip', b'_level', b'_normalArmorMax', b'_spacedArmorMax', b'_uiLogger', b'_modes', b'_currentModeId', b'_currentMode', b'_modulesMover', b'vehicleEntity', b'isClosing', b'__weakref__', b'_modulesSetup', b'_attackerSetup', b'_shellParams')
    ...
    def getTooltipData(self, event):
        ...
        if tooltipId == TOOLTIPS_CONSTANTS.CONTEXT_VEHICLE_MODULE:
            compactDescr = int(event.getArgument(b'compactDescr', 0))
            if tooltipId == TOOLTIPS_CONSTANTS.CONTEXT_VEHICLE_MODULE:
                isAttacker = bool(event.getArgument(b'isAttacker', False))
            ...
            if tooltipId == TOOLTIPS_CONSTANTS.ARMOR_INSPECTOR_SHELL:
                index = int(event.getArgument(b'shellIndex', 0))
...
class _ModulesMover(object):
    __slots__ = (b'_vehicleEntity', b'_hoveredDistance', b'_hoveredModule', b'_dragModule', b'_gunMultiplier', b'_turretMultiplier')
    ...
    def resetModulesRotation(self):
        if hasattr(self._vehicleEntity, b'appearance') and self._vehicleEntity.appearance is not None:
...
class _ModulesSetup(object):
    __slots__ = (b'_vehicleEntity', b'_initialVehicleCD', b'_initialTurretCD', b'_initialGunCD', b'_initialOutfit')
    ...
    def _initModulesData(self, viewModel):
        typeDescriptor = self._vehicleEntity.typeDescriptor
        hasTurrets = len(typeDescriptor.hull.fakeTurrets[b'lobby']) != len(typeDescriptor.turrets)
        currentTurret = typeDescriptor.turret
        currentGun = typeDescriptor.gun
    ...
    def _onTurretItemClick(self, args):
        if args is None:
            ...
            compactDescr = int(args.get(b'compactDescr'))
...
class _AttackerSetup(object):
    __slots__ = (b'_attackerVehicle', b'_parent')
    _itemsCache = dependency.descriptor(IItemsCache)
    _hangarSpace = dependency.descriptor(IHangarSpace)
    ...
    def _onAttackerGunItemClick(self, args):
        if args is None:
            ...
            gunCompactDecscr = int(args.get(b'compactDescr'))
    ...
    def _onAttackerShellItemClick(self, args):
        if args is None:
            ...
            activeGunShotIndex = int(args.get(b'index'))
