...
_DIRECTION_TO_LINE_TYPE = {b'r2l': (LineType.RIGHTTOLEFT), 
   b'l2r': (LineType.LEFTTORIGHT), 
   b'b2t': (LineType.BOTTOMTOTOP), 
   b't2b': (LineType.TOPTOBOTTOM), 
   b'r2b': (LineType.RIGHTTOBOTTOM), 
   b'b2r': (LineType.BOTTOMTORIGHT), 
   b't2r': (LineType.TOPTORIGHT), 
   b'r2t': (LineType.RIGHTTOTOP), 
   b'l2b': (LineType.LEFTTOBOTTOM), 
   b'b2l': (LineType.BOTTOMTOLEFT), 
   b't2l': (LineType.TOPTOLEFT), 
   b'l2t': (LineType.LEFTTOTOP)}
...
class TreePresenter(SubModelPresenter, IPresenterLocationController):
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.mono.vehicle_hub.tooltips.perk_tooltip():
            ...
            if nodeID is not None:
                ...
                _logger.error(b'Node type not in %s', (b', ').join([t.value for t in _STEP_TYPE_TO_TOOLTIP]))
            else:
                _logger.error(b'Missing nodeID to show tooltip')
        return super(TreePresenter, self).createToolTipContent(event, contentID)

    def _getCallbacks(self):
        return super(TreePresenter, self)._getCallbacks() + (
         (
          b'stats.freeXP', self.__onXPStatsChanged),
         (
          b'stats.vehTypeXP', self.__onXPStatsChanged))
    ...
    def __update(self):
        if not self.isLoaded:
            _logger.warning(b'TreePresenter is not loaded')
            return
        else:
