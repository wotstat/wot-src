...
ShellParams = namedtuple(b'ShellParams', (b'shellCaliber', b'shellPiercingFull', b'shellPiercingMin', b'shellPiercingMax', b'shellPiercingPowerRandomization', b'normalizationAngle', b'ricochetAngleCos', b'shellJetLossPPByDist', b'shellMayRicochet', b'shellCheckCaliberForRicochet', b'shieldPenetration', b'shellTypeMaxDamage', b'useHEShell'))
PenetrationInfo = namedtuple(b'PenetrationInfo', (b'chance', b'color', b'resultType'))
_EPS = 1e-05
...
def setShellParamsFromVehicle(shot, spaceID):
    ...
    if shell.kind in (SHELL_TYPES.ARMOR_PIERCING, SHELL_TYPES.ARMOR_PIERCING_CR):
        ...
        checkCaliberForRicochet = False
    if hasattr(shell.type, b'piercingPowerLossFactorByDistance'):
...
def _shouldHitRicochet(matInfo, hitAngleCos, params):
    if not getattr(matInfo, b'mayRicochet', False):
        return False
    ...
    if hitAngleCos > params.ricochetAngleCos:
        return False
    if not getattr(matInfo, b'checkCaliberForRicochet', False):
        return True
