...
VehicleHubCtx = namedtuple(b'VehicleHubCtx', (b'intCD', b'vehicleStrCD', b'style', b'outfit'))
...
class VehicleHubMainView(ViewComponent, IRoutableView):
    ...
    def _getCallbacks(self):
        ...
        return callbacksTuple + (
         ...
         (
          b'cache.vehsLock', self.__onVehsLockUpdate),
         (
    ...
    def __onNavigationChanged(self, state, event):
        self.__switchSubView(state, event.params.get(b'vhCtx'))
        return
    ...
    def __onResize(args):
        if args is None:
            ...
            xmin = float(args.get(b'xmin', 0))
            ymin = float(args.get(b'ymin', 0))
            xmax = float(args.get(b'xmax', screenWidth))
            ymax = float(args.get(b'ymax', screenHeight))
            g_eventBus.handleEvent(CameraRelatedEvents(CameraRelatedEvents.ON_RESIZE, ctx={b'xmin': xmin, b'ymin': ymin, b'xmax': xmax, b'ymax': ymax}))
            return
