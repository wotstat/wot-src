...
PrestigeBonusContext = namedtuple(b'PrestigeBonusContext', [
 b'vehCD',
 b'level',
 b'state'])
...
class PrestigeCustomizationBonusUIPacker(CustomizationBonusUIPacker):
    ...
    def _packSingleBonus(cls, bonus, item, label):
        ...
        if not customizations:
            ...
            itemTypeID = getItemTypeID(item.get(b'custType'))
            ...
            if c11nItem:
                if itemTypeID in (GUI_ITEM_TYPE.ATTACHMENT, GUI_ITEM_TYPE.STAT_TRACKER):
                    ...
                    model.setIcon((b'style_{}').format(bonus.getC11nItem(item).id))
            return model
...
def getVehSkillTreeRewardViewBonusPacker():
    ...
    mapping.update({b'customizations': (PrestigeCustomizationBonusUIPacker())})
...
def getVehSkillTreeRewardTooltipViewBonusPacker():
    mapping = getDefaultBonusPackersMap()
    mapping.update({b'customizations': (PrestigeCustomizationTooltipBonusUIPacker())})
    return BonusUIPacker(mapping)
...
def getPrestigeBonus(milestones, ctx):
    ...
    if not reward:
        ...
        bonusesConfig = reward.get(b'bonus', {})
        bonusesPrestige = []
