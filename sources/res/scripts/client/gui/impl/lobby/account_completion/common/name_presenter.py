import logging, re, typing, BigWorld
...
_NAME_PATTERN = re.compile(b'^[a-z0-9_]+$', re.I)
...
class RemoteNameValidator(object):
    __slots__ = (b'onStatusChanged', b'_localValidator', b'_remoteValidationCB', b'_status', b'_name', b'_currentFuture', b'_error', b'_suggestions', b'_requestWrapper')
    _wgnpDemoAccCtrl = dependency.descriptor(IWGNPDemoAccRequestController)
...
class NamePresenter(BaseFieldPresenter):
    __slots__ = (b'onChanged', b'_remoteValidator', b'_isLocked', b'_asyncScope')
    ...
    def remotelyValidate(self):
        ...
        try:
            ...
            _logger.debug(b'%s has been destroyed before got a response to remote nickname validation request', self)
    ...
    def _suggestionSelectHandler(self, args):
        suggestion = args.get(b'variant', b'')
