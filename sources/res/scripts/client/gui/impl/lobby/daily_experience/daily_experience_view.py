...
class DailyExperienceView(ViewImpl):
    ...
    def __updateViewModel(self):
        ...
        with self.viewModel.transaction() as model:
            model.setMultiplier(int(premiumBonusConfig.get(b'bonusFactor')))
            model.setLeftBonusCount(usesLeft)
            model.setTotalBonusCount(premiumAdditionalCount + wotPlusAdditionalCount)
            model.setWotPremiumMaxApplications(premiumBonusConfig.get(b'applyCount'))
    ...
    def _getCallbacks(self):
        return (
         (
          b'stats.applyAdditionalXPCount', self._onStatsUpdated),
         (
          b'stats.applyAdditionalWoTPlusXPCount', self._onStatsUpdated))
