...
class Collector20DossierBonusUIPacker(DossierBonusUIPacker):
    _REWARD_ORDER = (b'playerBadges', b'singleAchievements')
    ...
    def _getToolTip(cls, bonus):
        methodMapping = {b'playerBadges': (cls._getBadgeTooltip), 
           b'singleAchievements': (cls._getAchievementTooltip)}
        return cls.__applyPackMethodsInOrder(bonus, methodMapping)

    @classmethod
    def _pack(cls, bonus):
        methodMapping = {b'playerBadges': (cls._packBadges), 
           b'singleAchievements': (cls._packAchievements)}
        return cls.__applyPackMethodsInOrder(bonus, methodMapping)
    ...
    def __applyPackMethodsInOrder(cls, bonus, methodMapping):
        ...
        for dossierBonusName in cls._REWARD_ORDER:
            ...
            if method is not None:
                ...
                _logger.warning(b'Unsupported dossier bonus: %s', dossierBonusName)
...
def getCollector20RewardsBonusPacker():
    ...
    mapping.update({b'dossier': (Collector20DossierBonusUIPacker())})
