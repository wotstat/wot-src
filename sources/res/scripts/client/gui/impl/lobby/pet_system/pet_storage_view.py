...
class PetStorageView(ViewComponent):
    ...
    def __onPetSelect(self, args):
        petID = int(args.get(b'petID'))
        yield SelectActivePetProcessor(petID).request()
        return

    @adisp_process
    def __onBonusSelect(self, args):
        bonusID = args.get(b'bonusID')
        if not bonusID:
            _logger.error(b'Invalid bonusID received, args: %s', args)
            return
        bonusID = int(bonusID)
    ...
    def __updatePetName(self, args):
        petNameID = int(args.get(b'petNameID'))
        petID = int(args.get(b'petID'))
    ...
    def __updateStateBehavior(self, args):
        visibilityState = int(args.get(b'visibilityState'))
    ...
    def __onToPurchase(self):
        ...
        if not url and not shopUrl:
            _logger.error(b'Invalid urls for pet id %s', self.viewModel.getPetID())
            return
        if shopUrl:
            showShop(shopUrl)
        else:
