...
class PollView(ViewImpl):
    __slots__ = (b'__notID', b'__target', b'__show')
    ...
    def __init__(self, layoutID, ctx, flags=ViewFlags.LOBBY_TOP_SUB_VIEW):
        ...
        self.__notID = ctx.get(b'notID')
        self.__target = ctx.get(b'target')
        return
    ...
    def _getEvents(self):
        return (
         (
          self.viewModel.onGoToPoll, self.__onGoToPoll),
         (
    ...
    def _getListeners(self):
        return (
         (
          events.NotifyCenterShowItemEvent.CLOSE_POLL_WINDOW, self.__handlePollWindowClose, EVENT_BUS_SCOPE.LOBBY),)
