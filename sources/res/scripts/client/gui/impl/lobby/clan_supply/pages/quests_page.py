...
BUBBLES_ALIAS = b'clansupply_hangar_bubble'
...
class QuestsPage(SubModelPresenter):
    __slots__ = (b'__tooltips', b'__cachedQuestData', b'__notifier')
    ...
    def _getCallbacks(self):
        return (
         (
          b'stats.clanInfo', self.__updateClanInfo),)
    ...
    def __onClaimReward(self):
        ...
        if self.viewModel is None:
            ...
            if not response.isSuccess():
                ...
                _logger.warning(b'Failed to claim rewards. Code: %s.', response.getCode())
                return
            self.__notificationCtrl.setCounters(BUBBLES_ALIAS, 0)
    ...
    def __getQuestSquadState(conditionData):
        ...
        if key not in _QuestDataSquadStateToModelSquadState:
            _logger.error(b'Does not have key - %s for the mapping to the model.', key)
