...
class ProgressionPage(SubModelPresenter):
    __slots__ = (b'__tooltips', b'__rewardVehicleIntCD', b'__selectedStageID', b'__cachedProgressData', b'__cachedSettingsData', b'__rewardStyleID', b'__isFindSelectedStageNeeded')
    ...
    def _getCallbacks(self):
        return (
         (
          b'cache.dynamicCurrencies.tourcoin', self.__updateBalance),)
    ...
    def __updateProgression(self, settingsData, progressData):
        ...
        with self.viewModel.transaction() as tx:
            ...
            if not all((self.__rewardVehicleIntCD, self.__rewardStyleID)):
                tx.setStatus(ProgressionScreenStatus.ERROR)
                _logger.error(b'Can not find a vehicle or style in rewards.')
                return
    ...
    def __onSelectStage(self, kwargs):
        self.__selectedStageID = int(kwargs.get(b'id'))
        with self.viewModel.transaction() as tx:
            tx.setSelectedStageID(self.__selectedStageID)
    ...
    def __onPreviewClick(self):
        if not all((self.__rewardVehicleIntCD, self.__rewardStyleID)):
            _logger.error(b'Can not find a reward vehicle or style.')
            return
        else:
            style = self.__c11nService.getItemByID(GUI_ITEM_TYPE.STYLE, self.__rewardStyleID)
            if style is None:
                _logger.error(b'Style is invalid, id=%s', self.__rewardStyleID)
                return
    ...
    def __onBuyStage(self, kwargs):
        ...
        if self.viewModel is None:
            ...
            if not response.isSuccess():
                _logger.warning(b'Failed to purchase stage. Code: %s.', response.getCode())
            else:
    ...
    def __findStyleRewardID(pointSettings):
        ...
        customizationReward = findFirst((lambda r: r[0] == b'customizations'), stageSettings.rewards.items())
        if customizationReward is None:
            return styleID
        else:
            styleReward = findFirst((lambda item: item[b'custType'] == b'style'), customizationReward[1])
            if styleReward is not None:
                styleID = styleReward[b'id']
            return styleID
