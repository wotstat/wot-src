...
_SubModelInfo = typing.NamedTuple(b'_SubModelInfo', [
 (
  b'ID', ClanSupplyViews),
 (
  b'presenter', SubModelPresenter)])

class MainView(ViewImpl):
    __slots__ = (b'__tabId', b'__presenterLoader', b'__isClosingBySelf', b'__uiLogger')
    ...
    def __openInfoPage(self):
        url = GUI_SETTINGS.clanSupply.get(b'baseURL')
        if url is None:
            _logger.error(b'clanSupply.baseURL is missed')
