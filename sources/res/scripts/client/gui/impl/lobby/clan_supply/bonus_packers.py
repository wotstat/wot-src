...
class QuestReward(CONST_CONTAINER):
    INDUSTRIAL_RESOURCE = b'industrial_resource'
...
def getCurrencyReward(rewards):
    ...
    for name, value in rewards.items():
        if name not in QuestReward.CURRENCY_REWARDS:
            continue
        result[b'currencies'][_normalizeCurrencyName(name)][b'count'] = int(value)
...
def findVehicle(rewards):
    vehIntCD = None
    vehicleReward = findFirst((lambda r: r[0] == b'vehicles'), rewards.items())
...
def getClanSupplyBonusPacker(isProgression=False):
    ...
    if not isProgression:
        mapping[b'customizations'] = CustomizationsBonusPacker()
    else:
        mapping[b'customizations'] = Customization3Dand2DbonusUIPacker()
    return BonusUIPacker(mapping)


def _normalizeCurrencyName(name):
    if name == QuestReward.INDUSTRIAL_RESOURCE:
        return b'industrialResource'
    return name
...
def extractBonuses(invoiceData, productCode, callback, purchaseCache=None):
    ...
    rewards = deepcopy(invoiceData[b'data'])
    compensation = deepcopy(invoiceData.get(b'compensation', {}))
