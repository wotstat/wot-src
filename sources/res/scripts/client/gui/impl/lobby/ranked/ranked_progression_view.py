...
_R_LOOTBOX_TOOLTIP = R.views.dyn(b'gui_lootboxes').dyn(b'lobby').dyn(b'gui_lootboxes').dyn(b'tooltips').dyn(b'LootboxTooltip')
...
class RankedProgressionView(ViewImpl):
    __slots__ = (b'__tooltipItems', b'__periodicNotifier')
    ...
    def _getEvents(self):
        return (
         (
          self.__rankedController.onRankedPrbClosing, self.__onDestroy),
         (
    ...
    def __onSelectDivision(self, event, model=None):
        divisionID = int(event.get(b'divisionID', 0))
    ...
    def __setSeasonInfo(self, model=None):
        ...
        if season is not None:
            ...
            _logger.error(b'Incorrect ranked season info!')
        return
