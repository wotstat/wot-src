...
class RankedYearAwardView(ViewImpl):
    ...
    __slots__ = (b'__items', b'__bonuses', b'__points', b'__vehicleCD', b'__showRemainedSelection')
    ...
    def __setViewData(self, points):
        ...
        if awardType is None:
            ...
            _logger.warning(b"The number of points doesn't correspond to the award type")
    ...
    def __setBonuses(self, bonuses):
        with self.getViewModel().transaction() as tx:
            ...
            for index, reward in enumerate(bonuses):
                ...
                showCongrats = False
                if reward.get(b'bonusName', b'') == b'vehicles':
                    showCongrats = True
                ...
                self.__items[index] = createTooltipData(tooltip=reward.get(b'tooltip', None), isSpecial=reward.get(b'isSpecial', False), specialAlias=reward.get(b'specialAlias', b''), specialArgs=reward.get(b'specialArgs', None))
    ...
    def __removeSelectableYearRewards(rawAwards, count):
        selectableTokens = [token for token in rawAwards.get(b'tokens', {}) if token.startswith(YEAR_AWARD_SELECTABLE_OPT_DEVICE_PREFIX)]
        for token in selectableTokens:
            rewardCount = rawAwards[b'tokens'][token].get(b'count', 0) - count
            if rewardCount > 0:
                rawAwards[b'tokens'][token][b'count'] = rewardCount
            elif rewardCount == 0:
                rawAwards[b'tokens'].pop(token)
            if not rawAwards[b'tokens']:
                rawAwards.pop(b'tokens')
