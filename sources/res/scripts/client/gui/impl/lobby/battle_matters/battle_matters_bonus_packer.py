...
_REWARDS_ORDER = (
 b'vehicles', b'battleToken', b'tokens', b'crewBooks', b'customizations', b'items',
 PREMIUM_ENTITLEMENTS.PLUS, PREMIUM_ENTITLEMENTS.BASIC, Currency.CRYSTAL, Currency.GOLD, b'freeXP',
 Currency.CREDITS, b'blueprintsAny', BlueprintBonusTypes.BLUEPRINTS, b'goodies', b'slots')
...
def _itemsSortKey(model):
    return (
     indexesSortKey(_ITEMS_TYPES_ORDER, model.getItemType()),
     indexesSortKey(_DEVICES_TYPES_ORDER, model.getOverlayType()))


_CUSTOM_SORT_KEY = {(VehiclesBonus.VEHICLES_BONUS): _vehiclesSortKey, 
   b'customizations': _customizationsSortKey, 
   b'items': _itemsSortKey}
...
def blueprintsSortKey(bonus):
    ...
    if blueprintName == BlueprintsBonusSubtypes.NATION_FRAGMENT:
        return _blueprintsNationSortKey(bonus)
    return 0
...
def getBattleMattersBonusPacker():
    ...
    mapping.update({(VehiclesBonus.VEHICLES_BONUS): (BattleMattersVehiclesBonusUIPacker()), 
       ...
       b'entitlements': (BattleMattersEntitlementsBonusUIPacker()), 
       b'items': (ExtendedItemBonusUIPacker()), 
       b'dossier': (BattleMattersDossierBonusUIPacker())})
...
class BattleMattersEntitlementsBonusUIPacker(BaseBonusUIPacker):
    ...
    def _pack(cls, bonus):
        if bonus.getValue().id in cls._ITEMS_TO_SKIP:
            ...
            _logger.error(b'Not supported entitlement %s', bonus.getValue().id)
...
class BattleMattersDossierBonusUIPacker(DossierBonusUIPacker):
    _ITEMS_TO_SKIP = {
     (
      ACHIEVEMENT_BLOCK.STEAM, b'steamBriefingMedal')}
