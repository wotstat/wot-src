...
class DogTagConfirmState(LobbyState):
    ...
    def _onEntered(self, event):
        ...
        elif resultData.get(b'saveChanges'):
...
class DogTagState(GuiImplViewLobbyState):
    ...
    def _getViewLoadCtx(self, event):
        return {b'highlightedComponentId': (event.params.get(b'highlightedComponentId', -1)), 
           b'makeTopView': (event.params.get(b'makeTopView', True))}
...
class AnimatedDogTagState(GuiImplViewLobbyState):
    ...
    def _getViewLoadCtx(self, event):
        return {b'initBackgroundId': (event.params.get(b'initBackgroundId', None)), 
           b'initEngravingId': (event.params.get(b'initEngravingId', None))}
