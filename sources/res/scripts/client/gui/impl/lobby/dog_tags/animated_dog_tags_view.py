...
class AnimatedDogTagsView(ViewImpl):
    __slots__ = (b'_dogTagsHelper', b'_composer', b'__initBackgroundId', b'__initEngravingId', b'__uiLogger')
    ...
    def __onEquip(self, background, engraving):
        _logger.debug(b'DogTags::onEquip(%s, %s)', background, engraving)
    ...
    def __onDogTagDataChanged(self, diff):
        ...
        if (PlayerDogTagStorage.key, b'_r') in diff or UnlockedComponentsStorage.key in diff:
            self.__update()
        return
