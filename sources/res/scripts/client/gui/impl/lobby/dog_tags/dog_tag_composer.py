...
SECTION_ORDER_MAP = {(ComponentViewType.BACKGROUND): [
                                  BackgroundSection.TRIUMPH_MEDAL], 
   (ComponentViewType.ENGRAVING): [
...
class DogTagComposerLobby(DogTagComposerClient):
    ...
    def fillDTFeatureModel(self, model):
        ...
        _logger.debug(b'Dashboard dogtag images - engraving: %s, background: %s', engravingImage, bgImage)
        return
        ...
        for compId, comp in components.iteritems():
            if comp.viewType == componentType:
                ...
                if purpose not in SECTION_BY_PURPOSE:
                    _logger.error(b'Not supported purpose "%s"', purpose)
                    continue
