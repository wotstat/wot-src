...
class DogTagsView(ViewImpl):
    __slots__ = (b'_dogTagsHelper', b'_composer', b'_tooltipModelFactories')
    ...
    def __onEquip(self, background, engraving):
        ...
        dogTag = self._dogTagsHelper.getDisplayableDTForComponents([
         background, engraving], self._webCtrl.getAccountProfile())
        with self.viewModel.transaction() as tx:
            if dogTag:
                self._composer.fillModel(tx.equippedDogTag, dogTag)
        return
