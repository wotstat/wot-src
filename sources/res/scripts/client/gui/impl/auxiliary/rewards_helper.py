...
SeniorityAwards = namedtuple(b'SeniorityAwards', b'bonuses currencies')
...
def getSeniorityAwardsVehicles(vehiclesRewards, sortKey=None):
    vehicles = [vehCD for vehiclesDict in vehiclesRewards for vehCD in vehiclesDict if vehiclesDict[vehCD].get(b'compensatedNumber', 0) < 1]
    if sortKey:
...
def getProgressiveRewardBonuses(rewards, size=b'big', maxAwardCount=_DEFAULT_DISPLAYED_AWARDS_COUNT, packBlueprints=False, ctx=None):
    ...
    if rewards:
        for bonusType, bonusValue in iteritems(rewards):
            ...
            elif bonusType in (b'premium', b'premium_plus'):
