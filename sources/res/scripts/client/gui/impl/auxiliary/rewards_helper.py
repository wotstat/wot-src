...
_DEFAULT_ALIGN = b'center'
_DEFAULT_DISPLAYED_AWARDS_COUNT = 6
TMAN_TOKENS = b'tmanToken'

class BlueprintBonusTypes(object):
    BLUEPRINTS = b'blueprints'
    FINAL_BLUEPRINTS = b'finalBlueprints'
    BLUEPRINTS_ANY = b'blueprintsAny'
    ALL = (
     BLUEPRINTS, FINAL_BLUEPRINTS, BLUEPRINTS_ANY)


class CrewBonusTypes(object):
    CREW_BOOK_BONUSES = b'crewBooks'
    CREW_SKIN_BONUSES = b'crewSkins'
    ALL = (
...
_BONUSES_ORDER = (
 Currency.CREDITS, b'premium', Currency.GOLD, Currency.CRYSTAL, b'vehicles',
 b'freeXP', b'freeXPFactor', b'creditsFactor', b'tankmen', b'items', b'slots', b'berths', b'dossier',
 b'customizations', b'goodies', b'tokens', b'blueprints', CrewBonusTypes.CREW_SKIN_BONUSES,
 CrewBonusTypes.CREW_BOOK_BONUSES, b'finalBlueprints', Currency.EVENT_COIN, Currency.BPCOIN)
...
SeniorityAwards = namedtuple(b'SeniorityAwards', b'bonuses vehicles currencies')
...
class LootRewardDefModelPresenter(object):
    ...
    def _formatModel(self, model, ttId, showCongrats):
        with model.transaction() as m:
            m.setIcon(self._reward.get(b'imgSource') or b'')
            m.setLabelStr(self._reward.get(b'label') or b'')
            m.setTooltipId(ttId)
            m.setHasCompensation(bool(self._reward.get(b'hasCompensation', False)))
            m.setLabelAlign(self._reward.get(b'align', _DEFAULT_ALIGN) or _DEFAULT_ALIGN)
            m.setHighlightType(self._reward.get(b'highlightIcon') or b'')
            m.setOverlayType(self._reward.get(b'overlayIcon') or b'')
            m.setIsEnabled(bool(self._reward.get(b'isEnabled', True)))
            m.setRewardName(self._reward.get(b'bonusName') or b'')
            m.setSpecialAlias(self._reward.get(b'specialAlias') or b'')
        return


class LootRewardAnimatedModelPresenter(LootRewardDefModelPresenter):
    __slots__ = (b'__anmType', b'__anmRes', b'__soundId')
...
class CompensationModelPresenter(LootRewardAnimatedModelPresenter):
    ...
    def _formatModel(self, model, ttId, showCongrats):
        ...
        compensationReason = self._reward.get(b'compensationReason', None)
        with model.transaction() as tx:
            tx.setIconFrom(compensationReason.get(b'imgSource', b''))
            tx.setLabelBeforeStr(compensationReason.get(b'label', b''))
            tx.setBonusName(self._reward.get(b'bonusName', b''))
            tx.setLabelBefore(compensationReason.get(b'label', b''))
            tx.setIconBefore(compensationReason.get(b'imgSource', b''))
            tx.setLabelAfter(self._reward.get(b'label', b''))
            tx.setIconAfter(self._reward.get(b'imgSource', b''))
            tx.setLabelAlign(compensationReason.get(b'align', _DEFAULT_ALIGN) or _DEFAULT_ALIGN)
            tx.setLabelAlignAfter(self._reward.get(b'align', _DEFAULT_ALIGN) or _DEFAULT_ALIGN)
            specialArgs = compensationReason.get(b'specialArgs', None)
...
class BlueprintFinalFragmentModelPresenter(LootRewardAnimatedModelPresenter):
    ...
    def _setReward(self, reward):
        ...
        specialArgs = self._reward.get(b'specialArgs', None)
        if specialArgs and isinstance(specialArgs, (types.ListType, types.TupleType)):
            compactDescr = specialArgs[0]
            self.__vehicle = self.__itemsCache.items.getItemByCD(compactDescr)
        else:
            _logger.error(b'SpecialArgs for blueprint is not specified!')
        return
...
class CrewBookModelPresenter(LootRewardDefModelPresenter):
    ...
    def validate(cls, reward, rewardType):
        if rewardType == LootCongratsTypes.INIT_CONGRAT_TYPE_CREW_BOOKS:
            ...
            specialArgs = reward.get(b'specialArgs', None)
...
class BlueprintFragmentRewardPresenter(LootRewardDefModelPresenter):
    ...
    def _setReward(self, reward):
        ...
        if specialArgs and isinstance(specialArgs, (types.ListType, types.TupleType)):
            ...
            _logger.error(b'Can not find vehicle compactDescr in specialArgs!')
        return
...
def checkAndFillCustomizations(bonus, alwaysVisibleBonuses, bonuses):
    ...
    for customBonus in bonus:
        for bonusData in customBonus.getCustomizations():
            bonusType = bonusData.get(b'custType')
            bonusValue = bonusData.get(b'value')
            item = customBonus.getC11nItem(bonusData)
            if bonusType and bonusValue and bonusType == b'style' and _isSpecialStyle(item.intCD):
                hasStyle = True
                break
...
DEF_COMPENSATION_PRESENTERS = {b'vehicles': (VehicleCompensationModelPresenter()), 
...
RANKED_MODEL_PRESENTERS = {b'vehicles': (LootVehicleRewardPresenter())}

def getRewardsBonuses(rewards, size=b'big', awardsCount=_DEFAULT_DISPLAYED_AWARDS_COUNT):
    ...
    if rewards:
        for bonusType, bonusValue in rewards.iteritems():
            if bonusType == b'vehicles' and isinstance(bonusValue, list):
...
def getRewardRendererModelPresenter(reward, presenters=None, compensationPresenters=None, defPresenter=_DEF_MODEL_PRESENTER):
    ...
    bonusName = reward.get(b'bonusName', b'')
    hasCompensation = reward.get(b'hasCompensation', False)
    if hasCompensation:
        compensationReason = reward.get(b'compensationReason', None)
        if compensationReason:
            return compensationPresenters.get(compensationReason.get(b'bonusName'), defPresenter)
...
def getRewardTooltipContent(event, storedTooltipData=None, itemsCache=None):
    if event.contentID not in _COMPENSATION_TOOLTIP_CONTENT_RES_IDS:
        ...
        tooltipData = {b'iconBefore': (event.getArgument(b'iconBefore', b'')), 
           b'labelBefore': (event.getArgument(b'labelBefore', b'')), 
           ...
           b'wsFlags': (ViewFlags.VIEW), 
           b'viewModelClazz': LootBoxCompensationTooltipModel}
        if event.contentID == R.views.common.tooltip_window.loot_box_compensation_tooltip.CrewSkinsCompensationTooltipContent():
            ...
            tooltipData.update({b'iconBefore': (getCrewSkinIconBig(crewSkin.getIconID())), 
               b'labelBefore': (localizedFullName(crewSkin)), 
               b'countBefore': crewSkinCount, 
               b'layoutID': (R.views.common.tooltip_window.loot_box_compensation_tooltip.CrewSkinsCompensationTooltipContent())})
            ...
            tooltipData.update({b'iconBefore': (event.getArgument(b'iconBefore', b'')), 
               b'labelBefore': (event.getArgument(b'labelBefore', b'')), 
               b'iconAfter': (event.getArgument(b'iconAfter', b'')), 
               b'labelAfter': (event.getArgument(b'labelAfter', b'')), 
               ...
               b'viewModelClazz': LootBoxVehicleCompensationTooltipModel, 
               b'layoutID': (R.views.common.tooltip_window.loot_box_compensation_tooltip.LootBoxVehicleCompensationTooltipContent())})
        else:
            ...
            tooltipData.update({b'layoutID': layoutID})
        tooltipData[b'tooltipType'] = tooltipType
    ...
    if rewards:
        for rewardType, rewardValue in rewards.items():
            if rewardType in excluded:
                continue
            if rewardType == b'vehicles':
                vehicles = [vehCD for vehiclesDict in rewardValue for vehCD in vehiclesDict if vehiclesDict[vehCD].get(b'compensatedNumber', 0) < 1]
            elif rewardType == b'currencies':
                currencies = {name: value[b'count'] for name, value in rewardValue.items()}
            else:
...
def getProgressiveRewardBonuses(rewards, size=b'big', maxAwardCount=_DEFAULT_DISPLAYED_AWARDS_COUNT, packBlueprints=False, ctx=None):
    preparationRewardsCurrency(rewards)
    if packBlueprints:
    ...
    if rewards:
        for bonusType, bonusValue in rewards.iteritems():
            if bonusType == b'blueprints':
                bonus = getNonQuestBonuses(bonusType, bonusValue, ctx={b'isPacked': packBlueprints})
...
def getRoyaleBonuses(bonuses, size=b'big'):
    bonuses.sort(key=_keySortOrder)
    alwaysVisibleBonuses = []
    commonBonuses = []
    for b in bonuses:
...
def getRoyaleBonusesFromDict(rewards, size=b'big'):
    alwaysVisibleBonuses = []
    commonBonuses = []
    for bonusType, bonusValue in rewards.iteritems():
...
def getProgressiveRewardVO(currentStep, probability, maxSteps, descText=b'', isEnabled=True, showBg=False, align=prConst.WIDGET_LAYOUT_V, isHighTitle=False, hasCompleted=False):
    ...
    result = {b'steps': stepsVO, 
       b'stepIdx': currentStep, 
       b'widgetAlign': align, 
       b'titleText': (titleFormatter(backport.text(titleText))), 
       b'descText': descText, 
       b'btnTooltip': (makeTooltip(body=TOOLTIPS.PROGRESSIVEREWARD_WIDGET_LINKBTN)), 
       b'rewardTooltip': (TOOLTIPS.PROGRESSIVEREWARD_WIDGET), 
       b'showBg': showBg, 
...
def _checkAndFillBlueprints(blueprintsList, alwaysVisibleBonuses, bonuses):
    ...
    for blueprintBonus in blueprintsList:
        ...
        if blueprintName in BlueprintsBonusSubtypes.USE_CONGRATS:
            ...
            congratsType = blueprintName if blueprintName in BLUEPRINTS_CONGRAT_TYPES else b''
        else:
        ...
        if bonusName == b'vehicles':
            ...
            for vehData in vehicles:
                if b'customCompensation' in vehData:
                    money += Money.makeFromMoneyTuple(vehData[b'customCompensation'])
...
def _packBlueprints(rewards):
    rawBlueprints = rewards.get(b'blueprints', {})
    ...
    rewards[b'blueprints'] = fragments
    return
