...
def getPriceData(itemPrice):
    ...
    if itemPrice == ITEM_PRICE_FREE:
        return (backport.image(_IMAGE.free()), backport.text(_CARD_LOC.free.title()), FREE)
    return (b'', b'', b'')
...
def getPriceDescriptionData(itemPrice, value, descLoc, isHighlight=False):
    args = json.dumps({b'value': value, b'isHighlight': isHighlight}, True)
    if itemPrice.price > MONEY_ZERO_GOLD:
...
def packSkillReset(wulfList, priceData, goodiesCache=None, lobbyContext=None):
    ...
    form = goodiesCache.getRecertificationForm(currency=b'gold')
    if SwitchState.ENABLED.value == lobbyContext.getServerSettings().recertificationFormState() and form.enabled:
        ...
        cardModel.setKwargs(json.dumps({b'value': 100, b'storageCount': formsCount, b'isHighlight': True}, True))
...
def packRetrain(wulfList, priceData):
    ...
    def _packCustomValues(cardVM, customData):
        ...
        roleLevel = (b'{}-{}').format(minLevel, maxLevel) if isRange else minLevel
...
def packRecruit(wulfList, priceData):
    ...
    def _packCustomValues(cardVM, customData):
        ...
        roleLevel = cost.get(b'roleLevel', 0)
