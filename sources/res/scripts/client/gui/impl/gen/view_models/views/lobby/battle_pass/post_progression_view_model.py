from enum import IntEnum
from frameworks.wulf import Array, ViewModel
from gui.impl.gen.view_models.views.lobby.battle_pass.awards_widget_model import AwardsWidgetModel
from gui.impl.gen.view_models.views.lobby.battle_pass.chapter_simple_model import ChapterSimpleModel
from gui.impl.gen.view_models.views.lobby.battle_pass.level_model import LevelModel

class PostProgressionStatus(IntEnum):
    LOCKED = 0
    UNLOCKED = 1
    PAUSED = 2


class PostProgressionViewModel(ViewModel):
    __slots__ = (b'onOpenPointsInfo', b'onOpenInfoPage', b'onOpenChaptersBuyView', b'onClose', b'onProgressAchieved', b'onCycleCompleted')

    def __init__(self, properties=12, commands=6):
        super(PostProgressionViewModel, self).__init__(properties=properties, commands=commands)
        return

    @property
    def awardsWidget(self):
        return self._getViewModel(0)

    @staticmethod
    def getAwardsWidgetType():
        return AwardsWidgetModel

    def getPreviousLevel(self):
        return self._getNumber(1)

    def setPreviousLevel(self, value):
        self._setNumber(1, value)
        return

    def getCurrentLevel(self):
        return self._getNumber(2)

    def setCurrentLevel(self, value):
        self._setNumber(2, value)
        return

    def getPreviousLevelPoints(self):
        return self._getNumber(3)

    def setPreviousLevelPoints(self, value):
        self._setNumber(3, value)
        return

    def getCurrentLevelPoints(self):
        return self._getNumber(4)

    def setCurrentLevelPoints(self, value):
        self._setNumber(4, value)
        return

    def getPreviousCyclesCompletedCount(self):
        return self._getNumber(5)

    def setPreviousCyclesCompletedCount(self, value):
        self._setNumber(5, value)
        return

    def getCyclesCompletedCount(self):
        return self._getNumber(6)

    def setCyclesCompletedCount(self, value):
        self._setNumber(6, value)
        return

    def getEndDate(self):
        return self._getNumber(7)

    def setEndDate(self, value):
        self._setNumber(7, value)
        return

    def getChapterID(self):
        return self._getNumber(8)

    def setChapterID(self, value):
        self._setNumber(8, value)
        return

    def getLevels(self):
        return self._getArray(9)

    def setLevels(self, value):
        self._setArray(9, value)
        return

    @staticmethod
    def getLevelsType():
        return LevelModel

    def getChapters(self):
        return self._getArray(10)

    def setChapters(self, value):
        self._setArray(10, value)
        return

    @staticmethod
    def getChaptersType():
        return ChapterSimpleModel

    def getPostProgressionStatus(self):
        return PostProgressionStatus(self._getNumber(11))

    def setPostProgressionStatus(self, value):
        self._setNumber(11, value.value)
        return

    def _initialize(self):
        super(PostProgressionViewModel, self)._initialize()
        self._addViewModelProperty(b'awardsWidget', AwardsWidgetModel())
        self._addNumberProperty(b'previousLevel', 0)
        self._addNumberProperty(b'currentLevel', 0)
        self._addNumberProperty(b'previousLevelPoints', 0)
        self._addNumberProperty(b'currentLevelPoints', 0)
        self._addNumberProperty(b'previousCyclesCompletedCount', 0)
        self._addNumberProperty(b'cyclesCompletedCount', 0)
        self._addNumberProperty(b'endDate', 0)
        self._addNumberProperty(b'chapterID', 0)
        self._addArrayProperty(b'levels', Array())
        self._addArrayProperty(b'chapters', Array())
        self._addNumberProperty(b'postProgressionStatus')
        self.onOpenPointsInfo = self._addCommand(b'onOpenPointsInfo')
        self.onOpenInfoPage = self._addCommand(b'onOpenInfoPage')
        self.onOpenChaptersBuyView = self._addCommand(b'onOpenChaptersBuyView')
        self.onClose = self._addCommand(b'onClose')
        self.onProgressAchieved = self._addCommand(b'onProgressAchieved')
        self.onCycleCompleted = self._addCommand(b'onCycleCompleted')
        return
