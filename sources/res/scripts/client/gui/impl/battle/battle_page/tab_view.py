...
def checkArenaDataProvider(func):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            _logger.debug(b'[TabView] Trying to load tab view without arena data provider')
...
class TabView(ViewImpl):
    __slots__ = (b'_notificatorManager', b'_visitor', b'_battleCtx', b'__playerIndexes', b'_squadFinder')
    ...
    def _isAlly(self, vehicleInfo):
        return avatar_getter.getPlayerTeam() == vehicleInfo[b'team']
    ...
    def _fillPlayerListModel(self, playerList, playerInfo):
        ...
        try:
            try:
                ...
                LOG_ERROR(self._addStateForLogging(b'Exception while filling player list model. len(playerInfo)=%s' % len(playerInfo)), stack=True)
    ...
    def __setBattlePlayer(self, vehicleId, player):
        if vehicleId not in self.__playerIndexes:
            _logger.warning(b'[TabView]::__setBattlePlayer Vehicle %d was not found in the cached indexes.', vehicleId)
            return
        vehicleInfo = self._visitor.getArenaVehicles()[vehicleId]
        if not vehicleInfo:
            _logger.warning(b'[TabView]::__setBattlePlayer Vehicle %d info not found.', vehicleId)
            return
    ...
    def __getBattlePlayer(self, vehicleId):
        ...
        if vehicleInfo is None:
            _logger.warning(b'[TabView]::__getBattlePlayer Vehicle %d info not found.', vehicleId)
            return
        else:
    ...
    def _fillPlayerModel(self, vehicleId, vehicleInfo):
        ...
        if not playerVehicleID:
            LOG_WARNING(self._addStateForLogging(b'Can not create model for vehicle=%s. Avatar not available.' % vehicleId), stack=True)
            return None
        else:
            player = BattlePlayer()
            ...
            self._invalidateDenunciationInfo(player, vehicleInfo[b'accountDBID'])
            ...
            if isCurrentPlayer:
                ...
                self.viewModel.playerList.setHasClan(bool(vehicleInfo[b'clanAbbrev']))
                player.setAnonymizerTooltip(backport.text(self._getAnonymizerTooltipContent(player)(), fakeName=vehicleInfo[b'fakeName']))
            return player
    ...
    def _getTeam(vehicleInfo):
        if vehicleInfo is not None:
            return vehicleInfo.get(b'team', 0)
        else:
            return 0
    ...
    def _getPrebattleID(vehicleInfo):
        if vehicleInfo is not None:
            return vehicleInfo.get(b'prebattleID', 0)
        else:
            return 0
    ...
    def _invalidateVehicleStats(self, player):
        ...
        if vehicleId:
            ...
            kills = stats.get(b'frags', 0) - stats.get(b'tkills', 0)
    ...
    def _invalidateVehicleTypeInfo(self, player):
        ...
        if not vehicleId:
            ...
            if vehicleInfo is None:
                return
            vehicleTypeInfo = vehicleInfo[b'vehicleType']
            if vehicleTypeInfo:
                ...
                fillPrestigeEmblemModel(player.prestigeEmblemModel, vehicleInfo[b'prestigeLevel'], vehicleTypeInfo.type.compactDescr)
            return
    ...
    def _invalidateVehicleStatus(self, player):
        ...
        vehicleInfo = self._getVehicleInfo(vehicleId)
        if vehicleInfo:
            isTeamKiller = vehicleInfo[b'isTeamKiller']
            if player.getIsCurrentPlayer():
                isTeamKiller |= avatar_getter.isPlayerTeamKillSuspected()
            player.setIsTeamKiller(isTeamKiller)
            player.setIsKilled(not vehicleInfo[b'isAlive'])
            player.setIsLoaded(vehicleInfo[b'isAvatarReady'])
        return
    ...
    def _invalidatePersonalInfo(self, player):
        ...
        if not vehicleId:
            ...
            if vehicleInfo is None:
                return
            player.setClanAbbrev(vehicleInfo[b'clanAbbrev'])
            player.setDatabaseID(vehicleInfo[b'accountDBID'])
            player.setAvatarSessionID(vehicleInfo[b'avatarSessionID'] or b'')
            player.setHiddenUserName(vehicleInfo[b'fakeName'])
            player.setIgrType(vehicleInfo[b'igrType'])
            player.setUserName(vehicleInfo[b'name'])
            prefixBadge, suffixBadge = getSelectedByLayout(vehicleInfo[b'badges'][0])
        ...
        if not playerVehicleID:
            ...
            if ownVehicleInfo is None:
                if not self.sessionProvider.isReplayPlaying:
                    _logger.warning(b'[TabView] Vehicle %d info not found.', ownVehicleInfo)
                return set()
            if received:
                return {inv.creatorID for inv in invites if inv.receiverID == ownVehicleInfo[b'avatarSessionID']}
            return {inv.receiverID for inv in invites if inv.creatorID == ownVehicleInfo[b'avatarSessionID']}
    ...
    def _updateStatus(self, vehicleId, *args):
        if vehicleId not in self.__playerIndexes:
            ...
            with self.modifyBattlePlayer(vehicleId) as player:
                ...
                vehicleInfo = self._visitor.getArenaVehicles().get(vehicleId)
                if not vehicleInfo:
                    _logger.warning(b'[TabView] Vehicle %d info not found.', vehicleId)
                    return
                self._invalidateVehicleStatus(player)
            return
    ...
    def __getLiveTagVehicleSets(self, teamInfoLiveTagsComponent):
        return {tagInfo[b'tag']: set(tagInfo[b'vehicles']) for tagInfo in teamInfoLiveTagsComponent.liveTags}
    ...
    def _getLiveTagTooltipContent(self, player, validator):
        ...
        resStrBody = R.strings.tooltips.liveTags.body.dyn(b'currentPlayer' if player.getIsCurrentPlayer() else b'ally')
    ...
    def _canBeCommended(self, vehicleId):
        ...
        if vehicleId in ignoredUsers:
            ...
            if vehicleInfo is None:
                _logger.warning(b'[TabView] Vehicle %d info not found.', vehicleId)
                return False
            return self._visitor.hasCommendationsMessages() and self._isAlly(vehicleInfo) and vehicleInfo[b'avatarSessionID']
    ...
    def handleTabChange(self, tabAlias):
        _logger.debug(b'_handleFullStatsSelected: alias[%s]', tabAlias)
    ...
    def _addStateForLogging(self, msg):
        extMsg = b' [StateData]: playerIndexes: %s, battlePeriod: %s'
        extMsg = extMsg % (self.__playerIndexes, getattr(self.arena, b'period', -1))
        return msg + extMsg
