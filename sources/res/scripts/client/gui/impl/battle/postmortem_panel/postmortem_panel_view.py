...
class PostmortemPanelView(ViewImpl, CallbackDelayer):
    ...
    def isRatingWidgetEnabled(self):
        ...
        return (config and hasBonusCap and (config.enabledInterfaces.spectatorMode) and (config.enabled)) or False
    ...
    def __onRateButtonClick(self, rating):
        ...
        if rating is PlayerSatisfactionRating.NONE:
            _logger.warning(b'Received unmappable rating from widget callback: %s', rating)
            return
