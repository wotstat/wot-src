...
def getComp7DetailedHelpPages():
    return GUI_SETTINGS.comp7DetailedHelpPages.get(b'pages', [])


def getWhatsNewPages():
    return GUI_SETTINGS.whatsNewPageComp7Slides.get(b'pages', [])


def getIntroPages():
    return GUI_SETTINGS.IntroPagesComp7Slides.get(b'pages', [])


def getWhatsNewSeasonVehicles():
    return GUI_SETTINGS.whatsNewPageComp7Slides.get(b'seasonVehicles', [])


def getIntroVehicles():
    return GUI_SETTINGS.IntroPagesComp7Slides.get(b'seasonVehicles', [])


def getWhatsNewMapsAdded():
    return GUI_SETTINGS.whatsNewPageComp7Slides.get(b'newMaps', [])


def getWhatsNewMapsDeleted():
    return GUI_SETTINGS.whatsNewPageComp7Slides.get(b'deprecatedMaps', [])
    ...
    if version is None:
        ...
        if data[GuiSettingsBehavior.COMP7_VERSION_FLAG] != version:
            ...
            if version > MAX_VERSION:
                _logger.error(b'[COMP7] Version must be less 15')
