...
def showTournaments(url=None):
    ...
    g_eventBus.handleEvent(events.LoadViewEvent(SFViewLoadParams(alias, getUniqueViewName(alias)), ctx={b'url': (_getIgbHost() or url), 
       b'webHandlers': (createStrongholdsWebHandlers()), 
       b'returnAlias': (VIEW_ALIAS.LOBBY_HANGAR), 
       b'onServerSettingsChange': _serverSettingChanged}), EVENT_BUS_SCOPE.LOBBY)
    return
