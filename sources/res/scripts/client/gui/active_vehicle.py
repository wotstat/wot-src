...
class ActiveVehicleSeasonType(object):
    ...
    def __init__(self):
        self._vehTypeDescrToSeason = {b'customizationDisplayType': {}}
        return
    ...
    def __setitem__(self, vehTypeDescr, seasonType):
        if seasonType not in SeasonType.RANGE and seasonType != SeasonType.UNDEFINED:
            raise SoftException((b"Invalid SeasonType '{}'").format(seasonType))
    ...
    def __updateServer(self, vehTypeDescr, seasonType):
        ...
        if player is not None and hasattr(player, b'_doCmdInt2'):
    ...
    def __onSetActiveVehSeason(self, requestID, resultID, errorStr, ext=None):
        ...
        if isinstance(ext, dict) and ext.get(b'exception_message'):
            logFunc = _logger.error
        logFunc(b'__onSetActiveVehSeason requestID=%s resultID=%s, errorStr=%s, ext=%s', requestID, resultID, errorStr, ext)
        return
