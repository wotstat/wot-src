...
class BattleClientCache(IBattleClientCache):
    ...
    def save(self):
        ...
        blob = (b'').join(self.__chunks.itervalues())
        if len(blob) > _CACHE_MAX_LENGTH:
            LOG_ERROR((b'Could not store client cache on the server. Exceeded max size: {} > {}').format(len(blob), _CACHE_MAX_LENGTH))
            return False
        else:
            player = BigWorld.player()
            if player is None or not hasattr(player, b'storeClientCtx'):
                LOG_DEBUG(b'Avatar.storeClientCtx not found', player)
                return False
    ...
    def load(self):
        player = BigWorld.player()
        if player is None or not hasattr(player, b'clientCtx'):
            LOG_DEBUG(b'Avatar.clientCtx not found', player)
            return False
        else:
            ...
            if blob:
                ...
                while offset < len(blob):
                    ...
                    elif recordID in self.__records:
    ...
    def _unpackRecordHeader(cls, offset, buf):
        try:
            ...
            LOG_WARNING(b'Could not unpack record header: ', buf[offset:offset + _RECORD_HEADER_SIZE], e)
            recordID = 0
            recordLen = 0
    ...
    def _unpackRecord(cls, record, chunk):
        ...
        if chunk:
            try:
                ...
                status = False
                LOG_WARNING(b'Could not unpack the following record: ', chunk, e)
