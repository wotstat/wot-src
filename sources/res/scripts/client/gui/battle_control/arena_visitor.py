...
class _ArenaBonusTypeVisitor(IArenaVisitor):
    __slots__ = (b'_bonusType', b'_specificOverrides')
...
class _ArenaExtraDataVisitor(IArenaVisitor):
    ...
    @property
    def queueType(self):
        return self._extra.get(b'queueType', QUEUE_TYPE.UNKNOWN)

    def isLowLevelBattle(self):
        return 0 < self._extra.get(b'battleLevel', 0) < 4

    def isMapsInDevelopmentEnabled(self):
        return self._extra.get(b'isRandomEventsAllowed', False)

    def isGfHudMechanicEnabled(self, mechanicKey):
        paramsName = mechanicKey.mechanic.value
        return paramsName in self._extra.get(b'gfHudMechanics', frozenset())
...
class _ArenaVehiclesVisitor(IArenaVisitor):
    ...
    def getVehicleExtras(self, vehicleID):
        try:
            extras = self._vehicles[vehicleID][b'vehicleType'].extras[:]
...
class _ArenaModifiersVisitor(IArenaVisitor):
    __slots__ = (b'_modifiers', b'_shellData')
...
class _ClientArenaVisitor(IClientArenaVisitor):
    __slots__ = (b'__weakref__', b'_arena', b'_canSubscribe', b'_gui', b'_bonus', b'_type', b'_extra', b'_vehicles', b'_modifiers')
