...
class ViewPointsController(IViewPointsController):
    __slots__ = (b'__points', b'__arenaDP', b'__currentViewPointID', b'__currentVehicleID', b'__normalSortKey', b'__squadManSortKey')
    ...
    def switch(self, isNext=True):
        ...
        items = [(False, vInfo.vehicleID) for vInfo, _ in vehicles if b'unobservable' not in vInfo.vehicleType.tags]
    ...
    def __doSelect(self, isViewpoint, vehOrPointId):
        if isViewpoint:
            if vehOrPointId == self.__currentViewPointID:
                LOG_DEBUG(b'Skip switch to current view point!')
                return True
