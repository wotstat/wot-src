...
import BigWorld, BattleReplay, CommandMapping
...
_CONSUMERS_LOCKS = (
 ...
 b'chat')
CommandReceivedData = namedtuple(b'CommandReceivedData', (b'name', b'targetIdToAnswer'))
...
class CalloutController(CallbackDelayer, IViewComponentsController):
    ...
    __slots__ = (b'__isActive', b'__isCalloutEnabled', b'__isIBCEnabled', b'__commandReceivedData', b'__lastPersonalMsgTimestamp', b'__lastCalloutTimestamp', b'__ui', b'__radialKeyDown', b'__radialMenuIsOpen', b'__previousForcedGuiControlModeFlags')
    ...
    def __handleCalloutButtonEvent(self, event):
        ...
        if commands is None:
            ...
            if self.__commandReceivedData.name in _CALLOUT_COMMANDS_TO_REPLY_COMMANDS:
                ...
                _logger.error(b'Unsupported chat command name(%s) for replying to player(%d)', self.__commandReceivedData.name, self.__commandReceivedData.targetIdToAnswer)
            self.__executeHide(True, self.__commandReceivedData.name)
            return
