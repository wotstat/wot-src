...
class SoundEvents(object):
    EQUIPMENT_ACTIVATED = {(artefacts.AttackArtilleryFortEquipment): b'gui_clan_abl_art_apply'}
    EQUIPMENT_PREPARING = {(artefacts.AttackArtilleryFortEquipment): b'gui_clan_abl_art_aim'}
    EQUIPMENT_PREPARING_CANSELED = {(artefacts.AttackArtilleryFortEquipment): b'gui_clan_abl_art_cancel'}
    INSPIRE_ENTER = b'gui_clan_abl_insp_enter'
    INSPIRE_EXIT = b'gui_clan_abl_insp_exit'
    INSPIRE_START = b'gui_clan_abl_insp_start'
    INSPIRE_END = b'gui_clan_abl_insp_end'
    ARTILLERY_ENTER = b'gui_clan_abl_art_enter'
    ARTILLERY_EXIT = b'gui_clan_abl_art_exit'
    ARTILLERY_HIT_PC = b'imp_small_splash_HE_NPC_PC'
    ARTILLERY_HIT_NPC = b'imp_small_pierce_HE_NPC_NPC'


class StrongholdBattleSoundController(SoundPlayersBattleController):

    def startControl(self, *args):
        WWISE.activateRemapping(b'stronghold')
        super(StrongholdBattleSoundController, self).startControl(*args)
        return

    def stopControl(self):
        WWISE.deactivateRemapping(b'stronghold')
...
class _InspireSoundPlayer(VehicleStateSoundPlayer, CallbackDelayer):
    ...
    def _onVehicleStateUpdated(self, state, value):
        if not state == VEHICLE_VIEW_STATE.INSPIRE:
            ...
            isSource = value[b'isSourceVehicle']
            self.__isSource = isSource if isSource is not None else self.__isSource
            isInactivation = value[b'isInactivation']
            ...
            if value[b'duration'] is not None and totalDuration > 0:
                if self.__isSource:
...
class _AOEZoneSoundPlayer(VehicleStateSoundPlayer, CallbackDelayer):
    ...
    def _onVehicleStateUpdated(self, state, value):
        if not state == VEHICLE_VIEW_STATE.AOE_ZONE:
            ...
            if zoneData is None:
                ...
                self.__tillFinishAttackTime = 0
                return
            strikeAttackStartTime = zoneData[b'startTime'] - BigWorld.serverTime()
            strikeAttackEndTime = zoneData[b'endTime'] - BigWorld.serverTime()
    ...
    def __findLongest(zones):
        longestZone = None
        for zone in zones:
            equipment = vehicles.g_cache.equipments().get(zone[b'equipmentID'])
            if isinstance(equipment, artefacts.AttackArtilleryFortEquipment):
                if longestZone is None:
                    longestZone = zone
                elif longestZone[b'endTime'] < zone[b'endTime']:
                    longestZone = zone
