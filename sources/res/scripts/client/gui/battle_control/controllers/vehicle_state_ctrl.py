...
class _SpeedStateHandler(_StateHandler):
    __slots__ = (b'__speed', b'__maxSpeed', b'__isOwnVehicle')
    ...
    def _invalidate(self, vehicle):
        fwdSpeedLimit, bckwdSpeedLimit = vehicle.typeDescriptor.physics[b'speedLimits']
...
class _VehicleUpdater(object):
    ...
    def start(self):
        if self.__updateTI is None:
            self.__updateTI = TimeInterval(VEHICLE_UPDATE_INTERVAL, self, b'_update')
