import weakref, BigWorld, BattleReplay, Event
    ...
    def showAllyHitMessage(self, vehicleID=None, isMultiHit=False):
        if vehicleID is not None:
            ...
            if Supply.isSupply(vType.tags):
                self.onShowPlayerMessageByKey(b'ALLY_HIT_SUPPLY', {b'entity': ((b'({})').format(vType.shortNameWithPrefix))}, (
                 (
                  b'entity', vehicleID),))
                return
...
class BattleRoyaleBattleMessagesController(BattleMessagesController):
    ...
    def __playerChangedTeam(self):
        ...
        if not arenaDP:
            return False
        return arenaDP.getAllyTeams()[0] != BigWorld.player().team
