...
class SpectatorViewController(ViewComponentsController):
    ...
    def markerWasHovered(self, isHovered, targetID):
        if not isHovered or targetID is None:
            ...
            if vehicleData is not None:
                ...
                isUnobservable = vehicleDescription and b'unobservable' in vehicleDescription.tags
    ...
    def spectatorViewModeChanged(self, mode):
        ...
        g_eventBus.handleEvent(GameEvent(DeathCamEvent.DEATH_CAM_SPECTATOR_MODE, ctx={b'mode': mode}), scope=EVENT_BUS_SCOPE.BATTLE)
        return
