...
class ChatCommandsController(IBattleController):
    __slots__ = (b'__isEnabled', b'_arenaDP', b'__feedback', b'__ammo', b'__markersManager', b'__chatCommandsEnabled', b'__logger')
    ...
    def sendReplyChatCommand(self, targetID, action):
        ...
        self._sendChatCommand(command, b'Reply command not valid for command id: %d' % targetID)
        return
    ...
    def sendCancelReplyChatCommand(self, targetID, action):
        ...
        self._sendChatCommand(command, b'Cancel reply command not valid for command id: %d' % targetID)
        return
    ...
    def sendClearChatCommandsFromTarget(self, targetID, targetMarkerType):
        ...
        self._sendChatCommand(command, b'Clear chat commands not valid for command id: %d and marker type: %s' % (
    ...
    def sendAttentionToPosition3D(self, position, name):
        ...
        self._sendChatCommand(command, b'Minimap command for position (%d, %d, %d) not found' % (
    ...
    def sendAttentionToObjective(self, hqIdx, isAtk, action=None):
        if self.__isEnabled is False:
            ...
            self._sendChatCommand(command, b'Minimap command for objective with id: %d not found' % hqIdx)
            return
    ...
    def sendReloadingCommand(self):
        ...
        if tempCtrl is not None and tempCtrl.isOverheated:
            ...
            self._sendChatCommand(command, b'reloading command')
            return

    def _sendChatCommand(self, command, cmdName=None):
        if command is None:
            _logger.error(b'Command not found: %s', cmdName or b'<unknown>')
            return
        else:
