...
CONTEXTCOMMAND = b'CONTEXTCOMMAND'
CONTEXTCOMMAND2 = b'CONTEXTCOMMAND2'
...
class ChatCommandsController(IBattleController):
    __slots__ = (b'__isEnabled', b'__arenaDP', b'__feedback', b'__ammo', b'__markersManager', b'_uiPlayerSatisfactionRatingLogger')
                ...
                if cmdMap.isFired(keyboardCmd, key):
                    ...
                    if replyState == ReplyState.NO_REPLY:
                        if chatCmd in TARGET_TYPE_TRANSLATION_MAPPING and targetMarkerType in TARGET_TYPE_TRANSLATION_MAPPING[chatCmd] and markerSubType in TARGET_TYPE_TRANSLATION_MAPPING[chatCmd][targetMarkerType]:
                            ...
                            _logger.debug(b'Action %s (at key %s) is not supported for target type %s (cut type: %s)', chatCmd, key, targetMarkerType, markerSubType)
                            return
    ...
    def sendReplyChatCommand(self, targetID, action):
        ...
        if command:
            ...
            _logger.error(b'Reply command not valid for command id: %d', targetID)
        return
    ...
    def sendCancelReplyChatCommand(self, targetID, action):
        ...
        if command:
            ...
            _logger.error(b'Cancel reply command not valid for command id: %d', targetID)
        return
    ...
    def sendClearChatCommandsFromTarget(self, targetID, targetMarkerType):
        ...
        if command:
            ...
            _logger.error(b'Clear chat commands not valid for command id: %d and marker type: %s', targetID, targetMarkerType)
        return
    ...
    def sendAttentionToPosition3D(self, position, name):
        ...
        if command:
            ...
            _logger.error(b'Minimap command for position (%d, %d, %d) not found', positionVec.x, positionVec.y, positionVec.z)
        return
    ...
    def sendAttentionToObjective(self, hqIdx, isAtk, action=None):
        if self.__isEnabled is False:
            ...
            if command:
                ...
                _logger.error(b'Minimap command for objective with id: %d not found', hqIdx)
            return
    ...
    def sendCommandToBase(self, baseIdx, cmdName, baseName=b''):
        ...
        if command:
            ...
            _logger.error(b'Command not found: %s', cmdName)
        return
    ...
    def sendEpicGlobalCommand(self, cmdName, baseName=b''):
        if self.__isProhibitedToSendIfDeadOrObserver(cmdName) or self.__isEnabled is False:
            return
    ...
    def sendTargetedCommand(self, cmdName, targetID, isInRadialMenu=False):
        ...
        if command:
            ...
            _logger.error(b'Targeted command(%s) was not found or targetID(%d) is not valid', cmdName, targetID)
        return
    ...
    def sendReloadingCommand(self):
        ...
        if command:
            ...
            _logger.error(b'Can not create reloading command')
        return
    ...
    def __getAimedAtVehicleOrObject(self):
        ...
        if self.__isTargetCorrect(player, target):
            ...
            elif target.publicInfo[b'team'] == avatar_getter.getPlayerTeam():
                targetID = target.id
        ...
        if target is not None and isinstance(target, Vehicle.Vehicle):
            if target.isAlive():
                if player is not None and isPlayerAvatar():
                    ...
                    isAlly = target.publicInfo[b'team'] == player.team
