...
class DynSquadArenaController(object):
    ...
    def __handleBattleLoading(self, event):
        ...
        self._isBattleLoadingVisible = event.ctx[b'isShown']
        return
    ...
    def _inviteReceived(self, invite):
        LOG_DEBUG(b'Handler: Invite received', invite)
        return

    def _inviteSent(self, invite):
        LOG_DEBUG(b'Handler: Invite sent', invite)
        return

    def _squadCreatedImOwner(self, squadNum):
        LOG_DEBUG(b'Handler: Squad created by myself, Squad Number = ', squadNum)
        return

    def _squadCreatedImRecruit(self, squadNum):
        LOG_DEBUG(b"Handler: Squad created and I'm not commander, Squad Number = ", squadNum)
        return

    def _squadCreatedByAllies(self, squadNum):
        LOG_DEBUG(b'Handler: Allies created new squad: Squad number = ', squadNum)
        return

    def _squadCreatedByEnemies(self, squadNum):
        LOG_DEBUG(b'Handler: Enemies created new squad: Squad number = ', squadNum)
        return

    def _iAmJoinedSquad(self, squadNum):
        LOG_DEBUG(b"Handler: I'm just have joined the squad: Squad number = ", squadNum)
        return

    def _someoneJoinedAlliedSquad(self, squadNum, receiver):
        LOG_DEBUG(b'Handler: Someone just have joined Allied squad: Squad Number, receiver = ', squadNum, receiver)
        return

    def _someoneJoinedEnemySquad(self, squadNum, receiver):
        LOG_DEBUG(b'Handler: Someone just have joined Enemy squad: Squad Number, receiver = ', squadNum, receiver)
        return

    def _someoneJoinedMySquad(self, squadNum, receiver):
        LOG_DEBUG(b'Handler: Someone just have joined MY squad: Squad Number, receiver = ', squadNum, receiver)
        return
...
class DynSquadMessagesController(DynSquadArenaController):
    ...
    def __sendMessage(self, resource, **kwargs):
        g_messengerEvents.onCustomMessage(_DynSquadActionMessage(backport.text(resource(), **kwargs), kwargs.get(b'squadType', DYN_SQUAD_TYPE.OWN), kwargs.get(b'squadNum')))
        return
...
_DYN_SQUAD_PLATOON_JOINED = b'dyn_squad_platoon_joined'
_DYN_SQUAD_PLATOON_CREATED = b'dyn_squad_platoon_created'
_DYN_SQUAD_PLATOON_DISMISSED = b'dyn_squad_platoon_dismissed'
_DYN_SQUAD_BEEN_EXCLUDED_PLATOON = b'dyn_squad_been_excluded_platoon'
_DYN_SQUAD_LEFT_PLATOON = b'dyn_squad_left_platoon'
_DYN_SQUAD_ASSISTANCE_BEEN_REQUESTED = b'dyn_squad_assistance_been_requested'
_DYN_SQUAD_PLAYER_JOINED_PLATOON = b'dyn_squad_player_joined_platoon'
...
class _DynSquadSoundsController(DynSquadArenaController):
    ...
    def __playSound(self, sound):
        if self._isBattleLoadingVisible:
            ...
            if notifications is not None and hasattr(notifications, b'play'):
                notifications.play(sound)
            return


class DynSquadFunctional(IArenaVehiclesController):
    __slots__ = (b'__soundCtrl', b'__msgsCtrl', b'__eManager', b'__wasOwnSquadCreated', b'onDynSquadCreatedOrJoined')
