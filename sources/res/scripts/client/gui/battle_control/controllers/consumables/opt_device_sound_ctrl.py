...
_INIT_OPT_DEVICE_EVENTS = {b'improvedVentilation': (b'cons_ventilation_start', b'cons_ventilation_stop'), 
   b'additionalInvisibilityDevice': (b'cons_tracks_start', b'cons_tracks_stop'), 
   b'grousers': (b'cons_grousers_start', b'cons_grousers_stop'), 
   b'antifragmentationLining': (b'cons_spall_liner_start', b'cons_spall_liner_stop')}

class OptDeviceSoundController(object):
    __slots__ = (b'__playerOptDevices', b'__stopSoundEvents', b'__schedulerToolbox', b'__vehicleStateUpdatedHandlers', b'__radioCommunicationHelper')
    ...
    def playLightbulbEffect(self):
        if b'improvedRadioCommunication' in self.__playerOptDevices:
            SoundGroups.g_instance.playSound2D(b'cons_radio_interference')
        return

    def needGunRammerEffect(self):
        return b'tankRammer' in self.__playerOptDevices

    def playEnemySightedEffect(self, targetID):
        position = Math.Vector3(BigWorld.entities[targetID].position)
        SoundGroups.g_instance.playCameraOriented(b'cons_radio_sighted_for_team', position)
        return

    def onVehicleDetected(self, feedback):
        if b'improvedRadioCommunication' not in self.__playerOptDevices:
            return
    ...
    def __updateDeviceState(self, deviceName, deviceState, _=None):
        if b'improvedConfiguration' in self.__playerOptDevices:
            if deviceState == b'repaired':
                if self.__schedulerToolbox.isPlanned(deviceName):
                    self.__schedulerToolbox.delete(deviceName)
                    SoundGroups.g_instance.playSound2D(b'cons_toolbox_stop')
            elif deviceState == b'destroyed':
                self.__schedulerToolbox.delete(deviceName)
            elif deviceState == b'normal':
                if self.__schedulerToolbox.isPlanned(deviceName):
                    self.__schedulerToolbox.delete(deviceName)
                    SoundGroups.g_instance.playSound2D(b'cons_toolbox_stop')
        return
...
class _ImprovedRadioCommunicationEvent(object):
    ...
    def setSpotted(self, vehicleID):
        ...
        if vehicle is not None:
            SoundGroups.g_instance.playCameraOriented(b'cons_radio_sighted_for_team', Math.Vector3(vehicle.position))
        else:
    ...
    def startVehicleVisual(self, vProxy, isImmediate):
        if isImmediate and vProxy.id in self.__spottedVehicles:
            if abs(BigWorld.time() - self.__spottedVehicles[vProxy.id]) < self._TIME_DELTA:
                SoundGroups.g_instance.playCameraOriented(b'cons_radio_sighted_for_team', Math.Vector3(vProxy.position))
...
class _SchedulerToolboxSoundEvent(object):
    ...
    def __onPlay(self, deviceName=None):
        ...
        SoundGroups.g_instance.playSound2D(b'cons_toolbox_start')
        return
