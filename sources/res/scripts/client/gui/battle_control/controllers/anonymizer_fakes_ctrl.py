...
class _RelationData(object):
    __slots__ = (b'vehicleID', b'sessionID', b'databaseID', b'name')
...
class AnonymizerFakesController(IAnonymizerFakesController):
    __slots__ = (b'__fakeIDs', b'__relationsCache', b'__arenaDP', b'__avatarSessionID', b'__postProcs', b'__cooldown', b'__mergedDBIDs')
    ...
    def addBattleFriend(self, avatarSessionID):
        if self.__isCooldown(CLIENT_ACTION_ID.ADD_FRIEND):
            ...
            if user is not None and user.isFriend():
                _logger.error(b'%s is already friend', avatarSessionID)
                return
    ...
    def removeBattleFriend(self, avatarSessionID):
        if self.__isCooldown(CLIENT_ACTION_ID.REMOVE_FRIEND):
            ...
            if user is not None and not user.isFriend():
                _logger.error(b'%s is not a friend', avatarSessionID)
                return
    ...
    def addBattleIgnored(self, avatarSessionID):
        if self.__isCooldown(CLIENT_ACTION_ID.ADD_IGNORED):
            ...
            if user is not None and user.isIgnored() and not user.isTemporaryIgnored():
                _logger.error(b'%s is already permanent ignored', avatarSessionID)
                return
    ...
    def removeBattleIgnored(self, avatarSessionID):
        if self.__isCooldown(CLIENT_ACTION_ID.REMOVE_IGNORED):
            ...
            if user is not None and (not user.isIgnored() or user.isTemporaryIgnored()):
                _logger.error(b'%s is not permanent ignored', avatarSessionID)
                return
        ...
        if self.__isCooldown(CLIENT_ACTION_ID.SET_MUTE):
            ...
            if not user.isMuted():
                ...
                if accDBID > 0:
                    ...
                    _logger.debug(b'we do not know accDBID about %s. real mute call skipped', avatarSessionID)
                ...
                _logger.error(b'%s is already muted', avatarSessionID)
            return
    ...
    def unmute(self, avatarSessionID):
        if self.__isCooldown(CLIENT_ACTION_ID.UNSET_MUTE):
            ...
            if user is not None:
                if not user.isMuted():
                    _logger.error(b'%s is not muted', avatarSessionID)
                    return
                accDBID = self.__getAccDBID(avatarSessionID)
                ...
                if accDBID > 0:
                    ...
                    _logger.debug(b'we do not know accDBID about %s. real unmute call skipped', avatarSessionID)
                ...
                _logger.error(b'there is no muted entity for %s', avatarSessionID)
            return
        ...
        if self.__isCooldown(CLIENT_ACTION_ID.ADD_IGNORED):
            ...
            if user is not None:
                if user.isTemporaryIgnored():
                    _logger.error(b'%s is already temporary ignored', avatarSessionID)
                    return
                vehID = self.__arenaDP.getVehIDBySessionID(avatarSessionID)
                if not vehID:
                    _logger.error(b'can not get vehicleID for %s', avatarSessionID)
                    return
                ...
                if isCacheChanged:
                    ...
                    _logger.error(b'%s is already in battle cache as tmp ignored', avatarSessionID)
                self.__cooldown.process(CLIENT_ACTION_ID.ADD_IGNORED)
                g_messengerEvents.users.onBattleUserActionReceived(USER_ACTION_ID.TMP_IGNORED_ADDED, user)
            else:
                _logger.error(b'addTmpIgnored: mute does not create entity for %s', avatarSessionID)
            return
        ...
        if self.__isCooldown(CLIENT_ACTION_ID.REMOVE_IGNORED):
            ...
            if user is not None:
                if not user.isTemporaryIgnored():
                    _logger.error(b'%s is not temporary ignored', avatarSessionID)
                    return
                vehID = self.__arenaDP.getVehIDBySessionID(avatarSessionID)
                if not vehID:
                    _logger.error(b'can not get vehicleID for %s', avatarSessionID)
                ...
                if isCacheChanged:
                    ...
                    _logger.error(b'%s is not in battle cache as tmp ignored', avatarSessionID)
                self.__cooldown.process(CLIENT_ACTION_ID.REMOVE_IGNORED)
                g_messengerEvents.users.onBattleUserActionReceived(USER_ACTION_ID.TMP_IGNORED_REMOVED, user)
            else:
                _logger.error(b'there is no temporary ignored entity for %s', avatarSessionID)
            return
    ...
    def __modifyRelations(self, avatarSessionID, actionID):
        ...
        if not vehicleData.isValid(avatarSessionID):
            _logger.error(b'can not get vehicleInfo for %s', vehicleID)
            return
        ...
        if isCacheChanged:
            ...
            _logger.error(b'cache is not modified after %s on %s ', actionID, avatarSessionID)
        return
