...
class BurstPredictor(IAutoShootGunController.IBurstPredictor, CallbackDelayer):
    __slots__ = (b'__isShootingPossible', b'__needDeactivation', b'__predActivationTime', b'__state', b'__em', b'onStateChanged', b'onStateUpdated')
    ...
    def __invalidateState(self, newState):
        ...
        if isStateChanged:
            _logger.debug(b'BurstPredictor new state %s %s', AutoShootPredictionState.NAMES[newState], BigWorld.time())
...
def createBurstPredictor(setup):
    if setup.isReplayPlaying:
        return BurstReplayPredictor()
    return BurstPredictor()
