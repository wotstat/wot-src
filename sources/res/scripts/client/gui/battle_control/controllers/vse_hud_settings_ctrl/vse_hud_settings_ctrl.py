...
class VSEHUDSettingsController(IVSEHUDSettingsController):
    __slots__ = (b'_settings', b'_eManager', b'onSettingsChanged', b'onItemSettingsChanged')
    ...
    def setSettings(self, settingsID, settings):
        _logger.debug(b'New pve hud settings: settingsID=%s settings=%s.', settingsID, settings)
    ...
    def getSettings(self, settingsID):
        ...
        if settings is None:
            _logger.debug(b'Settings for settingsID=%s not set.', settingsID)
            return
        else:
            return settings

    def setItemSettings(self, settingsID, itemID, settings):
        _logger.debug(b'New pve hud item settings: settingsID=%s itemID=%s settings=%s.', settingsID, itemID, settings)
        itemsSettings = self._settings.setdefault(settingsID, {})
        if itemID in itemsSettings:
            _logger.debug(b'Item settings for settingsID=%s itemID=%s already set.', settingsID, itemID)
    ...
    def getItemSettings(self, settingsID, itemID):
        ...
        if itemsSettings is None:
            ...
            if itemSetting is None:
                _logger.debug(b'Item settings for settingsID=%s itemID=%s not set.', settingsID, itemID)
                return
            return itemSetting
