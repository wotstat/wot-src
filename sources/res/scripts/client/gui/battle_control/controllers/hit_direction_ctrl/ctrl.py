...
class HitDirectionController(IViewComponentsController):
    __slots__ = (b'__uiHitComponents', b'__isVisible', b'__callbackIDs', b'__damageIndicatorCrits', b'__damageIndicatorAllies', b'__damageIndicatorExtType', b'__arenaDP', b'__weakref__')
    ...
    def setViewComponents(self, *components):
        for component in components:
            ...
            if hitType not in self.__uiHitComponents:
                _logger.error(b'Controller doesnt have the component with hit type(%s)', hitType)
    ...
    def __handleGUIVisibility(self, event):
        self.setVisible(event.ctx[b'visible'])
        return
...
class EpicHitDirectionController(HitDirectionController):
    ...
    def _getAttackerTag(self, attackerVehType):
        if Supply.isSupply(attackerVehType.tags):
            return Supply.getSupplyTag(attackerVehType)
        return super(EpicHitDirectionController, self)._getAttackerTag(attackerVehType)
