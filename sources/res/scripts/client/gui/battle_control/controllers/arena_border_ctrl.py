...
class ArenaBorderController(IArenaLoadController):
    ...
    def _applySetting(self, showMode, drawType, color):
        ...
        g_eventBus.handleEvent(GameEvent(GameEvent.ARENA_BORDER_TYPE_CHANGED, {b'drawType': drawType}), scope=EVENT_BUS_SCOPE.BATTLE)
        distanceFaderMode = 0
        ...
        BigWorld.ArenaBorderHelper.updatePolygonBordersColor(self.__spaceID, self.settingsCore.getSetting(b'isColorBlind'))
        return
...
def createBorderController(setup):
    ...
    if guiVisitor.isBattleRoyale():
        return BattleRoyaleBorderCtrl()
    return ArenaBorderController()
