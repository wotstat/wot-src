...
SPOTTING_INDICATORS_PRIORITY = (
 ...
 (
  _TYPE.SPECIAL_RECON,))
_PRIORITY_BY_DETECTION_TYPE = {detectionType: priority for detectionType in enumerate(SPOTTING_INDICATORS_PRIORITY)}
...
def bindSpottingIndicator(indicator):
    ...
    if spot is not None:
        ...
        _logger.error(b'Tryng to bind this indicator: %s, when controller is None', indicator)
    return
...
class SpottingIndicatorsController(ISpottingIndicatorsController):
    __slots__ = (b'onSpottingIndicatorAction', b'_vehicleState', b'__detectionType', b'__visToggleByType', b'_durationByType', b'__showTimerID', b'__prevControllingVehID', b'__enabledByType', b'__weakref__')
...
class _Replay(SpottingIndicatorsController):
    ...
    def __repr__(self):
        return b'_Replay(SpottingIndicatorsController)'
