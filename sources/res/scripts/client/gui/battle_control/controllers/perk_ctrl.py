...
_UPDATE_FUN_PREFIX = b'_updatePerk'
_DATA_KEY_PERK_ID = b'perkID'
...
class PerksController(ViewComponentsController):
    ...
    def notifyRibbonChanges(self, ribbons):
        ...
        for perkID, perkData in sorted(viewitems(changes), key=(lambda item: item[1][b'endTime'])):
            if perkData[b'endTime'] > BigWorld.serverTime() and perkData.get(b'state', PerkState.ACTIVE):
                prevPerkEndTime = self._prevRibbonsState.get(perkID, {}).get(b'endTime', 0)
                if perkData[b'endTime'] - prevPerkEndTime > _INTERVAL_FOR_SAME_NOTIFICATION:
    ...
    def __checkPerksBySettings(self, perks):
        data = deepcopy(perks)
        for perk in data:
            if not self._canShowPerk(perk[b'perkID']):
                perk[b'state'] = PerkState.INACTIVE

        return data

    def __checkRibbonsBySettings(self, ribbons):
        return {perk for perk in ribbons if self._canShowPerk(perk[b'perkID'])}
