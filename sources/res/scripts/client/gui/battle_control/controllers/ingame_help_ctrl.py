...
class IngameHelpController(IBattleController):
    __slots__ = (b'__arenaVisitor', b'__currentHintContext')
    ...
    def showIngameHelp(self, vehicle):
        ctx = {b'vehName': (vehicle.typeDescriptor.type.userString if vehicle is not None else None)}
        ...
        if hasDetailedHelpScreen:
            ctx[b'currentHintCtx'] = self.__currentHintContext
    ...
    def __onShowBtnHint(self, event):
        self.__currentHintContext = event.ctx.get(b'hintCtx')
        return
