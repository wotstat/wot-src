...
MESSAGE_STATE_RESOURCE_MAP = {(CommendationsState.UNSENT): b'', 
   (CommendationsState.SENT): b'comms_sent', 
   (CommendationsState.RECEIVED): b'comms_received', 
   (CommendationsState.MUTUAL): b'comms_mutual'}
MESSAGE_STATE_MARKER_MAP = {(CommendationsState.UNSENT): b'', 
   (CommendationsState.SENT): b'sendCommendation', 
   (CommendationsState.RECEIVED): b'gotCommendation', 
   (CommendationsState.MUTUAL): b'mutualCommendation'}
...
class CommendationsMessagesController(IBattleController):
    ...
    def _onCommandReceived(self, cmd):
        ...
        if not (resourceName and vehicleID):
            _logger.warning(b'Missing chat args for command: resourceName=%s, vehicleID=%s', resourceName, vehicleID)
            return
    ...
    def _addToChat(self, targetID, resourceName):
        _logger.debug(b'[COMMS] messageReceived: targetID=%s, resourceName=%s', targetID, resourceName)
