...
class W2GTBattleController(IW2GTBattleController):
    ...
    def __tryInitializeData(self):
        if self.__isActive:
            ...
            if BigWorld.player().isObserver():
                _logger.debug(b"Can't initialize W2GT request. Player is Observer")
                return
            if self.__period > ARENA_PERIOD.BATTLE:
                _logger.debug(b"Can't make request for W2GT. Time to make it is over")
                return
            if not self.__settingsCore.serverSettings.settingsCache.isSynced():
                _logger.debug(b'Server settings is not ready yet')
                return
            if not self.__w2gtGameCtrl.isEnabled:
                _logger.debug(b'W2GT is disabled in server settings')
                return
            if self.__ownVehicle is None:
                ...
                if ownVehicle is None or not ownVehicle.isPlayerVehicle:
                    _logger.debug(b"Can't collect data for W2GT request. Player vehicle hasn't not ready yet")
                    return
            ...
            if self.__arenaUniqueID is None:
                if not self.isArenaSuitable(arena):
                    _logger.debug(b'Arena is not supported for W2GT')
                    return
            ...
            if self.__requestState == _RequestState.NONE:
                ...
                _logger.debug(b'Request for W2GT data is in progress or already has done')
            if self.__period > ARENA_PERIOD.BATTLE:
                _logger.debug(b'Time getting data response is over for W2GT')
                return
            self.__tryRun()
            return
    ...
    def __tryRun(self):
        ...
        if not self.__isActive and isActive and self.__isPlayerAlive:
            ...
            if not self.__receivedData.success:
                _logger.debug(b'Invalid data response for W2GT')
            ...
            if not dataMgr.zones:
                _logger.warning(b"Invalid data for W2GT. Zones weren't found")
                return
    ...
    def __getVehicleRole(self, vehTypeDescriptor):
        ...
        if not role or role == ROLE_TYPE_TO_LABEL[ROLE_TYPE.NOT_DEFINED]:
            return _DEFAULT_VEHICLE_ROLE.get(vehTypeDescriptor.type.classTag, b'')
        return role
