...
class _VehicleTrackers(_TrackersCollection, IVehicleTrackers):
    ...
    def __repr__(self):
        return (b'VehicleTrackers:\n{}').format((b'\n').join((b'{}: {}').format(k, v) for k, v in sorted(viewitems(self._trackers), key=(lambda item: item[0]))))
...
class _VehicleMechanicTrackers(_TrackersCollection, IVehicleMechanicTrackers):
    ...
    def __repr__(self):
        return (b'VehicleMechanicTrackers:\n{}').format((b'\n').join((b'{}: {}').format(k[0], v) for k, v in sorted(viewitems(self._trackers), key=(lambda item: item[0][0]))))
