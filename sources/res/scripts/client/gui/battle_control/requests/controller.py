...
class _AvatarCooldownManager(RequestCooldownManager):
    ...
    def lookupName(self, rqTypeID):
        ...
        return i18n.makeString(b'#system_messages:battle/request/%s' % str(rqName))

    def getDefaultCoolDown(self):
        return DEFAULT_COOLDOWN

    def _showSysMessage(self, msg):
        MessengerEntry.g_instance.gui.addClientMessage(g_settings.htmlTemplates.format(b'battleErrorMessage', ctx={b'error': msg}))
        return


class _AvatarRequester(RequestsByIDProcessor):

    class _Response(namedtuple(b'_Response', [
     b'code', b'errStr', b'data'])):
    ...
    def _makeResponse(self, code=0, errMsg=b'', data=None, ctx=None):
        response = self._Response(code, errMsg, data)
        if not response.isSuccess():
            LOG_WARNING(b'Avatar request error', ctx, response)
        return response
...
class AvatarRequestsController(RequestsController):
    ...
    def sendInvites(self, ctx, callback=None):
        return self._requester.doRequestEx(ctx, callback, b'sendInvitation', ctx.getPlayerIDs())
