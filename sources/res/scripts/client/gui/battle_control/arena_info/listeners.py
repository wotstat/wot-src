...
class _PeriodAdditionalInfo(namedtuple(b'_PeriodAdditionalInfo', [b'winStatus', b'winnerTeam', b'finishReason'])):
...
class _Listener(object):
    __slots__ = (b'_controllers', b'_visitor', b'_arenaDP')
...
class ArenaVehiclesListener(_Listener):
        ...
        if result:
            if self.__isRequiredDataExists():
                if self.__callbackID is not None:
                    self.__clearCallback()
                    self._invokeListenersMethod(b'invalidateArenaInfo')
                else:
    ...
    def __arena_onNewVehicleListReceived(self):
        self._arenaDP.buildVehiclesData(self._visitor.getArenaVehicles())
        self._invokeListenersMethod(b'invalidateVehiclesInfo', self._arenaDP)
        return
    ...
    def __arena_onVehicleAdded(self, vehicleID):
        ...
        if added is not None:
            self._invokeListenersMethod(b'addVehicleInfo', added, self._arenaDP)
        if updated:
            self._invokeListenersMethod(b'updateVehiclesInfo', updated, self._arenaDP)
        return
    ...
    def __arena_onVehicleKilled(self, victimID, *args):
        ...
        if flags != INVALIDATE_OP.NONE:
            self._invokeListenersMethod(b'invalidateVehicleStatus', flags, vo, self._arenaDP)
        return
    ...
    def __arena_onNewStatisticsReceived(self):
        self._arenaDP.buildStatsData(self._visitor.getArenaStatistics())
        self._invokeListenersMethod(b'invalidateVehiclesStats', self._arenaDP)
        return
    ...
    def __arena_onVehicleStatisticsUpdate(self, vehicleID):
        ...
        if flags != INVALIDATE_OP.NONE:
            self._invokeListenersMethod(b'updateVehiclesStats', [(flags, vo)], self._arenaDP)
        return
    ...
    def __arena_onTeamKiller(self, vehicleID, value):
        ...
        if flags != INVALIDATE_OP.NONE:
            self._invokeListenersMethod(b'invalidatePlayerStatus', flags, vo, self._arenaDP)
        return
    ...
    def __arena_onInteractiveStats(self, stats):
        ...
        if stats:
            self._invokeListenersMethod(b'updateVehiclesStats', stats, self._arenaDP)
        return
    ...
    def __arena_onGameModeSpecificStats(self, isStatic, stats):
        for vehicleID, vehicleStats in iteritems(stats):
            ...
            if isStatic:
                self._invokeListenersMethod(b'updateVehiclesInfo', [(flags, vo)], self._arenaDP)
    ...
    def __arena_onChatCommandTargetUpdate(self, isStatic, chatCommandStates):
        ...
        if updateFlag != INVALIDATE_OP.NONE:
            self._invokeListenersMethod(b'updateVehiclesStats', updateList, self._arenaDP)
        return

    def __arena_onChatCommandTriggered(self, chatCommands):
        self._invokeListenersMethod(b'updateTriggeredChatCommands', chatCommands, self._arenaDP)
        return

    def __arena_onUpdatePriorityChatCommand(self, vehicleID, chatCommand, duration):
        self._invokeListenersMethod(b'updatePriorityChatCommand', vehicleID, chatCommand, duration)
        return
    ...
    def __arena_onFogOfWarEnabled(self, flag):
        self._invokeListenersMethod(b'invalidateFogOfWarEnabledFlag', flag)
        return
...
class ContactsListener(_Listener):
    ...
    def __me_onUsersListReceived(self, tags):
        if _TAGS_TO_UPDATE & tags:
            self._invokeListenersMethod(b'invalidateUsersTags')
        return
    ...
    def __me_onBattleUserActionReceived(self, actionID, user):
        if actionID in (
         ...
         USER_ACTION_ID.MUTE_UNSET):
            self._invokeListenersMethod(b'invalidateUserTags', user)
        return


class PersonalInvitationsListener(_Listener):
    __slots__ = (b'__filter',)
    ...
    def __updateFilteredStatuses(self, filtered):
        ...
        for avatarSessionID, include, exclude in filtered:
            ...
            if vo is not None and flags != INVALIDATE_OP.NONE:
                LOG_DEBUG(b'Invitation status has been changed', avatarSessionID, vo.invitationDeliveryStatus)
                vos.append(vo)

        if vos:
            self._invokeListenersMethod(b'invalidateInvitationsStatuses', vos, self._arenaDP)
        return
...
class ArenaSpaceLoadListener(_Listener):
    ...
    def __onSpaceLoadUpdated(self, progress):
        self._invokeListenersMethod(b'updateSpaceLoadProgress', progress)
        return

    def __onSpaceLoadStarted(self):
        self._invokeListenersMethod(b'spaceLoadStarted')
        return

    def __onSpaceLoadCompleted(self):
        self.__state |= _LOAD_STATE_FLAG.SPACE_LOAD_COMPLETED
        self._invokeListenersMethod(b'spaceLoadCompleted')
        return

    def __onArenaLoadCompleted(self):
        self.__state |= _LOAD_STATE_FLAG.INFLUX_LOAD_COMPLETED
        self._invokeListenersMethod(b'arenaLoadCompleted')
        return


class ArenaTeamBasesListener(_Listener):
    __slots__ = (b'__baseIDs', b'__points')
    ...
    def __arena_onTeamBasePointsUpdate(self, team, baseID, points, timeLeft, invadersCnt, capturingStopped):
        self._invokeListenersMethod(b'invalidateTeamBasePoints', team, baseID, points, timeLeft, invadersCnt, capturingStopped)
        return

    def __arena_onTeamBaseCaptured(self, team, baseID):
        self._invokeListenersMethod(b'invalidateTeamBaseCaptured', team, baseID)
        return

    def __arena_onPeriodChange(self, period, *args):
        if period == ARENA_PERIOD.AFTERBATTLE:
            self._invokeListenersMethod(b'removeTeamsBases')
        return
    ...
    def __setPeriodInfo(self, controller=None):
        if self._visitor is not None:
            ...
            if controller is not None:
                ...
                self._invokeListenersMethod(b'setPeriodInfo', period, self._visitor.getArenaPeriodEndTime(), self._visitor.getArenaPeriodLength(), periodAddInfo)
        return

    def __arena_onPeriodChange(self, period, endTime, length, additionalInfo):
        self._invokeListenersMethod(b'invalidatePeriodInfo', period, endTime, length, _getPeriodAdditionalInfo(self._arenaDP, period, additionalInfo))
        return
...
class PositionsListener(_Listener):
    ...
    def __arena_onPositionsUpdated(self):
        ...
        self._invokeListenersMethod(b'updatePositions', _iterator)
        return
...
class ViewPointsListener(_Listener):
    ...
    def __arena_onViewPoints(self, viewPoints):
        self._invokeListenersMethod(b'updateViewPoints', viewPoints)
        return


class ListenersCollection(_Listener):
    __slots__ = (b'__vehicles', b'__teamsBases', b'__loader', b'__contacts', b'__period', b'__invitations', b'__positions', b'__viewPoints', b'__battleCtx')
    ...
    def addController(self, controller):
        ...
        try:
            ...
            LOG_ERROR(b'Controller is not valid', controller)
            return False
