...
class DefaultArenaGuiDescription(IArenaGuiDescription):
    __slots__ = (b'_visitor', b'_team', b'_questInfo', b'_isPersonalDataSet', b'_selectedQuestIDs', b'_selectedQuestInfo')
    ...
    def getTeamName(self, team):
        ...
        if b'opponents' in data:
            opponents = data[b'opponents']
            teamName = opponents.get(b'%s' % team, {}).get(b'name', teamName)
        return teamName
    ...
    def getSmallIcon(self):
        return self._visitor.getArenaIcon(b'battleLoading')

    def _getScreenImg(self, subdir):
        img = self._visitor.getArenaIcon(subdir)
        return img.replace(b'img://', b'') or settings.DEFAULT_SCREEN_MAP_IMAGE_RES_PATH

    def getScreenIcon(self):
        return self._getScreenImg(b'screen')

    def getRespawnIcon(self):
        return self._getScreenImg(b'respawn')
...
class ArenaWithBasesDescription(DefaultArenaGuiDescription):
    ...
    def getDescriptionString(self, isInBattle=True):
        return i18n.makeString((b'#arenas:type/{}/name').format(functions.getArenaSubTypeName(self._visitor.type.getID())))
...
class ArenaWithL10nDescription(IArenaGuiDescription):
    __slots__ = (b'_l10nDescription', b'_decorated')
...
class EpicBattlesDescription(ArenaDescriptionWithInvitation):
    ...
    def getReservesModifier(self):
        data = self._visitor.getArenaExtraData() or {}
        return data.get(b'reservesModifier')
