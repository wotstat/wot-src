...
class ArenaDataProvider(IArenaDataProvider):
    __slots__ = (b'__playerTeam', b'__playerVehicleID', b'__vInfoVOs', b'__vStatsVOs', b'__avatarsVIDs', b'__accountVIDs', b'__weakref__', b'__teamsOnArena', b'__squadFinder', b'__description', b'__invitationStatuses')
    ...
    def __del__(self):
        _logger.debug(b'Deleted: %r', self)
        return
    ...
    def addVehicleInfo(self, vehicleID, vInfo):
        ...
        if vehicleID not in self.__vInfoVOs:
            ...
            _logger.warning(b'Vehicle already exists: %r', self.__vInfoVOs[vehicleID])
    ...
    def updateVehicleDogTag(self, vID, vInfo):
        if vID not in self.__vInfoVOs:
            _logger.info(b'ArenaDataProvider.updateVehicleDogTag: no info about vehicle %s', vID)
            return (None, None)
        else:
            vInfoVO = self.__vInfoVOs[vID]
    ...
    def isSquadMan(self, vID, prebattleID=None):
        ...
        return self.__getStateFlag(vID, b'isSquadMan', playerTeam=self.__playerTeam, prebattleID=prebattleID)
    ...
    def isTeamKiller(self, vID):
        return self.__getStateFlag(vID, b'isTeamKiller', playerTeam=self.__playerTeam)

    def isObserver(self, vID):
        if vID not in self.__vInfoVOs:
            _logger.warning(b'vID not in __vInfoVOs, vID = %s', vID)
            return None
        else:
            return self.__getStateFlag(vID, b'isObserver')

    def isPlayerObserver(self):
        if self.__playerVehicleID not in self.__vInfoVOs:
            _logger.warning(b'vID not in __vInfoVOs, vID = %s', self.__playerVehicleID)
            return None
        else:
            return self.__getStateFlag(self.__playerVehicleID, b'isObserver')
    ...
    def __tryToGetRequiredData(self):
        ...
        if not self.__playerTeam:
            successful = False
            _logger.info(b"Player's team not found.")
        self.__playerVehicleID = avatar_getter.getPlayerVehicleID()
        if not self.__playerVehicleID:
            successful = False
            _logger.info(b"Player's vehicle ID not found.")
        if not successful:
            playerName = avatar_getter.getPlayerName()
            _logger.info(b'Uses slow player search by name')
    ...
    def __setPersonalDataOnce(self):
        if not self.__description.isPersonalDataSet():
            if self.__playerVehicleID is None:
                _logger.error(b"Player's vehicle ID not be None")
                return
            if self.__playerVehicleID in self.__vInfoVOs:
