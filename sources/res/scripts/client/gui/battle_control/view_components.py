...
class _ComponentsBridge(object):
    ...
    def registerViewComponents(self, *data):
        for item in data:
            if len(item) < 2:
                raise ComponentsBridgeError((b'Item is invalid: {}').format(item))
            ...
            if ctrlID in self.__components:
                ...
                if sameViewAliases:
                    raise ComponentsBridgeError(b'Linkage of controller ID to view alias have to be defined only once! ' + (b'Controller ID: {}, same view aliases: {}').format(ctrlID, sameViewAliases))
    ...
    def addViewComponent(self, componentID, component, rule=VIEW_COMPONENT_RULE.PROXY):
        if componentID not in self.__componentToCrl:
            ...
            for ctrlID in ctrlsIDs:
                ...
                if index is None:
                    _logger.error(b'View component data is broken: %r, %r, %r', ctrlID, componentID, self.__indexes)
                    continue
                ...
                if ctrlID in self.__ctrls:
                    ...
                    _logger.warning(b'Controller is not found: %r', ctrlID)
    ...
    def registerController(self, ctrl):
        if not isinstance(ctrl, IViewComponentsController):
            raise ComponentsBridgeError((b'Controller {0} is not extended IViewComponentsController').format(ctrl))
