...
def hasTurretRotator(vDesc):
    if vDesc is None:
        ...
        hasFakeTurret = vDesc.gun.turretYawLimits is not None and vDesc.hull.fakeTurrets.get(b'battle', ())
...
def getChassisIndicatorType(indicatorType, vDesc):
    newIndicatorType = indicatorType + VEHICLE_CHASSIS_INDICATOR_POSTFIX.get(vDesc.chassisType, b'')
    if not VEHICLE_INDICATOR_TYPE.hasValue(newIndicatorType):
        LOG_WARNING(b'Unknown indicatorType:', newIndicatorType)
    else:
