...
class BattleContext(IBattleContext):
    ...
    def start(self, arenaDP):
        ...
        if prefs is not None:
            self.__isShowVehShortName = prefs.readBool(b'showVehShortName', True)
    ...
    def isObserver(self, vID):
        if self.__arenaDP is not None:
            ...
            _logger.debug(b'BattleContext.isObserver: ArenaDP is None, vID = %s', vID)
            return
    ...
    def isPlayerObserver(self):
        if self.__arenaDP is not None:
            ...
            _logger.debug(b'BattleContext.isPlayerObserver: ArenaDP is None')
            return
    ...
    def getBattleTypeIconPathBig(self):
        return self.__arenaDP.getPersonalDescription().getBattleTypeIconPath(b'c_136x136')

    def getBattleTypeIconPathSmall(self):
        return self.__arenaDP.getPersonalDescription().getBattleTypeIconPath(b'c_64x64')
