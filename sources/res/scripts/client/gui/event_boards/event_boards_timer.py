...
FORMAT_EMPTY_STR = b''
FORMAT_DAY_STR = b'day'
FORMAT_HOUR_STR = b'hour'
FORMAT_MINUTE_STR = b'minute'
FORMAT_TOMORROW_STR = b'tomorrow'
FORMAT_TODAY_STR = b'today'
FORMAT_YESTERDAY_STR = b'yesterday'
...
def getFormattedRemainingTime(strEndDate, isRoundUp=False):
    if strEndDate:
        ...
        if dtime >= 0 and dtime != float(b'inf'):
...
def _stringToStruct(strDate):
    return time.strptime(strDate, b'%Y-%m-%dT%H:%M:%S')


def _calculatePeripheryTimeHelper(baseTimeUTC, primeTimes):
    peripheryStartTimeUTC = time.strptime(primeTimes.getStartTime(), b'%H:%M:%S')
    peripheryEndTimeUTC = time.strptime(primeTimes.getEndTime(), b'%H:%M:%S')
