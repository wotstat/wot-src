...
class MiniclientDescriptionWindow(object):
    ...
    def __openDescriptionInBrowser(self, event):
        if not self.bootcampCtrl.isInBootcamp():
            self.browserCtrl.load(url=(b'{0}/$LANGUAGE_CODE/greeting/mini_wot/').format(GUI_SETTINGS.baseUrls[b'webBridgeRootURL']), title=_ms(b'#miniclient:hangar/miniclient_description_window/title'), browserSize=(780, 450), showCloseBtn=True, showActionBtn=False, isAsync=True, showWaiting=False)((lambda success: True))
