    ...
    def inner(function):
        ...
        def wrapper(*args, **kwargs):
            ...
            if not _lobby.getServerSettings().isOffersEnabled():
                _logger.debug(b'[Offers] feature disabled.')
                return result
    ...
    def inner(function):
        ...
        def wrapper(self, *args, **kwargs):
            if not self.isSynced:
                _logger.debug(b'[Offers] items not synced.')
                return result
...
class OffersDataProvider(IOffersDataProvider):
    ...
    def start(self):
        self._ready = True
        _logger.debug(b'[Offers provider] ready to notify')
        self._notify()
        return

    def stop(self):
        self._ready = False
        _logger.debug(b'[Offers provider] not ready to notify')
        return

    def update(self, diff):
        changed = any(token.startswith(OFFER_TOKEN_PREFIX) for token in diff.get(b'tokens', {})) or b'offersData' in diff or b'eventsData' in diff and EVENT_CLIENT_DATA.OFFER in diff[b'eventsData'] or b'serverSettings' in diff and OFFERS_ENABLED_KEY in diff[b'serverSettings']
        if changed:
            self._pendingNotify = True
            self._cache.clear()
            _logger.debug(b'[Offers provider] changed, pending notification')
            self._notify()
        return
    ...
    def isCdnResourcesReady(self, callback=None, timeout=_CDN_SYNC_TIMEOUT):
        _logger.debug(b'[Offers provider] CDN resources cache ready check')
    ...
    def _onDisconnected(self):
        ...
        _logger.debug(b'[Offers provider] closed')
        return
    ...
    def _notify(self):
        if self._readyToNotify:
            ...
            if self._readyToNotify and self.isSynced:
                ...
                self._pendingNotify = False
                _logger.debug(b'[Offers provider] send notification')
                self.onOffersUpdated()
                raise AsyncReturn(None)
        _logger.debug(b'[Offers provider] can not send notification')
        return
    ...
    def _onServerSettingsChange(self, *args, **kwargs):
        if not self._lobbyContext.getServerSettings().isOffersEnabled():
            if not AccountSettings.getNotifications(OFFERS_DISABLED_MSG_SEEN) and self._getOffersData():
                ...
                msg = makeI18nError(b'offers/switch_off/body')
