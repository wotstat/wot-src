    ...
    def __invoke(self):
        ...
        if self.__fakePosition is None or not constants.IS_DEVELOPMENT:
            if self.__webController.isAvailable():
                ...
                if result.isSuccess():
                    results = RankedYearPositionCtx.getDataObj(result.data).get(b'results')
                    if results is not None and isinstance(results, dict):
                        self.__yearPosition = results.get(b'position')
                else:
                    _logger.debug(b'RankedYearPositionProvider: response is not success with code %s', result.getCode())
        else:
