...
class RankedBattlesStatsComposer(object):
    ...
    def amountBattles(self):
        if self.divisionsStats is not None:
            return sum(division.get(b'battles', 0) for division in self.divisionsStats.itervalues())
        else:
    ...
    def amountBattlesInLeagues(self):
        if self.divisionsStats is not None and self.__settings is not None:
            return sum(self.divisionsStats.get(divisionID, {}).get(b'battles', 0) for divisionID, division in self.__settings.divisions.iteritems() if division[b'isLeague'])
        else:
            return
    ...
    def amountSteps(self):
        if self.divisionsStats is None:
            ...
            for division in self.divisionsStats.itervalues():
                steps += division.get(b'rankChanges', 0)
    ...
    def currentSeasonEfficiency(self):
        ...
        if self.divisionsStats is not None:
            ...
            battles = 0
            for division in self.divisionsStats.itervalues():
                rankChanges += division.get(b'rankChanges', 0)
                battles += division.get(b'battles', 0)
