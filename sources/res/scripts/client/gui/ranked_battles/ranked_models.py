...
ShieldStatus = namedtuple(b'ShieldStatus', b'prevHP, hp, maxHP, shieldState, newShieldState')
RankData = namedtuple(b'RankData', b'rank, steps')
BattleRankInfo = namedtuple(b'BattleRankInfo', b'level, division, isGroup')
...
class RankStatus(CONST_CONTAINER):
    NOT_ACHIEVED = b'not_achieved'
    ACHIEVED = b'achieved'
    LOST = b'lost'


class RankChangeStates(CONST_CONTAINER):
    LEAGUE_EARNED = b'leagueEarned'
    DIVISION_EARNED = b'divisionEarned'
    QUAL_EARNED = b'qualificationEarned'
    QUAL_UNBURN_EARNED = b'qualificationUnburnEarned'
    RANK_EARNED = b'rankEarned'
    RANK_UNBURN_PROTECTED = b'rankUnburnProtected'
    RANK_SHIELD_PROTECTED = b'rankShieldProtected'
    RANK_LOST = b'rankLost'
    STEP_EARNED = b'stepEarned'
    BONUS_STEP_EARNED = b'bonusStepEarned'
    STEPS_EARNED = b'stepsEarned'
    BONUS_STEPS_EARNED = b'bonusStepsEarned'
    STEP_LOST = b'stepLost'
    NOTHING_CHANGED = b'nothingChanged'


class RankedCycle(namedtuple(b'RankedCycle', b'ID, status, startDate, endDate, ordinalNumber announceOnly')):
...
class RankedSeason(GameSeason):
    ...
    def _buildCycle(self, idx, status, number, rawData):
        return RankedCycle(idx, status, rawData[b'start'], rawData[b'end'], number, bool(rawData.get(b'announce', False)))
...
class RankProgress(object):
    ...
    def getAcquiredSteps(self):
        return filter(operator.methodcaller(b'isAcquired'), self._steps)

    def getBonusSteps(self):
        return filter(operator.methodcaller(b'isBonus'), self._steps)


class Division(object):
    __slots__ = (b'firstRank', b'lastRank', b'__divisionID', b'__currentRank', b'__isFinal', b'__rankIDs', b'__isQualification')
...
class Rank(object):
    _ICON_SIZES = {b'tiny': b'24x24', 
       b'small': b'58x80', b'medium': b'80x110', b'big': b'114x160', b'huge': b'190x260', b'final': b'216x300'}
    ...
    def getIcon(self, size):
        size = self._ICON_SIZES.get(size, b'58x80')
        ...
        return backport.image(R.images.gui.maps.icons.rankedBattles.ranks.num(size).dyn(b'rank%s_%s' % (self.__division.getID(), self.getUserName()))())
...
class PostBattleRankInfo(namedtuple(b'PostBattleRankInfo', (b'accRank', b'accStep', b'stepChanges', b'updatedStepChanges', b'prevAccRank', b'prevAccStep', b'prevMaxRank', b'prevMaxStep', b'shields', b'prevShields', b'isBonusBattle', b'stepsBonusBattles', b'efficiencyBonusBattles'))):
    __slots__ = ()

    @classmethod
    def fromDict(cls, dictWithInfo):
        accRank, accStep = dictWithInfo.get(b'accRank', (0, 0))
        stepChanges = dictWithInfo.get(b'rankChange', 0)
        updatedStepChanges = dictWithInfo.get(b'updatedRankChange', 0)
        prevAccRank, prevAccStep = dictWithInfo.get(b'prevAccRank', (0, 0))
        prevMaxRank, prevMaxStep = dictWithInfo.get(b'prevMaxRank', (0, 0))
        shields = dictWithInfo.get(b'shields', {})
        prevShields = dictWithInfo.get(b'prevShields', {})
        isBonusBattle = dictWithInfo.get(b'bonusBattleUsed', False)
        efficiencyBonusBattles = dictWithInfo.get(b'efficiencyBonusBattles', 0)
        stepsBonusBattles = dictWithInfo.get(b'stepsBonusBattles', 0)
...
class RankedDossier(namedtuple(b'RankedDossier', b'rank, step, vehRankCount, ladderPts, allStepsCount')):
