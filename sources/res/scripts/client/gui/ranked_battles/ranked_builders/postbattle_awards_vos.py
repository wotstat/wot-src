...
AwardBlock = namedtuple(b'AwardBlock', b'rankID, awards, qID')
AwardBlock.__new__.__defaults__ = (0, [], b'')
...
def _getQualificationVO(division, newDivision):
    return {b'division': (division.getID()), 
       b'newDivision': (newDivision.getID())}


def _getDivisionVO(division, newDivision):
    return {b'division': (division.getID()), 
       b'divisionName': (text_styles.middleTitle(backport.text(R.strings.ranked_battles.division.dyn(division.getUserID())()))), 
       b'newDivision': (newDivision.getID()), 
       b'newDivisionName': (text_styles.middleTitle(backport.text(R.strings.ranked_battles.division.dyn(newDivision.getUserID())())))}


def _getBonusBattleFields(stepsBonus, efficiencyBonus, isDaily):
    return {b'bonusBattleText': (_getBonusBattlesDiffLabel(stepsBonus + efficiencyBonus) if stepsBonus else b''), 
       b'bonusBattleTooltip': (_getBonusBattlesTooltip(stepsBonus, efficiencyBonus, isDaily) if stepsBonus else b'')}
...
def _getBlockVO(state, subTitle=b'', rankVO=None, qualificationVO=None, divisionVO=None, awards=None, bonusBattleText=b'', bonusBattleTooltip=b''):
    return {b'state': state, 
       b'subTitle': subTitle, 
       b'rankVO': rankVO, 
       b'qualVO': qualificationVO, 
       b'divisionVO': divisionVO, 
       b'awards': awards, 
       b'bonusBattleText': bonusBattleText, 
       b'bonusBattleTooltip': bonusBattleTooltip}
