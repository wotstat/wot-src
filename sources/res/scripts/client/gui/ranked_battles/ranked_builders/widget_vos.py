...
QualAddVOs = namedtuple(b'QualAddVOs', b'currentBattles, totalBattles')
LeagueAddVOs = namedtuple(b'LeagueAddVOs', b'currentEfficiencyVO, currentRatingVO, lastEfficiencyVO, lastRatingVO')
StateBlock = namedtuple(b'StateBlock', b'state, lastID, currentID, additionalVOs')
WidgetPreferences = namedtuple(b'WidgetPreferences', b'isAnimationEnabled, isHuge, hasAdditionalRankInfo')
...
def getVOsSequence(preferences, stateBlocks, ranks):
    ...
    for index, block in enumerate(stateBlocks):
        ...
        if builder is not None:
            ...
            _logger.error(b'Can not find builder for state = %s', block.state)
...
def _buildLeagueUpdateVO(preferences, stateBlock, sequenceState, ranks):
    ...
    if stateBlock.state != RANKEDBATTLES_ALIASES.LEAGUE_IDLE_STATE:
        ...
        title = b''
        infoText = b''
        newLeagueName = backport.text(R.strings.ranked_battles.rankedBattlesWidget.dyn((b'league{}').format(stateBlock.currentID))())
...
def _getStepsVO(rank, preferences, statesCollector=StepsStatesCollector()):
    ...
    return {b'steps': (statesCollector.getStates(rank)), b'infoText': infoText}
...
def _getQualificationVO(rank):
    return {b'imageSrc': (backport.image(R.images.gui.maps.icons.rankedBattles.divisions.c_80x110.c_0())), 
       b'smallImageSrc': (backport.image(R.images.gui.maps.icons.rankedBattles.divisions.c_58x80.c_0())), 
       b'isEnabled': True, 
...
def _getDivisionVO(divisionID, newDivisionID):
    return {b'division': divisionID, 
       b'newDivision': newDivisionID, 
       b'title': (backport.text(R.strings.ranked_battles.rankedBattlesWidget.newDivisionCongrat.title())), 
       b'infoText': (backport.text(R.strings.ranked_battles.rankedBattlesWidget.newDivisionCongrat.info()))}


def _getLeagueVO(leagueID, efficiencyVO, positionVO, title=b'', infoText=b''):
    return {b'league': leagueID, 
       b'efficiency': efficiencyVO, 
       b'position': positionVO, 
       b'title': title, 
       b'infoText': infoText}


def _getBlockVO(preferences, state, infoText=b'', nextInfoText=b'', rankLeftVO=None, rankRightVO=None, newRankVO=None, steps=None, newSteps=None, leagueVO=None, newLeagueVO=None, divisionVO=None, finalState=None):
    return {b'state': state, 
       b'rankLeftVO': rankLeftVO, 
       b'rankRightVO': rankRightVO, 
       b'newRankVO': newRankVO, 
       b'stepsContainerVO': (steps or {b'steps': (), 
                             b'infoText': b''}), 
       b'newStepsContainerVO': (newSteps or {b'steps': (), 
                                b'infoText': b''}), 
       b'infoText': infoText, 
       b'nextInfoText': nextInfoText, 
       b'divisionVO': divisionVO, 
       b'leagueVO': leagueVO, 
       b'newLeagueVO': newLeagueVO, 
       b'isHuge': (preferences.isHuge), 
       b'animationEnabled': (preferences.isAnimationEnabled), 
...
def _updateShield(rankVO):
    shield = rankVO.get(b'shield', None)
    if shield is not None:
        if shield[b'newState'] is not None:
            shield[b'state'] = shield[b'newState']
            shield[b'newState'] = None
            rankVO[b'shield'] = shield
    return rankVO
...
def _updateSteps(stepsVO):
    ...
    if stepsVO is not None:
        for step in stepsVO.get(b'steps'):
        ...
        infoText = stepsVO.get(b'infoText', b'')
    return {b'steps': resultVO, b'infoText': infoText}
