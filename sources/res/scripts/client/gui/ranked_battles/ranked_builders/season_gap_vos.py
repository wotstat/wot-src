...
StateBlock = namedtuple(b'StateBlock', b'state, rankID, division, leagueID, isSprinter, isYearRewardEnabled')
...
def getDataVO(stateBlock):
    ...
    if builder is not None:
        ...
        _logger.error(b'Can not find builder for state = %s', stateBlock.state)
...
def getEfficiencyVO(currentSeasonEfficiency):
    return {b'icon': b'efficiency', 
       b'label': (text_styles.alignText(text_styles.mainBig(backport.text(R.strings.ranked_battles.rankedBattleMainView.seasonGap.stats.efficiency())), b'center')), 
...
def getRatingVO(rating, isMastered):
    ...
    value = resultVO[b'value']
    if not isMastered:
        label = backport.text(R.strings.ranked_battles.rankedBattleMainView.seasonGap.stats.outOfRating())
        value = b' '
    resultVO[b'label'] = text_styles.alignText(text_styles.mainBig(label), b'center')
    resultVO[b'value'] = value
    return resultVO


def _getDataVO(state, leagueID=UNDEFINED_LEAGUE_ID, divisionID=ZERO_DIVISION_ID, rankID=ZERO_RANK_ID, disabled=False, title=b'', descr=b'', btnLabel=b'', btnVisible=False, sprinterImg=b''):
    if rankID in (ZERO_RANK_ID, ZERO_RANK_ID + 1) and divisionID == ZERO_DIVISION_ID:
        ...
        smallImage = backport.image(R.images.gui.maps.icons.rankedBattles.ranks.c_114x160.dyn((b'rank{}_{}').format(divisionID, rankID))())
        bigImage = backport.image(R.images.gui.maps.icons.rankedBattles.ranks.c_190x260.dyn((b'rank{}_{}').format(divisionID, rankID))())
    return {b'state': state, 
       b'leagueID': leagueID, 
       b'divisionImgSmall': smallImage, 
       b'divisionImgBig': bigImage, 
       b'disabled': disabled, 
       b'title': title, 
       b'descr': descr, 
       b'btnLabel': btnLabel, 
