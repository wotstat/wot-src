from constants import CURRENT_REALM
from gui.ranked_battles.ranked_helpers import getRankedBattlesInfoPageUrl
from ranked_common import SwitchState
from gui.impl import backport
from gui.impl.gen import R
from gui.Scaleform.genConsts.TOOLTIPS_CONSTANTS import TOOLTIPS_CONSTANTS
from gui.Scaleform.genConsts.RANKEDBATTLES_CONSTS import RANKEDBATTLES_CONSTS
from gui.Scaleform.genConsts.RANKEDBATTLES_ALIASES import RANKEDBATTLES_ALIASES
from gui.shared.utils.functions import makeTooltip
from helpers import time_utils
_EMPTY_WEB_PAGES = {
 RANKEDBATTLES_CONSTS.RANKED_BATTLES_YEAR_RATING_ID,
 RANKEDBATTLES_CONSTS.RANKED_BATTLES_INFO_ID}

def _getYearLeaderboardBG():
    defaultBG = R.images.gui.maps.icons.rankedBattles.bg.lb_default
    leaderboardBG = R.images.gui.maps.icons.rankedBattles.bg.dyn((b'lb_{}').format(CURRENT_REALM), defaultBG)
    return backport.image(leaderboardBG())


def _getSeasonOnMain(isMastered):
    mainMenuLinkage = RANKEDBATTLES_ALIASES.RANKED_BATTLES_DIVISIONS_VIEW_UI
    if isMastered:
        mainMenuLinkage = RANKEDBATTLES_ALIASES.RANKED_BATTLES_LEAGUES_VIEW_UI
    return {b'id': (RANKEDBATTLES_CONSTS.RANKED_BATTLES_RANKS_ID), 
       b'viewId': mainMenuLinkage, 
       b'linkage': mainMenuLinkage, 
       b'background': (backport.image(R.images.gui.maps.icons.rankedBattles.bg.main())), 
       b'tooltip': (makeTooltip(header=backport.text(R.strings.tooltips.rankedBattlesView.ranks.header()), body=backport.text(R.strings.tooltips.rankedBattlesView.ranks.body())))}


def _getSeasonOffMain(isYearLBEnabled):
    mainLinkage = b'BrowserViewStackExPaddingUI'
    mainViewId = RANKEDBATTLES_ALIASES.RANKED_BATTLES_WEB_SEASON_GAP_ALIAS
    if not isYearLBEnabled:
        mainLinkage = RANKEDBATTLES_ALIASES.RANKED_BATTLES_SEASON_GAP_VIEW_UI
        mainViewId = RANKEDBATTLES_ALIASES.RANKED_BATTLES_SEASON_GAP_VIEW_UI
    return {b'id': (RANKEDBATTLES_CONSTS.RANKED_BATTLES_RANKS_ID), 
       b'viewId': mainViewId, 
       b'linkage': mainLinkage, 
       b'background': (backport.image(R.images.gui.maps.icons.rankedBattles.bg.main())), 
       b'tooltip': (makeTooltip(header=backport.text(R.strings.tooltips.rankedBattlesView.ranks.header()), body=backport.text(R.strings.tooltips.rankedBattlesView.ranks.body())))}


def _getRewardsPage(isSeasonOff=False, isYearRewardEnabled=True):
    enabled = True
    viewID = RANKEDBATTLES_ALIASES.RANKED_BATTLES_REWARDS_UI
    if isSeasonOff:
        enabled = isYearRewardEnabled
        viewID = RANKEDBATTLES_ALIASES.RANKED_BATTLES_REWARDS_SEASON_OFF_ALIAS
    return {b'id': (RANKEDBATTLES_CONSTS.RANKED_BATTLES_REWARDS_ID), 
       b'viewId': viewID, 
       b'linkage': (RANKEDBATTLES_ALIASES.RANKED_BATTLES_REWARDS_UI), 
       b'background': (backport.image(R.images.gui.maps.icons.rankedBattles.bg.main())), 
       b'tooltip': (makeTooltip(header=backport.text(R.strings.tooltips.rankedBattlesView.rewards.header()), body=backport.text(R.strings.tooltips.rankedBattlesView.rewards.body() if enabled else R.strings.tooltips.rankedBattlesView.rewards.body.seasonOff()))), 
       b'enabled': enabled}


def _getShopPage(isRankedShop):
    if isRankedShop:
        return ({b'id': (RANKEDBATTLES_CONSTS.RANKED_BATTLES_SHOP_ID), b'viewId': (RANKEDBATTLES_ALIASES.RANKED_BATTLES_SHOP_ALIAS), b'linkage': b'BrowserViewStackExPaddingUI', b'background': (backport.image(R.images.gui.maps.icons.rankedBattles.bg.shop_bg())), b'tooltip': (makeTooltip(header=backport.text(R.strings.tooltips.rankedBattlesView.shop.header()), body=backport.text(R.strings.tooltips.rankedBattlesView.shop.body())))},)
    return ()


def _getRatingPage():
    return {b'id': (RANKEDBATTLES_CONSTS.RANKED_BATTLES_RATING_ID), b'viewId': (RANKEDBATTLES_ALIASES.RANKED_BATTLES_RAITING_ALIAS), 
       b'linkage': b'BrowserViewStackExPaddingUI', 
       b'background': (backport.image(R.images.gui.maps.icons.rankedBattles.bg.main())), 
       b'tooltip': (makeTooltip(header=backport.text(R.strings.tooltips.rankedBattlesView.rating.header()), body=backport.text(R.strings.tooltips.rankedBattlesView.rating.body())))}


def _getYearRatingPage(isYearLBVisible, yearLBSize, disabled):
    body = R.strings.tooltips.rankedBattlesView.yearRating.body
    if isYearLBVisible:
        return ({b'id': (RANKEDBATTLES_CONSTS.RANKED_BATTLES_YEAR_RATING_ID), b'viewId': (RANKEDBATTLES_ALIASES.RANKED_BATTLES_YEAR_RAITING_ALIAS), b'linkage': b'BrowserViewStackExPaddingUI', b'background': (_getYearLeaderboardBG()), b'tooltip': (makeTooltip(header=backport.text(R.strings.tooltips.rankedBattlesView.yearRating.header(), amount=yearLBSize), body=backport.text(body.disabled()) if disabled else backport.text(body()))), b'enabled': (not disabled)},)
    return ()


def _getInfoPage():
    return {b'id': (RANKEDBATTLES_CONSTS.RANKED_BATTLES_INFO_ID), b'viewId': (RANKEDBATTLES_ALIASES.RANKED_BATTLES_INFO_ALIAS), 
       b'linkage': b'BrowserViewStackExPaddingUI', 
       b'background': (backport.image(R.images.gui.maps.icons.rankedBattles.bg.main())), 
       b'tooltip': (makeTooltip(header=backport.text(R.strings.tooltips.rankedBattlesView.info.header()), body=backport.text(R.strings.tooltips.rankedBattlesView.info.body())))}


def getBubbleLabel(counter):
    if not bool(counter):
        return b''
    return backport.text(R.strings.ranked_battles.rankedBattleMainView.emptyBubble())


def getCountersData(rankedController, awardsCounter, infoCounter, yearRatingCounter, shopCounter):
    result = [
     {b'componentId': (RANKEDBATTLES_CONSTS.RANKED_BATTLES_INFO_ID), 
        b'count': infoCounter}]
    if rankedController.isYearRewardEnabled():
        result.append({b'componentId': (RANKEDBATTLES_CONSTS.RANKED_BATTLES_REWARDS_ID), 
           b'count': awardsCounter})
    if rankedController.isYearLBEnabled():
        result.append({b'componentId': (RANKEDBATTLES_CONSTS.RANKED_BATTLES_YEAR_RATING_ID), 
           b'count': yearRatingCounter})
    if rankedController.isRankedShopEnabled():
        result.append({b'componentId': (RANKEDBATTLES_CONSTS.RANKED_BATTLES_SHOP_ID), 
           b'count': shopCounter})
    return result


def getRankedMainSeasonOnItems(isRankedShop, yearLBState, isYearRewardEnabled, yearLBSize, isMastered):
    result = list()
    result.append(_getSeasonOnMain(isMastered))
    result.append(_getRewardsPage(isYearRewardEnabled=isYearRewardEnabled))
    result.extend(getRankedMainSeasonAllItems(isRankedShop, yearLBState, yearLBSize))
    return result


def getRankedMainSeasonOffItems(isRankedShop, yearLBState, isYearRewardEnabled, yearLBSize):
    result = list()
    result.append(_getSeasonOffMain(yearLBState == SwitchState.ENABLED))
    result.append(_getRewardsPage(True, isYearRewardEnabled))
    result.extend(getRankedMainSeasonAllItems(isRankedShop, yearLBState, yearLBSize))
    return result


def getRankedMainSeasonAllItems(isRankedShop, yearLBState, yearLBSize):
    result = list()
    result.extend(_getShopPage(isRankedShop))
    result.append(_getRatingPage())
    result.extend(_getYearRatingPage(yearLBState != SwitchState.HIDDEN, yearLBSize, disabled=yearLBState == SwitchState.DISABLED))
    if getRankedBattlesInfoPageUrl():
        result.append(_getInfoPage())
    return result


def getRankedMainSeasonOnHeader(season, itemID):
    title = backport.text(R.strings.ranked_battles.rankedBattle.title())
    mainDescr = b''
    extraDescr = b''
    tooltip = TOOLTIPS_CONSTANTS.RANKED_CALENDAR_DAY_INFO
    if itemID in _EMPTY_WEB_PAGES:
        title = b''
        tooltip = b''
    elif itemID == RANKEDBATTLES_CONSTS.RANKED_BATTLES_INFO_ID:
        mainDescr = backport.text(R.strings.ranked_battles.rankedBattleMainView.infoPage.header())
        tooltip = b''
    elif season is not None:
        startDate = season.getStartDate()
        endDate = season.getEndDate()
        timeDelta = time_utils.getTimeDeltaFromNowInLocal(time_utils.makeLocalServerTime(endDate))
        if timeDelta > time_utils.ONE_WEEK:
            extraDescr = backport.text(R.strings.ranked_battles.rankedBattleMainView.date.period(), start=backport.getLongDateFormat(startDate), finish=backport.getLongDateFormat(endDate))
        else:
            extraDescr = backport.getTillTimeStringByRClass(timeDelta, R.strings.ranked_battles.rankedBattleMainView.date)
        mainDescr = backport.text(R.strings.ranked_battles.rankedBattleMainView.season(), season=season.getUserName())
    return {b'title': title, 
       b'mainDescr': mainDescr, 
       b'extraDescr': extraDescr, 
       b'tooltip': tooltip}


def getRankedMainSeasonOffHeader(prevSeason, nextSeason, isYearGap, itemID):
    title = backport.text(R.strings.ranked_battles.rankedBattle.title())
    mainDescr = b''
    extraDescr = b''
    if itemID == RANKEDBATTLES_CONSTS.RANKED_BATTLES_RANKS_ID and isYearGap:
        mainDescr = backport.text(R.strings.ranked_battles.rankedBattleMainView.yearGap.header())
    elif itemID in _EMPTY_WEB_PAGES:
        title = b''
    elif itemID == RANKEDBATTLES_CONSTS.RANKED_BATTLES_INFO_ID:
        mainDescr = backport.text(R.strings.ranked_battles.rankedBattleMainView.infoPage.header())
    else:
        mainDescr = backport.text(R.strings.ranked_battles.rankedBattleMainView.seasonCompleteSingle(), season=prevSeason.getUserName())
        if nextSeason is not None:
            mainDescr = backport.text(R.strings.ranked_battles.rankedBattleMainView.seasonComplete(), season=prevSeason.getUserName())
            extraDescr = backport.text(R.strings.ranked_battles.rankedBattleMainView.seasonGap(), newSeason=nextSeason.getUserName(), date=backport.getLongDateFormat(nextSeason.getStartDate()))
    return {b'title': title, b'mainDescr': mainDescr, 
       b'extraDescr': extraDescr, 
       b'tooltip': b''}
