...
class HangarCameraSounds(IHangarCameraSounds):
    ...
    class _MoveCameraEvents(object):
        MOVE_TO_HERO = b'ue_hangar_generic_camera_fly_forward'
        MOVE_TO_MAIN = b'ue_hangar_generic_camera_fly_backward'

    class _CameraStates(object):
        STATE_NAME = b'STATE_hangar_camera_state_rotate_or_onproposal'
        MAIN_VEHICLE_AND_NOT_IN_IDLE = b'STATE_hangar_camera_state_rotate_or_onproposal_false'
        HERO_VEHICLE_OR_IN_IDLE = b'STATE_hangar_camera_state_rotate_or_onproposal_true'

    class _SelectedVehicleStates(object):
        STATE_NAME = b'STATE_hangar_tank_view'
        HERO_VEHICLE = b'STATE_hangar_tank_view_proposal'
        MAIN_VEHICLE = b'STATE_hangar_tank_view_main'
    ...
    def __handleEntityUpdated(self, event):
        ...
        if self.hangarSpace.spaceInited:
            isMainView = ctx[b'entityId'] == self.hangarSpace.space.vehicleEntityId
        else:
            isMainView = True
    ...
    def __handleIdleCameraActivation(self, event):
        if not self.__isMainView:
            return
        WWISE.WW_setState(self._CameraStates.STATE_NAME, self._CameraStates.HERO_VEHICLE_OR_IN_IDLE if event.ctx[b'started'] else self._CameraStates.MAIN_VEHICLE_AND_NOT_IN_IDLE)
        return
