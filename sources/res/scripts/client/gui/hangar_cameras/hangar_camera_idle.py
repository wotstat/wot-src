...
class HangarCameraIdle(HangarCameraSettingsListener, CallbackDelayer, TimeDeltaMeter):
    ...
    def __onCameraForceDisable(self, event):
        if not event.ctx[b'setIdle']:
            return
        isDisabled = event.ctx[b'isDisable']
        if isDisabled:
    ...
    def __startCameraIdle(self):
        g_eventBus.handleEvent(CameraRelatedEvents(CameraRelatedEvents.IDLE_CAMERA, ctx={b'started': True}), scope=EVENT_BUS_SCOPE.DEFAULT)
    ...
    def __updateEasingOut(self):
        ...
        if self.__camera is None:
            ...
            if self.__currentIdleTime < self.TIME_OUT:
                return 0.0
            g_eventBus.handleEvent(CameraRelatedEvents(CameraRelatedEvents.IDLE_CAMERA, ctx={b'started': False}), scope=EVENT_BUS_SCOPE.DEFAULT)
            return
