...
class _StateExtractor(object):
    ...
    @classmethod
    def getNDQState(cls):
        return cls.__lobbyContext.getServerSettings().getDailyQuestConfig().get(b'ndqSwitch', False)
...
class SwitcherListener(_NotificationListener):
    ...
    def __addMessage(self, featureName, newState):
        if self.__currentStates[featureName] == newState:
            ...
            messageData = {b'header': (backport.text(msgTitle))} if msgTitle else None
...
class BattlePassListener(_NotificationListener):
    ...
    def __notifyStartExtra(self, chapterID):
        ...
        SystemMessages.pushMessage(text=backport.text(R.strings.system_messages.battlePass.extraStarted.body(), name=chapterName), priority=NotificationPriorityLevel.HIGH, type=SM_TYPE.BattlePassExtraStart, messageData={b'header': header})
        return
    ...
    def __notifyFinishExtra(self, chapterID):
        ...
        SystemMessages.pushMessage(text=text, type=SM_TYPE.BattlePassExtraFinish, messageData={b'header': header})
        return
    ...
    def __notifyExtraWillEndSoon(self, chapterID):
        ...
        SystemMessages.pushMessage(text=text, type=SM_TYPE.BattlePassExtraWillEndSoon, messageData={b'header': header}, savedData={b'chapterID': chapterID})
        return
    ...
    def __checkAndNotify(self, oldMode=None, newMode=None):
        ...
        if needToPushStarted:
            self.__pushStarted()
        elif self.__isFinished != isFinished and isFinished or isModeChanged and newMode == b'disabled':
            self.__pushFinished()
        if isModeChanged:
            if newMode == b'paused':
                self.__pushPause()
    ...
    def __pushFinished(self):
        if self.__battlePass.isHoliday():
            ...
            SystemMessages.pushMessage(text=backport.text(R.strings.system_messages.battlePassH.switch_disable.body()), priority=NotificationPriorityLevel.HIGH, type=SystemMessages.SM_TYPE.BattlePassDefault, messageData={b'header': header, b'additionalText': b''})
        return
    ...
    def __pushStarted(self):
        ...
        SystemMessages.pushMessage(text=text, priority=NotificationPriorityLevel.HIGH, type=SystemMessages.SM_TYPE.BattlePassDefault, messageData={b'header': header, b'additionalText': b''})
...
class RecertificationFormStateListener(_NotificationListener):
    ...
    def _getState(self):
        if not self.__goodiesCache.getRecertificationForm(currency=b'credits').enabled:
...
class SeniorityAwardsTokenListener(BaseReminderListener):
    ...
    def start(self, model):
        ...
        if result:
            ...
            g_clientUpdateManager.addCallbacks({((b'cache.dynamicCurrencies.{}').format(WDR_CURRENCY)): (self.__onBalanceUpdate)})
            self.__tryNotify()
        return result
    ...
    def _createNotificationData(self, priority, **ctx):
        ...
        getCache().setPayload(gfDataID, {b'count': (str(count)), b'timeLeft': timeLeft})
        return NotificationData(self._getNotificationId(), {b'gfDataID': gfDataID}, priority, None)
...
class SeniorityAwardsManualClaimListener(_NotificationListener):
    ...
    def __tryNotify(self):
        ...
        if self.__seniorityAwardCtrl.isNeedToShowRewardNotification:
            ...
            model.addNotification(SeniorityAwardsDecorator(self.__ENTITY_ID, self.__TYPE, {b'gfDataID': (str(uuid.uuid4()))}, model, self.__TEMPLATE, priority, useCounterOnce=False, isNotify=self.__seniorityAwardCtrl.isNeedToShowNotificationBullet))
            parentScreen = SeniorityAwardsLogSpaces.NOTIFICATION_CENTER
...
class SeniorityAwardsVehicleSelectionListener(BaseReminderListener):
    ...
    def _createNotificationData(self, priority, **ctx):
        ...
        gfDataID = str(uuid.uuid4())
        getCache().setPayload(gfDataID, {b'count': (str(min(vehiclesCanSelect, allVehiclesForSelection)))})
        return NotificationData(entityID=self._getNotificationId(), savedData={b'gfDataID': gfDataID}, priorityLevel=priority, entity=None)
    ...
    def __isFinishNotificationActive(self):
        for notification in self.__eventNotifications.getEventsNotifications():
            if notification.eventType == AUCTION_FINISH_EVENT_TYPE:
                ...
                if self.__getTimeToStart(getTimestampByStrDate(str(data[b'startDate']))) <= self.__TIME_TO_SHOW_SOON:
                    return True
...
class CollectionsListener(_NotificationListener, EventsHandler):
    ...
    def __onCollectionsUpdatedEntryEvent(self, notification):
        if not isCollectionsUpdatedEntrySeen():
            self.__pushCollectionsCustomMessage(backport.text(self.__NOTIFICATIONS.updatedEntry.title()), backport.text(self.__NOTIFICATIONS.updatedEntry.text()), b'CollectionsEntrySysMessage', NOTIFICATION_TYPE.COLLECTIONS_ENTRY, self.__COLLECTION_ENTRY_ENTITY_ID)
        return
    ...
    def __pushCollectionsCustomMessage(self, title, text, messageType, notificationType, entityID, savedData=None):
        ...
        if not model.hasNotification(notificationType, entityID):
            message = {b'title': title, b'text': text}
...
class BattleMattersTaskReminderListener(BaseReminderListener, EventsHandler):
    ...
    def _createNotificationData(self, priority, **ctx):
        ...
        data = {b'questIndex': (currentQuest.getOrder() if currentQuest else len(self.__bmCtrl.getCompletedBattleMattersQuests()))}
...
class PrestigeListener(_NotificationListener):
    ...
    def __onVehicleBecomeElite(self, *vehicleIntCDs):
        ...
        for intCD in vehicleIntCDs:
            ...
            if intCD not in config.milestones:
                continue
            SystemMessages.pushMessage(text=backport.text(msgKey.text(), vehicle=vehicle.userName), type=SM_TYPE.InformationHeader, priority=NotificationPriorityLevel.HIGH, messageData={b'header': (backport.text(msgKey.title()))})
    ...
    def __tryNotify(self):
        ...
        if model is None:
            ...
            linkageData = {b'type': (gradeType.value), b'grade': grade, b'lvl': None}
...
class BaseExchangeRateWithDiscountsListener(BaseReminderListener):
    ...
    def _createNotificationData(self, priority, **ctx):
        data = {b'type': (self._discountTypeName), 
           b'format': (self.__discountShowFormat), 
           b'endTime': (self.__discountEndTime), 
           b'discountPercent': (self._discountPercent)}
        return NotificationData(self._getNotificationId(), data, priority, None)
...
class LootBoxSystemListener(_NotificationListener):
    __slots__ = (b'__isActive', b'__isLootBoxesWasStarted')
    __lootBoxes = dependency.descriptor(ILootBoxSystemController)
    __nameRes = [b'eventName', b'lowerCase']
    __START_ENTITY_ID = 0
    ...
    def __pushStarted(self, eventName):
        res = [b'serviceChannelMessages', b'start']
        model = self._model()
        if model is not None:
            ...
            messageData = {b'header': (backport.text(getTextResource(res + [NotificationPathPart.HEADER], eventName)(), eventName=eventNameText)), 
               b'text': (backport.text(getTextResource(res + [NotificationPathPart.TEXT], eventName)(), date=TimeFormatter.getShortDateFormat(localFinishTime)))}
            model.addNotification(LootBoxSystemDecorator(message=messageData, entityID=self.__START_ENTITY_ID, model=model, savedData={b'eventName': eventName}))
            self.__lootBoxes.setSetting(eventName, LOOT_BOXES_WAS_STARTED, True)
        return

    def __pushFinished(self, eventName, boxesCount):
        res = [
         b'serviceChannelMessages', b'finish']
        eventNameText = backport.text(getTextResource(self.__nameRes, eventName)())
        SystemMessages.pushMessage(text=backport.text(R.strings.lootbox_system.helpers.doubleBreakLine()) + backport.text(getTextResource(res + [NotificationPathPart.TEXT], eventName)()) if boxesCount > 0 else b'', priority=NotificationPriorityLevel.MEDIUM, type=SystemMessages.SM_TYPE.LootBoxSystemFinish, messageData={b'header': (backport.text(getTextResource(res + [NotificationPathPart.HEADER], eventName)(), eventName=eventNameText))})
        self.__lootBoxes.setSetting(eventName, LOOT_BOXES_WAS_FINISHED, True)
        return

    def __pushLootBoxesEnabled(self, eventName):
        res = [b'serviceChannelMessages', b'lootBoxesEnabled']
        eventNameText = backport.text(getTextResource(self.__nameRes, eventName)())
    ...
    def __pushLootBoxesDisabled(self, eventName):
        res = [
         b'serviceChannelMessages', b'lootBoxesDisabled']
        eventNameText = backport.text(getTextResource(self.__nameRes, eventName)())
        SystemMessages.pushMessage(text=backport.text(getTextResource(res + [NotificationPathPart.TEXT], eventName)()), priority=NotificationPriorityLevel.HIGH, type=SystemMessages.SM_TYPE.LootBoxSystemDisabled, messageData={b'header': (backport.text(getTextResource(res + [NotificationPathPart.HEADER], eventName)(), eventName=eventNameText))})
        return
...
class PM3NotificationListener(_NotificationListener):
    ...
    def __campaignSwitcherNotify(self, diff):
        for branchName, campaignSwitcher in PM_SWITCHES.MAP_BRANCH_NAME_TO_SWITCH_NAME.items():
            if campaignSwitcher in diff:
                ...
                if branch is None:
                    _logger.error(b'Personal Missions branch name "%s" is unknown', branchName)
                    continue
    ...
    def __operationSwitcherNotify(self):
        ...
        for operationID in disabledOperationsToNotify:
            ...
            if not operation:
                _logger.error(b'Wrong disabled personal mission operationID "%s"', operationID)
                continue
        ...
        for operationID in newEnabledOperations:
            ...
            if operation is None:
                _logger.error(b'Wrong enabled personal mission operationID "%s"', operationID)
                continue
    ...
    def __missionSwitcherNotify(self):
        ...
        for missionID in newDisabledMissionsToNotify:
            ...
            if not mission:
                _logger.error(b'Wrong disabled personal mission ID "%s"', missionID)
                continue
        ...
        for missionID in newEnabledMissions:
            mission = allMissions.get(missionID)
            if mission is None:
                _logger.error(b'Wrong enabled personal mission ID "%s"', missionID)
                continue
            operation = allOperations.get(mission.getOperationID())
...
class SkillTreePerkAvailableListener(BaseReminderListener):
    ...
    def start(self, model):
        ...
        if result:
            g_clientUpdateManager.addCallbacks({b'stats.vehTypeXP': (self.__onVehicleXPUpdated)})
            self.__tryNotify()
        return
    ...
    def _createNotificationData(self, priority, vehCD, nodeID, **ctx):
        gfDataID = str(uuid.uuid4())
        getCache().setPayload(gfDataID, {b'vehCD': vehCD, 
           b'nodeID': nodeID})
        data = {b'gfDataID': gfDataID}
...
class ChallengesListener(_NotificationListener):
    ...
    def __pushStartNotification(self, entityId, firstNotification):
        ...
        if model is not None:
            gfDataID = str(uuid.uuid4())
            getCache().setPayload(gfDataID, {b'first': firstNotification})
            model.addNotification(ChallengesStartDecorator(entityId, self.__TYPE, {b'gfDataID': gfDataID}, model, self.__TEMPLATE, NotificationPriorityLevel.HIGH))
        return
