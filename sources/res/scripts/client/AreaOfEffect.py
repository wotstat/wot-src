...
class EffectRunner(object):
    ...
    def prerequisites(self):
        ...
        for effect in viewvalues(self._equipment.effects):
            if not effect:
                continue
            for sequence in effect[b'sequences']:
    ...
    def playEffect(self, effectType, position, radius):
        ...
        if not effect:
            ...
            delayOffset = 0
            repeatDelay = effect[b'repeatDelay']
            repeatDelayDeviationPercent = effect[b'repeatDelayDeviationPercent']
            for _ in xrange(effect[b'repeatCount']):
            ...
            if effect[b'areaColor']:
                area = CombatSelectedArea.CombatSelectedArea()
                area.setup(position, Math.Vector3(0, 0, 0), Math.Vector2(radius * 2, radius * 2), CombatSelectedArea.DEFAULT_RADIUS_MODEL, effect[b'areaColor'], None)
    ...
    def _play(self, playID, effect, targetPosition):
        ...
        if effect[b'offsetDeviation']:
            radius = random.uniform(0, effect[b'offsetDeviation'])
            angle = random.uniform(0, 2 * math.pi)
            offset = Math.Vector3(radius * math.cos(angle), 0, radius * math.sin(angle))
            targetPosition = targetPosition + offset
        if effect[b'groundRaycast']:
...
class AreaOfEffect(BigWorld.Entity, EffectRunner):
    ...
    def __onAreaGOLoaded(self, objects, queue):
        ...
        t.transform = math_utils.createSRTMatrix(Math.Vector3(xScale, 1.0, zScale), (0.0, 0.0, 0.0), (
