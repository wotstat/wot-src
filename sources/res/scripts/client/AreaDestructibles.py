...
LaunchEffectArgs = namedtuple(b'LaunchEffectArgs', b'chunkID, destrIndex, moduleIndex,destrType, effectType, effectCategory, effectName, isHavokSpawnedDestructibles,pos, dir, scale')
...
def _extractEffectLists(desc):
    descType = desc[b'type']
    effectLists = []
    if descType == DestructiblesCache.DESTR_TYPE_TREE:
        effTypes = (b'fractureEffect', b'touchdownEffect')
        effDescs = (desc,)
    elif descType == DestructiblesCache.DESTR_TYPE_FALLING_ATOM:
        effTypes = (b'fractureEffect', b'touchdownEffect', b'touchdownBreakEffect')
        effDescs = (desc,)
    elif descType == DestructiblesCache.DESTR_TYPE_STRUCTURE:
        effTypes = (b'ramEffect', b'hitEffect', b'decayEffect')
        effDescs = desc[b'modules'].itervalues()
    else:
        effTypes = (b'effect', b'decayEffect')
        effDescs = (desc,)
...
def _printErrDescNotAvailable(spaceID, chunkID, destrIndex):
    ...
    if not objName:
        objName = b'unknown'
    LOG_ERROR(b'Destructible descriptor is not available, space: %s, object: %s, id: %d' % (
...
class DestructiblesManager(object):
    ...
    def onChunkLoad(self, chunkID, numDestructibles):
        if self.__spaceID is None:
            LOG_ERROR(b'Notification about chunk load came when no space started')
            return
        else:
    ...
    def __logErrorTooMuchDestructibles(self, chunkID):
        ...
        if player is None:
            LOG_ERROR(b'Number of destructibles more than 1024, chunk: %i %i' % (x, y))
        else:
            if x >= 0:
                xh = hex(x)[2:]
                xh = b'0000'[:4 - len(xh)] + xh
            else:
                xh = hex(65535 + x + 1)[2:]
            if y >= 0:
                yh = hex(y)[2:]
                yh = b'0000'[:4 - len(yh)] + yh
            else:
                yh = hex(65535 + y + 1)[2:]
            arenaTypeID = player.arenaTypeID
            LOG_ERROR(b'Number of destructibles more than 1024, arena: %s, chunk: %s' % (arenaTypeID, xh + yh + b'o'))
        return
    ...
    def __getEffectParams(self, lArgs):
        ...
        if destrType == DestructiblesCache.DESTR_TYPE_TREE:
            ...
            effectCat = b'fallingAtoms'
        elif destrType == DestructiblesCache.DESTR_TYPE_FRAGILE:
            effectCat = b'fragiles'
        elif destrType == DestructiblesCache.DESTR_TYPE_STRUCTURE:
            effectCat = b'structures'
    ...
    def __destroyFragile(self, chunkID, destrIndex, isNeedAnimation, isShotDamage):
        ...
        if isNeedAnimation:
            lArgs = LaunchEffectArgs(chunkID=chunkID, destrIndex=destrIndex, moduleIndex=-1, destrType=b'', effectType=b'decayEffect', effectName=b'', effectCategory=b'', pos=b'', dir=b'', scale=b'', isHavokSpawnedDestructibles=False)
            lArgsDecay = self.__getEffectParams(lArgs)
            if isShotDamage:
                lArgs = lArgs._replace(effectType=b'hitEffect')
            else:
                lArgs = lArgs._replace(effectType=b'effect')
    ...
    def onPlayModuleDestructionAnimation(self, chunkID, destrIndex, moduleIndex, isShotDamage, isHavokSpawnedDestructibles):
        args = LaunchEffectArgs(chunkID=chunkID, destrIndex=destrIndex, moduleIndex=moduleIndex, destrType=b'', effectType=b'', effectName=b'', effectCategory=b'', pos=b'', dir=b'', scale=b'', isHavokSpawnedDestructibles=isHavokSpawnedDestructibles)
    ...
    def __playModuleDestructionAnimation(self, effectArgs, isShotDamage):
        ...
        if effectArgs.effectName == b'':
            if isShotDamage:
                effectArgs = effectArgs._replace(effectType=b'hitEffect')
            else:
                effectArgs = effectArgs._replace(effectType=b'ramEffect')
    ...
    def __destroyModule(self, chunkID, destrIndex, matKind, isNeedAnimation, isShotDamage):
        ...
        effectArgs = LaunchEffectArgs(chunkID=chunkID, destrIndex=destrIndex, moduleIndex=moduleIndex, destrType=b'', effectType=b'', effectName=b'', effectCategory=b'', pos=b'', dir=b'', scale=b'', isHavokSpawnedDestructibles=False)
        if isNeedAnimation:
            if isShotDamage:
                effectArgs = effectArgs._replace(effectType=b'hitEffect')
            else:
    ...
    def __setFragileDestroyed(self, spaceID, chunkID, destrIndex, isNeedAnimation, isShotDamage):
        if spaceID != self.__spaceID:
            ...
            return BigWorld.destroyFragile(spaceID, chunkID, destrIndex, isNeedAnimation, isShotDamage)

    def __setModuleDestroyed(self, spaceID, chunkID, destrIndex, moduleIndex, isNeedAnimation, isShotDamage):
        if spaceID != self.__spaceID:
            return None
        else:
            return BigWorld.destroyModule(spaceID, chunkID, destrIndex, moduleIndex, isNeedAnimation, isShotDamage)
    ...
    def __dropDestructible(self, chunkID, destrIndex, dmgType, fallDirYaw, pitchConstr, fallSpeed, isAnimate, obstacleCollisionFlags):
        ...
        if dmgType == DestructiblesCache.DESTR_TYPE_FALLING_ATOM:
            if isAnimate:
                self.__launchFallEffect(chunkID, destrIndex, b'fractureEffect', fallDirYaw)
                if useEffectsOnTouchDown:
                    touchdownCallback = partial(self.__touchDownWithEffect, chunkID, destrIndex, fallDirYaw, b'touchdownEffect', b'touchdownBreakEffect')
                else:
            ...
            if isAnimate:
                ...
                self.__launchTreeFallEffect(chunkID, destrIndex, b'fractureEffect', fallDirYaw, tintColor)
                if useEffectsOnTouchDown:
                    touchdownCallback = partial(self.__launchTreeFallEffect, chunkID, destrIndex, b'touchdownEffect', fallDirYaw, tintColor)
            initialMatrix = self.__getDestrInitialMatrix(chunkID, destrIndex)
    ...
    def __launchEffect(self, args, callbackOnStop=None):
        if args.effectName == b'none':
            return
        else:
            effectVars = g_cache._getEffect(args.effectName, args.effectCategory, False)
            if not effectVars:
                LOG_ERROR(b'Could not find any effects vars for: ' + str(args.effectName) + b' - type: ' + str(args.effectType) + b' - cat: ' + str(args.effectCategory) + b' (' + str(args.destrType) + b')')
                return
            player = BigWorld.player()
            if player is None or isPlayerAccount():
                return
            effectStuff = random.choice(effectVars)
            effectID = player.terrainEffects.addNew(args.pos, effectStuff.effectsList, effectStuff.keyPoints, callbackOnStop, dir=args.dir, scale=args.scale, havokSpawnedDestructibles=args.isHavokSpawnedDestructibles, waitForKeyOff=args.effectType == b'lifetimeEffect')
            return effectID
    ...
    def __startLifetimeEffect(self, chunkID, destrIndex, moduleIndex):
        ...
        if chance is None or random.random() > chance:
            return
        lArgs = LaunchEffectArgs(chunkID=chunkID, destrIndex=destrIndex, moduleIndex=moduleIndex, destrType=b'', effectType=b'lifetimeEffect', effectName=b'', effectCategory=b'', pos=b'', dir=b'', scale=b'', isHavokSpawnedDestructibles=False)
        lArgs = self.__getEffectParams(lArgs)
    ...
    def __pickPrefferedTiltAngle(self, chunkID, destrIndex, hitDirYaw, desc):
        ...
        tiltYaw_localFrame = min(desc[b'preferredTiltDirections'] or [hitYaw_localFrame], key=(lambda angle: abs(angle - hitYaw_localFrame)))
    ...
    def __launchFallEffect(self, chunkID, destrIndex, effectType, fallDirYaw):
        ...
        if effectName == b'none':
            return
        else:
            effectVars = g_cache._getEffect(effectName, b'fallingAtoms', False)
    ...
    def __launchTreeFallEffect(self, chunkID, destrIndex, effectType, fallDirYaw, tintColor=COLOR_WHITE):
        ...
        if effectName == b'none':
            ...
            effectVars = g_cache._getEffect(effectName, b'trees', False)
            if effectVars is None:
                return
            effectStuff = random.choice(effectVars)
            chunkMatrix = BigWorld.getChunkMatrix(self.__spaceID, chunkID)
...
class _DestructiblesAnimator(object):
    ...
    def showFall(self, spaceID, chunkID, destrIndex, fallDirYaw, pitchConstr, discreteInitSpeed, isNeedAnimation, initialMatrix, touchdownCallback=None):
        ...
        body = {b'pitch': pitch, 
           b'pitchSpeed': pitchSpeed, 
           b'inertiaMoment': inertiaMoment, 
           b'mass': mass, 
           b'height': height, 
           b'springAngle': (fallingParams.get(1, 1)), 
           b'springStiffnes': stiffness, 
           b'springResist': (fallingParams.get(1, 2) * scale * scale), 
           b'airResist': (fallingParams.get(0, 2) * scale * scale), 
           b'pitchConstr': pitchConstr, 
           b'preRot': destrMatrix, 
           b'postRot': fallYawRot, 
           b'translation': translation, 
           b'chunkID': chunkID, 
           b'spaceID': spaceID, 
           b'destrIndex': destrIndex, 
           b'simulationTime': 0.0, 
           b'buryDepth': (fallingParams.get(0, 3) * scale), 
           b'destrType': (self.__FALL_ATOM)}
        if touchdownCallback is not None:
            body[b'touchdownCallback'] = touchdownCallback
        if isNeedAnimation:
            ...
            body[b'pitch'] = BigWorld.solveDestructibleFallPitch(weight, angStiffness, pitchConstr - fallingParams.get(1, 1), approxPitch)
    ...
    def showFallTree(self, spaceID, chunkID, destrIndex, fallDirYaw, pitchConstr, discreteInitSpeed, isNeedAnimation, initialMatrix, touchdownCallback=None):
        ...
        body = {b'pitch': pitch, 
           ...
           b'inertiaMoment': inertiaMoment, 
           b'mass': massScaled, 
           b'height': heightScaled, 
           b'springAngle': springAngle, 
           b'springStiffnes': stiffnessScaled, 
           b'springResist': (springResist * scale * scale), 
           b'airResist': (airResist * scale * scale), 
           b'pitchConstr': pitchConstr, 
           b'preRot': destrMatrix, 
           b'postRot': fallYawRot, 
           b'translation': translation, 
           ...
           b'destrIndex': destrIndex, 
           b'simulationTime': 0.0, 
           b'buryDepth': (buryDepth * scale), 
           b'destrType': (self.__FALL_TREE)}
        if touchdownCallback is not None:
            body[b'touchdownCallback'] = touchdownCallback
        if isNeedAnimation:
            self.__bodies.append(body)
            ...
            body[b'pitch'] = BigWorld.solveDestructibleFallPitch(weight, angStiffness, pitchConstr - springAngle, approxPitch)
            self.__positionBodyModel(body)
        return
    ...
    def __moveBody(self, body, dt):
        pitch = body[b'pitch']
        if pitch < 0.0:
            body[b'pitchSpeed'] = 0.0
            body[b'pitch'] = 0.0
            pitch = 0.0
        pitchConstr = body[b'pitchConstr']
        if pitch > pitchConstr:
            body[b'pitchSpeed'] = 0.0
            body[b'pitch'] = pitchConstr
            pitch = pitchConstr
            if BigWorld.isHavokActive() and body[b'destrType'] == self.__FALL_ATOM:
                touchdownCallback = body.get(b'touchdownCallback', None)
                if touchdownCallback is not None:
                    del body[b'touchdownCallback']
                    touchdownCallback()
                    return False
        height = body[b'height']
        mp = height * height * 0.25
        torque = 0.5 * height * physics_shared.G * body[b'mass'] * math.sin(pitch)
        torque -= body[b'pitchSpeed'] * body[b'airResist'] * mp
        if pitch + body[b'springAngle'] > pitchConstr:
            touchdownCallback = body.get(b'touchdownCallback', None)
            if touchdownCallback is not None:
                touchdownCallback()
                del body[b'touchdownCallback']
                if BigWorld.isHavokActive() and body[b'destrType'] == self.__FALL_ATOM:
                    return False
            anglePen = pitch + body[b'springAngle'] - pitchConstr
            torque -= anglePen * body[b'springStiffnes'] * mp
            torque -= body[b'pitchSpeed'] * body[b'springResist'] * mp
        inertialMoment = body[b'inertiaMoment']
        if inertialMoment != 0.0:
            body[b'pitchSpeed'] += dt * torque / inertialMoment
        body[b'simulationTime'] += dt
        if abs(body[b'pitchSpeed']) + abs(torque) < self.__STOP_SIMULATION_EPSILON or body[b'simulationTime'] > self.__MAX_SIMULATION_DURATION:
            return False
        else:
            body[b'pitch'] += dt * body[b'pitchSpeed']
            return True

    def __positionBodyModel(self, body):
        m = Math.Matrix()
        m.setRotateX(body[b'pitch'])
        m.preMultiply(body[b'preRot'])
        m.postMultiply(body[b'postRot'])
        m.translation = body[b'translation'] - (0.0, body[b'buryDepth'] * math.sin(body[b'pitch']), 0.0)
        windK = max(1.0 - body[b'simulationTime'] / 2.0, 0)
        spaceID = body[b'spaceID']
        BigWorld.setDestructibleMatrix(spaceID, body[b'chunkID'], body[b'destrIndex'], m, windK)
        return
