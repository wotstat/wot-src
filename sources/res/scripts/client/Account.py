...
class _ClientCommandProxy(object):
    ...
    def perform(self, commandName, *args):
        if self.__commandGateway is None:
            raise SoftException(b'Cliend command proxy is not ready')
        callback = args[-1]
        commandArgs = args[:-1]
...
class PlayerAccount(BigWorld.Entity, ClientChat):
    ...
    def showGUI(self, ctx):
        ...
        g_bootcamp.setBootcampParams({b'completed': (ctx.get(b'bootcampCompletedCount', 0)), 
           b'runCount': (ctx.get(b'bootcampRunCount', 0)), 
           b'needAwarding': (ctx.get(b'bootcampNeedAwarding', False))})
        currentVehInvID = ctx.get(b'currentVehInvID', 0)
        if currentVehInvID > 0:
            AccountSettings.setFavorites(CURRENT_VEHICLE, currentVehInvID)
        events.isPlayerEntityChanging = False
        if ctx.get(b'skipShowGUI', False):
    ...
    def requestPlayerInfo(self, databaseID, callback):
        if events.isPlayerEntityChanging:
            return
        proxy = lambda requestID, resultID, errorStr, ext={}: callback(resultID, ext.get(b'databaseID', 0L), ext.get(b'dossier', b''), ext.get(b'clanDBID', 0), ext.get(b'clanInfo', None), ext.get(b'globalRating', 0), ext.get(b'eSportSeasons', {}), ext.get(b'ranked', {}), ext.get(b'dogTag', {}), ext.get(b'battleRoyaleStats', {}), ext.get(b'wtr', None), ext.get(b'layout', None), ext.get(b'layoutState', None))
    ...
    def chooseQuestReward(self, eventType, questID, rewardID, callback=None):
        if callback is not None:
            ...
            proxy = None
        strArr = [questID.encode(b'utf8'), rewardID.encode(b'utf8')]
    ...
    def _update(self, triggerEvents, diff):
        ...
        serverSettingsKey = (b'serverSettings', b'_r')
        ...
        if not self.syncData.updatePersistentCache(diff, isFullSync):
            ...
            self._synchronizeCacheDict(self.personalMissionsLock, diff.get(b'cache', None), b'pmQuestsIDs', b'replace', events.onPMLocksChanged)
            ...
            if triggerEvents and not isFullSync:
                ...
                for vehInvID, lockReason in diff.get(b'cache', {}).get(b'vehsLock', {}).iteritems():
