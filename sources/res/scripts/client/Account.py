...
class _ClientCommandProxy(object):
    _COMMAND_SIGNATURES = (
     (
      b'doCmdStr', (lambda args: len(args) == 1 and _isStr(args[0]))),
     (
      b'doCmdIntStr', (lambda args: len(args) == 2 and _isInt(args[0]) and _isStr(args[1]))),
     (
      b'doCmdInt', (lambda args: len(args) == 1 and _isInt(args[0]))),
     (
      b'doCmdInt2', (lambda args: len(args) == 2 and all(_isInt(arg) for arg in args))),
     (
      b'doCmdInt3', (lambda args: len(args) == 3 and all(_isInt(arg) for arg in args))),
     (
      b'doCmdInt4', (lambda args: len(args) == 4 and all(_isInt(arg) for arg in args))),
     (
      b'doCmdInt2Str', (lambda args: len(args) == 3 and _isStr(args[2]) and all(_isInt(arg) for arg in args[:2]))),
     (
      b'doCmdInt3Str', (lambda args: len(args) == 4 and _isStr(args[3]) and all(_isInt(arg) for arg in args[:3]))),
     (
      b'doCmdIntArr', (lambda args: len(args) == 1 and _isIntList(args[0]))),
     (
      b'doCmdIntStrArr', (lambda args: len(args) == 2 and _isInt(args[0]) and _isStrList(args[1]))),
     (
      b'doCmdStrArr', (lambda args: len(args) == 1 and _isStrList(args[0]))),
     (
      b'doCmdIntArrStrArr', (lambda args: len(args) == 2 and _isIntList(args[0]) and _isStrList(args[1]))),
     (
      b'doCmdNoArgs', (lambda args: len(args) == 0)))
    ...
    def perform(self, commandName, *args):
        if self.__commandGateway is None:
            raise SoftException(b'Client command proxy is not ready')
        callback = args[-1]
        commandArgs = args[:-1]
        for commandType, signatureCheck in self._COMMAND_SIGNATURES:
            ...
            raise SoftException((b'Command "{}" failed. Given signature is not supported.').format(commandName))

        return


class PlayerAccount(BigWorld.Entity, ClientChat):
    __onStreamCompletePredef = {(AccountCommands.REQUEST_ID_PREBATTLES): b'receivePrebattles', 
       (AccountCommands.REQUEST_ID_PREBATTLE_ROSTER): b'receivePrebattleRoster'}
    ...
    def __init__(self):
        ...
        _logger.info(b'client Account.init')
    ...
    def onBecomePlayer(self):
        uniprof.enterToRegion(b'player.account.entering')
        _logger.info(b'Account.onBecomePlayer()')
        ...
        uniprof.exitFromRegion(b'player.account.entering')
        return

    def onBecomeNonPlayer(self):
        uniprof.enterToRegion(b'player.account.exiting')
        _logger.info(b'Account.onBecomeNonPlayer()')
        ...
        uniprof.exitFromRegion(b'player.account.exiting')
        return
    ...
    def onCmdResponseExt(self, requestID, resultID, errorStr, ext):
        ...
        if resultID == AccountCommands.RES_SHOP_DESYNC:
            self.shop.synchronize(ext.get(b'shopRev', None))
        callback = self.__onCmdResponse.pop(requestID, None)
        if callback is not None:
    ...
    def onIGRTypeChanged(self, data):
        try:
            ...
            events.onIGRTypeChanged(data.get(b'roomType'), data.get(b'igrXPFactor'))
            LOG_DEBUG(b'onIGRTypeChanged', data)
        except Exception:
    ...
    def onKickedFromQueue(self, queueType):
        LOG_DEBUG(b'onKickedFromQueue', queueType)
        self.battleQueueType = QUEUE_TYPE.UNKNOWN
        events.onKickedFromQueue(queueType)
        return

    def onArenaCreated(self):
        LOG_DEBUG(b'onArenaCreated')
        self.prebattle = None
    ...
    def onArenaJoinFailure(self, errorCode, errorStr):
        LOG_DEBUG(b'onArenaJoinFailure', errorCode, errorStr)
        self.battleQueueType = QUEUE_TYPE.UNKNOWN
    ...
    def onPrebattleJoinFailure(self, errorCode):
        LOG_DEBUG(b'onPrebattleJoinFailure', errorCode)
        events.onPrebattleJoinFailure(errorCode)
        return

    def onPrebattleLeft(self):
        LOG_DEBUG(b'onPrebattleLeft')
        self.prebattle = None
    ...
    def onKickedFromPrebattle(self, reasonCode):
        LOG_DEBUG(b'onKickedFromPrebattle', reasonCode)
        self.prebattle = None
        events.onKickedFromPrebattle(reasonCode)
        return

    def onCenterIsLongDisconnected(self, isLongDisconnected):
        LOG_DEBUG(b'onCenterIsLongDisconnected', isLongDisconnected)
    ...
    def onSendPrebattleInvites(self, dbID, name, clanDBID, clanAbbrev, prebattleID, status):
        LOG_DEBUG(b'onSendPrebattleInvites', dbID, name, clanDBID, clanAbbrev, prebattleID, status)
    ...
    def onClanInfoReceived(self, clanDBID, clanName, clanAbbrev, clanMotto, clanDescription):
        LOG_DEBUG(b'onClanInfoReceived', clanDBID, clanName, clanAbbrev, clanMotto, clanDescription)
    ...
    def showGUI(self, ctx):
        ...
        _logger.info(b'showGUI %r', ctx)
        self.databaseID = ctx[b'databaseID']
        if b'prebattleID' in ctx:
            self.prebattle = ClientPrebattle.ClientPrebattle(ctx[b'prebattleID'])
        self.battleQueueType = ctx.get(b'inQueue', QUEUE_TYPE.UNKNOWN)
        self._initTimeCorrection(ctx)
        if b'isLongDisconnectedFromCenter' in ctx:
            isLongDisconnectedFromCenter = ctx[b'isLongDisconnectedFromCenter']
    ...
    def requestPlayerInfo(self, databaseID, callback):
        if events.isPlayerEntityChanging:
            return
        proxy = lambda requestID, resultID, errorStr, ext={}: callback(resultID, ext.get(b'databaseID', 0), ext.get(b'dossier', b''), ext.get(b'clanDBID', 0), ext.get(b'clanInfo', None), ext.get(b'globalRating', 0), ext.get(b'eSportSeasons', {}), ext.get(b'ranked', {}), ext.get(b'dogTag', {}), ext.get(b'battleRoyaleStats', {}), ext.get(b'wtr', None), ext.get(b'layout', None), ext.get(b'layoutState', None), ext.get(b'serviceRecordCustomization', {}))
    ...
    def requestAccountDossier(self, accountID, callback):
        if events.isPlayerEntityChanging:
            return
        proxy = lambda requestID, resultID, errorStr, ext={}: callback(resultID, ext.get(b'dossier', b''))
    ...
    def requestPlayerClanInfo(self, databaseID, callback):
        if events.isPlayerEntityChanging:
            return
        proxy = lambda requestID, resultID, errorStr, ext={}: callback(resultID, errorStr, ext.get(b'clanDBID', 0), ext.get(b'clanInfo', None))
    ...
    def requestPlayerGlobalRating(self, accountID, callback):
        if events.isPlayerEntityChanging:
            return
        proxy = lambda requestID, resultID, errorStr, ext={}: callback(resultID, errorStr, ext.get(b'globalRating', 0))
    ...
    def prb_teamReady(self, team, force, gameplaysMask, callback):
        if events.isPlayerEntityChanging:
            ...
            prb = self.prebattle
            if prb is None:
                LOG_ERROR(b'No prebattle info')
                return
            s = prb.settings
            if s[b'type'] == PREBATTLE_TYPE.SQUAD and s.get(b'gameplaysMask', 0) != gameplaysMask:
    ...
    def getPersonalMissionReward(self, personalMissionsIDs, questType, needTamkman=False, tmanNation=0, tmanInnation=0, roleID=1, callback=None):
        arr = [
         questType, personalMissionsIDs, needTamkman, tmanNation, 
         tmanInnation, roleID]
        if callback is not None:
    ...
    def requestWGMBalanceInfo(self, callback):
        if events.isPlayerEntityChanging:
            return
        proxy = lambda requestID, resultID, errorStr, ext={}: callback(resultID, errorStr, ext)
        self._doCmdStr(AccountCommands.CMD_QUERY_BALANCE_INFO, b'', proxy)
        return
    ...
    def addBlueprintFragment(self, fragmentTypeCD, requestedCount, other=-1):
        LOG_DEBUG((b'Account.addBlueprintFragment: fragmentTypeCD={}, count={}').format(fragmentTypeCD, requestedCount))
    ...
    def _doCmdNoArgs(self, cmd, callback):
        return self.__doCmd(b'doCmdNoArgs', cmd, callback)

    def _doCmdStr(self, cmd, s, callback):
        return self.__doCmd(b'doCmdStr', cmd, callback, s)

    def _doCmdIntStr(self, cmd, int1, s, callback):
        return self.__doCmd(b'doCmdIntStr', cmd, callback, int1, s)

    def _doCmdInt(self, cmd, int_, callback):
        return self.__doCmd(b'doCmdInt', cmd, callback, int_)

    def _doCmdInt2(self, cmd, int1, int2, callback):
        return self.__doCmd(b'doCmdInt2', cmd, callback, int1, int2)

    def _doCmdInt3(self, cmd, int1, int2, int3, callback):
        return self.__doCmd(b'doCmdInt3', cmd, callback, int1, int2, int3)

    def _doCmdInt3Str(self, cmd, int1, int2, int3, s, callback):
        return self.__doCmd(b'doCmdInt3Str', cmd, callback, int1, int2, int3, s)

    def _doCmdInt4(self, cmd, int1, int2, int3, int4, callback):
        return self.__doCmd(b'doCmdInt4', cmd, callback, int1, int2, int3, int4)

    def _doCmdInt2Str(self, cmd, int1, int2, s, callback):
        return self.__doCmd(b'doCmdInt2Str', cmd, callback, int1, int2, s)

    def _doCmdIntArr(self, cmd, arr, callback):
        return self.__doCmd(b'doCmdIntArr', cmd, callback, arr)

    def _doCmdIntStrArr(self, cmd, int1, strArr, callback):
        return self.__doCmd(b'doCmdIntStrArr', cmd, callback, int1, strArr)

    def _doCmdIntArrStrArr(self, cmd, intArr, strArr, callback):
        return self.__doCmd(b'doCmdIntArrStrArr', cmd, callback, intArr, strArr)

    def _doCmdStrArr(self, cmd, strArr, callback):
        return self.__doCmd(b'doCmdStrArr', cmd, callback, strArr)

    def _update(self, triggerEvents, diff):
        LOG_DEBUG_DEV(b'_update', diff if triggerEvents else b'full sync')
        isFullSync = AccountSyncData.isFullSyncDiff(diff)
        if not self.syncData.updatePersistentCache(diff, isFullSync):
            return False
        else:
            self.syncData.revision = diff.get(b'rev', 0)
            ...
            self._synchronizeCacheDict(self.prebattleAutoInvites, diff.get(b'account', None), b'prebattleAutoInvites', b'replace', events.onPrebattleAutoInvitesChanged)
            self._synchronizeCacheDict(self.prebattleInvites, diff, b'prebattleInvites', b'update', (lambda : events.onPrebattleInvitesChanged(diff)))
            self._synchronizeCacheDict(self.clanMembers, diff.get(b'cache', None), b'clanMembers', b'replace', events.onClanMembersListChanged)
            self._synchronizeCacheDict(self.eventsData, diff, b'eventsData', b'replace', events.onEventsDataChanged)
            self._synchronizeCacheDict(self.personalMissionsLock, diff.get(b'cache', None), b'potapovQuestIDs', b'replace', events.onPMLocksChanged)
            self._synchronizeCacheDict(self.dailyQuests, diff, b'dailyQuests', b'replace', events.onDailyQuestsInfoChange)
            self._synchronizeCacheDict(self.weeklyQuests, diff, b'weeklyQuests', b'update', events.onWeeklyQuestsInfoChange)
            self._synchronizeCacheSimpleValue(b'globalRating', diff.get(b'account', None), b'globalRating', events.onAccountGlobalRatingChanged)
            self._synchronizeCacheDict(self.platformBlueprintsConvertSaleLimits, diff, b'platformBlueprintsConvertSaleLimits', b'replace', events.onPlatformBlueprintsConvertSaleLimits)
            self._synchronizeCacheDict(self.renewableSubscription, diff, b'renewableSub', b'replace', events.onRenewableSubscriptionStatusChanged)
            if ENABLE_FREE_PREMIUM_CREW:
                synchronizeDicts(diff.get(b'freePremiumCrew', {}), self.freePremiumCrew)
            ...
            if triggerEvents and not isFullSync:
                for vehTypeCompDescr in diff.get(b'stats', {}).get(b'eliteVehicles', ()):
                    events.onVehicleBecomeElite(vehTypeCompDescr)

                for vehInvID, lockReason in viewitems(diff.get(b'cache', {}).get(b'vehsLock', {})):
    ...
    def _synchronizeCacheDict(self, repDict, diffDict, key, syncMode, event):
        if syncMode not in (b'update', b'replace'):
            raise SoftException((b'Mode {} is not supported on the client').format(syncMode))
        if diffDict is None:
            return
        else:
            repl = diffDict.get((key, b'_r'), None)
            if repl is not None:
                repDict.clear()
                repDict.update(repl)
            ...
            if diff is not None:
                if isinstance(diff, dict):
                    ...
                    LOG_WARNING(b'_synchronizeCacheDict: bad diff=%r for key=%r' % (diff, key))
    ...
    def _initTimeCorrection(self, ctx):
        if b'serverUTC' in ctx:
            import helpers.time_utils as tm
            tm.setTimeCorrection(ctx[b'serverUTC'])
        return

    def _synchronizeEventNotifications(self, diff):
        diffDict = {b'added': [], b'removed': []}
        if diff is None:
            return
        else:
            initialEventsData = diff.get((b'eventsData', b'_r'), {})
            ...
            if initial is not None:
                ...
                diffDict[b'added'].extend(initial)
            updatedEventsData = diff.get(b'eventsData', {})
            ...
            if updated is not None:
                ...
                added = new - prev
                removed = prev - new
                diffDict[b'added'].extend([n.item for n in added])
                diffDict[b'removed'].extend([n.item for n in removed])
            if initial is not None or updated is not None:
                events.onEventNotificationsChanged(diffDict)
                LOG_DEBUG_DEV(b'Account.__synchronizeEventNotifications, diff=%s' % (diffDict,))
            return
    ...
    def _synchronizeServerSettings(self, diffDict):
        if diffDict is None:
            ...
            fullServerSettings = diffDict.get((b'serverSettings', b'_r'), None)
            if fullServerSettings is not None:
                serverSettings.clear()
                serverSettings.update(fullServerSettings)
            serverSettingsDiff = diffDict.get(b'serverSettings', None)
            if serverSettingsDiff is not None:
                if isinstance(serverSettingsDiff, dict):
                    ...
                    LOG_WARNING(b'_synchronizeCacheDict: bad diff=%r for key=%r' % (serverSettingsDiff, key))
            return
    ...
    def _synchronizeDisabledPersonalMissions(self, diffDict):
        if diffDict is None:
            ...
            serverSettings = self.serverSettings
            disabledSectionKeys = (b'disabledPersonalMissions', b'disabledPMOperations')
            serverSettingsDiff = diffDict.get(b'serverSettings', None)
            if serverSettingsDiff is not None:
                if isinstance(serverSettingsDiff, dict):
                    for sectionKey in disabledSectionKeys:
                        if sectionKey in serverSettingsDiff:
                            if isinstance(serverSettingsDiff[sectionKey], dict):
                                ...
                                LOG_WARNING(b'_synchronizeCacheDict: bad diff=%r for key=%r' % (
    ...
    def __cancelCommands(self):
        for requestID, callback in viewitems(self.__onCmdResponse):
            try:
                callback(requestID, AccountCommands.RES_NON_PLAYER, b'NON_PLAYER')
            except Exception:
                LOG_CURRENT_EXCEPTION()
...
class _AccountRepository(object):
    ...
    def __init__(self, name, className, initialServerSettings):
        ...
        self.customFilesCache = CustomFilesCache.CustomFilesCache(b'custom_data')
    ...
    def fileServerSettings(self):
        return self.serverSettings[b'file_server']


def delAccountRepository():
    global g_accountRepository
    LOG_DEBUG(b'delAccountRepository')
    if g_accountRepository is None:
        return
    else:
