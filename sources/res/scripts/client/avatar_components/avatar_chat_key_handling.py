...
_IS_NON_PLAYER_NOTIFICATION = b'_npc'
...
CommandNotificationData = namedtuple(b'CommandNotificationData', b'matrixProvider, targetID')
...
class AvatarChatKeyHandling(object):
    ...
    def __onSettingsChanged(self, diff):
        _logger.debug(b'Settings has changed, checking the battle communication.')
    ...
    def __getHQMatrixProvider(self, cmd, destructibleEntityID=None):
        destructibleEntityComp = getattr(self.guiSessionProvider.arenaVisitor.getComponentSystem(), b'destructibleEntityComponent', None)
    ...
    def __onCommandReceivedReply(self, commandName, cmd):
        ...
        if replyToActionName not in ONE_SHOT_COMMANDS_TO_REPLIES.keys():
            ...
            if notificationReply is None:
                _logger.warning(b'notificationReply is None for replyToActionName = %s !- Take a look at messenger_common_chat2.py', replyToActionName)
                return
    ...
    def __onCommandReceivedAttentionToPositionReply(self, replyToActionName, cmd):
        ...
        notificationReply = _ACTIONS.battleChatCommandFromActionID(repliedToActionID).soundNotificationReply
        if notificationReply is None:
            _logger.warning(b'soundNotificationReply is None for replyToActionName %s! - Take a look at messenger_common_chat2.py', replyToActionName)
            return
        else:
            self.__playSoundNotificationOnCommandReceived(cmd, MarkerType.VEHICLE_MARKER_TYPE, True, notificationReply)
            return

    def __onStaticMarkerAdded(self, areaID, creatorID, position, markerSymbolName, markerText=b'', numberOfReplies=0, isTargetForPlayer=False):
    ...
    def __switchSoundFXTo(self, enabled):
        prevValue = False
        if self.soundNotifications:
            prevValue = self.soundNotifications.isCategoryEnabled(b'fx')
            ...
            if enableVoice is True:
                if cmd.isReceiver():
                    if self.__arePrivateVoiceOverBlocked is False:
                        ...
                        enableVoice = False
                        _logger.info(b'Voice was blocked for the receiver of a private message due to flood prevention system!')
            cmdSenderVehicleID = self.__getVehicleIDForCmdSender(cmd)
    ...
    def __playSoundNotification(self, notification, sndPos=None, enableVoice=True, isSentByPlayer=True):
        if not self.soundNotifications or notification is None:
            return
        categoryVoiceIsEnabled = self.soundNotifications.isCategoryEnabled(b'voice')
