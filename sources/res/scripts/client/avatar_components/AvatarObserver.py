...
_STRATEGIC_VIEW = (
 CTRL_MODE_NAME.STRATEGIC, CTRL_MODE_NAME.ARTY, CTRL_MODE_NAME.SPG_ONLY_ARTY_MODE, CTRL_MODE_NAME.ASSAULT_SPG)

class ObservedVehicleData(object):
    __slots__ = (b'dispAngle',)
...
class AvatarObserver(CallbackDelayer):
    ...
    def onVehicleChanged(self):
        ...
        if not self.vehicle and self.observerSeesAll():
            ...
            if self.vehicle is not None:
                typeofveh = b'observed' if self.__observedVehicleID == self.vehicle.id else b'players'
                _logger.debug(b'Vehicle ID is %r and is %r', self.vehicle.id, typeofveh)
            ...
            if isObserving and self.vehicle is not None:
                ...
                self.guiSessionProvider.getArenaDP().switchCurrentTeam(self.vehicle.publicInfo[b'team'])
                ...
                if hasattr(self.vehicle.filter, b'enableStabilisedMatrix'):
                    self.vehicle.filter.enableStabilisedMatrix(True)
                if not self.vehicle.isDestroyed and typeofveh == b'observed':
    ...
    def updateObservedVehicleData(self):
        ...
        if vehicle is not None and hasattr(vehicle, b'ownVehicle'):
    ...
    def set_isObserverFPV(self, prev):
        _logger.debug(b'AvatarObserver.set_isObserverFPV() %r', self.isObserverFPV)
    ...
    def __switchToObservedControlMode(self):
        ...
        _logger.info(b'AvatarObserver.__switchToObservedControlMode(): %r, %r', self.__observerFPVControlMode, eMode)
        if self.observerSeesAll() and self.inputHandler.ctrlModeName == eMode:
            ...
            if filteredValue is None or filteredValue == Math.Vector3(0, 0, 0):
                _logger.info(b'AvatarObserver.__switchToObservedControlMode(): no filtered value yet.Rescheduling switch... %r', filteredValue)
