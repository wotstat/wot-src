...
_KB_MAPPING = {b'EPIC_GLOBAL_SAVETANKS': (CommandMapping.EPIC_GLOBAL_MSG_SAVE_TANKS), 
   b'EPIC_GLOBAL_TIME': (CommandMapping.EPIC_GLOBAL_MSG_TIME), 
   b'EPIC_GLOBAL_HQ': (CommandMapping.EPIC_GLOBAL_MSG_FOCUS_HQ), 
...
_ATTACKER_POSTFIX = b'_ATK'
_DEFENDER_POSTFIX = b'_DEF'
...
class AvatarEpicData(object):
    ...
    def showDestructibleShotResults(self, destructibleEntityID, hitFlagsList):
        LOG_DEBUG_DEV(b'showDestructibleShotResults', destructibleEntityID, hitFlagsList)
        if self.arena.componentSystem and hasattr(self.arena.componentSystem, b'destructibleEntityComponent'):
    ...
    def welcomeToSector(self, sectorID, groupID, groupState, goodGroup, actionTime, actionDuration):
        ...
        flComponent = self.dynamicComponents.get(b'avatarFrontlineComponent')
        if flComponent:
    ...
    def __sendGlobalChatCmdIfPossible(self, key):
        sectorBaseCmp = getattr(self.guiSessionProvider.arenaVisitor.getComponentSystem(), b'sectorBaseComponent', None)
