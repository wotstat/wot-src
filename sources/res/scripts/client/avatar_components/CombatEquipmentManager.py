...
class _DebugFrontLine(CallbackDelayer):
    ...
    def __init__(self, beginExplosionPos, endExplosionPos, areaWidth, velocity):
        ...
        self.model = BigWorld.Model(b'helpers/models/unit_cube.model')
...
class CombatEquipmentManager(object):
    ...
    def updatePlaneTrajectory(self, planeTrajectory):
        if _ENABLE_DEBUG_LOG:
            LOG_DEBUG(b'===== updatePlaneTrajectory =====')
    ...
    def onCombatEquipmentShotLaunched(self, equipmentID, position):
        if _ENABLE_DEBUG_LOG:
            LOG_DEBUG(b'===== onCombatEquipmentShotLaunched =====')
        ...
        if equipment is None:
            ...
            if equipment.wwsoundShot is None:
                LOG_DEBUG(b'LOG_GOGGI - wwsoundShot is None for ', equipment.name)
                return
            shotSoundPreDelay = 0.0
            if hasattr(equipment, b'shotSoundPreDelay'):
                if equipment.shotSoundPreDelay is not None:
                    shotSoundPreDelay = equipment.shotSoundPreDelay
            delay = 0.0
            if hasattr(equipment, b'delay'):
                delay = equipment.delay
            elif hasattr(equipment, b'minDelay'):
            ...
            LOG_DEBUG(b'LOG_GOGGI - actual shot delay: ', delay, b' equipment.wwsoundShot: ', equipment.wwsoundShot)
            return
    ...
    def showCarpetBombing(self, equipmentID, pos, direction, time):
        ...
        airstrikeID = shotEffect.get(b'airstrikeID')
        if airstrikeID is None:
            LOG_ERROR(b'EquipmentID %s has no airstrike shot effect settings' % equipmentID)
            return
        else:
            areaWidth, areaLength = bombEquipment.areaWidth, bombEquipment.areaLength
            if _ENABLE_DEBUG_LOG:
                LOG_DEBUG(b'Ideal', areaWidth, areaLength)
            ...
            if _ENABLE_DEBUG_LOG:
                LOG_DEBUG(b'delta', delay)
                LOG_DEBUG(b'pos, dir', pos, direction)
                LOG_DEBUG(b'Params for artyStrike effect', airstrikeID, beginExplosionPos, flatDir, areaWidth, areaLength, bombsPerWidth, bombsPerLength)
            if delay < 0.0:
