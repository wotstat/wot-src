...
class PrimaryHintController(object):
    SHOW_SOUND_ID = b'ob_main_tips_activity_start'
    COMPLETE_SOUND_ID = b'ob_main_tips_activity'
    HIDE_SOUND_ID = b'ob_main_tips_activity_done'
    TASK_START_SOUND_ID = b'ob_main_tips_activity_start'
    TASK_DONE_SOUND_ID = b'ob_main_tips_activity_done'
    HINT_IDS_TO_MUTE = tuple()
    HINT_IDS_TO_COMPLETE = (
     HintType.Move, HintType.MoveTurret, HintType.Shoot)
    ...
    def show(self):
        g_eventBus.handleEvent(HintActionEvent(EventAction.Show, ctx={b'hintType': (self.typeId), 
           b'completed': (self._completed), b'message': (self.message), 
           b'hideCallback': (self.onHided)}), self._scope)
        if self._completed:
