...
class DamageFromShotDecoder(object):
    ShotPoint = namedtuple(b'ShotPoint', (b'componentName', b'matrix', b'hitEffectCode', b'hitEffectGroup'))
    _HIT_EFFECT_CODE_TO_EFFECT_GROUP = {(VEHICLE_HIT_EFFECT.INTERMEDIATE_RICOCHET): b'armorBasicRicochet', 
       (VEHICLE_HIT_EFFECT.FINAL_RICOCHET): b'armorRicochet', 
       (VEHICLE_HIT_EFFECT.ARMOR_NOT_PIERCED): b'armorResisted', 
       (VEHICLE_HIT_EFFECT.ARMOR_PIERCED_NO_DAMAGE): b'armorResisted', 
       (VEHICLE_HIT_EFFECT.ARMOR_PIERCED): b'armorHit', 
       (VEHICLE_HIT_EFFECT.CRITICAL_HIT): b'armorCriticalHit', 
       (VEHICLE_HIT_EFFECT.ARMOR_PIERCED_DEVICE_DAMAGED): b'armorCriticalHit'}
    ...
    def decodeSegment(segment, collisionComponent, maxComponentIdx=TankPartIndexes.ALL[-1], vehicleDesc=None):
        if collisionComponent is None:
            ...
            return (
             compIdx, int(segment & 255), segStart - offset, segEnd + offset)
