...
if typing.TYPE_CHECKING:
    ...
    TYPE_VEH_HIT_POINT = TypeVar(b'TYPE_VEH_HIT_POINT', bound=PyFixedDictDataInstance)
DUMMY_NODE_PREFIX = b'DM'
MAX_FALLBACK_CHECK_DISTANCE = 10000.0
HitEffectMapping = namedtuple(b'HitEffectMapping', (b'componentName', b'hitTester'))

class DamageFromShotDecoder(object):
    ShotPoint = namedtuple(b'ShotPoint', (b'componentName', b'componentIdx', b'matrix', b'hitEffectCode', b'hitEffectGroup', b'isDynCollision', b'hitType', b'shellType', b'caliber', b'normal'))
    ...
    def encodeHitPoint(cls, hitPoint):
        return hitPoint[b'networkID'] << cls._ENCODED_SEGMENT_BITS | hitPoint[b'segment']
    ...
    def collideHitPoint(cls, compIdx, startPoint, endPoint, collisionComponent):
        ...
        if distance < 0.0:
            LOG_DEBUG_DEV(b'No hit collision found')
            return None
        else:
            minDist = distance
    ...
    def parseHitPoint(cls, hitPoint, collisionComponent):
        networkID = hitPoint[b'networkID']
        segment = hitPoint[b'segment']
        params = hitPoint[b'params']
    ...
    def getPartIndexByNetworkID(cls, spaceID, networkID):
        ...
        if not gameObject.valid:
            LOG_DEBUG_DEV((b"[DamageFromShotDecoder] Can't find game object for networkID {}").format(networkID))
            return
        else:
            ...
            LOG_DEBUG_DEV((b"[DamageFromShotDecoder] Can't find collision for networkID {}").format(networkID))
            return
    ...
    def parseDamageStickerHitPoint(cls, hitPoint, collisions, segLength=None):
        ...
        if parsedHitPoint is None:
            ...
            prefabEffIndex = HitParamsEncoder.getPrefabEffIndex(hitPoint[b'params'])
...
class RepaintParams(object):
    ...
    def getRepaintParams(vehicleDescr):
        tintGroups = vehicles.g_cache.customization(vehicleDescr.type.customizationNationID)[b'tintGroups']
        for i in tintGroups.keys():
            grp = tintGroups[i]
            repaintReplaceColor = Vector4(grp.x, grp.y, grp.z, 0.0) / 255.0

        refColor = vehicleDescr.type.repaintParameters[b'refColor'] / 255.0
        repaintReferenceGloss = vehicleDescr.type.repaintParameters[b'refGloss'] / 255.0
        repaintColorRangeScale = vehicleDescr.type.repaintParameters[b'refColorMult']
        repaintGlossRangeScale = vehicleDescr.type.repaintParameters[b'refGlossMult']
