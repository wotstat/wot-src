...
import BigWorld, BattleReplay, TriggersManager, WWISE, MusicControllerWWISE as MC
...
class _BCNameFormatter(player_format.PlayerFullNameFormatter):
    ...
    def _normalizePlayerName(name):
        msgid = (b'#bootcamp:{0}').format(name)
...
_BattleResults = namedtuple(b'_BattleResults', (
 b'type',
 b'typeStr'))
...
class BOOTCAMP_SOUND(object):
    NEW_UI_ELEMENT_SOUND = b'bc_new_ui_element_button'


class BOOTCAMP_UI_COMPONENTS(object):
    START_BATTLE_BUTTON = b'StartBattleButton'
    WELCOME_START_BATTLE_BUTTON = b'WelcomeStartBattleButton'
...
class Bootcamp(EventSystemEntity):
    ...
    def replayCallbackSubscribe(self):
        BattleReplay.g_replayCtrl.setDataCallback(b'bootcamp_setContext', self.replaySetContext)
        return
    ...
    def setContext(self, context):
        ...
        if context is None:
            ...
            if b'checkpoint' in self.__context:
                self.__checkpoint = self.__context[b'checkpoint']
            return
    ...
    def isLastLesson(self, lesson):
        return self.getContextIntParameter(b'lastLessonNum') == lesson
    ...
    def setBattleResults(self, arenaUniqueID, resultType, resultReason):
        ...
        self.__battleResults = _BattleResults(resultType, makeHtmlString(b'html_templates:bootcamp/battle_results', teamResult))
        return
    ...
    def start(self, lessonNum, isBattleLesson):
        LOG_DEBUG_DEV_BOOTCAMP(b'Starting bootcamp', lessonNum, isBattleLesson)
        ...
        self.__requestBootcampFinishFromBattle = False
        ctx = self.getContext()
        isRelogin = ctx[b'relogin']
        LOG_DEBUG_DEV_BOOTCAMP(b'IsRelogin', isRelogin)
        ...
        if not autoStartBattle:
            ...
            isNewbie = False
            if ctx[b'isNewbieSettings'] and not ctx[b'completed']:
                isNewbie = True
        ...
        self.__hangarSpace = ctx[b'hangarSpace']
        self.__hangarSpacePremium = ctx[b'hangarSpacePremium']
        self.__bonuses = ctx[b'bonuses']
        self.__version = ctx[b'version']
        showRewards = ctx[b'needAwarding']
        lastBattleLesson = self.getContextIntParameter(b'lastLessonNum') - 1
        if 0 <= lastBattleLesson < len(self.__bonuses[b'battle']):
            self.__bonuses[b'battle'][lastBattleLesson][b'showRewards'] = showRewards
        self.__nation = ctx[b'nation']
        self.__nationsData = ctx[b'nationsData']
        self.__nationsOrder = ctx.get(b'nations_order')
        self.__p[b'completed'] = ctx[b'completed']
        self.__p[b'needAwarding'] = ctx[b'needAwarding']
        self.__customizationData = ctx.get(b'customization', {})
        weave(self.__weaver)
        ...
        WWISE.activateRemapping(b'bootcamp')
    ...
    def stop(self, reason):
        ...
        WWISE.deactivateRemapping(b'bootcamp')
    ...
    def getBootcampOutfit(self, vehDescr):
        if vehDescr.name in self.__customizationData and self.isRunning():
            return Outfit(vehicleCD=vehDescr.makeCompactDescr(), component=getBootcampOutfit(vehDescr, self.__customizationData))
        else:
            return
    ...
    def onBattleLoaded(self, lessonId):
        ...
        LOG_DEBUG_DEV_BOOTCAMP(b'onBattleLoaded called')
        self.__currentState.deactivate()
    ...
    def onGarageLessonFinished(self, lessonId):
        LOG_DEBUG_DEV_BOOTCAMP(b'onGarageLessonFinished', lessonId)
        self.__account.base.completeBootcampLesson()
        lastLesson = self.getContextIntParameter(b'lastLessonNum')
        if self.__lessonId == lastLesson:
            LOG_DEBUG_DEV_BOOTCAMP(b'Finished last lesson', lessonId)
        else:
            self.enqueueBattleLesson()
    ...
    def onRequestBootcampFinish(self):
        LOG_DEBUG_DEV_BOOTCAMP(b'onRequestBootcampFinish')
        self.__account.base.requestBootcampQuit()
        return

    def finishBootcamp(self):
        LOG_DEBUG_DEV_BOOTCAMP(b'finishBootcamp', self.__currentState.id())
    ...
    def getInterludeVideoPageData(self, index):
        ...
        if self.__lessonId:
            ...
            if extraData[b'videos']:
                video = extraData[b'videos'][index]
                messageVO[b'messages'] = [video[b'messages']]
                messageVO[b'voiceovers'] = [voiceover for voiceover in video[b'voiceovers']]
                messageVO[b'exitEvent'] = g_bootcampEvents.onInterludeVideoFinished
        return messageVO
    ...
    def getInterludeVideoButtons(self):
        if self.__lessonId:
            extraData = self.getBattleResultsExtra(self.__lessonId - 1)
            if extraData[b'videos']:
                return [{b'image': (video[b'messages'][b'icon'])} for video in extraData[b'videos']]
        return []
    ...
    def getIntroPageData(self, isChoice=False, isInProgress=False):
        ...
        autoStart = parameters.get(b'introAutoStart', False)
        if BattleReplay.isPlaying():
            autoStart = True
        introPageData = {b'autoStart': autoStart, 
           b'lessonNumber': (self.__lessonId), 
           b'tutorialPages': (self.getBattleLoadingPages()), 
           b'showSkipOption': True, 
           b'isReferralEnabled': (self.isReferralEnabled()), 
           b'isChoice': isChoice}
    ...
    def previewNation(self, nationIndex):
        if nationIndex in self.__nationsData:
            ...
            vehicleFirstID = nationData[b'vehicle_first']
            vehicle = self.itemsCache.items.getVehicles()[vehicleFirstID]
            self.installModules(nationData, b'vehicle_first', vehicle.descriptor)
            ...
            LOG_ERROR_BOOTCAMP((b'Wrong nation index: {}').format(nationIndex))
        return

    @staticmethod
    def installModules(nationData, sectionName, vehicleDescr):
        if b'equip' in nationData and sectionName in nationData[b'equip']:
            equipIds = nationData[b'equip'][sectionName].split(b':')
            for equipId in equipIds:
                ...
                if vehicleDescr.mayInstallComponent(intEquipId):
                    ...
                    if ITEM_TYPE_NAMES[itemTypeIdx] == b'vehicleTurret':
                        vehicleDescr.installTurret(intEquipId, 0)
                    elif ITEM_TYPE_NAMES[itemTypeIdx] == b'vehicleGun':
                        turretDescr, _ = vehicleDescr.turrets[0]
                        if not [True for gun in turretDescr.guns if gun.id[1] == compID]:
                            LOG_ERROR_BOOTCAMP((b'Can not install gun {} for turret {}').format(intEquipId, turretDescr.compactDescr))
                            continue
                    vehicleDescr.installComponent(intEquipId)
                else:
                    LOG_ERROR_BOOTCAMP(b'Can not install module', intEquipId)
    ...
    def getPremiumType(self):
        if self.__bonuses is None:
            ...
            lastBattleField = b'last_battle_short' if self.__version == BootcampVersion.SHORT else b'last_battle'
            bonuses = self.__bonuses[lastBattleField][b'bonus']
    ...
    def getNationsOrder(self, *fields):
        ...
        realm = CURRENT_REALM if CURRENT_REALM in self.__nationsOrder else b'DEFAULT'
        realmConfig = self.__nationsOrder.get(realm, {})
        if userCountry in realmConfig:
            nationOrder = realmConfig.get(userCountry, {})
        else:
            nationOrder = realmConfig.get(b'DEFAULT', {})
    ...
    def isMarkerLittlePiercingEnabled(self):
        return self.__context.get(b'little_pierced', False)

    def getPredefinedPiercingPower(self):
        return self.__context.get(b'piercingPower', None)

    def isLittlePiercingDisabledModules(self, armor):
        return armor in (10, 15, 20)

    def checkLittlePiercingVehicle(self, entity):
        return entity.typeDescriptor.name == self.__context[b'little_pierced_vehicle']

    def isInGarageState(self):
        return self.__currentState.id() == STATE.IN_GARAGE

    def isResearchFreeLesson(self):
        return self.getLessonNum() >= self.getContextIntParameter(b'researchFreeLesson')

    def isEnableCriticalDamageIcon(self):
        return self.getLessonNum() >= self.getContextIntParameter(b'enableDamageIconLesson')

    def checkBigConsumablesIconsLesson(self):
        return self.__lessonId == self.getContextIntParameter(b'researchSecondVehicleLesson')

    def getBattleStatsLessonWin(self):
        return self.__context.get(b'battleStatsWin', [])

    def getBattleStatsLessonDefeat(self):
        return self.__context.get(b'battleStatsDefeat', [])
