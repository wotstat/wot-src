...
class SubtitlesBase(View):
    ...
    def playSound(self):
        self.stopSound()
        if self._content.get(b'voiceovers', False):
            self.__currSound = self._content[b'voiceovers'].pop(0)
            if self.__currSound.get(b'voiceover', False):
                self.soundManager.playSound(self.__currSound[b'voiceover'])
                if self.__currSound.get(b'subtitle', False) and AccountSettings.getSettings(SUBTITLES):
                    self._asShowSubtitle(self.__currSound[b'subtitle'])
    ...
    def stopSound(self):
        ...
        if self.__currSound.get(b'subtitle', False):
            self._asHideSubtitle()
            if self.__currSound.get(b'voiceover', False):
                self.soundManager.stopSound(self.__currSound[b'voiceover'])
        return

    def _update(self):
        self.__clearCallback()
        voiceover = self.__currSound.get(b'voiceover', b'')
