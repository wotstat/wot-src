...
class _PointcutHideMinimapNames(aop.Pointcut):
    ...
    def __init__(self):
        super(_PointcutHideMinimapNames, self).__init__(b'gui.Scaleform.daapi.view.battle.shared.minimap.common', b'SimplePlugin', b'_invoke', aspects=(
         _AspectHideMinimapNames,))
        return
...
class _PointcutLimitNotificationPopUpsCount(aop.Pointcut):
    ...
    def __init__(self):
        super(_PointcutLimitNotificationPopUpsCount, self).__init__(b'notification.NotificationPopUpViewer', b'NotificationPopUpViewer', b'_getSettings', aspects=(
         _AspectLimitNotificationPopUpsCount,))
        return
...
class _AspectDisableSettingsControls(aop.Aspect):
    ...
    def atCall(self, cd):
        LOG_DEBUG_DEV_BOOTCAMP(b'Disabling controls in settings window')
    ...
    def __disableControl(self, cd, controlPath):
        ...
        try:
            ...
            LOG_ERROR_BOOTCAMP(b'Error: No such page or control?', page, subpage, control)
...
class _AspectHideMinimapNames(aop.Aspect):
    ...
    def atCall(self, cd):
        name = cd.findArg(1, b'')
        if name == b'setVehicleInfo':
            return cd.changeArgs((4, b'arg', b''))
        return
