import BattleReplay, BigWorld, Settings, SoundGroups
...
_INTRO_VIDEO_PATH = b'videos/_tutorialInitial.usm'
_INTRO_VIDEO_LOOP_PATH = b'videos/_tutorialInitialLoop.usm'
_INTRO_VIDEO_MUSIC_START_EVENT = b'bc_music_video_intro_start'
_INTRO_VIDEO_MUSIC_STOP_EVENT = b'bc_music_video_intro_stop'
_INTRO_VIDEO_MUSIC_PAUSE_EVENT = b'bc_music_video_intro_pause'
_INTRO_VIDEO_MUSIC_RESUME_EVENT = b'bc_music_video_intro_resume'
_INTRO_VIDEO_MUSIC_TO_LOOP_EVENT = b'bc_music_transition_to_loop'
...
class StateBattlePreparing(AbstractState):
    ...
    def deactivate(self):
        LOG_DEBUG_DEV_BOOTCAMP(b'StateBattlePreparing.deactivate')
        g_playerEvents.onAvatarReady -= self.onAvatarReady
        g_playerEvents.onAvatarBecomePlayer -= self.onAvatarBecomePlayer
        g_bootcampEvents.onIntroVideoStop -= self.__onBCIntroVideoStop
        g_bootcampEvents.onBootcampGoNext -= self.__onBootcampGoNext
    ...
    def onAvatarBecomePlayer(self):
        LOG_DEBUG_DEV_BOOTCAMP(b'onAvatarBecomePlayer called')
        self.onBecomePlayer()
        return

    def onAvatarReady(self):
        LOG_DEBUG_DEV_BOOTCAMP(b'onAvatarReady called')
    ...
    def onArenaLoadCompleted(self):
        ...
        g_eventBus.handleEvent(events.LoadViewEvent(SFViewLoadParams(VIEW_ALIAS.BOOTCAMP_INTRO_FADEOUT), ctx={b'duration': 1000}), EVENT_BUS_SCOPE.BATTLE)
        return

    def onBecomePlayer(self):
        g_eventBus.handleEvent(events.LoadViewEvent(SFViewLoadParams(VIEW_ALIAS.BOOTCAMP_BATTLE_HIGHLIGHTS), ctx={b'descriptors': (g_bootcampHintsConfig.getItems())}), EVENT_BUS_SCOPE.BATTLE)
        return

    def getIntroVideoData(self):
        introVideoData = {b'showSkipOption': False}
        if self.isVideoPlayingLesson:
            introVideoData.update({b'video': _INTRO_VIDEO_PATH, 
               b'backgroundVideo': _INTRO_VIDEO_LOOP_PATH, 
               b'backgroundMusicStartEvent': _INTRO_VIDEO_MUSIC_START_EVENT, 
               b'backgroundMusicStopEvent': _INTRO_VIDEO_MUSIC_STOP_EVENT, 
               b'backgroundMusicPauseEvent': _INTRO_VIDEO_MUSIC_PAUSE_EVENT, 
               b'backgroundMusicResumeEvent': _INTRO_VIDEO_MUSIC_RESUME_EVENT, 
               b'backgroundMusicToLoopEvent': _INTRO_VIDEO_MUSIC_TO_LOOP_EVENT, 
               b'bufferTime': (Settings.g_instance.userPrefs.readFloat(Settings.VIDEO_BUFFERING_TIME, _DEFAULT_VIDEO_BUFFERING_TIME))})
        return introVideoData

    def onSpaceLoaded(self):
        LOG_DEBUG_DEV_BOOTCAMP(b'onSpaceLoaded called')
    ...
    def onVehicleOnAppearanceReady(self, vehicle):
        LOG_DEBUG_DEV_BOOTCAMP(b'onVehicleOnAppearanceReady called')
    ...
    def onAvatarOnEnterWorld(self, prereqs):
        LOG_DEBUG_DEV_BOOTCAMP(b'onAvatarOnEnterWorld called')
        if not self.isVideoPlayingLesson or self.__isIntroVideoFinished:
            return False
    ...
    def __onBootcampGoNext(self):
        LOG_DEBUG_DEV_BOOTCAMP(b'__onBootcampGoNext')
        self.__cancelHintSoundCallback()
    ...
    def __onBCIntroVideoStop(self):
        ...
        if not noSounds and self.isVideoPlayingLesson:
            ...
            SoundGroups.g_instance.playSound2D(b'vo_bc_welcome')
        LOG_DEBUG_DEV_BOOTCAMP(b'__onBCIntroVideoStop called')
    ...
    def __playHintSound(self):
        self.__cancelHintSoundCallback()
        SoundGroups.g_instance.playSound2D(b'bc_loading_tips')
        return
