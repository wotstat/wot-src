...
class _DirectionIndicatorCtrl(_IMarker):
    ...
    def clear(self):
        LOG_DEBUG_DEV_BOOTCAMP(b'_DirectionIndicatorCtrl.clear', hex(id(self)))
        if self._indicator is not None:
            self._indicator.remove()
            self._indicator = None
        return
...
class _AimMarker(object):
    ...
    def attachGUI(self, markers2D, minimap):
        ...
        if indicator is not None:
            ...
            LOG_ERROR_BOOTCAMP(b'Directional indicator not found', self.__triggerID)
        return
...
class _StaticWorldMarker2D(_IMarker):
    ...
    def addSetupStaticObject(self, markers2D, objectID, distance):
        offset = self.__initData.get(b'offset', Math.Vector3(0, 0, 0))
        if markers2D.addStaticObject(objectID, Math.Vector3(self.__initPosition[:]) + offset):
            markers2D.setupStaticObject(objectID, self.__initData.get(b'shape', b'arrow'), self.__initData.get(b'min-distance', 0), self.__initData.get(b'max-distance', 0), distance, self.__initData.get(b'color', b'yellow'))
            return True
        return False
...
class _StaticMinimapMarker2D(_IMarker):
    ...
    def clear(self):
        LOG_DEBUG_DEV_BOOTCAMP(b'_StaticMinimapMarker2D.clear', self.__markerID)
        minimap = self.__minimap()
        if minimap is not None and self.__markerID:
            minimap.delTarget(self.__markerID)
        self.__markerID = b''
...
class _StaticObjectMarker3D(_IMarker):
    ...
    def __init__(self, data, position):
        ...
        self.__path = data.get(b'path')
        offset = data.get(b'offset', Math.Vector3(0, 0, 0))
        self.__model = None
        self.__isMarkerVisible = True
        self.__action = data.get(b'action')
    ...
    def addMarkerModel(self):
        ...
        if self.__action:
            try:
                ...
                LOG_ERROR_BOOTCAMP(b'Action not found', self.__path, self.__action)
            except EnvironmentError:
                LOG_ERROR_BOOTCAMP(b'Player not on the world')
    ...
    def clear(self):
        LOG_DEBUG_DEV_BOOTCAMP(b'_StaticObjectMarker3D.clear', self.__model)
    ...
    def __onModelLoaded(self, position, resourceRefs):
        ...
        if self.__path not in resourceRefs.failedIDs:
            ...
            LOG_ERROR_BOOTCAMP(b'Model not found', self.__path)
        return
...
class BootcampMarkersManager(object):
    ...
    SHOW_SOUND_ID = b'bc_arrow_appears'
    HIDE_SOUND_ID = b'bc_arrow_disappears'
    ...
    def init(self, entitiesParams, bootcampGui):
        LOG_DEBUG_DEV_BOOTCAMP(b'BootcampMarkers.init')
    ...
    def clear(self):
        LOG_DEBUG_DEV_BOOTCAMP(b'BootcampMarkers.clear')
        g_bootcampEvents.onUIStateChanged -= self._onUIStateChanged
        InputHandler.g_instance.onKeyDown -= self._handleKeyEvent
        if not BattleReplay.g_replayCtrl.isPlaying:
    ...
    def onTriggerActivated(self, params):
        if params[b'type'] == TriggersManager.TRIGGER_TYPE.SNIPER_MODE:
    ...
    def addMarker(self, params):
        ...
        for markerParams in self.__markersParams:
            if params[b'name'] == markerParams[b'name']:
                break
        else:
                ...
                if name == markerParams[b'name']:
                    if name not in self.__markers:
                        ...
                        if self.__markerSoundShow is not None:
                            self.__markerSoundShow.stop()
                            self.__markerSoundShow = None
                        self.__markerSoundShow = SoundGroups.g_instance.WWgetSoundPos(self.SHOW_SOUND_ID, b'marker_sound_' + name + b'_' + self.SHOW_SOUND_ID, self.__markers[name].position)
                        if self.__markerSoundShow is not None:
                            self.__markerSoundShow.play()
                        else:
                            LOG_DEBUG_DEV_BOOTCAMP(b'Marker sound is None')
                    break
    ...
    def __createMarker(self, markerParams):
        entityID = markerParams[b'style']
        data = self.__entitiesParams.getEntity(entityID)
        typeID = 1
        self.__triggerID += 1
        position = markerParams[b'position']
        if position is None:
            LOG_ERROR_BOOTCAMP(b'Can not determine position of object', self.__triggerID)
            return
        else:
            indicatorCtrl = None
            if data.isIndicatorCreate():
                worldData = data.getWorldData()
                offset = worldData.get(b'offset', Math.Vector3(0, 0, 0))
                indicatorCtrl = _DirectionIndicatorCtrl((b'yellow', b'yellow'), Math.Vector3(position[:]) + offset)
            ...
            self.__markers[markerParams[b'name']] = areaMarker
            return
