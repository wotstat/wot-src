...
class BattleAssistant(BaseAssistant):
    ...
    HIGHLIGHTED_GUI_DICT = {b'Minimap': (BATTLE_VIEW_ALIASES.MINIMAP), 
       b'MinimapAppear': (BATTLE_VIEW_ALIASES.MINIMAP), 
       b'FragCorrelationBar': (BATTLE_VIEW_ALIASES.FRAG_CORRELATION_BAR), 
       b'ConsumableSlot4': (BATTLE_VIEW_ALIASES.CONSUMABLES_PANEL), 
       b'ConsumableSlot5': (BATTLE_VIEW_ALIASES.CONSUMABLES_PANEL), 
       b'ConsumableSlot6': (BATTLE_VIEW_ALIASES.CONSUMABLES_PANEL), 
       b'ConsumablesAppear': (BATTLE_VIEW_ALIASES.CONSUMABLES_PANEL)}
    ...
    def _onUIStateChanged(self, state):
        if state == UI_STATE.START:
            ...
            if prebattle.has_key(b'visible_hints'):
                ...
                self.__idClosePrebattleTimer = BigWorld.callback(prebattle[b'timeout'], self.__onClosePrebattle)
