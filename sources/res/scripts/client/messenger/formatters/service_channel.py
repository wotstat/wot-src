...
class ServerRebootFormatter(ServiceChannelFormatter):
    ...
    def format(self, message, *args):
        if message.data:
            ...
            return [
             MessageData(formatted, self._getGuiSettings(message, u'serverReboot'))]
        else:
            return [
...
class ServerRebootCancelledFormatter(ServiceChannelFormatter):
    ...
    def format(self, message, *args):
        if message.data:
            ...
            return [
             MessageData(formatted, self._getGuiSettings(message, u'serverRebootCancelled'))]
        else:
            return [
             MessageData(None, None)]
...
class FormatSpecialReward(object):
    ...
    def getString(self, message):
        ...
        if not formattedItems:
            ...
            return g_settings.msgTemplates.format(u'specialReward', ctx={u'specialRewardItems': formattedItems})
    ...
    def __formattedItems(self, message):
        ...
        if not itemsNames:
            ...
            return g_settings.htmlTemplates.format(u'specialRewardItems', ctx={u'names': ((u'<br/>').join(itemsNames))})
...
class BattleResultsFormatter(WaitItemsSyncFormatter):
    ...
    def _prepareFormatData(self, message):
        ...
        if platformCurrencies:
            ctx[u'platformCurrencyStr'] = u'<br/>' + (u'<br/>').join(g_settings.htmlTemplates.format(u'platformCurrency', {u'msg': (backport.text(R.strings.messenger.platformCurrencyMsg.received.dyn(currency)())), u'count': (backport.getIntegralFormat(countDict.get(u'count', 0)))}) for currency, countDict in iteritems(platformCurrencies))
        else:
    ...
    def __makeMapsTrainingMsgCtx(self, battleResults, ctx):
        vehTypeCompDescr = next(iter(battleResults[u'playerVehicles']))
    ...
    def __makeQuestsAchieve(self, message):
        ...
        if fmtMsg is not None:
            return g_settings.htmlTemplates.format(u'battleQuests', {u'achieves': fmtMsg})
        else:
            return u''

    def __makeVehicleLockString(self, vehicleNames, battleResults):
        locks = []
        for vehIntCD, battleResult in iteritems(battleResults.get(u'playerVehicles', {})):
    ...
    def __makeXpExString(self, xp, battleResKey, xpPenalty, battleResults):
        ...
        if exStrings:
            return (u' ({0:s})').format((u'; ').join(exStrings))
        return u''
    ...
    def __makeAchievementsAndBadgesStrings(self, battleResults):
        popUpRecords = []
        badges = []
        for vehBattleResults in itervalues(battleResults.get(u'playerVehicles', {})):
    ...
    def __makeBattlePassProgressionString(self, guiType, battleResults):
        battlePassString = u''
        value = sum(points for points in itervalues(battleResults.get(u'battlePassPoints', {}).get(u'vehicles', {})))
        ...
        if not battlePassString:
            return u''
        return g_settings.htmlTemplates.format(u'battlePass', ctx={u'battlePassProgression': battlePassString})
    ...
    def __makeCommendationsString(self, battleResults):
        ...
        if not commendationsReceived:
            return u''
        return g_settings.htmlTemplates.format(u'commendations', ctx={u'commendationsReceived': commendationsReceived})

    def __makePiggyBankString(self, credits_):
        if not credits_:
            return u''
        return g_settings.htmlTemplates.format(u'piggyBank', ctx={u'credits': (self.__makeCurrencyString(Currency.CREDITS, credits_))})
...
class CurrencyUpdateFormatter(ServiceChannelFormatter):
    ...
    def __getCurrencyString(self, currencyCode, amountDelta):
        if self.__ifPlatformCurrency(currencyCode):
            return backport.text(R.strings.messenger.platformCurrencyMsg.dyn(u'debited' if amountDelta < 0 else u'received').dyn(currencyCode)())
        return backport.text(R.strings.messenger.serviceChannelMessages.currencyUpdate.dyn(u'debited' if amountDelta < 0 else u'received').dyn(currencyCode)())
...
class GiftReceivedFormatter(ServiceChannelFormatter):
    __handlers = {u'money': (
                u'_GiftReceivedFormatter__formatMoneyGiftMsg',
                {1: u'creditsReceivedAsGift', 
...
class InvoiceReceivedFormatter(WaitItemsSyncFormatter):
        ...
        for vehicleDict in vehicles:
            for vehCompDescr, vehData in iteritems(vehicleDict):
                ...
                if u'rentCompensation' in vehData:
                    ...
                    ctx = {currency: (formatter(comp.get(currency))), u'vehicleName': vehicleName}
    ...
    def _formatData(self, assetType, data):
        ...
        if not operations:
            ...
            return g_settings.msgTemplates.format(self._getMessageTemplateKey(assetType), ctx={u'at': (self._getOperationTimeString(data)), 
    ...
    def __getPrestigePointsString(cls, prestigePoints):
        ...
        if operationType not in (u'add', u'set'):
            ...
            for vehIntCD, prestige in iteritems(vehicles):
                ...
                elif prestige.get(u'newPrestigeLevel', 0) < prestige.get(u'oldPrestigeLevel', 0):
                    return result
    ...
    def __getLootBoxString(self, data):
        ...
        if boxesList:
            return g_settings.htmlTemplates.format(u'lootBoxesInvoiceReceived', {u'boxes': ((u', ').join(boxesList))})
        else:
            return u''
...
class AdminMessageFormatter(ServiceChannelFormatter):
    ...
    def format(self, message, *args):
        ...
        if data:
            ...
            return [
             MessageData(formatted, self._getGuiSettings(message, u'adminMessage'))]
        else:
            return [
             MessageData(None, None)]
...
class PrebattleArenaFinishFormatter(PrebattleFormatter):
    ...
    def format(self, message, *args):
        ...
        if None in [prbType, winner, team, wins, finishReason]:
            ...
            return [
             MessageData(formatted, self._getGuiSettings(message, u'prebattleArenaFinish'))]
    ...
    def format(self, message, *args):
        ...
        if None in [prbType, team, wins, kickReason]:
            ...
            formatted = g_settings.msgTemplates.format(u'prebattleDestruction', ctx={u'desc': (self._makeDescriptionString(data, showBattlesCount=False)), 
               ...
               u'icon': (self._getIconId(prbType))})
            return [
             MessageData(formatted, self._getGuiSettings(message, u'prebattleDestruction'))]
...
class VehCamouflageTimedOutFormatter(ServiceChannelFormatter):
    ...
    def format(self, message, *args):
        ...
        return [
         MessageData(formatted, self._getGuiSettings(message, u'vehCamouflageTimedOut'))]
...
class VehEmblemTimedOutFormatter(ServiceChannelFormatter):
    ...
    def format(self, message, *args):
        ...
        return [
         MessageData(formatted, self._getGuiSettings(message, u'vehEmblemTimedOut'))]
...
class VehInscriptionTimedOutFormatter(ServiceChannelFormatter):
    ...
    def format(self, message, *args):
        ...
        return [
         MessageData(formatted, self._getGuiSettings(message, u'vehInscriptionTimedOut'))]
...
class ConverterFormatter(ServiceChannelFormatter):
    ...
    def __getBlueprintsMessageText(self, blueprints):
        ...
        if text:
            return g_settings.msgTemplates.format(self.__CONVERTER_BLUEPRINTS_TEMPLATE, {u'text': ((u'<br/>').join(text))})
        else:
            return
...
class PremiumAccountExpiryFormatter(ClientSysMessageFormatter):
    ...
    def format(self, data, *args):
        ...
        return [
         MessageData(formatted, self._getGuiSettings(args, u'durationOfPremiumAccountExpires'))]
...
class QuestAchievesFormatter(object):
    ...
    def getFormattedAchieves(cls, data, asBattleFormatter, processCustomizations=True, processTokens=True):
        ...
        entitlementsList = [(eID, eData.get(u'count', 0)) for eID, eData in iteritems(data.get(u'entitlements', {}))]
    ...
    def format(self, message, *args):
        ...
        if msgType == NC_MESSAGE_TYPE.POLL:
            ...
            if not self.__fetchPollData(data, settings):
                return []
        formatted = g_settings.msgTemplates.format(templateKey, ctx={u'topic': topic, u'body': body})
        return [
...
class ClanMessageFormatter(ServiceChannelFormatter):
    ...
    def format(self, message, *args):
        ...
        if data and u'event' in data:
            ...
            formatted = g_settings.msgTemplates.format(templateKey, ctx={u'message': message, u'fullClanName': fullName})
...
class LootBoxAchievesFormatter(QuestAchievesFormatter):
    ...
    def _processTokens(cls, data):
        result = []
        for token in data.get(u'tokens', {}):
...
class LootBoxSystemAchievesFormatter(QuestAchievesFormatter):
    ...
    def _processTokens(cls, data):
        result = []
        for token in list(data.get(u'tokens', {})):
    ...
    def __getLootBoxString(cls, token, tokenData):
        ...
        if lootBox is not None:
            ...
            count = tokenData.get(u'count', 0)
            return g_settings.htmlTemplates.format(u'systemBoxesReceived', {u'text': name, u'count': count})
        else:
            return u''
...
class BattlePassQuestAchievesFormatter(QuestAchievesFormatter):
        ...
        for tokenName, tokenData in iteritems(tokens):
            if constants.LOOTBOX_TOKEN_PREFIX in tokenName:
                ...
                if lootBox is not None:
                    name = lootBox.getUserName()
                    count = tokenData.get(u'count', 0)
                    boxesList.append(g_settings.htmlTemplates.format(u'systemBoxesReceived', {u'text': name, u'count': count}))
    ...
    def __processRewardChoiceTokens(cls, tokens):
        ...
        for rewardType, count in iteritems(tokens):
            result.append(g_settings.htmlTemplates.format(rewardSelectTemplate, {u'text': (backport.text(rChosenBonuses.dyn(rewardType)())), u'count': count}))
...
class CollectionsFormatter(QuestAchievesFormatter):
    ...
    def __formatItems(cls, items):
        if items:
            return cls._SEPARATOR.join((u'{} {}').format(cls.__BULLET, itemName) for itemName in items)
        return u''
...
class WinbackQuestAchievesFormatter(QuestAchievesFormatter):
    ...
    def getSelectableRewards(cls, awardsDict):
        if u'tokens' not in awardsDict:
            return []
        return [SelectableBonus({tokenId: tokenData}) for tokenId, tokenData in iteritems(awardsDict[u'tokens']) if cls.__winbackController.isWinbackOfferToken(tokenId)]
...
class TelecomStatusFormatter(WaitItemsSyncFormatter):
    ...
    def __getMessageContext(self, data):
        ...
        msgctx = {u'vehicles': (self.__getVehicleUserNames(vehTypeDescrs)), u'provider': providerLocName}
        ctx = {}
...
class TelecomReceivedInvoiceFormatter(InvoiceReceivedFormatter):
            ...
            for tman in tankmens:
                if isinstance(tman, bytes):
                    ...
                    if u'brotherhood' in tankmanDecsr.freeSkills:
                        hasBrotherhood = True
                        break
                elif u'brotherhood' in tman.get(u'freeSkills', []):
                    hasBrotherhood = True
                    break
    ...
    def _getMessageTemplateKey(self, data):
        return u'telecomVehicleReceived'
    ...
    def _formatData(self, assetType, data):
        ...
        if not vehicleNames:
            ...
            return g_settings.msgTemplates.format(self._getMessageTemplateKey(data), ctx=self._getMessageContext(data, vehicleNames), data={u'timestamp': (time.time())})
...
class TelecomRemovedInvoiceFormatter(TelecomReceivedInvoiceFormatter):
    ...
    def _getMessageTemplateKey(self, data):
        return u'telecomVehicleRemoved'
    ...
    def _getMessageContext(self, data, vehicleNames):
        ...
        msgctx = {u'vehicles': ((u', ').join(vehicleNames)), u'datetime': (self._getOperationTimeString(data)), 
...
class PiggyBankSmashedFormatter(ServiceChannelFormatter):
    ...
    def format(self, message, *args):
        ...
        if gold and credits_:
            ctx = {u'credits': (backport.getIntegralFormat(credits_)), u'gold': (backport.getGoldFormat(gold))}
...
class EnhancementsWipedOnVehiclesFormatter(ServiceChannelFormatter):
    ...
    def format(self, message, *args):
        if message.data:
            ...
            formatted = g_settings.msgTemplates.format(self.__TEMPLATE, {u'text': text, u'vehicleNames': vehNames})
            guiSettings = self._getGuiSettings(message, self.__TEMPLATE, priorityLevel=NotificationPriorityLevel.MEDIUM)
            return [
             MessageData(formatted, guiSettings)]
        else:
...
class BattlePassRewardFormatter(WaitItemsSyncFormatter):
        ...
        if message.data and isSynced:
            ...
            if u'reward' in data and u'ctx' in data:
                ...
                formatted = g_settings.msgTemplates.format(template, ctx={u'header': header, 
                   u'description': description, u'text': formattedBonuses, u'additionalText': additionalText}, data={u'savedData': savedData})
    ...
    def __makeCurrencyString(self, currency, amount):
        if amount:
            return g_settings.htmlTemplates.format(self.__CURRENCY_TEMPLATE_KEY, {u'currency': (backport.text(self.__MESSAGES.battlePassReward.buy.dyn(currency)())), u'amount': (getBWFormatter(currency)(amount))})
        return u''
    ...
    def __makeStarterPackText(self, starterPack):
        if starterPack:
            ...
            return (u'').join([breakLine, text, breakLine, formattedBonuses, breakLine])
        return u''
    ...
    def __makeCollectionMessage(self, entitlements, message):
        ...
        formatted = g_settings.msgTemplates.format(self.__COLLECTION_ITEMS_TEMPLATE, ctx={u'title': title, u'text': text}, data={u'savedData': {u'collectionId': collectionID}})
...
class BattlePassBoughtFormatter(WaitItemsSyncFormatter):
    ...
    def __makeAdditionalText(self, chapterID):
        ...
        priceText = g_settings.htmlTemplates.format(u'battlePassCurrency', {u'currency': (backport.text(self.__MESSAGES.battlePassReward.buy.dyn(currency)())), u'amount': (getBWFormatter(currency)(amount))}) if amount else u''
        breakLine = backport.text(self.__MESSAGES.battlePassReward.breakLine())
    ...
    def format(self, message, callback=None):
        ...
        if message.data and isSynced:
            ...
            if text:
                ...
                formatted = g_settings.msgTemplates.format(self.__template, {u'header': header, u'text': text})
                resultMessage = MessageData(formatted, self._getGuiSettings(message, self.__template))
        callback([resultMessage])
        return
    ...
    def format(self, message, callback=None):
        ...
        if message.data and isSynced:
            ...
            if rewards is not None:
                ...
                if formattedBonuses is None:
                    formattedBonuses = u''
                formatted = g_settings.msgTemplates.format(self.__template, {u'description': description, u'text': formattedBonuses})
                resultMessage = MessageData(formatted, self._getGuiSettings(message, self.__template))
        callback([resultMessage])
        return
...
class EpicLevelUpFormatter(WaitItemsSyncFormatter):
    ...
    def format(self, message, callback=None):
        ...
        if message.data and isSynced:
            ...
            formatted = g_settings.msgTemplates.format(self.__template, {u'title': title, u'body': body, u'awards': awards, u'achieves': achieves}, data={u'icon': icon, u'savedData': sysMsgExtraData})
...
class EpicQuestAchievesFormatter(QuestAchievesFormatter):
    ...
    def __processGoodiesResult(cls, data, result):
        ...
        for goodieID, ginfo in iteritems(data.get(u'goodies', {})):
    ...
    def __processBattlePassPoints(cls, data, result):
        value = sum(points for points in itervalues(data.get(u'battlePassPoints', {}).get(u'vehicles', {})))
    ...
    def __processTokens(cls, data, result):
        from gui.battle_pass.battle_pass_helpers import getOfferTokenByGift
        rewardChoiceTokens = {}
        for token, tokenData in iteritems(data.get(u'tokens', {})):
        ...
        for rewardType, count in iteritems(rewardChoiceTokens):
            message = g_settings.htmlTemplates.format(cls.__rewardTemplate, {u'text': (backport.text(rBonuses.dyn(rewardType)())), u'count': count})
            result.append(message)
...
class EpicSeasonEndFormatter(WaitItemsSyncFormatter):
    ...
    def format(self, message, callback=None):
        ...
        if message.data and isSynced:
            ...
            if rewards is not None:
                ...
                formatted = g_settings.msgTemplates.format(self.__template, {u'title': title, u'description': description, u'text': ((u'<br>').join(text))})
                resultMessage = MessageData(formatted, self._getGuiSettings(message, self.__template))
        callback([resultMessage])
        return
    ...
    def __formatRewardChoiceTokenString(self, itemCD, count, itemTypeID):
        ...
        return g_settings.htmlTemplates.format(self.__rewardTemplate, {u'text': text, u'count': count})
...
class CustomizationProgressionChangedFormatter(WaitItemsSyncFormatter):
    REQUIRED_KEYS = {
     u'custType', u'id', u'prevLevel', u'actualLevel'}
...
class DogTagComponentUnlockFormatter(DogTagFormatter):
    ...
    def format(self, message, *args):
        ...
        ctx = {u'title': title, u'message': messageString}
...
class MapboxSurveyAvailableFormatter(ServiceChannelFormatter):
    ...
    def format(self, message, *args):
        if not message:
            ...
            return [
             MessageData(formatted, self._getGuiSettings(message, self.__TEMPLATE, messageSubtype=message.get(u'msgType')))]
    ...
    def format(self, message, *args):
        ...
        if rewards is None:
            ...
            formatted = g_settings.msgTemplates.format(self.__TEMPLATE, {u'header': (backport.text(self.__STR_PATH.congrats.title())), 
               u'text': ((u'<br>').join(textItems))}, data={u'savedData': {u'rewards': rewards, u'battles': battles}})
            return [
             MessageData(formatted, self._getGuiSettings(message, self.__TEMPLATE, messageSubtype=message.get(u'msgType')))]
    ...
    def format(self, message, *args):
        if not message:
            ...
            formatted = g_settings.msgTemplates.format(self.__TEMPLATE, {u'header': (backport.text(messageData[u'header'])), 
               ...
               u'savedData': data})
            return [
             MessageData(formatted, self._getGuiSettings(message, self.__TEMPLATE, priorityLevel=messageData[u'priority']))]
    ...
    def format(self, message, *args):
        if not message:
            ...
            textItems = [backport.text(R.strings.recertification_form.serviceChannelMessages.Reset.body()),
             backport.text(R.strings.recertification_form.serviceChannelMessages.ResetUsed.body(), currency=backport.text(messageData[u'currency']), count=getNiceNumberFormat(data.get(u'count')))]
            formatted = g_settings.msgTemplates.format(self.__TEMPLATE, {u'header': (backport.text(messageData[u'header'])), 
...
class Customization2DProgressionChangedFormatter(WaitItemsSyncFormatter):
    ...
    def format(self, message, callback, *args):
        ...
        if isSynced and data and set(data.keys()) == self.REQUIRED_KEYS:
            ...
            formatted = g_settings.msgTemplates.format(u'Customization2DProgressionMessage', ctx={u'title': title, u'itemsList': itemsStr}, data={u'savedData': {u'toStyle': True, u'styleID': (data[u'id'])}})
            result = [
    ...
    def format(self, message, *args):
        ...
        if not data:
            ...
            if not header and not text:
                return [MessageData(None, None)]
            formatted = g_settings.msgTemplates.format(self.__TEMPLATE, ctx={u'text': text, u'header': header})
            return [
...
class IntegratedAuctionLostRateFormatter(ServiceChannelFormatter):
    ...
    def format(self, message, *args):
        ...
        if u'currency' not in messageData or u'amount' not in messageData:
            ...
            formatted = g_settings.msgTemplates.format(self.__TEMPLATE, ctx={u'header': header, u'text': text})
            return [
             MessageData(formatted, self._getGuiSettings(message, self.__TEMPLATE))]
...
class CollectionsItemsFormatter(ServiceChannelFormatter):
    ...
    def format(self, message, *args):
        ...
        if not (data and self.__collections.isEnabled()):
            ...
            formatted = g_settings.msgTemplates.format(self.__TEMPLATE, ctx={u'title': title, u'text': text}, data={u'savedData': {u'collectionId': collectionID}})
            return [
             MessageData(formatted, self._getGuiSettings(message, self.__TEMPLATE))]
...
class CollectionsRewardFormatter(ServiceChannelFormatter):
    ...
    def __makeMessageData(self, collectionID, feature, season, isFinal, message):
        ...
        formatted = g_settings.msgTemplates.format(template, ctx={u'title': title, u'text': text}, data={u'savedData': {u'collectionId': collectionID, u'bonuses': [message.data[u'reward']]}})
    ...
    def __makeFinalMessageData(self, collectionID, feature, season, message):
        ...
        formatted = g_settings.msgTemplates.format(self.__ITEMS_TEMPLATE, ctx={u'title': (backport.text(self.__MESSAGES.title.congratulation())), u'text': text}, data={u'savedData': {u'collectionId': collectionID}})
        ...
        if type == Achievements20SystemMessages.RATING_UPGRADE:
            ...
            if type == Achievements20SystemMessages.EDITING_AVAILABLE:
                return {u'title': (backport.text(self.__ACHIEVEMENTS_MESSAGES.editingEnabled.title())), 
                   u'button': (backport.text(self.__ACHIEVEMENTS_MESSAGES.editingEnabled.button()))}
            return
    ...
    def __getTemplate(self, type):
        if type in (Achievements20SystemMessages.RATING_UPGRADE, Achievements20SystemMessages.RATING_COMPLETE):
            ...
            if type == Achievements20SystemMessages.FIRST_ENTRY_WITHOUT_WTR:
                return u'achievementFirstEntryWithOutWTR'
            return
...
class PrestigeFormatter(ServiceChannelFormatter):
    ...
    def __makeMessageData(self, message, vehCD, lvl):
        ...
        formatted = g_settings.msgTemplates.format(self.__TEMPLATE, ctx={u'title': title, u'text': text}, data={u'savedData': {u'vehCD': vehCD}, u'linkageData': {u'type': (gradeType.value), u'grade': grade, u'lvl': lvl}})
...
class PostProgressionUnlockedFormatter(ServiceChannelFormatter):
    ...
    def __makeMessageData(self, message):
        formatted = g_settings.msgTemplates.format(self.__TEMPLATE, ctx={u'header': (backport.text(self.__MESSAGES_TEXT.header())), u'text': (backport.text(self.__MESSAGES_TEXT.text()))}, data={u'linkageData': {u'canClaimReward': False}})
        return MessageData(formatted, self._getGuiSettings(message, self.__TEMPLATE, messageType=message.type))
...
class SkillsCrewBoostersConversionFormatter(WaitItemsSyncFormatter, ServiceChannelFormatter):
    ...
    def __makeMessageData(self, message):
        ...
        for equipments in equipmentsReplaced:
            ...
            itemsData.append({u'oldName': (backport.text(R.strings.artefacts.dyn(oldEq.name).name())), u'newName': (backport.text(R.strings.artefacts.dyn(newEq.name).name())), 
...
class PrestigeMilestoneRewardFormatter(ServiceChannelFormatter):
    ...
    def format(self, message, *args):
        ...
        formatted = g_settings.msgTemplates.format(self.__TEMPLATE, ctx={u'text': text, u'title': title})
        return [
