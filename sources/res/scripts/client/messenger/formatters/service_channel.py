...
class BattleResultsFormatter(WaitItemsSyncFormatter):
    ...
    def format(self, message, callback):
        ...
        if message.data and isSynced:
            ...
            if arenaCreateTime and arenaType:
                ctx = {u'arenaName': (i18n.makeString(arenaType.name)), u'vehicleNames': u'N/A', 
                   u'xp': u'0', 
    ...
    def _prepareFormatData(self, message):
        ...
        if platformCurrencies:
            ctx[u'platformCurrencyStr'] = u'<br/>' + (u'<br/>').join(g_settings.htmlTemplates.format(u'platformCurrency', {u'msg': (backport.text(R.strings.messenger.platformCurrencyMsg.received.dyn(currency)())), u'count': (backport.getIntegralFormat(countDict.get(u'count', 0)))}) for currency, countDict in platformCurrencies.iteritems())
        else:
    ...
    def __makeGoldBankGainString(self, gold_):
        if not gold_:
            return u''
        return g_settings.htmlTemplates.format(u'goldBankGain', ctx={u'gold': (self.__makeCurrencyString(Currency.GOLD, gold_))})
...
class CurrencyUpdateFormatter(ServiceChannelFormatter):
    ...
    def _defaultFormatter(self, message, currencyCode, amountDelta, transactionTime, emitterID):
        ...
        return [
         MessageData(formatted, self._getGuiSettings(message, u'currencyUpdate'))]
    ...
    def _referralProgramFormatter(self, message, currencyCode, amountDelta, transactionTime, emitterID):
        if currencyCode == Currency.GOLD:
            ...
            return [
             MessageData(formatted, self._getGuiSettings(message, u'ReferralProgramCashbackInvoiceReceived'))]
...
class InvoiceReceivedFormatter(WaitItemsSyncFormatter):
    ...
    def _formatData(self, emitterID, assetType, data):
        ...
        if not operations:
            ...
            return g_settings.msgTemplates.format(self._getMessageTemplateKey(emitterID, assetType), ctx={u'at': (self._getOperationTimeString(data)), 
    ...
    def __getLootBoxString(self, data, assetType):
        ...
        if boxesList:
            return g_settings.htmlTemplates.format(u'eventLootBoxesInvoiceReceived', {u'boxes': ((u', ').join(boxesList))})
        else:
            return u''
...
class PrebattleKickFormatter(PrebattleFormatter):
    ...
    def format(self, message, *args):
        ...
        if prbType > 0 and kickReason > 0:
            ...
            result = [
             MessageData(formatted, self._getGuiSettings(message, u'prebattleKick'))]
        return result
...
class BootcampResultsFormatter(WaitItemsSyncFormatter):
    ...
    def __getAssetString(results, assetType, amountType):
        ...
        if amount:
            templateKeys = {(INVOICE_ASSET.GOLD): u'goldAccruedInvoiceReceived', (INVOICE_ASSET.CREDITS): u'creditsAccruedInvoiceReceived', 
               (INVOICE_ASSET.PREMIUM): u'premiumAccruedInvoiceReceived', 
               (INVOICE_ASSET.FREE_XP): u'freeXpAccruedInvoiceReceived'}
        ...
        if tokens:
            for tokenID, tokenData in tokens.iteritems():
                ...
                if tokenID.startswith(BATTLE_BONUS_X5_TOKEN):
                    ...
                    if lootBox:
                        itemsNames.append(makeHtmlString(u'html_templates:lobby/quests/bonuses', u'rawLootBox', {u'name': (lootBox.getUserName()), u'count': intCount}))
                    ...
                    if key:
                        itemsNames.append(makeHtmlString(u'html_templates:lobby/quests/bonuses', u'lootBoxKey', {u'name': text, u'count': intCount}))
...
class BattleMattersQuestAchievesFormatter(QuestAchievesFormatter):
    ...
    def isWithSelectableReward(cls, awardsDict):
        if u'tokens' not in awardsDict:
            return False
        return any(cls.__battleMattersController.isDelayedRewardToken(v) for v in awardsDict[u'tokens'].iterkeys())
...
class TelecomReceivedInvoiceFormatter(InvoiceReceivedFormatter):
    ...
    def _formatData(self, emitterID, assetType, data):
        ...
        if not vehicleNames:
            ...
            return g_settings.msgTemplates.format(self._getMessageTemplateKey(emitterID, assetType), ctx=self._getMessageContext(data, vehicleNames), data={u'timestamp': (time.time())})
...
class BattlePassRewardFormatter(WaitItemsSyncFormatter):
    ...
    def __makeCurrencyString(self, currency, amount):
        if amount:
            return g_settings.htmlTemplates.format(self.__CURRENCY_TEMPLATE_KEY, {u'currency': (backport.text(R.strings.messenger.serviceChannelMessages.battlePassReward.buy.dyn(currency)())), u'amount': (getBWFormatter(currency)(amount))})
        return u''
...
class BattlePassGiftByOfferFormatter(WaitItemsSyncFormatter):
    ...
    def format(self, message, callback=None):
        ...
        if message.data and isSynced and self.__battlePass.isValidChapterID(message.data.get(u'chapter')):
            ...
            templateParams = {u'text': text, u'header': header}
...
class BattlePassSeasonEndFormatter(WaitItemsSyncFormatter):
    ...
    def format(self, message, callback=None):
        ...
        if message.data and isSynced:
            ...
            if rewards is not None:
                ...
                formatted = g_settings.msgTemplates.format(self.__template, {u'description': description, u'text': ((u'<br>').join(text))})
                resultMessage = MessageData(formatted, self._getGuiSettings(message, self.__template))
        callback([resultMessage])
        return
    ...
    def __formatBlueprintsStrings(self, blueprints):
        ...
        for fragmentCD, count in blueprints.iteritems():
            ...
            text = self._BULLET + text
            rewardStrings.append(g_settings.htmlTemplates.format(self.__rewardTemplate, {u'text': text, u'count': count}))
...
class EpicQuestAchievesFormatter(QuestAchievesFormatter):
        ...
        for token, tokenData in data.get(u'tokens', {}).iteritems():
            ...
            if token.startswith(EPIC_OFFER_TOKEN_PREFIX):
                ...
                if offer is None:
                    ...
                    giftType = token.split(u':')[2]
                    rewardChoiceTokens[giftType] += gift.giftCount * tokenData.get(u'count', 1)
            elif token.startswith(EPIC_ARMORY_YARD_TOKEN_NAME) and tokenData.get(u'count', 0) > 0:
    ...
    def __processRewardChoiceTokens(cls, tokens):
        ...
        for rewardType, count in cls.__getOrderedTokens(tokens):
            result.append(g_settings.htmlTemplates.format(cls.__rewardTemplate, {u'text': (backport.text(rBonuses.dyn(rewardType)())), u'count': count}))
...
class ResourceWellRewardFormatter(WaitItemsSyncFormatter):
    ...
    def __getMainMessage(self, message):
        ...
        formatted = g_settings.msgTemplates.format(self.__TEMPLATE, ctx={u'title': title, u'text': text, u'additionalText': additionalText})
        return MessageData(formatted, self._getGuiSettings(message, self.__TEMPLATE))
    ...
    def __getAdditionalMessage(self, message):
        ...
        if reward is None:
            ...
            formatted = g_settings.msgTemplates.format(u'resourceWellInvoiceReceived', ctx={u'at': at, u'text': text})
            return MessageData(formatted, self._getGuiSettings(message, self.__TEMPLATE))
...
class ResourceWellNoVehiclesFormatter(WaitItemsSyncFormatter):
    ...
    def format(self, message, callback):
        ...
        if isSynced and isRuleCompleted:
            ...
            formatted = g_settings.msgTemplates.format(template, ctx={u'title': title, u'text': text})
...
class CollectionsRewardFormatter(ServiceChannelFormatter):
    ...
    def __makeMessageData(self, collection, isFinal, message):
        ...
        template = self.__REWARD_TEMPLATE
        formatted = g_settings.msgTemplates.format(template, ctx={u'title': title, u'text': text}, data={u'savedData': {u'collectionId': (collection.collectionId), 
                          u'bonuses': [
                                     message.data[u'reward']], 
                          u'isFinal': isFinal}})
    ...
    def __makeFinalMessageData(self, collection, message):
        ...
        formatted = g_settings.msgTemplates.format(self.__ITEMS_TEMPLATE, ctx={u'title': (backport.text(self.__MESSAGES.title.congratulation())), u'text': text}, data={u'savedData': {u'collectionId': (collection.collectionId)}})
...
class EarlyAccessCompensationFormatter(ServiceChannelFormatter):
    ...
    def format(self, message, *args):
        ...
        if not data or self.__earlyAccessController.getCurrentSeasonID() != 0:
            ...
            formatter = getBWFormatter(Currency.CREDITS)
            formatted = g_settings.msgTemplates.format(self.__COMPENSATION_TEMPLATE, ctx={u'tokens': (str(compensatedTokensCount)), u'credits': (formatter(compensationCredits))})
            return [
...
class EarlyAccessVehicleObtainFormatter(ServiceChannelFormatter):
    ...
    def format(self, message, *args):
        ...
        if not vehicles:
            ...
            formatted = g_settings.msgTemplates.format(self.__TEMPLATE, ctx={u'header': (backport.text(R.strings.early_access.serviceChannelMessages.obtainVehicle.title())), u'text': text})
            return [
...
class ParagonsLevelCompletedFormatter(ServiceChannelFormatter):
    __ACHIEVEMENT_NAMES = {
     u'ParagonsFirstSeason', u'Paragons_S2'}
...
class TradingCaravanMessageFormatter(ServiceChannelFormatter):
    ...
    def format(self, message, *args):
        ...
        return [
         MessageData(formatted, self._getGuiSettings(message, u'TradingCaravanCoinAdded'))]
...
class StallPurchaseReceiptMessageFormatter(ServiceChannelFormatter):
    ...
    def __formatDebitedMessage(self, message):
        ...
        return [
         MessageData(formatted, self._getGuiSettings(formatted, u'StallPurchaseReceipt'))]
