...
class WotPlusUnlockedFormatter(PremiumSubsAsyncFormatter):
    ...
    def _format(self, message):
        expiryTime = message.data.get(b'expiryTime', 0)
        ...
        formatted = g_settings.msgTemplates.format(b'WotPlusUnlockMessage', ctx={b'title': title, 
           b'text': text})
        return [
         MessageData(formatted, self._getGuiSettings(message, b'WotPlusUnlockMessage'))]


class PremiumSubsUpdatedFormatter(PremiumSubsAsyncFormatter):
    _MSG_TEMPLATE = b'InformationHeaderSysMessage'

    def _format(self, message):
        expiryTime = message.data.get(b'expiryTime', 0)
        oldExpiryTime = message.data.get(b'oldExpiryTime', 0)
        if self.subscriptionCtrl.getState() == WotPlusState.TRIAL:
            ...
            formatted = g_settings.msgTemplates.format(self._MSG_TEMPLATE, ctx={b'header': title, 
               b'text': text})
            return [
             MessageData(formatted, self._getGuiSettings(message, self._MSG_TEMPLATE))]
        return []
...
class PremiumSubsReceivedFromInvoiceFormatter(ServiceChannelFormatter):
    ...
    def format(self, message, *args):
        days = message.data.get(b'savedData', {}).get(b'premium_subs_days', 0)
...
class PassiveXpActivatedFormatter(IStandardMessageFormatter):
    ...
    def getText(self, message, *args):
        vehTypeCD = message.data.get(b'vehTypeCD')
...
class PassiveXpInvalidCrewFormatter(IStandardMessageFormatter):
    ...
    def __init__(self):
        super(PassiveXpInvalidCrewFormatter, self).__init__(b'PassiveXpInvalidCrewMessage')
        return
