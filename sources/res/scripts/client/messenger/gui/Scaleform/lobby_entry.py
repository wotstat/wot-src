...
class LobbyEntry(IGUIEntry):
    ...
    def __me_onErrorReceived(self, error):
        if error.isModal():
            DialogsInterface.showDialog(dialogs.SimpleDialogMeta(error.getTitle(), error.getMessage(), dialogs.I18nInfoDialogButtons(b'common/error')), (lambda *args: None))
        else:
    ...
    def __handleLazyChannelCtlInited(self, event):
        ...
        if controller is None:
            LOG_ERROR(b'Controller is not defined', ctx)
            return
        else:
            ctx.clear()
            channel = controller.getChannel()
    ...
    def __handlePrbChannelControllerInited(self, event):
        ...
        if not prbType:
            LOG_DEBUG(b'Prebattle type is not defined', ctx)
            return
        else:
            controller = ctx.get(b'controller')
            if controller is None:
    ...
    def __handleRqActivateChannel(self, event):
        ...
        if clientID is None:
            LOG_ERROR(b'clientID is not defined')
            return
        else:
            ctx = event.ctx
            component = ctx.get(b'component')
            if component is None:
                LOG_ERROR(b'UI component is not defined', ctx)
                return
            ctx.clear()
            controller = self.__channelsCtrl.getController(clientID)
            if controller:
    ...
    def __isCommonChatLocked():
        if getattr(BigWorld.player(), b'AccountNewbieChatLockComponent', False):
    ...
    def __getCommonChatTooltipData(isLocked):
        if isLocked:
            ...
            return {b'tooltipId': None, b'isWulfTooltip': False, 
               b'tooltipArgs': None, 
               b'label': (CHAT.CHANNELS_COMMON_TOOLTIP)}
