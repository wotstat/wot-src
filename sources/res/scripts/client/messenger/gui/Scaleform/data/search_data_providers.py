...
class SearchDataProvider(DAAPIDataProvider, ISearchHandler):
    ...
    def __del__(self):
        LOG_DEBUG(b'SearchDataProvider deleted:', self)
        return
    ...
    def init(self, flashObject, exHandlers=None):
        ...
        if exHandlers is not None and hasattr(exHandlers, b'__iter__'):
...
class SearchChannelsDataProvider(SearchDataProvider):
    ...
    def buildList(self, result):
        ...
        for item in result:
            self._list.append({b'id': (item.getID()), 
               b'name': (item.getFullName())})
    ...
    def onExcludeFromSearch(self, entity):
        ...
        self._list = [item for item in self._list if item[b'id'] != channelID]
        self.refresh()
        return
...
class SearchUsersDataProvider(SearchDataProvider):
    ...
    def __updateUserInSearch(self, user):
        for idx, item in enumerate(self._list):
            if item[b'dbID'] == user.getID():
                newItem = self._converter.makeVO(user)
                newItem[b'userProps'][b'clanAbbrev'] = item[b'userProps'][b'clanAbbrev']
