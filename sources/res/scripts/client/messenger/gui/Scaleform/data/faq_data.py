...
QUESTION_FORMAT = b'question_{0:d}'
ANSWER_FORMAT = b'answer_{0:d}'
SUPPORTED_LINKS_EVENTS = (
 OpenLinkEvent.SUPPORT,)
FAQItem = namedtuple(b'FAQItem', (b'number', b'question', b'answer'))
...
class FAQList(object):
    ...
    def __init__(self):
        ...
        self.__extraFormats = {b'contains_links': b'_FAQList__formatLinksInAnswer'}
    ...
    def __buildLinks(self):
        ...
        endTag = htmlFormat(b'closeLinkTagInFAQ')
        for link in SUPPORTED_LINKS_EVENTS:
            links[link] = {b'openTag': (htmlFormat(b'openLinkTagInFAQ', ctx={b'eventType': link})), 
               b'closeTag': endTag}
    ...
    def __formatLinksInAnswer(self, answer):
        try:
            ...
            _logger.exception(b'Link can not be added to answer')
        ...
        for suffix, methodName in self.__extraFormats.iteritems():
            ...
            if nextAnswer:
                ...
                if method and callable(method):
                    ...
                    _logger.error(b'Method %s is not found', methodName)
                break
