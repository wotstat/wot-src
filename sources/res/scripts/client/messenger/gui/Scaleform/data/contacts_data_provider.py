...
class _Category(object):
    __slots__ = (b'_converter', b'_visible', b'_avoid', b'_forced')
...
class _FriendsCategory(_Category):
    __slots__ = (b'_woGroup', b'_groups', b'__currentParent', b'__showEmptyItems')
        ...
        if pattern is None and not data and not hasWoContacts:
            ...
            if woList:
                if not hasWoContacts:
                    ...
                    if lastElement[b'gui'][b'id'] is None:
                        lastElement[b'data'][b'isVisible'] = False
                else:
...
class _FormationCategory(_Category):
    __slots__ = (b'_clan', b'__parentItemData')
...
class _OthersCategory(_Category):
    __slots__ = (b'_ignored', b'_pending')
...
class _ContactsCategories(object):
    __slots__ = (b'_categories', b'_onlineMode', b'_pattern', b'_cache')
...
class _OpenedTreeCreator(object):
    ...
    def __openTree(self, targetTreeItem, parent, isOpened):
        children = targetTreeItem.get(b'children', None)
        if isOpened:
            self.__openedTree.append(targetTreeItem)
        if children is not None:
            for child in children:
                self.__openTree(child, targetTreeItem, targetTreeItem.get(b'isOpened', False))
