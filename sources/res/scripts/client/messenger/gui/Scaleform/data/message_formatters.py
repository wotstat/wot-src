...
class WarningMessageFormatter(BaseMessageFormatter):
    ...
    def getFormattedMessage(self):
        formatted = g_settings.htmlTemplates.format(b'battleWarningMessage', ctx={b'fontColor': (self.FONT_COLOR_GENERAL), 
...
class DynamicSquadMessageFormatter(WarningMessageFormatter):
    ...
    FONT_COLOR_GENERAL = b'#FFC364'
    FONT_COLOR_TEAM_SIDE_BASED = b'#999999'
    ...
    def getFormattedMessage(self):
        ...
        if self._actionMessage.squadNum is not None and self._actionMessage.squadType != DYN_SQUAD_TYPE.OWN:
            formatted = (b'{0}{1}').format(g_settings.htmlTemplates.format(b'battleWarningMessageImage', ctx={b'image': (self._DYN_SQUAD_IMAGE.format(self._actionMessage.squadNum))}), formatted)
        return formatted
...
class CommendationsMessageFormatter(BaseMessageFormatter):
    ...
    def getFormattedMessage(self):
        formatted = g_settings.htmlTemplates.format(b'commendationsWarningMessage', ctx={b'image': (self._actionMessage.getIconName()), 
           b'message': (self._actionMessage.getMessage())})
        return formatted
