...
def editGroup(cm):
    cm.fireEvent(events.ContactsEvent(events.ContactsEvent.EDIT_GROUP, ctx={b'targetGroupName': (cm.targetGroupName)}), scope=EVENT_BUS_SCOPE.LOBBY)
    return


def removeGroup(cm):
    cm.fireEvent(events.ContactsEvent(events.ContactsEvent.REMOVE_GROUP, ctx={b'targetGroupName': (cm.targetGroupName)}), scope=EVENT_BUS_SCOPE.LOBBY)
    return
...
def createContactNote(cm):
    cm.fireEvent(events.ContactsEvent(events.ContactsEvent.CREATE_CONTACT_NOTE, ctx={b'databaseID': (cm.databaseID), b'userName': (cm.userName)}), scope=EVENT_BUS_SCOPE.LOBBY)
    return


def editContactNote(cm):
    cm.fireEvent(events.ContactsEvent(events.ContactsEvent.EDIT_CONTACT_NOTE, ctx={b'databaseID': (cm.databaseID), b'userName': (cm.userName)}), scope=EVENT_BUS_SCOPE.LOBBY)
    return
