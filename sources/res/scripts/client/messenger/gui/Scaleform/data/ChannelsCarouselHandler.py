...
class ChannelFindCriteria(ExternalCriteria):
    ...
    def find(self, name, obj):
        return getattr(obj, b'_clientID', 0) == self._criteria
...
class ChannelsCarouselHandler(object):
    ...
    def addChannel(self, channel, lazy=False, isNotified=False, isLocked=False, tooltipData=None):
        ...
        self.__channelsDP.addItem(clientID, {b'label': (channel.getFullName()), 
           b'canClose': (not isSystem), 
           b'isNotified': isNotified, 
           b'icon': (backport.image(R.images.gui.maps.icons.messenger.iconLock()) if isLocked else None), 
           b'order': order, 
    ...
    def notifyChannel(self, channel, message):
        clientID = channel.getClientID()
        self.__setItemField(clientID, b'isNotified', True)
    ...
    def updateChannel(self, channel):
        self.__setItemField(channel.getClientID(), b'label', channel.getFullName())
        return

    def updateChannelAccessibility(self, channel, isLocked, tooltipData):
        self.__setItemFields(channel.getClientID(), {b'isLocked': isLocked, 
           b'tooltipData': tooltipData, 
           b'icon': (backport.image(R.images.gui.maps.icons.messenger.iconLock()) if isLocked else None)})
        return
    ...
    def __handleCarouselInited(self, event):
        ...
        if isinstance(carousel, ChannelCarouselMeta):
            ...
            LOG_ERROR(b'Channel carousel must be extends ChannelCarouselMeta', carousel)
        return
    ...
    def __adjustAndAddChannel(self, event, targetList):
        ctx = event.ctx
        label = ctx.get(b'label')
        if label is None:
            LOG_ERROR(b'Label is not defined', event.ctx)
            return
        else:
            criteria = ctx.get(b'criteria')
            if criteria is None:
                LOG_ERROR(b'Criteria is not defined', event.ctx)
                return
            openHandler = ctx.get(b'openHandler')
            if openHandler is None:
                LOG_ERROR(b'Open handler is not defined', event.ctx)
                return
            layer = ctx.get(b'layer')
            if layer is None:
                LOG_ERROR(b'View type is not defined', event.ctx)
                return
    ...
    def __removeChannelFromList(self, event, targetList):
        ...
        if clientID in self.__handlers:
            criteria, _, layer = self.__handlers.pop(clientID)
            if event.ctx.get(b'closeWindow', True) and self.app is not None:
    ...
    def __handleRequestToChange(self, event):
        ...
        key = ctx.get(b'key')
        if key is None:
            LOG_ERROR(b'Key of item field is not defined', ctx)
            return
        else:
            value = ctx.get(b'value')
            if value is None:
                LOG_ERROR(b'Value of item field is not defined', ctx)
                return
            clientID = event.clientID
            if b'isShowByReq' in ctx and ctx[b'isShowByReq'] is True:
                self.__showByReqs[clientID] = (
                 key, value)
                isShow = ctx.get(b'showIfClosed', False)
                if isShow:
                    if clientID not in self.__handlers:
                        return
    ...
    def __handleRequestToShow(self, event):
        ...
        show = ctx.get(b'show')
        if show is None:
            LOG_ERROR(b'Flag "show" is not defined', ctx)
            return
        else:
            clientID = event.clientID
            if not clientID:
    ...
    def __handleOpenButtonClick(self, event):
        ...
        if not clientID:
            ...
            if layer == WindowLayer.WINDOW:
                ...
                if hasattr(view, b'onWindowMinimize') and callable(getattr(view, b'onWindowMinimize')):
                    view.onWindowMinimize()
                    return
            fields = {b'isNotified': False, b'isInProgress': False}
    ...
    def __handleOnWindowChangeFocus(self, event):
        self.__updateItemField(event.clientID, event.wndType, b'isWindowFocused', event.flag)
        return

    def __handleOnWindowChangeOpenState(self, event):
        self.__updateItemField(event.clientID, event.wndType, b'isWindowOpened', event.flag)
        return
