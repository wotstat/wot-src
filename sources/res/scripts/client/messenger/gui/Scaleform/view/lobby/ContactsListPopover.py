...
class ContactsListPopover(ContactsListPopoverMeta, ContactsCMListener):
    ...
    def changeGroup(self, contactDbID, contactName, groupData):
        ...
        if targetGroup == CONTACTS_ALIASES.GROUP_FRIENDS_CATEGORY_ID or targetParentGroup == CONTACTS_ALIASES.GROUP_FRIENDS_CATEGORY_ID:
            ...
            LOG_WARNING(b'Action can not be resolved', contactDbID, targetGroup, excludeGroup, targetParentGroup)
        return
    ...
    def getContactListSettings(self):
        ...
        onlineMode = None if settings[b'showOfflineUsers'] else True
        showOthers = bool(settings[b'showOthersCategory'])
        return (
    ...
    def _populate(self):
        ...
        self.as_setInitInfoS({b'isGroupSupported': (self.proto.contacts.isGroupSupported())})
        return
    ...
    def _onEditGroup(self, event):
        targetGroupName = event.ctx.get(b'targetGroupName', None)
    ...
    def _onCreateContactNote(self, event):
        userName = event.ctx.get(b'userName', None)
        databaseID = event.ctx.get(b'databaseID', None)
    ...
    def __onSettingsChanged(self, diff):
        ...
        onlineMode, showOthers = self.getContactListSettings()
        tree = self.pyTree
        if b'showOthersCategory' in diff:
            value = diff[b'showOthersCategory']
            if tree:
                showOthers = bool(value)
                isChanged = True
        if b'showOfflineUsers' in diff:
            value = diff[b'showOfflineUsers']
            if tree:
