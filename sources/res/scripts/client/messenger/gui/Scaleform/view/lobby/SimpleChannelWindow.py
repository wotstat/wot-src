...
class SimpleChannelWindow(ChannelWindowMeta):
    ...
    def __init__(self, ctx):
        ...
        self._clientID = ctx.get(b'clientID')
        self._controller = self.channelsCtrl.getController(self._clientID)
        if self._controller is None:
            raise ValueError((b'Controller for lobby channel by clientID={0:1} is not found').format(self._clientID))
        return
    ...
    def onFocusIn(self, alias):
        self.fireEvent(FocusEvent(FocusEvent.COMPONENT_FOCUSED, {b'clientID': (self._clientID)}))
        return
    ...
    def onWindowMinimize(self):
        ...
        if chat:
            chat.minimize()
            self.fireEvent(ChannelWindowEvent(events.ChannelWindowEvent.ON_WINDOW_MINIMIZE, {b'clientID': (self._clientID)}), scope=EVENT_BUS_SCOPE.LOBBY)
        self.destroy()
        return
    ...
    def _populate(self):
        ...
        self.fireEvent(ChannelWindowEvent(events.ChannelWindowEvent.ON_WINDOW_POPULATE, {b'clientID': (self._clientID)}), scope=EVENT_BUS_SCOPE.LOBBY)
        return
