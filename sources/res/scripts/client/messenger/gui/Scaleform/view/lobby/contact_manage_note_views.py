...
class ContactManageNoteView(ContactNoteManageViewMeta):
    ...
    def sendData(self, data):
        ...
        if userEntity is None:
            userProps = {b'userName': (data.name)}
        else:
            userProps = ContactConverter.makeBaseUserProps(userEntity)
        scheme = g_settings.getColorScheme(b'contacts')
        userProps[b'rgb'] = scheme.getColors(b'clanMember')[0]
        self._note = userEntity.getNote() if userEntity else b''
        if self._note:
    ...
    def _extendInitData(self, defData):
        defData[b'inputPrompt'] = i18n.makeString(MESSENGER.MESSENGER_CONTACTS_VIEW_MANAGENOTE_INPUT_PROMPT, symbols=CONTACT_LIMIT.NOTE_MAX_CHARS_COUNT)
        defData[b'groupMaxChars'] = CONTACT_LIMIT.NOTE_MAX_CHARS_COUNT
        defData[b'inputTooltip'] = i18n.makeString(MESSENGER.MESSENGER_CONTACTS_VIEW_MANAGENOTE_INPUT_TOOLTIP, maxChars=CONTACT_LIMIT.NOTE_MAX_CHARS_COUNT)
        return
