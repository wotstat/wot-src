...
class LobbyLayout(IChannelController):
    ...
    def __init__(self, channel, mBuilder=None):
        ...
        self._addListeners()
        self._memInputText = b''
        return

    def __del__(self):
        LOG_DEBUG(b'Channel controller deleted:', self)
        return
    ...
    def setMembersDP(self, membersDP):
        if self._membersDP:
            LOG_ERROR(b'Member data provider is defined', self._membersDP)
            ...
            LOG_ERROR(b'Member data provide must be extends MembersDataProvider', membersDP)
        return
...
class BattleLayout(IChannelController):
    ...
    def setView(self, view):
        if self._view:
            LOG_ERROR(b'View is defined', self._view)
        elif isinstance(view, IBattleChannelView):
            self._view = view
            self._view.addController(self)
        else:
            LOG_ERROR(b'View must be IBattleChannelView', self._view)
        return
    ...
    def sendMessage(self, message):
        ...
        if result:
            ...
            self._view.addMessage(g_settings.htmlTemplates.format(b'battleErrorMessage', ctx={b'error': errorMsg}))
        return result
