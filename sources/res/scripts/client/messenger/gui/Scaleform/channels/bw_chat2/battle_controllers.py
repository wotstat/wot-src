...
class _ChannelController(BattleLayout):
    ...
    def activate(self):
        g_eventBus.handleEvent(MessengerEvent(MessengerEvent.BATTLE_CHANNEL_CTRL_INITED, {b'controller': self}), scope=EVENT_BUS_SCOPE.BATTLE)
        return
    ...
    def __getNameSuffix(self, avatarSessionID):
        ...
        if sectorBaseComp is None:
            ...
            if 0 < sectorID < 7 and 0 < lane < 4:
                suffix = b'&lt;' + i18n.makeString(EPIC_BATTLE.ZONE_ZONE_TEXT) + b' ' + ID_TO_BASENAME[sectorID] + b'&gt;'
            elif nonCapturedBases == 0 or hqActive and sectorID > 6:
                suffix = b'&lt;' + i18n.makeString(EPIC_BATTLE.ZONE_HEADQUARTERS_TEXT) + b'&gt;'
            return suffix
