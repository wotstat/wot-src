...
class LazyChannelController(_ChannelController):
    ...
    def _fireInitEvent(self):
        g_eventBus.handleEvent(MessengerEvent(MessengerEvent.LAZY_CHANNEL_CTRL_INITED, {b'controller': self}), scope=EVENT_BUS_SCOPE.LOBBY)
        return

    def _fireDestroyEvent(self):
        g_eventBus.handleEvent(MessengerEvent(MessengerEvent.LAZY_CHANNEL_CTRL_DESTROYED, {b'controller': self}), scope=EVENT_BUS_SCOPE.LOBBY)
        return
    ...
    def __exitFromLazyChannel(self):
        ...
        if self._channel and self._channel.isJoined():
            LOG_DEBUG(b'Send request to exit from lazy channel', self._channel.getName())
            self.exit()
        return


class BSLazyChannelController(LazyChannelController):
    NOTIFICATION_ICON = {(constants.PREBATTLE_TYPE.CLAN): b'ClanBattleResultIcon-1', 
       (constants.PREBATTLE_TYPE.TOURNAMENT): b'TournamentBattleResultIcon-1'}
    NOTIFICATION_ICON_DEFAULT = b'BattleResultIcon-1'
    ...
    def __addNotification(self, invite):
        formatted = g_settings.htmlTemplates.format(b'inviteToSpecialBattle', ctx={b'icon': (self.NOTIFICATION_ICON.get(invite.prbType, self.NOTIFICATION_ICON_DEFAULT)), 
           b'text': (AutoInviteTextFormatter().getText(invite))})
...
class LobbyChannelController(_ChannelController):
    ...
    def _fireInitEvent(self):
        g_eventBus.handleEvent(MessengerEvent(MessengerEvent.LOBBY_CHANNEL_CTRL_INITED, {b'controller': self}), scope=EVENT_BUS_SCOPE.LOBBY)
        return

    def _fireDestroyEvent(self):
        g_eventBus.handleEvent(MessengerEvent(MessengerEvent.LOBBY_CHANNEL_CTRL_DESTROYED, {b'controller': self}), scope=EVENT_BUS_SCOPE.LOBBY)
        return
