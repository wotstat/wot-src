...
class ControllersCollection(IControllersCollection):
    ...
    def factory(self, channel):
        ...
        if protoType in self._factories:
            ...
            LOG_DEBUG(b'Controllers factory not found', protoType)
        return controller
    ...
    def _addController(self, channel):
        ...
        if not clientID:
            LOG_ERROR(b'Client ID for channel is not defined', channel)
            return
        else:
            if clientID is not self._controllers:
                controller = self.factory(channel)
                if controller is not None:
                    LOG_DEBUG(b'Channel controller is adding', channel)
                    self._controllers[channel.getClientID()] = controller
            else:
                LOG_WARNING(b'Controller already exists')
            return controller
    ...
    def _removeController(self, clientID):
        result = False
        if not clientID:
            LOG_ERROR(b'Client ID for channel is not defined')
            return False
        if clientID in self._controllers:
            LOG_DEBUG(b'Channel controller is deleting', clientID)
