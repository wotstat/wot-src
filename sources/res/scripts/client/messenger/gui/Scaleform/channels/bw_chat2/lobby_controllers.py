...
class UnitChannelController(LobbyLayout):
    ...
    def _fireInitEvent(self):
        g_eventBus.handleEvent(MessengerEvent(MessengerEvent.PRB_CHANNEL_CTRL_INITED, {b'prbType': (self._channel.getPrebattleType()), 
           b'controller': self}), scope=EVENT_BUS_SCOPE.LOBBY)
        return

    def _fireDestroyEvent(self):
        g_eventBus.handleEvent(MessengerEvent(MessengerEvent.PRB_CHANNEL_CTRL_DESTROYED, {b'prbType': (self._channel.getPrebattleType()), 
           b'controller': self}), scope=EVENT_BUS_SCOPE.LOBBY)
        return
