...
class MessageTemplates(templates.XMLCollection):
    ...
    def _make(self, source):
        ...
        section = source[b'bgIconHeight']
        if section is not None:
            data[b'bgIconHeight'] = source.readInt(b'bgIconHeight')
    ...
    def _makeButtonData(self, sourceID, section):
        ...
        if not action:
            ...
            textYPadding = section.readInt(b'textYPadding')
            if textYPadding > 0:
                result[b'textYPadding'] = textYPadding
            paddingHorizontal = section.readInt(b'paddingHorizontal')
            if paddingHorizontal > 0:
                result[b'paddingHorizontal'] = paddingHorizontal
