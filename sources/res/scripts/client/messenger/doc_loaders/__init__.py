...
_LOADERS = (
 (
  b'defUserPreferences', False, user_prefs.loadDefault),
 (
  b'colorsSchemes', True, colors_schemes.load),
 (
  b'settingsSet', True, settings_set.load),
 (
  b'htmlTemplatesSimple', True, html_templates.loadForOthers),
 (
  b'htmlTemplates', True, html_templates.loadForMessage))

def load(messengerSettings):
    section = ResMgr.openSection(MESSENGER_XML_FILE_PATH)
    xmlCtx = XMLCtx(MESSENGER_XML_FILE_PATH)
    if section is None:
        raise XMLError(xmlCtx, b'Messenger settings file is not found')
    tags = section.keys()
    for tag, isRequired, loader in _LOADERS:
        if tag in tags:
            ...
            raise XMLError(xmlCtx, (b'Tag "{0:>s}" not found').format(tag))
