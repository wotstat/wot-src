...
def readItem(xmlCtx, section, getter, converter=None, settings=None):
    name = readNoEmptyStr(xmlCtx, section, b'name', b'Item name is not defined')
    if b'value' not in section.keys():
        raise XMLError(xmlCtx, b'Item value is not defined')
    value = getattr(section, getter)(b'value')
    ...
    if settings is not None:
        if hasattr(settings, name):
            ...
            raise XMLError(xmlCtx, (b'Settings has not attribute {0:>s}').format(name))
    return (name, value)


def readIntItem(xmlCtx, section, settings=None):
    return readItem(xmlCtx, section, b'readInt', settings=settings)


def readFloatItem(xmlCtx, section, settings=None):
    return readItem(xmlCtx, section, b'readFloat', settings=settings)


def readRGBItem(xmlCtx, section, settings=None):
    name, value = readItem(xmlCtx, section, b'readVector3', settings=settings)
    return (
     name, _convertVector3ToRGB(xmlCtx, value, b'Value is not valid'))


def readStringItem(xmlCtx, section, settings=None):
    return readItem(xmlCtx, section, b'readString', settings=settings)


def readUnicodeItem(xmlCtx, section, settings=None):
    return readItem(xmlCtx, section, b'readWideString', settings=settings)


def readI18nStringItem(xmlCtx, section, settings=None):
    name, value = readItem(xmlCtx, section, b'readWideString', converter=html_translation, settings=settings)
    return (
     name, value)
