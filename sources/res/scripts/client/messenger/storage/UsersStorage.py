...
class UsersStorage(RevCachedStorage):
    __slots__ = (b'__contacts', b'__emptyGroups', b'__openedGroups', b'__clanMembersIDs', b'__breakers')
    ...
    def __repr__(self):
        return (b'UsersStorage(id=0x{0:08X}, len={1:n})').format(id(self), len(self.__contacts))
    ...
    def addUser(self, user):
        ...
        if key not in self.__contacts:
            ...
            _logger.error(b'User %r exists in storage', user)
        return
    ...
    def setUser(self, user):
        ...
        if key not in self.__contacts:
            ...
            if not user.setSharedProps(exists):
                _logger.error(b'User entity %r can not be replaced by %r', exists, user)
                return
            self.__contacts[key] = user
        return
    ...
    def removeUser(self, sessionID):
        try:
            ...
            _logger.warning(b'Trying to remove not existing contact %s', sessionID)
