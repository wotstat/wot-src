...
class ChannelsStorage(SimpleCachedStorage):
    ...
    def __repr__(self):
        return (b'ChannelsStorage(id=0x{0:08X}, len={1:n})').format(id(self), len(self.__channels))
    ...
    def addChannel(self, channel):
        ...
        if channel not in self.__channels:
            ...
            LOG_ERROR(b'Channel already exists in storage', channel)
            result = False
        return result
    ...
    def removeChannel(self, channel, clear=True):
        ...
        if channel in self.__channels:
            ...
            LOG_ERROR(b'Channel is not in storage', channel)
            result = False
        return result
