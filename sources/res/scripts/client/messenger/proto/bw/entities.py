...
class BWChannelEntity(ChannelEntity):
    ...
    def update(self, **kwargs):
        if b'other' in kwargs:
            self._data = kwargs[b'other'].getProtoData()
...
@ReprInjector.withParent((b'isOnline', b'isOnline'))
class BWUserEntity(LobbyUserEntity):
    __slots__ = (b'_isOnline',)
    ...
    def update(self, **kwargs):
        if b'isOnline' in kwargs:
            self._isOnline = kwargs[b'isOnline']
