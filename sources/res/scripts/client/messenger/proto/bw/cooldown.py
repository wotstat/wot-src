...
_reloadingChatCommandsWithParams = {(BATTLE_CHAT_COMMAND_NAMES.RELOADINGGUN): (b'rTime',), 
   (BATTLE_CHAT_COMMAND_NAMES.RELOADING_CASSETE): (b'rTime', b'ammoQuantityLeft'), 
   (BATTLE_CHAT_COMMAND_NAMES.RELOADING_CASSETE_AUTO_SHOOT): (b'rTime', b'ammoQuantityLeft'), 
   (BATTLE_CHAT_COMMAND_NAMES.RELOADING_READY_CASSETE): (b'ammoInCassete',), 
   (BATTLE_CHAT_COMMAND_NAMES.RELOADING_READY_CASSETE_AUTO_SHOOT): (b'ammoInCassete',)}
...
def isOperationInCooldownEx(operationName):
    ...
    if operation is not None:
        ...
        LOG_ERROR(b"Can't find operation = %s in chat_shared.CHAT_COMMANDS" % operationName)
    return result
...
def getOperationInCooldownMsg(operation, period, params=None):
    if operation in _battleChatShortcuts:
        ...
        operationKey = b'#%s:command/%s' % (MESSENGER_I18N_FILE, operation)
        operationName = i18n.makeString(operationKey)
        if operationName.startswith(b'command'):
            operationName = operation
    if period > 0:
        message = i18n.makeString(b'#%s:client/error/commandInCooldown/limited' % MESSENGER_I18N_FILE, operationName, period)
    else:
        message = i18n.makeString(b'#%s:client/error/commandInCooldown/unlimited' % MESSENGER_I18N_FILE, operationName)
    return message
