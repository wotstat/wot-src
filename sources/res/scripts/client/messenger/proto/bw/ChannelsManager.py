...
class ChannelsManager(ChatActionsListener):
    ...
    def __init__(self):
        ChatActionsListener.__init__(self, {(CHAT_RESPONSES.channelNotExists): b'_ChannelsManager__onChannelNotExists', 
           (CHAT_RESPONSES.commandInCooldown): b'_ChannelsManager__onCommandInCooldown'})
    ...
    def exitFromLazyChannels(self):
        ...
        for channel in channels:
            if channel.isJoined():
                LOG_DEBUG(b'Send request to exit from lazy channel', channel.getName())
    ...
    def removeChannelFromClient(self, channel):
        if channel.isJoined():
            LOG_DEBUG(b'Client is removing channel to which player is joined', channel)
    ...
    def __onRequestChannels(self, chatAction, join=False):
        ...
        data = chatActionDict.get(b'data', [])
        requestID = chatActionDict.get(b'requestID', -1)
        channels = set()
        isStore = requestID == -1
        ...
        for channelData in data:
            ...
            if isMucEnabled and not received.isSystem() and not received.isLazy() and not received.isPrivate() and not received.isClan():
                g_logOutput.error(CLIENT_LOG_AREA.OBSOLETE, b'Game server sends BW channel on which player is joined, but MUC are enabled. BW channel is ignored', received)
                continue
    ...
    def __onSelfLeaveChat(self, chatAction):
        LOG_DEBUG(b'onSelfLeaveChat:%s' % (dict(chatAction),))
        wrapper = ChatActionWrapper(**dict(chatAction))
        channel = self.channelsStorage.getChannel(entities.BWChannelLightEntity(wrapper.channel))
        if channel:
            channel.setJoined(False)
    ...
    def __onChannelDestroyed(self, chatAction):
        LOG_DEBUG(b'onChannelDestroyed : %s' % (dict(chatAction),))
        wrapper = ChatActionWrapper(**dict(chatAction))
        channel = self.channelsStorage.getChannel(entities.BWChannelLightEntity(wrapper.channel))
    ...
    def __onChannelInfoUpdated(self, chatAction):
        result = chatAction[b'data'] if chatAction.has_key(b'data') else {}
    ...
    def __onChannelNotExists(self, _, chatAction):
        channelID = chatAction[b'channel']
        LOG_DEBUG(b'channelId : %s' % (channelID,))
    ...
    def __onCommandInCooldown(self, actionResponse, chatAction):
        ...
        result = False
        if data[b'command'] == b'findChatChannels':
            result = True
            self.onFindChannelsFailed(chatAction.get(b'requestID', -1), actionResponse, data)
        return result
    ...
    def __makeMemberFromDict(self, memberData):
        member = None
        if b'id' in memberData:
            kwargs = {}
            if b'nickName' in memberData:
                kwargs[b'nickName'] = memberData[b'nickName']
            if b'status' in memberData:
                kwargs[b'status'] = memberData[b'status']
            member = entities.BWMemberEntity(memberData[b'id'], **kwargs)
        return member
