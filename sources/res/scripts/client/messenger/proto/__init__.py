...
class ProtoPluginsDecorator(IProtoPlugin):
    ...
    def connect(self, scope):
        self._invoke(b'connect', scope)
        return

    def disconnect(self):
        self._invoke(b'disconnect')
        return

    def view(self, scope):
        self._invoke(b'view', scope)
        return

    def goToReplay(self):
        self._invoke(b'goToReplay')
        return

    def setFilters(self, msgFilterChain):
        self._invoke(b'setFilters', weakref.proxy(msgFilterChain))
        return
