...
class ChatStateExtension(PyExtension):
    ...
    def parseTag(self, pyGlooxTag):
        result = pyGlooxTag.filterXPath((b'|').join(CHAT_STATE.RANGE))
        if result:
...
class DelayExtension(PyExtension):
    ...
    def parseTag(self, pyGlooxTag):
        stamp = pyGlooxTag.findAttribute(b'stamp')
        if stamp:
            try:
                tm = time.strptime(stamp, b'%Y-%m-%dT%H:%M:%SZ')
                tm = tm[0:8] + (0,)
                sentAt = calendar.timegm(tm)
            except ValueError:
                try:
                    dt = datetime.strptime(stamp, b'%Y-%m-%dT%H:%M:%S.%fZ')
...
class MessageIDExtension(PyExtension):
    ...
    def parseTag(self, pyGlooxTag):
        return pyGlooxTag.findAttribute(b'uuid')
...
class ChatHistoryQuery(PyExtension):
    ...
    def __init__(self, jid, limit):
        ...
        self.setAttribute(b'with', str(jid))
        self.setAttribute(b'limit', limit)
        return
...
class PrivateHistoryItem(PyExtension):
    ...
    def parseTag(self, pyGlooxTag):
        requestID = pyGlooxTag.findAttribute(b'request-id')
        isFinal = pyGlooxTag.findAttribute(b'final')
        if isFinal:
...
class _MucPrivilegesExtension(PyExtension):
    ...
    def __init__(self, affiliation=b'', role=b''):
        super(_MucPrivilegesExtension, self).__init__(_TAG.WG_MUC_PRIVILEGES)
        self.setAttribute(b'affiliation', affiliation)
        self.setAttribute(b'role', role)
        return
    ...
    def parseTag(self, pyGlooxTag):
        affiliation = pyGlooxTag.findAttribute(b'affiliation') or b'none'
        role = pyGlooxTag.findAttribute(b'role') or b'none'
        return (
         affiliation, role)
...
class MessageWgSharedExtension(WgSharedExtension):
    ...
    def parseTag(self, pyGlooxTag):
        ...
        info[b'affiliation'] = affiliation
        info[b'role'] = role
        return info
...
class _MessageCustomExtension(PyExtension):
    ...
    def __init__(self, msgType, state=CHAT_STATE.UNDEFINED):
        ...
        self.setAttribute(b'type', msgType)
    ...
    def parseTag(self, pyGlooxTag):
        ...
        if info:
            message.accountDBID = info[b'dbID']
            message.accountName = info[b'name']
            message.accountRole = info[b'role']
            message.accountAffiliation = info[b'affiliation']
...
class ChatMessageHolder(MessageQuery):
    ...
    def __init__(self, msgType, to, msgBody=b'', state=CHAT_STATE.UNDEFINED):
        if state:
...
class MessageHandler(PyHandler):
    ...
    def getFilterString(self):
        return (b"/{0}[@type='{1}']").format(self._ext.getName(), self._typeAttr)
...
class UserRoomConfigurationFormSet(OwnerConfigurationFormSet):
    ...
    def __init__(self, to, room, password=b''):
        fields = (
         Field(b'text-single', b'muc#roomconfig_roomname', room),
         Field(b'boolean', b'muc#roomconfig_persistentroom', 1),
         Field(b'boolean', b'muc#roomconfig_publicroom', 1),
         Field(b'boolean', b'muc#roomconfig_membersonly', 0),
         Field(b'boolean', b'muc#roomconfig_allowinvites', 1),
         Field(b'boolean', b'muc#roomconfig_survive_reboot', 1))
        if password:
            fields += (
             Field(b'boolean', b'muc#roomconfig_passwordprotectedroom', 1),
             Field(b'text-single', b'muc#roomconfig_roomsecret', password))
