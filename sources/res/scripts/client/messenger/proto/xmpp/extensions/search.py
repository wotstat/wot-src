...
class ChannelSearchQuery(PyQuery):
    ...
    def __initExtension(self):
        filterExtension = PyExtension(_TAG.FILTER).setXmlNs(_NS.WG_MUC_ROOMS).setAttribute(b'roomname-prefix', self._token).setAttribute(b'max-entries', self._results_count)
...
class UsersSearchQuery(PyQuery):
    ...
    def __initExtension(self):
        filterExtension = PyExtension(_TAG.ITEM).setXmlNs(_NS.WG_SPA_RESOLVER).setAttribute(b'nickname', self._nickname)
...
class NicknamePrefixSearchQuery(PyQuery):
    ...
    def __initExtension(self):
        return PyExtension(_TAG.WG_NICKNAME_PREFIX_SEARCH).setXmlNs(_NS.WG_SPA_RESOLVER).setAttribute(b'prefix', self._prefix).setAttribute(b'limit', self._limit)
