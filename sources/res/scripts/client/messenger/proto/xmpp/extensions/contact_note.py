...
class NoteID(PyExtension):
    ...
    def __init__(self, dbID=0):
        super(NoteID, self).__init__(b'key')
        self._dbID = dbID
        return
...
class NoteText(PyExtension):
    ...
    def __init__(self, text=b''):
        super(NoteText, self).__init__(b'value')
        self._text = text
        return
...
class NoteItem(PyExtension):
    ...
    def __init__(self, dbID=0, text=b''):
...
class NoteList(PyExtension):
    ...
    def __init__(self, items=None, rsm=None):
        ...
        self.setAttribute(b'name', b'contact-notes')
        if items:
            for item in items:
                self.setChild(item)
