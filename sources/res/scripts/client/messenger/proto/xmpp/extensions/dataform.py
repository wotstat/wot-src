...
Field = namedtuple(b'Field', (b'type', b'var', b'value'))
...
class FieldElement(PyExtension):
    ...
    def __init__(self, field=None):
        ...
        if field is not None:
            self.setAttribute(b'type', field.type)
            self.setAttribute(b'var', field.var)
    ...
    def parseTag(self, pyGlooxTag):
        fieldType = pyGlooxTag.findAttribute(b'type')
        fieldVar = pyGlooxTag.findAttribute(b'var')
        tag = findFirst(None, pyGlooxTag.filterXPath(self.getXPath(suffix=b'value')))
        if tag:
            value = tag.getCData()
        else:
            value = b''
...
class DataForm(PyExtension):
    ...
    def __init__(self, fields=None):
        ...
        if not self._isEmpty:
            self.setAttribute(b'type', FORM_TYPE.SUBMIT)
            for field in fields:
    ...
    def parseTag(self, pyGlooxTag):
        formType = pyGlooxTag.findAttribute(b'type')
