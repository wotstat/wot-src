...
class ConnectionHandler(ClientEventsHandler):
    ...
    def connect(self):
        if self.__reconnectCallbackID is None:
            ...
            g_logOutput.debug(CLIENT_LOG_AREA.CONNECTION, b'Connection already is processing')
        return

    def disconnect(self):
        client = self.client()
        if client:
            g_logOutput.debug(CLIENT_LOG_AREA.CONNECTION, b'Sends request to disconnect and removes all listeners')
            client.disconnect()
        self.clear()
        return
    ...
    def __doConnect(self):
        ...
        if not client.isDisconnected():
            g_logOutput.warning(CLIENT_LOG_AREA.CONNECTION, b'Client already is connected(ing)', client.getConnectionAddress(), client.getConnectionState())
            return
        jid = self.__connectionsInfo.getPlayerFullJID()
        if jid:
            cType, host, port = self.__connectionsInfo.getNextConnection()
            g_logOutput.debug(CLIENT_LOG_AREA.CONNECTION, b'Connect to XMPP sever', jid, host, port)
            if cType == CONNECTION_IMPL_TYPE.TCP:
                client.connect(str(jid), host, port)
            elif cType == CONNECTION_IMPL_TYPE.BOSH:
                client.connectBosh(str(jid), host, port, b'/bosh/')
            else:
                g_logOutput.error(CLIENT_LOG_AREA.CONNECTION, b'This type of connection is not supported', cType)
        else:
            g_logOutput.error(CLIENT_LOG_AREA.CONNECTION, b'JID is empty')
        return
    ...
    def __doLogin(self):
        ...
        if not client.isConnecting():
            g_logOutput.warning(CLIENT_LOG_AREA.LOGIN, b'Client is not connecting', client.getConnectionAddress(), client.getConnectionState())
            yield lambda callback: callback(None)
            return
        g_logOutput.debug(CLIENT_LOG_AREA.TOKEN, b'Sends request to SPA')
        response = yield self.__tokenRequester.request()
        g_logOutput.debug(CLIENT_LOG_AREA.TOKEN, b'Response is received from SPA', response)
        if not response:
            g_logOutput.error(CLIENT_LOG_AREA.TOKEN, b'Received chat token is empty')
            return
        if response.isValid():
            if response.getDatabaseID() == getPlayerDatabaseID():
                g_logOutput.debug(CLIENT_LOG_AREA.LOGIN, b'Login to XMPP sever')
                client.login(response.getCredential())
            else:
                g_logOutput.error(CLIENT_LOG_AREA.LOGIN, b"Player's database ID mismatch", getPlayerDatabaseID())
        else:
            g_logOutput.warning(CLIENT_LOG_AREA.TOKEN, b'Received chat token is not valid', response)
    ...
    def __handleConnect(self):
        g_logOutput.debug(CLIENT_LOG_AREA.CONNECTION, b'Client is connected')
        self.__cancelReconnectCallback()
        self.__reqTokenBackOff.reset()
        self.__doLogin()
        return
    ...
    def __handleLogin(self):
        g_logOutput.debug(CLIENT_LOG_AREA.LOGIN, b'Client is login')
        self.__connectionsInfo.clear()
        self.__reqTokenBackOff.reset()
    ...
    def __handleDisconnect(self, reason, description):
        ...
        if not client:
            return
        g_logOutput.debug(CLIENT_LOG_AREA.CONNECTION, b'Client is disconnected', reason, description)
        self.__cancelReconnectCallback()
        self.__cancelReqTokenCallback()
        ...
        if self.isInGameServer() and reason != DISCONNECT_REASON.BY_REQUEST:
            ...
            g_logOutput.debug(CLIENT_LOG_AREA.CONNECTION, (b'Will try to reconnect after {0} seconds').format(delay), description)
            host, port = self.__connectionsInfo.getLastAddress()
    ...
    def __handleTokenError(self):
        ...
        if tries < _MAX_REQ_TOKEN_TRIES:
            ...
            self.__handleDisconnect(DISCONNECT_REASON.OTHER_ERROR, b'Received chat token is not valid')
        return
