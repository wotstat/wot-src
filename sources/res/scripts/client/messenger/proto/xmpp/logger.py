...
class LogHandler(ClientEventsHandler):
    __slots__ = (b'__loggers',)

    def __init__(self):
        super(LogHandler, self).__init__()
        self.__loggers = {b'out': (_GlooxLogger())}
        return
...
def sendEventToServer(eventType, host, port, errorCode=0, errorDescr=b'', tries=1):
    player = BigWorld.player()
    sender = getattr(player, b'logXMPPEvents', None)
    if sender is None or not callable(sender) or not player.isPlayer or player.isDestroyed:
        return
    address = (b'{0}:{1}').format(host, port)
    currentTime = time.time()
    g_logOutput.debug(CLIENT_LOG_AREA.GENERIC, b'Sends log to server', [
