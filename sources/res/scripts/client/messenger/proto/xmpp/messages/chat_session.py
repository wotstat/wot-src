...
class ChatSessionHistoryRequester(ClientHolder):
    ...
    def __setChannelHistory(self, jid):
        ...
        if channel:
            g_messengerEvents.channels.onHistoryReceived(sorted(self.__history, key=operator.attrgetter(b'sentAt')), channel)
        return
    ...
    def addMessage(self, jid, message):
        if message.isHistory():
            ...
            if self.__gameSession.isPrivateMessagesForbidden:
                _logger.info(b'Account has restriction with PrivateMessagesForbidden')
                return
            if exists is None and (g_settings.userPrefs.chatContactsListOnly or self.__gameSession.isNonFriendPrivateMessagesForbidden):
                ...
                if not contact or not USER_TAG.filterClosedContactsTags(contact.getTags()):
                    _logger.info(b"There is not closed contact in player's contacts list, contact's message is ignored id=%d, nickname=%s", jid.getDatabaseID(), nickname)
                    return
            if exists is None:
