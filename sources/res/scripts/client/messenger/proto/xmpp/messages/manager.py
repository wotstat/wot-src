...
class MessagesManager(ClientEventsHandler):
    ...
    @xmpp_query()
    def joinToMUC(self, jid, password=b'', name=b''):
    ...
    def _addCommonChannelToStorage(self):
        ...
        if sysChannelConfig is not None and sysChannelConfig[b'enabled']:
            sysChannelEntity = entities.XmppSystemChannelEntity(mucChannelType=XMPP_MUC_CHANNEL_TYPE.STANDARD, name=sysChannelConfig[b'userString'])
