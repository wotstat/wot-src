...
class _UserPresence(ClientHolder):
    __slots__ = (b'__scope', b'__isInBootcamp')
...
class VoipHandler(object):
    ...
    def __voip_onChannelEntered(self, uri, isTestChannel, isRejoin):
        if isTestChannel:
            return
        if self.playerCtx.getCachedItem(b'lastVoipUri') != uri and not isRejoin:
            g_messengerEvents.users.onUsersListReceived({USER_TAG.MUTED})
        else:
            self.playerCtx.setCachedItem(b'lastVoipUri', uri)
        return

    def __voip_onChannelLeft(self, _, wasTest):
        if wasTest:
            return
        self.playerCtx.setCachedItem(b'lastVoipUri', b'')
        g_messengerEvents.users.onUsersListReceived({USER_TAG.MUTED})
        return
...
class ContactsManager(ClientEventsHandler):
    ...
    def __addTasks(self, actionID, jid, shadowMode, *tasks):
        if shadowMode:
            if all([task.canShadowMode() for task in tasks]):
                ...
                g_logOutput.error(_LOG.PY_WRAPPER, b"Trying to shadow tasks seq, but some tasks can't be shadowed")
    ...
    def __handleRosterResult(self, generator):
        g_logOutput.debug(_LOG.ROSTER, b'Roster result is received')
        self.__seq.sync(0, generator())
        return

    def __handleRosterItemSet(self, jid, name, groups, sub, clanInfo):
        g_logOutput.debug(_LOG.ROSTER, b'Roster push is received', jid, name, groups, sub, clanInfo)
        ...
        if not dbID:
            ...
            if resource.presence == PRESENCE.UNAVAILABLE:
                if user and not user.isCurrentPlayer():
                    ...
                    g_logOutput.debug(_LOG.RESOURCE, b'Resource is removed', user.getName(), jid.getResource(), resource)
                ...
                if user.isCurrentPlayer():
                    ...
                    g_logOutput.debug(_LOG.RESOURCE, b'Resource is set', user.getName(), jid.getResource(), resource)
            if user:
    ...
    def __onSeqsInited(self):
        g_logOutput.debug(_LOG.GENERIC, b'Starts to process contacts tasks')
