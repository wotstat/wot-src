...
def _syncBlockItem(storage, jid, name=b'', dbID=0, clanInfo=None):
...
class AddBlockItemTask(_BlockItemTask):
    ...
    def set(self, pyGlooxTag):
        ...
        jid, info = findFirst(None, result, (b'', {}))
        if jid != self._jid:
            return
        else:
            user = _syncBlockItem(self.usersStorage, self._jid, **info)
            if user:
                g_logOutput.debug(CLIENT_LOG_AREA.BLOCK_LIST, b'Block item is added', jid, info)
    ...
    def set(self, pyGlooxTag):
        ...
        if jid != self._jid:
            ...
            if user and user.getItemType() in XMPP_ITEM_TYPE.PERSISTENT_BLOCKING_LIST:
                ...
                g_logOutput.debug(CLIENT_LOG_AREA.BLOCK_LIST, b'Block item is removed', jid, info)
...
class SyncBlockItemTask(IQTask):
    ...
    def set(self, pyGlooxTag):
        for xPath, clazz in self._handlers.iteritems():
            ...
            if not result:
                continue
            jid = result[0].findAttribute(b'jid')
            if jid:
