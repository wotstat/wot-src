...
class InboundSubscriptionsBatch(object):
    ...
    def _populate(self, taskQueue):
        ...
        if jids:
            g_logOutput.debug(_LOG.SUBSCRIPTION, b'Inbound subscriptions are canceled automatically', jids[:10])
            jids = []
        ...
        if jids:
            g_logOutput.debug(_LOG.SUBSCRIPTION, b'Inbound subscriptions are approved automatically', jids[:10])
        if self._newRqs:
            g_logOutput.debug(_LOG.SUBSCRIPTION, b'New inbound subscriptions are received', self._newRqs[:10])
            g_messengerEvents.users.onFriendshipRequestsAdded(self._newRqs)
        if self._oldRqs:
            g_logOutput.debug(_LOG.SUBSCRIPTION, b'Inbound subscriptions are ignored to display', self._oldRqs[:10])
