...
class IQTask(_Task):
    __slots__ = (b'_iqID', b'_shadowPath', b'_error')
    ...
    def error(self, pyGlooxTag=None):
        ...
        if self._shadowPath is None:
            if self._error:
                ...
                g_logOutput.error(_LOG_AREA.PY_WRAPPER, b'Error is not resolved on the client', self.__class__.__name__, pyGlooxTag.getXml())
        self._result = TaskResult.CLEAR
        return
...
class SeqTaskQueue(object):
    __slots__ = (b'__queue', b'__wait', b'__others', b'__multi', b'__isSuspend', b'onInited')
...
class ContactTaskQueue(object):
    __slots__ = (b'__queue', b'__isSuspend', b'__pending', b'__syncByIQ', b'onSeqTaskRequested')
    ...
    def sync(self, jid, name=b'', groups=None, sub=None, clanInfo=None, defaultTask=None):
        if not jid.getDatabaseID():
            g_logOutput.error(_LOG_AREA.SYNC, (b'JID "{0}" is invalid').format(jid))
            return
