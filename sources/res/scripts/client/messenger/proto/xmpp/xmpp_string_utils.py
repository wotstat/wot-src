...
def validateRosterItemGroup(name):
    if not name:
        return (b'', ClientContactError(CONTACT_ERROR_ID.GROUP_EMPTY))
    else:
        try:
...
def validateUserRoomName(name):
    if not name:
        return (b'', ClientChannelError(CHANNEL_ERROR_ID.NAME_EMPTY))
    else:
        try:
            name = ResourcePrep.prepare(name)
        except XmppStringPrepError:
...
def validateUserRoomPwd(password, isRetype=False):
    if not password:
        ...
        try:
            ...
            return (b'', ClientChannelError(errorID))
...
def validateUserRoomPwdPair(password, retype):
    ...
    if error is not None:
        ...
        if error is not None:
            return (b'', error)
        if password != retype:
            return (b'', ClientChannelError(CHANNEL_ERROR_ID.PASSWORDS_NOT_EQUALS))
