...
class ContactItem(object):
    __slots__ = (b'_jid', b'_sub', b'_tags', b'_trusted', b'_resources')
    ...
    def __repr__(self):
        return (b'{0}({1})').format(self.__class__.__name__, self._jid)
    ...
    def update(self, **kwargs):
        if b'trusted' in kwargs:
            self.setTrusted(kwargs[b'trusted'])
        if b'jid' in kwargs and b'resource' in kwargs:
            jid = kwargs[b'jid']
            resource = kwargs[b'resource']
            if resource:
...
class RosterItem(ContactItem):
    ...
    def __repr__(self):
        return (b'RosterItem(jid={0}, groups={1}, resource={2}, sub={3}/{4})').format(self._jid, self._groups, self._resources.getHighestPriority(), _SUB_NAMES[self._sub[0]], _SUB_NAMES[self._sub[1]])
    ...
    def update(self, **kwargs):
        ...
        if b'groups' in kwargs:
            self._groups = kwargs[b'groups']
        if b'sub' in kwargs:
            self._sub = kwargs[b'sub']
            self._updateSubTags()
        return
