...
class NicknameResolver(ClientEventsHandler):
    ...
    def __doCallback(self, result=None, error=None):
        if error:
            g_logOutput.error(CLIENT_LOG_AREA.GENERIC, b'Error has been received on requesting nicknames', error)
        if self.__callback:
        ...
        if iqID != self.__iqID:
            ...
            if iqType == IQ_TYPE.RESULT:
                ...
                for dbID, name, error in handler.handleTag(pyGlooxTag):
                    if error:
                        errorOutput(CLIENT_LOG_AREA.PY_WRAPPER, b'Item is invalid', dbID, name, error)
                        continue
                    user = getter(dbID)
                    if not user:
