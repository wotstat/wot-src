...
def getChatErrorMessage(errorID, kwargs):
    ...
    if i18nKey is not None:
        ...
        msg = (b'{0}\\{1}').format(errorName, kwargs)
    return msg
...
class _BattleCommandGenericError(_BattleCommandError):
    __slots__ = (b'_commandMessageKey',)
...
class _SimpleAdminCommandError(_AdminCommandError):
    ...
    def __init__(self, errorID, kwargs=None):
        super(_SimpleAdminCommandError, self).__init__(getChatErrorMessage(errorID, kwargs or {b'actionName': (i18n.makeString(I18N_MESSENGER.CUSTOM_CLIENT_ACTION_ADMIN_CHAT_COMMAND))}))
        return
...
def createBroadcastError(args, broadcastID):
    ...
    if errorID == _ERRORS.IN_CHAT_BAN:
        error = _ChatBanErrorShort(makeLocalServerTime(args[b'floatArg1']), args[b'strArg1'], args[b'strArg2'])
...
def createBattleCommandError(args, command):
    ...
    if errorID == _ERRORS.IN_COOLDOWN:
        ...
        cooldownTimeRemaining = args[b'floatArg1']
...
def createVOIPError(args, actionID):
    ...
    if actionID == _ACTIONS.GET_VOIP_CREDENTIALS:
        if errorID == _ERRORS.IN_COOLDOWN:
            ...
            logOnly = True
            error = b'The player has received the error to the request for getting of voip credential. Perhaps voip connection to the server is lost, the server is reconnecting to voip.'
    elif actionID == _ACTIONS.REQUEST_ECHO_CHANNEL:
        if errorID == _ERRORS.IN_COOLDOWN:
            cooldown = args[b'floatArg1']
