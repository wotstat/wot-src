...
_ActionsCollection = namedtuple(b'_ActionsCollection', b'initID deInitID onBroadcastID broadcastID')
...
class _EntityChatHandler(bw2_provider.ResponseSeqHandler):
    ...
    def broadcast(self, text, *args):
        if self.__isInited:
            ...
            _logger.warning(b'TODO: Adds error message')
        return
    ...
    def _onEntityChatInit(self, _, args):
        if self.__isInited:
            if not BattleReplay.g_replayCtrl.isPlaying:
                _logger.warning(b'EntityChat already is inited %r', self.__class__.__name__)
            return
...
class UnitChatHandler(_EntityChatHandler):
    ...
    def send(self, decorator):
        ...
        if command:
            ...
            _logger.error(b'Unit command not found %s', decorator.getCommandText())
        return
    ...
    def _doInit(self, args):
        if b'int32Arg1' in args:
            self.__doCreateChannel(args[b'int32Arg1'])
        else:
            _logger.error(b'Type of prebattle is not defined %r', args)
        return
    ...
    def _addHistory(self, iterator):
        if self.__history is None:
            ...
            if self.__channel is not None:
                ...
                g_messengerEvents.channels.onHistoryReceived(sorted(messages, key=operator.attrgetter(b'sentAt')), self.__channel)
        else:
...
class BattleChatCommandHandler(bw2_provider.ResponseDictHandler, IBattleCommandFactory):
    ...
    def send(self, decorator):
        ...
        if command:
            ...
            _logger.error(b'Battle command is not found %r', decorator)
        return
    ...
    def _onResponseFailure(self, ids, args):
        ...
        if command:
            ...
            if error:
                ...
                _logger.warning(b'Error is not resolved on the client %d, %r', command.getID(), args)
        return
    ...
    def __isSilentMode(self, cmd):
        arenaDP = self.__sessionProvider.getArenaDP()
        if not cmd.isMuteTypeMessage() or cmd.getSenderID() == b'' or arenaDP is None:
            return False
    ...
    def __processCommand(self, cmd):
        ...
        if cmd.isIgnored():
            ...
            _logger.debug(b"Chat command id: %s, text: '%s' is ignored", cmd.getID(), cmd.getCommandText())
            return
        else:
    ...
    def _onResponseFailure(self, ids, args):
        if super(AdminChatCommandHandler, self)._onResponseFailure(ids, args):
            ...
            if error:
                ...
                _logger.warning(b'Error is not resolved on the client %d, args: %r', ids, args)
        return
...
def getCoolDownConfig(bonusChecker):
    if bonusChecker(BONUS_CAPS.SPAM_PROTECTION):
        ...
        spamProtection = teamInfo.dynamicComponents.get(b'spamProtection') if teamInfo else None
