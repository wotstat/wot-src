...
class _ReceivedCmdDecorator(ReceivedBattleChatCommand):
        ...
        if i18nKey is not None:
            if self.isOnMinimap():
                if self.isSPGAimCommand():
                    ...
                    if reloadTime > 0:
                        i18nArguments[b'reloadTime'] = reloadTime
                        i18nKey += b'_reloading'
            elif self.hasTarget():
                i18nArguments[b'target'] = self._getTarget()
                if self.isSPGAimCommand():
                    reloadTime = self._protoData[b'floatArg1']
                    if reloadTime > 0:
                        i18nArguments[b'reloadTime'] = reloadTime
                    ...
                    i18nArguments[b'target'] = self._getTargetShortName()
            elif self.isBaseRelatedCommand():
                strArg = self._protoData[b'strArg1']
                if strArg != b'':
                    i18nArguments[b'strArg1'] = strArg
                    i18nKey += b'_numbered'
                ...
                if mapsCtrl and mapsCtrl.hasMinimapGrid():
                    ...
                    i18nKey += b'_gridInfo'
                    i18nArguments[b'gridId'] = mapsCtrl.getMinimapCellNameById(cellId)
            else:
    ...
    def messageOnMarker(self):
        ...
        return unicode(text, b'utf-8', errors=b'ignore')
    ...
    def _getTargetShortName(self):
        ...
        return (b'({})').format(vehicleType.shortNameWithPrefix)
