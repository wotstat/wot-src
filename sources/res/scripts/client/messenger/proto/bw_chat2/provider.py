...
class BWChatProvider(object):
    __slots__ = (b'__weakref__', b'__handlers', b'__msgFilters', b'__coolDown', b'__idGen', b'__isEnabled', b'__queue', b'__battleCmdCooldowns', b'__replayHelper')
    ...
    def onActionReceived(self, actionID, reqID, args):
        ...
        for handler in handlers:
            try:
                ...
                LOG_ERROR(b'Handler has been invoked with error', handler)
    ...
    def registerHandler(self, actionID, handler):
        ...
        if handler in handlers:
            LOG_WARNING(b'Handler already is exist', actionID, handler)
        elif not hasattr(handler, b'__self__') or not isinstance(handler.__self__, ActionsHandler):
            LOG_ERROR(b'Class of handler is not subclass of ActionsHandler', handler)
            return
        if callable(handler):
            handlers.add(handler)
        else:
            LOG_ERROR(b'Handler is invalid', handler)
        return
    ...
    def __sendAction(self, actionID, reqID, args=None):
        ...
        if not player:
            LOG_ERROR(b'Player is not defined')
            return False
    ...
    def __sendActionsFromQueue(self):
        ...
        while self.__queue:
            ...
            if actionID in invokedIDs:
                LOG_WARNING(b'Action is ignored, your must send action after event "showGUI" is invoked', actionID, reqID, args)
                continue
...
class ChatProviderReplayHelper(object):
    ...
    def __storeUnitChatActions(self, actionID, reqID, args):
        if actionID == _ACTIONS.INIT_UNIT_CHAT:
            args = dict(args)
            args[b'strArg1'] = transport.z_dumps([])
