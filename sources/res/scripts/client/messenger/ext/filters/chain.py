...
class FiltersChain(object):
    ...
    def addFilter(self, name, filterObj, order=-1, removed=None):
        ...
        if not inFilter and not outFilter or inFilter and outFilter:
            LOG_WARNING((b'Invalid filter (name = {0:>s}, object = {1!r:s}))').format(name, filterObj))
            return
        else:
            if inFilter and name not in self.__inFilterNames:
                if name in self.__outFilterNames:
                    LOG_WARNING((b'Invalid filter (name = {0:>s}, object = {1!r:s})).Filter name must be unique.').format(name, filterObj))
                    return
                self.__inFilters.append({b'name': name, 
                   b'filter': filterObj, 
                   b'order': (order if order > 0 else len(self.__inFilters) + 1), 
                   b'lock': False})
                LOG_DEBUG(b'Incoming filter added', name)
                for removedName in removed:
                    self.__doRemoveInFilter(removedName)

                self.__prepareInFilters()
            elif inFilter:
                LOG_DEBUG((b'Filter (name = {0:>s}, object = {1!r:s})) is already added to chain of incoming filters').format(name, filterObj))
            if outFilter and name not in self.__outFilterNames:
                if name in self.__inFilterNames:
                    LOG_WARNING((b'Invalid filter (name = {0:>s}, object = {1!r:s})).Filter name must be unique.').format(name, filterObj))
                    return
                self.__outFilters.append({b'name': name, 
                   b'filter': filterObj, 
                   b'order': (order if order > 0 else len(self.__outFilters) + 1), 
                   b'lock': False})
                LOG_DEBUG(b'Outgoing filter added', name)
                for removedName in removed:
                    self.__doRemoveOutFilter(removedName)

                self.__prepareOutFilters()
            elif outFilter:
                LOG_DEBUG((b'Filter (name = {0:>s}, object = {1!r:s})) is already added to chain of outgoing filters').format(name, filterObj))
            return
    ...
    def chainIn(self, senderID, text):
        for filterInfo in self.__inFilters:
            text = filterInfo[b'filter'].filter(senderID, text)

        return text

    def chainOut(self, text, limits):
        for filterInfo in self.__outFilters:
            text = filterInfo[b'filter'].filter(text, limits)

        return text

    def reset(self, contactId):
        if self.__getInFilterByName(b'floodFilter') is not None:
            self.__getInFilterByName(b'floodFilter')[b'filter'].resetHistory(contactId)
        return
    ...
    def __doRemoveInFilter(self, name):
        ...
        if name in self.__inFilterNames:
            ...
            if not self.__inFilters[idx][b'lock']:
                self.__inFilters.pop(idx)
                LOG_DEBUG(b'Incoming filter removed', name)
                result = True
            else:
                LOG_WARNING((b'Incoming filter (name = {0:>s}) can not remove.It is locked.').format(name))
        return result
    ...
    def __doRemoveOutFilter(self, name):
        ...
        if name in self.__outFilterNames:
            ...
            if not self.__outFilters[idx][b'lock']:
                self.__outFilters.pop(idx)
                LOG_DEBUG(b'Outgoing filter removed', name)
                result = True
            else:
                LOG_WARNING((b'Outgoing filter (name = {0:>s}) can not remove.It is locked.').format(name))
        return result
