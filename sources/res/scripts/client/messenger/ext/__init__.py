...
MESSENGER_OLDICT_FILE_PATH = b'text/messenger_oldictionary.xml'
MESSENGER_DOMAIN_FILE_PATH = b'text/messenger_dndictionary.xml'
...
def isBattleChatEnabled(common=False):
    ...
    arena = getattr(avatar, b'arena', None)
    if arena is None:
        LOG_ERROR(b'ClientArena not found')
        return result
    else:
        ...
        if guiType in constants.ARENA_GUI_TYPE.BATTLE_CHAT_SETTING_SUPPORTED:
            result = not (g_settings.userPrefs.disableBattleChat or avatar.battleChatRestriction[b'isBattleChatDisabled'] and not BattleReplay.isPlaying())
...
def isNotFriendSenderIgnored(user, areFriendsOnly):
    if user:
        if areFriendsOnly:
            if user.isFriend():
                return False
            LOG_DEBUG(b'Invite is ignored, shows invites from friends only', user)
            return True
        if user.isIgnored():
            LOG_DEBUG(b'Invite is ignored, there is the contact in ignore list', user)
            return True
    elif areFriendsOnly:
        LOG_DEBUG(b'Invite is ignored, shows invites from friends only', user)
