...
class _ColorScheme(defaultdict):
    ...
    def getHexStr(self, key):
        return (b'{0:06X}').format(self[key][self.__current])
    ...
    def iterHexs(self):
        for key, colors in self.iteritems():
            yield (
             key, (b'{0:06X}').format(colors[self.__current]))

        return


_ServiceChannelSettings = namedtuple(b'_ServiceChannelSettings', (b'highPriorityMsgLifeTime', b'highPriorityMsgAlphaSpeed', b'mediumPriorityMsgLifeTime', b'mediumPriorityMsgAlphaSpeed', b'stackLength', b'padding'))

class _LobbySettings(object):
    __slots__ = (b'serviceChannel', b'messageRawFormat', b'badWordFormat', b'__messageFormats')
    ...
    def getMessageFormat(self, key):
        try:
            ...
            LOG_ERROR(b'Message formatter not found', key)
            return self.messageRawFormat

        return

    def onSettingsLoaded(self, root):
        for key in (b'groups', b'rosters'):
            colorScheme = root.getColorScheme(key)
            for name, userColor in colorScheme.iterHexs():
                if name == b'breaker':
                    timeColor = colorScheme.getHexStr(b'other')
                else:
                    timeColor = userColor
                self.__messageFormats[name] = self.messageRawFormat % {b'user': userColor, 
                   b'time': timeColor}

        return


_BattleMessageLifeCycle = namedtuple(b'_MessageInBattle', (b'lifeTime', b'alphaSpeed'))

class _BattleSettings(object):
    __slots__ = (b'messageLifeCycle', b'messageFormat', b'targetFormat', b'inactiveStateAlpha', b'hintText', b'toolTipText', b'numberOfMessagesInHistory', b'receivers', b'alphaForLastMessages', b'chatIsLockedToolTipText', b'chatIsLockedParentalControlToolTipText', b'chatIsLockedCountryControlToolTipText', b'chatIsLockedOtherControlToolTipText', b'recoveredLatestMessages', b'lifeTimeRecoveredMessages', b'lastReceiver', b'toolTipTextWithMuteInfo', b'battleRoyaleTooltip')
    ...
    def __init__(self):
        ...
        self.targetFormat = b'%(target)s'
        self.inactiveStateAlpha = 100
        self.toolTipText = b''
        self.chatIsLockedToolTipText = b''
        self.chatIsLockedParentalControlToolTipText = b''
        self.chatIsLockedCountryControlToolTipText = b''
        self.chatIsLockedOtherControlToolTipText = b''
        self.toolTipTextWithMuteInfo = b''
        self.battleRoyaleTooltip = b''
...
_UserPrefs = namedtuple(b'_UserPrefs', (b'version', b'datetimeIdx', b'enableOlFilter', b'enableSpamFilter', b'invitesFromFriendsOnly', b'storeReceiverInBattle', b'disableBattleChat', b'chatContactsListOnly', b'receiveFriendshipRequest', b'receiveInvitesInBattle'))
...
class MessengerSettings(object):
    __slots__ = (b'__colorsSchemes', b'__messageFormatters', b'__eManager', b'__isUserPrefsInited', b'__defaultUserPrefsSnapshot', b'lobby', b'battle', b'userPrefs', b'htmlTemplates', b'msgTemplates', b'server', b'onUserPreferencesUpdated', b'onColorsSchemesUpdated')
    ...
    def __init__(self):
        ...
        self.htmlTemplates = XMLCollection(b'', b'')
        self.msgTemplates = MessageTemplates(b'', b'')
    ...
    def init(self):
        self.__colorsSchemes.update({b'groups': (_ColorScheme([b'default'])), 
           b'rosters': (_ColorScheme([b'online', b'offline'])), 
           b'contacts': (_ColorScheme([b'online', b'offline'])), 
           b'battle/player': (_ColorScheme([b'default', b'colorBlind'])), 
           b'battle/message': (_ColorScheme([b'default', b'colorBlind'])), 
           b'battle/receiver': (_ColorScheme([b'default', b'colorBlind']))})
    ...
    def getColorScheme(self, key):
        try:
            ...
            LOG_ERROR(b'Color scheme not found', key)
            return
    ...
    def __accs_onSettingsChanged(self, diff):
        if b'isColorBlind' in diff:
            result = False
            for colorScheme in self.__colorsSchemes.itervalues():
                csName = b'colorBlind' if diff[b'isColorBlind'] else b'default'
