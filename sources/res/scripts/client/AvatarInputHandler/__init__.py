...
_CTRLS_DESC_MAP = {(_CTRL_MODE.ARCADE): (
   ...
   (_CTRL_MODE.DUAL_GUN): (
                         control_modes.DualGunControlMode, b'sniperMode', _CTRL_TYPE.USUAL), 
   (_CTRL_MODE.TWIN_GUN): (
                         control_modes.TwinGunControlMode, b'sniperMode', _CTRL_TYPE.USUAL), 
   ...
   (_CTRL_MODE.PREBATTLE_HIGHLIGHTS): (
                                     control_modes.PrebattleHighlightsControlMode, b'videoMode', _CTRL_TYPE.OPTIONAL), 
   (_CTRL_MODE.VIDEO): (
   ...
   (_CTRL_MODE.DEATH_FREE_CAM): (
                               control_modes.DeathFreeCamMode, b'freeVideoMode', _CTRL_TYPE.USUAL), 
   (_CTRL_MODE.KILL_CAM): (
                         kill_cam_modes.KillCamMode, b'killCamMode', _CTRL_TYPE.USUAL), 
   (_CTRL_MODE.LOOK_AT_KILLER): (
                               kill_cam_modes.LookAtKillerMode, b'killCamMode', _CTRL_TYPE.USUAL), 
   (_CTRL_MODE.VEHICLES_SELECTION): (
                                   VehiclesSelectionControlMode, b'vehiclesSelection', _CTRL_TYPE.USUAL)}
...
class DynamicCameraSettings(object):
    ...
    def __init__(self, dataSec):
        self.__dynamic = {b'caliberImpulses': [], b'massSensitivity': []}
        ...
        if caliberSettings is not None:
            ...
            _logger.error(b'<caliberImpulses> dataSection is not found!')
        ...
        if sensitivitySettings is not None:
            ...
            _logger.error(b'<massSensitivity> dataSection is not found!')
...
class AvatarInputHandler(CallbackDelayer, ComponentController):
    ...
    isFlashBangAllowed = property((lambda self: self.__curCtrl not in (self.__ctrls[b'video'], self.__ctrls[b'killcam'])))
    ...
    def __init__(self):
        ...
        ComponentController.__init__(self, b'AvatarInputHandler')
    ...
    def activatePostmortem(self):
        ...
        if battleReplayInProgress or avatar.isObserver() or nextCtrlMode == _CTRL_MODE.POSTMORTEM:
            ...
            _logger.error(b'Attempt to go to an unsupported CTRL Mode %s', nextCtrlMode)
        return
    ...
    def deactivatePostmortem(self):
        if self.ctrlModeName not in _CTRL_MODE.POSTMORTEM_CONTROL_MODES:
            _logger.warning(b'Trying to deactivate postmortem when it is not active. Current mode: %s', self.ctrlModeName)
            return
    ...
    def onControlModeChanged(self, eMode, **kwargs):
        ...
        if BattleReplay.isPlaying() and eMode == _CTRL_MODE.PREBATTLE_HIGHLIGHTS:
            ...
            if eMode in _CTRL_MODE.KILL_CAM_MODES and hasattr(prevCtrl, b'camera'):
    ...
    def onObserverControlModeChanged(self, eMode):
        _logger.debug(b'onObserverControlModeChanged: %s %s ', eMode, self.isObserverFPV)
        if not self.isObserverFPV:
            ...
            _logger.debug(b'onObserverControlModeChanged: %s %s', eMode, targetPos)
    ...
    def _readCfg(self):
        sec = ResMgr.openSection(INPUT_HANDLER_CFG)
        if sec is None:
            _logger.error(b'can not open <%s>.', INPUT_HANDLER_CFG)
            return
        else:
    ...
    def __checkSections(self, section):
        for _, desc in _CTRLS_DESC_MAP.items():
            ...
            if not section.has_key(desc[1]):
                _logger.error(b'Invalid section <%s> in <%s>.', desc[1], INPUT_HANDLER_CFG)
    ...
    def isControlModeChangeAllowed(self):
        ...
        if BigWorld.time() - self.__lastSwitchTime < _CONTROL_MODE_SWITCH_COOLDOWN:
            _logger.warning(b'Control mode switch is on cooldown')
            return False
...
class _VertScreenshotCamera(object):
    ...
    def enable(self, doEnable):
        ...
        if not doEnable:
            ...
            _logger.debug(b'Vertical screenshot camera is disabled')
            return
        ...
        for name in self.__watcherNames:
            try:
                ...
                _logger.warning(b'Failed to get/set watcher %s', name)
        ...
        _logger.debug(b'Vertical screenshot camera is enabled')
        return
