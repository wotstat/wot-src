...
AUTOSHOOT_SHELL_TYPE = b'AUTOSHOOT'
INPUT_HANDLER_CFG = b'gui/avatar_input_handler.xml'
...
_CTRLS_DESC_MAP = {(_CTRL_MODE.ARCADE): (
                       control_modes.ArcadeControlMode, b'arcadeMode', _CTRL_TYPE.USUAL), 
   (_CTRL_MODE.STRATEGIC): (
                          control_modes.StrategicControlMode, b'strategicMode', _CTRL_TYPE.USUAL), 
   (_CTRL_MODE.ARTY): (
                     control_modes.ArtyControlMode, b'artyMode', _ARTY_CTRL_TYPE), 
   (_CTRL_MODE.SNIPER): (
                       control_modes.SniperControlMode, b'sniperMode', _CTRL_TYPE.USUAL), 
   (_CTRL_MODE.POSTMORTEM): (
                           control_modes.PostMortemControlMode, b'postMortemMode', _CTRL_TYPE.USUAL), 
   (_CTRL_MODE.DEBUG): (
                      control_modes.DebugControlMode, None, _CTRL_TYPE.DEVELOPMENT), 
   (_CTRL_MODE.CAT): (
                    control_modes.CatControlMode, None, _CTRL_TYPE.DEVELOPMENT), 
   (_CTRL_MODE.VIDEO): (
                      control_modes.VideoCameraControlMode, b'videoMode', _CTRL_TYPE.OPTIONAL), 
   (_CTRL_MODE.MAP_CASE): (
                         MapCaseMode.MapCaseControlMode, b'strategicMode', _CTRL_TYPE.USUAL), 
   (_CTRL_MODE.MAP_CASE_ARCADE): (
                                MapCaseMode.ArcadeMapCaseControlMode, b'arcadeMode', _CTRL_TYPE.USUAL), 
   (_CTRL_MODE.MAP_CASE_EPIC): (
                              MapCaseMode.EpicMapCaseControlMode, b'strategicMode', _CTRL_TYPE.USUAL), 
   (_CTRL_MODE.MAP_CASE_ARCADE_EPIC_MINEFIELD): (
                                               MapCaseMode.AracdeMinefieldControleMode, b'arcadeEpicMinefieldMode', _CTRL_TYPE.USUAL), 
   (_CTRL_MODE.RESPAWN_DEATH): (
                              RespawnDeathMode.RespawnDeathMode, b'postMortemMode', _CTRL_TYPE.USUAL), 
   (_CTRL_MODE.DEATH_FREE_CAM): (
                               epic_battle_death_mode.DeathFreeCamMode, b'epicVideoMode', _CTRL_TYPE.USUAL), 
   (_CTRL_MODE.DUAL_GUN): (
                         control_modes.DualGunControlMode, b'dualGunMode', _CTRL_TYPE.USUAL), 
...
class DynamicCameraSettings(object):
    ...
    def __init__(self, dataSec):
        self.__dynamic = {b'caliberImpulses': [], b'massSensitivity': [], b'autoShootImpulsesCoeffs': []}
        caliberSettings = dataSec[b'caliberImpulses']
        if caliberSettings is not None:
            self.__dynamic[b'caliberImpulses'] = self.__readRange(caliberSettings)
        else:
            LOG_ERROR(b'<caliberImpulses> dataSection is not found!')
        autoShootImpulsesCoeffs = dataSec[b'autoShootImpulsesCoeffs']
        if autoShootImpulsesCoeffs is not None:
            self.__dynamic[b'autoShootImpulsesCoeffs'] = self.__readRange(autoShootImpulsesCoeffs)
        else:
            LOG_ERROR(b'<autoShootImpulsesCoeffs> dataSection is not found!')
        sensitivitySettings = dataSec[b'massSensitivity']
        if sensitivitySettings is not None:
            self.__dynamic[b'massSensitivity'] = self.__readRange(sensitivitySettings)
        else:
            LOG_ERROR(b'<massSensitivity> dataSection is not found!')
        self.__dynamic[b'collisionSpeedToImpulseRatio'] = cameras.readFloat(dataSec, b'collisionSpeedToImpulseRatio', 0, 1000, 1.0)
        self.__dynamic[b'minCollisionSpeed'] = cameras.readFloat(dataSec, b'minCollisionSpeed', 0, 1000, 1.0)
        self.__dynamic[b'zeroDamageHitSensitivity'] = cameras.readFloat(dataSec, b'zeroDamageHitSensitivity', 0, 1000, 1.0)
        self.__dynamic[b'ownShotImpulseDelay'] = cameras.readFloat(dataSec, b'ownShotImpulseDelay', 0.0, 1000, 0.0)
        self.__dynamic[b'excludedShellTypes'] = dataSec.readString(b'excludedShellTypes').split()
        return

    def getGunImpulse(self, caliber):
        impulseMagnitude = 0.0
        for minCaliber, magnitude in self.__dynamic[b'caliberImpulses']:
    ...
    def getAutoShootGunImpulseCoeff(self, caliber):
        impulseCoeff = 0.0
        for minCaliber, coeff in self.__dynamic[b'autoShootImpulsesCoeffs']:
            if caliber < minCaliber:
                break
            impulseCoeff = coeff
    ...
    def getSensitivityToImpulse(self, vehicleMass):
        sensitivity = 0.0
        for mass, sense in self.__dynamic[b'massSensitivity']:
            if vehicleMass < mass:
                break
            sensitivity = sense
...
class AvatarInputHandler(CallbackDelayer, ScriptGameObject):
    ...
    isFlashBangAllowed = property((lambda self: self.__ctrls[b'video'] != self.__curCtrl))
    ...
    _DYNAMIC_CAMERAS_ENABLED_KEY = b'global/dynamicCameraEnabled'
    ...
    def __init__(self, spaceID):
        ...
        ScriptGameObject.__init__(self, spaceID, b'AvatarInputHandler')
    ...
    def __constructComponents(self):
        ...
        if not vehicle:
            _logger.debug(b'Attempt to construct components when vehicle is none')
            return
        else:
    ...
    def getDesiredShotPoint(self, ignoreAimingMode=False, ignoreDetached=False):
        if not ignoreDetached and self.__isDetached:
            return None
        else:
    ...
    def activatePostmortem(self, isRespawn):
        ...
        onPostmortemActivation = getattr(self.__curCtrl, b'onPostmortemActivation', None)
    ...
    def deactivatePostmortem(self):
        if self.ctrlModeName not in (_CTRL_MODE.POSTMORTEM, _CTRL_MODE.RESPAWN_DEATH, _CTRL_MODE.DEATH_FREE_CAM):
            LOG_WARNING(b'Trying to deactivate postmortem when it is not active. Current mode:', self.ctrlModeName)
            return
        self.onControlModeChanged(b'arcade')
        arcadeMode = self.__ctrls[b'arcade']
    ...
    def onControlModeChanged(self, eMode, **args):
        if self.steadyVehicleMatrixCalculator is not None:
            self.steadyVehicleMatrixCalculator.relinkSources()
        _logger.debug(b'onControlModeChanged %s', eMode)
        if not self.__isArenaStarted and not self.__isModeSwitchInPrebattlePossible(eMode):
            return
        else:
            player = BigWorld.player()
            isObserverMode = b'observer' in player.vehicleTypeDescriptor.type.tags if player is not None else True
    ...
    def onVehicleControlModeChanged(self, eMode):
        LOG_DEBUG(b'onVehicleControlModeChanged: ', eMode, self.isObserverFPV)
        if not self.isObserverFPV:
            ...
            LOG_DEBUG(b'onVehicleControlModeChanged: ', eMode, targetPos)
    ...
    def onVehicleShaken(self, vehicle, impulsePosition, impulseDir, caliber, shellType, shakeReason):
        if shellType in self.__dynamicCameraSettings.settings[b'excludedShellTypes']:
            return
        else:
            if shakeReason == _ShakeReason.OWN_SHOT_DELAYED:
                ...
                delayTime = self.__dynamicCameraSettings.settings[b'ownShotImpulseDelay']
                self.delayCallback(delayTime, shakeFuncBound)
                return
            camera = getattr(self.ctrl, b'camera', None)
            if camera is None:
                return
            ...
            if avatarVehicle is None or not avatarVehicle.isAlive():
                return
            avatarVehicleTypeDesc = getattr(avatarVehicle, b'typeDescriptor', None)
            if avatarVehicleTypeDesc is not None:
                avatarVehWeightTons = avatarVehicleTypeDesc.physics[b'weight'] / 1000.0
            ...
            if shakeReason == _ShakeReason.OWN_SHOT:
                ...
                if shakeReason in (_ShakeReason.HIT, _ShakeReason.HIT_NO_DAMAGE):
                    zeroDamageHitSensitivity = self.__dynamicCameraSettings.settings[b'zeroDamageHitSensitivity']
    ...
    def onVehicleCollision(self, vehicle, impactVelocity):
        if impactVelocity < self.__dynamicCameraSettings.settings[b'minCollisionSpeed']:
            return
        else:
            camera = getattr(self.ctrl, b'camera', None)
            if camera is None:
            ...
            if avatarVehicle is None or not avatarVehicle.isAlive():
                return
            if vehicle is avatarVehicle:
                impulse = Math.Vector3(0, impactVelocity * self.__dynamicCameraSettings.settings[b'collisionSpeedToImpulseRatio'], 0)
    ...
    def onVehicleDeath(self, vehicle, exploded):
        if not exploded:
            ...
            impulseValue = self.__dynamicCameraSettings.getGunImpulse(caliber)
            avatarVehicleWeightInTons = avatarVehicle.typeDescriptor.physics[b'weight'] / 1000.0
    ...
    def __identifyVehicleType(self):
        ...
        if veh is None:
            ...
            self.__isSPG = b'SPG' in vehTypeDesc.tags
            self.__isATSPG = b'AT-SPG' in vehTypeDesc.tags
            ...
            self.__isOnlyArty = b'spgOnlyArtyMode' in vehTypeDesc.tags
            return
    ...
    def reloadDynamicSettings(self):
        ...
        self.__dynamicCameraSettings = DynamicCameraSettings(sec[b'dynamicCameraCommon'])
        try:
            self.__ctrls[b'sniper'].camera.aimingSystem.reloadConfig(sec[b'sniperMode'][b'camera'])
        except Exception:
            pass
    ...
    def _readCfg(self):
        sec = ResMgr.openSection(INPUT_HANDLER_CFG)
        if sec is None:
            LOG_ERROR(b'can not open <%s>.' % INPUT_HANDLER_CFG)
            return
        else:
            self.__checkSections(sec)
            keySec = sec[b'keys']
            if keySec is not None:
                self.__showMarkersKey = getattr(Keys, keySec.readString(b'showMarkersKey', b''), None)
                self.__alwaysShowAimKey = getattr(Keys, keySec.readString(b'alwaysShowAimKey', b''), None)
            self.__dynamicCameraSettings = DynamicCameraSettings(sec[b'dynamicCameraCommon'])
            return sec
            ...
            try:
                ...
                if ctrlType != _CTRL_TYPE.DEVELOPMENT or ctrlType == _CTRL_TYPE.DEVELOPMENT and constants.HAS_DEV_RESOURCES:
                    if name not in self.__ctrls:
                        ...
                        _logger.warning(b'Control "%s" is already added to the list! Please check OVERWRITE_CTRLS_DESC_MAP. %s is skipped!', name, classType)
            except Exception:
                _logger.error(b'Error while setting ctrls %s %s %s', name, desc, constants.HAS_DEV_RESOURCES)
    ...
    def __checkSections(self, section):
        for _, desc in _CTRLS_DESC_MAP.items():
            ...
            if not section.has_key(desc[1]):
                LOG_ERROR(b'Invalid section <%s> in <%s>.' % (desc[1], INPUT_HANDLER_CFG))
    ...
    def __onSettingsChanged(self, diff):
        if b'dynamicCamera' in diff or b'horStabilizationSnp' in diff:
            dynamicCamera = self.settingsCore.getSetting(b'dynamicCamera')
            horStabilizationSnp = self.settingsCore.getSetting(b'horStabilizationSnp')
            self.enableDynamicCamera(dynamicCamera, horStabilizationSnp)
        if b'hullLockEnabled' in diff:
            hullLock = self.settingsCore.getSetting(b'hullLockEnabled')
    ...
    def __onRequestFail(self):
        player = BigWorld.player()
        if player is not None:
            player.showVehicleError(b'cantSwitchEngineDestroyed')
        return
    ...
    def isControlModeChangeAllowed(self):
        ...
        if BigWorld.time() - self.__lastSwitchTime < _CONTROL_MODE_SWITCH_COOLDOWN:
            LOG_WARNING(b'Control mode switch is on cooldown')
            return False
...
class _VertScreenshotCamera(object):
    ...
    def __init__(self):
        ...
        self.__farPlane = 0.0
        self.__watcherNames = (b'Render/Fov', b'Render/Near Plane', b'Render/Far Plane', b'Render/Objects Far Plane/Enabled', b'Render/Shadows/qualityPreset', b'Visibility/Draw tanks', b'Visibility/Control Points', b'Visibility/GUI', b'Visibility/particles', b'Visibility/Sky', b'Client Settings/Script tick', b'Render/Terrain/AdaptiveMesh/cascades enabled', b'Render/Water/out land water')
        return
    ...
    def enable(self, doEnable):
        ...
        if not doEnable:
            ...
            BigWorld.setWatcher(b'Render/Fog/enabled', True)
            BigWorld.setWatcher(b'Occlusion Culling/Enabled', True)
            BigWorld.setWatcher(b'Render/Terrain/AdaptiveMesh/cascades enabled', True)
            BigWorld.setWatcher(b'Render/Water/out land water', True)
            LOG_DEBUG(b'Vertical screenshot camera is disabled')
            return
        ...
        for name in self.__watcherNames:
            try:
                ...
                if name.startswith(b'Visibility'):
                    BigWorld.setWatcher(name, False)
            except TypeError:
                LOG_WARNING(b'Failed to get/set watcher', name)

        self.__nearPlane = BigWorld.projection().nearPlane
        self.__farPlane = BigWorld.projection().farPlane
        BigWorld.setWatcher(b'Render/Fog/enabled', False)
        ...
        BigWorld.setWatcher(b'Render/Shadows/qualityPreset', 4)
        BigWorld.setWatcher(b'Client Settings/Script tick', False)
        BigWorld.setWatcher(b'Occlusion Culling/Enabled', False)
        BigWorld.setWatcher(b'Render/Terrain/AdaptiveMesh/cascades enabled', False)
        BigWorld.setWatcher(b'Render/Water/out land water', False)
        LOG_DEBUG(b'Vertical screenshot camera is enabled')
        return


def getAvatarInputHandlerConfig(manager):
    manager.addInstance(IPlayerCacheController, PlayerCacheController(), finalizer=b'destroy')
    return
