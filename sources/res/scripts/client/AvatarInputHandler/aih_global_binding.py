...
class BINDING_ID(object):
    ...
    RANGE = (
     ...
     STRATEGIC_CAMERA, CHARGE_MARKER_STATE, SPG_SHOTS_INDICATOR_STATE,
     DUAL_ACC_GUN_MARKER_DATA_PROVIDER, CLIENT_ASSAULT_SPG_GUN_MARKER_DATA_PROVIDER,
     SERVER_ASSAULT_SPG_GUN_MARKER_DATA_PROVIDER, ASSAULT_SPG_CAMERA_STATE)
...
class _Observable(object):
    ...
    def subscribe(self, subscriber):
        if subscriber not in self.__subscribers:
            ...
            _logger.error(b'Subscriber %r already added to observable %r', subscriber, self)
        return
...
class _GlobalDataDescriptor(object):
    __slots__ = (b'__bindingID', b'__reader', b'__writer')
    __storage = {}
    ...
    def __set__(self, instance, value):
        if not self.__writer:
            raise AttributeError((b'{} can not set value of descriptor').format(instance))
        self.__storage[self.__bindingID].change(value)
        return

    def __get__(self, instance, _=None):
        if not self.__reader:
            raise AttributeError((b'{} can not get value of descriptor').format(instance))
        if instance is None:
    ...
    def subscribe(cls, bindingID, subscriber):
        if bindingID in cls.__storage:
            ...
            _logger.error(b'The bindingID %d is not found in the storage', bindingID)
        return
...
def bindRO(bindingID):
    if bindingID not in BINDING_ID.RANGE:
        raise SoftException((b'bindingID is invalid: {}').format(bindingID))
