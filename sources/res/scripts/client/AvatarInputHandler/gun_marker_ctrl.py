...
class _CrosshairShotResults(object):
    ...
    shellExtraData = namedtuple(b'shellExtraData', (b'hasNormalization', b'mayRicochet', b'checkCaliberForRicochet', b'jetLossPPByDist'))
    ...
    def getShotResult(cls, hitPoint, collision, direction, excludeTeam=0, piercingMultiplier=1):
        if collision is None:
            ...
            fullPiercingPower *= computeDistanceFactor(shell, dist, b'pierceFactor')
    ...
    def __sendDebugInfo(cls, piercingData, minPP=None, maxPP=None, fullPP=None):
        if constants.IS_DEVELOPMENT:
            g_eventBus.handleEvent(GunMarkerEvent(GunMarkerEvent.UPDATE_PIERCING_DATA, ctx={b'piercingData': piercingData, 
               b'minPP': minPP, 
               b'maxPP': maxPP, 
