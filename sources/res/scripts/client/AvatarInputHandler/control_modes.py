...
class VideoCameraControlMode(_GunControlMode):
    ...
    def __init__(self, dataSection, avatarInputHandler):
        ...
        locationXmlPath = b'spaces/' + BigWorld.player().arena.arenaType.geometryName + b'/locations.xml'
...
class ArcadeControlMode(_GunControlMode):
    ...
    def enable(self, **args):
        ...
        self._cam.enable(args.get(b'preferredPos'), args.get(b'closesDist', False), turretYaw=args.get(b'turretYaw'), gunPitch=args.get(b'gunPitch'), initialVehicleMatrix=args.get(b'initialVehicleMatrix'), arcadeState=args.get(b'arcadeState'), camTransitionParams=args.get(b'camTransitionParams', {}))
        player = BigWorld.player()
...
class MultiGunGunControlMode(SniperControlMode):
    ...
    def _setupCamera(self, dataSection):
        self._cam = self._CAMERA_CLS(dataSection[b'camera'], defaultOffset=self._defaultOffset, binoculars=self._binoculars)
        return
...
class PostMortemControlMode(IControlMode, CallbackDelayer):
    ...
    def __init__(self, dataSection, avatarInputHandler):
        ...
        self.__transitionDuration = dataSection[b'camera'].readFloat(b'linearTransitionDuration')
    ...
    def enable(self, **args):
        ...
        if player is None:
            ...
            self.__isObserverMode = b'observer' in player.vehicleTypeDescriptor.type.tags
            newVehicleID = args.get(b'newVehicleID')
            ...
            shouldSwitchToAlly = bool(args.get(b'immediateSwitchToAllyVehicle', False))
            camDuration = args.get(b'transitionDuration', -1)
            camMatrix = args.get(b'camMatrix')
            hasDuration = self.__hasCameraTransitionDuration()
            keepRotation = args.get(b'prevModeName', b'') == CTRL_MODE_NAME.DEATH_FREE_CAM
            camTransitionParams = {b'cameraTransitionDuration': (camDuration if hasDuration and not shouldSwitchToAlly else 0), 
               b'camMatrix': camMatrix, 
               b'keepRotation': (keepRotation or args.get(b'keepCameraSettings', False)), 
               b'newVehicleID': newVehicleID, 
               b'pivotSettings': (args.get(b'pivotSettings')), 
               b'distanceFromFocus': (args.get(b'distanceFromFocus'))}
            ...
            isDelayRequired = bool(args.get(b'bPostmortemDelay'))
    ...
    def __getVehicleMatrix(self, vehicleID):
        ...
        if not vehicle:
            ...
            if vehicle.appearance.compoundModel is not None:
                vehicleMatrix = vehicle.appearance.compoundModel.node(b'HP_gunJoint')
...
def _isEnabledChangeModeByScroll(camera, aih):
    return not camera.getUserConfigValue(b'sniperModeByShift') or aih.isObserverFPV
...
class DeathFreeCamMode(VideoCameraControlMode, CallbackDelayer):
    ...
    def __init__(self, dataSection, avatarInputHandler):
        ...
        self.__curVehicleID = None
        self.__disableHints = int(dataSection.readFloat(b'disableHints'))
    ...
    def canObserveVehicle(player, vehicleID):
        ...
        if vehicleData is None:
            ...
            isVehicleAlive = vehicleData.get(b'isAlive', False)
            vehicleTeam = vehicleData.get(b'team', 0)
            vehicleDescription = vehicleData.get(b'vehicleType', None)
            vehicleType = vehicleDescription.type if vehicleDescription is not None else None
            isUnobservable = b'unobservable' in vehicleType.tags if vehicleType is not None else False
    ...
    def enable(self, **args):
        ...
        curVehicleID = args.get(b'curVehicleID')
        if curVehicleID != -1:
            self.__curVehicleID = curVehicleID
            self.delayCallback(args.get(b'transitionDuration', 0), self.guiSessionProvider.shared.feedback.showVehicleMarker, curVehicleID)
    ...
    def __getAliveAllies(self):
        ...
        sortedIds = {b'leftItemsIDs': [], b'rightItemsIDs': []}
        sortedIds = sortedIdComposer.compose(sortedIds)
        aliveAllies = [vID for vID in sortedIds[b'leftItemsIDs'] if BigWorld.player().arena.vehicles.get(vID)[b'isAlive']]
        return aliveAllies
    ...
    def __changeVehicle(self, vehicleID, transitionTime=-1):
        ...
        if targetMode == CTRL_MODE_NAME.POSTMORTEM:
            ...
            _logger.debug(b'Unsupported Ctrl mode switch attempt %s', targetMode)
        return
