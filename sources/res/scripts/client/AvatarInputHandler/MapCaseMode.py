...
class _FLMinesSensor(object):
    ...
    def isIntersectMine(self):
        allyMines = [e for e in BigWorld.entities.values() if (e.__class__.__name__ == b'BasicMine' or e.__class__.__name__ == b'FLBasicMine') and self._sessionProvider.getArenaDP().isAlly(e.ownerVehicleID)]
...
class _Comp7ArenaBoundPlaneStrikeSelector(_Comp7ArenaBoundArtilleryStrikeSelector):
    ...
    def __init__(self, position, equipment):
        ...
        if direction is None:
            _logger.error(b'Missing flyDirection for arena [geometryName=%s, gameplayName=%s]; teamID=%s', arenaType.geometryName, arenaType.gameplayName, player.team)
...
class MapCaseControlModeBase(IControlMode, CallbackDelayer):
    ...
    def enable(self, **args):
        ...
        arcadeState = args.get(b'arcadeState', None)
        ...
        equipmentID = args.get(b'equipmentID', None)
