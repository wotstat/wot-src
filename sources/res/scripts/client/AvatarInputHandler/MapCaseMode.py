...
class _VehiclesSelector(object):
    ...
    def _validateVehicle(self, vehicle):
        return hasattr(vehicle, b'isStarted') and vehicle.isStarted and vehicle.isAlive() and (not vehicle.isPlayerVehicle or self.__selectPlayer)
...
class _FLMinesSensor(object):
    ...
    def isIntersectMine(self):
        allyMines = [e for e in BigWorld.entities.values() if e.__class__.__name__ == b'BasicMine' and self._sessionProvider.getArenaDP().isAlly(e.ownerVehicleID)]
...
class _ArenaBoundsAreaStrikeSelector(_AreaStrikeSelector):
    ...
    def __init__(self, position, equipment, direction=_DEFAULT_STRIKE_DIRECTION):
        ...
        color = None
        aimLimits = getattr(equipment, b'arenaAimLimits', None)
        if aimLimits:
...
_STRIKE_SELECTORS = {(artefacts.RageArtillery): _ArtilleryStrikeSelector, (artefacts.RageBomber): _BomberStrikeSelector, 
...
class MapCaseControlModeBase(IControlMode, CallbackDelayer):
    ...
    def __init__(self, dataSection, avatarInputHandler):
        ...
        self.__cam = self._createCamera(dataSection[b'camera'], dataSection.readVector2(b'defaultOffset'))
    ...
    def enable(self, **args):
        ...
        arcadeState = args.get(b'arcadeState')
        ...
        if arcadeState is None and any(cls == type(overridePreferredPosition) for cls in (Vector3, tuple)):
            ...
            targetPos = args.get(b'preferredPos', Vector3(0, 0, 0))
        if self._acceptsArcadeState:
            self.__cam.enable(targetPos, args.get(b'saveDist', arcadeState is None), arcadeState=arcadeState)
        else:
            self.__cam.enable(targetPos, args.get(b'saveDist', True))
        self.__aimingMode = args.get(b'aimingMode', self.__aimingMode)
        ...
        equipmentID = args.get(b'equipmentID')
    ...
    def handleKeyEvent(self, isDown, key, mods, event=None):
        ...
        if cmdMap.isFired(CommandMapping.CMD_CM_ALTERNATE_MODE, key) and isDown:
            ...
            if replayCtrl.isPlaying:
                ...
                arcadeMode = BigWorld.player().inputHandler.ctrls.get(b'arcade', None)
    ...
    def onMinimapClicked(self, worldPos):
        teleport = getattr(self.__cam, b'teleport', None)
    ...
    def _setAimingLimits(self, equipment):
        if not isinstance(equipment, ArcadeEquipmentConfigReader) or equipment.maxApplyRadius <= 0:
            ...
            if not vehicle:
                _logger.warning(b'[MapCase] Can not set aiming circle visible, no vehicle attached.')
                return
            dynamicObjects = self._dynamicObjectsCache.getConfig(player.arenaGuiType)
            if dynamicObjects is None:
                _logger.warning(b'[MapCase] Can not set aiming circle visible, no visual registered.')
                return
            aimingCircleSettings = dynamicObjects.getAimingCircleRestrictionEffect(equipment).get(b'ally')
            if aimingCircleSettings is None:
                _logger.warning(b'[MapCase] Can not set aiming circle visible, visual without <ally> section.')
                return
            if vehicle.appearance.isTerrainCircleVisible:
                _logger.warning(b'[MapCase] Can not set aiming circle visible, other circle terrain visible.')
                return
    ...
    def _clearAimingLimits(self):
        ...
        if not self.__aimingCircleTerrainActivated:
            ...
            if not vehicle:
                _logger.warning(b'[MapCase] Can not hide aiming circle, no vehicle attached.')
                return
            if not vehicle.appearance.isTerrainCircleVisible:
                _logger.debug(b'[MapCase] Aiming circle, already hidden.')
                return
    ...
    def turnOff(self, sendStopEquipment=True):
        ...
        if BigWorld.player().observerSeesAll():
            if not prevMode[self._MODE_NAME] or prevMode[self._MODE_NAME] == self.__aih.ctrlModeName:
                LOG_WARNING(b'Skip switching to previouse mode', prevMode[self._MODE_NAME])
                return
    ...
    def activateEquipment(self, equipmentID, preferredPos=None):
        ...
        if equipment.clientSelector:
            strikeSelectorConstructor = importClass(equipment.clientSelector, b'AvatarInputHandler.MapCaseMode')
        else:
            typeEq = type(equipment)
            if typeEq in _STRIKE_SELECTORS:
                strikeSelectorConstructor = _STRIKE_SELECTORS.get(type(equipment))
            else:
                cd = makeIntCompactDescrByID(b'equipment', NONE_INDEX, equipmentID)
        ...
        if strikeSelectorConstructor is None:
            LOG_ERROR(b'Cannot use equipment with id', equipmentID)
            return
        else:
            self.__activeSelector.destroy()
...
def activateMapCase(equipmentID, deactivateCallback, controlMode):
    ...
    if isinstance(inputHandler.ctrl, controlMode):
        ...
        if currentMode in MAP_CASE_MODES:
            _logger.warning(b'MapCaseMode is active! Attempt to switch MapCaseModes simultaneously!')
            return
        controlMode.deactivateCallback = deactivateCallback
        arcadeState = None
