...
class CollisionStrategy(object):
    ...
    def getCollisionFunction(cls, collisionStrategy):
        if collisionStrategy == CollisionStrategy.COLLIDE_DYNAMIC_AND_STATIC:
            ...
            _logger.error(b'CollisionStrategy: getCollisionFunction: Unknown strategy %i.', collisionStrategy)
            return
...
def getTurretYawGunPitch(vehTypeDescr, vehicleMatrix, targetPos, compensateGravity=False):
    ...
    gravity = (compensateGravity or vehTypeDescr.shot).gravity if 1 else 0.0
...
def shootInSkyPointHelper():
    ...
    if vehicle is not None and vehicle.inWorld and vehicle.isStarted and not vehicle.isTurretDetached:
        ...
        vType = BigWorld.player().arena.vehicles[BigWorld.player().playerVehicleID][b'vehicleType']
...
def shootInSkyPoint(startPos, direction):
    ...
    if quadraticSubEquation >= 0:
        ...
        _logger.info(b"The point at distance from vehicle's gun can't be calculated. The maximum distance is set.")
        dist = shotDesc.maxDistance
...
def getVehicleAngles(vehicleDescriptor):
    ...
    gunAngleMin = gunLimits[b'minPitch'][0][1]
    gunAngleMax = gunLimits[b'maxPitch'][0][1]
    hullAngleMin = vehicleDescriptor.type.hullAimingParams[b'pitch'][b'wheelsCorrectionAngles'][b'pitchMin']
    hullAngleMax = vehicleDescriptor.type.hullAimingParams[b'pitch'][b'wheelsCorrectionAngles'][b'pitchMax']
    return (
