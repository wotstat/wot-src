...
class VideoCamera(ICamera, CallbackDelayer, TimeDeltaMeter):
    ...
    def _handleKeySwitches(self, key, isDown):
        if isDown:
            if self._keySwitches[b'keySwitchInertia'] == key:
                self._inertiaEnabled = not self._inertiaEnabled
                return True
            if self._keySwitches[b'keySwitchRotateAroundPoint'] == key:
                self.__rotateAroundPointEnabled = not self.__rotateAroundPointEnabled
                return True
            if self._keySwitches[b'keySwitchLandCamera'] == key:
                ...
                return True
            if self._keySwitches[b'keySetDefaultFov'] == key:
                BigWorld.projection().fov = self.__defaultFov
                return True
            if self._keySwitches[b'keySetDefaultRoll'] == key:
                self._ypr.z = 0.0
                return True
            if self._keySwitches[b'keyRevertVerticalVelocity'] == key:
                self.__isVerticalVelocitySeparated = False
                return True
            if self._keySwitches[b'keyBindToVehicle'] == key:
    ...
    def _readCfg(self, configDataSec):
        ...
        self._keySwitches[b'keyRevertVerticalVelocity'] = getattr(Keys, configDataSec.readString(b'keyRevertVerticalVelocity', b'KEY_Z'))
        ...
        rotationMappings[getattr(Keys, configDataSec.readString(b'keyRotateLeft', b'KEY_LEFTARROW'))] = Vector3(-1, 0, 0)
        rotationMappings[getattr(Keys, configDataSec.readString(b'keyRotateRight', b'KEY_RIGHTARROW'))] = Vector3(1, 0, 0)
        rotationMappings[getattr(Keys, configDataSec.readString(b'keyRotateUp', b'KEY_UPARROW'))] = Vector3(0, -1, 0)
        rotationMappings[getattr(Keys, configDataSec.readString(b'keyRotateDown', b'KEY_DOWNARROW'))] = Vector3(0, 1, 0)
        rotationMappings[getattr(Keys, configDataSec.readString(b'keyRotateClockwise', b'KEY_HOME'))] = Vector3(0, 0, -1)
        rotationMappings[getattr(Keys, configDataSec.readString(b'keyRotateCClockwise', b'KEY_END'))] = Vector3(0, 0, 1)
        self._readRotationSettings(configDataSec, rotationMappings)
        self._keySwitches[b'keySetDefaultRoll'] = getattr(Keys, configDataSec.readString(b'keySetDefaultRoll', b'KEY_R'))
        zoomMappings = dict()
        zoomMappings[getattr(Keys, configDataSec.readString(b'keyZoomGrowUp', b'KEY_INSERT'))] = -1
        zoomMappings[getattr(Keys, configDataSec.readString(b'keyZoomGrowDown', b'KEY_DELETE'))] = 1
        self._readZoomSettings(configDataSec, zoomMappings)
        self._keySwitches[b'keySwitchInertia'] = getattr(Keys, configDataSec.readString(b'keySwitchInertia', b'KEY_P'))
        ...
        self._keySwitches[b'keySwitchRotateAroundPoint'] = getattr(Keys, configDataSec.readString(b'keySwitchRotateAroundPoint', b'KEY_C'))
        aroundPointMappings = dict()
        aroundPointMappings[getattr(Keys, configDataSec.readString(b'keyTargetRadiusIncrement', b'KEY_NUMPAD7'))] = 1
        aroundPointMappings[getattr(Keys, configDataSec.readString(b'keyTargetRadiusDecrement', b'KEY_NUMPAD1'))] = -1
        aroundPointRadiusVelocity = configDataSec.readFloat(b'targetRadiusVelocity', 3.0)
        ...
        self._keySwitches[b'keySwitchLandCamera'] = getattr(Keys, configDataSec.readString(b'keySwitchLandCamera', b'KEY_L'))
        self._keySwitches[b'keySetDefaultFov'] = getattr(Keys, configDataSec.readString(b'keySetDefaultFov', b'KEY_F'))
        self._keySwitches[b'keyBindToVehicle'] = getattr(Keys, configDataSec.readString(b'keyBindToVehicle', b'KEY_B'), None)
        predefVelocitySec = configDataSec[b'predefinedVelocities']
        ...
        predefVelocitySec = configDataSec[b'predefinedVerticalVelocities']
        if predefVelocitySec is not None:
            for v in predefVelocitySec.items():
                key = getattr(Keys, v[0], None)
                if key is not None:
    ...
    def _readMovementSettings(self, configDataSec):
        ...
        movementMappings[getattr(Keys, configDataSec.readString(b'keyMoveLeft', b'KEY_A'))] = Vector3(-1, 0, 0)
        movementMappings[getattr(Keys, configDataSec.readString(b'keyMoveRight', b'KEY_D'))] = Vector3(1, 0, 0)
        keyMoveUp = getattr(Keys, configDataSec.readString(b'keyMoveUp', b'KEY_PGUP'))
        keyMoveDown = getattr(Keys, configDataSec.readString(b'keyMoveDown', b'KEY_PGDN'))
        movementMappings[keyMoveUp] = Vector3(0, 1, 0)
        movementMappings[keyMoveDown] = Vector3(0, -1, 0)
        movementMappings[getattr(Keys, configDataSec.readString(b'keyMoveForward', b'KEY_W'))] = Vector3(0, 0, 1)
        movementMappings[getattr(Keys, configDataSec.readString(b'keyMoveBackward', b'KEY_S'))] = Vector3(0, 0, -1)
        linearSensitivity = configDataSec.readFloat(b'linearVelocity', 40.0)
        linearSensitivityAcc = configDataSec.readFloat(b'linearVelocityAcceleration', 30.0)
        linearIncDecKeys = (
         getattr(Keys, configDataSec.readString(b'keyLinearVelocityIncrement', b'KEY_I')),
         getattr(Keys, configDataSec.readString(b'keyLinearVelocityDecrement', b'KEY_K')))
    ...
    def _readRotationSettings(self, configDataSec, rotationMappings):
        ...
        rotationSensitivityAcc = configDataSec.readFloat(b'angularVelocityAcceleration', 0.8)
        rotationIncDecKeys = (
         getattr(Keys, configDataSec.readString(b'keyAngularVelocityIncrement', b'KEY_O')),
         getattr(Keys, configDataSec.readString(b'keyAngularVelocityDecrement', b'KEY_L')))
    ...
    def _readZoomSettings(self, configDataSec, zoomMappings):
        zoomSensitivity = configDataSec.readFloat(b'zoomVelocity', 2.0)
        zoomSensitivityAcc = configDataSec.readFloat(b'zoomVelocityAcceleration', 1.5)
        zoomIncDecKeys = (
         getattr(Keys, configDataSec.readString(b'keyZoomVelocityIncrement', b'KEY_NUMPADMINUS')),
         getattr(Keys, configDataSec.readString(b'keyZoomVelocityDecrement', b'KEY_ADD')))
