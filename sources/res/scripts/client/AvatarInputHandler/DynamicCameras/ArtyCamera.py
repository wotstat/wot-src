...
class ArtyCamera(CameraWithSettings, CallbackDelayer):
    ...
    def _createAimingSystem():
        if BigWorld.player().isObserver():
            return ArtyAimingSystemRemote()
        return ArtyAimingSystem()
    ...
    def create(self, onChangeControlMode=None):
        ...
        self.__positionHysteresis = PositionHysteresis(self._cfg[b'hPositionThresholdSq'])
        self.__timeHysteresis = TimeHysteresis(self._cfg[b'hTimeThreshold'])
    ...
    def enable(self, targetPos, saveDist, switchToPos=None, switchToPlace=None):
        ...
        if switchToPlace == SwitchToPlaces.TO_TRANSITION_DIST:
            self.__camDist = math_utils.clamp(self._cfg[b'distRange'][0], self._cfg[b'distRange'][1], self._cfg[b'transitionDist'])
        elif switchToPlace == SwitchToPlaces.TO_RELATIVE_POS and switchToPos is not None:
            minDist, maxDist = self._cfg[b'distRange']
            ...
            self.__camDist = math_utils.clamp(self._cfg[b'distRange'][0], self._cfg[b'transitionDist'], self.__camDist)
        self.__desiredCamDist = self.__camDist
        ...
        self.__rotation = max(self._aimingSystem.direction.pitch, self._cfg[b'minimalPitch'])
    ...
    def __choosePitchLevel(self, aimPoint):
        useHighPitch = (aimPoint - self._aimingSystem.hitPoint).lengthSquared > self._cfg[b'highPitchThresholdSq']
        if useHighPitch:
    ...
    def __getCollisionTestOrigin(self, aimPoint, direction):
        ...
        if direction.x * direction.x > direction.z * direction.z and not math_utils.almostZero(direction.x):
            ...
            LOG_WARNING(b"Can't find point on direction ray. Using half point as collision test origin")
    ...
    def __calculateIdealState(self, deltaTime):
        ...
        impactPitch = max(direction.pitch, self._cfg[b'minimalPitch'])
    ...
    def __interpolateStates(self, deltaTime, rotation, desiredDistance, collisionDist):
        lerpParam = math_utils.clamp(0.0, 1.0, deltaTime * self._cfg[b'interpolationSpeed'])
        collisionLerpParam = math_utils.clamp(0.0, 1.0, deltaTime * self._cfg[b'distInterpolationSpeed'])
    ...
    def __cameraUpdate(self):
        ...
        if self.__onChangeControlMode is not None and self.__switchers.needToSwitch(self.__dxdydz.z, self.__desiredCamDist, self._cfg[b'distRange'][0], self._cfg[b'distRange'][1], self._cfg[b'transitionDist']):
            self.__onChangeControlMode(*self.__switchers.getSwitchParams())
        if not self.__transitionEnabled and self.__desiredCamDist - TRANSITION_DIST_HYSTERESIS <= self._cfg[b'transitionDist']:
            self.__transitionEnabled = True
    ...
    def __switchersDistRange(self):
        distRange = self._cfg[b'distRange']
        if self.__switchers.isEnabled():
            distRange = self.__switchers.getDistLimitationForSwitch(distRange[0], distRange[1], self._cfg[b'transitionDist'])
        return distRange

    def _readConfigs(self, dataSec):
        if not dataSec:
            LOG_WARNING(b'Invalid section <artyMode/camera> in avatar_input_handler.xml')
        super(ArtyCamera, self)._readConfigs(dataSec)
        dynamicSection = dataSec[b'dynamics']
        self.__dynamicCfg.readImpulsesConfig(dynamicSection)
        self.__positionOscillator = createOscillatorFromSection(dynamicSection[b'oscillator'], False)
        self.__positionNoiseOscillator = createOscillatorFromSection(dynamicSection[b'randomNoiseOscillatorFlat'])
        return
    ...
    def _readBaseCfg(self, dataSec):
        ...
        bcfg[b'scrollSensitivity'] = readFloat(dataSec, b'scrollSensitivity', 0.005, 10.0, 0.025)
        bcfg[b'angularSpeed'] = readFloat(dataSec, b'angularSpeed', pi / 720.0, pi / 0.5, pi / 360.0)
        bcfg[b'distRange'] = readVec2(dataSec, b'distRange', (1.0, 1.0), (10000.0, 10000.0), (2.0, 30.0))
        bcfg[b'transitionDist'] = readFloat(dataSec, b'transitionDist', 1.0, 10000.0, 60.0)
        bcfg[b'minimalPitch'] = readFloat(dataSec, b'minimalPitch', pi / 36.0, pi / 3.0, pi / 18.0)
        bcfg[b'maximalPitch'] = readFloat(dataSec, b'maximalPitch', pi / 6.0, pi / 3.0, pi / 3.5)
        bcfg[b'interpolationSpeed'] = readFloat(dataSec, b'interpolationSpeed', 0.0, 100.0, 5.0)
        bcfg[b'distInterpolationSpeed'] = readFloat(dataSec, b'distInterpolationSpeed', 0.0, 10.0, 5.0)
        bcfg[b'highPitchThreshold'] = readFloat(dataSec, b'highPitchThreshold', 0.1, 10.0, 3.0)
        bcfg[b'hTimeThreshold'] = readFloat(dataSec, b'hysteresis/timeThreshold', 0.0, 10.0, 0.5)
        bcfg[b'hPositionThreshold'] = readFloat(dataSec, b'hysteresis/positionThreshold', 0.0, 100.0, 7.0)
    ...
    def _readUserCfg(self):
        ...
        if dataSec is not None:
            dataSec = dataSec[b'artyMode/camera']
    ...
    def _makeCfg(self):
        ...
        cfg[b'angularSpeed'] = bcfg[b'angularSpeed']
        cfg[b'minimalPitch'] = bcfg[b'minimalPitch']
        cfg[b'maximalPitch'] = bcfg[b'maximalPitch']
        cfg[b'interpolationSpeed'] = bcfg[b'interpolationSpeed']
        cfg[b'distInterpolationSpeed'] = bcfg[b'distInterpolationSpeed']
        cfg[b'highPitchThresholdSq'] = bcfg[b'highPitchThreshold'] * bcfg[b'highPitchThreshold']
        cfg[b'hTimeThreshold'] = bcfg[b'hTimeThreshold']
        cfg[b'hPositionThresholdSq'] = bcfg[b'hPositionThreshold'] * bcfg[b'hPositionThreshold']
    ...
    def __enableSwitchers(self, updateTransitionEnabled=True):
        if updateTransitionEnabled and self.__desiredCamDist - TRANSITION_DIST_HYSTERESIS >= self._cfg[b'transitionDist']:
