...
def _boolFromString(value):
    return value.lower() in (b'yes', b'true', b'1')
...
def _registerLocalConstantWatcher(constants, name, convert):
    ...
    watcherName = name
    if watcherName.startswith(b'_'):
        watcherName = watcherName[1:]
    BigWorld.addWatcher(b'KillCam/' + watcherName, getValue, setValue)
    return


_registerLocalConstantWatcher(locals(), b'_MIN_TIME_TO_PLAYER', _floatFromString)
_registerLocalConstantWatcher(locals(), b'_MAX_TIME_TO_PLAYER', _floatFromString)
_registerLocalConstantWatcher(locals(), b'_MIN_DISTANCE_TO_PLAYER', _floatFromString)
_registerLocalConstantWatcher(locals(), b'_MAX_DISTANCE_TO_PLAYER', _floatFromString)
_registerLocalConstantWatcher(locals(), b'_FINISH_MOVE_DEGREE', _floatFromString)
_registerLocalConstantWatcher(locals(), b'_LOOK_AT_KILLER_PITCH_OFFSET', _floatFromString)
_registerLocalConstantWatcher(locals(), b'_LOOK_AT_KILLER_YAW_OFFSET', _floatFromString)
_registerLocalConstantWatcher(locals(), b'LOOK_AT_KILLER_DURATION', _floatFromString)
_registerLocalConstantWatcher(locals(), b'_LOOK_AT_KILLER_TRANSITION', _floatFromString)
_registerLocalConstantWatcher(locals(), b'_RESUME_TIME_SPEED', _floatFromString)
_registerLocalConstantWatcher(locals(), b'_SPOTTED_TRAJECTORY_DELAY', _floatFromString)
_registerLocalConstantWatcher(locals(), b'_KILLER_MARKER_DELAY', _floatFromString)
_registerLocalConstantWatcher(locals(), b'_HIDE_BUSHES_DURING_TRAJECTORY', _boolFromString)

class StartCamDirection(Enum):
    AWAY_FROM_TARGET = b'fromKiller'
    TOWARDS_TARGET = b'towardsKiller'
...
class KillCamera(ArcadeCamera):
    ...
    def __init__(self, dataSec, defaultOffset=None):
        ...
        self.__spinManagerConfig = dataSec[b'cameraSpinParams']
        ...
        self.__configCamDistance = dataSec.readFloat(b'cameraDistance')
        self.__configPivotSettings = (dataSec.readFloat(b'heightAboveBase'), dataSec.readFloat(b'focusRadius'))
        self.__cameraCollisionScaleMult = dataSec[b'advancedCollider'].readFloat(b'cameraCollisionScaleMult')
        self.__configMovementDuration = dataSec.readFloat(b'killCamMovementDuration')
        self.__totalSpinDuration = self.__spinManagerConfig.readFloat(b'spinDuration')
        self.__spinVectorAttacker = self.__spinManagerConfig.readVector2(b'spinAnglesAttacker')
        self.__spinVectorPlayer = self.__spinManagerConfig.readVector2(b'spinAnglesPlayer')
        distRange = dataSec.readVector2(b'pauseDistRange')
        self.__pauseDistRange = MinMax(distRange.x, distRange.y)
        self.__pauseAngleRange = dataSec.readVector2(b'pauseAngleRange')
        ...
        self.__defaultAngleRange = self._cfg[b'angleRange']
        self.__defaultDistanceRange = self._cfg[b'distRange']
        self.__timeInMovement = 0.0
        self.__prevTime = 0.0
        spottedSec = dataSec[b'spotted']
        unspottedSec = dataSec[b'unspotted']
        self.__phase3EndAngleHE = dataSec.readFloat(b'phase3EndAngleHE')
        self.__spottedKillerRadius = spottedSec.readFloat(b'spottedKillerRadius')
        self.__spottedPlayerRadius = spottedSec.readFloat(b'spottedPlayerRadius')
        self.__spottedPlayerHERadiusOffset = spottedSec.readFloat(b'spottedPlayerHERadiusOffset')
        self.__spottedKillerExplosionPitchOffset = spottedSec.readFloat(b'spottedKillerExplosionPitchOffset')
        self.__spottedKillerExplosionYawOffset = spottedSec.readFloat(b'spottedKillerExplosionYawOffset')
        self.__spottedPhase1MarkerFadeOut = 1
        self.__spottedPhase2MarkerFadeOut = 1
        self.__trajectoryDelay = spottedSec.readFloat(b'trajectoryDelay')
        self.__finalSpinDelay = spottedSec.readFloat(b'finalSpinDelay')
        self.__unspottedRadius = unspottedSec.readFloat(b'unspottedRadius')
        self.__unspottedHERadiusOffset = unspottedSec.readFloat(b'unspottedHERadiusOffset')
        self.__unspottedInitAngle = unspottedSec.readFloat(b'unspottedInitAngle')
        self.__unspottedPhase1Angle = unspottedSec.readFloat(b'unspottedPhase1Angle')
        self.__unspottedToPhase1SpinDuration = unspottedSec.readFloat(b'unspottedToPhase1SpinDuration')
        self.__unspottedPhase1FreezeDuration = unspottedSec.readFloat(b'unspottedPhase1FreezeDuration')
        self.__unspottedPhase2Angle = unspottedSec.readFloat(b'unspottedPhase2Angle')
        self.__unspottedToPhase2SpinDuration = unspottedSec.readFloat(b'unspottedToPhase2SpinDuration')
        self.__unspottedPhase2FreezeDuration = unspottedSec.readFloat(b'unspottedPhase2FreezeDuration')
        self.__unspottedPhase3Angle = unspottedSec.readFloat(b'unspottedPhase3Angle')
        self.__unspottedToPhase3SpinDuration = unspottedSec.readFloat(b'unspottedToPhase3SpinDuration')
        self.__unspottedPhase3FreezeDuration = unspottedSec.readFloat(b'unspottedPhase3FreezeDuration')
        self.__unspottedPitchOffset = unspottedSec.readFloat(b'unspottedPitchOffset')
        return
    ...
    def isNonPenetratingExplosion(self):
        return self.__projectileData[b'isExplosion'] and not self.hasProjectilePierced
    ...
    def startKillerVision(self, killerVehicleID, playerVehicleID, isRicochet, projData, onEndingCallback=(lambda *args: None)):
        ...
        if killerVehicleID is None:
            ...
            _logger.debug(b"Can't move camera to killer vehicle")
            return
    ...
    def startPlayerVision(self, projData, onEndingCallback=(lambda *args: None)):
        ...
        lastPoint = self.__trajectoryPoints[-1]
        projHitPoint = self.__projectileData[b'impactPoint']
        if projHitPoint:
            center = projHitPoint
        else:
            center = lastPoint
    ...
    def __setCurrentState(self, state):
        ...
        if killCamCtrl:
            ctx = {b'configMovementDuration': (self.__configMovementDuration)}
    ...
    def __setCameraSettings(self, targetMP=None, pivotSettings=None, cameraDistance=None, yawPitch=None):
        ...
        if cameraDistance is not None:
            if cameraDistance < 2:
                _logger.warning(b'Setting distance to low value: %s', cameraDistance)
    ...
    def __prepareMatrixWithTrajectoryPoints(self):
        ...
        if self.__projectileData[b'impactPoint']:
            trajectory[-1] = self.__projectileData[b'impactPoint']
        if self.isNonPenetratingExplosion:
            cameraLength = self.__getCameraLength()
    ...
    def __setKillCamDistance(self, distance, spgOffset, duration=0):
        isExplosion = self.__projectileData[b'isExplosion']
        if isExplosion:
            explosionRadius = self.__projectileData[b'explosionRadius']
