...
class CollisionVolumeGroup(namedtuple(b'CollisionVolumeGroup', (
 b'minVolume',
 b'lowSpeedLimit',
 b'vehicleVisibilityLimit',
 b'approachSpeed',
 b'cameraSpeedFactor',
 b'criticalDistance',
 b'canSkip'))):
...
VOLUME_GROUPS_NAMES = [
 b'tiny', b'small', b'medium', b'large']
...
ArcadeCameraState = namedtuple(b'ArcadeCameraState', (b'camDist', b'zoomSwitcherState'))
...
class ArcadeCamera(CameraWithSettings, CallbackDelayer, TimeDeltaMeter):
    ...
    def __init__(self, dataSec, defaultOffset=None):
        ...
        if defaultOffset is not None:
            ...
            self.__cam = self._createCamera(self.__adCfg[b'enable'])
            if self.__adCfg[b'enable']:
                self.__cam.initAdvancedCollider(self.__adCfg[b'fovRatio'], self.__adCfg[b'rollbackSpeed'], self.__adCfg[b'minimalCameraDistance'], self.__adCfg[b'speedThreshold'], self.__adCfg[b'minimalVolume'])
                for group_name in VOLUME_GROUPS_NAMES:
                    self.__cam.addVolumeGroup(self.__adCfg[b'volumeGroups'][group_name])
    ...
    def cloneState(self, **kwargs):
        ...
        currentState = kwargs.get(b'state')
        if not currentState:
    ...
    def create(self, onChangeControlMode=None, postmortemMode=False, smartPointCalculator=True):
        ...
        self.__aimingSystem = aimingSystemClass(refineVehicleMProv(targetMat), self._cfg[b'heightAboveBase'], self._cfg[b'focusRadius'], self.__calcAimMatrix(), self._cfg[b'angleRange'], smartPointCalculator)
        if self.__adCfg[b'enable']:
            self.__aimingSystem.initAdvancedCollider(self.__adCfg[b'fovRatio'], self.__adCfg[b'rollbackSpeed'], self.__adCfg[b'minimalCameraDistance'], self.__adCfg[b'speedThreshold'], self.__adCfg[b'minimalVolume'])
            for group_name in VOLUME_GROUPS_NAMES:
                self.__aimingSystem.addVolumeGroup(self.__adCfg[b'volumeGroups'][group_name])

        self.setCameraDistance(self._cfg[b'startDist'])
        self.__aimingSystem.pitch = self._cfg[b'startAngle']
    ...
    def enable(self, preferredPos=None, closesDist=False, postmortemParams=None, turretYaw=None, gunPitch=None, camTransitionParams=None, initialVehicleMatrix=None, arcadeState=None):
        ...
        newVehicleID = camTransitionParams.get(b'newVehicleID', 0) if camTransitionParams else 0
        if newVehicleID:
        ...
        if isObserverInBattle and isPlayerVehicleAttached or isWaitingForVehicle:
            ...
            if not self.isInArcadeZoomState() or arcadeState is not None:
                if arcadeState is None:
                    state = None
                    newCameraDistance = self._cfg[b'distRange'].max
                else:
            ...
            if camTransitionParams is not None and BigWorld.camera() is not self.__cam:
                pivotSettings = camTransitionParams.get(b'pivotSettings', None)
                if pivotSettings is not None:
                    self.__aimingSystem.setPivotSettings(pivotSettings[0], pivotSettings[1])
                cameraTransitionDuration = camTransitionParams.get(b'cameraTransitionDuration', -1)
                hasTransitionCamera = cameraTransitionDuration > 0
                keepRotation = hasTransitionCamera or camTransitionParams.get(b'keepRotation', False)
                if keepRotation:
                    distanceFromFocus = camTransitionParams.get(b'distanceFromFocus', None)
    ...
    def _getFovRatio(self):
        return self.__adCfg[b'fovRatio']
    ...
    def __calculateInputInertiaMinMax(self):
        if self.settingsCore.getSetting(b'dynamicFov'):
            _, minFov, maxFov = self.settingsCore.getSetting(b'fov')
            kMin = minFov / maxFov
        else:
            kMin = 1.0
    ...
    def update(self, dx, dy, dz, rotateMode=True, zoomMode=True, updatedByKeyboard=False):
        ...
        self.__curSense = self._cfg[b'keySensitivity'] if updatedByKeyboard else self.__sensitivity * self._userCfg[b'sensitivity'] if self.__sensitivity else self._cfg[b'sensitivity']
        self.__curScrollSense = self._cfg[b'keySensitivity'] if updatedByKeyboard else self.__scrollSensitivity
    ...
    def setUserConfigValue(self, name, value):
        if name not in self._userCfg:
            ...
            if name not in (b'keySensitivity', b'sensitivity', b'scrollSensitivity'):
                ...
                if name == b'fovMultMinMaxDist' and getattr(self, b'_ArcadeCamera__aimingSystem', None) is not None:
    ...
    def _updateProperties(self, state=None):
        ...
        self._distRange = state.distRange if state else self._cfg[b'distRange']
        self.__overScrollProtectOnMax = state.overScrollProtectOnMax if state else self._cfg[b'overScrollProtectOnMax']
        self.__overScrollProtectOnMin = state.overScrollProtectOnMin if state else self._cfg[b'overScrollProtectOnMin']
        self.__sensitivity = state.sensitivity if state else None
        self.__scrollSensitivity = state.scrollSensitivity if state else self._cfg[b'scrollSensitivity']
        if state is None:
    ...
    def _updateCameraSettings(self, newDist):
        ...
        if state:
            ...
            self.aimingSystem.setAnglesRange(self._cfg[b'angleRange'])
            self.setPivotSettings(self._cfg[b'heightAboveBase'], self._cfg[b'focusRadius'])
        ...
        if self.isInArcadeZoomState():
            self._userCfg[b'startDist'] = newDist
        heightAboveBase, _ = self.getPivotSettings()
        diff = heightAboveBase - self._cfg[b'heightAboveBase']
    ...
    def _cameraUpdate(self):
        ...
        self.__rotationMatrixVectorHelper.z = -impulseDeviation.x * self.__dynamicCfg[b'sideImpulseToRollRatio'] - self.__noiseOscillator.deviation.z
    ...
    def __calcAimOffset(self, oscillationsZoomMultiplier):
        ...
        yawFromImpulse = self.__impulseOscillator.deviation.x * self.__dynamicCfg[b'sideImpulseToYawRatio']
        xImpulseDeviationTan = math.tan(-(yawFromImpulse + self.__noiseOscillator.deviation.x) * oscillationsZoomMultiplier)
        pitchFromImpulse = self.__impulseOscillator.deviation.z * self.__dynamicCfg[b'frontImpulseToPitchRatio']
    ...
    def __getAbsoluteDistRange(self):
        minDist = self._cfg[b'distRange'].min
        maxDist = self._cfg[b'distRange'].max
    ...
    def __calcCurOscillatorAcceleration(self, deltaTime):
        ...
        if vehicle is None:
            ...
            curVelocity = getattr(vehFilter, b'velocity', math_utils.VectorConstant.Vector3Zero)
            ...
            return oscillatorAcceleration * self.__dynamicCfg[b'accelerationSensitivity']
    ...
    def __updateOscillators(self, deltaTime):
        ...
        zoomMultiplier = math_utils.lerp(1.0, self.__dynamicCfg[b'zoomExposure'], relDist)
    ...
    def reload(self):
        ...
        cameraSec = ResMgr.openSection(b'gui/avatar_input_handler.xml/arcadeMode/camera/')
    ...
    def _readConfigs(self, dataSec):
        if dataSec is None:
            LOG_WARNING(b'Invalid section <arcadeMode/camera> in avatar_input_handler.xml')
        ...
        if enableShift:
            ...
            shiftSensitivity = dataSec.readFloat(b'shiftSensitivity', 0.5)
            self.__shiftKeySensor = KeySensor(movementMappings, shiftSensitivity)
            self.__shiftKeySensor.reset(Math.Vector3())
        dynamicsSection = dataSec[b'dynamics']
        self.__impulseOscillator = createOscillatorFromSection(dynamicsSection[b'impulseOscillator'], False)
        self.__movementOscillator = createOscillatorFromSection(dynamicsSection[b'movementOscillator'], False)
        ...
        self.__dynamicCfg[b'accelerationSensitivity'] = readFloat(dynamicsSection, b'accelerationSensitivity', -1000, 1000, 0.1)
        self.__dynamicCfg[b'frontImpulseToPitchRatio'] = math.radians(readFloat(dynamicsSection, b'frontImpulseToPitchRatio', -1000, 1000, 0.1))
        self.__dynamicCfg[b'sideImpulseToRollRatio'] = math.radians(readFloat(dynamicsSection, b'sideImpulseToRollRatio', -1000, 1000, 0.1))
        self.__dynamicCfg[b'sideImpulseToYawRatio'] = math.radians(readFloat(dynamicsSection, b'sideImpulseToYawRatio', -1000, 1000, 0.1))
        ...
        self.__dynamicCfg[b'zoomExposure'] = readFloat(dynamicsSection, b'zoomExposure', 0.0, 1000.0, 0.25)
        accelerationFilter = math_utils.RangeFilter(self.__dynamicCfg[b'accelerationThreshold'], self.__dynamicCfg[b'accelerationMax'], 100.0, math_utils.SMAFilter(ArcadeCamera._FILTER_LENGTH))
        maxAccelerationDuration = readFloat(dynamicsSection, b'maxAccelerationDuration', 0.0, 10000.0, ArcadeCamera._DEFAULT_MAX_ACCELERATION_DURATION)
        ...
        if advancedCollider is None:
            ...
            cfg[b'enable'] = advancedCollider.readBool(b'enable', False)
            cfg[b'fovRatio'] = advancedCollider.readFloat(b'fovRatio', 2.0)
            cfg[b'rollbackSpeed'] = advancedCollider.readFloat(b'rollbackSpeed', 1.0)
        ...
        self.__zoomStateSwitcher.loadConfig(dataSec[b'additionalZoomStates'])
        return
    ...
    def _readBaseCfg(self, dataSec):
        ...
        bcfg[b'angleRange'] = readVec2(dataSec, b'angleRange', (0, 0), (180, 180), (10, 110))
        distRangeVec = readVec2(dataSec, b'distRange', (1, 1), (100, 100), (2, 20))
        bcfg[b'distRange'] = MinMax(distRangeVec.x, distRangeVec.y)
        bcfg[b'minStartDist'] = readFloat(dataSec, b'minStartDist', bcfg[b'distRange'][0], bcfg[b'distRange'][1], bcfg[b'distRange'][0])
        bcfg[b'optimalStartDist'] = readFloat(dataSec, b'optimalStartDist', bcfg[b'distRange'][0], bcfg[b'distRange'][1], bcfg[b'distRange'][0])
        bcfg[b'angleRange'][0] = math.radians(bcfg[b'angleRange'][0]) - math.pi * 0.5
        bcfg[b'angleRange'][1] = math.radians(bcfg[b'angleRange'][1]) - math.pi * 0.5
        bcfg[b'fovMultMinMaxDist'] = MinMax(readFloat(dataSec, b'fovMultMinDist', 0.1, 100, 1.0), readFloat(dataSec, b'fovMultMaxDist', 0.1, 100, 1.0))
        bcfg[b'focusRadius'] = readFloat(dataSec, b'focusRadius', -100, 100, 3)
        bcfg[b'heightAboveBase'] = readFloat(dataSec, b'heightAboveBase', 0, 100, 4)
        bcfg[b'overScrollProtectOnMax'] = readFloat(dataSec, b'overScrollProtectOnMax', 0, 10, 0)
        bcfg[b'overScrollProtectOnMin'] = readFloat(dataSec, b'overScrollProtectOnMin', 0, 10, 0)
        return
    ...
    def _readUserCfg(self):
        ...
        if dataSec is not None:
            dataSec = dataSec[b'arcadeMode/camera']
        ...
        if ucfg[b'startDist'] < bcfg[b'minStartDist']:
            ucfg[b'startDist'] = bcfg[b'optimalStartDist']
        ucfg[b'startAngle'] = readFloat(dataSec, b'startAngle', 5, 180, 60)
        ucfg[b'startAngle'] = math.radians(ucfg[b'startAngle']) - math.pi * 0.5
        ucfg[b'fovMultMinMaxDist'] = MinMax(readFloat(dataSec, b'fovMultMinDist', 0.1, 100, bcfg[b'fovMultMinMaxDist'].min), readFloat(dataSec, b'fovMultMaxDist', 0.1, 100, bcfg[b'fovMultMinMaxDist'].max))
        return
    ...
    def _makeCfg(self):
        ...
        cfg[b'angleRange'] = bcfg[b'angleRange']
        cfg[b'distRange'] = bcfg[b'distRange']
        cfg[b'minStartDist'] = bcfg[b'minStartDist']
        cfg[b'focusRadius'] = bcfg[b'focusRadius']
        cfg[b'heightAboveBase'] = bcfg[b'heightAboveBase']
        cfg[b'overScrollProtectOnMax'] = bcfg[b'overScrollProtectOnMax']
        cfg[b'overScrollProtectOnMin'] = bcfg[b'overScrollProtectOnMin']
        ...
        cfg[b'startDist'] = ucfg[b'startDist']
        cfg[b'startAngle'] = ucfg[b'startAngle']
        cfg[b'fovMultMinMaxDist'] = ucfg[b'fovMultMinMaxDist']
        cfg[b'sniperModeByShift'] = ucfg[b'sniperModeByShift']
        return
    ...
    def writeUserPreferences(self):
        ...
        ds.writeFloat(b'arcadeMode/camera/fovMultMaxDist', ucfg[b'fovMultMinMaxDist'].max)
        startAngle = math.degrees(ucfg[b'startAngle'] + math.pi * 0.5)
        ds.writeFloat(b'arcadeMode/camera/startAngle', startAngle)
        return
...
class ArcadeCameraEpic(ArcadeCamera):
    ...
    def reload(self):
        ...
        import ResMgr
        ResMgr.purge(b'gui/avatar_input_handler.xml')
        cameraSec = ResMgr.openSection(b'gui/avatar_input_handler.xml/arcadeEpicMinefieldMode/camera/')
        self._reloadConfigs(cameraSec)
        return
