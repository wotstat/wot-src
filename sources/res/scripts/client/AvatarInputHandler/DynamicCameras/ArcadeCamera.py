...
class ArcadeCamera(CameraWithSettings, CallbackDelayer, TimeDeltaMeter):
    ...
    def __init__(self, dataSec, defaultOffset=None):
        ...
        if defaultOffset is not None:
            ...
            self.__cam = BigWorld.HomingCamera(self.__adCfg[b'enable'])
    ...
    def create(self, onChangeControlMode=None, postmortemMode=False, smartPointCalculator=True):
        ...
        self.__aimingSystem = aimingSystemClass(self.__refineVehicleMProv(targetMat), self._cfg[b'heightAboveBase'], self._cfg[b'focusRadius'], self.__calcAimMatrix(), self._cfg[b'angleRange'], not postmortemMode and smartPointCalculator)
        if self.__adCfg[b'enable']:
        ...
        self.setCameraDistance(self._baseCfg[b'optimalStartDist'])
    ...
    def update(self, dx, dy, dz, rotateMode=True, zoomMode=True, updatedByKeyboard=False):
        ...
        self.__curSense = self._cfg[b'keySensitivity'] if updatedByKeyboard else self.__sensitivity
    ...
    def __updateProperties(self, state=None):
        ...
        self.__distRange = state.distRange if state else self._cfg[b'distRange']
        ...
        self.__sensitivity = state.sensitivity * self._userCfg[b'sensitivity'] if state else self._cfg[b'sensitivity']
