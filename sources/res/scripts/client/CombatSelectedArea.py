...
MARKER_HEIGHT = 5.0
DEFAULT_RADIUS_MODEL = b'content/Interface/CheckPoint/CheckPoint.visual'
DEFAULT_ROTATE_MODEL = b'content/Interface/TargetPoint/rectangle2_1.visual'
...
class CombatSelectedArea(object):
    ...
    def setup(self, position, direction, size, visualPath, color, marker):
        self.__fakeModel = model = BigWorld.Model(b'')
        rootNode = model.node(b'')
    ...
    def addLine(self, position, color, width, height):
        if self.__fakeModel is None:
            self.__fakeModel = BigWorld.Model(b'')
        rootNode = self.__fakeModel.node(b'')
    ...
    def setSelectingDirection(self, value=False):
        if value and self.__terrainRotatedArea is None:
            ...
            self.__rotateModelNode = self.__fakeModel.node(b'', math_utils.createRTMatrix(Math.Vector3(-self.__matrix.yaw, 0.0, 0.0), Math.Vector3((-self.__size.x - objectSize.x) * 0.5, 0.0, (self.__size.y + objectSize.y) * 0.5)))
