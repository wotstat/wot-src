...
class DeathComponentManager(CGF.ComponentManager):
    ...
    @onAddedQuery(CGF.GameObject, DeathComponent, RemoveOnDeathComponent, tickGroup=b'Simulation')
    def onAddedRemoveOnDeath(self, gameObject, _, removeOnDeathComponent):
        LOG_DEBUG_DEV(b'Game object name=%s, id=%s was removed because of death', gameObject.name, gameObject.id)
    ...
    def onAddedSpawnOnDeath(self, gameObject, _, spawnOnDeathComponent, transform):
        LOG_DEBUG_DEV(b'Prefab "%s" was loaded because of death into Game object name=%s, id=%s', spawnOnDeathComponent.prefabPath, gameObject.name, gameObject.id)
    ...
    def onAddedSoundOnDeath(self, gameObject, _, soundOnDeathComponent, transform):
        LOG_DEBUG_DEV(b'Sound prefab "%s" was loaded because of death into Game object name=%s, id=%s', soundOnDeathComponent.soundPath, gameObject.name, gameObject.id)
    ...
    def onAddedEffectOnDeath(self, gameObject, _, effectOnDeathComponent, transform):
        LOG_DEBUG_DEV(b'Effect prefab "%s" was loaded because of death into Game object name=%s, id=%s', effectOnDeathComponent.effectPath, gameObject.name, gameObject.id)
    ...
    def onAddedChangeModelOnDeath(self, gameObject, _, changeModelOnDeathComponent):
        ...
        if dynamicModelComponent:
            LOG_DEBUG_DEV(b'Game object name=%s, id=%s changed his DynamicModelComponent because of death to a new one "%s"', gameObject.name, gameObject.id, changeModelOnDeathComponent.modelPath)
