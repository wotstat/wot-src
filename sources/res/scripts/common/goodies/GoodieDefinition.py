...
class ExpirationCountException(SoftException):
    ...
    def __init__(self, state):
        msg = (b'The number of expirations does not match the total number of goodies. {}').format(state)
...
class GoodieDefinition(object):
    ...
    def __repr__(self):
        return (b'{}, {}, {}, {}, {}, {}, {}, {}').format(self.uid, self.enabled, self.lifetime, self.useby, self.counter, self.resource, self.value, self.expireAfter, self.roundToEndOfGameDay)
            ...
            if resource.__class__ == self.resource:
                if self.variety in GOODIE_VARIETY.DISCOUNT_LIKE:
                    ...
                    if self.target.limit is not None and resource.value - result > self.target.limit:
                        raise OverLimitException(b'Discount is over the limit %s' % self.target.limit)
                ...
                raise SoftException(b'Programming error, Goodie is not a discount or booster %s' % self.variety)
