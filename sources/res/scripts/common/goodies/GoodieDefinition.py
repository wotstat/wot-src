            ...
            if resource.__class__ == self.resource:
                if self.variety in GOODIE_VARIETY.DISCOUNT_LIKE:
                    ...
                    if self.target.limit is not None and resource.value - result > self.target.limit:
                        raise OverLimitException(b'Discount is over the limit' % self.target.limit)
                    else:
                ...
                raise SoftException(b'Programming error, Goodie is not a discount or booster' % self.variety)
