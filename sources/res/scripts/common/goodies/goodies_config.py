...
def _readGoodies(reader, subsectionName):
    ...
    if section is None:
        ...
        for packet_name, packet in section.items():
            ...
            expireAfter = helpers_common.parseDuration(packet.readString(b'expireAfter', b'0'))
            if expireAfter == 0:
                expireAfter = None
            roundToEndOfGameDay = packet.readBool(b'roundToEndOfGameDay', True)
            lifetime = helpers_common.parseDuration(packet.readString(b'lifetime', b'0'))
            if lifetime == 0:
                lifetime = None
