...
GoodieData = namedtuple(b'GoodieData', b'variety target enabled lifetime useby counter autostart condition resource')
...
GOODIE_TARGET_TO_TEXT = {BuyPremiumAccount: b'premium', 
   ...
   FreeExperienceConversion: b'free_xp_conversion', 
...
GOODIE_TEXT_TO_TARGET = {b'premium': (GOODIE_TARGET_TYPE.ON_BUY_PREMIUM), 
   ...
   b'free_xp_conversion': (GOODIE_TARGET_TYPE.ON_FREE_XP_CONVERSION), 
...
def loadDefinitions(d):
    ...
    for uid, d in d[b'goodies'].iteritems():
        ...
        goodies[b'goodies'][uid] = GoodieDefinition(uid=uid, variety=v_variety, target=target, enabled=v_enabled, lifetime=v_lifetime, useby=v_useby, counter=v_limit, autostart=v_autostart, resource=resource, value=value, condition=condition)
...
def wipe(goodies, pdata, leaveGold):
    if leaveGold:
        for goodieID in pdata[b'goodies'].keys():
