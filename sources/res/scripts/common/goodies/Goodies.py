...
class Goodies(object):
    ...
    def __expire(self, goodieID):
        LOG_DEBUG_DEV((b'[GOODIE] __expire goodie {} ').format(goodieID))
    ...
    def __erase(self, goodieID, reasonID):
        LOG_DEBUG_DEV((b'[GOODIE] __erase goodie {}, reason id {} ').format(goodieID, reasonID))
        goodie = self.actualGoodies.get(goodieID, None)
        if goodie is None:
            return
        else:
    ...
    def __decrement(self, goodieID, amount, reasonID, skipActive):
        LOG_DEBUG_DEV((b'[GOODIE] __decrement goodie {}, amount {}, reason id {} ').format(goodieID, amount, reasonID))
        goodieDefinition = self.definedGoodies.get(goodieID, None)
        if goodieDefinition is None:
            return
        else:
    ...
    def load(self, goodieID, state, finishTime, counter, expirations):
        try:
            ...
            raise GoodieException(b'Goodie is not found', goodieID)
    ...
    def extend(self, goodieID, state, counter, expiryTime):
        ...
        if goodieDefinition.isExpirable():
            if not expiryTime:
                LOG_WARNING(b'Cannot add expirable reserve without a positive expirationTime.')
                return
    ...
    def activate(self, goodieID):
        goodie = self.actualGoodies.get(goodieID, None)
        if goodie is None:
            LOG_WARNING(b"Couldn't find goodie by id={}", goodieID)
            return
        else:
            if goodie.isActive():
                LOG_WARNING((b"Couldn't activate goodie(id={}) because it is already activated!").format(goodieID))
                return
            defined = self.definedGoodies[goodieID]
            if not defined.isTimeLimited():
                LOG_WARNING((b"Couldn't activate goodie(id={}) because it has unlimited time!").format(goodieID))
                return
            ...
            if oldGoodieID is not None:
                if self.actualGoodies.compareByResource(oldGoodieID, goodieID):
                    ...
                    LOG_WARNING((b"Couldn't activate goodie(id={}) because replacing is forbidden!").format(goodieID))
                    return
            if self.activeGoodiesCount() >= MAX_ACTIVE_BOOSTERS:
                LOG_WARNING((b"Couldn't activate goodie(id={}) because limit of activated boosters is reached!").format(goodieID))
                return
