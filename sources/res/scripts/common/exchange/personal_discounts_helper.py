...
def createCommonDiscount(exchangeType, exchangeRate):
    return ExchangeDiscountInfo(isPersonal=False, exchangeType=exchangeType, discountType=ExchangeDiscountType.UNLIMITED, showFormat=ExchangeRateShowFormat.COEFFICIENT, goldRateValue=exchangeRate.goldRateValue, resourceRateValue=exchangeRate.resourceRateValue, amountOfDiscount=0, discountLifetime=0, tokenName=b'')
...
def calculateGoldExchangeWithDiscounts(discounts, goldExchangeAmount, defaultRate, currentTime):
    if goldExchangeAmount < 0 or defaultRate.resourceRateValue < 1:
        _logger.error(b'Exchange rate is lower then 1, resourceRateValue=%d or incorrect exchange amount=%d', defaultRate.resourceRateValue, goldExchangeAmount)
        return 0
    defaultDiscount = createCommonDiscount(b'default', defaultRate)
    ...
    _logger.error(b'Error calculating gold exchange, left amount=%d, defaultRate=%d ', leftGoldToExchange, defaultRate.resourceRateValue)
    return 0
...
def calculateResourceExchangeWithDiscounts(discounts, resourceAmount, defaultRate, currentTime):
    if resourceAmount < 0 or defaultRate.resourceRateValue < 1:
        _logger.error(b'Exchange rate is lower then 1, resourceRateValue=%d or incorrect exchange amount=%d', defaultRate.resourceRateValue, resourceAmount)
        return (0, 0)
    defaultDiscount = createCommonDiscount(b'default', defaultRate)
    discounts = discounts or []
    discounts.append(defaultDiscount)
    ...
    _logger.error(b'Error calculating resource exchange, left amount=%d, defaultRate=%d ', leftResourceAmount, defaultRate.resourceRateValue)
    return (0, 0)
