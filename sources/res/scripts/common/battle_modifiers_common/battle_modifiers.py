...
BattleParams.ALL = set(v for k, v in BattleParams.__dict__.iteritems() if not k.startswith(b'_') and k not in (b'FAKE_PARAM', b'ALL', b'MAX'))
BattleParams.MAX = max(BattleParams.ALL)

class ConstantsSet(object):
    __slots__ = (b'VEHICLE_CIRCULAR_AOI_RADIUS', b'VEHICLE_CIRCULAR_AOI_RADIUS_HYSTERESIS_MARGIN')
...
class ModifiersContext(object):
    __slots__ = (b'__modifiers', b'__vehType', b'__shellDescr', b'__gun')
    ...
    def __repr__(self):
        return (b'ModifiersContext(vehicle = {}, shell = {}, modifiers = {})').format(self.__vehType.name, self.__shellDescr, self.__modifiers)
