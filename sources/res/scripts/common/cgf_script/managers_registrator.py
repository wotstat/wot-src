...
def onProcessQuery(*args, **kwargs):
    ...
    def wrapper(func):
        return CGF.Reaction(Reactions.ON_PROCESS_QUERY, func, kwargs.get(b'tickGroup', b'Simulation'), kwargs.get(b'period', 0.0), args)
...
def onAddedQuery(*args, **kwargs):
    ...
    def wrapper(func):
        return CGF.Reaction(Reactions.ON_ADDED_QUERY, func, kwargs.get(b'tickGroup', b'Init'), 0.0, args)
...
def onRemovedQuery(*args, **kwargs):
    ...
    def wrapper(func):
        return CGF.Reaction(Reactions.ON_REMOVED_QUERY, func, kwargs.get(b'tickGroup', b'Init'), 0.0, args)

    return wrapper


class ManagerRegistrator(object):
    __slots__ = (b'__wrapper',)
...
def generateRule(cls, category, domain):
    ...
    try:
        ...
        raise SoftException(e.message + b':\n' + rule_class_definition)

    return


def autoregister(presentInAllWorlds=False, category=b'', domain=CGF.DomainOption.DomainClient, creationPredicate=None):
