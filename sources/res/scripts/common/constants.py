...
if CURRENT_REALM == b'NA':
    ...
    AUTH_REALM = b'CT'
elif CURRENT_REALM == b'SB':
    AUTH_REALM = b'SB'
elif CURRENT_REALM == b'RU':
    DEFAULT_LANGUAGE = b'ru'
    AUTH_REALM = b'RU'
elif CURRENT_REALM in (b'EU', b'ST', b'QA', b'DEV'):
    pass
...
ARENA_GAMEPLAY_NAMES = (b'ctf', b'domination', b'assault', b'nations', b'ctf2', b'domination2', b'assault2', b'fallout', b'fallout2', b'fallout3', b'fallout4', b'ctf30x30', b'domination30x30', b'sandbox', b'bootcamp', b'epic', b'maps_training', b'rts', b'rts_1x1', b'rts_bootcamp', b'comp7')
if IS_EDITOR:
    ARENA_GAMEPLAY_READABLE_NAMES = (b'Capture The Flag', b'Domination', b'Assault', b'Nations', b'Capture The Flag 2', b'Domination 2', b'Assault 2', b'Fallout Bomb', b'Fallout 2 Flag', b'Fallout 3', b'Fallout 4', b'Capture The Flag 30 vs 30', b'Domination 30 vs 30', b'Sandbox', b'Bootcamp', b'Epic', b'Maps Training', b'RTS', b'RTS 1 vs 1', b'RTS Boot Camp', b'Comp7')
ARENA_GAMEPLAY_IDS = dict((value, index) for index, value in enumerate(ARENA_GAMEPLAY_NAMES))
ARENA_GAMEPLAY_MASK_DEFAULT = 1048575
VALID_TRAINING_ARENA_GAMEPLAY_IDS = frozenset(ARENA_GAMEPLAY_IDS[gameplayName] for gameplayName in (b'ctf', b'domination', b'assault', b'nations', b'ctf2', b'domination2', b'assault2', b'epic', b'ctf30x30', b'domination30x30', b'rts', b'rts_1x1', b'rts_bootcamp', b'comp7'))
...
class ARENA_GUI_TYPE_LABEL:
    LABELS = {(ARENA_GUI_TYPE.UNKNOWN): b'special', 
       ...
       (ARENA_GUI_TYPE.WINBACK): b'winback'}
...
ARENA_PERIOD_NAMES = {v: k for k, v in ARENA_PERIOD.__dict__.items() if not k.startswith(b'_')}
...
ARENA_SYNC_OBJECT_NAMES = {v: k for k, v in ARENA_SYNC_OBJECTS.__dict__.items() if not k.startswith(b'_')}
...
JOIN_FAILURE_NAMES = {v: k for k, v in JOIN_FAILURE.__dict__.items() if not k.startswith(b'_')}
...
KICK_REASON_NAMES = {v: k for k, v in KICK_REASON.__dict__.items() if not k.startswith(b'_')}
...
FINISH_REASON_NAMES = {v: k for k, v in viewitems(FINISH_REASON.__dict__) if not k.startswith(b'_')}
...
PREBATTLE_TYPE_NAMES = {v: k for k, v in viewitems(PREBATTLE_TYPE.__dict__) if not k.startswith(b'_')}
...
class PREBATTLE_ERRORS:
    ...
    ONSLAUGHT_ROSTER_LIMIT = b'ONSLAUGHT_ROSTER_LIMIT'
    ...
    PROHIBITED_ARENA_TYPE = b'PROHIBITED_ARENA_TYPE'
...
class RENEWABLE_SUBSCRIPTION_ENTITLEMENTS(object):
    CORE = b'premium_subs'
    PRO = b'premium_pro_subs'
    ALL_TYPES = (
     CORE, PRO)


PRIME_GAMING_SUBSCRIPTION_PRODUCT = b'prime_subscription'
...
class ENTITLEMENT_OPS:
    GRANT = b'grant'
    FORCED_GRANT = b'forced_grant'
    CONSUME = b'consume'
    FORCED_CONSUME = b'forced_consume'
    ALL = (
...
OPTIONAL_DEVICES_USAGE_CONFIG = b'optional_devices_usage_config'
...
class Configs(enum.Enum):
    ...
    NOVICE_MATCHMAKING_SYSTEM = b'novice_matchmaking_system'
    TRIGGER_SYSTEM_CONFIG = b'trigger_system_config'
    PRESTIGE_CONFIG = b'prestige_config'
    PRESTIGE_MILESTONES_CONFIG = b'prestige_milestones_config'
    STEAM_SHADE_CONFIG = b'steam_shade_config'
    SYSTEM_CHANNELS = b'system_channels'
    REFERRAL_PROGRAM_CONFIG = b'referral_program_config'
    FAIRPLAY_CONFIG = b'fairplay_config'
    ARENA_DYN_OBSTACLES_CONFIG = b'arena_dyn_obstacles_config'
    POSTMORTEM_SETTINGS_CONFIG = b'postmortem_settings_config'
    LIVE_OPS_EVENTS_CONFIG = b'live_ops_events_config'
    ADVANCED_ACHIEVEMENTS_CONFIG = b'advanced_achievements_config'
    LOOTBOXES_TOOLTIP_CONFIG = b'lootboxes_tooltip_config'
    UNIT_ASSEMBLER_CONFIG = b'unit_assembler_config'
    EASY_TANK_EQUIP_CONFIG = b'easy_tank_equip_config'
    PLAYER_SATISFACTION_CONFIG = b'player_satisfaction_config'
    COMMENDATIONS_CONFIG = b'commendations_config'
    VEH_PLAYLISTS_CONFIG = b'veh_playlists_config'
    SENIORITY_AWARDS_CONFIG = b'seniority_awards_config'
    WTR_CONFIG = b'wtr_config'
    WEEKLY_QUESTS_CONFIG = b'weekly_quests_config'
    WEEKLY_QUESTS_CONFIGS = b'weekly_quests_configs'
    ...
    GAMEFACE_HUD_CONFIG = b'gameface_hud_config'
...
SPA_RESTR_NAME_TO_RESTR_TYPE = {b'game': (RESTRICTION_TYPE.BAN), 
   ...
   b'all': (RESTRICTION_TYPE.MINORS_RESTRICTION), 
   b'modes': (RESTRICTION_TYPE.ARENA_BAN)}
RESTR_TYPE_TO_SPA_NAME = {x[1]: x[0] for x in SPA_RESTR_NAME_TO_RESTR_TYPE.items()}

class SPA_ATTRS:
    ANONYM_RESTRICTED = b'/wot/game/anonym_restricted/'
    GOLFISH_BONUS_APPLIED = b'/common/goldfish_bonus_applied/'
    LOGGING_ENABLED = b'/wot/game/logging_enabled/'
    STEAM_ALLOW = b'/wot/steam/allow/'
    RSS = b'/wot/game/service/rss/'
...
class BAN_TYPE(object):
    ...
    COMMUNICATION_DENIED = b'communication_denied'
    PUBLIC_COMMUNICATION_DENIED = b'public_communication_denied'
    PRIVATE_COMMUNICATION_DENIED = b'private_communication_denied'
    NONFRIEND_PRIVATE_COMMUNICATION_DENIED = b'nonfriend_private_communication_denied'
    ARENA_BAN_PREFIX = b'arena_ban_'
...
class BAN_REASON(object):
    PARENTAL_CONTROL = b'#ban_reason:parental_control'
    COUNTRY = b'#ban_reason:country'
    OTHER = b'#ban_reason:other'
...
class RESTRICTION_KEY(object):
    PRIVATE_CHAT = b'privateChat'
    PUBLIC_CHAT = b'publicChat'
    SETTINGS = b'settings'
    SESSION = b'session'
    PAYMENT = b'payment'
...
class EQUIPMENT_STAGES:
    ...
    def toString(cls, value):
        return {0: b'notrunning', 
           ...
           (cls.STARTUP_COOLDOWN): b'startupCooldown', 
           (cls.INTERRUPTED): b'interrupted', 
...
class ATTACK_REASON(object):
    ...
    VEHICLE_EXPLOSION = b'vehicle_explosion'
    BUNKER_DESTROYED = b'bunker_destroyed'
    MINEFIELD_ZONE = b'minefield_zone'
...
class VEHICLE_HIT_EFFECT:
    ...
    _GROUP_MAPPING = {INTERMEDIATE_RICOCHET: b'armorBasicRicochet', 
       ...
       ARMOR_NOT_PIERCED: b'armorResisted', 
       ARMOR_PIERCED_NO_DAMAGE: b'armorResisted', 
       ARMOR_PIERCED: b'armorHit', 
       CRITICAL_HIT: b'armorCriticalHit', 
       ARMOR_PIERCED_DEVICE_DAMAGED: b'armorCriticalHit'}
...
SYS_MESSAGE_CLAN_EVENT_NAMES = {v: k for k, v in SYS_MESSAGE_CLAN_EVENT.__dict__.items() if not k.startswith(b'_')}
...
SYS_MESSAGE_FORT_EVENT_NAMES = {v: k for k, v in SYS_MESSAGE_FORT_EVENT.__dict__.items() if not k.startswith(b'_')}
...
FORT_BUILDING_TYPE_NAMES = {v: k for k, v in FORT_BUILDING_TYPE.__dict__.items() if not k.startswith(b'_')}
...
INT_USER_SETTINGS_KEYS = {(USER_SERVER_SETTINGS.VERSION): b'Settings version', 
   ...
   (USER_SERVER_SETTINGS.SENIORITY_AWARDS): b'seniority awards settings', 
   (USER_SERVER_SETTINGS.REFERRAL_PROGRAM): b'referral program settings', 
   (USER_SERVER_SETTINGS.ADVANCED_ACHIEVEMENTS_STORAGE): b'advanced achievements storage', 
   116: b'Once only hints', 
   118: b'Ranked carousel filter 3', 
   119: b'Frontline carousel filter 3', 
   120: b'Mapbox carousel filter 3', 
   121: b'Competitive7x7 carousel filter 3', 
   122: b'Fun Random carousel filter 3', 
   123: b'Carousel filter 3', 
   (USER_SERVER_SETTINGS.PERSONAL_MISSION_3): b'prsonal mission 3 settings', 
...
OBSERVER_VEH_NAME = b'ussr:Observer'
...
PREBATTLE_INVITE_STATUS_NAMES = {v: k for k, v in PREBATTLE_INVITE_STATUS.__dict__.items() if not k.startswith(b'_')}
...
GLOBAL_MAP_DIVISION_NAMES = {v: k for k, v in GLOBAL_MAP_DIVISION.__dict__.items() if not k.startswith(b'_')}
...
class VEHICLE_SIEGE_STATE:
    ...
    def toString(cls, siegeState):
        return {(cls.DISABLED): b'disabled', 
           (cls.SWITCHING_ON): b'switching_on', 
           (cls.ENABLED): b'siege_enabled', 
           (cls.SWITCHING_OFF): b'switching_off', 
           (cls.PILLBOX_ENABLED): b'pillbox_enabled'}.get(siegeState, b'')
...
VEHICLE_ATTRS_TO_SYNC = frozenset([b'circularVisionRadius', b'gun/piercing', b'gun/shots/speed'])
VEHICLE_ATTRS_TO_SYNC_ALIASES = {b'gun/piercing': b'gunPiercing', b'gun/shots/speed': b'gunShotsSpeed'}
...
class SHELL_MECHANICS_TYPE:
    LEGACY = b'LEGACY'
    MODERN = b'MODERN'
    NON_PIERCING_DAMAGE = b'NON_PIERCING_DAMAGE'
...
GRACE_PERIOD_RESET_PERK = b'grace_period_reset_perk'
...
class BotNamingType(object):
    ...
    CUSTOM = 3
    DEFAULT = CREW_MEMBER
    CONFIG_TYPE = (b'BotNamingTypeEnumType', b'py::BotNamingTypeEnum')
...
class ClansConfig(object):
    ...
    CLAN_SUPPLY_ENABLED = b'clanSupplyEnabled'
...
class PerkData(namedtuple(b'PerkData', [b'level', b'args'])):
    pass


class CrewContextArgs(namedtuple(b'CrewContextArgs', [b'skillData'])):
    pass


class SkillProcessorArgs(object):
    __slots__ = (b'level', b'levelIncrease', b'skillsEfficiency', b'isActive', b'isFire', b'skillConfig', b'hasActiveTankmanForBooster', b'tankmenSkillConfig', b'isBoostedSkill')
...
class GroupSkillProcessorArgs(object):
    __slots__ = (b'factor', b'baseAvgLevel', b'skillEfficiency')
...
class ReloadRestriction(object):
    CYCLE_RELOAD = 1.0
    OTHER_RELOAD = 2.5
    NON_CYCLIC_TAGS = frozenset((b'clip', b'autoreload', b'autoShoot', b'twinGun', b'dualGun'))
...
class POSTMORTEM_MODIFIERS(object):
    ENABLED_IF_NO_ALLY = b'enabledIfNoAlly'
    DISABLE_TANK_TARGET_FOLLOW = b'disableTankFollow'
    DISABLE_TANK_CYCLE = b'disableTankCycle'
    FORCED_IF_NO_LIVES = b'forcedIfNoLives'
    POSTMORTEM = {
...
DEFAULT_POSTMORTEM_SETTINGS = {b'deathfreecam': False, b'killcam': False, b'lookAtKiller': False}
...
COMP7_SCENE = b'COMP7'
COMP7_LIGHT_SCENE = b'COMP7_LIGHT'
...
class WoTPlusBonusType(object):
    ...
    BATTLE_BONUSES = b'battle_bonuses'
    BADGES = b'badges'
    ADDITIONAL_BONUSES = b'additional_bonuses'
    OPTIONAL_DEVICES_ASSISTANT = b'optional_devices_assistant'
    PRO_BOOST = b'pro_boost'
    SERVICE_RECORD_CUSTOMIZATION = b'service_record_customization'
    BATTLE_PASS_PLUS = b'battle_pass_plus'
...
DEATH_ZONE_MASK_PATTERN = b'deathzone_mask_'
...
class FAIRPLAY_VIOLATION_SYS_MSG_SAVED_DATA:
    ARENA_BONUS_TYPE = b'arenaBonusType'
    PENALTY_TYPE = b'penaltyType'
    ARENA_TYPE_ID = b'arenaTypeID'
    ARENA_TIME = b'arenaTime'
    PUNISHMENT_REASON = b'punishmentReason'
    BAN_DURATION = b'banDuration'
    IS_AFK_VIOLATION = b'isAfkViolation'
    COMP7_PENALTY = b'comp7Penalty'
    COMP7_IS_QUALIFICATION = b'comp7IsQualification'
...
INFINITE_SHELL_TAG = b'infinite'
INFINITE_SHELL_COUNT = 9
FORCE_FINITE_SHELL_TAG = b'forceFinite'
...
PICKLER_PROTOCOL_METHODS = frozenset((b'__getinitargs__', b'__getstate__', b'__setstate__', b'__getnewargs__'))
...
class SquadManStates(enum.Enum):
    ...
    NOT_READY_PLAYER = b'notReadyPlayer'
...
class VehicleArmorTags(str, enum.Enum):
    ...
    GUN = b'gun'
    WHEELS = b'wheels'
    SURVEYING = b'surveying'
    GUN_MASK = b'gunMask'


class VehicleTurretTags(str, enum.Enum):
    CANNOT_DETACH = b'cannotDetach'
    GUN_CARRIAGE = b'gunCarriage'
    TURRETLESS = b'turretless'
    OSCILLATING = b'oscillating'
...
class CONCENTRATION_MODE_STATE:
    ...
    def toString(cls, value):
        return {(cls.ACTIVE): b'active', 
           ...
           (cls.IDLE): b'idle', 
           (cls.DEPLOYING): b'deploying', 
           (cls.DISABLED): b'disabled'}.get(value)
...
class HEATING_ZONES_GUN_STATE(object):
    ...
    @classmethod
    def toString(cls, value):
        return {(cls.IDLE): b'idle', 
           (cls.LOW): b'low', 
           (cls.MEDIUM): b'medium', 
           (cls.HIGH): b'high'}.get(value, value)
...
class RECHARGEABLE_NITRO_STATE:
    ...
    def toString(cls, value):
        return {(cls.NOT_RUNNING): b'not running', 
           ...
           (cls.DISABLED): b'disabled', 
           (cls.PRIMED): b'primed', 
           (cls.DEPLETING): b'depleting'}.get(value)
...
class STATIONARY_RELOAD_STATE:
    ...
    @classmethod
    def toString(cls, value):
        return {(cls.FIXING_GUN): b'fixing gun', 
           (cls.RELOADING): b'reloading', 
           (cls.PREPARING): b'preparing', 
           (cls.FINISHING): b'finishing', 
           (cls.IDLE): b'idle'}.get(value)
...
class DEBUFFS_TYPES:
    ...
    COMMANDER = 10
    DRIVER = 11
    _NAME_TO_IDX = {b'overturn': OVERTURN, 
       b'stun': STUN, 
       b'ammoBay': AMMOBAY, 
       b'fire': FIRE, 
       b'gunner1': GUNNER1, 
       ...
       b'loader1': LOADER1, 
       b'loader2': LOADER2, 
       b'radioman1': RADIOMAN1, 
       b'radioman2': RADIOMAN2, 
