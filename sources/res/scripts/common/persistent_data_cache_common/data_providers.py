...
class PDProvider(object):
    __slots__ = (b'_logger', b'_name', b'_factory', b'_serializer', b'_data')
    ...
    def create(self):
        if self._data is not None:
            self._logger.debug(b'Data has been created already.')
            return self._data
        else:
            try:
                self._data = self._factory()
            except Exception:
                self._logger.exception(b'Failed to create data for <%s>.', self._name)

            if self._data is None:
                raise SoftException((b'Failed to create data for <{}>.').format(self._name))
            self._logger.debug(b'Data has been created.')
            return self._data

    def serialize(self, rawData):
        result = self._serializer.serialize(rawData)
        self._logger.debug(b'Data has been serialized.')
        return result

    def deserialize(self, serializedData):
        result = self._serializer.deserialize(serializedData)
        self._logger.debug(b'Data has been deserialized.')
        return result
