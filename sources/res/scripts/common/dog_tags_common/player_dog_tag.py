from __future__ import absolute_import
import typing
from future.utils import viewvalues
from dog_tags_common.components_config import componentConfigAdapter
from dog_tags_common.config.common import ComponentViewType
from soft_exception import SoftException
if typing.TYPE_CHECKING:
    from typing import Dict

class DogTagComponent(object):

    def __init__(self, compId, value, grade):
        self._componentDefinition = componentConfigAdapter.getComponentById(compId)
        self._value = value
        self._grade = grade
        return

    @property
    def compId(self):
        return self._componentDefinition.componentId

    @property
    def value(self):
        return self._value

    @property
    def grade(self):
        return self._grade

    @property
    def componentDefinition(self):
        return self._componentDefinition


class PlayerDogTag(object):

    def __init__(self, comps):
        self._components = {}
        for comp in comps:
            if self._components.get(comp.componentDefinition.viewType, None):
                raise SoftException(b'Cannot have more than one component of the same view type')
            self._components[comp.componentDefinition.viewType] = comp

        return

    def getComponentByType(self, viewType):
        return self._components.get(viewType, None)

    def getComponentIter(self):
        return iter(viewvalues(self._components))

    def __eq__(self, other):
        if isinstance(other, PlayerDogTag):
            return self.__dict__ == other.__dict__
        return False

    def __ne__(self, other):
        return not self.__eq__(other)

    def __repr__(self):
        retStr = b'('
        for component in self._components.values():
            retStr += (b'DogTagComponent(compId={}, value={}, grade={}), ').format(component.compId, component.value, component.grade)

        retStr += b')'
        return retStr

    __str__ = __repr__
    __hash__ = None

    @staticmethod
    def fromDict(dtDict):
        comps = [DogTagComponent(comp[b'id'], comp[b'progress'], comp[b'grade']) for comp in dtDict[b'components']]
        return PlayerDogTag(comps)

    def toDict(self):
        return {b'components': [{b'id': (comp.compId), b'progress': (comp.value), b'grade': (comp.grade)} for comp in self.getComponentIter()]}


class DisplayableDogTag(object):

    def __init__(self, playerDT, nickName=b'', clanTag=b''):
        self._nickName = nickName
        self._clanTag = clanTag
        self._playerDT = playerDT
        return

    def getNickName(self):
        return self._nickName

    def getClanTag(self):
        return self._clanTag

    def getComponentByType(self, viewType):
        return self._playerDT.getComponentByType(viewType)

    def __repr__(self):
        return (b'<PlayerDogTag(nickname={}, clantag={}, components={})>').format(self._nickName, self._clanTag, self._playerDT)

    __str__ = __repr__

    @staticmethod
    def fromDict(dtDict):
        return DisplayableDogTag(PlayerDogTag.fromDict(dtDict), dtDict[b'playerName'], dtDict[b'clanTag'])
