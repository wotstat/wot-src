...
class ValidateException(DogTagsException):
    ...
    ERR_STR = {HAS_GRADES: (PREFIX + b'Component with id = %s cannot have grades. Grades: %s'), 
       ...
       HAS_PROGRESS_KEY: (PREFIX + b'Component of type STATIC with id=%s cannot has progress key.'), 
       HAS_LIGHTING: (PREFIX + b'Custom lighting is supported only for STATIC components.')}
