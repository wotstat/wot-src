...
NEAREST_TIME_BOUNDARY = b'nearestTimeBoundary'
...
class Source(object):
    ...
    def readFromInternalFile(self, path, curTime, auxData=None):
        ...
        if not section.has_key(b'quests'):
            ...
            return self.__readXML(section[b'quests'], curTime, auxData=auxData)
    ...
    def __readXML(self, section, curTime, gStartTime=DEFAULT_QUEST_START_TIME, gFinishTime=DEFAULT_QUEST_FINISH_TIME, auxData=None):
        ...
        for typeName, questSection in section.items():
            ...
            announceTime = info[b'announceTime']
            finishTime = info[b'finishTime']
            if auxData is not None:
            ...
            if not announceTime <= curTime <= finishTime:
                LOG_WARNING(b'Skipping outdated quest', info[b'id'], curTime, announceTime, finishTime)
                continue
    ...
    def __readHeader(self, eventType, questSection, curTime, gStartTime, gFinishTime):
        ...
        if progressExpiryTime < finishTime:
            raise SoftException(b'Invalid progress expiry time. progressExpiryTime:%s < finishTime:%s (id:%s)' % (
    ...
    def __getConditionReaders(self, eventType):
        condition_readers = {b'greater': (self.__readCondition_DateTimeOrFloat), 
           ...
           b'convert': (self.__readBattleResultsConditionList), 
           b'expires': (self.__readCondition_DateTimeOrFloat), 
           b'limit': (self.__readCondition_int), 
           b'rate': (self.__readCondition_float), 
           ...
           b'wotPlus': (self.__readCondition_bool), 
           b'wotPlusDailyAttendance': (self.__readCondition_bool), 
           ...
           b'isWinbackQuestsEnabled': (self.__readCondition_bool), 
           b'isDailyQuestsEnabled': (self.__readCondition_bool), 
           b'isWeeklyQuestsEnabled': (self.__readCondition_bool), 
           b'dailyXPFactor': (self.__readBattleResultsConditionList), 
           b'isSteamAllowed': (self.__readCondition_bool), 
           b'isFirstLogin': (self.__readCondition_bool), 
           b'totalBattles': (self.__readBattleResultsConditionList), 
           b'lastLogout': (self.__readBattleResultsConditionList), 
           b'noviceType': (self.__readBattleResultsConditionList), 
           ...
           b'pmActiveOperation': (self.__readCondition_int)}
        if eventType in EVENT_TYPE.LIKE_BATTLE_QUESTS:
            condition_readers.update({b'value': (self.__readCondition_bool), 
               ...
               b'vehicleBlockedByArmor': (self.__readBattleResultsConditionList), 
               ...
               b'beyondVisionRadius': (self.__readCondition_true), 
               b'whileMoving': (self.__readCondition_true), 
               b'whileMovingAtSpeed': (self.__readCondition_int), 
               ...
               b'damageDealt': (self.__readCondition_int), 
               b'spotEnemy': (self.__readCondition_true), 
               b'directHitsReceived': (self.__readCondition_int), 
               b'whileStill': (self.__readCondition_true), 
               ...
               b'uniqueVehicles': (self.__readCondition_true), 
    ...
    def __getAvailableBonuses(self, eventType):
        if eventType in (EVENT_TYPE.BATTLE_QUEST, EVENT_TYPE.PERSONAL_QUEST, EVENT_TYPE.NT_QUEST):
            QUEST_BONUS_TYPES.update((b'xp', b'tankmenXP', b'xpFactor', b'creditsFactor', b'freeXPFactor', b'tankmenXPFactor'))
        if eventType in (EVENT_TYPE.NT_QUEST,):
            QUEST_BONUS_TYPES.update((b'vehicleXP', b'vehicleXPFactor'))
    ...
    def __readCondition_keyResults(self, _, section, node):
        name = section.asString
        if not (name in getBattleResultsNames() or name == b'addQuestCompleted'):
    ...
    def deserialize(self, serializedData):
        ...
        if NEAREST_TIME_BOUNDARY in auxData:
            if auxData[NEAREST_TIME_BOUNDARY] <= time.time():
                raise SoftException(b'Quest validation failed: Got an outdated quest!')
        return (
         rawData, auxData)
