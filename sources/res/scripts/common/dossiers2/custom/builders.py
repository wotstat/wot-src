...
getAccountDossierVersion = lambda dossierCompDescr: getDossierVersion(dossierCompDescr, b'<' + VERSION_RECORD_FORMAT, ACCOUNT_DOSSIER_VERSION)
getVehicleDossierVersion = lambda dossierCompDescr: getDossierVersion(dossierCompDescr, b'<' + VERSION_RECORD_FORMAT, VEHICLE_DOSSIER_VERSION)
getTankmanDossierVersion = lambda dossierCompDescr: getDossierVersion(dossierCompDescr, b'<' + VERSION_RECORD_FORMAT, TANKMAN_DOSSIER_VERSION)

def _initializeNewDossier(dossierType, dossier):
    if dossierType == DOSSIER_TYPE.ACCOUNT:
        dossier[b'total'][b'creationTime'] = int(time.time())
