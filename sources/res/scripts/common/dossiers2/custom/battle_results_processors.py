...
def updateAccountDossier(dossierDescr, battleResults, dossierXP, vehDossiers, maxVehResults, avatarResults):
    ...
    if checkAny(bonusType, BONUS_CAPS.DOSSIER_SORTIE):
        __updateAggregatedValues(dossierDescr.expand(b'fortSortiesInClan'), dossierDescr.expand(b'fortSortiesInClan'), battleResults, dossierXP, frags8p)
    if checkAny(bonusType, BONUS_CAPS.DOSSIER_FORT_BATTLE):
        __updateAggregatedValues(dossierDescr.expand(b'fortBattlesInClan'), dossierDescr.expand(b'fortBattlesInClan'), battleResults, dossierXP, frags8p)
    ...
    if checkAny(bonusType, BONUS_CAPS.DOSSIER_RANKED):
        seasonBlock = (b'rankedSeason{}').format(avatarResults[b'rankedSeasonNum'])
    ...
    if checkAny(bonusType, BONUS_CAPS.DOSSIER_MAXSORTIE):
        values = __updateMaxValues(dossierDescr.expand(b'maxFortSortiesInClan'), battleResults, dossierXP)
    ...
    if checkAny(bonusType, BONUS_CAPS.DOSSIER_MAXFORTBATTLE):
        values = __updateMaxValues(dossierDescr.expand(b'maxFortBattlesInClan'), battleResults, dossierXP)
    ...
    if checkAny(bonusType, BONUS_CAPS.DOSSIER_MAXRANKED):
        seasonBlock = (b'maxRankedSeason{}').format(avatarResults[b'rankedSeasonNum'])
...
def updateRated7x7Dossier(dossierDescr, battleResults, dossierXP):
    ...
    if BONUS_CAPS.checkAny(bonusType, BONUS_CAPS.DOSSIER_RATED7X7):
        __updateAggregatedValues(dossierDescr.expand(b'rated7x7'), dossierDescr.expand(b'rated7x7'), battleResults, dossierXP, frags8p=0)
    if BONUS_CAPS.checkAny(bonusType, BONUS_CAPS.DOSSIER_MAXRATED7X7):
        __updateMaxValues(dossierDescr.expand(b'maxRated7x7'), battleResults, dossierXP)
    return
...
def updatePotapovQuestAchievements(accDossierDescr, progress):
    ...
    for questID, (_, state) in viewitems(progress):
        ...
        if state >= potapov_quests.PQ_STATE.NEED_GET_ADD_REWARD:
            pqAchievements = tileInfo[b'achievements'] or {}
    ...
    for seasonID, minCounter, dossierBlockName, achievementName in ((1, 1, b'singleAchievements', b'firstMerit'),
     (1, 1, b'steamAchievements', b'steamDoPotapovQuestMedal'),
     (2, 5, b'singleAchievements', b'newMeritPM2'),
     (2, 1, b'steamAchievements', b'steamDoPotapovQuestMedal'),
     (3, 1, b'steamAchievements', b'steamDoPotapovQuestMedal'),
     (3, 1, b'singleAchievements', b'meritPM3'),
     (4, 1, b'singleAchievements', b'meritPM3'),
     (4, 1, b'steamAchievements', b'steamDoPotapovQuestMedal')):
    ...
    if completedBranches and not accDossierDescr[b'steamAchievements'][b'steamDoAllBranchPotapovQuestMedal']:
        accDossierDescr[b'steamAchievements'][b'steamDoAllBranchPotapovQuestMedal'] = True
...
def __updateDossierCommonPart(dossierType, dossierDescr, results, dossierXP, avatarResults):
    ...
    if BONUS_CAPS.checkAny(bonusType, BONUS_CAPS.DOSSIER_15X15):
        __updateAggregatedValues(dossierDescr.expand(b'a15x15'), dossierDescr.expand(b'a15x15_2'), results, dossierXP, frags8p)
    if BONUS_CAPS.checkAny(bonusType, BONUS_CAPS.DOSSIER_30X30):
        __updateAggregatedValues(dossierDescr.expand(b'a30x30'), dossierDescr.expand(b'a30x30'), results, dossierXP, frags8p)
    if BONUS_CAPS.checkAny(bonusType, BONUS_CAPS.DOSSIER_7X7):
        __updateAggregatedValues(dossierDescr.expand(b'a7x7'), dossierDescr.expand(b'a7x7'), results, dossierXP, frags8p)
    if BONUS_CAPS.checkAny(bonusType, BONUS_CAPS.DOSSIER_RATED7X7):
        __updateAggregatedValues(dossierDescr.expand(b'rated7x7'), dossierDescr.expand(b'rated7x7'), results, dossierXP, frags8p)
    ...
    if BONUS_CAPS.checkAny(bonusType, BONUS_CAPS.DOSSIER_FALLOUT):
        __updateAggregatedValues(dossierDescr.expand(b'fallout'), dossierDescr.expand(b'fallout'), results, dossierXP, frags8p)
    ...
    if BONUS_CAPS.checkAny(bonusType, BONUS_CAPS.DOSSIER_GLOBAL_MAP):
        if dossierDescr.isBlockInLayout(b'globalMapCommon'):
            __updateAggregatedValues(dossierDescr.expand(b'globalMapCommon'), dossierDescr.expand(b'globalMapCommon'), results, dossierXP, frags8p)
    if BONUS_CAPS.checkAny(bonusType, BONUS_CAPS.DOSSIER_SORTIE):
        __updateAggregatedValues(dossierDescr.expand(b'fortSorties'), dossierDescr.expand(b'fortSorties'), results, dossierXP, frags8p)
    if BONUS_CAPS.checkAny(bonusType, BONUS_CAPS.DOSSIER_FORT_BATTLE):
        __updateAggregatedValues(dossierDescr.expand(b'fortBattles'), dossierDescr.expand(b'fortBattles'), results, dossierXP, frags8p, determineWinnerTeam(avatarResults))
    if BONUS_CAPS.checkAny(bonusType, BONUS_CAPS.DOSSIER_EPIC_BATTLE):
        __updateAggregatedValues(dossierDescr.expand(b'epicBattle'), dossierDescr.expand(b'epicBattle'), results, dossierXP, frags8p)
    ...
    if BONUS_CAPS.checkAny(bonusType, BONUS_CAPS.DOSSIER_MAX15X15):
        maxValuesChanged = __updateMaxValues(dossierDescr.expand(b'max15x15'), results, dossierXP)
    if BONUS_CAPS.checkAny(bonusType, BONUS_CAPS.DOSSIER_MAX30X30):
        maxValuesChanged = __updateMaxValues(dossierDescr.expand(b'max30x30'), results, dossierXP)
    if BONUS_CAPS.checkAny(bonusType, BONUS_CAPS.DOSSIER_MAX7X7):
        maxValuesChanged = __updateMaxValues(dossierDescr.expand(b'max7x7'), results, dossierXP)
    if BONUS_CAPS.checkAny(bonusType, BONUS_CAPS.DOSSIER_MAXRATED7X7):
        maxValuesChanged = __updateMaxValues(dossierDescr.expand(b'maxRated7x7'), results, dossierXP)
    if BONUS_CAPS.checkAny(bonusType, BONUS_CAPS.DOSSIER_MAXSORTIE):
        maxValuesChanged = __updateMaxValues(dossierDescr.expand(b'maxFortSorties'), results, dossierXP)
    if BONUS_CAPS.checkAny(bonusType, BONUS_CAPS.DOSSIER_MAXFORTBATTLE):
        maxValuesChanged = __updateMaxValues(dossierDescr.expand(b'maxFortBattles'), results, dossierXP)
    ...
    if BONUS_CAPS.checkAny(bonusType, BONUS_CAPS.DOSSIER_GLOBAL_MAP):
        if dossierDescr.isBlockInLayout(b'maxGlobalMapCommon'):
            maxValuesChanged = __updateMaxValues(dossierDescr.expand(b'maxGlobalMapCommon'), results, dossierXP)
    if BONUS_CAPS.checkAny(bonusType, BONUS_CAPS.DOSSIER_MAXFALLOUT):
        maxValuesChanged = __updateMaxValues(dossierDescr.expand(b'maxFallout'), results, dossierXP)
    if BONUS_CAPS.checkAny(bonusType, BONUS_CAPS.DOSSIER_MAX_EPIC_BATTLE):
        maxValuesChanged = __updateMaxValues(dossierDescr.expand(b'maxEpicBattle'), results, dossierXP)
...
def __updateVehicleDossierImpl(vehTypeCompDescr, dossierDescr, results, dossierXP):
    ...
    if BONUS_CAPS.checkAny(bonusType, BONUS_CAPS.DOSSIER_RANKED):
        __updateAggregatedValues(dossierDescr.expand(b'ranked_10x10'), dossierDescr.expand(b'ranked_10x10'), results, dossierXP, 0)
    if BONUS_CAPS.checkAny(bonusType, BONUS_CAPS.DOSSIER_MAXRANKED):
        __updateMaxValues(dossierDescr.expand(b'maxRanked_10x10'), results, dossierXP)
    ...
    _updatePerBattleSeries(achievements, b'invincibleSeries', results[b'deathCount'] == 0 and results[b'damageReceived'] == 0 and not isSPG)
    _updatePerBattleSeries(achievements, b'diehardSeries', results[b'deathCount'] == 0 and not isSPG)
...
def __updateAccountDossierCuts(dossierDescr, results, dossierXP, vehTypeCompDescr, vehDossierDescr, avatarResults):
    ...
    if BONUS_CAPS.checkAny(bonusType, BONUS_CAPS.DOSSIER_RANKED):
        ...
        cut[vehTypeCompDescr] = (
         ...
         veh[b'wins'],
         veh[b'xp'])
        seasonBlock = (b'rankedCutSeason{}').format(avatarResults[b'rankedSeasonNum'])
    ...
    if BONUS_CAPS.checkAny(bonusType, BONUS_CAPS.DOSSIER_EPIC_BATTLE):
        ...
        epicBattleAccountDossierCut[vehTypeCompDescr] = (
         epicBattleVehicleDossier[b'battlesCount'],
         epicBattleVehicleDossier[b'wins'],
