...
def _set_TOTAL_STATS_DEPENDENCIES():
    ...
    TOTAL_STATS_DEPENDENCIES.update({b'treesCut': [
                   _updateLumberjack]})
    return
...
def _updateMedalCarius(dossierDescr, dossierBlockDescr, key, value, prevValue):
    ...
    medalCariusCfg = RECORD_CONFIGS[b'medalCarius']
    ...
    curClass = dossierDescr[b'achievements'][b'medalCarius']
    if curClass == 0 or curClass > medalClass:
        dossierDescr[b'achievements'][b'medalCarius'] = medalClass
    return
...
def _updateMedalKnispel(dossierDescr, dossierBlockDescr, key, value, prevValue):
    ...
    medalKnispelCfg = RECORD_CONFIGS[b'medalKnispel']
    ...
    curClass = dossierDescr[b'achievements'][b'medalKnispel']
    if curClass == 0 or curClass > medalClass:
        dossierDescr[b'achievements'][b'medalKnispel'] = medalClass
    return
...
def _updateBeasthunter(dossierDescr, dossierBlockDescr, key, value, prevValue):
    medals, _ = divmod(value, RECORD_CONFIGS[b'beasthunter'])
...
def _updateSinai(dossierDescr, dossierBlockDescr, key, value, prevValue):
    medals, _ = divmod(value, RECORD_CONFIGS[b'sinai'])
...
def _updatePattonValley(dossierDescr, dossierBlockDescr, key, value, prevValue):
    medals, _ = divmod(value, RECORD_CONFIGS[b'pattonValley'])
...
def _updateMousebane(dossierDescr, dossierBlockDescr, key, value, prevValue):
    medals, _ = divmod(value, RECORD_CONFIGS[b'mousebane'])
...
def _updateLumberjack(dossierDescr, dossierBlockDescr, key, value, prevValue):
    if value >= RECORD_CONFIGS[b'lumberjack'] and not dossierDescr[b'achievements'][b'lumberjack']:
        dossierDescr[b'achievements'][b'lumberjack'] = 1
        dossierDescr.addPopUp(b'achievements', b'lumberjack', 1)
    return
