...
_SUPPORTED_FORMATS = frozenset(b'cbBhHiIlLqQfd')

class IBlockBuilderWithRecordsLayout(object):
    name = None
    recordsLayout = []

    def build(self, dossierDescr, compDescr=b''):
...
class StaticSizeBlockBuilder(IBlockBuilderWithRecordsLayout):
    ...
    def __init__(self, name, recordsLayout, eventsHandlers, popUpRecords, logRecords=None):
        ...
        for record, recordPacking in layout:
            ...
            size = struct.calcsize(b'<' + fmt)
            recordPacking[b'type'] = type
            recordPacking[b'format'] = fmt
            recordPacking[b'offset'] = offset
            recordPacking[b'maxValue'] = maxValue
            initial[record] = 0
            offset += size
            self.__format += fmt
