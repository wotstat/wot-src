...
class SpaceVisibilityFlags(object):
    FLAGS_CONFIG_SECTION = b'spaceVisibilityFlags'
    _CONFIG_FILE = b'space.settings'
    _HANGAR_CONFIG_SECTION = b'hangarSettings'
    _CONFIG_DIRECTORY = b'spaces'
    _ERROR_BIT_INDEX = -1
    ...
    def __init__(self, geometryName):
        ...
        if not self.__formMapping(flagsSection):
            raise SoftException(b'Cannot create SpaceVisibilityFlags for space.settings %s' % settingsPath)
        return
    ...
    def __formMapping(self, flagsSection):
        if flagsSection is not None:
            ...
            for name, data in flagsSection.items():
                if name == b'type':
                    tag = data.readString(b'tag')
                    bitIndex = data.readInt(b'bitIndex', SpaceVisibilityFlags._ERROR_BIT_INDEX)
    ...
    def __formSettingPath(geometryName):
        return (b'{config_directory}/{geometryName}/{file}').format(config_directory=SpaceVisibilityFlags._CONFIG_DIRECTORY, geometryName=geometryName, file=SpaceVisibilityFlags._CONFIG_FILE)


def _openSection(path):
    section = ResMgr.openSection(path)
    if section is None:
        LOG_WARNING(b'space.settings file is missing %s' % path)
    return section
