...
OK = GM_ERROR.OK
GM_ERROR_NAMES = dict([(v, k) for k, v in GM_ERROR.__dict__.iteritems() if not k.startswith(b'_')])
...
class GlobalMapBase(OpsUnpacker):
    _opsFormatDefs = initOpsFormatDef({(GM_OP.UNPACK_BATTLE): (b'', b'_unpackBattle'), 
       (GM_OP.FINISH_BATTLE): (b'q', b'_finishBattle'), 
       (GM_OP.REMOVE_BATTLE): (b'q', b'_removeBattle'), 
       (GM_OP.UNPACK_BATTLE_UNIT): (b'', b'_unpackBattleUnit'), 
       (GM_OP.REMOVE_BATTLE_UNIT): (b'q', b'_removeBattleUnit')})
    FORMAT_HEADER = b'<ii'
    SIZE_HEADER = struct.calcsize(FORMAT_HEADER)
    FORMAT_ADD_BATTLE_HEADER = b'<qHii?'
    SIZE_ADD_BATTLE_HEADER = struct.calcsize(FORMAT_ADD_BATTLE_HEADER)
    FORMAT_ADD_BATTLE_UNIT_HEADER = b'<qh'
    ...
    def __repr__(self):
        s = b''
        if self.battles:
            s += b'battles(%s)' % len(self.battles)
    ...
    def pack(self, isForced=False):
        ...
        if self.battles:
            ...
            for battleID, data in self.battles.iteritems():
                peripheryID = data[b'peripheryID']
                createTime = data[b'createTime']
                startTime = data[b'startTime']
                isFinished = data[b'isFinished']
                localizedData = packPascalString(cPickle.dumps(data[b'localizedData'], -1))
    ...
    def unpack(self, packedData):
        ...
        for i in xrange(lenBattles):
            ...
            self.battles[battleID] = {b'peripheryID': peripheryID, 
               b'createTime': createTime, 
               b'startTime': startTime, 
               b'isFinished': isFinished, 
               b'localizedData': localizedData}
    ...
    def deserialize(self, pdata):
        if not isinstance(pdata, dict) or not pdata:
            ...
            return
        self._dirty = True
        self.battles = pdata[b'battles']
        self.battleUnits = {}
        try:
            self.pack()
        except Exception as e:
            LOG_CURRENT_EXCEPTION()
            LOG_DEBUG_DEV(b'GlobalMapBase.self: %s' % repr(self))
            raise e
    ...
    def _unpackBattle(self, packedData):
        ...
        self.battles[battleID] = {b'peripheryID': peripheryID, 
           ...
           b'startTime': startTime, 
           b'isFinished': isFinished, 
           b'localizedData': (cPickle.loads(localizedDataStr))}
    ...
    def _finishBattle(self, battleID):
        if battleID in self.battles:
            self.battles[battleID][b'isFinished'] = True
