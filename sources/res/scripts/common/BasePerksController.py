...
_DO_DEBUG_LOG = False
PERK_SHOT_DISPERSION_FACTOR = b'perkShotDispersion'
PERK_SHOT_DISPERSION_WHILE_GUN_DAMAGED_MOD = b'perkShotDispersionWhileGunDamagedMod'
...
class BasePerksController(object):
    ...
    def __init__(self, owner, scopedPerks):
        LOG_DEBUG_DEV((b'ABILITY_SYSTEM DEBUG scopedPerks: {} ').format(scopedPerks))
        self._owner = owner
        ...
        for factorName in factors:
            try:
                ...
                if factorName in self._modifiedFactors:
                    LOG_ERROR((b'ABILITY_SYSTEM ERROR factor {} was modified but does not have a collector.').format(factorName))
                continue

            collector(factorName, factors)

        if _DO_DEBUG_LOG and IS_DEVELOPMENT:
            LOG_DEBUG((b'ABILITY_SYSTEM DEBUG onCollectFactors, factors diff: {}').format([(b'{} {} -> {}').format(k, oldFactors[k], factors[k]) for k in self._modifiedFactors if k in factors and oldFactors[k] != factors[k]]))
        return
    ...
    def updateScopedPerks(self, newScopedPerks):
        ...
        for scope in self._scopeContextMap:
            ...
            self._logUpdatedPerks(b'updateScopedPerks. VehicleID = {}. Scope = {}. Added perks = {}', self.vehicleID, scope, addedPerks)
            ...
            self._logUpdatedPerks(b'updateScopedPerks. VehicleID: {}. Scope: {}. Removed perks: {}', self.vehicleID, scope, removedPerks)
            ...
            self._logUpdatedPerks(b'updateScopedPerks. VehicleID = {}. Scope = {}. Changed perks = {}', self.vehicleID, scope, changedPerks)
            if newScope is not None:
                ...
                for perkTuple in perksTuple:
                    ...
                    if plan is not None:
                        ...
                        LOG_ERROR((b'[PerksController] No plan for perkID:{0} vehicleID:{1} after applySelectedSetup ').format(perkID, self.vehicleID))
    ...
    def _findChangedPerks(cls, oldScopedPerks, newScopedPerks, perksToCheck):
        ...
        for perkTuple0 in perksTuple0:
            ...
            for perkTuple1 in perksTuple1:
                ...
                if len(skillData0) != len(skillData1) or skillData0.get(b'booster') != skillData1.get(b'booster') or skillData0.get(b'crew') != skillData1.get(b'crew'):
    ...
    def _buildAttributeFactorsCollectorMap(self):
        ...
        self._attrFactorCollectors.update({b'chassis/terrainResistance': (self._collectTerrainResistance), 
           b'chassis/terrainResistanceRotation': (self._collectTerrainResistance), 
           b'invisibility': (self._collectInvisibility), 
           b'damageMonitoringDelay': (self._collectMinAttributeFactor), 
           b'artNotificationDelay': (self._collectMinAttributeFactor)})
        return
    ...
    def _collectInvisibility(self, invisibilityKey, factors):
        ...
        invisibility[0] += self._collectModifiersAddScopes(self._modifiedFactors[b'invisibilityAdd'])
        invisibility[1] *= self._collectModifiersMulScopes(self._modifiedFactors[b'invisibilityMul'])
        return
