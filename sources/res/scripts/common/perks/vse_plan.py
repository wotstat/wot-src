...
def callOnValidState(f):
    ...
    def w(self, *args, **kwargs):
        if getattr(self, b'_plan') is None:
            LOG_DEBUG_DEV((b'VsePlan.{} called when plan is None').format(f.__name__))
            return
        else:
...
class VsePlan(object):
    _PLAN_PATH = b'vscript/plans/{}.xml'
    _PLAN_ID_PREFIX = b'abilityPerks/perk_'
    ...
    def __init__(self, owner, scopeId, level, perkId, onReadyCallback, contextArgs):
        ...
        LOG_DEBUG_DEV(b'init plan', self._ownerId, self._planId)
        return
    ...
    def load(self, contextCreator, isAutoStart=False):
        ...
        if not perks.g_cache.perks.validatePerk(self.perkId):
            LOG_ERROR(b"Perk '%s' not found" % self.perkId)
            return
        if IS_CELLAPP:
    ...
    def start(self):
        if self._isPlanStarted:
            ...
            if self._plan is not None:
                ...
                LOG_ERROR(b"VsePlan: Plan '%s' not created before start" % self._planId)
            return
    ...
    def triggerVSPlanEvent(self, event):
        ...
        if context is None:
            LOG_ERROR(b'VsePlan: context not ready or not set! Event: %s' % event)
            return
        else:
            if isinstance(event, str):
                context.triggerEvent(event)
            elif isinstance(event, tuple) and hasattr(event, b'_fields'):
    ...
    def destroy(self):
        if self._plan is not None:
            ...
            LOG_WARNING((b'[PerksController] No plan for perkID:{0} vehicleID:{1} after destroy in applySelectedSetup ').format(self.perkId, ownerId))
        LOG_DEBUG_DEV(b'destroy plan', self._ownerId, self._planId)
        self._plan = None
        self._isPlanLoaded = False
        self._isPlanStarted = False
        self._isAutoStart = False
    ...
    def _onPlanPreLoaded(self, future=None):
        try:
            LOG_DEBUG_DEV(b'_onPlanPreLoaded plan', self._ownerId, self._planId, self._plan)
            if self._owner is None or self._owner.isDestroyed:
                return
            if IS_CELLAPP:
                future.get()
            self._usedEvents = []
            self._isPlanLoaded = self._plan.load(self._planId, b'', (ASPECT.SERVER, ASPECT.CLIENT), [], self._usedEvents)
            ...
            LOG_ERROR(b"VsePlan: Plan xml '%s' not pre-loaded." % self._planId)
