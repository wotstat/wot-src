...
def init():
    ...
    for _, (typeXmlCtx, typeSection) in _xml.getItemsWithContext(xmlCtx, section, b'type'):
        ...
        if g_destructibleEntitiesCache.getDestructibleEntityType(destructibleEntityType.id) is not None:
            _xml.raiseWrongXml(typeXmlCtx, b'id', b'duplicate id' % destructibleEntityType.id)
...
def _readPointList(xmlCtx, section):
    result = []
    for _, ((stateCompXmlCtx, _), point) in _xml.getItemsWithContext(xmlCtx, section, b'point'):
...
DestructibleMaterialInfo = namedtuple(b'DestructibleMaterialInfo', (b'kind', b'armor', b'effectMaterialIdx', b'extra', b'vehicleDamageFactor', b'useHitAngle', b'mayRicochet', b'collideOnceOnly', b'checkCaliberForRichet', b'checkCaliberForHitAngleNorm'))
