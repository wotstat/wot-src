...
ProfileVehicle = namedtuple(b'ProfileVehicle', (b'vehCompDescr', b'vehOutfitCD', b'seasonType', b'marksOnGun', b'prestigeLevel', b'stFrags'))
...
UNIT_ASSEMBLER_IMPL_NAMES = {k.lower(): v for k, v in viewitems(UnitAssemblerImplType.__dict__) if isinstance(v, int) and not k.startswith(b'_')}
...
class UnitPlayerDataKey(object):
    ...
    ACCOUNT_NEW_WTR = b'accountNewWTR'
    HAS_WINBACK_TOKEN = b'hasWinbackToken'
...
class UnitBase(OpsUnpacker):
    ...
    def __init__(self, limitsDefs=None, slotDefs=None, slotCount=0, packedRoster=b'', extrasInit=None, packedUnit=b'', rosterTypeID=ROSTER_TYPE.UNIT_ROSTER, extrasHandlerID=EXTRAS_HANDLER_TYPE.EMPTY, prebattleTypeID=PREBATTLE_TYPE.UNIT):
    ...
    def _setEstimatedTimeInQueue(self, estimatedTimeInQueue):
        LOG_DEBUG_DEV(b'_setEstimatedTimeInQueue ', estimatedTimeInQueue)
    ...
    _VEHICLE_PROFILE_HEADER = b'<qBBII'
