...
UnitVehicle = namedtuple(b'UnitVehicle', (b'vehInvID', b'vehTypeCompDescr', b'vehLevel', b'vehClassIdx'))
ProfileVehicle = namedtuple(b'ProfileVehicle', (b'vehCompDescr', b'vehOutfitCD', b'seasonType', b'marksOnGun', b'statTrackFrags'))
...
VEHICLE_CLASS_SPG = VEHICLE_CLASS_INDICES.get(b'SPG', _BAD_CLASS_INDEX)
...
CMD_NAMES = dict([(v, k) for k, v in CLIENT_UNIT_CMD.__dict__.items() if not k.startswith(b'__')])
...
UNIT_ASSEMBLER_IMPL_TO_CONFIG = {(UnitAssemblerImplType.SQUAD): b'squad', 
   (UnitAssemblerImplType.EPIC): b'epic'}
...
VEHICLE_TAGS_GROUP_BY_UNIT_MGR_FLAGS = {(UNIT_MGR_FLAGS.EVENT): (
                          (b'event_battles',), tuple()), 
   (UNIT_MGR_FLAGS.EPIC): (
                         tuple(), BATTLE_MODE_VEHICLE_TAGS - {b'epic_battles'}), 
   (UNIT_MGR_FLAGS.BATTLE_ROYALE): (
                                  (b'battle_royale',), tuple()), 
   (UNIT_MGR_FLAGS.STRONGHOLD): (
                               tuple(), BATTLE_MODE_VEHICLE_TAGS - {b'clanWarsBattles'}), 
   (UNIT_MGR_FLAGS.COMP7): (
                          tuple(), BATTLE_MODE_VEHICLE_TAGS - {b'comp7'})}
...
UNIT_MGR_FLAGS_TO_UNIT_MGR_ENTITY_NAME = {(UNIT_MGR_FLAGS.EVENT): b'EventUnitMgr', 
   (UNIT_MGR_FLAGS.MAPBOX): b'MapBoxUnitMgr', 
   (UNIT_MGR_FLAGS.SQUAD): b'SquadUnitMgr', 
   (UNIT_MGR_FLAGS.EPIC): b'SquadUnitMgr', 
   (UNIT_MGR_FLAGS.BATTLE_ROYALE): b'SquadUnitMgr', 
   (UNIT_MGR_FLAGS.SPEC_BATTLE): b'SpecUnitMgr', 
   (UNIT_MGR_FLAGS.STRONGHOLD): b'StrongholdUnitMgr', 
   (UNIT_MGR_FLAGS.COMP7): b'Comp7UnitMgr', 
   (UNIT_MGR_FLAGS.RANKED): b'RankedUnitMgr'}
...
class UnitPlayerDataKey(object):
    ...
    OFFLINE = b'offline'
    IGRTYPE = b'igrType'
    PERIPHERY_ID = b'peripheryID'
    TIME_JOIN = b'timeJoin'
    RATING = b'rating'
    BATTLES_COUNT = b'battlesCount'
    MATCHMAKER_STATS = b'matchmakerStats'
    MAP_BLACKLIST = b'mapBlackList'
    ACCOUNT_WTR = b'accountWTR'
    CSR_RATING = b'CSRRating'
    CLAN_DBID = b'clanDBID'
    CLAN_ABBREV = b'clanAbbrev'
    CLAN_NAME = b'clanName'
    CLAN_ROLE = b'clanRole'
    IS_PREMIUM = b'isPremium'
    VEH_DICT = b'vehDict'
    VEH_BATTLES_COUNT = b'vehBattlesCount'
...
class UnitBase(OpsUnpacker):
    _opsFormatDefs = initOpsFormatDef({(UNIT_OP.SET_VEHICLE): (b'qIi', b'_setVehicle'), 
       (UNIT_OP.SET_MEMBER): (b'qB', b'_setMember'), 
       (UNIT_OP.DEL_MEMBER): (b'B', b'_delMemberBySlot'), 
       (UNIT_OP.ADD_PLAYER): (b'', b'_unpackPlayer'), 
       (UNIT_OP.REMOVE_PLAYER): (b'q', b'_removePlayer'), 
       (UNIT_OP.READY_MASK): (b'H', b'_setReadyMask'), 
       (UNIT_OP.SET_SLOT): (b'', b'_unpackRosterSlot'), 
       (UNIT_OP.CLEAR_VEHICLE): (b'q', b'_clearVehicle'), 
       (UNIT_OP.VEHICLE_DICT): (b'', b'_unpackVehicleDict'), 
       (UNIT_OP.UNIT_FLAGS): (b'H', b'_setUnitFlags'), 
       (UNIT_OP.CLOSE_SLOT): (b'B', b'_closeSlot'), 
       (UNIT_OP.OPEN_SLOT): (b'B', b'_openSlot'), 
       (UNIT_OP.SET_COMMENT): (
                             b'', b'setComment', b'S', [b'']), 
       (UNIT_OP.CHANGE_ROLE): (b'qH', b'_changePlayerRole'), 
       (UNIT_OP.MODAL_TIMESTAMP): (b'i', b'_setModalTimestamp'), 
       (UNIT_OP.GIVE_LEADERSHIP): (b'Q', b'_giveLeadership'), 
       (UNIT_OP.EXTRAS_UPDATE): (
                               b'', b'updateUnitExtras', b'S', [b'']), 
       (UNIT_OP.EXTRAS_RESET): (None, b'resetExtras'), 
       (UNIT_OP.GAMEPLAYS_MASK): (b'i', b'_setGameplaysMask'), 
       (UNIT_OP.SET_VEHICLE_LIST): (
                                  b'q', b'_setVehicleList', b'N', [(b'H', b'iI')]), 
       (UNIT_OP.ARENA_TYPE): (b'i', b'_setArenaType'), 
       ...
       (UNIT_OP.REMOVE_SEARCH_FLAGS): (b'H', b'removeAutoSearchFlags')})
    MAX_PLAYERS = 250

    def __init__(self, limitsDefs={}, slotDefs={}, slotCount=0, packedRoster=b'', extrasInit=None, packedUnit=b'', rosterTypeID=ROSTER_TYPE.UNIT_ROSTER, extrasHandlerID=EXTRAS_HANDLER_TYPE.EMPTY, prebattleTypeID=PREBATTLE_TYPE.UNIT):
        if packedUnit:
    ...
    def _delMemberBySlot(self, slotIdx):
        ...
        if not member:
            ...
            accountDBID = member[b'accountDBID']
            self.setMemberReady(accountDBID, False)
    ...
    def _setProfileVehicle(self, accountDBID, vehCompDescr, vehOutfitCD, seasonType, marksOnGun, statTrackFrags):
        LOG_DEBUG((b'_setProfileVehicle: accountDBID={}').format(accountDBID))
    ...
    def _delProfileVehicle(self, accountDBID):
        LOG_DEBUG((b'_delProfileVehicle: accountDBID={}').format(accountDBID))
        self._playerProfileVehicles.pop(accountDBID, None)
    ...
    def _setEstimatedTimeInQueue(self, estimatedTimeInQueue):
        LOG_DEBUG_DEV(b'_setEstimatedTimeInQueue %d' % estimatedTimeInQueue)
    ...
    def _setRosterSlot(self, rosterSlotIdx, packedSlot):
        ...
        if slot.vehTypeCompDescr is None and slot.nationMask == 0 and slot.vehClassMask == 0:
            ...
            if neighbourSlot and packedSlot == roster.DEFAULT_SLOT_PACK:
                LOG_DEBUG(b'_setRosterSlot: removing default slotIdx=%s' % rosterSlotIdx)
                roster.slots.pop(rosterSlotIdx, None)
            else:
                roster.slots[rosterSlotIdx] = slot
                if neighbourSlot and neighbourSlot.pack() == roster.DEFAULT_SLOT_PACK:
                    LOG_DEBUG(b'_setRosterSlot: removing default neighbour slotIdx=%s' % rosterSlotIdx)
    ...
    _SLOT_PLAYERS = b'<Bq'
    _IDS = b'<IBI'
    _VEHICLE_DICT_HEADER = b'<Hq'
    _VEHICLE_DICT_ITEM = b'<Ii'
    _VEHICLE_PROFILE_HEADER = b'<qBBi'
    _PLAYER_SEARCH_FLAGS_TUPLE = b'<qH'
    ...
    def pack(self):
        ...
        for slotIdx, member in members.iteritems():
            packed += struct.pack(self._SLOT_PLAYERS, slotIdx, member[b'accountDBID'])
    ...
    def getCommanderDBID(self):
        return self._members.get(LEADER_SLOT, {}).get(b'accountDBID', 0)
    ...
    def updateUnitExtras(self, updateStr):
        ...
        LOG_DEBUG_DEV(b'updateUnitExtras', oldExtras, newExtras)
        self.storeOp(UNIT_OP.EXTRAS_UPDATE, updateStr)
        for accountDBID, playerData in self._players.iteritems():
            if playerData and playerData.get(b'role', 0) & UNIT_ROLE.INVITED == 0:
    ...
    def __repr__(self):
        ...
        for accountDBID, playerData in self._players.iteritems():
            repr += b'\n    [%d] %s role=%s' % (accountDBID, playerData,
             reprBitMaskFromDict(playerData.get(b'role', 0), UNIT_ROLE_NAMES))
    ...
    def dump(self):
        ...
        for slotIdx, member in self._members.iteritems():
            repr += b'%d:%s, ' % (slotIdx, member.get(b'accountDBID', 0))

        repr += b'},'
        repr += b'\n state=%02X, rdy=%02X, fullRdy=%02X, closed=%02X' % (
         self._flags, self._readyMask, self._fullReadyMask, self._closedSlotMask)
        repr += b', stamp:%s, timeInQueue:%d, free=%r' % (self._modalTimestamp, self._estimatedTimeInQueue,
         list(self._freeSlots))
        repr += b'\n vehs(%s)={' % len(self._vehicles)
        for accountDBID, veh in self._vehicles.iteritems():
            repr += b'%d:%s, ' % (accountDBID, str(veh))

        repr += b'},'
        repr += b'\n plrs(%s)={' % len(self._players)
        for accountDBID, playerData in self._players.iteritems():
            repr += b'%d:%r:%02X, ' % (accountDBID, playerData.get(b'nickName', b''),
             playerData.get(b'role', 0))
    ...
    def _setSquadSize(self, newSquadSize):
        LOG_DEBUG_DEV(b'_setSquadSize', newSquadSize, len(self._players), self._squadSize)
    ...
    def getPointsSum(self):
        sum = 0
        for slotIdx, member in self._members.iteritems():
            accountDBID = member.get(b'accountDBID', 0)
            vehs = self._vehicles.get(accountDBID)
            if vehs:
                sum += vehs[0].vehLevel
    ...
    def checkVehicleLevelsRange(self, lvlsByClass, commonLvls):
        for slotIdx, member in self._members.iteritems():
            accountDBID = member.get(b'accountDBID', None)
    ...
    def _unpackRosterSlot(self, packedOps):
        rosterSlotIdx = struct.unpack_from(b'<B', packedOps)[0]
    ...
    def _packFullVehDictUpdates(self):
        for accountDBID, playerData in self._players.iteritems():
            if playerData and playerData.get(b'role', 0) & UNIT_ROLE.INVITED == 0:
                vehDict = playerData.get(b'vehDict')
                if vehDict:
    ...
    def _giveLeadership(self, newLeaderDBID):
        ...
        prevLeaderDBID = self._members[LEADER_SLOT][b'accountDBID']
    ...
    def __packPlayerData(self, accountDBID, **kwargs):
        ...
        packed += packPascalString(kwargs.get(UnitPlayerDataKey.NICKNAME, b''))
        packed += packPascalString(kwargs.get(UnitPlayerDataKey.CLAN_ABBREV, b''))
        badges = kwargs.get(UnitPlayerDataKey.BADGES, BadgesCommon.selectedBadgesEmpty())
        packed += BadgesCommon.packPlayerBadges(badges)
        packed += self.__packPlayerExtraData(self._makePlayerExtraDataForClient(kwargs.get(b'extraData', {})))
        return packed
