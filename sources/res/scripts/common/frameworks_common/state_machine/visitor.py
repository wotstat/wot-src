...
def getAncestors(node, upper=None):
    if not isinstance(node, Node):
        raise NodeError((b'Invalid argument "node" = {}').format(node))
    if upper is not None and not isinstance(upper, Node):
        raise NodeError((b'Invalid argument "upper" = {}').format(upper))
    result = []
...
def isDescendantOf(node, ancestor):
    ...
    if not isinstance(ancestor, Node):
        raise NodeError((b'Invalid argument "ancestor" = {}').format(ancestor))
    found = node.getParent()
