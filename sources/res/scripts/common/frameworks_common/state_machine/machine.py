...
class StateMachine(_states.State):
    __slots__ = (b'__isRunning', b'_entered', b'__history', b'__observers')

    def __init__(self, stateID=b'', flags=_states.StateFlags.UNDEFINED):
    ...
    def _process(self, transitions, event):
        ...
        _logger.debug(b'%r: Snapshot before exiting states = %r', self, self._entered)
        exited = self.__exit(transitions)
        _logger.debug(b'%r: Snapshot after exiting states = %r', self, self._entered)
        ...
        _logger.debug(b'%r: Snapshot after entering states = %r', self, self._entered)
    ...
    def __notify(self, states, flag, event):
        for state in states:
            ...
            if not stateID:
                _logger.warning(b'%r: %r has no ID, can not notify observers', self, state)
                continue
