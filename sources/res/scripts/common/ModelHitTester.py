...
class HitTesterManager(with_metaclass(ReflectionMetaclass, object)):
    NORMAL_MODEL_TAG = b'normal'
    CRASHED_MODEL_TAG = b'crashed'
    CLIENT_MODEL_TAG = b'collisionModelClient'
    SERVER_MODEL_TAG = b'collisionModelServer'
    CLIENT_CAPSULE_TAG = b'capsuleScale'
    ...
    def __createHitTester(self, section, modelTag=None):
        ...
        if not bspModelName:
            raise SoftException(b'<%s> is missing or wrong' % modelTag)
        modelTagDown = modelTag + b'Down'
        bspModelNameDown = section.readString(modelTagDown)
        modelTagUp = modelTag + b'Up'
...
class ModelHitTester(object):
    __slots__ = (b'__bspModel', b'__bspModelName', b'__bspModelDown', b'__bspModelNameDown', b'__bspModelUp', b'__bspModelNameUp', b'__extraCapsuleScale', b'bbox', b'bboxDown', b'bboxUp')
    ...
    def loadBspModel(self):
        if self.__bspModel is not None:
            _logger.error(b'Can not load bsp model, because it is already loaded!')
            return
        else:
            if self.bspModelName is None:
                _logger.error(b'Can not load bsp model, bspModelName is None')
                return
            bspModel = BigWorld.WGBspCollisionModel()
            if not bspModel.setModelName(self.bspModelName):
                raise SoftException(b"wrong collision model '%s'" % self.bspModelName)
            self.__bspModel = bspModel
            self.bbox = bspModel.getBoundingBox()
            if not self.bbox:
                _logger.error(b"Couldn't find bounding box for the part the name '%s'", self.bspModelName)
            if self.__bspModelNameDown:
                bspModel = BigWorld.WGBspCollisionModel()
                if not bspModel.setModelName(self.__bspModelNameDown):
                    raise SoftException(b"wrong collision model '%s'" % self.__bspModelNameDown)
            ...
            if self.__bspModelNameUp:
                ...
                if not bspModel.setModelName(self.__bspModelNameUp):
                    raise SoftException(b"wrong collision model '%s'" % self.__bspModelNameUp)
    ...
    def localHitTestFull_debug(self, start, stop, value=0):
        LOG_DEBUG(b'localHitTestFull_debug', self.bspModelName, start, stop)
...
SegmentCollisionResult = namedtuple(b'SegmentCollisionResult', (b'dist', b'hitAngleCos', b'armor'))
if hasattr(BigWorld, b'wg_segmentMayHitVolume'):
    segmentMayHitVolume = BigWorld.wg_segmentMayHitVolume
if hasattr(BigWorld, b'wg_coneMayHitVolume'):
