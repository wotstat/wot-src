...
class CustomizationOutfit(SerializableComponent):
    ...
    fields = OrderedDict((
     ...
     (
      b'overrideDefaultAttachments', intField())))
    __slots__ = (b'modifications', b'paints', b'camouflages', b'decals', b'styleId', b'projection_decals', b'insignias', b'personal_numbers', b'sequences', b'overrideDefaultAttachments', b'attachments', b'styleProgressionLevel', b'serial_number')
    ...
    def makeCompDescr(self):
        ...
        for typeId in CustomizationType.APPLIED_TO_TYPES:
            componentsAttrName = (b'{}s').format(lower(CustomizationTypeNames[typeId]))
    ...
    def dismountComponents(self, applyArea, dismountTypes=CustomizationType.DISMOUNT_TYPE):
        ...
        for c11nType in dismountTypes:
            if c11nType is CustomizationType.STYLE:
                continue
            components = getattr(self, (b'{}s').format(lower(CustomizationTypeNames[c11nType])))
         ...
         (
          ApplyArea.GUN_REGIONS_VALUE, vehDescr.gun)):
            for componentName, (area, _) in vehiclePart.customizableVehicleAreas.iteritems():
                components = getattr(self, (b'{}s').format(lower(componentName)))
    ...
    def countComponents(self, componentId, typeId):
        ...
        if typeId in CustomizationType.APPLIED_TO_TYPES:
            outfitComponents = getattr(self, (b'{}s').format(lower(CustomizationTypeNames[typeId])))
    ...
    def removeComponent(self, componentId, typeId, count):
        ...
        if count > 0:
            if typeId in CustomizationType.APPLIED_TO_TYPES:
                attr = (b'{}s').format(lower(CustomizationTypeNames[typeId]))
