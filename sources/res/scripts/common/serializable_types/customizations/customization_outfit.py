...
__all__ = (b'CustomizationOutfit', b'getAllItemsFromOutfit')

def _setComponentsRegion(component, region):
    if hasattr(component, b'appliedTo'):
        setattr(component, b'appliedTo', region)
    elif hasattr(component, b'slotId'):
        setattr(component, b'slotId', region)
    else:
        LOG_ERROR((b'Unable to set region {0} for component {1}').format(region, component))
    return
...
class CustomizationOutfit(with_metaclass(ReflectionMetaclass, SerializableComponent)):
    ...
    fields = OrderedDict((
     (
      b'modifications', intArrayField()),
     (
      b'paints', customArrayField(PaintComponent.customType)),
     (
      b'camouflages', customArrayField(CamouflageComponent.customType)),
     (
      b'decals', customArrayField(DecalComponent.customType)),
     (
      b'styleId', intField(nonXml=True)),
     (
      b'projection_decals', customArrayField(ProjectionDecalComponent.customType)),
     (
      b'insignias', customArrayField(InsigniaComponent.customType)),
     (
      b'personal_numbers', customArrayField(PersonalNumberComponent.customType)),
     (
      b'sequences', customArrayField(SequenceComponent.customType)),
     (
      b'attachments', customArrayField(AttachmentComponent.customType)),
     (
      b'styleProgressionLevel', intField()),
     (
      b'serial_number', strField()),
     (
      b'stat_trackers', customArrayField(StatTrackerComponent.customType))))
    __slots__ = (b'modifications', b'paints', b'camouflages', b'decals', b'styleId', b'projection_decals', b'insignias', b'personal_numbers', b'sequences', b'attachments', b'styleProgressionLevel', b'serial_number', b'stat_trackers')
    ...
    def __init__(self, modifications=None, paints=None, camouflages=None, decals=None, projection_decals=None, personal_numbers=None, styleId=0, insignias=None, sequences=None, attachments=None, styleProgressionLevel=0, serial_number=None, stat_trackers=None):
        ...
        self.serial_number = serial_number or b''
        self.stat_trackers = stat_trackers or []
        super(CustomizationOutfit, self).__init__()
        return

    def __bool__(self):
        for v in getattr(type(self), b'__slots__'):
            if getattr(self, v):
                return True
    ...
    def makeCompDescr(self):
        ...
        for typeId in CustomizationType.APPLIED_TO_TYPES:
            componentsAttrName = (b'{}s').format(CustomizationTypeNames[typeId].lower())
    ...
    def dismountComponents(self, applyArea, dismountTypes=CustomizationType.DISMOUNT_TYPE):
        ...
        for c11nType in dismountTypes:
            if c11nType is CustomizationType.STYLE:
                continue
            components = getattr(self, (b'{}s').format(CustomizationTypeNames[c11nType].lower()))
         ...
         (
          ApplyArea.GUN_REGIONS_VALUE, vehDescr.gun)):
            for componentName, (area, _) in viewitems(vehiclePart.customizableVehicleAreas):
                components = getattr(self, (b'{}s').format(componentName.lower()))
    ...
    def countComponents(self, componentId, typeId):
        ...
        if typeId in CustomizationType.APPLIED_TO_TYPES:
            outfitComponents = getattr(self, (b'{}s').format(CustomizationTypeNames[typeId].lower()))
    ...
    def removeComponent(self, componentId, typeId, count):
        ...
        if count > 0:
            if typeId in CustomizationType.APPLIED_TO_TYPES:
                attr = (b'{}s').format(CustomizationTypeNames[typeId].lower())
        ...
        for itemDescr, count in listitems(outfitItems):
            ...
            if cid == CustomizationType.STYLE and item.isRent or item.isProgressive():
                ...
                try:
                    ...
                    if not self.styleId or b'styleOnly' not in item.tags:
                        isNeedRemove = True
