...
def getDefaultChassisXPhysicsCfg():
    return {b'wheelRadius': 0.4, 
       ...
       b'gimletCOMOffset': ZERO_VECTOR3, 
...
def getDefaultWheeledChassisXPhysicsCfg():
    return dict(getDefaultChassisXPhysicsCfg(), **{b'axleSteeringLockAngles': (0.0, 0.0, 0.0, 30.0), 
       ...
       b'isWheeledOnMoveRotation': False})
...
def updateCommonConf():
    BigWorld.wg_setupPhysicsParam(b'CONTACT_ENERGY_POW', CONTACT_ENERGY_POW)
    BigWorld.wg_setupPhysicsParam(b'CONTACT_ENERGY_POW2', CONTACT_ENERGY_POW2)
    BigWorld.wg_setupPhysicsParam(b'SLOPE_FRICTION_FUNC_DEF', SLOPE_FRICTION_FUNC_DEF)
    BigWorld.wg_setupPhysicsParam(b'SLOPE_FRICTION_FUNC_VAL', SLOPE_FRICTION_FUNC_VAL)
    BigWorld.wg_setupPhysicsParam(b'SLOPE_FRICTION_MODELS_FUNC_VAL', SLOPE_FRICTION_MODELS_FUNC_VAL)
    BigWorld.wg_setupPhysicsParam(b'CONTACT_FRICTION_TERRAIN', CONTACT_FRICTION_TERRAIN)
    BigWorld.wg_setupPhysicsParam(b'CONTACT_FRICTION_STATICS', CONTACT_FRICTION_STATICS)
    BigWorld.wg_setupPhysicsParam(b'CONTACT_FRICTION_EXTRA', CONTACT_FRICTION_EXTRA)
    BigWorld.wg_setupPhysicsParam(b'CONTACT_FRICTION_DESTRUCTIBLES', CONTACT_FRICTION_DESTRUCTIBLES)
    BigWorld.wg_setupPhysicsParam(b'CONTACT_FRICTION_VEHICLES', CONTACT_FRICTION_VEHICLES)
    BigWorld.wg_setupPhysicsParam(b'VEHICLE_ON_BODY_DEFAULT_FRICTION', VEHICLE_ON_BODY_DEFAULT_FRICTION)
    BigWorld.wg_setupPhysicsParam(b'ROLLER_REACTION_COEFF_FOR_STATIC', ROLLER_REACTION_COEFF_FOR_STATIC)
    BigWorld.wg_setupPhysicsParam(b'ROLLER_FRICTION_GAIN_MIN', ROLLER_FRICTION_GAIN_MIN)
    BigWorld.wg_setupPhysicsParam(b'ROLLER_FRICTION_GAIN_MAX', ROLLER_FRICTION_GAIN_MAX)
    BigWorld.wg_setupPhysicsParam(b'ROLLER_FRICTION_ANGLE_MIN', ROLLER_FRICTION_ANGLE_MIN)
    BigWorld.wg_setupPhysicsParam(b'ROLLER_FRICTION_ANGLE_MAX', ROLLER_FRICTION_ANGLE_MAX)
    BigWorld.wg_setupPhysicsParam(b'ARENA_BOUNDS_FRICTION_HOR', ARENA_BOUNDS_FRICTION_HOR)
    BigWorld.wg_setupPhysicsParam(b'ARENA_BOUNDS_FRICTION_VERT', ARENA_BOUNDS_FRICTION_VERT)
    BigWorld.wg_setupPhysicsParam(b'USE_PSEUDO_CONTACTS', USE_PSEUDO_CONTACTS)
    BigWorld.wg_setupPhysicsParam(b'CONTACT_PENETRATION', CONTACT_PENETRATION)
    BigWorld.wg_setupPhysicsParam(b'WARMSTARTING_VEHICLE_VEHICLE', WARMSTARTING_VEHICLE_VEHICLE)
    BigWorld.wg_setupPhysicsParam(b'WARMSTARTING_VEHICLE_STATICS', WARMSTARTING_VEHICLE_STATICS)
    BigWorld.wg_setupPhysicsParam(b'WARMSTARTING_THRESHOLD', WARMSTARTING_THRESHOLD)
    return
...
def configurePhysics(physics, baseCfg, typeDescr, gravityFactor, updateSiegeModeFromCfg):
    ...
    for name, mode in viewitems(cfg[b'modes']):
        tDescr = typeDescr.siegeVehicleDescr if typeDescr.hasSiegeMode and name == b'siegeMode' else typeDescr
...
def updatePhysics(physics, typeDesc, isSoftUpdate=False, gravityMultiplier=1.0):
    ...
    baseCfg = defaultVehicleDescr.type.xphysics[b'detailed']
    ...
    cfg = __buildConfigurations(cfg)
    for name, mode in viewitems(cfg[b'modes']):
        tDescr = typeDesc.siegeVehicleDescr if typeDesc.hasSiegeMode and name == b'siegeMode' else typeDesc
        if isSoftUpdate:
            applyVehDescrMiscFactors(tDescr, mode)
        configurePhysicsMode(mode, tDescr, gravityFactor)
...
def configurePhysicsMode(cfg, typeDesc, gravityFactor):
    ...
    cfg[b'gimletCOMOffset'] = cfg[b'chassis'][b'gimletCOMOffset']
    ...
    cfg[b'railCOMOffset'] = cfg[b'chassis'][b'railCOMOffset']
    ...
    cfg[b'engine'][b'smplEngJoinRatio'] = ENGINE_RADIUS / cfg[b'chassis'][b'wheelRadius']
    ...
    cfg[b'gimlet'][b'gimletCOMOffset'] = cfg[b'gimletCOMOffset']
    ...
    cfg[b'rail'][b'railCOMOffset'] = cfg[b'railCOMOffset']
