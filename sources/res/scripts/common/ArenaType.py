...
def _readCache(isFullCache):
    ...
    for key, value in rootSection.items():
        ...
        if key != b'map':
            continue
        if value.readBool(b'isHangar'):
            if IS_CLIENT:
                SpaceVisibilityFlagsFactory.create(value.readString(b'name'))
            continue
    ...
    return (g_cache, g_geometryCache, g_spaceCache, g_geometryNamesToIDs, g_gameplayNames, g_gameplaysMask, svfCache)
...
class _CacheSerializer(WGPickleSerializer):
    ...
    def deserialize(self, serializedData):
        ...
        svfCache = SpaceVisibilityFlagsFactory._g_cache
        return (cache, geometryCache, spaceCache, geometryNamesToIDs, gameplayNames, gameplaysMask, svfCache)
...
def init(isFullCache=True):
    pdc.load(b'arena_type_cache', partial(_readCache, isFullCache), _CacheSerializer())
    return
...
def __readBonusTypeCfgs(geometryName, section, defaultXml, bonusType):
    overrides = section[b'bonusTypeOverrides'] or defaultXml[b'bonusTypeOverrides']
    if overrides is None:
        ...
        try:
            cfg = {}
            if IS_CELLAPP or IS_BASEAPP:
                cfg[b'estimatedLoad'] = _readFloat(b'estimatedLoad', bonusOverrides, defaultXml, ARENA_ESTIMATED_LOAD_DEFAULT)
            if __hasKey(b'maxPlayersInTeam', bonusOverrides, defaultXml):
                cfg[b'maxPlayersInTeam'] = __readMaxPlayersInTeam(bonusOverrides, defaultXml)
            if __hasKey(b'runDelay', bonusOverrides, defaultXml):
                cfg[b'runDelay'] = _readInt(b'runDelay', bonusOverrides, defaultXml)
            if __hasKey(b'runDelayDev', bonusOverrides, defaultXml):
                cfg[b'runDelayDev'] = _readInt(b'runDelayDev', bonusOverrides, defaultXml)
...
def __readGameplayCfg(gameplayName, section, defaultXml, geometryCfg):
    try:
        ...
        for setting in (b'battleEndWarningAppearTime', b'battleEndWarningDuration', b'battleEndingSoonTime'):
            cfg[setting] = 0
...
def __readTeamNumbers(section, maxTeamsInArena):
    if not (section.has_key(b'squadTeamNumbers') or section.has_key(b'soloTeamNumbers')):
        ...
        return (set(), set())
    squadTeamNumbers = {int(v) for v in section.readString(b'squadTeamNumbers', b'').split()}
    soloTeamNumbers = {int(v) for v in section.readString(b'soloTeamNumbers', b'').split()}
