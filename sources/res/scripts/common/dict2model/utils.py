...
_ISO8601_RE = re.compile(b'(?P<year>\\d{4})-(?P<month>\\d{1,2})-(?P<day>\\d{1,2})[T ](?P<hour>\\d{1,2}):(?P<minute>\\d{1,2})(?::(?P<second>\\d{1,2})(?:\\.(?P<microsecond>\\d{1,6})\\d{0,6})?)?(?P<tzinfo>Z|[+-]\\d{2}(?::?\\d{2})?)?$')
...
class UTC(datetime.tzinfo):
    ...
    def localize(self, dt, is_dst=False):
        if dt.tzinfo is not None:
            raise SoftException(b'Not naive datetime (tzinfo is already set)')
    ...
    def normalize(self, dt, is_dst=False):
        if dt.tzinfo is self:
            ...
            if dt.tzinfo is None:
                raise SoftException(b'Naive time - no tzinfo set')
...
def fromIso(dateString):
    if not _ISO8601_RE.match(dateString):
        raise SoftException(b'Not a valid ISO8601-formatted string.')
    if b'.' in dateString:
        dtNomsTz, mstz = dateString.split(b'.')
        msNoTz = mstz[:len(mstz) - len(mstz.lstrip(b'0123456789'))]
        datestring = (b'.').join((dtNomsTz, msNoTz))
        return datetime.datetime.strptime(datestring[:26], b'%Y-%m-%dT%H:%M:%S.%f')
    return datetime.datetime.strptime(dateString[:19], b'%Y-%m-%dT%H:%M:%S')
...
def castToNumber(numType, value, exceptionClass=SoftException):
    try:
        ...
        raise exceptionClass(b'Not a valid number.')
    except OverflowError:
        raise exceptionClass(b'Number too large.')
...
def _mergeDicts(destination, source, deep=True):
    ...
    for key in source:
        if key in destination:
            ...
            if isinstance(dstValue, set) and isinstance(srcValue, set):
