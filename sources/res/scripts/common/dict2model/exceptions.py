...
class ValidationErrorMessage(object):
    ...
    def __add__(self, other):
        if not isinstance(other, ValidationErrorMessage):
            raise SoftException((b'Unsupported error message type: {}').format(type(other)))
...
class AccessToFieldDeniedError(SoftException):
    ...
    def __init__(self, message):
        errorMsg = b'Field is not public and can`t be used. Method: %s.' % message
