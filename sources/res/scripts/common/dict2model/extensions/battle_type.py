...
class BattleTypeModel(models.Model):
    __slots__ = (b'arenaBonusType', b'gameplayNames')
    ...
    def _reprArgs(self):
        return (b'arenaBonusType={}, gameplayNames={}').format(self.arenaBonusType, self.gameplayNames)


_battleTypeSchema = Schema[BattleTypeModel](fields={b'arenaBonusType': (String(required=True, deserializedValidators=validate.OneOf(ARENA_BONUS_TYPE_NAMES))), 
   b'gameplayNames': (UniCapList(fieldOrSchema=String(deserializedValidators=validate.OneOf(ARENA_GAMEPLAY_NAMES)), required=False, default=list))}, checkUnknown=True, modelClass=BattleTypeModel)

class BattleTypesModel(models.Model):
    __slots__ = (b'battleTypes', b'_gameplayByBattleType')
    ...
    def _reprArgs(self):
        return (b'battleTypes={}').format(self.battleTypes)


BattleTypesModelType = typing.TypeVar(b'BattleTypesModelType', bound=BattleTypesModel)
...
class BattleTypesSchema(Schema[BattleTypesModelType]):
    ...
    def __init__(self, fields, modelClass=BattleTypesModel, checkUnknown=True, serializedValidators=None, deserializedValidators=None):
        fields_ = {b'battleTypes': (UniCapList(fieldOrSchema=_battleTypeSchema, required=False, default=list))}
