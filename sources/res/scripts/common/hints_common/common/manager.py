    ...
    if section is None:
        ...
        if not isinstance(hints, list):
            hints = [
             hints]
        if not hints:
            _logger.debug(b'File <%s> section [%s] empty or does not exist.', xmlPath, HINTS_TAG)
        ResMgr.purge(xmlPath, True)
        return hints


class BaseHintsModelsManager(object):
    __slots__ = (b'_importedSchemas', b'_defaultSchema', b'_schemaTag')
    ...
    def _registerFromXml(self, xmlPath):
        ...
        for index, rawHintData in enumerate(_readHintsXml(xmlPath)):
            try:
                schemaLocation = rawHintData.pop(self._schemaTag, b'') if isinstance(rawHintData, dict) else b''
                ...
                error = exceptions.ValidationErrorMessage(ve.error.data, title=(b'Hint[{}]').format(index))
                errors = errors + error if errors else error

        try:
            self._validateRegistered()
        except exceptions.ValidationError as ve:
            error = exceptions.ValidationErrorMessage(ve.error.data, title=b'Validate registered models')
            errors = errors + error if errors else error
    ...
    def _importSchema(self, location):
        if not isinstance(location, str):
            raise exceptions.ValidationError((b'Wrong schema location type. {} != str.').format(type(location)))
        if location in self._importedSchemas:
            ...
            if len(locationParts) != 2:
                raise exceptions.ValidationError(b'Wrong schema location format. Example: schemas.base:magicSchema.')
            modulePath, name = locationParts
            try:
                module = importlib.import_module(modulePath)
            except ImportError:
                raise exceptions.ValidationError((b'Wrong schema module path: {}.').format(modulePath))

            schema = getattr(module, name, None)
            if schema is None:
                raise exceptions.ValidationError((b'Wrong schema variable name: {}.').format(name))
            self._checkSchemaType(schema)
