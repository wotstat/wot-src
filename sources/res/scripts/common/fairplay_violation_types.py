...
_CONFIG_FILE = b'scripts/item_defs/fairplay_violation_types.xml'
...
def _readViolations(section):
    if section is None:
        raise SoftException(b'Cannot find root section')
    violations = []
    for violation, item in section.items():
        if violation == _XML_NAMESPACE:
            continue
        vType = item.readString(b'violationType')
        if vType and not hasattr(FairplayViolationType, vType.upper()):
            raise SoftException((b'Wrong Attibute {} in class {}').format(vType, FairplayViolations))
        if not vType:
            raise SoftException((b'Empty Attibute violationType in class {}').format(vType, FairplayViolations))
...
def initViolationTypes(violationTypes):
    for idx, (violationType, vType) in enumerate(violationTypes):
        if hasattr(FairplayViolations, violationType):
            raise SoftException((b'Attibute {} already in class {}').format(violationType, FairplayViolations))
