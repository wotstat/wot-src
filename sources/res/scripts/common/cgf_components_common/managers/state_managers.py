...
class StateSwitcherManager(CGF.ComponentManager):
    ...
    def __validateSettings(self, switcher):
        if not switcher.normal.isValid():
            _logger.warning(b'Incorrect switcher setup, missing Normal state prefab')
        if not switcher.damaged.isValid():
            _logger.warning(b'Incorrect switcher setup, missing Damaged state prefab')
        if not switcher.critical.isValid():
            _logger.warning(b'Incorrect switcher setup, missing Critical state prefab')
        return
