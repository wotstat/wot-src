...
t2sort__skip = {dict: _iterDict__skip, set: _iterSet__skip, 
...
def adler32_combine(*sums):
    a, b = (0, 1)
    for n in sums:
        aa, bb = unpack(b'>HH', pack(b'>l', n))
        a = (a + aa) % 65521
        b = (b + bb + 65520) % 65521

    return unpack(b'>l', pack(b'>HH', a, b))[0]
...
def __is_iterable(data):
    return bool(getattr(data, b'__iter__', None))
