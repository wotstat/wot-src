...
UNIT_API_NAMES = {v: k for k, v in viewitems(UNIT_API.__dict__) if not k.startswith(b'_')}
...
class UnitClientAPI(object):
    ...
    def createFunRandomSquad(self, subModeID):
        unitExtrasInitStr = json.dumps({b'funEventID': subModeID})
        return self._doCreate(PREBATTLE_TYPE.FUN_RANDOM, unitExtrasInitStr=unitExtrasInitStr)

    def createSquadByPrbType(self, prbType, queueType=0, unitExtrasInitStr=b'', modeExtrasStr=b''):
    ...
    def doUnitCmd(self, clientUnitCmdID, argInt64=0, argInt32=0, argStr=b''):
