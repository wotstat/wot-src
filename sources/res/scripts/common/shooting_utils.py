...
__DIST_LIMIT_TAGS = [
 b'lightTank', b'mediumTank', b'heavyTank', b'AT-SPG']
__STEEP_AIMING_TAGS = []
__MIXED_AIMING_TAGS = []
...
def getPyShootingTest(spaceID, vehicleDescr):
    ...
    if staticPitch is not None:
        if not isPitchHullAimingEnabled:
            ...
            pitchLimitsDesc = {b'minPitch': zeroLimits, 
               b'maxPitch': zeroLimits, 
               b'absolute': (
                           staticPitch, staticPitch)}
    if pitchLimitsDesc is None:
        pitchLimitsDesc = gunDescr.pitchLimits
    shootingTest.minGunPitchLimits = pitchLimitsDesc[b'minPitch']
    shootingTest.maxGunPitchLimits = pitchLimitsDesc[b'maxPitch']
