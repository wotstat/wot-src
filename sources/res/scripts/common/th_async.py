...
def await_deferred(d):
    ...
    futureCallback = getattr(d, b'addCallback', None)
    if futureCallback is not None:
        futureCallback(callback)
        futureErrback = getattr(d, b'addErrback', None)
...
class _Promise(object):
    __slots__ = (b'__value_set', b'__future_set', b'__exc_info', b'__value', b'__future', b'__cancelled', b'__cancel', b'__weakref__')
    ...
    def __str__(self):
        return (b'{}(value_set={}, value={}, exc_info={}, future_set={})').format(self.__class__.__name__, self.__value_set, self.__value, self.__exc_info, self.__future_set)
...
class _AsyncExecutorNonRec(object):
    ...
    def __resume_cont(self):
        storage = []
        return (
         (lambda first, *rest: storage.extend((first, rest)) if not storage else self.__step(first, *rest)),
         storage)
