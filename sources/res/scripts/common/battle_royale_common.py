...
class BattleRoyaleVehicleStats(object):
    ...
    def getLossesCount(self):
        return sum(count for place, count in viewitems(self.__rawData[b'brplaces']) if place != 1)
    ...
    def getPositionSum(self):
        return sum(k * v for k, v in viewitems(self.__rawData[b'brplaces']))

    def getAchivedLevelSum(self):
        return sum(k * v for k, v in viewitems(self.__rawData[b'brlevels']))
