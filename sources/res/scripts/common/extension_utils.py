...
class ResMgr(object):
    ...
    class __metaclass__(type):
        ...
        def __getattr__(self, item):
            ...
            return getattr(self if item in (b'openSection', b'addToCache') else rmgr, item)
    ...
    def addToCache(ftPath, xml):
        ...
        if len(xmlPaths) > 1 and not mergeRequired:
            raise SoftException(b'Multiple standalone resources for one relative path found: %s', ftPath)
