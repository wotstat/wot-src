...
class ValueReplayConnector(object):
    ...
    def indexToNames(self, idx):
        ...
        if subIndex > 0:
            ...
            return (
             subname1 + b'_' + subname2, secondName)
        return (
         subname1, secondName)
...
class ValueReplay:
    ...
    def pack(self):
        ...
        self.__packed = struct.pack((b'<H%sI' % size), size, *self.__replay)
        return self.__packed

    def unpack(self):
        size = struct.unpack(b'<H', self.__packed[:2])[0]
        return struct.unpack(b'<%sI' % size, self.__packed[2:])

    def __setitem__(self, key, value):
        if self.__connector.index(key) not in self.__appliedValues or key == self.__recordName or key in self.__tags:
            raise SoftException(b'Cannot overload item %s:%s' % (key, value))
    ...
    def __delitem__(self, key):
        if self.__connector.index(key) not in self.__appliedValues or key == self.__recordName or key in self.__tags:
            raise SoftException(b'Unexpected arg %s' % (key,))
    ...
    def tag(self, other):
        idx = self.__validate(other)
        if self.__tags:
            raise SoftException(b'Just one tag is allowed %s, %s' % (other, self.__tags))
        self.__appliedValues.add(idx)
    ...
    def applyFactorToTag(self, other):
        idx = self.__validate(other)
        if not self.__tags:
            raise SoftException(b'There is no any tagged values %s' % (other,))
        self.__appliedValues.add(idx)
    ...
    def __iter__(self):
        if not self.__replay:
            raise SoftException(b'Invalid usage of __iter__')
        finalResult = 0
        tags = self.__tags
    ...
    def __contains__(self, item):
        if not self.__replay:
            raise SoftException(b'Invalid usage of __contains__')
        connector = self.__connector
        for replay in self.__replay:
    ...
    def __checkParam(self, param):
        if param == self.__recordName:
            raise SoftException(b'Invalid usage %s. Cannot apply wrapped value to itself.' % (param,))
        return
    ...
    def __validate(self, param1, param2=None, initial=False):
        ...
        if not initial and not appliedValues:
            raise SoftException(b'Invalid usage %s. Call __setInitial before.' % (param1,))
        idx = self.__connector.index(param1, param2)
        if idx in appliedValues:
            raise SoftException(b'Unexpected arg (%s,%s). Argument has been already applied.' % (param1, param2))
        return idx
    ...
    def __setInitial(self, other):
        ...
        if appliedValues or replay:
            raise SoftException(b'Invalid usage %s' % (other,))
    ...
    def __getFactorValue(self, other):
        other = other.lower()
        if b'factor100' in other or b'factors100' in other:
            return 100.0
