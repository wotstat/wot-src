...
class CFunctions(unittest.TestCase):
    ...
    def S(self):
        return c_longlong.in_dll(self._dll, b'last_tf_arg_s').value

    def U(self):
        return c_ulonglong.in_dll(self._dll, b'last_tf_arg_u').value
    ...
    def test_ulong_plus(self):
        ...
        self.assertEqual(self._dll.tf_bL(b' ', 4294967295L), 1431655765)
        self.assertEqual(self.U(), 4294967295L)
        return
...
try:
    ...
    class stdcall_dll(WinDLL):
        ...
        def __getattr__(self, name):
            if name[:2] == b'__' and name[-2:] == b'__':
                raise AttributeError(name)
            func = self._FuncPtr((b's_' + name, self))
            setattr(self, name, func)
            return func


@need_symbol(b'WinDLL')
