...
class SlicesTestCase(unittest.TestCase):
    ...
    def test_setslice_cint(self):
        ...
        self.assertRaises(TypeError, setslice, a, 0, 5, b'abcde')
        self.assertRaises(TypeError, setitem, a, slice(0, 5), b'abcde')
        self.assertRaises(TypeError, setslice, a, 0, 5, [27, 28, 29, 30, 31])
        self.assertRaises(TypeError, setitem, a, slice(0, 5), [
         27, 28, 29, 30, 31])
        self.assertRaises(TypeError, setslice, a, 0, 5, [13, 32, 18, 33, 34])
        self.assertRaises(TypeError, setitem, a, slice(0, 5), [
         13, 32, 18, 33, 34])
    ...
    def test_char_ptr(self):
        ...
        res = dll.my_strdup(s)
        self.assertEqual(res[:len(s)], range(ord(b'a'), ord(b'z') + 1))
        self.assertEqual(res[:len(s):], range(ord(b'a'), ord(b'z') + 1))
        dll.my_free(res)
        return
    ...
    def test_wchar_ptr(self):
        ...
        if sizeof(c_wchar) == sizeof(c_short):
            ...
            self.skipTest(b'Pointers to c_wchar are not supported')
        res = dll.my_wcsdup(s)
        tmpl = range(ord(b'a'), ord(b'z') + 1)
