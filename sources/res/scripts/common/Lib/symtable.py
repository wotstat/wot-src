...
class SymbolTable(object):
    ...
    def __repr__(self):
        if self.__class__ == SymbolTable:
            ...
            kind = b'%s ' % self.__class__.__name__
        if self._table.name == b'global':
            return (b'<{0}SymbolTable for module {1}>').format(kind, self._filename)
        else:
            return (b'<{0}SymbolTable for {1} in {2}>').format(kind, self._table.name, self._filename)
...
class Symbol(object):
    ...
    def __repr__(self):
        return (b'<symbol {0!r}>').format(self.__name)
    ...
    def get_namespace(self):
        if len(self.__namespaces) != 1:
            raise ValueError, b'name is bound to multiple namespaces'
...
if __name__ == b'__main__':
    ...
    mod = symtable(src, os.path.split(sys.argv[0])[1], b'exec')
