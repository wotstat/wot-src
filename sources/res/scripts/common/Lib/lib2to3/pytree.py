...
class Base(object):
    ...
    def set_prefix(self, prefix):
        warnings.warn(b'set_prefix() is deprecated; use the prefix property', DeprecationWarning, stacklevel=2)
        self.prefix = prefix
        return

    def get_prefix(self):
        warnings.warn(b'get_prefix() is deprecated; use the prefix property', DeprecationWarning, stacklevel=2)
        return self.prefix
    ...
    if sys.version_info < (3, 0):
        ...
        def __str__(self):
            return unicode(self).encode(b'ascii')
...
class Node(Base):
    ...
    def __repr__(self):
        return b'%s(%s, %r)' % (self.__class__.__name__,
         type_repr(self.type),
         self.children)
...
class Leaf(Base):
    ...
    def __repr__(self):
        return b'%s(%r, %r)' % (self.__class__.__name__,
         self.type,
         self.value)
...
class BasePattern(object):
    ...
    def __repr__(self):
        ...
        return b'%s(%s)' % (self.__class__.__name__, (b', ').join(map(repr, args)))
...
class WildcardPattern(BasePattern):
    ...
    def generate_matches(self, nodes):
        ...
        elif self.name == b'bare_name':
            yield self._bare_name_matches(nodes)
        elif hasattr(sys, b'getrefcount'):
