...
class _ModifiedArgv0(object):
    ...
    def __enter__(self):
        if self._saved_value is not self._sentinel:
            raise RuntimeError(b'Already preserving saved value')
...
def _get_filename(loader, mod_name):
    for attr in (b'get_filename', b'_get_filename'):
...
def _get_module_details(mod_name, error=ImportError):
    try:
        ...
        if loader is None:
            raise error(b'No module named %s' % mod_name)
        ispkg = loader.is_package(mod_name)
    except ImportError as e:
        raise error(format(e))

    if ispkg:
        if mod_name == b'__main__' or mod_name.endswith(b'.__main__'):
            raise error(b'Cannot use package as __main__ module')
        __import__(mod_name)
        try:
            pkg_main_name = mod_name + b'.__main__'
            return _get_module_details(pkg_main_name)
        except ImportError as e:
            raise error((b'%s; %r is a package and cannot ' + b'be directly executed') % (e, mod_name))
    ...
    if code is None:
        raise error(b'No code object available for %s' % mod_name)
    filename = _get_filename(loader, mod_name)
    return (mod_name, loader, code, filename)


def _get_main_module_details(error=ImportError):
    main_name = b'__main__'
    try:
        return _get_module_details(main_name)
    except ImportError as exc:
        if main_name in str(exc):
            raise error(b"can't find %r module in %r" % (
...
def _run_module_as_main(mod_name, alter_argv=True):
    try:
        if alter_argv or mod_name != b'__main__':
        ...
        msg = b'%s: %s' % (sys.executable, exc)
        sys.exit(msg)

    pkg_name = mod_name.rpartition(b'.')[0]
    main_globals = sys.modules[b'__main__'].__dict__
    if alter_argv:
        sys.argv[0] = fname
    return _run_code(code, main_globals, None, b'__main__', fname, loader, pkg_name)
...
def _get_code_from_file(fname):
    with open(fname, b'rb') as f:
        code = read_code(f)
    if code is None:
        with open(fname, b'rU') as f:
            code = compile(f.read(), fname, b'exec')
    return code
...
if __name__ == b'__main__':
    if len(sys.argv) < 2:
        print >> sys.stderr, b'No module specified for execution'
    else:
        del sys.argv[0]
