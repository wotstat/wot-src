...
if not (sys.platform == b'win32' or hasattr(_multiprocessing, b'recvfd')):
    raise ImportError(b'pickling of connections not supported')
...
def _get_listener():
    ...
    if _listener is None:
        ...
        try:
            if _listener is None:
                debug(b'starting listener and thread for sending handles')
...
def _serve():
    ...
    while 1:
        try:
            ...
            if not is_exiting():
                import traceback
                sub_warning(b'thread for sharing handles raised exception :\n' + b'-' * 79 + b'\n' + traceback.format_exc() + b'-' * 79)
...
def reduce_handle(handle):
    if Popen.thread_is_spawning():
        ...
        sub_debug(b'reducing handle %d', handle)
...
def rebuild_handle(pickled_data):
    ...
    if inherited:
        return handle
    sub_debug(b'rebuilding handle %d', handle)
