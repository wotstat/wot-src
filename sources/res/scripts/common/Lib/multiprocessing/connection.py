...
default_family = b'AF_INET'
families = [b'AF_INET']
if hasattr(socket, b'AF_UNIX'):
    default_family = b'AF_UNIX'
    families += [b'AF_UNIX']
if sys.platform == b'win32':
    default_family = b'AF_PIPE'
    families += [b'AF_PIPE']
...
def arbitrary_address(family):
    ...
    if family == b'AF_UNIX':
        return tempfile.mktemp(prefix=b'listener-', dir=get_temp_dir())
    if family == b'AF_PIPE':
        return tempfile.mktemp(prefix=b'\\\\.\\pipe\\pyc-%d-%d-' % (
         os.getpid(), _mmap_counter.next()), dir=b'')
    raise ValueError(b'unrecognized family')
    return
...
def address_type(address):
    if type(address) == tuple:
        return b'AF_INET'
    if type(address) is str and address.startswith(b'\\\\'):
        return b'AF_PIPE'
    if type(address) is str:
        return b'AF_UNIX'
    raise ValueError(b'address type of %r unrecognized' % address)
    return
...
class Listener(object):
    ...
    def __init__(self, address=None, family=None, backlog=1, authkey=None):
        ...
        if authkey is not None and not isinstance(authkey, bytes):
            raise TypeError, b'authkey should be a byte string'
...
if sys.platform != b'win32':
    ...
    def Pipe(duplex=True):
        address = arbitrary_address(b'AF_PIPE')
        if duplex:
    ...
    while 1:
        ...
        try:
            ...
            if e.args[0] != errno.ECONNREFUSED or _check_timeout(t):
                debug(b'failed to connect to address %s', address)
                raise
            time.sleep(0.01)
        else:
            break
    ...
    class PipeListener(object):
        ...
        def __init__(self, address, backlog=None):
            ...
            self._handle_queue = [
             handle]
            self._last_accepted = None
            sub_debug(b'listener created with address=%r', self._address)
        ...
        def _finalize_pipe_listener(queue, address):
            sub_debug(b'closing listener with address=%r', address)
            for handle in queue:
                close(handle)
...
def deliver_challenge(connection, authkey):
    ...
    if response == digest:
        ...
        raise AuthenticationError(b'digest received was wrong')
    return
...
def answer_challenge(connection, authkey):
    ...
    response = connection.recv_bytes(256)
    if response != WELCOME:
        raise AuthenticationError(b'digest sent was rejected')
    return
...
class ConnectionWrapper(object):
    ...
    def __init__(self, conn, dumps, loads):
        ...
        self._dumps = dumps
        self._loads = loads
        for attr in (b'fileno', b'close', b'poll', b'recv_bytes', b'send_bytes'):
