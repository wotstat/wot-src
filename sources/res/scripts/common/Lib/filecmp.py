...
def _do_cmp(f1, f2):
    bufsize = BUFSIZE
    with open(f1, b'rb') as fp1:
        with open(f2, b'rb') as fp2:
            while True:
                b1 = fp1.read(bufsize)
                b2 = fp2.read(bufsize)
                if b1 != b2:
...
class dircmp:
    ...
    def report(self):
        print b'diff', self.left, self.right
        if self.left_only:
            self.left_only.sort()
            print b'Only in', self.left, b':', self.left_only
        if self.right_only:
            self.right_only.sort()
            print b'Only in', self.right, b':', self.right_only
        if self.same_files:
            self.same_files.sort()
            print b'Identical files :', self.same_files
        if self.diff_files:
            self.diff_files.sort()
            print b'Differing files :', self.diff_files
        if self.funny_files:
            self.funny_files.sort()
            print b'Trouble with common files :', self.funny_files
        if self.common_dirs:
            self.common_dirs.sort()
            print b'Common subdirectories :', self.common_dirs
        if self.common_funny:
            self.common_funny.sort()
            print b'Common funny cases :', self.common_funny
        return
...
def demo():
    import sys, getopt
    options, args = getopt.getopt(sys.argv[1:], b'r')
    if len(args) != 2:
        raise getopt.GetoptError(b'need exactly two args', None)
