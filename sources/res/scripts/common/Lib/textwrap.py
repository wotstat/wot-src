...
__all__ = [
 b'TextWrapper', b'wrap', b'fill', b'dedent']
_whitespace = b'\t\n\x0b\x0c\r '

class TextWrapper():
    whitespace_trans = string.maketrans(_whitespace, b' ' * len(_whitespace))
    unicode_whitespace_trans = {}
    uspace = ord(u' ')
    for x in map(ord, _whitespace):
        unicode_whitespace_trans[x] = uspace

    wordsep_re = re.compile(b'(\\s+|[^\\s\\w]*\\w+[^0-9\\W]-(?=\\w+[^0-9\\W])|(?<=[\\w\\!\\"\\\'\\&\\.\\,\\?])-{2,}(?=\\w))')
    wordsep_simple_re = re.compile(b'(\\s+)')
    sentence_end_re = re.compile(b'[%s][\\.\\!\\?][\\"\\\']?\\Z' % string.lowercase)

    def __init__(self, width=70, initial_indent=b'', subsequent_indent=b'', expand_tabs=True, replace_whitespace=True, fix_sentence_endings=False, break_long_words=True, drop_whitespace=True, break_on_hyphens=True):
        self.width = width
