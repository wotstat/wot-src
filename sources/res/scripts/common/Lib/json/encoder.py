...
ESCAPE = re.compile(b'[\\x00-\\x1f\\\\"\\b\\f\\n\\r\\t]')
ESCAPE_ASCII = re.compile(b'([\\\\"]|[^\\ -~])')
...
ESCAPE_DCT = {b'\\': b'\\\\', 
   ...
   b'\t': b'\\t'}
for i in range(32):
    ESCAPE_DCT.setdefault(chr(i), (b'\\u{0:04x}').format(i))

INFINITY = float(b'inf')
...
def encode_basestring(s):
    ...
    return b'"' + ESCAPE.sub(replace, s) + b'"'
...
def py_encode_basestring_ascii(s):
    ...
    def replace(match):
        ...
        try:
            ...
            n = ord(s)
            if n < 65536:
                return (b'\\u{0:04x}').format(n)
            n -= 65536
            s1 = 55296 | n >> 10 & 1023
            s2 = 56320 | n & 1023
            return (b'\\u{0:04x}\\u{1:04x}').format(s1, s2)

        return

    return b'"' + str(ESCAPE_ASCII.sub(replace, s)) + b'"'
...
class JSONEncoder(object):
    ...
    def __init__(self, skipkeys=False, ensure_ascii=True, check_circular=True, allow_nan=True, sort_keys=False, indent=None, separators=None, encoding=b'utf-8', default=None):
    ...
    def encode(self, o):
        if isinstance(o, basestring):
            if isinstance(o, str):
                ...
                if _encoding is not None and not _encoding == b'utf-8':
    ...
    def iterencode(self, o, _one_shot=False):
        ...
        def floatstr(o, allow_nan=self.allow_nan, _repr=FLOAT_REPR, _inf=INFINITY, _neginf=-INFINITY):
            if o != o:
                ...
                return _repr(o)
            if not allow_nan:
                raise ValueError(b'Out of range float values are not JSON compliant: ' + repr(o))
            return text
        ...
        if not lst:
            ...
            if markers is not None:
                ...
                if markerid in markers:
                    raise ValueError(b'Circular reference detected')
                markers[markerid] = lst
            buf = b'['
            if _indent is not None:
                _current_indent_level += 1
                newline_indent = b'\n' + b' ' * (_indent * _current_indent_level)
        ...
        if not dct:
            ...
            for key, value in items:
                if isinstance(key, basestring):
                    ...
                    raise TypeError(b'key ' + repr(key) + b' is not a string')
                if first:
                    first = False
                else:
