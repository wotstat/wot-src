...
class TabSet(Frame):
    ...
    def __init__(self, page_set, select_command, tabs=None, n_rows=1, max_tabs_per_row=5, expand_tabs=False, **kw):
        ...
        self._tab_rows = []
        self.padding_frame = Frame(self, height=2, borderwidth=0, relief=FLAT, background=self.cget(b'background'))
    ...
    def add_tab(self, tab_name):
        if not tab_name:
            raise InvalidNameError(b"Invalid Tab name: '%s'" % tab_name)
        if tab_name in self._tab_names:
            raise AlreadyExistsError(b"Tab named '%s' already exists" % tab_name)
    ...
    def remove_tab(self, tab_name):
        if tab_name not in self._tab_names:
            raise KeyError(b"No such Tab: '%s" % page_name)
    ...
    class TabButton(Frame):
        ...
        def __init__(self, name, select_command, tab_row, tab_set):
            ...
            self.button = Radiobutton(self, text=name, command=self._select_event, padx=5, pady=1, takefocus=FALSE, indicatoron=FALSE, highlightthickness=0, selectcolor=b'', borderwidth=0)
        ...
        def _init_masks(self):
            ...
            background = page_set.pages_frame.cget(b'background')
...
class TabbedPageSet(Frame):
    ...
    def add_page(self, page_name):
        if not page_name:
            raise InvalidNameError(b"Invalid TabPage name: '%s'" % page_name)
        if page_name in self.pages:
            raise AlreadyExistsError(b"TabPage named '%s' already exists" % page_name)
    ...
    def remove_page(self, page_name):
        if page_name not in self.pages:
            raise KeyError(b"No such TabPage: '%s" % page_name)
...
def _tabbed_pages(parent):
    ...
    root.geometry(b'+%d+%d' % (x, y + 175))
    root.title(b'Test tabbed pages')
    tabPage = TabbedPageSet(root, page_names=[b'Foobar', b'Baz'], n_rows=0, expand_tabs=False)
    tabPage.pack(side=TOP, expand=TRUE, fill=BOTH)
    Label(tabPage.pages[b'Foobar'].frame, text=b'Foo', pady=20).pack()
    Label(tabPage.pages[b'Foobar'].frame, text=b'Bar', pady=20).pack()
    Label(tabPage.pages[b'Baz'].frame, text=b'Baz').pack()
    entryPgName = Entry(root)
    buttonAdd = Button(root, text=b'Add Page', command=(lambda : tabPage.add_page(entryPgName.get())))
    buttonRemove = Button(root, text=b'Remove Page', command=(lambda : tabPage.remove_page(entryPgName.get())))
    labelPgName = Label(root, text=b'name of page to add/remove:')
