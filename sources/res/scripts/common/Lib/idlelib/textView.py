...
class TextViewer(Toplevel):
    ...
    def __init__(self, parent, title, text, modal=True, _htest=False):
        ...
        self.geometry(b'=%dx%d+%d+%d' % (750, 500,
    ...
    def CreateWidgets(self):
        ...
        self.buttonOk = Button(frameButtons, text=b'Close', command=self.Ok, takefocus=FALSE)
...
def view_file(parent, title, filename, encoding=None, modal=True):
    try:
        if encoding:
            import codecs
            textFile = codecs.open(filename, b'r')
        else:
            textFile = open(filename, b'r')
    except IOError:
        tkMessageBox.showerror(title=b'File Load Error', message=b'Unable to load file %r .' % filename, parent=parent)
    except UnicodeDecodeError as err:
        showerror(title=b'Unicode Decode Error', message=str(err), parent=parent)
    else:
...
if __name__ == b'__main__':
    import unittest
    unittest.main(b'idlelib.idle_test.test_textview', verbosity=2, exit=False)
