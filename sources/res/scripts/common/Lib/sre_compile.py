    ...
    for op, av in pattern:
        if op in LITERAL_CODES:
            ...
            if flags & SRE_FLAG_TEMPLATE:
                raise error, b'internal: unsupported template operator'
                emit(OPCODES[REPEAT])
                skip = _len(code)
                emit(0)
                emit(av[0])
            ...
            if av[0] >= 0:
                ...
                lo, hi = av[1].getwidth()
                if lo != hi:
                    raise error, b'look-behind requires fixed-width pattern'
                emit(lo)
            _compile(code, av[1], flags)
            emit(OPCODES[SUCCESS])
            code[skip] = _len(code) - skip
            ...
            raise ValueError, (b'unsupported operand type', op)
...
def _compile_charset(charset, flags, code, fixup=None, fixes=None):
    ...
    for op, av in _optimize_charset(charset, fixup, fixes, flags & SRE_FLAG_UNICODE):
        ...
        if op is NEGATE:
            ...
            raise error, b'internal: unsupported set operator'
...
def _optimize_charset(charset, fixup, fixes, isunicode):
    ...
    q = 0
    while True:
        p = charmap.find(b'\x01', q)
        if p < 0:
            break
        if len(runs) >= 2:
            runs = None
            break
        q = charmap.find(b'\x00', p)
        if q < 0:
...
def compile(p, flags=0):
    ...
    if p.pattern.groups > 100:
        raise AssertionError(b'sorry, but this version only supports 100 named groups')
