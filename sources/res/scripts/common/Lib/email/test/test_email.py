import os, sys, time, base64, difflib, unittest, warnings, textwrap
...
class TestMessageAPI(TestEmailBase):
    ...
    def test_make_boundary(self):
        msg = MIMEMultipart(b'form-data')
        self.assertEqual(msg.items()[0][1], b'multipart/form-data')
        msg.as_string()
        self.assertEqual(msg.items()[0][1][:33], b'multipart/form-data; boundary="==')
        return

    def test_message_rfc822_only(self):
        fp = openfile(findfile(b'msg_46.txt'))
        msgdata = fp.read()
    ...
    def test_decode_bogus_uu_payload_quietly(self):
        msg = Message()
        msg.set_payload(b'begin 664 foo.txt\n%<W1F=0000H \n \nend\n')
        msg[b'Content-Transfer-Encoding'] = b'x-uuencode'
        ...
        self.assertEqual(sfp.getvalue(), b'')
        return
    ...
    def test_get_param_with_quotes(self):
        msg = email.message_from_string(b'Content-Type: foo; bar*0="baz\\"foobar"; bar*1="\\"baz"')
        self.assertEqual(msg.get_param(b'bar'), b'baz"foobar"baz')
        msg = email.message_from_string(b'Content-Type: foo; bar*0="baz\\"foobar"; bar*1="\\"baz"')
        self.assertEqual(msg.get_param(b'bar'), b'baz"foobar"baz')
        return
    ...
    def test_get_content_charset(self):
        msg = Message()
        msg.set_charset(b'us-ascii')
        self.assertEqual(b'us-ascii', msg.get_content_charset())
...
class TestEncoders(unittest.TestCase):
    ...
    def test_encode7or8bit(self):
        eq = self.assertEqual
        msg = email.MIMEText.MIMEText(b'\xca\xb8', _charset=b'euc-jp')
        eq(msg[b'content-transfer-encoding'], b'7bit')
        return
...
class TestLongHeaders(TestEmailBase):
    ...
    def test_long_received_header(self):
        ...
        msg[b'Received-2'] = h
        self.assertEqual(msg.as_string(), b'Received-1: from FOO.TLD (vizworld.acl.foo.tld [123.452.678.9]) by\n\throthgar.la.mastaler.com (tmda-ofmipd) with ESMTP;\n\tWed, 05 Mar 2003 18:10:18 -0700\nReceived-2: from FOO.TLD (vizworld.acl.foo.tld [123.452.678.9]) by\n hrothgar.la.mastaler.com (tmda-ofmipd) with ESMTP;\n Wed, 05 Mar 2003 18:10:18 -0700\n\n')
        return
    ...
    def test_string_headerinst_eq(self):
        ...
        msg = Message()
        msg[b'Received'] = Header(h, header_name=b'Received', continuation_ws=b'\t')
        msg[b'Received'] = h
    ...
    def test_long_lines_with_different_header(self):
        ...
        eq(msg.as_string(), b'List: List-Unsubscribe: <https://lists.sourceforge.net/lists/listinfo/spamassassin-talk>,\n <mailto:spamassassin-talk-request@lists.sourceforge.net?subject=unsubscribe>\nList: List-Unsubscribe: <https://lists.sourceforge.net/lists/listinfo/spamassassin-talk>,\n <mailto:spamassassin-talk-request@lists.sourceforge.net?subject=unsubscribe>\n\n')
        return
...
class TestFromMangling(unittest.TestCase):
    ...
    def test_mangle_from_in_preamble_and_epilog(self):
        s = StringIO()
        g = Generator(s, mangle_from_=True)
        msg = email.message_from_string(textwrap.dedent(b'            From: foo@bar.com\n            Mime-Version: 1.0\n            Content-Type: multipart/mixed; boundary=XXX\n\n            From somewhere unknown\n\n            --XXX\n            Content-Type: text/plain\n\n            foo\n\n            --XXX--\n\n            From somewhere unknowable\n            '))
        g.flatten(msg)
        self.assertEqual(len([1 for x in s.getvalue().split(b'\n') if x.startswith(b'>From ')]), 2)
        return
...
class TestMIMEText(unittest.TestCase):
    ...
    def test_7bit_unicode_input(self):
        eq = self.assertEqual
        msg = MIMEText(u'hello there', _charset=b'us-ascii')
        eq(msg.get_charset().input_charset, b'us-ascii')
        eq(msg[b'content-type'], b'text/plain; charset="us-ascii"')
        return
    ...
    def test_7bit_unicode_input_no_charset(self):
        ...
        eq(msg.get_charset(), b'us-ascii')
        eq(msg[b'content-type'], b'text/plain; charset="us-ascii"')
        self.assertIn(b'hello there', msg.as_string())
        return

    def test_8bit_unicode_input(self):
        teststr = u'\u043a\u0438\u0440\u0438\u043b\u0438\u0446\u0430'
        eq = self.assertEqual
        msg = MIMEText(teststr, _charset=b'utf-8')
        eq(msg.get_charset().output_charset, b'utf-8')
        eq(msg[b'content-type'], b'text/plain; charset="utf-8"')
        eq(msg.get_payload(decode=True), teststr.encode(b'utf-8'))
        return
...
class TestRFC2047(unittest.TestCase):
    ...
    def test_rfc2047_B_bad_padding(self):
        ...
        for q, a in data:
            ...
            self.assertEqual(dh, [(a, b'iso-8859-1')])

        return

    def test_rfc2047_Q_invalid_digits(self):
        s = b'=?iso-8859-1?Q?andr=e9=zz?='
        self.assertEqual(decode_header(s), [
         (b'andr\xe9=zz', b'iso-8859-1')])
        return
...
class TestMiscellaneous(TestEmailBase):
    ...
    def test_parsedate_y2k(self):
        self.assertEqual(Utils.parsedate_tz(b'25 Feb 03 13:47:26 -0800'), Utils.parsedate_tz(b'25 Feb 2003 13:47:26 -0800'))
        self.assertEqual(Utils.parsedate_tz(b'25 Feb 71 13:47:26 -0800'), Utils.parsedate_tz(b'25 Feb 1971 13:47:26 -0800'))
        return
    ...
    def test_parseaddr_multiple_domains(self):
        self.assertEqual(Utils.parseaddr(b'a@b@c'), (b'', b''))
        self.assertEqual(Utils.parseaddr(b'a@b.c@c'), (b'', b''))
        self.assertEqual(Utils.parseaddr(b'a@172.17.0.1@c'), (b'', b''))
        return
    ...
    def test_parseaddr_preserves_quoted_pairs_in_addresses(self):
        eq = self.assertEqual
        eq(Utils.parseaddr(b'""example" example"@example.com'), (b'', b'""example" example"@example.com'))
        eq(Utils.parseaddr(b'"\\"example\\" example"@example.com'), (b'', b'"\\"example\\" example"@example.com'))
        eq(Utils.parseaddr(b'"\\\\"example\\\\" example"@example.com'), (b'', b'"\\\\"example\\\\" example"@example.com'))
        return
...
class TestIterators(TestEmailBase):
    ...
    def test_pushCR_LF(self):
        ...
        self.assertEqual((b'').join([il for il, n in imt]), (b'').join(om))
        return
    ...
    def test_push_random(self):
        ...
        chunksize = 5
        chars = b'abcd \t\r\n'
        s = (b'').join(choice(chars) for i in range(n)) + b'\n'
...
class TestFeedParsers(TestEmailBase):
    ...
    def test_newlines(self):
        m = self.parse([b'a:\nb:\rc:\r\nd:\n'])
        self.assertEqual(m.keys(), [b'a', b'b', b'c', b'd'])
        m = self.parse([b'a:\nb:\rc:\r\nd:'])
        self.assertEqual(m.keys(), [b'a', b'b', b'c', b'd'])
        m = self.parse([b'a:\rb', b'c:\n'])
        self.assertEqual(m.keys(), [b'a', b'bc'])
        m = self.parse([b'a:\r', b'b:\n'])
        self.assertEqual(m.keys(), [b'a', b'b'])
        m = self.parse([b'a:\r', b'\nb:\n'])
        self.assertEqual(m.keys(), [b'a', b'b'])
        return
    ...
    def test_long_lines(self):
        M, N = (1000, 20000)
        m = self.parse([b'a:b\n\n'] + [b'x' * M] * N)
        self.assertEqual(m.items(), [(b'a', b'b')])
        self.assertEqual(m.get_payload(), b'x' * M * N)
        m = self.parse([b'a:b\r\r'] + [b'x' * M] * N)
        self.assertEqual(m.items(), [(b'a', b'b')])
        self.assertEqual(m.get_payload(), b'x' * M * N)
        m = self.parse([b'a:\r', b'b: '] + [b'x' * M] * N)
        self.assertEqual(m.items(), [(b'a', b''), (b'b', b'x' * M * N)])
        return
...
class TestParsers(TestEmailBase):
    ...
    def test_CRLFLF_at_end_of_part(self):
        m = b'From: foo@bar.com\nTo: baz\nMime-Version: 1.0\nContent-Type: multipart/mixed; boundary=BOUNDARY\n\n--BOUNDARY\nContent-Type: text/plain\n\nbody ending with CRLF newline\r\n\n--BOUNDARY--\n'
        msg = email.message_from_string(m)
        self.assertTrue(msg.get_payload(0).get_payload().endswith(b'\r\n'))
        return
...
class TestCharset(unittest.TestCase):
    ...
    def test_codecs_aliases_accepted(self):
        charset = Charset(b'utf8')
        self.assertEqual(str(charset), b'utf-8')
        return
...
class TestHeader(TestEmailBase):
    ...
    def test_ascii_add_header(self):
        msg = Message()
        msg.add_header(b'Content-Disposition', b'attachment', filename=b'bud.gif')
        self.assertEqual(b'attachment; filename="bud.gif"', msg[b'Content-Disposition'])
        return

    def test_nonascii_add_header_via_triple(self):
        msg = Message()
        msg.add_header(b'Content-Disposition', b'attachment', filename=(b'iso-8859-1', b'', b'Fu\xdfballer.ppt'))
        self.assertEqual(b'attachment; filename*="iso-8859-1\'\'Fu%DFballer.ppt"', msg[b'Content-Disposition'])
        return

    def test_encode_unaliased_charset(self):
        res = Header(b'abc', b'iso-8859-2').encode()
        self.assertEqual(res, b'=?iso-8859-2?q?abc?=')
...
class TestSigned(TestEmailBase):
    ...
    def test_long_headers_as_string(self):
        original, msg = self._msg_and_obj(b'msg_45.txt')
