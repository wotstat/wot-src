...
class Header():
    ...
    def __init__(self, s=None, charset=None, maxlinelen=None, header_name=None, continuation_ws=b' ', errors=b'strict'):
        ...
        cws_expanded_len = len(continuation_ws.replace(b'\t', SPACE8))
        self._chunks = []
        if s is not None:
    ...
    def __unicode__(self):
        ...
        for s, charset in self._chunks:
            nextcs = charset
            if uchunks:
                if lastcs not in (None, b'us-ascii'):
                    if nextcs in (None, b'us-ascii'):
                        uchunks.append(USPACE)
                        nextcs = None
                elif nextcs not in (None, b'us-ascii'):
                    uchunks.append(USPACE)
            lastcs = nextcs
    ...
    def append(self, s, charset=None, errors=b'strict'):
        ...
        if charset != b'8bit':
            if isinstance(s, str):
                incodec = charset.input_codec or b'us-ascii'
                ustr = unicode(s, incodec, errors)
                outcodec = charset.output_codec or b'us-ascii'
    ...
    def encode(self, splitchars=b';, '):
        newchunks = []
        maxlinelen = self._firstlinelen
        lastlen = 0
        for s, charset in self._chunks:
            targetlen = maxlinelen - lastlen - 1
            if targetlen < charset.encoded_header_len(b''):
...
def _split_ascii(s, firstlen, restlen, continuation_ws, splitchars):
    ...
    for line in s.splitlines():
        ...
        joiner = eol + b' '
        joinlen = len(joiner)
        wslen = len(continuation_ws.replace(b'\t', SPACE8))
        this = []
        linelen = 0
        for part in cre.split(line):
            ...
            if ch == b' ' and onfirstline and len(this) == 1 and fcre.match(this[0]):
                this.append(part)
                linelen += partlen
            elif curlen + partlen > maxlen:
                if this:
                    lines.append(joiner.join(this) + eol)
                if partlen > maxlen and ch != b' ':
                    subl = _split_ascii(part, maxlen, restlen, continuation_ws, b' ')
                    lines.extend(subl[:-1])
                    this = [subl[-1]]
                else:
                    this = [
