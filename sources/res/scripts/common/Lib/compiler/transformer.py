...
def parse(buf, mode=b'exec'):
    if mode == b'exec' or mode == b'single':
        return Transformer().parsesuite(buf)
    if mode == b'eval':
        return Transformer().parseexpr(buf)
    raise ValueError(b"compile() arg 3 must be 'exec' or 'eval' or 'single'")
    return
...
def Node(*args):
    ...
    if kind in nodes:
        ...
        raise WalkerError, b"Can't find appropriate Node type: %s" % str(args)
    return
...
class Transformer():
    ...
    def compile_node(self, node):
        ...
        raise WalkerError, (b'unexpected node type', n)
        return
    ...
    def expr_stmt(self, nodelist):
        ...
        raise WalkerError, b"can't get here"
        return
    ...
    def import_from(self, nodelist):
        ...
        if nodelist[idx + 1][0] == token.STAR:
            return From(fromname, [(b'*', None)], level, lineno=nodelist[0][2])
        else:
    ...
    def shift_expr(self, nodelist):
        ...
        for i in range(2, len(nodelist), 2):
            ...
            if nodelist[i - 1][0] == token.LEFTSHIFT:
                ...
                raise ValueError, b'unexpected token: %s' % nodelist[i - 1][0]
    ...
    def term(self, nodelist):
        ...
        for i in range(2, len(nodelist), 2):
            ...
            if t == token.STAR:
                ...
                raise ValueError, b'unexpected token: %s' % t
    ...
    def decode_literal(self, lit):
        if self.encoding:
            if self.encoding not in (b'utf-8', b'iso-8859-1'):
                lit = unicode(lit, b'utf-8').encode(self.encoding)
            return eval(b'# coding: %s\n%s' % (self.encoding, lit))
        else:
            return eval(lit)
    ...
    def com_arglist(self, nodelist):
        ...
        while i < len(nodelist):
            ...
            if i < len(nodelist) and nodelist[i][0] == token.EQUAL:
                ...
                raise SyntaxError, b'non-default argument follows default argument'
            i = i + 1
    ...
    def com_augassign(self, node):
        ...
        if l.__class__ in (Name, Slice, Subscript, Getattr):
            return l
        raise SyntaxError, b"can't assign to %s" % l.__class__.__name__
        return
    ...
    def com_assign(self, node, assigning):
        while 1:
            ...
            if t in (symbol.exprlist, symbol.testlist, symbol.testlist_safe, symbol.testlist_comp):
                ...
                if len(node) > 2:
                    raise SyntaxError, b"can't assign to operator"
                node = node[1]
            elif t == symbol.power:
                ...
                if t == token.LPAR:
                    ...
                    if node[0] == token.RPAR:
                        raise SyntaxError, b"can't assign to ()"
                elif t == token.LSQB:
                    node = node[2]
                ...
                raise SyntaxError, b"can't assign to literal"
            else:
                raise SyntaxError, b'bad assignment (%s)' % t
        ...
        for i in range(1, len(node), 2):
            if i + 1 < len(node):
                if node[i + 1][0] == symbol.list_for:
                    raise SyntaxError, b"can't assign to list comprehension"
            assigns.append(self.com_assign(node[i], assigning))
    ...
    def com_assign_trailer(self, primary, node, assigning):
        ...
        if t == token.LPAR:
            raise SyntaxError, b"can't assign to function call"
        raise SyntaxError, b'unknown trailer type: %s' % t
        return
    ...
    def com_list_comprehension(self, expr, node):
        return self.com_comprehension(expr, None, node, b'list')
    ...
    def com_comprehension(self, expr1, expr2, node, type):
        ...
        while node:
            ...
            if t == b'for':
                ...
                raise SyntaxError, b'unexpected comprehension element: %s %d' % (
                 node, lineno)
        ...
        if type == b'list':
            ...
            raise ValueError(b'unexpected comprehension type: ' + repr(type))
            return
    ...
    def com_generator_expression(self, expr, node):
        ...
        while node:
            ...
            if t == b'for':
                ...
                raise SyntaxError, b'unexpected generator expression element: %s %d' % (
                 node, lineno)
    ...
    def com_dictorsetmaker(self, nodelist):
        ...
        if len(nodelist) == 1 or nodelist[1][0] == token.COMMA:
            ...
            if nodelist[1][0] == symbol.comp_for:
                expr = self.com_node(nodelist[0])
                return self.com_comprehension(expr, None, nodelist[1], b'set')
            else:
                if len(nodelist) > 3 and nodelist[3][0] == symbol.comp_for:
                    key = self.com_node(nodelist[0])
                    value = self.com_node(nodelist[2])
                    return self.com_comprehension(key, value, nodelist[3], b'dict')
                items = []
    ...
    def com_apply_trailer(self, primaryNode, nodelist):
        ...
        raise SyntaxError, b'unknown node type: %s' % t
        return

    def com_select_member(self, primaryNode, nodelist):
        if nodelist[0] != token.NAME:
            raise SyntaxError, b'member must be a name'
            ...
            while i < len_nodelist:
                ...
                if node[0] == token.STAR:
                    if star_node is not None:
                        raise SyntaxError, b'already have the varargs indentifier'
                    star_node = self.com_node(nodelist[i + 1])
                    i = i + 3
                    continue
                elif node[0] == token.DOUBLESTAR:
                    if dstar_node is not None:
                        raise SyntaxError, b'already have the kwargs indentifier'
                ...
                if len_nodelist != 2 and isinstance(result, GenExpr) and len(node) == 3 and node[2][0] == symbol.comp_for:
                    raise SyntaxError, b'generator expression needs parenthesis'
                args.append(result)
                i = i + 2
    ...
    def com_argument(self, nodelist, kw, star_node):
        ...
        if len(nodelist) == 2:
            if kw:
                raise SyntaxError, b'non-keyword arg after keyword arg'
            if star_node:
                raise SyntaxError, b'only named arguments may follow *expression'
        ...
        if n[0] != token.NAME:
            raise SyntaxError, b"keyword can't be an expression (%s)" % n[0]
...
_doc_nodes = [
 ...
 symbol.factor,
 symbol.power]
_cmp_types = {(token.LESS): b'<', 
   (token.GREATER): b'>', 
   (token.EQEQUAL): b'==', 
   (token.EQUAL): b'==', 
   (token.LESSEQUAL): b'<=', 
   (token.GREATEREQUAL): b'>=', 
   (token.NOTEQUAL): b'!='}
_legal_node_types = [
 symbol.funcdef,
 symbol.classdef,
 ...
 symbol.power,
 symbol.atom]
if hasattr(symbol, b'yield_stmt'):
    _legal_node_types.append(symbol.yield_stmt)
if hasattr(symbol, b'yield_expr'):
