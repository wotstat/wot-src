...
def PBKDF1(password, salt, dkLen, count=1000, hashAlgo=None):
    ...
    if dkLen > digest:
        raise TypeError(b'Selected hash algorithm has a too short digest (%d bytes).' % digest)
    if len(salt) != 8:
        raise ValueError(b'Salt is not 8 bytes long.')
...
def PBKDF2(password, salt, dkLen=16, count=1000, prf=None):
    ...
    i = 1
    while len(key) < dkLen:
        U = previousU = prf(password, salt + struct.pack(b'>I', i))
...
class _S2V(object):
    ...
    def update(self, item):
        if not item:
            raise ValueError(b'A component cannot be empty')
        if self._n_updates == 0:
            raise TypeError(b'Too many components passed to S2V')
        self._n_updates -= 1
