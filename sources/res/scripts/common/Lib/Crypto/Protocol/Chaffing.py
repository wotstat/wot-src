...
class Chaff:
    ...
    def __init__(self, factor=1.0, blocksper=1):
        if not 0.0 <= factor <= 1.0:
            raise ValueError, b"'factor' must be between 0.0 and 1.0"
        if blocksper < 0:
            raise ValueError, b"'blocksper' must be zero or more"
...
if __name__ == b'__main__':
    ...
    print b'Adding chaff...'
    c = Chaff(factor=0.5, blocksper=2)
    chaffed = c.chaff(source)
    from base64 import encodestring
    wheat = []
    print b'chaffed message blocks:'
    ...
    print b'Undigesting wheat...'
    newtext = (b'').join(wheat)
    if newtext == text:
        print b'They match!'
    else:
        print b'They differ!'
