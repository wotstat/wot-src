...
class AllOrNothing:
    ...
    def undigest(self, blocks):
        if len(blocks) < 2:
            raise ValueError, b'List must be at least length 2.'
        ...
        text = b(b'').join(map(long_to_bytes, parts[:-1]))
...
if __name__ == b'__main__':
    ...
    def usage(code, msg=None):
        if msg:
            print msg
        print usagemsg % {b'program': (sys.argv[0]), b'ciphermodule': ciphermodule}
        sys.exit(code)
        return


    try:
        opts, args = getopt.getopt(sys.argv[1:], b'c:l', [b'cipher=', b'aslong'])
    except getopt.error as msg:
        usage(1, msg)

    if args:
        usage(1, b'Too many arguments')
    for opt, arg in opts:
        if opt in (b'-h', b'--help'):
            usage(0)
        elif opt in (b'-c', b'--cipher'):
            ciphermodule = arg
        elif opt in (b'-l', b'--aslong'):
            aslong = 1

    module = __import__(b'Crypto.Cipher.' + ciphermodule, None, None, [b'new'])
    x = AllOrNothing(module)
    print b'Original text:\n=========='
    print __doc__
    print b'=========='
    msgblocks = x.digest(b(__doc__))
    print b'message blocks:'
