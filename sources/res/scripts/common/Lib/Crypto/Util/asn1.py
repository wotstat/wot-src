...
class DerObject(object):
    ...
    def __init__(self, asn1Id=None, payload=b(b''), implicit=None, constructed=False):
        if asn1Id == None:
            self._idOctet = None
            return
        else:
    ...
    def _convertTag(self, tag):
        ...
        if not (_isInt(tag)) and 0 <= tag < 31:
            raise ValueError(b'Wrong DER tag')
        return tag
    ...
    def _decodeLen(self, s):
        ...
        if payloadLength <= 127:
            raise ValueError(b'Not a DER length tag (but still valid BER).')
    ...
    def decode(self, derEle):
        ...
        try:
            b = s.read_byte()
            raise ValueError(b'Unexpected extra data after the DER structure')
        except EOFError:
            pass
    ...
    def _decodeFromStream(self, s):
        ...
        if self._idOctet != None:
            if idOctet != self._idOctet:
                raise ValueError(b'Unexpected DER tag')
        else:
...
class DerInteger(DerObject):
    ...
    def __init__(self, value=0, implicit=None):
        DerObject.__init__(self, 2, b(b''), implicit, False)
        self.value = value
        return
...
class DerSequence(DerObject):
    ...
    def __init__(self, startSeq=None, implicit=None):
        DerObject.__init__(self, 16, b(b''), implicit, True)
        if startSeq == None:
            self._seq = []
        else:
            self._seq = startSeq
        ...
        for item in self._seq:
            try:
                ...
                try:
                    ...
                    raise ValueError(b'Trying to DER encode an unknown object')
...
class DerOctetString(DerObject):
    ...
    def __init__(self, value=b(b''), implicit=None):
...
class DerNull(DerObject):
    ...
    def __init__(self):
        DerObject.__init__(self, 5, b(b''), False)
        return
...
class DerObjectId(DerObject):
    ...
    def __init__(self, value=b'', implicit=None):
        DerObject.__init__(self, 6, b(b''), implicit, False)
        self.value = value
        return

    def encode(self):
        comps = map(int, self.value.split(b'.'))
        if len(comps) < 2:
            raise ValueError(b'Not a valid Object Identifier string')
        ...
        for v in comps[2:]:
            ...
            enc[-1] &= 127
            self.payload += b(b'').join(map(bchr, enc))
...
class DerBitString(DerObject):
    ...
    def __init__(self, value=b(b''), implicit=None):
        DerObject.__init__(self, 3, b(b''), implicit, False)
        self.value = value
        return

    def encode(self):
        self.payload = b(b'\x00') + self.value
        return DerObject.encode(self)
    ...
    def _decodeFromStream(self, s):
        ...
        if self.payload and bord(self.payload[0]) != 0:
            raise ValueError(b'Not a valid BIT STRING')
        self.value = b(b'')
        if self.payload:
...
class DerSetOf(DerObject):
    ...
    def __init__(self, startSet=None, implicit=None):
        DerObject.__init__(self, 17, b(b''), implicit, True)
        self._seq = []
    ...
    def add(self, elem):
        ...
        if self._elemOctet != eo:
            if self._elemOctet:
                raise ValueError(b'New element does not belong to the set')
            self._elemOctet = eo
        ...
        while True:
            try:
                ...
                if setIdOctet < 0:
                    ...
                    raise ValueError(b'Not all elements are of the same DER type')
                if setIdOctet != 2:
                    self._seq.append(p._recording)
                else:
                    derInt = DerInteger()
