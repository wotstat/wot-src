...
class AESGenerator(object):
    ...
    _four_kiblocks_of_zeros = b(b'\x00') * block_size * 4096
    ...
    def reseed(self, seed):
        if self.key is None:
            self.key = b(b'\x00') * self.key_size
    ...
    def _pseudo_random_data(self, bytes):
        assert 0 <= bytes <= self.max_bytes_per_request, b'You cannot ask for more than 1 MiB of data per request'
    ...
    def _generate_blocks(self, num_blocks):
        if self.key is None:
            raise AssertionError(b'generator must be seeded before use')
        retval = []
