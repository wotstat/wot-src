...
class PBES1(object):
    ...
    def decrypt(data, passphrase):
        ...
        cipher_params = {}
        if pbe_oid == b'1.2.840.113549.1.5.3':
            hashmod = MD5
            ciphermod = DES
        elif pbe_oid == b'1.2.840.113549.1.5.6':
            hashmod = MD5
            ciphermod = ARC2
            cipher_params[b'effective_keylen'] = 64
        elif pbe_oid == b'1.2.840.113549.1.5.10':
            hashmod = SHA1
            ciphermod = DES
        elif pbe_oid == b'1.2.840.113549.1.5.11':
            hashmod = SHA1
            ciphermod = ARC2
            cipher_params[b'effective_keylen'] = 64
        else:
            raise ValueError(b'Unknown OID')
...
class PBES2(object):
    ...
    def encrypt(data, passphrase, protection, prot_params=None, randfunc=None):
        ...
        if protection == b'PBKDF2WithHMAC-SHA1AndDES-EDE3-CBC':
            key_size = 24
            module = DES3
            protection = DES3.MODE_CBC
            enc_oid = b'1.2.840.113549.3.7'
        elif protection == b'PBKDF2WithHMAC-SHA1AndAES128-CBC':
            key_size = 16
            module = AES
            protection = AES.MODE_CBC
            enc_oid = b'2.16.840.1.101.3.4.1.2'
        elif protection == b'PBKDF2WithHMAC-SHA1AndAES192-CBC':
            key_size = 24
            module = AES
            protection = AES.MODE_CBC
            enc_oid = b'2.16.840.1.101.3.4.1.22'
        elif protection == b'PBKDF2WithHMAC-SHA1AndAES256-CBC':
            key_size = 32
            module = AES
            protection = AES.MODE_CBC
            enc_oid = b'2.16.840.1.101.3.4.1.42'
        else:
            raise ValueError(b'Unknown mode')
        iv = randfunc(module.block_size)
        salt = randfunc(prot_params.get(b'salt_size', 8))
        count = prot_params.get(b'iteration_count', 1000)
        key = PBKDF2(passphrase, salt, key_size, count)
        key_derivation_func = newDerSequence(DerObjectId(b'1.2.840.113549.1.5.12'), newDerSequence(DerOctetString(salt), DerInteger(count)))
        cipher = module.new(key, protection, iv)
        encrypted_data = cipher.encrypt(pad(data, cipher.block_size))
        encryption_scheme = newDerSequence(DerObjectId(enc_oid), DerOctetString(iv))
        encrypted_private_key_info = newDerSequence(newDerSequence(DerObjectId(b'1.2.840.113549.1.5.13'), newDerSequence(key_derivation_func, encryption_scheme)), DerOctetString(encrypted_data))
    ...
    def decrypt(data, passphrase):
        ...
        if pbe_oid != b'1.2.840.113549.1.5.13':
            raise ValueError(b'Not a PBES2 object')
        ...
        if key_derivation_oid != b'1.2.840.113549.1.5.12':
            raise ValueError(b'Unknown KDF')
        ...
        if len(pbkdf2_params) > 3:
            raise ValueError(b'Unsupported PRF for PBKDF2')
        encryption_scheme = decode_der(DerSequence, pbes2_params[1])
        encryption_oid = decode_der(DerObjectId, encryption_scheme[0]).value
        if encryption_oid == b'1.2.840.113549.3.7':
            ciphermod = DES3
            key_size = 24
        elif encryption_oid == b'2.16.840.1.101.3.4.1.2':
            ciphermod = AES
            key_size = 16
        elif encryption_oid == b'2.16.840.1.101.3.4.1.22':
            ciphermod = AES
            key_size = 24
        elif encryption_oid == b'2.16.840.1.101.3.4.1.42':
            ciphermod = AES
            key_size = 32
        else:
            raise ValueError(b'Unsupported cipher')
        if pbkdf2_key_length and pbkdf2_key_length != key_size:
            raise ValueError(b'Mismatch between PBKDF2 parameters and selected cipher')
