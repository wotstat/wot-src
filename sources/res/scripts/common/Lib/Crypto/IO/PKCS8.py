...
def unwrap(p8_private_key, passphrase=None):
    if passphrase:
        ...
        if not found:
            raise ValueError(b'Unsupported PKCS#5 Object ID ')
    pk_info = decode_der(DerSequence, p8_private_key)
    if len(pk_info) == 2 and not passphrase:
        raise ValueError(b'Not a valid clear PKCS#8 structure (maybe it is encrypted?)')
    if not 3 <= len(pk_info) <= 4 or pk_info[0] != 0:
        raise ValueError(b'Not a valid PrivateKeyInfo SEQUENCE')
    algo_id = decode_der(DerSequence, pk_info[1])
    if not 1 <= len(algo_id) <= 2:
        raise ValueError(b'Not a valid AlgorithmIdentifier SEQUENCE')
    algo = decode_der(DerObjectId, algo_id[0]).value
    private_key = decode_der(DerOctetString, pk_info[2]).payload
    if len(algo_id) == 2 and algo_id[1] != b(b'\x05\x00'):
        params = algo_id[1]
    else:
        params = None
    return (
     algo, private_key, params)
