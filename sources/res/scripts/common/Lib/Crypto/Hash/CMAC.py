...
class _SmoothMAC(object):
    ...
    def __init__(self, block_size, msg=b(b''), min_digest=0):
        self._bs = block_size
        self._buffer = []
        self._buffer_len = 0
        self._total_len = 0
    ...
    def _update(self, data_block):
        raise NotImplementedError(b'_update() must be still implemented')
        return

    def _digest(self, left_data):
        raise NotImplementedError(b'_digest() must be still implemented')
        return
    ...
    def digest(self):
        ...
        if self._buffer_len > 0:
            self.update(b(b''))
        left_data = b(b'').join(self._buffer)
...
class CMAC(_SmoothMAC):
    ...
    def __init__(self, key, msg=None, ciphermod=None):
        if ciphermod is None:
            raise TypeError(b'ciphermod must be specified (try AES)')
        ...
        if ciphermod.block_size == 8:
            ...
            raise TypeError(b'CMAC requires a cipher with a block size of 8 or 16 bytes, not %d' % (
    ...
    def copy(self):
        ...
        for m in [b'_tag', b'_k1', b'_k2', b'_IV']:
