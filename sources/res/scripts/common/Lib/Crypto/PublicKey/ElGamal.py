__revision__ = b'$Id$'
__all__ = [
 b'generate', b'construct', b'error', b'ElGamalobj']
...
def construct(tup):
    ...
    if len(tup) not in (3, 4):
        raise ValueError(b'argument for construct() wrong length')
...
class ElGamalobj(pubkey):
    ...
    def _decrypt(self, M):
        if not hasattr(self, b'x'):
            raise TypeError(b'Private key not available in this object')
    ...
    def _sign(self, M, K):
        ...
        p1 = self.p - 1
        if GCD(K, p1) != 1:
            raise ValueError(b'Bad K value: GCD(K,p-1)!=1')
        a = pow(self.g, K, self.p)
        t = (M - self.x * a) % p1
        while t < 0:
            t = t + p1
