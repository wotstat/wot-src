...
class _RSAKey(object):
    ...
    def _decrypt(self, c):
        if not self.has_private():
            raise TypeError(b'No private key')
        if hasattr(self, b'p') and hasattr(self, b'q') and hasattr(self, b'u'):
...
def rsa_construct(n, e, d=None, p=None, q=None, u=None):
    ...
    if d is None:
        ...
        if p is not None and q is not None:
            ...
            if not spotted:
                raise ValueError(b'Unable to compute factors p and q from exponent d.')
...
class _DSAKey(object):
    ...
    def has_private(self):
        return hasattr(self, b'x')

    def _sign(self, m, k, blind):
        if not self.has_private():
            raise TypeError(b'No private key')
        if not 1L < k < self.q:
            raise ValueError(b'k is not between 2 and q-1')
