...
__revision__ = b'$Id$'
__all__ = [b'new', b'PKCS1OAEP_Cipher']
...
class PKCS1OAEP_Cipher:
    ...
    def encrypt(self, message):
        ...
        if ps_len < 0:
            raise ValueError(b'Plaintext is too long.')
    ...
    def decrypt(self, ct):
        ...
        if len(ct) != k or k < hLen + 2:
            raise ValueError(b'Ciphertext with incorrect length.')
        ...
        if bord(y) != 0:
            valid = 0
        if not valid:
            raise ValueError(b'Incorrect decryption.')
        return db[hLen + one + 1:]


def new(key, hashAlgo=None, mgfunc=None, label=b(b'')):
