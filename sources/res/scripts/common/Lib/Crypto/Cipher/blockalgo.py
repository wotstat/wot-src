...
def _getParameter(name, index, args, kwargs, default=None):
    ...
    if len(args) > index:
        if param:
            raise TypeError(b"Parameter '%s' is specified twice" % name)
        param = args[index]
...
class _CBCMAC(_SmoothMAC):
    ...
    def _ignite(self, data):
        if self._mac:
            raise TypeError(b'_ignite() cannot be called twice')
...
class BlockAlgo:
    ...
    def __init__(self, factory, key, *args, **kwargs):
        self.mode = _getParameter(b'mode', 0, args, kwargs, default=MODE_ECB)
        self.block_size = factory.block_size
        self._factory = factory
        self._tag = None
        if self.mode == MODE_CCM:
            if self.block_size != 16:
                raise TypeError(b'CCM mode is only available for ciphers that operate on 128 bits blocks')
            self._mac_len = kwargs.get(b'mac_len', 16)
            if self._mac_len not in (4, 6, 8, 10, 12, 14, 16):
                raise ValueError(b"Parameter 'mac_len' must be even and in the range 4..16")
            self.nonce = _getParameter(b'nonce', 1, args, kwargs)
            if not (self.nonce) and 7 <= len(self.nonce) <= 13:
                raise ValueError(b"Length of parameter 'nonce' must be in the range 7..13 bytes")
            self._key = key
            self._msg_len = kwargs.get(b'msg_len', None)
            self._assoc_len = kwargs.get(b'assoc_len', None)
    ...
    def _start_gcm(self, factory, key, *args, **kwargs):
        if self.block_size != 16:
            raise TypeError(b'GCM mode is only available for ciphers that operate on 128 bits blocks')
        self.nonce = _getParameter(b'nonce', 1, args, kwargs)
        if not self.nonce:
            raise TypeError(b'MODE_GCM requires a nonce')
        self._mac_len = kwargs.get(b'mac_len', 16)
        if not (self._mac_len) and 4 <= self._mac_len <= 16:
            raise ValueError(b"Parameter 'mac_len' must not be larger than 16 bytes")
        self._next = [
         self.update, self.encrypt, self.decrypt,
         self.digest, self.verify]
        self._done_assoc_data = False
        ...
        if len(self.nonce) == 12:
            self._j0 = bytes_to_long(self.nonce + b(b'\x00\x00\x00\x01'))
        else:
    ...
    def _start_siv(self, factory, key, *args, **kwargs):
        ...
        if rem:
            raise ValueError(b'MODE_SIV requires a key twice as long as for the underlying cipher')
        self.nonce = _getParameter(b'nonce', 1, args, kwargs)
    ...
    def _start_eax(self, factory, key, *args, **kwargs):
        self.nonce = _getParameter(b'nonce', 1, args, kwargs)
        if not self.nonce:
            raise TypeError(b'MODE_EAX requires a nonce')
        self._next = [
         self.update, self.encrypt, self.decrypt,
         self.digest, self.verify]
        self._mac_len = kwargs.get(b'mac_len', self.block_size)
        if not (self._mac_len) and 4 <= self._mac_len <= self.block_size:
            raise ValueError(b"Parameter 'mac_len' must not be larger than %d" % self.block_size)
    ...
    def _start_PGP(self, factory, key, *args, **kwargs):
        ...
        self.IV = _getParameter(b'IV', 1, args, kwargs)
        if self.IV is None:
            self.IV = _getParameter(b'iv', 1, args, kwargs)
        if not self.IV:
            raise ValueError(b'MODE_OPENPGP requires an IV')
        IV_cipher = factory.new(key, MODE_CFB, b(b'\x00') * self.block_size, segment_size=self.block_size * 8)
        if len(self.IV) == self.block_size:
            self._encrypted_IV = IV_cipher.encrypt(self.IV + self.IV[-2:] + b(b'\x00') * (self.block_size - 2))[:self.block_size + 2]
        elif len(self.IV) == self.block_size + 2:
            self._encrypted_IV = self.IV
            self.IV = IV_cipher.decrypt(self.IV + b(b'\x00') * (self.block_size - 2))[:self.block_size + 2]
            if self.IV[-2:] != self.IV[-4:-2]:
                raise ValueError(b'Failed integrity check for OPENPGP IV')
            self.IV = self.IV[:-2]
        else:
            raise ValueError(b'Length of IV must be %d or %d bytes for MODE_OPENPGP' % (
    ...
    def _start_ccm(self, assoc_len=None, msg_len=None):
        if self._cipherMAC.can_reduce():
            ...
            assoc_len_encoded = b(b'')
            if self._assoc_len > 0:
                if self._assoc_len < 65536 - 256:
                    enc_size = 2
                elif self._assoc_len < 4294967296L:
                    assoc_len_encoded = b(b'\xff\xfe')
                    enc_size = 4
                else:
                    assoc_len_encoded = b(b'\xff\xff')
                    enc_size = 8
    ...
    def update(self, assoc_data):
        if self.mode not in (MODE_CCM, MODE_EAX, MODE_SIV, MODE_GCM):
            raise TypeError(b'update() not supported by this mode of operation')
        if self.update not in self._next:
            raise TypeError(b'update() can only be called immediately after initialization')
    ...
    def encrypt(self, plaintext):
        if self.mode == MODE_OPENPGP:
            ...
            if padding_length > 0:
                if self._done_last_block:
                    raise ValueError(b'Only the last chunk is allowed to have length not multiple of %d bytes', self.block_size)
                self._done_last_block = True
                padded = plaintext + b(b'\x00') * padding_length
        ...
        if self.mode in (MODE_CCM, MODE_EAX, MODE_SIV, MODE_GCM):
            if self.encrypt not in self._next:
                raise TypeError(b'encrypt() can only be called after initialization or an update()')
            self._next = [
        ...
        if self.mode == MODE_OPENPGP:
            ...
            if padding_length > 0:
                if self._done_last_block:
                    raise ValueError(b'Only the last chunk is allowed to have length not multiple of %d bytes', self.block_size)
                self._done_last_block = True
                padded = ciphertext + b(b'\x00') * padding_length
                res = self._cipher.decrypt(padded)[:len(ciphertext)]
            else:
                res = self._cipher.decrypt(ciphertext)
            return res
        if self.mode == MODE_SIV:
            raise TypeError(b'decrypt() not allowed for SIV mode. Use decrypt_and_verify() instead.')
        if self.mode in (MODE_CCM, MODE_EAX, MODE_GCM):
            if self.decrypt not in self._next:
                raise TypeError(b'decrypt() can only be called after initialization or an update()')
            self._next = [
    ...
    def digest(self):
        if self.mode not in (MODE_CCM, MODE_EAX, MODE_SIV, MODE_GCM):
            raise TypeError(b'digest() not supported by this mode of operation')
        if self.digest not in self._next:
            raise TypeError(b'digest() cannot be called when decrypting or validating a message')
        self._next = [
         self.digest]
    ...
    def hexdigest(self):
        return (b'').join([b'%02x' % bord(x) for x in self.digest()])

    def verify(self, mac_tag):
        if self.mode not in (MODE_CCM, MODE_EAX, MODE_SIV, MODE_GCM):
            raise TypeError(b'verify() not supported by this mode of operation')
        if self.verify not in self._next:
            raise TypeError(b'verify() cannot be called when encrypting a message')
        self._next = [
         self.verify]
        res = 0
    ...
    def encrypt_and_digest(self, plaintext):
        return (
         self.encrypt(plaintext), self.digest())
