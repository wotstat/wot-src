...
def list_public_methods(obj):
    return [member for member in dir(obj) if not member.startswith(b'_') and hasattr(getattr(obj, member), b'__call__')]
...
class SimpleXMLRPCDispatcher():
    ...
    def _marshaled_dispatch(self, data, dispatch_method=None, path=None):
        try:
            ...
            response = xmlrpclib.dumps(xmlrpclib.Fault(1, b'%s:%s' % (exc_type, exc_value)), encoding=self.encoding, allow_none=self.allow_none)
        ...
        if not self.is_rpc_path_valid():
            ...
            try:
                ...
                if hasattr(self.server, b'_send_traceback_header') and self.server._send_traceback_header:
                    ...
                    self.send_header(b'X-traceback', traceback.format_exc())
...
class CGIXMLRPCRequestHandler(SimpleXMLRPCDispatcher):
    ...
    def handle_get(self):
        ...
        response = BaseHTTPServer.DEFAULT_ERROR_MESSAGE % {b'code': code, 
           b'message': message, 
           b'explain': explain}
        print b'Status: %d %s' % (code, message)
        print b'Content-Type: %s' % BaseHTTPServer.DEFAULT_ERROR_CONTENT_TYPE
        print b'Content-Length: %d' % len(response)
        print
        sys.stdout.write(response)
        return
...
if __name__ == b'__main__':
    print b'Running XML-RPC server on port 8000'
