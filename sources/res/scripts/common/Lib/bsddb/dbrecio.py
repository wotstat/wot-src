...
class DBRecIO:
    ...
    def truncate(self, size=None):
        ...
        if size is None:
            ...
            raise IOError(errno.EINVAL, b'Negative size not allowed')
        elif size < self.pos:
            self.pos = size
        self.db.put(self.key, b'', txn=self.txn, dlen=self.len - size, doff=size)
        return
    ...
    def write(self, s):
        ...
        if self.pos > self.len:
            self.buflist.append(b'\x00' * (self.pos - self.len))
            self.len = self.pos
    ...
    def writelines(self, list):
        self.write(string.joinfields(list, b''))
        return
