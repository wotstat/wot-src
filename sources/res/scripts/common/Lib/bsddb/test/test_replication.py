...
class DBReplicationManager(DBReplication):
    ...
    def test01_basic_replication(self):
        ...
        if db.version() >= (5, 2):
            self.site = self.dbenvMaster.repmgr_site(b'127.0.0.1', master_port)
            self.site.set_config(db.DB_GROUP_CREATOR, True)
            self.site.set_config(db.DB_LOCAL_SITE, True)
            self.site2 = self.dbenvMaster.repmgr_site(b'127.0.0.1', client_port)
            self.site3 = self.dbenvClient.repmgr_site(b'127.0.0.1', master_port)
            self.site3.set_config(db.DB_BOOTSTRAP_HELPER, True)
            self.site4 = self.dbenvClient.repmgr_site(b'127.0.0.1', client_port)
            self.site4.set_config(db.DB_LOCAL_SITE, True)
            d = {(db.DB_BOOTSTRAP_HELPER): [
            ...
            for i, j in zip([self.site, self.site2, self.site3, self.site4], [
             master_port, client_port, master_port, client_port]):
                addr = i.get_address()
                self.assertEqual(addr, (b'127.0.0.1', j))
            ...
            self.dbenvMaster.repmgr_set_local_site(b'127.0.0.1', master_port)
            self.dbenvClient.repmgr_set_local_site(b'127.0.0.1', client_port)
            self.dbenvMaster.repmgr_add_remote_site(b'127.0.0.1', client_port)
            self.dbenvClient.repmgr_add_remote_site(b'127.0.0.1', master_port)
        ...
        if time.time() > timeout:
            import sys
            print >> sys.stderr, b'XXX: timeout happened beforestartup was confirmed - see issue 3892'
            startup_timeout = True
        d = self.dbenvMaster.repmgr_site_list()
        self.assertEqual(len(d), 1)
        d = d.values()[0]
        self.assertEqual(d[0], b'127.0.0.1')
        ...
        if db.version() >= (4, 6):
            ...
            self.assertIn(b'msgs_queued', d)
        self.dbMaster = db.DB(self.dbenvMaster)
        txn = self.dbenvMaster.txn_begin()
        self.dbMaster.open(b'test', db.DB_HASH, db.DB_CREATE, 438, txn=txn)
        txn.commit()
        import time, os.path
        timeout = time.time() + 10
        while time.time() < timeout and not os.path.exists(os.path.join(self.homeDirClient, b'test')):
            time.sleep(0.01)
        ...
        while True:
            ...
            try:
                self.dbClient.open(b'test', db.DB_HASH, flags=db.DB_RDONLY, mode=438, txn=txn)
        ...
        if time.time() >= timeout and startup_timeout:
            self.skipTest(b'replication test skipped due to random failure, see issue 3892')
...
class DBBaseReplication(DBReplication):
    ...
    def test01_basic_replication(self):
        ...
        self.assertIn(b'master_changes', d)
        txn = self.dbenvMaster.txn_begin()
        self.dbMaster.put(b'ABC', b'123', txn=txn)
        txn.commit()
        import time
    ...
    if db.version() >= (4, 6):
        ...
        def test03_master_election(self):
            ...
            while not self.dbenvClient.rep_stat()[b'startup_complete']:
                pass
