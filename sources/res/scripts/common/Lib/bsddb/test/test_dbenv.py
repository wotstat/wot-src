...
class DBEnv_general(DBEnv):
    ...
    if db.version() >= (4, 7):
        ...
        def test_getset_intermediate_dir_mode(self):
            ...
            for mode in [b'rwx------', b'rw-rw-rw-', b'rw-r--r--']:
                self.env.set_intermediate_dir_mode(mode)
                self.assertEqual(mode, self.env.get_intermediate_dir_mode())

            self.assertRaises(db.DBInvalidArgError, self.env.set_intermediate_dir_mode, b'abcde')
            return
    ...
    if db.version() >= (4, 4):
        ...
        def test_mutex_stat(self):
            ...
            self.assertIn(b'mutex_inuse_max', stat)
            return
    ...
    def test_mp_max_write(self):
        ...
        def test_lg_dir(self):
            for i in [b'a', b'bb', b'ccc', b'dddd']:
...
class DBEnv_log(DBEnv):
    ...
    def test_log_file(self):
        ...
        self.assertEqual(b'log.0000000001', log_file[-14:])
        return
...
class DBEnv_memp(DBEnv):
    ...
    def setUp(self):
        ...
        self.db.open(b'test', db.DB_HASH, db.DB_CREATE, 432)
        return
    ...
    def test_memp_stat_1(self):
        ...
        self.assertIn(b'cache_miss', stats[0])
    ...
    def test_memp_stat_2(self):
        ...
        self.assertIn(b'test', stats)
        self.assertIn(b'page_in', stats[b'test'])
        return
...
class DBEnv_logcursor(DBEnv):
    ...
    def setUp(self):
        ...
        txn = self.env.txn_begin()
        self.db = db.DB(self.env)
        self.db.open(b'test', db.DB_HASH, db.DB_CREATE, 432, txn=txn)
        txn.commit()
