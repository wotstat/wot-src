...
class ConcurrentDataStoreBase(BaseThreadedTestCase):
    ...
    def test01_1WriterMultiReaders(self):
        if verbose:
            print b'\n', b'-=' * 30
            print b'Running %s.test01_1WriterMultiReaders...' % self.__class__.__name__
        ...
        for x in xrange(self.writers):
            ...
            wt = Thread(target=self.writerThread, args=(
             self.d, a, b), name=b'writer %d' % x)
            writers.append(wt)
    ...
    def writerThread(self, d, keys, readers):
        ...
        if verbose:
            print b'%s: creating records %d - %d' % (name, start, stop)
        ...
        for x in keys:
            ...
            if verbose and x % 100 == 0:
                print b'%s: records %d - %d finished' % (name, start, x)
            count2 -= 1
            if not count2:
                readers.pop().start()
                count2 = count
        ...
        if verbose:
            print b'%s: finished creating records' % name
        if verbose:
            print b'%s: thread finished' % name
        return
    ...
    def readerThread(self, d, readerNum):
        ...
        for i in xrange(5):
            ...
            if verbose:
                print b'%s: found %d records' % (name, count)
            c.close()
...
class SimpleThreadedBase(BaseThreadedTestCase):
    ...
    def test02_SimpleLocks(self):
        if verbose:
            print b'\n', b'-=' * 30
            print b'Running %s.test02_SimpleLocks...' % self.__class__.__name__
        keys = range(self.records)
        import random
        random.shuffle(keys)
        records_per_writer = self.records // self.writers
...
class ThreadedTransactionsBase(BaseThreadedTestCase):
    ...
    def test03_ThreadedTransactions(self):
        if verbose:
            print b'\n', b'-=' * 30
            print b'Running %s.test03_ThreadedTransactions...' % self.__class__.__name__
        keys = range(self.records)
        import random
        random.shuffle(keys)
        records_per_writer = self.records // self.writers
            ...
            try:
                ...
                if verbose:
                    if sys.version_info < (2, 6):
                        print b'%s: Aborting transaction (%s)' % (name, val[1])
                    else:
                        print b'%s: Aborting transaction (%s)' % (name,
                         val.args[1])
                txn.abort()
        ...
        while self.doLockDetect:
            ...
            try:
                ...
                if verbose and aborted:
                    print b'deadlock: Aborted %d deadlocked transaction(s)' % aborted
            except db.DBError:
                pass
...
def test_suite():
    ...
    if have_threads:
        ...
        print b'Threads not available, skipping thread tests.'
    return suite
