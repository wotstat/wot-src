...
class DB_general(DB):
    ...
    if db.version() >= (4, 6):
        ...
        def test_priority(self):
            flags = [
             db.DB_PRIORITY_VERY_LOW, db.DB_PRIORITY_LOW,
...
class DB_txn(DB):
    ...
    def test_get_transactional(self):
        self.assertFalse(self.db.get_transactional())
        self.db.open(b'XXX', dbtype=db.DB_HASH, flags=db.DB_CREATE | db.DB_AUTO_COMMIT)
...
class DB_recno(DB):
    ...
    def test_re_source(self):
        for i in [b'test', b'test2', b'test3']:
