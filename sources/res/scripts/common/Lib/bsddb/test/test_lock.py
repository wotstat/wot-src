...
class LockingTestCase(unittest.TestCase):
    ...
    def test01_simple(self):
        if verbose:
            print b'\n', b'-=' * 30
            print b'Running %s.test01_simple...' % self.__class__.__name__
        anID = self.env.lock_id()
        if verbose:
            print b'locker ID: %s' % anID
        lock = self.env.lock_get(anID, b'some locked thing', db.DB_LOCK_WRITE)
        if verbose:
            print b'Acquired lock: %s' % lock
        self.env.lock_put(lock)
        if verbose:
            print b'Released lock: %s' % lock
    ...
    def test02_threaded(self):
        if verbose:
            print b'\n', b'-=' * 30
            print b'Running %s.test02_threaded...' % self.__class__.__name__
        threads = []
        threads.append(Thread(target=self.theThread, args=(
         db.DB_LOCK_WRITE,)))
        threads.append(Thread(target=self.theThread, args=(
         db.DB_LOCK_READ,)))
        threads.append(Thread(target=self.theThread, args=(
    ...
    def test04_lock_timeout2(self):
        ...
        lock = self.env.lock_get(anID, b'shared lock', db.DB_LOCK_WRITE)
        start_time = time.time()
        self.assertRaises(db.DBLockNotGrantedError, self.env.lock_get, anID2, b'shared lock', db.DB_LOCK_READ)
        end_time = time.time()
    ...
    def theThread(self, lockType):
        ...
        anID = self.env.lock_id()
        if verbose:
            print b'%s: locker ID: %s' % (name, anID)
        for i in xrange(1000):
            lock = self.env.lock_get(anID, b'some locked thing', lockType)
            if verbose:
                print b'%s: Acquired %s lock: %s' % (name, lt, lock)
            self.env.lock_put(lock)
            if verbose:
                print b'%s: Released %s lock: %s' % (name, lt, lock)
...
def test_suite():
    ...
    if have_threads:
        ...
        suite.addTest(unittest.makeSuite(LockingTestCase, b'test01'))
    return suite
