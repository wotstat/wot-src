...
if sys.version_info[0] >= 3:
    ...
    class logcursor_py3k(object):
        ...
        def __next__(self):
            v = getattr(self._logcursor, b'next')()
            if v is not None:
                v = (
                 v[0], v[1].decode(charset))
            return v
    ...
    class DB_py3k(object):
        ...
        def __contains__(self, k):
            if isinstance(k, str):
                k = bytes(k, charset)
            return getattr(self._db, b'has_key')(k)
        ...
        def get(self, key, default=b'MagicCookie', txn=None, flags=0, dlen=-1, doff=-1):
            if isinstance(key, str):
                key = bytes(key, charset)
            if default != b'MagicCookie':
...
verbose = 0
if b'verbose' in sys.argv:
    verbose = 1
    sys.argv.remove(b'verbose')
if b'silent' in sys.argv:
    verbose = 0
    sys.argv.remove(b'silent')

def print_versions():
    print
    print b'-=' * 38
    print db.DB_VERSION_STRING
    print b'bsddb.db.version():   %s' % (db.version(),)
    if db.version() >= (5, 0):
        print b'bsddb.db.full_version(): %s' % repr(db.full_version())
    print b'bsddb.db.__version__: %s' % db.__version__
    print b'bsddb.db.cvsid:       %s' % db.cvsid
    suffix = b'__'
    print b'py module:            %s' % getattr(bsddb, b'__file' + suffix)
    print b'extension module:     %s' % getattr(bsddb, b'__file' + suffix)
    print b'python version:       %s' % sys.version
    print b'My pid:               %s' % os.getpid()
    print b'-=' * 38
    return
...
def get_new_path(name):
    ...
    try:
        import os
        path = os.path.join(get_new_path.prefix, name + b'_' + str(os.getpid()) + b'_' + str(get_new_path.num))
        get_new_path.num += 1
    finally:
...
def get_new_environment_path():
    path = get_new_path(b'environment')
    import os
    try:
...
import os, os.path
get_new_path.prefix = os.path.join(os.environ.get(b'TMPDIR', os.path.join(os.sep, b'tmp')), b'z-Berkeley_DB')
get_new_path.num = 0
...
if sys.version_info[0] < 3:
    ...
    import sys
    print >> sys.stderr, b'Work to do!'

def suite(module_prefix=b'', timing_check=None):
    test_modules = [
     1, 
     2, 
     3, 
