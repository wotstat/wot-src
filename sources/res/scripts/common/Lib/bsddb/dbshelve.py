...
def open(filename, flags=db.DB_CREATE, mode=432, filetype=db.DB_HASH, dbenv=None, dbname=None):
    if type(flags) == type(b''):
        ...
        if sflag == b'c':
            ...
            raise db.DBError, b"flags should be one of 'r', 'w', 'c' or 'n' or use the bsddb.db.DB_* flags"
    d = DBShelf(dbenv)
...
class DBShelf(MutableMapping):
    ...
    def __repr__(self):
        if self._closed:
            return b'<DBShelf @ 0x%x - closed>' % id(self)
        else:
    ...
    def append(self, value, txn=None):
        ...
        raise DBShelveError, b'append() only supported when dbshelve opened with filetype=dbshelve.db.DB_RECNO'
        return
    ...
    def associate(self, secondaryDB, callback, flags=0):
        ...
        def _shelf_callback(priKey, priData, realCallback=callback):
            if sys.version_info[0] < 3 or isinstance(priData, bytes):
                ...
                data = cPickle.loads(bytes(priData, b'iso8859-1'))
...
class DBShelfCursor:
    ...
    def get(self, *args):
        count = len(args)
        method = getattr(self, b'get_%d' % count)
        method(*args)
        return
    ...
    def _extract(self, rec):
        if rec is None:
            ...
            return (key, cPickle.loads(bytes(data, b'iso8859-1')))
            return
