...
if sys.version_info[0] >= 3:
    ...
    w.__enter__()
    try:
        warnings.filterwarnings(b'ignore', message=b'the cPickle module has been removed in Python 3.0', category=DeprecationWarning)
        import cPickle as pickle
    finally:
        w.__exit__()
...
class LikeCond(Cond):
    ...
    def __init__(self, likestr, re_flags=re.IGNORECASE):
        ...
        for char in chars_to_escape:
            likestr = likestr.replace(char, b'\\' + char)

        self.likestr = likestr.replace(b'%', b'.*')
        self.re = re.compile(b'^' + self.likestr + b'$', re_flags)
        return
...
_table_names_key = b'__TABLE_NAMES__'
_columns = b'._COLUMNS__'
...
class bsdTableDB:
    ...
    def __init__(self, filename, dbhome, create=0, truncate=0, mode=384, recover=0, dbflags=0):
        ...
        if sys.version_info[0] >= 3:
            ...
            class cursor_py3k(object):
                ...
                def set_range(self, search):
                    v = self._dbcursor.set_range(bytes(search, b'iso8859-1'))
                    if v is not None:
                        v = (
                         v[0].decode(b'iso8859-1'),
                         v[1].decode(b'iso8859-1'))
                    return v

                def __next__(self):
                    v = getattr(self._dbcursor, b'next')()
                    if v is not None:
                        v = (
                         v[0].decode(b'iso8859-1'),
                         v[1].decode(b'iso8859-1'))
            ...
            class db_py3k(object):
                ...
                def has_key(self, key, txn=None):
                    return getattr(self._db, b'has_key')(bytes(key, b'iso8859-1'), txn=txn)

                def put(self, key, value, flags=0, txn=None):
                    key = bytes(key, b'iso8859-1')
                    if value is not None:
                        value = bytes(value, b'iso8859-1')
        ...
        try:
            if not getattr(self.db, b'has_key')(_table_names_key, txn):
                getattr(self.db, b'put_bytes', self.db.put)(_table_names_key, pickle.dumps([], 1), txn=txn)
        except:
            txn.abort()
            raise
        else:
    ...
    def _db_print(self):
        print b'******** Printing raw database for debugging ********'
        cur = self.db.cursor()
        try:
            key, data = cur.first()
            while 1:
    ...
    def CreateTable(self, table, columns):
        ...
        try:
            if contains_metastrings(table):
                raise ValueError(b'bad table name: contains reserved metastrings')
            for column in columns:
                if contains_metastrings(column):
                    raise ValueError(b'bad column name: contains reserved metastrings')

            columnlist_key = _columns_key(table)
            if getattr(self.db, b'has_key')(columnlist_key):
                raise TableAlreadyExists, b'table already exists'
            txn = self.env.txn_begin()
            getattr(self.db, b'put_bytes', self.db.put)(columnlist_key, pickle.dumps(columns, 1), txn=txn)
            tablelist = pickle.loads(getattr(self.db, b'get_bytes', self.db.get)(_table_names_key, txn=txn, flags=db.DB_RMW))
            tablelist.append(table)
            self.db.delete(_table_names_key, txn=txn)
            getattr(self.db, b'put_bytes', self.db.put)(_table_names_key, pickle.dumps(tablelist, 1), txn=txn)
            txn.commit()
            txn = None
    ...
    def ListTableColumns(self, table):
        if contains_metastrings(table):
            raise ValueError, b'bad table name: contains reserved metastrings'
        columnlist_key = _columns_key(table)
        if not getattr(self.db, b'has_key')(columnlist_key):
            return []
        else:
            pickledcolumnlist = getattr(self.db, b'get_bytes', self.db.get)(columnlist_key)
    ...
    def CreateOrExtendTable(self, table, columns):
        try:
            ...
            try:
                columnlist_key = _columns_key(table)
                txn = self.env.txn_begin()
                oldcolumnlist = pickle.loads(getattr(self.db, b'get_bytes', self.db.get)(columnlist_key, txn=txn, flags=db.DB_RMW))
                oldcolumnhash = {}
                ...
                if newcolumnlist != oldcolumnlist:
                    ...
                    getattr(self.db, b'put_bytes', self.db.put)(columnlist_key, pickle.dumps(newcolumnlist, 1), txn=txn)
                txn.commit()
                txn = None
    ...
    def __load_column_info(self, table):
        try:
            tcolpickles = getattr(self.db, b'get_bytes', self.db.get)(_columns_key(table))
        except db.DBNotFoundError:
            raise TableDBError, b'unknown table: %r' % (table,)
    ...
    def __new_rowid(self, table, txn):
        ...
        while not unique:
            ...
            newid = struct.pack((b'B' * _rowid_str_len), *blist)
            if sys.version_info[0] >= 3:
                newid = newid.decode(b'iso8859-1')
            try:
    ...
    def Insert(self, table, rowdict):
        txn = None
        try:
            if not getattr(self.db, b'has_key')(_columns_key(table)):
                raise TableDBError, b'unknown table'
            if table not in self.__tablecolumns:
                self.__load_column_info(table)
            for column in rowdict.keys():
                if not self.__tablecolumns[table].count(column):
                    raise TableDBError, b'unknown column: %r' % (column,)
