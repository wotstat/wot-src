...
def urlopen(url, data=None, timeout=socket._GLOBAL_DEFAULT_TIMEOUT, cafile=None, capath=None, cadefault=False, context=None):
    ...
    if cafile or capath or cadefault:
        if context is not None:
            raise ValueError(b"You can't pass both context and any of cafile, capath, and cadefault")
        if not _have_ssl:
...
class Request:
    ...
    def __getattr__(self, attr):
        if attr in (b'_Request__r_type', b'_Request__r_host'):
            getattr(self, b'get_' + attr[12:])()
    ...
    def get_full_url(self):
        if self.__fragment:
            return b'%s#%s' % (self.__original, self.__fragment)
        else:
    ...
    def get_type(self):
        if self.type is None:
            ...
            if self.type is None:
                raise ValueError, b'unknown url type: %s' % self.__original
        return self.type
...
class OpenerDirector:
    ...
    def add_handler(self, handler):
        ...
        for meth in dir(handler):
            if meth in (b'redirect_request', b'do_open', b'proxy_open'):
                continue
            i = meth.find(b'_')
            protocol = meth[:i]
            condition = meth[i + 1:]
...
class HTTPRedirectHandler(BaseHandler):
    ...
    def redirect_request(self, req, fp, code, msg, headers, newurl):
        m = req.get_method()
        if code in (301, 302, 303, 307) and m in (b'GET', b'HEAD') or code in (301, 302, 303) and m == b'POST':
            newurl = newurl.replace(b' ', b'%20')
            newheaders = dict((k, v) for k, v in req.headers.items() if k.lower() not in (b'content-length', b'content-type'))
    ...
    def http_error_302(self, req, fp, code, msg, headers):
        if b'location' in headers:
            newurl = headers.getheaders(b'location')[0]
        elif b'uri' in headers:
            newurl = headers.getheaders(b'uri')[0]
        else:
            return
        ...
        if not (newurl_lower.startswith(b'http://') or newurl_lower.startswith(b'https://') or newurl_lower.startswith(b'ftp://')):
            raise HTTPError(newurl, code, msg + b" - Redirection to url '%s' is not allowed" % newurl, headers, fp)
...
def _parse_proxy(proxy):
    ...
    if not r_scheme.startswith(b'/'):
        scheme = None
        authority = proxy
    elif not r_scheme.startswith(b'//'):
...
def randombytes(n):
    if os.path.exists(b'/dev/urandom'):
        f = open(b'/dev/urandom')
        s = f.read(n)
        f.close()
        return s
    else:
...
class AbstractDigestAuthHandler:
    ...
    def http_error_auth_reqed(self, auth_header, host, req, headers):
        ...
        if self.retried > 5:
            raise HTTPError(req.get_full_url(), 401, b'digest auth failed', headers, None)
        else:
            self.retried += 1
        if authreq:
    ...
    def get_cnonce(self, nonce):
        dig = hashlib.sha1(b'%s:%s:%s:%s' % (self.nonce_count, nonce, time.ctime(),
    ...
    def get_algorithm_impls(self, algorithm):
        ...
        if algorithm == b'MD5':
            ...
            raise ValueError(b'Unsupported digest authentication algorithm %r' % algorithm.lower())
    ...
    def do_request_(self, request):
        ...
        if request.has_data():
            ...
            if not request.has_header(b'Content-length'):
                request.add_unredirected_header(b'Content-length', b'%d' % len(data))
        sel_host = host
...
class FTPHandler(BaseHandler):
    ...
    def ftp_open(self, req):
        ...
        try:
            ...
            raise URLError, b'ftp error: %s' % msg, sys.exc_info()[2]
...
class CacheFTPHandler(FTPHandler):
    ...
    def connect_ftp(self, user, passwd, host, port, dirs, timeout):
        key = (user, host, port, (b'/').join(dirs), timeout)
