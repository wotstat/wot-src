    ...
    class DebugPlanLoader(object):
        ...
        def regContext(self, context):
            for ctx in self.__contextAll:
                ...
                LOG_DEBUG_DEV(b'VSContext %s was registered' % type(context).__name__)
        ...
        def unregContext(self, context):
            for ctx in self.__contextAll[:]:
                if ctx() == context:
                    ...
                    LOG_DEBUG_DEV(b'VSContext %s was unregistered' % type(context).__name__)
                    break
