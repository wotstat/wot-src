...
class WeightSequence(Block, Example):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._weightSlots = [self._makeDataInputSlot(b'w' + str(i), SLOT_TYPE.FLOAT) for i in range(n)]
        self._outSlots = [self._makeEventOutputSlot(b'out' + str(i)) for i in range(n)]
        return
...
class TestOnResult(Block, Example):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._out = self._makeEventOutputSlot(b'out')
        self._onResult = self._makeEventOutputSlot(b'onResult')
        self._onResult.setEditorData(EDITOR_CUSTOM_ICON.EVENT_DELAYED)
        self._sqr = self._makeDataOutputSlot(b'sqr', SLOT_TYPE.INT, None)
        return
