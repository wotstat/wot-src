...
_CONFIG_FILE = b'scripts/item_defs/player_ranks.xml'
if typing.TYPE_CHECKING:
    from ResMgr import DataSection

@unique
class ALGORITHM_NAMES(Enum):
    BASIC_THRESHOLD = b'BasicThreshold'
    NONE = b'None'
...
class BasicThresholdSettings(AlgorithmSettings):
    _BasicThresholdRankSettings = namedtuple(b'_BasicThresholdRankSettings', (b'rank', b'order', b'threshold'))

    def _readBasicThresholdRankSettings(self, section, order):
        rank = parseRankFromSectionName(section.name)
        return self._BasicThresholdRankSettings(rank, order, section.readInt(b'threshold'))
...
BonusSettings = NamedTuple(b'BonusSettings', [(b'factor100ByRank', dict)])
...
PlayerRanksSettings = NamedTuple(b'PlayerRankSettings', [
 (
  b'initialRank', int), (b'algorithm', Type[AlgorithmSettings]), (b'bonus', BonusSettings)])

def makePlayerRankSettings(section=None):
    initialRank = section.readInt(b'initialRank') if section else PLAYER_RANK.NO_RANK
    bonus = makeBonusSettings(section[b'bonus'] if section else None)
    if section is None:
