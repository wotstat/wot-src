...
class ActionItem(SimpleItem):
    __slots__ = (b'name', b'actionType', b'locName', b'imgName', b'tooltipSection')
    ...
    def readFromXML(self, xmlCtx, section, *args):
        ...
        if IS_CLIENT or IS_WEB:
            ...
            self.tooltipSection = _xml.readStringWithDefaultValue(xmlCtx, section, b'tooltipSection', TTC_TOOLTIP_SECTIONS.EQUIPMENT).split()
        return
...
class Modification(ActionItem):
    ...
    def readFromXML(self, xmlCtx, section, *args):
        ...
        xmlCtx = (xmlCtx, section.name)
        self.modifiers = readModifiers(xmlCtx, section[b'modifiers'], (attributes_static_factory,), False)
...
class TreeStep(SimpleItem):
    __slots__ = (b'priceTag', b'action', b'unlocks', b'requiredUnlocks', b'vehicleFilter', b'level')
        ...
        for stepID, step in self.steps.iteritems():
            for unlockID in step.unlocks:
                if self.steps[unlockID].level < step.level:
                    _xml.raiseWrongXml(xmlCtx, None, (b'Invalid step level for step: {}').format(stepID))
