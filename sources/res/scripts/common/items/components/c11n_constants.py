...
SLOT_DEFAULT_ALLOWED_MODEL = b'default'
...
class ItemTags(object):
    VEHICLE_BOUND = b'vehicleBound'
    PREMIUM_IGR = b'premiumIGR'
    IGR = b'IGR'
    RARE = b'rare'
    HIDDEN_IN_UI = b'hiddenInUI'
    DIM = b'dim'
    FULL_RGB = b'fullRGB'
    NATIONAL_EMBLEM = b'nationalEmblem'
    STYLE_ONLY = b'styleOnly'
    PROGRESSION_REQUIRED = b'progression_required'
    ADD_NATIONAL_EMBLEM = b'addNationalEmblem'
    CLAN_HIDDEN = b'clanHidden'
    DISABLE_VERTICAL_MIRROR = b'disableVerticalMirror'
    STYLE_PROGRESSION = b'styleProgression'
    QUESTS_PROGRESSION = b'questsProgression'
    PROGRESSION_REWIND_ENABLED = b'progression_rewind_enabled'
    ONLY_VERTICAL_MIRROR = b'onlyVerticalMirror'
    HIDE_IF_INCOMPATIBLE = b'hideIfIncompatible'
    STYLE_SERIAL_NUMBER = b'styleSerialNumber'
    LOCKED_ON_VEHICLE = b'lockedOnVehicle'
    STYLE_STAT_TRACK_FRAGS = b'styleStatTrackFrags'
...
class ProjectionDecalDirectionTags(object):
    PREFIX = b'direction_'
    ANY = PREFIX + b'any'
    LEFT_TO_RIGHT = PREFIX + b'left_to_right'
    RIGHT_TO_LEFT = PREFIX + b'right_to_left'
    LEFT = b'left'
    RIGHT = b'right'
    FRONT = b'front'
    ALL = (
...
class ProjectionDecalFormTags(object):
    PREFIX = b'formfactor_'
    SQUARE = PREFIX + b'square'
    RECT1X2 = PREFIX + b'rect1x2'
    RECT1X3 = PREFIX + b'rect1x3'
    RECT1X4 = PREFIX + b'rect1x4'
    RECT1X6 = PREFIX + b'rect1x6'
    ALL = (
...
class ApplyArea(object):
    ...
    def getAppliedCount(appliedTo, appliedRange=RANGE_REGIONS_VALUE):
        return bin(appliedTo & appliedRange).count(b'1')
...
class SeasonType(object):
    ...
    def fromArenaKind(arenaKind):
        ...
        raise SoftException(b'unknown arenaKind', arenaKind)
        return


SeasonTypeNames = {getattr(SeasonType, k): k for k in dir(SeasonType) if not k.startswith(b'_') and isinstance(getattr(SeasonType, k), int)}
...
DecalTypeNames = {getattr(DecalType, k): k for k in dir(DecalType) if not k.startswith(b'_')}
DecalGroups = {(DecalType.EMBLEM): [
                      52, 
                      53, 
                      54, 
                      55, 
                      ...
                      57, 
                      58], 
   (DecalType.INSCRIPTION): [
                           b'special',
                           b'clan_content',
                           b'other',
                           b'historical']}
ProjectionDecalGroups = [
 57, 
 61, 
 55, 
 62, 
 ...
 52]
ColorGroupsMap = (
 (b'national', b'color_group_02'),
 (b'base', b'color_group_03'),
 (b'special', b'color_group_04'),
 (b'unhistoric', b'color_group_05'))
StylesGroups = [
 b'special_styles',
 b'rented_styles',
 b'historical_styles',
...
C11N_GUN_REGION = 0
C11N_MASK_REGION = 2
C11N_GUN_APPLY_REGIONS = {b'GUN': C11N_GUN_REGION, 
   b'GUN_2': C11N_MASK_REGION}
CUSTOMIZATION_SLOTS_VEHICLE_PARTS = (b'hull', b'chassis', b'turret', b'gun')
UNBOUND_VEH_KEY = 0
...
CamouflageTilingType.RANGE = tuple([getattr(CamouflageTilingType, k) for k in dir(CamouflageTilingType) if not k.startswith(b'_') and k not in (b'RANGE', b'NONE')])
CamouflageTilingTypeNames = {getattr(CamouflageTilingType, k): k for k in dir(CamouflageTilingType) if not k.startswith(b'_') and k not in (b'RANGE', b'NONE')}
...
class SLOT_TYPE_NAMES(object):
    ...
    FIXED_PROJECTION_DECAL = b'fixedProjectionDecal'
...
class EDITING_STYLE_REASONS(object):
    IS_EDITABLE = b'isEditable'
    NOT_EDITABLE = b'notEditable'
    NOT_REACHED_LEVEL = b'notReachedLevel'
    NOT_HAVE_ANY_PROGRESSIVE_DECALS = b'notHaveAnyProgressiveDecals'
    ENABLED = (
     IS_EDITABLE,)
