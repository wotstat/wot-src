...
class SupplySlot(CategoriesHolder):
    ...
    def initSlot(slotSection):
        slotClass = slotSection.readString(b'slotClass')
        if slotClass:
            slotObj = importClass(slotClass, b'items.components.supply_slots_components')()
        else:
...
class SupplySlotsCache(object):
    ...
    def getSlotDescrsByTags(self, itags=(), etags=()):
        ...
        if not itags.isdisjoint(etags):
            return {}
        return {i: sd for i, sd in viewitems(self.__slotDescrs) if sd.tags.isdisjoint(etags) and bool(itags) ^ sd.tags.isdisjoint(itags)}
