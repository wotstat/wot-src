...
class SlotCategories(object):
    ...
    ALL = frozenset([FIREPOWER, SURVIVABILITY, MOBILITY, STEALTH, TACTICS, FIRESUPPORT, RECONNAISSANCE, 
     UNIVERSAL])


class CategoriesHolder(object):
    __slots__ = (b'categories',)
...
class LevelsFactor(object):
    ...
    def __str__(self):
        return (b'{}: (opType={}, values={})').format(self.__class__.__name__, self.opType, self.values)
    ...
    def applyLevelToAttrsDict(self, level, attrsDict, attrName):
        if self.opType is None:
            LOG_CODEPOINT_WARNING(b'Should not apply factors with no opType!')
            return
        else:
    ...
    def readLevelsFactor(xmlCtx, factorSection):
        attrName = _xml.readString(xmlCtx, factorSection, b'attribute')
        opTypeName = _xml.readString(xmlCtx, factorSection, b'type')
        if opTypeName not in AttrsOperation.ALL:
            raise SoftException((b'Unknown opTypeName ({}) for LevelsFactor').format(opTypeName))
        values = LevelsFactor._readFactorValues(xmlCtx, factorSection, b'valueByLevel')
