...
def checkItemInCompDescr(item, customizationElementCompDescr):
    ...
    if itemType == CustomizationType.STYLE:
        ...
        path = (
         (b'{}s').format(lower(CustomizationTypeNames[itemType])), (b'id', None))
...
def parseOutfitDescr(outfitDescr):
    ...
    try:
        ...
        LOG_ERROR(b'Bad outfit descr', base64.b64encode(outfitDescr))
...
def getBootcampOutfit(vehDescr, customizationConfig):
    ...
    styleId = tankCustomization.get(b'styleId', 0)
...
class OutfitLogEntry(object):
    ...
    def toDict(self):
        return {k: v for k, v in self.__dict__.iteritems() if not k.startswith(b'_')}
