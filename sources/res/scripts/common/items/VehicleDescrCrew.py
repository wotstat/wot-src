...
class VehicleDescrCrew(object):
    ...
    def callSkillProcessor(self, skillName, *args):
        try:
            ...
            LOG_ERROR(b'Failed to process skill (arenaUniqueID, vehicleID, skillName, skillData):', self.__getUniqueArenaID(), self.__getVehicleID(), skillName, self._skills.get(skillName) or self._extendedSkills.get(skillName), stack=True)
    ...
    def onCollectFactors(self, factors):
        ...
        if self._levelIncreaseByVehicle != newLevelIncreaseByVehicle:
            ...
            if hasattr(self, b'_vehicle') and self._perksLevelIncrease is None:
    ...
    def _calculateSkillEfficiencies(self):
        ...
        for skillName in tankmen.ROLES:
            if isFire:
                ...
                baseSummLevel, summLevel, numInactive = computeSummSkillLevel({b'crew': skillData}, isAbility=False)
        ...
        otherSkills = {}
        for skillName in (b'repair', b'fireFighting', b'camouflage'):
        ...
        for skillName, skillData in otherSkills.iteritems():
            if skillData is None or isFire and skillName != b'fireFighting':
                efficiency = 0.0
                baseAvgLevel = 0.0
            else:
                ...
                baseSummLevel, summLevel, numInactive = computeSummSkillLevel({b'crew': skillData} if perkData is None else perkData.args.skillData)
    ...
    def _calculateUniversalistAddition(self, commonLevelIncrease):
        ...
        if numInactive:
            skillData = self._skills.get(b'commander_universalist')
            if skillData is not None:
                level = skillData[0][1]
                universalistAddition = (level + commonLevelIncrease) / numInactive
                universalistAddition *= tankmen.getSkillsConfig().getSkill(b'commander_universalist').efficiency
        return universalistAddition
    ...
    def _processSkills(self, skillEfficiencies):
        ...
        for skillName, skillData in skills.iteritems():
            ...
            perkID = skillConfig.vsePerk
            perkData = self._perks.get(perkID)
            ccs = self._findBestTankmanForSkill({b'crew': skillData} if perkData is None else perkData.args.skillData)
    ...
    def _updateFireFightingFactors(self, a):
        self._factors[b'healthBurnPerSecLossFraction'] = a.factor / 0.57
        if _DO_DEBUG_LOG:
            LOG_DEBUG(b"Factor/baseAvgLevel of skill '%s': (%s, %s)" % (
             b'fireFighting', a.factor, a.baseAvgLevel))
        return
    ...
    def _process_commander_eagleEye(self, a):
        self._process_perk(a, b'circularVisionRadius', b'circularVisionRadiusB')
        return
    ...
    def _findBestTankmanForSkill(self, skillData, isAbility=True):
        ...
        ccs.hasActiveTankmanForBooster = any(self._activityFlags[idxInCrew] for idxInCrew in skillData.get(b'b_crew', ()))
        ccs.boosterMultiplier = skillData.get(b'booster')
        return ccs
        ...
        for compactDescr, roles in zip(crewCompactDescrs, crewRoles):
            ...
            for skillName, level in descr.skillLevels:
                ...
                if vsePerk is not None:
                    perkData = perks.setdefault(vsePerk, PerkData(0, CrewContextArgs({b'crew': []})))
                    perkData.args.skillData[b'crew'].append((idxInCrew, levelOnVehicle))
    ...
    def _computeSummSkillLevel(self, skillData, isAbility=True):
        ...
        commanderIdx = self._commanderIdx
        for idx, level in skillData.get(b'crew', []):
    ...
    def _rebuildSkillProcessors(cls):
        return {b'commander': (cls._updateCommanderFactors), 
           b'radioman': (cls._updateRadiomanFactors), 
           b'driver': (cls._updateDriverFactors), 
           b'gunner': (cls._updateGunnerFactors), 
           b'loader': (cls._updateLoaderFactors), 
           b'repair': (cls._updateRepairFactors), 
           b'fireFighting': (cls._updateFireFightingFactors), 
           b'camouflage': (cls._updateCamouflageFactors), 
           b'commander_universalist': None, 
           b'commander_tutor': None, 
           b'commander_expert': None, 
           b'commander_sixthSense': None, 
           b'commander_eagleEye': (cls._process_commander_eagleEye), 
           b'commander_enemyShotPredictor': None, 
           b'driver_tidyPerson': None, 
           ...
           b'gunner_gunsmith': None, 
           b'gunner_sniper': None, 
           b'gunner_rancorous': None, 
           b'loader_pedant': None, 
           b'loader_desperado': None, 
           b'loader_intuition': None, 
           b'radioman_finder': (cls._process_radioman_finder), 
           b'radioman_inventor': None, 
           b'radioman_lastEffort': None, 
           b'radioman_retransmitter': None}
