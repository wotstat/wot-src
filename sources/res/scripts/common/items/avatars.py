from extension_utils import ResMgr
from items import _xml, common_extras
from constants import IS_CLIENT, IS_CELLAPP, ITEM_DEFS_PATH
g_cache = None

def init():
    global g_cache
    g_cache = Cache()
    return


class Cache(object):

    def __init__(self):
        self.__commonConfig = None
        return

    @property
    def commonConfig(self):
        config = self.__commonConfig
        if config is None:
            configXmlPath = ITEM_DEFS_PATH + b'avatar.xml'
            configXml = ResMgr.openSection(configXmlPath)
            if configXml is None:
                _xml.raiseWrongXml(None, configXmlPath, b'can not open or read')
            if IS_CLIENT or IS_CELLAPP:
                extras, extrasDict = common_extras.readExtras((None, configXmlPath), configXml, b'extras', b'avatar_extras')
                config = self.__commonConfig = {b'extras': extras, b'extrasDict': extrasDict}
            configXml = None
            ResMgr.purge(configXmlPath, True)
        return config
