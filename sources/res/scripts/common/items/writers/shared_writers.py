...
def writeAttachmentSlots(slotDS, slot):
    ...
    slotDS.writeVector3(b'scale', slot.scale)
    _xml.rewriteBool(slotDS, b'hiddenForUser', slot.hiddenForUser, False)
    _xml.rewriteBool(slotDS, b'enableVisTunnel', slot.enableVisTunnel, False)
    _xml.rewriteString(slotDS, b'applyType', slot.applyType, b'')
    _xml.rewriteString(slotDS, b'size', slot.size, b'')
    _xml.rewriteInt(slotDS, b'hangerId', slot.hangerId, 0)
    _xml.rewriteVector3(slotDS, b'hangerRotation', slot.hangerRotation, (0, 0, 0))
    if len(slot.compatibleModels) == 1 and slot.compatibleModels[0] == b'default':
        slotDS.deleteSection(b'compatibleModels')
    else:
        slotDS.write(b'compatibleModels', (b' ').join(slot.compatibleModels))
...
def writeEmblemSlots(slotDS, slot):
    if slot.type not in (b'attachment', b'sequence', b'paint', b'camouflage', b'style', b'effect', b'projectionDecal', b'fixedProjectionDecal'):
        _xml.rewriteBool(slotDS, b'isMirrored', slot.isMirrored, slot.type in (b'insignia', b'insigniaOnGun'))
    ...
    if slot.type in (b'insigniaOnGun', b'clan'):
...
def writeSlotPrefabs(slotPrefabs, section):
    sectIndex = section.getFirstIndex(b'slotPrefabs')
    section.deleteSection(b'slotPrefabs')
    if not slotPrefabs:
        return
    prefabsSection = section.insertSection(b'slotPrefabs', sectIndex)
...
def writeObjectSlots(objectSlots, section):
    sectIndex = section.getFirstIndex(b'objectSlots')
    section.deleteSection(b'objectSlots')
    if not objectSlots:
        return
    slotsSection = section.insertSection(b'objectSlots', sectIndex)
    for slot in objectSlots:
        slotSection = slotsSection.createSection(b'slot')
        slotSection.write(b'name', slot.name)
        slotSection.write(b'type', slot.type)
        slotSection.write(b'position', slot.position)
        slotSection.write(b'rotation', slot.rotation)
