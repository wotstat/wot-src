...
class BaseCustomizationItemXmlReader(object):
    ...
    def _readFromXml(self, target, xmlCtx, section, cache=None):
        ...
        if section.has_key(b'priceGroup'):
            target.priceGroup = intern(section.readString(b'priceGroup'))
    ...
    def _readClientOnlyFromXml(self, target, xmlCtx, section, cache=None):
        if IS_EDITOR:
            target.i18n = I18nExposedComponentMeta(section.readString(b'name'), section.readString(b'userString'), section.readString(b'description'), section.readString(b'longDescriptionSpecial'))
        else:
            target.i18n = shared_components.I18nExposedComponent(section.readString(b'userString'), section.readString(b'description'), section.readString(b'longDescriptionSpecial'))
        return
    ...
    def _parseGroup(userString):
        parts = userString.split(b'/')
        if len(parts) != 2:
            return b''
        if not parts[0].startswith(b'#vehicle_customization:'):
            return b''
        return parts[1]
...
class PaintXmlReader(BaseCustomizationItemXmlReader):
    ...
    def _readFromXml(self, target, xmlCtx, section, cache=None):
        ...
        if section.has_key(b'usages'):
            xmlSubCtx = (
             xmlCtx, b'usages')
            for name, sub in ix.getChildren(xmlCtx, section, b'usages'):
                ctype, cost = self._readUsage(xmlSubCtx, sub)
                for i in ApplyArea.RANGE:
                    if ctype & i:
                        target.usageCosts[i] = cost

        if section.has_key(b'userString') and section.name == b'itemGroup':
            target.colorGroup = self._parseGroup(ix.readString(xmlCtx, section, b'userString'))
        return
...
class DecalXmlReader(BaseCustomizationItemXmlReader):
    ...
    def _readFromXml(self, target, xmlCtx, section, cache=None):
        ...
        if section.name == b'itemGroup' and section.has_key(b'userString'):
            target.decalGroup = self._parseGroup(ix.readString(xmlCtx, section, b'userString'))
...
class ProjectionDecalXmlReader(BaseCustomizationItemXmlReader):
    ...
    def _readFromXml(self, target, xmlCtx, section, cache=None):
        ...
        target.emissionSettings = {b'emissionMap': (section.readString(b'emissionMap', b'')), 
           b'emissionPatternMap': (section.readString(b'emissionPatternMap', b'')), 
           b'forwardEmissionBrightness': (section.readFloat(b'forwardEmissionBrightness', DEFAULT_FORWARD_EMISSION)), 
           b'deferredEmissionBrightness': (section.readFloat(b'deferredEmissionBrightness', DEFAULT_DEFERRED_EMISSION)), 
           b'emissionAnimationSpeed': (section.readFloat(b'emissionAnimationSpeed', DEFAULT_EMISSION_ANIMATION_SPEED))}
        if section.name == b'itemGroup' and section.has_key(b'userString'):
            target.projectionDecalGroup = self._parseGroup(ix.readString(xmlCtx, section, b'userString'))
        return
...
class AttachmentXmlReader(BaseCustomizationItemXmlReader):
    ...
    def _readClientOnlyFromXml(self, target, xmlCtx, section, cache=None):
        ...
        target.sequenceId = ix.readIntOrNone(xmlCtx, section, b'sequenceId')
        target.attachmentLogic = ix.readStringOrNone(xmlCtx, section, b'attachmentLogic')
        target.initialVisibility = ix.readBool(xmlCtx, section, b'initialVisibility', True)
        if IS_EDITOR:
            target.name = ix.readStringOrNone(xmlCtx, section, b'name')
        return
...
class CamouflageXmlReader(BaseCustomizationItemXmlReader):
    ...
    def _readFromXml(self, target, xmlCtx, section, cache=None):
        ...
        target.exclusionImpact = section.readFloat(b'exclusionImpact', 1.0)
        target.forceUv0 = section.readBool(b'forceUv0', False)
        ...
        target.emissionSettings = {b'emissionMap': (section.readString(b'emissionMap', b'')), 
           ...
           b'deferredEmissionBrightness': (section.readFloat(b'deferredEmissionBrightness', DEFAULT_DEFERRED_EMISSION)), 
           b'emissionAnimationSpeed': (section.readFloat(b'emissionAnimationSpeed', DEFAULT_EMISSION_ANIMATION_SPEED))}
        target.normalSettings = {b'normalMap': (section.readString(b'normalMap', b'')), 
           b'normalMapFactor': (section.readFloat(b'normalMapFactor', DEFAULT_NORMAL_MAP_FACTOR)), 
           b'normalMaxLod': (section.readInt(b'normalMaxLod', DEFAULT_NORMAL_MAX_LOD))}
        if IS_EDITOR:
            if target.camoTypeIndex == -1 and callable(getattr(target, b'setCamoType', None)):
                try:
...
class StyleXmlReader(BaseCustomizationItemXmlReader):
    ...
    def _readFromXml(self, target, xmlCtx, section, cache=None):
        ...
        if section.name == b'itemGroup' and section.has_key(b'userString'):
            target.styleGroup = self._parseGroup(ix.readString(xmlCtx, section, b'userString'))
        return
    ...
    def _readClientOnlyFromXml(self, target, xmlCtx, section, cache=None):
        ...
        if section.has_key(b'styleProgressions'):
            styleProgressions = {}
            for i, (spSectionName, spSection) in enumerate(section[b'styleProgressions'].items()):
                stageId = i + 1
...
class InsigniaXmlReader(BaseCustomizationItemXmlReader):
    ...
    def _readFromXml(self, target, xmlCtx, section, cache=None):
        ...
        if section.name == b'itemGroup' and section.has_key(b'userString'):
            target.insigniaGroup = self._parseGroup(ix.readString(xmlCtx, section, b'userString'))
        return
    ...
    def _readClientOnlyFromXml(self, target, xmlCtx, section, cache=None):
        ...
        if section.has_key(b'texture'):
            target.texture = section.readString(b'texture')
        target.canBeMirrored = section.readBool(b'canBeMirrored', False)
        target.emissionSettings = {b'emissionMap': (section.readString(b'emissionMap', b'')), 
           b'emissionPatternMap': (section.readString(b'emissionPatternMap', b'')), 
           b'forwardEmissionBrightness': (section.readFloat(b'forwardEmissionBrightness', DEFAULT_FORWARD_EMISSION)), 
           b'deferredEmissionBrightness': (section.readFloat(b'deferredEmissionBrightness', DEFAULT_DEFERRED_EMISSION)), 
...
def readCustomizationCacheFromXml(cache, folder):
    ...
    _readProhibitedNumbers((None, b'personal_numbers/prohibitedNumbers.xml'), ResMgr.openSection(pgFile))
    ResMgr.purge(pgFile)
...
def _validateStyles(cache):
    ...
    def customizationItemInOutfits(style, itemID, itemType):
        for season in SeasonType.RANGE:
            ...
            if outfit:
                customizationItems = getattr(outfit, (b'{}s').format(lower(CustomizationTypeNames[itemType])))
...
def __readProgressLevel(xmlCtx, section):
    ...
    for sectionName, subSections in section.items():
        if sectionName == b'price':
            ...
            conditions = level.setdefault(b'conditions', list())
            condition = {}
