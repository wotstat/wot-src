...
_HANGERS_PATH = ITEM_DEFS_PATH + b'/customization/attachments/hangers.xml'
_logger = logging.getLogger(__name__)

def _readEmblemSlot(ctx, subsection, slotType):
    descr = shared_components.EmblemSlot(_xml.readVector3(ctx, subsection, b'rayStart'), _xml.readVector3(ctx, subsection, b'rayEnd'), _xml.readVector3(ctx, subsection, b'rayUp'), _xml.readPositiveFloat(ctx, subsection, b'size'), subsection.readBool(b'hideIfDamaged', False), slotType, subsection.readBool(b'isMirrored', slotType in (b'insignia', b'insigniaOnGun')), subsection.readBool(b'isUVProportional', True), _xml.readIntOrNone(ctx, subsection, b'emblemId'), _xml.readInt(ctx, subsection, b'slotId'), subsection.readBool(b'applyToFabric', True), _readCompatibleModels(subsection, ctx))
...
def _readAttachmentSlot(ctx, subsection, slotType):
    descr = shared_components.AttachmentSlotDescription(slotType=slotType, slotId=_xml.readInt(ctx, subsection, b'slotId'), position=_xml.readVector3(ctx, subsection, b'position', c11n_constants.DEFAULT_POSITION), rotation=_xml.readVector3(ctx, subsection, b'rotation', c11n_constants.DEFAULT_ROTATION), scale=_xml.readVector3(ctx, subsection, b'scale', c11n_constants.DEFAULT_SCALE), attachNode=_xml.readStringOrNone(ctx, subsection, b'attachNode'), hiddenForUser=_xml.readBool(ctx, subsection, b'hiddenForUser', False), enableVisTunnel=_xml.readBool(ctx, subsection, b'enableVisTunnel', False), applyType=_xml.readStringOrEmpty(ctx, subsection, b'applyType'), size=_xml.readStringOrEmpty(ctx, subsection, b'size'), hangerId=_xml.readNonNegativeInt(ctx, subsection, b'hangerId', 0), hangerRotation=_xml.readVector3(ctx, subsection, b'hangerRotation', Math.Vector3(0, 0, 0)))
    _verifySlotId(ctx, slotType, descr.slotId)
...
def __getInitedSlotIdRanges():
    ...
    return __customizationConstants[b'slot_id_ranges']


def getAttachmentSlotSizes():
    _initCustomizationConstants()
    return __customizationConstants[b'attachment_slot_sizes']
...
def getAttachmentSlotScale(applyType, baseSize, selectedSize):
    ...
    baseScale = slotSizes[baseSize][b'size']
    selectedScale = slotSizes[selectedSize][b'size']
...
def getAttachmentHangingEffects():
    _initCustomizationConstants()
    return __customizationConstants[b'attachment_hanging_effects']


def getAttachmentHangingEffect(applyType, rarity):
    return getAttachmentHangingEffects().get(applyType, {}).get(rarity, b'')
...
def _initCustomizationConstants():
    ...
    if __customizationConstants is not None:
        ...
        __customizationConstants[b'slot_id_ranges'] = {}
        slots = _xml.getSubsection(xmlCtx, section, b'slot_id_ranges')
        for partName, _ in _xml.getChildren(xmlCtx, section, b'slot_id_ranges'):
            __customizationConstants[b'slot_id_ranges'][partName] = {}
            for itemName, item in _xml.getChildren(xmlCtx, slots, partName):
                ...
                __customizationConstants[b'slot_id_ranges'][partName][itemName] = (range_min, range_max)

        __customizationConstants[b'attachment_slot_sizes'] = {}
        attachmentSlotTypes = _xml.getSubsection(xmlCtx, section, b'attachment_slot_sizes')
        for typeName, _ in _xml.getChildren(xmlCtx, section, b'attachment_slot_sizes'):
            __customizationConstants[b'attachment_slot_sizes'][typeName] = {}
            for sizeName, item in _xml.getChildren(xmlCtx, attachmentSlotTypes, typeName):
                size = _xml.readVector3(xmlCtx, item, b'size')
                premountSize = _xml.readVector3(xmlCtx, item, b'premount_size')
                mountSize = _xml.readVector3(xmlCtx, item, b'mount_size')
                shift = _xml.readVector2(xmlCtx, item, b'shift')
                __customizationConstants[b'attachment_slot_sizes'][typeName][sizeName] = {b'size': size, b'premountSize': premountSize, 
                   b'mountSize': mountSize, 
                   b'shift': shift}

        __customizationConstants[b'attachment_hanging_effects'] = {}
        attachmentSlotTypes = _xml.getSubsection(xmlCtx, section, b'attachment_hanging_effects')
        for typeName, _ in _xml.getChildren(xmlCtx, section, b'attachment_hanging_effects'):
            __customizationConstants[b'attachment_hanging_effects'][typeName] = {}
            for rareName, value in _xml.getChildren(xmlCtx, attachmentSlotTypes, typeName):
                __customizationConstants[b'attachment_hanging_effects'][typeName][rareName] = value.asString
...
def _initHangers():
    ...
    if __hangers is not None:
        ...
        if section is None:
            _xml.raiseWrongXml(None, filePath, b'can not open or read')
        xmlCtx = (None, filePath)
        for _, item in _xml.getChildren(xmlCtx, section, b'hanger'):
            id = _xml.readInt(xmlCtx, item, b'id')
            modelName = _xml.readString(xmlCtx, item, b'modelName')
            crashModelName = _xml.readStringOrNone(xmlCtx, item, b'crashModelName')
            hanger = {b'modelName': modelName, b'crashModelName': crashModelName}
            __hangers[id] = hanger
...
def readFloatPair(xmlCtx, section, subsectionName):
    ...
    if count < 1 or count > 2:
        _xml.raiseWrongXml(xmlCtx, subsectionName, b'One or two elements are expected')
    if count == 1:
...
def readPrefabsSets(section, prefabsKeys):
    if section is None:
        ...
        prefabs = {b'default': {k: section.readStrings(k) for k in prefabsKeys}}
        for k, v in (section[b'sets'] or {}).items():
...
def readSlotPrefabs(section):
    slotPrefabsSection = section[b'slotPrefabs']
...
def readObjectSlots(xmlCtx, section):
    ...
    def readSlot(ctx, subsection):
        slotType = _xml.readString(ctx, subsection, b'type')
        if slotType not in component_constants.ObjectSlotType.ALL:
            _xml.raiseWrongXml(ctx, b'type', (b'Unsupported object slot type: {}').format(slotType))
        return shared_components.ObjectSlot(name=_xml.readString(ctx, subsection, b'name'), type=slotType, position=_xml.readVector3(ctx, subsection, b'position', component_constants.ZERO_VECTOR3), rotation=_xml.readVector3(ctx, subsection, b'rotation', component_constants.ZERO_VECTOR3))

    slotsCtx, slotsSection = _xml.getSubSectionWithContext(xmlCtx, section, b'objectSlots', throwIfMissing=False)
    if not slotsSection:
        if IS_UE_EDITOR:
            return []
        return component_constants.EMPTY_LIST
    return [readSlot(ctx, section) for _, (ctx, section) in _xml.getItemsWithContext(slotsCtx, slotsSection, b'slot')]
