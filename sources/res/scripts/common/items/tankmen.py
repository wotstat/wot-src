...
def init(preloadEverything, pricesToCollect):
    ...
    if preloadEverything:
        _g_skillsConfig = pdc.load(b'tankmen_skills_config', getSkillsConfig)
        _g_nationsConfig = pdc.load(b'tankmen_nations_config', _createNationsConfig)
...
ALL_SKILLS_MASK = getSkillsMask([s for s in LEARNABLE_ACTIVE_SKILLS if s != b'reserved'])
...
def generateCompactDescr(passport, vehicleTypeID, role, roleLevel=MAX_SKILL_LEVEL, skills=(), lastSkillLevel=MAX_SKILL_LEVEL, dossierCompactDescr=b'', freeSkills=(), initialXP=0, skillsEfficiencyXP=MAX_SKILLS_EFFICIENCY_XP, rolesBonusSkills=None):
    pack = struct.pack
    ...
    cd += tf.pack()
    fmt = b'>H' if tf.extendedVehicleTypeID else b'B'
    ...
    if tf.hasBonusSkills:
        ...
        cd += pack((str(1 + numBonusSkills) + b'B'), numBonusSkills, *bonusSkills)
    freeXP = 0
    cd += pack(b'>3HI', firstNameID, lastNameID, iconID, freeXP)
...
def stripNonBattle(compactDescr):
    ...
    if tf.hasBonusSkills:
        length, = struct.unpack(b'B', compactDescr[l])
        l += length + 1
    l += 10
...
def parseNationSpecAndRole(compactDescr):
    ...
    if tf.extendedVehicleTypeID:
        vehicleTypeID, = struct.unpack(b'>H', cd[:2])
        cd = cd[2:]
    else:
        vehicleTypeID, = struct.unpack(b'B', cd[:1])
        cd = cd[1:]
...
class TankmanDescr(object):
    ...
    def getRoleBonusSkills(self, activeRoles):
        for role in activeRoles:
            if role in self.__rolesBonusSkills:
                ...
                for skill in skills + [b'any'] * additional:
                    yield (
                     role, skill)

            else:
                for skill in [b'any'] * NPS.MAX_BONUS_SKILLS_PER_ROLE:
                    yield (
                     role, skill)
    ...
    def selectedFreeSkills(self):
        return [skill for skill in self.freeSkills if skill != b'any']
    ...
    def selectedSkills(self):
        return [skill for skill in self._skills if skill != b'any']
    ...
    def newFreeSkillsCount(self):
        return self.freeSkills.count(b'any')
    ...
    def skillLevels(self, activeRoles):
        ...
        for skillName in self._skills:
            if skillName == b'any' or majorRole not in ROLES_BY_SKILLS[skillName]:
                continue
    ...
    def checkSkillsRestrictionsByVehicleTags(self):
        self.checkRestrictionsByVehicleTags(b'lockCrewSkills', b'Changing tankmans skills is forbidden for current vehicle.')
        return

    def checkCrewRestrictionsByVehicleTags(self):
        self.checkRestrictionsByVehicleTags(b'lockCrew', b'Crew cannot be changed on this tank.')
        return
    ...
    def validateSkill(self, skillName, utilizationType, battleOnly=False):
        ...
        if utilizationType == SkillUtilization.FREE_SKILL:
            if skillName not in COMMON_SKILLS and skillName not in SKILLS_BY_ROLES[self.role]:
                raise SoftException(b'Cannot learn free skill (%s) for this tankman' % skillName)
            if b'any' not in self.freeSkills:
                raise SoftException(b'no free skill slots available')
            if battleOnly:
                return
            if skillName in self.freeSkills:
                raise SoftException(b'free skill (%s) is already learned' % skillName)
        elif utilizationType == SkillUtilization.MAJOR_SKILL:
            if len(self.skills) >= NPS.MAX_MAJOR_PERKS:
                raise SoftException(b'Maximum available perks (%s)' % skillName, NPS.MAX_MAJOR_PERKS)
            if battleOnly:
                return
            ...
            if role is None:
                raise SoftException((b"Skill {} doesn't have a role").format(skillName))
            if skillName in COMMON_SKILLS:
                raise SoftException((b'Can\'t learn "{}" as bonus skill').format(skillName))
            numBonusSkills = self.selectedBonusSkillsCount(role)
            if numBonusSkills == NPS.MAX_BONUS_SKILLS_PER_ROLE:
                raise SoftException((b'Can\'t learn "{}" skill with index = {}').format(skillName, numBonusSkills))
            if battleOnly:
                return
            if skillName in self.__rolesBonusSkills[role]:
                raise SoftException((b'Bonus skill "{}" is already learned').format(skillName))
            ...
            if numBonusSkills > math.ceil(skillProgress / 2.0):
                raise SoftException((b'Tried to learn skill "{}" by index {}, while can have only {}').format(skillName, numBonusSkills, skillProgress))
        return
    ...
    def validateSkillEquipment(self, vehDescr, idxInCrew, equipment):
        if b'crewSkillBattleBooster' not in equipment.tags:
            return False
    ...
    def isFreeSkillCompletionRequiped(self, curUtilization):
        return self.freeSkills.count(b'any') and curUtilization != SkillUtilization.BONUS_SKILL
    ...
    def addSkill(self, skillName, utilizationType=SkillUtilization.MAJOR_SKILL, useFree=True):
        if useFree and self.isFreeSkillCompletionRequiped(utilizationType) and self.validateSkillUtilization(skillName, SkillUtilization.FREE_SKILL):
            ...
            self.replaceFreeSkill(b'any', skillName)
        else:
    ...
    def dropSkills(self, xpReuseFraction=MIN_XP_REUSE_FRACTION, throwIfNoChange=True, truncateXP=True):
        ...
        for skillName in self._skills:
            self.replaceFreeSkill(skillName, b'any')
    ...
    def dropIrrelevantSkills(self):
        ...
        for skill in self.irrelevantSkills:
            ...
            if skill in self.freeSkills:
                self.replaceFreeSkill(skill, b'any')
            else:
    ...
    def validatePassport(self, isPremium, isFemale, fnGroupID, firstNameID, lnGroupID, lastNameID, iGroupID, iconID):
        ...
        if lastNameID is not None:
            ...
            if lastNameID not in group.lastNames:
                return (False, b'Invalid last name', None)
        if iconID is not None:
            if iGroupID not in groups:
                return (False, b'Invalid i group', None)
            ...
            if iconID not in group.icons:
                return (False, b'Invalid icon id', None)
    ...
    def makeCompactDescr(self):
        ...
        cd += flags.pack()
        fmt = b'>H' if flags.extendedVehicleTypeID else b'B'
        ...
        if flags.hasBonusSkills:
            ...
            numBonusSkills = len(bonusSkills)
            cd += pack((str(1 + numBonusSkills) + b'B'), numBonusSkills, *bonusSkills)
        cd += pack(b'>3HI', self.firstNameID, self.lastNameID, self.iconID, self.__freeXP)
    ...
    def __initFromCompactDescr(self, compactDescr, battleOnly):
        ...
        try:
            ...
            if flags.extendedVehicleTypeID:
                self.vehicleTypeID, = unpack(b'>H', cd[:2])
                cd = cd[2:]
            else:
                self.vehicleTypeID, = unpack(b'B', cd[0])
                cd = cd[1:]
            self.__vehicleTags = vehicles.g_list.getList(nationID)[self.vehicleTypeID].tags
            roleID, xp1, xp2, numSkills = unpack(b'4B', cd[:4])
            ...
            if flags.hasBonusSkills:
                ...
                cd = cd[numSkills:]
            self.firstNameID, self.lastNameID, self.iconID, self.__freeXP = unpack(b'>3HI', cd[:10].ljust(10, b'\x00'))
            cd = cd[10:]
            self.__gid = None
    ...
    def isRestorable(self):
        ...
        res &= not (b'lockCrew' in self.__vehicleTags and b'unrecoverable' in self.__vehicleTags)
        return res
...
def makeTmanDescrByTmanData(tmanData):
    ...
    rolesBonusSkills = tmanData.get(b'bonusSkills', {})
    ...
    if not set(skills + freeSkills).isdisjoint(set(bonusSkills)):
        raise SoftException(b'Bonus skills and skills must be disjoint.')
    ...
    skillsEfficiencyXP = tmanData.get(b'skillsEfficiencyXP', MAX_SKILLS_EFFICIENCY_XP)
    ...
    if fnGroupID != lnGroupID and fnGroupID != iGroupID and lnGroupID != iGroupID:
        raise SoftException(b'Do not create tankmen with all three groups different because "def findGroupsByIDs" won\'t be able to clearly tell which group tankman belongs to. "Also see "def _readNationConfigSection" - it creates unordered dictionary of groups. On production exist some tankmen, which have 2 IDs from one group and 1 ID from other group. But please don\'t make any more of them.')
    groupID = findBestGroupByGroupIDs(fnGroupID, lnGroupID, iGroupID)
    group = groups[groupID]
    if role not in group.roles:
        raise SoftException((b'Tankman, with nation = "{}"; isPremium = "{}" and groupID = {}, can\'t have role "{}". Available roles = {}').format(nations.NAMES[nationID], isPremium, groupID, role, group.rolesList))
    passport = (nationID, isPremium, isFemale, firstNameID, lastNameID, iconID)
    tmanCompDescr = generateCompactDescr(passport, vehicleTypeID, role, skills=skills, lastSkillLevel=lastSkillLevel, freeSkills=freeSkills, initialXP=tmanData.get(b'freeXP', 0), skillsEfficiencyXP=skillsEfficiencyXP, rolesBonusSkills=rolesBonusSkills)
...
def getRecruitInfoFromToken(tokenName, **kwargs):
    skillIndices = kwargs.get(b'skillIndices', SKILL_INDICES)
    activeSkills = kwargs.get(b'activeSkills', skills_constants.ACTIVE_SKILLS)
    activeFreeSkills = kwargs.get(b'activeSkills', skills_constants.ACTIVE_FREE_SKILLS)
    ...
    if len(parts) != RECRUIT_TMAN_TOKEN_TOTAL_PARTS:
        ...
        try:
            ...
            for nation in result[b'nations']:
                if len([g for g in viewvalues(getNationGroups(nation, result[b'isPremium'])) if g.name == parts[3]]) != 1:
            ...
            if parts[7] != b'':
                ...
                chosenFreeSkills = [skill for skill in freeSkills if skill != b'any']
                ...
                for skill in freeSkills:
                    if skill != b'any' and skill not in activeFreeSkills:
            ...
            if parts[10] != b'':
                ...
                for role in roles:
                    ...
                    result[b'roles'].append(skillIndices[role])
    ...
    for slotID, tmanDescr in enumerate(crew):
        ...
        if tmanDescr.roleLevel < MAX_SKILL_LEVEL:
            if not resultMask & crew_books_constants.CREW_BOOK_PROPERTIES_MASKS.ROLE_LEVEL:
                resultMsg += b'One of crew members has not enough level of specialization; '
    ...
    for slotIdx, tankmanDescr in enumerate(tankmenDescrs):
        if tankmanDescr:
            if tankmanDescr.needXpForVeteran:
                ...
                sortingList.append((float(b'inf'), slotIdx))
