...
def getFinalRetrainCost(tmanDescr, cost):
    discountMult = 1.0
    if tmanDescr:
        discountMult = cost[b'discounts'].get((b'perk_{}').format(tmanDescr.getFullSkillsCount()), 1.0)
    return (cost[b'credits'] * discountMult, cost[b'gold'] * discountMult)
...
def parseDuration(timeStr):
    ...
    if timeStr == b'0':
        return 0
    negative = timeStr[0] == b'-'
    if negative:
        timeStr = timeStr[1:]
    parts = timeStr.split(b' ')
    duration = 0
    for part in parts:
...
def castNumberToPrettyStr(value):
    if isinstance(value, float):
        return str(value).rstrip(b'0').rstrip(b'.')
...
def encodeSegment(bbox, componentIndex, startPoint, endPoint):
    ...
    if test is None:
        LOG_WARNING(b'Cannot encode segment', startPoint, endPoint, bbox)
...
def reprSlots(self):
    return b'%s(%s)' % (
     ...
     (b', ').join(b'%s=%s' % (name, getattr(self, name, b'NOT_FOUND')) for name in self.__slots__))
...
def setDamageSticker(sticker, effectsIndex, prefabEffIndex, isPierced):
    ...
    hitType = HitParamsEncoder.getHitType(sticker[b'params'])
    if not isPierced and hitType in VEHICLE_HIT_EFFECT.PIERCED_HITS:
        hitType = VEHICLE_HIT_EFFECT.ARMOR_NOT_PIERCED
        sticker[b'params'] = HitParamsEncoder.setHitType(sticker[b'params'], hitType)
    stickerTypeName = b'armorPierced' if isPierced else b'armorResisted'
    stickerID = cache.shotEffects[effectsIndex][b'targetStickers'][stickerTypeName]
    if stickerID is None:
        return
    else:
        priority = cache.damageStickers[b'descrs'][stickerID][b'priority']
        sticker[b'segment'] = setEncodedSegmentContextData(sticker[b'segment'], stickerID)
        sticker[b'params'] = HitParamsEncoder.setPrefabEffIndex(sticker[b'params'], prefabEffIndex)
        return priority
