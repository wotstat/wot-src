...
def groupComponent(**specs):
    ...
    def decorator(componentCls):
        ...
        for base in componentCls.__bases__:
            baseReader = getattr(base, b'groupComponentReader', None)
...
class DynComponentsGroupsRepo(object):
    _CONFIG = b'scripts/item_defs/dyn_components_groups.xml'
    _DOMAIN = b'server' if IS_CELLAPP else b'client'
...
class DynComponentsGroupsReader(object):
    ...
    def __init__(self, configPath, domain=b'', hotReload=False):
    ...
    def getGroup(self, groupName):
        ...
        if groupName not in self._groups:
            raise SoftException(b'No dynamic components group "(%s)" found!' % groupName)
        return self._groups.get(groupName, [])

    def _readGroups(self, section):
        groups = {}
        for groupName, groupSection in section.items():
            if groupName.startswith(b'xmlns:'):
                continue
            if groupName in groups:
                raise SoftException(b'Duplicated dynamic components group "(%s)" in xml config!' % groupName)
    ...
    def _readGroup(self, section):
        ...
        for componentType, componentSection in section.items():
            ...
            componentReader = getattr(componentClass, b'groupComponentReader', None)
