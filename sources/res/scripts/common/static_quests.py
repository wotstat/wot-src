...
STATIC_QUEST_SOURCE_PATH = [
 b'scripts/item_defs/static_quests',
 b'@(EXTENSION)/scripts/item_defs/static_quests']
_logger = logging.getLogger(__name__)

def __isServerOnlyQuest(node):
    info = node.info
    return info.get(b'serverOnly', False)


StaticQuest = namedtuple(b'StaticQuest', [b'questID', b'questData'])
...
def __getStaticQuestsFromFileByEventType(sectionList, eventType, auxData=None):
    ...
    _logger.debug(b'Read %d quests of type %s', len(quests), eventType)
    return quests
...
class _StaticQuestsCache(object):
    ...
    def __init__(self, quests):
        ...
        _logger.debug(b'Create all static quests, there size is %d', len(self.__quests))
        return
...
def _createCache():
    ...
    _logger.debug(b'Length of sources from directory %s with static quests is %d', STATIC_QUEST_SOURCE_PATH[0], len(staticQuestSectionList))
    auxData = {}
...
def init():
    ...
    if g_staticCache is None:
        g_staticCache, _ = pdc.load(b'static_quests', _createCache)
    return
