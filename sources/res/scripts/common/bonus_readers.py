...
class IntHolder(int):
    ...
    def __new__(cls, value, **kwargs):
        return super(IntHolder, cls).__new__(cls, value, **{k: v for k, v in kwargs.iteritems() if k in (b'base',)})
...
def __readBonus_vehicle(bonus, _name, section, eventType, checkLimit):
    ...
    if section.has_key(b'vehCompDescr'):
        vehCompDescr = section[b'vehCompDescr'].asString.decode(b'base64')
    ...
    if section.has_key(b'withSlot'):
        extra[b'withSlot'] = True
    if section.has_key(b'checkRecycleBin'):
        extra[b'checkRecycleBin'] = True
    if section.has_key(b'force'):
        extra[b'force'] = True
...
def __readBonus_tankmen(bonus, vehTypeCompDescr, section, eventType, checkLimit):
    ...
    for subsection in section.values():
        ...
        tmanData = {b'isFemale': (subsection.readBool(b'isFemale', False)), 
           ...
           b'roleLevel': (subsection.readInt(b'roleLevel', 50)), 
...
def __readBonus_rent(bonus, _name, section):
    ...
    if section.has_key(b'expires'):
        rent[b'expires'] = section[b'expires'].asString
...
def __readBonus_outfits(bonus, _name, section):
    outfits = {}
    for seasonTypeName, seasonTypeID in {b'winter': (SeasonType.WINTER), 
       b'summer': (SeasonType.SUMMER), 
       b'desert': (SeasonType.DESERT), 
       b'event': (SeasonType.EVENT)}.iteritems():
        if section.has_key(seasonTypeName):
            outfits[seasonTypeID] = section[seasonTypeName].asString.decode(b'base64')
...
def __readBonus_tokens(bonus, _name, section, eventType, checkLimit):
    id = section[b'id'].asString
    checkLogExtInfoLen(id, b'token')
    if id.startswith(tankmen.RECRUIT_TMAN_TOKEN_PREFIX) and tankmen.getRecruitInfoFromToken(id) is None:
        raise SoftException((b'Invalid tankman token format: {}').format(id))
    token = bonus.setdefault(b'tokens', {})[id] = {}
    ...
    token[b'count'] = 1
    if section.has_key(b'count'):
        token[b'count'] = __readIntWithTokenExpansion(section[b'count'])
    ...
    if checkLimit and token[b'count'] > INVOICE_LIMITS.TOKENS_MAX:
        raise SoftException(b'Invalid count of tankman token with id %s with amount %d when limit is %d.' % (
         id, token[b'count'], INVOICE_LIMITS.TOKENS_MAX))
    return
    ...
    if section[b'expires'].has_key(b'endOfGameDay'):
        ...
        if section[b'expires'].has_key(b'after'):
            ...
            if expires[b'at'] is None:
                raise SoftException(b'Invalid expiry time for %s' % id)
        return
...
def __readBonus_blueprint(bonus, _name, section, eventType, checkLimit):
    ...
    if not isUniversalFragment(compDescr):
        ...
        if compDescr not in cache[b'vehiclesInTrees']:
            raise SoftException(b'Invalid vehicle type %s. Vehicle is not in research tree.' % compDescr)
...
def __readBonus_optionalData(config, bonusReaders, section, eventType):
    ...
    if section.has_key(b'probability'):
        probabilities = map(float, section.readString(b'probability', b'').split())
    ...
    if section.has_key(b'mainRotationBranch'):
    ...
    for name, subsection in section.items():
        ...
        if probabilitiesList is None:
            ...
            for i in xrange(probabilityStageCount):
                ...
                if isAlmostEqual(probabilitiesList[i], 0.0, epsilon=5e-06) and subBonus.get(b'properties', {}).get(b'compensation', False):
                    if nullProbabilityCompensations[i]:
                        raise SoftException(b"Compensation with 0.0 probability already exists inside 'oneof'")
...
def __readBonusRotation(config, bonusReaders, section, eventType, checkLimit):
    bonus = {}
    if config.get(b'probabilityStageCount'):
        raise SoftException(b'probabilityStageCount is not allowed to be used with rotation')
    limitIDs, subBonus = __readBonusSubSection(config, bonusReaders, section[b'rotation'], eventType, checkLimit)
    if not set(subBonus.keys()) == {b'groups'}:
        raise _xml.raiseWrongXml(None, b'rotation', b'The rotation should consist only <group> nodes.')
    rotationLevelCount = len(subBonus[b'groups'])
    if rotationLevelCount <= 1:
        raise SoftException(b'The number of nodes <group> must be greater than 1')
    config.setdefault(b'rotationLevelCount', rotationLevelCount)
    bonus[b'rotation'] = subBonus
    return (
     limitIDs, bonus)
...
def __readBonus_preferredMapSlots(bonus, _name, section, eventType, checkLimit):
    preferredMapSlots = bonus.setdefault(b'preferredMapSlots', {})
    for _, subsection in section.items():
        slotID = subsection.readInt(b'id')
        days = subsection.readInt(b'days')
        if days > 0:
...
__BONUS_READERS = {b'meta': __readMetaSection, 
   ...
   b'paragonsUnlocks': __readBonus_paragonsUnlocks, 
   ...
   b'preferredMapSlots': __readBonus_preferredMapSlots, 
...
def __readBonusConfig(section):
    ...
    for name, data in section.items():
        ...
        elif name == b'rotationLevelCount':
            raise SoftException(b'<rotationLevelCount> is a reserved field, cannot be filled in')
        else:
...
def readBonusSection(bonusRange, section, eventType=None, checkLimit=True):
    if section is None:
        ...
        readerBonus = __readBonusRotation if section.has_key(b'rotation') else __readBonusSubSection
