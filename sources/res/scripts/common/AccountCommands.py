...
PLAYER_CMD_NAMES = dict([(v, k) for k, v in globals().items() if k.startswith(b'CMD_')])
KEYS_ARE_UNIQUE = len(PLAYER_CMD_NAMES) == len(set(key for key in globals() if key.startswith(b'CMD_')))
...
LOCK_REASON_NAMES = dict([(v, k) for k, v in LOCK_REASON.__dict__.items() if not k.startswith(b'__')])
