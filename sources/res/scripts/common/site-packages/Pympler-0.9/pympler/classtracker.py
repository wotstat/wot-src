...
class _ClassObserver(object):
    __slots__ = (b'init', b'name', b'detail', b'keep', b'trace')
...
class TrackedObject(object):
    __slots__ = (b'ref', b'id', b'repr', b'name', b'birth', b'death', b'trace', b'snapshots', b'_resolution_level', b'__dict__')
    ...
    def __getstate__(self):
        state = {}
        for name in getattr(TrackedObject, b'__slots__', ()):
            if hasattr(self, name) and name not in (b'ref', b'__dict__'):
...
class Snapshot(object):
    ...
    def label(self):
        if not self.desc:
            return b'%.3fs' % self.timestamp
        return b'%s (%.3fs)' % (self.desc, self.timestamp)
...
class ClassTracker(object):
    ...
    def track_class(self, cls, name=None, resolution_level=0, keep=False, trace=False):
        if not isclass(cls):
            raise TypeError(b'only class objects can be tracked')
        if name is None:
            name = cls.__module__ + b'.' + cls.__name__
    ...
    def start_periodic_snapshots(self, interval=1.0):
        if not self._periodic_thread:
            self._periodic_thread = PeriodicThread(self, interval, name=b'BackgroundMonitor')
    ...
    def create_snapshot(self, description=b'', compute_total=False):
        try:
