...
def get_size(objects):
    ...
    for o in objects:
        try:
            ...
            print b'IGNORING: type=%s; o=%s' % (str(type(o)), str(o))
...
def get_diff(left, right):
    ...
    res[b'+'] = get_not_included(right, left_objects)
    res[b'-'] = get_not_included(left, right_objects)
    return res
...
def filter(objects, Type=None, min=-1, max=-1):
    res = []
    if min > max:
        raise ValueError(b'minimum must be smaller than maximum')
    if Type is not None:
...
def _is_containerobject(o):
    return bool(getattr(type(o), b'__flags__', 0) & _Py_TPFLAGS_HAVE_GC)
