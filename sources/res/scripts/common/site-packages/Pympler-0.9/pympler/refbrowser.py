    ...
    def _get_tree(self, root, maxdepth):
        ...
        for o in objects:
            ...
            if not self.repeat and _id in self.already_included:
                s = self.str_func(o)
                res.children.append(b'%s (already included, id %s)' % (
                 s, _id))
                continue
    ...
    def _print(self, tree, prefix, carryon):
        ...
        if len_children > 0:
            ...
            for b in range(1, len_children):
                ...
                if b == len_children - 1:
                    if len(carryon.strip(b' ')) == 0:
                        return
                    self.stream.write(carryon[:-2].rstrip() + b'\n')
...
try:
    ...
    class _TreeNode(_TreeWidget.TreeNode):
        ...
        def drawtext(self):
            ...
            menu.add_command(label=b'reload referrers', command=self.reload_referrers)
            menu.add_command(label=b'print', command=self.print_object)
            menu.add_separator()
            menu.add_command(label=b'expand', command=self.expand)
            menu.add_separator()
            menu.add_command(label=b'Close Popup Menu')

            def do_popup(event):
                menu.post(event.x_root, event.y_root)
                return

            self.label.bind(b'<Button-3>', do_popup)
...
def gui_default_str_function(o):
    return summary._repr(o) + b'(id=%s)' % id(o)
...
class InteractiveBrowser(RefBrowser):
    ...
    def __init__(self, rootobject, maxdepth=3, str_func=gui_default_str_function, repeat=True):
        if tkinter is None:
            raise ImportError(b'InteractiveBrowser requires Tkinter to be installed.')
        RefBrowser.__init__(self, rootobject, maxdepth, str_func, repeat)
        return

    def main(self, standalone=False):
        window = tkinter.Tk()
        sc = _TreeWidget.ScrolledCanvas(window, bg=b'white', highlightthickness=0, takefocus=1)
