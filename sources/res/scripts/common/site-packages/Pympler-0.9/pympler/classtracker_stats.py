...
__all__ = [
 b'Stats', b'ConsoleStats', b'HtmlStats']

def _merge_asized(base, other, level=0):
    ref2key = lambda ref: ref.name.split(b':')[0]
...
def _format_trace(trace):
    ...
    for fname, lineno, func, src, _ in trace:
        if src:
            for line in src:
                lines.append(b'    ' + line.strip() + b'\n')

        lines.append(b'  %s:%4d in %s\n' % (fname, lineno, func))
...
class Stats(object):
    ...
    def load_stats(self, fdump):
        if isinstance(fdump, type(b'')):
            fdump = open(fdump, b'rb')
    ...
    def sort_stats(self, *args):
        criteria = (b'classname', b'tsize', b'birth', b'death', b'name', b'repr', b'size')
        if not set(criteria).issuperset(set(args)):
            raise ValueError(b'Invalid sort criteria')
        if not args:
            args = criteria
    ...
    def annotate_snapshot(self, snapshot):
        if hasattr(snapshot, b'classes'):
            ...
            for classname in list(self.index.keys()):
                ...
                snapshot.classes[classname][b'merged'] = merged
...
class ConsoleStats(Stats):
    ...
    def _print_refs(self, refs, total, prefix=b'    ', level=1, minsize=0, minpct=0.1):
        lrefs = list(refs)
        lrefs.sort(key=(lambda x: x.size))
        lrefs.reverse()
        for ref in lrefs:
            if ref.size > minsize and ref.size * 100.0 / total > minpct:
                self.stream.write(b'%-50s %-14s %3d%% [%d]\n' % (
                 trunc(prefix + str(ref.name), 50),
                 pp(ref.size),
                 int(ref.size * 100.0 / total),
                 level))
                self._print_refs(ref.refs, total, prefix=prefix + b'  ', level=level + 1)

        return

    def print_object(self, tobj):
        if tobj.death:
            self.stream.write(b'%-32s ( free )   %-35s\n' % (
             trunc(tobj.name, 32, left=1), trunc(tobj.repr, 35)))
        else:
            self.stream.write(b'%-32s 0x%08x %-35s\n' % (
    ...
    def print_summary(self):
        ...
        fobj = self.stream
        fobj.write(b'---- SUMMARY ' + b'-' * 66 + b'\n')
        for snapshot in self.snapshots:
            self.annotate_snapshot(snapshot)
            fobj.write(b'%-35s %11s %12s %12s %5s\n' % (
            ...
            for classname in classlist:
                ...
                fobj.write(b'  %-33s %11d %12s %12s %4d%%\n' % (
                 trunc(classname, 33),
                 info[b'active'],
                 pp(info[b'sum']),
                 pp(info[b'avg']),
...
class HtmlStats(Stats):
    ...
    class_summary = b'<p>%(cnt)d instances of %(cls)s were registered. The\n        average size is %(avg)s, the minimal size is %(min)s, the maximum size\n        is %(max)s.</p>\n'
    class_snapshot = b'<h3>Snapshot: %(name)s, %(total)s occupied by instances\n        of class %(cls)s</h3>\n'

    def print_class_details(self, fname, classname):
        fobj = open(fname, b'w')
        fobj.write(self.header % (classname, self.style))
        fobj.write(b'<h1>%s</h1>\n' % classname)
        sizes = [tobj.get_max_size() for tobj in self.index[classname]]
        total = 0
        for s in sizes:
            total += s

        data = {b'cnt': (len(self.index[classname])), b'cls': classname}
        data[b'avg'] = pp(total / len(sizes))
        data[b'max'] = pp(max(sizes))
        data[b'min'] = pp(min(sizes))
        fobj.write(self.class_summary % data)
        fobj.write(self.charts[classname])
        fobj.write(b'<h2>Coalesced Referents per Snapshot</h2>\n')
        for snapshot in self.snapshots:
            if classname in snapshot.classes:
                merged = snapshot.classes[classname][b'merged']
                fobj.write(self.class_snapshot % {b'name': (snapshot.desc), 
                   b'cls': classname, 
                   b'total': (pp(merged.size))})
                if merged.refs:
                    self._print_refs(fobj, merged.refs, merged.size)
                else:
                    fobj.write(b'<p>No per-referent sizes recorded.</p>\n')

        fobj.write(b'<h2>Instances</h2>\n')
        for tobj in self.index[classname]:
            fobj.write(b'<table id="tl" width="100%" rules="rows">\n')
            fobj.write(b'<tr><td id="hl" width="140px">Instance</td>' + b'<td id="hl">%s at 0x%08x</td></tr>\n' % (
             tobj.name, tobj.id))
            if tobj.repr:
                fobj.write(b'<tr><td>Representation</td>' + b'<td>%s&nbsp;</td></tr>\n' % tobj.repr)
            fobj.write(b'<tr><td>Lifetime</td><td>%s - %s</td></tr>\n' % (
             pp_timestamp(tobj.birth), pp_timestamp(tobj.death)))
            if tobj.trace:
                trace = b'<pre>%s</pre>' % _format_trace(tobj.trace)
                fobj.write(b'<tr><td>Instantiation</td><td>%s</td></tr>\n' % trace)
            for timestamp, size in tobj.snapshots:
                fobj.write(b'<tr><td>%s</td>' % pp_timestamp(timestamp))
                if not size.refs:
                    fobj.write(b'<td>%s</td></tr>\n' % pp(size.size))
                else:
                    fobj.write(b'<td>%s' % pp(size.size))
    ...
    def create_title_page(self, filename, title=b''):
        fobj = open(filename, b'w')
        fobj.write(self.header % (title, self.style))
        fobj.write(b'<h1>%s</h1>\n' % title)
        fobj.write(b'<h2>Memory distribution over time</h2>\n')
        fobj.write(self.charts[b'snapshots'])
        fobj.write(b'<h2>Snapshots statistics</h2>\n')
        fobj.write(b'<table id="nb">\n')
        classlist = list(self.index.keys())
        classlist.sort()
        for snapshot in self.snapshots:
            fobj.write(b'<tr><td>\n')
            fobj.write(b'<table id="tl" rules="rows">\n')
            fobj.write(b'<h3>%s snapshot at %s</h3>\n' % (
             snapshot.desc or b'Untitled',
             pp_timestamp(snapshot.timestamp)))
            data = {}
            data[b'sys'] = pp(snapshot.system_total.vsz)
            data[b'tracked'] = pp(snapshot.tracked_total)
            data[b'asizeof'] = pp(snapshot.asizeof_total)
            data[b'overhead'] = pp(getattr(snapshot, b'overhead', 0))
            ...
            if snapshot.tracked_total:
                ...
                for classname in classlist:
                    ...
                    data[b'cls'] = b'<a href="%s">%s</a>' % (path, classname)
                    data[b'sum'] = pp(data[b'sum'])
                    data[b'avg'] = pp(data[b'avg'])
    ...
    def create_lifetime_chart(self, classname, filename=b''):
        try:
            from pylab import figure, title, xlabel, ylabel, plot, savefig
        except ImportError:
            return HtmlStats.nopylab_msg % (classname + b' lifetime')
        ...
        y = [c for t, c in cnt]
        figure()
        xlabel(b'Execution time [s]')
        ylabel(b'Instance #')
        title(b'%s instances' % classname)
        plot(x, y, b'o')
        savefig(filename)
        return self.chart_tag % os.path.basename(filename)

    def create_snapshot_chart(self, filename=b''):
        try:
            from pylab import figure, title, xlabel, ylabel, plot, fill, legend, savefig
            import matplotlib.mlab as mlab
        except ImportError:
            return self.nopylab_msg % b'memory allocation'
        ...
        polys = []
        for cn in classlist:
            pct = [snapshot.classes[cn][b'pct'] for snapshot in self.snapshots]
            if pct and max(pct) > 3.0:
                sz = [float(fp.classes[cn][b'sum']) / 1048576 for fp in self.snapshots]
                sz = [sx + sy for sx, sy in zip(base, sz)]
                xp, yp = mlab.poly_between(times, base, sz)
                polys.append(((xp, yp), {b'label': cn}))
                poly_labels.append(cn)
                base = sz

        figure()
        title(b'Snapshot Memory')
        xlabel(b'Execution Time [s]')
        ylabel(b'Virtual Memory [MiB]')
        sizes = [float(fp.asizeof_total) / 1048576 for fp in self.snapshots]
        plot(times, sizes, b'r--', label=b'Total')
        sizes = [float(fp.tracked_total) / 1048576 for fp in self.snapshots]
        plot(times, sizes, b'b--', label=b'Tracked total')
    ...
    def create_pie_chart(self, snapshot, filename=b''):
        ...
        for k, v in list(snapshot.classes.items()):
            if v[b'pct'] > 3.0:
                classlist.append(k)
                sizelist.append(v[b'sum'])

        sizelist.insert(0, snapshot.asizeof_total - pylab_sum(sizelist))
        classlist.insert(0, b'Other')
        title(b'Snapshot (%s) Memory Distribution' % snapshot.desc)
        figure(figsize=(8, 8))
    ...
    def create_html(self, fname, title=b'ClassTracker Statistics'):
        self.basedir = os.path.dirname(os.path.abspath(fname))
        self.filesdir = os.path.splitext(fname)[0] + b'_files'
        ...
        self.annotate()
        self.charts = {}
        fn = os.path.join(self.filesdir, b'timespace.png')
        self.charts[b'snapshots'] = self.create_snapshot_chart(fn)
        for fp, idx in zip(self.snapshots, list(range(len(self.snapshots)))):
            fn = os.path.join(self.filesdir, b'fp%d.png' % idx)
            self.charts[fp] = self.create_pie_chart(fp, fn)

        for cn in list(self.index.keys()):
            fn = os.path.join(self.filesdir, cn.replace(b'.', b'_') + b'-lt.png')
            self.charts[cn] = self.create_lifetime_chart(cn, fn)

        for cn in list(self.index.keys()):
            fn = os.path.join(self.filesdir, cn.replace(b'.', b'_') + b'.html')
            self.links[cn] = fn
