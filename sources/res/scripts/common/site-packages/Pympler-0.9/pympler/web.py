import sys, os, threading
from inspect import getouterframes
from shutil import rmtree
from tempfile import mkdtemp
from threading import Thread
from weakref import WeakValueDictionary
from wsgiref.simple_server import make_server
from pympler import asizeof
from pympler.garbagegraph import GarbageGraph
from pympler.process import get_current_threads, ProcessMemoryInfo
from pympler.util.compat import dumps
from pympler.util.stringutils import safe_repr
try:
    import bottle
except ImportError:
    from pympler.util import bottle

class ServerState(threading.local):

    def __init__(self):
        self.server = None
        self.stats = None
        self.garbage_graphs = None
        self.id2ref = WeakValueDictionary()
        self.id2obj = dict()
        return

    def clear_cache(self):
        self.garbage_graphs = None
        return


server = ServerState()

def get_ref(obj):
    oid = id(obj)
    try:
        server.id2ref[oid] = obj
    except TypeError:
        server.id2obj[oid] = obj

    return str(oid)


def get_obj(ref):
    oid = int(ref)
    return server.id2ref.get(oid) or server.id2obj[oid]


pympler_path = os.path.dirname(os.path.abspath(__file__))
static_files = os.path.join(pympler_path, b'templates')
bottle.TEMPLATE_PATH.append(static_files)

@bottle.route(b'/')
@bottle.view(b'index')
def root():
    pmi = ProcessMemoryInfo()
    return dict(processinfo=pmi)


@bottle.route(b'/process')
@bottle.view(b'process')
def process():
    pmi = ProcessMemoryInfo()
    threads = get_current_threads()
    return dict(info=pmi, threads=threads)


@bottle.route(b'/tracker')
@bottle.view(b'tracker')
def tracker_index():
    stats = server.stats
    if stats and stats.snapshots:
        stats.annotate()
        timeseries = []
        for cls in stats.tracked_classes:
            series = []
            for snapshot in stats.snapshots:
                series.append(snapshot.classes.get(cls, {}).get(b'sum', 0))

            timeseries.append((cls, series))

        series = [s.overhead for s in stats.snapshots]
        timeseries.append((b'Profiling overhead', series))
        if stats.snapshots[0].system_total.data_segment:
            series = [s.system_total.data_segment - s.tracked_total - s.overhead for s in stats.snapshots]
            timeseries.append((b'Data segment', series))
            series = [s.system_total.code_segment for s in stats.snapshots]
            timeseries.append((b'Code segment', series))
            series = [s.system_total.stack_segment for s in stats.snapshots]
            timeseries.append((b'Stack segment', series))
            series = [s.system_total.shared_segment for s in stats.snapshots]
            timeseries.append((b'Shared memory', series))
        else:
            series = [s.total - s.tracked_total - s.overhead for s in stats.snapshots]
            timeseries.append((b'Other', series))
        timeseries = [dict(label=label, data=list(enumerate(data))) for label, data in timeseries]
        return dict(snapshots=stats.snapshots, timeseries=dumps(timeseries))
    else:
        return dict(snapshots=[])

    return


@bottle.route(b'/tracker/class/:clsname')
@bottle.view(b'tracker_class')
def tracker_class(clsname):
    stats = server.stats
    if not stats:
        bottle.redirect(b'/tracker')
    stats.annotate()
    return dict(stats=stats, clsname=clsname)


@bottle.route(b'/refresh')
def refresh():
    server.clear_cache()
    bottle.redirect(b'/')
    return


@bottle.route(b'/traceback/:threadid')
@bottle.view(b'stacktrace')
def get_traceback(threadid):
    threadid = int(threadid)
    frames = sys._current_frames()
    if threadid in frames:
        frame = frames[threadid]
        stack = getouterframes(frame, 5)
        stack.reverse()
        stack = [(get_ref(f[0].f_locals),) + f[1:] for f in stack]
    else:
        stack = []
    return dict(stack=stack, threadid=threadid)


@bottle.route(b'/objects/:oid')
@bottle.view(b'referents')
def get_obj_referents(oid):
    referents = {}
    obj = get_obj(oid)
    if type(obj) is dict:
        named_objects = asizeof.named_refs(obj)
    else:
        refs = asizeof._getreferents(obj)
        named_objects = [(repr(type(x)), x) for x in refs]
    for name, o in named_objects:
        referents[name] = (
         get_ref(o), type(o).__name__,
         safe_repr(o, clip=48), asizeof.asizeof(o))

    return dict(referents=referents)


@bottle.route(b'/static/:filename')
def static_file(filename):
    return bottle.static_file(filename, root=static_files)


def _compute_garbage_graphs():
    if server.garbage_graphs is None:
        server.garbage_graphs = GarbageGraph().split_and_sort()
    return server.garbage_graphs


@bottle.route(b'/garbage')
@bottle.view(b'garbage_index')
def garbage_index():
    garbage_graphs = _compute_garbage_graphs()
    return dict(graphs=garbage_graphs)


@bottle.route(b'/garbage/:index')
@bottle.view(b'garbage')
def garbage_cycle(index):
    graph = _compute_garbage_graphs()[int(index)]
    graph.reduce_to_cycles()
    objects = graph.metadata
    objects.sort(key=(lambda x: -x.size))
    return dict(objects=objects, index=index)


def _get_graph(graph, filename):
    try:
        rendered = graph.rendered_file
    except AttributeError:
        try:
            graph.render(os.path.join(server.tmpdir, filename), format=b'png')
            rendered = filename
        except OSError:
            rendered = None

    graph.rendered_file = rendered
    return rendered


@bottle.route(b'/garbage/graph/:index')
def garbage_graph(index):
    graph = _compute_garbage_graphs()[int(index)]
    reduce_graph = bottle.request.GET.get(b'reduce', b'')
    if reduce_graph:
        graph = graph.reduce_to_cycles()
    if not graph:
        return
    else:
        filename = b'garbage%so%s.png' % (index, reduce_graph)
        rendered_file = _get_graph(graph, filename)
        if rendered_file:
            bottle.static_file(rendered_file, root=server.tmpdir)
        else:
            return
        return


@bottle.route(b'/help')
def show_documentation():
    bottle.redirect(b'https://pympler.readthedocs.io/en/latest/')
    return


class PymplerServer(bottle.ServerAdapter):

    def run(self, handler):
        self.server = make_server(self.host, self.port, handler)
        self.server.serve_forever()
        return


def start_profiler(host=b'localhost', port=8090, tracker=None, stats=None, debug=False, **kwargs):
    if tracker and not stats:
        server.stats = tracker.stats
    else:
        server.stats = stats
    try:
        server.tmpdir = mkdtemp(prefix=b'pympler')
        server.server = PymplerServer(host=host, port=port, **kwargs)
        bottle.debug(debug)
        bottle.run(server=server.server)
    finally:
        rmtree(server.tmpdir)

    return


class ProfilerThread(Thread):

    def __init__(self, group=None, target=None, name=b'Pympler web frontend', **kwargs):
        super(ProfilerThread, self).__init__(group=group, target=target, name=name)
        self.kwargs = kwargs
        self.daemon = True
        return

    def run(self):
        start_profiler(**self.kwargs)
        return


def start_in_background(**kwargs):
    thread = ProfilerThread(**kwargs)
    thread.start()
    return thread
