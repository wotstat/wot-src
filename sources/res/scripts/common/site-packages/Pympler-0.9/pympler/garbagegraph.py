from pympler.refgraph import ReferenceGraph
from pympler.util.stringutils import trunc, pp
import sys, gc
__all__ = [
 b'GarbageGraph', b'start_debug_garbage', b'end_debug_garbage']

class GarbageGraph(ReferenceGraph):

    def __init__(self, reduce=False, collectable=True):
        if collectable:
            gc.set_debug(gc.DEBUG_SAVEALL)
        else:
            gc.set_debug(0)
        gc.collect()
        ReferenceGraph.__init__(self, gc.garbage, reduce)
        return

    def print_stats(self, stream=None):
        if not stream:
            stream = sys.stdout
        self.metadata.sort(key=(lambda x: -x.size))
        stream.write(b'%-10s %8s %-12s %-46s\n' % (b'id', b'size', b'type', b'representation'))
        for g in self.metadata:
            stream.write(b'0x%08x %8d %-12s %-46s\n' % (g.id, g.size,
             trunc(g.type, 12),
             trunc(g.str, 46)))

        stream.write(b'Garbage: %8d collected objects (%s in cycles): %12s\n' % (
         self.count, self.num_in_cycles, pp(self.total_size)))
        return


def start_debug_garbage():
    gc.collect()
    gc.disable()
    return


def end_debug_garbage():
    gc.set_debug(0)
    gc.enable()
    return
