...
import numbers, copy, sys, re, binascii, mmap, os, struct, operator, collections, array
...
LITERAL_RE = re.compile(b'(?P<name>0(x|o|b))(?P<value>.+)', re.IGNORECASE)
        ...
        for token in tokens:
            ...
            if name == b'bool':
                if length is not None:
                    raise ValueError(b"You can't specify a length with bool tokens - they are always one bit.")
                length = 1
...
class Bits(object):
    ...
    def __repr__(self):
        ...
        if isinstance(self._datastore._rawarray, MmapByteArray):
            ...
            return (b"{0}(filename='{1}'{2}{3})").format(self.__class__.__name__, self._datastore._rawarray.source.name, lengthstring, offsetstring)
        else:
            s = self.__str__()
            lengthstring = b''
            if s.endswith(b'...'):
                lengthstring = (b' # length={0}').format(length)
            return (b"{0}('{1}'){2}").format(self.__class__.__name__, s, lengthstring)
    ...
    def _setuint(self, uint, length=None):
        ...
        if uint < 0:
            raise CreationError(b'uint cannot be initialsed by a negative number.')
        s = hex(uint)[2:]
        s = s.rstrip(b'L')
        if len(s) & 1:
            s = b'0' + s
    ...
    def _readbin(self, length, start):
        ...
        try:
            ...
            c = (b'{0:0{1}b}').format(int(binascii.hexlify(str(b)), 16), 8 * len(b))
...
class BitArray(Bits):
        ...
        if not bs.len:
            ...
            if pos is None:
                try:
                    ...
                    raise TypeError(b'insert require a bit position for this type.')
        ...
        if not bs.len:
            ...
            if pos is None:
                try:
                    ...
                    raise TypeError(b'overwrite require a bit position for this type.')
    ...
    def ror(self, bits, start=None, end=None):
        ...
        if bits < 0:
            raise ValueError(b'Cannot rotate right by negative amount.')
        start, end = self._validate_slice(start, end)
        bits %= end - start
        if not bits:
            return
    ...
    def rol(self, bits, start=None, end=None):
        if not self.len:
            raise Error(b'Cannot rotate an empty bitstring.')
        if bits < 0:
            raise ValueError(b'Cannot rotate left by negative amount.')
        start, end = self._validate_slice(start, end)
        bits %= end - start
        if not bits:
            return
