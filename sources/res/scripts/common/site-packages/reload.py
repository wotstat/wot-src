...
LEVEL2 = 2
DEBUG = NO_DEBUG
OUTPUT_DIR = b'.jit/output'
SRC_DIRS = (b'wot', b'wot_ext')
...
def writeErr(*args):
    ...
    msg = (b' ').join(newLst)
    sys.stderr.write(b'pydev debugger: %s\n' % (msg,))
    return
...
def codeObjectsEqual(code0, code1):
    for d in dir(code0):
        if d.startswith(b'_') or b'lineno' in d:
            continue
...
class Reload(object):
        ...
        try:
            ...
            try:
                if kind not in (imp.PY_COMPILED, imp.PY_SOURCE):
                    notifyError(b'Could not find source to reload (mod: %s)' % (modname,))
                    return
            ...
            newNamespace.clear()
            newNamespace[b'__name__'] = modns[b'__name__']
            ...
            for name in newnames - oldnames:
                notifyInfo0(b'Added:', name, b'to namespace')
    ...
    def _handleNamespace(self, namespace, isClassNamespace=False):
        onFinish = None
        if isClassNamespace:
            xreloadAfterUpdate = getattr(namespace, b'__xreload_after_reload_update__', None)
            if xreloadAfterUpdate is not None:
                self.foundChange = True
                onFinish = lambda : xreloadAfterUpdate()
        elif b'__xreload_after_reload_update__' in namespace:
            xreloadAfterUpdate = namespace[b'__xreload_after_reload_update__']
            self.foundChange = True
    ...
    def _update(self, namespace, name, oldobj, newobj, isClassNamespace=False):
        try:
            notifyInfo2(b'Updating: ', oldobj)
            if oldobj is newobj:
                return
            if type(oldobj) is not type(newobj):
                notifyError(b'Type of: %s changed... Skipping.' % (oldobj,))
                return
            ...
            if isinstance(newobj, classtype):
                ...
                return
            if hasattr(newobj, b'__metaclass__') and hasattr(newobj, b'__class__') and newobj.__metaclass__ == newobj.__class__:
                self._updateClass(oldobj, newobj)
                return
            if namespace is not None:
                if oldobj != newobj and str(oldobj) != str(newobj) and repr(oldobj) != repr(newobj):
                    xreloadOldNew = None
                    if isClassNamespace:
                        xreloadOldNew = getattr(namespace, b'__xreload_old_new__', None)
                        if xreloadOldNew is not None:
                            self.foundChange = True
                            xreloadOldNew(name, oldobj, newobj)
                    elif b'__xreload_old_new__' in namespace:
                        namespace[name] = newobj
                        xreloadOldNew = namespace[b'__xreload_old_new__']
                        xreloadOldNew(namespace, name, oldobj, newobj)
                        self.foundChange = True
        except:
            notifyError(b'Exception found when updating %s. Proceeding for other items.' % (name,))
            traceback.print_exc()
    ...
    def _updateMethod(self, oldmeth, newmeth):
        if hasattr(oldmeth, b'im_func') and hasattr(newmeth, b'im_func'):
            self._update(None, None, oldmeth.im_func, newmeth.im_func)
        elif hasattr(oldmeth, b'__func__') and hasattr(newmeth, b'__func__'):
    ...
    def _updateClass(self, oldclass, newclass):
        ...
        for name in newnames - oldnames:
            ...
            notifyInfo0(b'Added:', name, b'to', oldclass)
            self.foundChange = True

        for name in oldnames - newnames:
            notifyInfo(b'Removed:', name, b'from', oldclass)
            delattr(oldclass, name)

        for name in (oldnames & newnames) - set([b'__dict__', b'__doc__']):
            self._update(oldclass, name, olddict[name], newdict[name], isClassNamespace=True)

        oldBases = getattr(oldclass, b'__bases__', None)
        newBases = getattr(newclass, b'__bases__', None)
        toSet = lambda bases: set(map((lambda x: x.__name__), bases))
        if toSet(oldBases) != toSet(newBases):
            notifyError(b'Changing the hierarchy of a class is not supported. %s may be inconsistent., %s, %s' % (
