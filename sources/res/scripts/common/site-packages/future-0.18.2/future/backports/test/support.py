...
if sys.platform.startswith(u'win'):
    ...
    def _is_gui_available():
        ...
        class USEROBJECTFLAGS(ctypes.Structure):
            _fields_ = [
             (
              u'fInherit', ctypes.wintypes.BOOL),
             (
              u'fReserved', ctypes.wintypes.BOOL),
             (
              u'dwFlags', ctypes.wintypes.DWORD)]
            ...
            if platform.system() == sysname:
                ...
                try:
                    ...
                    if version < min_version:
                        ...
                        raise unittest.SkipTest(u'%s version %s or higher required, not %s' % (
        ...
        def wrapper(*args, **kw):
            if sys.platform == u'darwin':
                ...
                try:
                    ...
                    if version < min_version:
                        min_version_txt = (u'.').join(map(str, min_version))
                        raise unittest.SkipTest(u'Mac OS X %s or higher required, not %s' % (
...
def check_sizeof(test, o, size):
    ...
    msg = u'wrong size for %s: got %d, expected %d' % (
     type(o), result, size)
...
def run_doctest(module, verbosity=None, optionflags=0):
    ...
    if verbose:
        print(u'doctest (%s) ... %d tests with zero failures' % (
         module.__name__, t))
    return (
     f, t)
