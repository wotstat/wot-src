...
def newround(number, ndigits=None):
    ...
    if ndigits is None:
        return_int = True
        ndigits = 0
    if hasattr(number, b'__round__'):
        return number.__round__(ndigits)
    else:
        if ndigits < 0:
            raise NotImplementedError(b'negative ndigits not supported yet')
        exponent = Decimal(b'10') ** (-ndigits)
        if PYPY:
            if b'numpy' in repr(type(number)):
                number = float(number)
