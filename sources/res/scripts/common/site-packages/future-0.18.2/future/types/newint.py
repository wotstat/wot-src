...
class newint(with_metaclass(BaseNewInt, long)):
    ...
    def __new__(cls, x=0, base=10):
        try:
            ...
            if not isint(val):
                raise TypeError((b'__int__ returned non-int ({0})').format(type(val)))
            if base != 10:
                if not (istext(val) or isbytes(val) or isinstance(val, bytearray)):
                    raise TypeError(b"int() can't convert non-string with explicit base")
                try:
            ...
            try:
                ...
                try:
                    ...
                    raise TypeError((b"newint argument must be a string or a number,not '{0}'").format(type(val)))
    ...
    def __lshift__(self, other):
        if not isint(other):
            raise TypeError(b"unsupported operand type(s) for <<: '%s' and '%s'" % (
    ...
    def to_bytes(self, length, byteorder=b'big', signed=False):
        if length < 0:
            raise ValueError(b'length argument must be non-negative')
        ...
        if signed and self < 0:
            ...
            num = 2 ** bits + self
            if num <= 0:
                raise OverflowError(b'int too smal to convert')
        elif self < 0:
            raise OverflowError(b"can't convert negative int to unsigned")
        num = self
        if byteorder not in (b'little', b'big'):
            raise ValueError(b"byteorder must be either 'little' or 'big'")
        h = b'%x' % num
        s = newbytes((b'0' * (len(h) % 2) + h).zfill(length * 2).decode(b'hex'))
        if signed:
            high_set = s[0] & 128
            if self > 0 and high_set:
                raise OverflowError(b'int too big to convert')
            if self < 0 and not high_set:
                raise OverflowError(b'int too small to convert')
        if len(s) > length:
            raise OverflowError(b'int too big to convert')
        if byteorder == b'big':
            return s
    ...
    @classmethod
    def from_bytes(cls, mybytes, byteorder=b'big', signed=False):
        if byteorder not in (b'little', b'big'):
            raise ValueError(b"byteorder must be either 'little' or 'big'")
        if isinstance(mybytes, unicode):
            raise TypeError(b'cannot convert unicode objects to bytes')
        elif isinstance(mybytes, Iterable):
            mybytes = newbytes(mybytes)
        b = mybytes if byteorder == b'big' else mybytes[::-1]
        if len(b) == 0:
            b = b'\x00'
        num = int(native(b).encode(b'hex'), 16)
