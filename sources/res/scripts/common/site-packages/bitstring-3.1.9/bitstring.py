...
class Error(Exception):
    ...
    def __init__(self, *params):
        self.msg = params[0] if params else b''
...
class ConstByteStore(object):
    __slots__ = (b'offset', b'_rawarray', b'bitlength')
...
class MmapByteArray(object):
    __slots__ = (b'filemap', b'filelength', b'source', b'byteoffset', b'bytelength')
...
if sys.version_info[0] == 2:
    for i in range(256):
        BYTE_REVERSAL_DICT[i] = chr(int((b'{0:08b}').format(i)[::-1], 2))

else:
    for i in range(256):
        BYTE_REVERSAL_DICT[i] = bytes([int((b'{0:08b}').format(i)[::-1], 2)])
...
def tidy_input_string(s):
    s = (b'').join(s.split()).lower()
    return s


INIT_NAMES = (
 b'uint', b'int', b'ue', b'se', b'sie', b'uie', b'hex', b'oct', b'bin', b'bits',
 b'uintbe', b'intbe', b'uintle', b'intle', b'uintne', b'intne',
 b'float', b'floatbe', b'floatle', b'floatne', b'bytes', b'bool', b'pad')
TOKEN_RE = re.compile(b'(?P<name>' + (b'|').join(INIT_NAMES) + b')(:(?P<len>[^=]+))?(=(?P<value>.*))?$', re.IGNORECASE)
DEFAULT_UINT = re.compile(b'(?P<len>[^=]+)?(=(?P<value>.*))?$', re.IGNORECASE)
MULTIPLICATIVE_RE = re.compile(b'(?P<factor>.*)\\*(?P<token>.+)')
LITERAL_RE = re.compile(b'(?P<name>0([xob]))(?P<value>.+)', re.IGNORECASE)
STRUCT_PACK_RE = re.compile(b'(?P<endian>[<>@])?(?P<fmt>(?:\\d*[bBhHlLqQfd])+)$')
STRUCT_SPLIT_RE = re.compile(b'\\d*[bBhHlLqQfd]')
REPLACEMENTS_BE = {b'b': b'intbe:8', b'B': b'uintbe:8', b'h': b'intbe:16', 
   b'H': b'uintbe:16', b'l': b'intbe:32', 
   b'L': b'uintbe:32', b'q': b'intbe:64', 
   b'Q': b'uintbe:64', b'f': b'floatbe:32', 
   b'd': b'floatbe:64'}
REPLACEMENTS_LE = {b'b': b'intle:8', b'B': b'uintle:8', b'h': b'intle:16', 
   b'H': b'uintle:16', b'l': b'intle:32', 
   b'L': b'uintle:32', b'q': b'intle:64', 
   b'Q': b'uintle:64', b'f': b'floatle:32', 
   b'd': b'floatle:64'}
PACK_CODE_SIZE = {b'b': 1, b'B': 1, b'h': 2, b'H': 2, b'l': 4, b'L': 4, b'q': 8, 
   b'Q': 8, b'f': 4, b'd': 8}
_tokenname_to_initialiser = {b'hex': b'hex', b'0x': b'hex', b'0X': b'hex', b'oct': b'oct', b'0o': b'oct', 
   b'0O': b'oct', b'bin': b'bin', b'0b': b'bin', b'0B': b'bin', 
   b'bits': b'auto', b'bytes': b'bytes', b'pad': b'pad'}
...
def structparser(token):
    ...
    if not m:
        ...
        endian = m.group(b'endian')
        if endian is None:
            return [token]
        formatlist = re.findall(STRUCT_SPLIT_RE, m.group(b'fmt'))
        fmt = (b'').join([f[-1] * int(f[:-1]) if len(f) != 1 else f for f in formatlist])
        if endian == b'@':
...
def tokenparser(fmt, keys=None, token_cache={}):
    ...
    meta_tokens = ((b'').join(f.split()) for f in fmt.split(b','))
    return_values = []
    ...
    for meta_token in meta_tokens:
        ...
        if not m:
            ...
            factor = int(m.group(b'factor'))
            meta_token = m.group(b'token')
        ...
        for token in tokens:
            ...
            if not m1:
                ...
                if not m2:
                    raise ValueError((b"Don't understand token '{0}'.").format(token))
            if m1:
                name = m1.group(b'name')
                length = m1.group(b'len')
                if m1.group(b'value'):
                    value = m1.group(b'value')
            else:
                name = b'uint'
                length = m2.group(b'len')
                if m2.group(b'value'):
                    value = m2.group(b'value')
            if name == b'bool':
                if length is not None and length != b'1':
                    raise ValueError(b'You can only specify one bit sized bool tokens or leave unspecified.')
                length = 1
            if length is None and name not in (b'se', b'ue', b'sie', b'uie'):
                stretchy_token = True
            if length is not None:
                try:
                    ...
                    raise ValueError(b"Can't read a token with a negative length.")
                except ValueError:
                    if not keys or length not in keys:
                        raise ValueError((b"Don't understand length '{0}' of token.").format(length))
...
BRACKET_RE = re.compile(b'(?P<factor>\\d+)\\*\\(')
...
def expand_brackets(s):
    ...
    while True:
        ...
        if count:
            raise ValueError((b"Unbalanced parenthesis in '{0}'.").format(s))
        if start == 0 or s[start - 1] != b'*':
            s = s[0:start] + s[start + 1:p] + s[p + 1:]
        else:
            m = BRACKET_RE.search(s)
            if m:
                factor = int(m.group(b'factor'))
                matchstart = m.start(b'factor')
                s = s[0:matchstart] + (factor - 1) * (s[start + 1:p] + b',') + s[start + 1:p] + s[p + 1:]
            else:
                raise ValueError((b"Failed to parse '{0}'.").format(s))

    return s


OCT_TO_BITS = [(b'{0:03b}').format(i) for i in xrange(8)]
BIT_COUNT = dict(zip(xrange(256), [bin(i).count(b'1') for i in xrange(256)]))

class Bits(object):
    __slots__ = b'_datastore'
            ...
            if isinstance(auto, basestring):
                try:
                    ...
                    if offset is not None:
                        raise CreationError(b'offset should not be specified when using string initialisation.')
                    if length is not None:
                        raise CreationError(b'length should not be specified when using string initialisation.')
    ...
    def _initialise(self, auto, length, offset, **kwargs):
        if length is not None and length < 0:
            raise CreationError(b'bitstring length cannot be negative.')
        if offset is not None and offset < 0:
            raise CreationError(b'offset must be >= 0.')
        if auto is not None:
            ...
            try:
                ...
                if length is not None or offset is not None:
                    raise CreationError(b'Cannot use length or offset with this initialiser.')
            except KeyError:
                try:
                    init_with_length_only[k](self, v, length)
                    if offset is not None:
                        raise CreationError(b'Cannot use offset with this initialiser.')
                except KeyError:
                    if offset is None:
                        offset = 0
                    try:
                        ...
                        raise CreationError(b"Unrecognised keyword '{0}' used to initialise.", k)
    ...
    def __lt__(self, other):
        raise TypeError((b'unorderable type: {0}').format(type(self).__name__))
        return
    ...
    def __getitem__(self, key):
        ...
        if isinstance(key, slice):
            ...
            key += length
        if not 0 <= key < length:
            raise IndexError(b'Slice index out of range.')
    ...
    def __str__(self):
        ...
        if length > MAX_CHARS * 4:
            return (b'').join((b'0x', self._readhex(MAX_CHARS * 4, 0), b'...'))
        if length < 32 and length % 4 != 0:
            return b'0b' + self.bin
        if not length % 4:
            return b'0x' + self.hex
        bits_at_end = length % 4
        return (b'').join((b'0x', self._readhex(length - bits_at_end, 0),
         b', ', b'0b',
    ...
    def __repr__(self):
        ...
        try:
            pos = self._pos
            pos_string = b'' if pos == 0 else (b', pos={0}').format(pos)
        except AttributeError:
            pos_string = b''
        ...
        if isinstance(self._datastore._rawarray, MmapByteArray):
            ...
            if self._datastore.byteoffset or self._offset:
                offsetstring = b', offset=%d' % (self._datastore._rawarray.byteoffset * 8 + self._offset)
            lengthstring = b', length=%d' % length
            return (b"{0}(filename='{1}'{2}{3}{4})").format(self.__class__.__name__, self._datastore._rawarray.source.name, lengthstring, offsetstring, pos_string)
        else:
            s = self.__str__()
            lengthstring = b''
            if s.endswith(b'...'):
                lengthstring = (b' # length={0}').format(length)
            return (b"{0}('{1}'{2}){3}").format(self.__class__.__name__, s, pos_string, lengthstring)
    ...
    def __invert__(self):
        if not self.len:
            raise Error(b'Cannot invert empty bitstring.')
        s = self._copy()
        s._invert_all()
        return s
    ...
    def __lshift__(self, n):
        if n < 0:
            raise ValueError(b'Cannot shift by a negative amount.')
        if not self.len:
            raise ValueError(b'Cannot shift an empty bitstring.')
        n = min(n, self.len)
    ...
    def __mul__(self, n):
        if n < 0:
            raise ValueError(b'Cannot multiply by a negative integer.')
        if not n:
            return self.__class__()
        s = self._copy()
        s._imul(n)
    ...
    def __and__(self, bs):
        bs = Bits(bs)
        if self.len != bs.len:
            raise ValueError(b'Bitstrings must have the same length for & operator.')
        s = self._copy()
        s._iand(bs)
        return s
        ...
        if token_length == 0:
            ...
            if value is None:
                if token_length is None:
                    error = (b'Token has no value ({0}=???).').format(name)
                else:
                    error = (b'Token has no value ({0}:{1}=???).').format(name, token_length)
                raise ValueError(error)
            try:
                b = cls(**{(_tokenname_to_initialiser[name]): value})
            except KeyError:
                if name in (b'se', b'ue', b'sie', b'uie'):
                    b = cls(**{name: (int(value))})
                elif name in (b'uint', b'int', b'uintbe', b'intbe', b'uintle', b'intle', b'uintne', b'intne'):
                    b = cls(**{name: (int(value)), b'length': token_length})
                elif name in (b'float', b'floatbe', b'floatle', b'floatne'):
                    b = cls(**{name: (float(value)), b'length': token_length})
                elif name == b'bool':
                    if value in (1, b'True', b'1'):
                        b = cls(bool=True)
                    elif value in (0, b'False', b'0'):
                        b = cls(bool=False)
                    else:
                        raise CreationError(b"bool token can only be 'True' or 'False'.")
                else:
                    raise CreationError(b"Can't parse token name {0}.", name)

            if token_length is not None and b.len != token_length:
                msg = b'Token with length {0} packed with value of length {1} ({2}:{3}={4}).'
        ...
        if isinstance(s, Bits):
            ...
            if isinstance(s, io.BytesIO):
                ...
                if length + byteoffset * 8 + offset > s.seek(0, 2) * 8:
                    raise CreationError(b'BytesIO object is not long enough for specified length and offset.')
            ...
            if isinstance(s, file):
                ...
                if length + byteoffset * 8 + offset > m.filelength * 8:
                    raise CreationError(b'File is not long enough for specified length and offset.')
                self._datastore = ConstByteStore(m, length, offset)
                return
            if length is not None:
                raise CreationError(b"The length keyword isn't applicable to this initialiser.")
            if offset:
                raise CreationError(b"The offset keyword isn't applicable to this initialiser.")
            ...
            if isinstance(s, numbers.Integral):
                if s < 0:
                    msg = b"Can't create bitstring of negative length {0}."
                    raise CreationError(msg, s)
                data = bytearray((s + 7) // 8)
                self._datastore = ByteStore(data, s, 0)
                return
            if isinstance(s, collectionsAbc.Iterable):
                self._setbin_unsafe((b'').join(str(int(bool(x))) for x in s))
                return
            raise TypeError((b'Cannot initialise bitstring from {0}.').format(type(s)))
            return

    def _setfile(self, filename, length, offset):
        with open(filename, b'rb') as source:
            if offset is None:
                offset = 0
            if length is None:
    ...
    def _setbytes_safe(self, data, length=None, offset=0):
        ...
        elif length + offset > len(data) * 8:
            msg = b'Not enough data present. Need {0} bits, have {1}.'
    ...
    def _getbytes(self):
        if self.len % 8:
            raise InterpretError(b'Cannot interpret as bytes unambiguously - not multiple of 8 bits.')
    ...
    def _setuint(self, uint, length=None):
        ...
        if length is None or length == 0:
            raise CreationError(b'A non-zero length must be specified with a uint initialiser.')
        if uint >= 1 << length:
            msg = b'{0} is too large an unsigned integer for a bitstring of length {1}. The allowed range is [0, {2}].'
            raise CreationError(msg, uint, length, (1 << length) - 1)
        if uint < 0:
            raise CreationError(b'uint cannot be initialised by a negative number.')
        s = hex(uint)[2:]
        s = s.rstrip(b'L')
        if len(s) & 1:
            s = b'0' + s
    ...
    def _readuint_msb0(self, length, start):
        if not length:
            raise InterpretError(b'Cannot interpret a zero length bitstring as an integer.')
        offset = self._offset
    ...
    def _setint(self, int_, length=None):
        if length is None and hasattr(self, b'len') and self.len != 0:
            length = self.len
        if length is None or length == 0:
            raise CreationError(b'A non-zero length must be specified with an int initialiser.')
        if int_ >= 1 << length - 1 or int_ < -(1 << length - 1):
            raise CreationError(b'{0} is too large a signed integer for a bitstring of length {1}. The allowed range is [{2}, {3}].', int_, length, -(1 << length - 1), (1 << length - 1) - 1)
        if int_ >= 0:
    ...
    def _setuintbe(self, uintbe, length=None):
        if length is not None and length % 8 != 0:
            raise CreationError(b'Big-endian integers must be whole-byte. Length = {0} bits.', length)
        self._setuint(uintbe, length)
        return

    def _readuintbe(self, length, start):
        if length % 8:
            raise InterpretError(b'Big-endian integers must be whole-byte. Length = {0} bits.', length)
    ...
    def _setuintle(self, uintle, length=None):
        if length is not None and length % 8 != 0:
            raise CreationError(b'Little-endian integers must be whole-byte. Length = {0} bits.', length)
    ...
    def _readuintle(self, length, start):
        if length % 8:
            raise InterpretError(b'Little-endian integers must be whole-byte. Length = {0} bits.', length)
        ...
        if not offset:
            ...
            while endbyte - chunksize + 1 >= startbyte:
                val <<= 8 * chunksize
                val += struct.unpack(b'<L', bytes(self._datastore.getbyteslice(endbyte + 1 - chunksize, endbyte + 1)))[0]
                endbyte -= chunksize
    ...
    def _setfloat(self, f, length=None):
        if length is None and hasattr(self, b'len') and self.len != 0:
            length = self.len
        if length is None or length == 0:
            raise CreationError(b'A non-zero length must be specified with a float initialiser.')
        if length == 32:
            b = struct.pack(b'>f', f)
        elif length == 64:
            b = struct.pack(b'>d', f)
        else:
            raise CreationError(b'floats can only be 32 or 64 bits long, not {0} bits', length)
    ...
    def _readfloat(self, length, start):
        if not (start + self._offset) % 8:
            ...
            if length == 32:
                f, = struct.unpack(b'>f', bytes(self._datastore.getbyteslice(startbyte, startbyte + 4)))
            elif length == 64:
                f, = struct.unpack(b'>d', bytes(self._datastore.getbyteslice(startbyte, startbyte + 8)))
        elif length == 32:
            f, = struct.unpack(b'>f', self._readbytes(32, start))
        elif length == 64:
            f, = struct.unpack(b'>d', self._readbytes(64, start))
        try:
            return f
        except NameError:
            raise InterpretError(b'floats can only be 32 or 64 bits long, not {0} bits', length)
    ...
    def _setue(self, i):
        if i < 0:
            raise CreationError(b'Cannot use negative initialiser for unsigned exponential-Golomb.')
        if not i:
            self._setbin_unsafe(b'1')
            return
        tmp = i + 1
        leadingzeros = -1
        while tmp > 0:
        ...
        binstring = b'0' * leadingzeros + b'1' + Bits(uint=remainingpart, length=leadingzeros).bin
    ...
    def _readue(self, pos):
        ...
        try:
            ...
            raise ReadError(b'Read off end of bitstring trying to read code.')
    ...
    def _getue(self):
        try:
            ...
            raise InterpretError(b'Bitstring is not a single exponential-Golomb code.')
    ...
    def _setuie(self, i):
        if i < 0:
            raise CreationError(b'Cannot use negative initialiser for unsigned interleaved exponential-Golomb.')
        self._setbin_unsafe(b'1' if i == 0 else b'0' + (b'0').join(bin(i + 1)[3:]) + b'1')
        return
    ...
    def _getuie(self):
        try:
            ...
            raise InterpretError(b'Bitstring is not a single interleaved exponential-Golomb code.')
    ...
    def _setbool(self, value):
        if value in (1, b'True'):
            self._setbytes_unsafe(bytearray(b'\x80'), 1, 0)
        elif value in (0, b'False'):
            self._setbytes_unsafe(bytearray(b'\x00'), 1, 0)
        else:
            raise CreationError(b'Cannot initialise boolean with {0}.', value)
        return

    def _getbool(self):
        if self.length != 1:
            msg = b'For a bool interpretation a bitstring must be 1 bit long, not {0} bits.'
    ...
    def _setbin_safe(self, binstring):
        ...
        binstring = binstring.replace(b'0b', b'')
        self._setbin_unsafe(binstring)
        return
    ...
    def _setbin_unsafe(self, binstring):
        ...
        padded_binstring = binstring + b'0' * (boundary - length) if len(binstring) < boundary else binstring
        try:
            bytelist = [int(padded_binstring[x:x + 8], 2) for x in xrange(0, len(padded_binstring), 8)]
        except ValueError:
            raise CreationError(b'Invalid character in bin initialiser {0}.', binstring)
    ...
    def _readbin(self, length, start):
        ...
        c = (b'{:0{}b}').format(int(binascii.hexlify(b), 16), 8 * len(b))
    ...
    def _setoct(self, octstring):
        ...
        octstring = octstring.replace(b'0o', b'')
        binlist = []
        for i in octstring:
            try:
                binlist.append(OCT_TO_BITS[int(i)])
            except (ValueError, IndexError):
                raise CreationError(b"Invalid symbol '{0}' in oct initialiser.", i)

        self._setbin_unsafe((b'').join(binlist))
        return

    def _readoct(self, length, start):
        if length % 3:
            raise InterpretError(b'Cannot convert to octal unambiguously - not multiple of 3 bits.')
        if not length:
            return b''
        end = oct(self._readuint(length, start))[LEADING_OCT_CHARS:]
        if end.endswith(b'L'):
            end = end[:-1]
        middle = b'0' * (length // 3 - len(end))
        return middle + end
    ...
    def _sethex(self, hexstring):
        ...
        hexstring = hexstring.replace(b'0x', b'')
        ...
        try:
            ...
            raise CreationError(b'Invalid symbol in hex initialiser.')
    ...
    def _readhex(self, length, start):
        if length % 4:
            raise InterpretError(b'Cannot convert to hex unambiguously - not multiple of 4 bits.')
        if not length:
            return b''
        ...
        try:
            ...
            s = str(binascii.hexlify(s).decode(b'utf-8'))
    ...
    def _readtoken(self, name, pos, length):
        ...
        if length is not None and int(length) > self.length - pos:
            raise ReadError((b'Reading off the end of the data. Tried to read {0} bits when only {1} available.').format(int(length), self.length - pos))
        try:
            ...
            raise ValueError((b"Can't parse token {0}:{1}").format(name, length))
        except TypeError:
    ...
    def _validate_slice_msb0(self, start, end):
        ...
        if not 0 <= end <= self.len:
            raise ValueError(b'end is not a valid position in the bitstring.')
        if not 0 <= start <= self.len:
            raise ValueError(b'start is not a valid position in the bitstring.')
        if end < start:
            raise ValueError(b'end must not be less than start.')
        return (
         start, end)
    ...
    def _readlist(self, fmt, pos, **kwargs):
        ...
        for i, f in enumerate(fmt):
            if isinstance(f, numbers.Integral):
                fmt[i] = (b'bits:{0}').format(f)

        for f_item in fmt:
            stretchy, tkns = tokenparser(f_item, tuple(sorted(kwargs.keys())))
            if stretchy:
                if stretchy_token:
                    raise Error(b"It's not possible to have more than one 'filler' token.")
        ...
        if not stretchy_token:
            ...
            for name, length, _ in tokens:
                ...
                if name in kwargs and length is None:
                    value, pos = self._readtoken(b'uint', pos, kwargs[name])
                    lst.append(value)
                    continue
            ...
            for token in tokens:
                ...
                if stretchy_token:
                    if name in (b'se', b'ue', b'sie', b'uie'):
                        raise Error(b"It's not possible to parse a variablelength token after a 'filler' token.")
                    elif length is None:
                        raise Error(b"It's not possible to have more than one 'filler' token.")
    ...
    def _find_msb0(self, bs, start=None, end=None, bytealigned=None):
        bs = Bits(bs)
        if not bs.len:
            raise ValueError(b'Cannot find an empty bitstring.')
        start, end = self._validate_slice(start, end)
        if bytealigned is None:
            bytealigned = globals()[b'bytealigned']
    ...
    def findall(self, bs, start=None, end=None, count=None, bytealigned=None):
        if count is not None and count < 0:
            raise ValueError(b'In findall, count must be >= 0.')
        bs = Bits(bs)
        start, end = self._validate_slice(start, end)
        if bytealigned is None:
            bytealigned = globals()[b'bytealigned']
    ...
    def cut(self, bits, start=None, end=None, count=None):
        start, end = self._validate_slice(start, end)
        if count is not None and count < 0:
            raise ValueError(b'Cannot cut - count must be >= 0.')
        if bits <= 0:
            raise ValueError(b'Cannot cut - bits must be >= 0.')
        c = 0
    ...
    def split(self, delimiter, start=None, end=None, count=None, bytealigned=None):
        ...
        if not delimiter.len:
            raise ValueError(b'split delimiter cannot be empty.')
        start, end = self._validate_slice(start, end)
        if bytealigned is None:
            bytealigned = globals()[b'bytealigned']
        if count is not None and count < 0:
            raise ValueError(b'Cannot split - count must be >= 0.')
        if count == 0:
            return
        else:
    ...
    def all(self, value, pos=None):
        ...
        for p in pos:
            if p < 0:
                p += length
            if not 0 <= p < length:
                raise IndexError((b'Bit position {0} out of range.').format(p))
    ...
    len = property(_getlength, doc=b'The length of the bitstring in bits. Read only.\n                      ')
    length = property(_getlength, doc=b'The length of the bitstring in bits. Read only.\n                      ')
    bool = property(_getbool, doc=b'The bitstring as a bool (True or False). Read only.\n                    ')
    hex = property(_gethex, doc=b'The bitstring as a hexadecimal string. Read only.\n                   ')
    bin = property(_getbin, doc=b'The bitstring as a binary string. Read only.\n                   ')
    oct = property(_getoct, doc=b'The bitstring as an octal string. Read only.\n                   ')
    bytes = property(_getbytes, doc=b'The bitstring as a bytes object. Read only.\n                      ')
    int = property(_getint, doc=b"The bitstring as a two's complement signed int. Read only.\n                      ")
    uint = property(_getuint, doc=b"The bitstring as a two's complement unsigned int. Read only.\n                      ")
    float = property(_getfloat, doc=b'The bitstring as a floating point number. Read only.\n                      ')
    intbe = property(_getintbe, doc=b"The bitstring as a two's complement big-endian signed int. Read only.\n                      ")
    uintbe = property(_getuintbe, doc=b"The bitstring as a two's complement big-endian unsigned int. Read only.\n                      ")
    floatbe = property(_getfloat, doc=b'The bitstring as a big-endian floating point number. Read only.\n                      ')
    intle = property(_getintle, doc=b"The bitstring as a two's complement little-endian signed int. Read only.\n                      ")
    uintle = property(_getuintle, doc=b"The bitstring as a two's complement little-endian unsigned int. Read only.\n                      ")
    floatle = property(_getfloatle, doc=b'The bitstring as a little-endian floating point number. Read only.\n                      ')
    intne = property(_getintne, doc=b"The bitstring as a two's complement native-endian signed int. Read only.\n                      ")
    uintne = property(_getuintne, doc=b"The bitstring as a two's complement native-endian unsigned int. Read only.\n                      ")
    floatne = property(_getfloatne, doc=b'The bitstring as a native-endian floating point number. Read only.\n                      ')
    ue = property(_getue, doc=b'The bitstring as an unsigned exponential-Golomb code. Read only.\n                      ')
    se = property(_getse, doc=b'The bitstring as a signed exponential-Golomb code. Read only.\n                      ')
    uie = property(_getuie, doc=b'The bitstring as an unsigned interleaved exponential-Golomb code. Read only.\n                      ')
    sie = property(_getsie, doc=b'The bitstring as a signed interleaved exponential-Golomb code. Read only.\n                      ')
...
class BitArray(Bits):
    ...
    def __setitem__(self, key, value):
        try:
            ...
            if isinstance(value, numbers.Integral):
                ...
                if value in (1, -1):
                    self._set(key)
                    return
                raise ValueError((b'Cannot set a single bit with integer {0}.').format(value))
            value = Bits(value)
            if value.len == 1:
                if value[0]:
                    self._set(key)
        ...
        if step != 1:
            ...
            self._setbin_unsafe((b'').join(temp))
            return
        else:
            if not isinstance(value, numbers.Integral):
                try:
                    ...
                    raise TypeError((b'Bitstring, integer or string expected. Got {0}.').format(type(value)))
    ...
    def replace(self, old, new, start=None, end=None, count=None, bytealigned=None):
        ...
        new = Bits(new)
        if not old.len:
            raise ValueError(b'Empty bitstring cannot be replaced.')
        start, end = self._validate_slice(start, end)
        if bytealigned is None:
            bytealigned = globals()[b'bytealigned']
        ...
        if not bs.len:
            ...
            if pos is None:
                try:
                    ...
                    raise TypeError(b'insert needs a bit position specified when used on a BitArray.')

            if pos < 0:
                pos += self.len
            if not 0 <= pos <= self.len:
                raise ValueError(b'Invalid insert position.')
            self._insert(bs, pos)
            return
        ...
        if not bs.len:
            ...
            if pos is None:
                try:
                    ...
                    raise TypeError(b'overwrite needs a bit position specified when used on a BitArray.')

            if pos < 0:
                pos += self.len
            if pos < 0 or pos + bs.len > self.len:
                raise ValueError(b'Overwrite exceeds boundary of bitstring.')
    ...
    def set(self, value, pos=None):
        ...
        try:
            ...
            if pos < 0:
                pos += self.len
            if not 0 <= pos < length:
                raise IndexError((b'Bit position {0} out of range.').format(pos))
            f(pos)
    ...
    def ror(self, bits, start=None, end=None):
        if not self.len:
            raise Error(b'Cannot rotate an empty bitstring.')
        if bits < 0:
            raise ValueError(b'Cannot rotate by negative amount.')
    ...
    def byteswap(self, fmt=None, start=None, end=None, repeat=True):
        ...
        if fmt is None or fmt == 0:
            ...
            if fmt < 0:
                raise ValueError((b'Improper byte length {0}.').format(fmt))
            bytesizes = [
             fmt]
        elif isinstance(fmt, basestring):
            m = STRUCT_PACK_RE.match(fmt)
            if not m:
                raise ValueError((b'Cannot parse format string {0}.').format(fmt))
            formatlist = re.findall(STRUCT_SPLIT_RE, m.group(b'fmt'))
            bytesizes = []
            for f in formatlist:
                if len(f) == 1:
            ...
            for bytesize in bytesizes:
                if not isinstance(bytesize, numbers.Integral) or bytesize < 0:
                    raise ValueError((b'Improper byte length {0}.').format(bytesize))

        else:
            raise TypeError(b'Format must be an integer, string or iterable.')
        repeats = 0
    ...
    int = property(Bits._getint, Bits._setint, doc=b"The bitstring as a two's complement signed int. Read and write.\n                      ")
    uint = property(Bits._getuint, Bits._setuint, doc=b"The bitstring as a two's complement unsigned int. Read and write.\n                      ")
    float = property(Bits._getfloat, Bits._setfloat, doc=b'The bitstring as a floating point number. Read and write.\n                      ')
    intbe = property(Bits._getintbe, Bits._setintbe, doc=b"The bitstring as a two's complement big-endian signed int. Read and write.\n                      ")
    uintbe = property(Bits._getuintbe, Bits._setuintbe, doc=b"The bitstring as a two's complement big-endian unsigned int. Read and write.\n                      ")
    floatbe = property(Bits._getfloat, Bits._setfloat, doc=b'The bitstring as a big-endian floating point number. Read and write.\n                      ')
    intle = property(Bits._getintle, Bits._setintle, doc=b"The bitstring as a two's complement little-endian signed int. Read and write.\n                      ")
    uintle = property(Bits._getuintle, Bits._setuintle, doc=b"The bitstring as a two's complement little-endian unsigned int. Read and write.\n                      ")
    floatle = property(Bits._getfloatle, Bits._setfloatle, doc=b'The bitstring as a little-endian floating point number. Read and write.\n                      ')
    intne = property(Bits._getintne, Bits._setintne, doc=b"The bitstring as a two's complement native-endian signed int. Read and write.\n                      ")
    uintne = property(Bits._getuintne, Bits._setuintne, doc=b"The bitstring as a two's complement native-endian unsigned int. Read and write.\n                      ")
    floatne = property(Bits._getfloatne, Bits._setfloatne, doc=b'The bitstring as a native-endian floating point number. Read and write.\n                      ')
    ue = property(Bits._getue, Bits._setue, doc=b'The bitstring as an unsigned exponential-Golomb code. Read and write.\n                      ')
    se = property(Bits._getse, Bits._setse, doc=b'The bitstring as a signed exponential-Golomb code. Read and write.\n                      ')
    uie = property(Bits._getuie, Bits._setuie, doc=b'The bitstring as an unsigned interleaved exponential-Golomb code. Read and write.\n                      ')
    sie = property(Bits._getsie, Bits._setsie, doc=b'The bitstring as a signed interleaved exponential-Golomb code. Read and write.\n                      ')
    hex = property(Bits._gethex, Bits._sethex, doc=b'The bitstring as a hexadecimal string. Read and write.\n                       ')
    bin = property(Bits._getbin, Bits._setbin_safe, doc=b'The bitstring as a binary string. Read and write.\n                       ')
    oct = property(Bits._getoct, Bits._setoct, doc=b'The bitstring as an octal string. Read and write.\n                       ')
    bool = property(Bits._getbool, Bits._setbool, doc=b'The bitstring as a bool (True or False). Read and write.\n                    ')
    bytes = property(Bits._getbytes, Bits._setbytes_safe, doc=b'The bitstring as a ordinary string. Read and write.\n                      ')
...
class ConstBitStream(Bits):
    ...
    def __new__(cls, auto=None, length=None, offset=None, pos=0, **kwargs):
        ...
        if x._pos < 0 or x._pos > x._datastore.bitlength:
            raise CreationError((b'Cannot set pos to {0} when length is {1}').format(pos, x._datastore.bitlength))
        return x
    ...
    def _getbytepos(self):
        if self._pos % 8:
            raise ByteAlignError(b'Not byte aligned in _getbytepos().')
        return self._pos // 8

    def _setbitpos(self, pos):
        if pos < 0:
            raise ValueError(b'Bit position cannot be negative.')
        if pos > self.len:
            raise ValueError(b'Cannot seek past the end of the data.')
        self._pos = pos
        return
    ...
    def read(self, fmt):
        if isinstance(fmt, numbers.Integral):
            if fmt < 0:
                raise ValueError(b'Cannot read negative amount.')
            if fmt > self.len - self._pos:
                raise ReadError(b'Cannot read {0} bits, only {1} available.', fmt, self.len - self._pos)
            ...
            if len(token) != 1:
                self._pos = p
                raise ValueError((b'Format string should be a single token, not {0} tokens - use readlist() instead.').format(len(token)))
    ...
    def readto(self, bs, bytealigned=None):
        if isinstance(bs, numbers.Integral):
            raise ValueError(b'Integers cannot be searched for')
        bs = Bits(bs)
        oldpos = self._pos
        p = self.find(bs, self._pos, bytealigned=bytealigned)
        if not p:
            raise ReadError(b'Substring not found')
        self._pos += bs.len
    ...
    pos = property(_getbitpos, _setbitpos, doc=b'The position in the bitstring in bits. Read and write.\n                      ')
    bitpos = property(_getbitpos, _setbitpos, doc=b'The position in the bitstring in bits. Read and write.\n                      ')
    bytepos = property(_getbytepos, _setbytepos, doc=b'The position in the bitstring in bytes. Read and write.\n                      ')
...
def pack(fmt, *values, **kwargs):
    ...
    try:
        for name, length, value in tokens:
            ...
            if length is not None:
                length = int(length)
            if value is None and name != b'pad':
                value = next(value_iter)
            s._addright(BitStream._init_with_token(name, length, value))

    except StopIteration:
        raise CreationError(b'Not enough parameters present to pack according to the format. {0} values are needed.', len(tokens))
    ...
    raise CreationError(b'Too many parameters present to pack according to the format.')
    return
...
def _switch_lsb0_methods(lsb0):
    ...
    name_to_read = {b'uint': (Bits._readuint), b'uintle': (Bits._readuintle), 
       b'uintbe': (Bits._readuintbe), 
       b'uintne': (Bits._readuintne), 
       b'int': (Bits._readint), 
       b'intle': (Bits._readintle), 
       b'intbe': (Bits._readintbe), 
       b'intne': (Bits._readintne), 
       b'float': (Bits._readfloat), 
       b'floatbe': (Bits._readfloat), 
       b'floatle': (Bits._readfloatle), 
       b'floatne': (Bits._readfloatne), 
       b'hex': (Bits._readhex), 
       b'oct': (Bits._readoct), 
       b'bin': (Bits._readbin), 
       b'bits': (Bits._readbits), 
       b'bytes': (Bits._readbytes), 
       b'ue': (Bits._readue), 
       b'se': (Bits._readse), 
       b'uie': (Bits._readuie), 
       b'sie': (Bits._readsie), 
       b'bool': (Bits._readbool)}
    return
...
set_msb0()
init_with_length_and_offset = {b'bytes': (Bits._setbytes_safe), b'filename': (Bits._setfile)}
init_with_length_only = {b'uint': (Bits._setuint), b'int': (Bits._setint), 
   b'float': (Bits._setfloat), 
   b'uintbe': (Bits._setuintbe), 
   b'intbe': (Bits._setintbe), 
   b'floatbe': (Bits._setfloat), 
   b'uintle': (Bits._setuintle), 
   b'intle': (Bits._setintle), 
   b'floatle': (Bits._setfloatle), 
   b'uintne': (Bits._setuintne), 
   b'intne': (Bits._setintne), 
   b'floatne': (Bits._setfloatne)}
init_without_length_or_offset = {b'bin': (Bits._setbin_safe), b'hex': (Bits._sethex), 
   b'oct': (Bits._setoct), 
   b'ue': (Bits._setue), 
   b'se': (Bits._setse), 
   b'uie': (Bits._setuie), 
   b'sie': (Bits._setsie), 
   b'bool': (Bits._setbool)}
ConstBitArray = Bits
BitString = BitStream
__all__ = [
 121, 112, 114, 110, 
