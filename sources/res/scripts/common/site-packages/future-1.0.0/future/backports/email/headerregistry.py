...
class BaseHeader(str):
    ...
    def fold(self, **_3to2kwargs):
        ...
        header = parser.Header([
         parser.HeaderLabel([
          parser.ValueTerminal(self.name, u'header-name'),
          parser.ValueTerminal(u':', u'header-sep')]),
         parser.CFWSList([parser.WhiteSpaceTerminal(u' ', u'fws')]),
         self._parse_tree])
...
class AddressHeader(object):
    ...
    def parse(cls, value, kwds):
        ...
        elif not hasattr(value, u'__iter__'):
            value = [
             value]
        groups = [(hasattr(item, u'addresses') or Group)(None, [item]) if 1 else item for item in value]
        defects = []
