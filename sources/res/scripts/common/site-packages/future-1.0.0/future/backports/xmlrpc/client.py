...
class ProtocolError(Error):
    ...
    def __repr__(self):
        return u'<ProtocolError for %s: %s %s>' % (
...
class Fault(Error):
    ...
    def __repr__(self):
        return u'<Fault %s: %r>' % (ensure_new_type(self.faultCode),
...
class DateTime(object):
    ...
    def make_comparable(self, other):
        if isinstance(other, DateTime):
            ...
            raise TypeError(u"Can't compare %s and %s" % (
...
class Marshaller(object):
    ...
    def dumps(self, values):
        ...
        if isinstance(values, Fault):
            write(u'<fault>\n')
            dump({u'faultCode': (values.faultCode), u'faultString': (values.faultString)}, write)
            write(u'</fault>\n')
        else:
            write(u'<params>\n')
            for v in values:
...
class MultiCall(object):
    ...
    def __call__(self):
        ...
        for name, args in self.__call_list:
            marshalled_list.append({u'methodName': name, u'params': args})
