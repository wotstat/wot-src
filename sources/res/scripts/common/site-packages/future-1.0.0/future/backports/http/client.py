...
def parse_headers(fp, _class=HTTPMessage):
    ...
    hstring = bytes(b'').join(headers).decode(u'iso-8859-1')
...
class HTTPConnection(object):
    ...
    def _tunnel(self):
        ...
        if code != 200:
            self.close()
            raise socket.error(u'Tunnel connection failed: %d %s' % (code,
             message.strip()))
        while True:
...
class LineTooLong(HTTPException):
    ...
    def __init__(self, line_type):
        HTTPException.__init__(self, u'got more than %d bytes when reading %s' % (
         _MAXLINE, line_type))
        return
