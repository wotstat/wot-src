...
class newstr(with_metaclass(BaseNewStr, unicode)):
    no_convert_msg = b"Can't convert '{0}' object to str implicitly"
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if isinstance(args[0], unicode):
            ...
            if b'encoding' in kwargs or len(args) > 1:
    ...
    def __contains__(self, key):
        errmsg = b"'in <string>' requires string as left operand, not {0}"
        if type(key) == newstr:
            newkey = key
    ...
    def join(self, iterable):
        errmsg = b'sequence item {0}: expected unicode string, found bytes'
    ...
    def decode(self, *args):
        raise AttributeError(b'decode method has been disabled in newstr')
        return

    def encode(self, encoding=b'utf-8', errors=b'strict'):
        from future.types.newbytes import newbytes
        if errors == b'surrogateescape':
            if encoding == b'utf-16':
                raise NotImplementedError(b'FIXME: surrogateescape handling is not yet implemented properly')
            mybytes = []
            for c in self:
                code = ord(c)
                if 55296 <= code <= 56575:
    ...
    unorderable_err = b'unorderable types: str() and {0}'
    ...
    def maketrans(x, y=None, z=None):
        if y is None:
            if not isinstance(x, dict):
                raise TypeError(b'if you give only one argument to maketrans it must be a dict')
            result = {}
            for key, value in x.items():
                if len(key) > 1:
                    raise ValueError(b'keys in translate table must be strings or integers')
                result[ord(key)] = value

        elif not isinstance(x, unicode) and isinstance(y, unicode):
            raise TypeError(b'x and y must be unicode strings')
        if not len(x) == len(y):
            raise ValueError(b'the first two maketrans arguments must have equal length')
        result = {}
    ...
    def isprintable(self):
        raise NotImplementedError(b'fixme')
        return
