...
class newrange(Sequence):
    ...
    def __init__(self, *args):
        if len(args) == 1:
            ...
            raise TypeError(b'range() requires 1-3 int arguments')
        try:
        ...
        if step == 0:
            raise ValueError(b'range() arg 3 must not be zero')
        elif step < 0:
            stop = min(stop, start)
        else:
            stop = max(stop, start)
    ...
    def __repr__(self):
        if self._step == 1:
            return b'range(%d, %d)' % (self._start, self._stop)
        return b'range(%d, %d, %d)' % (self._start, self._stop, self._step)
    ...
    def index(self, value):
        try:
            ...
            raise ValueError(b'%r is not in range' % value)
    ...
    def __getitem__(self, index):
        ...
        if index < 0 or index >= self._len:
            raise IndexError(b'range object index out of range')
