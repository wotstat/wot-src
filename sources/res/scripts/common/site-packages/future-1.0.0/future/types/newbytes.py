...
class newbytes(with_metaclass(BaseNewBytes, _builtin_bytes)):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if len(args) == 0:
            ...
            if isinstance(args[0], _builtin_bytes):
                ...
                try:
                    if b'encoding' in kwargs:
                        encoding = kwargs[b'encoding']
                    if b'errors' in kwargs:
                        errors = kwargs[b'errors']
                except AssertionError:
                    raise TypeError(b'Argument given by name and position')

                if encoding is None:
                    raise TypeError(b'unicode string argument without an encoding')
                newargs = [
                 encoding]
                if errors is not None:
                    newargs.append(errors)
                value = args[0].encode(*newargs)
            elif hasattr(args[0], b'__bytes__'):
                ...
                if len(args[0]) == 0:
                    ...
                    try:
                        ...
                        raise ValueError(b'bytes must be in range(0, 256)')

            elif isinstance(args[0], Integral):
                if args[0] < 0:
                    raise ValueError(b'negative count')
                value = b'\x00' * args[0]
            else:
                value = args[0]
    ...
    def __repr__(self):
        return b'b' + super(newbytes, self).__repr__()

    def __str__(self):
        return b'b' + (b"'{0}'").format(super(newbytes, self).__str__())
    ...
    def __mod__(self, vals):
        if isinstance(vals, newbytes):
            ...
            vals = tuple(newvals)
        elif hasattr(vals.__class__, b'__getitem__') and hasattr(vals.__class__, b'iteritems'):
    ...
    def join(self, iterable_of_bytes):
        errmsg = b'sequence item {0}: expected bytes, {1} found'
    ...
    def fromhex(cls, string):
        return cls(string.replace(b' ', b'').decode(b'hex'))
    ...
    def encode(self, *args):
        raise AttributeError(b'encode method has been disabled in newbytes')
        return

    def decode(self, encoding=b'utf-8', errors=b'strict'):
        from future.types.newstr import newstr
        if errors == b'surrogateescape':
    ...
    def rindex(self, sub, *args):
        ...
        if pos == -1:
            raise ValueError(b'substring not found')
        return
    ...
    unorderable_err = b'unorderable types: bytes() and {0}'
