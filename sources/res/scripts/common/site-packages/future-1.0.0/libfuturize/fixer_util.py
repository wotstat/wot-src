...
def canonical_fix_name(fix, avail_fixes):
    if b'.fix_' in fix:
        ...
        if fix.startswith(b'fix_'):
            fix = fix[4:]
        found = [f for f in avail_fixes if f.endswith((b'fix_{0}').format(fix))]
        if len(found) > 1:
            raise ValueError(b'Ambiguous fixer name. Choose a fully qualified module name instead from these:\n' + (b'\n').join(b'  ' + myf for myf in found))
        elif len(found) == 0:
            raise ValueError(b'Unknown fixer. Use --list-fixes or -l for a list.')
        return found[0]
...
def NameImport(package, as_name=None, prefix=None):
    if prefix is None:
        prefix = u''
    children = [
     Name(u'import', prefix=prefix), package]
...
def future_import(feature, node):
    ...
    if does_tree_import(u'__future__', feature, node):
        ...
        import_ = FromImport(u'__future__', [Leaf(token.NAME, feature, prefix=b' ')])
...
def touch_import_top(package, name_to_import, node):
    ...
    if does_tree_import(package, name_to_import, root):
        ...
        found = False
        for name in [b'absolute_import', b'division', b'print_function',
         b'unicode_literals']:
            if does_tree_import(b'__future__', name, root):
                found = True
                break
...
def check_future_import(node):
    ...
    if not (node.type == syms.import_from and hasattr(node.children[1], b'value') and node.children[1].value == u'__future__'):
        return set()
...
SHEBANG_REGEX = b'^#!.*python'
ENCODING_REGEX = b'^#.*coding[:=]\\s*([-\\w.]+)'
