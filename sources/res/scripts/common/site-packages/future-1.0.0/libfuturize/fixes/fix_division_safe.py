...
class FixDivisionSafe(fixer_base.BaseFix):
    ...
    def start_tree(self, tree, name):
        ...
        self.skip = b'division' in tree.future_features
        return
        ...
        if node.type == self.syms.term:
            ...
            for child in node.children:
                ...
                if match_division(child) and not is_floaty(child):
                    ...
                    children[0].prefix = u''
                    children = [
                     wrap_in_fn_call(b'old_div', children + [Comma(), child.next_sibling.clone()], prefix=node.prefix)]
                    skip = True
                else:
                    children.append(child.clone())

            if matched:
                if hasattr(Node, b'fixers_applied'):
