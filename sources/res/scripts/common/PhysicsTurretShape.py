...
class PhysicsTurretShape:
    ...
    PARAMS_DESC = {b'zScale': (0.0, 2.0, 1.0, 0), 
       b'zPos': (-2.0, 2.0, 0.0, 1), 
       b'xScale': (0.0, 2.0, 1.0, 2), 
       b'xPos': (-2.0, 2.0, 0.0, 3), 
       b'yScale': (0.0, 2.0, 1.0, 4), 
       b'yPos': (-2.0, 2.0, 0.0, 5), 
       b'zRounding': (0.0, 1.0, 0.5, 6), 
       b'zRoundingCenter': (0.0, 1.0, 0.5, 7), 
       b'xRounding': (0.0, 1.0, 0.5, 8), 
       b'xRoundingCenter': (0.0, 1.0, 0.5, 9), 
       b'xSlope': (0.0, 1.0, 0.5, 10), 
       b'xSlopeCenter': (0.0, 1.0, 0.5, 11), 
       b'zSlope': (0.0, 1.0, 0.6, 12), 
       b'zSlopeCenter': (0.0, 1.0, 0.1, 13), 
       b'topSlope': (0.0, 1.0, 0.5, 14)}
    ...
    def __recalcPolys(self):
        ...
        h = bbMax.y - bbMin.y
        l = bbMax.z - bbMin.z
        sw = w * params[b'xScale']
        sh = h * params[b'yScale']
        sl = l * params[b'zScale']
        centerY = bbMin.y + h * 0.5 + params[b'yPos']
        minY = centerY - sh * 0.5
        maxY = centerY + sh * 0.5
        centerZ = bbMin.z + l * 0.5 + params[b'zPos']
        minZ = centerZ - sl * 0.5
        maxZ = centerZ + sl * 0.5
        centerX = bbMin.x + w * 0.5 + params[b'xPos']
        minX = centerX - sw * 0.5
        maxX = centerX + sw * 0.5
        roundedDeltaZ = sl * params[b'zRounding']
        alignedDeltaZ = sl - roundedDeltaZ
        roundingCenterZ = minZ + roundedDeltaZ * params[b'zRoundingCenter'] + alignedDeltaZ * 0.5
        alignedMinZ = roundingCenterZ - alignedDeltaZ * 0.5
        alignedMaxZ = roundingCenterZ + alignedDeltaZ * 0.5
        roundedDeltaX = sw * params[b'xRounding']
        alignedDeltaX = sw - roundedDeltaX
        roundingCenterX = minX + roundedDeltaX * params[b'xRoundingCenter'] + alignedDeltaX * 0.5
        alignedMinX = roundingCenterX - alignedDeltaX * 0.5
        alignedMaxX = roundingCenterX + alignedDeltaX * 0.5
        zTopScale = 1.0 - params[b'zSlope']
        topL = zTopScale * sl
        roundedTopDeltaZ = roundedDeltaZ * zTopScale
        alignedTopDeltaZ = alignedDeltaZ * zTopScale
        topCenterZ = minZ + topL * 0.5 + params[b'zSlopeCenter'] * params[b'zSlope'] * sl
        topMinZ = topCenterZ - topL * 0.5
        topMaxZ = topCenterZ + topL * 0.5
        topRoundingCenterZ = topMinZ + roundedTopDeltaZ * params[b'zRoundingCenter'] + alignedTopDeltaZ * 0.5
        topAlignedMinZ = topRoundingCenterZ - alignedTopDeltaZ * 0.5
        topAlignedMaxZ = topRoundingCenterZ + alignedTopDeltaZ * 0.5
        xTopScale = 1.0 - params[b'xSlope']
        topW = xTopScale * sw
        roundedTopDeltaX = roundedDeltaX * xTopScale
        alignedTopDeltaX = alignedDeltaX * xTopScale
        topCenterX = minX + topW * 0.5 + params[b'xSlopeCenter'] * params[b'xSlope'] * sw
        topMinX = topCenterX - topW * 0.5
        topMaxX = topCenterX + topW * 0.5
        topRoundingCenterX = topMinX + roundedTopDeltaX * params[b'xRoundingCenter'] + alignedTopDeltaX * 0.5
        ...
        pt.y = minY + (1.0 - params[b'topSlope']) * (maxY - minY)
        plPt = verts[8]
        v2 = pt - plPt
        if v2.length < 0.001:
...
class _Scene:
    ...
    def __init__(self, name=b'scene', height=400, width=400):
        self.name = name
        self.items = []
        self.height = height
        self.width = width
    ...
    def strarray(self):
        var = [b'<?xml version="1.0"?>\n',
         b'<svg height="%d" width="%d" >\n' % (self.height, self.width),
         b' <g style="fill-opacity:1.0; stroke:black;\n',
         b'  stroke-width:1;">\n']
    ...
    def display(self, prog=b'explorer'):
        import os
        os.system(b'%s %s' % (prog, self.svgname))
        return
