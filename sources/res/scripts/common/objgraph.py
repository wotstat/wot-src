...
__author__ = b'Marius Gedminas (marius@gedmin.as)'
__copyright__ = b'Copyright (c) 2008-2017 Marius Gedminas and contributors'
__license__ = b'MIT'
__version__ = b'3.4.1'
__date__ = b'2019-04-23'
try:
    basestring
except NameError:
    basestring = str
...
try:
    import graphviz
    if get_ipython().__class__.__name__ != b'TerminalInteractiveShell':
        IS_INTERACTIVE = True
...
def show_growth(limit=10, peak_stats=None, shortnames=True, file=None, filter=None):
    ...
    if result:
        ...
        for name, stats_count, delta in result:
            file.write(b'%-*s%9d %+9d\n' % (width, name, stats_count, delta))

    return


def get_new_ids(skip_update=False, limit=10, sortby=b'deltas', shortnames=None, file=None, _state={}):
    if not _state:
        _state[b'old'] = collections.defaultdict(set)
        _state[b'current'] = collections.defaultdict(set)
        _state[b'new'] = collections.defaultdict(set)
        _state[b'shortnames'] = True
    new_ids = _state[b'new']
    if skip_update:
        return new_ids
    else:
        old_ids = _state[b'old']
        current_ids = _state[b'current']
        if shortnames is None:
            shortnames = _state[b'shortnames']
        else:
            _state[b'shortnames'] = shortnames
        gc.collect()
        objects = gc.get_objects()
        ...
        index_by_sortby = {b'old': 1, b'current': 2, b'new': 3, b'deltas': 4}
        ...
        print(b'=' * (width + 52), file=file)
        print(b'%-*s%13s%13s%13s%13s' % (
         width, b'Type', b'Old_ids', b'Current_ids', b'New_ids', b'Count_Deltas'), file=file)
        print(b'=' * (width + 52), file=file)
        for row_class, old, current, new, delta in rows:
            print(b'%-*s%13d%13d%+13d%+13d' % (
...
def show_chain(*chains, **kw):
    backrefs = kw.pop(b'backrefs', True)
...
def is_proper_module(obj):
    return inspect.ismodule(obj) and obj is sys.modules.get(getattr(obj, b'__name__', None))
...
def _show_graph(objs, edge_func, swap_source_target, max_depth=3, extra_ignore=(), filter=None, too_many=10, highlight=None, filename=None, extra_info=None, refcounts=False, shortnames=True, output=None, cull_func=None):
    ...
    if filename and output:
        raise ValueError(b'Cannot specify both output and filename.')
    if output:
        f = output
    elif filename and filename.endswith(b'.dot'):
        f = codecs.open(filename, b'w', encoding=b'utf-8')
        dot_filename = filename
    elif IS_INTERACTIVE:
        is_interactive = True
        f = StringIO()
    else:
        fd, dot_filename = tempfile.mkstemp(prefix=b'objgraph-', suffix=b'.dot', text=True)
        f = os.fdopen(fd, b'w')
        if getattr(f, b'encoding', None):
            import io
            f = io.TextIOWrapper(f.detach(), b'utf-8')
    f.write(b'digraph ObjectGraph {\n  node[shape=box, style=filled, fillcolor=white];\n')
    queue = []
    depth = {}
    ignore = set(extra_ignore)
    ignore.add(id(objs))
    ...
    while queue:
        ...
        if highlight and highlight(target):
            ...
            s = 0.6
            v = 0.5 + v * 0.5
        f.write(b'  %s[fillcolor="%g,%g,%g"];\n' % (
         _obj_node_id(target), h, s, v))
        if v < 0.5:
            f.write(b'  %s[fontcolor=white];\n' % _obj_node_id(target))
        if hasattr(getattr(target, b'__class__', None), b'__del__'):
            f.write(b'  %s->%s_has_a_del[color=red,style=dotted,len=0.25,weight=10];\n' % (
             _obj_node_id(target),
             _obj_node_id(target)))
            f.write(b'  %s_has_a_del[label="__del__",shape=doublecircle,height=0.25,color=red,fillcolor="0,.5,1",fontsize=6];\n' % _obj_node_id(target))
        if tdepth >= max_depth:
            continue
        ...
        if skipped > 0:
            ...
            if swap_source_target:
                label = b'%d more references' % skipped
                edge = b'%s->too_many_%s' % (_obj_node_id(target),
                 _obj_node_id(target))
            else:
                label = b'%d more backreferences' % skipped
                edge = b'too_many_%s->%s' % (_obj_node_id(target),
                 _obj_node_id(target))
            f.write(b'  %s[color=red,style=dotted,len=0.25,weight=10];\n' % edge)
            f.write(b'  too_many_%s[label="%s",shape=box,height=0.25,color=red,fillcolor="%g,%g,%g",fontsize=6];\n' % (
             _obj_node_id(target), label, h, s, v))
            f.write(b'  too_many_%s[fontcolor=white];\n' % _obj_node_id(target))
    ...
    if output:
        ...
        f.close()
        print(b'Graph written to %s (%d nodes)' % (dot_filename, nodes))
...
def _present_graph(dot_filename, filename=None):
    if filename == dot_filename:
        return
    if not filename and _program_in_path(b'xdot'):
        print(b'Spawning graph viewer (xdot)')
        subprocess.Popen([b'xdot', dot_filename], close_fds=True)
    elif _program_in_path(b'dot'):
        if not filename:
            print(b'Graph viewer (xdot) not found, generating a png instead')
            filename = dot_filename[:-4] + b'.png'
        _, ext = os.path.splitext(filename)
        cmd = [b'dot', b'-T' + ext[1:], b'-o' + filename, dot_filename]
        dot = subprocess.Popen(cmd, close_fds=False)
        dot.wait()
        if dot.returncode != 0:
            print(b'dot failed (exit code %d) while executing "%s"' % (
             dot.returncode, (b' ').join(cmd)))
        else:
            print(b'Image generated as %s' % filename)
    elif not filename:
        print(b'Graph viewer (xdot) and image renderer (dot) not found, not doing anything else')
    else:
        print(b'Image renderer (dot) not found, not doing anything else')
    return


def _obj_node_id(obj):
    return (b'o%d' % id(obj)).replace(b'-', b'_')
...
def _obj_label(obj, extra_info=None, refcounts=False, shortnames=True):
    if shortnames:
        ...
        label = [
         _long_typename(obj)]
    if refcounts:
        label[0] += b' [%d]' % (sys.getrefcount(obj) - 4)
...
def _long_typename(obj):
    ...
    module = getattr(objtype, b'__module__', None)
    if module:
        return b'%s.%s' % (module, name)
    else:
        return name
...
def _short_repr(obj):
    ...
    if _isinstance(obj, types.MethodType):
        ...
        if obj.__self__:
            return name + b' (bound)'
        return name
    if _isinstance(obj, types.LambdaType) and obj.__name__ == b'<lambda>':
        return b'lambda: %s:%s' % (os.path.basename(obj.__code__.co_filename),
         obj.__code__.co_firstlineno)
    if _isinstance(obj, types.FrameType):
        return b'%s:%s' % (obj.f_code.co_filename, obj.f_lineno)
    if _isinstance(obj, (tuple, list, dict, set)):
        return b'%d items' % len(obj)
    return repr(obj)[:40]
...
def _edge_label(source, target, shortnames=True):
    if _isinstance(target, dict) and target is getattr(source, b'__dict__', None):
        return b' [label="__dict__",weight=10]'
    else:
        if _isinstance(source, types.FrameType):
            if target is source.f_locals:
                return b' [label="f_locals",weight=10]'
            if target is source.f_globals:
                return b' [label="f_globals",weight=10]'
        if _isinstance(source, types.MethodType):
            try:
                if target is source.__self__:
                    return b' [label="__self__",weight=10]'
                if target is source.__func__:
                    return b' [label="__func__",weight=10]'
            except AttributeError:
                if target is source.im_self:
                    return b' [label="im_self",weight=10]'
                if target is source.im_func:
                    return b' [label="im_func",weight=10]'

        if _isinstance(source, types.FunctionType):
            for k in dir(source):
                if target is getattr(source, k):
                    return b' [label="%s",weight=10]' % _quote(k)

        if _isinstance(source, dict):
            for k, v in iteritems(source):
                if v is target:
                    if _isinstance(k, basestring) and _is_identifier(k):
                        return b' [label="%s",weight=2]' % _quote(k)
                    if shortnames:
                        tn = _short_typename(k)
                    else:
                        tn = _long_typename(k)
                    return b' [label="%s"]' % _quote(tn + b'\n' + _safe_repr(k))

        return b''


_is_identifier = re.compile(b'[a-zA-Z_][a-zA-Z_0-9]*$').match
...
def _program_in_path(program):
    ...
    path = [True for file in path if os.path.isfile(file) or os.path.isfile(file + b'.exe')]
    return bool(path)
