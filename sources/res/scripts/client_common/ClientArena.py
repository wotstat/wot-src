...
class ClientArena(object):
    __onUpdate = {(ARENA_UPDATE.SETTINGS): b'_ClientArena__onArenaSettingsUpdate', 
       (ARENA_UPDATE.VEHICLE_LIST): b'_ClientArena__onVehicleListUpdate', 
       (ARENA_UPDATE.VEHICLE_ADDED): b'_ClientArena__onVehicleAddedUpdate', 
       (ARENA_UPDATE.PERIOD): b'_ClientArena__onPeriodInfoUpdate', 
       (ARENA_UPDATE.STATISTICS): b'_ClientArena__onStatisticsUpdate', 
       (ARENA_UPDATE.VEHICLE_STATISTICS): b'_ClientArena__onVehicleStatisticsUpdate', 
       (ARENA_UPDATE.VEHICLE_KILLED): b'_ClientArena__onVehicleKilled', 
       (ARENA_UPDATE.AVATAR_READY): b'_ClientArena__onAvatarReady', 
       (ARENA_UPDATE.BASE_POINTS): b'_ClientArena__onBasePointsUpdate', 
       (ARENA_UPDATE.BASE_CAPTURED): b'_ClientArena__onBaseCaptured', 
       (ARENA_UPDATE.TEAM_KILLER): b'_ClientArena__onTeamKiller', 
       (ARENA_UPDATE.VEHICLE_UPDATED): b'_ClientArena__onVehicleUpdatedUpdate', 
       ...
       (ARENA_UPDATE.VEHICLE_RECOVERED): b'_ClientArena__onVehicleRecovered', 
       (ARENA_UPDATE.FOG_OF_WAR): b'_ClientArena__onFogOfWar', 
       (ARENA_UPDATE.RADAR_INFO_RECEIVED): b'_ClientArena__onRadarInfoReceived', 
       (ARENA_UPDATE.VEHICLE_DESCR): b'_ClientArena__onVehicleDescrUpdate'}
    ...
    def __init__(self, arenaUniqueID, arenaTypeID, arenaBonusType, arenaGuiType, arenaExtraData, spaceID):
        ...
        battleModifiersDescr = arenaExtraData.get(b'battleModifiersDescr', ()) if arenaExtraData else ()
    ...
    hasObservers = property((lambda self: any(b'observer' in v[b'vehicleType'].type.tags for v in self.__vehicles.itervalues() if v[b'vehicleType'] is not None) or ARENA_BONUS_TYPE_CAPS.checkAny(self.bonusType, ARENA_BONUS_TYPE_CAPS.SSR)))
    ...
    def __onArenaSettingsUpdate(self, argStr):
        ...
        LOG_DEBUG_DEV(b'__onArenaSettingsUpdate', arenaSettings)
    ...
    def __onVehicleListUpdate(self, argStr):
        ...
        LOG_DEBUG_DEV(b'__onVehicleListUpdate', vehiclesList)
    ...
    def __onVehicleAddedUpdate(self, argStr):
        ...
        LOG_DEBUG_DEV(b'__onVehicleAddedUpdate', infoAsTuple)
        vehID, info = self.__vehicleInfoAsDict(infoAsTuple)
    ...
    def __onVehicleDescrUpdate(self, argStr):
        ...
        extVehicleTypeData = {EXT_DATA_PROGRESSION_KEY: (info[b'vehPostProgression']), 
           EXT_DATA_SLOT_KEY: (info[b'customRoleSlotTypeId']), 
           EXT_DATA_MODIFIERS_KEY: (self.battleModifiers)}
        self.__vehicles[vehID][b'vehicleType'] = self.__getVehicleType(compactDescr, extVehicleTypeData)
        self.__vehicles[vehID][b'maxHealth'] = maxHealth
        self.onVehicleUpdated(vehID)
        return
    ...
    def __onViewPoints(self, argStr):
        ...
        LOG_DEBUG(b'[VIEW POINTS] received view points', self.__viewPoints)
    ...
    def __onInteractiveStats(self, argStr):
        ...
        LOG_DEBUG_DEV(b'[RESPAWN] onInteractiveStats', stats)
        return
    ...
    def __vehicleInfoAsDict(self, info):
        extVehicleTypeData = {EXT_DATA_PROGRESSION_KEY: (info[25]), 
           ...
           EXT_DATA_MODIFIERS_KEY: (self.battleModifiers)}
        infoAsDict = {b'vehicleType': (self.__getVehicleType(info[1], extVehicleTypeData)), 
           b'name': (info[2]), 
           b'team': (info[3]), 
           b'isAlive': (info[4]), 
           b'isAvatarReady': (info[5]), 
           b'isTeamKiller': (info[6]), 
           b'accountDBID': (info[7]), 
           b'clanAbbrev': (info[8]), 
           b'clanDBID': (info[9]), 
           b'prebattleID': (int(info[10])), 
           b'isPrebattleCreator': (bool(info[11])), 
           b'forbidInBattleInvitations': (bool(info[12])), 
           b'events': (info[13]), 
           b'igrType': (info[14]), 
           b'personalMissionIDs': (info[15]), 
           b'personalMissionInfo': (info[16]), 
           b'ranked': (info[17]), 
           b'outfitCD': (info[18]), 
           b'avatarSessionID': (info[19]), 
           b'wtr': (int(info[20])), 
           b'fakeName': (info[21]), 
           b'badges': (info[22]), 
           b'overriddenBadge': (info[23]), 
           b'maxHealth': (info[24]), 
           b'vehPostProgression': (info[25]), 
           b'customRoleSlotTypeId': (info[26]), 
           b'botDisplayStatus': (info[27]), 
           b'statTrackFrags': (info[28]), 
           b'selectedComp7Skill': (info[29])}
        return (
         info[0], infoAsDict)
    ...
    def __vehicleStatisticsAsDict(self, stats):
        return (
         stats[0], {b'frags': (stats[1]), b'teamKillFrags': (stats[2])})
