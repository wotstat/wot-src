...
class ClientUnit(UnitBase):
    ...
    def _unpackPlayer(self, packedOps):
        ...
        try:
            ...
            filtered = dict(item for item in self._players.iteritems() if item[1].get(b'role', 0) & UNIT_ROLE.INVITED == 0)
    ...
    def _giveLeadership(self, memberDBID):
        prevRoleFlags = self._players[memberDBID][b'role']
        UnitBase._giveLeadership(self, memberDBID)
        newRoleFlags = self._players[memberDBID][b'role']
        prev_creatorDBID = self._creatorDBID
