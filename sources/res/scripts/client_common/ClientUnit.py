...
_ExternalPrebattleExtra = namedtuple(b'ExternalPrebattleExtra', (b'rev',))
_ExternalPrebattleExtra.__new__.__defaults__ = (0,)
_FunRandomSquadExtra = namedtuple(b'FunRandomSquadExtra', (b'funEventID',))
...
class ClientUnit(UnitBase):
    ...
    def __init__(self, limitsDefs=None, slotDefs=None, slotCount=0, packedRoster=b'', extrasInit=None, packedUnit=b''):
    ...
    def getSelectedVehicleLevels(self):
        return sorted({vehInfo.vehLevel for vehInfo in viewvalues(self._vehicles)})
    ...
    def _unpackPlayer(self, packedOps):
        ...
        try:
            ...
            filtered = dict(item for item in viewitems(self._players) if item[1].get(b'role', 0) & UNIT_ROLE.INVITED == 0)
        ...
        if hasPlayer:
            LOG_DEBUG_DEV(b'onUnitPlayerInfoChanged', accountDBID, self.getPlayer(accountDBID))
            self.onUnitPlayerInfoChanged(accountDBID, self.getPlayer(accountDBID))
        return nextOps

    def _addPlayer(self, accountDBID, **kwargs):
        if b'role' in kwargs and kwargs[b'role'] & UNIT_ROLE.CREATOR == UNIT_ROLE.CREATOR and self._playerSlots.get(accountDBID) == LEADER_SLOT:
    ...
    def _changePlayerRole(self, accountDBID, roleFlags):
        ...
        if accountDBID in self._players:
            prevRoleFlags = self._players[accountDBID][b'role']
    ...
    def _giveLeadership(self, newLeaderDBID):
        prevRoleFlags = self._players[newLeaderDBID][b'role']
        UnitBase._giveLeadership(self, newLeaderDBID)
        newRoleFlags = self._players[newLeaderDBID][b'role']
        prev_creatorDBID = self._creatorDBID
