...
class FakeDataAccessor(base.BaseDataAccessor):
    ...
    @fake_method(example=(lambda params: [{b'status': (random.choice(params[1] or (b'active', b'declined', b'cancelled', b'accepted', b'expired', b'error', b'deleted'))), b'created_at': (datetime.now()), b'updated_at': (datetime.now()), b'sender_id': (random.randrange(1, 10000)), b'id': (random.randrange(1, 1000000)), b'account_id': (params[0]), b'clan_id': (random.randrange(1, 10000)), b'status_changer_id': (random.randrange(1, 10000)), b'comment': ((b'Welcome {}!').format(random.randrange(1, 10000)) if random.choice((1, 0)) else b'')} for i in range(random.randrange(0, 1000))]))
    @paginated_method
    ...
    @fake_method(example=(lambda params: [{b'status': (random.choice(params[1] or (b'active', b'declined', b'cancelled', b'accepted', b'expired', b'error', b'deleted'))), b'created_at': (datetime.now()), b'updated_at': (datetime.now()), b'sender_id': (random.randrange(1, 10000)), b'id': (random.randrange(1, 1000000)), b'account_id': (random.randrange(1, 10000)), b'clan_id': (params[0]), b'status_changer_id': (random.randrange(1, 10000)), b'comment': ((b'Welcome {}!').format(random.randrange(1, 10000)) if random.choice((1, 0)) else b'')} for i in range(random.randrange(0, 1000))]))
    @paginated_method
    ...
    def get_wgsh_common_unit_info(self, periphery_id, unit_id, rev, fields=None):
        return self._request_data(b'wgsh_common_unit_info', unit_id)

    @fake_method(example={b'available_reserves': {b'ARTILLERY_STRIKE': [], b'HIGH_CAPACITY_TRANSPORT': [], b'REQUISITION': [], b'AIRSTRIKE': []}, b'requisition_bonus_percent': None, 
       b'selected_reserves': [
                            None,
                            None,
                            None]})
    def get_wgsh_account_unit_info(self, periphery_id, unit_id, rev, fields=None):
        return self._request_data(b'wgsh_account_unit_info', unit_id)
    ...
    @fake_method(example={b'data': {b'promo_name': b'Bang bang bang', 
                 b'type': b'news', 
                 b'image': b'//webbrg-ru.wgcdn.co/dcont/fb/image/9.12.jpg', 
