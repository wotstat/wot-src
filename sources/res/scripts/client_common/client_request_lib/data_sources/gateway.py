import zlib, json, urllib, BigWorld
...
class GatewayDataAccessor(base.BaseDataAccessor):
        ...
        def wrapper(something):
            ...
            def wrapped(response, func=something):
                try:
                    ...
                    LOG_ERROR(b'Can not process request response')
                    data = None
                    headers = None
    ...
    def login(self, callback, account_id, spa_token, jwt):
        if jwt:
            ...
            auth_type = b'Basic'
            auth_data = b64encode((b':').join([str(account_id), str(spa_token)]))
    ...
    def _request_data(self, callback, url, get_data=None, method=b'GET', post_data=None, headers=None, converters=None):
        ...
        headers = tuple((b'{}: {}').format(k, v) for k, v in default_headers.iteritems() if v)
    ...
    def agate_v6_fetch_product_list_personal(self, callback, request_data, fields=None):
        url = b'/agate/api/v6/commerce/fetchProductListPersonal/'
        return self._request_data(callback, url, method=b'POST', post_data=request_data)

    def agate_v6_get_user_subscriptions(self, callback, request_data, fields=None):
        url = b'/agate/api/v6/commerce/getUserSubscriptions/'
        return self._request_data(callback, url, method=b'POST', post_data=request_data)
    ...
    def get_gift_system_wait_response(self, callback, req_event_id, meta_info):
        url = b'/giftsystem/wait_response/players'
        get_params = {b'event_id': req_event_id}
    ...
    def post_gift_system_gift_multiple(self, callback, entitlement_code, receiver_ids, meta_info):
        url = b'/giftsystem/gift/multiple'
        post_data = {b'entitlement_code': entitlement_code, b'receiver_ids': receiver_ids}
        post_data.update(meta_info)
        return self._request_data(callback, url, method=b'POST', post_data=post_data)
    ...
    def get_inventory_entitlements(self, callback, entitlement_codes):
        ...
        if entitlement_codes:
            urlencoded_string = urllib.urlencode([(b'entitlement_codes', code) for code in entitlement_codes])
            url = (b'{}?{}').format(url, urlencoded_string)
        return self._request_data(callback, url, method=b'GET')

    def get_storefront_products(self, callback, storefront, user_country):
        url = (b'/shop/storefront/{}/products/').format(storefront)
        headers = {b'X-User-Country': user_country}
        return self._request_data(callback, url, method=b'GET', headers=headers)

    def buy_storefront_products(self, callback, storefront, productCode, requestData, user_country):
        url = (b'shop/storefront/{}/buy/product/{}/').format(storefront, productCode)
        headers = {b'X-User-Country': user_country}
        return self._request_data(callback, url, method=b'POST', headers=headers, post_data=requestData)
    ...
    def get_statistic_lootbox(self, callback):
        url = b'/statbox/api/v1/get_statistic_lootbox'
        return self._request_data(callback, url)
