...
class CommonTankAppearance(ScriptGameObject):
    ...
    def __init__(self, spaceID):
        ...
        self._vehicle = None
        self.__wheelsGameObject = ScriptGameObject(spaceID, b'Tank.Wheels.Root')
        self.__filter = None
        ...
        self.__trackScrollCtl.setFlyingInfo(DataLinks.createBoolLink(self.flyingInfoProvider, b'isLeftSideFlying'), DataLinks.createBoolLink(self.flyingInfoProvider, b'isRightSideFlying'))
    ...
    def prerequisites(self, typeDescriptor, vID, health, isCrewActive, isTurretDetached, outfitCD, renderMode=None):
        ...
        for name, builders in physicalTracksBuilders.iteritems():
            for index, builder in enumerate(builders):
                prereqs.append(builder.createLoader(self.spaceID, (b'{0}{1}PhysicalTrack').format(name, index), skin))
    ...
    def construct(self, isPlayer, resourceRefs):
        ...
        self.collisions = self.createComponent(BigWorld.CollisionComponent, resourceRefs[b'collisionAssembler'])
        ...
        lodLink = DataLinks.createFloatLink(lodCalcInst, b'lodDistance')
        ...
        if not self.damageState.isCurrentModelDamaged:
            ...
            dirtEnabled = BigWorld.dirtEnabled() and b'HD' in self.typeDescriptor.type.tags
        ...
        self.prefabsResourceRefs = {}
        self.prefabsResourceRefs.update(resourceRefs[b'DefaultPrefabAttachments'])
        self.prefabsResourceRefs.update(resourceRefs[b'StylePrefabAttachments'])
    ...
    def activate(self):
        ...
        if hasattr(self.filter, b'placingCompensationMatrix') and self.swingingAnimator is not None:
    ...
    def _destroyStickers(self):
        _logger.debug(b'Attaching VehicleStickers for vehicleType: %s', self.typeDescriptor)
        self._detachStickers()
        self._vehicleStickers = None
        return
    ...
    def __assembleNonDamagedOnly(self, resourceRefs, isPlayer, lodLink, lodStateLink):
        ...
        if generalWheelsAnimatorConfig is not None:
            ...
            for _ in xrange(scrollableWheelsCount):
                ...
                wheelsScroll.append(DataLinks.createFloatLink(retriever, b'value'))
            ...
            for _ in xrange(steerableWheelsCount):
                retrieverGameObject = ScriptGameObject(self.spaceID)
                retriever = retrieverGameObject.createComponent(NetworkFilters.FloatFilterRetriever)
                wheelsSteering.append(DataLinks.createFloatLink(retriever, b'value'))
                self.filterRetrievers.append(retriever)
                self.filterRetrieverGameObjects.append(retrieverGameObject)
    ...
    def __assembleSwinging(self, lodLink):
        ...
        if hullNode is None:
            ...
            if hasattr(self.filter, b'placingCompensationMatrix'):
                self.swingingAnimator.placingCompensationMatrix = self.filter.placingCompensationMatrix
            return
    ...
    def _onCameraChanged(self, cameraName, currentVehicleId=None):
        if self.id != BigWorld.player().playerVehicleID:
            return
        isEnabled = not cameraName == b'sniper'
        for modelAnimator in self.__modelAnimators:
