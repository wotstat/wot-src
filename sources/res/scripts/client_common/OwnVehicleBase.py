...
Cooldowns = namedtuple(b'Cooldowns', [b'id', b'leftTime', b'baseTime'])
_DO_LOG = False
...
class OwnVehicleBase(BigWorld.DynamicScriptComponent):
    ...
    def onObservedByEnemy(self, isObserved):
        ...
        if not avatar:
            return
        if _DO_LOG:
            self._doLog((b'onObservedByEnemy {}').format(isObserved))
        ...
        if len(parts) == 2 and (parts[0] == b'set' or parts[0] == b'setNested' or parts[0] == b'setSlice'):
            if hasattr(self, parts[1]):
                ...
                if func:
                    ...
                    return attrFunc
        raise AttributeError((b'OwnVehicle does not have attrubure {}').format(item))
        return
