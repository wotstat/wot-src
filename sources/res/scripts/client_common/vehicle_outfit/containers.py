...
class OutfitContainer(object):
    ...
    def __str__(self):
        result = (b'OutfitContainer (areaID={}):').format(self._areaID)
...
class MultiSlot(object):
    __slots__ = (b'_regions', b'_slotTypes', b'_items', b'_locks')
    ...
    def __str__(self):
        result = (b'MultiSlot (slotType={}):').format(self._slotTypes)
    ...
    def _validateType(self, other):
        if type(self) is not type(other):
            raise SoftException(b'Comparable MultiSlots has different types', self, other)
        return
    ...
    def __isItemFitSlot(self, itemDescriptor, idx):
        if idx >= len(self._regions):
            _logger.warning(b'Invalid slot idx %s', idx)
            return False
        ...
        if not result:
            _logger.warning(b'item type %s does not correspond to allowed slot types %s', itemType, self._slotTypes)
        return result


class ProjectionDecalsMultiSlot(MultiSlot):
    __slots__ = (b'_limit', b'_order')
    ...
    def set(self, intCD, idx=0, component=None):
        if len(self._items) + (idx not in self._items) > self._limit:
            _logger.warning(b'MultiSlot is filled %s', self._slotTypes)
            return False
