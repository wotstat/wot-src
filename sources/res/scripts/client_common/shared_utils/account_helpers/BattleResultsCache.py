...
CACHE_DIR = os.path.join(os.path.dirname(unicode_from_utf8((constants.IS_BOT or BigWorld.getPreferencesFilePath)() if 1 else b'.')[1]), b'battle_results')
...
def getFolderName(uniqueFolderName, arenaUniqueID):
    ...
    return os.path.join(CACHE_DIR, base64.b32encode(b'%s;%s' % (uniqueFolderName, battleStartDay)))
