...
class SectorGroup(object):
    ...
    def __getWayPointsForSector(self, arenaType):
        ...
        if arenaType is None:
            ...
            if hasattr(arenaType, b'sectorWayPoints'):
                sectorWayPoints = arenaType.sectorWayPoints
                for point in sectorWayPoints:
                    if self.isInSector(point[b'position']):
                        if self.__wayPoints.get(point[b'team'], None) is None:
                            self.__wayPoints[point[b'team']] = list()
                        self.__wayPoints[point[b'team']].append(point[b'position'])
...
class SectorsArenaComponent(ClientArenaComponent):
    ...
    def getSectorById(self, sectorID):
        ...
        LOG_WARNING(b'Requested Sector not found ', sectorID)
        return
