...
class EffectData(object):
    ...
    TimerTuple = namedtuple(b'TimerTuple', (b'period', b'callbackID'))
    ...
    def updateTerrainCircle(self):
        ...
        if vehicle is not None and self.radius is not None:
            effectSettings = self.visualSettings.get(b'ally') if isAlly else self.visualSettings.get(b'enemy')
...
InspireArgs = namedtuple(b'InspireArgs', (b'isSourceVehicle', b'isInactivation', b'endTime', b'duration', b'primary', b'equipmentID'))
HealPointArgs = namedtuple(b'HealPointArgs', (b'isSourceVehicle', b'isInactivation', b'endTime', b'duration', b'senderKey'))
...
class ArenaEquipmentComponent(ClientArenaComponent, CallbackDelayer):
    ...
    def activate(self):
        ...
        self.addSyncDataCallback(ARENA_SYNC_OBJECTS.SMOKE, b'', self.__onSmokeScreenUpdated)
        ...
        if dynamicObjects is not None:
            ...
            self.__inspiringEffect = Effect(b'inspire', InspireArgsAdapter, VEHICLE_VIEW_STATE.INSPIRE, FEEDBACK_EVENT_ID.VEHICLE_INSPIRE, dynamicObjects.getInspiringEffect())
            if arenaGuiType in (
             ARENA_GUI_TYPE.BATTLE_ROYALE, ARENA_GUI_TYPE.EPIC_BATTLE, ARENA_GUI_TYPE.EPIC_TRAINING):
                self.__healingEffect = Effect(b'healPoint', HealPointArgsAdapter, VEHICLE_VIEW_STATE.HEALING, FEEDBACK_EVENT_ID.VEHICLE_HEAL_POINT, dynamicObjects.getHealPointEffect())
            if arenaGuiType == ARENA_GUI_TYPE.BATTLE_ROYALE:
                self.__repairPointEffect = Effect(b'repairPoint', HealPointArgsAdapter, VEHICLE_VIEW_STATE.REPAIR_POINT, FEEDBACK_EVENT_ID.VEHICLE_REPAIR_POINT, dynamicObjects.getRepairPointEffect())
        self.__subscribe()
        return
    ...
    def deactivate(self):
        ...
        self.__unsubscribe()
        self.removeSyncDataCallback(ARENA_SYNC_OBJECTS.SMOKE, b'', self.__onSmokeScreenUpdated)
        ...
        for key, value in args.iteritems():
            if value is not None:
                ...
                if smokeItem is not None:
                    LOG_ERROR_DEV(b'Smoke Item is not none! Needs to be deleted or updated ', key, value)
                else:
