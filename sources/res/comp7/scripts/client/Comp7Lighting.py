...
class Comp7LightingTriggers(Enum):
    DEFAULT = b'default_lighting'
    FLYBY = b'purchase_lighting'
...
class Comp7Lighting(BigWorld.Entity):
    ...
    def prerequisites(self):
        if not self.animationStateMachine:
            _logger.warning(b'Property animationStateMachine for Comp7Lighting must be set!')
            return ()
    ...
    def onEnterWorld(self, prereqs):
        if self.animationStateMachine in prereqs.failedIDs:
            _logger.warning(b'Failed to load %s', self.animationStateMachine)
            return
        else:
            self.__animator = prereqs[self.animationStateMachine]
            if self.__animator is None:
                _logger.warning(b'Loaded None animator for %s', self.animationStateMachine)
                return
    ...
    def setTrigger(self, triggerName):
        if self.__animator is None:
            _logger.warning(b'Could not trigger state machine, animator is not loaded')
            return
        else:
