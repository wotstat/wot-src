...
class Comp7RewardsFormatter(SyncTokenQuestsSubFormatter):
    ...
    def __formatRegularMessage(self, message, rewardsData, rewardType):
        if not rewardsData:
            ...
            return g_settings.msgTemplates.format(self.__REGULAR_REWARD_MESSAGE_TEMPLATE, ctx={b'title': (backport.text(self.__R_SYS_MESSAGES.dyn(rewardType).title())), 
    ...
    def __formatQualificationRewardMessage(self, message, questIDs):
        ...
        return g_settings.msgTemplates.format(self.__QUALIFICATION_REWARD_MESSAGE_TEMPLATE, ctx={b'title': (backport.text(self.__R_SYS_MESSAGES.qualificationReward.title())), 
           b'body': (backport.text(self.__R_SYS_MESSAGES.qualificationReward.body(), maxRank=rankNames[-1], ranks=backport.text(R.strings.comp7_ext.listSeparator()).join(rankNames), rewards=formattedRewards))})
