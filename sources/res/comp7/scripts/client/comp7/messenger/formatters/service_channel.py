...
class Comp7BattleQuestsFormatter(object):
    ...
    def __formatTokensReward(self, message, questID):
        ...
        if rewardsData:
            ...
            return g_settings.msgTemplates.format(b'comp7RegularRewardMessage', ctx={b'title': (backport.text(R.strings.comp7_ext.system_messages.tokenWeeklyReward.title())), 
               b'body': (backport.text(R.strings.comp7_ext.system_messages.tokenWeeklyReward.body(), at=TimeFormatter.getLongDatetimeFormat(time_utils.makeLocalServerTime(message.sentTime)), rewards=achievesFormatter.formatQuestAchieves(rewardsData, asBattleFormatter=False)))})
        else:
            return
    ...
    def __formatWeeklyReward(message, questID):
        ...
        if rewardsData:
            achievesFormatter = QuestAchievesFormatter()
            return g_settings.msgTemplates.format(b'comp7RegularRewardMessage', ctx={b'title': (backport.text(R.strings.comp7_ext.system_messages.weeklyReward.title())), 
               b'body': (backport.text(R.strings.comp7_ext.system_messages.weeklyReward.body(), at=TimeFormatter.getLongDatetimeFormat(time_utils.makeLocalServerTime(message.sentTime)), rewards=achievesFormatter.formatQuestAchieves(rewardsData, asBattleFormatter=False)))})
        else:
            return
...
class Comp7BattleResultsFormatter(BattleResultsFormatter):
    ...
    def _getFairplayData(self, message):
        ...
        if params:
            ...
            savedData.update({(FAIRPLAY_VIOLATION_SYS_MSG_SAVED_DATA.COMP7_PENALTY): (extraData.get(b'penalty', 0)), 
               (FAIRPLAY_VIOLATION_SYS_MSG_SAVED_DATA.COMP7_IS_QUALIFICATION): (extraData.get(b'qualActive', False))})
        return (
         text, params)
    ...
    def _prepareFormatData(self, message):
        ...
        guiType = message.data.get(b'guiType', 0)
        battleResKey = message.data.get(b'isWinner', 0)
        if guiType == ARENA_GUI_TYPE.COMP7:
            isQualificationBattle = message.data.get(b'comp7QualActive', False)
