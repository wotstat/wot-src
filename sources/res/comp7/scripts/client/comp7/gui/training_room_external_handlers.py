...
class Comp7TrainingRoomHandler(TrainingRoomBaseHandler):
    _GUI_TYPE_NAME = b'comp7'

    def getArenaFilter(self):
        return _isComp7ArenaFilter

    def getArenaData(self):
        return {b'size': (self.getMaxPlayersInTeam()), 
           b'canChangeArenaTime': False, 
           b'additionalInfo': (self.getAdditionalInfo())}
    ...
    def getClientMessageData(self, errorType=None):
        if errorType == PREBATTLE_ERRORS.ONSLAUGHT_ROSTER_LIMIT:
            return (
             SYSTEM_MESSAGES.TRAINING_ERROR_ONSLAUGHTROSTERLIMIT, {b'numPlayers': (self.getMaxPlayersInTeam())})
        else:
            return
...
def _shouldComp7PlayerStateBeValidated(propertyName):
    if propertyName != b'state':
        return False
    propertyValue = prb_getters.getClientPrebattle().properties[b'state']
