...
class EntitlementsCache(object):
    ...
    def update(self, retryTimes=None, force=False):
        ...
        try:
            ...
            if isSuccess:
                balance = {ent: result.get(ent, {}).get(b'amount', 0) for ent in entitlements}
                ...
                _logger.error(b'Failed to update comp7 entitlements cache through agate')
                self.__setStatus(CacheStatus.ERROR)
        except Exception as e:
            logging.error(b'An exception has occurred while updating entitlements cache: %s', e)
            self.__setStatus(CacheStatus.ERROR)
    ...
    def __getCacheEntitlements(self):
        return self.__getCacheConfig().get(b'entitlements', set())

    def __getCacheTTL(self):
        return self.__getCacheConfig().get(b'TTL', 0)
