...
_DEFAULT_PREVIEW_VEHICLE = b'uk:GB91_Super_Conqueror'

class YearlyRewardsPage(PageSubModelPresenter):
    __slots__ = (b'__tooltips', b'__bonusData')
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.comp7.mono.lobby.tooltips.season_point_tooltip():
            params = {b'state': (SeasonPointState(event.getArgument(b'state'))), b'ignoreState': (event.getArgument(b'ignoreState'))}
    ...
    def __onStylePreviewOpen(self, cardIndex):
        ...
        styleBonus = findFirst((lambda bonus: bonus.getName() == b'customizations'), bonuses)
    ...
    def __onVehiclePreviewOpen(self, cd, cardIndex):
        bonuses = self.__bonusData[cardIndex]
        styleBonus = findFirst((lambda bonus: bonus.getName() == b'styleProgressToken'), bonuses)
    ...
    def __showStylePreview(self, styleBonus, cardIndex, vehicleCD=None):
        ...
        params = {b'backCallback': (partial(showComp7MetaRootTab, self.pageId, index=cardIndex))}
    ...
    def __setCards(self, model):
        ...
        sortedRewards = sorted(self.__comp7Controller.getYearlyRewards().main, key=(lambda data: data[b'cost']))
        for idx, rewardsData in enumerate(sortedRewards):
            cardSeasonPoints = seasonPointsGenerator.getNext(rewardsData[b'cost'] - prevRewardsCost)
            prevRewardsCost = rewardsData[b'cost']
            ...
            self.__bonusData[idx] = self.__fillRewards(cardModel, rewardsData[b'bonus'])
    ...
    def __setCardVehicleData(self, model, bonuses):
        vehicleBonus = bonuses.get(b'vehicles')
