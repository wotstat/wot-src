...
class ProgressionPage(PageSubModelPresenter):
    ...
    def createToolTip(self, event):
        if event.contentID == R.views.common.tooltip_window.backport_tooltip_content.BackportTooltipContent():
            ...
            if tooltipId == TOOLTIPS_CONSTANTS.SHOP_VEHICLE:
                ...
                itemCD = int(event.getArgument(b'customizationId'))
                level = int(event.getArgument(b'progressionLevel'))
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.comp7.mono.lobby.tooltips.general_rank_tooltip():
            params = {b'rank': (Rank(event.getArgument(b'rank'))), b'divisions': (event.getArgument(b'divisions')), 
               b'from': (event.getArgument(b'from')), 
               b'to': (event.getArgument(b'to'))}
            return GeneralRankTooltip(params=params)
        else:
            if contentID == R.views.comp7.mono.lobby.tooltips.division_tooltip():
                params = {b'rank': (Rank(event.getArgument(b'rank'))), b'division': (Division(event.getArgument(b'division'))), 
                   b'from': (event.getArgument(b'from')), 
                   b'to': (event.getArgument(b'to'))}
            ...
            if contentID == R.views.comp7.mono.lobby.tooltips.day_tooltip():
                rankValue = int(event.getArgument(b'rank') or 0)
                divisionValue = int(event.getArgument(b'division') or 0)
                ...
                params = {b'index': (event.getArgument(b'index', 0)), 
                   b'isQualification': (event.getArgument(b'isQualification', False)), 
                   b'hasBattles': (event.getArgument(b'hasBattles', False)), 
                   b'seasonName': (SeasonName(event.getArgument(b'seasonName'))), 
                   b'diff': (event.getArgument(b'diff', 0)), 
                   b'rank': rank, 
                   b'division': division, 
                   b'ratingPoints': (event.getArgument(b'ratingPoints', 0)), 
                   b'rankInactivityPenalty': (event.getArgument(b'rankInactivityPenalty', 0)), 
                   b'currentDayIndex': (event.getArgument(b'currentDayIndex', 0))}
                return DayTooltip(params=params)
            if contentID == R.views.comp7.mono.lobby.tooltips.rank_indicator_tooltip():
                divisionValue = int(event.getArgument(b'division') or 0)
                division = Division(divisionValue) if Division.A <= divisionValue <= Division.E else None
                params = {b'statisticsMode': (StatisticsMode(event.getArgument(b'statisticsMode'))), 
                   b'rank': (Rank(event.getArgument(b'rank'))), 
                   b'seasonName': (SeasonName(event.getArgument(b'seasonName'))), 
                   b'maxAchievedRatingPoints': (event.getArgument(b'maxAchievedRatingPoints')), 
                   b'division': division, 
                   b'ratingPoints': (event.getArgument(b'ratingPoints')), 
                   b'diff': (event.getArgument(b'diff')), 
                   b'dayOfMaxRatingIndex': (event.getArgument(b'dayOfMaxRatingIndex'))}
                return RankIndicatorTooltip(params=params)
            if contentID == R.views.comp7.mono.lobby.tooltips.battles_indicator_tooltip():
                params = {b'statisticsMode': (StatisticsMode(event.getArgument(b'statisticsMode'))), b'soloBattlesCount': (event.getArgument(b'soloBattlesCount')), 
                   b'superPlatoonBattlesCount': (event.getArgument(b'superPlatoonBattlesCount'))}
                return BattlesIndicatorTooltip(params=params)
            if contentID == R.views.comp7.mono.lobby.tooltips.wins_indicator_tooltip():
                params = {b'statisticsMode': (StatisticsMode(event.getArgument(b'statisticsMode'))), b'winRate': (event.getArgument(b'winRate')), 
                   b'winsCount': (event.getArgument(b'winsCount')), 
                   b'lossCount': (event.getArgument(b'lossCount')), 
                   b'drawCount': (event.getArgument(b'drawCount'))}
                return WinsIndicatorTooltip(params=params)
            if contentID == R.views.comp7.mono.lobby.tooltips.damage_indicator_tooltip():
                params = {b'statisticsMode': (StatisticsMode(event.getArgument(b'statisticsMode'))), b'averageDamageDealt': (event.getArgument(b'averageDamageDealt')), 
                   b'recordDamageDealt': (event.getArgument(b'recordDamageDealt')), 
                   b'recordDamageDealtVehicleName': (event.getArgument(b'recordDamageDealtVehicleName'))}
                return DamageIndicatorTooltip(params=params)
            if contentID == R.views.comp7.mono.lobby.tooltips.prestige_indicator_tooltip():
                params = {b'statisticsMode': (StatisticsMode(event.getArgument(b'statisticsMode'))), b'averagePrestige': (event.getArgument(b'averagePrestige')), 
                   b'recordPrestige': (event.getArgument(b'recordPrestige')), 
                   b'recordPrestigeVehicleName': (event.getArgument(b'recordPrestigeVehicleName'))}
        ...
        if not self.__progressionData:
            ...
            for dayData in allDaysData:
                ...
                if dayData:
                    ...
                    _logger.debug(b'__updateDailyStats day %s ; %s', str(dayData.dayIndex), str(dayStatsModel))
    ...
    def __updateCustomizationTasks(self):
        if self.__comp7Controller.isQualificationActive():
            ...
            if style is None:
                _logger.debug(b'Comp7 Progression style is not found')
                return
            ...
            for item in style.alternateItems:
                ...
                if not quests or not isC11nItemTokenAttainable(allTokens, item):
                    continue
                quest = first(quests)
                iconKey = item.texture.split(b'/')[-1].split(b'.')[0]
    ...
    def __onCustomizationProgressShown(self, args):
        decalCD = int(args.get(b'customizationId', 0))
        with self.getViewModel().transaction() as tx:
            for task in tx.getCustomizationTasks():
                ...
                _logger.error(b'Customization task not found decalCD = %s', decalCD)
    ...
    def __onOpenCustomization(self, args):
        ...
        style = getComp7ProgressionStyle()
        if style is None:
            _logger.error(b'Style could not be found')
            return
        else:
