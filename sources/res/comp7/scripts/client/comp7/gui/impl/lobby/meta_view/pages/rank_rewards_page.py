...
class RankRewardsPage(PageSubModelPresenter):
    __slots__ = (b'__bonusData', b'__rankQuests', b'__periodicQuests', b'__itemIndex')
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.lobby.tooltips.AdditionalRewardsTooltip():
            ...
            rank = int(event.getArgument(b'rank'))
    ...
    def __setRankRewards(self, itemModel, division):
        ...
        if rankQuest is None:
            _logger.error(b'Missing Competitive7x7 Rank Quests for division %s.', division)
            return
        else:
    ...
    def __onPreviewOpen(self, rank, index):
        ...
        params = {b'outfit': (getPreviewOutfit(style, bonus.getBranchID(), bonus.getProgressLevel(), compactDescr)), 
           b'backCallback': (partial(comp7_events.showComp7MetaRootTab, self.pageId, index=index))}
