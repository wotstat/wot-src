...
_OFFER_REWARDS_ORDER = (b'deluxe', b'modernized_devices_t3')

def _getComp7BonusPackersMap():
    mapping = getDefaultBonusPackersMap()
    mapping.update({b'dossier': (Comp7DossierBonusUIPacker()), 
       b'dogTagComponents': (Comp7DogTagUIPacker()), 
       b'customizations': (Comp7CustomizationBonusUIPacker()), 
       b'vehicles': (Comp7VehicleBonusUIPacker()), 
       b'tankmen': (Comp7TankmenBonusUIPacker()), 
...
def getComp7YearlyMetaBonusPacker():
    ...
    mapping.update({SELECTABLE_BONUS_NAME: (Comp7YearlyMetaOfferPacker()), 
       b'customizations': (Comp7CustomizationBonusUIPacker()), 
       b'tankmen': (Comp7YearlyCrewBonusUIPacker())})
    return BonusUIPacker(mapping)
...
def _getComp7YearlyBonuses(specificCustomization=None):
    ...
    bonuses.update({b'slots': Comp7YearlySlotsBonus, 
       b'customizations': (specificCustomization or Comp7YearlyCustomizationsBonus)})
    return bonuses
...
class Comp7CrystalBonusPacker(SimpleBonusUIPacker):
    ...
    def _packSingleBonus(cls, bonus, label):
        ...
        model.setIsPeriodic(bonus.getContext().get(b'isPeriodic', False))
        return model
...
class Comp7StyleProgressBonusUIPacker(BaseBonusUIPacker):
    ...
    def _packSingleBonus(cls, bonus, level=None):
        ...
        if camo is not None:
            ...
            _logger.error(b'Missing camouflage for Comp7StyleProgressBonus: styleID=%s; level=%s', styleID, level)
            icon = b''
            label = b''
        model.setIcon(icon)
...
class Comp7CustomizationBonusUIPacker(CustomizationBonusUIPacker):
    _ICON_NAME = b'style'
    _ICON_NAME_PROGRESSION = b'progressionStyle'
    ...
    def _packSingleBonus(cls, bonus, item, label):
        ...
        model.setStyleID(styleID)
        if item.is3D:
            model.setName(b'styleProgressToken')
            model.setValue(b'')
...
class Comp7TankmenBonusUIPacker(BaseBonusUIPacker):
    ...
    def _packSingleBonus(cls, bonus):
        cd = bonus.get(b'tmanCompDescr', bonus)
        tankmanData = tankmen.TankmanDescr(cd)
        model = Comp7BonusModel()
        model.setName(b'tankwoman' if tankmanData.isFemale else b'tankman')
    ...
    def _getToolTip(cls, bonus):
        tooltipData = []
        for tankmanData in bonus.getValue():
            cd = tankmanData.get(b'tmanCompDescr', tankmanData)
...
class Comp7YearlyCrewBonusUIPacker(TankmenBonusUIPacker):
    ...
    def _pack(cls, bonus):
        ...
        cls._packCommon(bonus, model)
        model.setName(b'onslaught_yearly_crew')
...
class Comp7OfferBonusUIPacker(BaseBonusUIPacker):
    ...
    def _pack(cls, bonus):
        ...
        for offerToken in sorted(bonus.getValue().iterkeys(), key=_getOfferRewardTokensSortKey(_OFFER_REWARDS_ORDER)):
            ...
            if giftCount <= 0:
                continue
            offerTokenCategory = offerToken.split(b':')[2]
...
def packRanksRewardsQuestBonuses(quest, periodicQuest=None):
    ...
    if periodicQuest is not None:
        ...
        for b in periodicBonuses:
            b.updateContext({b'isPeriodic': True})
...
def packYearlyRewardVehicleBonuses(bonuses):
    bonusData = []
    vehicleBonus = first(getNonQuestBonuses(b'vehicles', bonuses.get(b'vehicles')))
    bonusData.append(getVehicleCrewReward(vehicleBonus))
    customizations = bonuses.get(b'customizations') or []
    customizations.sort(key=(lambda c: c.get(b'id')), reverse=True)
    bonusData.extend(getNonQuestBonuses(b'customizations', customizations, bonusesDict=_getComp7YearlyBonuses(specificCustomization=Comp7YearlyCustomizationsBonus3D)))
    return packQuestBonuses(bonusData, bonusPacker=getComp7YearlyBonusPacker(), order=_YEARLY_REWARDS_BONUSES_ORDER)
