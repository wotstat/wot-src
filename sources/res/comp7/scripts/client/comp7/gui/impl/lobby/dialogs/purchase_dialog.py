...
class PurchaseDialog(ViewComponent):
    ...
    def __setProducts(self, viewModel):
        ...
        if not products:
            _logger.warning(b'Failed to set product info, no products')
    ...
    def __updateData(self, *_):
        if self.viewModel.getPageState() == PageState.CONFIRMATION:
            products = self.__comp7ShopController.getProducts()
            if not products:
                _logger.warning(b'Failed to update product info, no products')
    ...
    def __startCameraFlyby(self):
        ...
        if cameraManager is not None:
            ...
            _logger.warning(b'Could not start comp7 fly-by, camera manager is None')
        return
    ...
    def __setLightingTrigger(self, trigger):
        ...
        if entity is not None:
            ...
            _logger.warning(b'Comp7Lighting entity must be placed in comp7 hangar space!')
        return
