...
SOUND_NAME = b'comp_7_bans_choice_confirm'

class BanView(ViewImpl, BattleGUIKeyHandler):
    __slots__ = (b'__selectionTimer',)
    ...
    def __init__(self):
        ...
        self.__usersStorage = storage_getter(b'users')()
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.comp7.mono.battle.tooltips.ban_show_tooltip():
            params = {b'longName': (event.getArgument(b'longName', b'')), b'type': (event.getArgument(b'type', b'')), 
               b'isPremium': (event.getArgument(b'isPremium', False)), 
               b'confirmedChoice': (event.getArgument(b'confirmedChoice', False)), 
               b'vehicleCD': (event.getArgument(b'vehicleCD', False)), 
               b'roleKey': (event.getArgument(b'roleKey', False))}
