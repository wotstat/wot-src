...
def isFirstWeeklyQuest(qID):
    ...
    return qID and qID.extraInfo == b'1_1'
...
def isComp7OfferYearlyRewardGiftToken(token):
    return isComp7OfferYearlyRewardToken(token) and token.endswith(b'_gift')
...
def parseQuestsByDivision(quests, parser, questType):
    ...
    for quest in quests:
        ...
        if division is not None:
            ...
            _logger.error(b'Division number could not be parsed - %s', quest.getID())

    return questsByDivision


class Comp7ParsedQuestID(object):
    __questIDMatcher = re.compile((b'comp7_(\\d+)_(\\d+)_({})_(.+)').format((b'|').join(el.value for el in Comp7QuestType))).match
    __slots__ = (b'season', b'questType', b'extraInfo')
