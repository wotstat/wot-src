...
class ShopPage(PageSubModelPresenter):
    __slots__ = (b'__currentPreviewVehicle', b'__currentPreviewStyle', b'__currentItemCD', b'__productSelectorMethods', b'__productSetterMethods', b'__blur', b'__rotationHelper', b'__requestStatus', b'__products', b'__productItems', b'__productCdToCode', b'__switchCameraToDefault', b'__switchVehicleToDefault', b'__hangarVehId')
    ...
    __DEFAULT_PRODUCT_SELECTOR_NAME = b'default'
    ...
    def createToolTip(self, event):
        if event.contentID == R.views.common.tooltip_window.backport_tooltip_content.BackportTooltipContent():
            ...
            itemCD = int(event.getArgument(b'id'))
            tooltipData = None
            if tooltipId in (
    ...
    def initialize(self, **params):
        ...
        if b'productCD' in params:
            self.__currentItemCD = params[b'productCD']
    ...
    def __isRefferalStateLoaded(self):
        ...
        if isinstance(currentState, UntrackedState):
            ...
            if isinstance(stateParams, _UntrackedStateForwardedParams):
                ctx = stateParams.kwargs.get(b'ctx', {})
                if ctx.get(b'browser_alias') == VIEW_ALIAS.REFERRAL_PROGRAM_WINDOW:
                    return True
        return False
    ...
    def __updateData(self, *_, **__):
        ...
        if not self.__comp7ShopController.isShopEnabled:
            _logger.warning(b'comp7 shop disabled')
    ...
    def __processData(self):
        with self.viewModel.transaction() as tx:
            ...
            if hasDataError:
                _logger.warning(b'no products were found for comp7 shop')
                self.__switchToErrorState()
                return
    ...
    def __showWaiting(self):
        if self.viewModel.getShopState() == ShopState.ERROR:
            return
        Waiting.show(b'loadingData', overlapsUI=False, isSingle=True)
        return
    ...
    def __goToPreview(self):
        ...
        if itemType == GUI_ITEM_TYPE.VEHICLE:
            params = {b'resetAppearance': False, b'heroInteractive': False, 
               b'hiddenBlocks': (
            ...
            params = {b'resetAppearance': False}
