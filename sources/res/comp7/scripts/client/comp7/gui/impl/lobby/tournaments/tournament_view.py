    ...
    def __fillTeamData(self, teamModel, teamID, matchState=None, teamMatchScore=0):
        if teamID is None:
            ...
            if teamData is not None:
                ...
                teamModel.logos.setX48(logoUrls.get(IngameTournamentLogoSize.SMALL, b''))
                teamModel.logos.setX86(logoUrls.get(IngameTournamentLogoSize.MEDIUM, b''))
                teamModel.logos.setX260(logoUrls.get(IngameTournamentLogoSize.LARGE, b''))
                teamModel.logos.setX522(logoUrls.get(IngameTournamentLogoSize.EXTRA_LARGE, b''))
                ...
                _logger.warning(b'Team with ID %s not found in in teams', teamID)
            return
    ...
    def __onWatchStreamOne(self):
        if self._data:
            ...
            if url:
                ...
                _logger.warning(b'Could not open translation link one, url is not set')
        else:
            _logger.warning(b'Could not open translation link one, data is not set')
        return
    ...
    def __onWatchStreamTwo(self):
        if self._data:
            ...
            if url:
                ...
                _logger.warning(b'Could not open translation link two, url is not set')
        else:
            _logger.warning(b'Could not open translation link two, data is not set')
        return
