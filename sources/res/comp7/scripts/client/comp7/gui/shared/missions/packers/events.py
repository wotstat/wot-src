...
class Comp7WeeklyQuestPacker(object):
    ...
    __slots__ = (b'__isUnavailablePeriod', b'__isComp7Available', b'__hasSuitableVehicles')
    ...
    def __getQuestState(self, quest):
        ...
        if self.__isComp7Available:
            ...
            if not isQuestAvailable and not self.__hasSuitableVehicles and quest.getID().endswith(b'_1_1'):
...
class Comp7TokenQuestPacker(object):
    ...
    def getData(quest):
        if quest:
            tokenConditions = quest.accountReqs.getConditions().find(b'token')
            if tokenConditions:
                return (
                 u'folder',
