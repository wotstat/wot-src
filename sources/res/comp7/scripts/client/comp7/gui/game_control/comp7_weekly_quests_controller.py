...
class Comp7WeeklyQuestsController(IComp7WeeklyQuestsController, IGlobalListener):
    ...
    def __getUpdatedQuests(cls):
        ...
        if not battleQuests or not tokenQuests:
            _logger.error(b'Events cache failed to provide quests to the Comp7WeeklyQuestsController.')
...
class _Comp7WeeklyQuests(object):
    __slots__ = (b'sortedBattleQuests', b'sortedTokenQuests', b'oldQuest', b'newQuest', b'numBattleQuests', b'numCompletedBattleQuests', b'numBattleQuestsToCompleteByTokenQuestIdx', b'timeOfNewQuests')
    ...
    def __init__(self, battleQuests, tokenQuests, oldQuest, newQuest, numBattleQuests, numCompletedBattleQuests, timeOfNewQuests):
        ...
        self.numBattleQuestsToCompleteByTokenQuestIdx = tuple(quest.accountReqs.getConditions().find(b'token').getNeededCount() for _, quest in tokenQuests)
    ...
    def __repr__(self):
        return (b'{}\n{}').format(self.__class__.__name__, (b'\n').join((b'{}={}').format(n, getattr(self, n, b'priv')) for n in self.__slots__))
