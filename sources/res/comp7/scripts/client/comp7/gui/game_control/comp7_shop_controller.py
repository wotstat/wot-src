...
ShopPageProductInfo = namedtuple(b'_ShopPageProductInfo', (b'code', b'description', b'rank', b'category', b'purchasable', b'entitlements', b'currencyName', b'originalPrice', b'discountPrice', b'limitedQuantity', b'limitsPurchaseAllowed', b'discounts'))
...
class Comp7ShopController(IComp7ShopController):
    ...
    __PRODUCTS_STOREFRONT = b'comp7_shop'
    ...
    def __requestProducts(self, force=False):
        ...
        if not self.__isResponseReadyToProcess(response):
            ...
            logging.error(b'Response received, but products were misconfigured: %s', response)
            return
        try:
            self.__processShopResponse(response)
            self.__setStatus(ShopControllerStatus.DATA_READY)
        except Exception as e:
            logging.error(b'An exception has occurred while processing shop response: %s', e)
            self.__setStatus(ShopControllerStatus.ERROR)
    ...
    def __onNewMaxRank(self, newMaxRank):
        ...
        _logger.info(b'Request products due to onNewMaxRank event')
    ...
    def __convertProducts(self, products, accountLimits):
        ...
        for product in products:
            ...
            if not productRank:
                _logger.warning(b'No rank for category %s at product %s', product.category, productCode)
                continue
            limitsData = findFirst((lambda limit, code=productCode: limit.productCode == code), accountLimits)
            if not limitsData:
                _logger.warning(b'No limits data for product %s', productCode)
                continue
            entitlements = {}
            for entitlement in product.entitlements:
                cd = entitlement.get(b'cd', 0)
                if not cd:
                    continue
                entitlements[int(cd)] = entitlement[b'type']
    ...
    def __sendNewProductsForRankMessage(self):
        pushMessage(type=SM_TYPE.Comp7ShopItemsAvailableForRank, text=backport.text(R.strings.comp7_ext.system_messages.shop.offer.newItems.body()), messageData={b'header': (backport.text(R.strings.comp7_ext.system_messages.shop.offer.newItems.title()))})
        return

    def __sendNewDiscountsForRankMessage(self):
        pushMessage(type=SM_TYPE.Comp7ShopItemsAvailableForRank, text=backport.text(R.strings.comp7_ext.system_messages.shop.offer.newDiscounts.body()), messageData={b'header': (backport.text(R.strings.comp7_ext.system_messages.shop.offer.newDiscounts.title()))})
        return
