...
class Comp7Validator(BaseActionsValidator):
    ...
    def _validate(self):
        ...
        if not self.__comp7Ctrl.hasSuitableVehicles():
            return ValidationResult(False, PRE_QUEUE_RESTRICTION.LIMIT_NO_SUITABLE_VEHICLES, ctx={b'levels': (self.__comp7Ctrl.getModeSettings().levels)})
        if not self.__comp7Ctrl.hasEnoughReadyToFightVehicles():
            return ValidationResult(False, PRE_QUEUE_RESTRICTION.LIMIT_NOT_ENOUGH_SUITABLE_VEHICLES, ctx={b'amount': (self.__comp7Ctrl.getModeSettings().minVehiclesRequired)})
