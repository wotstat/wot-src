...
class Comp7ProfileTechniquePage(ProfileTechniquePage):
    ...
    def __init__(self, *args):
        ...
        if data is not None and data.get(b'battlesType') == PROFILE_DROPDOWN_KEYS.COMP7:
            self.requestDossier(PROFILE_DROPDOWN_KEYS.COMP7)
            vehicleCD = data.get(b'vehicleCD')
    ...
    def _getDefaultTableHeader(self, isFallout=False, showMarkOfMastery=False):
        ...
        if isComp7BattleType:
            result.append(self._createTableBtnInfo(b'prestigePoints', 62 if self._isPrestigeVisible() else 70, 6, PROFILE.SECTION_TECHNIQUE_SORT_TOOLTIP_PRESTIGEPOINTS, b'descending', iconSource=RES_ICONS.MAPS_ICONS_FILTERS_PRESTIGEPOINTS))
        return result
