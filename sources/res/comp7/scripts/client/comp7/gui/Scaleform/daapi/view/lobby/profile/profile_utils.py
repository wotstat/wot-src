...
class _MaxPrestigePointsField(commonProfileUtils._AbstractField):
    ...
    def __call__(self, targetData, isCurrentUser):
        ...
        tooltip = (b'{}/vehicle').format(self._tooltip) if vehicle is not None else self._tooltip
    ...
    def __getVehicle(self, targetData):
        vehGetter = getattr(targetData, b'getMaxPrestigePointsVehicle', None)
...
class _MaxWinSeriesField(commonProfileUtils._AbstractField):
    ...
    def _buildData(self, targetData, isCurrentUser):
        return (b'{}/{}').format(backport.getIntegralFormat(ProfileUtils.getValueOrUnavailable(targetData.getMaxWinSeries())), backport.getIntegralFormat(ProfileUtils.getValueOrUnavailable(targetData.getMaxSquadWinSeries())))
...
class _MaxEquipmentDamageDealtField(commonProfileUtils._AbstractField):
    ...
    def __getVehicle(self, targetData):
        vehGetter = getattr(targetData, b'getMaxEquipmentDamageDealtVehicle', None)
        if vehGetter is not None:
            return self.itemsCache.items.getItemByCD(vehGetter())
        else:
            return
...
class _MaxHealthRepairField(commonProfileUtils._AbstractField):
    ...
    def __getVehicle(self, targetData):
        vehGetter = getattr(targetData, b'getMaxHealthRepairVehicle', None)
        if vehGetter is not None:
            return self.itemsCache.items.getItemByCD(vehGetter())
        else:
            return
