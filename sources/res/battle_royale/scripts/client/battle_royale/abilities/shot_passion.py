...
class ShotPassionComponent(object):
    ...
    turretNode = ComponentProperty(type=CGFMetaTypes.LINK, value=CGF.GameObject, editorName=b'Turret Node')
    gunNode = ComponentProperty(type=CGFMetaTypes.LINK, value=CGF.GameObject, editorName=b'Gun Node')
...
class ShotPassionNodeComponent(object):
    ...
    category = b'Abilities'
    domain = CGF.DomainOption.DomainClient
    effectTemplate = ComponentProperty(type=CGFMetaTypes.STRING, value=b'', editorName=b'Effect Template')
    maxAnimationStage = ComponentProperty(type=CGFMetaTypes.INT, value=0, editorName=b'Max animation stage')
...
class ShotPassionManager(CGF.ComponentManager):
    ...
    def __setupVFX(self, nodeGO, stage):
        ...
        nodeGO.createComponent(GenericComponents.AnimatorComponent, effectTemplate.format(math_utils.clamp(0, nodeComponent.maxAnimationStage, stage)), 0, 1, -1, True, b'')
        nodeGO.deactivate()
        nodeGO.activate()
        return
