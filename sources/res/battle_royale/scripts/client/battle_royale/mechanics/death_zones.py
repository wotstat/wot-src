...
class DeathZoneUpdaterManager(CGF.ComponentManager):
    ...
    @onProcessQuery(ArenaInfoDeathZonesComponent, tickGroup=b'Simulation', updatePeriod=_UPDATE_PERIOD)
    def onProcess(self, deathZones):
        if deathZones.updatedZones:
            g_eventBus.handleEvent(DeathZoneEvent(DeathZoneEvent.UPDATE_DEATH_ZONE, ctx={b'deathZones': deathZones}), scope=EVENT_BUS_SCOPE.BATTLE)
...
class DeathZonesRule(Rule):
    ...
    maxAlpha = ComponentProperty(type=CGFMetaTypes.FLOAT, value=0.5, editorName=b'Max Alpha')
    height = ComponentProperty(type=CGFMetaTypes.FLOAT, value=16.0, editorName=b'Height')
    activeColor = ComponentProperty(type=CGFMetaTypes.VECTOR4, value=Math.Vector4(0.8, 0.0, 0.0, 0.0), editorName=b'Active color')
    waitingColor = ComponentProperty(type=CGFMetaTypes.VECTOR4, value=Math.Vector4(1.0, 0.6, 0.0, 0.0), editorName=b'Waiting Color')

    @registerManager(DeathZoneDrawManager)
    def registerDeathZonesDrawManager(self):
        return (self.maxAlpha, self.height, self.activeColor, self.waitingColor)
