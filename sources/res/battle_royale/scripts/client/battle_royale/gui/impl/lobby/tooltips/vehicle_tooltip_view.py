...
class VehicleTooltipView(ViewImpl):
    ...
    def __fillPrice(self, model):
        ...
        showTestDrivePrice = self.__rentState in (EquipmentPanelCmpRentStates.STATE_TEST_DRIVE_ACTIVE,
        ...
        model.setRentLabel(backport.text(R.strings.battle_royale.tooltips.testDriveInfo.dyn(b'rightLabel' if showTestDrivePrice else b'rentFor')()).format(days=rentDays))
    ...
    def __fillStatus(self, vehicle, model):
        ...
        model.setStatusText(status[b'header'])
        model.setStatusLevel(status[b'level'])
        return
