...
def getBonusPacker():
    ...
    packer.getPackers().update({b'currencies': (CurrenciesBonusUIPacker()), 
       ...
       b'token': tokenBonusPacker, 
       b'battleToken': tokenBonusPacker, 
       b'battlePassPoints': (BRBattlePassPointsBonusPacker())})
    return packer
...
def _buildDescription(conditionalModel):
    ...
    if conditionalModel.getConditionType() == CONDITION_GROUP_AND:
        separator = (b' {} ').format(backport.text(R.strings.quests.dailyQuests.postBattle.conditionTypeAnd()))
        result = separator.join(descriptions)
    elif conditionalModel.getConditionType() == CONDITION_GROUP_OR:
        separator = (b' {} ').format(backport.text(R.strings.quests.dailyQuests.postBattle.conditionTypeOr()))
        result = separator.join(descriptions)
    else:
        result = descriptions[0] if descriptions else b''
    return result
