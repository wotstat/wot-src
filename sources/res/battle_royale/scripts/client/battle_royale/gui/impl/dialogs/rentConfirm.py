...
class BrMoney(Money):
    ...
    def getCurrency(self, byWeight=True):
        ...
        if self._values.get(res) is None:
            return BR_COIN
        else:
            return res
...
class RentConfirm(DialogTemplateView):
    ...
    def _onLoading(self, *args, **kwargs):
        ...
        if vehicle.name:
            vehName = vehicle.name.split(b':')[1]
            vehName = vehName.replace(b'-', b'_')
            resID = R.images.gui.maps.icons.battleRoyale.rentVehicles.dyn(vehName)()
            if resID > 0:
                _list = [
                 R.images.gui.maps.icons.battleRoyale.rentVehicles.background()]
