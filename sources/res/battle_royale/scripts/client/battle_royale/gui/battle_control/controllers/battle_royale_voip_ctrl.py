...
class BRVOIPController(IBRVOIPController):
    ...
    def startControl(self, *_, **__):
        _logger.debug(b'BR VOIPController started.')
        self.__subscribe()
        return

    def stopControl(self):
        _logger.debug(b'BR VOIPController stopped.')
        self.__unsubscribe()
        return
    ...
    def toggleChannelConnection(self):
        ...
        if not self.isTeamVoipEnabled:
            _logger.error(b'Failed to toggle team VOIP channel. Joining is not allowed.')
            return
