...
class _BattleRoyaleArenaLevel(object):
    ...
    __slots__ = (b'__xp', b'__level', b'__percent', b'__baseXP', b'__targetXP', b'__diffXp', b'__diffXpAfterLevel', b'__xpToLevel', b'__maxLevel', b'__levelIsChanged', b'__xpIsChanged', b'__isMaxLvlAchieved', b'__currentObservedVehicleID', b'__observedVehicleIsChanged')
    ...
    def updateXP(self, xp, observedVehicleID):
        if not self.__xpToLevel:
            ...
            if battleXP and b'xpToLvl' in battleXP:
                self.__xpToLevel = battleXP[b'xpToLvl']
...
class _UpgradesAvailability(object):
    __slots__ = (b'__viewComponents', b'__cbCooldownID', b'__available', b'__isStrategicMode')
    __STRATEGIC_MODES = (b'mapcase', b'arcadeMapcase')
...
class _ProgressionWindowCtrl(object):
    __slots__ = (b'onTriggered', b'__progressionWindow')
...
class _VehicleHolder(object):
    __slots__ = (b'__initialVehicle', b'__vehicle', b'__currentVehicleLevel', b'__vehicleChangeCallback', b'__modulesHolder')
...
class _ModuleChangeRequester(object):
    __slots__ = (b'__awaitingResponse', b'__intCDToUpgrade', b'__vehicleInUpgrade')
    ...
    def request(self, moduleItem, vehicle):
        ...
        if not canDoRequest:
            _logger.info(b'%s "%s" could not be installed. %s', moduleItem.itemTypeName, moduleItem.name, reason)
            return False
    ...
    def processResponse(self, intCD, isSuccessfull):
        if intCD not in self.__awaitingResponse:
            _logger.warning(b'Module %d is not in the waiting list!', intCD)
            return False
...
class ProgressionController(IProgressionController, ViewComponentsController):
    __slots__ = (b'onPageTriggered', b'__progressionWindowCtrl', b'_viewComponents', b'__modulesStorage', b'__averageLevel', b'__enemiesAmount', b'__vehicleModulesStorage', b'__enemyTeamsAmount', b'__isStarted', b'__upgradesAvailability', b'__tmpProgressionRecord', b'onVehicleUpgradeStarted', b'onVehicleUpgradeFinished', b'__vehicleHolder', b'__moduleChangeReq', b'__initialModulesRecord', b'__battleRoyaleArenaLevel', b'notificationManager')
    __itemsFactory = dependency.descriptor(IGuiItemsFactory)
    __sessionProvider = dependency.descriptor(IBattleSessionProvider)
