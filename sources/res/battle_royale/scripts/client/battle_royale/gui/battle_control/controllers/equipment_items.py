...
MapCaseMode = importlib.import_module(b'AvatarInputHandler.MapCaseMode')
...
class _RepairBattleRoyaleCrewAndModules(_RepairCrewAndModules):
    ...
    def __hasDebuffComponentsForStop(self, vehicle):
        return bool([True for comp in vehicle.dynamicComponents.values() if hasattr(comp, b'canBeStoppedRepairKit') and comp.canBeStoppedRepairKit])
    ...
    def canActivate(self, entityName=None, avatar=None):
        ...
        if not result or not avatar:
            ...
            if not vehicle or vehicle.health >= vehicle.maxHealth:
                return (False, _ActivationError(b'vehicleIsNotDamaged', {b'name': (self._descriptor.userString)}))
            return (
             True, None)
...
CorrodingShotInfo = namedtuple(b'CorrodingShotInfo', (b'stage', b'endTime'))
...
class _CorrodingShot(_TriggerItem):
    __slots__ = (b'__crosshairIndicator',)
...
class _AdaptationHealthRestore(_TriggerItem):
    ...
    def getTags(self):
        return (b'adaptationHealthRestore',)
...
def registerBREquipmentsItems():
    registerEquipmentItem(b'large_repairkit_battle_royale', _RepairBattleRoyaleCrewAndModules, _ReplayItem)
    registerEquipmentItem(b'regenerationKit', _RegenerationKitBattleRoyaleItem, _ReplayItem)
    registerEquipmentItem(b'arcade_minefield_battle_royale', _ArcadeMineFieldBattleRoyaleItem, _BRReplayMinefield)
    registerEquipmentItem(b'healPoint', _BuffItem, _ReplayItem)
    registerEquipmentItem(b'afterburning_battle_royale', _AfterburningItem, _ReplayAfterburningItem)
    registerEquipmentItem(b'repairpoint', _RepairPointItem, _ReplayItem)
    registerEquipmentItem(b'selfBuff', _BuffItem, _ReplayItem)
    registerEquipmentItem(b'trappoint', _GameplayConsumableItem, _ReplayItem)
    registerEquipmentItem(b'arcade_bomber_battle_royale', _ArcadeBomber, _BRReplayBomber)
    registerEquipmentItem(b'spawn_kamikaze', _BotSpawner, _BRReplaySpawnBot)
    registerEquipmentItem(b'arcade_smoke_battle_royale_with_damage', _ArcadeSmoke, _BRReplaySmoke)
    registerEquipmentItem(b'berserker', _BuffItem, _ReplayItem)
    registerEquipmentItem(b'fireCircle', _FireCircle, _ReplayItem)
    registerEquipmentItem(b'adaptationHealthRestore', _AdaptationHealthRestore, _ReplayItem)
    registerEquipmentItem(b'corrodingShot', _CorrodingShot, _BRReplayCorrodingShot)
    registerEquipmentItem(b'clingBrander', _BotSpawner, _BRReplaySpawnBot)
    registerEquipmentItem(b'thunderStrike', _ThunderStrike, _BRReplayThunderStrike)
    registerEquipmentItem(b'shotPassion', _ShotPassion, _ReplayItem)
    registerEquipmentItem(b'arcade_bomber_with_own_damage_battle_royale', _ArcadeBomber, _BRReplayBomber)
    return
