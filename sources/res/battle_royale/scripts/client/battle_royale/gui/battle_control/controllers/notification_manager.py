...
class RespawnMessage(CallbackDelayer):
    ...
    def __repr__(self):
        return b'RespawnMessage(msgType: %s, messageVO: %s, delay: %s, time: %s, showBefore: %s)' % (
...
class NotificationManager(object):
    ...
    def update(self):
        ...
        if prevUpdateNeeded and self.__needToUpdate:
            logger.warning(b'Something wrong with NotificationManager.update, dont need to do more than 2 calls')
    ...
    def setCurrentHandler(self, handler):
        if self._curHandler == handler:
            logger.warning(b'curHandler == handler %s', handler)
            return
        self.updateHandlersDependencies()
    ...
    def onHandlerRealised(self):
        if not self._curHandler:
            logger.warning(b'Wrong call from handler. There is no active handlers')
            return
        else:
            self._curHandler = None
            self.updateHandlersDependencies()
...
class UpgradeHandler(INotificationHandler):
    ...
    def tryToProceed(self):
        ...
        if self.__isUpgradeVisible:
            ...
            return (
             True, False)
        logger.warning(b'UpgradeHandler.tryToProceed return False after updateseems like manager change internal state of handler')
        return (
         False, True)
...
class RespawnHandler(INotificationHandler):
    ...
    def update(self):
        ...
        if not msg:
            if self._curMsg:
                logger.warning(b'_curMsg not empty %s', self._curMsg.msgType)
            self._curMsg = None
            self.__curStateMsg = None
            return
        else:
    ...
    def send(self, message):
        if message.msgType in InvalidMessageTypeRange:
            logger.warning(b'try send technical msg %s', message.msgType)
            return
