...
NotificationObj = namedtuple(b'NotificationObj', [b'typeIdx', b'messageVO'])
...
allNotificationTypes = (
 NotificationType.respActivatedMsg,
...
DeleteMsgTypeMapping = {(NotificationType.respActivatedMsg): allNotificationTypes, 
   ...
   (NotificationType.allyInBattleMsg): (
                                      NotificationType.allyInBattleMsg, NotificationType.respActivatedMsg,
                                      NotificationType.stayInCoverMsg), 
   (NotificationType.respFinishedMsg): (
                                      NotificationType.respActivatedMsg, NotificationType.allyInBattleMsg), 
   (NotificationType.allyRespawnedMessage): (
                                           NotificationType.stayInCoverMsg, NotificationType.allyInBattleMsg), 
   (NotificationType.respNotAvailableMsg): (
                                          NotificationType.respActivatedMsg,
                                          NotificationType.stayInCoverMsg,
                                          NotificationType.allyInBattleMsg,
...
class NotificationManager(object):
    ...
    def addMessageToQueues(self, idx, message):
        ...
        if idx in (NotificationType.allyInBattleMsg,
         NotificationType.stayInCoverMsg):
            self.__curStateMsg = NotificationObj(idx, message.copy())
        elif idx in (NotificationType.allyRespawnedMessage,
         NotificationType.respNotAvailableMsg):
            self.__curStateMsg = None
    ...
    def __checkRespawn(self):
        ...
        if self.__respawnQueue:
            ...
            if not msg:
                self.currMessageType = MessageType.NONE
                return
            msg.messageVO[b'delay'] = max(msg.messageVO[b'delay'], self.__upgradeHideTimeout)
            if msg.typeIdx in (NotificationType.respActivatedMsg, NotificationType.stayInCoverMsg,
             NotificationType.allyInBattleMsg):
                msg.messageVO[b'time'] = max(0, msg.messageVO[b'showBefore'] - msg.messageVO.get(b'delay') - BigWorld.serverTime())
            elif self.__isUpgradeVisible:
                msg.messageVO[b'time'] = self.RESPAWN_FAST_SHOW_TIME
            showTime = msg.messageVO.get(b'time') + msg.messageVO.get(b'delay')
            msg.messageVO.pop(b'showBefore')
    ...
    def __getValidRespawnMessage(self):
        ...
        while BigWorld.serverTime() > msg.messageVO[b'showBefore']:
            if self.__respawnQueue:
                msg = self.__respawnQueue.pop(0)
            else:
                msg = None
