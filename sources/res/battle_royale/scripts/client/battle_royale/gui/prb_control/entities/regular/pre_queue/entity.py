...
class JoinBRPreQueueModeCtx(JoinPreQueueModeCtx):
    __slots__ = (b'__selectedSubModeID',)

    def __init__(self, queueType, selectedSubModeID, flags=FUNCTIONAL_FLAG.UNDEFINED, waitingID=b''):
...
class BRQueueCtx(SpecialModeQueueCtx):
    ...
    def __init__(self, entityType, vInventoryID, currentSubModeID, waitingID=b''):
...
class BattleRoyaleEntryPoint(PreQueueEntryPoint):
    ...
    def select(self, ctx, callback=None):
        ...
        if not self.__battleRoyaleController.isEnabled():
            _logger.error(b'Battle Royale Controller is Disabled')
            ...
            if neededSubModeID is None or neededSubModeID not in BattleRoyaleSubMode.ALL_RANGE:
                _logger.error(b'Trying select sub mode not in BattleRoyaleSubMode.ALL_RANGE %s.', neededSubModeID)
                if callback is not None:
                    callback(False)
                g_prbCtrlEvents.onPreQueueJoinFailure(PRE_QUEUE_JOIN_ERRORS.DISABLED)
                return
...
class BattleRoyaleEntity(PreQueueEntity):
    ...
    def _makeQueueCtxByAction(self, action=None):
        ...
        return BRQueueCtx(self._queueType, invID, currentSubModeID, waitingID=b'prebattle/join')
    ...
    def _doQueue(self, ctx):
        ...
        _logger.debug(b'Sends request on queuing to the Battle Royale - %r', ctx)
        return

    def _doDequeue(self, ctx):
        BigWorld.player().AccountBattleRoyaleComponent.dequeueBattleRoyale()
        _logger.debug(b'Sends request on dequeuing from the Battle Royale')
        return
