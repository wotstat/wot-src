...
class HeaderBlockConstructor(object):
    ...
    def __constructVehicle(cls, vehicle):
        block = []
        block.append(formatters.packAtlasIconTextBlockData(title=text_styles.highTitle(vehicle.userName), atlas=ATLAS_CONSTANTS.COMMON_BATTLE_LOBBY, icon=b'vehicle', iconPadding=formatters.packPadding(right=12), txtGap=1, txtPadding=formatters.packPadding(top=7)))
        return block
...
class ParamsBlockConstructor(object):
    ...
    def construct(module, currentModule, vehicle):
        ...
        if module.itemTypeID != GUI_ITEM_TYPE.VEHICLE:
            ...
            leftPadding = -45
        for item in params:
            bottomPadding = 10 if item.get(b'isLastInGroup', False) else 0
            block.append(formatters.packTextParameterBlockData(name=item[b'description'], value=item[b'value'], valueWidth=110, padding=formatters.packPadding(left=leftPadding, bottom=bottomPadding)))
