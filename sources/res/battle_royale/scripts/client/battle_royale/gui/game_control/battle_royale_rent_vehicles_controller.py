import logging, math, typing, time
...
def _getTillTimeStringByRClass(timeValue, isRoundUp=False):
    ...
    if timeValue >= time_utils.ONE_MINUTE:
        return backport.text(R.strings.battle_royale.status.timeLeft.min(), min=str(tm.tm_min))
    return backport.text(R.strings.battle_royale.status.timeLeft.min(), min=b'1')
...
class BattleRoyaleRentVehiclesController(IBattleRoyaleRentVehiclesController):
    ...
    def getRentState(self, intCD=None):
        ...
        if vehicle is None:
            _logger.error(b'Unable to get rent state for non existing vehicle')
            return
        else:
            if vehicle.isRented:
    ...
    def getFormatedRentTimeLeft(self, intCD=None, isRoundUp=False):
        seconds = self.getRentTimeLeft(intCD)
        if seconds > 0:
            return _getTillTimeStringByRClass(seconds, isRoundUp)
        return b''

    @_defaultCurrentVehicle
    def getPendingRentDays(self, intCD=None):
        if not self.__wasInTestDrive(intCD) or self.isInTestDriveRent(intCD):
            return self.getNextTestDriveDaysTotal(intCD)
        return int(self.getNextRentDaysTotal(intCD))
    ...
    def getNextTestDriveDaysTotal(self, intCD=None):
        ...
        return int(economics.get(b'testDrive', 0))
    ...
    def purchaseRent(self, intCD=None):
        ...
        if state == EquipmentPanelCmpRentStates.STATE_TEST_DRIVE_AVAILABLE:
            ...
            if self.isEnoughMoneyToPurchase(intCD, state=state):
                ...
                _logger.warn(b'not enough money to purchase rent! (%s)', intCD)
    ...
    def __getRentTimeLeft(self, intCD):
        vehicle = self.__itemsCache.items.getItemByCD(intCD)
        if vehicle is None or not vehicle.isRented:
            return 0
        return vehicle.rentLeftTime
    ...
    def __testDriveExpiredTime(self, intCD):
        return self.__itemsCache.items.battleRoyale.testDriveExpired.get(intCD, float(b'inf'))

    def __wasInTestDrive(self, intCD):
        return self.__testDriveExpiredTime(intCD) != float(b'inf')

    def __getEconomicsRent(self, intCD):
        return self.__getEconomics(intCD).get(b'rent', {})

    def __getEconomicsTestDriveRent(self, intCD):
        testDriveRent = self.__getEconomics(intCD).get(b'testDriveRent', None)
        if testDriveRent is None:
            currency = self.__getRentCurrency(intCD)
            if currency is None:
                return {}
            return {b'currency': currency, b'amount': 0}
        else:
            return testDriveRent

    def __getRentCurrency(self, intCD):
        rent = self.__getEconomicsRent(intCD)
        return rent.get(b'currency')
    ...
    def __readRentPrice(self, rent, default=0):
        currency = rent.get(b'currency')
        if currency is None:
            return DynamicMoney()
        else:
            amount = rent.get(b'amount', default)
    ...
    def __getRentPrice(self, intCD):
        ...
        if state == EquipmentPanelCmpRentStates.STATE_TEST_DRIVE_AVAILABLE:
            ...
            if state == EquipmentPanelCmpRentStates.STATE_RENT_AVAILABLE:
                return self.getRentPrice(intCD)
            return
    ...
    def __addEventHandlers(self):
        ...
        g_clientUpdateManager.addCallbacks({(b'stats.{}').format(c): self.__onBalanceUpdate for c in _RENT_CURRENCIES})
        g_clientUpdateManager.addCallbacks({b'battleRoyale': (self.__updateBattleRoyale), 
    ...
    def __calculateVehicleRentTimeUpdate(self, intCD):
        ...
        if timeLeft <= time_utils.ONE_DAY:
            return timeLeft % time_utils.ONE_MINUTE or time_utils.ONE_MINUTE
        return timeLeft % time_utils.ONE_HOUR or time_utils.ONE_HOUR
