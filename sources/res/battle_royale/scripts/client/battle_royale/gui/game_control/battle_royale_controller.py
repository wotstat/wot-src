...
BattleRoyaleProgressionPoints = namedtuple(b'BattleRoyaleProgressionPoints', [b'lastInRange', b'points'])
...
PERFORMANCE_GROUP_LIMITS = {(BattleRoyalePerfProblems.HIGH_RISK): [{(BATTLE_ROYALE_GAME_LIMIT_TYPE.SYSTEM_DATA): {b'osBit': 1, b'graphicsEngine': 0}}, {(BATTLE_ROYALE_GAME_LIMIT_TYPE.HARDWARE_PARAMS): {(HARDWARE_SCORE_PARAMS.PARAM_GPU_MEMORY): 490}}, {(BATTLE_ROYALE_GAME_LIMIT_TYPE.SYSTEM_DATA): {b'graphicsEngine': 0}, (BATTLE_ROYALE_GAME_LIMIT_TYPE.HARDWARE_PARAMS): {(HARDWARE_SCORE_PARAMS.PARAM_RAM): 2900}}], (BattleRoyalePerfProblems.MEDIUM_RISK): [{(BATTLE_ROYALE_GAME_LIMIT_TYPE.HARDWARE_PARAMS): {(HARDWARE_SCORE_PARAMS.PARAM_GPU_SCORE): 300}}, {(BATTLE_ROYALE_GAME_LIMIT_TYPE.HARDWARE_PARAMS): {(HARDWARE_SCORE_PARAMS.PARAM_CPU_SCORE): 50000}}]}
...
class BattleRoyaleController(Notifiable, SeasonProvider, IBattleRoyaleController, IPrbListener):
    ...
    TOKEN_QUEST_ID = b'token:br:title:'
    ...
    def onLobbyInited(self, event):
        ...
        g_clientUpdateManager.addCallbacks({b'battleRoyale': (self.__updateRoyale), 
           b'cache.dynamicCurrencies': (self.__updateDynamicCurrencies)})
    ...
    def getVehicleEquipment(self, vehicleName):
        ...
        if vehicleName not in vehiclesSlotsConfig:
            _logger.error(b"Get equipment for vehicle '%r' failed. Vehicle not found in config. Slots config %s", vehicleName, vehiclesSlotsConfig)
            return []
    ...
    def getTournamentBannerData(self):
        ...
        for bannerData in self.getModeSettings().tournamentsWidget.get(b'widgets', []):
            if bannerData[b'startDate'] <= currentTime < bannerData[b'endDate']:
                return bannerData
    ...
    def __getIntroVideoURL(self):
        if not hasattr(GUI_SETTINGS, b'battleRoyaleVideo'):
            return None
        else:
            introVideoUrl = GUI_SETTINGS.battleRoyaleVideo.get(b'introVideo')
            if introVideoUrl:
    ...
    def __serverSettingsChangeBrowserHandler(self, browser, diff):
        if not diff.get(Configs.BATTLE_ROYALE_CONFIG.value, {}).get(b'isEnabled'):
            browser.onCloseView()
        return
    ...
    def __onChangeProfileAlias(self, event):
        if self.isBattleRoyaleMode() and BATTLE_ROYALE_STATS_ENABLED:
            event.ctx = {b'selectedAlias': (VIEW_ALIAS.PROFILE_STATISTICS)}
        return

    def __onProfileStatisticSelectBattlesType(self, event):
        if not BATTLE_ROYALE_STATS_ENABLED:
            return
        eventOwner = event.ctx.get(b'eventOwner')
        if eventOwner == b'battleRoyale':
            event.ctx[b'battlesType'] = PROFILE_DROPDOWN_KEYS.BATTLE_ROYALE_SOLO
    ...
    def __onProfileTechniqueSelectBattlesType(self, event):
        if not BATTLE_ROYALE_STATS_ENABLED:
            return
        eventOwner = event.ctx.get(b'eventOwner')
        if event.ctx.get(b'eventOwner') == b'battleRoyale':
            event.ctx[b'battlesType'] = PROFILE_DROPDOWN_KEYS.BATTLE_ROYALE_SOLO
    ...
    def __onSelectVehicleInHangar(self, event):
        ...
        prevVehicleInvID = event.ctx[b'prevVehicleInvID']
    ...
    def __getTournamentBannerAvailability(self):
        return self.getModeSettings().tournamentsWidget[b'isWidgetEnabled'] and self.getTournamentBannerData() is not None

    def __getTournamentBannerTimerDelta(self):
        if self.getModeSettings().tournamentsWidget[b'isWidgetEnabled']:
            currentTime = time_utils.getCurrentLocalServerTimestamp()
            prevBannerData = None
            for bannerData in self.getModeSettings().tournamentsWidget.get(b'widgets', []):
                if currentTime < bannerData[b'startDate']:
                    if prevBannerData and currentTime < prevBannerData[b'endDate'] < bannerData[b'startDate']:
                        startDate = prevBannerData[b'endDate']
                    else:
                        startDate = bannerData[b'startDate']
                    return max(0, startDate - time_utils.getCurrentLocalServerTimestamp())
                prevBannerData = bannerData

            if prevBannerData:
                return max(0, prevBannerData[b'endDate'] - time_utils.getCurrentLocalServerTimestamp())
        return 0
