...
PERFORMANCE_GROUP_LIMITS = {(BattleRoyalePerfProblems.HIGH_RISK): [{(BATTLE_ROYALE_GAME_LIMIT_TYPE.SYSTEM_DATA): {b'osBit': 1, b'graphicsEngine': 0}}, {(BATTLE_ROYALE_GAME_LIMIT_TYPE.HARDWARE_PARAMS): {(HARDWARE_SCORE_PARAMS.PARAM_GPU_MEMORY): 490}}, {(BATTLE_ROYALE_GAME_LIMIT_TYPE.SYSTEM_DATA): {b'graphicsEngine': 0}, (BATTLE_ROYALE_GAME_LIMIT_TYPE.HARDWARE_PARAMS): {(HARDWARE_SCORE_PARAMS.PARAM_RAM): 2900}}], (BattleRoyalePerfProblems.MEDIUM_RISK): [{(BATTLE_ROYALE_GAME_LIMIT_TYPE.HARDWARE_PARAMS): {(HARDWARE_SCORE_PARAMS.PARAM_GPU_SCORE): 150}}, {(BATTLE_ROYALE_GAME_LIMIT_TYPE.HARDWARE_PARAMS): {(HARDWARE_SCORE_PARAMS.PARAM_CPU_SCORE): 50000}}]}
...
class BattleRoyaleController(Notifiable, SeasonProvider, IBattleRoyaleController, IGlobalListener):
    ...
    def getDefaultAmmoCount(self, itemKey, intCD=None, vehicleName=None):
        if itemKey in AmmoTypes.SHELLS or itemKey in AmmoTypes.CHARGES:
            ...
            if itemKey == AmmoTypes.ITEM:
                if intCD is None:
                    _logger.error(b'Cannot get count of equipment. Equipment id not specified')
                    return 0
                if vehicleName is None:
                    _logger.error(b"Cannot get count of '%r' equipment. Vehicle not specified", intCD)
                    return 0
                vehiclesSlotsConfig = self.getModeSettings().vehiclesSlotsConfig
                if vehicleName not in vehiclesSlotsConfig:
                    _logger.error(b"Cannot get count of '%r' equipment. Vehicle '%r' not found", intCD, vehicleName)
                    return 0
                ...
                _logger.warning(b"Can't get default ammo count. Unknown item key: '%r'.", itemKey)
            return 0
    ...
    def getBrVehicleEquipmentIds(self, vehicleName):
        vehiclesSlotsConfig = self.getModeSettings().vehiclesSlotsConfig
        if vehicleName not in vehiclesSlotsConfig:
            _logger.error(b"Get equipment for vehicle '%r' failed. Vehicle not found. Slots config %s", vehicleName, vehiclesSlotsConfig)
            return None
        else:
    ...
    def getBrCommanderSkills(self):
        ignoredSkillNames = (b'commander_sixthSense',)
