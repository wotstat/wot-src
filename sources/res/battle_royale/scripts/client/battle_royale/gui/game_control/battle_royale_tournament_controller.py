...
class BattleRoyaleTournamentController(IBattleRoyaleTournamentController):
    ...
    def __onCollectPrebattleInvites(self, autoInvites):
        ...
        parsedTokens = [BattleRoyaleTourmanentToken(token) for token in tokens.getTokens() if token.startswith(b'br_trn') and tokens.isTokenAvailable(token)]
        ...
        for tokenData in availableTokens:
            if counts[key(tokenData)] > 1:
                sessionName = (b'{} [{}]').format(tokenData.tournamentID, tokenData.participantShortDescr)
            else:
            ...
            descr = {b'localized_data': b'', 
               b'descr': {b'event_name': (backport.text(R.strings.battle_royale.tournament.description())), 
                          b'session_name': sessionName}, 
               b'type': (backport.text(typeLabel()))}
            ...
            autoInvites[currentInviteID] = {b'peripheryID': (tokenData.peripheryID), 
               b'type': (PREBATTLE_TYPE.BATTLE_ROYALE_TOURNAMENT), 
               b'startTime': startTimeTimeStamp, 
               b'description': descr, 
               b'isValid': True, 
               b'addInfo': (tokenData.data)}
    ...
    def __joinResult(self, requestID, resultID, errorStr):
        ...
        if resultID == AccountCommands.RES_FAILURE:
            _logger.error(b'joining to battle royale tournament failed with error = %r', errorStr)
    ...
    def __leaveResult(self, requestID, resultID, errorStr):
        if resultID == AccountCommands.RES_FAILURE:
            _logger.error(b'leave from current tournament failed with error = %r', errorStr)
            self.__pushErrorSystemMessage(errorStr)
    ...
    def __readyResult(self, requestID, resultID, errorStr):
        if resultID == AccountCommands.RES_FAILURE:
            _logger.error(b'tournament ready request with failed, str = %r', errorStr)
            self.__pushErrorSystemMessage(errorStr)
        else:
            self.__isReady = True
        return
    ...
    def __notReadyResult(self, requestID, resultID, errorStr):
        if resultID == AccountCommands.RES_FAILURE:
            _logger.error(b'tournament not ready request fini with failed, str = %r', errorStr)
            self.__pushErrorSystemMessage(errorStr)
        else:
            self.__isReady = False
        return
    ...
    def __pushErrorSystemMessage(self, stringCode):
        ...
        SystemMessages.pushI18nMessage(errorStr, type=SystemMessages.SM_TYPE.Warning, priority=b'medium')
        return
