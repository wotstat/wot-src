...
class _DynamicAliases(CONST_CONTAINER):
    SELECT_RESPAWN_SOUND_PLAYER = b'selectRespawnSoundPlayer'
    PROGRESSION_MESSAGES_PLAYER = b'progressionMessagesPlayer'
    RADAR_SOUND_PLAYER = b'radarSoundPlayer'
    LEVEL_SOUND_PLAYER = b'levelSoundPlayer'
    ENEMIES_AMOUNT_SOUND_PLAYER = b'enemiesAmountSoundPlayer'
    PHASE_SOUND_PLAYER = b'phaseSoundPlayer'
    POSTMORTEM_SOUND_PLAYER = b'postmortemSoundPlayer'
    INSTALL_MODULE_SOUND_PLAYER = b'installModuleSoundPlayer'
    ARENA_PERIOD_SOUND_PLAYER = b'arenaPeriodSoundPlayer'
    EQUIPMENT_SOUND_PLAYER = b'equipmentSoundPlayer'
    VEH_UPGRADE_EFFECT_PLAYER = b'vehicleUpgradeEffectPlayer'
    SPAWNED_BOT_MSG_PUBLISHER = b'SpawnedBotMsgPublisher'
    MINEFIELD_MSG_PUBLISHER = b'MinefieldMsgPublisher'
    RESPAWN_PANEL = b'RespawnPanel'
...
class BattleRoyalePage(BattleRoyalePageMeta, ISpawnListener):
    ...
    def __onCameraChanged(self, ctrlMode, _=None):
        ...
        if ctrlMode in teamPanelAliases:
            args = {b'hidden': {BATTLE_VIEW_ALIASES.BATTLE_TEAM_PANEL}, b'visible': {BATTLE_VIEW_ALIASES.PLAYERS_PANEL}}
        else:
            args = {b'hidden': {BATTLE_VIEW_ALIASES.PLAYERS_PANEL}, b'visible': {
                          BATTLE_VIEW_ALIASES.BATTLE_TEAM_PANEL}}
        hideInVideoModeAliases = {BATTLE_VIEW_ALIASES.BATTLE_LEVEL_PANEL,
         BATTLE_VIEW_ALIASES.BR_RESPAWN_MESSAGE_PANEL,
         BATTLE_VIEW_ALIASES.VEHICLE_MESSAGES,
         BATTLE_VIEW_ALIASES.RADAR_BUTTON}
        args[b'hidden' if ctrlMode == CTRL_MODE_NAME.VIDEO else b'visible'].update(hideInVideoModeAliases)
        args[b'hidden' if ctrlMode == CTRL_MODE_NAME.VIDEO or not self.isPostmortemTipsVisible else b'visible'].add(BATTLE_VIEW_ALIASES.POSTMORTEM_PANEL)
