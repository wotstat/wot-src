...
class EquipmentsTooltipData(BlocksTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        if module is None:
            ...
            headerBlockItem = [
             formatters.packBuildUpBlockData(self.__constructHeaderBlock(module, leftPadding, innerBlockLeftPadding), padding=formatters.packPadding(left=leftPadding, right=rightPadding, top=topPadding, bottom=verticalPadding))]
            ...
            if hasattr(module, b'tooltipMovie') and module.tooltipMovie:
    ...
    def __constructCooldownBlock(module, verticalPadding, innerBlockLeftPadding):
        ...
        if cooldownSeconds > 0:
            return formatters.packTextBlockData(text=(b' ').join([text_styles.stats(int(cooldownSeconds)),
             b'  ',
    ...
    def __getCooldownTime(item):
        if hasattr(item, b'cooldownTime'):
