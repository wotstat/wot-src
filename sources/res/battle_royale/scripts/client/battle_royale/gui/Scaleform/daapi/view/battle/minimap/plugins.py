...
import typing, BigWorld, Math, Placement, ArenaInfo, math_utils
...
class BattleRoyalePersonalEntriesPlugin(CenteredPersonalEntriesPlugin):
    ...
    def __getDirectionLineEntryID(self):
        ...
        if _S_NAME.DIRECTION_ENTRY in cameraIDs:
            return cameraIDs[_S_NAME.DIRECTION_ENTRY]
        else:
            return
    ...
    def __addSectorForVehicle(self, vehicle):
        ...
        if sector is None:
            _logger.info(b'Sector can not be initialized now! Sector is None')
        else:
            _logger.info(b'Sector can be initialized now! Sector value=%s', str(sector))
...
_TimeParamsForAs = namedtuple(b'_TimeParamsForAs', b'fadeIn fadeOut lifetime')
    ...
    def _addLootEntry(self, typeId, xzPosition):
        ...
        if lEntryId is not None:
            ...
            if lootTypeParam is None:
                _logger.warning(b'Error in loot entry creation, typeId = %s', str(typeId))
            else:
