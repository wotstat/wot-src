...
RADAR_PLUGIN = b'radar'
DETECTOR_PLUGIN = b'detector'
VEHICLES_PLUGIN = b'vehicles'
AIRDROP_PLUGIN = b'airdrop'
PERSONAL_PLUGIN = b'personal'
PINGING_PLUGIN = b'pinging'
DEATH_ZONES_PLUGIN = b'deathZones'
_logger = logging.getLogger(__name__)

class BattleRoyalePersonalEntriesPlugin(CenteredPersonalEntriesPlugin):
    __slots__ = (b'__viewRangeEntityID', b'__restoreMatrixCbkID')
    ...
    def initControlMode(self, mode, available):
        ...
        if self._isInArcadeMode():
            ...
            if vehicle is not None:
                ...
                _logger.info(b'Initialize sector when vehicle will be created.')
    ...
    def __onVehicleEnterWorld(self, vehicle):
        ...
        if vehicle.id == playerVehId:
            sector = getCircularVisionAngle(vehicle)
            _logger.info(b'Vehicle is created and sector can be initialized now! value=%s', str(sector))
            if sector is not None:
                self.__addSectorEntity(sector)
            else:
                _logger.warning(b'Vehicle has no "circularVisionAngle" property. Sector could not been initialized!')
            self.__clearVehicleHandler()
        return
...
class DeathZonesPlugin(SimplePlugin):
    ...
    def __onDeathZoneUpdated(self, event):
        targetList = []
        deathZones = event.ctx[b'deathZones']
...
class BaseBattleRoyaleEntriesPlugin(EntriesPlugin):
    ...
    def _createEntry(self, uniqueID, entryPosition, symbol, container):
        ...
        if entry is not None:
            ...
            _logger.error(b'Could not create minimap entry, unqiueId: %s', str(uniqueID))
        return entry
    ...
    def _onLootPickedUp(self, event):
        entry = self._entries.get(event.ctx[b'id'])
        if entry is not None:
            self._destroyEntry(event.ctx[b'id'])
        return
...
class DetectorPlugin(BaseBattleRoyaleEntriesPlugin):
    ...
    def __addLootEntry(self, loot):
        ...
        if not (marker and markerBig):
            _logger.warning(b'Could not find marker for loot with typeId: %s', str(loot.typeID))
            return
        else:
...
class RadarPlugin(BaseBattleRoyaleEntriesPlugin, IRadarListener):
    ...
    def __addVehicleEntry(self, vehicleID, vehicleData):
        if self._arenaDP.getPlayerVehicleID() == vehicleID:
            ...
            if vehicleID in self.__visibilitySystemSpottedVehicles:
                _logger.debug(b'Vehicle marker spotted by radar is not displayeddue to vehicle marker spotted by visibility system is still visible!')
                return
    ...
    def __addLootEntry(self, lootID, lootData):
        ...
        if not (marker and markerBig):
            _logger.warning(b'Cannot find marker for loot with typeId = %s', str(typeID))
            return
        else:
    ...
    def __getVehicleEntryName(self, vehicleID):
        ...
        if avatar_getter.isVehiclesColorized():
            return (b'team{}').format(vInfo.team)
        if vInfo.team == 21:
            return b'br_enemy_bot'
        return b'enemy'
...
class AirDropPlugin(EntriesPlugin):
    ...
    def __initMarkers(self):
        for placement in BigWorld.entities.valuesOfType(b'Placement'):
            self.__showMarker(placement.id, placement.position)

        for arenaInfo in BigWorld.entities.valuesOfType(b'ArenaInfo'):
    ...
    def __onAirDropSpawned(self, event):
        self.__showMarker(event.ctx[b'id'], event.ctx[b'position'])
        return
    ...
    def __removeMarker(self, event):
        self._delEntryEx(event.ctx[b'id'])
        return
...
class BattleRoyalStaticMarkerPlugin(IntervalPlugin):
    ...
    def __addStaticMarker(self, areaID, creatorID, position, locationMarkerSubtype, markerText=b'', numberOfReplies=0, isTargetForPlayer=False):
        ...
        if model:
            self._invoke(model.getID(), b'playAnimation')
...
class BattleRoyaleVehiclePlugin(ArenaVehiclesPlugin):
    ...
    def _notifyVehicleAdded(self, vehicleID):
        ...
        if self.__radarSpottedVehiclesPlugin is not None:
            ...
            _logger.warning(b"Couldn't update radar plugin. The reference is None!")
        return
    ...
    def _onMinimapFeedbackReceived(self, eventID, entityID, value):
        if eventID == FEEDBACK_EVENT_ID.MINIMAP_SHOW_MARKER and entityID != self._getPlayerVehicleID() and entityID in self._entries:
            ...
            if entry.isInAoI():
                marker, _ = value
                self._parentObj.invoke(entry.getID(), b'setAnimation', marker)
        return
    ...
    def _setVehicleInfo(self, vehicleID, entry, vInfo, guiProps, isSpotted=False):
        ...
        if avatar_getter.isVehiclesColorized():
            ...
            if arenaBonusType == ARENA_BONUS_TYPE.BATTLE_ROYALE_TRN_SOLO:
                playerName = b''
            if not isBot:
                entryName = (b'team{}').format(vInfo.team)
        self.parentObj.invoke(entry.getID(), b'show', marker, playerName, playerFakeName, playerClan, entryName)
        return
