...
class RespawnMessagePanel(BRRespawnMessagePanelMeta, ISpawnListener, IArenaPeriodController, IArenaVehiclesController, INotificationManagerListener):
    ...
    def __onPeriodChange(self, period, timesGoneFromStart):
        if period == ARENA_PERIOD.BATTLE and not self.__respawnTimestampSent:
            bonusType = self.sessionProvider.arenaVisitor.getArenaBonusType()
            isSquadMode = bonusType in ARENA_BONUS_TYPE.BATTLE_ROYALE_SQUAD_RANGE
            respawnPeriod = (isSquadMode or RespawnAbility()).soloRespawnPeriod if 1 else RespawnAbility().platoonRespawnPeriod
    ...
    def __addMessage(self, idx, message):
        if self.notificationManager:
            if idx in (NotificationType.respActivatedMsg, NotificationType.allyInBattleMsg,
             NotificationType.stayInCoverMsg):
                message[b'showBefore'] = BigWorld.serverTime() + message[b'time'] + message[b'delay']
            else:
                message[b'showBefore'] = BigWorld.serverTime() + ONE_MINUTE
    ...
    def sendMessage(self, message, sendAgain=True):
        if not message:
            ...
            self.__endTime = message[b'time'] + message[b'delay'] + BigWorld.serverTime()
            if not sendAgain:
                return
            self.__delayTime = message[b'delay']
            message[b'delay'] = 0
    ...
    def __onDelayFinished(self):
        ...
        if not self._message:
            ...
            self._message[b'time'] = int(self._calculate())
