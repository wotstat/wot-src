...
class LootPickUpSN(_BRLocalizationProvider, sn_items.TimerSN):
    ...
    def __updateText(self):
        self._vo[b'title'] = backport.text(self._stringResource.loot.pickup(), lootType=self.__getLootType())
        return
...
class BRBuffSN(_BRLocalizationProvider, sn_items.BuffSN):
    ...
    def _updateTimeValues(self, value):
        self._updateTimeParams(value[b'duration'], value[b'endTime'])
        return

    def _isValidForUpdateVisibility(self, value):
        return value[b'duration'] > 0
...
class ShotPassionSN(BRBuffSN):
    ...
    def _update(self, value):
        stage = value.get(b'stage', 0)
        self._vo[b'additionalInfo'] = self.__constructCounter(stage)
        self._vo[b'additionalState'] = self.__getCounterState(stage)
...
class BRHealingSN(_BRLocalizationProvider, sn_items.HealingSN):
    ...
    def _isValidForUpdateVisibility(self, value):
        return value.get(b'duration', 0) > 0.0 and super(BRHealingSN, self)._isValidForUpdateVisibility(value)


class BRHealingCooldownSN(_BRLocalizationProvider, sn_items.HealingCooldownSN):

    def _isValidForUpdateVisibility(self, value):
        return value.get(b'duration', 0) > 0.0 and super(BRHealingCooldownSN, self)._isValidForUpdateVisibility(value)
...
class FireCircleSN(_BRLocalizationProvider, sn_items.SmokeSN):
    ...
    def _update(self, value):
        duration = value.get(b'duration', 0.0) if isinstance(value, dict) else value
        endTime = value.get(b'endTime')
        if duration > 0.0:
            self._setVisible(True)
            self._updateTimeParams(duration, endTime)
            self._sendUpdate()
...
class ThunderStrikeSN(_BRLocalizationProvider, sn_items.SmokeSN):
    ...
    def _update(self, value):
        duration = value.get(b'duration', 0.0)
        if duration > 0.0:
...
class AdaptationHealthRestoreSN(_BRLocalizationProvider, sn_items.TimerSN):
    ...
    def _update(self, value):
        restoreHealth = value.get(b'restoreHealth')
        ...
        if restoreHealth is not None:
            self._vo[b'additionalInfo'] = (b'').join((b'+', str(restoreHealth)))
            if restoreHealth > 0:
