...
class VEHICLE(object):
    ...
    TAKE_TEST_DRIVE = b'takeToRent'
    TAKE_RENT = b'takeToRent'
...
class BRVehicleContextMenuHandler(SimpleVehicleCMHandler):
    ...
    def __init__(self, cmProxy, ctx=None):
        handlers = {(VEHICLE.STATS): b'showVehicleStats', 
           (VEHICLE.TAKE_TEST_DRIVE): b'takeToTestDrive', 
           (VEHICLE.TAKE_RENT): b'takeToRent'}
    ...
    def _generateOptions(self, ctx=None):
        ...
        if vehicle is None:
            ...
            if BATTLE_ROYALE_STATS_ENABLED:
                options.extend([
                 self._makeItem(VEHICLE.STATS, MENU.contextmenu(VEHICLE.STATS), {b'enabled': True})])
            ...
            if rentState != EquipmentPanelCmpRentStates.STATE_NORMAL:
                ...
                if rentState in testDriveStates:
                    ...
                    options.extend([
                     self._makeItem(VEHICLE.TAKE_TEST_DRIVE, text, {b'enabled': (rentState == EquipmentPanelCmpRentStates.STATE_TEST_DRIVE_AVAILABLE)})])
                    ...
                    options.extend([
                     self._makeItem(VEHICLE.TAKE_RENT, text, {b'enabled': isEnabled})])
            return options
    ...
    def showVehicleStats(self):
        shared_events.showVehicleStats(self.getVehCD(), b'battleRoyale')
        return
