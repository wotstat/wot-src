...
def getTillBlock(isSeasonStarted, tillEnd, timeLeft, leftPadding=0):
    ...
    return formatters.packTextBlockData(text_styles.main(backport.text(tillEndStr() if tillEnd else tillStartStr())) + b' ' + text_styles.stats(backport.getTillTimeStringByRClass(timeLeft, R.strings.menu.headerButtons.battle.types.ranked.availability)), padding=formatters.packPadding(left=leftPadding))
