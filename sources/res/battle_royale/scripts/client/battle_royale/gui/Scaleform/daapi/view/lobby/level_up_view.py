...
class BattleRoyaleLevelUpView(BattleRoyaleLevelUpViewMeta):
    ...
    def _populate(self):
        ...
        battleRoyaleInfo = self.__ctx[b'reusableInfo'].personal.getBattleRoyaleInfo()
        title, _ = battleRoyaleInfo.get(b'accBRTitle', (None, None))
        prevTitle, _ = battleRoyaleInfo.get(b'prevBRTitle', (None, None))
        ...
        data = {b'awards': awardsVO, 
           b'awardsSmall': awardsSmallVO, 
           b'epicMetaLevelIconData': (after_battle_reward_view_helpers.getProgressionIconVODict(cycleNumber, title)), 
           b'levelUpText': lvlReachedText, 
           b'backgroundImageSrc': (backport.image(R.images.gui.maps.icons.battleRoyale.backgrounds.back_congrats())), 
           b'maxLvlReached': (self.__maxLvlReached)}
        self.as_setDataS(data)
        return

    def __getBonuses(self, level):
        questsProgressData = self.__ctx[b'reusableInfo'].personal.getQuestsProgress()
