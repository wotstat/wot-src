...
class LootDetector(Notifiable):
    ...
    def __updateParams(self):
        ...
        if not vehicle.isAlive():
            return False
        coneVisibilityComponent = vehicle.dynamicComponents.get(b'coneVisibility')
    ...
    def __detectLoots(self):
        loots = {e for e in BigWorld.entities.valuesOfType(b'Loot') if pointSectorIntersection(self.__observationPoint, self.__viewDirection, self.__sectorRadius, self.__sectorAngle, Math.Vector2(e.position.x, e.position.z))} if self.__updateParams() else set()
