...
class SpawnedBotMsgPlayerMsgs(IProgressionListener, IViewComponentsCtrlListener):
    SPAWNED_BOT_DESTROYED_ID = b'SPAWNED_BOT_DESTROYED'
    ...
    def setVehicleChanged(self, vehicle, newModuleIntCD, vehicleRecreated):
        if not self.__initialized and b'observer' not in vehicle.descriptor.type.tags:
            ...
            if self.__started:
                ...
                if arena:
                    ...
                    _logger.error(b"'%s' cannot subscribe to arena events because arena is not ready yet or already destroyed", SpawnedBotMsgPlayerMsgs.__name__)
            else:
                _logger.info(b'Vehicle can not spawn bots, that is why no need to initialize %s', self)
        return
    ...
    def __onArenaVehicleKilled(self, targetID, attackerID, equipmentID, reason, numVehiclesAffected):
        ...
        if killedVehicle is None:
            ...
            if killedVehicle.getMasterVehID() == playerVehicleId:
                vehicleName = killedVehicle.typeDescriptor.name.split(b':')[1]
                msgStr = (b'').join((vehicleName, b'_DESTROYED'))
