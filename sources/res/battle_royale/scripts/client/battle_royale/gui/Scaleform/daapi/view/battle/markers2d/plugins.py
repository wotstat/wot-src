...
class BattleRoyaleVehicleMarkerPlugin(VehicleMarkerPlugin):
    ...
    def _onVehicleFeedbackReceived(self, eventID, vehicleID, value):
        ...
        if vehicleID not in self._markers:
            ...
            if eventID == FEEDBACK_EVENT_ID.VEHICLE_REPAIR_POINT:
                if value.get(b'duration') is None:
    ...
    def _updateStatusMarkerState(self, vehicleID, isShown, handle, statusID, duration, animated, isSourceVehicle, blinkAnim=True):
        ...
        if isShown:
            self._invokeMarker(handle, b'showStatusMarker', statusID, self._getMarkerStatusPriority(statusID), isSourceVehicle, duration, currentlyActiveStatusID, self._getMarkerStatusPriority(currentlyActiveStatusID), animated, blinkAnim)
        else:
            self._invokeMarker(handle, b'hideStatusMarker', statusID, currentlyActiveStatusID, animated)
        return

    def _getGuiPropsName(self, vInfo, guiProps):
        isBot = vInfo.team == 21
        if guiProps.isFriend:
            entryName = b'ally' if isSpawnedBot(vInfo.vehicleType.tags) else b'squadman'
        else:
            entryName = b'enemy'
    ...
    def __onCameraChanged(self, cameraName, currentVehicleId=None):
        if currentVehicleId is None:
            ...
            elif self.__hasRepairingMarker(currentVehicleId):
    ...
    def __updateRepairingMarker(self, vehicleID, isShow=True):
        ...
        if repairData is not None:
            ...
            if markerID == marker.getMarkerID():
                duration = value.get(b'duration', 0) if isShow else 0.0
