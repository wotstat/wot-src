...
class HangarVehicleModulesConfigurator(VehicleModulesConfiguratorCmp):
    ...
    def _updateColumns(self):
        ...
        while columnIdx < totalColumns:
            ...
            if not columnVo[b'selected']:
                columnVo[b'selected'] = False
...
class HangarVehicleInfo(BattleRoyaleVehicleInfoMeta, IGlobalListener):
    ...
    def _populate(self):
        ...
        self.as_setDataS({b'btnCloseLabel': (backport.text(R.strings.battle_royale.hangarVehicleInfo.closeBtn())), 
           b'infoIconSource': (backport.image(R.images.gui.maps.icons.library.info())), 
           b'infoText': (text_styles.highlightText(backport.text(R.strings.battle_royale.hangarVehicleInfo.moduleTreeTip(), key=text_styles.neutral(br_helpers.getHotKeyString(CommandMapping.CMD_UPGRADE_PANEL_SHOW))))), 
           b'vehTitle': (text_styles.grandTitle(self.__vehicle.shortUserName)), 
           b'vehTypeIcon': (getTypeBigIconPath(self.__vehicle.type)), 
           b'tutorialText': (backport.text(R.strings.battle_royale.hangarVehicleInfo.tutorialText()))})
        return
