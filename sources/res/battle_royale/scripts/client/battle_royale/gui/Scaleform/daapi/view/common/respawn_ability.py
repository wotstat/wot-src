...
class RespawnAbility(object):
    ...
    soloRespawnPeriod = property((lambda self: int(self.getParams(ARENA_BONUS_TYPE.BATTLE_ROYALE_SOLO, b'respawnPeriod'))))
    soloTimeToRessurect = property((lambda self: self.getParams(ARENA_BONUS_TYPE.BATTLE_ROYALE_SOLO, b'timeToRessurect')))
    platoonRespawnPeriod = property((lambda self: int(self.getParams(ARENA_BONUS_TYPE.BATTLE_ROYALE_SQUAD, b'respawnPeriod'))))
    platoonTimeToRessurect = property((lambda self: self.getParams(ARENA_BONUS_TYPE.BATTLE_ROYALE_SQUAD, b'timeToRessurect')))
    ...
    def getParams(cls, battleType, key):
        ...
        if config:
            return config[key]
        _logger.warning(b'RespawnAbility have no config or no key %s in config', key)
        return
