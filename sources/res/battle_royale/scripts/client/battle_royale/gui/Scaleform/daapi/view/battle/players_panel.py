...
class PlayersPanel(IBattleFieldListener, IArenaVehiclesController, ISpawnListener, BattleRoyaleTeamPanelMeta):
    __slots__ = (b'__vehicleIDs', b'__selectedByVehicleIds', b'__spawnPointsViewActive', b'__isRespawning', b'__respawningDelayCallbackID')
    ...
    def updateVehiclesInfo(self, updated, arenaDP):
        for _, vInfoVO in updated:
            if vInfoVO.vehicleID in self.__vehicleIDs:
                ...
                _logger.debug(b'updateRespawnTime as_setPlayerStatusS %s %s %s %s', index, vInfoVO.isAlive(), vInfoVO.isReady(), self.__isRespawning)
    ...
    def updateRespawningStatus(self, timeLeft):
        ...
        if not self.__vehicleIDs:
            _logger.warning(b'PlayersPanel vehicleIDs empty')
            return
        else:
            arenaDP = self.__sessionProvider.getArenaDP()
            ...
            _logger.debug(b'updateRespawnTime as_setPlayerStatusS %s %s %s %s', 0, vInfoVO.isAlive(), vInfoVO.isReady(), self.__isRespawning)
