...
class BattleUpgradePanel(BattleUpgradePanelMeta, IArenaVehiclesController, IProgressionListener, INotificationManagerListener):
    __slots__ = (b'__level', b'__upgrades', b'__localVisible', b'__isEnabled', b'__attentionEffect', b'notificationManager')
    ...
    def setVehicleChangeResponse(self, itemCD, success):
        if success:
            ...
            self.__sessionProvider.shared.messages.onShowPlayerMessageByKey(b'VEHICLE_UPGRADE', {b'module': (getTreeModuleHeader(item)), 
               b'moduleType': (backport.text(moduleKey()) if moduleKey else b'')})
    ...
    def __updateUpgrades(self, addToManager=True):
        ...
        if upgrades:
            ...
            if hasTwoModules:
                ...
                data = {b'firstItem': (self.__getModuleInfo(first, 0)), b'secondItem': (self.__getModuleInfo(second, 1)), 
                   b'title': (backport.text(titleKey()))}
                if not self.__textInited:
                    key = br_helpers.getHotKeyString(CommandMapping.CMD_UPGRADE_PANEL_SHOW)
                    data[b'description'] = backport.text(R.strings.battle_royale.upgradePanel.description(), key=key)
                ...
                logger.warning(b'Unexpected modules count. Modules list: %s', str(upgrades))
        else:
            self.__updateVisibility(False, addToManager)
        return

    def __getModuleInfo(self, moduleItem, index):
        moduleInfo = {b'header': (getTreeModuleHeader(moduleItem)), b'parameters': (getShortListParameters(moduleItem, self._getVehicle())), 
           b'module': {b'icon': (getTreeModuleIcon(moduleItem)), 
                       b'intCD': (moduleItem.intCD), 
                       b'available': True}}
        if not self.__textInited:
            moduleInfo[b'hotKeys'] = br_helpers.getHotKeyListByIndex(index)
            moduleInfo[b'hotKeysVKeys'] = br_helpers.getHotKeyVkListByIndex(index)
        return moduleInfo
