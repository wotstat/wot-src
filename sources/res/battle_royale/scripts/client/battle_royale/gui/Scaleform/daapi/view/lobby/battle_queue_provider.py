...
class BattleRoyaleQueueProvider(RandomQueueProvider):
    ...
    def processQueueInfo(self, qInfo):
        ...
        for mode in constants.BattleRoyaleMode.ALL:
            modesData.append({b'type': (backport.text(R.strings.menu.prebattle.battleRoyale.dyn(mode)())), 
               b'icon': (backport.image(R.images.battle_royale.gui.maps.icons.battleQueue.dyn(mode)())), 
    ...
    def getAdditionalParams(self):
        ...
        return {b'battleTypeIconPath': iconPath, 
           b'subTitle': (backport.text(titleRes()) if titleRes is not None and titleRes.exists() else b'')}
