        ...
        if isSpawnedBot(vInfoVO.vehicleType.tags):
            ...
            if botVehicle:
                ...
                for vI in self.__sessionProvider.getArenaDP().getVehiclesInfoIterator():
                    ...
                    defBotName = b' '
                    _logger.warning(b'The Master vehicle of a bot (id=%s) has not been found, use default name %s', botVehID, defBotName)
                ...
                defBotName = b' '
                _logger.warning(b'Bot vehicle (id=%s) has not been found, use default name %s', botVehID, defBotName)
                vInfoVO = VehicleArenaInfoVO(botVehID)
                return super(BattleRoyalePlayerFullNameFormatter, self).format(vInfoVO, playerName=defBotName)
