...
class LSArenaPhasesComponent(DynamicScriptComponent):
    ...
    def set_activePhase(self, prev):
        _logger.info(b'LSArenaPhasesComponent set activePhase=%s', self.activePhase)
    ...
    def set_isBCMarkersCleanupEnabled(self, prev):
        _logger.info(b'LSArenaPhasesComponent set isBCMarkersCleanupEnabled=%s', self.isBCMarkersCleanupEnabled)
    ...
    def getInstance():
        if not isPlayerAvatar():
            ...
            if not player.arena or not player.arena.arenaInfo:
                return
            return getattr(player.arena.arenaInfo, b'LSArenaPhasesComponent', None)

    def _onBattleLoading(self, event):
        if event.ctx.get(b'isShown'):
            self._setupEnvironment(True)
        return
