...
class _ExtConsumables(object):
    __slots__ = (b'layout', b'installed', b'isAutoEquip')
...
class LSInventorySessionCache(object):
    ...
    def _onClientSynchronize(cls, isFullSync, diff):
        ...
        dataResetKey = (cls.DATA_KEY, b'_r')
...
class LSAccountEquipmentController(BigWorld.StaticScriptComponent):
    ...
    def getVehicleLayout(self, vehInvID):
        return self.inventory.get(b'layout', {}).get(vehInvID, self.EMPTY_LAYOUT)

    def getVehicleInventory(self, vehInvID):
        return self.inventory.get(b'eqs', {}).get(vehInvID, [])

    def getVehicleSettings(self, vehInvID):
        return self.inventory.get(b'settings', {}).get(vehInvID, {})

    def isVehicleAutoEquipEnabled(self, vehInvID):
        return self.getVehicleSettings(vehInvID).get(b'autoMaintenance', last_stand_constants.LS_DEFAULT_AUTO_MAINTENANCE)
