...
class BuyBundleProcessor(Processor):
    ...
    def __pushSuccessMessage(self):
        ...
        messageData = {b'title': (backport.text(R.strings.last_stand_system_messages.serviceChannelMessages.bundlePurchase.title())), 
           b'description': (backport.text(R.strings.last_stand_system_messages.serviceChannelMessages.bundlePurchase.description(), bundle=bundleName)), 
           b'additionalText': (backport.text(R.strings.last_stand_system_messages.serviceChannelMessages.bundlePurchase.additionalText(), money=formatPrice(Money(**{(self._price.currency): (self._price.amount * self._count)}), useStyle=True)))}
        SystemMessages.pushMessage(b'', type=SM_TYPE.lsPurchaseBundleForGold, priority=NotificationPriorityLevel.MEDIUM, messageData=messageData)
        return
