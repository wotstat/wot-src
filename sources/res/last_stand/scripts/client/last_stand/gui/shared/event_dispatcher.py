...
def _getUrl(urlName=None, url=None, lobbyContext=None):
    ...
    if url:
        ...
        return hostUrl + (b'' if urlName is None else GUI_SETTINGS.lookup(b'lsShop').get(urlName))
...
def showRewardPathView(openFromQuestCard=False):
    ...
    LastStandRewardPathState.goTo(ctx={b'openFromQuestCard': openFromQuestCard})
    return
...
def showModuleInfo(itemCD, vehicleDescr):
    itemCD = int(itemCD)
    g_eventBus.handleEvent(events.LoadViewEvent(SFViewLoadParams(LAST_STAND_HANGAR_ALIASES.LS_MODULE_INFO, _getModuleInfoViewName(itemCD, vehicleDescr)), ctx={b'moduleCompactDescr': itemCD, 
