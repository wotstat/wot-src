...
class LSItemBuyProcessorMessage(ItemBuyProcessorMessage):
    ...
    def _getMsgCtx(self):
        ...
        res[b'kind'] = backport.text(R.strings.last_stand_system_messages.ls_equipment.title())
        return res
...
class LSBuyAndInstallConsumablesProcessor(BuyAndInstallConsumablesProcessor):
    ...
    def _successHandler(self, code, ctx=None):
        additionalMessages = []
        if ctx:
            additionalMessages = [LSItemBuyProcessorMessage(self.itemsCache.items.getItemByCD(cd), count, Money.makeFromMoneyTuple(price)).makeSuccessMsg() for cd, price, count in ctx.get(b'eqs', [])]
...
class LSBuyAndInstallConsumables(BuyAndInstallConsumables):
    __slots__ = (b'__confirmOnlyExchange',)
