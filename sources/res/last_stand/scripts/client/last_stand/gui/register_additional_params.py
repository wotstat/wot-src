...
def _registerLSOpenTabWebApi():
    ...
    OpenTabWebApi.addTabIdCallback(b'last_stand', _selectLSMode)
    return
...
def registerAdditionalParams(personality):
    ...
    registerLobbyTooltipsBuilders([
     (
      b'last_stand.gui.scaleform.daapi.view.tooltips.tooltip_builders', LS_TOOLTIP_SET)])
...
def registerConditionFormatterIcons():
    ...
    BATTLE_RESULTS_KEYS.update({b'ls_pickedUpSouls': b'ls_pickedUpSouls', 
       b'ls_phasesCompleted': b'ls_phasesCompleted', 
       b'ls_killsByTeam': (CONDITION_ICON.KILL_VEHICLES)})
    return
