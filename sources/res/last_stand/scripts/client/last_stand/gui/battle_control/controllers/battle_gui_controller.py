...
class LSBattleGoal(object):
    UNKNOWN = None
    DESTROY_ENEMIES = b'last_stand.destroyEnemies'
    STOP_CONVOY = b'last_stand.stopConvoy'
    CONVOY_DESTROYED_ENEMIES_LEFT = b'last_stand.convoyDestroyedEnemiesLeft'
    WAVE_FINISHED = b'last_stand.waveFinished'
    LAST_WAVE_FINISHED = b'last_stand.lastWaveFinished'
...
class LSBattleGUIController(LSBattleGUIControllerBase):
    ...
    def updateConvoyStatus(self, convoyStatus):
        ...
        if convoyStatus and all(data[b'isDead'] for data in convoyStatus):
            self._convoyStatus = None
    ...
    def updateHealthBreakpoints(self, healthBreakpoints):
        self._healthBreakpoints = []
        if self._enemiesInfo.get(b'totalHealth', 0) > 0:
            self._healthBreakpoints = [decimal_round(float(value) / self._enemiesInfo[b'totalHealth'], 2) for value in healthBreakpoints]
    ...
    def _getRelevantGoal(self):
        if not self._getAliveAllyVehicles() or self._enemiesInfo.get(b'totalEnemies', 0) <= 0:
            return LSBattleGoal.UNKNOWN
        if self._convoyDestroyed and self._enemiesInfo.get(b'aliveEnemies', 0) > 0:
            return LSBattleGoal.CONVOY_DESTROYED_ENEMIES_LEFT
        if self._convoyStatus:
            return LSBattleGoal.STOP_CONVOY
        if self._enemiesInfo.get(b'aliveEnemies', 0):
    ...
    def _getHintParams(self):
        return {b'num': (self._enemiesInfo.get(b'aliveEnemies', 0))}
