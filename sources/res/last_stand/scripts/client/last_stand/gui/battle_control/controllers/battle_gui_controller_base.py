...
class LSBattleGUIControllerBase(IArenaVehiclesController):
    ...
    def _showRelevantBattleHint(self, prevGoal, newGoal):
        if self.battleHintsCtrl is None:
            _logger.warning(b'BattleHintsController is not loaded. Battle hint (%s) will not be displayed.', newGoal)
            return False
        else:
            ...
            if not success and not BattleReplay.g_replayCtrl.isPlaying:
                _logger.warning(b'Hint (%s) will not be displayed. Will retry on the next update.', newGoal)
            return success
