...
class LSEquipmentItemBase(DynComponentsGroupItem):
    ...
    def inCriticalCondition(self, avatar=None):
        ...
        if avatar.isVehicleDrowning:
            return (True, _ActivationError(b'LS_vehicleIsDrowning', {b'name': (self._descriptor.userString)}))
        else:
            return (
             False, None)
...
class LSEquipmentItem(LSEquipmentItemBase):
    ...
    def canActivate(self, entityName=None, avatar=None):
        ...
        if vehicle.lsSoulsContainer.souls < self._getUsageCost(vehicle):
            return (False, _ActivationError(b'LS_notEnoughSouls', {b'name': (self._descriptor.userString)}))
        return (result, error)
...
class _HpRepairAndCrewHeal(LSEquipmentItem):
    ...
    def canActivate(self, entityName=None, avatar=None):
        ...
        if vehicle.health >= vehicle.maxHealth and not deviceStates and not avatar_getter.isVehicleStunned():
            return (False, _ActivationError(b'LS_selfRepair', {b'name': (self._descriptor.userString)}))
...
class LSDamageShield(_AbilityWithDuration):
    _BLOCKED_COMPONENT = (b'LS_obeliskShield',)
    ...
    def inCriticalCondition(self, avatar=None):
        ...
        if inCritical:
            ...
            if avatar.fireInVehicle:
                return (True, _ActivationError(b'LS_vehicleIsOnFire', {b'name': (self._descriptor.userString)}))
            vehicle = BigWorld.entities[avatar.playerVehicleID]
            if b'lsFire' in vehicle.dynamicComponents:
                return (True, _ActivationError(b'LS_vehicleIsOnLSFire', {b'name': (self._descriptor.userString)}))
            if avatar.isVehicleOverturned:
                return (True, _ActivationError(b'LS_vehicleIsOverturned', {b'name': (self._descriptor.userString)}))
            return (False, None)
...
class _FastReload(_AbilityWithDuration):
    INSTANT_RELOAD = b'LS_instantReload'
    DEVICES = (b'gun',)
    ERROR_NAME = b'LS_gunDestroyed'
        ...
        if not result:
            ...
            if ammoCtrl.getGunSettings().isCassetteClip:
                ...
                if quantityInClip >= quantity:
                    return (False, _ActivationError(b'LS_instantReload', {b'name': (self._descriptor.userString)}))
            ...
            return (
             False,
             _ActivationError(b'LS_instantReload', {b'name': (self._descriptor.userString)}))
