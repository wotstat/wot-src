...
def slotsPlayerSortKey(slot):
    ...
    return (not player, roleIndex, player.get(b'timeJoin', 0))
    ...
    if isinstance(entity, UnitEntity):
        ...
        for slotInfo in unitFullData.slotsIterator:
            ...
            for slot in slots:
                ...
                if player is None:
                    continue
                if slotInfo.player.dbID != player[b'dbID']:
                    continue
                ...
                additionalMsg = backport.text(R.strings.last_stand_platoon.platoon.difficulty.maxAvailable(), icon=(b'level_{0}').format(QUEUE_TYPE_TO_DIFFICULTY_LEVEL[maxAvailableQueue].value))
                slot.update({b'additionalMsg': additionalMsg, 
                   b'isVisibleAdtMsg': True})
