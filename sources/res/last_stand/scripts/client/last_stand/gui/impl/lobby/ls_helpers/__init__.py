...
class UseHeaderNavigationImpossible(object):
    __slots__ = (b'_hide', b'_show', b'_confirmationHelper')
...
def getBoosterFactorsParam(boosterName, ctrl=None):
    ...
    boosterFactors = next((booster for booster in lsBoosters if booster.get(b'name') == boosterName), None)
    result = {}
    for factor in boosterFactors.get(b'factors', []):
        if factor[b'operation'] == BOOSTER_FACTOR_OPERATIONS.ADD:
            result[factor[b'key'] + b'_' + factor[b'operation']] = getNiceNumberFormat(abs(factor[b'value']))
        elif factor[b'operation'] == BOOSTER_FACTOR_OPERATIONS.ADD_PERCENT:
            if factor[b'key'] not in BOOSTER_FACTOR_NAMES.RETENTION_FACTORS:
                result[factor[b'key'] + b'_' + factor[b'operation']] = getNiceNumberFormat(abs(factor[b'value'] * 100))
            else:
                result[factor[b'key'] + b'_' + factor[b'operation']] = getNiceNumberFormat(abs(1 + factor[b'value']) * 100)
