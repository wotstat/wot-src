...
class LSTokenBoosterFormatter(TokenBonusFormatter):
    ...
    def _getFormattedBonus(self, tokenID, token, bonus):
        ...
        return PreformattedBonus(bonusName=bonus.getName(), label=b'', images={(AWARDS_SIZES.SMALL): (backport.image(icons.c_68x68.dyn(boosterName)())), 
...
class LSCustomizationsBonusFormatter(CustomizationsBonusFormatter):
    ...
    def _getOverlayType(cls, item):
        if cls._getItemTypeName(item) == b'attachment':
            result = {}
    ...
    def _createCustomizationBonus(self, bonus, item, data):
        ...
        return PreformattedBonus(bonusName=bonus.getName(), images=self._getImages(c11nItem), userName=self._getUserName(c11nItem), label=self._formatBonusLabel(item.get(b'value')), labelFormatter=self._getLabelFormatter(bonus), isSpecial=True, specialAlias=TOOLTIPS_CONSTANTS.TECH_CUSTOMIZATION_ITEM_AWARD, specialArgs=CustomizationTooltipContext(itemCD=data.get(b'intCD')), isCompensation=self._isCompensation(bonus), align=LABEL_ALIGN.RIGHT, itemTypeName=self._getItemTypeName(c11nItem), overlayType=self._getOverlayType(c11nItem))
...
def getLSMetaFormattersMap():
    ...
    mapping.update({b'items': (LSItemsBonusFormatter()), 
       (Currency.CREDITS): (LSCreditsBonusFormatter()), 
       b'vehicles': (LSVehiclesBonusFormatter()), 
       b'battleToken': (LSTokenBonusFormatter()), 
       ...
       b'dossier': (LSDossierBonusFormatter()), 
       b'goodies': (LSGoodiesBonusFormatter()), 
       b'battlePassPoints': (LSBattlePassBonusFormatter()), 
       b'crewBooks': (LSCrewBooksBonusFormatter()), 
       b'customizations': (LSCustomizationsBonusFormatter())})
    return mapping


def getLSMetaAwardsFormattersMap():
    mapping = getLSMetaFormattersMap()
    mapping.update({b'vehicles': (LSVehiclesAwardsBonusFormatter()), 
       b'customizations': (LSAwardsCustomizationsBonusFormatter())})
    return mapping
...
class LSBonusesAwardsComposer(CurtailingAwardsComposer):
    ...
    def _packMergedBonuses(self, mergedBonuses, size=AWARDS_SIZES.SMALL):
        ...
        return PreformattedBonus(bonusName=b'default', label=backport.text(R.strings.last_stand_lobby.reward.rest(), count=mergedBonusCount), images=imgs, isSpecial=True, specialAlias=TOOLTIPS_CONSTANTS.ADDITIONAL_AWARDS, specialArgs=self._getShortBonusesData(mergedBonuses, size), userName=b'')
