...
VEHICLE_MARKER = b'LSVehicleMarkerUI'
...
class LSVehicleMarkerPlugin(RespawnableVehicleMarkerPlugin):
    ...
    def _getActiveCommandTime(self, action):
        if action == LS_OBELISK_VEH_MARKER:
            return LS_OBELISK_ACTIVE_TIME
        return super(LSVehicleMarkerPlugin, self)._getActiveCommandTime(action)
    ...
    def _onClearObeliskMarker(self, vehicleID):
        if vehicleID not in self._markers:
            return
        marker = self._markers[vehicleID]
        self._invokeMarker(marker.getMarkerID(), b'stopActionMarker')
    ...
    def __onVehicleBuffIconAdded(self, vehicleId, icon):
        ...
        if marker is not None:
            self._invokeMarker(marker.getMarkerID(), b'showEnemyBuff', icon)
        return
    ...
    def __onVehicleInvulnerabilityChanged(self, vehicleID, isInvulnerable):
        ...
        if marker is not None:
            ...
            self._invokeMarker(marker.getMarkerID(), b'setHealthBarMode', isHPVisible)
        return
