...
CREATE_LS_SQUAD = b'createLSSquad'
...
def createLSSquadHandler(cm, ctrl=None):
    if not ctrl.isEventPrb():
        ...
        yield ctrl.prbDispatcher.doSelectAction(action, fadeCtx={b'layer': (WindowLayer.OVERLAY), 
           b'waitForLayoutReady': (R.views.last_stand.mono.lobby.hangar())})
    else:
        arenaUniqueID = getattr(cm, b'arenaUniqueID', None)
        cm.doSelect(PREBATTLE_ACTION_NAME.LAST_STAND_SQUAD, (cm.databaseID,), extData={b'arenaUniqueID': arenaUniqueID})
    return
...
def lsSquadOptionBuilder(cm, options, userCMInfo, ctrl=None):
    ...
    if not ctrl.isAvailable():
        ...
        if not cm.isSquadAlreadyCreated(PREBATTLE_TYPE.LAST_STAND):
            lsSquadItem = cm.makeItem(CREATE_LS_SQUAD, backport.text(R.strings.last_stand_menu.contextMenu.createLastStandSquad()), optInitData={b'enabled': (not cm.prbEntity.isInQueue()), b'textColor': HIGHLIGHT_COLOR})
            if squadItem:
