...
def lsCopyTo(ribbonType, isMergeCopy=False):
    ...
    def decorator(cls):
        setattr(cls, b'LSIsMergeCopy', isMergeCopy)
...
class _LSEquipmentActivatedRibbon(_LSGameplayActionRibbon):
    EQUIPMENT_TO_RIBBON = {b'LS_nitroSituational': (LS_BATTLE_EFFICIENCY_TYPES.LS_NITRO_ACTIVATED)}
        ...
        for destEffType, sourceClasses in self.RIBBON_TYPES_COPY_TO.items():
            ...
            for r in destRibbons:
                for other in sourceRibbons:
                    if r.aggregate(other) and getattr(other, b'LSIsMergeCopy', False):
