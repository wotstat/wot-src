...
class LSConsumablesPanel(ConsumablesPanelMeta):
    ...
    def __init__(self):
        ...
        self._shellsCount = {}
        self._lsEquipmentDataCache = {b'usageCost': {}, b'duration': {}}
        return
    ...
    def _initIndexes(cls):
        cls._LS_EQUIPMENT_START_IDX, cls._LS_EQUIPMENT_END_IDX, cls._AMMO_START_IDX, cls._AMMO_END_IDX = _PRESETS_CUSTOM_INDEXES.get(getSettings(AccountSettingsKeys.AMMO_PANEL_PRESET), b'default')
        return
    ...
    def _addLSEquipmentSlot(self, idx, intCD, item):
        ...
        if self._isLSEmptySlot(item):
            self.as_addAbilitySlotS(idx=idx, keyCode=bwKey, sfKeyCode=sfKey, quantity=0, timeRemaining=0, reloadingTime=0, iconPath=b'', tooltipText=EMPTY_EQUIPMENT_TOOLTIP)
            return
        else:
            ...
            if not self._isLSPassive(item):
                ...
                self.as_addPassiveAbilitySlotS(idx=idx, iconPath=iconPath, state=b'green', tooltipText=toolTip)
    ...
    def _handleEquipmentPressed(self, intCD, entityName=None, idx=None):
        ...
        if ctrl is None:
            ...
            if item is None or item.getActivationCode() is None and b'avatar' not in item.getDescriptor().tags:
                return
    ...
    def _onUpdateLSEquipmentTimer(self):
        ctrl = self.sessionProvider.shared.equipments
        if ctrl is not None:
            for intCD, item in ctrl.iterEquipmentsByTag(b'LS_equipment'):
                if intCD not in self._cds:
                    continue
                idx = self._cds.index(intCD)
    ...
    def _onVehicleStateUpdated(self, state, value):
        ...
        if ctrl is None:
            ...
            if state == VEHICLE_VIEW_STATE.DEVICES:
                ...
                isCrewUpdated = itemName in TANKMEN_ROLES_ORDER_DICT[b'enum']
                equipmentTag = b'medkit' if isCrewUpdated else b'repairkit'
    ...
    def _isAnyEquipmentModuleInState(self, equipment, checkDeviceStates):
        ...
        if b'extinguisher' in equipmentTags:
            correction = True
            entityName = None
    ...
    def _isLSEquipment(item):
        return b'LS_equipment' in item.getDescriptor().tags

    @classmethod
    def _isLSEmptySlot(cls, item):
        return cls._isLSEquipment(item) and b'LS_emptySlot' in item.getDescriptor().tags
    ...
    def _isLSSituational(cls, item):
        return b'LS_situational' in item.getDescriptor().tags

    @staticmethod
    def _isSuperShell(item):
        return b'lsSuperShell' in item.getDescriptor().tags

    @staticmethod
    def _hasUsageCost(item):
        return hasattr(item.getDescriptor(), b'usageCost')

    def _getUsageCost(self, intCD, variant, useCache=True):
        if useCache and intCD in self._lsEquipmentDataCache[b'usageCost']:
            return self._lsEquipmentDataCache[b'usageCost'][intCD]
        ...
        self._lsEquipmentDataCache[b'usageCost'][intCD] = cost
        return cost

    def _getDurationSeconds(self, intCD, variant, useCache=True):
        if useCache and intCD in self._lsEquipmentDataCache[b'duration']:
            return self._lsEquipmentDataCache[b'duration'][intCD]
        ...
        self._lsEquipmentDataCache[b'duration'][intCD] = duration
        return duration
