from __future__ import absolute_import
import Event
from collections import namedtuple
from functools import cmp_to_key
from past.builtins import cmp
from future.utils import viewvalues, viewkeys, viewitems
from adisp import adisp_async
from gui import SystemMessages
from gui.shared.utils import decorators
from gui.ClientUpdateManager import g_clientUpdateManager
from gui.server_events.bonuses import getNonQuestBonuses, TokensBonus
from last_stand.gui.shared.gui_items.processors.processors import BuyBundleProcessor
from last_stand.skeletons.ls_controller import ILSController
from last_stand.skeletons.ls_shop_controller import ILSShopController
from last_stand_common.last_stand_constants import ShopSettings, ProgressPointsSettings
from helpers import dependency
from skeletons.gui.server_events import IEventsCache
Price = namedtuple(b'Price', (b'currency', b'amount'))

class Bundle(namedtuple(b'Bundle', (b'bundleID', b'groupID', b'orderInGroup', b'descrGroupKey', b'limit', b'isWebShopBundle', b'price', b'oldPrice', b'url', b'bonuses'))):

    def getCtx(self):
        return dict(self._asdict())


class LSShopController(ILSShopController):
    eventsCache = dependency.descriptor(IEventsCache)
    lsCtrl = dependency.descriptor(ILSController)

    def __init__(self):
        super(LSShopController, self).__init__()
        self.onShopSettingsUpdated = Event.Event()
        self.onBundlesUpdated = Event.Event()
        self._bundles = {}
        return

    def init(self):
        g_clientUpdateManager.addCallbacks({b'tokens': (self.__handleTokensUpdate)})
        self.lsCtrl.onSettingsUpdate += self.__updateSettings
        return

    def fini(self):
        g_clientUpdateManager.removeObjectCallbacks(self)
        self.lsCtrl.onSettingsUpdate -= self.__updateSettings
        self.onShopSettingsUpdated.clear()
        self.onBundlesUpdated.clear()
        self._bundles = {}
        return

    def keyBundles(self):
        return sorted(viewvalues(self._bundles), key=cmp_to_key((lambda b1, b2: cmp(b1.bundleID, b2.bundleID))))

    def getBundleByID(self, bundleID):
        return self._bundles.get(bundleID)

    def getProgressPointsInBundle(self, bundleID):
        bundle = self._bundles.get(bundleID)
        if not bundle:
            return 0
        for bonus in bundle.bonuses:
            if not isinstance(bonus, TokensBonus):
                continue
            tokens = bonus.getTokens()
            for token in viewvalues(tokens):
                if token.id != ProgressPointsSettings.TOKEN:
                    continue
                return token.count

        return 0

    def isEnabled(self):
        return self._getConfig().get(b'enabled', False)

    def onLobbyStarted(self, ctx):
        super(LSShopController, self).onLobbyStarted(ctx)
        self._initBundles()
        return

    @adisp_async
    @decorators.adisp_process(b'buyItem')
    def purchaseBundle(self, bundleID, count, callback):
        bundle = self.getBundleByID(bundleID)
        result = yield BuyBundleProcessor(bundleID, bundle.price, count).request()
        if result.userMsg:
            SystemMessages.pushMessage(result.userMsg, type=result.sysMsgType)
        callback(result.success)
        return

    def getPurchaseCount(self, bundleID):
        return self.eventsCache.questsProgress.getTokenCount(bundleID + ShopSettings.PURCHASED_SUFFIX)

    def checkIsEnoughBundles(self):
        for bundle in self.keyBundles():
            if bundle.limit is not None:
                limit = bundle.limit - self.getPurchaseCount(bundle.bundleID)
                if limit > 0:
                    return True

        return False

    def _initBundles(self):
        self._bundles = dict((bundleID, Bundle(bundleID, int(self._getBundleGroupID(bundleID)), int(self._getBundleOrderInGroup(bundleID)), self._getBundleDescrGroupKey(bundleID), self._getBundleLimit(bundleID), self._isWebShopBundle(bundleID), Price(*self._getPrice(bundleID, b'price')), Price(*self._getPrice(bundleID, b'oldPrice')), self._getBundleUrl(bundleID), self._getBonuses(bundleID))) for bundleID in viewkeys(self._getBundles()))
        return

    def _getBundleGroupID(self, bundleID):
        return self._getBundles().get(bundleID, {}).get(b'groupId')

    def _getBundleOrderInGroup(self, bundleID):
        return self._getBundles().get(bundleID, {}).get(b'orderInGroup')

    def _getBundleDescrGroupKey(self, bundleID):
        return self._getBundles().get(bundleID, {}).get(b'descrGroupKey')

    def _getBundleLimit(self, bundleID):
        return self._getBundles().get(bundleID, {}).get(b'limit')

    def _getBundleUrl(self, bundleID):
        return self._getBundles().get(bundleID, {}).get(b'url')

    def _isWebShopBundle(self, bundleID):
        return self._getBundles().get(bundleID, {}).get(b'isWebShopBundle', True)

    def _getPrice(self, bundleID, priceKey):
        return self._getBundles().get(bundleID, {}).get(priceKey, (None, 0))

    def _getBundles(self):
        return self._getConfig().get(b'bundles', {})

    def _getBonuses(self, bundleID):
        rewards = []
        bonuses = self._getBundles().get(bundleID, {}).get(b'bonus', {})
        for bonusType, bonusValue in viewitems(bonuses):
            rewards.extend(getNonQuestBonuses(bonusType, bonusValue))

        return rewards

    def _getConfig(self):
        return self.lsCtrl.getModeSettings().shop

    def __updateSettings(self):
        self._initBundles()
        self.onShopSettingsUpdated()
        return

    def __handleTokensUpdate(self, diff):
        for token in diff:
            if token.startswith(ShopSettings.SHOP_BUNDLE_PREFFIX):
                self.onBundlesUpdated()

        return
