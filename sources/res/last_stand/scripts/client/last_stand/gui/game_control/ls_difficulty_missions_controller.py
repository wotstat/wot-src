...
class DifficultyMission(namedtuple(b'DifficultyMission', (b'missionID', b'bonusRewards', b'difficulty', b'index', b'isCompleted'))):
...
def getFormattedMissionsList(missionsIdx):
    return (b'|').join(str(idx - 1) for idx in missionsIdx)
...
class LSDifficultyMissionsController(ILSDifficultyMissionsController):
    ...
    def getIndexes(self, missionID):
        _, difficulty, index = missionID.split(b':')
