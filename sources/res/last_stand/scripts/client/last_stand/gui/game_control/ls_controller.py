...
class _LSConfig(namedtuple(b'_LSConfig', (b'isEnabled', b'isBattlesEnabled', b'startDate', b'endDate', b'artefactsSettings', b'vehicles', b'shop', b'isPromoScreenEnabled', b'isIntroVideoEnabled', b'isInfoPageEnabled', b'isMetaInfoEnabled', b'isLootBoxEntryPointEnabled', b'isHangar3dPointVisible', b'isHangar3dPointRewardVisible', b'prominentBonus', b'modeSelectorShowRewards', b'lsBoostersConfig', b'metaConfigs', b'storyPointsList', b'isParallaxEnabled', b'isObeliskRadialMenuEnabled', b'createVivoxTeamChannels'))):
    __slots__ = ()
...
_VehiclesConfig = namedtuple(b'_VehiclesConfig', (b'allowedVehicles', b'allowedLevels', b'forbiddenClassTags', b'forbiddenVehicles'))
...
class LSController(ILSController, Notifiable, SeasonProvider, IGlobalListener, PerformanceAnalyzerMixin):
    ...
    def getVehiclesConfig(self):
        ...
        return _VehiclesConfig(limits.get(b'allowedVehicles', []), limits.get(b'allowedLevels', []), limits.get(b'forbiddenClassTags', []), limits.get(b'forbiddenVehicles', []))
    ...
    def __selectMode(self, name):
        ...
        if dispatcher is None:
            ...
            yield dispatcher.doSelectAction(PrbAction(name), fadeCtx={b'layer': (WindowLayer.OVERLAY), 
               b'waitForLayoutReady': (R.views.last_stand.mono.lobby.hangar() if isLastStand else None)})
            return

    @staticmethod
    def __getQueueTypeLocalizedName(queueType):
        difficultyLevel = QUEUE_TYPE_TO_DIFFICULTY_LEVEL[queueType].value
        return backport.text(R.strings.last_stand_lobby.difficult.dyn((b'level_{}').format(difficultyLevel))())
