...
class LastStandEntryPoint(SquadEntryPoint):
    ...
    def makeDefCtx(self):
        return LastStandSquadSettingsCtx(waitingID=b'prebattle/create', accountsToInvite=self._accountsToInvite, arenaUniqueID=self.arenaUniqueID)

    def setExtData(self, extData):
        self.arenaUniqueID = extData.get(b'arenaUniqueID')
        return
...
class LastStandSquadEntity(SquadEntity):
    ...
    def _setDifficultyLevel(self, ctx, callback=None):
        ...
        if not pPermissions.canChangeRosters():
            LOG_ERROR(b'Player can not change difficulty level', pPermissions)
            if callback:
                callback(False)
            return
        self._requestsProcessor.doRequest(ctx, b'doUnitCmd', CLIENT_UNIT_CMD.SET_LS_UNIT_DIFFICULTY_LEVEL, ctx.getQueueType(), 0, b'', callback=callback)
    ...
    def _updateEntityType(self, *args, **kwargs):
        ...
        if not pInfo.isCommander():
            return
        ctx = SetDifficultyLevelUnitCtx(self.currentQueueType, waitingID=b'prebattle/change_settings')
