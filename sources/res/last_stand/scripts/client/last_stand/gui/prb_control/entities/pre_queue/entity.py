...
class LastStandEntity(PreQueueEntity):
    ...
    def _doQueue(self, ctx):
        ...
        LOG_DEBUG(b'Sends request on queuing to the  LastStand battles', self.currentQueueType, ctx)
        return

    def _doDequeue(self, ctx):
        self._accountComponent.dequeueBattle(self.currentQueueType)
        LOG_DEBUG(b'Sends request on dequeuing from the LastStand battles', self.currentQueueType)
        return
    ...
    def _makeQueueCtxByAction(self, action=None):
        return LastStandQueueCtx(g_currentVehicle.item.invID, entityType=self.currentQueueType, waitingID=b'prebattle/join')
