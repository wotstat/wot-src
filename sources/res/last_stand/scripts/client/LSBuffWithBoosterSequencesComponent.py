...
@groupComponent(sequences=ListParam(valueParam=ObjParam(sequence=StrParam(), bindNode=StrParam(), offset=Vector3Param(), loopCount=IntParam(default=-1), autoStart=BoolParam(default=True), visibleTo=StrParam(default=b'all'), sniperModeVisibleTo=StrParam(default=b'all'), checkNodeExists=BoolParam(default=False), boosterFactors=StrParam(default=_DEFAULTS))))
