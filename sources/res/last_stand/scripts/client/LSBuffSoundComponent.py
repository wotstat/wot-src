...
@groupComponent(soundEvents=ListParam(valueParam=ObjParam(activationSound3D=StrParam(), deactivationSound3D=StrParam(), playTo=StrParam(default=b'self'), activationSound2D=StrParam(), deactivationSound2D=StrParam())))
class LSBuffSoundComponent(DynamicScriptComponent):
    ...
    def soundObject(self):
        comp = self.entity.dynamicComponents.get(b'LSVehicleSoundComponent')
        if comp:
