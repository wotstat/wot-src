...
class FunProgressionRewardsBaseFormatter(ServiceChannelFormatter, TokenQuestsSubFormatter, FunAssetPacksMixin, FunProgressionWatcher):
    __INFO_TEMPLATE = b'InformationHeaderSysMessage'
    __PROGRESSION_STAGE_TEMPLATE = b'FunRandomProgressionStage'
    ...
    def _format(self, message, *_):
        ...
        completedQuestIDs = self.getQuestOfThisGroup(messageData.get(b'completedQuestIDs', set()))
    ...
    def _formatProgressionCompletion(self, progression, rewardsFmt):
        ...
        if resetText:
            return text_styles.concatStylesToMultiLine(messageText, b'', resetText)
        else:
            return messageText
    ...
    def _formatSingleQuestCompletion(self, qInfo, rewards):
        ...
        if messageText:
            return MessageData(g_settings.msgTemplates.format(template, {b'header': messageHeader, b'text': messageText}), self._getGuiSettings(None, key=template, priorityLevel=priority, decorator=decorator))
        else:
...
class FunRandomLootBoxFormatter(QuestAchievesFormatter, FunAssetPacksMixin):
    ...
    def _processTokens(cls, tokens):
        result = []
        tokensData = tokens.get(b'tokens', {})
        ...
        for token in sortedTokens:
            if token.startswith(LOOTBOX_TOKEN_PREFIX):
                ...
                if lootBox and lootBox.getCategory() == FEP_CATEGORY:
                    ...
                    result.append(g_settings.htmlTemplates.format(b'funRandomLootBox', {b'text': lbName, b'count': (tokensData[token].get(b'count', 1))}))
...
class FunModeItemsQuestAsyncFormatter(AsyncTokenQuestsSubFormatter, FunAssetPacksMixin):
    ...
    def _format(self, message, *_):
        messageData = message.data or {}
        messageDataList = []
        for qID in self.getQuestOfThisGroup(messageData.get(b'completedQuestIDs', set())):
    ...
    def _formatModeItemsSingleQuest(self, rewards):
        ...
        messageData = g_settings.msgTemplates.format(template, {b'header': messageHeader, b'text': messageText})
