...
_INFO_HEADER_TEMPLATE = b'InformationHeaderSysMessage'
_NEW_SUB_MODES_TEMPLATE = b'FunRandomNewSubModes'
_WARNING_HEADER_TEMPLATE = b'WarningHeaderSysMessage'
...
class FunRandomNotificationsFormatter(ServiceChannelFormatter, FunAssetPacksMixin, FunProgressionWatcher, FunSubModesWatcher):
    ...
    def format(self, message, *_):
        ...
        ctx = {b'header': messageHeader, b'text': messageText}
        if messageText and isEnabledByLUI:
            return [MessageData(g_settings.msgTemplates.format(template, ctx=ctx, data={b'savedData': {b'message': message}}), self._getGuiSettings(None, key=template, priorityLevel=priority, decorator=decorator))]
        else:
            return []
    ...
    def _getSubModesText(cls, subModesIDs, multipleTextRes, singleTextRes):
        ...
        if not subModes:
            _logger.error(b'Empty sub modes. Check hasAnySubMode defence.')
            return None
        else:
            if len(subModes) > 1:
