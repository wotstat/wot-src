...
class FunRandomMessageAwardsComposer(CurtailingAwardsComposer):
    ...
    def _packBonus(self, bonus, size=AWARDS_SIZES.SMALL):
        ...
        if size == AWARDS_SIZES.S232X174:
            bonusDict[b'padding'] = MAIN_REWARD_PADDING
            bonusDict[b'gap'] = MAIN_REWARD_GAP
            if bonus.align == LABEL_ALIGN.RIGHT:
                bonusDict[b'label'] = bonus.getFormattedLabel(text_styles.highTitle)
        elif size == AWARDS_SIZES.SMALL:
            bonusDict[b'gap'] = SMALL_REWARD_GAP
    ...
    def _packMergedBonuses(self, mergedBonuses, size=AWARDS_SIZES.SMALL):
        ...
        if size == AWARDS_SIZES.SMALL:
            bonusesDict[b'gap'] = SMALL_REWARD_GAP
        return bonusesDict
    ...
    def __packCompensationData(self, bonus, bonusDict, size):
        ...
        if vehicle:
            ...
            specialArgs = {b'labelBefore': b'', 
               ...
               b'labelAfter': (bonus.getFormattedLabel()), 
               b'bonusName': (bonus.bonusName), 
               b'countBefore': 1, 
            ...
            uiData[b'vehicleType'] = (b'{}_elite').format(formattedTypeName) if vehicle.isElite else formattedTypeName
            ...
            bonusDict[b'isWulfTooltip'] = True
            bonusDict[b'tooltip'] = TOOLTIPS_CONSTANTS.VEHICLE_COMPENSATION
            bonusDict[b'specialArgs'] = [listitems(specialArgs)]
            bonusDict[b'overlayType'] = b'vehicleCompensation_%s' % size
            bonusDict[b'hasCompensation'] = False
        return


class FunRandomLootboxAutoOpenFormatter(AsyncAutoLootBoxSubFormatter, FunAssetPacksMixin):
    __MESSAGE_TEMPLATE = b'FunRandomLootBoxesAutoOpenMessage'
    ...
    def format(self, message, callback):
        ...
        if isSynced:
            ...
            formattedRewards = g_settings.msgTemplates.format(self.__MESSAGE_TEMPLATE, ctx={b'header': (self.getModeUserName()), 
               b'rewardText': (backport.text(R.strings.fun_random.notification.lootboxAutoOpen.rewardText()))}, data={b'linkageData': (self._collectBonusesData(data))})
    ...
    def _collectBonusesData(self, data):
        ...
        for lootBoxID, lootBoxData in viewitems(data):
            if self._isBoxOfThisGroup(lootBoxID):
                ...
                rewards = lootBoxData.get(b'rewards', {})
        ...
        if len(legendaryFormatted) == 1:
            return {b'mainReward': (first(legendaryFormatted)), 
               b'rewards': (composer.getFormattedBonuses(self.__getRawBonuses(otherRewards), AWARDS_SIZES.SMALL)), 
               b'bgIcon': bgIcon}
        else:
            ...
            if len(mainFormatted) == 1:
                return {b'mainReward': (first(mainFormatted)), 
                   b'rewards': [], b'bgIcon': bgIcon}
            return {b'mainReward': None, 
               b'rewards': (composer.getFormattedBonuses(rawBonuses, AWARDS_SIZES.SMALL)), 
               b'bgIcon': bgIcon}
            return
