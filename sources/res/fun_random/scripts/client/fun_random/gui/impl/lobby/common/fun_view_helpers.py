...
FUN_RANDOM_MAPPING = {b'tokens': FunRandomLootBoxTokenBonusPacker, 
   b'lootBox': FunRandomLootBoxTokenBonusPacker, 
...
def getConditionText(rootStrPath, levels):
    battleCondition = rootStrPath.dyn(b'battleCondition')
    components = [backport.text(battleCondition()) if battleCondition.exists() else b'']
    levelCondition = rootStrPath.dyn(b'levelCondition')
...
def getCompensatedFunRandomBonusPacker():
    mapping = getDefaultBonusPackersMap()
    mapping.update(FUN_RANDOM_MAPPING)
    mapping.update({b'vehicles': FunRandomLootBoxVehiclesBonusUIPacker})
    return BonusUIPacker(mapping)
...
def getFunRandomSpecialBonusPacker():
    mapping = getDefaultBonusPackersMap()
    mapping.update(FUN_RANDOM_MAPPING)
    mapping.update({b'tokens': FunRandomRewardLootBoxTokenBonusPacker, 
       b'lootBox': FunRandomRewardLootBoxTokenBonusPacker})
    return BonusUIPacker(mapping)


def defineProgressionStatus(progression):
    if progression is not None:
        return _PROGRESSION_STATUS_MAP[progression.state.isCompleted, progression.state.isLastProgression, progression.hasUnlimitedProgression]
    else:
