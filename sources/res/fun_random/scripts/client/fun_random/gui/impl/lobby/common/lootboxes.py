...
FEP_CATEGORY = b'FEPLootBoxes'

class FunRandomLootBoxTypes(object):
    ORDINARY = b'fep_ordinary'
    UNUSUAL = b'fep_unusual'
    RARE = b'fep_rare'
    EPIC = b'fep_epic'
    LEGENDARY = b'fep_legendary'
    ALL = (
...
class FunRandomLootBoxTokenBonusPacker(TokenBonusUIPacker, FunAssetPacksMixin):
    ...
    def __packLootBox(cls, tokenID, model, bonus):
        ...
        rarity = lootBox.getType().split(b'_')[-1]
        ...
        model.setValue(str(bonus.getValue().get(tokenID, {}).get(b'count', 0)))
        return model
...
def packLootboxes(lootboxes, lbConfig, lootboxesModel, packer, visibleLBAwardsNames, localeRes, tooltips=None):
    ...
    for lb in lootboxes:
        ...
        if lootBoxData:
            ...
            lootboxModel.setIconKey(lootboxType.split(b'_')[-1])
...
def packLootboxRewards(lootBoxData, rewardsModel, packer, tooltipData=None):
    ...
    if lootBoxData:
        ...
        for data in parseBonusesWithProbabilities(lootBoxData):
            for bonus, probability in zip(data[b'bonuses'], data[b'probabilities']):
            ...
            for item in items:
                if item and len(item) == 4:
                    ...
                    if rawData:
                        ...
                        bonuses.append({b'probabilities': probabilities, 
                           b'bonuses': rawDataBonuses})
