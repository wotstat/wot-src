...
class FunProgressionQuestPostBattleInfo(QuestPostBattleInfo, FunProgressionWatcher):
    ...
    def _getProgresses(self, pCur, pPrev):
        ...
        if stage is not None and prevCounter < currCounter:
            return [{b'progrTooltip': None, b'progrBarType': (formatters.PROGRESS_BAR_TYPE.SIMPLE), b'currentProgrVal': (currCounter - stage.prevRequiredCounter), b'maxProgrVal': (stage.requiredCounter - stage.prevRequiredCounter), b'description': (backport.text(descriptionRes(), triggerDescription=self.event.getDescription())), b'progressDiff': (b'+ %s' % backport.getIntegralFormat(currCounter - prevCounter)), b'progressDiffTooltip': (TOOLTIPS.QUESTS_PROGRESS_EARNEDINBATTLE)}]
        else:
            return []
    ...
    def _getStatus(self, pCur=None):
        if self.event.isCompleted(progress=pCur):
            return (MISSIONS_STATES.COMPLETED, backport.text(R.strings.quests.details.status.completed()))
        return (
         MISSIONS_STATES.NONE, b'')
