...
class FunRandomEntryPoint(PreQueueEntryPoint):
    ...
    def select(self, ctx, callback=None):
        ...
        if not self.__funRandomController.isEnabled():
            _logger.warning(b'Trying to get into fun random when FEP feature is disabled.')
            ...
            _logger.error(b'Trying to get into invalid fun random sub mode %s.', desiredSubModeID)
            self.__abortSelection(PRE_QUEUE_JOIN_ERRORS.DISABLED, callback)
        elif not desiredSubMode.isAvailable():
            _logger.debug(b'Trying to get into fun random sub mode %s when it is not available.', desiredSubModeID)
...
class FunRandomEntity(PreQueueEntity, FunAssetPacksMixin):
    ...
    def _doQueue(self, ctx):
        ...
        _logger.debug(b'Sends request for queuing to the fun event battle %s', ctx)
        return

    def _doDequeue(self, ctx):
        BigWorld.player().AccountFunRandomComponent.dequeueFunRandom()
        _logger.debug(b'Sends request for dequeueing from the fun event battle')
        return
    ...
    def _makeQueueCtxByAction(self, action=None):
        ...
        return FunRandomQueueCtx(invID, desiredSubModeID, waitingID=b'prebattle/join')
