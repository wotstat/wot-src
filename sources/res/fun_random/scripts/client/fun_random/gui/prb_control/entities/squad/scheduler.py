...
class FunRandomSquadScheduler(FunRandomScheduler):
    ...
    def notifySubModeSwitching(self, subModeID):
        SystemMessages.pushMessage(backport.text(self._FUN_RES_ROOT.notification.squadModeSwitch(), **self._getMessageParams(subModeID)), messageData={b'header': (backport.text(self._FUN_RES_ROOT.notification.squadModeSwitch.title()))}, type=SystemMessages.SM_TYPE.InformationHeader)
        return
