...
class FunRandomSquadEntryPoint(SquadEntryPoint):
    ...
    def select(self, ctx, callback=None):
        ...
        if not self.__funRandomController.isEnabled():
            _logger.warning(b'Trying to create fun random squad when FEP feature is disabled.')
            self.__abortSelection(UNIT_ERROR.OFF_SEASON, callback)
        elif desiredSubModeID == UNKNOWN_EVENT_ID or desiredSubMode is None:
            _logger.error(b'Trying to create fun random squad of invalid fun random sub mode %s.', desiredSubModeID)
            self.__abortSelection(UNIT_ERROR.BAD_PARAMS, callback)
        elif not desiredSubMode.isSquadAvailable():
            _logger.debug(b'Trying to create fun random squad of unavailable fun random sub mode %s.', desiredSubModeID)
...
class FunRandomSquadEntity(SquadEntity):
    ...
    def init(self, ctx=None):
        ...
        g_clientUpdateManager.addCallbacks({b'inventory.1': (self.__onInventoryVehiclesUpdated)})
    ...
    def changeFunSubMode(self, ctx, callback=None):
        if not self._isInCoolDown(REQUEST_TYPE.CHANGE_FUN_SUB_MODE, coolDown=ctx.getCooldown()):
            self._requestsProcessor.doRequest(ctx, b'doCustomUnitCmd', CLIENT_UNIT_CMD.CHANGE_FUN_EVENT_ID, uint64Arg=ctx.getDesiredSubModeID(), callback=callback)
