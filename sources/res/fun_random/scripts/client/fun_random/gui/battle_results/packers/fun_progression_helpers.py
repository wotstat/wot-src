...
_START_STAGE = 0
_PbsProgress = namedtuple(b'_PbsProgress', (b'description', b'isUnlimitedProgression', b'bonuses', b'previousStage', b'currentStage', b'maximumStage', b'previousPoints', b'currentPoints', b'maximumPoints', b'earnedPoints', b'stageRequiredCounters'))

class _PostbattleProgressionHelper(object):
    __slots__ = (b'_triggers', b'_altTriggers', b'_executors', b'_countersProgress')
    ...
    def _getPoints(self):
        return (
         sum(data.get(b'diff', 0) for data in viewvalues(self._countersProgress)),
         sum(data.get(b'total', 0) for data in viewvalues(self._countersProgress)))
