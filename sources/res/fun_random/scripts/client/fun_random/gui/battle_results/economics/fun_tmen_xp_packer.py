...
class FunTmenXpPacker(CurrencyPacker):
    ...
    def packModel(cls, model, currencyType, battleResults):
        ...
        if isPremiumAdvertisingShown(currencyType, battleResults):
            ...
            premFactor = premRecords.getFactor(b'appliedPremiumTmenXPFactor100')
    ...
    def _getEarnedConfig(cls, battleResults):
        ...
        if reusable.personal.avatar.hasPenalties():
            ...
            tmenXps = personalResults.xpProgress.get(intCD, {}).get(b'xpByTmen', [])
            if not tmenXps:
                return cls._EARNED
            records = []
            for _, tman in vehicle.crew:
                if tman is None:
                    continue
                records.append(CurrencyRecord(recordNames=(), subtractRecords=(), baseAccountValueExtractor=partial(_getTmenXpValue, tID=tman.invID, xpList=tmenXps), premiumAccountValueExtractor=partial(_getTmenXpValue, tID=tman.invID, xpList=tmenXps), detailsValuesExtractors=(), capsToBeChecked=None, paramName=b'tmenXP', label=R.strings.ingame_gui.tankmen.dyn(tman.role), modifiers=(), showZeroValue=False, currencyType=CurrenciesConstants.TMEN_XP))
