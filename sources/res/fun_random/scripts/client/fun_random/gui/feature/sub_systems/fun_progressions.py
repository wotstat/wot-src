...
class FunProgressions(IFunRandomController.IFunProgressions, Notifiable):
    ...
    def __buildProgression(self, pConfig, isFirst, isLast, quests, tokens):
        ...
        triggers = tuple(quests.get(PROGRESSION_TRIGGER_TEMPLATE.format(pConfig.name, trigger[b'id'])) for trigger in pConfig.triggers)
        for t in triggers:
            if t is not None:
        ...
        if pConfig.unlimitedTrigger and pConfig.unlimitedExecutor:
            ...
            uTrigger = quests.get(PROGRESSION_UNLIMITED_TRIGGER_TEMPLATE.format(pConfig.name, pConfig.unlimitedTrigger[b'id']))
        ...
        if newProgression and newProgression.state.isLastProgression:
            ...
            for name in progressionNames:
                for trigger in triggers:
                    triggersQuests.append(quests.get(PROGRESSION_TRIGGER_TEMPLATE.format(name, trigger[b'id'])))
