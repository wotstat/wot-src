...
class FunSubModeConfigurationProvider(FunBaseConfigurationProvider):
    ...
    _EMPTY_CONFIGURATION_PATH = b'fun_random/gui/configs/gamemodes/fun_sub_modes/fun_sub_mode_empty.xml'

    def getConfigurationModel(self):
        return self._configurationModel

    def _getRuntimeConfiguration(self, subMode, *args):
        return {b'subMode': {b'assetsPointer': (subMode.getAssetsPointer()), 
                        b'pbsEfficiency': (subMode.getSettings().client.postbattle.get(b'postbattleEfficiency', {}))}}
