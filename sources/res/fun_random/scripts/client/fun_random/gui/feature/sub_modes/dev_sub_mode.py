...
class FunDevSubMode(FunBaseSubMode):
    ...
    def __repr__(self):
        return (b'FunDevSubMode id={}, isEnabled={}').format(self._settings.eventID, self._settings.isEnabled)

    def clearNotification(self):
        super(FunDevSubMode, self).clearNotification()
        _logger.info(b'%s cleared all inner timers', self)
        return
    ...
    def startNotification(self):
        ...
        _logger.info(b'%s starts inner timers for status tracking', self)
        return

    def stopNotification(self):
        super(FunDevSubMode, self).stopNotification()
        _logger.info(b'%s stops inner timers for status tracking', self)
        return

    def _updateSettings(self, subModeSettings):
        _logger.info(b'%s received new settings %s', self, subModeSettings)
        return super(FunDevSubMode, self)._updateSettings(subModeSettings)

    def _subModeStatusTick(self):
        _logger.info(b'%s triggering SUB_STATUS_TICK event', self)
        super(FunDevSubMode, self)._subModeStatusTick()
        return

    def _subModeStatusUpdate(self):
        _logger.info(b'%s triggering SUB_STATUS_UPDATE event', self)
