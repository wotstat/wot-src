...
class FunModeConfigurationProvider(FunBaseConfigurationProvider):
    ...
    _EMPTY_CONFIGURATION_PATH = b'fun_random/gui/configs/gamemodes/fun_modes/fun_mode_empty.xml'
    _UNDEFINED_ASSETS_PACK_PATH = b'fun_random/gui/configs/gamemodes/fun_modes/assets_packs/fun_assets_undefined.xml'

    def __init__(self, *args):
        super(FunModeConfigurationProvider, self).__init__(b'', *args)
        return
    ...
    def _getRuntimeConfiguration(self, settings, *args):
        ...
        configuration = {b'mode': {b'assetsPack': {b'assetsPointer': (settings.assetsPointer)}}}
        deepMergeDicts(configuration, {b'mode': {b'assetsPack': assetsPackConfig}})
