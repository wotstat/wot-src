...
class FunProgressionQuestsHandler(ServiceChannelHandler, FunProgressionWatcher):
    ...
    def _needToShowAward(self, ctx):
        if super(FunProgressionQuestsHandler, self)._needToShowAward(ctx):
            return any(self._funRandomCtrl.progressions.isProgressionExecutor(qID) for qID in _getMessage(ctx).data.get(b'completedQuestIDs', set()))
        return False
    ...
    def __showAward(self):
        ...
        for lootboxID, lootboxData in viewitems(self.__lootBoxData):
            ...
            if lb and lb.getCategory() == FEP_CATEGORY:
                ...
                awardList.append(lootboxData.get(b'rewards', {}))

        if legendaryRewards:
            data = {b'lootBoxType': (FunRandomLootBoxTypes.LEGENDARY), b'mainRewards': (getMergedCompensatedBonuses(legendaryRewards)), 
               b'addRewards': (getMergedCompensatedBonuses(otherRewards))}
