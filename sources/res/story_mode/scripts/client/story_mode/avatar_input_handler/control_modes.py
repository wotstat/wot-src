...
class StoryModeArcadeCamera(ArcadeCamera):
    ...
    def create(self, onChangeControlMode=None, postmortemMode=False, smartPointCalculator=True):
        prevStartDist = self._cfg[b'startDist']
        prevStartAngle = self._cfg[b'startAngle']
        self._cfg[b'startDist'] = self._START_DISTANCE
        self._cfg[b'startAngle'] = math.radians(self._START_ANGLE) - math.pi * 0.5
        super(StoryModeArcadeCamera, self).create(onChangeControlMode, postmortemMode, smartPointCalculator)
        self._cfg[b'startDist'] = prevStartDist
        self._cfg[b'startAngle'] = prevStartAngle
        return
...
class StoryModeArcadeControlModeStartCamera(ArcadeControlMode):
    ...
    def _setupCamera(self, dataSection):
        self._cam = StoryModeArcadeCamera(dataSection[b'camera'], defaultOffset=self._defaultOffset)
        return
...
class SMEntityViewMode(IControlMode):
    ...
    def __init__(self, dataSection, _):
        self._cam = ArcadeCamera(dataSection[b'camera'], dataSection.readVector2(b'defaultOffset'))
        self._entityID = None
    ...
    def handleKeyEvent(self, isDown, key, mods, event=None):
        ...
        if reconItem is not None:
            ...
            if stage in (STAGES.ACTIVATING, STAGES.ACTIVE, STAGES.DEACTIVATING):
                args = {b'name': (reconItem.getDescriptor().userString)}
                if CommandMapping.g_instance.isActiveList(self._MOVE_CMDS):
                    self._showMessage(b'ability_unable_to_move', args)
                    return False
                if CommandMapping.g_instance.isActive(CommandMapping.CMD_CM_SHOOT):
                    self._showMessage(b'ability_unable_to_fire', args)
                    return False
