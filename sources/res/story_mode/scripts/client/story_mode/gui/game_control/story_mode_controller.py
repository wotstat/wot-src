...
class StoryModeController(IStoryModeController, IGlobalListener):
    ...
    def quitBattle(self):
        ...
        _logger.debug(b'quitBattle')
        self.__isQuittingBattle = True
        if not player:
            _logger.error(b'quitBattle: player is not available')
            return
        else:
    ...
    @staticmethod
    def goToBattle():
        _logger.debug(b'goToBattle')
        if not isPlayerAvatar():
            _logger.error(b'goToBattle method can be called only for Avatar.')
            return
        else:
    ...
    def goToHangar(self, guiCtx=None):
        ...
        if not isAccount or isInQueue:
            _logger.error(b'goToHangar method cant be called: isAccount=%s, isInQueue=%s', isAccount, isInQueue)
            return
        else:
            ...
            guiCtx.update({b'inQueue': (QUEUE_TYPE.UNKNOWN), 
               b'skipHangar': False})
    ...
    def onOutroVideoComplete(self, arenaUniqueID):
        guiCtx = self._lobbyContext.getGuiCtx()
        if guiCtx.get(b'skipHangar', False) and guiCtx.get(b'showOutroVideo', False):
            self._delayedBattleResultsID = arenaUniqueID
            BigWorld.player().battleQueueType = QUEUE_TYPE.UNKNOWN
            guiCtx.update({b'inQueue': (QUEUE_TYPE.UNKNOWN), 
               b'skipHangar': False, 
               b'showOutroVideo': False})
            g_playerEvents.onAccountShowGUI(guiCtx)
        return
    ...
    def __onAccountComponentsSynced(self, ctx, result):
        isStoryModeQueue = ctx.get(b'inQueue', QUEUE_TYPE.UNKNOWN) == QUEUE_TYPE.STORY_MODE
        ...
        if not (isStoryModeQueue and skippedHangar and result):
            ...
            if lastArenaUniqueID:
                ...
                if resultVO and resultVO[b'isForceOnboarding'] and self.isEnabled():
                    missionId = resultVO[b'missionId']
                    lastMissionId = self.missions.onboardingLastMissionId
                    if missionId != lastMissionId or resultVO[b'finishResult'] != PLAYER_TEAM_RESULT.WIN:
    ...
    def __chooseSelectedMissionsIds(self, missionType=MissionType.REGULAR, precondition=(lambda m: True), condition=(lambda m: True)):
        ...
        _logger.debug(b'[Controller] Missions <%s>, <%s> chosen.', firstSuitable, lastEnabled)
    ...
    def __chooseEventSelectedMissionIds(self):
        ...
        if selectorType not in self._eventMissionSelectorMap:
            _logger.error(b'Unknown event mission selector type %s', selectorType)
    ...
    def __assignSelectedMission(self):
        ...
        if eMissionId != MissionId.UNDEFINED:
            ...
            _logger.debug(b'[Controller] Mission <%s> automatically selected.', self.__selectedMissionId)
            return
