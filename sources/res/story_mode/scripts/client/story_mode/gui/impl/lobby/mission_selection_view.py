...
PATH_TO_CONFIGS = b'story_mode/gui/parallax/configs.json'
...
class MissionSelectionView(BasePrbView):
    ...
    def createToolTipContent(self, event, contentID):
        ...
        if contentID == R.views.story_mode.mono.lobby.tooltips.mission_tooltip():
            ...
            if mission is None:
                _logger.error(b'Mission ID=%s not exists', selectedMissionId)
                return
            ...
            if contentID == R.views.story_mode.mono.lobby.tooltips.difficulty_tooltip():
                ...
                return DifficultyTooltip(event.getArgument(b'difficulty'), event.getArgument(b'isSelected'), isLocked=isLocked, isAutoCompleteCondition=isAutoCompleteCondition)
    ...
    def __onChangeTab(self, args):
        ...
        if missionId == MissionId.UNDEFINED:
            _logger.error(b'[Missions view] Tab(event=%s) can not be selected. No suitable missions in tab.', isEvent)
            return
    ...
    def __onTaskUnlocked(self, args):
        setUnlockedTaskShown(self.getViewModel().selectedMission.getMissionId(), int(args[b'taskId']))
        return
    ...
    def __readParallaxConfig(self, missionId, model, isParallaxEnabled):
        if not isParallaxEnabled:
            ...
            _logger.info(b'story_mode_settings.xml parallaxEnabled is False')
            return
        else:
            if isLowPreset():
                model.setIsParallaxEnabled(False)
            ...
            if missionParallaxConfig is None:
                model.setIsParallaxEnabled(False)
                _logger.info(b'cannot find config for %s', missionId)
                return
            parallaxStructure = _readSection(missionParallaxConfig.get(b'structurePath', b''))
            if parallaxStructure == b'':
                model.setIsParallaxEnabled(False)
                _logger.info(b'cannot find parallax structure:%s', missionParallaxConfig.get(b'structurePath'))
                return
            model.setIsParallaxEnabled(True)
            chunks = _getChunks(missionParallaxConfig.get(b'chunksCount', 0), missionParallaxConfig.get(b'chunkName', b''), missionParallaxConfig.get(b'chunksDataDirPath', b''))
            modelParallax = model.parallax
            modelParallax.setMissionId(missionId)
            modelParallax.setPerspective(missionParallaxConfig.get(b'perspective', 0))
            modelParallax.setPerspectiveOriginX(missionParallaxConfig.get(b'perspectiveOriginX', 0))
            modelParallax.setPerspectiveOriginY(missionParallaxConfig.get(b'perspectiveOriginY', 0))
            modelParallax.setWrapperWidth(missionParallaxConfig.get(b'wrapperWidth', 0))
            modelParallax.setWrapperHeight(missionParallaxConfig.get(b'wrapperHeight', 0))
            modelParallax.setOverallScale(missionParallaxConfig.get(b'overallScale', PARALLAX_DEFAULT_SCALE))
            modelParallax.setXTilt(missionParallaxConfig.get(b'xTilt', 0.0))
            modelParallax.setXTiltRange(missionParallaxConfig.get(b'xTiltRange', 0.0))
            modelParallax.setYTilt(missionParallaxConfig.get(b'yTilt', 0.0))
            modelParallax.setYTiltRange(missionParallaxConfig.get(b'yTiltRange', 0.0))
            modelParallax.setXSlide(missionParallaxConfig.get(b'xSlide', 0.0))
            modelParallax.setYSlide(missionParallaxConfig.get(b'ySlide', 0.0))
            modelParallax.setParallaxStructure(parallaxStructure)
            modelParallax.setAtlas(chunks)
            modelParallax.setChunkFileExt(missionParallaxConfig.get(b'chunkFileExt', b''))
            modelParallax.setChunksAssetsPath(missionParallaxConfig.get(b'chunksAssetsPath', b''))
            return
