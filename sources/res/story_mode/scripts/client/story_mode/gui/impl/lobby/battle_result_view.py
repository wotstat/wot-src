...
class BattleResultView(ViewImpl):
    __slots__ = (b'_uiLogger', b'__arenaUniqueId', b'__idGen', b'__bonusCache')
    ...
    _MAIN_REWARDS = (b'freeXP', b'credits')
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.story_mode.mono.lobby.tooltips.battle_result_stat_tooltip():
            stat = event.getArgument(b'stat', b'')
    ...
    def __getDetailedStatsForStatTooltip(self, stat):
        ...
        if battleResults:
            ...
            if stat == StatEnum.ASSIST:
                return [(vehicle[b'damageAssisted'], backport.text(battleResultStat.assist.total()))]
            if stat == StatEnum.DAMAGE:
                return [(vehicle[b'damageDealt'], backport.text(battleResultStat.damage.total()))]
            if stat == StatEnum.ARMOR_USE:
                return [(vehicle[b'damageBlockedByArmor'], backport.text(battleResultStat.armorUse.blocked()))]
        return []
    ...
    def __fillViewModel(self):
        ...
        with self.getViewModel().transaction() as model:
            ...
            if mission is None:
                _logger.error(b'Mission ID=%s not exists', missionId)
                return
    ...
    def __fillProgressLevels(self, missionProgressModel, progressLevelsModels, battleResults):
        ...
        progressionInfo = battleResults.get(b'progressionInfo', {})
        ...
        with progressLevelsModels.transaction() as model:
            vehicle = battleResults[b'vehicle']
            model.addViewModel(self.__createProgressModel(vehicle[b'kills'], self._ICON_KILLS, text.kills(), StatEnum.KILLS))
            model.addViewModel(self.__createProgressModel(vehicle[b'damageDealt'], self._ICON_DAMAGE, text.damageDealt(), StatEnum.DAMAGE))
            model.addViewModel(self.__createProgressModel(vehicle[b'damageAssisted'], self._ICON_ASSIST, text.damageAssisted(), StatEnum.ASSIST))
            model.addViewModel(self.__createProgressModel(vehicle[b'damageBlockedByArmor'], self._ICON_DAMAGE_BLOCKED, text.damageBlockedByArmor(), StatEnum.ARMOR_USE))
        return
    ...
    def __fillRewards(self, mainRewardsModel, otherRewardsModel):
        battleResults = self._battleResultsService.getResultsVO(self.__arenaUniqueId)
        mainRewards = [r for r in battleResults[b'rewards'] if r.getName() in self._MAIN_REWARDS]
        otherRewards = [r for r in battleResults[b'rewards'] if r.getName() not in self._MAIN_REWARDS]
