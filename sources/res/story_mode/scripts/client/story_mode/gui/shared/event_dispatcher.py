...
def showIntroVideo(missionId):
    _logger.debug(b'showIntroVideo for mission with id: %s', missionId)
    g_eventBus.handleEvent(events.LoadViewEvent(SFViewLoadParams(VIEW_ALIAS.STORY_MODE_INTRO_VIDEO_WINDOW), ctx={b'missionId': missionId}), EVENT_BUS_SCOPE.BATTLE)
    return


def showOutroVideo(missionId, arenaUniqueID):
    _logger.debug(b'showOutroVideo for mission with id: %s', missionId)
    g_eventBus.handleEvent(events.LoadViewEvent(SFViewLoadParams(VIEW_ALIAS.STORY_MODE_OUTRO_VIDEO_WINDOW), ctx={b'missionId': missionId, 
       b'arenaUniqueID': arenaUniqueID}), EVENT_BUS_SCOPE.LOBBY)
    return
...
def showPrebattleWindow(missionId):
    _logger.debug(b'showPrebattleWindow')
...
def showPrebattleAndGoToQueue(missionId):
    _logger.debug(b'showPrebattleAndGoToQueue')
...
def showEpilogueWindow():
    _logger.debug(b'showEpilogueWindow')
...
def showOnboardingBattleResultWindow(finishReason, missionId):
    _logger.debug(b'showOnboardingBattleResultWindow')
...
def showQueueWindow(isSkipButtonVisible=False):
    _logger.debug(b'showQueueWindow: isSkipButtonVisible%s', isSkipButtonVisible)
...
def showCongratulationsWindow(isCloseVisible=False, onClose=None, awardData=None):
    _logger.debug(b'showCongratulationsWindow')
