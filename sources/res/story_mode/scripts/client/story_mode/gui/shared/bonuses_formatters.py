...
_IMG_PATH_PREFIX = b'img://gui'

def getImgPath(path):
    if path is None:
        return b''
    else:
        if path.startswith(b'img:'):
            return path
...
class StoryModeBonusesAwardsComposer(CurtailingAwardsComposer):
    ...
    def _packMergedBonuses(self, mergedBonuses, size=AWARDS_SIZES.SMALL):
        ...
        return PreformattedBonus(bonusName=b'default', label=backport.text(R.strings.marathon.reward.rest(), count=mergedBonusCount), isSpecial=True, images=imgs, specialAlias=TOOLTIPS_CONSTANTS.ADDITIONAL_AWARDS, specialArgs=self._getShortBonusesData(mergedBonuses, size), userName=b'')
