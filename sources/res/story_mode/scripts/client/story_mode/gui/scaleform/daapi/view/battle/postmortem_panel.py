...
BUNKER_CODE = b'DEATH_FROM_BUNKER'
SHOT_CODES = (b'DEATH_FROM_SHOT', b'DEATH_FROM_DEVICE_EXPLOSION_AT_SHOT')
BOT_CODE = b'DEATH_FROM_BOT'
...
class StoryModePostmortemPanel(PvePostmortemPanel):
    ...
    def _showOwnDeathInfo(self):
        if not self._deathAlreadySet:
            ...
            if deathInfo is not None and self.__isBunkerTurret(deathInfo[b'killerVehicle']):
                self.as_setDeadReasonInfoS(self._makeReasonInfo(deathInfo), False, b'', b'', b'', b'', None)
                return
