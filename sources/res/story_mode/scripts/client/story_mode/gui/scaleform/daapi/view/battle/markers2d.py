...
_BUNKER_OWNER = b'enemy'
_BUNKER_ALIVE_STATE = b'alive'
BUNKER_SYMBOL = b'BunkerMarkerUI'
...
class StoryModeMarkersManager(MarkersManager):
    MARKERS_MANAGER_SWF = b'story_mode|storyModeBattleVehicleMarkersApp.swf'
    __slots__ = ()

    def _setupPlugins(self, arenaVisitor):
        setup = super(StoryModeMarkersManager, self)._setupPlugins(arenaVisitor)
        setup[b'vehicles'] = StoryModeVehicleMarkerPlugin
...
class StoryModeVehicleMarkerPlugin(RespawnableVehicleMarkerPlugin, MarkerPluginWithOffsetInZoom):
    ...
    def _voiceoverHandler(self):
        ...
        if ctx is None:
            ...
            vehicleId = ctx.get(b'vehicleId', None)
            if not vehicleId:
                return
            ...
            if marker.setSpeaking(flag):
                self._invokeMarker(markerId, b'setSpeaking', flag)
            return
...
class BunkersPlugin(MarkerPluginWithOffsetInZoom):
    __slots__ = (b'_markers', b'__clazz', b'__entitiesDamageType', b'_distanceUpdateCallback', b'__offsetCache')
    ...
    def __onDestructibleEntityAdded(self, entity):
        ...
        if destructibleComponent is None:
            ...
            self._invokeMarker(handle, b'setName', backport.text(R.strings.sm_battle.bunker()))
