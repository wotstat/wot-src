...
class BaseVideo(IntroVideoMeta):
    ...
    def __init__(self, ctx):
        ...
        self._missionId = ctx.get(b'missionId')
        if self._missionId is None:
            _logger.error(b'missionId is None')
            return
        else:
            data = self._getSettings()
            if data is None:
                _logger.error(b'data not exists for OutroVideoSettingsModel')
                return
            ...
            if self._videoSettings is None:
                _logger.error(b'_videoSettings is None for mission ID=%s', self._missionId)
            return
    ...
    def _populate(self):
        ...
        self.as_setDataS({b'skipButtonLabel': (backport.text(R.strings.sm_battle.common.skipBtn())), 
           b'loadingText': (backport.text(R.strings.sm_battle.introVideo.loading())), 
           b'loadingImage': (self._getLoadingImage()), 
           b'video': (self._videoSettings.videoPath), 
           b'isPausedAfterLoad': isPausedAfterLoad})
        if self._videoSettings is not None:
