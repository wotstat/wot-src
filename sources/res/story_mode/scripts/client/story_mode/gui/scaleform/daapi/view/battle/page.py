...
STORY_MODE_CONFIG = ComponentsConfig(config=(
 (
...
_STORY_MODE_EXTERNAL_COMPONENTS = (
...
class StoryModeBattlePage(ClassicPage):
    ...
    def reload(self):
        ...
        self.fireEvent(events.GameEvent(events.GameEvent.GUI_VISIBILITY, {b'visible': True}), scope=EVENT_BUS_SCOPE.BATTLE)
        player = BigWorld.player()
        if isinstance(player, AvatarChatKeyHandling):
            player.setKeyHandling(False)
        _logger.debug(b'-=>> Story mode battle page is reloaded.')
    ...
    def _populate(self):
        ...
        if isinstance(player, AvatarChatKeyHandling):
            player.setKeyHandling(False)
        _logger.debug(b'-=>> Story mode battle page is created.')
        return

    def _dispose(self):
        super(StoryModeBattlePage, self)._dispose()
        g_playerEvents.onRoundFinished -= self.__onRoundFinished
        _logger.debug(b'-=>> Story mode battle page is destroyed.')
        return
    ...
    def _onBattleLoadingStart(self):
        ...
        missionId = self.sessionProvider.arenaVisitor.extra.getValue(b'missionId')
        _logger.debug(b'_onBattleLoadingStart: missionId=%s', missionId)
    ...
    def _handleGUIToggled(self, event):
        if not self.sessionProvider.arenaVisitor.extra.getValue(b'isForceOnboarding') or BattleReplay.isPlaying():
