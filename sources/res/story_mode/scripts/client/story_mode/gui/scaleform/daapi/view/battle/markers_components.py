...
class LootMarkerComponent(World2DLocationMarkerComponent):
    ...
    def configReader(cls, section):
        ...
        config.update({b'loot_type': (section.readString(b'loot_type'))})
        return config
    ...
    def _setupMarker(self, gui, **kwargs):
        ...
        lootType = self._config[b'loot_type']
        gui.invokeMarker(self.componentID, b'updateLootType', lootType, backport.text(R.strings.sm_battle.lootCapturing.dyn(lootType)()))
    ...
    def _onStartCapturing(self, startTime, captureTime):
        ...
        if not gui:
            return
        gui.invokeMarker(self.componentID, b'updateLootingVisible', True)
    ...
    def _onStopCapturing(self):
        ...
        if not gui:
            return
        gui.invokeMarker(self.componentID, b'updateLootingVisible', False)
        SoundGroups.g_instance.playSound2D(VDAY_LOOT_CAPTURE_STOP_SOUND)
        WWISE.WW_setRTCPGlobal(VDAY_LOOT_CAPTURE_RTPC, 0)
        self._isStickyFromConfig = self._config.get(b'is_sticky', True)
...
class LootMinimapComponent(BaseMinimapMarkerComponent):
    ...
    def _setupMarker(self, gui, **kwargs):
        ...
        gui.invoke(self.componentID, b'setLootType', self._config[b'loot_type'])
...
class SMBitmapComponent(BaseMinimapMarkerComponent):
    ...
    def _setupMarker(self, gui, **kwargs):
        ...
        gui.invoke(self._componentID, b'setIcon', self._config[b'bitmapName'])
        return
