...
class SPGZone(AreaOfEffect, TriggersManager.ITriggerListener):
    _TRIGGER_NAME_TEMPLATE = b'spg_zone_{}'
    ...
    def onTriggerActivated(self, args):
        if args[b'type'] == TriggersManager.TRIGGER_TYPE.AREA and args[b'name'] == self._triggerName and self.__hitPredictionDuration > 0:
    ...
    def onTriggerDeactivated(self, args):
        if args[b'type'] == TriggersManager.TRIGGER_TYPE.AREA and args[b'name'] == self._triggerName and self._hpCallbackId:
            BigWorld.cancelCallback(self._hpCallbackId)
            self._removeHitPrediction()
        return
