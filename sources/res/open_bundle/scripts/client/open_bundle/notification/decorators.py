...
class OpenBundleStartDecorator(MessageDecorator):
    ...
    def __makeEntity(data):
        return g_settings.msgTemplates.format(data[b'template'], data={b'linkageData': (data[b'data'])})
...
class OpenBundleReminderDecorator(MessageDecorator):
    ...
    def __makeEntity(self, message, savedData):
        return g_settings.msgTemplates.format(b'OpenBundleReminderSysMessage', ctx=message, data={b'savedData': savedData})

    def __makeSettings(self):
        return NotificationGuiSettings(isNotify=True, priorityLevel=g_settings.msgTemplates.priority(b'OpenBundleReminderSysMessage'))

    def __updateEntityButtons(self):
        if self._entity is None:
            return
        else:
            self._entity[b'buttonsStates'] = {b'submit': (NOTIFICATION_BUTTON_STATE.DEFAULT if self.__openBundle.isBundleActive(self.getSavedData().get(b'bundleID')) else NOTIFICATION_BUTTON_STATE.VISIBLE)}
            return
