...
def registerOpenBundleWebApi():
    ...
    def _showOpenBundle(_, cmd, openBundle=None):
        bundleID = cmd.custom_parameters.get(b'bundle_id', False)
        if openBundle.isBundleActive(bundleID=bundleID):
            showOpenBundleMainView(bundleID=bundleID)
        else:
            _logger.error(b'Bundle with ID: %s is not active', bundleID)
        return

    OpenTabWebApi.addTabIdCallback(b'open_bundle', _showOpenBundle)
    return
