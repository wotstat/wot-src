...
class ProcessNextStepProcessor(Processor):
    ...
    def _request(self, callback):
        openBundleAccountComponent = getattr(BigWorld.player(), b'OpenBundleAccountComponent', None)
        if openBundleAccountComponent is None:
            callback(makeError(b'Cannot find the OpenBundleAccountComponent'))
            return
        else:
    ...
    def _errorHandler(self, code, errStr=b'', ctx=None):
        ...
        SystemMessages.pushMessage(text=backport.text(getTextResource(self.__bundleID, (b'serviceChannelMessages', b'openBundleError'))()), type=SystemMessages.SM_TYPE.ErrorSimple, priority=NotificationPriorityLevel.MEDIUM)
        return super(ProcessNextStepProcessor, self)._errorHandler(code, errStr, ctx)

    def _successHandler(self, code, ctx=None):
        if self.__openBundle.isUnicNotificationCell(self.__bundleID, ctx.get(b'receivedCell')) and ctx.get(b'randomBonus') and not self.__preformatCompensationValue(ctx.get(b'randomBonus')):
            ctx.update({b'bundleID': (self.__bundleID)})
    ...
    def __pushCommonNotification(self, ctx, cellRewards=True):
        money = Money.makeFromMoneyTuple(ctx.get(b'price'))
        currency = money.getCurrency()
        fmt = OpenBundleAchievesFormatter.formatData(ctx if cellRewards else {b'fixedBonus': (ctx.get(b'fixedBonus'))})
        header = backport.text(getTextResource(self.__bundleID, (b'serviceChannelMessages', b'simplePurchase'))(), eventName=backport.text(getTextResource(self.__bundleID, (b'bundle', b'name'))()))
        if fmt is not None:
            SystemMessages.pushMessage(text=fmt, type=SystemMessages.SM_TYPE.OpenBundleRewards, priority=NotificationPriorityLevel.MEDIUM, messageData={b'header': header, 
               b'time': (TimeFormatter.getLongDatetimeFormat(makeLocalServerTime(getServerUTCTime()))), 
               b'spentCurrency': (g_settings.htmlTemplates.format((b'openBundlePriceIn{}').format(currency[0].upper() + currency[1:]), {b'amount': (money.get(currency))}))})
        return
