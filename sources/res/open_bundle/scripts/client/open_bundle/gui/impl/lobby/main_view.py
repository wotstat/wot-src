...
class _CellState(str, Enum):
    ...
    NEWLY_RECEIVED = b'newlyReceived'
    RECEIVED = b'received'
...
class MainView(ViewImpl):
    ...
    def _getCallbacks(self):
        return (((b'stats.{}').format(c), self.__fillPrice) for c in Currency.ALL)
    ...
    def __fillCellBonusInfo(self, cellModel, cellName):
        ...
        defaultProbability = cellBonusInfo[b'probability']
    ...
    def __fillBonuses(self, cellModel, cellBonusInfo):
        bonuses = hideInvisible(sortBonuses(cellBonusInfo[b'bonuses']))
    ...
    def __calculateProbability(self, cellName, defaultProbability):
        if cellName in self.__receivedCells or cellName in self.__newlyReceivedCells:
            return -1
        receivedProbabilitiesSum = sum((self.__openBundle.getCellBonusInfo(self.__bundle.id, cell) or {})[b'probability'] for cell in self.__receivedCells.union(self.__newlyReceivedCells))
    ...
    def __processNextStep(self):
        ...
        if step is None:
            ...
            if self.viewStatus not in (ViewStatus.DESTROYING, ViewStatus.DESTROYED):
                if result.success:
                    self.__newlyReceivedCells.add(result.auxData.get(b'receivedCell'))
                    self.__updateModel()
                    return
    ...
    def __showPreview(self, bonusType, bonusId, styleID, attachmentsToken):
        ...
        elif bonusType == b'customizations':
    ...
    def __makeNotificationLockKey(self):
        return (b'_').join((b'OpenBundleMainView', str(self.uniqueID)))
