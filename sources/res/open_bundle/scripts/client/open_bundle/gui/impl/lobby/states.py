...
class OpenBundleState(GuiImplViewLobbyState):
    ...
    def getNavigationDescription(self):
        ...
        if self.__params.get(b'bundleID') is not None:
            text = getTextResource(self.__params.get(b'bundleID'), (b'bundle', b'name_uppercased'))
        return LobbyStateDescription(title=backport.text(text()), infos=(
         LobbyStateDescription.Info(tooltipBody=backport.text(R.strings.open_bundle_lobby_default.tooltips.header.info()), onMoreInfoRequested=(lambda : showIntro(self.__params.get(b'bundleID')))),))

    def serializeParams(self):
        return {b'bundleID': (self.__params.get(b'bundleID'))}

    def _onEntered(self, event):
        super(OpenBundleState, self)._onEntered(event)
        self.__params = event.params
        bundleID = self.__params.get(b'bundleID')
