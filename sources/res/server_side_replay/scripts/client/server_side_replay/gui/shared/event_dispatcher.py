...
def showReplays(location=None):
    ...
    if uiLoader.windowsManager.getViewByLayoutID(layoutID) is None:
        g_eventBus.handleEvent(events.LoadGuiImplViewEvent(GuiImplViewLoadParams(layoutID=layoutID, viewClass=ReplaysRootView, scope=ScopeTemplates.LOBBY_SUB_SCOPE), ctx={b'location': location}), scope=EVENT_BUS_SCOPE.LOBBY)
    return
