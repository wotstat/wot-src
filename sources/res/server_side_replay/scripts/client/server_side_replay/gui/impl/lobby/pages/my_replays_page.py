...
class MyReplaysPage(PageSubModelPresenter):
    ...
    def __updateState(self, loadedData=None, model=None):
        if loadedData in self.__loadingData:
            ...
            return
        bestReplaysObj = self.__serverReplayProvider.getMyReplays(useFake=False, vehicleType=list(self.__filterState[ToggleGroupType.VEHICLETYPE.value]), vehicleLevel=list(self.__filterState[ToggleGroupType.VEHICLETIER.value]), nation=list(self.__filterState[ToggleGroupType.NATION.value]), vehicleCDs=list(self.__filterState[ToggleGroupType.VEHICLECD.value]), sortBy=b'replay_date')
    ...
    def __updateReplays(self, allReplaysData, model=None):
        _logger.info(b'__updateReplays: %s', allReplaysData)
    ...
    def __makeReplayModel(self, replayInfo, model=None):
        ...
        for record in replayInfo.achievements:
            achievementId = record.get(b'achievement_type_id')
            achievementValue = record.get(b'achievement_value')
        ...
        playerModel.setClanTagColor(b'#' + hex(replayInfo.clan_color)[2:].zfill(6) if replayInfo.clan_color is not None else b'')
        return replayModel
    ...
    def __onResetFilter(self, *args):
        _logger.info(b'__onResetFilter: %s', args)
    ...
    def __onWatch(self, index):
        _logger.info(b'__onLike: %s', index)
    ...
    def __onSort(self, param):
        _logger.info(b'__onSort: %s', param)
