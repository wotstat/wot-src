...
def _readVisualScriptPlan(dataSection):
    ...
    return {b'aspects': (dataSection.readString(b'aspects', b'').split()), 
       b'plan': (readVisualScriptPlan(dataSection[b'plan']))}


def _readGoldReserveGains(section):
    goldReserveGains = {}
    for value in section.values():
        typeName = value.readString(b'arenaBonusType').strip()
        arenaType = ARENA_BONUS_TYPE_NAMES.get(typeName, None)
        if arenaType is None:
            raise SoftException((b"Wrong arena type: '{}'").format(typeName.strip()))
        goldReserveGains[arenaType] = {b'win': (value.readInt(b'win')), 
           b'loss': (value.readInt(b'loss')), 
           b'draw': (value.readInt(b'draw')), 
           b'minLevel': (value.readInt(b'minLevel')), 
           b'minTop': (value.readInt(b'minTop', -1)), 
           b'topType': (value.readString(b'topType', b'fareTeamXPPosition').strip())}
