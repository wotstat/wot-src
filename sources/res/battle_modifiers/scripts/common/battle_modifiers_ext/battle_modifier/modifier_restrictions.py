...
class IModifierValidator(object):
    ...
    def __call__(self, modificationNode):
        if not self._validate(modificationNode):
            ...
            raise SoftException((b'[BattleModifier] Validation error for param {}: {}').format(paramId, errorMsg))
        return
...
class UseTypeValidator(IModifierValidator):
    ...
    def _readConfig(self, config, _):
        ...
        for useTypeName in config.asString.split():
            if useTypeName not in UseType.NAMES:
                raise SoftException((b"[BattleParams] Unknown use type '{}'").format(useTypeName))
    ...
    def _errorMessage(self, modificationNode):
        return (b"use type '{}' is not allowed").format(UseType.ID_TO_NAME[modificationNode.useType])
...
class DataTypeValidator(IModifierValidator):
    ...
    def _validate(self, modificationNode):
        ...
        try:
            ...
            raise SoftException((b"Keys should be strings and all values should be hashable: '{}'").format(value))
...
class MinValidator(IValueValidator):
    ...
    def _errorMessage(self, modificationNode):
        return (b'value should be at least {}, given value - {}').format(self.arg, modificationNode.value)
...
class MaxValidator(IValueValidator):
    ...
    def _errorMessage(self, modificationNode):
        return (b'value should be no more than {}, given value - {}').format(self.arg, modificationNode.value)
...
def readRestrictions(config, paramId, allowedRestrictions=None):
    if not config.has_key(b'restrictions'):
        ...
        minValue = None
        maxValue = None
        for restrictionName, restrictionSection in config[b'restrictions'].items():
            if restrictionName not in ModifierRestriction.NAMES:
                raise SoftException((b"[BattleParams] Unknown restriction '{}'").format(restrictionName))
            restrictionId = ModifierRestriction.NAME_TO_ID[restrictionName]
            if restrictionId not in allowedRestrictions:
                raise SoftException((b"[BattleParams] Restriction '{}' isn't allowed").format(restrictionName))
            if restrictionId in validators:
                raise SoftException((b"[BattleParams] Duplicate restriction '{}'").format(restrictionName))
