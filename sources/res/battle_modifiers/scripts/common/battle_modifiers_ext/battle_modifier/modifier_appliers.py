...
def _engineSoundsApplier(_, paramVal, __=None):
    return WWTripleSoundConfig(wwsound=b'', wwsoundPC=(b'_').join((paramVal, b'pc')), wwsoundNPC=(b'_').join((paramVal, b'npc')))
...
_customAppliers = {(BattleParams.VEHICLE_HEALTH): {(UseType.MUL): (lambda val, paramVal, _=None: int(ceilTo(val * paramVal, VEHICLE_HEALTH_DECIMALS)))}, 
   ...
   (BattleParams.VSE_MODIFIER): (lambda val, paramVal, _=None: paramVal[b'plan'] if val in paramVal[b'aspects'] else None)}
