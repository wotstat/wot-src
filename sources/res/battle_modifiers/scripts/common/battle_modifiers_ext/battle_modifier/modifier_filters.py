...
class ModificationNode(object):
    ...
    def __call__(self, value, ctx=None):
        ...
        if DEBUG_MODIFIERS:
            LOG_DEBUG((b"[BattleModifiers][Debug] Apply modifier '{}' ({} {}, min {}, max {}): {} -> {}").format(self.param.name, UseType.ID_TO_NAME[self.useType], self.value, self.minValue if self.minValue is not None else self.param.minValue, self.maxValue if self.maxValue is not None else self.param.maxValue, value, modifiedValue))
...
class ModificationTree(object):
    ...
    def __readConfig(self, config):
        ...
        if not resDict:
            raise SoftException(ERROR_TEMPLATE.format(b'Invalid modifier', self.param.name))
        self.nodes = resDict
    ...
    def __readNodes(self, config, nodeType, accDict, accDescr):
        ...
        if not self.param.domain & NodeType.SUPPORTED_DOMAINS[nodeType]:
            raise SoftException(ERROR_TEMPLATE.format((b"'{}' filters can't be used").format(nodeType), self.param.name))
    ...
    def __readShellNode(self, config, accDict, accDescr):
        if not config.has_key(b'filter'):
            raise SoftException(ERROR_TEMPLATE.format(b'Shell node should contain filter', self.param.name))
        ...
        if not shellKeys:
            raise SoftException(ERROR_TEMPLATE.format(b'Some filter keys should be provided for shell node', self.param.name))
        shellNode = self.__readModificationNode(config)
        if not shellNode:
            raise SoftException(ERROR_TEMPLATE.format(b'Shell node should contain root part', self.param.name))
    ...
    def __readShellFilterKeys(self, config, isImproved):
        ...
        for key in keys:
            if key not in ShellKind.ALL_REGULAR:
                raise SoftException(ERROR_TEMPLATE.format((b"Unknown shell filter key '{}'").format(key), self.param.name))
    ...
    def __readVehicleNode(self, config, accDict, accDescr):
        if not config.has_key(b'filter'):
            raise SoftException(ERROR_TEMPLATE.format(b'Vehicle node should contain filter', self.param.name))
        vehiclKeys = self.__readVehicleFilterKeys(config[b'filter'])
        if not vehiclKeys:
            raise SoftException(ERROR_TEMPLATE.format(b'Some filter keys should be provided for vehicle node', self.param.name))
        vehicleDict = {}
        vehicleDescr = [
        ...
        if not vehicleDict:
            raise SoftException(ERROR_TEMPLATE.format(b'Invalid vehicle node', self.param.name))
    ...
    def __readVehicleFilterKeys(self, config):
        ...
        for key in keys:
            ...
            if level is not None and MIN_VEHICLE_LEVEL <= level <= MAX_VEHICLE_LEVEL:
                continue
            raise SoftException(ERROR_TEMPLATE.format((b"Unknown vehicle filter key '{}'").format(key), self.param.name))
