...
class VehicleModifier(object):
    ...
    def __modifyShell(cls, shell, modifiers):
        ...
        if modifiers.haveDomain(ModifierDomain.SHELL):
            ...
            damage = shell.damage
            shell.damage = (
             modifiers(BattleParams.ARMOR_DAMAGE, damage[0]),
             modifiers(BattleParams.DEVICE_DAMAGE, damage[1]))
    ...
    def __modifyShellType(cls, shellType, modifiers):
        ...
        if isModernWithExplosionEffect and shellType.armorSpalls.isActive:
            ...
            armorSpalls.damages = (
             modifiers(BattleParams.ARMOR_SPALLS_ARMOR_DAMAGE, damages[0]),
             modifiers(BattleParams.ARMOR_SPALLS_DEVICE_DAMAGE, damages[1]))
    ...
    def __modifyPhysicsServer(cls, physics, modifiers):
        ...
        for engine in detailed[b'engines'].itervalues():
        ...
        for chassis in detailed[b'chassis'].itervalues():
            ...
            for ground in chassis[b'grounds'].itervalues():
    ...
    def __modifyPhysicsClient(cls, physics, modifiers):
        physics = copy.deepcopy(physics)
        for engine in physics[b'engines'].itervalues():
            engine[b'smplEnginePower'] = modifiers(BattleParams.ENGINE_POWER, engine[b'smplEnginePower'])
            msSpeed = modifiers(BattleParams.FW_MAX_SPEED, engine[b'smplFwMaxSpeed'] * KMH_TO_MS)
            engine[b'smplFwMaxSpeed'] = msSpeed * MS_TO_KMH
