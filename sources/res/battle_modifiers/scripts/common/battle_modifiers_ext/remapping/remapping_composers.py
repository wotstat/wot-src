...
class _BaseComposer(IComposer):
    __slots__ = (b'_conditions', b'_targetTemplate', b'_specialRules')
    ...
    def getValue(self, ctx, oldValue):
        ...
        if resStr is not None:
            ...
            for condition in self._conditions:
                resStr = resStr.replace((b'').join((_START_PATTERN, condition.getName(), _END_PATTERN)), condition(ctx))
