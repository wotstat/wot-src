...
class BattleModifier(ModifierBase):
    ...
    def __repr__(self):
        return (b'BattleModifier(paramName = {}, gameplayImpactName = {}, modificationTree = {})').format(self.param.name, GameplayImpact.ID_TO_NAME[self.gameplayImpact], self.__modificationTree)
    ...
    def _readParam(self, config):
        ...
        if paramId is None:
            raise SoftException((b"[BattleModifiers] Unknown param name '{}'").format(paramName))
...
class FakeBattleModifier(ModifierBase):
    ...
    def _readParam(self, config):
        if not config.has_key(FAKE_PARAM_NAME):
            raise SoftException(b'Missing fakeParams section in FakeBattleModifier')
    ...
    def __readUseType(self, config):
        ...
        if useType not in clientData.useTypes:
            raise SoftException(ERROR_TEMPLATE.format(b'[Fake modifier] You should define physical type to unlock dimensional use types', clientData.resName))
        return useType
...
class BattleModifiers(battle_modifiers.BattleModifiers):
    __slots__ = (b'__modifiers', b'__domain', b'__id', b'__descr', b'__battleDescr')
    ...
    def __repr__(self):
        return (b'BattleModifiers({})').format(self.__modifiers.values())
    ...
    def __readConfig(self, config):
        ...
        for modifierName, modifierSection in config.items():
            ...
            if modifier.param.id in modifiers:
                ...
                LOG_WARNING((b"[BattleModifiers] Ignore multiple modifiers for param '{}'").format(paramName))
                continue
...
def _readGlobalBattleModifiers():
    ...
    LOG_DEBUG((b'[BattleModifiers] Use global battle modifiers: {}').format(modifiers))
    return modifiers
