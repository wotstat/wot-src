...
class ProgressionQuestsPresenter(TooltipPositionerMixin, Comp7LightOverlapCtrlMixin, ViewComponent[ProgressionQuestsModel], IGlobalListener):
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.comp7_light.mono.lobby.tooltips.battle_quest_tooltip():
            ...
            battleQuest = self.__comp7LightProgressionController.getProgressionData()[b'battleQuests'].get(questId)
    ...
    def _rawUpdate(self):
        ...
        with self.viewModel.transaction() as vm:
            ...
            modelQuests.clear()
            for questId in battleQuests[b'questsOrder']:
                modelQuests.addViewModel(self.__updateBattleQuestData(battleQuests[b'battleQuests'][questId]))
    ...
    def __onMarkAsViewed(self):
        ...
        for questID in battleQuests[b'questsOrder']:
            model = WidgetQuestModel()
            isCompleted, lastSeenProgress = packMissionItem(model, battleQuests[b'battleQuests'][questID], DailyQuestUIDataPacker)
