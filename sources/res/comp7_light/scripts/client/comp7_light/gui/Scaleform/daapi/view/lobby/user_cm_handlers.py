...
def addComp7LightSquadInfo(handler, options, userCMInfo, comp7LightCtrl=None):
    ...
    if not comp7LightCtrl.isEnabled():
        ...
        newComp7LightSquadItem = handler._makeItem(USER.CREATE_COMP7_LIGHT_SQUAD, MENU.contextmenu(USER.CREATE_COMP7_LIGHT_SQUAD), optInitData={b'enabled': (canCreate and isEnabled), 
           b'textColor': 13347959})
        if isinstance(handler, UserVehicleCMHandler):
            if comp7LightCtrl.isModePrbActive():
                regularSquadItem = findFirst((lambda it: it[b'id'] == USER.CREATE_SQUAD), options)
