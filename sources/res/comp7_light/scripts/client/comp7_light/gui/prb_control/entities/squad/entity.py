...
class Comp7LightSquadEntryPoint(SquadEntryPoint):
    ...
    def makeDefCtx(self):
        return SquadSettingsCtx(PREBATTLE_TYPE.COMP7_LIGHT, waitingID=b'prebattle/create', accountsToInvite=self._accountsToInvite)
    ...
    def _doCreate(self, unitMgr, ctx):
        unitMgr.createSquadByPrbType(PREBATTLE_TYPE.COMP7_LIGHT, modeExtrasStr=json.dumps({b'squadSize': (self.__squadSize)}))
        return
...
class Comp7LightRosterSettings(DynamicRosterSettings):
    ...
    def _extractSettings(self, unit):
        ...
        settings[b'maxSlots'] = unit.getSquadSize()
        return settings
