...
class Comp7LightEntity(PreQueueEntity):
    ...
    def _makeQueueCtxByAction(self, action=None):
        ...
        return Comp7LightQueueCtx(self._queueType, invID, waitingID=b'prebattle/join')

    def _doQueue(self, ctx):
        BigWorld.player().AccountComp7LightComponent.enqueueComp7Light(ctx.getVehicleInventoryID())
        LOG_DEBUG(b'Sends request on queuing to the Competitive7x7 tier VIII', ctx)
        return

    def _doDequeue(self, ctx):
        BigWorld.player().AccountComp7LightComponent.dequeueComp7Light()
        LOG_DEBUG(b'Sends request on dequeuing from the Competitive7x7 tier VIII')
        return
