...
def getViewSettings():
    ...
    return (
     ViewSettings(WINBACK_ALIASES.WINBACK_BROWSER_VIEW, WinbackBrowserView, b'browserScreen.swf', WindowLayer.TOP_SUB_VIEW, WINBACK_ALIASES.WINBACK_BROWSER_VIEW, ScopeTemplates.LOBBY_SUB_SCOPE),
     ComponentSettings(HANGAR_ALIASES.WINBACK_WIDGET, WinbackEntryPointWidget, ScopeTemplates.DEFAULT_SCOPE))


def getBusinessHandlers():
    return (
     WinbackPackageBusinessHandler(),)
...
class WinbackPackageBusinessHandler(PackageBusinessHandler):
    ...
    def __init__(self):
        listeners = (
         (
          WINBACK_ALIASES.WINBACK_BROWSER_VIEW, self.loadViewByCtxEvent),)
