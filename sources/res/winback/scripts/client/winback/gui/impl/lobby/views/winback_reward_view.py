...
class WinbackRewardView(ViewImpl):
    __slots__ = (b'__selectedRewards', b'__tooltipData', b'__bonuses', b'__isOnlyDaily', b'__isLastWindow', b'__stageNumber')
    ...
    def __init__(self, ctx=None):
        ...
        self.__tooltipData = {}
        self.__selectedRewards = ctx.get(b'selectedRewards', False)
        self.__isOnlyDaily = ctx.get(b'isOnlyDaily', False)
        self.__isLastWindow = ctx.get(b'isLastWindow', False)
        self.__stageNumber = ctx.get(b'stageNumber', 0)
        self.__bonuses = getWinbackBonuses(ctx.get(b'bonuses', {}), received=True)
    ...
    def createToolTipContent(self, event, contentID):
        ...
        if tooltipData is None:
            ...
            if contentID == R.views.winback.lobby.tooltips.CompensationTooltip():
                return CompensationTooltip(**tooltipData.specialArgs)
            return
    ...
    def _showInHangar(self):
        ...
        if firstVehicle:
            ...
            showHangar()
            _logger.error(b"Can't find vehicle in bonuses")
            self.destroyWindow()
        return
