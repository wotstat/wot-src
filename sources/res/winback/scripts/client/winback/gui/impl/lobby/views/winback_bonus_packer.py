...
WINBACK_DISCOUNTS = b'winbackDiscounts'
_BONUS_COUNT = b'count'
...
def getWinbackBonusPacker():
    ...
    packer.getPackers().update({b'currencies': (CurrenciesBonusUIPacker()), 
       ...
       b'tmanToken': (TmanTemplateCountableBonusPacker()), 
    ...
    if VehiclesBonus.VEHICLES_BONUS in bonusesData:
        ...
        if not isinstance(vehiclesBonusesData, list):
            bonusesData[VehiclesBonus.VEHICLES_BONUS] = [
             vehiclesBonusesData]
        slots = bonusesData.get(b'slots', 0)
        for vehiclesData in bonusesData[VehiclesBonus.VEHICLES_BONUS]:
            for vehCD, vehData in vehiclesData.items():
                ...
                if slots > 0 and not vehData.get(b'rent'):
                    vehData[b'slot'] = 1
                    slots -= 1

        if b'slots' in bonusesData and slots == 0:
            bonusesData.pop(b'slots')
...
def isSelectableBonus(bonusName):
    return bonusName in (RewardName.SELECTABLE_VEHICLE_DISCOUNT.value,
...
def handleWinbackOfferTokens(bonuses):
    ...
    for tokenID in tokens:
        result.append(WinbackSelectableRewardManager.createBonusFromId(tokenID.replace(b'_gift', b'')))
...
class WinbackSelectableVehiclesBonusUIPacker(BaseBonusUIPacker):
    ...
    def _getContentId(cls, bonus):
        return [
         R.views.winback.lobby.tooltips.SelectableRewardTooltip()]
...
class ExtendedCurrencyBonusUIPacker(SimpleBonusUIPacker):
    ...
    def _getContentId(cls, bonus):
        if bonus.isCompensation():
            return [R.views.winback.lobby.tooltips.CompensationTooltip()]
        return super(ExtendedCurrencyBonusUIPacker, cls)._getContentId(bonus)
    ...
    def _getToolTip(cls, bonus):
        if bonus.isCompensation():
            ...
            specialArgs = {b'isDiscount': (ctx.get(b'isDiscount', False)), 
               b'level': (ctx.get(b'level', -1)), 
               b'bonuses': (
                          bonus,)}
            return [
             createTooltipData(isSpecial=True, specialArgs=specialArgs)]
...
VERSUS_AI_PROGRESSION_TOKEN_TYPE = b'VersusAIProgressionToken'
...
class WinbackTokenBonusUIPacker(TokenBonusUIPacker):
    ...
    def __packWinbackProgressionToken(cls, model, bonus, complexToken, token):
        model.setValue(str(bonus.getCount()))
        name = (b'token_{}').format(cls.__getProgressionNameFromToken(token))
    ...
    def __getProgressionNameFromToken(cls, token):
        name = token.id.split(b':')[1].split(b'_')[-1]
        return name
