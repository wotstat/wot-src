...
def getBonusPacker():
    ...
    packer.getPackers().update({b'blueprints': (WinbackBlueprintUIPacker()), 
...
def _sortByName(firstVehicleTuple, secondVehicleTuple):
    ...
    firstUserName = firstVehicleDict[b'vehicle'].userName
    secondUserName = secondVehicleDict[b'vehicle'].userName
...
def _sortAndClearVehicles(vehicles):
    ...
    for nation in GUI_NATIONS:
        ...
        for vehicleType in _TYPES_ORDER:
            sortedByName = sorted([(cd, veh) for cd, veh in sortedByType if veh[b'vehicle'].type == vehicleType], cmp=_sortByName)
...
class WinbackSelectableRewardView(ViewImpl):
    ...
    def createToolTipContent(self, event, contentID):
        ...
        if contentID == R.views.winback.lobby.tooltips.SelectableRewardTooltip():
            ...
            return SelectableRewardTooltip(**self.__bonuses[level][b'tooltipData'].specialArgs)
