...
class WinbackWidgetView(ViewImpl):
    ...
    def _getEvents(self):
        return (
         ...
         (
          self.__winbackController.winbackProgression.onProgressPointsUpdated, self.__updateModel),
         (
          self.__winbackController.winbackProgression.onSettingsChanged, self.__updateModel))
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.winback.lobby.tooltips.WidgetTooltipView():
            return WidgetTooltipView()
        return super(WinbackWidgetView, self).createToolTipContent(event, contentID)
