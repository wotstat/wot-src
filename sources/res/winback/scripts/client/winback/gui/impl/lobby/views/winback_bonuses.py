...
class WinbackSelectableBonus(SelectableBonus):
    ...
    def getLevel(self):
        return self.data.get(b'level', -1)
    ...
    def _getFirstGift(self):
        tokenName = self.getTokenId().replace(b'_gift', b'')
    ...
    def getTooltip(self):
        specialArgs = {b'level': (self.getLevel()), 
           b'isDiscount': (self._isDiscount), 
           b'purchaseDiscount': (self.getPurchaseDiscount()), 
           b'researchDiscount': (self.getResearchDiscount())}
        return [
         backport.createTooltipData(isSpecial=True, specialArgs=specialArgs)]
...
class WinbackVehicleBonus(SimpleBonus):
    ...
    def getRentData(self, vehCD):
        return self._getVehData(vehCD).get(b'rent', {})
    ...
    def getRentValue(self, vehCD):
        ...
        return rentData.get(b'wins', 0) or rentData.get(b'battles', 0) or rentData.get(b'time', 0)
    ...
    def getCrewLevel(self, vehCD):
        return self._getVehData(vehCD).get(b'crewLvl')

    def getTankmenRoleLevelsAndSkills(self, vehCD):
        tankmen = self._getVehData(vehCD).get(b'tankmen', [])
        tankmenSkills = [tankman.get(b'skills', []) for tankman in tankmen]
        tankmenRoleLevels = [tankman.get(b'roleLevel', -1) for tankman in tankmen]
    ...
    def _getRentTooltip(self, vehicleCD):
        ...
        specialArgs = (
         ...
         self._getVehData(vehicleCD).get(b'slot', 0) > 0,
         bool(self._getVehData(vehicleCD).get(b'unlockModules', [])))
...
class WinbackVehicleDiscountBonus(WinbackVehicleBonus):
    __slots__ = (b'currencyDiscount', b'xpDiscount')
    ...
    def getPrices(self, vehCD):
        return self._getVehData(vehCD).get(b'prices', (None, None))

    def getXPs(self, vehCD):
        return self._getVehData(vehCD).get(b'xps', (None, None))
    ...
    def _createVehicleTooltip(self, vehicleCD):
        return backport.createTooltipData(isSpecial=True, specialArgs=[
         ...
         True, True], specialAlias=TOOLTIPS_CONSTANTS.WINBACK_DISCOUNT_AWARD_VEHICLE)
