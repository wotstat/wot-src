...
class WinbackSelectableRewardManager(SelectableRewardManager):
    ...
    def getRawBonusOptions(cls, bonus):
        ...
        return {gift.id: {b'option': (gift.rawBonuses), b'count': (gift.giftCount), b'limit': (gift.limit())} for gift in offer.getAllGifts()}
    ...
    def createBonusFromId(cls, tokenID):
        ...
        if token is None:
            ...
            if cls.isOfferCompensated(tokenID):
                return cls.createCompensationBonusFromOffer(tokenID)
            return cls._createSelectableBonus(tokenID, token)

    @staticmethod
    def getSelectableBonusName(tokenID):
        offerType = tokenID.split(b':')[2]
    ...
    def _createSelectableBonus(cls, tokenID, token):
        ...
        if bonusName is None:
            ...
            data.update({b'level': level})
            return WinbackSelectableBonus(bonusName, {tokenID: data})

    @classmethod
    def isOfferCompensated(cls, tokenID):
        compensationTokenID = tokenID + b'_compensation'
        return cls._itemsCache.items.tokens.getTokenCount(compensationTokenID) > 0

    @classmethod
    def createCompensationBonusFromOffer(cls, tokenID):
        questId = tokenID.replace(b'offer:', b'').replace(b':', b'_') + b'_compensation'
        quest = first(cls._eventsCache.getAllQuests((lambda q: q.getID() == questId)).values())
        supportedBonus = b'credits'
        ctx = {b'level': (getLevelFromSelectableToken(tokenID)), 
           b'isDiscount': (cls.getSelectableBonusName(tokenID) == RewardName.SELECTABLE_VEHICLE_DISCOUNT.value)}
        if quest:
            return first(quest.getBonuses(isCompensation=True, bonusName=supportedBonus, ctx=ctx))
        else:
            return
