...
class WinbackProgressionController(object):
    ...
    PROGRESSION_FILTER_FUNC_KEY = b'WinbackFuncKey'
    ...
    def isFinished(self):
        if not self.isEnabled:
            return False
        return self.getCurPoints() >= self._getPointsForLevel() * len(self.__stages)

    def __onTokensUpdate(self, tokens):
        if self.__winbackController.progressionToken in tokens:
            self.onProgressPointsUpdated()
        if any(self.__winbackController.isWinbackOfferToken(token) and token.endswith(b'_gift') for token in tokens):
            self.onGiftTokenUpdated()
        return

    def setSettings(self, settings, isProgressionSwitched):
        self.__stages = sorted([stage for stage in settings.get(b'awardList', []) if stage[0] is not None], key=(lambda s: s[0]))
        self.__stageRatings = [stage[0] for stage in self.__stages]
        self.__pointsForLevel = first(self.__stageRatings, 0)
        self.__isEnabled = bool(self.__stages) and settings.get(b'isProgressionEnabled', False)
    ...
    def getProgressionData(self):
        ...
        return {b'currentPoints': currentPoints, 
           b'pointsForLevel': (self._getPointsForLevel()), 
           b'previousPoints': previousPoints, 
    ...
    def getTokensIdsByRewardFromLevel(self, level):
        if level < 1:
            ...
            return [tokenID.replace(b'_gift', b'') for tokenID in bonuses.get(b'tokens', {}).keys()]
    ...
    def _getPointsForLevel(self):
        if self.__pointsForLevel == 0:
            _logger.error(b'WinbackProgressionController without stages or incorrect stage configuration')
