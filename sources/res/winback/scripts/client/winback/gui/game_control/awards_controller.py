...
class WinbackProgressionStageHandler(ServiceChannelHandler):
    ...
    def _showAward(self, ctx):
        _, message = ctx
        stages = message.data.get(b'stages', set())
        for stage in stages:
            if stage.get(b'showAwardWindow', False):
                showAwardsView({b'bonuses': (stage[b'detailedRewards']), 
                   b'isOnlyDaily': False, 
                   b'selectedRewards': False, 
                   b'isLastWindow': (stage[b'finishStage']), 
                   b'stageNumber': (stage[b'stage'])})
...
class WinbackQuestHandler(MultiTypeServiceChannelHandler):
    ...
    def __init__(self, awardCtrl):
        super(WinbackQuestHandler, self).__init__((
         SYS_MESSAGE_TYPE.tokenQuests.index(), SYS_MESSAGE_TYPE.battleResults.index()), awardCtrl)
        self.__quests = {}
        return
    ...
    def _showAward(self, ctx):
        ...
        for bonusesIndex, bonuses in enumerate(splittedBonuses):
            ...
            window = WinbackRewardWindow(ctx={b'quests': (quests.keys()[fromIdx:toIdx]), b'bonuses': bonuses, 
               b'isOnlyDaily': isOnlyDaily, 
    ...
    def _needToShowAward(self, ctx):
        _, message = ctx
        if not super(WinbackQuestHandler, self)._needToShowAward(ctx):
            return False
        return bool(self.__checkWinbackQuests(message.data))
    ...
    def _getMainBonusesList(self, bonuses):
        ...
        for bonusName, bonusData in bonuses.items():
            if bonusName == b'premium_plus':
                ...
                result += [{b'tokens': {tokenName: (bonusData.get(tokenName))}} for tokenName in bonusData.keys() if self.__winbackController.isWinbackOfferToken(tokenName)]
            elif bonusName == VehiclesBonus.VEHICLES_BONUS:
                result += [{(VehiclesBonus.VEHICLES_BONUS): {vehicleCD: vehicleData}} for vehicleBlock in bonusData for vehicleCD, vehicleData in vehicleBlock.iteritems() if vehicleData.get(b'compensatedNumber', 0) <= 0]
    ...
    def __checkWinbackQuests(self, data):
        ...
        if qIDs:
            self.__quests = OrderedDict(sorted([(qID, data.get(b'detailedRewards', {}).get(qID)) for qID in qIDs], key=(lambda item: getIdxFromQuestID(item[0]))))
        return qIDs
...
def registerWinbackProgressionAwardController():
    ...
    registerAwardControllerHandlers((
     WinbackProgressionStageHandler,
     WinbackQuestHandler))
    return
