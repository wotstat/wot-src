...
class WinbackController(IWinbackController, IGlobalListener):
    __slots__ = (b'__state', b'__activeProgressionKey', b'__winbackProgressionController', b'__winbackHintsController')
    ...
    def progressionName(self):
        return self.activeProgressionConfig.get(b'progressionName')

    @property
    def winbackPromoURL(self):
        return self.activeProgressionConfig.get(b'promoURL', b'').format(languageCode=getLanguageCode())

    @property
    def winbackInfoPageURL(self):
        return self.activeProgressionConfig.get(b'infoPageUrl', b'').format(languageCode=getLanguageCode())

    @property
    def accessToken(self):
        return self.activeProgressionConfig.get(b'accessToken')

    @property
    def progressionToken(self):
        return self.activeProgressionConfig.get(b'progressionToken')
    ...
    def isWinbackOfferToken(self, offerToken):
        if not self.winbackConfig.isEnabled:
            return False
        offerTokenPrefix = self.activeProgressionConfig.get(b'offerTokenPrefix')
        if offerTokenPrefix:
            return offerToken.startswith(offerTokenPrefix)
        return False
    ...
    def getHeaderFlagState(self):
        ...
        return {b'visible': True, 
           b'enable': (self.isProgressionEnabled() and totalCount > 0), 
           b'flagIcon': flagIcon, 
           b'flagLabel': label, 
           b'flagMain': (self.__getHangarFlag()), 
           b'tooltip': (TOOLTIPS_CONSTANTS.WINBACK_QUESTS_PREVIEW)}

    def __getHangarFlag(self):
        imageName = (b'versus_ai_{}').format(self.progressionName)
    ...
    def _isWinbackOfferGiftToken(self, token):
        return self.isWinbackOfferToken(token) and token.endswith(b'_gift')
    ...
    def __onTokensUpdate(self, diff, _):
        ...
        if not tokens:
            ...
            if any(token.endswith(b':access') for token in tokens.keys()):
                self.__updateSettings()
    ...
    def __updateSettings(self, force=False):
        ...
        isProgressionSwitched = isProgressionSwitched or self.isProgressionEnabled() != self.activeProgressionConfig.get(b'isProgressionEnabled', False)
    ...
    def __getActiveProgressionKey(self):
        for key, item in self.winbackConfig.progressions.items():
            accessToken = item.get(b'accessToken')
    ...
    def __hasPromoToken(self):
        promoToken = self.activeProgressionConfig.get(b'showPromoToken')
