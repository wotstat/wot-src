...
def showProgressionView(activeTab=None):
    ...
    if _getWinbackProgressionViewIfHas() is not None:
        ...
        g_eventBus.handleEvent(events.LoadGuiImplViewEvent(GuiImplViewLoadParams(viewRes, view, scope=ScopeTemplates.LOBBY_SUB_SCOPE), ctx={b'menuName': activeTab}), scope=EVENT_BUS_SCOPE.LOBBY)
        return
