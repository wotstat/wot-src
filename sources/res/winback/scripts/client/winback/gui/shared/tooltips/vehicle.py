...
class WinbackExtendedVehicleInfoTooltipData(VehicleInfoTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        allModulesAvailable = params.get(b'allModulesAvailable', False)
        if showCrew or showVehicleSlot or allModulesAvailable:
            additionalItemsBlocks = [
             formatters.packTextBlockData(text_styles.middleTitle(backport.text(R.strings.tooltips.vehicle.additional.header())), padding=formatters.packPadding(left=20))]
            if allModulesAvailable:
                additionalItemsBlocks.append(formatters.packImageTextBlockData(title=b'', desc=text_styles.main(backport.text(R.strings.tooltips.vehicle.allModules.header())), img=backport.image(R.images.gui.maps.icons.quests.bonuses.small.allModules()), imgPadding=formatters.packPadding(left=35, top=10), txtPadding=formatters.packPadding(left=25, top=20)))
            if showCrew:
                tankmenRoleLevels, tankmenSkills = params.get(b'tmanRoleLevel', ([], []))
                ...
                additionalItemsBlocks.append(formatters.packImageTextBlockData(title=b'', desc=text_styles.main(backport.text(crewText(), value=crewLevel, skills=crewSkillsString)), img=backport.image(crewImg()), imgPadding=formatters.packPadding(left=imgPaddingLeft, top=10), txtPadding=formatters.packPadding(left=txtPaddingLeft, top=txtPaddingTop)))
            if showVehicleSlot:
...
class WinbackDiscountExtendedVehicleInfoTooltipData(VehicleInfoTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        if self.context.getParams().get(b'showDiscount', False):
