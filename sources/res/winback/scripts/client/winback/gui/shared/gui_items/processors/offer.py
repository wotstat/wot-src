...
class WinbackMultipleOfferProcessor(ReceiveMultipleOfferGiftsProcessor):
    ...
    def __updateBlueprints(self, successData):
        blueprints = successData.get(b'blueprints')
        if not blueprints:
            return
        else:
            for offerID, giftIDs in self._chosenGifts.iteritems():
                ...
                for giftID in giftIDs:
                    ...
                    if gift is None:
                        _logger.error(b'Wrong giftID=%s for offerID=%s', giftID, offerID)
                        continue
                    chosenBlueprintsBonuses = gift.rawBonuses.get(b'blueprints', {})
