...
class WinbackQuestsPreviewTooltipData(QuestsPreviewTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        if quests:
            items.append(self._getHeader(len(quests), b'', R.strings.tooltips.hangar.header.quests.description()))
    ...
    def _getQuests(self, *args):
        return sorted(self.__winbackController.winbackProgression.questContainer.getAvailableQuests().values(), key=(lambda q: (
         q.isCompleted(), -q.getPriority(), q.getID())))
