...
class WinbackQuestAchievesFormatter(QuestAchievesFormatter):
    ...
    def _formatDiscounts(cls, rewards):
        ...
        if b'blueprints' in rewards or b'goodies' in rewards:
            ...
            if winbackBonuses:
                return backport.text(discountsRes.discountHeader()) + (b', ').join(results)
        return
...
class WinbackProgressionAchievesFormatter(QuestAchievesFormatter):
    ...
    def getFormattedAchieves(cls, data, asBattleFormatter, processCustomizations=True, processTokens=True):
        rewards = deepcopy(data)
        tokenResult = cls._preprocessTokens(rewards.get(b'tokens', {})) if processTokens else None
        boostersResult = cls._preprocessBoosters(rewards.get(b'goodies', {}))
    ...
    def _preprocessBoosters(cls, goodies):
        ...
        if boostersStrings:
            return g_settings.htmlTemplates.format(b'winbackBoostersInvoiceReceived', ctx={b'boosters': ((b', ').join(boostersStrings))})
        else:
            return
    ...
    def _preprocessTokens(cls, tokens):
        ...
        for tokenID in tokens.keys():
            tankmanTokenResult = cls._processTankmanToken(tokenID, tokens[tokenID].get(b'count', 1))
            if tankmanTokenResult:
                tankmenResult.append(tankmanTokenResult)
                del tokens[tokenID]
            elif cls.__winbackController.isWinbackOfferToken(tokenID):
    ...
    def _processTankmanToken(tokenID, count):
        ...
        if tankmanInfo is not None:
            return g_settings.htmlTemplates.format(b'winbackTankman', {b'name': (tankmanInfo.getFullUserName() if tankmanInfo.getLastName() else tankmanInfo.getFirstName()), 
               b'count': count})
        else:
            return
    ...
    def _processSelectableBonusToken(tokenID):
        compensationTemplateSuffix = b''
        compensationValue = 0
        if WinbackSelectableRewardManager.isOfferCompensated(tokenID):
            compensationTemplateSuffix = b'Compensation'
        ...
        formattedResult = g_settings.htmlTemplates.format((b'').join([b'winback', selectableType, compensationTemplateSuffix]), ctx={b'level': (int2roman(level)), b'value': compensationValue})
        return (
         level, formattedResult)


class WinbackProgressionSystemMessageFormatter(ServiceChannelFormatter):
    __TEMPLATE_BASE = b'ProgressionSystemMessage'
    __TEMPLATE_SIMPLE = b'winbackProgressionSystemMessage'
    __winbackController = dependency.descriptor(IWinbackController)
        ...
        for tokenID in tokens.keys():
            if self.__winbackController.isWinbackOfferToken(tokenID):
                ...
                if WinbackSelectableRewardManager.isOfferCompensated(tokenID):
                    ...
                    continue
                return b'SelectRewardButton'

        return b'ProgressionButton'

    def _getTemplate(self, data):
        return (b'').join([
         b'VersusAI', self.__winbackController.progressionName,
         self.__TEMPLATE_BASE, self._selectButtonTemplate(data)])
    ...
    def _formatSingleStageCompletion(self, message, stageInfo):
        ...
        if not rewardsData:
            ...
            formattedMessage = g_settings.htmlTemplates.format(self.__TEMPLATE_SIMPLE, ctx={b'header': messageHeader, 
               b'body': messageBody, 
               b'awards': formattedRewards})
            return MessageData(g_settings.msgTemplates.format(template, ctx={b'message': formattedMessage}, data={}), self._getGuiSettings(message, template, decorator=decorator))
