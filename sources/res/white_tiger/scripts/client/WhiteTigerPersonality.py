...
class ClientWhiteTigerBattleMode(wt_constants.WhiteTigerBattleMode):
    ...
    def registerAdditionalSystemMessageTypes(self):
        ...
        SM_TYPE.inject([b'WTLootBoxRewards', b'WTLootBoxRerollRewards'])
        return


def preInit():
    _logger.debug(b'preInit personality: %r', __name__)
...
def init():
    _logger.debug(b'init: %r', __name__)
    LOBBY_EXT_PACKAGES = [
     b'white_tiger.gui.Scaleform.daapi.view.lobby',
     b'white_tiger.gui.Scaleform.daapi.view.lobby.header']
    ...
    g_overrideScaleFormViewsConfig.initExtensionBattlePackages(__name__, [
     b'white_tiger.gui.Scaleform.daapi.view.battle.white_tiger'], wt_constants.ARENA_GUI_TYPE.WHITE_TIGER)
    return
