...
class _WTItem(_TriggerItem):
    ...
    def canActivate(self, entityName=None, avatar=None):
        ...
        if self._isLocked:
            result = False
            error = _ActivationError(b'wtEquipmentRestriction', None)
            return (
             result, error)
        else:
...
class _TeleportItem(_WTItem):
    ...
    def __init__(self, descriptor, quantity, stage, timeRemaining, _, tags=None):
        ...
        self.teleportPoints = [{b'guid': (udo.guid), b'position': (udo.position.x, udo.position.z), b'position3D': (udo.position.x, udo.position.y, udo.position.z), b'yaw': (udo.yaw), b'index': (udo.positionNumber)} for udo in BigWorld.userDataObjects.values() if isinstance(udo, TeleportKeyPoint)]
    ...
    def apply(self, pointGuid):
        ...
        for spawn in self.teleportPoints:
            if spawn[b'guid'] == pointGuid:
                chosen = spawn
                break

        BigWorld.player().setEquipmentApplicationPoint(self.getEquipmentID(), Vector3(chosen[b'position3D']), Vector2(chosen[b'yaw'], 0))
        return
...
class _HyperionItem(_WTItem):
    ...
    def setLocked(self, isLocked):
        ...
        if vehicleID is not None:
            vehicle = BigWorld.entities.get(vehicleID)
            if self._stage == EQUIPMENT_STAGES.ACTIVE and b'wtTeleportDebuff' in vehicle.dynamicComponents:
...
def registerWTEquipmentsItems():
    registerEquipmentItem(b'builtinAfterburning_wt', _WTItem, _ReplayItem)
    registerEquipmentItem(b'builtinInstantStunShoot_wt', _WTItem, _ReplayItem)
    registerEquipmentItem(b'builtinImpulse_wt', _WTItem, _ReplayItem)
    registerEquipmentItem(b'builtinShield_wt', _WTItem, _ReplayItem)
    registerEquipmentItem(b'builtinSelfRepair_wt', _WTSelfRepairItem, _ReplayItem)
    registerEquipmentItem(b'builtinStronghold_wt', _WTStrongholdItem, _ReplayItem)
    registerEquipmentItem(b'builtinTeleport_wt', _TeleportItem, _ReplayItem)
    registerEquipmentItem(b'builtinChargedShot_wt', _ShellOverrideItem, _ReplayItem)
    registerEquipmentItem(b'builtinExplosiveShot_wt', _ShellOverrideItem, _ReplayItem)
    registerEquipmentItem(b'builtinHyperion_wt', _HyperionItem, _ReplayItem)
    return
