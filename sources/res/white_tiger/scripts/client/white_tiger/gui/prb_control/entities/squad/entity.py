...
class WhiteTigerBattleSquadEntryPoint(SquadEntryPoint):
    ...
    def makeDefCtx(self):
        return SquadSettingsCtx(PREBATTLE_TYPE.WHITE_TIGER, waitingID=b'prebattle/create', accountsToInvite=self._accountsToInvite)
...
class WhiteTigerBattleSquadEntity(SquadEntity):
    ...
    def _setVehicle(self, ctx, callback=None):
        ...
        if not vehTypeCD:
            _logger.error(b'Vehicle is not valid  %r', ctx)
            if callback:
                callback(False)
            return
        self._requestsProcessor.doRequest(ctx, b'doCustomUnitCmd', CLIENT_UNIT_CMD.WT_SQUAD_SELECT_VEHICLE, uint64Arg=vehTypeCD, callback=callback)
        return
