...
class WhiteTigerEntity(PreQueueEntity, WhiteTigerVehicleSwitcher):
    ...
    def requeue(self, vehicle=None, vehInvID=None):
        if not self.isInQueue():
            raise SoftException(b'Player is not in the white tiger queue')
        if vehicle is not None:
            g_currentVehicle.selectVehicle(vehicle.invID)
        elif vehInvID is not None:
            g_currentVehicle.selectVehicle(vehInvID)
        else:
            raise SoftException(b'No vehicle data or vehicle inventory id provided')
    ...
    def _doQueue(self, ctx):
        ...
        LOG_DEBUG(b'Sends request on queuing to the White Tiger battles', self._queueType, ctx)
        return

    def _doDequeue(self, ctx):
        self._accountComponent.dequeueBattle(QUEUE_TYPE.WHITE_TIGER)
        LOG_DEBUG(b'Sends request on dequeuing from the White Tiger battles', self._queueType)
        return
    ...
    def _makeQueueCtxByAction(self, action=None):
        return WhiteTigerQueueCtx(g_currentVehicle.item.invID, entityType=self._queueType, waitingID=b'prebattle/join')
