...
class WhiteTigerBattleScheduler(BaseScheduler):
    ...
    def __show(self, status, isInit=False):
        ...
        if not self.__isConfigured:
            ...
            SystemMessages.pushMessage(text=backport.text(strRes.primeTime.notAvailable.body()), messageData={b'header': (backport.text(strRes.primeTime.notAvailable.header()))}, type=SystemMessages.SM_TYPE.WarningHeader, priority=NotificationPriorityLevel.HIGH)
        return
