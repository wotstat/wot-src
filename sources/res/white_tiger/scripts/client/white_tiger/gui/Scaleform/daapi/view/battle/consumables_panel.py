...
class WhiteTigerConsumablesPanel(WhiteTigerConsumablesPanelMeta):
    ...
    def _buildEquipmentSlotTooltipText(self, item):
        ...
        if not {
         b'repairkit', b'medkit'} & descriptor.tags:
            ...
            body = (b'').join((body, additionalStr))
        if reloadingTime > 0:
    ...
    def __createEmptyEquipmentSlot(self, idx):
        ...
        self.as_addWhiteTigerEquipmentSlotS(idx=idx, keyCode=bwKey, sfKeyCode=sfKey, quantity=0, timeRemaining=0, reloadingTime=0, iconPath=b'', tooltipText=backport.text(R.strings.ingame_gui.consumables_panel.equipment.tooltip.empty()), animation=ANIMATION_TYPES.NONE, tag=None, stage=EQUIPMENT_STAGES.NOT_RUNNING)
