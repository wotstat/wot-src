...
def clockSh(vspace=-6, hspace=-6):
    return icons._getIcon(b'clockSh', vspace=vspace, hspace=hspace)
...
class EventBattlesSelectorTooltip(BlocksTooltipData):
    ...
    def _packPeriods(self, periods):
        if periods:
            ...
            for periodStart, periodEnd in periods:
                startTime = formatDate(b'%H:%M', periodStart)
                endTime = formatDate(b'%H:%M', periodEnd)
    ...
    def __formatTillEndBlock(timeLeft):
        return formatters.packTextBlockData(text_styles.main(backport.text(_STR_PATH.tooltips.selector.timeTable.tillEnd())) + b' ' + _getLeftTime(timeLeft))
...
class EventBattlesCalendar(BlocksTooltipData):
    ...
    def __packFooter(self):
        ...
        if endDate is None:
            _logger.error(b"Current season doesn'n have end date")
            return b''
        else:
...
class EventQuestsTooltipData(BlocksTooltipData):
    ...
    def __packHeader(self, quests, vehicle):
        ...
        return formatters.packImageTextBlockData(title=text_styles.highTitle(title), desc=desc, img=backport.image(R.images.white_tiger.gui.maps.icons.quests.dyn((b'{}_header').format(vehicle.eventType))()), txtPadding=formatters.packPadding(top=18), descPadding=formatters.packPadding(top=15, left=-4), txtOffset=20)
    ...
    def __packBottom(self, count):
        return formatters.packTextBlockData(text=makeHtmlString(b'html_templates:lobby/textStyle', b'alignText', {b'align': b'center', 
           b'message': (text_styles.main(backport.text(R.strings.event.tooltips.header.quests.bottom(), count=count)))}), padding=formatters.packPadding(bottom=20))
