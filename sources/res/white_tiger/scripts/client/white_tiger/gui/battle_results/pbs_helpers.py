...
def getEventID(reusable):
    return reusable.personal.avatar.extensionInfo.get(b'wtEventID', UNKNOWN_EVENT_ID)


def getTmenXp(reusable):
    personalResults = reusable.personal
    intCD, _ = first(personalResults.getVehicleItemsIterator())
    return personalResults.xpProgress.get(intCD, {}).get(b'xpByTmen', [])
...
def getTotalGoldToShow(reusable):
    ...
    return sum([records.findRecord(recordName) for recordName in (b'eventGoldList_',)])
...
def getWTAchievement(battleResults):
    ...
    leftAchievements, _ = reusable.personal.getAchievements(results[b'personal'])
...
def _getTokenCount(reusable, tokenName):
    questTokens = reusable.personal.avatar.extensionInfo.get(b'questTokensCount', {})
    if tokenName not in questTokens:
        return 0
    return questTokens[tokenName].get(b'diff', 0)


def getEquipCoin(reusable):
    return reusable.personal.avatar.extensionInfo.get(b'eventEquipCoin', 0)
