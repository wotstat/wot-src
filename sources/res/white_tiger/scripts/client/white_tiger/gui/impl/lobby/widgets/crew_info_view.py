...
class CrewInfoView(ViewComponent[CrewInfoViewModel]):
    ...
    def __fillModel(self):
        if not self.__wtCtrl.isSelectedVehicleWTVehicle():
            ...
            if info is None:
                _logger.error(b'There is no special characteristics of the event vehicle  %s to get commander role', vehicle.name)
                return
            commander = self.__getCommander(vehicle)
            if commander is None:
                _logger.error(b'There is no commander in the event vehicle %s', vehicle.name)
                return
