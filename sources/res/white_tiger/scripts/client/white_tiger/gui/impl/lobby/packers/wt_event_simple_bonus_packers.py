import logging, copy
from collections import namedtuple
import constants
from gui.impl import backport
from gui.impl.gen import R
from white_tiger.gui.impl.gen.view_models.views.lobby.portals.wt_portal_bonus_model import WtPortalBonusModel
from gui.impl.backport import createTooltipData
from white_tiger.gui.impl.lobby.wt_event_constants import BonusGroup, WhiteTigerLootBoxes
from gui.server_events.bonuses import mergeBonuses, getSplitBonusFunction, RandomCrewbook, RandomEventStyle3d, RandomEventStyle2d, RandomEventDecal, LootBoxTokensBonus, RandomEventInscription, RandomEventEmblem, RandomEventCamouflage, WtCustomizationsBonus
from gui.Scaleform.genConsts.TOOLTIPS_CONSTANTS import TOOLTIPS_CONSTANTS
from gui.shared.money import Currency
from gui.shared.missions.packers.bonus import BaseBonusUIPacker, SimpleBonusUIPacker, GoodiesBonusUIPacker, ItemBonusUIPacker, VehiclesBonusUIPacker, CustomizationBonusUIPacker
from gui.shared.utils.functions import makeTooltip
from white_tiger.gui.impl.lobby.packers.wt_event_bonuses_packers import WTCrewBookBonusUIPacker, WTCrewSkinBonusUIPacker
from helpers.dependency import replace_none_kwargs
from skeletons.gui.game_control import ILootBoxesController
from shared_utils import findFirst
from white_tiger.gui.impl.lobby.packers import isStyle3D
_logger = logging.getLogger(__name__)
_GroupedBonuses = namedtuple(b'_GroupedBonuses', (b'main', b'additional', b'vehicle'))
_RANDOM_STYLE_3D = b'randomStyle3d'
_RANDOM_STYLE_2D = b'randomStyle2d'
_RANDOM_DECAL = b'randomDecal'
_RANDOM_CREWBOOK = b'randomCrewbook'
_INSCRIPTION = b'inscription'
_EMBLEM = b'emblem'
_CAMOUFLAGE = b'camouflage'
_CREWBOOK = b'crewBooks'
_CREWSKINS = b'crewSkins'
HUNTER_BONUSES_ORDER = [
 Currency.CREDITS, b'goodies', b'items', constants.PREMIUM_ENTITLEMENTS.PLUS, b'slots',
 b'customizations', b'tmanToken', b'battleToken']
BOSS_BONUSES_ORDER = [Currency.GOLD, Currency.CREDITS, b'crewBooks', _RANDOM_CREWBOOK,
 b'goodies', constants.PREMIUM_ENTITLEMENTS.PLUS, _RANDOM_STYLE_2D, _RANDOM_DECAL, b'items']
_ICON_PATH = R.images.gui.maps.icons
_EXT_ICON_PATH = R.images.white_tiger.gui.maps.icons
_BONUS_TYPE_TO_GROUP = {_RANDOM_STYLE_3D: (BonusGroup.STYLE_3D), 
   b'vehicles': (BonusGroup.VEHICLES)}
NO_SPLITTED_BONUSES = frozenset((
 _RANDOM_CREWBOOK, _RANDOM_DECAL, _RANDOM_STYLE_2D, _RANDOM_STYLE_3D, _EMBLEM, _CREWBOOK))

def getWtSplitBonusFunction(bonus):
    if bonus.getName() in NO_SPLITTED_BONUSES:
        return None
    else:
        return getSplitBonusFunction(bonus)


def mergeWtBonuses(bonuses):
    for composer in _BONUS_COMPOSERS:
        bonuses = composer(bonuses)

    return mergeBonuses(bonuses)


_AWARD_VIEW_BONUS_POSITIONS = {b'vehicles': 2, 
   b'mainPrizeDiscount': 1, 
   b'ticket': 1, 
   b'tmanToken': 1}

def mergeWtProgressionBonuses(bonuses):
    bonuses = _composeProgressionCustomizations(bonuses)
    mergeBonuses(bonuses)
    if len(bonuses) > 1:
        newli = copy.copy(bonuses)
        for idx, x in enumerate(newli):
            name = x.getName()
            if name in _AWARD_VIEW_BONUS_POSITIONS:
                newIdx = _AWARD_VIEW_BONUS_POSITIONS[name]
                bonuses.insert(newIdx, bonuses.pop(idx))

    return bonuses


def getWtBonusGroup(bonuses):
    for bonus in bonuses:
        bonusName = bonus.getName()
        group = _BONUS_TYPE_TO_GROUP.get(bonusName)
        if group is not None:
            return group
        if bonusName == b'battleToken' and isinstance(bonus, LootBoxTokensBonus):
            return BonusGroup.LOOTBOX

    return BonusGroup.OTHER


def fillPortalRewardModel(model, bonuses):
    for bonus in bonuses:
        if not bonus.isShowInGUI():
            continue
        bonusProcessor = _SEGREGATE_BONUSES.get(bonus.getName())
        if bonusProcessor is not None:
            bonusProcessor.pack(model, bonus)

    return


def sortBonuses(bonus, orderList):
    bonusName = bonus.getName()
    if bonusName in orderList:
        return orderList.index(bonusName)
    return len(orderList)


@replace_none_kwargs(boxCtrl=ILootBoxesController)
def _getBonusStatus(bonusType, bonusGroup, boxCtrl=None):
    rewards = boxCtrl.getLootBoxesRewards(WhiteTigerLootBoxes.WT_BOSS)
    if not rewards:
        _logger.error(b'There is not boss rewards')
        return None
    else:
        rewards = rewards.get(bonusGroup)
        bonus = findFirst((lambda b: b.getName() == bonusType), rewards.bonuses)
        if not bonus:
            _logger.error(b'Bonus is absent in the boss lootBox')
            return None
        if all([bonus.getC11nItem(item).fullCount() > 0 for item in bonus.getValue()]):
            return _Status.COLLECTED
        return _Status.NOT_COLLECTED


def _getStyles3dStatus():
    return _getBonusStatus(_RANDOM_STYLE_3D, BonusGroup.CUSTOMIZATION)


class _Status(object):
    COLLECTED = b'collected'
    NOT_COLLECTED = b'notCollected'


STATUS_GETTERS = {_RANDOM_STYLE_3D: _getStyles3dStatus}

def _createBonus(bonusClazz, bonusName, bonuses):
    valueItems = []
    for bonus in bonuses:
        value = bonus.getValue()
        if isinstance(value, list):
            valueItems.extend(value)
        else:
            valueItems.append(value)

    return bonusClazz(bonusName, valueItems)


def _composeCrewBooks(bonuses):
    other, commonCrewBooks = [], []
    for bonus in bonuses:
        if bonus.getName() != b'crewBooks':
            other.append(bonus)
            continue
        for book, count in bonus.getItems():
            if book is None or not count:
                continue
            if book.isCommon():
                commonCrewBooks.append(bonus)
            else:
                other.append(bonus)

    if len(commonCrewBooks) > 1:
        other.append(_createBonus(RandomCrewbook, _RANDOM_CREWBOOK, commonCrewBooks))
    return other


def _composeCustomization(bonuses):
    styles2d, styles3d, decals, inscriptions, camouflages, emblems, other = ([], [], [], [], [], [], [])
    for bonus in bonuses:
        if bonus.getName() != b'customizations':
            other.append(bonus)
            continue
        for item in bonus.getCustomizations():
            if item is None:
                continue
            customizationItem = bonus.getC11nItem(item)
            itemType = customizationItem.itemTypeName
            if itemType == b'style':
                if isStyle3D(customizationItem):
                    styles3d.append(bonus)
                else:
                    styles2d.append(bonus)
            elif itemType in (b'decal', b'projectionDecal'):
                decals.append(bonus)
            elif itemType == b'inscription':
                inscriptions.append(bonus)
            elif itemType == b'emblem':
                emblems.append(bonus)
            elif itemType == b'camouflage':
                camouflages.append(bonus)
            else:
                other.append(bonus)

    if styles2d:
        other.append(_createBonus(RandomEventStyle2d, _RANDOM_STYLE_2D, styles2d))
    if styles3d:
        other.append(_createBonus(RandomEventStyle3d, _RANDOM_STYLE_3D, styles3d))
    if decals:
        other.append(_createBonus(RandomEventDecal, _RANDOM_DECAL, decals))
    if inscriptions:
        other.append(_createBonus(RandomEventInscription, _INSCRIPTION, inscriptions))
    if emblems:
        other.append(_createBonus(RandomEventEmblem, _EMBLEM, emblems))
    if camouflages:
        other.append(_createBonus(RandomEventCamouflage, _CAMOUFLAGE, camouflages))
    return other


def _composeProgressionCustomizations(bonuses):
    progBonus = []
    for bonus in bonuses:
        bonusName = bonus.getName()
        if bonusName == b'battleToken':
            continue
        elif bonusName == b'customizations':
            progBonus.append(WtCustomizationsBonus(bonusName, bonus.getValue()))
        else:
            progBonus.append(bonus)

    return progBonus


_BONUS_COMPOSERS = (
 _composeCrewBooks, _composeCustomization)

class StylePacker(object):
    _TOOLTIP_TYPE = None

    @classmethod
    def pack(cls, model, bonus):
        item = WtPortalBonusModel()
        item.setIsCollected(STATUS_GETTERS.get(cls._TOOLTIP_TYPE) == _Status.COLLECTED)
        model.addViewModel(item)
        return


class Event3dStylePacker(StylePacker):
    _TOOLTIP_TYPE = _RANDOM_STYLE_3D


class Event2dStylePacker(StylePacker):
    _TOOLTIP_TYPE = _RANDOM_STYLE_2D


class EventDecalPacker(StylePacker):
    _TOOLTIP_TYPE = _RANDOM_DECAL


class SimpleEventCountablePacker(SimpleBonusUIPacker):

    @classmethod
    def _packSingleBonus(cls, bonus, label=b''):
        model = WtPortalBonusModel()
        model.setIcon(bonus.getName())
        return model


class SimpleEventGoldPacker(SimpleEventCountablePacker):

    @classmethod
    def _getToolTip(cls, bonus):
        return [
         createTooltipData(tooltip=makeTooltip(backport.text(R.strings.event.bonusTooltip.gold.header()), backport.text(R.strings.event.bonusTooltip.gold.body())))]


class SimpleEventPremiumPacker(SimpleBonusUIPacker):

    @classmethod
    def _packSingleBonus(cls, bonus, label=b''):
        model = WtPortalBonusModel()
        model.setIcon(b'premium_plus_universal')
        return model


class SimpleEventGoodiesPacker(GoodiesBonusUIPacker):

    @classmethod
    def _packIconBonusModel(cls, bonus, icon, count, label):
        model = WtPortalBonusModel()
        model.setIcon(icon)
        return model

    @classmethod
    def _getBoostersToolTip(cls, tooltipData, booster):
        boosterName = booster.boosterGuiType
        tooltipData.append(createTooltipData(tooltip=makeTooltip(backport.text(R.strings.event.bonusTooltip.dyn(boosterName).header()), backport.text(R.strings.event.bonusTooltip.dyn(boosterName).body()))))
        return


class SimpleEventItemPacker(ItemBonusUIPacker):

    @classmethod
    def _packSingleBonus(cls, bonus, item, count):
        model = WtPortalBonusModel()
        model.setIcon(item.getGUIEmblemID())
        return model


class CustomizationEventItemPacker(CustomizationBonusUIPacker):

    @classmethod
    def _packSingleBonus(cls, bonus, item, count):
        model = WtPortalBonusModel()
        customizationItem = bonus.getC11nItem(item)
        if isStyle3D(customizationItem):
            model.setIcon(R.images.gui.maps.icons.quests.bonuses.small.style_3d())
        else:
            model.setIcon(R.images.gui.maps.icons.quests.bonuses.small.style())
        return model

    @classmethod
    def _getIcon(cls, item):
        itemTypeName = cls._getTypeIcon(item)
        return (b'_').join([itemTypeName, str(item.id)])

    @classmethod
    def _getTypeIcon(cls, item):
        itemTypeName = b'style3d' if isStyle3D(item) else str(item.itemTypeName)
        return itemTypeName


class SimpleEventCrewBookPacker(WTCrewBookBonusUIPacker):

    @classmethod
    def _packSingleBonus(cls, bonus, book, count):
        model = WtPortalBonusModel()
        model.setIcon(book.getBonusIconName())
        return model


class SimpleEventCrewSkinPacker(WTCrewSkinBonusUIPacker):

    @classmethod
    def _packSingleBonus(cls, bonus, book, count, _=b''):
        model = WtPortalBonusModel()
        return model


class SimpleEventRandomElementPacker(BaseBonusUIPacker):

    @classmethod
    def _pack(cls, bonus):
        model = WtPortalBonusModel()
        return [
         model]


class SimpleEventRandomCrewBookPacker(BaseBonusUIPacker):

    @classmethod
    def _pack(cls, bonus):
        model = WtPortalBonusModel()
        return [
         model]

    @classmethod
    def _getToolTip(cls, bonus):
        xp = bonus.getXP()
        bookType = bonus.getBookType()
        return [
         createTooltipData(tooltip=makeTooltip(backport.text(R.strings.tooltips.crewBooks.storage.filters.dyn(bookType).title()), backport.text(R.strings.tooltips.selectableCrewbook.info(), exp=xp)))]


class EmptyPacker(BaseBonusUIPacker):

    @classmethod
    def _pack(cls, bonus):
        return []


class WtEventVehiclesBonusUIPacker(VehiclesBonusUIPacker):

    @classmethod
    def _getTooltipAlias(cls):
        return TOOLTIPS_CONSTANTS.WT_PORTAL_VEHICLE


def getDefaultWtSimpleBonusPackersMap():
    return {(Currency.CREDITS): (SimpleEventCountablePacker()), 
       (Currency.GOLD): (SimpleEventGoldPacker()), 
       b'goodies': (SimpleEventGoodiesPacker()), 
       b'items': (SimpleEventItemPacker()), 
       b'slots': (SimpleEventCountablePacker()), 
       (constants.PREMIUM_ENTITLEMENTS.PLUS): (SimpleEventPremiumPacker()), 
       b'crewBooks': (SimpleEventCrewBookPacker()), 
       b'battleToken': (EmptyPacker()), 
       b'vehicles': (EmptyPacker()), 
       _RANDOM_CREWBOOK: (SimpleEventRandomCrewBookPacker()), 
       _RANDOM_STYLE_2D: (SimpleEventRandomElementPacker()), 
       _RANDOM_DECAL: (SimpleEventRandomElementPacker()), 
       _INSCRIPTION: (SimpleEventRandomElementPacker()), 
       _EMBLEM: (SimpleEventRandomElementPacker()), 
       _CAMOUFLAGE: (SimpleEventRandomElementPacker()), 
       _CREWSKINS: (SimpleEventCrewSkinPacker()), 
       b'customizations': (CustomizationEventItemPacker())}


_SEGREGATE_BONUSES = {_RANDOM_STYLE_3D: (Event3dStylePacker()), 
   _RANDOM_STYLE_2D: (Event2dStylePacker()), 
   _RANDOM_DECAL: (EventDecalPacker())}
