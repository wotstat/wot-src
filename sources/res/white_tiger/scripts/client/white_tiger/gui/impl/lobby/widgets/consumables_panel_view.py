...
class ConsumablesPanelView(ViewComponent[HangarConsumablesPanelViewModel]):
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.white_tiger.mono.lobby.tooltips.ammunition_panel_tooltip():
            return AmmunitionTooltipView(intCD=event.getArgument(b'id'))
    ...
    def __fillEquipment(self, array, item):
        ...
        for eq in item.consumables.layout:
            if not eq or b'hidden' in eq.tags:
                continue
            if b'repairkit' in eq.tags or b'medkit' in eq.tags:
                regulars.append(eq)
            elif b'Hyperion' in eq.name:
                ultimates.append(eq)
            elif b'builtin' in eq.tags or b'trigger' in eq.tags:
                abilities.append(eq)
