...
class WtVehicleParamsTooltipView(ViewImpl):
    ...
    def _onLoading(self, *args, **kwargs):
        with self.viewModel.transaction() as model:
            parameter = kwargs.get(b'parameter')
            if parameter is None:
                _logger.error(b'There is not parameter name in args to build tooltip.')
                return
            aspect = self.__getAspect(parameter)
            if aspect is None:
                _logger.error(b'Parameter %s is absent in vehicle characteristics', parameter)
                return
