...
_VOICE_CHAT_ENTER_SOUND_EVENT = b'comp_7_voicechat_enter'
_VOICE_CHAT_EXIT_SOUND_EVENT = b'comp_7_voicechat_exit'

class Comp7VOIPController(IComp7VOIPController):
    __slots__ = (b'__messageShown', b'__cooldownCallback', b'__isTeamChannelAvailable')
    ...
    def startControl(self, *_, **__):
        _logger.debug(b'[COMP7] Comp7VOIPController started.')
        self.__subscribe()
        return

    def stopControl(self):
        _logger.debug(b'[COMP7] Comp7VOIPController stopped.')
        self.__unsubscribe()
        self.__messageShown = False
    ...
    def toggleChannelConnection(self, automatic=False):
        if self.__sessionProvider.isReplayPlaying:
            ...
            if self.__cooldownCallback is not None:
                _logger.info(b'Failed to toggle Comp7 team VOIP channel. Cooldown is in progress.')
                return
            if not self.isTeamVoipEnabled:
                _logger.error(b'Failed to toggle Comp7 team VOIP channel. Joining is not allowed.')
                return
