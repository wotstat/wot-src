...
class Comp7CoreConsumablesPanel(ConsumablesPanel):
    ...
    def __addPoiEquipmentSlot(self, intCD, item):
        ...
        if idx is not None:
            ...
            _logger.error(b'Unknown PointOfInterest Type: %s', poiType)
        return
    ...
    def __updatePrebattleRoleSkill(self):
        ...
        tooltip = TOOLTIP_FORMAT.format(roleSkill.userString, body or b'')
    ...
    def __buildRoleEquipmentTooltipText(self):
        vehicle = self.__prebattleCtrl.getCurrentGUIVehicle()
        if not vehicle:
            _logger.info(b'Cannot create roleEquipment tooltip, spawnInfoForVehicle not received yet')
            return None
        else:
            roleSkill, body = getRoleEquipmentTooltipParts(vehicle, self._modeController)
            return TOOLTIP_FORMAT.format(roleSkill.userString if roleSkill else b'', body or b'')
