...
_ROLE_EQUIPMENT_READY = b'ROLE_EQUIPMENT_READY'
_POI_EQUIPMENT_USED = b'POI_EQUIPMENT_USED'
_POI_EQUIPMENT_USED_BY_ENEMY = b'POI_EQUIPMENT_USED_BY_ENEMY'
_POI_EQUIPMENT_USED_BY_ALLY = b'POI_EQUIPMENT_USED_BY_ALLY'
_ROLE_EQUIPMENT_APPLIED = b'ROLE_EQUIPMENT_APPLIED'
_ROLE_EQUIPMENT_PROMOTED = b'ROLE_EQUIPMENT_PROMOTED'
...
class Comp7CorePlayerMessages(PlayerMessages):
    ...
    def __onEquipmentUpdated(self, _, item):
        ...
        if item.getStage() == EQUIPMENT_STAGES.ACTIVE and item.getPrevStage() != EQUIPMENT_STAGES.ACTIVE:
            ...
            self.sessionProvider.shared.messages.showVehicleMessage(_ROLE_EQUIPMENT_APPLIED, {b'name': (equipment.userString)})
        if item.becomeReady and item.becomeAvailable:
            equipment = item.getDescriptor()
            self.sessionProvider.shared.messages.showVehicleMessage(_ROLE_EQUIPMENT_READY, {b'equipment': (equipment.userString)})
        return
    ...
    def __onPoiEquipmentUsed(self, equipment, vehicleID):
        if vehicleID == self.sessionProvider.shared.vehicleState.getControllingVehicleID():
            self.sessionProvider.shared.messages.showVehicleMessage(_POI_EQUIPMENT_USED, {b'equipment': (equipment.userString)})
        elif vehicleID == ENEMY_VEHICLE_ID:
            self.showMessage(_POI_EQUIPMENT_USED_BY_ENEMY, {b'equipment': (equipment.userString)})
        else:
            self.showMessage(_POI_EQUIPMENT_USED_BY_ALLY, {b'name': (self.sessionProvider.getCtx().getPlayerFullName(vehicleID, showClan=False)), 
               b'equipment': (equipment.userString)})
        return
