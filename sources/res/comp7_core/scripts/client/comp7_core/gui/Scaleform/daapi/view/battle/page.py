...
class _DynamicAliases(CONST_CONTAINER):
    LOBBY_NOTIFIER = b'lobbyNotifier'
...
_SOUND_NAME = b'comp_7_bans_fly_backward'
...
class Comp7BattlePage(Comp7BattlePageMeta):
    ...
    def showHelp(self):
        ...
        event_dispatcher.toggleHelpDetailed(ctx={b'isComp7': (arenaBonusType in self.__COMP7_RANKED_ARENA_BONUS_TYPES), 
           b'isComp7Light': (arenaBonusType in self.__COMP7_LIGHT_ARENA_BONUS_TYPES)})
        return
    ...
    def __handleCalloutDisplayEvent(self, event):
        self.__processCallout(needShow=event.ctx[b'isDown'])
        return
    ...
    def __updateCurrVehicleInfo(self, vehicle=None):
        ...
        if ctrl is None:
            ...
            if vehicle is not None and not avatar_getter.isObserver():
                ...
                isEliteOrPremium = vehicle.isPremium or bool(rawData.get(b'isElite', False))
                self.as_updateVehicleStatusS({b'tankType': ((b'{}_elite').format(vehicleClass) if isEliteOrPremium else vehicleClass), 
                   b'vehicleLevel': (int2roman(vehicle.level)), 
                   b'vehicleName': (vehicle.userName), 
                   b'roleId': vehicleRole, 
                   b'roleMessage': (getRoleMessage(vehicleRole)), 
                   b'vehicleCD': (vehicle.intCD), 
                   b'isElite': isEliteOrPremium})
            return
...
class _ComponentsVisibilityManager(object):
    ...
    def __perksPanelPredicate(self):
        return self.__arenaPeriod >= ARENA_PERIOD.BATTLE and self.__isBattleLoaded and avatar_getter.getPlayerTeam() == BigWorld.player().arena.vehicles[self.__controllingVehicleID][b'team']
