...
class VersusAIEntity(PreQueueEntity):
    ...
    def _makeQueueCtxByAction(self, action=None):
        ...
        return SpecialModeQueueCtx(self._queueType, invID, waitingID=b'prebattle/join')
    ...
    def doSelectAction(self, action):
        ...
        if name == PREBATTLE_ACTION_NAME.VERSUS_AI:
            return SelectResult(True)
        return super(VersusAIEntity, self).doSelectAction(action)

    def _doQueue(self, ctx):
        BigWorld.player().AccountVersusAIComponent.enqueueVersusAI(ctx)
        _logger.debug(b'Sends request on queuing to the VersusAI - %r', ctx)
        return

    def _doDequeue(self, ctx):
        BigWorld.player().AccountVersusAIComponent.dequeueVersusAI()
        _logger.debug(b'Sends request on dequeuing from the VersusAI')
        return
