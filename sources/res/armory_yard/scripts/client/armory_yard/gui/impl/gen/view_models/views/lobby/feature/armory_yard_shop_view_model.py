from enum import IntEnum
from frameworks.wulf import Array
from frameworks.wulf import ViewModel
from gui.impl.gen.view_models.common.price_model import PriceModel
from armory_yard.gui.impl.gen.view_models.views.lobby.feature.armory_yard_shop_item import ArmoryYardShopItem

class BackButtonState(IntEnum):
    ARMORY = 0
    INGAMESHOP = 1


class ArmoryYardShopViewModel(ViewModel):
    __slots__ = (b'onBack', b'onClose', b'onCloseIntro', b'onShowIntro', b'onBuyProduct')

    def __init__(self, properties=5, commands=5):
        super(ArmoryYardShopViewModel, self).__init__(properties=properties, commands=commands)
        return

    @property
    def tokenPrice(self):
        return self._getViewModel(0)

    @staticmethod
    def getTokenPriceType():
        return PriceModel

    def getIsIntroVisible(self):
        return self._getBool(1)

    def setIsIntroVisible(self, value):
        self._setBool(1, value)
        return

    def getCurrency(self):
        return self._getNumber(2)

    def setCurrency(self, value):
        self._setNumber(2, value)
        return

    def getBackButtonState(self):
        return BackButtonState(self._getNumber(3))

    def setBackButtonState(self, value):
        self._setNumber(3, value.value)
        return

    def getItems(self):
        return self._getArray(4)

    def setItems(self, value):
        self._setArray(4, value)
        return

    @staticmethod
    def getItemsType():
        return ArmoryYardShopItem

    def _initialize(self):
        super(ArmoryYardShopViewModel, self)._initialize()
        self._addViewModelProperty(b'tokenPrice', PriceModel())
        self._addBoolProperty(b'isIntroVisible', False)
        self._addNumberProperty(b'currency', 0)
        self._addNumberProperty(b'backButtonState')
        self._addArrayProperty(b'items', Array())
        self.onBack = self._addCommand(b'onBack')
        self.onClose = self._addCommand(b'onClose')
        self.onCloseIntro = self._addCommand(b'onCloseIntro')
        self.onShowIntro = self._addCommand(b'onShowIntro')
        self.onBuyProduct = self._addCommand(b'onBuyProduct')
        return
