...
class ArmoryYardShopController(IArmoryYardShopController, IGlobalListener):
    ...
    def conversionPrices(self):
        return self.__settings.get(b'conversionPrices', {})
    ...
    @property
    def isEnabled(self):
        return self.__settings.get(b'isEnabled', False)
    ...
    def __isCoinUpdate(self, diff):
        if ARMORY_YARD_COIN_NAME in diff.get(b'dynamicCurrencies', {}):
        ...
        if newLimits is None:
            ...
            for productId, pdataLimit in newLimits.iteritems():
                ...
                if product is not None:
                    if b'limit' in product and pdataLimit >= product[b'limit']:
                        del self.__products[productId]
                    else:
                        self.__products[productId][b'currCount'] = pdataLimit
    ...
    def __productUpdate(self, limits):
        self.__products = {}
        for productId, productParams in self.__settings.get(b'products', {}).iteritems():
            limit = limits.get(productId, 0)
            if b'limit' in productParams and limit >= productParams[b'limit']:
                continue
            self.__products[productId] = productParams
            self.__products[productId][b'currCount'] = limit
    ...
    def __subscribe(self):
        ...
        g_clientUpdateManager.addCallbacks({b'cache': (self.__isCoinUpdate), 
    ...
    def isBundle(self, productId):
        ...
        return productInfo and b'exclusiveVehicle' in productInfo
