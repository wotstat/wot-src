...
class ResourceWellListener(_NotificationListener):
    ...
    def __getSingleVehicleText(self):
        ...
        vehicleName = text_styles.crystal(vehicle.shortUserName if vehicle is not None else b'')
    ...
    def __getTwoVehiclesText(self):
        ...
        if len(rewardIDs) != 2:
            _logger.error(b'At parallel launch must be two vehicles, got %s', rewardIDs)
            return
        else:
            vehicles = [self.__resourceWell.getRewardVehicle(rewardID) for rewardID in rewardIDs]
            vehicleNames = [text_styles.crystal(vehicle.shortUserName if vehicle is not None else b'') for vehicle in vehicles]
    ...
    def __pushFinished(self):
        SystemMessages.pushMessage(text=backport.text(R.strings.messenger.serviceChannelMessages.resourceWell.end.text()), type=SM_TYPE.ResourceWellEnd, messageData={b'title': (backport.text(R.strings.messenger.serviceChannelMessages.resourceWell.end.title()))})
