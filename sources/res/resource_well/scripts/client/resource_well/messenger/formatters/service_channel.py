...
class ResourceWellRewardFormatter(WaitItemsSyncFormatter):
    ...
    def format(self, message, callback):
        ...
        if isSynced:
            rewardID = message.data.get(b'rewardID')
            if rewardID is None:
    ...
    def __getMainMessage(self, rewardID, message):
        ...
        formatted = g_settings.msgTemplates.format(self.__TEMPLATE, ctx={b'title': (backport.text(self.__R_MESSAGES.title(), vehicle=vehicleName)), 
           b'text': text, 
    ...
    def __getAdditionalMessage(self, rewardID, message):
        ...
        if rewardConfig is None:
            ...
            slots = rewardConfig.bonus.get(b'slots')
            if not slots:
                return MessageData(None, None)
...
class ResourceWellNoVehiclesFormatter(WaitItemsSyncFormatter):
    ...
    __BUTTON_TEMPLATE = b'ResourceWellNoVehiclesWithButtonMessage'
    ...
    def format(self, message, callback):
        ...
        if not isSynced or not self.__resourceWell.isEnabled():
            ...
            rewardID = message.data.get(b'rewardID')
            soldOutRewardIDs = message.data.get(b'soldOutRewardIDs', set())
            hasBalance = message.data.get(b'hasBalance', False)
            ...
            formatted = g_settings.msgTemplates.format(template, ctx={b'title': (backport.text(self.__R_MESSAGES.noVehicles.title())), 
               b'text': text})
            callback([
