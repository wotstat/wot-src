...
class ResourceWellVehiclePreview(VehiclePreview):
    ...
    def __init__(self, ctx):
        self.__numberStyle = ctx.get(b'numberStyle')
        self.__rewardID = ctx.get(b'rewardID')
    ...
    def _getExitEvent(self):
        ...
        ctx = exitEvent.ctx
        ctx.update({b'numberStyle': (self.__numberStyle), 
           b'rewardID': (self.__rewardID)})
        topPanelData = ctx.get(b'topPanelData', {})
        if topPanelData:
            ...
            if appearance is not None and appearance.outfit.style is not None:
                topPanelData.update({b'currentTabID': (TabID.PERSONAL_NUMBER_VEHICLE)})
                ctx.update({b'style': (self.__numberStyle)})
            else:
                topPanelData.update({b'currentTabID': (TabID.BASE_VEHICLE)})
                ctx.update({b'style': None})
        return exitEvent
