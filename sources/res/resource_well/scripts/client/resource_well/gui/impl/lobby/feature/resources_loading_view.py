...
class ResourcesLoadingView(ViewImpl):
    ...
    def createToolTipContent(self, event, contentID):
        if contentID == R.views.resource_well.mono.lobby.tooltips.progress_tooltip():
            return ProgressTooltip(progress=self.viewModel.getProgression(), diff=event.getArgument(b'progressDiff'))
        if contentID == R.views.resource_well.mono.lobby.tooltips.max_progress_tooltip():
            return MaxProgressTooltip(currentValue=event.getArgument(b'currentValue'), maxValue=event.getArgument(b'maxValue'), resourceType=event.getArgument(b'type'))
    ...
    def _getCallbacks(self):
        return (
         (
          b'stats.gold', self.__updateCurrencies),
         (
          b'stats.credits', self.__updateCurrencies),
         (
          b'stats.crystal', self.__updateCurrencies),
         (
          b'stats.freeXP', self.__updateCurrencies),
         (
          b'blueprints', self.__updateBlueprints))
    ...
    def __loadResources(self, args):
        ...
        if self.__isDestroyed():
            _logger.info(b'View is already destroyed.')
            return
        else:
    ...
    def __showNextSerialRewardConfirm(self, rewardID, resources):
        ...
        if self.__isDestroyed():
            _logger.info(b'View is already destroyed.')
            return
        if not result.success and processor.responseCtx.get(b'isUserCancelAction', False):
            self.destroyWindow()
            return
    ...
    def __onLoadResources(self, result, responseCtx, nextRewardID=None):
        if not result.success and result.userMsg == UNAVAILABLE_REWARD_ERROR:
            ...
            if result.success and result.auxData is not None and result.auxData.get(b'isRewardObtained', False):
                self.destroyWindow()
                return
            self.__updateLoadingError(isError=not result.success and not responseCtx.get(b'isUserCancelAction', False))
