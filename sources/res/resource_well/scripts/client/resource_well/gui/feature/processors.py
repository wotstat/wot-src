...
class ResourceWellTakeBackProcessor(Processor):
    ...
    def _request(self, callback):
        Waiting.show(b'getResourcesBack')
        resourceWellAccComponent = getattr(BigWorld.player(), b'ResourceWellAccountComponent', None)
        if resourceWellAccComponent is None:
            callback(makeError(b'Can not find the ResourceWellAccountComponent'))
            return
        else:
    ...
    def _successHandler(self, code, ctx=None):
        Waiting.hide(b'getResourcesBack')
