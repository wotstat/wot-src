...
class ResourceWellRewardHandler(ServiceChannelHandler):
    ...
    def _needToShowAward(self, ctx):
        if not super(ResourceWellRewardHandler, self)._needToShowAward(ctx):
            return False
        _, message = ctx
        if not message.data.get(b'rewardID', b''):
            _logger.error(b'RewardID is not found.')
            return False
        return True

    def _showAward(self, ctx):
        _, message = ctx
        showResourceWellAwardWindow(message.data[b'rewardID'], serialNumber=message.data.get(b'serialNumber', b''))
        return
