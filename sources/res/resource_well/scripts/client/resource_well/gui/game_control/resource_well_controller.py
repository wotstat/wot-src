...
class ResourceWellController(IResourceWellController, EventsHandler):
    ...
    def getRewardVehicle(self, rewardID):
        rewardConfig = self.config.getRewardConfig(rewardID)
        vehicleCD = first(rewardConfig.bonus.get(b'vehicles', {}))
        if vehicleCD is None:
            _logger.error(b'Vehicle is not found in config.')
            return
        else:
    ...
    def getRewardStyleID(self, rewardID):
        ...
        if not rewardConfig.isSerial:
            ...
            return first(itervalues(rewardConfig.bonus[b'vehicles']), {}).get(b'customization', {}).get(b'styleId')
    ...
    def isRewardCountAvailable(self, rewardID):
        ...
        if not requester:
            _logger.error(b'Requester not found. rewardID: %s', rewardID)
            return False
    ...
    def _getCallbacks(self):
        return (
         ...
         (
          b'inventory.1.compDescr', self.__onInventoryUpdated))
    ...
    def __onClientUpdated(self, diff, _):
        ...
        if b'initialAmounts' in diff[RESOURCE_WELL_PDATA_KEY]:
            self.__setNumberInitialValues()
        self.onEventUpdated()
        return
