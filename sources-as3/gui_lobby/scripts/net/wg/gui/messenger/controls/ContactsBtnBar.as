package net.wg.gui.messenger.controls
{
   ...
   public class ContactsBtnBar extends UIComponentEx
   {
      ...
      override protected function draw() : void
      {
         ...
         var _loc4_:uint = 0;
         super.draw();
         var _loc1_:Boolean = Boolean(isInvalid(OK_VISIBLE_INV));
         if(_loc1_)
         {
            this.btnOk.visible = this.isOkButtonVisible;
         }
         if(_loc1_ || Boolean(isInvalid(InvalidationType.SIZE)))
         {
            _loc2_ = 10;
            this.bg.width = _width;
            _loc3_ = this.isOkButtonVisible ? uint(this.btnOk.width + _loc2_) : 0;
            _loc4_ = uint(_width - _loc3_ - this.btnCancel.width >> 1);
            this.btnOk.x = _loc4_;
