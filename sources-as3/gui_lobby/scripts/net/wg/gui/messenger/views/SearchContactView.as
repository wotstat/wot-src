package net.wg.gui.messenger.views
{
   ...
   public class SearchContactView extends SearchContactViewMeta implements ISearchContactViewMeta
   {
      ...
      private var _searchDP:IDataProvider = new DAAPIDataProvider();
      ...
      override protected function draw() : void
      {
         ...
         {
            if(Boolean(isDAAPIInited) && Boolean(StringUtils.isNotEmpty(this._searchText)))
            {
               this._searchInitialized = true;
               searchS(this._searchText);
            }
         }
         var _loc1_:Boolean = Boolean(isInvalid(CHECK_ENABLING_INV));
         if(_loc1_)
         {
            _loc5_ = false;
            _loc6_ = int(this.list.selectedIndex);
            if(this._searchDP.length > 0 && _loc6_ != -1)
            {
               _loc7_ = this._searchDP.requestItemAt(_loc6_);
               if(Boolean(_loc7_))
               {
                  if(Boolean(_loc7_.userProps.tags))
                  {
                     _loc5_ = !(Boolean(UserTags.isCurrentPlayer(_loc7_.userProps.tags)) || Boolean(UserTags.isFriend(_loc7_.userProps.tags)));
                  }
                  else
                  {
                     _loc5_ = true;
         ...
         if(_loc1_ || _loc2_ || Boolean(isInvalid(INIT_INVALID)))
         {
         ...
         if(Boolean(isInvalid(LBL_INVALID)) && Boolean(this._extLabel))
         {
         ...
         var _loc4_:Boolean = Boolean(isInvalid(SEARCH_DISABLED_COOLDOWN_INV));
         if(_loc4_)
         {
            this.waitingIndicator.visible = _loc3_;
         }
         if(_loc4_ || Boolean(isInvalid(SEARCH_TEXT_VALIDATION_INV)))
         {
            if(_loc3_)
            {
...
class SearchViewInitData extends ExtContactsViewInitVO
{
   ...
   public function SearchViewInitData(param1:Object)
   {
      super(param1);
   }
}
