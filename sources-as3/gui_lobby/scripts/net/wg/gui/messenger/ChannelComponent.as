package net.wg.gui.messenger
{
   ...
   public class ChannelComponent extends ChannelComponentMeta implements IChannelComponent
   {
      ...
      private static const LINK_HOVER_STYLE:String = "a:hover {color:#FEFEEC;}";
      ...
      private var _isMessageAreaRMBClicked:Boolean = false;
      ...
      override protected function configUI() : void
      {
         ...
         this.messageArea.addEventListener(MouseEvent.CLICK,this.onMessageAreaClickHandler);
         ...
         _loc2_.parseCSS(LINK_HOVER_STYLE);
         this.messageArea.textField.styleSheet = _loc2_;
      ...
      override protected function onBeforeDispose() : void
      {
         ...
         this.messageArea.removeEventListener(MouseEvent.CLICK,this.onMessageAreaClickHandler);
         this.messageArea.dispose();
         if(Boolean(this.messageAreaScrollBar))
         {
      ...
      override protected function draw() : void
      {
         ...
         if(this._isJoined && Boolean(isInvalid(INVALIDATE_HISTORY)))
         {
            this.setHistory();
         }
         if(this._isReturnToNormalScale && Boolean(isInvalid(InvalidationType.SIZE)))
         {
      ...
      {
         if(Boolean(this._externalHintTF) && this._externalHintTF.visible)
         {
      ...
      {
         if(Boolean(initialized) && this._isJoined && this._isPopulated && this._isConfigured)
         {
            this.updateFocus();
         }
      }
      ...
      {
         if(this.messageInput != null && Boolean(this.messageInput.enabled) && this.messageInput.focused == 0)
         {
      ...
      {
         if(this._isMessageAreaRMBClicked)
         {
            onLinkClickS(param1.text);
         }
      }
      
      private function onMessageAreaClickHandler(param1:MouseEvent) : void
      {
         this._isMessageAreaRMBClicked = App.utils.commons.isRightButton(param1);
      }
