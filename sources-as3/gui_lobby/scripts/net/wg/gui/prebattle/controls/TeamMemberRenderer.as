package net.wg.gui.prebattle.controls
{
   ...
   public class TeamMemberRenderer extends TeamMemberRendererBase
   {
      ...
      override protected function showToolTips() : void
      {
         ...
         if(Boolean(model) && Boolean(model.vType))
         {
      ...
      {
         super.draw();
         if(Boolean(model) && Boolean(isInvalid(InvalidationType.SIZE)))
         {
      ...
      override protected function afterSetData() : void
      {
         ...
         if(_loc1_ != PrebattleStateString.UNKNOWN)
         {
            ...
            if(Boolean(this.status_icon.currentLabels.indexOf(_loc1_,0)))
            {
         ...
         if(Boolean(this.hasPermissionsIcon))
         {
      ...
      override protected function updateAfterStateChange() : void
      {
         ...
         if(Boolean(model) && model.isCreator)
         {
            this.commander_icon.visible = true;
            this.status_icon.visible = false;
         }
         ...
         var _loc1_:Number = Boolean(model) ? model.getCurrentColor() : Number.NaN;
         if(!isNaN(_loc1_))
         {
      ...
      private function updateNicknamePosX() : void
      {
         ...
         textField.x = _loc1_ ? NICK_NAME_EXTENDED_X : NICK_NAME_X;
         this.numberField.x = _loc1_ ? NUMBER_FIELD_EXTENDED_X : NUMBER_FIELD_X;
      ...
      {
         if(Boolean(model) && isNaN(model.accID))
         {
            return;
         }
