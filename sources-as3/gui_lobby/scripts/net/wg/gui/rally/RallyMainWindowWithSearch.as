package net.wg.gui.rally
{
   ...
   public class RallyMainWindowWithSearch extends RallyMainWindowWithSearchMeta implements IRallyMainWindowWithSearchMeta
   {
      ...
      {
         if(Boolean(isInvalid(UPDATE_AUTO_SEARCH_MODEL)) && Boolean(this._autoSearchModel))
         {
            this.autoSearch.setData(this._autoSearchModel);
            this.autoSearch.visible = true;
         }
         if(Boolean(isInvalid(UPDATE_AUTO_SEARCH_MAIN_LABEL)) && this._mainLabel != Values.EMPTY_STR)
         {
            this.autoSearch.setMainLabel(this._mainLabel);
         }
         if(Boolean(isInvalid(UPDATE_AUTO_SEARCH_TIME_DIRECTION)) && this._timeDirection != 0)
         {
            this.autoSearch.setTimeDirection(this._timeDirection);
         }
         if(Boolean(isInvalid(UPDATE_AUTO_SEARCH_COUNT_DOWN_SECONDS)) && this._countDownSeconds >= 0)
         {
            this.autoSearch.setCountDownSeconds(this._countDownSeconds);
         }
         super.draw();
         if(!this._updatedBtns && Boolean(isInvalid(UPDATE_AUTO_SEARCH_BUTTONS)) && Boolean(this._autoSearchModel))
         {
      ...
      {
         if(Boolean(hasFocus) && !this.autoSearch.hasForm)
         {
