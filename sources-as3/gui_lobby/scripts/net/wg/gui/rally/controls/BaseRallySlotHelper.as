package net.wg.gui.rally.controls
{
   ...
   public class BaseRallySlotHelper extends SlotRendererHelper
   {
      ...
      override public function initControlsState(param1:IRallySimpleSlotRenderer) : void
      {
         ...
         if(Boolean(_loc2_.takePlaceBtn))
         {
            _loc2_.takePlaceBtn.visible = false;
         }
         if(Boolean(_loc2_.takePlaceFirstTimeBtn))
         {
      ...
      override public function updateComponents(param1:IRallySimpleSlotRenderer, param2:IRallySlotVO) : void
      {
         ...
         if(Boolean(param2))
         {
            _loc5_ = Boolean(param2.player) ? Boolean(UserTags.isCurrentPlayer(param2.player.tags)) : false;
            _loc4_ = _loc3_.index > 0 && param2.canBeTaken && !param2.isClosed && !param2.isFreezed && !param2.isCommanderState;
            if(Boolean(_loc3_.takePlaceFirstTimeBtn))
            {
               _loc3_.takePlaceFirstTimeBtn.visible = _loc4_ && !param2.isCurrentUserInSlot;
            }
            if(Boolean(_loc3_.takePlaceBtn))
            {
               _loc3_.takePlaceBtn.visible = _loc4_ && param2.isCurrentUserInSlot;
            }
            if(Boolean(_loc3_.contextMenuArea))
            {
               _loc3_.contextMenuArea.visible = Boolean(param2) && Boolean(param2.player) && !_loc5_;
            }
            if(!param2.isClosed)
            {
               _loc3_.vehicleBtn.visible = true;
               if(Boolean(param2.selectedVehicle))
               {
               ...
               _loc3_.vehicleBtn.selectState(!param2.selectedVehicle && Boolean(param2.player) && _loc5_,chooseVehicleText);
               if(Boolean(param2.player))
               {
                  ...
                  this.isAlertIconVisible = _loc5_ && Boolean(param2.selectedVehicle) && !param2.selectedVehicle.isReadyToFight;
               }
            }
            else
            {
