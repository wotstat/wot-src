package net.wg.gui.components.controls
{
   ...
   public class MainMenuButton extends SoundButtonEx
   {
      ...
      private var _iconY:int = MainMenuButtonSize.ICON_Y[MainMenuButtonSize.REGULAR];
      ...
      private var _commonsUtils:ICommons = App.utils.commons;
      ...
      override protected function calculateWidth() : Number
      {
         ...
         var _loc1_:Number = Number(actualWidth);
         if(!constraintsDisabled && Boolean(initialized))
         {
      ...
      override protected function draw() : void
      {
         ...
         var _loc2_:int = 0;
         super.draw();
         if(Boolean(this.fxTextField1) && Boolean(isInvalid(InvalidationType.STATE,INVALIDATE_CAPS,INVALIDATE_PADDING,INVALIDATE_TEXT_COLOR,INVALIDATE_TEXT_SIZE)))
         {
            if(Boolean(this._textColorOver) && Boolean(!selected) && state == ComponentState.OVER)
            {
               this.fxTextField1.textColor = this._textColorOver;
            }
            else if(Boolean(_textColor) && enabled)
            {
               this.fxTextField1.textColor = _textColor;
            }
            if(Boolean(this._iconType))
            {
      ...
      {
         if(this._isTooltipSpecial && Boolean(_tooltip))
         {
            this._tooltipMgr.showSpecial(_tooltip,null);
         }
         if(this._isWulfTooltip && Boolean(_tooltip))
         {
      ...
      {
         if(Boolean(data) && data.value == BROWSER)
         {
      ...
      {
         if(Boolean(selected) || !enabled)
         {
            filters = null;
            this._isBlinking = false;
            return;
         }
         if(Boolean(this.fxTextField1))
         {
      ...
      {
         return Boolean(this.fxTextField1) ? this.fxTextField1.x + this.fxTextField1.textWidth + TEXTFIELD_PADDING + (_paddingHorizontal << 1) : 0;
      }
      ...
      [Inspectable(name="caps",type="Boolean",defaultValue="true")]
      ...
      public function set iconType(param1:String) : void
      {
         ...
         if(Boolean(this.fxTextField1) && Boolean(StringUtils.isNotEmpty(this._iconType)))
         {
            this.fxTextField1.x = this._iconOffset;
         }
         else
      ...
      {
         if(Boolean(data) && data.value == Aliases.BROWSER)
         {
            App.utils.scheduler.cancelTask(this.changeEffectState);
            filters = null;
         }
