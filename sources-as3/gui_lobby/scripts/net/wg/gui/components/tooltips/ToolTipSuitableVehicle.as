package net.wg.gui.components.tooltips
{
   ...
   public class ToolTipSuitableVehicle extends ToolTipSpecial
   {
      ...
      override protected function onDispose() : void
      {
         ...
         if(Boolean(this._conditionBtnList))
         {
      ...
      override protected function redraw() : void
      {
         ...
         if(Boolean(_loc2_.conditions))
         {
            ...
            {
               _loc7_ = int(this._conditionBtnList.length);
               _loc8_ = uint(_loc2_.conditions.length);
               while(_loc7_++ < _loc8_)
               {
                  this._conditionBtnList.push(this.createVehicleButton());
               }
            }
            _loc5_ = uint(_loc2_.conditions.length);
            _loc4_ = 0;
            while(_loc4_ < _loc5_)
            {
               if(Boolean(_loc2_.conditions[_loc4_]))
               {
         ...
         if(Boolean(!_loc2_.isCreator) && Boolean(_loc2_.vehiclesList) && _loc2_.vehiclesList.length > 0)
         {
            _loc3_ = Utils.instance.createSeparate(content);
            _loc3_.y = topPosition ^ 0;
            separators.push(_loc3_);
            ...
            _loc5_ = uint(_loc2_.vehiclesList.length);
            _loc4_ = 0;
            while(_loc4_ < _loc5_)
            {
      ...
      private function addCondition(param1:MovieClip, param2:Object, param3:Number, param4:CSVehicleButton) : Number
      {
         ...
         if(param2.hasOwnProperty("vehicle") && Boolean(param2["vehicle"]))
         {
            _loc5_ = true;
            _loc6_ = true;
         }
         else if(Boolean(param2.hasOwnProperty("nationIDRange")) && Boolean(param2["nationIDRange"]) || Boolean(param2.hasOwnProperty("vLevelRange")) && Boolean(param2["vLevelRange"]) || Boolean(param2.hasOwnProperty("vTypeRange")) && Boolean(param2["vTypeRange"]))
         {
            _loc5_ = true;
         }
         if(_loc5_)
      ...
      private function addSuitableVehicleBlockItem(param1:VehicleVO, param2:Number) : Number
      {
         ...
         return param2 + (_loc3_.height + this.MARGIN_BEETWEEN_BLOCKS);
      }
   }
}
