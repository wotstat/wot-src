package net.wg.gui.components.advanced
{
   ...
   public class SortableHeaderButtonBar extends ButtonBarEx
   {
      ...
      override protected function draw() : void
      {
         ...
         super.draw();
         if(Boolean(this._lastChangedButton) && Boolean(isInvalid(BTN_ENABLING_CHANGED)))
         {
            getButtonAt(this._lastChangedButton.index).enabled = this._lastChangedButton.enabled;
         }
         if(Boolean(StringUtils.isNotEmpty(this._sortDirection)) && Boolean(isInvalid(SORT_DIRECTION_INVALID)))
         {
      ...
      public function updateButtonsEnabledState(param1:Boolean) : void
      {
         ...
         {
            _loc2_.enabled = param1 && Boolean(SortingButtonVO(_loc2_.data).enabled);
         }
      }
...
class BtnEnablingData
{
   ...
   public function BtnEnablingData(param1:int, param2:Boolean)
   {
      super();
      this.index = param1;
      this.enabled = param2;
