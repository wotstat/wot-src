package net.wg.gui.components.advanced.tutorial
{
   ...
   public class TutorialHint extends UIComponentEx
   {
      ...
      override protected function draw() : void
      {
         ...
         {
            if(Boolean(isInvalid(InvalidationType.DATA)) || Boolean(isInvalid(InvalidationType.SIZE)))
            {
      ...
      private function layoutTextAndBg() : void
      {
         ...
         var _loc2_:int = this.hintText.width + (TEXT_PADDING << 1) > HINT_BG_MIN_WIDTH ? int(this.hintText.width + (TEXT_PADDING << 1)) : HINT_BG_MIN_WIDTH;
         var _loc3_:int = int(this.hintBG.height);
