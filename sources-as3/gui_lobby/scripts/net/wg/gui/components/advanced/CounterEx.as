package net.wg.gui.components.advanced
{
   ...
   public class CounterEx extends UIComponentEx
   {
      ...
      private var _patternSpaces:RegExp = / /g;
      ...
      {
         if(Boolean(this.font != Values.EMPTY_STR && this.size > 0) && Boolean(this.color) && this.speed > 0)
         {
      ...
      private function createLines() : void
      {
         ...
         var _loc6_:int = int(this._tfMetrics.lines);
         if(Boolean(this._endingChar.length))
         {
         ...
         while(_loc7_ < _loc6_)
         {
            ...
            if(_loc7_ > 0 && _loc7_ % 3 == 0 && Boolean(StringUtils.isNotEmpty(this.localizationSymbol)))
            {
               _loc4_ = this.createTextLine();
      ...
      private function setLinesPosition(param1:Number) : void
      {
         ...
         var _loc2_:int = int(this.lines.length);
         var _loc4_:int = 0;
         while(_loc4_ < _loc2_)
         {
            ...
            if(Boolean(!this.checkTruePosition[_loc4_].stand && this.checkTruePosition[_loc4_ + 1].stand) && Boolean(_loc3_) || param1 == Math.abs(this.number))
            {
            ...
            {
               this.lines[_loc4_].gotoAndStop(START_FRAME + ANIMATE_STEP_FRAMES * int((param1 / Math.pow(10,_loc4_) ^ 0) % 10));
            }
            _loc4_++;
         }
      }
