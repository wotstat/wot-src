package net.wg.gui.components.carousels
{
   ...
   public class CarouselBase extends CoreList implements IDraggable
   {
      ...
      private var _padding:Padding = new Padding(0);
      ...
      {
         super.draw();
         if(!_usingExternalRenderers && Boolean(isInvalid(InvalidationType.SIZE)) && this.useShadow)
         {
            this.setChildIndex(this.shadow,this.numChildren - 1);
         }
         if(Boolean(isInvalid(INIT_CAROUSEL)) && dataProvider != null && dataProvider.length > 0)
         {
      ...
      {
         param1.x = param2 == 0 ? Number(this._padding.left) : param3 + (this.slotImageWidth + param4);
      ...
      {
         if(Boolean(this.leftArrow) && Boolean(this.leftArrow.visible))
         {
      ...
      {
         var _loc2_:int = Boolean(_renderers) ? int(_renderers.length) : 0;
         var _loc3_:int = 0;
         while(_loc3_ < _loc2_)
         {
      ...
      private function startSlideByArrow(param1:String, param2:Boolean) : void
      {
         ...
         if(!this._skippCall)
         {
            ...
            if(!this._arrowSlideScheduled && Boolean(container))
            {
               this._slideSelfAcceleratorDynamic = SLIDE_SELF_ACCELERATOR;
