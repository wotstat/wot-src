package net.wg.gui.bootcamp.battleResult
{
   ...
   public class BCBattleResult extends BCBattleResultMeta implements IBCBattleResultMeta
   {
      ...
      public static const ALTERNATIVE:String = "_alt";
      
      public static const STAGE_SMALL_SIZE:int = 1070;
      ...
      private static const RESULT_ALTERNATIVE_MARGIN_Y:int = 22;
      
      private static const RESULT_ALTERNATIVE_FHD_MARGIN_Y:int = 63;
      ...
      private var _cursor:ICursorManager = App.cursor;
      ...
      override protected function configUI() : void
      {
         ...
         this.btnHangar.addEventListener(MouseEvent.ROLL_OUT,this.onBtnHangarRollOutHandler);
      ...
      override protected function draw() : void
      {
         ...
         if(Boolean(this._stageDimensions) && Boolean(isInvalid(InvalidationType.LAYOUT)))
         {
            this.validateLayout();
         }
      }
      ...
      override protected function onDispose() : void
      {
         ...
         this.btnHangar.removeEventListener(MouseEvent.ROLL_OUT,this.onBtnHangarRollOutHandler);
      ...
      {
         if(Boolean(this._data) && this._data.alternativeLayout)
         {
            return (this.isFHD ? BIG : SMALL) + ALTERNATIVE;
         }
         return this._stageDimensions.y <= STAGE_SMALL_SIZE ? SMALL : BIG;
      }
      
      private function get isFHD() : Boolean
      {
         return this._stageDimensions.x >= StageSizeBoundaries.WIDTH_1920 && this._stageDimensions.y >= StageSizeBoundaries.HEIGHT_1080;
      }
      ...
      private function updateBg() : void
      {
         var _loc1_:int = 0;
         var _loc2_:int = 0;
         if(Boolean(this._stageDimensions) && Boolean(this._bgProportion))
         {
      ...
      private function validateLayout() : void
      {
         ...
         this.result.gotoAndStop(_loc1_);
         if(_baseDisposed)
         {
            return;
         }
         this.playerStatus.gotoAndStop(_loc1_);
         if(_baseDisposed)
         {
            return;
         }
         ...
         this.content.x = this.bottomGlowMC.x = _loc2_ >> 1;
         this.btnHangar.x = _loc2_ - this.btnHangar.width >> 1;
         this.btnAnimation.x = this.btnHangar.x + (this.btnHangar.width >> 1);
         this.playerStatus.x = _loc2_ - this.playerStatus.width - PLAYER_STATUS_MARGIN_X >> 0;
         var _loc4_:int = Math.max(_loc3_ * CONTENT_MARGIN_Y_SCALE,CONTENT_MIN_MARGIN_Y);
         this.content.y = _loc3_ - _loc4_ - this.content.tapeYPos >> 0;
         if(this._stageDimensions.y <= BCBattleResult.STAGE_SMALL_SIZE)
         {
            ...
            this.btnHangar.y = _loc3_ - this.btnHangar.height + BUTTON_MARGIN_SMALL_Y >> 0;
         }
         else
         {
            ...
            this.btnHangar.y = _loc3_ - this.btnHangar.height + BUTTON_MARGIN_Y >> 0;
         }
         ...
         if(Boolean(this._data) && this._data.alternativeLayout)
         {
            if(this.isFHD)
            {
               this.result.y += RESULT_ALTERNATIVE_FHD_MARGIN_Y;
            }
            else
            {
               this.result.y += RESULT_ALTERNATIVE_MARGIN_Y;
            }
         }
         this.playerStatus.y = _loc3_ - this.playerStatus.height - PLAYER_STATUS_MARGIN_Y >> 0;
         this.btnAnimation.y = this.btnHangar.y + (this.btnHangar.height >> 1) >> 0;
         this.bottomGlowMC.y = _loc3_;
      ...
      private function onBtnHangarRollOutHandler(param1:MouseEvent) : void
      {
         this._cursor.forceSetCursor(Cursors.ARROW);
      }
