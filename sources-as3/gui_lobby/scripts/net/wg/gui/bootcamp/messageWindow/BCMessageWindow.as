package net.wg.gui.bootcamp.messageWindow
{
   ...
   public class BCMessageWindow extends BCMessageWindowMeta implements IBCMessageWindowMeta
   {
      ...
      {
         super.draw();
         if(Boolean(this._messagesQueue) && Boolean(isInvalid(InvalidationType.DATA)))
         {
            this.createMessage();
         }
         if(Boolean(this._contentRenderer) && Boolean(isInvalid(STAGE_RESIZED)))
         {
      ...
      private function clearBlurWindows() : void
      {
         ...
         if(Boolean(this._blurWindows))
         {
            while(Boolean(this._blurWindows.length))
            {
