package net.wg.gui.lobby.window
{
   ...
   public class MissionAwardWindow extends MissionAwardWindowMeta implements IMissionAwardWindowMeta
   {
         ...
         if(this._dataVO != null)
         {
            if(isInvalid(InvalidationType.DATA))
            {
               ...
               while(_loc3_ < QUEST_CONDITION_COUNTS)
               {
                  ...
                  _loc2_.visible = Boolean(_loc4_) && _loc4_.length > _loc3_;
                  if(_loc2_.visible)
                  {
            ...
            if(Boolean(isInvalid(UPDATE_POSITION_INV)) && this.isImageIsLoaded)
            {
               this.updatePosition();
            }
         }
      ...
      {
         var _loc1_:int = int(this._dataVO.awards.length);
      ...
      private function updateVisibleComponents() : void
      {
         ...
         this.conditionsBlock.visible = !_loc1_ && Boolean(this._dataVO.conditions);
