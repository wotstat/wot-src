package net.wg.gui.lobby.window
{
   ...
   public class ConfirmItemWindow extends ConfirmItemWindowMeta implements IConfirmItemWindowMeta
   {
      ...
      {
         super.draw();
         if(Boolean(isInvalid(SETTINGS_INVALID)) && window != null && this._settingsVO != null)
         {
      ...
      protected function applyData() : void
      {
         ...
         var _loc4_:int = int(_loc3_.getPriceValueByName(_loc2_));
         var _loc5_:int = NORMAL_COLOR;
         var _loc6_:Boolean = false;
         if(this.data.hasSeveralPrices)
         {
            _loc7_ = _loc1_.selectedIndex != -1 ? int(_loc1_.selectedIndex) : 1;
      ...
      private function applyCurrency() : void
      {
         ...
         var _loc2_:String = this.data.currency;
         var _loc3_:int = int(Currencies.INDEX_FROM_NAME[_loc2_]);
         var _loc4_:uint = this.data.stepSize;
         if(this.data.hasSeveralPrices)
         {
            _loc6_ = int(content.dropdownMenu.selectedIndex);
            _loc7_ = this.data.price.items[_loc6_];
            _loc3_ = int(Currencies.INDEX_FROM_NAME[_loc7_.name]);
         }
      ...
      private function applyResult() : void
      {
         ...
         var _loc7_:int = int(Currencies.INDEX_FROM_NAME[_loc6_]);
         _loc9_ = this.data.price;
         var _loc10_:Vector.<PriceVO> = _loc9_.items;
         if(this.data.hasSeveralPrices)
         {
            _loc12_ = int(content.dropdownMenu.selectedIndex);
            _loc13_ = _loc10_[_loc12_];
            this._currency = _loc13_.name;
            _loc7_ = int(Currencies.INDEX_FROM_NAME[this._currency]);
            if(_loc7_ != CURRENCIES_CONSTANTS.GOLD_INDEX)
            {
               _loc3_ = ZERO_STRING;
               _loc8_ = int(_loc9_.getPriceValueByName(this._currency));
