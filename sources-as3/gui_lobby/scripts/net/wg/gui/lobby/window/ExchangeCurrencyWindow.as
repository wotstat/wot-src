   ...
   public class ExchangeCurrencyWindow extends BaseExchangeWindow implements IExchangeWindowMeta
   {
      ...
      override protected function draw() : void
      {
         ...
         if(isInvalid(SET_WALLET_STATUS_INVALID))
         {
            ...
            if(Boolean(this.onHandHaveNotGold) && Boolean(this.resultHaveNotGold))
            {
               _loc3_ = uint(_loc2_.goldStatus);
               _loc1_ = !this.onHandHaveNotGold.updateStatus(_loc3_);
               this.resultHaveNotGold.updateStatus(_loc3_);
            }
            if(Boolean(this.onHandPrimaryCurrencyText))
            {
               this.onHandPrimaryCurrencyText.visible = _loc1_;
            }
            if(Boolean(this.resultPrimaryCurrencyText))
            {
               this.resultPrimaryCurrencyText.visible = _loc1_;
            }
            if(Boolean(this.onHandHaveNotCredits) && Boolean(this.resultHaveNotCredits))
            {
               _loc4_ = uint(_loc2_.creditsStatus);
               _loc5_ = Boolean(this.onHandHaveNotCredits.updateStatus(_loc4_));
            ...
            if(Boolean(this.onHandSecondaryCurrencyText))
            {
               this.onHandSecondaryCurrencyText.visible = _loc1_;
            }
            if(Boolean(this.resultSecondaryCurrencyText))
            {
