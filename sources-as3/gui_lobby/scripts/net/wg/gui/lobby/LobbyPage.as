package net.wg.gui.lobby
{
   ...
   public class LobbyPage extends LobbyPageMeta implements ILobbyPage
   {
      ...
      override public function updateStage(param1:Number, param2:Number) : void
      {
         ...
         var _loc4_:int = this.messengerBar.visible ? MessengerBar.BAR_VISIBLE_HEIGHT : 0;
         var _loc5_:int = this.header.visible ? int(TOP_SUB_VIEW_POSITION) : 0;
         ...
         for each(_loc8_ in _loc3_)
         {
            if(Boolean(_loc8_))
            {
               _loc7_ = this._ignoreTopOffset && _loc8_ == this.subViewContainer && Boolean(this.header.visible);
               _loc8_.y = _loc7_ ? 0 : _loc5_;
               _loc8_.updateStage(param1,_loc7_ ? _loc6_ + _loc5_ : _loc6_);
            }
         }
