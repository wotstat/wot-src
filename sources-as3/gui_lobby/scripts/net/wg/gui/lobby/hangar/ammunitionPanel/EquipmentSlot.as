package net.wg.gui.lobby.hangar.ammunitionPanel
{
   ...
   public class EquipmentSlot extends DeviceSlot
   {
      ...
      {
         super.configUI();
         if(Boolean(this.equipSlotOverlay))
         {
      ...
      override protected function draw() : void
      {
         ...
         var _loc10_:int = 0;
         super.draw();
         if(slotData != null && Boolean(isInvalid(InvalidationType.DATA)))
         {
            _loc1_ = isEmpty();
            ...
            if(_loc1_ || Boolean(StringUtils.isEmpty(_loc5_)))
            {
            ...
            if(!_loc4_ && Boolean(StringUtils.isEmpty(this.alertIcon.source)))
            {
               this.alertIcon.source = RES_ICONS.MAPS_ICONS_LIBRARY_ICON_ALERT_32X32;
            }
            _loc6_ = slotData.bgHighlightType;
            _loc7_ = Boolean(StringUtils.isNotEmpty(_loc6_));
            ...
            _loc9_ = Boolean(StringUtils.isNotEmpty(_loc8_));
