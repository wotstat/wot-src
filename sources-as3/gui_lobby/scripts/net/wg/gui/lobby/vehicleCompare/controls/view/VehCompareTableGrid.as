package net.wg.gui.lobby.vehicleCompare.controls.view
{
   ...
   public class VehCompareTableGrid extends UIComponentEx implements IDragDelegate, IDragDropHitArea
   {
      ...
      private var _renderers:Vector.<ITableGridLine> = new Vector.<ITableGridLine>(0);
      
      private var _selectedRenderers:Vector.<ITableGridLine> = new Vector.<ITableGridLine>(0);
      
      private var _bottomRenderer:ITableGridLine = null;
      
      private var _highlightedRenderer:ITableGridLine = null;
      
      private var _rowsSelected:Dictionary = new Dictionary();
      ...
      override protected function draw() : void
      {
         ...
         var _loc6_:int = 0;
         super.draw();
         if(Boolean(this._data) && Boolean(isInvalid(INV_UPDATE_SELECTED_ROWS)))
         {
         ...
         if(!isNaN(this._nextStageYToSelect) && Boolean(isInvalid(INV_SELECT_STAGE_Y)))
         {
      ...
      {
         if(Boolean(this._mouseDownPos) && Boolean(Math.abs(this._mouseDownPos.x - param1.stageX) <= CLICK_MOUSE_RADIUS) && Math.abs(this._mouseDownPos.y - param1.stageY) <= CLICK_MOUSE_RADIUS)
         {
