package net.wg.gui.lobby.vehicleCompare.configurator
{
   ...
   public class VehConfModulesButton extends SoundButtonEx
   {
      ...
      private static const MODULE_HEIGHT:int = 76;
      ...
      {
         this.applyBgAlpha(Boolean(focused) ? BG_ROLL_OVER_ALPHA : BG_ROLL_OUT_ALPHA);
      }
      ...
      {
         var _loc2_:VehConfModuleSlot = null;
         var _loc3_:DeviceSlotVO = null;
         var _loc4_:UIComponentEx = null;
         if(Boolean(this._dataVOs) && Boolean(isInvalid(InvalidationType.DATA)))
         {
            for each(_loc3_ in this._dataVOs)
            {
               _loc2_ = VehConfModuleSlot(this[_loc3_.slotType]);
               if(Boolean(_loc2_))
               {
         ...
         if(_loc1_)
         {
            for each(_loc4_ in this._modules)
            {
               _loc4_.y = this.bg.height - MODULE_HEIGHT >> 1;
               _loc4_.scaleY = 1 / actualScaleY;
            }
         }
      }
