package net.wg.gui.lobby.vehicleCompare
{
   ...
   public class VehicleModulesTree extends ModulesTree
   {
      ...
      private static const DEFAULT_LINE_THICKNESS:int = 1;
      
      private static const MAX_NODES_ON_LEVEL:int = 6;
      
      private static const ERROR_MSG:String = "VehicleModulesTree. Wrong index: ";
      ...
      override protected function initialize() : void
      {
         ...
         rGraphics.lineThickness = DEFAULT_LINE_THICKNESS;
         maxNodesOnLevel = MAX_NODES_ON_LEVEL;
      }
      ...
      public function setNodesStates(param1:Array) : void
      {
         ...
         while(_loc8_ < _loc2_)
         {
            ...
            _loc5_ = null;
            _loc4_ = false;
            _loc7_ = int(_dataProvider.getIndexByID(_loc6_));
            if(_loc7_ > -1 && _dataProvider.length > _loc7_)
            {
               _loc4_ = Boolean(_dataProvider.setState(_loc7_,0,_loc3_[1]));
               _loc5_ = getNodeByID(_loc6_);
            }
            else
            {
               App.utils.asserter.assert(false,ERROR_MSG + _loc7_);
            }
            if(_loc4_)
            {
