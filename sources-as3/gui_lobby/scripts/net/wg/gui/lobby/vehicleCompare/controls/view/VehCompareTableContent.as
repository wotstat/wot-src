package net.wg.gui.lobby.vehicleCompare.controls.view
{
   ...
   public class VehCompareTableContent extends UIComponentEx
   {
      ...
      override protected function draw() : void
      {
         ...
         var _loc5_:int = 0;
         super.draw();
         if(Boolean(this._carouselDataProvider) && Boolean(isInvalid(VEH_PARAMS_DATA_INV)))
         {
            this.scrollList.dataProvider = this._carouselDataProvider;
         }
         if(Boolean(this._vehParamsInfoDataProvider) && Boolean(isInvalid(VEH_PARAMS_INFO_INV)))
         {
         ...
         if(Boolean(this._paramsDeltaData) && Boolean(isInvalid(VEH_PARAMS_DELTA_DATA_INV)))
         {
            this._paramsDeltaData.paramIndex = -1;
            _loc1_ = int(this._vehParamsInfoDataProvider.length);
            _loc2_ = 0;
            while(_loc2_ != _loc1_)
            {
         ...
         if(Boolean(this._vehParamsInfoDataProvider) && Boolean(isInvalid(InvalidationType.SIZE)))
         {
      ...
      {
         if(Boolean(this._carouselDataProvider))
         {
