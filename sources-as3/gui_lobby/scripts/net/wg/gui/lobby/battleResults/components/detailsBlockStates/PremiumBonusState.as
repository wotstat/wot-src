      ...
      override protected function draw() : void
      {
         ...
         if(this._data != null)
         {
            if(isInvalid(InvalidationType.DATA))
            {
               ...
               _loc1_ = Boolean(StringUtils.isNotEmpty(this._data.xpValue));
            ...
            {
               this._commons.updateTextFieldSize(this.description,false,true);
               ...
               if(this.xpValue.visible)
               {
                  ...
                  _loc2_ += this.xpValue.height + XP_VALUE_OFFSET | 0;
               }
               if(this.statusBonusLabel.visible)
               {
                  _loc2_ += this.statusBonusLabel.height + STATUS_BONUS_LABEL_OFFSET | 0;
               }
               ...
               _loc3_ = Boolean(StringUtils.isNotEmpty(this._data.statusBonusTooltip));
               this.infoIcon.visible = _loc3_;
               this.statusBonusLabel.x = this.description.x - (_loc3_ ? STATUS_LABEL_NO_ICON_OFFSET : STATUS_LABEL_ICON_OFFSET);
               this.statusBonusLabel.width = backgroundIcon.width - this.statusBonusLabel.x - (_loc3_ ? this.infoIcon.width : 0);
               if(_loc3_)
               {
                  _loc4_ = this.statusBonusLabel.length;
                  _loc5_ = this.statusBonusLabel.getCharBoundaries(_loc4_ - 1);
                  if(_loc5_ != null)
                  {
                     this.infoIcon.x = this.statusBonusLabel.x + _loc5_.x + _loc5_.width | 0;
                     this.infoIcon.y = this.statusBonusLabel.y + _loc5_.y - INFO_ICON_OFFSET | 0;
                  }
               }
            }
         }
