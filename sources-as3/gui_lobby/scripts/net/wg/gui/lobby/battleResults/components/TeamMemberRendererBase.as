package net.wg.gui.lobby.battleResults.components
{
   ...
   public class TeamMemberRendererBase extends SoundListItemRenderer
   {
      ...
      {
         super.draw();
         if(this._isDataChanged && Boolean(isInvalid(InvalidationType.DATA)))
         {
         ...
         if(Boolean(this._isCommonStatsVOChanged) && Boolean(this._commonStatsVO) && Boolean(isInvalid(INVALIDATE_COMMON_DATA)))
         {
      ...
      protected function getColorForAlias(param1:String, param2:Number) : Number
      {
         ...
         try
         {
            result = Number(App.colorSchemeMgr.getRGB(alias));
            if(!result)
            {
               result = defaultColor;
            }
      ...
      {
         return Boolean(this._commonStatsVO) ? this._commonStatsVO.wasInBattle : false;
      }
      
      override protected function handleMouseRelease(param1:MouseEvent) : void
      {
         var _loc2_:Object = null;
         if(Boolean(App.utils.commons.isRightButton(param1)) && Boolean(this._vo))
         {
            _loc2_ = {
