      ...
      private static function onVehicleBtnRollOverHandler(param1:RallySimpleSlotRenderer, param2:IRallySlotVO, param3:* = null) : void
      {
         ...
         switch(param1.vehicleBtn.currentState)
         {
            ...
            case CSVehicleButton.SELECTED_VEHICLE:
               if(Boolean(param3) && param3.type == CSVehicleButton.ALERT_DATA_TYPE)
               {
      ...
      override public function updateComponents(param1:IRallySimpleSlotRenderer, param2:IRallySlotVO) : void
      {
         ...
         if(param1 is SortieSlot)
         {
            ...
            if(!_loc8_.isClosed)
            {
               ...
               {
                  _loc9_ = Boolean(_loc8_.player) && Boolean(UserTags.isCurrentPlayer(_loc8_.player.tags));
            ...
            _loc7_.vehBrokenIcon.visible = false;
            _loc7_.unfrozeVehicleBtn.visible = false;
            if(_loc8_.selectedVehicle != null && _loc8_.selectedVehicle.state == FROZEN_VEHICLE_STATE && !isAlertIconVisible)
            {
               if(_loc8_.canUnfrozeVehicles)
               {
                  _loc7_.unfrozeVehicleBtn.visible = true;
               }
               else
               {
                  _loc7_.vehBrokenIcon.visible = _loc8_.hasFrozenVehicle;
               }
               _loc7_.unfrozeVehicleBtn.enabled = _loc8_.unfrozeVehiclesBtnEnabled;
            }
         }
         if(!_loc3_)
         {
