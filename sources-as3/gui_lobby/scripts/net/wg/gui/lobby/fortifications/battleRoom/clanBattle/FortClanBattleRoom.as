package net.wg.gui.lobby.fortifications.battleRoom.clanBattle
{
   ...
   public class FortClanBattleRoom extends FortClanBattleRoomMeta implements IFortClanBattleRoomMeta
   {
      ...
      override protected function onChooseVehicleRequest(param1:RallyViewsEvent) : void
      {
         ...
         if(Boolean(_loc2_.slotData.selectedVehicle))
         {
      ...
      override protected function configUI() : void
      {
         ...
         if(Boolean(this.btnFreezeJournal))
         {
      ...
      {
         super.draw();
         this._isCommander = Boolean(rallyData) ? rallyData.isCommander : false;
         this._canInvite = Boolean(rallyData) ? rallyData.canInvite : false;
         this._canUnfreeze = Boolean(rallyData) ? rallyData.canUnfreeze : false;
         ...
         {
            if(Boolean(this._configureButtonData))
            {
         ...
         if(teamSection.actionButtonData != null && Boolean(isInvalid(RallyInvalidationType.ACTION_BUTTON_DATA)))
         {
      ...
      override protected function onDispose() : void
      {
         ...
         if(Boolean(this.waitingListButtonBar))
         {
            this.waitingListButtonBar.dispose();
            this.waitingListButtonBar = null;
         }
         if(Boolean(this.teamButtonBar))
         {
      ...
      {
         var _loc1_:int = int(this._reserveSlots.length);
         var _loc2_:int = 0;
         while(_loc2_ < _loc1_)
         {
