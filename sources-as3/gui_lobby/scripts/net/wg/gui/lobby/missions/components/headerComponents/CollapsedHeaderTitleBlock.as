package net.wg.gui.lobby.missions.components.headerComponents
{
   ...
   public class CollapsedHeaderTitleBlock extends HeaderTitleBlockBase
   {
      ...
      {
         super.draw();
         if(Boolean(this._data) && Boolean(this.action) && Boolean(isInvalid(InvalidationType.DATA)))
         {
            this.action.setData(this._data.actionVO);
            if(Boolean(this.action.getActionId()))
            {
               mouseChildren = true;
            }
            invalidateSize();
      ...
      private function onClickHandler(param1:MouseEvent) : void
      {
         ...
         {
            if(Boolean(this.action) && param1.currentTarget == this.action.linkBtn)
            {
