package net.wg.gui.lobby.barracks
{
   ...
   public class BarracksItemRenderer extends SoundListItemRenderer
   {
      ...
      override protected function draw() : void
      {
         ...
         if(!_baseDisposed)
         {
            if(Boolean(isInvalid(INVALIDATE_IN_TANK)) && Boolean(this.selection))
            {
            ...
            if(Boolean(isInvalid(INVALIDATE_PARAMS)) && Boolean(data))
            {
               _loc2_ = new Point(mouseX,mouseY);
               _loc2_ = localToGlobal(_loc2_);
               if(this._isMouseOver && Boolean(hitTestPoint(_loc2_.x,_loc2_.y,true)))
               {
                  if(!(this._buy || this.empty) && Boolean(this.btnDismiss.hitTestPoint(_loc2_.x,_loc2_.y,true)))
                  {
               ...
               _loc3_ = [];
               _loc4_ = int(this._tankmanData.roles.length);
               _loc5_ = 0;
               while(_loc5_ < _loc4_)
               {
               ...
               if(!(this._buy || this.empty))
               {
                  ...
                  this.rank.visible = this._tankmanData.isRankNameVisible && Boolean(StringUtils.isNotEmpty(this._tankmanData.rank));
                  if(this.rank.visible)
                  {
                     this.rank.text = this._tankmanData.rank;
                     _loc9_ = this._tankmanData.notRecruited ? NOT_RECRUITED_DATE_COLOR : RANK_NAME_COLOR;
      ...
      private function onClickHandler(param1:MouseEvent) : void
      {
         ...
         if(Boolean(this._utils.commons.isRightButton(param1)) && (!this._buy && !this._empty))
         {
