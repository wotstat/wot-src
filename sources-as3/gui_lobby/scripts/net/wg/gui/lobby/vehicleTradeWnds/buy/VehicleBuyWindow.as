   ...
   public class VehicleBuyWindow extends VehicleBuyWindowMeta implements IVehicleBuyWindowMeta
   {
      ...
      override protected function draw() : void
      {
         ...
         if(this._dataVo != null && Boolean(isInvalid(InvalidationType.DATA)))
         {
            ...
            this._selectedTab = Boolean(this._dataVo.contentData) && this._dataVo.contentData.selectedTab != Values.DEFAULT_INT ? this._dataVo.contentData.selectedTab : 0;
         ...
         if(this._dataVo != null && Boolean(isInvalid(INVALID_DEPENDENT_LABEL)))
         {
            this.updateDependentLabels(this._dataVo.isTradeIn,this._selectedTab);
         }
         if(Boolean(this._content) && Boolean(isInvalid(INVALID_TRADE_IN_WARNNING_MESSAGE)))
         {
            this._content.setTradeInWarningMessagege(this._tradeInWarningMessage);
         }
         if(Boolean(this._content) && Boolean(isInvalid(INVALID_TOTAL_CREDITS)))
         {
            this._content.updateTotalCredits(this._totalCredits);
         }
         if(Boolean(this._content) && Boolean(isInvalid(INVALID_TOTAL_GOLD)))
         {
            this._content.updateTotalGold(this._totalGold);
         }
         if(Boolean(this._content) && Boolean(isInvalid(INVALID_TRADEOFF_VEHICLE)))
         {
            this._content.updateTradeOffVehicle(this._vehicleBuyTradeOffVo);
         }
         if(isInvalid(INVALID_SUBMIT_BTN))
         {
            if(this._needUpdateSubmitFocus && Boolean(this.submitBtn.enabled) && this._isSetSubmitFocus)
            {
               App.utils.scheduler.scheduleOnNextFrame(setFocus,this.submitBtn);
               this._needUpdateSubmitFocus = false;
            }
         }
         if(Boolean(this._content) && Boolean(isInvalid(INVALID_CONTENT_HEIGHT)))
         {
            ...
            if(this._needUpdateWndPosition)
            {
               ...
               {
                  _loc1_ = Number(window.contentPadding["vertical"]);
               }
