package net.wg.gui.lobby.vehicleTradeWnds.sell
{
   ...
   public class SellDevicesComponent extends UIComponentEx
   {
      ...
      {
         super.draw();
         if(Boolean(this._complexDevicesArr))
         {
      ...
      public function getNextPosition(param1:Boolean = true) : int
      {
         ...
         return _loc2_ + (param1 ? PADDING_FOR_NEXT_ELEMENT : -PADDING_FOR_NEXT_ELEMENT);
      }
      ...
      {
         var _loc1_:Number = this.isActive ? this._devicesCount * SCROLL_RENDERER_HEIGHT + SCROLL_RENDERER_HEAD_HEIGHT : 0;
         if(this._isExtended)
         {
      ...
      private function getRenderer(param1:SellDialogElementVO) : ISaleItemBlockRenderer
      {
         ...
         for each(_loc3_ in _loc2_)
         {
            ...
            if(Boolean(_loc4_) && _loc4_.id == param1.id)
            {
               return _loc3_;
            }
         }
