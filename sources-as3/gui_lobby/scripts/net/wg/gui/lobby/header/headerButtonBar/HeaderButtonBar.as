package net.wg.gui.lobby.header.headerButtonBar
{
   ...
   public class HeaderButtonBar extends ButtonBar implements IHelpLayoutComponent
   {
      ...
      override protected function populateRendererData(param1:Button, param2:uint) : void
      {
         ...
         {
            _loc5_ = true;
            if(Boolean(this._lastRightLeftAlignItem))
            {
      ...
      public function getLayoutProperties() : Vector.<HelpLayoutVO>
      {
         ...
         if(Boolean(_renderers) && _renderers.length > 0)
         {
            _loc9_ = int(_renderers.length);
            _loc10_ = 0;
      ...
      private function updateFreeSizeButtons() : void
      {
         ...
         var _loc6_:int = 0;
         var _loc7_:int = 0;
         var _loc8_:int = int(_renderers.length);
         var _loc9_:int = 0;
         while(_loc9_ < _loc8_)
         {
            _loc1_ = _renderers[_loc9_];
            _loc5_ = _loc1_.headerButtonData;
            if(Boolean(_loc5_) && _loc5_.resizePriority > 0)
            {
