package net.wg.gui.lobby.header.headerButtonBar
{
   ...
   public class HBC_Squad extends HeaderButtonContentItem
   {
      ...
      private var _isWideScreen:Boolean = wideScreenPrc > WIDE_SCREEN_PRC_BORDER;
      ...
      override protected function draw() : void
      {
         ...
         {
            _loc1_ = Boolean(this._squadDataVo.squadExtendInfoVo.squadManStates.length) && this._squadDataVo.squadExtendInfoVo.squadManStates.length <= NORMAL_SIZE_PLATOON;
            if(_loc1_)
            {
      ...
      override protected function configUI() : void
      {
         ...
         var _loc1_:int = int(this._containersOrder.length);
         var _loc2_:int = 0;
         while(_loc2_ < _loc1_)
         {
      ...
      override protected function updateSize() : void
      {
         ...
         var _loc1_:Boolean = Boolean(this._squadDataVo.squadExtendInfoVo.squadManStates.length) && this._squadDataVo.squadExtendInfoVo.squadManStates.length <= NORMAL_SIZE_PLATOON;
         ...
         {
            _loc5_ = int(this.searchingAnimation.width);
         }
         var _loc6_:int = 0;
         var _loc7_:int = 0;
         if(this.textField.visible)
         {
            ...
            if(_loc6_ > 0)
            {
               _loc7_ = int(DIMENSIONS.additionalTextPadding);
            }
         }
         ...
         switch(this._squadDataVo.squadExtendInfoVo.platoonState)
         {
            ...
            case PlatoonShieldIconContainer.STATES.inPlatoon:
               ...
               if(_loc1_)
               {
                  ...
                  this.updateIconsVisibility();
                  _loc2_ = int(this._squadDataVo.squadExtendInfoVo.squadManStates.length);
      ...
      {
         this.clearOrderList();
         var _loc1_:int = int(this._innerOrder.length);
         ...
         if(Boolean(this._containersDefPositionsX))
         {
      ...
      {
         return Boolean(super.isNeedUpdateFont()) || useFontSize != this.textField.getTextFormat().size;
      }
      ...
      {
         this.restoreDefaultPositions();
         this.mainIconContainer.x = this._isWideScreen ? this.textField.x + this.textField.width + DIMENSIONS.shieldDistanceToText >> 0 : Number(this._containersDefPositionsX[this._containersOrder.indexOf(this.leftIconContainer)]);
      }
      
      private function getPaddingDependingOnState(param1:String) : int
      {
         var _loc2_:int = 0;
         var _loc3_:Boolean = Boolean(this._squadDataVo.squadExtendInfoVo.squadManStates.length) && this._squadDataVo.squadExtendInfoVo.squadManStates.length <= NORMAL_SIZE_PLATOON;
         switch(param1)
         {
            case PlatoonShieldIconContainer.STATES.create:
               if(!_loc3_)
               {
                  _loc2_ = int(DIMENSIONS.createPadding);
               }
               ...
               {
                  _loc2_ = int(DIMENSIONS.minResCreateShieldPadding);
               }
               break;
            case PlatoonShieldIconContainer.STATES.squadEvent:
            case PlatoonShieldIconContainer.STATES.inPlatoon:
               ...
               {
                  _loc2_ = int(DIMENSIONS.platoonOrSquadPadding);
               }
               else
               {
                  _loc2_ = int(DIMENSIONS.minResTripleShieldPadding);
               }
               break;
            case PlatoonShieldIconContainer.STATES.searching:
               if(this._isWideScreen)
               {
                  _loc2_ = int(DIMENSIONS.searchingPadding);
               }
               else
               {
                  _loc2_ = int(DIMENSIONS.minResSearchPadding);
               }
         }
         return _loc2_;
      }
      ...
      {
         var _loc3_:int = int(this._containersOrder.length);
         var _loc4_:int = 0;
         while(_loc4_ < _loc3_)
         {
      ...
      private function setSubiconsVisibility(param1:Boolean, param2:int = 0) : void
      {
         var _loc5_:int = 0;
         var _loc6_:int = 0;
         var _loc3_:int = int(this._innerOrder.length);
         var _loc4_:int = 0;
         while(_loc4_ < _loc3_)
         {
            _loc5_ = int(this._innerOrder[_loc4_].length);
            _loc6_ = 0;
            while(_loc6_ < _loc5_)
            {
      ...
      private function updateIconsVisibility() : void
      {
         ...
         var _loc2_:int = int(this._squadDataVo.squadExtendInfoVo.squadManStates.length);
      ...
      override public function set data(param1:Object) : void
      {
         ...
         if(Boolean(this._squadDataVo) && Boolean(this._squadDataVo.squadExtendInfoVo) && Boolean(this._squadDataVo.squadExtendInfoVo.squadManStates))
         {
