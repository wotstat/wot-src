package net.wg.gui.lobby.rankedBattles19
{
   ...
   public class RankedBattlesPage extends RankedBattlesPageMeta implements IRankedBattlesPageMeta, IStageSizeDependComponent
   {
      ...
      private static const INFO_PAGE_BROWSER_WIDTH:int = 1180;
      
      private static const INFO_PAGE_BROWSER_X_SHIFT:int = 88;
      ...
      override protected function draw() : void
      {
         ...
         super.draw();
         var _loc1_:Boolean = Boolean(isInvalid(InvalidationType.DATA));
         var _loc2_:Boolean = Boolean(isInvalid(InvalidationType.SIZE));
         ...
         if(Boolean(this._countersData) && Boolean(isInvalid(COUNTERS_INVALID)))
         {
      ...
      private function updateContentSize() : void
      {
         ...
         if(Boolean(this.menu.selectedItem) && this.menu.selectedItem.viewId == RANKEDBATTLES_ALIASES.RANKED_BATTLES_INFO_ALIAS)
         {
            this.onInfoPageCompleteHandler(null);
         }
      }
      ...
      private function onContentViewChangedHandler(param1:ViewStackEvent) : void
      {
         ...
         var _loc3_:BrowserViewStackExPadding = null;
         ...
         if(param1.viewId == RANKEDBATTLES_ALIASES.RANKED_BATTLES_INFO_ALIAS)
         {
            _loc3_ = BrowserViewStackExPadding(this.content.currentView);
            _loc3_.addEventListener(Event.COMPLETE,this.onInfoPageCompleteHandler);
         }
      }
      ...
      private function onInfoPageCompleteHandler(param1:Event) : void
      {
         var _loc2_:BrowserViewStackExPadding = this.content.currentView as BrowserViewStackExPadding;
         if(!_loc2_ || !_loc2_.browser)
         {
            return;
         }
         _loc2_.removeEventListener(Event.COMPLETE,this.onInfoPageCompleteHandler);
         if(_width < INFO_PAGE_BROWSER_WIDTH)
         {
            _loc2_.browser.width = _width - INFO_PAGE_BROWSER_X_SHIFT * 2;
            _loc2_.browser.x = INFO_PAGE_BROWSER_X_SHIFT;
         }
         else
         {
            _loc2_.browser.x = 0;
