package net.wg.gui.lobby.rankedBattles19.view.rewards
{
   ...
   public class RankedBattlesRewardsYearView extends RankedBattlesRewardsYearMeta implements IRankedBattlesRewardsYearMeta
   {
      ...
      override protected function draw() : void
      {
         ...
         var _loc9_:int = 0;
         super.draw();
         var _loc1_:Boolean = Boolean(isInvalid(InvalidationType.SIZE)) || Boolean(isInvalid(INV_VIEW_PADDING));
         var _loc2_:Boolean = Boolean(isInvalid(InvalidationType.DATA));
         _loc3_ = uint(_width - viewPadding.left >> 1);
         var _loc4_:uint = uint(_height - viewPadding.top >> 1);
         if(Boolean(this._data))
         {
            if(_loc2_)
            {
               ...
               if(StringUtils.isNotEmpty(this._counterValue))
               {
                  ...
                  if(this.rewardsContainer.isChooseRewardVisible && Boolean(isInvalid(COUNTER_INVALID)))
                  {
      ...
      override protected function setData(param1:RankedRewardsYearVO) : void
      {
         ...
         var _loc4_:int = int(param1.rewards.length);
         var _loc5_:int = 0;
         while(_loc5_ < _loc4_)
         {
            _loc3_ = param1.rewards[_loc5_];
            _loc2_ ||= RANKEDBATTLES_CONSTS.RANKED_REWARDS_YEAR_MAIN_AVAILABLE_FOR.indexOf(_loc3_.id) >= 0 && (_loc3_.status == RANKEDBATTLES_CONSTS.YEAR_REWARD_STATUS_CURRENT || _loc3_.status == RANKEDBATTLES_CONSTS.YEAR_REWARD_STATUS_CURRENT_FINAL);
            _loc5_++;
         }
