   ...
   public class RankedBattlesHangarWidget extends RankedBattlesHangarWidgetMeta implements IRankedBattlesHangarWidgetMeta, ISoundable
   {
      ...
      override protected function draw() : void
      {
         ...
         if(this._model != null)
         {
            if(isInvalid(InvalidationType.DATA))
            {
               ...
               this.shineAnim.stop();
               if(Boolean(this._model.divisionVO))
               {
                  this.division.update(this._model.divisionVO);
               }
               if(Boolean(this._model.leagueVO))
               {
            ...
            else if(this._currentAnimationStep < this._animationSteps.length && Boolean(isInvalid(INV_FINAL_STATE)))
            {
               if(this._animationEnabled)
               {
      ...
      public function recalculateLayout(param1:Boolean = false, param2:Boolean = false, param3:Boolean = false) : void
      {
         ...
         else
         {
            ...
            if(param3 && Boolean(StringUtils.isNotEmpty(this._infoTF.htmlText)))
            {
      ...
      private function clearAnimationSteps() : void
      {
         ...
         if(Boolean(this._animationSteps))
         {
      ...
      private function setFinalStateData(param1:Boolean = true) : void
      {
         ...
         if(Boolean(_loc2_))
         {
            ...
            _loc4_ = Boolean(_loc3_) && Boolean(_loc3_.length);
      ...
      private function getShieldOffset(param1:IRankIcon) : int
      {
         ...
         if(isNaN(_loc2_))
         {
            _loc2_ = param1.hasVisibleShield() ? Number(RankShield.SHIELD_OFFSETS[this._widgetSize]) : 0;
         }
         return _loc2_;
      }
      ...
      private function runStepsAnimation() : void
      {
         this.runStepsShineAnimation();
         this.animationFinished();
         if(this._model.isHuge && Boolean(StringUtils.isEmpty(this.stepsContainer.getStepsState())))
         {
      ...
      private function resizeHit() : void
      {
         ...
         if(this._model == null || this._model.leagueVO == null)
         {
            ...
            if(_loc1_ == _loc2_)
            {
               _loc3_ = int(_loc1_.x);
               _loc4_ = int(_loc1_.y);
               _loc5_ = int(_loc1_.width);
               _loc6_ = int(_loc1_.height);
            }
            else
            {
      ...
      private function onRollOverHandler(param1:MouseEvent) : void
      {
         ...
         if(Boolean(this._model) && !this._model.isHuge)
         {
