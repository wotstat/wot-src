package net.wg.gui.lobby.rankedBattles19.components
{
   ...
   public class ImageContainer extends UIComponentEx
   {
      ...
      {
         super.draw();
         if(Boolean(StringUtils.isNotEmpty(this._source)) && Boolean(isInvalid(InvalidationType.DATA)))
         {
            this.img.source = this._source;
         }
         if(this._imgPoint != null && Boolean(isInvalid(InvalidationType.LAYOUT)))
         {
      ...
      {
         if(Boolean(StringUtils.isNotEmpty(param1)) && this._source != param1)
         {
            this._source = param1;
            invalidateData();
         }
