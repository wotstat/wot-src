package net.wg.gui.lobby.rankedBattles19.view.rewards.league
{
   ...
   public class RewardsLeagueContainer extends Sprite implements IDisposable
   {
      ...
      {
         var _loc1_:int = 0;
         if(Boolean(this._rewardsData) && this._rewardsData.length > 0)
         {
            _loc1_ = int(this._rewardsData.length);
            if(!this._renderers)
            {
      ...
      {
         return Boolean(this._renderers) && this._renderers.length > 0 ? Number(this._renderers[0].height) : Number(Values.ZERO);
      }
