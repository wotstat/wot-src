package net.wg.gui.lobby.rankedBattles19.components.divisionsContainer
{
   ...
   public class DivisionsContainer extends UIComponentEx implements IGroupedControl
   {
      ...
      override protected function draw() : void
      {
         ...
         if(this._data != null)
         {
            if(isInvalid(InvalidationType.DATA))
            {
               ...
               _loc2_ = int(this._divisions.length);
               if(_loc2_ > _loc1_)
               {
            ...
            if(this._selectedIndex > -1 && Boolean(isInvalid(INV_SELECTED_INDEX)))
            {
               _loc3_ = this._divisions[this._selectedIndex];
               if(_loc3_ != this._lastSelectedDivision)
               {
                  if(Boolean(this._lastSelectedDivision))
                  {
      ...
      {
         return Boolean(this._data) && this._selectedIndex >= 0 ? this._data[this._selectedIndex] : null;
      }
      ...
      private function onDivisionClickHandler(param1:MouseEvent) : void
      {
         ...
         var _loc3_:int = int(this._divisions.length);
         var _loc4_:int = 0;
         while(_loc4_ < _loc3_)
         {
