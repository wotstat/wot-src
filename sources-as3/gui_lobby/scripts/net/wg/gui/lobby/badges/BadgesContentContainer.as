package net.wg.gui.lobby.badges
{
   ...
   public class BadgesContentContainer extends UIComponentEx
   {
      ...
      override protected function draw() : void
      {
         ...
         var _loc3_:int = 0;
         super.draw();
         if(this._receivedBadges != null && Boolean(isInvalid(InvalidationType.DATA)))
         {
            this.receivedList.dataProvider = this._receivedBadges.badgesData;
            invalidateSize();
            invalidate(INV_START_BADGE);
         }
         if(this._notReceivedBadges != null && Boolean(isInvalid(InvalidationType.DATA)))
         {
         ...
         if(this._receivedBadges != null && Boolean(isInvalid(INV_START_BADGE)))
         {
            if(Boolean(this._currentBadge))
            {
               this._currentBadge.selected = false;
            }
            _loc2_ = int(this._receivedBadges.badgesData.length);
            _loc3_ = 0;
            while(_loc3_ < _loc2_)
            {
