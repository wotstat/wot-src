package net.wg.gui.lobby.vehicleCustomization
{
   ...
   public class CustomizationCarousel extends ScrollCarousel implements IFocusChainContainer, IMagneticClickHandler
   {
      ...
      private var _bookmarkBackings:Vector.<CustomizationCarouselBookmark> = new Vector.<CustomizationCarouselBookmark>();
      ...
      override protected function updateLayout(param1:int, param2:int = 0) : void
      {
         ...
         if(Boolean(this.shopEntryPointBtn))
         {
            this.shopEntryPointBtn.x = SHOP_ENTRY_X;
            this.shopEntryPointBtn.y = this._isMinResolution ? SHOP_ENTRY_Y_SMALL : SHOP_ENTRY_Y_BIG;
         }
         ...
         if(_loc6_.minHorizontalScrollPosition == _loc6_.maxHorizontalScrollPosition && _loc6_.maxHorizontalScrollPosition == 0)
         {
            ...
            {
               _loc9_ = Number(scrollList.viewPort.visibleWidth);
         ...
         var _loc8_:int = int(scrollList.width);
         _loc4_ -= _loc7_ << 1;
         ...
         this.projectionDecalHint.height = this._isMinResolution ? HIT_AREA_HEIGHT_MIN : HIT_AREA_HEIGHT;
      ...
      override protected function draw() : void
      {
         ...
         if((this._oldWidth != _width || this._isMinResolution != _loc1_) && Boolean(isInvalid(InvalidationType.SIZE)))
         {
         ...
         if(this._data != null && scrollList.viewPort != null && Boolean(isInvalid(InvalidationType.DATA)))
         {
            this.scrollBar.clearBookmarks();
         ...
         if(Boolean(isInvalid(INVALID_BOOKMARKS_LAYOUT)) && !isInvalid(InvalidationType.DATA))
         {
            this.layoutBookmarks();
         }
      }
      ...
      public function selectSlot(param1:int, param2:Boolean = false) : CustomizationCarouselRendererVO
      {
         ...
         var _loc4_:int = int(this._dataProvider.length);
         var _loc5_:int = 0;
         while(_loc5_ < _loc4_)
         {
      ...
      public function setData(param1:CustomizationCarouselDataVO) : void
      {
         ...
         {
            this._data = param1;
            if(Boolean(this._layoutController))
            {
      ...
      private function layoutBookmarks() : void
      {
         ...
         var _loc2_:int = int(this._bookmarkBackings.length);
         var _loc5_:int = 0;
         while(_loc5_ < _loc2_)
         {
      ...
      {
         return this._isMinResolution ? SHOP_BTN_WIDTH_SMALL : SHOP_BTN_WIDTH_BIG;
      }
