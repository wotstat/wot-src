package net.wg.gui.lobby.clans.profile
{
   ...
   public class ClanProfileMainWindowBaseHeader extends UIComponentEx implements IClanHeaderComponent
   {
      ...
      {
         super.draw();
         if(Boolean(isInvalid(InvalidationType.DATA)) && Boolean(this._baseInfo))
         {
      ...
      {
         if(Boolean(this.clanEmblem) && Boolean(this.clanEmblem.filters))
         {
      ...
      {
         if(Boolean(this._baseInfo) && this.clanNameTF.text != this._baseInfo.name)
         {
