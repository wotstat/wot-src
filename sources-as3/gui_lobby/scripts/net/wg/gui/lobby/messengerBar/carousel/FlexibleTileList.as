package net.wg.gui.lobby.messengerBar.carousel
{
   ...
   public class FlexibleTileList extends TileList
   {
      ...
      override protected function calculateRendererTotal(param1:Number, param2:Number) : uint
      {
         ...
         var _loc3_:Number = Number(availableWidth);
         _autoColumnWidth = getAvailableItemWidth(Boolean(_dataProvider) ? int(_dataProvider.length) : 0,this.minRendererWidth,this.maxRendererWidth,_loc3_);
      ...
      override protected function drawLayout() : void
      {
         ...
         var _loc1_:uint = Boolean(_renderers) ? uint(_renderers.length) : 0;
         var _loc2_:Boolean = Boolean(isInvalid(InvalidationType.DATA));
         if(_loc1_ > 0)
         {
            _loc3_ = Number(rowHeight);
            _loc4_ = 0;
            ...
            while(_loc7_ < _loc1_)
            {
               ...
               {
                  _loc4_ = Number(_autoColumnWidth);
               }
               _loc8_.width = _loc4_;
