package net.wg.gui.lobby.techtree.nodes
{
   ...
   public class ResearchRoot extends UIComponentEx implements IRenderer, ITutorialCustomComponent
   {
      ...
      private var _universalBtnStyles:IUniversalBtnStyles = App.utils.universalBtnStyles;
      ...
      override protected function onDispose() : void
      {
         ...
         this.price.dispose();
         this.price = null;
         if(Boolean(this._experienceBlock))
         {
         ...
         if(Boolean(this._changeNationBtn))
         {
      ...
      {
         super.draw();
         if(Boolean(this._nodeData) && Boolean(this._data) && Boolean(isInvalid(InvalidationType.DATA)))
         {
            this.populateUI();
            invalidateLayout();
         }
      ...
      public function getColorIndex(param1:IRenderer = null) : uint
      {
         ...
         if(Boolean(param1 && param1.isElite()) || Boolean(this.isPremium()) || this.isElite() && this.isUnlocked())
         {
         ...
         else if(Boolean(param1) && Boolean(param1.isUnlocked()) && this.isNext2Unlock())
         {
            _loc2_ = ColorIndex.UNLOCKED;
         }
         return _loc2_;
      ...
      {
         return Boolean(this._nodeData) ? this._nodeData.earnedXP : 0;
      }
      ...
      {
         return Boolean(this._nodeData) ? this._nodeData.id : 0;
      }
      ...
      {
         return Boolean(this._nodeData) ? this._nodeData.level : 0;
      }
      ...
      private function populateUI() : void
      {
         ...
         if(Boolean(this.price.visible) && Boolean(this._data.itemPrices))
         {
         ...
         this.earlyAccessCoinsTF.visible = Boolean(this.price.visible) && this.isEarlyAccess() && this.isNext2Unlock() && !this._nodeData.isEarlyAccessLocked;
      ...
      private function updateLayout() : void
      {
         ...
         this.internalXShift = this._size == SMALL_SIZE && Boolean(this.tradeOffWidget.visible) ? TRADE_OFF_WIDGET_DO_INTERNAL_X_SHIFT : int(Values.ZERO);
         var _loc2_:Boolean = Boolean(this._experienceBlock) && Boolean(this._experienceBlock.visible);
         if(_loc2_)
         {
         ...
         this.paragonsInfoDivider.visible = Boolean(this.paragonsInfo.visible) && _loc2_;
