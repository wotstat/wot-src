package net.wg.gui.lobby.prestige.components.common
{
   ...
   public class PrestigeEmblem extends UIComponentEx
   {
      ...
      override protected function draw() : void
      {
         ...
         if(_loc1_ || Boolean(isInvalid(INVALIDATE_EMBLEM)))
         {
            this._emblem.source = this.getEmblemIcon();
         }
         if(_loc1_ || Boolean(isInvalid(INVALIDATE_LEVEL)))
         {
            this.setDigitsGroup();
         }
         if(Boolean(this._digitsGroup) && Boolean(isInvalid(InvalidationType.LAYOUT)))
         {
      ...
      private function setDigitsGroup() : void
      {
         ...
         this.cleanDigitsGroup();
         if(this._isMaximumEmblem || Boolean(StringUtils.isEmpty(this._level)))
         {
            return;
         }
         ...
         var _loc1_:uint = uint(this._level.length);
      ...
      private function cleanDigitsGroup() : void
      {
         ...
         var _loc3_:uint = uint(this._digitsGroup.numChildren);
         _loc1_ = 0;
         while(_loc1_ < _loc3_)
         {
