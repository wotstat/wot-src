package net.wg.gui.lobby.components
{
   ...
   public class AwardItemRendererEx extends UIComponentEx implements IListItemRenderer, IUpdatable, IScrollerItemRenderer
   {
      ...
      override protected function onDispose() : void
      {
         ...
         this.img.dispose();
         this.img = null;
         if(Boolean(this.overlayImg))
         {
            this.overlayImg.dispose();
            this.overlayImg = null;
         }
         if(Boolean(this.highlightImg))
         {
      ...
      override protected function draw() : void
      {
         ...
         if(isInvalid(InvalidationType.DATA))
         {
            if(Boolean(this._data))
            {
               ...
               if(Boolean(this._data.hasCompensation) && Boolean(this._data.compensationReason) && this._data.compensationReason.itemName == ITEM_NAME_VEHICLE)
               {
               ...
               this.awardObtainedIcon.visible = this._data.isObtained && Boolean(StringUtils.isNotEmpty(_loc1_)) && Boolean(StringUtils.isNotEmpty(this._data.imgSource));
               ...
               else
               {
                  ...
                  this.awardObtainedIcon.visible = this._data.areTokensPawned && Boolean(StringUtils.isNotEmpty(_loc6_));
                  if(Boolean(this.awardObtainedIcon.visible) && this.awardObtainedIcon.source != _loc6_)
                  {
                     this.awardObtainedIcon.source = _loc6_;
                  }
               }
               _loc2_ = this._data.overlayType;
               _loc3_ = Boolean(StringUtils.isNotEmpty(_loc2_));
               ...
               if(Boolean(this.overlayImg))
               {
                  this.overlayImg.visible = !_loc3_ && Boolean(StringUtils.isNotEmpty(this._data.overlayIcon));
               ...
               _loc5_ = Boolean(StringUtils.isNotEmpty(_loc4_));
               ...
               if(Boolean(this.highlightImg))
               {
                  this.highlightImg.visible = !_loc5_ && Boolean(StringUtils.isNotEmpty(this._data.highlightIcon));
         ...
         if(Boolean(this._data) && Boolean(isInvalid(InvalidationType.SIZE)))
         {
            if(this._hasSize)
            {
               if(this.img.bitmapWidth >= this._rendererWidth || this.img.bitmapHeight >= this._rendererHeight || this._allowRescaleUpward && (this.img.bitmapWidth != this._rendererWidth || this.img.bitmapHeight != this._rendererHeight))
               {
                  ...
                  this.img.scaleX = this.img.scaleY = _loc8_ > _loc9_ ? _loc9_ : _loc8_;
               }
      ...
      override public function set enabled(param1:Boolean) : void
      {
         ...
         if(param1 && Boolean(this._data))
         {
            _loc2_ = new Point(mouseX,mouseY);
            _loc2_ = localToGlobal(_loc2_);
            if(hitTestPoint(_loc2_.x,_loc2_.y,true))
            {
               if(Boolean(this.starIcon.visible) && Boolean(this.starIcon.hitTestPoint(_loc2_.x,_loc2_.y,true)))
               {
