   ...
   public class DescriptionBlock extends UIComponentEx
   {
      ...
      override protected function draw() : void
      {
         ...
         if(isInvalid(InvalidationType.DATA))
         {
            ...
            this.descrTF.y = Boolean(this._title) ? Math.round(this.lableTF.y + this.lableTF.textHeight + TITLE_PADDING) : this.lableTF.y;
            _loc1_ = 0;
            if(Boolean(this._descr))
            {
               _loc1_ = Math.round(this.descrTF.y + this.descrTF.textHeight + BOTTOM_PADDING);
            }
            else if(Boolean(this._title))
            {
