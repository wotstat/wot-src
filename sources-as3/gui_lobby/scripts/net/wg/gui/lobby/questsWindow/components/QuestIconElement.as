package net.wg.gui.lobby.questsWindow.components
{
   ...
   public class QuestIconElement extends AbstractResizableContent
   {
      ...
      override protected function draw() : void
      {
         ...
         var _loc2_:int = 0;
         super.draw();
         if(Boolean(isInvalid(InvalidationType.DATA)) && Boolean(this.dataVO))
         {
            ...
            _loc2_ = 0;
            if(Boolean(this.dataVO.counter))
            {
               _loc2_ = this.counterTF.textWidth + TEXT_PADDING;
            }
            if(Boolean(this.dataVO.icon))
            {
               this.icon.x = _loc2_;
      ...
      {
         if(Boolean(this.dataVO.dataType) && Boolean(this.dataVO.dataName) && Boolean(this.dataVO.dataBlock))
         {
            App.toolTipMgr.showSpecial(this.dataVO.dataType,null,this.dataVO.dataBlock,this.dataVO.dataName);
         }
         else if(Boolean(this.dataVO.dataType))
         {
