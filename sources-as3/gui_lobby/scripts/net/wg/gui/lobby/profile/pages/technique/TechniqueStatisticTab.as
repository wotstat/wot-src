package net.wg.gui.lobby.profile.pages.technique
{
   ...
   public class TechniqueStatisticTab extends ResizableContent
   {
      ...
      override protected function draw() : void
      {
         ...
         if(this._data != null)
         {
            _loc1_ = Boolean(isInvalid(INVALIDATE_PRESTIGE_VISIBILITY));
            ...
            if(_loc1_)
            {
               ...
               else if(Boolean(this._prestigeProgressInject) && !this._isPrestigeAvailable)
               {
                  this.removePrestigeWidget();
                  invalidateLayout();
               }
            ...
            if((_loc1_ || _loc2_) && Boolean(currentDimension))
            {
               invalidateSize();
            }
            if(isInvalid(InvalidationType.LAYOUT))
            {
               this._group.y = Boolean(this._prestigeProgressInject) ? PRESTIGE_HEIGHT : DEFAULT_GROUP_Y;
            }
         }
         if(currentDimension != null && Boolean(isInvalid(InvalidationType.SIZE)))
         {
      ...
      private function removePrestigeWidget() : void
      {
         ...
         if(Boolean(this._prestigeProgressInject))
         {
            this._prestigeProgressInject.dispatchEvent(new Event(Event.RESIZE,true));
