package net.wg.gui.lobby.profile.pages.awards
{
   ...
   public class AwardsMainContainer extends UIComponentEx
   {
      ...
      private static function applyBlockData(param1:AwardsTileListBlock, param2:Array) : Boolean
      {
         ...
         var _loc4_:int = Boolean(_loc3_) ? int(_loc3_.length) : 0;
         var _loc5_:int = Boolean(param2) ? int(param2.length) : 0;
         if(_loc4_ != _loc5_)
         {
            _loc6_ = true;
         }
         else
         {
            ...
            while(_loc8_ < _loc5_)
            {
               ...
               _loc9_ = Boolean(_loc7_.isEquals(_loc3_[_loc8_]));
               if(!_loc9_)
               {
                  _loc6_ = true;
                  break;
      ...
      override protected function draw() : void
      {
         ...
         if(isInvalid(InvalidationType.DATA))
         {
            ...
            while(_loc2_ < _loc1_)
            {
               if(Boolean(this._allMedalsData))
               {
         ...
         if(isInvalid(InvalidationType.SIZE))
         {
            ...
            while(_loc6_ < _loc5_)
            {
               ...
               _loc4_.visible = Boolean(_loc4_.dataProvider) && _loc4_.dataProvider.length > 0;
               if(_loc4_.visible)
               {
                  _loc4_.y = _loc3_;
