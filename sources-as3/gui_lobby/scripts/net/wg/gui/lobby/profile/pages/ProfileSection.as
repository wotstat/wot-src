package net.wg.gui.lobby.profile.pages
{
   ...
   public class ProfileSection extends ProfileSectionMeta implements IProfileSectionMeta, IResizableContent
   {
      ...
      {
         super.draw();
         if(Boolean(this.currentDimension) && Boolean(isInvalid(ResizableInvalidationTypes.CURRENT_DIMENSION_INVALID)))
         {
            this.applyResizing();
         }
         if(Boolean(this.currentData) && Boolean(isInvalid(DOSSIER_DATA_INVALID)))
         {
            ...
            if(Boolean(this.battlesDropdown))
            {
               this.battlesDropdown.addEventListener(Event.CHANGE,this.onDropDownChangeHandler);
            }
            this.applyData(this.currentData);
         }
         if(Boolean(this._isActive && this._isDataInitialized) && Boolean(this._animationManager) && Boolean(isInvalid(ANIMATION_INVALID)))
         {
