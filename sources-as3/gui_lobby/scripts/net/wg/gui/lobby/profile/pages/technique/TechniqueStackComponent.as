package net.wg.gui.lobby.profile.pages.technique
{
   ...
   public class TechniqueStackComponent extends UIComponentEx
   {
      ...
      private var _actualCounters:Vector.<SoundButtonEx> = new Vector.<SoundButtonEx>();
      ...
      override protected function draw() : void
      {
         ...
         if(isInvalid(INVALIDATE_PRESTIGE_VISIBILITY))
         {
            if(this._isOtherPlayerMode)
            {
               ...
               else if(Boolean(this._prestigeWidgetInject) && !this._showPrestigeEmblem)
               {
                  this.removePrestigeEmblem();
               }
            }
            else if(Boolean(this.viewStack) && Boolean(this.viewStack.currentView != null) && this._isStatisticSection)
            {
               _loc1_ = this.viewStack.currentView as TechniqueStatisticTab;
               if(Boolean(_loc1_) && !_loc1_.isDisposed())
               {
                  _loc1_.updatePrestigeVisibility(this._showPrestigeEmblem);
               }
            }
         }
         if(Boolean(this._countersToSet) && Boolean(isInvalid(INVALIDATE_BTN_COUNTERS)))
         {
            _loc2_ = Values.EMPTY_STR;
            _loc3_ = null;
            _loc4_ = new CounterProps(CounterProps.DEFAULT_OFFSET_X,CounterProps.DEFAULT_OFFSET_Y);
            this.removeCounters();
            _loc5_ = int(this._countersToSet.length);
            _loc6_ = 0;
            while(_loc6_ < _loc5_)
            {
         ...
         {
            _loc8_ = this._prestigeWidgetInject != null ? int(PRESTIGE_SIZE) : 0;
      ...
      override protected function onDispose() : void
      {
         ...
         if(Boolean(this._countersToSet))
         {
      ...
      {
         if(Boolean(this._prestigeWidgetInject))
         {
      ...
      {
         if(Boolean(this._actualCounters))
         {
            while(Boolean(this._actualCounters.length))
            {
