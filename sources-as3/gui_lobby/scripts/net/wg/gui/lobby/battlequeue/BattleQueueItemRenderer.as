      ...
      override protected function draw() : void
      {
         ...
         if(Boolean(isInvalid(InvalidationType.DATA)) && Boolean(this._data))
         {
            if(Boolean(this.highlight))
            {
               ...
               this.countField.textColor = _loc1_ ? HIGHLIGHT_COLOR : this._countDefaultColor;
               this.textField.textColor = _loc1_ ? HIGHLIGHT_COLOR : this._textDefaultColor;
            }
            else
            {
               this._utils.asserter.assertNotNull(this.highlight,WARNING_HIGHLIHGHT);
            }
            if(Boolean(this.textField) && this._data.type != null)
            {
               this.textField.text = this._data.type;
            }
            if(Boolean(this.countField))
            {
               this.countField.text = String(this._data.count);
            }
            if(Boolean(this.tankType) && Boolean(StringUtils.isNotEmpty(this._data.icon)))
            {
               this.tankType.source = this._data.icon;
            }
         }
         if(Boolean(isInvalid(INVALIDATE_COUNT)) && Boolean(this._data))
         {
            if(Boolean(this.countField))
            {
               this.countField.text = String(this._data.count);
