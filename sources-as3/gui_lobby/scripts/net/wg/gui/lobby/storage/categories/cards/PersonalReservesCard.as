package net.wg.gui.lobby.storage.categories.cards
{
   ...
   public class PersonalReservesCard extends BaseCard
   {
      ...
      override protected function configUI() : void
      {
         ...
         this.alertTF.mouseEnabled = this.alertTF.mouseWheelEnabled = false;
         titleTF.mouseEnabled = titleTF.mouseWheelEnabled = false;
         image.mouseEnabled = false;
         this.cornerImage.mouseEnabled = false;
         ...
         sellButton.minWidth = SELL_BUTTON_MIN_WIDTH;
      }
      ...
      override protected function draw() : void
      {
         ...
         if(Boolean(_data))
         {
            if(isInvalid(InvalidationType.DATA))
            {
               ...
               if(Boolean(_data.customData) && _data.customData.hasOwnProperty("expiryTime"))
               {
               ...
               sellButton.label = _data.sellButtonLabel;
               sellButton.validateNow();
               invalidateSize();
            }
            if(isInvalid(InvalidationType.SIZE))
            {
               ...
               updateInventoryPosition(_loc1_);
               ...
               {
                  descriptionTF.width = inInventoryIcon.x - descriptionTF.x >> 0;
               }
               _loc2_ = _sizeVO.size.width < CARD_SMALL_WIDTH ? DESCRIPTION_SMALL_CARD_MAX_LINES : DESCRIPTION_BIG_CARD_MAX_LINES;
               App.utils.commons.truncateHtmlTextMultiline(descriptionTF,_data.description,_loc2_,DESCRIPTION_TRANSACTION_SYMBOL);
      ...
      override protected function getRollOverTweens() : Vector.<Tween>
      {
         ...
         if(Boolean(sellButton.visible) && _loc1_ + _container.height > sellButton.y)
         {
