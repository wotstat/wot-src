package net.wg.gui.lobby.eventBoards.components.view
{
   ...
   public class EventBoardTableContent extends BaseDAAPIComponent implements IUIComponentEx
   {
      ...
      private var _renderers:Vector.<IListItemRenderer> = new Vector.<IListItemRenderer>(0);
      ...
      public function getRendererPosition(param1:uint) : Number
      {
         ...
         return DisplayObject(this._renderers[param1]).y - AWARDS_HEIGHT;
      }
      ...
      override protected function draw() : void
      {
         ...
         if(Boolean(this._data) && Boolean(isInvalid(INV_MY_PLACE)))
         {
            if(this._selectedIndex < this._renderers.length && Boolean(this._renderers[this._selectedIndex]))
            {
      ...
      private function setupRenderers(param1:EventBoardTableRendererContainerVO) : void
      {
         ...
         var _loc2_:int = int(param1.tableDP.length);
         var _loc3_:int = 0;
         while(_loc3_ < _loc2_)
         {
