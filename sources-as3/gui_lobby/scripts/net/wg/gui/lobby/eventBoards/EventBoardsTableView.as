package net.wg.gui.lobby.eventBoards
{
   ...
   public class EventBoardsTableView extends EventBoardsTableViewMeta implements IEventBoardsTableViewMeta, IPopOverCaller
   {
      ...
      private var _viewOpacity:Number = 0;
      ...
      override protected function draw() : void
      {
         ...
         if(Boolean(this._headerData) && Boolean(isInvalid(INV_HEADER_DATA)))
         {
            this.header.update(this._headerData);
         }
         if(Boolean(this._tableHeaderData) && Boolean(isInvalid(INV_TABLE_HEADER_DATA)))
         {
            this.tableHeader.update(this._tableHeaderData);
         }
         if(Boolean(this._statusData) && Boolean(isInvalid(INV_STATUS_DATA)))
         {
            this.status.update(this._statusData);
         }
         if(Boolean(this._tableData) && Boolean(isInvalid(InvalidationType.DATA)))
         {
            this._content = EventBoardTableContent(this.scrollPane.target);
         ...
         if(Boolean(this._tableData) && Boolean(isInvalid(INV_MY_PLACE)))
         {
         ...
         if(isInvalid(INV_SCROLL_POSITION))
         {
            ...
            if(Boolean(this._content) && this._scrollIndex > 1)
            {
               this._content.validateNow();
         ...
         if(Boolean(this._firstAwardVO) && Boolean(isInvalid(INV_AWARD)))
         {
            this.tableTopShadow.visible = false;
            this.awards.setData(this._firstAwardVO);
            if(Boolean(this._secondAwardVO))
            {
               this.awardsOther.setData(this._secondAwardVO);
            }
         }
         if(Boolean(this._firstAwardVO) && Boolean(isInvalid(INV_AWARD_POSITION)))
         {
            this.tableTopShadow.visible = false;
            if(Boolean(this._secondAwardVO))
            {
