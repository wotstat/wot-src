package net.wg.gui.cyberSport.controls
{
   ...
   public class VehicleSelector extends UIComponentEx implements IViewStackContent
   {
      ...
      private var _localSelectionOverrides:Vector.<SelectionInfo> = new Vector.<SelectionInfo>();
      ...
      {
         if(Boolean(this.list.dataProvider))
         {
            this._selectedItemVO = null;
         }
      ...
      private function dispatchUpdate(param1:VehicleSelectorItemVO = null, param2:Boolean = false) : void
      {
         if(!this._multiSelection)
         {
            if(Boolean(param1))
            {
               if(Boolean(this._selectedItemVO) && this._selectedItemVO.intCD != param1.intCD)
               {
      ...
      private function onListSortAppliedHandler(param1:SortableScrollingListEvent) : void
      {
         ...
         if(!this._multiSelection)
         {
            _loc3_ = false;
            _loc4_ = 0;
            _loc5_ = int(this.list.dataProvider.length);
            _loc6_ = null;
            _loc4_ = 0;
            while(_loc4_ < _loc5_)
            {
      ...
      private function onAllCheckBoxSelectHandler(param1:Event) : void
      {
         ...
         var _loc2_:Boolean = Boolean(this.allCheckBox.selected);
         for each(_loc3_ in this.list.dataProvider)
         {
            _loc3_.selected = _loc2_ && Boolean(_loc3_.enabled);
...
class SelectionInfo
{
   ...
   public function SelectionInfo(param1:VehicleAlertVO, param2:Boolean)
   {
      super();
      this.vehicle = param1;
