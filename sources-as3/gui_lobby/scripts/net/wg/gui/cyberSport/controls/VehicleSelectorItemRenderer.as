package net.wg.gui.cyberSport.controls
{
   ...
   public class VehicleSelectorItemRenderer extends ListItemRendererWithFocusOnDis
   {
      ...
      protected var statesSelectedNotReady:Vector.<String> = Vector.<String>(["notReady_selected_",""]);
      
      protected var statesNotReady:Vector.<String> = Vector.<String>(["notReady_",""]);
      ...
      {
         var _loc1_:Boolean = Boolean(this.model) ? this.model.selected : false;
      ...
      override protected function draw() : void
      {
         ...
         super.draw();
         if(Boolean(this.model) && Boolean(isInvalid(InvalidationType.DATA)))
         {
            if(selected)
            {
            ...
            else if(Boolean(!enabled && !this._mouseOverAlert) && Boolean(this.model.tooltip) && Boolean(this.hitTestPoint(_loc1_.x,_loc1_.y,true)))
            {
      ...
      private function showAlertTooltip() : void
      {
         ...
         if(Boolean(this.model.state) && this.model.state != Values.EMPTY_STR)
         {
      ...
      {
         if(Boolean(this.model && this.model.tooltip) && Boolean(!enabled) && !this._mouseOverAlert)
         {
            this._toolTipMgr.showComplex(this.model.tooltip);
         }
      }
