package net.wg.gui.cyberSport.controls
{
   ...
   public class CSVehicleButton extends SoundButtonEx implements IVehicleButton
   {
      ...
      {
         if(_newFrame != null && Boolean(isInvalid(InvalidationType.STATE)))
         {
            this.vehicleLevel.setState(_newFrame);
         }
         super.draw();
         if(!this._updatedFlag && Boolean(isInvalid(UPDATE_INIT_DATA)))
         {
            if(Boolean(this._vehicleModel))
            {
               this.initMedallion();
            }
            else if(Boolean(this._rangeModel))
            {
               this.updateRange();
            }
            else
         ...
         if(Boolean(isInvalid(INV_FROZEN_ICON)) || Boolean(isInvalid(INV_ALERT_ICON)))
         {
            this.frozenIcon.visible = this._showFrozenIcon && Boolean(this._vehicleModel) && !this._vehicleModel.isReadyToFight;
            this.alertIcon.visible = !this._showFrozenIcon && this._showAlertIcon && Boolean(this._vehicleModel) && !this._vehicleModel.isReadyToFight;
         }
      }
      ...
      private function changeViewState() : void
      {
         ...
         this.vehicleLevel.visible = _loc2_ && Boolean(this._vehicleModel) && this._vehicleModel.level != 0;
      ...
      {
         if(Boolean(this._rangeModel) && Boolean(this._rangeModel.nationIDRange))
         {
      ...
      {
         if(Boolean(this._rangeModel) && Boolean(this._rangeModel.vTypeRange))
         {
      ...
      {
         if(Boolean(this._rangeModel) && Boolean(this._rangeModel.vLevelRange))
         {
      ...
      [Inspectable(name="vehicleCount",defaultValue="-1")]
