   ...
   public class UnitSlotHelper extends BaseRallySlotHelper
   {
      ...
      private static function onSlotLabelRollOverHandler(param1:RallySimpleSlotRenderer, param2:RallySlotVO) : void
      {
         ...
         else if(param2.compatibleVehiclesCount == 0 && !param2.player)
         {
            ...
            {
               if(Boolean(param2.restrictions) && (Boolean(param2.restrictions[0]) || Boolean(param2.restrictions[1])))
               {
         ...
         if(Boolean(param1))
         {
            if(param1 is SlotRenderer)
            {
               ...
               if(Boolean(param2))
               {
                  ...
                  _loc3_.levelLbl.alpha = Boolean(param2.selectedVehicleLevel) ? 1 : DESELECTED_VEHICLE_LEVEL_ALPHA;
                  if(!param2.isClosed)
                  {
                     if(param2.isCommanderState)
                     {
                     ...
                     {
                        _loc4_ = false;
                        if(Boolean(param2) && Boolean(param2.player))
                        {
                           _loc4_ = Boolean(param2.player) && Boolean(UserTags.isCurrentPlayer(param2.player.tags));
                        }
