package net.wg.gui.cyberSport.views.autoSearch
{
   ...
   public class CSAutoSearchMainView extends UIComponentEx implements ICSAutoSearchMainView
   {
      ...
      {
         if(Boolean(this._formView) && Boolean(this._formView.isOnStage) && Boolean(this._formView.visible))
         {
      ...
      {
         super.draw();
         if(Boolean(isInvalid(InvalidationType.DATA)) && Boolean(this._model))
         {
            this.initForm();
         }
         if(Boolean(this._formView))
         {
            if(Boolean(isInvalid(UPDATE_MAIN_LABEL)) && this._mainLabel != Values.EMPTY_STR)
            {
               App.utils.scheduler.scheduleOnNextFrame(this._formView.setMainLabel,this._mainLabel);
            }
            if(Boolean(isInvalid(UPDATE_TIME_DIRECTION)) && this._timeDirection != 0)
            {
               App.utils.scheduler.scheduleOnNextFrame(this._formView.setTimeDirection,this._timeDirection);
            }
            if(Boolean(isInvalid(UPDATE_COUNT_DOWN_SECONDS)) && this._countDownSeconds >= 0)
            {
               App.utils.scheduler.scheduleOnNextFrame(this._formView.setCountDownSeconds,this._countDownSeconds);
            }
         }
         if(!this._buttonsUpdated && Boolean(isInvalid(UPDATE_BUTTONS_STATE)))
         {
