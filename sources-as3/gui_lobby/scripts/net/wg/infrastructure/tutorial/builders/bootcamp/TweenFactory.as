package net.wg.infrastructure.tutorial.builders.bootcamp
{
   ...
   public class TweenFactory
   {
      ...
      public function getTweeForComponent(param1:DisplayObject, param2:TutorialTweenEffectVO) : Tween
      {
         ...
         if(Boolean(_loc3_ & TWEEN_EFFECT_TYPES.TOP))
         {
            _loc4_.y = _loc5_.y;
            _loc5_.y -= _loc5_.height;
         }
         else if(Boolean(_loc3_ & TWEEN_EFFECT_TYPES.BOTTOM))
         {
            _loc4_.y = _loc5_.y;
            _loc5_.y += _loc5_.height;
         }
         else if(Boolean(_loc3_ & TWEEN_EFFECT_TYPES.RIGHT))
         {
            _loc4_.x = _loc5_.x;
            _loc5_.x += _loc5_.width;
         }
         else if(Boolean(_loc3_ & TWEEN_EFFECT_TYPES.LEFT))
         {
            _loc4_.x = _loc5_.x;
            _loc5_.x -= _loc5_.width;
         }
         if(Boolean(_loc3_ & TWEEN_EFFECT_TYPES.ALPHA))
         {
            _loc4_.alpha = 1;
            _loc5_.alpha = 0;
         }
