package net.wg.infrastructure.managers.impl.cursor.base
{
   ...
   public class DroppingCursorManager extends ResizingCursorManager
   {
      ...
      private var _dropObjects:Dictionary = new Dictionary(true);
      ...
      {
         return Boolean(super.cursorIsFree()) && !this.isOnDropping;
      }
      ...
      {
         if(Boolean(this._dropSenderInfo) && this._dropSenderInfo.container == param1)
         {
            forceSetCursor(Cursors.DRAG_OPEN);
            this.setDropping(false);
      ...
      private function mouseDnDropHandler(param1:MouseEvent) : void
      {
         ...
         if(isLeftButton(param1))
         {
            ...
            _loc5_ = Boolean(_loc2_.container.onBeforeDrop(_loc4_,_loc3_));
            if(_loc5_)
            {
      ...
      private function droppingHandler(param1:MouseEvent) : void
      {
         ...
         if(_loc2_.state != BaseInfo.STATE_STARTED)
         {
            ...
            _loc4_ = Boolean(_loc2_.container.onStartDrop(_loc3_,this._dropItem,this._dropItem.mouseX,this._dropItem.mouseY));
            if(_loc4_)
            {
...
class DropInfo extends BaseInfo
{
   ...
   public function DropInfo(param1:IDroppable, param2:String)
   {
