package net.wg.infrastructure.managers.impl
{
   ...
   public class LoaderManager extends LoaderManagerMeta implements ILoaderManager
   {
      ...
      override protected function loadView(param1:LoadViewVO) : void
      {
         ...
         param1.cached = Boolean(App.cacheMgr) && Boolean(App.cacheMgr.isCached(param1.configVO.url));
      ...
      {
         var data:LoadViewVO;
         var config:ViewSettingsVO;
         var alias:String;
         var statsEntity:StatsEntity;
         var view:IView;
         ...
         data = this._loaderToInfo[loader];
         config = data.configVO;
         alias = data.alias;
         this._loaderTimerValue = getTimer() - this._loaderTimerValue;
         statsEntity = this._stats[config.url];
         if(!statsEntity)
         {
         ...
         data.cached = Boolean(App.cacheMgr) && Boolean(App.cacheMgr.add(config.url,loader,App.utils.classFactory.getClass(getQualifiedClassName(view))));
         if(data.cached)
         {
...
class StatsEntity
{
   ...
   public function StatsEntity(param1:String)
   {
      super();
      this.url = param1;
      this.cached = false;
