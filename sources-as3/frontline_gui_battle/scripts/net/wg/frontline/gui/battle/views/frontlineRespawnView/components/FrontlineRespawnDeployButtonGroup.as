package net.wg.frontline.gui.battle.views.frontlineRespawnView.components
{
   ...
   public class FrontlineRespawnDeployButtonGroup extends MovieClip implements IDisposable
   {
      ...
      public function FrontlineRespawnDeployButtonGroup()
      {
         super();
         mouseEnabled = false;
      ...
      private function updateNotifyPositions() : void
      {
         ...
         {
            this.deployNotification.y = this._isWide ? DEPLOY_NOTIFICATION_Y_WIDE : DEPLOY_NOTIFICATION_Y_NARROW;
            this.respawnNotification.y = this._isWide ? RESPAWN_NOTIFICATION_Y_WIDE : RESPAWN_NOTIFICATION_Y_NARROW;
            this.bg.y = this._isWide ? BG_Y_WIDE : BG_Y_NARROW;
            this.bg.scaleY = this._isWide ? BG_SCALE_WIDE : BG_SCALE_NARROW;
            _loc1_ = false;
         }
         else if(this._isTimerVisible)
         {
            this.deployNotification.y = this._isWide ? SINGLE_NOTIFICATION_Y_WIDE : SINGLE_NOTIFICATION_Y_NARROW;
         }
         else if(this.respawnNotification.visible)
         {
            this.respawnNotification.y = this._isWide ? SINGLE_NOTIFICATION_Y_WIDE : SINGLE_NOTIFICATION_Y_NARROW;
         }
         else
         {
            this.bg.y = BG_Y_WIDE;
         ...
         this.deployNotification.x = this.respawnNotification.x = this._isWide ? NOTIFICATION_X_WIDE : NOTIFICATION_X_NARROW;
         if(_loc1_)
         {
            this.bg.y = this._isWide ? BG_Y_WIDE : BG_Y_NARROW_SINGLE_LINE;
            this.bg.scaleY = this._isWide ? BG_SCALE_WIDE : BG_SCALE_NARROW_SINGLE_LINE;
         }
      }
      ...
      private function setDeployButtonState(param1:Boolean) : void
      {
         if(param1 && !this._introAnimationActive && !this.deployButton.enabled)
         {
            ...
            dispatchEvent(new FrontlineRespawnEvent(FrontlineRespawnEvent.DEPLOYMENT_BUTTON_READY));
         }
