package net.wg.frontline.gui.battle.views.stats.components
{
   ...
   public class PlayerScrollingList extends ScrollingList
   {
      ...
      private var _contextMenuMgr:IContextMenuManager = App.contextMenuMgr;
      ...
      {
         super.draw();
         if(Boolean(isInvalid(InvalidationType.DATA)) && Boolean(_scrollBar))
         {
      ...
      {
         var _loc5_:FrontlineStatsPlayerRenderer = null;
         var _loc6_:uint = 0;
         var _loc7_:ListData = null;
         var _loc8_:DAAPIVehicleInfoVO = null;
         var _loc9_:FrontlineVehicleStatsVO = null;
         var _loc2_:uint = param1.length;
         var _loc3_:uint = uint(_renderers.length);
         var _loc4_:uint = 0;
         while(_loc4_ < _loc3_)
         {
            _loc5_ = getRendererAt(_loc4_) as FrontlineStatsPlayerRenderer;
            ...
            _loc5_.addEventListener(FrontlineFullStatsRendererEvent.RENDERER_CLICKED,this.onPlayerItemRendererClickedHandler);
            if(Boolean(param1[_loc4_]))
            {
               ...
               if(Boolean(_loc8_))
               {
                  ...
                  _loc9_ = (_dataProvider as FrontlineVehicleDataProvider).requestEpicData(_loc8_.vehicleID);
                  if(Boolean(_loc9_))
                  {
      ...
      {
         var _loc1_:FrontlineVehicleDataProvider = null;
         ...
         {
            _loc1_ = dataProvider as FrontlineVehicleDataProvider;
            if(Boolean(_loc1_))
            {
      ...
      override protected function onDispose() : void
      {
         ...
         var _loc1_:uint = uint(_renderers.length);
         var _loc2_:uint = 0;
         while(_loc2_ < _loc1_)
         {
            _loc3_ = getRendererAt(_loc2_) as FrontlineStatsPlayerRenderer;
            if(Boolean(_loc3_))
            {
               _loc3_.removeEventListener(FrontlineFullStatsRendererEvent.RENDERER_CLICKED,this.onPlayerItemRendererClickedHandler);
            }
            _loc2_++;
         }
      ...
      {
         var _loc2_:FrontlineVehicleDataProvider = _dataProvider as FrontlineVehicleDataProvider;
         var _loc3_:int = -1;
         if(Boolean(_loc2_))
         {
      ...
      public function setInteractive(param1:Boolean) : void
      {
         ...
         if(Boolean(_renderers))
         {
            _loc2_ = 0;
            _loc3_ = int(_renderers.length);
            _loc2_ = 0;
            while(_loc2_ < _loc3_)
            {
               _loc4_ = this.getRendererAt(_loc2_) as FrontlineStatsPlayerRenderer;
               if(Boolean(_loc4_))
               {
      ...
      public function setupSquadHandler(param1:FrontlineFullStatsTableCtrl) : void
      {
         this._squadHandler = param1;
      }
      
      private function setupDynamicSquad(param1:FrontlineStatsPlayerRenderer) : void
      {
      ...
      private function onPlayerItemRendererClickedHandler(param1:FrontlineFullStatsRendererEvent) : void
      {
         ...
         if(Boolean(_loc2_))
         {
            ...
            if(Boolean(_loc3_ && App.utils.commons.isRightButton(param1.mEvent)) && Boolean(!PlayerStatus.isActionDisabled(_loc3_.playerStatus)) && !UserTags.isCurrentPlayer(_loc3_.userTags))
            {
