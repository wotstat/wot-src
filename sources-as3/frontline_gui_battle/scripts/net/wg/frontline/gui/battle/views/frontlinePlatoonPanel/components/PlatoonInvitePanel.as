package net.wg.frontline.gui.battle.views.frontlinePlatoonPanel.components
{
   ...
   public class PlatoonInvitePanel extends BattleUIComponent
   {
      ...
      override protected function draw() : void
      {
         ...
         var _loc3_:int = 0;
         super.draw();
         if(Boolean(isInvalid(InvalidationType.DATA)) && this._platoonInviteRenderers.length > 0)
         {
            _loc1_ = int(this._platoonInviteRenderers.length);
            _loc2_ = 0;
            _loc3_ = 0;
            while(_loc3_ < _loc1_)
            {
      ...
      public function updatePlatoonInvite(param1:Number, param2:DAAPIVehicleInfoVO, param3:int) : void
      {
         ...
         if(InvitationStatus.isReceivedInactive(param3))
         {
            ...
            if(_loc4_ > -1 && Boolean(this._platoonInviteRenderers))
            {
         ...
         {
            _loc5_ = App.utils.classFactory.getComponent(FrontlineLinkages.PLAYER_PLATOON_INVITE_UI,PlatoonInviteRenderer);
            if(this._platoonInviteRenderers.length >= this._maxInvites)
            {
               this.removeInvite(FIRST_LIST_ELEMENT);
            }
            _loc6_ = int(this._platoonInviteRenderers.length);
      ...
      {
         var _loc2_:int = int(this._platoonInviteRenderers.length);
         var _loc3_:int = 0;
         while(_loc3_ < _loc2_)
         {
      ...
      {
         if(Boolean(param1 != -1) && Boolean(this._platoonInviteRenderers) && param1 < this._platoonInviteRenderers.length)
         {
      ...
      private function removePlatoonInvites() : void
      {
         ...
         if(Boolean(this._platoonInviteRenderers))
         {
