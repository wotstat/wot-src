package net.wg.gui.battle.battleloading.renderers
{
   ...
   public class BasePlayerItemRenderer extends BattleUIComponentsHolder implements IBattleLoadingRenderer
   {
      ...
      override protected function draw() : void
      {
         ...
         if(isInvalid(InvalidationType.DATA))
         {
            ...
            else
            {
               ...
               if(Boolean(this._playerActionMarker))
               {
      ...
      protected function updateLayout() : void
      {
         ...
         if(Boolean(this.selfBg))
         {
            this.selfBg.x = this.isExtendedLayout ? this._defaultSelfBgXPosition - PRESTIGE_LEVEL_OFFSET : this._defaultSelfBgXPosition;
         }
         ...
         var _loc1_:int = StringUtils.isNotEmpty(this.model.suffixBadgeType) ? int(BADGE_SIZE - SUFFIX_BADGE_OFFSET) : 0;
         ...
         if(Boolean(this._vehicleField))
         {
            if(Boolean(this._icoIGR) && this._icoIGR.visible)
            {
      ...
      {
         if(Boolean(this._playerActionMarker) && Boolean(this.model.vehicleAction))
         {
      ...
      {
         if(Boolean(this._prestigeLevel))
         {
      ...
      private function updateState() : void
      {
         ...
         if(Boolean(_loc3_) && Boolean(this._vehicleIcon))
         {
            this._vehicleIcon.transform.colorTransform = _loc3_.colorTransform;
         }
         if(Boolean(this._prestigeLevel))
         {
            this._prestigeLevel.alpha = _loc1_ ? PRESTIGE_LEVEL_DEFAULT_ALPHA : PRESTIGE_LEVEL_DEAD_ALPHA;
         }
         _loc2_ = PlayerStatusSchemeName.getSchemeForVehicleLevel(!_loc1_);
         _loc3_ = this._colorMgr.getScheme(_loc2_);
         if(Boolean(_loc3_) && Boolean(this._vehicleLevelIcon))
         {
