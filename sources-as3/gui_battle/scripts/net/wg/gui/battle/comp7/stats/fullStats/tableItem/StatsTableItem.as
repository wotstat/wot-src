package net.wg.gui.battle.comp7.stats.fullStats.tableItem
{
   ...
   public class StatsTableItem extends StatsTableItemCommon implements IFullStatsPoiHolder
   {
      ...
      override protected function draw() : void
      {
         ...
         {
            this._roleSkillLevel.alpha = isDead ? ROLE_SKILL_LEVEL_DEAD_ALPHA : 1;
            if(isDead)
            {
               this._roleSkillLevel.setSkillLevel(0);
            }
            this._rankIcon.alpha = isDead ? RANK_DEAD_ALPHA : 1;
         }
         if(isInvalid(FullStatsValidationType.RANK))
         {
            _loc1_ = Boolean(StringUtils.isNotEmpty(this._rank)) && Boolean(StringUtils.isNotEmpty(this._rankDivision));
         ...
         {
            if(Boolean(this._noSoundIcon))
            {
               _loc4_ = Boolean(userProps) && !this._voiceChatConnected;
               if(_loc4_)
               {
      ...
      private function updateNoSoundIconY() : void
      {
         if(Boolean(this._noSoundIcon))
         {
            this._noSoundIcon.y = this._squadIcon.visible ? this._noSoundIconSquadY : this._noSoundIconOriginalY;
         }
      }
