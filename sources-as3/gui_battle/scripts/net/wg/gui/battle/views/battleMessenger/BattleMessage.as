package net.wg.gui.battle.views.battleMessenger
{
   ...
   public class BattleMessage extends BattleUIComponentsHolder
   {
      ...
      {
         if(this._userInteractionCallback != null && Boolean(StringUtils.isNotEmpty(this.messageID)))
         {
            this.isOpenedToxicPanel = true;
            if(Boolean(this._userInteractionCallback(true,this.mouseOut,this.messageID,this.y,this.width,this.height)))
            {
      ...
      private function onEndLifeHandler() : void
      {
         ...
         if(this._fadeTime > 0)
         {
            _loc1_ = this.alpha < this._lastMessageAlpha ? this.alpha * this._fadeTime : this._fadeTime;
