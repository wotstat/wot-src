package net.wg.gui.battle.views.destroyTimers
{
   ...
   public class DestroyTimer extends FrameAnimationTimer implements IStatusNotification
   {
      ...
      override protected function onDispose() : void
      {
         ...
         this.desc.dispose();
         this.desc = null;
         if(Boolean(iconBitmap) && iconSpr.contains(iconBitmap))
         {
            iconSpr.removeChild(iconBitmap);
         }
         iconSpr = null;
         if(Boolean(iconBitmap) && Boolean(iconBitmap.bitmapData))
         {
      ...
      override protected function draw() : void
      {
         ...
         if(Boolean(iconBitmap) && isInvalid(ICON_BTM_POSITION_VALIDATE))
         {
      ...
      {
         var _loc2_:String = param1.color;
         if(Boolean(StringUtils.isNotEmpty(_loc2_)) && this._currentColor != _loc2_)
         {
            this._currentColor = _loc2_;
            this.updateColor();
         }
         if(Boolean(StringUtils.isNotEmpty(param1.iconName)) && currentIconName != param1.iconName)
         {
            currentIconName = param1.iconName;
            updateIcon();
         }
