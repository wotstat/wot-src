package net.wg.gui.battle.views.destroyTimers
{
   ...
   public class EpicDestroyTimersPanel extends EpicDestroyTimersPanelMeta implements IEpicDestroyTimersPanelMeta
   {
      ...
      override protected function draw() : void
      {
         ...
         super.draw();
         if(Boolean(this._resupplyTimer) && isInvalid(InvalidationType.SIZE))
         {
         ...
         if(isInvalid(INVALID_RESUPPLY_STATE))
         {
            if(this._resupplyTimer.isActive)
            {
               ...
               this._resupplyTimer.x = _loc1_ ? (this._resupplyTimer.isSmallSize ? RESUPPLY_TIMER_SHIFT_SMALL : RESUPPLY_TIMER_SHIFT_HUGE) : Number(Values.ZERO);
               evaluateState();
            }
         }
      }
      ...
      {
         return this._isTimerShow || Boolean(super.isAnyTimerInCenter());
      }
      ...
      {
         if(Boolean(this._resupplyTimer))
         {
