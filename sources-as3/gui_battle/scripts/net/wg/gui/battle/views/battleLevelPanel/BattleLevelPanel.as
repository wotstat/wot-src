package net.wg.gui.battle.views.battleLevelPanel
{
   ...
   public class BattleLevelPanel extends BattleLevelPanelMeta implements IBattleLevelPanelMeta
   {
      ...
      override protected function draw() : void
      {
         ...
         {
            _loc1_ = int(this.progressBar.width);
            _loc2_ = -_loc1_ >> 1 | 0;
      ...
      private function removeFromData(param1:uint) : void
      {
         ...
         if(Boolean(this._data))
         {
            ...
            _loc3_ = int(this._data.length);
            _loc4_ = _loc3_ - 1;
            while(_loc4_ >= 0)
            {
               if(this._data[_loc4_].state == param1 && (!_loc2_ || _loc2_ && Boolean(StringUtils.isEmpty(this._data[_loc4_].currentLevel))))
               {
      ...
      {
         if(Boolean(this._midTextRunTween))
         {
