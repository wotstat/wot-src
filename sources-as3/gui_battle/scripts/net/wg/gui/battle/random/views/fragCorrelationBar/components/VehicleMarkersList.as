package net.wg.gui.battle.random.views.fragCorrelationBar.components
{
   ...
   public class VehicleMarkersList implements IVehicleMarkerAnimFinishedHandler, IDisposable
   {
      ...
      private var _vehicleMarkers:Vector.<FCVehicleMarker> = new Vector.<FCVehicleMarker>(0);
      
      private var _vehicleIDs:Vector.<Number> = new Vector.<Number>();
      ...
      private var _observerIDs:Vector.<Number> = new Vector.<Number>();
      ...
      private var _activeTierGroupings:Vector.<TierGroupingElement> = new Vector.<TierGroupingElement>();
      
      private var _activeTierLevels:Vector.<int> = new Vector.<int>();
      ...
      public function VehicleMarkersList(param1:MovieClip, param2:Boolean, param3:String)
      {
         ...
         this._markerShift = param2 ? MARKER_SHIFT : int(-MARKER_SHIFT);
         this._color = param3;
      }
      ...
      public function sort() : void
      {
         ...
         if(this._vehicleIDs != null)
         {
            ...
            if(_loc3_)
            {
               _loc1_ = this._markerStartPosition > 0 ? ENEMY_TIER_GROUPING_START_POSITION : ALLY_TIER_GROUPING_START_POSITION;
               _loc2_ = this._markerStartPosition > 0 ? TIER_GROUP_VEH_ICON_OFFSET : int(-TIER_GROUP_VEH_ICON_OFFSET);
      ...
      public function updateVehicleStatus(param1:Number, param2:uint, param3:Vector.<Number>) : void
      {
         ...
         if(this._observerIDs.indexOf(param1) == -1)
         {
            ...
            if(Boolean(_loc4_) && Boolean(param3))
            {
      ...
      private function positionTierInfo(param1:int, param2:int, param3:int) : void
      {
         ...
         {
            _loc4_ = this._markerShift > 0 ? ENEMY_TIER_GROUPING_SHIFT : ALLY_TIER_GROUPING_SHIFT;
            if(param2 == 9)
            {
