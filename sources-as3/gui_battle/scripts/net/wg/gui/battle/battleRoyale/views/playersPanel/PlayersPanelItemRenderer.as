package net.wg.gui.battle.battleRoyale.views.playersPanel
{
   ...
   public class PlayersPanelItemRenderer extends BattleUIComponent
   {
      ...
      private var _colorSchemeManager:IColorSchemeManager = App.colorSchemeMgr;
      ...
      override protected function draw() : void
      {
         ...
         {
            _loc1_ = uint(COLOR_WHITE);
            if(!this._data.isAlive)
            {
               _loc1_ = uint(COLOR_RED);
            }
            else if(this._data.isObserved)
            {
               _loc1_ = uint(COLOR_YELLOW);
            }
            ...
            this.vehicleTypeIcon.alpha = !this._data.isAlive || this._data.isObserved ? VEH_TYPE_ALPHA_SPECIAL : VEH_TYPE_ALPHA_NORMAL;
      ...
      public function setIsRespawnAvailable(param1:Boolean) : void
      {
         ...
         if(this._isRespawnAvailable != param1)
         {
            ...
            _loc2_ = param1 ? PLAYER_NAME_X : int(PLAYER_NAME_X - NO_RESP_X_OFFSET);
            _loc3_ = param1 ? VEHICLE_LEVEL_X : int(VEHICLE_LEVEL_X - NO_RESP_X_OFFSET);
            _loc4_ = param1 ? VEHICLE_TYPE_X : int(VEHICLE_TYPE_X - NO_RESP_X_OFFSET);
            _loc5_ = param1 ? VEHICLE_NAME_X : int(VEHICLE_NAME_X - NO_RESP_X_OFFSET);
      ...
      {
         var _loc1_:Number = this._data.hasRespawn && this._isRespawnAvailable ? RESP_ALPHA_NORMAL : RESP_ALPHA_NONE;
      ...
      {
         return Boolean(this._data) ? this._data.vehicleID : int(Values.DEFAULT_INT);
      }
