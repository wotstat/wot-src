package net.wg.infrastructure.managers.impl
{
   ...
   public class GameInputManager extends GameInputManagerMeta implements IGameInputManager
   {
      ...
      private var _priorities:Vector.<Number> = new <Number>[0];
      
      private var _prioritiesLen:Number = this._priorities.length;
      ...
      private static function removeKeyHandler(param1:Number, param2:String, param3:Function, param4:Dictionary) : Boolean
      {
         ...
         if(Boolean(param4[param1]))
         {
            if(Boolean(param4[param1][param2]))
            {
      ...
      private function onInputHandler(param1:InputEvent) : void
      {
         ...
         try
         {
            ...
            if(Boolean(event.handled) || this._ignoredKeyCode == details.code)
            {
               return;
            }
            i = 0;
            ...
            if(Boolean(callback.cancelEvent))
            {
            ...
            else if(Boolean(this._exclusiveHandlers) && this._exclusiveHandlers[details.code] == details.value)
            {
            ...
            if(callback.isIgnoreText && focused != null)
            {
               ...
               if(focused.type == TextFieldType.INPUT || Boolean(focusedParent && focusedParent.enabled) && Boolean(focusedParent.editable))
               {
                  return;
               }
            }
      ...
      private function onFocusInHandler(param1:FocusEvent) : void
      {
         ...
         if(Boolean(_loc2_))
         {
            ...
            if(Boolean(_loc2_.type == TextFieldType.INPUT && _loc3_ && _loc3_.enabled && _loc3_.editable) && Boolean(this.hasExclusiveHandlers()) && Boolean(App.voiceChatMgr.isVOIPAvailableS()))
            {
...
class GameInputCallback implements IDisposable
{
   ...
   public function GameInputCallback(param1:Function, param2:Boolean, param3:String = null)
   {
      super();
