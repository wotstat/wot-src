package net.wg.gui.components.tooltips.VO
{
   ...
   public class SuitableVehicleVO extends DAAPIDataClass implements IDisposable
   {
         ...
         switch(param1)
         {
            case "vehicles":
               ...
               if(Boolean(_loc3_) && _loc3_.length > 0)
               {
                  ...
                  this.leftCount = _loc3_.length - _loc4_ > 0 ? uint(_loc3_.length - _loc4_) : 0;
               }
               return false;
            case "conditions":
               if(Boolean(App.instance))
      ...
      override protected function onDispose() : void
      {
         ...
         if(Boolean(this.vehiclesList) && this.vehiclesList.length > 0)
         {
            while(Boolean(this.vehiclesList.length))
            {
