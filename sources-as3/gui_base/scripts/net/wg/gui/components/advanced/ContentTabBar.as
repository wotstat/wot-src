package net.wg.gui.components.advanced
{
   ...
   public class ContentTabBar extends ButtonBarEx
   {
      ...
      {
         var _loc2_:int = 0;
         var _loc1_:Boolean = Boolean(isInvalid(InvalidationType.RENDERERS)) || Boolean(isInvalid(InvalidationType.DATA)) || Boolean(isInvalid(InvalidationType.SETTINGS)) || Boolean(isInvalid(InvalidationType.SIZE));
         if(_loc1_)
         {
         ...
         if(_loc1_ || Boolean(isInvalid(INVALID_LAYOUT)))
         {
      ...
      override protected function updateContainerPosition() : void
      {
         ...
         if(centerTabs)
         {
            _loc2_ = int(container.width);
            _loc3_ = int(container.numChildren);
            while(_loc3_ > 0)
            {
               _loc3_--;
      ...
      override protected function populateRendererData(param1:Button, param2:uint) : void
      {
         ...
         if(Boolean(_loc3_) && _loc3_.hasOwnProperty("enabled"))
         {
      ...
      private function calculateRendererWidth() : void
      {
         ...
         var _loc2_:Number = 0;
         var _loc3_:uint = uint(_dataProvider.length);
         var _loc4_:int = 0;
         while(_loc4_ < _loc3_)
         {
            if(_baseDisposed)
