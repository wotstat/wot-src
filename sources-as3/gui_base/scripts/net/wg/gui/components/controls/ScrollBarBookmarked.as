package net.wg.gui.components.controls
{
   ...
   public class ScrollBarBookmarked extends ScrollBar
   {
      ...
      private var _bookmarks:Vector.<SoundButtonEx> = new Vector.<SoundButtonEx>(0);
      
      private var _bookmarkPositions:Vector.<Number> = new Vector.<Number>(0);
      ...
      override protected function updateThumbPosition() : void
      {
         ...
         thumb.visible = true;
         var _loc4_:Boolean = Boolean(thumb.visible) && enabled;
         if(Boolean(upArrow))
         {
            upArrow.enabled = _loc4_ && _position > _minPosition;
            upArrow.validateNow();
         }
         if(Boolean(downArrow))
         {
      ...
      {
         var _loc1_:int = int(this._bookmarks.length);
         var _loc2_:int = 0;
         while(_loc2_ < _loc1_)
         {
      ...
      public function setVisibleBookmarks(param1:Boolean) : void
      {
         ...
         if(this._isBookmarkVisible != param1)
         {
            ...
            _loc2_ = int(this._bookmarks.length);
            _loc3_ = 0;
            while(_loc3_ < _loc2_)
            {
      ...
      {
         if(Boolean(this._animateThumbTween))
         {
      ...
      private function onBookmarkClickHandler(param1:MouseEvent) : void
      {
         ...
         this.stopAnimateThumbTween();
         this.tweenPercent = 0;
         offsetIdx = this._bookmarks.indexOf(e.target);
         targetY = Number(e.target.y);
