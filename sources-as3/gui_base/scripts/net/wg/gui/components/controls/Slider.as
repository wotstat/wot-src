package net.wg.gui.components.controls
{
   ...
   public class Slider extends scaleform.clik.controls.Slider
   {
      ...
      private var _fillPadding:Padding = new Padding(3,6,3,2);
      ...
      public function Slider()
      {
         ...
         offsetLeft = 3;
         offsetRight = 4;
         if(Boolean(track) && Boolean(track["bg"]) && Boolean(track["bg"]["patternMc"]))
         {
            this.patternMc = track["bg"]["patternMc"];
         }
         if(Boolean(track) && Boolean(track["bg"]) && Boolean(track["bg"]["disablePatternMc"]))
         {
            this.disablePatternMc = track["bg"]["disablePatternMc"];
         }
         if(Boolean(track) && Boolean(track["hitMc"]))
         {
      ...
      override protected function onDispose() : void
      {
         ...
         if(Boolean(this.patternMc))
         {
            this.patternMc.dispose();
            this.patternMc = null;
         }
         if(Boolean(this.disablePatternMc))
         {
