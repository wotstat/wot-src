package net.wg.gui.components.controls
{
   ...
   public class InfoIcon extends UIComponentEx implements IInfoIcon
   {
      ...
      private function isIcoAvailable(param1:String) : Boolean
      {
         ...
         var _loc3_:uint = uint(currentLabels.length);
         var _loc4_:uint = 0;
         while(_loc4_ < _loc3_)
         {
      ...
      [Inspectable(name="icoType",defaultValue="info",type="list",enumeration="info,alert")]
      ...
      private function onHitMouseOverHandler(param1:MouseEvent) : void
      {
         ...
         if(Boolean(this._tooltip) && Boolean(enabled))
         {
            if(this.tooltipType.value == ToolTipShowType.COMPLEX.value)
            {
               _loc2_ = null;
               if(Boolean(this._tooltipMaxWidth))
               {
