package net.wg.gui.components.controls.scroller
{
   ...
   public class ScrollerBase extends UIComponentEx implements IScrollerBase
   {
      ...
      private var _velocityBufferX:Vector.<Number> = new Vector.<Number>(VELOCITY_BUFFER_MAX_SIZE + 1,true);
      
      private var _velocityBufferY:Vector.<Number> = new Vector.<Number>(VELOCITY_BUFFER_MAX_SIZE + 1,true);
      ...
      override protected function draw() : void
      {
         ...
         var _loc2_:Boolean = Boolean(isInvalid(INVALIDATION_TYPE_LAYOUT));
         var _loc3_:Boolean = Boolean(isInvalid(INVALIDATION_TYPE_PENDING_SCROLL));
         if(_loc1_ || Boolean(isInvalid(InvalidationType.SIZE)) || _loc3_ || _loc2_)
         {
         ...
         var _loc4_:Boolean = Boolean(isInvalid(INVALIDATION_TYPE_SCROLL));
         if(_loc4_ && Boolean(hasEventListener(Event.SCROLL)))
         {
            dispatchEvent(new Event(Event.SCROLL));
         }
         var _loc5_:Boolean = Boolean(isInvalid(InvalidationType.SIZE));
         if(_loc1_ || _loc5_ || _loc4_ || _loc3_ || _loc2_)
         {
            this.layoutChildren();
         }
         if(Boolean(isInvalid(INVALIDATION_TYPE_CLIPPING)) || _loc5_)
         {
            this.createMask();
         }
         if(_loc5_ && Boolean(hasEventListener(ScrollEvent.UPDATE_SIZE)))
         {
      ...
      final public function scrollToPosition(param1:Number, param2:Number, param3:Number = NaN) : void
      {
      ...
      protected function createMask() : void
      {
         ...
         else if(Boolean(this._container.mask))
         {
      ...
      private function onDragUpdateHandler(param1:Event) : void
      {
         ...
         var _loc3_:Number = Number(this.mouseX);
         var _loc4_:Number = Number(this.mouseY);
