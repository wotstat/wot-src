   ...
   public class SoundButton extends Button implements ISoundButton
   {
      ...
      override protected function changeFocus() : void
      {
         ...
         if(_focusIndicator == null)
         {
            ...
            {
               if(Boolean(this.useFocusedAsSelect) && (Boolean(_focused || _displayFocus)) && !this.selected)
               {
                  this.selected = true;
               }
            }
            ...
            {
               setState(Boolean(_focused) || Boolean(_displayFocus) ? ComponentState.OVER : ComponentState.OUT);
            }
         ...
         else
         {
            ...
            else
            {
               ...
               {
                  _newFocusIndicatorFrame = Boolean(_focused) || Boolean(_displayFocus) ? SHOW_LABEL : HIDE_LABEL;
               }
               invalidateState();
            }
            if(Boolean(_pressedByKeyboard) && !_focused)
      ...
      {
         if(Boolean(this._mutedSoundTypes))
         {
      ...
      override public function set enabled(param1:Boolean) : void
      {
         ...
         if(Boolean(this.focusable) && Boolean(this.focused) && !param1)
         {
            this.focused = 0;
         }
      ...
      [Inspectable(type="String",defaultValue="normal")]
