package net.wg.gui.components.controls
{
   ...
   public class NumericStepper extends scaleform.clik.controls.NumericStepper
   {
      ...
      private var _labelVal:String = this._emptyFieldPattern;
      ...
      private var _currentTextColorId:String = this._textColorId;
      ...
      {
         if(Boolean(this.focusIndicator) && Boolean(this._newFocusIndicatorFrame) && this._oldFocusIndicatorFrame != this._newFocusIndicatorFrame)
         {
      ...
      private function updateFocusIndicatorStateData() : void
      {
         ...
         {
            this._newFocusIndicatorFrame = this.hasFocus || Boolean(_displayFocus) ? "show" : "hide";
         }
      }
      ...
      private function initItems() : void
      {
         ...
         var _loc1_:Number = Number(this.scaleX);
         var _loc2_:Number = Number(this.scaleY);
         var _loc3_:Number = Number(this.actualWidth);
         var _loc4_:Number = Number(this.numChildren);
         setActualScale(1,1);
         var _loc5_:Number = 0;
         while(_loc5_ < _loc4_)
         {
      ...
      {
         if(this._canManualInput && enabled && Boolean(_focusable))
         {
      ...
      private function checkLockValue(param1:Number) : Number
      {
         ...
         return Math.max(_minimum,Math.min(_maximum,stepSize * Math.round(param1 / stepSize)));
      }
      ...
      private function checkTextColor(param1:Number, param2:String) : void
      {
         ...
         else if(Boolean(this._skipValues) && this._skipValues.indexOf(param1) != -1)
         {
      ...
      {
         return Boolean(this._skipValues) && this._skipValues.indexOf(_value) != -1;
      }
