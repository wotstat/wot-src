package net.wg.gui.components.battleDamagePanel.components
{
   ...
   public class DamageLogDetailsController implements IDisposable
   {
      ...
      private var _poolRenderers:Vector.<DamageLogRenderer> = new Vector.<DamageLogRenderer>();
      
      private var _lastYValue:int = 0;
      
      private var _damageLogDetailsImages:Sprite = new Sprite();
      
      private var _damageLogDetailsText:Sprite = new Sprite();
      ...
      public function updateRenderersBG(param1:Boolean) : void
      {
         ...
         if(this._isCroppedBG != param1)
         {
            ...
            _loc2_ = int(this._poolRenderers.length);
            _loc3_ = 0;
            while(_loc3_ < _loc2_)
            {
      ...
      private function makePool(param1:int) : void
      {
         ...
         var _loc2_:int = this._atlasName == ATLAS_CONSTANTS.BATTLE_ATLAS ? int(Math.max(BattleDamageLogConstants.DEFAULT_POOL_COUNT,param1)) : BattleDamageLogConstants.SETTINGS_POOL_COUNT;
         var _loc4_:int = 0;
         while(_loc4_ < _loc2_)
         {
            _loc3_ = new Sprite();
