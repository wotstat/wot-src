package net.wg.gui.components.carousels.filters
{
   ...
   public class FilterCounter extends UIComponentEx implements IFilterCounter
   {
      ...
      {
         super.draw();
         if(this._isShow != this._lastShow && Boolean(isInvalid(SHOW_HIDE_INVALID)))
         {
            this.disposeTweens();
            if(this._isShow)
            {
         ...
         if(this._hasCounter)
         {
            ...
            if(Boolean(StringUtils.isNotEmpty(this._countText)) && Boolean(isInvalid(COUNT_TEXT_INVALID)))
            {
