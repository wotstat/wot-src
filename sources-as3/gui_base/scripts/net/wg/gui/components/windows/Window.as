package net.wg.gui.components.windows
{
   ...
   public class Window extends scaleform.clik.controls.Window implements IWindow
   {
      ...
      {
         if(Boolean(_content) && Boolean(contains(_content)))
         {
            removeChild(_content);
         }
         if(Boolean(this.windowContent.canDrag) && Boolean(initialized))
         {
      ...
      override protected function draw() : void
      {
         ...
         if(Boolean(isInvalid(INVALID_SRC_VIEW)) && Boolean(this.windowContent))
         {
            this.updateSource();
            ...
            if(Boolean(this.windowContent.canDrag) && !this._draggingRegistered)
            {
               try
               {
         ...
         if(!this._isDisposing && (Boolean(isInvalid(InvalidationType.SIZE)) || buttonPositionsChanged))
         {
      ...
      private function updateTopButtonPositions() : Boolean
      {
         ...
         var _loc2_:int = 0;
         if(Boolean(closeBtn) && Boolean(closeBtn.visible))
         {
            _loc2_++;
         }
         if(Boolean(this.minimizeBtn) && Boolean(this.minimizeBtn.visible))
         {
         ...
         if(Boolean(titleBtn) && Boolean(titleBtn.visible))
         {
      ...
      override public function set scaleY(param1:Number) : void
      {
         ...
         var _loc2_:Number = Number(height);
         super.scaleY = param1;
      ...
      public function set useBottomBtns(param1:Boolean) : void
      {
         ...
         this.formBgPadding.bottom = this._useBottomBtns ? this.getBgFormDefPadding().bottom + BTN_PADDING_Y : Number(this.getBgFormDefPadding().bottom);
         invalidate(INVALID_PADDING);
      }
      ...
      {
         if(this.windowContent != null && Boolean(this.windowContent.canMinimize))
         {
