package net.wg.gui.lobby.settings.components
{
   ...
   public class KeysScrollingList extends ScrollingListPx
   {
      ...
      public function keysWasChanged() : Boolean
      {
         ...
         var _loc2_:uint = uint(dataProvider.length);
         var _loc3_:uint = 0;
         while(_loc3_ < _loc2_)
         {
      ...
      public function restoreDefault() : void
      {
         ...
         var _loc1_:uint = uint(dataProvider.length);
         var _loc2_:uint = 0;
         while(_loc2_ < _loc1_)
         {
      ...
      public function updateDataProvider() : void
      {
         ...
         if(Boolean(_dataProvider) && Boolean(_renderers))
         {
      ...
      override protected function dispatchItemEvent(param1:Event) : Boolean
      {
         ...
         {
            _loc4_ = uint(ButtonEvent(param1).controllerIdx);
         }
         ...
         {
            _loc4_ = uint(MouseEventEx(param1).mouseIdx);
         }
         var _loc5_:uint = 0;
         if(param1 is ButtonEvent)
         {
            _loc5_ = uint(ButtonEvent(param1).buttonIdx);
         }
         else if(param1 is MouseEventEx)
         {
            _loc5_ = uint(MouseEventEx(param1).buttonIdx);
         }
         var _loc6_:Boolean = false;
         if(param1 is ButtonEvent)
         {
            _loc6_ = Boolean(ButtonEvent(param1).isKeyboard);
         }
