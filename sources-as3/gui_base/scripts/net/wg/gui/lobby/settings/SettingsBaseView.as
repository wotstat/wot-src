   ...
   public class SettingsBaseView extends SettingsNewCountersForm implements IViewStackContent, ISettingsBase
   {
      ...
      protected function trySetLabel(param1:String, param2:SettingsControlProp, param3:String = "", param4:String = "") : void
      {
         ...
         if(Boolean(this._data[param1]))
         {
            ...
            if(Boolean(this._data[param1].hasLabel) && Boolean(this[param1 + SettingsConfigHelper.TYPE_LABEL]))
            {
               if(this[param1 + SettingsConfigHelper.TYPE_LABEL] is LabelControl)
               {
                  _loc6_ = this[param1 + SettingsConfigHelper.TYPE_LABEL];
                  _loc6_.text = _loc5_;
                  _loc6_.visible = _loc6_.belongsTo != null ? Boolean(_loc6_.belongsTo.visible) : true;
               }
               else
               {
            ...
            else if(Boolean(this[param1 + SettingsConfigHelper.TYPE_CHECKBOX]) && this[param1 + SettingsConfigHelper.TYPE_CHECKBOX].label == Values.EMPTY_STR)
            {
      ...
      final protected function findSelectedIndexForDD(param1:Object, param2:Array) : Number
      {
         var _loc3_:int = 0;
         ...
         while(_loc5_ < _loc4_)
         {
            if(Boolean(param2[_loc5_].hasOwnProperty(OPTION_DATA_KEY)) && param1 == param2[_loc5_].data)
            {
               _loc3_ = _loc5_;
               break;
            }
